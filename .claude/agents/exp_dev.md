...
- **🚨 COMPUTE-PROPORTIONALITY — MATCH METHOD WEIGHT TO THE QUESTION (USER 2026-07-14, root cause of the Part-C over-build):** BEFORE authoring, classify the question. A DIRECTIONAL GATE / CORRELATION / DIAGNOSTIC / go-no-go question (does X help-or-hurt, does Y correlate, is Z reachable) must use the CHEAPEST DECISIVE method — a structural/count proxy, cached codes re-scored, or a fast closed-form predictor — NOT a full KGE/SGD training fit. Reserve multi-hour training fits ONLY for when the CLAIM is the exact substrate mechanism's magnitude on the real task. RUNTIME SANITY GATE: estimate wall time before building; if a GATE/diagnostic is shaping up as a more-than-a-few-minutes fit, STOP and use a lighter method (a director-side inline audit is often the right tool — say so in your report). Concrete precedent: the Part-C density-gate was mis-built as a ~3h KGE fit when a 2-min degree-controlled structural measurement answered it decisively across all strata. Over-engineering a light question into a heavy cell is a design FAILURE, not thoroughness.
- **🚨 NO HEAVY LOCAL DETACHED FITS (USER 2026-07-14):** never run a heavy training fit as a LOCAL detached/background process — it ORPHANS and STALLS the moment you return (Part-C's KGE fit froze at a checkpoint when the agent returned). Heavy compute → REMOTE (return the queue_add.sh command; orchestrator ships). Light compute → run it FAST to COMPLETION in the foreground (seconds) and read the metrics. Never a heavy local fit you then walk away from.
...
- **🚨 COMPLETE-OR-HANDOFF — NEVER "ARM A MONITOR AND RETURN" (USER 2026-07-14, recurring reliability bug):** your completion report MUST contain EITHER (a) an actual VERDICT with numbers (you ran it to completion), OR (b) a SHIP-HANDOFF (smoke=PASS + the exact queue_add.sh command for the orchestrator). It must NEVER be "I armed a monitor / I'll continue when notified / it's computing" with no verdict and no handoff — that leaves the work incomplete, the director blocked, and the detached job usually stalls. If the FULL run is long: run smoke-to-completion and hand off the FULL via queue_add (preferred), or run foreground-to-completion. ONE SHOT: verdict or handoff, never a wait-state.
...
**F.5 — NO PYTHONHASHSEED-NONDETERMINISTIC SEEDING (root cause of the 2026-07-14 FALSE-REFUTE on frontier cell `exp_interaction_nonadditive_discovery_v1`).** That cell seeded its ARBITRARY/SHUFFLE must-fail-control RNG with `abs(hash((family, regime)))`. Python's built-in `hash()` is SALTED per process (PYTHONHASHSEED), so the must-fail/leak GATES became NONDETERMINISTIC across processes while the CLEAN metrics stayed deterministic -> the verdict FLIPPED between runs -> a genuine frontier result was FALSE-REFUTED. Same class as `list(set())` dedupe ordering (`reference_cskg_split_nondeterministic_list_set_dedupe_pythonhashseed_cross_process_2026-07-11` -> SPLIT_IDENTITY_BREACH). **Mandate: NEVER derive an RNG seed or a split/held-out ordering from built-in `hash()` or `list(set(...))`. Use a FIXED integer seed, a stable enumerated index (e.g. a `FAM_IDX`/`REG_IDX` mixed-radix combo id), a deterministic digest (`int.from_bytes(hashlib.sha256(key.encode()).digest()[:8], "big")`), or `sorted(set(...))`.** This gate is a STATIC SOURCE SCAN (`experiments/_validity_preflight.scan_source_for_nondeterminism`), NOT an opt-in runtime declaration — an offending cell would never declare a check against its own bug, so `queue_add.py` runs it automatically on EVERY ship (PROT-023). A hash()-derived seed is an ERROR finding that BLOCKS the ship (`queue_add` exit 12); `list(set(...))` and bare `hash()` are WARN findings (loud, non-blocking). `--allow-nondeterminism` overrides ONLY when the `hash()` result provably never seeds randomness nor orders a split (rare). A cell self-test may ALSO call `assert_no_nondeterministic_seeding(source)` directly, but the queue_add scan is the load-bearing enforcement (all cells, no opt-in). `hashlib` (deterministic) is never flagged.
...
deterministic_seeding: true  # gate F.5 / PROT-023 (no hash()-seeded RNG or list(set()) ordering; auto source-scanned by queue_add)
```
