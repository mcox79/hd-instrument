...
- ✅ [SHIPPED 2026-06-22] **Substrate REPL portal v1** (`tools/substrate_repl.py`): USER can now `python tools/substrate_repl.py` and talk to substrate via ConceptNet KG (n8 CERT 585 mechanism). 5000-10000 triple KG; substrate retrieves at top-1 4/6 / top-3 5/6 on random queries; semantically-coherent alternatives on misses. Cache-warm restart ~2s. v2 adds Pythia-160m semantic encoder for free-text input. v3 adds g1 generation for bidirectional conversation.
...
- 2026-06-22 **MedQA n8-pattern ingest HONEST_NEGATIVE-at-smoke** (Option C disposition; spawn adbc6cfd HALTed-not-dispatched per Fix #16): cell-author surfaced encoder mean-pool over long medical vignettes collapses keysep (off-diagonal cosine 0.9865; <2% shift on adding answer); substrate-mechanism sound, upstream encoder geometry fails. Finding atomized as META `META_encoder_mean_pool_over_long_uniform_topic_text_collapses_keysep_needs_contrastive_projection` (deferred to Skunkworks next cycle). Cell NOT committed (diagnostic artifact only); v2 with n9-style learned contrastive projection queued. HotpotQA (1k dev; naturally multi-hop with (s,p,o)-decomposable chains) queued as alternative cross-domain ingest
- 2026-06-22 **CERT 585→586 RATIFIED + c3 META atom WRITTEN** (commit 6ff3616f hdi_skunkworks deferred follow-on): `math::T3/EXP_c3_compressed_sequence_replay_v1` chain-grade (cert_ledger row `3ca5e79680203356`); `META_software_substrate_no_hebbian_window_sequence_binding_is_architecture_not_timing` audit_lesson (row `b14431cf28ade69b`); substrate 177277→177279 atoms / 647→649 ledger rows; A5 invariants preserved
- 2026-06-22 **`hdlab/whitening.py` SHIPPED (TIER-2 #4)** (commit d834dbc3): ZCA + PCA encoder-residual primitive — 4 chain-grade atoms — most-evidenced gap; fit/transform/effective_rank API; smoke pre-eigenvalue 2.3e4 → post 1.16 ratio + planted rank-16 correctly recovered. 4 of 7 hdlab backlog items now closed
- 2026-06-22 **Substrate REPL Portal v2 ENGLISH chat SHIPPED** (commit ad597d69): MiniLM semantic codebook builder + chat backend routes free English text → MiniLM encode → nearest entity → substrate W matrix retrieval; "what is a cat" → cat (cos=0.784) → AtLocation:warm_bed; substrate-only-decode preserved (MiniLM is ingest-stage encoder analog). Structured `?` path bypasses MiniLM entirely
- 2026-06-22 **Dashboard v2 lean redesign SHIPPED** (commits 2d8f8b76 → 4fa7144b → 87af7287): single-page no-tabs, 5 sections (headlines/chat/live/history/capability/commits); auto-aggregated from existing sources (no new work to keep updated); /api/dashboard/v2/{headlines,history,capability,activity} + /api/substrate_chat backend; legacy preserved at /legacy; sys.path fix for pickle.load (KGStore unpickle needed _REPO on path)
