...
- **CURRENT (reactive):** (a) whitening-revival GPU cell-author (exp_dev, after the GATE-1 code-diff projection-fix) -> I DISPATCH [item#3 chain-grade-at-bound on isotropized keys, P~0.60-0.75, PoC-confirmed ZCA 0.843]; (b) 2 non-urgent USER decisions [phase05 restore + local_cpu runner restart]. **dense-KV thread FINALIZED:** learned-key collapse MM atomized (T3/EXP_dense_KV_learned_key_calibration_v1, CERT 583/177264, CERT-neutral, NOT tagged for my reciprocal); GATE-1-repro-gap = exp_dev cheap code-diff (no more GPU); attention-retrieval = working real-key path O(M*d).

### Next 3 (if bandwidth opens, no event trigger)
1. Consolidate the session's dispatch-discipline lessons into ONE clean note -> Skunkworks to atomize (verify-the-RESULT-not-just-dispatch / GPU-free-check-before-dispatch / ckpt-key-must-include-ALL-result-params / match-precision-AND-eval-protocol-of-the-cited-referent / verify-it-STARTS-past-model-load). I surface; cert-owner atomizes (no Store-write by me = concurrency-safe).
2. TRIM this bloated orchestrator section (compress closed flagship-probe+L-build+data-referent+smoke-clobber history to 1-line summaries each; this section is ~9 huge paragraphs).
3. Read-only audit other GPU-queue cells for the CONFIG_VERSION ckpt-key gap (omit result-affecting params -> stale-resume risk on re-dispatch); verify-per-cell before flagging (no name-similarity false-alarms).

- **DENSE-KV FOLLOW-UP: COMPLETE = HALT (HARD_FAIL by-design). NO chain-grade-at-bound upgrade.** exp_dense_KV_envelope_learned_key_calibration_v1_gpu (012925f7), ran full 3-seed, verified off metrics.json. **GATE-1 meter-check FAILED: cal_mean=0.411 != CERT591's 0.827 -> meter_valid=False -> cell correctly HALTED + refused to interpret learned-key arms** (ARM1-learned ~0.01 but meter-suspect; ARM2 softmax ~1.0). The gate WORKED (inflation-backstop: gated upgrade on meter-validity, it failed, HALTed vs minting bogus cert). Dispatch was CLEAN (fp16 loaded no-OOM, all lessons applied). **dense-KV envelope MM (e08199ed) STANDS; upgrade BLOCKED.** WHY-0.411 flagged -> **Skunkworks re-VET CONFIRMED = FALSE HALT (protocol mismatch, validates my flag): GATE-1 did 10000-way retrieval vs CERT591's 2500-way (HELDOUT_FRAC 0.25) + train 4000 vs 7500 -> 0.411 >> chance 0.0001 = projection+meter WORK, not a real failure.** Lesson: verify-the-referent extends to the cited number's IMPLICIT EVAL PROTOCOL (candidate-set-size + train-size), not just model+precision. **CORRECTED re-run = STILL HALT (2nd, cal=0.604).** Param-fix (dce89655 TRAIN_M=7500/CAL_POOL=2500) MOVED cal 0.411->0.604 (pool-diagnosis directionally right) but **~0.22 gap REMAINS** (further CERT591-setup mismatch). (Caught + fixed a stale-ckpt resume mid-way: CONFIG_VERSION omits TRAIN_M/CAL_POOL -> cleared partials + re-dispatched run_index=3 = verify-the-RESULT caught it.) **GATE-2 (POOL-INDEPENDENT, load-bearing): ARM1-learned COLLAPSES ~0.01-0.02 on real anisotropic keys; ARM2 holds ~1.0 -> no-upgrade verdict ROBUST off GATE-2 alone.** RECOMMENDED (note ...STILL_HALT...): Skunkworks ACCEPT GATE-2 (don't chase GATE-1 reproduction, 2 HALTs/diminishing returns); GATE-1-repro-gap = separate revival question. **Director ENDORSED my recommend (accept GATE-2, HOLD re-dispatch, GATE-1-gap = cheap code-trace diagnostic NOT GPU).** STORAGE-CHAIN STATUS: item#3 RAW-keys = MM (collapses, robust x2 runs); item#3 WHITENED-keys = **NEXT upgrade path (whitening-revival cell, exp_dev authoring; Skunkworks PoC: isotropize recovers ARM1 0.81-0.84; P~0.60-0.75) -> I DISPATCH when authored**; item#4 attention-over-learned-keys = VIABLE at O(M*d). CERT 583/177261 unchanged. Reactive on the whitening-revival cell-author + Skunkworks MM atomization.
