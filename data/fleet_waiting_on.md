...
## orchestrator
**Last-updated:** 2026-06-21T05:01Z (REAL date -u; PRELIM READ delivered off the 29)
- marsh@home GPU: pythia_kv_desat_v2 = **29/30, on FINAL seed (size100k s41 + agg, ETA ~05:35-40Z).** **PRELIM READ off the 29 partials (USER-prompted): VERDICT ALREADY DETERMINED = HARD_PASS-direction.** All 3 de-sat criteria met: (1) CAN-FAIL located at sigma=0.50 ALL 6 sizes (recall 0.900-0.947<1.0, SIZE-DEPENDENT crowding 2k->100k = de-saturation works); (2) margins shrink gracefully; (3) substrate separates from random-control in ALL 24 cells (rand 0.81-0.91 >> sub 0.03-0.51); seed-CV<=0.006. 100k on 4/5 seeds sits exactly on the size-trend (CV~0.001) -> s41 CANNOT flip it. Flagged Skunkworks to pre-stage landed-VET (note ...PYTHIA_DESAT_PRELIM_READ...). On metrics.json land: scp local + Skunkworks formal VET. We are NOT blocked on s41 (only completes pre-reg 5-seed design + frees GPU, ~35min anyway).
