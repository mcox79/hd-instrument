...
## orchestrator
**Last-updated:** 2026-06-21T08:55:05Z (REAL date -u; FLAGSHIP recovered -- bf16 fix CLEARED OOM, running)
- **FLAGSHIP: FAILED 2x (CUDA OOM, float32 footprint) -- I MISSED THE 1ST FAIL FOR 2h (verify-start lesson) -- now RECOVERED + RUNNING.** Failed 02:09Z + re-OOM'd on free GPU (= float32 footprint > 6.8GB cap, NOT contention). Exp-Dev pushed bf16 fix (4e65cfb0, ~5.6GB). Re-dispatched run_index=3 -> **VERIFIED past model-load: GPU 95% util, 6.6GB under cap, computing.** ETA ~2-3h. Will confirm 1st partial next, then watch->scp->probe_gate->4-layer witness. LESSON BANKED+APPLIED: after dispatch confirm the run STARTS+progresses (queued != running). + 2nd lesson: on a shared GPU box, verify the dispatched run actually claimed+cleared load, don't assume from 'queued'.
