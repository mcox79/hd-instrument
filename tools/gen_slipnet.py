"""Research 5X ARCHITECTURAL Sprint-1: SLIPNET-SUBSTRATE (cross-domain NEW mechanism, Hofstadter Copycat). P=0.45.
RELATION-TYPE activation spreading (not entity geometry -> avoids P9 confound; not raw degree -> beyond SME's degree
ceiling). Nodes are distinguished by their MIX of relation TYPES; cross-domain correspondence recovered from relation-type
structure. Tests if relation-type structure beats a degree-only baseline (the deep cross-domain question). Pure-FHRR. Write-tool authored."""
import pathlib
...
'''
(EXP / "exp_slipnet_substrate_cpu_v1.py").write_text(CELL, encoding="utf-8"); print("wrote slipnet_substrate")
