"""Research NEGATIVE-RESOLUTION P4: TRAINED-CONFIDENCE-HEAD (continuous calibrated confidence; resolves LAP4-3).
Rank-transform failed (corr 0.10, ECE 0.325). The discriminative signal is margin-distance-from-threshold + score-distribution
shape. Train a logistic head (PP-225 pattern) on those features -> per-sample continuous confidence. Pure-numpy. Write-tool authored."""
import pathlib
...
'''
(EXP / "exp_negres_confidence_head_cpu_v1.py").write_text(CELL, encoding="utf-8"); print("wrote confidence_head")
