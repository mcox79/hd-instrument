"""Generate Q-A3 depth ladder scripts for N=16384 and N=8192."""
import pathlib

def make_gpu_script(L, N):
    need = L - 2
    smoke_tail = need - 3
    full_tail = need - 5
    n_str = str(N)
    w_gb = N * N * 4 / 1e9
    w_limit = 1.2 if N == 16384 else 1.0
    alpha_inner = 100 / N
    return f"""\"\"\"
q_a3_l{L}_cross_layer_composition_v1_n{N} -- Q-A3/PP-12: L={L} cross-layer composition at N={N}.

CONTEXT (v378 cycle 48 all-night burst):
  N={N} series continues depth ladder.
  Per-stage alpha = 100/{N} = {alpha_inner:.4f} << alpha_c=0.138.

PRE-REGISTERED BANDS (Q-A3 L={L} N={N}):
...
PROT-018: anchor has _n{N}; N MUST = {N}.
...
ANCHOR_NAME = "q_a3_l{L}_cross_layer_composition_v1_n{N}"
_N_SUFFIX = {N}
N = {N}
...
HF_LACC = 0.5
NOISE_FRAC = 0.10
_W_ONDEMAND_GB = {N} * {N} * 4 / 1e9
assert _W_ONDEMAND_GB < {w_limit}, f"W on-demand: {{_W_ONDEMAND_GB:.3f}} GB"
if RUN_MODE == "smoke":
    N_ACTIVE = 512; SEEDS = [7, 17]; M_INNER = 20
    M_MID = [10, 5, 3] + [2] * {smoke_tail}
    M_OUTER = 2; N_QUERIES = 2
else:
    N_ACTIVE = N; SEEDS = [7, 17, 23, 31, 41]; M_INNER = 100
    M_MID = [50, 25, 12, 6, 3] + [2] * {full_tail}
    M_OUTER = 2; N_QUERIES = 2
...
if __name__ == "__main__":
    base = pathlib.Path("d:/AI/hd-instrument/experiments")
    for L in [118, 119, 120, 121, 122]:
        content = make_gpu_script(L, 16384)
        fname = base / f"exp_q_a3_l{L}_cross_layer_composition_v1_n16384.py"
        fname.write_text(content, encoding="utf-8")
        print(f"Written {fname.name}")
    for L in [78, 79, 80]:
        content = make_gpu_script(L, 8192)
        fname = base / f"exp_q_a3_l{L}_cross_layer_composition_v1_n8192.py"
        fname.write_text(content, encoding="utf-8")
        print(f"Written {fname.name}")
    print("ALL DONE")
