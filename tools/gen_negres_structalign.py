"""Research NEGATIVE-RESOLUTION TIER-1 P2: STRUCTURAL-ALIGNMENT-MAPPING (cross-domain analogy 0.244 -> >=0.40).
Resolves STRETCH4-2. Gentner systematicity as a vector op: infer held-out relation transform per-dim, weight by cross-shot
CONCENTRATION (resultant length) -- structurally-consistent dims = the relation; inconsistent dims = entity-specific noise,
projected out. Score candidates with concentration-weighted RotatE distance. NO retraining. torch(CPU)+FB15K. Write-tool authored."""
import pathlib
...
'''
(EXP / "exp_negres_struct_align_cpu_v1.py").write_text(CELL, encoding="utf-8"); print("wrote struct_align")
