"""Research 5X ARCHITECTURAL Sprint-1: ADDITIVE-ONLY self-modification certification (self-mod stability, P=0.50, substrate-only).
Certifies the universal finding: ADDITIVE self-modification (M += new) is stable to capacity, while IN-PLACE edits
(subtract-old + add-new, ROME/MEMIT analog) degrade as ~edits^2/N (edit budget ~sqrt(N)). Empirical certification. Pure-FHRR. Write-tool authored."""
import pathlib
...
'''
(EXP / "exp_additive_only_cert_cpu_v1.py").write_text(CELL, encoding="utf-8"); print("wrote additive_only_cert")
