"""Research REVIVAL_SUBSTRATE_NATIVE Sprint-3: BILINGUAL-DUAL-SUBSTRATE (translation, P=0.45, substrate-only).
Hub-and-spoke interlingua: each language has a Tier-3 word codebook bound to shared CONCEPTS (Tier-1/2). Translate via
concept pivot (word_L1 -> concept -> word_L2). Tests N-language translation scales LINEARLY (N codebooks) not N^2 (pairs):
a 3rd language pivots through the same interlingua with no new pair training. Pure-FHRR. Write-tool authored."""
import pathlib
...
'''
(EXP / "exp_bilingual_dual_substrate_cpu_v1.py").write_text(CELL, encoding="utf-8"); print("wrote bilingual_dual_substrate")
