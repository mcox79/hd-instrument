...
'''
C = []
C.append(dict(anchor="ndcg_ranking_quality_cpu_v1", tag="CHEAP-CAP NDCG ranking quality",
  title="substrate cleanup-score ranking achieves NDCG@10 >= 0.6 vs graded relevance",
  desc="Ranked retrieval quality: a query has candidates at graded relevance tiers (bound at graded strengths); rank by substrate cleanup score and measure NDCG@10 against the ideal relevance order. Tests substrate as a graded RANKER, not just top-1.",
  prereg="HARD-PASS NDCG@10 >= 0.6. MIDDLE >= 0.45. HARD-FAIL < 0.45.",
  body='''
def _selftest():
...
'''))
C.append(dict(anchor="dependency_with_audit_cpu_v1", tag="CHEAP-CAP dependency K-hop + audit trail",
  title="theorem-dependency closure with a per-dependency Merkle audit trail",
  desc="Combines K-hop dependency traversal with a hash-chained audit trail: each resolved dependency edge is appended to a Merkle chain so the derivation is replayable/verifiable. Measures closure recall AND that the audit chain reproduces (tamper-evident).",
  prereg="HARD-PASS closure recall >= 0.95 AND audit-chain reproduces 100pct (tamper-evident). MIDDLE recall >= 0.85. HARD-FAIL < 0.85.",
  body='''
def _selftest():
