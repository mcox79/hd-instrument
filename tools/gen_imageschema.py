"""Research REVIVAL_SUBSTRATE_NATIVE Sprint-1: IMAGE-SCHEMA-CODEBOOK (cross-domain + embodied, P=0.55, substrate-only).
Encode Lakoff/Johnson image schemas (CONTAINER/PATH/FORCE/...) + conceptual metaphors; abstract concepts are GROUNDED in
schemas via metaphor bindings. Test: retrieve an abstract concept's grounding image-schema -- embodied grounding of
abstract cognition, substrate-only (no LLM). Pure-FHRR. Write-tool authored."""
import pathlib
...
'''
(EXP / "exp_image_schema_codebook_cpu_v1.py").write_text(CELL, encoding="utf-8"); print("wrote image_schema_codebook")
