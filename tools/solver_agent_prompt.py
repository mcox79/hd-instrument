...
PHASES = (
    ...
    "PARTIAL or FAILED: write \"what would it take to convert this to a FULL PASS\" -- list EVERY lead, RESEARCH each (literature + the "
    "organs that own it), then CHASE each lead you can build here (build, measure the same way) until the bar is met or the remainder is "
    "bounded by arithmetic or by your remit; hand the rest to strategy as brief-ready leads with numbers. "
    ...
    "5b. THE OWNER'S PUSH SCRIPT (distilled from the owner-run solver sessions, 2026-09-14 -- run it on yourself BEFORE finalizing; "
    "answer each with numbers, then ACT on it): (i) how do we perform against the BRAIN at each rung of the chain, and where exactly "
    "is signal lost? (ii) research every wall again and PROTOTYPE your way past it -- a prototype that LOCATES the signal is allowed, "
    "but a heuristic is NOT the landed form: state the learned / graded form each prototype must become; (iii) is the wiring alone "
    "sufficient to reach brain level? if not, name what else is (an upstream or representational gap) with its number; (iv) the brain "
    ...
    "the rungs NOT cracked); ALTERNATE PATHS -- other ways to do this read that are similarly or MORE brain-foundational than what you "
    "shipped (brain structure + computation, the math, what it would take, why not now), so strategy can queue them; "
    ...
    "submission prompt in a code box that includes the original problem name; commit your files path-limited. YOUR CELL AND WITNESS MUST RUN GREEN ON THE TREE AS LANDED, not only on the unpatched tree: detect the landed state from the LIVE modules (a function or field only your diff adds) and, when present, run every arm against the live modules with no materialize / git apply / monkeypatch; keep the unpatched path for the tree before landing; a self-test check that asserts the DEFECT'S PRESENCE must assert its ABSENCE on the landed tree (three cells in one day were written against the unpatched tree and failed at landing: pri 125, 129, 131). YOUR DIFF MUST BE COMPLETE ON THE SHIPPED PATH: if it widens a signature or adds a parameter, every call site in hdlab/ AND tools/ (the asset builders) must pass the new argument in the diff itself -- a builder that runs your organ without the argument your cell passed silently ships a different organ (2026-09-14: a teacher shipped oblique-only because its association was never passed at the builder's call site). Your final message = "
