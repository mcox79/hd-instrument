"""AGGRESSIVE_OVERNIGHT THRUST-3 CODE: CODE-1 FUNCTION-COMPOSE (substrate-only). Substrate composes a function from program-
shards (an ordered op-sequence per a spec) + EXECUTES it on a tiny interpreter (the ops are real numeric ops). Tests the
composed function computes the correct output -- substrate-native program composition. Pure-FHRR + numeric exec. Write-tool authored."""
import pathlib
...
def run() -> Dict:
    ...
    consts = cphasor(NK, N, g); slots = cphasor(STEPS, N, g)
    def apply(op, k, x):
    ...
    for _ in range(TR):
        ...
        fn = cnorm(sum((slots[s] * opv[prog[s][0]] * consts[prog[s][1]] for s in range(STEPS)), np.zeros(N, dtype=np.complex64)))
        # RECOVER program in order + EXECUTE on test inputs
        rec = []
        for s in range(STEPS):
            comp = fn * np.conj(slots[s]); ro = OPS[cidx(comp, opbook)]; rk = cidx(comp * np.conj(opv[ro]), consts); rec.append((ro, rk))
        for xi in [1, 3, -2]:
            xg = xi; xr = xi
            for (o, k) in prog:
                xg = apply(o, k, xg)
...
'''
(EXP / "exp_code1_function_compose_cpu_v1.py").write_text(CELL, encoding="utf-8"); print("wrote code1_function_compose")
