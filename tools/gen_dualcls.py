"""Research REVIVAL_SUBSTRATE_NATIVE Sprint-2: D2.1 DUAL-SUBSTRATE-CLS (continual, P=0.40, substrate-only, DISCRIMINATING).
Complementary Learning Systems: a FAST substrate (hippocampal: high plasticity, recency) + a SLOW substrate (cortical:
consolidates frequent items via replay, stable). Tests the dual pair retains BOTH recent AND old-consolidated memories
better than either alone (stability-plasticity). Genuinely can land MIDDLE. Pure-FHRR. Write-tool authored."""
import pathlib
EXP = pathlib.Path(__file__).resolve().parent.parent / "experiments"
CELL = r'''"""
exp_d2_1_dual_cls_cpu_v1.py -- D2.1 DUAL-SUBSTRATE-CLS (complementary learning systems) -- CPU.
...
ANCHOR_NAME = "d2_1_dual_cls_cpu_v1"
...
def _selftest():
    print("[selftest] PASS: dual-cls", flush=True)
def run() -> Dict:
    g = np.random.default_rng(710); M = 200; T = 600 if not SMOKE else 200; FAST_DECAY = 0.90; SLOW_DECAY = 0.999; CONS = 3
...
'''
(EXP / "exp_d2_1_dual_cls_cpu_v1.py").write_text(CELL, encoding="utf-8"); print("wrote d2_1_dual_cls")
