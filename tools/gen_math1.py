"""AGGRESSIVE_OVERNIGHT THRUST-2 MATH: MATH-1 ALGEBRA-SIMPLIFY (substrate-only). Substrate stores algebraic rewrite rules as
schemas and applies them compositionally to simplify expressions (e.g. x+x->2x, x*1->x, x+0->x, x*0->0, a*b+a*c->a*(b+c)).
Tests rule-application accuracy on generated expressions. Pure-numpy/symbolic + FHRR rule-store. Write-tool authored."""
import pathlib
...
'''
(EXP / "exp_math1_algebra_simplify_cpu_v1.py").write_text(CELL, encoding="utf-8"); print("wrote math1_algebra_simplify")
