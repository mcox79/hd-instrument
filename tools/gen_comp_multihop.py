"""Research WAVE-5 P4 reasoning-at-depth: COMP-23 MULTI-HOP-THROUGH-COMPOSITES.
Does K-hop graph traversal survive when each node is a deep L3 composite (not an atom)? Per-node cleanup (COMP-DEPTH P0
mechanism) makes composite-nodes behave like atomic nodes for traversal. 3-hop through composite nodes. Pure-FHRR. Write-tool authored."""
import pathlib
...
def run() -> Dict:
    ...
    for _ in range(TR):
        ...
        for h in range(NN):
            for r in range(NR):
                G = G + nodes[h] * rels[r] * nodes[nxt[h, r]]
        # 3-hop query
        start = int(g.integers(0, NN)); path = [int(g.integers(0, NR)) for _ in range(3)]
        cur = start; gold = start
        for r in path:
            gold = int(nxt[gold, r])
        # traverse WITH node cleanup
        cv = nodes[start]
        for r in path:
            cand = G * np.conj(cv) * np.conj(rels[r]); cv = nodes[int(np.argmax((nodes @ np.conj(cand)).real))]
        hit += int(int(np.argmax((nodes @ np.conj(cv)).real)) == gold)
        # traverse WITHOUT node cleanup (let noise compound)
        cv = nodes[start]
        for r in path:
...
'''
(EXP / "exp_comp23_multihop_composites_cpu_v1.py").write_text(CELL, encoding="utf-8"); print("wrote comp23_multihop_composites")
