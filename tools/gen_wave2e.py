"""Research WAVE-2 laptop: LAP2-2 BELIEF-REVISION (AGM) + LAP2-9 PREDICTIVE-CODING. Pure-FHRR. Write-tool authored."""
import pathlib
...
C = [
    dict(anchor="lap2_2_belief_revision_cpu_v1", tag="LAP2-2 BELIEF-REVISION-1", title="AGM belief revision (prioritized contraction + expansion)", desc="Stream of prioritized assertions; higher-priority val supersedes via exact erasure; query final belief set.", prereg="HARD-PASS posterior>=0.85. MIDDLE>=0.70. HARD-FAIL<0.70.", body=BELIEF),
    dict(anchor="lap2_9_predictive_coding_cpu_v1", tag="LAP2-9 PREDICTIVE-CODING", title="prediction-error encoding (store residuals not full input)", desc="Markov sequence over a small transition codebook; store delta indices; reconstruct; compare to full-encoding.", prereg="HARD-PASS pred-recall>=0.85. MIDDLE>=0.70. HARD-FAIL<0.70.", body=PREDCODE),
]
for c in C:
