"""AGGRESSIVE_OVERNIGHT THRUST-2 MATH: MATH-4 PROOF-CHAINS (substrate-only). Substrate stores inference rules (A=>B) and
chains modus ponens from premises to a conclusion. Tests multi-step proof derivation accuracy vs chain length -- genuinely
can degrade. Pure-FHRR rule-store + chained unbind/cleanup. Write-tool authored."""
import pathlib
...
def run() -> Dict:
    g = np.random.default_rng(822); NPROP = 60; IMPL = cphasor(1, N, g)[0]
...
'''
(EXP / "exp_math4_proof_chains_cpu_v1.py").write_text(CELL, encoding="utf-8"); print("wrote math4_proof_chains")
