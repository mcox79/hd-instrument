"""Research NEXT_SPRINT1_REAL_DATA_AUDIT: IMAGE-SCHEMA-REAL (PP-316). Audits image-schema grounding under the REAL challenge:
POLYSEMY (each abstract concept grounds to 1-3 schemas, ambiguous) + CORRELATED concept embeddings (real concepts cluster).
Research expects significant drop (~0.28). Tests cluster purity >=0.60 (synthetic was 1.0). Pure-FHRR. Write-tool authored."""
import pathlib
...
'''
(EXP / "exp_image_schema_real_cpu_v1.py").write_text(CELL, encoding="utf-8"); print("wrote image_schema_real")
