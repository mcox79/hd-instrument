"""Generate Q-A3 scripts for given L values and N values."""
import sys
...
def gen_q_a3_n16384(L):
    mid_count = L - 2
    smoke_mmid = [10, 5, 3] + [2] * (mid_count - 3)
    full_mmid = [50, 25, 12, 6, 3] + [2] * (mid_count - 5)

    def list_lines(lst, indent=8):
        rows = []
        for i in range(0, len(lst), 20):
            chunk = lst[i:i + 20]
            rows.append(' ' * indent + ', '.join(str(x) for x in chunk) + ',')
        return '\n'.join(rows)

    smoke_str = list_lines(smoke_mmid)
    full_str = list_lines(full_mmid)
    anchor = f"q_a3_l{L}_cross_layer_composition_v1_n16384"
    rung = L - 19

    lines = []
    lines.append(f'"""')
    lines.append(f'{anchor} -- Q-A3/PP-12: L={L} cross-layer composition at N=16384.')
    lines.append(f'')
    lines.append(f'CONTEXT (v372 cycle 42 all-night burst):')
    lines.append(f'  N=16384 series: L=20..L=65 all EXACT-1.0000 (L=65 NEW DEEPEST cycle 41).')
    lines.append(f'  L={L} is rung {rung} in the N=16384 depth ladder.')
    lines.append(f'  ECC criterion: depth UNLIMITED when per-stage max(alpha_k) < alpha_c=0.138.')
    lines.append(f'  Per-stage alpha < 0.01 << alpha_c. Theory predicts continued EXACT fidelity.')
    lines.append(f'')
    lines.append(f'SCIENTIFIC QUESTION:')
    lines.append(f'  Does cross-layer composition fidelity remain EXACT-1.0 at L={L} N=16384?')
    lines.append(f'')
    lines.append(f'PRE-REGISTERED BANDS (PP-12/Q-A3 L={L} N=16384):')
    lines.append(f'  HARD-PASS: all {L} level fidelities >= 0.9999 unanimous (5/5 seeds) AND l{L}_acc >= 0.5.')
    lines.append(f'  MIDDLE: any L_fid in [0.85, 0.9999) OR graceful degradation.')
    lines.append(f'  HARD-FAIL: any L_fid < 0.85 OR l{L}_acc < 0.5.')
    lines.append(f'')
    lines.append(f'FORMULA SELF-TESTS (PROT-022):')
    lines.append(f'  1. L={L} chain: {L-1}-ctx Hadamard roundtrip recovers xi_L1.')
    lines.append(f'  2. All alphas < alpha_c=0.138 at N=16384. M_INNER=100 -> alpha=0.0061 < 0.138.')
    lines.append(f'  3. GPU memory > 0 after W build.')
    lines.append(f'')
    lines.append(f'PROT-018: anchor has _n16384; N MUST = 16384.')
    lines.append(f'PROT-021: seed checkpoints keyed with run_mode + L.')
    lines.append(f'QUEUE: overnight_queue (GPU; on-demand W build).')
    lines.append(f'TIMEOUT ESTIMATE: ceil(1.5 * {22 + (L - 65)} * 1.0) = {int(1.5*(22+(L-65)))+1}s. Use PROT-019 floor: 21600s.')
    lines.append(f'"""')
...
def gen_q_a3_n8192(L):
    ...
    smoke_str = list_lines(smoke_mmid)
    full_str = list_lines(full_mmid)
    anchor = f"q_a3_l{L}_cross_layer_composition_v1_n8192"

    lines = []
    lines.append(f'"""')
    lines.append(f'{anchor} -- Q-A3/PP-12: L={L} cross-layer composition at N=8192.')
    lines.append(f'')
    lines.append(f'CONTEXT (v372 cycle 42 all-night burst):')
    lines.append(f'  N=8192 series: L=19..L=40 all EXACT-1.0000 (L=40 NEW DEEPEST cycle 41).')
    lines.append(f'  L={L} is rung {L - 19} in the N=8192 depth ladder (cross-N ladder paired with N=16384 L={L}).')
    lines.append(f'  ECC criterion: depth UNLIMITED when per-stage max(alpha_k) < alpha_c=0.138.')
    lines.append(f'  L={L-1} per-stage alpha = 100/8192 = 0.0122 << alpha_c. Theory predicts EXACT fidelity.')
    lines.append(f'')
    lines.append(f'SCIENTIFIC QUESTION:')
    lines.append(f'  Does cross-layer composition fidelity remain EXACT-1.0 at L={L} N=8192?')
    lines.append(f'  If yes: extends N=8192 cross-N series to L={L} (cross-N at L={L}).')
    lines.append(f'')
    lines.append(f'MEMORY ESTIMATE (on-demand W):')
    lines.append(f'  One W matrix at a time: 8192 x 8192 x 4 bytes = 268 MB. Well within 8 GB GPU.')
    lines.append(f'')
    lines.append(f'PRE-REGISTERED BANDS (Q-A3 L={L} N=8192):')
    lines.append(f'  HARD-PASS: all {L} level fidelities >= 0.9999 unanimous (5/5 seeds) AND l{L}_acc >= 0.5.')
    lines.append(f'  MIDDLE: any L_fid in [0.85, 0.9999) OR graceful degradation.')
    lines.append(f'  HARD-FAIL: any L_fid < 0.85 OR l{L}_acc < 0.5.')
    lines.append(f'')
    lines.append(f'FORMULA SELF-TESTS (PROT-022):')
    lines.append(f'  1. L={L} chain: {L-1}-ctx Hadamard roundtrip recovers xi_L1.')
    lines.append(f'     [INPUT: 2-element +-1 vectors, {L-1} context ops] [EXPECTED: decode = xi_L1 exactly]')
    lines.append(f'  2. All alphas < alpha_c=0.138 at N=8192. M_INNER=100 -> alpha=0.0122 < 0.138.')
    lines.append(f'  3. GPU memory > 0 after W build.')
    lines.append(f'  4. Memory on-demand per W: 8192*8192*4 = 268 MB < 1 GB.')
    lines.append(f'')
    lines.append(f'PROT-018: anchor has _n8192; N MUST = 8192.')
    lines.append(f'PROT-021: seed checkpoints keyed with run_mode + L.')
    lines.append(f'QUEUE: overnight_queue (GPU; on-demand W build at N=8192).')
    lines.append(f'TIMEOUT ESTIMATE: L=40 N=8192 elapsed ~6s (5-seed). L={L} near-linear.')
    lines.append(f'  ceil(1.5 * {6 + (L - 40)} * 1.0 * 1) = {int(1.5*(6+(L-40)))+1}s. Use PROT-019 floor: 21600s.')
    lines.append(f'"""')
...
if __name__ == "__main__":
    # Single authorized deep marketing/demo anchor (per capacity-stress redirect): ONE L=10000.
    for L in [10000]:
