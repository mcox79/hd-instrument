"""AGGRESSIVE_OVERNIGHT THRUST-3 CODE: CODE-6 ALGORITHM-COMPOSITION (substrate-only). Substrate composes a multi-step data-
processing pipeline (filter/map/reduce/sort/take) from primitive shards (role-separated op/param) and EXECUTES it on real
arrays. Tests pipeline-output correctness -- compositional algorithm building beyond single functions. Pure-FHRR + exec. Write-tool authored."""
import pathlib
...
'''
(EXP / "exp_code6_algorithm_compose_cpu_v1.py").write_text(CELL, encoding="utf-8"); print("wrote code6_algorithm_compose")
