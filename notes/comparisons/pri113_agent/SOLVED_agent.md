...
**THE DISAGREEMENT CLASSES, from `probe_states.json`'s own items:**

| the PREDICATE-SLOT read is right (20) | n | | the copular STATE reader is right (9) | n |
|---|---|---|---|---|
| **locative inversion / presentational** -- `BELOW is a list`, `HERE is a copy` (x3), `Here is a revised draft` (x2) | **6** | | **UD's LEFT-head convention for a transliterated name** -- `al - Qaeda` | 1 |
| **interrogative inversion** -- `Is that a money MAKER ?`, `is it for GUITAR ?`, `Are you FREE for lunch` (x3) | **5** | | the NP run walks into a second DP -- `the WAY the greatest bear market` | 1 |
| **complex locative / PP predicate** -- `is not up to the TASK`, `The game is at 12` (x2) | **3** | | an ADJ + NUM run -- `Today is GOOD 12:30 ?` (x2) | 2 |
| **the right-hand head rule** -- `money - REDISTRIBUTORS`, `al - Qaeda OPERATION` | **2** | | a hyphen-appositive -- `a done DEAL - a lesson` | 1 |
| **complement-clause locality** -- `I 'm not SURE how the market will react` (x2) | **2** | | a relative-clause copula -- `which is the DIFFERENCE between` | 1 |
| stranded copula -- `more miserable than it 's ever BEEN`; and one other | **2** | | the OTHER clause of a two-copula sentence -- `I am not sure how RELIABLE that is` | 2 |
| | | | a clause-final elided copula -- `I am SURE you 've already gone` | 1 |

**Every one of the slot read's 20 wins is a construction this brief added** -- locative inversion, interrogative
inversion, the complex locative, the right-hand head rule, complement-clause locality. **The state reader has no
construction knowledge at all**; it reads a tree. And `I am not sure how reliable that is` is the sentence that
makes the case for the UNION on its own: it has TWO copular clauses, each organ finds one predicate and misses the
other, and only together do they get both.
...
| **the 15 CONSTRUCTIONS** (the arc cue is NOT in this row -- see the correction below) | **0.8414** | 0.7997 | **0.8200** | **0.6120** | 2296 |
...
> ### CORRECTION, CAUGHT BY THE DECOMPOSITION I RAN TO CHECK MYSELF -- AND IT CHANGES WHAT THIS ROW IS
>
> I ran the same GUM measurement a second time with `HDLAB_PREDICATION_ARC_TAU` unset, to separate "the
> constructions carry UD-EWT conventions" from "the arc cue costs precision out of supply". **The two runs came
> back BYTE-IDENTICAL** -- same recall, same precision, same 2296 fires. The cause is in my own cell:
> `ARC_TAU` is read at import and is consulted **only on the board path** (`_patch_consumers`), never on the
> `--participant` path, which builds its sites from `predicate_sites(..., cons=SHIPPED_CONS)` alone.
>
> **So the GUM row above is the FIFTEEN CONSTRUCTIONS WITHOUT THE ARC CUE, and I labelled it as the shipped arm.**
> The in-supply comparison it is set against (0.8802) DOES include the arc cue, so the 0.8802-vs-0.6120 gap I drew
> conclusions from was comparing two different arms. The correct in-supply number for the constructions alone is
> **0.8383** (section 11i), so the honest out-of-supply gap is **0.8383 -> 0.6120**, not 0.8802 -> 0.6120.
>
> **What survives the correction:** the mechanism still generalises (0.1230 -> 0.6120, fivefold, CI-separated, every
> twin far below); it still generalises WORSE than in supply; and the precision cost out of supply
> (-0.0083 CI[-0.0153,-0.0017]) is now attributable to the **CONSTRUCTIONS**, not to the arc cue -- which makes the
> "some of my constructions carry UD-EWT's conventions" reading STRONGER, not weaker, since the arc cue (the one
> component that reads the governor rather than a word list) is not implicated at all.
>
> **What did NOT survive, and has now been measured.** The proper sweep (`--arcgrade --pop gum --cap 1200`) was
> launched on finding this and has landed. **It vindicates the default-ON and it settles where the precision cost
> comes from** (366 non-verbal clauses; the `rec.NONV` column is GUM's 366, not UD-EWT's 167 -- the cell's header
> string is hard-coded and mislabels it, noted here so nobody reads the wrong denominator):
>
> | tau | recall | precision | F1 | on the 366 non-verbal | d(precision) vs the live floor |
> |---|---|---|---|---|---|
> | 0.00 (raw union) | 0.8498 | 0.7954 | 0.8217 | 0.6475 | -0.0125 CI[-0.0199,-0.0055] |
> | 0.05 | 0.8460 | 0.7999 | 0.8223 | 0.6311 | -0.0080 CI[-0.0150,-0.0013] |
> | 0.10 / 0.20 | 0.8453 | 0.7998 | 0.8219 | 0.6284 | -0.0081 CI[-0.0151,-0.0014] |
> | 0.30 | 0.8453 | 0.8002 | 0.8221 | 0.6284 | -0.0078 CI[-0.0147,-0.0012] |
> | **0.50 (shipped) / 0.70 / 0.90** | **0.8447** | **0.8001** | 0.8218 | **0.6257** | **-0.0079 CI[-0.0148,-0.0013]** |
> | off | 0.8414 | 0.7997 | 0.8200 | 0.6120 | -0.0083 CI[-0.0153,-0.0017] |
>
> **THE ARC CUE IS PRECISION-NEUTRAL OUT OF SUPPLY AND ADDS RECALL.** At the shipped tau = 0.5 it takes the
> non-verbal clauses 0.6120 -> **0.6257** (+5 clauses) while precision goes **-0.0083 -> -0.0079**, i.e. very
> slightly BETTER, not worse. **So the CI-separated precision cost out of supply is entirely the CONSTRUCTIONS' --
> it is there with the arc cue off and it does not grow when the arc cue is on.** And the flat region is flat out of
> supply too (tau 0.05-0.90 spans 0.6257-0.6311 at precision -0.0078 to -0.0081), so the operating point transfers
> even though the word sets do not.
>
> **The default-ON therefore stands on two populations, not one** -- which is what I wanted before recommending it,
> and it is the opposite of what I feared when I wrote the caution above. The caution was right to exist and wrong
> in its guess.
>
> *Recorded in full because it is the second labelling error this session's own controls caught (the first was the
> mis-specified LEFT-FRONTED ablation row, 11i). Both were caught by running a check whose only purpose was to
> disagree with me, and neither would have been visible from the headline.*
...
| 3 | **event/state fired on >= 0.90 of the 167** | **NOT MET** | **0.8802** (147 of 167); 0.9363 on the 157 reachable at this rung; **0.6120 out of supply on GUM** (constructions only -- see the correction in 11j) |
...
- **Recall, OUT OF SUPPLY (11j). NOT exhausted, and this is the finding that matters most.** GUM/GENTLE gives
  ...
  a long way from the **0.8383** the same arm reaches in supply, with a small CI-separated precision cost that in
  supply does not exist. **The gap between 0.8383 and 0.6120 is the honest measure of how much of my fifteen
  constructions is English and how much is UD-EWT**, and closing it is alternate path D (learn the memberships from
...
OUT-OF-SUPPLY CONTROL (GUM/GENTLE, 1200 sentences, 12+ genres, 1545 subject-bearing clauses; THE FIFTEEN
CONSTRUCTIONS, NO ARC CUE): the mechanism GENERALISES -- non-verbal recall 0.1230 -> 0.6120, a FIVEFOLD rise,
recall +0.1165 CI[+0.1016,+0.1320] CI-separated, every twin beaten CI-separated on both -- but it generalises WORSE
than the matching in-supply arm (0.6120 against 0.8383) and precision is -0.0083 CI[-0.0153,-0.0017], CI-SEPARATED
DOWN where in supply it is flat. A LABELLING ERROR WAS CAUGHT HERE BY MY OWN DECOMPOSITION AND IS RECORDED IN FULL:
I first called that row the shipped arm INCLUDING the arc cue, and the arc-off rerun came back BYTE-IDENTICAL,
because the cell reads ARC_TAU only on the board path. The arc cue's own out-of-supply sweep was then run and
VINDICATES the default-ON: at tau 0.5 it takes GUM's non-verbal clauses 0.6120 -> 0.6257 while precision moves
-0.0083 -> -0.0079 (slightly BETTER), so the CI-separated out-of-supply precision cost is entirely the
CONSTRUCTIONS' -- it is there with the arc cue off and does not grow when it is on -- and the flat region transfers
(tau 0.05-0.90). The default-ON now stands on TWO populations. pri 110's occupancy read was
