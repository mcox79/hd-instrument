...
>
> **What does NOT survive:** any claim about the arc cue out of supply. The proper sweep
> (`--arcgrade --pop gum --cap 1200`) was launched on finding this and lands in
> `data/exp_nonverbal_predication_participants_agent_v1/arcgrade_cap1200.json`. **Strategy should read it before
> landing the arc cue's default-ON**, because the board that justified that default was UD-EWT-only.
>
...
- **Recall, OUT OF SUPPLY (11j). NOT exhausted, and this is the finding that matters most.** GUM/GENTLE gives
  ...
  counts), not more rules. *The arc cue is not implicated in that gap -- it is not in either number; its
  out-of-supply sweep was launched after the correction in 11j and is the one open measurement.*
...
because the cell reads ARC_TAU only on the board path. The arc cue is UNMEASURED out of supply and its sweep is the
one open measurement -- read it before landing its default-ON. pri 110's occupancy read was
