...
- 2026-07-07 (Stage-4 advance-scouting, parallel to Stage-3): ranked Stage-4 gaps (sequential predictive
  generation is THE gate; thalamic routing + N-way basal-ganglia are downstream, correctly deferred; M3
  cortex NoiseChannel is ALREADY BUILT, verified on disk, not a gap) and specified the cheapest GPU probe --
  wire per-step CA3-style cleanup into context accumulation (composes proven positional-binding + cleanup-
  attractor + hetero-associative readout), testing whether the substrate's repeated context-depth HARD_FAIL
  (4 prior cells, 3 HARD_FAIL + 1 MIDDLE_BAND, 0 HARD_PASS) is a noise-compounding artifact (fixable, same
  mechanism as reasoning-depth's per-hop survival law) or a representation-capacity ceiling (needs a
  structurally different context encoding). Startable now on the freed GPU, no Stage-3 dependency.
  P_deflated=0.20-0.25 on "substrate does non-trivial native generation at all." ->
  notes/research_stage4_generation_load_bearing_gap_and_gpu_probe_2026-07-07.md
...
- 2026-07-07: Resonator restart-budget geometric-race LAW derived (n=2 self-improvement-loop instance) -> notes/research_resonator_restart_budget_geometric_race_law_2026-07-07.md (Model A budget p_basin(K5/6)=0.060/0.024; Model B wall q=2 p_basin(K5/6)=0.030/0.003; smoke-pilot already disfavors both at p<1e-3; call: FULL K-sweep more likely MIDDLE/WALL than clean BUDGET, P_deflated=0.40)
- glass-box QA benchmark REFRAMED as weakness-decomposition instrument (not scoreboard, per USER mid-drill correction): notes/research_glass_box_qa_weakness_decomposition_benchmark_2026-07-07.md -- ranked weakness map (composition/branching #1, encoder-970K-scale #2, self-audit-5th-rung #3, per-item-logging-GAP #4 blocking clean attribution, refuse-gate #5 = strongest/protect); fresh on-disk smoke finding (beam-helps/semantic-seed-hurts opposite-sign decomposition) is the new artifact this cycle.
- 2026-07-07 (research, 2x-negative revival drill on semantic-seeded-store HARD_FAIL): pressure-tested whether the semantic benefit from BGE seeding can be captured without the store-collision that killed SEM_BEAM (0.227 vs RANDOM_BEAM 0.502) -> notes/research_semantic_retrieval_capacity_decouple_revival_2026-07-07.md. HEADLINE: partial revival, not full -- decoupled two-representation / semantics-as-reranker (mechanisms 1+3, same architecture) is a real, cheap, low-risk candidate NEVER YET TESTED (add a SEM_RERANK arm reusing RANDOM_BEAM's beam output + already-cached BGE vectors, store untouched, near-zero marginal cost); sparse/low-overlap semantic codes (mechanism 4) has genuine distinct literature support (Willshaw f^2-scaling) but needs a structural codebook redesign, second-tier research thread not a cheap test; whitened/decorrelated in-store seeding (mechanism 2) is the worst-fitted -- the correlation to remove IS the semantic signal here (no separable nuisance direction), and a new quantitative check shows the classical alpha_c(rho) formula (predicts only ~4% capacity loss at measured rho=0.20) badly underestimates the actual ~55% relative collapse, meaning this store already operates far past its classical wall in an already-fragile chained-composition regime. Cheap decisive test + HARD-PASS/HARD-FAIL pre-registered in the note. P_deflated: SEM_RERANK HARD-PASS 0.25-0.35; mechanism-4 ceiling 0.15-0.20; mechanism-2 clearing bar 0.05-0.10 (weakest, most confidently closed).

- 02:31Z: self_improvement_regime_ranking_revival -> notes/research_self_improvement_regime_ranking_revival_2026-07-07.md (density HARD_FAIL = honest null, not closure; ranked reasoning-depth capacity-provisioning as rank-1 revival target ahead of pre-identified resonator, zero new GPU cost; P_deflated=0.50 capped)
2026-07-08T03:41:54Z research: brain-grounded self-capacity extrapolation -> notes/research_brain_grounded_extrapolation_self_capacity_prediction_2026-07-07.md (ranked brain mechanisms for out-of-range self-prediction; disk-verified mechanistic-proxy substitution test; GPU-testable percolation/transmission-coefficient cell proposal)
