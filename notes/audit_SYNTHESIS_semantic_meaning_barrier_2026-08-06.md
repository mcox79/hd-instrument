...
## ✅ FIRST INCREMENT BUILT + HARD_PASS -- the audit direction is VALIDATED (commit 17dd3567b, exp_evaluative_bridging_inference_v1, Director disk-VET all axes)
Drove the #1 rec (construction->valuation->BRIDGING-INFERENCE). Strict ADD, reuses owned GoalOutcomeRegister UNMODIFIED (no new binding operator), invariant-clean (supply the bridging SCHEMA, substrate does retrieve+value+bind), gated to fire ONLY on the OUTCOME_NEVER_TYPED (verb-typing-missed) case. RESULT (HARD_PASS): zero-lexical-overlap bridging **1.0 (9/9) vs lexical-only 0.0** (mg2 "good boy" correct all seeds) -- solves the EXACT class no similarity/verb-typing/theme-match can reach. OVER-FIRE CONTROLS ALL CLEAN (the make-or-break for a new mechanism): valence-correct pos->MET/neg->UNMET 1.0, bystander->no-bridge 1.0 (no wrong-entity bind), non-evaluative->unchanged 1.0 (strict ADD), scramble collapses 0.0 (uses the goal<->outcome content link, not a hidden cue). **This VALIDATES the strategic shift: lexical->INFERENTIAL meaning is correct + buildable from owned organs.** HONEST FOLLOW-UPS (flagged): (1) valuation still a hand-lexicon (EVAL_POS/NEG), NOT grounded_appraisal_sim (no text entry point -- the feature-source/grounding gap, separate follow-up); (2) NEW coref confound found: production GeneralRecencyEntityResolver mis-resolves "He wanted to help his mother"->"mother" (object roster-key) not "henry" (pronoun subject) -- blocks naive end-to-end wiring (real cheap-ish coref fix). NEXT: fix the coref goal-subject confound -> end-to-end wire + re-probe; wire ToM (islanded) for illocutionary force; DEEP: ground the valuation + abstract concepts in reward-PE.

## ✅✅ INFERENTIAL LAYER VALIDATED END-TO-END ON REAL PROSE (2026-08-06, commit d157941c6, Director disk-VET incl no-regression witness run myself)
Fixed the coref confound (GeneralRecencyEntityResolver.subject_entity: two-pass first-name-anywhere -> SINGLE-PASS first-nominal, position-aware; can-fail all pass incl He-wanted-help-his-mother->henry-not-mother) + WIRED the bridging-inference into production as a strict Tier-3 ADD + re-ran the real-prose probe. RESULT (HARD_PASS): the bridge FIRES on mg2 in production -> types the outcome AND resolves owner=HENRY end-to-end; OUTCOME_NEVER_TYPED 4->3; **organ owner-acc 0.50->0.60**. NO REGRESSION (48/48 + 12/12 + flip-control byte-identical, cert 220/3, Director ran test_goal_owner_select.py=4-passed). **HONEST+IMPORTANT: the coref fix DE-INFLATED the recency baseline 0.70->0.60** (its 0.70 was artifactual -- the old bug accidentally landed one item right; on correct coref recency=0.60) so **the organ now TIES recency 0.60 on a FAIR comparison** (was losing to an inflated baseline). first_mention holds 0.70 (remaining gap = 3 pre-reg'd OTHER-construction blockers: affect-state "how proud", negation "could not find", dialogue-splitter -- NOT verb-OOV). **THE WHOLE ARC VALIDATED: probe->barrier->deep-audit->inferential-layer->END-TO-END on real prose, zero regression, owned organs. Audit's lexical->INFERENTIAL call CONFIRMED on real narrative.** NEXT: affect-state predicate bridging (same pattern as praise: "how proud"=positive affect->goal-met); negation-scope; then ground the valuation (reward-PE) + wire ToM.
