...
## WHAT CHANGED LATELY
- **16 Sep, 18:15.** Landed: the reader now remembers, for the length of one page, the answer to the most expensive question it asks itself (which sense of a verb is meant, decided by letting activation spread through its dictionary), instead of asking it twice for the same verb in the same sentence. Reading a page is about a third faster and the reading is unchanged to the byte on all twelve modern test pages. Found on the way: that expensive step barely changes what the reader records (it moves about one field per page), so the next lever is to only run it when the word's usual sense is not already a clear winner, which people measurably do; filed. On the product board every answer is unchanged to the byte and the whole board ran a third faster (the reader's own block 43 percent faster).
- **16 Sep, 16:45.** Landed: when the reader tracks what a character wants, it now checks the right things before calling the goal met: the right person, the right thing, and the right time. Before, 'she wanted to buy a car' counted as met when anyone bought anything, and a goal stated later in a story leaked into predictions about earlier events. On 400 modern short stories the check on the thing is right 94 times in 100 (the old rule: 50), every same-sentence completion is recovered, and no goal leaks backward in time. The cost downstream is two items in fifty, not clearly separated from zero, both the same case of a thing named twice. Found on the way: an over-strict version vetoed correct goals on the strength of a shaky pronoun reading; the repair is to count a conflict only when the step that raised it could actually know. On the product board every row is unchanged to the byte; the new 'where is this person now' check now shows on the board itself (about 43 in a hundred against 19 for the simplest rule).
- **16 Sep, 12:14.** Landed: when the story says 'Tom went into the barn', the reader now records that Tom is in the barn by name, and it keeps that answer, because the step that hands people to the map no longer throws away the reader's own decision about who 'he' is. On eight modern test passages 'where is Tom now' went from about 4 right in a hundred to about 40, clearly separated, and naming the place adds another 21 on top. Also: the map-reading part of the reader existed twice and had drifted apart across four landings; it is one organ now, and the check that was supposed to guard it can fail again. On the product board every existing row is unchanged to the byte; the new 'where is this person now' check reads about 43 in a hundred against 19 for the simplest 'last place mentioned' rule, on 47 questions over eight short modern passages we wrote ourselves, so it is a small, honest instrument rather than a found corpus.
- **16 Sep, 10:52.** Landed: the reader now decides 'is this a new person or thing, or one I already have a file for?' by one weighed competition of cues it learns from its own reading and keeps updating as it reads, opening a new file only when nothing it holds is a good enough match, with 'a' versus 'the' shifting the bar the way a first mention should. Grouping mentions into the right people and things improved by about seven points in a hundred on 28 modern documents, clearly separated; by which entity it picks, pronoun answers improved by about eight points in a hundred, clearly separated. Found on the way: what looked like a drop in pronoun answers was the scorer checking the exact word rather than the entity; the exact-word half is the next rung (mentions are one word wide today). On the product board the seven component rows are unchanged; the reader's pronoun row and its salience row moved up a little. Two costs, both traced to the same missing piece (mentions are one word wide): a new row that scores the reader's own grouping decisions sits about 23 points in a hundred below the simplest 'same word, same thing' rule, and the inferred-emotion check dropped from 90 to 64 in a hundred on 50 short passages because a character's file swallows the phrase next to it. The width fix is the next rung and now carries both as its pass bars; the risk of leaving the new decision on is that emotion inference stays degraded until that rung lands.
...
Last full check: 2026-09-16T22:52:22.061422+00:00 (64 on record). Generated 2026-09-16T22:54:42+00:00.
...
**Next in the queue:**
- /dev/hd-instrument/data/remote_cpu_queue (two variants: the system Python311 and that copy's venv) spawning tools/orchestrator/remote_state_emitter -- targets: -
...
| How a character probably feels about an event | Feelings, goals, beliefs | right 64 in 100 | a little better than the simple rule (44 in 100), not yet convincingly | unchanged since the previous check | brain model; some details still open |
