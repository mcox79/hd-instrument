...
- **PENDING levers (in-flight at compaction):** (a) RAW_CONTINUOUS (unquantized float) readout eval (a344f4, remote GPU, read-only, exp_encoder_retrieval_regime_density_curve_v1) -- last dense variant; (b) OPQ learned-rotation before block-argmax (a8c105) -- lifts block-code retrieval, must NOT break SBC algebra; (c) bypass code-capacity ceiling K128-vs-K256 (aa97d4).
- **>>> THE DECISION (USER's call, once pending levers land): retrieval is stuck ~0.30 vs 0.35. Options: (1) ACCEPT 0.30 as v1 "good-enough, improve later" (BCT compatibility loss CONFIRMED -- encoder swaps are non-breaking, so ship-now-improve-later is SAFE); (2) PUSH harder (bigger code / OPQ / exotic); (3) RELAX the 0.35 scoreboard. Retrieval-at-sparse looks GENUINELY structurally hard (not just process). <<<**
