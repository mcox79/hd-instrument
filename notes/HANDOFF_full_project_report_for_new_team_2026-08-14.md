...
> **METRIC CAVEAT (added 2026-08-14, later the same night as the rest of this doc — keep this when editing).** A gate is a CI-separated margin above `max(orthographic, frequency, scramble)` on the IDENTICAL scorer/n/pool/gold — never a bare absolute number. The baseline must be STANDALONE: a shortcut (e.g. a spelling channel) added ON TOP of the system under test is a decomposition of that system, not a floor. Two passages below were corrected in place, with visible banners, because they violated this: §0's "6× its scramble floor" framing (scramble is the WEAKEST available baseline, not a meaningful one to beat) and §7's "the orthographic floor is UNMEASURED" (it has since been measured, and beats the live readout). Read every percentage in this doc against this caveat, not in isolation — see `exp_orthographic_floor_vet_v1` (`58a125c88`) and `exp_graded_path_vs_orthographic_floor_v1` (`39f3fe2a1`) for the measurements that forced the corrections.
...
- **The QUALITY of what it grounds does NOT yet clear its own gate.** The live grounding-readout is at a measured floor: open-vocabulary hit@1 **4.80% vs 0.80% scramble** — 6× its scramble floor, but the ≥10% revival gate is RETIRED (2026-08-14) and C3 now FAILS the four-condition hardened gate (`tools/c3_gate.py`; `notes/c3_gate_hardening_2026-08-14.md`). The defect is localized and specific: **within-neighbourhood separation** — the substrate lands in the right *neighbourhood* but picks the wrong *member* (axon→dendrite, artery→vessel). Retrieval is healthy (self-retrieval 0.786); supply is closed; the problem is discriminating near-paradigmatic members.
  ...
  > **CORRECTION (2026-08-14, later the same night).** The "6× its scramble floor" framing above is RETIRED and marked NEVER RE-QUOTE — scramble (0.80%) is the WEAKEST available baseline, so beating it 6× is not evidence of anything. The framing that survives: **4.80% is BELOW the orthographic floor of 8.70%, CI-separated**, on the identical items/pool/gold (`exp_orthographic_floor_vet_v1`, commit `58a125c88`). A zero-meaning character-trigram arm (`A6_TRIGRAM_ONLY`, no substrate signal at all) scores **0.0870 [0.0783, 0.0960]** against our **0.0480 [0.0413, 0.0548]** — spelling alone beats the read-out by 1.8×, CIs do not overlap. Even the crudest attack, longest shared prefix, scores 0.0588 and still beats us. "We underperform a spell-checker" is now established (this reverses an earlier same-day audit, `9ca1cffa2`, that had left the question open). This does NOT invalidate the growth-mechanism claim in the paragraph above — concept/fact count is a different claim from readout quality — see the METRIC CAVEAT at the top of this document.
...
- **H1 — grounding-readout QUALITY is the live gate, and it is NOT passing** (`exp_grounding_readout_known_answer_v1`, metrics `204eba1a0`). Banked-facts arm **AT_FLOOR** (GOLD_HIT 0.0251 vs scramble 0.0125, CI crosses zero low). 2AFC **MIDDLE_BAND** (0.5393 vs 0.50 chance, short of 0.60). **Open-vocab readout (closest to the live loop, n=4000/5491 anchors): hit@1 4.80% vs scramble 0.80% — 6× floor, but the ≥10% gate is RETIRED; C3 FAILS the four-condition hardened gate and is UNMEASURED on 3 of its 4 conditions (`tools/c3_gate.py`).** Self-retrieval 0.786 (retrieval healthy). **Defect = within-neighbourhood separation** (axon→dendrite, artery→vessel: right neighbourhood, wrong member). *Correction:* an earlier "65.7% tautology" figure was an **eligibility bug** (fix `1b2022522` → live path emits 0% tautology), not a meaning failure.
  ...
  > **CORRECTION:** "6× floor" here is the same retired framing corrected in §0 above with the orthographic-floor numbers — see that banner. The "**UNMEASURED on 3 of its 4 conditions**" clause is a DIFFERENT, still-accurate claim: it means the C3 gate's own readout cell (`B5_OPEN_REAL`) never computed median rank, separation margin, or a string-form control itself (`notes/c3_gate_hardening_2026-08-14.md` §3) — do not confuse it with "the orthographic floor is unmeasured" (§7's correction below), which is a separate statement and is now false.
- **H2 — graded comparator landed default-ON** (`38f7a0d5c`, live path 0.6395→0.6980, +0.058). **Do NOT cite 0.7495** (unshipped d=1024 arm; mechanism claim separately refuted). Landed-VET (`f05b8a88a`): the *number* survives, the *mechanism claim is withdrawn*.
  ...
  > **CORRECTION (2026-08-14, later the same night).** "The number survives, the mechanism claim is withdrawn" above is now itself dead — both halves need restating. First, a currency clarification the original line did not make: **0.6395→0.6980 (+0.058) is a near-neighbour 2AFC accuracy (chance 0.50, pool 2,377)**, not a C3 open-vocabulary hit@1 number — do not read it as progress toward the grounding-quality gate in §7/§H1 (`notes/STATUS.md` records this explicitly as a DO-NOT-REDO: quoting it as a C3 number is "WRONG CURRENCY"). Second, on the metric the gate actually measures: `exp_graded_path_vs_orthographic_floor_v1` (`39f3fe2a1`) tested the graded switch directly against open-vocab hit@1 and found it NULL — ON minus OFF = +0.0015, CI [-0.0055, +0.0083], spans zero, at draw-noise scale. The related **+0.0602** figure (0.6395→0.6997) that had circulated as "the number" was traced to a DIFFERENT cell (`exp_graded_divisive_comparator_v1`) measuring the same 2AFC task, not C3 hit@1 (pool 5,491) — a gain measured on one scorer was carried to another where it does not exist. The graded comparator's own 2AFC HARD_PASS is NOT invalidated by this — it stands on its own scorer — but it must not be cited toward the open-vocabulary grounding-quality question this report is actually gated on. See `notes/graded_path_does_not_clear_the_orthographic_floor_2026-08-14.md`.
...
2. **The quality of what it grounds does not yet clear its own gate.** Open-vocab grounding hit@1 is 4.80% vs 0.80% scramble — a real floor, but scramble is the WEAKEST baseline (frequency is 1.85%; the orthographic floor is UNMEASURED) and the ≥10% criterion is RETIRED in favour of the four-condition hardened gate (`tools/c3_gate.py`). The defect is *within-neighbourhood separation*: correct neighbourhood, wrong member. This is exactly the "meaning is a ~380-word hand-lexicon while a 39.7k-word norms island + a 237.7M-token encoder sit unused" gap (§5.3 MEH #1) showing up as a measured shortfall.
  ...
  > **CORRECTION (2026-08-14, later the same night) — the single most consequential stale line in this report.** "The orthographic floor is UNMEASURED" above is false and was left uncorrected long enough to mislead: it told a reader the strongest floor had never been run, when it has been run and we lost to it. Measured, on the identical items/pool/gold as the 4.80% headline: spelling-alone (`A6_TRIGRAM_ONLY`, zero substrate signal — literally `t_mat[sel] @ tq`, cosine between character-trigram vectors) scores **0.0870 [0.0783, 0.0960]**, our read-out scores **0.0480 [0.0413, 0.0548]**; delta **+0.0390**, CI excludes zero (`exp_orthographic_floor_vet_v1`, commit `58a125c88`). Even the crudest orthographic attack, longest-shared-prefix (`A7_PREFIX_ONLY`), scores 0.0588 and beats us. Frequency (1.85%) is genuinely the WEAKEST named floor, not the strongest — do not average it with the (now-measured) orthographic floor. Read this together with the METRIC CAVEAT at the top of the document: a gate is a margin above the STRONGEST standalone floor, and the strongest floor here is spelling, which we do not clear.
