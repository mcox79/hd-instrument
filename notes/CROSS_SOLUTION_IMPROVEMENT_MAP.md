...
**WIP frontier:** pri-1 grow_the_causal_mechanism (`predictive_world_model` rung-1→relabel + `causal_reasoner` BF_SPIRIT | causal-testimony store, sm.causal_links, pos_tagger, WN | **the generative rung-2/3 counterfactual SIMULATION over a bound per-item situation-model; coupling-sign needs grounded interventional Δ; web-scale corpus; incremental_parser**) · pri-2 point-vector (proposed sequential_meaning_channel | grounded_similarity, dep_ppmi, WordNet taxonomic, ConceptSpace | **grow the learned channel by reading; learned channel is RELATION-BLIND; incremental_parser; bigger gold**) · pri-5 measure_end_to_end (reading_grounding_loop BF_SPIRIT | canonicalize_fast, grounded_similarity, distributional bundle | **grounded-dominant read + self-calibrating SDT accept; a coverage-QUALITY instrument; incremental_parser**) · pri-6 reward-audit (catalog | 7-seed import closure | **a live control loop that CONSUMES vigor; dials #2-6; V=M@R; ToM organ**).

**Coref/entity/causal/knowledge (CONT-23→29):** de-leak (online_entity_cluster/crosstype_bridge/crosstype_live_adapter BF_SPIRIT | live parse, coref, salience_binder, typed_spokes | KB coverage) · crosstype/route_unified (crosstype_bridge BF_SPIRIT | parsed tokens, ACT-R salience, Centering | live-wire; occupation-KB=located-neg) · obl-spatial (joint_relation_frontend BF_SPIRIT, spatial_relational_model BF | exact-MAP marginals | **arc scorer surface-feature; AtLocationClassFit; powered spatial gold**) · generate_dont_retrieve (predictive_world_model BF_SPIRIT | pos_tagger, simplewiki | **N400 loop-closure=world-model; rich event key; event_type MFS→WSD**) · reader-audit (catalog | substrate.read trace | gold-coref leak, C7 read-out) · grounding-audit (catalog | reading_grounding_loop trace | **G1 read-out loses to counting; G2 grounded_similarity inert**) · C7 attractor (iterative_attractor/cleanup_family BF | canonicalize_fast, conceptual_meaning | **fusion-wire gated on coverage; point-vector rep=deep lever**) · name-bridge (safe_kb_gate BF | DBpedia, typed_spokes, recency | **doc-local COVERAGE; generative route WORLD-MODEL-GATED**) · acquire_p31 (typed_spokes C8 BF_SPIRIT | DBpedia InstanceOf, C5 closure | live-wire; **C8 asset unbuilt**).
