...
| 2 | Salience prior operating point (role prominence, history depth, decay) | ACT-R base-level B = ln Σ w(role)·Δt^−d | `salience_binder.actr_activation` with w=(4,2.5,2,1), d=2 (fitted on LitBank subject pronouns) | BF (computation) / swept params | d=0.5 collapses (0.32); d 2–3 best; role magnitudes for OBJECT pronouns UNSWEPT → probe v10 (written) | probes v6/v10 |
...
2. **Rung 2, the prior's operating point (params):** run probe v10; adopt nothing, sweep; report.
