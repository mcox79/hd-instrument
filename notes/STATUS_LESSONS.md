...
## FOLDED FROM THE PLAN STATE BLOCK 2026-08-23 (SH-7 entry, full text)

> ## **LANDED 08-22 (this session, integration lane): BOTH SOLVER HAND-OFFS ARE IN**
> **SH-7 `HDI_FRESH_RUN=<tag>`** (`experiments/_seed_checkpoint.get_output_dir`, `ed9ce6273`) sends a cell to an EMPTY sibling so it genuinely recomputes -- no deletion, landed dir never opened for writing, byte-identity BY CONSTRUCTION. **The load-bearing control is the NEGATIVE one and it is a witness, not a comment: env unset -> paths byte-identical, else every landed directory in the repo is silently orphaned with nothing erroring** (`test_fresh_recompute_redirect.py` 6/6, and I injected the bug to confirm the guard FIRES). 🔻 **THE `87` of `421` (~21%) FIGURE IS WITHDRAWN 08-23 -- IT UNDERCOUNTED BY MATCHING A VARIABLE NAME.** **RE-MEASURED, AND COVERAGE IS `71.2%`: `3,495` of `4,908` landed results route through the helper and CAN be re-run today; `1,413` replay.** *The old count matched `OUTPUT_DIR = helper(...)`, but the common shape is `out_dir = get_output_dir(...)` -- a LOCAL, lowercase, inside a function. Which variable receives the path has nothing to do with whether the redirect fires.* **`tools/reproducibility_inventory.py` parses the AST instead, and a positive control in a FRESH PROCESS confirms both helpers really redirect under `HDI_FRESH_RUN`.** ⚠️ *My own first re-measurement repeated the SAME error and read `2` of `4,908` -- a catastrophe I nearly reported.*
...
## FOLDED FROM STATUS.md 2026-08-23 (twenty-second pass, the 08-22 lookup entry)
...
## FOLDED FROM STATUS.md 2026-08-23 (twenty-seventh pass, meaning-channel entry at full length)
...
- 🧠 **THE MEANING CHANNEL: SIX MEASUREMENTS 08-23, ALL IN `notes/problems/reader_meaning_channel/`
  ...
  *NOT false -- unmeasurable on our assets, same disposition as the adjective row. Doing the power
  ...
  ⚠️ **SCOPE -- IT CHANGES THE INSTRUCTION: this is ATTRIBUTE segregation, not PER-ITEM.** *The
  segregated arm assumes you KNOW WHICH SLOT to read; in cortex that is free because slots are
  ANATOMICAL AND TYPED.* **At `D=256`, up to `32` ATTRIBUTE streams sit at/above the 8-dim floor;
  PER-ITEM slots give `2.56` dims at 100 items and need an index.** ➡️ **NOT "a slot per word" --
  I nearly wrote that. It is: MEANING GETS ITS OWN ATTRIBUTE-TYPED SLOT, separate from whatever
  else the reading loop accumulates.** *The constraint is not tight; we are simply not using it.*
...
## FOLDED FROM THE PLAN 2026-08-23 (meaning-channel + resume entries, full text)
...
## FOLDED FROM THE PLAN 2026-08-23 (Q115 + GUI tab, full text)
...
## FOLDED FROM THE PLAN 2026-08-23 (Q116 + random-credit, full text)
