...
### 4f. THE DOWNSTREAM PROPAGATION -- pri 113's own participant instrument, and a located negative that is no longer separated
...
### 4g. AN OPERATING-POINT MOVE THAT IS NOT MINE TO SET (the phase diagram)

The in-order beam width is a swept parameter of the organ, not of this cue. At `HDLAB_ARM_BEAM=16` (the live
default is 8), the same one-process A/B gives shipped 0.5740 and **patched 0.6746** (+0.1006 CI[+0.0568,+0.1489]),
verbal 0.8617 -> 0.8683, **UAS 0.6362 -> 0.6414**. So a wider beam is worth another +0.0178 on top of this
solution and +0.0044 UAS -- it closes 38% of the remaining decode gap to the whole-sentence search -- and it helps
the SHIPPED organ too (0.5385 -> 0.5740). It roughly doubles the read cost, which is why the default is strategy's
call and not mine; it is reported here as a measured, free move on the phase diagram.
...
|---|---|---|---|
| non-verbal subject attachment | **0.6568** | a supervised neural parser is ~0.95 on nsubj; a competent human reader is at ceiling | (1) the in-order decode, worth **+0.0473** measured (the same scores under the search decode give 0.7041); (2) the tree convention, a further **+0.0178** (forced re-attach 0.7219); (3) beyond that the arc scorer's REPRESENTATION -- pri 113 sections 23/25/32's contextual encoder, untested in strong form |
