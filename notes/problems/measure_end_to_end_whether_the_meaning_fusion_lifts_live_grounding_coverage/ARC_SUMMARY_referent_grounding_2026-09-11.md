...
`.venv/Scripts/python.exe verification/test_meaning_fusion_live_coverage.py` (38/38; W37-W41 are this arc).
...
| exp_meaning_fusion_referent_live_rsa_v1.py (W41) | the referent spoke vs the LOOP'S OWN live reps (E.build_curriculum_field D/G/Gd) on MEN graded RSA over the loop-reachable concrete-noun vocab; twin + pair-bootstrap CI | BF_SPIRIT: same convergent-cue separate-pool fusion + DINOv2 frozen at ingest; MEN non-circular. Closes the offline->live bridge: DINOv2 is the strongest single LIVE channel (0.698 > grounded 0.53; incumbent-D at chance -0.015) and adds +0.312 R2 over live {D,G} CI-sep twin-losing |
...
3. WIRE (if owner wants the referent spoke live): CLOSED (W41, 2026-09-11). The referent spoke is now ALSO validated
   against the loop's OWN live representations (E.build_curriculum_field D/G/Gd) on the powered MEN graded-relatedness
   gold over the loop-reachable concrete-noun vocab (n=125): DINOv2 is the STRONGEST single LIVE channel (rho 0.698 >
   grounded 0.53; the loop's live distributional rep is AT CHANCE -0.015 there) and adds +0.312 R2 over live {D,G}
   CI-sep [0.182,0.440] twin-losing. Wiring guidance: on the concrete-depictable slice the visual channel dominates the
   distributional channel -- the convergent-cue precision grid already loads it highest. The synonym-ranking FRONTIER and
   coverage-COUNT stay data-blocked for this modality (measured: SimLex-high -> 0 referent anchor queries, MEN>=40 -> 13)
   -- so expect the live lift on concrete-object RELATEDNESS, not on the count/frontier metrics.
