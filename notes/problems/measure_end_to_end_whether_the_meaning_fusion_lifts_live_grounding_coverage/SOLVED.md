...
## UPDATE 2026-09-11 (W41, BF-push cron) -- THE LAST OPEN BRIDGE CLOSED: the referent spoke measured against the LOOP'S OWN LIVE representations (not curated vectors) -- it is the STRONGEST single live channel and ADDS CI-separated twin-losing

Cron: "any more we can push, brain-foundationally?" The honest self-audit found ONE genuine gap the ARC_SUMMARY had
flagged as open (next-steps item 3): W37-W40 validated the DINOv2 referent spoke only against CURATED concept vectors
(grounded = a Lancaster-norm lookup; text = a SEQ channel grown over the concept list). It was NEVER tested against what
the reading-grounding loop ACTUALLY holds -- the distributional ConceptSpace bundle canonicalize ranks over (INCUMBENT)
and the loop's own grounded ATL rep. That is the exact object the problem name is about ("the LIVE grounding loop"). So
before accepting the carried-over "SimLex-coverage-bound ~18 pairs" caveat, we tested the STRONGER version (discipline:
do not generalize a narrow failure to impossible; test the brain-real version and MEASURE).

NEW CELL: experiments/exp_meaning_fusion_referent_live_rsa_v1.py -- brings the referent spoke into the loop's OWN
representational field (E.build_curriculum_field's D/G/Gd, the live canonicalize reps) and measures, on the powered MEN
graded-relatedness gold restricted to the loop's REACHABLE concrete-noun vocabulary, whether the referent adds over the
loop's live {text, grounded}. Same convergent-cue separate-pool fusion (Ma-Pouget/Ernst-Banks), DINOv2 frozen at ingest,
shuffled-visual twin, pair-bootstrap CI. Own data dir; NO hdlab write; recall path untouched.

MEASURED POWERING (full curriculum, the number that upgrades the caveat): the loop has 556 anchors / 4678 words; 54
anchors and 381 words are referent-covered. The current live gold SimLex-high yields ZERO referent-covered live anchor
queries (not "~18" -- literally 0; that is WHY the referent was never on the live synonym-ranking frontier). MEN gives
125 pairs with both words referent-covered and in the curriculum (well powered for graded RSA) but only 13 near-synonym
(MEN>=40) referent-covered anchor-pool queries -> the loop's SYNONYM-RANKING frontier is genuinely data-blocked for this
modality, while MEN's GRADED-RELATEDNESS RSA (the correct shape for a graded gold) is powered.

RESULT (n_ref=125, seed 20260911, 2000-boot):
- LIVE per-channel rho on MEN: incumbent-D = -0.015 (the loop's live distributional rep is AT CHANCE on concrete-object
  relatedness), grounded-G = 0.530, distinctive-Gd = 0.550, VISUAL-DINOv2 = 0.698, clip-diag = 0.751. The DINOv2 referent
  is the STRONGEST single LIVE channel on the concrete-noun slice -- above the loop's own grounded rep.
- THE WIN (CI-separated, twin losing): VISUAL adds +0.312 R2 over the loop's live {D, G} (CI [0.182, 0.440]; twin +0.005;
  real-minus-twin CI [0.167, 0.428]); +0.278 R2 over live {D, Gd} (CI [0.165, 0.402], twin-losing). This is a LARGER
  additive gain than the offline curated test (+0.086, W39) -- not because the referent got better, but because the loop's
  LIVE text channel is blind on this slice, so the referent supplies perceptual/semantic structure nothing else in the
  live loop has. On the full both-in-curriculum slice (n=1281) incumbent-D climbs to 0.199 (distributional co-occurrence
  works for the mixed/relational vocabulary) -- confirming the referent's live value is concentrated EXACTLY where the
  loop's distributional channel is weakest (concrete depictable objects).
- FUSION SANITY: the recovery-gated convergent-cue fusion(D,G,V) scores 0.730 vs best-single 0.735 -- NO dilution
  (best weights (0.25,0.25,0.5) correctly load the visual channel highest); it does not beat the best single CI-sep here
  only because the visual channel is ALREADY the near-best single on this slice (there is little left to add on top).

HONEST BOUNDARY. This is graded RSA on the concrete-noun slice the loop can reach (n=125); it is NOT the coverage-COUNT
(W19: quality-blind) nor the synonym-ranking FRONTIER (W19/W20 -- data-blocked for this modality: SimLex->0, MEN>=40->13).
Those metrics stay as previously located. What W41 adds is the missing measurement the problem name demands: the referent
lever verified against the loop's ACTUAL live representations, where it is not merely additive but the DOMINANT single
channel for concrete-object relatedness, CI-sep twin-losing. This CLOSES ARC_SUMMARY next-step item 3 ("validated on
similarity golds, NOT on the live loop") -- it now IS validated on the live loop's own representations, brain-foundationally.

SUBSTRATE INCORPORATION MANIFEST (W41): INCORPORATE -- when the referent spoke is wired into the live loop, expect and
accept that on the concrete-depictable slice the VISUAL channel dominates the loop's distributional channel (which is at
...
---
INTEGRATED_BY_STRATEGY 2026-09-11 — reverified 38/38 first-hand before AND after the landing. LANDED LIVE: `hdlab.reading_grounding_loop.FusedSenseRanker` (grounded-distinctive + grown-SEQ + referent convergent-cue read, SDT accept criterion) at the grounding gate, default ON; directional all-content tracking ON; ROUTE-B sidecar persistence; `tools/grow_seq_store.py` (1M Simple-Wiki lines) + `Substrate()` merge of `data/foundation/seq_store_v1`; visual-referent arm in `hdlab.sensorimotor_spoke` (FOUNDATION asset at ingest); `grounded_similarity.grounded_reliability`; NEW board arm `grounding_coverage_quality` (the coverage-QUALITY instrument: FUSED MRR@0.5 0.375 vs incumbent 0.014, grounded-only 0.175, twin loses, n=87 smoke). Witness `verification/test_fused_sense_ranker_live.py` 8/8. NOT wired (per W21–W23): valence axis / confidence reweighting. §2b + registry + BF ledger + KNOWLEDGE_ASSET_REGISTER + INTEGRATION_LEDGER updated.
