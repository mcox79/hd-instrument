...
- **PRECISE LOCALIZATION OF THE LAST NON-BF ATOM:** the pooling FIX (role-filler binding) is BF (circular
  convolution, PINNED); the readout/fusion/decision are BF; the grounded channel is BF. The ONE remaining
  non-BF dependency for the fully-BF fix is the SYNTACTIC-ROLE SOURCE -- a brain-foundational parser
  (`incremental_parser` / predictive-coding structure-building) replacing the frozen-supervised arc-eager. The
  BF fix is complete modulo that parser. (A BF parser is a mapped follow-on problem, not this deliverable.)
