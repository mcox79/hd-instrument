...
priority:
review: EXCELLENT
review_text: "SOLVED (owner-DONE) integrated 2026-08-30 — a live-validated ToM x TIME composition win. Reverified FIRST-HAND: test_belief_timeline.py 70/70 PASS (scaffold-free; recomputes the live end-to-end + CI-sep from source, not cached metrics). A per-agent BELIEF TIMELINE generalizes belief_partition from an n=1 snapshot to n ordered observation-gated changes: belief_A(X,T) = the latest event about X that A OBSERVED with order <= T, persisting between observations (Dowty inertia), ordered by the REAL temporal-order register, read out on the substrate's OWN belief_partition FHRR organs. THE CAPABILITY NUMBER IS THE LIVE END-TO-END 0.902 [0.805,0.976] vs the timeline-agnostic (current-belief-only) floor 0.463 [0.317,0.634], CI-separated, driven by the REAL observation-cue extractor (perceptual_access_ledger, 0.951 extraction) on 15 real-English multi-event passages — imperfect extraction in the loop; oracle 1.000, live-vs-oracle gap 0.098 = the known observation-cue front-end residual. The constructed-gold 1.000 [1.000,1.000] vs floor 0.460 is a MECHANISM-correctness demonstration, gated by an info-free twin (shuffled event/observation order, p95 0.535 < 1.000) and a positive control on belief-at-T != final-observed (n=218, floor 0.000). COMPOSITION WITH THE REAL TIME REGISTER IS LOAD-BEARING: register-ordered belief 1.000 vs narration-ordered 0.000 on flashback prose (twin ties on linear controls), and the combined LIVE stack (live temporal-order + live observation cue jointly) is >=0.90 on flashback prose in-witness. BRAIN-FAITHFUL, 5 online drills each CORRECTING the design: per-agent belief separate from reality (TPJ/mPFC, Saxe) PINNED; observation-gating seeing->knowing (Wimmer & Perner) PINNED; sample-and-hold persistence (Dowty; the frame problem) PINNED; register-ordered chronology (Reichenbach; MTL) PINNED; decoupled store beating the curse-of-knowledge (Birch & Bloom). Honest OUR-INVENTION flags: the FHRR bind() algebra (unpinned at neural level, defensible computational model — SEM/Franklin 2020), the closed inference-schema set, thresholds/tempering. Rich extra layers, all witnessed: decaying confidence (access-decays/value-persists, Spearman 0.995 vs twin 0.04), evidence-gated INFERRED update (Sodian-Wimmer dissociation), distributional/partial belief as an FHRR superposition narrowing over time (F1 1.000 vs crisp 0.494), a GRADED posterior ranking below-MAP candidates (Bayesian ToM 0.957 vs floors 0.505), and recency->precision unification (confidence = posterior entropy, Spearman 1.0). DEFLATION (honest): the constructed 1.000s are mechanism demos not capability; the capability headline is the 0.902 LIVE; the flashback-register composition is a load-bearing demonstration (small n) not a powered capability claim. Still EXCELLENT: a live-validated cross-organ ToM x TIME composition beating the timeline-agnostic floor CI-separated with a real extractor in the loop, honest about its bounds, composing two already-integrated organs. hdlab landing QUEUED (Q111): promote experiments/belief_timeline.py (sample-and-hold belief rep + FHRR temporal-context rep + observation-gated update, built purely on hdlab.belief_partition/binding/graded_temporal_context) to hdlab/belief_timeline.py as a standalone default-off organ; the reader wiring is coupled (part of the situation_model/assembly path). Audit §2b folded (new GOALS/ToM x TIME composition organ; belief-separation + observation-gating + sample-and-hold PINNED, FHRR bind + inference-schema-set OUR-INVENTION-flagged). NEXT primed: the observation-cue front-end residual (the 0.098 live gap) is the named follow-on."
---

> ## ✅ SOLVER REVIEW — INTEGRATED 2026-08-30 (grade: EXCELLENT; SOLVED owner-DONE)
> **Verdict:** a live-validated ToM×TIME composition win. Reverified first-hand (`test_belief_timeline.py` **70/70
> PASS**, scaffold-free — recomputes the live e2e from source). The capability number is the **LIVE end-to-end 0.902
> [0.805,0.976] vs the timeline-agnostic floor 0.463 [0.317,0.634] CI-separated**, with the REAL observation-cue extractor
> (0.951) in the loop (oracle 1.000, gap 0.098 = the observation front-end residual). The constructed-gold 1.000 is
> mechanism-correctness (info-free shuffled-order twin p95 0.535 loses; positive control belief-at-T≠final n=218 floor
> 0.000). **Composition with the REAL temporal-order register is load-bearing** — register-order 1.000 vs narration-order
> 0.000 on flashback prose. Brain-faithful (belief-separation, observation-gating, sample-and-hold/Dowty inertia,
> Reichenbach chronology — all PINNED; FHRR bind + inference-schema-set OUR-INVENTION-flagged), 5 self-correcting online
> drills, and witnessed extra layers (decaying confidence, evidence-gated inference, distributional/graded posterior,
> precision=entropy). Deflation: the 1.000s are mechanism demos, the capability is the 0.902 live; flashback composition is
> a load-bearing demo (small n). **Grade EXCELLENT.** hdlab landing **QUEUED (Q111)**: promote `experiments/belief_timeline.py`
> → `hdlab/belief_timeline.py` (default-off standalone; built purely on `belief_partition`/`binding`/`graded_temporal_context`);
> reader wiring is coupled. Audit §2b folded. Next primed: the observation-cue front-end residual (the 0.098 live gap).
