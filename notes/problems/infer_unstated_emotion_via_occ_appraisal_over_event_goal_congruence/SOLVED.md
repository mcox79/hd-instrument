...
- **PINNED -- goals carry a FAILED status that is tracked, not deleted** (Lutz & Radvansky 1997: failed > completed
  > neutral activation; Suh & Trabasso 1993 reinstatement). The goal register's OWN docstring pins this; its
  track_status only implemented it for a NEGATED goal clause, not a THWARTING event -- an incomplete realization.
...
- **event<->goal MATCHING is Phase-1-gated** (the dominant ceiling). I EVALUATED the LIVE meaning hub
  `hdlab.bridging_inference` for it (the brief's suggested lever): relatedness is DIRECTIONALLY correct
  (sell~buy 0.29) but **POLARITY-BLIND** (win~lose 0.26 -- high relatedness, OPPOSITE outcome) and thin-margin
  (land~offer 0.05, reach~fail -0.05). It can find that an event BEARS ON a goal but not whether it SATISFIES or
  THWARTS it -- the exact ToM desire-meaning located negative ("meaning-channel margins too thin"). Shipping it
  would LOWER fidelity, so it is deliberately NOT wired; it is a filed next-problem (Section 7).
...
- **event<->goal semantic matching (the meaning channel)** -- the appraisal's ceiling. Needs a POLARITY-AWARE
  event<->goal relatedness (does event E satisfy or thwart goal G), which `bridging_inference` (polarity-blind) +
  `context_grounded_valence` (HARM/HELP event valuation, torch; an INPUT at most -- no goal/experiencer notion)
  each supply half of. The faithful build = naive-utility / converse-relation over the ATL hub -> Phase-1. HIGH.
...
is the appraisal's bound (density phase cut: oracle density-invariant 1.000, end-to-end 0.94->0.06 sparse) and it
routes into Phase-1 -- `bridging_inference` is directionally-correct but POLARITY-BLIND, the same wall the ToM
chain's desire-meaning and the goal register's Tier-2 negatives already point to. This fills part of the Tier-6
AFFECT-GOALS-SOCIAL fidelity gap (previously un-fidelity-scored): the appraisal COMPUTATION is now scored and PINNED.
...
- The headline gold is CONSTRUCTED (by me), not externally authored. WITHDRAW FIRST if wrong: I mitigate with the
  ...
  (e.g. Rashkin 2018 Story-Commonsense Plutchik labels over ROCStories, pre-authorized).
...
OCC-appraisal arm (unstated-emotion valence+type) -- board-invisible today. (3) The Phase-1 event<->goal
POLARITY-AWARE matcher (converse-relation / naive-utility over the ATL hub) -- the measured ceiling; the highest-
value follow-on. (4) Acquire an external modern OCC/emotion-attribution gold (Rashkin 2018 over ROCStories) to
corroborate the constructed headline. (5) Fold the AUDIT UPDATE into BRAIN_FOUNDATIONAL_AUDIT.md.
