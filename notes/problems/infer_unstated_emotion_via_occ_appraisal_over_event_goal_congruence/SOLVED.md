...
## Section 6b -- THE UNIFYING ARCHITECTURE (the synthesis of every wall drilled; the recommended LANDING design)
Across all four research drills the SAME architecture kept surfacing, and it is both the EFFICIENCY and the
brain-FIDELITY upgrade the accumulated knowledge points to. Every resolver branch I built -- same-agent satisfy /
beneficiary satisfy / thwart / converse / antonym / beneficiary-dispossess / maintain-thwart / goal-script
terminal-state / prospect confirm-disconfirm -- is ONE computation: a Hobbs-style weighted-abductive MATCH of each
incoming clause against the KNOWLEDGE of each OPEN GOAL (its head/theme/role-fillers/converse-partners/terminal-state
markers). The brain runs this as ONE uniform operation (Hobbs, Stickel, Appelt & Martin 1993: reference resolution,
metonymy, and implicit-event inference are the SAME abductive match) over an OPEN-GOAL REGISTRY (Zwaan & Radvansky
1998 motivation dimension) with Kintsch 1988 construction-integration settling.

RECOMMENDED LANDING (strategy, instead of 8 ad-hoc flags): a unified `resolve_goal_outcome(open_goals, clause,
assets)` that (1) maintains an OPEN-GOAL REGISTRY updated ONLINE clause-by-clause; (2) for each new clause, scores it
against EVERY open goal via the shared knowledge assets (converse/perspective table, beneficiary/role-filler frames,
goal-script terminal-state KB) with a Hobbs cost; (3) pops the goal ATTAIN/THWART at the lowest-cost match; (4) fires
the OCC appraisal (+ the agency/prospect dimensions). This SUBSUMES the ad-hoc branches (ONE pass, ONE asset store =
the compute efficiency), MATCHES the brain's architecture (the fidelity), REMOVES the branch-ordering hazards I hit
during the build (the correctness win), and NATURALLY yields MIXED emotions (multiple open goals resolving on one
clause -> relief + disappointment). PROTOTYPED + MEASURED (`_occ_unified_resolver.py` + `exp_occ_unified_resolver_v1.py`):
BEHAVIOR-PRESERVING (the dominant emotion reproduces the branch results -- main OCC gold 0.940 = the headline exactly,
converse 0.833 = branch, sparse+goal-script 0.727 ~= branch) AND MIXED EMOTIONS for free (3/3 two-goal items yield
BOTH: satisfaction+disappointment, relief+disappointment -- which the single-emotion branch code cannot express).
The build surfaced + fixed two real unification bugs (a converse partner mis-counted as a WordNet antonym; pronoun
goal-agents unbound) -- exactly the ordering/coref hazards a unified resolver removes. This is the recommended
LANDING design (strategy lands the ONE resolver, not the 8 flags).
...
**P4 -- THE TRUE-PHASE-1 TAIL (smaller than thought; all STRUCTURED-ASSET or gold-gated).** After P3, the residual:
THEME COREFERENCE (report=file, garden=flowers) -- PROBED the LIVE `bridging_inference` ATL hub for it and it is
a LOCATED NEGATIVE (rel(report,file)=0.047, rel(garden,flowers)=0.066, rel(keys,house)=0.015, several OOV) -> the
distributional hub is too thin + non-directional; the faithful build uses a STRUCTURED lexical asset (WordNet
meronymy/synonymy, ConceptNet PartOf), the SAME class as the converse table + goal-script KB. PREDICAMENT->goal
inference (stranded->get-home, Graesser why-question); multi-hop result-states; and the NORM channel (pride/shame,
anger-justification-defeat). CROSS-CUTTING LESSON (measured 3x): every STRUCTURED relation the appraisal needs
(converse, result-state, meronymy) is a KNOWLEDGE-ASSET job; the distributional similarity hub is ONLY the
fuzzy-similarity fallback (rel(win,lose)~=rel(sell,buy) -> similarity cannot sign or direct these relations).
