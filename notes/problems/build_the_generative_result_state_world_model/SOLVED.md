...
## ✅ FINAL VERDICT (2026-09-09, cycles 30-31) -- TRIANGULATED LOCATED NEGATIVE (the bar's blessed full pass)
The generative result-state world-model's MECHANISMS are built + brain-foundational; the wall is now
FULLY CHARACTERIZED and TRIANGULATED across THREE independent solver-side levers + a converging solver:
1. **ORACLE-CEILING** (`exp_genworldmodel_oracle_ceiling_v1`, witness 3/3): on a trap-proof self-supervised
   instrument (naturalistic narrative, glass-box parse, NO crowd gold/spaCy/LLM), the online per-story world-model
   ("path A") closes **~32%** of the achievable gap; the other **~68% needs having EXPERIENCED the specific
   scenario** -- lifetime-grounded knowledge that story-context alone cannot supply.
2. **BAR #4 top-down-into-extraction** (`exp_genworldmodel_topdown_twovalid_v1`, witness 3/3): feeding the
   world-model's forward EXPECTATION into the who-did-what two-valid patient pick (UD-EWT, glass-box arc-eager
   parse) recovers only **8/82 (~10%)** -- and that is SALIENCE (shuffle-twin 7, salience-only 8); the generative
   expectation adds ~1 case. NOT load-bearing.
3. **FORWARD-MODEL / MEANING / GROUNDING** (cycles 24-28): context-free knowledge in ANY form (associative,
   directed ATOMIC, online-learned transitions) + grounding-as-similarity are all NOT load-bearing.
+ the converging solver (`generate_dont_retrieve_causal_edges...`): a coverage-maxed knowledge model discriminates
   at CHANCE (AUC 0.50). SAME wall.
**THE ROOT (proven, not asserted): a story-specific-knowledge GRANULARITY gap.** The mechanisms are right; generic
knowledge (any coverage) is satisfied by too many candidates to single out the cause; only where the story STATES
the specific knowledge (the goal slice) do we win. The remaining move is NOT another cell -- it is a knowledge
program.

## 🎯 KEY FUTURE WORK (the ceiling-mover -- focus next)
**THE LIFETIME-LEARNER / GROUNDED-KNOWLEDGE PROGRAM (Q111 / strategy).** The ~68% residual = story-specific,
fine-grained causal knowledge the brain acquires from a lifetime of grounded experience. It is NOT suppliable by a
static KB (refuted), grounding-as-similarity (refuted), a context-only world-model (32% ceiling), or a top-down
expectation over generic knowledge (bar #4 negative). The one unrefuted, brain-foundational, LLM-free lever
(`notes/research_causal_knowledge_acquisition_2026-09-09.md`): **directed causal knowledge is NOT an embodiment
blocker** -- CAUSAL-LINGUISTIC TESTIMONY (connectives / counterfactuals / generics = the author STATING the causal
link, Harris-Koenig; CausalNet/CausalBank precedent) sidesteps the covariation-identifiability failure. Concrete
program: an ONLINE per-story world-model built on a grounded lifetime learner that mines causal TESTIMONY at
corpus scale (extend `hdlab/causal_network.py` -> corpus-scale via `hdlab/kg_traversal.py`; richest untested piece
= COUNTERFACTUAL-TESTIMONY mining), consolidated through the meaning-channel / consolidation-gate north-star, and
applied per-story. This is a strategy-scale program, not a solver cell. SECONDARY: wire the converging solver's
owner-DONE world-model + counterfactual-necessity read live over `situation_reader.EventRecord` (Q111).
BANKED (proven, brain-foundational, reusable): the goal-slice result-state check; the structured bound-event
carrier (glass-box parse, spaCy removed); the intrinsic surprisal/necessity evaluation FRAME (trap-proof, replaces
the position-artifact benchmarks); the oracle-ceiling characterization method. DROP FOREVER on this task:
TellMeWhy/GLUCOSE (position-artifact crowd golds), spaCy at inference, any context-free/generic knowledge KB.
...
## QUESTIONS
[UPDATED 2026-09-09 -- supersedes the retracted subset-positive framing below.] One labelling call for the owner:
- I file this **SOLVED as a rigorous, TRIANGULATED LOCATED NEGATIVE** (the bar's explicitly-blessed full pass):
  the mechanisms are built + brain-foundational; the wall is fully characterized (story-specific-knowledge
  GRANULARITY) and confirmed across THREE independent solver-side levers + a converging solver; the exact residual
  (~68%, lifetime-grounded knowledge) and the next lever (the lifetime-learner / causal-testimony program) are
  named with numbers. See the FINAL VERDICT + KEY FUTURE WORK block at the TOP of this file.
- **HONEST BOUND (retraction owned):** there is NO capability win -- the earlier GOAL-subset positive was measured
  against a BROKEN position floor and is WITHDRAWN (the nearest-non-adjacent floor 0.68 dominates it); and
  TellMeWhy/GLUCOSE/spaCy are dropped as non-brain-foundational. What stands is the located negative + the banked
  brain-foundational mechanisms + the intrinsic evaluation frame. If the label requires a capability win, this is
  a LOCATED NEGATIVE, not a PARTIAL-with-a-win. Owner controls the verdict.

## NEXT STEPS (priority-ordered)
> **⚠️ SUPERSEDED (2026-09-09): the P1-P3 below predate the triangulation + retraction. THE key future work is the
> LIFETIME-LEARNER / GROUNDED-KNOWLEDGE + CAUSAL-TESTIMONY program -- see "KEY FUTURE WORK" in the FINAL VERDICT
> block at the TOP of this file. The P1 multi-step rollout below is a generic-knowledge lever now shown (oracle +
> bar #4) to hit the same granularity wall; do NOT pursue it as the completing lever.**
...
INTEGRATED_BY_STRATEGY: 2026-09-09 (CONT-29). Owner-DONE. A rigorous TRIANGULATED LOCATED NEGATIVE (the bar's blessed
full pass). DISPOSITION: the generative result-state world-model MECHANISMS are built + 100%-brain-foundational and
BANKED, and they OVERLAP the already-integrated causal world-model (`generate_dont_retrieve_causal_edges` ->
`hdlab.predictive_world_model` [online predictive-coding forward model + counterfactual necessity] + the additive
`sm.causal_antecedent`/`sm.predictive_necessity` read + the intrinsic-necessity board arm, committed 1d3d3bbf2 /
71f6e3c29). So there is NOTHING NEW to wire live from this problem -- its "secondary" future work (wire the converging
solver's world-model + necessity read live over EventRecord) is DONE. The WALL is a story-specific
causal-mechanism-knowledge GRANULARITY gap (proven: generic knowledge at 100% coverage ties at chance; the causal edge
is orthogonal to association; the intervenable WorldState.do() machinery is built but starved), NOT mechanism/coverage/
embodiment. Its own broken-position-floor goal 'win' RETRACTED. KEY FUTURE WORK routed to the NEW pri-1 brief
`grow_the_causal_mechanism_knowledge_foundation_by_mining_directed_causal_linguistic_testimony` (the lifetime
causal-knowledge learner -- mine directed causal-linguistic testimony, consolidate, apply per-story online). Audit S2b
+ ledger CONT-29. priority 1 dropped; review EXCELLENT.
