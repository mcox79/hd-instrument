...
priority:
review: EXCELLENT
review_text: "SOLVED (owner-DONE) integrated 2026-08-29 — bar MET, EXCELLENT. Reverified FIRST-HAND: test_state_register.py 61/61 PASS. (SETUP NOTE: the witness reads CACHED exp metrics, not scaffold-free; the committed live-coref metrics.json was a --no-real-baseline SMOKE run [baseline {skipped:true}, no n_targets], so the witness first errored KeyError:'n_targets' — I diagnosed it as a stale artifact, REGENERATED the full metrics by running exp_state_register_serves_live_coref_v1.py [real-LitBank baseline recomputed to 0.327 on 582 targets, matching the solver], then the witness passed 61/61. A setup issue, NOT a solution defect.) The missing Zwaan-Radvansky ENTITIES(state) dimension: a per-entity STATE-HISTORY register (sibling of the SPACE location_register) reading 'had been X'/copular/resultant-of-telic states into each entity's timeline over intervals, default-persisting, closing only on an explicit incompatible state; reuses the location_register interval bookkeeping; coref supplies the entity key. TRACKING (construction gold, isolates the mechanism): register 1.000 vs the strongest stateless floor 0.719, both info-free twins lose, EMPTY register 0.429=chance (not gameable), distance-robust (flat 1.0 at K=20 vs a windowed floor→0). SEMANTIC (ATL-hub): a guarded WordNet matcher 0.950 vs exact-string 0.350 (exact recovers 0% of synonym queries, guarded 92%; 'is X unwell?' matches stored 'ill'); the 3 research guards (privative / open-vs-closed scale / typed antonymy) are LOAD-BEARING (1.000 vs unguarded 0.714 on traps). SERVE — THE LIVE-ORGAN PAYOFF: the register improves the ACTUAL hdlab CorefReader on state-decisive same-gender pronouns from CHANCE (0.54) to 0.96 (resolution runs through the REAL coref code; the register only re-ranks its candidate pool by state-consistency; the shuffled-states twin collapses); the live coref genuinely resolves real-LitBank (baseline 0.327 on 582 targets) — an improvement to the REAL reader, not a constructed-task win. BRAIN-FOUNDATIONAL with TWO research corrections that killed wrong intuitions BEFORE building: (1) the perfect is a CANCELLABLE default, NOT entailed-closed → do NOT auto-close pluperfects (Dowty; the drill killed the first design); (2) REJECTED an aspect confidence-discount because Vos et al. 2025 found the perfect is the MORE reliable state cue (opposite the hypothesis). HONEST bounds (I concur): real-prose extraction coverage 0.331 / precision ~0.65, bound by spaCy on 19c syntax (the shared corpus-age parse cap — retired at aggregate by the archaic integration but real for extraction); the antonym-supersession CLOSURE channel has ~0 natural incidence in LitBank (proven on construction gold). Grade EXCELLENT (a genuinely-missing PINNED organ, full control battery, two research corrections, a semantic upgrade, AND a live-organ serve). hdlab landing QUEUED (Q111): promote the spaCy-free CORE (StateRegister + state_match + the state-track classes) → hdlab/state_register.py (sibling of location_register; is_in_state/state_at/had_been/occurrences_of), KEEP the extraction adapter (StateReader + extract_state_events, parser-dependent) experiment-side (the SPACE split); then wire into the ENTITIES stack (coref key + the TIME-skipped 'had been X' channel) + make the state-consistency re-rank a default coref candidate filter (serve proven against the live organ) — coupled live-reader wiring, part of the assembly. AUDIT §2b folded (new ENTITIES(state) organ). NEXT primed: a VerbNet result-state lexicon + a 'but now' cancel cue; evidentiality/reportative confidence ('was said to be')."
---

> ## ✅ SOLVER REVIEW — INTEGRATED 2026-08-29 (grade: EXCELLENT; SOLVED owner-DONE)
> **Verdict:** bar MET, an exemplary build. Reverified first-hand (`test_state_register.py` **61/61 PASS** — after
> regenerating the full metrics; the committed artifact was a `--no-real-baseline` smoke run, so the witness first errored
> on a missing key. I diagnosed it as a stale artifact, ran the full exp [real-LitBank baseline → 0.327 on 582 targets,
> matching the solver], and it passed. A setup issue, not a defect). The missing ENTITIES(state) dimension: a per-entity
> STATE-HISTORY register (sibling of the SPACE `location_register`) reading "had been X"/copular/resultant states into each
> entity's timeline over intervals, default-persisting.
> **Measured:** TRACKING **1.000 vs the strongest stateless floor 0.719** (twins lose; EMPTY register 0.429 = chance, not
> gameable; distance-robust); SEMANTIC **0.950 vs exact-string 0.350** (a guarded WordNet matcher — "is X unwell?" matches
> "ill"; the 3 research guards are load-bearing). **THE LIVE-ORGAN SERVE (the payoff):** the register improves the ACTUAL
> hdlab CorefReader on state-decisive same-gender pronouns from **chance 0.54 → 0.96** (resolution runs through the real
> coref code; the register only re-ranks its candidate pool; the shuffled-states twin collapses; the live coref genuinely
> resolves real-LitBank, baseline 0.327/582). This is an improvement to the *real reader*, not a constructed-task win.
> **Rigor:** two research drills that killed wrong intuitions BEFORE building — the perfect is a CANCELLABLE default so
> pluperfects are NOT auto-closed (Dowty); an aspect confidence-discount was REJECTED because the perfect is the MORE
> reliable cue (Vos et al. 2025, opposite the hypothesis). Honest bounds owned (extraction coverage 0.331 / precision ~0.65,
> capped by the parser on 19c syntax; the closure channel has ~0 natural incidence). **Grade EXCELLENT.**
> **Landing QUEUED (Q111):** promote the spaCy-free CORE (`StateRegister` + `state_match` + state-track classes) →
> `hdlab/state_register.py` (sibling of `location_register`), keep the parser-dependent extraction (`StateReader`/
> `extract_state_events`) experiment-side (the SPACE split); then wire it into the ENTITIES stack (coref key + the
> TIME-skipped "had been X" channel) + make the state-consistency re-rank a default coref filter — coupled reader wiring,
> part of the assembly. **Audit** §2b folded (new ENTITIES(state) organ). **Next primed:** a VerbNet result-state lexicon +
> a "but now" cancel cue; reportative confidence ("was said to be").
