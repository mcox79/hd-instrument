...
| **RECURSIVE 2nd-order ToM** (the #3 perf-vs-brain gap: ToM was 1st-order only) | `exp_theory_of_mind_recursive_v1.py` | nested belief metarepresentation, presence-gated (Leslie 1987; Perner & Wimmer 1985 ice-cream-van) over FHRR | **BF_UNPINNED** (no pinned neural equation; labelled honestly) | 2nd-order false-belief items: ORDER2 1.00 vs the 1st-order ceiling 0.00 (+1.00 CI-sep) vs reality 0.00; twin 0.26 (+0.745 CI-sep); consistency control agrees |
| **INTEGRATION capstone** (the #4 perf-vs-brain gap: nothing wired the pieces together) | `exp_integrated_reward_cluster_agent_v1.py` | goal-directed control loop composing successor-feature value + model-based planning (Tolman cognitive map; Daw 2011; Stachenfeld 2017) | **BF** | one acting loop: the composed agent beats the flat model-free gate +1.7 CI-sep and the no-replan ablation +0.4 CI-sep in navigated reward (SF-in-navigation margin small, honestly reported -- decisive per-organ proofs are the component cells) |
| **THE LANDING, investigated on REAL data** -- SF over the grounded ATL basis (Lancaster+Warriner+Brysbaert) + modern GUM | `exp_grounded_successor_features_landing_v1.py` + `verification/test_grounded_successor_features_landing.py` (5/5) | successor features over grounded meaning (Stachenfeld/Barreto); the promotion-ready `GroundedSuccessorFeatures` organ | **BF** (organ) / **LOCATED NEGATIVE** (this substrate) | grounded features carry info (SF beats the info-free twin +0.30 CI-sep) but do NOT beat the tabular constant on held-out words (-0.42); a word's grounded MEANING does not predict its co-occurrence successors better than the global average (0.50 vs 0.81) -- fast-mixing word dynamics wash out the successor structure. Holds for co-occurrence AND a WordNet semantic-map. **Corrected landing: wire SF over the situation/task graph, NOT the lexical stream.** |
