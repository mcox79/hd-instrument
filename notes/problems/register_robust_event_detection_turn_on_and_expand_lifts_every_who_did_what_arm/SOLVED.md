...
- **Parser fidelity (OOD 19c tagging + attachment) -- and this is where the THREE residuals of THIS problem CONVERGE.**
  I probed each to ground, and all three point at ONE lever: (a) the copula AGENT slice fails on OOD nsubj
  holder-attachment; (b) the open-class 19c recovery caps at 0.56 vs ~1.0 on the OOD tagger; (c) the density-robust
  causal fix fails THREE ways because structural cause-selection inherits the OOD parse error. So the
  register-robust JOINT-DECODED tagger-parser (the filed CRF problem) is not one of several upstream options -- it is
  the SINGLE highest-leverage lever this whole problem points at, with three separate payoffs.
So: no further optimization is stranded HERE; every residual has been probed to ground, and the upstream levers are
named, evaluated, and (2 of 3) already filed -- with the register-robust parser identified as the convergent one.
