...
result: "The turn-on rises on BOTH who-did-what arms CI-separated, twin losing, NO dimension regressing -- via TWO brain-faithful levers (16 board LitBank docs, doc-level paired bootstrap nboot=2000). RECOMMENDED CONFIG = SCOPED-predicate_recall + copula-readout: agent-arm 0.7104->0.7333 = +0.0230 CI[+0.0103,+0.0392] half=0.0145; patient-arm 0.2265->0.2854 = +0.0589 CI[+0.0491,+0.0698] half=0.0104; both CI-separated. Lever A = predicate_recall (P6 open-class noisy-channel verb recovery, already wired default-OFF): agent-arm +0.0082 CI[+0.0047,+0.0121], patient-arm +0.0084 CI[+0.0046,+0.0137], agent/open +0.0098, patient/open +0.0108, all CI-sep; it BEATS the random-verbhood twin CI-sep on the patient arm. Lever B = COPULA silo-unification (the EXPAND half; a sort-aware READOUT, NO new firing): a copula-gov who-did-what query reads the HOLDER (agent) / PROPERTY (patient) from the ALREADY-DETECTED sm.entity_states (default-ON) instead of only sm.events -- patient/be arm 0.0000->0.2590 = +0.2590 CI[+0.2059,+0.3192] over base AND +0.2518 CI[+0.1988,+0.3133] over the deranged-state twin (twin=0.0072); lever-B-alone whole-arm patient +0.0505 CI[+0.0402,+0.0613] CI-sep, agent +0.0148 CI[+0.0000,+0.0327] (touches 0). agent/be +0.0971 CI[+0.0000,+0.2086] POSITIVE-not-sep (the board's degenerate 'Who did be?' under-specifies which state; the state dim's OWN instrument scores the binding 0.677R/0.872P). The blanket predicate_recall REGRESSES the causal dim -0.0594 CI[-0.1122,-0.0177] on the FIXED-question-set instrument (its connective cause-SELECTION heuristic is density-brittle); SCOPING recall out of the causal candidate set makes causal byte-identical to OFF (+0.0000) while RETAINING the arm gain (agent +0.0082 / patient +0.0084 CI-sep) -- so the turn-on lands causal-neutral. coref byte-identical (0.5149), temporal byte-identical on the fixed qset (0.8358). FP ~1.0 extra events/sentence."
...
controls: "(1) INFO-FREE TWIN x2, one per lever: random-verbhood promotion (count-matched) for predicate_recall LOSES CI-sep on the patient arm (+0.0054 open, +0.0042 whole-arm); deranged state<->sentence binding for the copula readout LOSES CI-sep (patient/be twin 0.0072 vs 0.2590, +0.2518). (2) NO-REGRESSION on every other dimension's RIGHT instrument, with the SELF-DERIVED dims measured on a FIXED question-set (built from OFF, answered by both) to isolate answer quality from question-set drift: coref (FIXED external LitBank gold) BYTE-IDENTICAL (0.5149==0.5149); temporal (fixed qset) BYTE-IDENTICAL (0.8358==0.8358); causal (fixed qset) -- the blanket turn-on REGRESSES -0.0594 CI[-0.1122,-0.0177] (a REAL CI-sep cost the naive self-derived read HID as a +0.0224 'gain' via question-set inflation), CAUSE LOCATED (connective cause-selection is density-brittle) and FIXED by scoping recall out of the causal candidate set -> byte-identical to OFF (+0.0000) with the arm gain retained. (3) ADDITIVE-BY-CONSTRUCTION: predicate_recall only ADDS events (ON strict superset of OFF, witnessed); the copula readout fires only on the copula slice when the event readout returns None (never overrides a dynamic answer; sm.events untouched -> lever B carries NO regression by construction). (4) CLASS PARTITION (open/be/have/do) exhaustive per arm -> headroom localized to copula 'be'; have/do already fire as main verbs (agent baselines 0.756/0.811). (5) FP BUDGET named: ~1.0 extra events/sentence at the P6 modern-calibrated threshold; the who-did-what readout matches on gov-verb LEMMA at the queried SENTENCE, so an FP event on an unrelated token almost never reaches an answer (the precision gate the bar asks for is the additive-only + lemma-and-sentence match)."
files_changed: "experiments/exp_event_detection_crossarm_v1.py (predicate_recall cross-arm x verb-class + coverage), experiments/exp_event_detection_crossarm_copula_v1.py (copula silo-unification, whole-arm + deranged-state twin), experiments/exp_event_detection_crossarm_full_v1.py (composite headline table + both twins), experiments/exp_event_detection_noregress_v1.py (other-dims no-regression, fixed-qset, FP budget), experiments/exp_event_detection_causal_scope_v1.py (the scoped-recall causal fix), verification/test_event_detection_crossarm_organ.py (scaffold-free witness), notes/problems/register_robust_event_detection_turn_on_and_expand_lifts_every_who_did_what_arm/{BRAIN_MECHANISM_DRILL.md, SOLVED.md}. NO hdlab/ file changed -- proposed diff below; strategy lands it (Q111)."
