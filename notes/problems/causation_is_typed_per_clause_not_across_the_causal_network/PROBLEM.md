...
priority:
review: STRONG
review_text: "SOLVED (owner-DONE) integrated 2026-08-30 — a RIGOROUS, three-fold-ENUMERATED NEGATIVE (a PASS) plus two non-circular positive brain-validations. Reverified FIRST-HAND: test_causal_network_edge_typer.py 15/15 PASS (scaffold-free — recomputes every headline from source, incl. the real-text negative W11b). The MECHANISM works on constructed gold: discourse cross-event edge typing (CAUSE/ENABLE/PREVENT/SEQUENTIAL) NET 1.000 [1.000,1.000] vs the connective/adjacency placeholder 0.271 (n=48 connective-neutral cross-SENTENCE passages, extraction given), isolated from single-clause typing (perclause 0.729), twins lose; a 2nd intentional front-end feeds the SAME Wolff typer (built-across 1.000 vs physical-only 0.300). BUT THE HONEST CAPABILITY TEST (the owner pushed for it): on REAL cross-sentence causation the typer 0.158 (3/19) does NOT beat a majority-CAUSE placeholder 0.842 (16/19) — a rigorous negative, enumerated with counts (the absence-claim-requires-enumeration discipline met): real cross-sentence non-CAUSE causation is (1) RARE (prevention/enabling packs WITHIN a clause = the single-clause typer's domain), (2) lexically UNCOVERED (the genuine ENABLE cases use open/unlock, absent from the FrameNet force lexicon; the typer abstains on 13/19), (3) MENTAL for the bulk (a different brain system). So the constructed 1.000 is a MECHANISM demonstration, NOT a real-text capability win. TWO NON-CIRCULAR POSITIVE BRAIN-VALIDATIONS survive: graded necessity reproduces Trabasso and van den Broek's causal-weight ordering (physical>...>enabling, rho 1.000 vs twin p95 0.771 — the discrete CAUSE/ENABLE/PREVENT read-out is a lossy projection of a graded necessity rep); and the force-configuration model predicts human causal-verb judgments (CICL causative-verbs, Cao et al. 2023: force-config predictions vs human proportion-yes r=0.948 vs shuffle-twin p95 0.350 — non-circular, the config is decoded from the stimulus labels and the target is INDEPENDENT human data). Corpus-age confound flagged honestly (LitBank/McGuffey are 19th-c./~200yr; no modern causal-annotated corpus on disk) → feeds the p1 corpus-migration. VERDICT: the negative CLOSES the discourse-level causal-network-typing route as a real-text lever (rare/uncovered/mental — do NOT build a discourse edge-typer expecting a real-text win) AND the two positives validate the already-landed force_dynamics_typer's brain-faithfulness against human data + Trabasso. Grade STRONG (exemplary enumerated negative + non-circular human validation; deflated from EXCELLENT only because the capability is null — the constructed 1.000 is not a real-text win). NO hdlab landing earned into real causal-network typing (measured net-zero, brain-faithfully). The graded-necessity read-out (rho 1.000) is a constructed mechanism demo, NOT landed; the human-validation confirms existing hdlab code with no new module. Audit §2b folded (discourse-level causal-network typing SUSPECTED-lever → MEASURED-NEGATIVE, three-fold enumerated; force model human-validated Cao 2023 r=0.948; graded necessity reproduces Trabasso). NEXT: the causal-network real-text lever is CLOSED; causation value now lives in the single-clause typer + the graded-necessity representation, and in modern-corpus revalidation (p1)."
---

> ## ✅ SOLVER REVIEW — INTEGRATED 2026-08-30 (grade: STRONG; SOLVED owner-DONE)
> **Verdict:** a rigorous, three-fold-enumerated NEGATIVE (a PASS) + two non-circular positive brain-validations.
> Reverified first-hand (`test_causal_network_edge_typer.py` **15/15 PASS**, scaffold-free — recomputes every headline incl.
> the real-text negative). The mechanism works on constructed gold (**NET 1.000 vs placeholder 0.271**, isolated from
> single-clause 0.729, twins lose) — but the **honest capability test the owner pushed for shows the typer 0.158 (3/19)
> does NOT beat majority-CAUSE 0.842 (16/19)** on real cross-sentence causation, because real cross-sentence non-CAUSE
> causation is **(1) rare, (2) lexically uncovered (abstains 13/19), (3) mental for the bulk** (enumerated with counts).
> Two non-circular positives survive: **graded necessity reproduces Trabasso's ordering (rho 1.000 vs twin 0.771)** and the
> **force model predicts human causal-verb ratings (Cao 2023, r=0.948 vs shuffle 0.350)**. Corpus-age flagged (→ p1).
> **No hdlab landing earned into real causal-network typing** (measured net-zero, brain-faithfully); the graded-necessity
> read-out is a constructed demo (not landed), and the human validation confirms the already-landed force typer.
> **Grade STRONG** (deflated from EXCELLENT only because the capability is null). Audit §2b folded; `priority:` cleared.
> **Route closed:** do not build a discourse edge-typer expecting a real-text win; causation value lives in the
> single-clause typer + the graded-necessity representation + modern-corpus revalidation (p1).
