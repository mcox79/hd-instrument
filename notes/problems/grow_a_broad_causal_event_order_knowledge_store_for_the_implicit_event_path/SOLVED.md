...
2. **HIGH -- BUILD OUT the COMPREHENSION-COUPLING ORGAN (prototyped: `exp_causal_order_comprehension_organ_v1`).**
   The 5-mechanism ensemble (both-narrated 0.62 + operator-coref 0.86 + coherence-causal-plausibility 0.59 + C-I settle
   coverage + store) reaches 0.5852 @ cov 0.862 (+0.0234 over the prior, beats the twin CI-sep) -- the architecture is
   PROVEN (prior + confident comprehension overrides, highest-precision first). It is NOT yet CI-separated over the
   prior at n=1924 (underpowered; the dominant `ci_settle` workhorse fires 990 @ 0.59 = aggregate-level). To EXCEED,
   the per-pair comprehension of the dominant implicit stratum must improve -- the deep situation-model reader (event
   coreference at scale over predicate-argument events + a generative causal simulation). Reuses landed organs
   (`coherence_reader`, `predicate_argument_frontend`, `causal_reasoner`). This IS the situation-model coupling below,
   now with a working ensemble scaffold to extend. NOT a
