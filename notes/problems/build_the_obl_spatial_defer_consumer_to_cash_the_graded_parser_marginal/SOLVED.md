...
**The confident-wrong core is not recoverable by any sentence-internal cue.** Referential is the RIGHT direction
(Stage-0: the low-noun's definiteness predicts gold LOW-attach at AUC 0.5685>0.5; best augmented AUC 0.782) -- but a
sentence-level proxy is far too weak to override a 0.99-confident wrong marginal. The research drill converges: these
are REFERENTIAL disambiguations ("watched the Americans IN the embassy" -> the *Americans* who are in the embassy;
"chanted slogans IN the city" -> the *chanting* happens there) resolved by the discourse referential context
(Altmann & Steedman 1988; Tanenhaus et al. 1995), which a sentence-level parser structurally lacks. **The broken link
is the ABSENT cross-sentence discourse coreference-candidate-count loop** -- the recurrent top-down situation->parse
loop the parser SOLVED already named as THE missing organ, now confirmed from the spatial side with three failed
sentence-internal cues.
