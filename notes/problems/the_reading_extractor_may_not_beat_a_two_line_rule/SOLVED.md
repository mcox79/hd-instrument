...
INTEGRATED_BY_STRATEGY: "2026-08-25 (owner-DONE re-integration) -- EXCELLENT; headline re-verified WITNESS PASS (ELABORATE_DOES_NOT_BEAT_TWO_LINE_REPLACE); elaborate reader loses to a two-line rule -> REPLACE (do not route patient selection through the perceptron; do not weight animacy for English). Deeper: two-line is NOT the ceiling -- filler-gap dorsal-parser recovers reversibles, cue-based retrieval reproduces human interference; frontier = a stronger parser + construction gate (a NEW problem to package). Precise-voice selector in situation_reader = PROVEN-READY deliberate landing. Only the headline witness re-run this round; deeper witnesses to re-verify before landing on them."
---
