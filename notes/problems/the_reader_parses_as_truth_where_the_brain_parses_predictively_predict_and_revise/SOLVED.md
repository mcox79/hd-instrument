...
## THE EXACT MECHANISM — now identified (owner's core challenge: "do we understand EXACTLY how the brain does this?")
Honest answer: we did NOT, and that WAS the problem — I was building a functional approximation. A deep research
drill (17 papers) pins the exact mechanism across Marr's three levels, and it explains BOTH failure signatures
(the cue plateau AND the homograph mis-tag):
- **COMPUTATIONAL:** who-did-what is JOINT GENERATIVE Bayesian inference — infer the most probable message
  (lexical category + syntactic structure + thematic roles TOGETHER) given noisy input, under a generative model
  = a valence/structural PRIOR (Jurafsky 1996; Levy 2008) × a selectional LIKELIHOOD P(filler|role,verb) (McRae
  1998) × a NOISE model over the string (Gibson/Bergen/Piantadosi 2013). **The Competition Model I built is
  explicitly an ALGORITHMIC-LEVEL APPROXIMATION of this joint posterior — and the field itself says "a
  linear/additive implementation is a weak approximation" of the Bayesian computation.** THAT is the plateau,
  named: a linear weighted cue-SUM cannot express the context-dependent, multiplicative cue interaction of a
  joint posterior.
- **ALGORITHMIC:** incremental, beam-limited MAP / analysis-by-synthesis over (category, structure, role) tuples
  jointly, word-by-word (Hale 2001 surprisal; Demberg & Keller PLTAG; MacDonald/Tanenhaus constraint-based).
  Resource-bounded → "good-enough" errors (Ferreira 2003) emerge as a bounded approximation, NOT a different
  computation (Q5: good-enough discriminates DEPTH, not generative-vs-discriminative).
- **IMPLEMENTATIONAL:** explicit structured ROLE-FILLER BINDING in left mid-STC — agent/patient in DISTINCT
  neural subspaces INDEPENDENT of syntactic position (Frankland & Greene 2015 = a variable binding, NOT a
  cue-sum), verb-specific proto-roles refined toward PFC (Frankland & Greene 2020); computationally a
  tensor-product / VSA binding (Smolensky 1990) — which the substrate ALREADY has (FHRR role-filler binding).
- **WHY MY PIPELINE FAILS THE WAY IT DOES (both, one cause):** category is resolved JOINTLY with structure in
  the brain (LIFG pre-activates noun/verb from structural context within 100ms, BEFORE the word's own features;
  additive category/semantic ERPs) — so "clay FORMS mud" is never mis-tagged (the predicate slot forbids a noun).
  My BOTTOM-UP tag→parse→assign commits category before structure → the ~71% homograph misses. And my LINEAR
  cue-sum is the weak approximation of the joint posterior → the role-assignment plateau. ONE root cause: I built
  the discriminative approximation, not the generative computation.
- **THE EXACT BUILD TO REPLICATE IT (research synthesis, glass-box, NO LLM):** a lexicalized GENERATIVE grammar
  with valence/argument slots (Jurafsky 1996), trained on a role-labeled treebank (PropBank/OntoNotes), decoded
  by an incremental left-corner / PLTAG-style parser maintaining a BEAM of joint (category, structure, role)
  hypotheses scored by prior(structure) × likelihood(fillers|structure) — with the selectional-fit model
  repurposed as the LIKELIHOOD term INSIDE the posterior (not an independent cue) + a noisy-channel correction
  layer (Gibson 2013). Category/attachment/role all fall out of ONE MAP derivation, never committed
  independently. This is a genuine FOUNDATION BUILD (a probabilistic generative parser) — a dedicated problem,
  the real answer to the reader's parse-as-truth thesis. FALSIFIER: build it at the SAME information content and
  if it still plateaus at 0.61 with the same error distribution, the ceiling is data not mechanism.
  DECISIVE CHEAP PRE-TEST (buildable now): hold the cues fixed and swap ONLY the combination rule (linear sum →
  reliability-weighted joint posterior). **RESULT: the joint-posterior is WORSE than the linear sum** (QA-SRL
  0.509→0.493, -0.0164 [-0.024,-0.009]; 19c 0.334→0.301, -0.0325 [-0.038,-0.027]). INTERPRETATION (honest, and
  it SHARPENS the finding, not refutes it): a valid joint posterior requires CALIBRATED generative likelihoods;
  my cues are miscalibrated HEURISTICS, so the multiplicative combination lets a wrong cue VETO the right
  candidate (a sum only nudges). **You CANNOT shortcut the generative model by reweighting heuristic cues — the
  negative proves it.** The joint-generative mechanism only pays off with a real generative grammar producing
  calibrated P(structure)·P(fillers|structure) — i.e. the FULL foundation build is required, it is not
  approximable by a combination-rule swap on the discriminative cues. So: the exact mechanism is IDENTIFIED
  (joint generative inference) and my approximation is DIAGNOSED (linear cue-sum of miscalibrated heuristics),
  but replicating it needs the probabilistic generative valence parser (a dedicated foundation problem), which
  this negative shows cannot be faked cheaply.
