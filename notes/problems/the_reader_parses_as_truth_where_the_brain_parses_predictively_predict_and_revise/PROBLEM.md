...
priority:
review: EXCELLENT
review_text: "Reverified 8/8 (source) + 4/4 (landing) first-hand. A predict-and-revise parse-RECALL pass that recovers the DROPPED patient the batch parse leaves empty -- CI-separated over the strongest real floor on BOTH modern QA-SRL (+0.0599) and 19c LitBank (+0.0592); gains localize to non-canonical constructions (passive 0.287->0.567, pre-verbal-gap 0.023->0.511) with canonical recall PROTECTED. Full control battery: random-loci twin LOSES CI-sep, uniform-prior twin loses, revise-everywhere loses. DECISIVE drill: the entire gain is a STRUCTURAL DROP-FILL (dropped-only, NO surprisal gate, +0.0625 over wired and TIES the surprisal-gated pass) -> confirms p2's refuted re-selection from the recall side AND means the wire needs NO fitted asset. REUSES the validated relcl_resolver as the drop-fill target (owner-prompted enumeration; more brain-faithful than a position heuristic). LANDED (Q111): default-off predict_revise flag on SituationReader -- a post-read pass fills '?' patients via relcl_resolver + a nearest-nominal position fallback, preserving the original on patient_prerevise; witness test_predict_revise_recall_landing_organ.py 4/4 (default-off byte-identical on 219 events + recall protected [107 canonical unchanged, 0 wrongly changed] + 111/111 fills BYTE-EXACT to the validated drill). Registered predict_revise_live_reader_v1. Follow-on packaged: the verb-role exemplar store (the WHICH-argument lever)."
---
