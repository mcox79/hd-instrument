...
priority:
review: EXCELLENT
review_text: "Reverified 15/15 first-hand (the SOLVED.md's 13/13 was a stale doc count). A RIGOROUS LOCATED NEGATIVE (a full PASS per the bar): reasoning over the reader's OWN live situation model on a real modern inference benchmark (MCScript2 before/after, n=1128). The reasoning MECHANISM works and is brain-faithful (the reused transitive_ordering read-out beats a shuffled-timeline twin CI-sep +0.036 [0.0044,0.0671]; recovers true order under narrative reordering 0.737 vs 0.263), but does NOT beat the surface floors. The VALUE is the localization, discriminated by a chain of controlled experiments each REFUTING a cheaper hypothesis: retrieval rule-out (coverage 0.55->0.69, accuracy flat) killed 'my matcher is the wall'; the precision/SR aggregator ablation PLATEAUS at chance (0.505/0.514/0.523) killing 'the crude tally'; the 90-98%-in-passage partition killed 'commonsense not in text'; and -- exemplary -- the oracle clean-extraction LOOP-CLOSER REFUTED the solver's OWN prior conclusion 'extraction is the binding wall' (clean spaCy raised coverage 0.36->0.74 but accuracy stayed at chance). The dominant residual is SEMANTIC cross-narrative event ALIGNMENT = the brain's DG/CA3 PATTERN-SEPARATION function; composing content_addressable_retrieval's graded partial-cue with GROUNDED codes lifts the learned script-order to a NEAR-positive (0.593, beats its shuffled twin CI-sep +0.111 [0.019,0.200]; symbol codes only tie at 0.48). The fix is EMPIRICALLY DE-RISKED by a gold-free headroom drill (98% of confusable pairs separable by particle/arg/prep; 51% of Qs hinge on a particle the current verb+object code ignores) -> projected 0.59->0.70-0.80, NOT human 0.97 (~10-20% irreducible). REUSE-not-reinvent (transitive_ordering is the brain's ONE general cognitive-map organ, PINNED; Behrens 2018). Independently corroborated by the Aug-2026 script_grain_acquisition_loop MCScript2 HARD_FAIL (same signature). One process miss the solver flagged themselves: re-derived that known negative by not checking experiment_index first (the convergence became a strength). AUDIT UPDATE folded into sec 2b. LANDING: no load-bearing hdlab wire (capability did not clear the floor); the de-risked FIX is PACKAGED as the follow-on the_reader_conflates_similar_events_needs_a_soft_and_conjunctive_grounded_aligner (priority 4). Two optional low-cost substrate fixes OWED (persist frame_induction to disk; key timeline_register by (lemma,sent_idx))."
---
