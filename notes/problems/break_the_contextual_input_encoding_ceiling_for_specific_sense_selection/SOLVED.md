...
2. **The redirect, now mechanism-level:** the lever is the QUALITY of the world-knowledge connection matrix `W`
   -- dense + graded + clean + SENSE-DISCRIMINATIVE. This IS the priority-1 sibling `build_the_controlled_
   knowledge_growth_consolidation_gate_for_the_learner`, and this problem hands it a sharpened target: the
   consolidation gate must build a `W` that is (a) SENSE-DISCRIMINATIVE (topic-relatedness reinforces the wrong
   sense -- measured: C-I over a topic `W` = 0.22 < 0.32), (b) CLEAN (ConceptNet-noise regresses -0.004), and
   (c) DENSE (a_s scales with `W` density). The consumer is the QUERY side (100% of the loss), and the readout
   is joint C-I settling over `W` (no pre-filter), not classify-then-weight.
