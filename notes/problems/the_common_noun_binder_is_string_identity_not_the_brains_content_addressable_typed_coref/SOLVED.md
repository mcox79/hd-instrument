...
5. **ADD the conceptual-meaning distributional bridge cue (right-organ tie-in, 2026-09-10).** In the NON-WRITING bridge,
   license a candidate additionally when `hdlab.conceptual_meaning.ConceptualChannel.similarity(anaphor_head, cand_head)
   >= 0.40` (IDF-weighted WordNet-gloss feature cosine -- a DISTRIBUTIONAL/definitional type route that catches
   different-head bridges the is-a C5 + DBpedia C8 routes miss). Measured: 0.5580 -> 0.5622 (+0.0042 CI[+0.0021,+0.0066]
   CI-sep over the C5+C8 baseline; +0.0368 CI-sep over the de-leaked floor; twin loses). BF_SPIRIT (validated vs GloVe-300);
   tau=0.40 SWEPT not adopted. SAFE because it is the NON-WRITING bridge (a graded cue is safe here; the prior located
   negative that "graded HURTS" was the WRITING identity gate). Drop-in ref: `exp_cn_conceptual_bridge_v1.py`
   (`resolve_param(..., extra_bridge=...)`). Small but real -- the right BF organ, tied into the right consumer.
6. **ADD the SITUATION-MODEL FOCUS bridge -- the DOMINANT-loss fix (2026-09-10, biggest single coref gain).** The audit
   (`exp_cn_diffhead_route_audit_v1.py`) proved the dominant loss (the different-head slice, 31.5% of mentions) is 75%
   NOT type-linkable, and ~80% of that residual is DISCOURSE-IDENTITY (same entity re-described by a non-taxonomic common
   noun: "the study"=="the project") -- which is-a/DBpedia/lexical-similarity all miss, and which the event world-model
   (wrong grain) and a type KB (refuted, <25% coverage) do NOT address. The BF fix is a Kintsch situation-model FOCUS
   bridge: for a DEFINITE common noun with no same-head and no type-license antecedent, resolve (NON-WRITING) to the
   most-salient (ACT-R) prior entity of a COMPATIBLE COARSE ONTOLOGICAL CLASS (WordNet supersense group
   ABSTRACT/OBJECT/PERSON/GROUP/LOCATION/... -- Rosch basic-level ontology). Kintsch construction-integration default-to-
   focus + Centering Cb/Cf + Lewis-Vasishth ACT-R + coarse-class coherence; all BF_SPIRIT; NON-WRITING (Nieuwland) so a
   graded default is safe. MEASURED (n=2855): baseline 0.5622 -> **0.5776 (+0.0154 CI[+0.0094,+0.0225] CI-sep)**; vs the
   de-leaked floor **+0.0522 CI-sep**; info-free twin LOSES **+0.0413 CI-sep** (coarse-class signal load-bearing);
   different-head slice **0.1346 -> 0.1869** (+47 correct -- the discourse-identity recovery); same-head no-regress (-3).
   Drop-in ref: `exp_cn_discourse_focus_v1.py` (`resolve_param(..., focus_bridge=True)` + `coarse_class`). This is the
   FIRST lever to move the dominant loss, and it is a DIFFERENT organ from both refuted attempts (entity-in-focus +
   coarse-class coherence, NOT is-a type and NOT event prediction). CUMULATIVE binder: 0.5482 (deployed) -> 0.5580
   ...
   **8. ADD the HEIM/LOEBNER UNIQUENESS bridge -- the one LOAD-BEARING piece of the two proposed for the discourse-identity
   residual (2026-09-10, `exp_cn_bridge_ablation_v1.py`; witness `verification/test_cn_uniqueness_bridge.py` 5/5;
   BF_CHAIN_CONFIRMATION C10).** For a definite common noun with NO same-head and NO type/focus license, if exactly ONE
   gn-compatible referent is in the IMMEDIATE focus window (=6 mentions), bind to it NON-WRITING -- definiteness alone
   (Heim familiarity + Loebner pragmatic-definite uniqueness) forces the identification, no knowledge needed. MEASURED
   (n=2855): **0.5776 -> 0.5818 (+0.0042 CI[+0.0018,+0.0069] CI-sep)**; info-free twin LOSES **+0.0392 CI-sep**; different-head
   slice **0.1869 -> 0.1991 (+11)**; same-head no-regress; window sweep 6/10/16/24/40/60 all CI-sep (tighter=better). Drop-in:
   `resolve_param(..., uniqueness_bridge=True, uniq_window=6)`. This is a SEPARATE, precise organ from item 6 (fires only on
   the sole-in-focus residual the focus bridge's coarse-class license misses). **LOAD-BEARING VERDICT (of the four pieces
   built): uniqueness WINS; coherence/parallelism HURTS (salience beats parallelism); learned near-identity carries a REAL
   signal (beats its scramble twin +0.041) but is NOT load-bearing (750 train pairs too sparse -> false-bridges cancel; the
   information limit -> the LEARNER/knowledge-growth program); dependency-context substitutability is INERT (subsumed by the
   focus bridge).** Cumulative binder 0.5482 -> 0.5580 -> 0.5622 -> 0.5776 -> **0.5818**; de-leaked-floor margin +0.0564 CI-sep.
