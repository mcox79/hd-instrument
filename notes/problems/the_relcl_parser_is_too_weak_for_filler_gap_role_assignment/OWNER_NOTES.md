...
- **UPGRADE the architecture from an if/else GATE to two PARALLEL COMPETING routes + a conflict term (see
  the architecture-fidelity audit below).** Run the two-line heuristic and the filler-gap resolver ALWAYS,
  in parallel; their DISAGREEMENT is a gold-free difficulty/confidence readout (the semantic-P600 analog).
  Measured: same accuracy as the gate, plus a per-item conflict signal that predicts where the heuristic
  route errs (0.746 vs 0.201 on real QA-SRL). The brain does not switch routes; it runs both and monitors
  their conflict.
...
And auditing the machinery AROUND it: the individual parts are fine, but the assembly line wiring (react
instead of predict, stage instead of interleave, switch instead of run-both-and-watch-for-conflict) is not
how the brain reads -- I fixed the switch (run both routes, use their disagreement as a free confidence
signal) and flagged "make the reader predict ahead" as the biggest next win.
...
2. Land it as two PARALLEL competing routes + a conflict term (not an if/else gate): same accuracy, plus a
   gold-free per-item difficulty/confidence signal (the semantic-P600 analog) that predicts where the fast
   heuristic errs. The brain runs both routes and monitors their conflict.
3. Redirect stage-1 role-assignment effort AWAY from strengthening the general dependency parser (measured
   harmful here) and toward (a) the construction gate on natural text and (b) stage 2 (meaning), per the
   plan's Phase-1 ordering.
4. For nested / center-embedded relatives, wire the p3 content-addressable retrieval mechanism to bind the
...
5. Treat filler-gap binding and the p3 store as ONE retrieval primitive (the audit's E1/E2/E3 unification,
...
6. FLAG two whole-reader opportunities as NEW problems: (a) make the reader PREDICTIVE (pre-activation +
   surprisal -- the biggest architecture-fidelity gain); (b) soften the exhaustive parse into good-enough /
   noisy-channel partial structure. Both are beyond this brief and touch every stage.
7. Orthogonally, extend `_is_participle` to irregular past participles to lift the reversible-passive
