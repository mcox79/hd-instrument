...
**NOT implemented (named honestly, needs research not a quick build):** a PRECISION-WEIGHTED ENSEMBLE of the two
forward predictors (learned GEK for in-domain/short + persistence gist for real prose) is the theoretically-right
unification (Friston reliability-weighted cue integration), but setting the per-stream precision weights WITHOUT
labels is unsolved -- the GEK is confidently-WRONG on OOD prose (high self-confidence != accuracy), so self-entropy
precision does not track the regime. The brain solves this with online prediction-outcome CALIBRATION (invariant-
compliant online learning) -- a real follow-on, not a one-cell change. I did not build a version that would only
pretend to work.
