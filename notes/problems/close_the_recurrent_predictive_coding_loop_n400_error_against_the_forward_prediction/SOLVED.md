...
- **DEFINITIVE CONCLUSION (pinned rigorously by FIVE glass-box levers -- 4 SEM variants + the diverse KL distribution,
  all TESTED):** the glass-box brain-foundational ARCHITECTURE (SEM schema-switch) beats the incumbent vs ACTUAL
  HUMANS (0.62 vs 0.53, +0.10-0.12, robust) -- that is what LANDS NOW. But human-LEVEL segmentation (~0.9) requires a
  NEURAL forward model (GPT-2-scale), and NO glass-box forward model -- linear/episodic dynamics, structured scenes, or
  a diverse-corpus co-occurrence distribution -- crosses that wall (tested, not assumed). This is the substrate's
  FUNDAMENTAL glass-box-vs-learned-model tension, now located precisely + quantified at event segmentation: the
  architecture is free (glass-box), the forward-model richness is the ceiling. Whether to admit a neural forward model
  (an OFFLINE-frozen learned LM is invariant-compliant at INFERENCE but strains the "no long training runs / glass-box"
  philosophy) is a STRATEGIC OWNER decision, not a solver one -- see QUESTIONS.
...
## QUESTIONS
- **THE STRATEGIC INVARIANT DECISION (§4l, the frontier is now a decision, not a build).** I tested FIVE glass-box
  levers and pinned the ceiling by test: the SEM schema-switch ARCHITECTURE beats the incumbent vs actual humans
  (0.62 vs 0.53) and lands now, but NO glass-box forward model reaches human-level (~0.9) -- that needs a NEURAL
  forward model (Kumar's GPT-2). An OFFLINE-frozen learned LM is invariant-compliant at INFERENCE (no LLM call at
  runtime) but strains the substrate's "no long training runs / glass-box" philosophy. **DO WE admit an offline-frozen
  neural forward model to reach human-level event segmentation, or accept the glass-box architecture win (0.62) as the
  ceiling?** This is an OWNER/strategy call about the core invariant -- not a solver decision, and not resolvable by
  more solver experiments (I have exhausted the glass-box space). MY RECOMMENDATION: land the SEM architecture now
  (real, validated, glass-box); file the neural-forward-model question as a separate strategic problem -- it is the
  same glass-box-vs-learned-model ceiling the substrate has hit at meaning/WSD, now quantified at segmentation.
