...
priority:
review: EXCELLENT
review_text: "Reverified 10/10 first-hand. The ASSEMBLY completion: the assembled reader was N PARALLEL SILOS (each dimension stored the MARGINALS, nothing stored the JOINT -- the BINDING PROBLEM). The solver ASSEMBLED three already-built-but-unwired organs (multibank + n400 + hippocampal) + the FHRR bind into a tiered bound-event-token backbone and proved it stores the JOINT the silos cannot, on REAL event structure, BOTH genres: JOINT coref 1.000 CI-ABOVE late-fusion-of-marginals 0.600 (sep +0.400) on LitBank old fiction AND UD-EWT modern web; binding-shuffle collapses it (pos-recognition 1.000->0.12 -- the conjunctive-memory dissociation, Konkel & Cohen 2009); info-free twin null; MUST-CHUNK fires (flat bundle 1.0->0.40 @ M=256 while multibank + DG/CA3 hold); cued retrieval 1.00 (bound token) vs 0.01 (silo, chance 1/M). NECESSITY (beyond the bar): under PARAPHRASE grounded distributed binding 0.379 BEATS symbolic-exact 0.217 (chance) -- grounded binding is NECESSARY for graded matching, not just sufficient. Walls drilled to root cause: the ecological bottleneck is FRONT-END role assignment (agent-role 0.271 while event recall 0.953 -> the parser is the lever, NOT the codec); the N400 chunk tier is necessary but its content boundary doesn't beat fixed-size on retrieval; the CA3 completion path is low-fidelity (DG-at-retrieval; EC->CA3-direct is the fix, completes 1.00); the fusion wall is corpus-size-bound. Honest bounds all stated by the solver (a representational ceiling 1.000, not a downstream comprehension score; constructed recombination/paraphrase probes not annotated coref; slot_attention unused). WIRE LANDED (Q111, a8db1b679): a default-off bind_event_tokens flag on SituationReader builds sm.event_tokens + sm.episodic_store via a NEW thin assembler hdlab/bound_event_backbone.py that COMPOSES existing organs only, promoted VERBATIM so a wired token is torch-EQUAL to the validated cell's; witness test_bound_event_backbone_landing_organ.py 5/5 (default-off byte-identical + all 219 tokens byte-exact + resolve==joint_decide 30/30 + tiered store assembled). Registered bound_event_token_backbone_live_reader_v1. This is the step from the reader HAVING features to UNDERSTANDING which goes with which -- the prerequisite for reasoning (p6). Flip-on default-off/owner-gated."
---
