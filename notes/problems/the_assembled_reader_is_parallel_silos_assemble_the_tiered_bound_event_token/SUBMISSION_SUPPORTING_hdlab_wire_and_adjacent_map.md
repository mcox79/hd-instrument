...
| `hippocampal_encoder` (DG+CA3) | episodic store + pattern completion | Marr 1971 CA3; CLS (McClelland 1995) | **HIGH** — DG pattern-separation keeps the tiered store flat where the flat FHRR bundle collapses | prior DG HARD_FAILs were on the WRONG tier (active-read), CONFIRMED here: on the EPISODIC store DG/CA3 holds retrieval flat to M=256 (the brief's re-scope is correct) |
