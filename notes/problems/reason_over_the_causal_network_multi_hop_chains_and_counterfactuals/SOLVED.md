...
- **U7 -- THE c-LEVER TESTED WITH A REAL DIRECTED CAUSAL KB (the identifiability wall, MEASURED to a number).** Rather
  than defer the c-lever, I wired the on-disk CSKG directed causal knowledge (86k /r/Causes, /r/HasPrerequisite,
  /r/CausesDesire, /r/HasSubevent, /r/MotivatedByGoal edges -- a static KB, glass-box, NO LLM at inference; REUSE not
  rebuild) as the densification's plausibility gate and measured it on TellMeWhy (`exp_causal_reasoner_cskg_v1.py`,
  witness K1-K3). Result: **CSKG covers only 17.3% of the TRUE narrative cause-pairs**, and the CSKG-gated densification
  does NOT raise c over the topical baseline (cskg_backoff 0.211 vs dense 0.234, NOT CI-sep). WHY (the drilled wall):
  narrative causation is CONTEXTUAL and SPECIFIC ("danced terribly -> broke a toe"; "enjoyed NLP -> became a CS") --
  a generic causal KB has the generic links (ring->sound) but not the contextual ones, and where it fires it is
