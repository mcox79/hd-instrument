...
* **`common_noun_coref` under the is-a change** — see §6; the row needs the reader's own role-mention snapshot,
  which the full-size rows arm did not install.
...
**WHAT SHIPS AND WHY.** The diff routes the copula half to the predicate slot and **retires the apposition
half**. Keeping it would mean keeping a frozen supervised perceptron on the live path for a cue whose own
precision against the gold it was trained on is 0.26 — and the owner's rule is *fix to brain-foundational or
REMOVE, never keep for a metric.* A BF replacement is filed as a lead (§9), not left running on the perceptron.
...
1. **Build the object-control frame in `verb_subcat_frames`** (verb + OBJECT + infinitival complement counts
   from UD-EWT train). *Why:* the purpose arm's only shortfall against the perceptron is the `objonly`
   configuration; the lexicalist frame is the brain's mechanism and it currently cannot see that frame.
   *Expected:* closes the -0.0235 parity gap and removes the last "the perceptron knew something we don't".
...
4. **Re-seed the is-a bridge from `definitional_extraction` + the meaning hub** instead of a syntactic
   apposition scan (§6, §9.3), and score it on `common_noun_coref`, not on UD `appos` recall.
