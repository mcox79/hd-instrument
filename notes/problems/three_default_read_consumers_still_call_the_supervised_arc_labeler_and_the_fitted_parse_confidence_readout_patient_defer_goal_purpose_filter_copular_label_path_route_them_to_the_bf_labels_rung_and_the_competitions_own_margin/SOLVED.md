...
reverify: ".venv/Scripts/python.exe experiments/exp_labels_rung_to_live_consumers_v1.py --self-test   # 17/17, including a LIVE check that the defect reproduces (a NOT_BF module is imported during a default read), that the probe names more than the brief's three consumers, that the BF patient confidence ranks the object above the subject, that the predicate slot finds the copular predicate with no labeler, and that observing outcomes MOVES the purpose arm (online plasticity).  THEN: .venv/Scripts/python.exe verification/test_no_not_bf_organ_on_the_default_read.py --docs 16   # the probe witness (W0 proves it can fire).  The measurement arms, each writing only to its own directory: --patient --state (UD-EWT test, ~4 min), --goal --isa (~4 min), --probe --identity (~12 min), --rows --gum-docs 16 --ud-cap 400 (~40 min, both arms one process); --goal --rows --gum-docs 8 --ud-cap 0 (~17 min, THREE arms one process -- this is the is-a load-bearing test).  On disk: metrics.json (probe/identity/patient/goal/state/isa/rows), metrics_patient.json (the corrected PAIRED statistic), metrics_isa_goal.json (the stronger apposition schema), metrics_gumrows.json (the three-arm common-noun row)."
---
...
* **The purpose/complement rung is at PARITY, not above** (0.6589 vs 0.6822, CI [-0.1145, +0.0630]). The
  missing computation is named: `verb_subcat_frames` stores only P(infinitival complement) per verb, not the
  **object-control frame** (*verb + OBJECT + infinitival complement*), which is precisely the `objonly`
  configuration where the perceptron wins. That is a build, not a mystery.
...
infinitive attaches to the VERB or to the NOUN in the PP** — which is the heads rung's job, and the arm's head
category is already a cue in the table (`headcat`). That it still scores 0.5000 says the arm mis-attaches these
infinitives. An ORACLE-HEADS arm (the same competition with GOLD heads feeding `headcat`/`headismv`) is the
probe that would confirm it; it was still running at submission. **Filed as the first next step, with the
number attached.**

1. **Feed the purpose arm a correct attachment for the PP-interrupted infinitive** (and re-run the oracle-heads
   probe to bound it). *Why:* the arm's entire deficit vs the frozen perceptron is 32 items in one
   configuration, and the governor swap is already refuted as the cause. *Also build:* the **object-control
   frame** in `verb_subcat_frames` (verb + OBJECT + infinitival complement counts from UD-EWT train) — the arm
   already WINS `objonly` (0.6944 vs 0.6667) with only a scalar P(complement), so a two-slot frame should widen
   that.
