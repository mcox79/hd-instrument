...
INTEGRATED_BY_STRATEGY 2026-09-15 09:22 local -- DONE by strategy after first-hand reverify: the six-file diff applied AND the sentence source's DEFAULT flipped to cased (lower=False; no default-off, one convention -- the 19 bare verification callers now mirror the live reader). Witnesses green with the cased source: cell self-test, lexical_categories 7/7, cache_shared 4/4, predicate_recall 30/30, copular 6/6, causation_typed, double_parse 14/14, coref_graded_pick 7/7 (repeatability 12/12 holds under case), affected_entity PASS, referent_per_np 9/9, state_qa 9/9, tense_agnostic, precwt 6/6, role_route 4/4. FULL BOARD A/B (pri113a -> pri116a): all seven rows BYTE-IDENTICAL, AGG 0.6205 -- by construction (the rows never run the reader; the agent's finding), so the gain is gauged on the reader's own instruments: PROPN F1 through the reader 0.49 -> 0.85, entity files 2,459 -> 2,563, name-typed mentions 242 -> 599, patient +0.0215 sep. Follow-ons: the board rows rebuilt on the reader (to file), 'hand down a code, not a string' (to file).
