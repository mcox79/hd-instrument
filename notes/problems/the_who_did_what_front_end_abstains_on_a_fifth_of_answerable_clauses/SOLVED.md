...
- **CLEAN WIRE LANDED (§0g item 2, default-off, witnessed 11/11, commit `6ad17ed95`): the STRUCTURAL-DO candidate filter** — `hdlab/structural_do.py` (`is_bare_do` + frozen `CLEAN_PREPS`) + a `structural_do_recover` flag on `SituationReader` (in `CAPABILITY_FLAGS`). `_read_events_wired` records whether the bound patient is a BARE post-verbal DO (`EventRecord.patient_is_bare_do`); the `verb_subcat` gate OVERRIDES its veto for a bare-DO patient. Default-off byte-identical; requires `role_route='wired'` + `verb_subcat_gate` (so flipping it affects ONLY wired-route readers — the positional default is untouched). Witness `test_structural_do_recover_landing_organ.py` (default-off byte-identical; flag-on recovers ride→horse; oblique `sit on bench` NOT recovered).
  - **TURN-ON MEASURED (no-default-off), on the CURRENT reader, 19c LitBank who-did-what (n=382): effective (abstention=wrong) OFF 0.2723 → ON 0.3115 = +0.0393 — but recovery PRECISION is only 0.385 (39 recovered, 15 correct / 24 not-matching-gold).** The +0.039 is net-positive ONLY because on the abstention=wrong metric a wrong recovery is NEUTRAL (both abstention and wrong-patient count wrong); the LOW precision means most recovered patients don't match gold. That 0.385 is CONFOUNDED by the ~76% oblique-contaminated 19c gold (a "wrong" recovery may be the reader picking the true DO while gold_head is an oblique) → it is a LOWER BOUND, unclear. **DECISION: KEEP DEFAULT-OFF (measured reason): net-positive on isolated who-did-what but the recovery precision is low/confounded on the contaminated gold, so flipping it default-on would feed low-confidence patients to downstream consumers (world_state/causation/hd_fact_store) where a wrong patient asserts a FALSE fact — an unmeasured harm. A clean turn-on needs (a) a CLEAN 19c gold to measure true recovery precision + an info-free (random-recovery) twin LOSING, and (b) a downstream-consumer regression check. Landed + available; flip gated on those two.** Ref impl `exp_whodidwhat_composed_pipeline_v1.composed_who_did_what`.
...
## 0k. THE JOINT-DECODED PARSER — BUILT + trainable, remaining challenge precisely located (cell: `..._joint_decoded_parser_v1`)
I built the impactful thing rather than only spec'ing it: a minimal JOINT-DECODED arc-eager parser on the substrate's
own operator — POS is a scored `SHIFT_V` action, trained jointly with the dynamic-oracle averaged perceptron. It
trains and converges (40k updates). TWO root causes were found first-hand, each a real result:
1. **DATA (the register blocker, concretely):** the modern tagger mis-tags verbs only **1.5%** of the time, so real
   `SHIFT_V` training examples are far too rare to learn from. FIX built: **synthetic mis-tag injection** (corrupt a
   fraction of gold-VERB tokens' input POS to NOUN — glass-box, simulates the 19c register shift) gives the correction
   abundant training signal. This is the answer to "no register-native data exists."
2. **DECODER (greedy is insufficient — beam required):** even with abundant signal, the GREEDY decoder cannot fire
   `SHIFT_V` (score strongly negative, 0 recovery). Traced first-hand: at the shift point a mis-tagged VERB and a
   verb-readable NOUN look identical (the verb's subject/object are not yet attached), so verb-readable-noun HARD
   NEGATIVES swamp the rare positives and push `SHIFT_V` negative. This is EXACTLY why Bohnet & Nivre / the drill
   specify **BEAM SEARCH** — the beam looks ahead to see the arguments attach before committing the category; greedy
   cannot. My greedy build confirms the requirement precisely.
NET: the joint parser is no longer a spec — it is a trainable scaffold with the data fix built and the one remaining
piece (beam-decoded joint inference) precisely characterized and motivated. The post-hoc noisy-channel override (§0j,
0.50 @ 0.92 on 19c) remains the best WORKING result; the beam joint parser is the path to a general, deployable one.

> ### 🆕 THIS IS A NEW BUILD — file as a SEPARATE PROBLEM, NOT part of this one.
> **`beam_decoded_joint_pos_dependency_parser_with_synthetic_register_mistag_augmentation`** (proposed slug). It is a
> focused ML build (beam transition parser + early-update training + retrain), cross-task and doubly-compounding
> (POS feeds the parser AND the meaning channel), NOT a wire into this problem. What is already done and handed over:
> the trainable joint scaffold (`experiments/exp_whodidwhat_joint_decoded_parser_v1.py`), the synthetic-mistag data
> fix, and the first-hand proof (5 refuted post-hoc/greedy approaches) that beam lookahead is required. Owner/strategy
> should open it as its own problem with this as the starting evidence. It is OUT OF SCOPE for THIS submission.
