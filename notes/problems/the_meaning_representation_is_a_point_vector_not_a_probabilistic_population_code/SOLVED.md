...
files_changed: "experiments/exp_ppc_precision_tracks_correctness_v1.py, experiments/exp_ppc_fusion_v1.py, experiments/exp_ppc_grow_by_reading_v1.py, experiments/exp_ppc_no_regression_v1.py, experiments/exp_ppc_selective_prediction_v1.py, experiments/exp_all_bf_upstream_trace_v1.py (fixes the NOT_BF parser BF-ly + top-down loss trace), experiments/exp_bf_learned_channel_landing_v1.py (the LANDABLE parser-free channel), experiments/exp_ppc_second_gold_powerup_v1.py (SimLex+SimVerb robustness), experiments/exp_ppc_grow_to_strength_v1.py (grow-by-reading on modern Simple-Wikipedia, executed), experiments/exp_ppc_reliability_deferral_v1.py (convergent-agreement deferral), experiments/exp_ppc_population_reliability_v1.py (PHASE-DIAGRAM move: dense population-consensus reliability -> CI-sep deferral), experiments/exp_meaning_depth_gap_v1.py (identifies the next gap: learned meaning is relation-blind), experiments/exp_valence_polarity_meaning_channel_v1.py (fix layer 1: affective valence un-blinds antonymy, CI-sep), experiments/exp_directional_consequence_channel_v1.py (layer 2a: path-direction separates scalar converses), experiments/exp_role_asymmetry_converse_channel_v1.py (layer 2b: drills the transfer-converse wall to role-binding), experiments/exp_ppc_all_v1.py (driver), verification/test_ppc_meaning_representation.py (57/57), notes/problems/the_meaning_representation_is_a_point_vector_not_a_probabilistic_population_code/{NEXT_GAP_generative_predictive_meaning.md,COMPONENT_REGISTER.md}, notes/problems/the_meaning_representation_is_a_point_vector_not_a_probabilistic_population_code/{SOLVED.md,BF_AUDIT_UPDATE.md,ALL_BF_UPSTREAM_TRACE.md,HDLAB_INTEGRATION_SPEC.md,_working_notes.md}. NO hdlab/ writes (Q111)."
...
## WHAT TO INTEGRATE (executive summary for strategy -- read this first)
This submission is a rigorous PARTIAL on the stated bar plus a deep, fully-researched forward map. The integrable
core, separated from the research trail:
- **The BAR result:** item 1 PASS (intrinsic precision = accumulated-evidence GAIN tracks correctness, CI-sep on 2
  golds), item 3 PASS (recall byte-identical), item 2 LOCATED NEGATIVE (fusion-reweighting is at the equal-weight
  ceiling -- a fitted weight fails too -- thoroughly closed across the full exposure range). §2, §3.
- **LAND NOW (Q111), 3 net-positive/fidelity items, all reuse + BF, `HDLAB_INTEGRATION_SPEC.md`:**
  1. the PARSER-FREE learned channel (ConceptSpace ROUTE-B store + directional typing + existing PPMI) -- removes
     the NOT_BF parser at no accuracy cost; grows by reading (SEQ 0.07->0.16, ~80% of the ontology). §3b/§3c/§3d.
  2. the AFFECTIVE-VALENCE dimension (reuse `affect_lexicon`+Warriner) -- a CI-sep capability: un-blinds meaning to
     antonymy (syn-vs-antonym AUC 0.535->0.75, no WordNet). §3f-gap / `NEXT_GAP...md` §5b.
  3. population-consensus RELIABILITY -> deferral (replace `convergent_cue_reader.w` with the intrinsic gain ratio;
     the population read gives CI-sep deferral, +0.049). §3e / §4.
- **FILE AS NEW PROBLEMS (forward map, mechanisms fully specified):** (a) the generative role-binding meaning channel
  for transfer converses (buy/sell) -- the wall researched to the bottom (needs argument-role binding, not
  co-occurrence); (b) end-to-end live grounding-coverage measurement. `NEXT_GAP_generative_predictive_meaning.md`.
- **Reverify everything:** `.venv/Scripts/python.exe verification/test_ppc_meaning_representation.py` (57/57).
...
### 3d. REUSE-FAITHFUL + GROW-BY-READING EXECUTED + THE FUSION ROUTE THOROUGHLY CLOSED
Three follow-ons that make this fully BF, reuse-faithful, and settle the fusion question:
- **REUSE, not a new organ (owner directive).** The parser-free channel IS the substrate's EXISTING co-occurrence
  store: feeding `ConceptSpace.observe_context_counts` (ROUTE-B, default-off) with DIRECTION+DISTANCE-TYPED context
  lemmas reproduces the directional counts BYTE-IDENTICALLY (witness A6), read via the substrate's EXISTING PPMI
  (`distributional_meaning_channel.ppmi_svd` / `dep_ppmi_matrix`). The only new bit is the order typing (the
  `sequence_memory` principle). `HDLAB_INTEGRATION_SPEC.md` is reframed around this reuse.
...
- **The fusion-reweighting route is now thoroughly closed (a settled located negative).** Across the FULL exposure
  ...
  0.226 -> -0.016; once every word has ample evidence the count stops discriminating); (b) RAW gain HURTS the
  fusion (frequency domination: 0.236 vs equal 0.259 at 500k), and the brain-foundational SATURATING form
  (log-gain, Weber-Fechner / saturating Poisson Fisher information) recovers only to PARITY (0.260 vs 0.259,
  +0.0016, not CI-sep). Equal-weight is Bayes-optimal here at every exposure -- the meaning fusion is either
...
**Where we lose signal NOW: not at any component -- at the LEARNED representation's RELATION-BLINDNESS** (measured,
`exp_meaning_depth_gap_v1`, SimVerb relation labels). The channels are LOW-correlated (0.08-0.18: diverse, NOT
redundant), but only the SUPPLIED ontology can tell "same meaning" from "OPPOSITE meaning": synonym-vs-antonym AUC =
GROUNDED 0.508, SEQ 0.531 (both ~chance), TAXONOMIC 0.875. The learnable channels (perceptual, distributional) are
RELATION-BLIND -- they score antonyms/cohyponyms AS SIMILAR as synonyms (grounded: SYN 0.43 ~ ANT 0.42 ~ COHYP
0.47), because antonyms share contexts and perceptual features. The fusion barely beats the ontology not from
redundancy but because the two channels the substrate can LEARN are weak on the one thing that matters. **THE NEXT
GAP (identified with a number): a LEARNED meaning channel that is NOT relation-blind -- that captures meaning
POLARITY / consequence (love vs hate, buy vs sell) without a supplied ontology.** Detail + candidate mechanisms:
`NEXT_GAP_generative_predictive_meaning.md`.
