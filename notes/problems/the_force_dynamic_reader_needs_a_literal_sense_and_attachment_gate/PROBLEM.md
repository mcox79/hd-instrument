...
priority:
review: EXCELLENT
review_text: "SOLVED (owner-DONE) integrated 2026-08-30 — the owner-directed prerequisite that UNBLOCKS the causation live-wiring, delivered as a generalization-tested literalness gate. Reverified FIRST-HAND: test_literalness_gate_organ.py 7/7 (scaffold-free). ⚠️ REVERIFY RESOLVED TWO STALE FRONTMATTER NUMBERS UPWARD: the SOLVED `result:` line says the RACE held-out is 'DIRECTIONAL but NOT CI-separated (n=55, +0.092 [0.000,0.186])', but the WITNESS recomputes the LARGER rerun — held-out RACE generalization IS CI-SEPARATED at n=130 (+0.089 [0.033,0.158], twin loses); and the witness is 7/7 not the frontmatter's stale '5/5'. The stronger OWNER_NOTES headline holds on reverify (the `result:` line just wasn't updated after the n=130 rerun). WHAT IT IS: a glass-box FORCE-AFFORDANCE gate (VETO architecture — grounded simulation attempted by default, blocked on a detected violation; Bergen/Barsalou) that lets the force-dynamic reader ENGAGE only on LITERAL physical events and ABSTAIN otherwise, emitting a THREE-WAY label (ENGAGE_PHYSICAL / FORCE_NONPHYSICAL / ABSTAIN so social-force metaphor is tagged not discarded). ABSTAIN if (1) an opaque VOBJ idiom fires (Giora stored-unit), (2) a force role is KNOWN-ABSTRACT (selectional violation / N400, Wilks) over antagonist+agonist+the Talmy motion-GROUND, (3) attachment is wrong. REUSES the WSD/idiom organs (did NOT rebuild — as the brief directed). RESULT (150 hand-adjudicated MODERN clauses, UD-EWT + MCScript2, base rate 0.560): FIRE-PRECISION 0.716 [0.631,0.800] vs the strongest floor FIRE_ANY 0.560 (+0.156 [0.102,0.216] CI-sep, null p95 0.596), RECALL 0.929. DOWNSTREAM END-TO-END (wired in front of the actual typer+estimator): cuts the reader's false-physical-type rate on non-literal clauses 0.89→0.41 (54% fewer figurative mislabels) while keeping literal coverage 0.86; 9/10 on the brief's own examples ('news broke'→ABSTAIN, 'branch broke'→CAUSE, 'deal fell through'→ABSTAIN, 'crushed by criticism'→ABSTAIN; miss='opened up'). GENERALIZATION (the owner's priority, done STRUCTURALLY — ZERO fit params): held-out unseen essay genre (RACE, n=130) +0.089 CI-sep, twin loses; and it generalizes the brain's way (WordNet IS-A on the roles: novel boulder/kettle ENGAGE, nostalgia/bureaucracy VETO). CONTROLS (each excludes something): info-free TWIN (shuffled sense + permuted concreteness, matched fire rate) 0.523 LOSES CI-sep; PER-COMPONENT ablation (concreteness = workhorse 0.684; +Talmy Ground +0.061; +attachment +0.032; the compositional WSD sense-posterior veto is net-NEGATIVE → left OFF, HONEST — matches the parent WSD finding that the frame-posterior is a fallible literalness cue); THRESHOLD SWEEP robust over c_min 0.15–0.50 (not a knife-edge); POSITIVE CONTROL 10 literal-vs-figurative minimal pairs 0.80 vs 0.50; PR-curve Pareto-DOMINATES the base-rate floor; SECOND INDEPENDENT (blind) ADJUDICATOR Cohen kappa 0.932/0.982 — the gate edge holds on THEIR labels (not a single-annotator artifact). BRAIN-FAITHFUL: grounded simulation is GRADED not gated (LIT>MET>IDIOM>ABS; Raposo/Desai) → target re-cut to conventional-figurative+idiom; PINNED selectional-violation→figurative (Wilks/N400), stored-unit idiom (Giora), Talmy motion Ground. HONEST LIMITS: recall 0.929 is a small CI-sep drop from always-fire 1.0 (unavoidable for any abstaining gate); the effect is modest and SMALLER on essay prose (concrete-role conventional metaphor is denser there); residual false-engages are concrete-role figuratives ('opened up', social force, relocation) = the seeded next problems. Grade EXCELLENT (reuses the WSD/idiom organs as directed, veto architecture is brain-faithful, beats the real floor CI-sep, HALVES figurative mislabels end-to-end through the REAL typer, generalizes structurally to a held-out genre, 2nd-adjudicator-validated, honest about the WSD-posterior negative + the recall cost). ✅ UNBLOCKS `wire_the_causation_typer_into_the_live_reader` — the gate's hdlab landing is COUPLED with that causation live-wiring (wire the gate + typer + patient-tendency into `_read_causation`: engage only on literal, correctly-attached physical events). Audit §2b folded. Seeded next problems (new organs, owner Q113): a SOCIAL/INSTITUTIONAL-FORCE reader (the FORCE_NONPHYSICAL bin has no consumer; the leading uncaught class on essay prose); a context-WSD / conventional-metaphor inventory for concrete-role figuratives (caps essay-prose generalization)."
---

> ## ✅ SOLVER REVIEW — INTEGRATED 2026-08-30 (grade: EXCELLENT; SOLVED owner-DONE)
> **Verdict:** the owner-directed prerequisite, delivered as a generalization-tested literalness gate — **it UNBLOCKS the
> causation wiring**. Reverified first-hand (`test_literalness_gate_organ.py` **7/7**). The reverify RESOLVED two stale
> frontmatter numbers UPWARD: the held-out RACE generalization is **CI-separated at n=130 (+0.089 [0.033,0.158], twin loses)**
> — the `result:` line's "not-CI-sep, n=55" was pre-rerun. A glass-box FORCE-AFFORDANCE **veto** gate (grounded simulation by
> default; abstain on an opaque idiom / a known-abstract force role / bad attachment), reusing the WSD/idiom organs as directed.
> **Fire-precision 0.716 vs floor 0.560 (+0.156 CI-sep), recall 0.929;** end-to-end it **HALVES figurative mislabels
> (0.89→0.41)** through the real typer while keeping 0.86 literal coverage. Generalizes STRUCTURALLY (zero fit params) to an
> unseen essay genre and via WordNet IS-A (boulder engages, bureaucracy vetoes). Controls: twin loses, WSD-posterior net-NEGATIVE
> (honestly left OFF), **2nd blind adjudicator κ=0.93** (gate edge holds on their labels). **Grade EXCELLENT.** The gate's hdlab
> landing is **COUPLED with `wire_the_causation_typer_into_the_live_reader`** (now UNBLOCKED). Audit §2b folded; `priority:` cleared.
