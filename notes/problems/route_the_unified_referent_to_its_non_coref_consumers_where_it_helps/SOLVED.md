...
- **Remaining fidelity levers (named, not built here):** frame-ELEMENT-restricted FrameNet agent ingest; a real
  anaphoricity/discourse-new detector (Ng & Cardie 2002) to replace the licensing-proxy gate; an animacy classifier to
  replace the person stoplist; and the reader-wide incremental-predictive-PARSER gap (the deepest, out of scope here).
