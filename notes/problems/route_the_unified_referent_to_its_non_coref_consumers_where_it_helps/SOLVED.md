...
- **THE NEGATIVE, FULLY UNDERSTOOD (owner: "if it's truly brain foundational it should have worked") -- it is a
  ...
  mechanism is now NET STRONGLY POSITIVE. The last-mile novelty gate is the BRAIN's
  RETRIEVAL-CONFIDENCE mechanism, NOT a trained classifier (Heim familiarity = a referent is familiar iff retrievable;
  McElree/Lewis-Vasishth low-activation = novel): built `cue_conf` (abstain when the best non-predicated candidate's
  ACT-R activation < a SWEPT threshold = the McElree speed-accuracy tradeoff, ONLINE, no training) -- MEASURED it
  raises precision 0.735->0.95 (gold) and CLOSES the C1 regression (-0.0026 -> +0.0000 safe) on live. (The Hawkins
  establishing-modifier heuristic was a located negative -- 0.59->0.56 precision, "the director OF X" is usually
  anaphoric in bio register.) **DEPLOYABLE: full-referent COMPETITION + a live-tuned confidence threshold (the SAT
  operating point) -- C3 up to +0.086 (liberal) down to C1-exactly-safe (conservative) -- all glass-box, ONLINE, NO
  training, NO trained classifier anywhere in the chain.**
