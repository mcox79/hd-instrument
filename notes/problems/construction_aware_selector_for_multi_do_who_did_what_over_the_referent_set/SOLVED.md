...
### 7a. THE WHOLE PIPELINE, COMPOSED AND PROVEN (`exp_construction_whole_composition_v1`)
The complete buildable composition — S1 referent-per-NP + S1a indefinite-pronoun coverage + S2 Right-hand-Head-Rule-
over-the-NP-span (Williams 1981, extending `np_head_reduce` to skip intervening adjective/adverb modifiers) + S3 the
deployed feature-competition selector, held byte-fixed — measured with full controls on cleaned-DO:

| | 19c LitBank (n=669) | modern QA-SRL (n=1261) |
|---|---|---|
| base deployed (rnp + selector) | 0.9283 | 0.8914 |
| **WHOLE (+indef +RHR)** | **0.9387** | 0.8914 |
| **WHOLE vs base (the proof)** | **+0.0105 CI[+0.0015,+0.0209] CI-SEP** | +0.0000 n.s. (win is 19c-specific) |
| component: +indef only | +0.0105 CI-SEP (twin loses) | +0.0000 |
| component: +RHR-head only | **+0.0000 (documented NULL)** | +0.0000 |
| no-regression single-DO | +0.0118 CI-sep (helps, no regress) | +0.0000 |

**PROVEN:** the whole composition beats the deployed base **+0.0105 CI-separated** on 19c, its info-free twins lose
(indef twin +0.0105 CI-sep; head twin +0.0897 CI-sep), and it does not regress single-DO. The win is **entirely the
brain-foundational indefinite-pronoun SOURCE coverage** (DRT introduction for quantified NPs). It is **19c-specific**
(the modern tagger doesn't mis-tag, and modern text lacks archaic "thee") — an honest, register-scoped result, and the
effect sits AT the sign-flip null p95 (0.0105), so it is real-but-modest, not a large lever.

**Robustness of the indef win (not over-fit):** the +0.0105 is driven ENTIRELY by the clean indefinite/archaic-pronoun
set (everybody/somebody/thee...); the risky quantifiers (all/none/each/one/other/others) add **exactly +0.0000** — they
introduce no distractors. So the win is a principled coverage fix, not a threshold-tuned artifact.

**The RHR head-refinement is a rigorously DOCUMENTED NULL, and its mechanism is the finding:** it changes 4 clauses —
**2 fixed** (adjective mis-tags: "cheery-looking"→woman, "nicens"→boy) and **2 broken** ("the river *winds*", "my heart
*breaks*" — where the tagger mis-tagged the *verb* as NOUN, so RHR wrongly drops the true head). Both the fix and the
break are the SAME root — **register POS-tagger noise** — and a structural head rule cannot tell an adjective-mis-tag
from a verb-mis-tag without fixing the tagger. **This PROVES the remaining parse-recoverable lever is the register-
native POS tagger (the parent's filed problem), not a structural rule bolted onto the selector.** A convenient
structural patch on noisy tags just moves the errors around (net zero); the brain-faithful fix is the tagger.
