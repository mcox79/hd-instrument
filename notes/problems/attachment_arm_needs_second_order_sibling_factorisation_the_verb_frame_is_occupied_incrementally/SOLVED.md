...
INTEGRATED_BY_STRATEGY 2026-09-13 14:00 local -- owner-DONE 13:30; reverified first-hand: the diff landed VERBATIM in hdlab/graded_role_assigner.py (ROLE_TO_SLOT / CORE_SLOTS / _frame_slots / _slot_capacity / assign_slots_hard / assign_slots_incr / observe_frame_outcome; joint decode in coarse_roles; commit 09a997ab8) + slot_capacity accrual in tools/build_coarse_role_validities.py (counts: obj 9516/0, iobj 647/0, byagent 306/0, subj 13048/84 = capacity one confirmed); role witness 23/23; 'She saw him yesterday' -> him obj, yesterday dep. DISPOSITION: LANDED SELECTABLE, DEFAULT OFF on the live chain -- measured on the who-did-what PATIENT board read under the brain-foundational governor: occupancy off 0.7954, on 0.7781, gated on head confidence 0.781 (n=347); the solver's +0.067 OBJ-precision win holds on the supervised parse and gold heads, so the mechanism is right and the governor's core-argument arcs are the wall (exactly the solver's located limit); flips on with HDLAB_ROLE_SLOT_OCCUPANCY=1 when the heads levers (pri 97 main assertion, pri 94 PP) land. The solver's self-test now fails 2/9 BY CONSTRUCTION (its 'baseline' is the live labeler). Follow-on filed inside pri 97/94.
