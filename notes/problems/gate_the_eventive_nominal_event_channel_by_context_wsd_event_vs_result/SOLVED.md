...
files_changed: "experiments/_nominal_wsd_gate.py (the gate organ + grounded readout path, self-test PASS); experiments/exp_nominal_wsd_gate_v1.py (the measurement: flat/parse/combined arms + floor + twins + theta sweep + positive control); experiments/exp_nominal_overfire_enumeration_v1.py (the over-firing population enumeration); experiments/exp_nominal_gate_representation_probe_v1.py (the WHY: event/object sig cosine w2v 0.93 vs grounded 0.51); experiments/exp_nominal_gate_grounded_v1.py (the grounded-representation prototype -- REFUTED at MAVEN power); experiments/_selectional_preference.py (the DERIVED selectional-preference organ from GUM gold parses) + experiments/exp_nominal_gate_selectional_v1.py (derived vs hand cue -- TIES, data-starved at MAVEN power); experiments/exp_nominal_gate_frequency_v1.py (the omitted FREQUENCY/DOMINANCE prior -- best single precision (dom_only) but same frontier, MAVEN 23653 chains); experiments/exp_nominal_gate_endtoend_v1.py (THE DECISIVE end-to-end reasoner test -- gating is net-NEGATIVE, ungated wins 0.4937=87% of ceiling); experiments/exp_temporal_chain_diagnosis_v1.py (UP THE CHAIN -- the ceiling itself is only 0.5681 because the reasoner guesses text-order for 94% of pairs; the located non-brain-foundational gap is the reasoner's missing world-knowledge temporal inference) + experiments/exp_temporal_gek_inference_v1.py (GEK order prior -- ties iconicity overall) + experiments/exp_temporal_gek_entropy_v1.py (GEK AS AN ENTROPY/IRREVERSIBILITY DETECTOR -- strongest-asymmetry decile beats iconicity +0.143) + experiments/exp_temporal_grow_order_v1.py (grow order knowledge from explicit cues -- mechanism right, coverage 1.6% data-starved) + experiments/exp_temporal_causal_reasoner_v1.py (full iconicity-default + causal/entropy override with derived sink-ness -- does NOT raise the ceiling; RE-DERIVED the landed temporal_script_schema; Sec 4i correction) + experiments/exp_temporal_script_override_v1.py (the LANDED script organ as a gated override -- confident subset 0.625>0.549 but ~4% coverage on narrated pairs; the wire target is the IMPLICIT-event path, Sec 4j + Sec 7 item 5); verification/test_nominal_wsd_gate.py (scaffold-free witness, W1-W4 PASS). NO hdlab write (Q111 -- proposed diff in Sec 7)."
...
### 4h. UP THE CHAIN — the real non-brain-foundational gap is the REASONER, not extraction (the deepest finding).
The gold-event CEILING is only 0.5681 answered-correct: with PERFECT extraction the reader still answers barely more
than half of BEFORE/AFTER pairs, where a competent human is ~90%+. Decomposing that ceiling
(`exp_temporal_chain_diagnosis_v1`, 1586 gold pairs): the reasoner has an EXPLICIT temporal cue (subordinating
connective after/before/since/until, or past-perfect tense-anteriority) for only **5.9% of pairs** (accuracy 0.808
there); the other **94.1% fall back to NARRATIVE ICONICITY -- guessing that text order = time order -- at 0.553, barely
above chance.** So the ENTIRE extraction debate this brief is about (my gate, even the nominal channel itself) is
fighting over the ~2-point edge from the confident 6%, while the **~35-point gap to human comprehension is the reasoner
GUESSING on 94% of pairs.** The brain does not guess text-order: it infers order from EVENT-SCHEMA / CAUSAL WORLD
KNOWLEDGE ("he was shot. he died." -> shooting before dying, no connective; script/Result/Explanation discourse
relations). The connective+tense reasoner has none of this. **This is the located, dominant non-brain-foundational gap
UP THE CHAIN -- a distinct organ (schema/script-based + causal temporal inference), and the substrate already HAS the
seeds for it (`hdlab.temporal_script_schema`, `hdlab.generalized_event_knowledge`, `hdlab.schema_exemplar_bayes`) but
the temporal reasoner does not consult them.** It is the highest-leverage problem in this area and dwarfs the gate.

**CORRECTED BRAIN-FOUNDATIONAL FRAMING (a key fix -- I had called iconicity a non-brain-foundational hack; it is NOT).**
Iconicity (order-of-mention = order-of-occurrence) is the brain's DEFAULT assumption (Zwaan's chronological
assumption). The brain OVERRIDES it, confidently, with CAUSAL / IRREVERSIBILITY knowledge (Trabasso & van den Broek
causal-network model): causes precede effects because effects are IRREVERSIBLE -- the arrow of time is entropy ("you
can't unscramble an egg"). So the brain-foundational architecture is **iconicity DEFAULT + confident CAUSAL/entropy
OVERRIDE**, and we already have the default (~0.55). Two prototypes tested the override from the knowledge store:
(a) GEK forward-asymmetry as a temporal-order prior -- OVERALL it ties iconicity (it re-learns narrative order), BUT
GEK is in effect an ENTROPY/IRREVERSIBILITY DETECTOR: on the STRONGEST-asymmetry decile it beats iconicity +0.143
(0.548 vs 0.405; deciles 2-3 +0.09/+0.17), and on the weak-asymmetry majority it loses -- i.e. asymmetry STRENGTH is a
confidence/irreversibility signal, and the confident override subset is real but small-coverage (~3% of pairs at the
strong end, because GEK is built from narrow ROCStories); (b) GROWING an explicit-cue order store (mine "after/before"
clauses -> type-level order) -- consolidated 4597 ordered types but covers only 1.6% of TB-Dense's newswire pairs (a
DATA-COVERAGE limit, not a mechanism failure). So the honest answer to "did this succeed / is it 100% brain
foundational": at the extraction stage the channel works (turn it on ungated); the SYSTEM does not reach brain-level
temporal comprehension; the located gap is a CONFIDENT CAUSAL/IRREVERSIBILITY OVERRIDE on the iconicity default, and
the substrate's knowledge store + learning loop CAN grow it -- the prototypes show the mechanism is right (strong-arrow
pairs beat iconicity) but data-starved for newswire; the real build is a well-powered CAUSAL/entropy event-order store
(from a large corpus + irreversible-verb lexical aspect), used as a strength-gated override. This is a distinct,
high-leverage NEW problem, seeded here.

### 4i. CORRECTION -- I OVERSTATED THE CHAIN GAP: the reasoner is already at the HUMAN ceiling, and I re-derived built organs.
Checking the prior work (`notes/BRAIN_FOUNDATIONAL_AUDIT.md`, the owner-DONE `reason_over_event_time_order_and_duration_
on_a_modern_gold`) after the fact corrects three errors in Sec 4g/4h, and I own them:
(1) **I INVENTED the human ceiling.** I asserted "a competent human is ~90%+" with no evidence -- the SAME error I
criticized. The documented human agreement on THIS task (holistic dense TLINKs) is **IAA 59-64%** (vs 85-98% for the
easier in-context-QA framing). So the task is inherently ~60%-agreeable -- a NOISE CEILING, like the event-segmentation
ceiling in the recurrent-loop problem; I forgot my own "compute the noise ceiling first" lesson.
(2) **I measured the WRONG reasoner.** My end-to-end/ceiling cells use the SIMPLIFIED `_temporal_ordering_multiframe`
(connective+tense only -> ceiling 0.5681). The LANDED `hdlab.temporal_reasoner` (cue -> TIMEX date-anchoring ->
iconicity + transitive closure) already scores **0.6225** on TB-Dense BEFORE/AFTER (cue channel 0.909, date 0.830,
first-hand at landing, witness 18/18). **0.6225 is AT the top of the 59-64% human band -- the system is essentially at
the achievable ceiling on this task, not 35 points below it.** The ~35-point "gap" was an artifact of an invented human
number AND a weakened reasoner.
(3) **I re-derived already-built, already-documented organs.** `hdlab.temporal_script_schema` already mines 177,800
ordered verb-pairs (Chambers & Jurafsky narrative chains) from ROCStories for implicit-event order (0.6022 on its
covered 29%); the landed reasoner already implements the iconicity-default + cue-override cascade; and "the causal-MARKER
channel loses to iconicity; the real lever is causal-WORLD-KNOWLEDGE inference" is a SHELVED located negative (a filed
follow-on, P4 SDRT world-knowledge reader). My GEK-asymmetry / sink-ness / grown-order prototypes reproduced that organ,
worse, and hit the same wall it already documented.
**CORRECTED BOTTOM LINE:** on temporal ORDERING the system is at/near the human agreement ceiling; it IS effectively
brain-foundational there. The residual causal-world-knowledge lever is real but SMALL (the arrow-bearing subset), already
has an organ (`temporal_script_schema`, LATENT -- not yet a live scored consumer), and is a filed follow-on -- NOT a
35-point non-brain-foundational hole. What genuinely stands from this drill: the NOMINAL-GATE refutation (Sec 4g -- a
relative comparison, reasoner-agnostic, still valid) and the correct recommendation to turn the nominal channel ON
UNGATED. The lesson (again): CHECK THE NOISE CEILING AND THE PRIOR WORK before declaring a gap.

### 4j. ACTIVATING THE LATENT SCRIPT/SCHEMA ORGAN -- the brain-foundational wire is the IMPLICIT-event path, not an iconicity override.
Using the LANDED `hdlab.temporal_script_schema` (177,800 Chambers-Jurafsky verb-pair orders, NOT a re-derivation) as a
confidence-gated override on iconicity (`exp_temporal_script_override_v1`, TB-Dense gold ceiling): aggregate lift
+0.0006 (ns) -- BUT where the organ is CONFIDENT (evidence>=30, |p_before-0.5|>=0.1) it is right 0.625 vs iconicity
0.549 (a real +0.076), just at ~4% coverage; the shuffled-counts twin loses (0.542 < 0.549). So the organ carries a
genuine world-knowledge signal, it simply almost never fires on NARRATED newswire pairs -- because those are already
handled by iconicity (the brain's default too). **The correct brain-foundational wire is therefore NOT an iconicity
override on narrated events; it is the IMPLICIT-EVENT path** -- the reasoner currently returns "unknown -- needs world
knowledge" on implicit-event queries (an event the text does not narrate), and the organ answers exactly those at 0.60
(its landed TRACIE validation, vs the timeline register's abstention). That converts UNKNOWNs into answers on the
organ's validated turf -- a real capability the reader lacks today -- with ZERO risk to the narrated-event path. Proposed
diff in Sec 7 item 5.
...
5. **(ADJACENT, owner-directed) ACTIVATE the latent `hdlab.temporal_script_schema` on the IMPLICIT-EVENT path (Sec 4j).**
   In `hdlab.temporal_reasoner.TemporalReasoner.before(a,b)`, BEFORE returning the implicit-event/`UNKNOWN` verdict (the
   case where neither a CUE path nor a DATE path connects the pair AND the pair is not both-narrated in the register),
   consult `TemporalScriptSchema.load().order(lemma_a, lemma_b)` gated on `evidence(...) >= E_MIN` (recommend ~10-30) and
   `|p_before-0.5| >= M_MIN` (~0.1); return its `before/after` + signal `'script'` when confident, else keep `UNKNOWN`.
   Additive + default-safe: it fires ONLY where the reasoner currently abstains (implicit events), so the narrated-event
   path (cue/date/iconicity) is BYTE-IDENTICAL; `available()==False` -> abstain (asset-less safe). Validated turf: 0.60
   on TRACIE implicit-event (the organ's landing measurement) vs today's abstention. It does NOT change TB-Dense
   before/after (those pairs are narrated -> handled before this branch). This is the brain-foundational activation of a
   filed, latent organ, prototyped in `exp_temporal_script_override_v1` (the gate + twin) -- strategy lands the hdlab hook.
