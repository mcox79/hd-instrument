...
priority:
review: EXCELLENT
review_text: "SOLVED (owner-DONE) integrated 2026-08-30 — the OWNER-PRIORITY McGuffey→modern migration (asked ~10x), delivered as a rigorous, mostly-NEGATIVE, corpus-age-confound-MADE-NUMERIC result. Reverified FIRST-HAND: test_mcguffey_migration.py 19/19 (scaffold-free, all recomputed). Built a MODERN situation-model role eval from UD-EWT gold parse (330 passages / 700 in-scope agent/patient queries; transparent UD-deprel→role, NO LLM) in the McGuffey gold shape, and ran the IDENTICAL reader pipeline on McGuffey vs modern under one scorer. (1) McGUFFEY'S ROLE EVAL IS DEGENERATE: 90.85% of in-scope gold is 'agent', so a trivial always-agent floor scores 0.908 [.863,.948] and the celebrated organ (vargs 0.856) LOSES to it — the original eval never gated against this strongest majority-class floor (only a positional-reader 0.517 + twin 0.627). The brief's headline '0.517→0.742' is corrected to disk (0.483→0.736) AND it fails the strongest-floor test — a major honest correction: the celebrated McGuffey role number was partly a degenerate-eval artifact. (2) THE ORGAN DOES NOT GENERALIZE TO MODERN TEXT (exactly the owner's generalization concern): vargs 0.596 < floor 0.659; COLLAPSES on non-canonical order to 0.288 — below the coin-flip twin (0.576) and CI-separated BELOW the floor. McGuffey's ~0% non-canonical rate STRUCTURALLY HID this — the corpus-age confound made numeric. (3) FIXABLE brain-fidelity gap, not a ceiling: root-caused (the front-end reads roles off AUXILIARIES 'has/is/was', not the content verb); a passive-aware content-verb assigner recovers non-canonical 0.288→0.559 CI-sep (voice-scrambled twin loses 0.458; canonical unhurt 0.624→0.646). BRAIN DEPTH (3 literature drills, each confirmed/corrected the design): learned surface cues WALL on unseen constructions (0.000 held-out inversion; the Competition Model's conflict-validity predicts it — a linear cue-sum reaches NEITHER domain); grounded thematic-fit CLEARS the wall (non-canon 0.688 vs surface 0.039, construction-independent); conflict-validity is a GATE not a linear weight; precision/reliability-weighted integration PINNED (Ernst&Banks 2002; Gibson noisy-channel 2013; Feldman&Friston precision=gain) — two on-disk wrong approaches fenced (scalar-over-fused inert; margin-gating HARD_FAIL). Implementing the brain mechanism RE-DERIVES the owner-DONE graded_role_assigner design (role = GRAMMATICAL FUNCTION + VOICE, not surface position; an independent parser solves passive 0.948 / inversion 0.731 / fronting 0.944 where position ~0). SELF-CORRECTED TWO of its own errors: 87% of the 'inversion wall' was its OWN gold mislabeling existential-'there' pivots as agents (fixed at source; genuine inversion is only 11%, 83% quotative = the landed rule already handles it); inversion is a PARSE problem, not thematic-fit. COREF dimension already migrated (owner-DONE LitBank): graded 0.328 vs rand-twin 0.132. HONEST BOUNDS: role on modern UD-EWT + coref on 19c LitBank — no single modern NARRATIVE corpus on the shelf has BOTH gold coref AND gold roles (46.6% of modern core-args are pronouns, invisible to string-identity); some deepening directional; UD-EWT is web not narrative. Grade EXCELLENT (delivers the owner's repeated request; a rigorous mostly-negative that makes the corpus-age confound NUMERIC, exposes a degenerate eval, quantifies a generalization collapse, root-causes the fixable fidelity gap, re-derives an owner-DONE design, and self-corrects). STRATEGY LANDINGS QUEUED (Q111 — DEDICATED efforts, NOT rushed): (1) swap the default role/situation-model eval to the modern UD-EWT gold + retire McGuffey-as-primary (keep it only as an archaic-register robustness check) — the McGuffey reference is DIFFUSE across ~9 hdlab files, and the swap has RE-BASELINING implications (reader numbers reported vs McGuffey must be re-based vs modern); (2) land the BUILT-but-queued locative/existential/expletive-'there' subject-override into `graded_role_assigner` + rebuild the who-did-what cache (corpus-scale). Both flagged to the owner as the next dedicated actions; recorded in the wire-don't-island debt. Audit §2b folded. Seeded next problems: (A) flagship reversible/ambiguous role residual via grounded thematic-fit + a surprisal/route-conflict GATE (architecture pinned, 2 wrong approaches fenced, sole invention = the reliability estimator; shares the distributional_meaning_channel dependency); (B) a both-gold modern NARRATIVE situation-model gold (the 46.6%-pronoun gap); (C) a grounded/learned animacy cue (its principle generalizes, its hard-coded word list does not: fire-rate 0.71→0.12 on modern)."
---

> ## ✅ SOLVER REVIEW — INTEGRATED 2026-08-30 (grade: EXCELLENT; SOLVED owner-DONE)
> **Verdict:** the owner's ~10×-requested migration, delivered as a rigorous, mostly-NEGATIVE result that makes the
> corpus-age confound numeric. Reverified first-hand (`test_mcguffey_migration.py` **19/19**, all recomputed).
> **(1)** McGuffey's role eval is **DEGENERATE** — 90.85% "agent", so a trivial always-agent floor (0.908) BEATS the
> celebrated organ (0.856); the original eval never gated against it. **(2)** On modern text the organ does **not**
> generalize — 0.596 < floor 0.659, and **collapses to 0.288 on non-canonical order** (CI-sep below floor); McGuffey's
> ~0% non-canonical rate hid it. **(3)** Fixable fidelity gap (reads auxiliaries not the content verb; passive-aware fix
> recovers 0.288→0.559). Brain depth: surface cues wall on unseen constructions, grounded thematic-fit + precision-weighting
> clear it, and the mechanism **re-derives the owner-DONE `graded_role_assigner`**. Self-corrected two of its own errors
> (existential-"there" gold-noise). **Grade EXCELLENT.** **Two strategy landings QUEUED (Q111, dedicated efforts):**
> swap the default eval to modern UD-EWT (retire degenerate McGuffey-as-primary — diffuse across ~9 files, re-baselining
> implications) + land the existential-subject override into `graded_role_assigner` + cache rebuild. Audit §2b folded;
> `priority:` cleared. **The celebrated McGuffey role numbers were partly a degenerate-eval artifact — a load-bearing correction.**
