...
- **CAN LEARN (grow from reading) -- the graph GROWS, but naive corpus-learned edges do NOT improve WSD (a clean,
  REPRODUCIBLE can-fail NEGATIVE).** `learn_from_text()` grows syntagmatic edges from LitBank co-occurrence via the
  cross-situational gate (Yu & Smith; the specced brain mechanism) -- grew **16,621 real structured edges** from 12k
  sentences (a LEARNED version of SyntagNet's manual edges). The completed can-fail test (n_eval=638, all 3 arms one
  config) REFUTES a WSD gain: WiC margin base=+0.0784 [0.035,0.125], **learned=+0.0674 [0.024,0.113] (BELOW base),
  info-free shuffle-twin=+0.0752 [0.030,0.121]** -- learned is below BOTH base and the random-rewire twin, all CIs
  heavily overlap: `learning_helps=False`. So the growth is graph-perturbation NOISE, not useful structure. WHY
  (located): the edges are MFS-disambiguated (crude/circular -- the graph should disambiguate them, not a heuristic),
  from a NARROW corpus (2-3 novels, poor coverage of WiC's general vocab), and SPARSE (16-21k vs SyntagNet's 88k
  manual). The naive version is a fair test of a WEAK impl, and it fairly fails; the FAITHFUL learned graph needs
  context-disambiguated edges + broad reading + the CLS schema-gate + sense split/merge
  (`LEARNED_GRAPH_brain_mechanism_spec.md`). NET: the organ can grow + be added to; making growth IMPROVE WSD is the
  open north-star, now with a clean negative baseline to beat.
  - **SECURE METHOD (owner asked for robustness):** `experiments/exp_learn_resumable_v1.py` -- a RESUMABLE, CHECKPOINTED
    harness (base adjacency + learned edges + each arm cached to disk; skip-on-rerun) that survives the shared box's
    intermittent OOM/fleet kills -- each relaunch resumes from the last checkpoint until DONE. It reproduced the base
    arm exactly (+0.0784) and delivered the clean 3-arm verdict above where the monolithic run kept dying.
...
---
INTEGRATED_BY_STRATEGY: 2026-09-01 (EXCELLENT). Reverified 5/5 first-hand (test_grounded_semantic_graph_ladder.py). The NORTH-STAR meaning organ: a grounded relational semantic graph read by SPREADING ACTIVATION (PPR over WordNet + MFS-disambiguated gloss + ConceptNet thematic edges) CLEARS the context-shuffle twin on HELD-OUT WiC (+0.0521 CI[0.0229,0.0807]) AND beats MFS on standard all-words WSD (+0.0295 CI-sep, ~UKB 67.3, glass-box/LM-free); residual located to named components (SyntagNet + freq-seeding), NOT granularity. Rigorous controls (gloss-edges load-bearing, IC-neutral, grounded-node ablation, damping-sweep). Honest SemCor cross-task boundary (MFS beats the walk there). WIRE QUEUED (Q111, WIRING_MAP): promote experiments/grounded_semantic_graph_organ.py (~verbatim) to hdlab + reframe reading_grounding_loop.canonicalize FLAT cosine -> graph diffusion; default-off, witnessed. Follow-ons NOT yet issued: the brain-foundational COMPLETION (emit the SETTLED vector not a label) + the LEARNED graph (north-star, grow/retune, specced). NEW owner-authorized data assets: SyntagNet 1.0 (CC BY-NC-SA), Raganato WSD framework. AUDIT UPDATE -> 2b; priority cleared. Grade EXCELLENT. NO push.
