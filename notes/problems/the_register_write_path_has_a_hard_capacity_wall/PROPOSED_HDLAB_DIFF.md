...
the OPERATION `S = λ·S + bind(role,item)`; sweep λ. Default `leak=0.0` reproduces the flat sum byte-for-byte.
...
- **Activity-adaptive λ** (leak grows with buffer magnitude, a divisive-normalization-like gain) is a higher-fidelity
  FORM to offer as `leak="adaptive"`; measured equivalent-or-better on recent recovery, worth a sweep (OPTIONAL).
