...
priority:
review: EXCELLENT
review_text: "SOLVED (owner-DONE) integrated 2026-08-29 — bar MET (a positive capability result). Reverified FIRST-HAND: test_register_leaky_write.py 11/11 PASS. The register WRITE-path capacity wall is fixed the brain's way: a CONTINUOUS asymmetric leaky/recency write (S=λ·S+bind(role,item)) holds recent-4 recovery = 1.000 at EVERY load N∈{16..768}, where the STRONGEST flat floor (flat sum read by the LANDED decode_serial theta-gamma readout — NOT a flat+argmax strawman) holds to N=64 then collapses (0.175@128, 0.100@256, 0.025@512); paired LEAKY − flat+SERIAL is CI-separated from N=128 on (+0.825→+0.975); info-free shuffled-key twin ~0.02. FORM is the brain's: recovery-by-recency GRADED/monotonic (3-bin 1.00/0.958/0.508 = the primate 66/45/39 shape, Konecky 2017); a hard bounded QUEUE is a STEP. The fundamental single-store trade is confirmed (leaky UNIFORM recovery collapses 0.45→0.019 — the leak buys RECENT by decaying OLD → the 2nd store is brain-necessary). SECOND STORE (salience-gated HDFactStore hand-off): a weighted-OR(prediction-error, schema-congruence) gate recovers 0.643 of salient events vs the FIFO/eviction-order floor 0.247 (+0.395 CI-sep), OR beats PE-only + CONG-only, leaky-only 0.056; commit-by-salience-not-age, PE an INDEPENDENT channel — the SELF-derived-salience NEGATIVE control does NOT beat FIFO, faithfully reproducing the on-disk exp_attention_salience_reliability_gate HARD_FAIL (salience must be a separate circuit — VTA/LC PE). Brain-grounded (2 primary-source drills: Warden-Miller 2007/Konecky 2017 recency gradient = MEASURED/PINNED-WEAK; SLIMM/Tse/van Kesteren salience gate = PINNED P=0.78). FIDELITY DEEPENING (owner 'keep pushing'): the single-timescale leak is a first-order approximation — the brain holds a SPECTRUM of timescales (Bernacchia 2011; Murray 2014); a MULTI-timescale cascade recovers ~3× more of the recency window (reach 43 vs single 6, CI-sep every load) without sacrificing recent, reach stays FINITE (2nd store still needed) — with TWO self-caught citation corrections (Fusi/Benna-Fusi are the consolidation stage, not the capacity theorem). AUDIT UPGRADE: Watters 2026 — primate frontal WM is a GAIN-WEIGHTED SUPERPOSITION beating slots → the substrate's superposition-register FORM is now PINNED at the population-code READOUT level (a dent in 'VSA binding unpinned' at readout, NOT the bind() algebra). Grade EXCELLENT. hdlab landing QUEUED (a full concrete diff exists in PROPOSED_HDLAB_DIFF.md, verdict-independent): (1) a `leak` param on AccumulateRegister (0.0=flat/byte-identical default; >0=asymmetric leaky recency write) threaded through make_situation_register + the multibank backend; (2) a thin register_consolidation helper (salience=max(w_pe·PE, w_cong·CONG), commit to HDFactStore iff >θ; PE independent, CONG from script_grain MDL; commit-most-salient). AUDIT §2b folded. Honest caveats (I concur): NOT run real-text end-to-end (the wall is an FHRR-algebra property, content-agnostic — the synthetic load sweep is the correct instrument; the full reading-score lift is a landing follow-on); the salience channels are MODELLED, not read from the live PE/MDL organs (gate policy established, live-channel reliability is the landing's job). NEXT PROBLEM primed: multi-timescale/stage-appropriate timescales (build on the landed TGPH cell)."
---

> ## ✅ SOLVER REVIEW — INTEGRATED 2026-08-29 (grade: EXCELLENT; SOLVED owner-DONE — a positive capability result)
> **Verdict:** bar MET, exemplary. Reverified first-hand (`test_register_leaky_write.py` **11/11 PASS**). A CONTINUOUS
> asymmetric leaky/recency write (`S = λ·S + bind(role, item)`) holds recent-4 recovery = **1.000 at every load N∈{16..768}**,
> where the STRONGEST flat floor (flat sum read by the LANDED `decode_serial` theta-gamma readout — not a strawman) collapses
> past N=64 (0.100@256); paired lift CI-separated from N=128 (+0.825→+0.975); shuffled-key twin ~0.02. **Form is the brain's:**
> recovery-by-recency graded/monotonic (1.00/0.958/0.508 = the primate 66/45/39 shape), a hard queue is a step. The
> single-store trade is confirmed (leaky UNIFORM collapses 0.45→0.019 → a 2nd store is brain-necessary). **Second store:** a
> weighted-OR(prediction-error, schema-congruence) salience gate recovers 0.643 of salient events vs the FIFO floor 0.247
> (+0.395 CI-sep); commit-by-salience-not-age, PE an INDEPENDENT channel — the **self-derived-salience negative control does
> NOT beat FIFO, faithfully reproducing an on-disk HARD_FAIL** (salience must be a separate circuit).
> **Rigor:** exemplary — strongest-floor (the landed serial readout, not a strawman), a full control suite, brain-grounded
> (Warden-Miller/Konecky; SLIMM/Tse/van Kesteren), and an owner-pushed **fidelity deepening** (a multi-timescale cascade
> extends the window ~3× CI-sep without harming recent, reach stays finite so the 2nd store still stands) **with two
> self-caught citation corrections** (Fusi/Benna-Fusi are the consolidation stage, not the capacity theorem).
> **Audit upgrade:** Watters 2026 — primate frontal WM is a gain-weighted SUPERPOSITION beating slots → the substrate's
> superposition-register FORM is now PINNED at the population-code READOUT level (a dent in "VSA binding unpinned" — at
> readout, not the bind() algebra). **Grade EXCELLENT.**
> **Landing QUEUED (Q111 — a full concrete diff exists in `PROPOSED_HDLAB_DIFF.md`, verdict-independent):** (1) a `leak`
> param on `AccumulateRegister` (0.0 = flat/byte-identical default; >0 = asymmetric leaky recency write) threaded through
> `make_situation_register` + the multibank backend; (2) a thin `register_consolidation` helper (salience gate → commit to
> `HDFactStore` by salience). **Audit** §2b folded. Caveats (I concur): not run real-text end-to-end (the wall is an
> FHRR-algebra property — the synthetic load sweep is the correct instrument; the reading-score lift is a landing follow-on);
> the salience channels are modelled, not yet read from the live PE/MDL organs (the landing's job). **Next problem primed:**
> stage-appropriate multi-timescale registers (build on the landed TGPH cell).
