...
file accrues WEIGHTED FINE type evidence from the document's own structured predicates: POSSESSED-NOUN relations
("X's capital/president/border" -> country; "X's CEO/employees" -> organization; "X's album" -> musician; "X's wife"
-> person) -- the novel fine generative signal (a 78-entry offline relational lexicon of what predicates apply to
what kinds) -- plus deverbal agency (write->writer), apposition/copula in-text is-a, head-in-name, title, gender.
At the anaphor, GRADED content-addressable match of each candidate's file against the head + recency; argmax (soft
settling, NO hard filter).
...
| **generative type-file** | **0.480** | **+0.0640 CI[+0.0080,+0.1202] CI-SEP** (vs recency +0.0880 CI-sep) |
| shuffle-the-file twin | 0.352 | gen beats twin **+0.1280 CI-sep** -> the accrued type<->entity link is real |
- **WITHIN-ITEM ASYMMETRY (matched control):** the file licenses the GOLD anaphor head **1.91** vs a MISMATCHED
  cross-family head **0.33** = **+1.58 CI[+1.14,+2.12] CI-sep** -> the accrued type is FINE and entity-specific, NOT
  the coarse over-licensing that sank the discourse-flag route. Witness `test_namebridge_generative.py` 5/5.
