...
3. **REACHABLE COVERAGE via a DISAMBIGUATION-confident entity linker -- [PROTOTYPED; BRAIN-FOUNDATIONAL SAFETY
   mechanism; coverage-neutral located negative] (`exp_namebridge_entity_linker_v1`).** Built the brain-faithful
   linker with tools in place: the DISCOURSE who-is-who file CONFIRMS the KB candidate (Bruce-Young/IAC: episodic
   context gates whether the consolidated recognition's type facts fire; else abstain/NIL). RESULT: it WORKS as a
   mechanism -- it NEUTRALIZES the wrong-entity noise that refuted the ungated broad KB (cls_full 0.604 ->
   cls_linked 0.607, no longer hurting) -- but adds NO net coverage over the clean base (cls_unified 0.607 ==
   cls_linked 0.607; witness G10). WHY: (a) reachable is already at its clean-KB ceiling; (b) the residual facets
   are ENTITY-SPECIFIC POSITIONS (Dvorak->director is a Wikidata P39 position, not a P106 occupation) beyond a
   standard occupation-KB. So this linker is a brain-foundational SAFETY tool (makes broad-KB acquisition safe to
   use, gated by discourse-consistency) -- valuable, but NOT a coverage lever here. A coverage push would need top-k
   candidate generation + P39 positions + FINE consistency, but those facets are entity-specific = opportunity #1's
   learned-world-model frontier, not this linker.
...
| DISCOURSE-GATED entity linker (`exp_namebridge_entity_linker_v1`) | Bruce-Young/IAC familiarity gate (context-confirmed recognition) | **BRAIN-FOUNDATIONAL mechanism** -- works (neutralizes noise); coverage-neutral here |
