...
3. **REACHABLE COVERAGE via a DISAMBIGUATION-confident entity KB -- [NEEDS A TOOL WE DO NOT HAVE] (modest).** Reachable
   is 68% typed; pushing it needs a clean entity-LINKED KB with per-mention DISAMBIGUATION confidence. TOOL GAP: our
   entity typing is surface-lookup (DBpedia/probe) or noisy top-1 search -- there is NO glass-box entity-linker with a
   confidence/NIL gate (the Bruce-Young familiarity mechanism, confirmed by research). This is the SAME entity-
   resolution component #1 needs; filing it as the precondition. NOT buildable with current tools -- flagged.
