...
| **register/multibank WRITE path (`AccumulateRegister.add_event`) -- the BIGGEST gap found** | accumulates events (flat sum) | **MEASURED (W9/W10): flat write has a HARD capacity wall; read-norm CANNOT move it (raw==divnorm every load); recent recovery collapses to 0.13 @N=256** | **write-time ASYMMETRIC leaky/recency gain -> recent recovery 1.0 at any load (W9); continuous leak > hard queue (fidelity); + a CONTENT/salience-gated commit to the existing HDFactStore for old events** | **asymmetric recency suppression = MEASURED / PINNED-WEAK (Warden & Miller 2007 + Konecky 2017 primate PFC single-unit, monotonic recency gradient). NOTE: Buschman 2011 is a LOAD effect (simultaneous display), NOT the recency cite. CLS consolidation REFUTED as the 2nd-store analogy (it is content/schema-organized, not recency) -> use HDFactStore + a salience gate** |
