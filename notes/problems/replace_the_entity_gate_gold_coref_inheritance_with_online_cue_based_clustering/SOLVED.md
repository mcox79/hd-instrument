...
- **BOARD NO-REGRESS** (smoke, de-leak ON): core aggregate stable (model 0.6294 vs floor 0.5882, no arm crash), coref FLAT (0.504 vs 0.506 — the leak was in the entity/experiencer path, NOT the separate pronoun coref stream), who-did-what/state/wic healthy. In the full `--run` the heavy NEW arms (causal, temporal) ran CRASH-FREE under the flip; the authoritative full-population AGGREGATE table is a documented follow-on (the `--run` table emission was truncated by a subprocess/backgrounding harness quirk this session — NOT a de-leak regression).
