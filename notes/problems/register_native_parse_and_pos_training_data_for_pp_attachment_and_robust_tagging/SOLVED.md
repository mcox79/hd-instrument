...
- **Composition `P(patient|agent,verb)` on the clean ~15% direct-object slice:** the one genuine selection lever
  left. The mechanism is REAL on clean gold (verb-shuffle twin loses +0.097 CI-sep) but composition is UNBUILT
  (~50/50 per the drill; owned selection problem), and it is bounded to ~15% of items -- not a large absolute gain.
**Net:** the parser/tagger/data path is MAXED; the residual is (a) a gold-cleaning re-scope, (b) a copular is-a
binding, (c) a ~50/50 composition lever on 15% of items -- named + bounded below, none of it this problem's
mechanism. Do not expect the 19c number to jump; most of the gap is not a solvable mechanism deficit.
