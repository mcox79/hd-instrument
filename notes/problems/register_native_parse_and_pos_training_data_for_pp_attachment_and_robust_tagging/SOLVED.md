...
- **PROTOTYPED, TO SEE THE EFFECT (`exp_19c_thematic_fit_reestimation_prototype_v1`; hand-off artifact for the
  ...
  `exp_pivot_selectional_knowledge_richness_2afc_v1` "KNOWLEDGE_POVERTY_WAS_THE_WALL".
- **SECOND PROTOTYPE increment -- RICHER REPRESENTATION eliminated too (`exp_19c_distributional_thematic_fit_
  prototype_v1`):** built a register-native 100-d PPMI-SVD distributional space over 19c exposure (reusing
  `hdlab.distributional_meaning_channel.ppmi_svd`) and re-ran verb-role thematic-fit selection with it (n=1191,
  coverage 0.71). It ALSO ties its twins: DIST vs verb-shuffle +0.008 CI[-0.024,+0.040] ns; DIST vs the 12-d
  grounded +0.001 ns; loses to position (-0.087). So a richer representation is NOT the wall either. Of the drill's
  three candidate ingredients, the two TRACTABLE ones (re-estimation, richer representation) are EMPIRICALLY
  ELIMINATED here; the remaining one is **COMPOSITION** `P(patient|agent,verb)` -- consistent with Bicknell/Chersoni
  (a marginal verb-role store is a bag-of-arguments model, at chance on the compositional items). CONFOUND (honest,
  drill-flagged): these 19c prototypes are graded on the CONTAMINATED who-did-what gold (many "patients" are
  PP-obliques, so a verb->PATIENT fit is the wrong cue), so the DEFINITIVE test of composition needs a cleaned
  direct-object gold FIRST. That (cleaned gold + a compositional model) is the owned selection problems' substantial
  build (drill P(success) ~0.5). WHAT THIS SESSION ESTABLISHED: the 19c selection lever is NOT re-estimation and NOT
  representation -- narrowing the owned build to composition + gold-cleaning.
...
## QUESTIONS
- Do you still want the one route I did NOT exhaust -- hand-building a small gold 19c parse corpus and training on
  it -- given the diagnosis caps its ceiling at ~8% of the reachability residual and it does not touch the
  selection bottleneck? My recommendation is NO; I flag it because the brief explicitly invited building gold.
