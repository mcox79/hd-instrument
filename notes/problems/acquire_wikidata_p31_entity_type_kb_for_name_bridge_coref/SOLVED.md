...
## 4. PERFORMANCE vs THE BRAIN + the mechanism-diff (where we lose signal)
The operation is right (license-by-type, recency-select; two CLS routes). Where a competent reader beats us is pure
KNOWLEDGE BREADTH + granularity: a human's semantic memory types Dvorak (composer), the obscure saints, and the
legal parties; ours (this DBpedia snapshot) does not (168 misses). Of those misses, the CLS lens splits them
honestly: (a) FAMOUS-but-KB-incomplete (Dvorak) = a coverage-SCALING gap (a fuller/Wikidata KB closes it -- s.5);
(b) DOCUMENT-LOCAL / obscure (a random interviewee "Jenna", "R&R Mountain Escapes LLC", legal "Petitioner") = the
brain would NOT have these in semantic memory either -- it binds them EPISODICALLY from in-text apposition, which is
exactly the route that supplies the other 27 wins. So the residual is not a mechanism failure; it is the correct
division of labour between the two routes, with our consolidated store simply smaller than a human's.
