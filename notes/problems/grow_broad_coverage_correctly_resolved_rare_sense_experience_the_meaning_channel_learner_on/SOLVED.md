...
files_changed: "experiments/exp_rare_sense_episodic_vs_prototype_v1.py (STAGE 0: episodic-vs-prototype, gold ceiling); experiments/exp_rare_sense_propose_verify_episodic_v1.py (STAGE 1: vanilla-margin PBV located negative); experiments/exp_rare_sense_pbv_v2_brain_faithful_v1.py (STAGE 1.5: the brain-foundational PBV-v2 upgrade); experiments/exp_rare_sense_coverage_growth_v1.py (STAGE 2: external-corpus coverage growth + consolidation gate + pw deploy); experiments/exp_rare_sense_all_brain_faithful_v1.py (STAGE 3: every component faithful); experiments/exp_rare_sense_full_chain_signal_loss_v1.py (full upstream->downstream signal-loss trace + coarse/human-like scoring); experiments/exp_rare_sense_polysemy_homonymy_v1.py (the brain-faithful polysemy/homonymy inventory split -- 71% real homonymy vs 29% polysemy artifact); experiments/exp_rare_sense_selectional_pruning_v1.py (selectional/thematic-role pruning -- located negative, dominant-biased prior); experiments/exp_rare_sense_recurrent_competition_v1.py (the drill's fix: Bayesian log-prior + strong-context readout, honestly diagnosed as a weighted sum not recurrence); experiments/exp_rare_sense_best_readout_v1.py (DEV-SELECTED stacked best readout: precision + Bayesian, +0.072 CI-sep on the rare tail, Pareto win over the wired readout); experiments/exp_rare_sense_ideal_full_solution_v1.py (the assembled IDEAL pipeline S1->S4; episodic redundant over the readout); experiments/exp_rare_sense_ideal_generalization_v1.py (frozen-weight generalization: holds CI-sep 6/6 across POS/frequency/fresh-split); experiments/exp_rare_sense_cls_rollback_safety_v1.py (cls_growth safe-growth reuse); experiments/exp_rare_sense_trace_sharpening_v1.py (trace-sharpening optimization probe); verification/test_rare_sense_episodic_coverage_growth.py (26-check witness)."
...
## DRILLING THE SELECTIONAL NEGATIVE -> A NEW READOUT WIN (`exp_rare_sense_selectional_pruning_v1.py`, `exp_rare_sense_recurrent_competition_v1.py`)
Pursued the named opportunity (selectional/thematic candidate pruning). A learned P(sense-supersense | thematic-role)
prior HURT the rare tail (context 0.315 -> +selectional 0.260, and a SHUFFLED-ROLE twin BEAT it) -- a role-frequency
prior is the dominant-sense bias by another name, so it fights the subordinate tail. An aggressive drill (MacDonald 1994;
McRae competition-integration; Rodd settling; Erk exemplar selectional prefs) named the precise mechanism-diff: we
resolved senses by ADDING a marginal prior in a feed-forward pass; the brain enters frequency as a DECAYING INITIAL
STATE and lets context/thematic constraints WIN BY COMPETITION. Building the brain-faithful version recovered signal:
**BAYESIAN sense selection -- argmax(log P(sense) + w * diagnostic-context), frequency as a weak log-prior/resting bias
(suppresses implausible ultra-rare senses) with STRONG context deciding -- lifts rare-sense a_s from 0.316 (feed-forward
readout) to 0.360, +0.043 CI-separated, coarse 0.499 -> 0.544; the shuffled-context twin LOSES CI-sep (0.237); robust
across the context weight (K=2-8/gain=1-3, +0.024 to +0.045).** HONESTY: my "recurrent competition" loop with fixed
support is ALGEBRAICALLY this weighted sum (confirmed: RECUR_ctx == WSUM exactly); GENUINE nonlinear feedback recurrence
(lateral inhibition) is WORSE (0.343, over-competes); and the episodic exemplar added INSIDE the loop HURTS (0.294 --
over-amplified coverage-asymmetric noise). So the win is the Bayesian log-prior+context readout (the brain's
frequency-modulated competition, linearized), NOT nonlinear recurrence. It is a READOUT gain (bigger than P9's
precision-weighting +0.023); it does NOT cross the input-representation wall (still 0.36 fine / 0.54 coarse), but it is
landable and it correctly demotes frequency from an additive vote to a resting bias.

## THE SYNTACTIC-FRAME CUE -- RESEARCHED AND SKIPPED WITH A NUMBER (not laziness)
The last named readout lever (Gleitman syntactic-frame / subcategorization as a rare-sense selector). Researched
quantitatively (Roland-Jurafsky; McCarthy-Carroll; WSD feature-ablation): it is a VERB-ONLY, high-precision /
very-low-recall SCALPEL -- verbs average ~2.2-2.7 frames but more senses, so a frame is diagnostic only for the SUBSET
of senses that OWN a distinctive frame; used as a prior it is Zipfian/dominant-biased (would REPEAT the role-prior
failure); selectional-preference cousins score 37% on polysemous and do NOT beat first-sense; nouns are resolved by
topical context, not syntax; expected aggregate impact <1%. DECISION = SKIP, on evidence: our subordinate test
population is NOUN-MAJORITY (59.7% nouns, 40.3% verbs), the lever is verb-only, the recs carry no dependency parse to
extract frames from, and the gain would be <1% aggregate on a lever the readout (Bayesian+precision) and the input
wall already dominate. This CLOSES the readout-lever search: flat -> precision (P9) -> Bayesian log-prior is the
maximized glass-box readout; selectional-role and syntactic-frame are researched located negatives/skips; the residual
is the input representation (the §2 boundary).
...
## THE IDEAL FULL BRAIN-FOUNDATIONAL SOLUTION (SYNTHESIS) + GENERALIZATION (`exp_rare_sense_ideal_full_solution_v1.py`, `exp_rare_sense_ideal_generalization_v1.py`)
Assembled the winning components into one pipeline, each stage the brain's actual operation, dev-selected, traced:
S1 ATL hub-and-spoke keys 0.314 -> S2 LIFG/pMTG precision retrieval 0.336 -> **S3 Bayesian competition (log-prior as a
DECAYING RESTING BIAS + strong context; MacDonald/McRae linearized) 0.387 [THE IDEAL] -> S4 + CLS episodic memory 0.379
(REDUNDANT, -0.008 n.s.)**. Coarse 0.493 -> 0.569; ideal>wired +0.065 CI-sep; twin loses; polysemy coarse 0.848
(near-solved), homonymy the real residual (0.383 fine). KEY SYNTHESIS FINDING: the CLS episodic memory -- a proven
better mechanism in isolation (STAGE 0) -- is REDUNDANT over the optimal readout, because episodic max-echo and the
context readout extract the SAME frozen-w2v signal; so the readout IS the glass-box ceiling and adding mechanisms does
not help. This is the cleanest statement of the §2 wall: every glass-box operation reads the same thin input.
GENERALIZATION (FROZEN SemCor-dev weights, ZERO re-tune): the ideal readout beats the wired readout CI-sep in 6/6
independent conditions -- NOUN +0.052, VERB +0.047, HIGH-skew-freq +0.059, LOW-skew-freq +0.041, a FRESH mod-3 document
split +0.037, ALL +0.050 -- so it is not a tuned-split artifact; it holds across word class, frequency band, and unseen
documents. (Caveat: all within SemCor/one sense inventory; cross-corpus WiC/SemEval is the strongest untested next.)

## FOR STRATEGY -- a LANDABLE readout win, DEV-SELECTED (independent of the §2 outcome) (`exp_rare_sense_best_readout_v1.py`)
The Bayesian log-prior + strong-context readout (`argmax(log P(sense) + w * precision_weighted_context)`, frequency as a
weak RESTING BIAS that regularizes against implausible ultra-rare senses -- NOT an additive dominant-forcing vote),
STACKED on P9's precision-weighting. Weights (gamma=2.0, top-k=0, w=5.0) DEV-SELECTED on the EVEN-doc split, reported on
ODD-doc test (no test-tuning). Traced chain on the rare tail: A0 flat 0.316 -> A1 +precision 0.338 (+0.023, reproduces
P9) -> **A2 +Bayesian 0.387, +0.072 over the wired readout CI-sep [dev-selected]**, coarse 0.495 -> 0.569;
shuffled-context twin LOSES (0.228). CONSUMER (all sub+dom population): the wired context-only readout is only 0.456
(it ignores the prior); A2 = 0.629 (+0.172, a strict PARETO win over the wired readout). HONESTY: pure MFS gets 0.683
all-pop / 0.000 tail -- so A2 sits BELOW pure-MFS on all-pop by 0.054 (it trades a little head accuracy for a large,
REAL tail capability -- the tail gain beats the shuffled twin, so it is context signal, not a prior shift). For a
MEANING channel where rare senses are the goal, A2 is the right operating point; a caller wanting max all-pop can lower
w toward MFS. PROPOSED wire: add optional `sense_prior` + `prior_weight` to `hdlab/diagnostic_context_wsd` (default
prior_weight=0 = byte-identical); combine in log-space. Bigger than P9's precision-weighting and STACKS with it.

## FOR STRATEGY (the episodic/coverage wire -- a PROPOSED change, NOT landed; per §2 outcome)
