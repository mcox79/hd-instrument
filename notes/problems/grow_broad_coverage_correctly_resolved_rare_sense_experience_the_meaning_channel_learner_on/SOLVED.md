...
## FOR STRATEGY -- a LANDABLE readout win (independent of the §2 outcome)
The Bayesian log-prior + strong-context readout (`argmax(log P(sense) + w * diagnostic_context)`, frequency as a weak
resting bias not an additive vote; w in K*gain ~= 4-8) is a real, controlled, brain-faithful READOUT improvement over
the currently-wired feed-forward biased-competition: rare-sense a_s 0.316 -> 0.360 (+0.043 CI-sep), coarse 0.499 ->
0.544, shuffled-context twin loses, robust across w, no MFS/dominant regression (it only sharpens the tail). PROPOSED
wire: add an optional `sense_prior` + `prior_weight` to `hdlab/diagnostic_context_wsd` (default prior_weight=0 =
byte-identical) so a caller with a frequency prior can combine it in log-space with the context score. Bigger than P9's
precision-weighting (+0.023) and stacks with it. Dev-select w on a held-out split (do NOT test-tune).
