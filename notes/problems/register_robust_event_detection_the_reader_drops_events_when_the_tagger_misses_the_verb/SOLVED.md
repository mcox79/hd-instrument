...
Reverified first-hand: `verification/test_register_predicate_detector.py` **12/12** (modern 0.899 / 19c-transfer 0.5625 @ FP≤0.5, info-free twin loses CI-sep on all registers, ZERO 19c labels, crosses the parent's 0.16 modern wall; the additive/no-regression + CRF axis-1 rungs all pass). Bar met the brain-foundational way (a LEARNED noisy-channel combination, not a heuristic; register-invariant, transfers modern→19c). Actions:
- **WIRE LANDED (Q111, default-off `predicate_recall`):** shipped the static asset → `data/frontend_assets/predicate_detector_ud_qasrl.json`; promoted the scoring VERBATIM (feats_parsefree / verb_margin / morph_finite + has_verb_reading / frame_verb_cue) → **`hdlab/predicate_detector.py`** (`PredicateDetector`, pure-Python logistic reproducing sklearn predict_proba to 1e-16, NO experiments/ dep, NO LLM — WordNet lexical gate only); wired an ADDITIVE `predicate_recall` flag into `situation_reader._tense_agnostic_extract` (both branches) that fires an extra event for each tagger-dropped non-VERB non-AUX token the detector promotes, then re-sorts by token index. Default OFF = byte-identical. Landing witness **`verification/test_predicate_recall_landing_organ.py` 15/15** (byte-faithful scoring; rescues 'presents', rejects noun-flanked distractors; flag-ON strict superset of flag-OFF; normal sentence byte-identical; factory-covered; end-to-end read() additive 219→346 events on real 19c prose). Registered `predicate_recall_wire_v1`.
- **TURN-ON MEASURED → KEPT DEFAULT-OFF with a MEASURED reason (no-default-off):** on the board's 16 LitBank docs, ON vs OFF is **who-did-what (events) 0.2519→0.2530 (+0.0011, n=1830 → NOT CI-separated = flat), coref questions 0.5688→0.5688 (+0.0000), sm.coref_acc 0.5255→0.5255 (+0.0000)** — ZERO consumer regression (the additive-only property holds through the consumers; unlike referent-per-NP which crashed coref, the extra records are EVENT predicates not new coref mentions). The gain is FLAT because the who-did-what eval SUPPLIES the clean main-verb slice, hiding the recovery (the exact defect this problem names); meanwhile at the modern-calibrated threshold it fires ~1.4 FP/sent on the ~76%-contaminated 19c gold (346 vs 219 events/doc), above the 0.5 precision budget. So default-ON gives no measured board benefit for real event-stream noise → KEEP OFF; the value is FREE-TEXT event-recall deployment (no live board consumer measures it yet, like bind_entity_states→P3 / referent_per_np→P5). Turn on when a free-text event-recall consumer exists.
- **§2b AUDIT UPDATE folded** (newest entry): the LEARNED noisy-channel predicate detector; the parent's "modern override 0.16" was a SINGLE-CUE artifact not a fidelity ceiling; the deeper-fidelity target = the JOINT-DECODED likelihood-trained POS+parse (CRF objective, +0.224 19c prototyped).
- **UPSTREAM IMPROVEMENT FILED as a problem** (per owner "post as problems where reasonable"): the CRF calibrated-posterior tagger + the joint-decoded POS+parse ("one lever, two payoffs" — fixes the tagger's 19c verb recall AND the parser's register-brittle coherence cue) → `notes/problems/upgrade_the_pos_tagger_to_a_calibrated_joint_decoded_posterior...`.
- **DID NOT land the CRF upgrade inline:** it needs a crfsuite runtime dependency + swaps a SHARED load-bearing component (every tagger consumer), so it is a measured effort (the filed problem), not a heartbeat swap — v1 (the validated, dependency-free asset) is what landed.
