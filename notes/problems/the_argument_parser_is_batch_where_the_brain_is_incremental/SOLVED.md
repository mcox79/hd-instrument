...
Re-verified scaffold-free FIRST-HAND (verify_incremental_argstruct_builder.py PASS: incremental beats batch +0.025-0.035
CI-sep; twin loses -0.145; beats positional +0.031; prefix-consistent 0.989 vs 0.943; glass-box no-heads). The
incremental left-corner builder (eager verb-slot projection, bounded Now-or-Never, NO arc graph) beats the batch UD
parser at candidate-argument identification (+0.0352 F1, n=28,149) via a precision gain (batch over-generates +1.03
args/predicate). HONEST attribution: the win is the EAGER BOUNDED attachment (good-enough), NOT prediction (+0.0007 NS)
or revision (-0.0101; but revision IS brain-faithful -- garden-path positive control +0.0852, zero false-fire). Downstream:
batch parse doesn't earn its place for word-order role assignment; structure helps on passives (+0.0344). hdlab landing
EARNED -> QUEUED proven-ready (new incremental-builder organ replacing candidate_generator's arc-parse candidate source
behind a flag; role assigner unchanged, Beber 2025 structure/role separation; revision default-OFF; route to relcl).
AUDIT UPDATEs folded (§2b + Tier-1 batch-parse-REPLACEABLE-as-candidate-generator + the substrate-wide DISCRETE->GRADED
deviation, honestly bounded as a canonical-English task ceiling + a p1 signal-quality symptom). SUCCESSOR packaged =
`discrete_where_the_brain_is_graded_in_parsing_and_role_assignment` (the cross-organ graded cue-based competition gap;
test on non-canonical/ambiguous populations). Review EXCELLENT + SOLVER REVIEW in PROBLEM.md; priority cleared. Committed.
