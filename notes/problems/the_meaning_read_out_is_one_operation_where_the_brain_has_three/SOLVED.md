...
- **Dimension/standard SELECTION is not built** -- I test per-NAMED-dimension recovery, not online selection of
  the relevant scale for an arbitrary pair. That is the semantic-control organ's job (a buildable gap).
- **The denotational non-affective scales** (size, temperature, speed) are tested only via concreteness on disk;
  the pure gradable-denotational crowd-ordered golds (Wilkinson/CROWD, WordNet-independent) are the named
  follow-up.
...
8. **NEW (deeper drill, method caveat): rating-recovery rho is BLIND to log-compression + tuned coding** (monotone
   invariance). The brain's tuned/Weber code and the pole-anchored reference-point comparator (congruity,
   markedness-RT) are real fidelity gaps that need a DISCRIMINATION/comparison benchmark to score -- flagged, not
   built. Record so the audit does not read rho saturation as full fidelity.
...
10. **NEW (deepest drill, PROBE E): the brain-foundational magnitude CODE is FPE(log degree) in FHRR, and the
    reference-point comparator is a native UNBIND.** The substrate's Fractional Power Encoding (`hdlab/binding`;
    `hdlab/quality_relation` Ch. B) is currently LINEAR = uniform-resolution = the wrong code. Encoding LOG(degree)
    yields the tuned Weber number-neuron code (scale-invariant kernel; log-Gaussian tuning) -- proven on-substrate
    (`exp_fpe_log_weber_magnitude_v1.py`, all gates pass). The reference-point comparator (drill-2 FILED gap) is
    `unbind(FPE_log x, FPE_log ref) = FPE_log(x/ref)`, decode corr 1.000, directional. Record: magnitude should be
    a composable hypervector, not a flat scalar; upgrade Ch. B linear->log + grounded degree; rho is
    monotone-BLIND to the code (validate on confusability, needs per-worker comparison data).
...
test (recover the ruler position, don't compare pairs) had thousands of items sitting on disk. Two bonus
findings made the answer better than asked: describing-words are really three jobs (a ruler for gradable ones, a
feeling-tone map for good/bad ones, and the plain noun tool for words like "wooden"), and verbs were already
handled -- so the one operation the reader is truly missing is the adjective ruler.
...
7. Encode the DEGREE as FPE(LOG(grounded degree)) in the FHRR substrate (PROBE E, proven): the tuned Weber
   number-neuron code, magnitude as a composable hypervector, comparator = `unbind`. Upgrade `hdlab/quality_
   relation` Ch. B from linear->log + a grounded-degree lexicon (Warriner extremity / Lancaster / log-frequency)
   in place of the 23-word hand axis. The ENCODER + COMPARATOR are built as a substrate proof AND VALIDATED
   against human behaviour (PROBE H: the FPE-log kernel predicts human number-comparison RT/error at rho 0.96/0.92,
   beats a difference-based kernel CI-separated, size effect holds) -- no gap remains data-blocked.

---

## INTEGRATED_BY_STRATEGY (2026-08-27)

**Grade: EXCELLENT** (exceptionally deep, honest, well-controlled). Re-verified FIRST-HAND (strategy ran
`verification/verify_perclass_meaning_operations.py` -> ALL CHECKS PASS): adjective signed-magnitude valence rho 0.698 vs
incumbent 0.117 (+0.58 CI-sep) / random 0.048 (+0.65 CI-sep); dominance CI-sep; per-class specificity (nouns 0.599 +
verbs 0.492 win the cosine, adjectives 0.479<0.585 lose); Moyer distance effect (far 0.911 > near 0.593); verb relational
op beats the blended cosine +0.27, shuffled twin losing. Bar MET -- the adjective op CI-separates at power on an
INDEPENDENT non-WordNet human gold, resolving the n=111 power wall. Argument adversarially audited and holds: the op is
brain-PINNED (Walsh ATOM signed magnitude; opposition anchored by the explicit antonym relation); it REFINES the brief
(the cosine is wrong for adjectives ONLY -- verbs win with the gloss); the deep probes (A-H) are exceptional and each
controlled (opposition relational not geometric; ATOM single-axis refuted; perceptual grounding doubles concreteness;
intensity is markedness; the magnitude CODE is FPE(log degree) validated against 240k human number-comparison trials, the
log PINNED by Laughlin efficient coding); exemplary self-correction (caught a signed-rho orientation bug; honest
negatives -- VerbNet does not beat the gloss, pairwise similarity is the wrong currency).

**hdlab:** NO file landed (Q111 honored). **EARNED proven-ready (a substantial future build):** (1) a new
`hdlab/scalar_adjective_operation.py` = per-dimension bipolar signed-magnitude axes (offline static asset), EVALUATIVE
dims (valence/dominance) anchored from explicit WordNet antonym poles, DENOTATIONAL dims (concreteness/size) from
Lancaster PERCEPTUAL strength (doubled concreteness recovery 0.26->0.53), axes INDEPENDENT per dimension (ATOM single-axis
refuted), OPPOSITION read from the explicit relation (not the projection), fine DEGREE/INTENSITY from MARKEDNESS
(frequency/AoA), encoded as FPE(LOG(degree)) in the FHRR substrate (the tuned Weber code; comparator = `unbind`); (2)
OPERATION-ROUTE the meaning read-out by word class with a can-fail gradability gate -- noun/verb/classificatory-adj stay
on the gloss (already correct), gradable/evaluative adj -> the magnitude op; (3) wire dimension SELECTION to the
semantic-control (context-override WSD) organ (PROBE G, built + validated); (4) upgrade `hdlab/quality_relation` Ch.B
linear->log + a grounded-degree lexicon. Do NOT replace the verb gloss op with VerbNet (net-neutral). review: +
review_text: + SOLVER REVIEW written to PROBLEM.md; priority cleared; AUDIT UPDATE folded into BRAIN_FOUNDATIONAL_AUDIT.md
(§2b). Committed (no push).

**Consolidation status:** p3 (meaning operation-routing) integrated -- 2 of the 3 parallel solvers now in (p1 front-end +
p3 meaning). p2 (entity store) submitted, awaits owner_verdict: DONE. The EARNED hdlab landings queued: the p1
`graded_role_assigner` (front-end) + this p3 `scalar_adjective_operation` + operation-routing.
