...
## 11. THE PRODUCT ROWS -- EXACTLY UNCHANGED, AND I VERIFIED THAT IS NOT AN ARTEFACT

pri 122's reader-driven cell (`exp_board_rows_on_the_reader_v1.run_ud`, **unmodified, called as a library so its
own landed directory is never written**), both arms in ONE PROCESS, 600 UD-EWT test sentences read through
`SituationReader.read`:

| row | n | shipped | patched | delta | its strongest floor | its twin |
|---|---|---|---|---|---|---|
| who_did_what_agent | 512 | 0.7363 | 0.7363 | **0.0000** | 0.7969 | 0.2559 |
| who_did_what_patient | 401 | 0.6808 | 0.6808 | **0.0000** | 0.6384 | 0.1970 |
| state | 110 | 0.7455 | 0.7455 | **0.0000** | 0.4909 | 0.6818 |

Identical under BOTH provenances the cell publishes (`reader_textonly` and
`reader_textonly_pron_discovered`). **The board is NOT DOWN. It is also not up, and three exact zeros are exactly
the shape that should be distrusted, so I checked the obvious failure first.**

**THE SWAP DOES REACH THE READER -- verified, not assumed.** With the patched organ bound onto the live
`hdlab.attachment_arm` and the frontend parser rebuilt, `hdlab.frontend.parser()._AA is hdlab.attachment_arm` is
True, its table carries the new cue (`'iexp' in _tab['strength']`), and on
*"I have a plan to leave the city and a chance to send a draft today ."* the live parse CHANGES exactly as
intended: `leave` moves from `have` to **`plan`**, `send` moves from `leave` to **`chance`**, and `plan` moves
from under `leave` to under `have`. And `run_ud` constructs a FRESH `SituationReader` per chunk per mode, so arm
B is not reusing arm A's read.

**So the zeros are a real fact about the reader, and it is the same one pri 117 recorded one arc over: the
who-did-what rows iterate GOLD VERBS and the state row reads the copular binding path, so an infinitival
governor never enters any of the three.** This rung's value is real and currently **board-invisible**, which per
the project's own discipline is an argument for an instrument arm, not for withholding the rung.

⚠️ **AND THE HONEST CAVEAT: this is a 600-sentence CAPPED board, which is a CONTROL and not a no-regress
verdict.** Capped exact zeros are underpowered by construction. What it does establish is that the change does
not move these rows at this size in either direction, and it took 741 s for both arms; a full-size, both-arms
board is strategy's call at integration.

## 11b. A DEFECT IN MY OWN CELL, CAUGHT BY RE-RUNNING THE HEADLINE

Recorded because it is exactly the class of error this project keeps paying for. The accrual arm sets
`HDLAB_ARM_INFIN_CTX=1` so the new asset CARRIES the refuted channel's cells (making that switch measurable
without a re-accrual). It did not restore the variable, so running `--accrue --live-ab` in ONE process leaked the
refuted channel into the A/B and printed **0.7745 -> 0.7359 with advcl_purpose 0.5915** -- the refuted arm's exact
profile, reported as if it were the shipped form. The override is now scoped to the module the accrual loads and
restored immediately, and the headline re-runs to the digit from a FRESH accrual in the same process
(0.6766 -> 0.7745, +0.0979 CI[+0.0564,+0.1381]; 5,767 lexical keys, 45 `iexp` cells, 28 `islot` cells, all
regenerated deterministically). **The lesson for the next cell: an env override used to build one arm must be
restored in the same function, because every later `patched_module` in the process reads it at import.**
...
## 20. SIX OF THE REMAINING `acl_nominal` MISSES, BY NAME (so the next build is not guessing)

| sentence (truncated) | the infinitival | the gold governor | what the organ did | why |
|---|---|---|---|---|
| "... is the fact that the political **will** to end the crisis ..." | end | **will** (UD NOUN, the organ reads it **AUX**) | chose `fact` | the CATEGORIES rung: the lexical key became the auxiliary lemma, whose expectation is low |
| "The **decision** to sidestep the obvious ..." | sidestep | decision | chose ROOT | the competition preferred the gold and the decode still lost it (1 of the 7) |
| "... has cell , car **numbers** to reach me ." | reach | numbers | chose ROOT | out-scored by a PROPN |
| "... whether you will have a **chance** to send a draft ..." | send | chance (expectation 0.574) | chose `have` | out-scored by the adjacent VERB's frame + locality |
| "... I 've taken **liberty** to turn them into ..." | turn | liberty | chose `taken` | the same: a light verb adjacent to the nominal |
| "... looking for an **individual** to join their team ..." | join | individual | chose `looking` | the same |
...
INTEGRATED_BY_STRATEGY 2026-09-16 00:52 local -- DONE by strategy: the 16-hunk diff APPLIED (attachment_arm + every builder call site) with the new asset; reverify on the landed tree: test_attachment_arm 17/17, test_attachment_arm_fastpath, test_attachment_coordination 6/6, test_state_qa_consumer_organ, the cell's --self-test 18/18 (data/hook_state/pri133_witnesses.log); the product board pri133a. pri133a vs pri131a: headline 0.5088 == 0.5088, component 0.6201 == 0.6201, every model row byte-identical; twin controls moved only. Phase-7 probe outstanding (purpose hand-off; the decode gap measured on all arcs; posterior retrieval; lexical reallocation) -- its results land as a follow-up commit.
