...
status: PARTIAL
round_2: "Supervisor probe answered (sections 10-21). The chain is now understood arc by arc: of the 1160 core arguments the live governor mis-attaches 286 -- 12.4% wrong CLAUSE (105 under another predicate + 38 flung to root) and 10.3% a CONSTITUENCY error (101 absorbed into a nominal phrase + 18 under a function word), near-equal halves needing different fixes. The head-repair curve prices the requirement: this rung's win becomes CI-separated when the attachment arm reaches ~0.95 on core-argument arcs (it is at 0.753). FIVE opportunities built and measured, ALL REJECTED, each with its mechanism: (1) growing the counts by reading simplewiki treebank-free with a 0.893-precision agreement teacher looked like a large win (live subjects +0.0197 CI-sep) until its info-free twin -- the same mass on RANDOM clause-local heads -- MATCHED and at x2 BEAT it, proving a single-role teacher can only move the prior (the twin costs the balanced held-out metric -0.0183 CI-sep); (2a) replacing the MAP label with the graded posterior at the board agent read -0.0204 CI-sep (the MAP abstention carries competence information); (3) both precision gates swept in full, 0.5/0.5 confirmed the joint optimum and the matched-threshold hypothesis refuted; (4) the slot competition -0.0302 CI-sep live, board patient 0.7936->0.7857, byte-identical on the agent read; (5, mine) hierarchical configuration backoff, which turns the only non-positive gold population positive but costs the balanced metric -0.0323 CI-sep. ACCEPTED as a finding, not a change: (2b) the role competition's own margin separates right from wrong at AUC 0.716 with 0.99 accuracy at 20% coverage and 0.92 at 40% against a 0.755 base -- an unexploited precision signal no consumer reads. NO SHIPPED NUMBER CHANGED in round 2."
...
reverify: ".venv/Scripts/python.exe experiments/exp_role_competition_voice_embedding_arguments_v1.py --self-test   (then --cache once, then --run --ablate, --agent, --board-patient). ROUND 2 additions are in sections 10-21 and in data/exp_role_competition_voice_embedding_arguments_v1/{head_repair_curve,copular_repair,config_backoff,grow_metrics,grow_twin_metrics,posterior_consumer_metrics}.json"
---
