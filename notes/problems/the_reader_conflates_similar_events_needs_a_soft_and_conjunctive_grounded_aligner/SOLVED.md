...
- **MEANING-CHANNEL FIX PROTOTYPED (exp_atl_hub_mention_resolution_mcscript_v1.py) -- a reusable brain-fidelity
  CORRECTION, but it does not break the ceiling.** Built the brain-faithful event-mention resolver as an ATL
  hub-and-spokes FUSED concept similarity (lexical/derivational + sensorimotor-grounded + WordNet-taxonomic), used
  per-role inside the conjunctive aligner. CONCRETE, LANDABLE FINDING: for event-IDENTITY resolution the TAXONOMIC
  spoke (WordNet wup + derivational, which solves nominalization order<->ordered and synonymy check<->identification
  0.71) beats the incumbent SENSORIMOTOR-only kernel (story-own-order resolution proxy 0.551 vs 0.532, n=301) --
  and a naive MAX-fusion is DRAGGED DOWN by the grounded spoke's FALSE POSITIVES (the "wire-rotate"->"want-bag"
  spurious cosine from the trace). So the grounded/sensorimotor spoke is the WRONG spoke for event IDENTITY (it is
  for perceptual similarity; the apple/orange confound in grounded_similarity's own docstring); the aligner's per-slot
  kernel should be TAXONOMIC-primary, grounded-as-OOV-fallback. BUT even taxonomic resolution is +0.019 (not CI-sep)
  and caps ~0.55 -- confirming the ceiling is NOT primarily resolvable by a better meaning kernel; the residual
  ("ask for IDENTIFICATION"=="check his age/license"; "wire ROTATE"=="watched it DROP") needs CONTEXTUAL/WORLD
  knowledge (frame/script inference) the glass-box substrate's BROKEN meaning channel does not supply.
