...
INTEGRATED_BY_STRATEGY 2026-09-14 16:41 local -- DONE by strategy after first-hand reverify: the full diff applied (thematic_role_labeler is_passive_predicate + counts asset; graded_role_assigner 5 call sites; relcl_resolver precise_passive -> the organ (strict superset); arc_labeler D5 deleted; the board arm's duplicate hybrid retired; the experiment twin repaired). Witnesses: byhead landing 15/15 (repaired by strategy: it had crashed on a tuple return since 09-06), coarse_role_competition 37/37, graded_role_assigner green, relcl_resolver green; FULL BOARD A/B (pri110a -> pri111a): who_did_what_agent 0.8271 -> 0.8559 (floor 0.8468: CI-SEP ABOVE ITS FLOOR for the first time), patient 0.8120 -> 0.8151, five dims identical, AGG 0.6152 -> 0.6191. Method finding recorded in memory (fold a detector onto one organ -> check the construction's own population; strict superset). Leads routed: pri 115 (by-NP head), the participle/adjective boundary + D3 voice_cues (briefs to file), validity re-accrual -> pri 114.

---

# 21. POST-LANDING (strategy, 2026-09-14 17:05) -- the two PATIENT byte-identity failures are (c), and the invariant is untestable as written

**VERDICT: (c). Neither the agent pick nor the voice cue reaches the patient. Both witnesses compare readers
that run SEQUENTIALLY IN ONE PROCESS against a category organ that carries PASSAGE STATE, so the comparison
measures reading ORDER, not the flag under test.** It is pri 112's effect, and the fold is not implicated.

## 21.1 What actually differs -- 2 rows in 2033 events, and neither is a changed patient

Reproducing `test_cmrole_agent_landing_organ.py` B1 exactly (8 LitBank docs, the witness's own reader
construction and its own read order `base -> off -> on`):

| doc | sent | predicate | patient OFF | patient ON | agent OFF |
|---|---|---|---|---|---|
| `1023_bleak_house_brat` | 59 | `shirking` | `many` | **`<no event>`** | `shirking` |
| `1064_the_masque_of_the_red_death_brat` | 73 | `disgust` | `disgust` | **`<no event>`** | `rumour` |

**TOTAL patient diffs: 2 over 8 docs. Events: 2033 with cm_agent OFF, 2031 with cm_agent ON.** So the
invariant does not fail because a patient CHANGED -- it fails because **two events EXIST in one arm and not
in the other**, and the witness compares per-doc lists of `(sent_idx, predicate, patient)`, which a missing
row breaks. Both are degenerate predicates that should not be events at all: `shirking` is read as a
predicate whose AGENT is itself, and `disgust` as one whose PATIENT is itself. They are gerund/deverbal
nominals -- precisely the tokens whose VERB-vs-NOUN reading a passage-level category prior moves.

## 21.2 The code path -- and it is NOT the one the probe suggested

I read all three candidate paths and eliminated each:

- **The joint frame-slot decode's passive swap?** No. `SLOT_OCCUPANCY` is `HDLAB_ROLE_SLOT_OCCUPANCY`,
  default `"0"` -- the joint decode is OFF on the live path.
- **The patient route's `precise_passive` widening (D2)?** No, and it cannot be: D2 changes the patient route
  **identically in both arms**. An invariant of the form *ON == OFF* cannot be broken by a change that is
  present in ON and OFF alike.
- **The D5 deletion?** No, same argument, and measured separately: deleting `label_voice_correct` is
  byte-identical on all three label populations (18.2).
- **The agent pick reaching the patient?** **No such path exists.** On the positional route
  (`situation_reader.py:2110-2117`) the patient comes from `_assign_roles(...)` and the cm_agent block
  overwrites `agent` ONLY -- the code even says so: *"PATIENT is left EXACTLY as _assign_roles produced it
  (byte-identical)."*

**THE ACTUAL PATH IS SHARED PROCESS STATE.** `hdlab/lexical_categories.py` keeps PASSAGE-level mutable
state on the organ -- `self._reg` (Heim file cards), `self._reg_gen`, `self._doc_shape` (the passage
register) -- and `posterior(words, observe=True)` WRITES it. That organ is a **process-wide singleton**
(`LC.get()`, held by `frontend.Tagger`). Both witnesses build several readers and read the same documents
through them **one after another in one process**, so every arm is tagged against a register left behind by
the arms before it. The cm_agent stack performs additional tagging/parsing passes (the agent competition's
candidate read, `incremental_subject_before`, `_cached_tag`), so ON and OFF do not leave the register in the
same state -- and the PREDICATE DETECTOR that runs next sees different categories for exactly the borderline
deverbal nominals above.

**Both witnesses have the identical structural exposure:** `test_cmrole_agent_landing_organ.py` runs
`base -> off -> on` over 8 docs; `test_cmrole_agent_board_organ.py` runs five arms
`pos_OFF -> pos_ON -> cm_dense -> cm_ON -> twin_ON` over the same 8 docs and then asserts
`cm_ON == pos_ON` -- the 4th arm against the 2nd.

## 21.3 THE SEPARATION THE PROBE ASKED FOR -- two processes, and the order sweep

**TWO PROCESSES, SAME EIGHT DOCUMENTS, ONE READER EACH -- the invariant HOLDS:**

| configuration | events over the 8 docs | patient diffs vs the other arm |
|---|---|---|
| `cm_agent=False`, its own process | **2036** | -- |
| `cm_agent=True`, its own process | **2036** | **0 -- PATIENT BYTE-IDENTICAL** |

**AND THE CLINCHER -- REVERSING THE TWO ARMS IN ONE PROCESS MAKES THE WITNESS PASS:**

| in-process read order | events OFF | events ON | patient diffs | the witness's own assertion |
|---|---|---|---|---|
| `base -> off -> on` (**the witness's own order**) | 2033 | 2031 | **2** | **FAILS** |
| `base -> on -> off` (**the same three readers, arms swapped**) | **2036** | **2036** | **0** | **PASSES** |
| each arm alone, own process | 2036 | 2036 | 0 | passes |

**The invariant's truth value depends on the order the arms are read in.** That is conclusive: there is no
data path from the agent pick to the patient -- if there were, swapping which arm runs second could not make
the difference vanish. Under the swap both arms return exactly the 2036 events each returns when run alone.

*Correcting myself mid-section: from the first two numbers (2036 alone, 2033 second, 2031 third) I wrote that
the count falls monotonically with position in the process. **The swap refutes that** -- there `on` runs
second and `off` third and both give 2036. The dependence is on the SEQUENCE of configurations, not on depth;
`base -> off` perturbs the register in a way `base -> on` does not. I do not know which read in the `off`
configuration does it and I am not going to guess: that is pri 112's to localise, and the claim I can support
is the one the table states.*

## 21.4 What I propose

**(c), so there is nothing in the fold to repair.** I am proposing no change to `hdlab/`. What needs fixing
is the two witnesses, and there are two honest ways:

**PREFERRED -- make the invariant testable instead of re-pinning it.** The assertion *"the agent competition
must never touch the patient"* is true and worth keeping; it simply cannot be tested by reading documents
through several readers in one process while a process-global organ carries passage state. The fix is one
line per witness: **open a fresh passage boundary before each arm** (`hdlab.lexical_categories.get()
.new_document()` -- the organ already exposes exactly this, and its docstring says a consumer "calls
`new_document()` at each passage boundary"), or read each arm in its own subprocess as I did here. With the
arms isolated the invariant passes at 2036 == 2036, 0 diffs -- and note it also passes, unchanged, if the two
arms are merely SWAPPED, which is how little the witness is really testing today. *I have not written this change: the witnesses
are strategy's and the right form (reset vs subprocess) is a fleet-wide call that also affects the other
multi-arm witnesses.*

**FALLBACK -- if the witnesses stay in one process**, the honest re-pin is not about patients at all but about
order: *"the patient signatures of two arms read in the same process agree on every event both arms detect;
the arms may differ on at most N borderline predicates (2 of 2033 here, both deverbal nominals), and that
difference is the passage register's order-dependence (pri 112), not the agent route."* I would rather not
ship that wording -- it bakes a known defect into a green check.

**WHAT THIS IS REALLY EVIDENCE FOR.** pri 112 is filed as "the category organ's passage register is written
by every consumer's re-tag, out of order", and its cost was attributed on the W3a first-read effect. **Here
is a second, independent consequence with its own number: two landed witnesses cannot state a true invariant,
and the live reader's own event count moves by 5 in 2036 (0.25%) purely with the order its consumers ran in.** That
belongs in pri 112's record, and it is the first evidence I have seen that the effect reaches *which events
exist at all*, not only their features.

**ONE THING I WANT ON RECORD AGAINST MYSELF.** I cannot prove from here that these two witnesses were green
*before* my diff, because I may not revert `hdlab/` to check. What I can prove is the mechanism and that the
mechanism is flag-independent (2036 == 2036 in isolation). If the fold did anything, it moved a borderline
token's category mass and so changed *which* 2 of 2033 events the pre-existing order-dependence tips -- it
did not create a path from the agent pick to the patient, because measurement and code read agree that no
such path exists.
