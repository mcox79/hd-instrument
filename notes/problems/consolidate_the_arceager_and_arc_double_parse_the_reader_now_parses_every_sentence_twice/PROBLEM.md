---
review: EXCELLENT
review_text: Reverified first-hand test_double_parse_consolidation.py -- the SUBSTANTIVE checks pass (W3 the 6 non-front-end dims BYTE-IDENTICAL default==consolidated; W5 copular no-regress modern 1.000/archaic 0.700; W6 space where_is no-regress -0.015 CI incl 0; W8 zero batch parses end-to-end). W1/W4/W7 are premise-STALE post-landing (they assert the pre-landing double-parse still exists; the landing correctly removed it). LANDED (Q111, DEFAULT-ON): hdlab/situation_reader._cached_parse_heads computes the ONE shared per-read parse via arc-eager when parser_arceager (serving the role path AND the front-end); _router_roles reads from that shared cache instead of a separate _ae_parse call; the base ArcParser is never called on the read path (kept loadable as a byte-identity reference). Role heads byte-identical (same arceager weights); only the front-end switches batch->incremental (measured no-regress). ~5% read-cost cut (whole batch parse eliminated); full board zero-regression (confirmed by the post-tier board run). Corrected the fidelity record: arc-eager incremental = PINNED brain-foundational; arc-factored batch = no cognitive correlate, retired from the read path; the stale "arc-eager copular is 19c-negative" claim REFUTED post-robust_cop. Follow-on filed: space_where_is...lazy_locative_pp_bridging (pri 5). §2b folded. INTEGRATED 2026-09-05.
---
