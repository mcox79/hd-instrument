...
- **The core bar is measured at the glass-box `extract_entity_states`/parser level; the CANONICAL-ENTITY read-back
  is now COMPOSED (binding + coref) and measured end-to-end on real LitBank docs** (0.384 cross-sentence
  answerable, token-only floor 0, witnessed I6). What is NOT done: routing it through the LITERAL
  `SituationReader.read()` method -- that needs the hdlab landing of the `entity_states` slot (Q111, strategy), and
  the cross-sentence coverage uses GOLD LitBank coref (the runtime resolver is the glass-box `run_match_or_allocate`,
  which carries its own coref error).
...
## THE FIVE IMPROVEMENTS, ALL BUILT + WITNESSED (owner: "do them all"; then "witness them + compose end-to-end")
All five prototyped brain-faithfully on the ARC-EAGER tree, and now WITNESSED scaffold-free 6/6
(`verification/test_copular_improvements_organ.py`, I1-I6) alongside the END-TO-END read-back. Measured:
...
**END-TO-END READ-BACK COMPOSED (owner: "compose the end-to-end read-back") + WITNESSED.** The binding is composed
with coreference so the is-a/attribute fact binds to the CANONICAL entity, not the surface token -- so "what is X"
is answerable via a DIFFERENT-sentence mention (a later "he"/"the captain"): **143/372 = 0.384 of copular
predications on real LitBank docs become cross-sentence answerable**, where a token-only within-clause reader's
cross-mention floor is **0 by construction**. This is the brain's benefit (bind to the discourse referent, query
by any mention -- Dijksterhuis 2024) delivered end-to-end on real discourse. (Routing it through the LITERAL
`SituationReader.read()` method still needs the hdlab landing of the `entity_states` slot -- Q111, strategy.)
