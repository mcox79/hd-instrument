...
result: "BAR CLEARED (SOLVED). The recommended stack -- CURATED taxonomic sense signatures + SHARED-CORE COARSENING -- as the live sense-selection read-out BEATS the current live PPR select_sense reader on raw WiC accuracy: curated+coarse 0.6639 vs live PPR 0.6006 = +0.0633 CI-sep [+0.037,+0.090] (paired, n=2038 dev+test), the info-free twins LOSE (shuffled-signature +0.031 CI-sep; mis-seeded-context +0.083 dev/+0.093 test CI-sep), and no other dim regresses (coarsening is SCOPED to the same/diff-sense judgment path -- it would hurt fine-sense a_s, so it is NOT applied there). Two corroborations: coarsening ALSO lifts the live PPR reader itself +0.0275 CI-sep (a general read-out win), and curated beats PPR at EQUAL coarsening +0.0358 CI-sep (taxonomic > associative for sense discrimination -- the two-meaning-systems point on a live metric). The exhaustive brain-foundational exploration + literature that led here is below. --- REROUTED (the brief's two named proposals are located negatives; the disk outranks the brief). The curated meaning foundation, wired as taxonomic sense signatures read against context on gold WiC (the board's ONE live meaning-consumed metric, via grounded_semantic_graph.select_sense / PPR), is the FIRST glass-box mechanism to achieve CI-SEPARATED REAL per-context sense discrimination -- it beats its mis-seeded-context twin (WiC dev n=638 +0.0831 CI[+0.038,+0.128]; test n=1400 +0.0929 CI[+0.063,+0.123]; both CI-sep) AND its shuffled-signature info-free twin (+0.0309 CI[+0.019,+0.043] CI-sep), where the live PPR select_sense reader (0.618, real-minus-twin +0.047 NOT-sep) and the gloss/associative baseline (dev +0.0266 NOT-sep) FAIL. Leak-free curated WiC acc = 0.624 (dev) / 0.653 (test) / 0.650 (dev+test), majority floor 0.500. The CURATED KNOWLEDGE is the lever (gloss-only fails the mis-seeded twin on dev; curated crosses it on the SAME split). HONEST NEGATIVES -- FIVE brain-foundational mechanisms tested on the raw-accuracy wall, ALL neutral-to-negative: biased-competition diagnostic readout (neutral, -0.002), ATL distinctive-feature/whitening (hurts monotonically), structured SYNTACTIC context (hurts -0.024, even on long sentences -0.060), GRADED shared-core sense equality (neutral -0.003), grounded-spoke FUSION (hurts -0.039). So the raw-accuracy ceiling (~0.644 leak-free vs 0.80 human) is NOT the readout, the selection structure, the sense granularity, the feature-weighting, or a second spoke -- and it is NOT a trained encoder (retracted; the brain does not use one). Then I TESTED the SUPPLY side too (owner: 'we have a new much larger knowledge store' + 'we've done a ton of work on grounding'): the new 80k x 300-d associative_similarity_store (40M broad tokens) does NOT lift raw WiC (0.639 vs 0.644), NOR leak-free sense-tagged SemCor USAGE (0.60), NOR word-level Binder grounding (hurts), NOR -- the decisive one -- SENSE-TAGGED REAL GROUNDING (per-synset Lancaster sensorimotor + Binder-65 brain-attributes built via WordNet inheritance so bank.n.01[river] and bank.n.09[finance] get DISTINCT grounded vectors, cos 0.21): covered-subset acc GROUND12 0.584 / GROUND68 0.500 / FUSED 0.588, ALL below CURATED 0.617 (the owner's 'missing sense-tagged grounding' hypothesis, built net-new and TESTED -> located negative as a static signature). The frozen asset's 0.72 was pure EXAMPLES LEAKAGE (WiC is built from those sentences), NOT a supply signal (SemCor usage, the leak-free version, does not help). So the algorithm (5 mechanisms), every static supply (richer distributional, word-grounding, sense-usage, sense-tagged real grounding), AND a dynamic Kintsch Construction-Integration reader (built; 0.60 < static 0.644) all cap ~0.644 -- EXCEPT the fix an ERROR ANALYSIS diagnosed. Errors are dominated by OVER-SPLITTING (gold-SAME acc 0.494=chance; false-diff 0.253 >> false-same 0.103; polysemy 0.544 << homonymy 0.622): WordNet's senses are finer than the WiC judgment, so same-sense uses land on different-but-related synsets. COARSENING WordNet to shared-core senses (predict SAME iff same lexicographer-file/supersense -- brain-faithful shared-core polysemy, Rodd 2002) LIFTS WiC 0.643->0.664 (+0.0206 CI-sep [+0.004,+0.036]), rebalancing gold-SAME 0.494->0.655 -- the FIRST raw-accuracy gain over static, glass-box, no encoder. LITERATURE PLACEMENT (research-confirmed, primary sources): 0.644/0.664 is ABOVE the WiC paper's own non-contextual + sense-embedding baselines (DeConf 58.7 / SW2V 58.1 / w2v-avg 54-59) and IN THE BERT-BASE RANGE (60.2-65.4), glass-box/no-LM -- we are AT/ABOVE the knowledge-based frontier, not failing a wall. The residual to human 0.80 (best-ever machine ST-MoE 77.7) is DEEP CONTEXTUALIZATION (the static->BERT gap; shallow contextual models ELMo/context2vec barely beat static -- only deep transformers jump), the INVARIANT-BOUNDARY owner decision -- NOT a knowledge/grounding gap, and NOT the Kintsch dynamic reader (which is DISCOURSE-level; WiC is isolated single sentences, so C-I is out of scope and lost to static). The dynamic reader is the right architecture for discourse comprehension, not the WiC lever."
floor: "Majority-class floor 0.500 (balanced WiC). Strongest LIVE-READER floor = the live PPR select_sense reader, recomputed PAIRED on the SAME dev+test population (exp_wic_optimization_stack_v1): PPR 0.6006; the recommended stack CURATED+COARSENING = 0.6639 BEATS it +0.0633 CI-sep [+0.037,+0.090] -- the bar is cleared on raw accuracy. Secondary floors (all recomputed per population): curated-EXACT over PPR +0.0432 CI-sep; coarsening over curated-exact +0.0206 CI-sep pooled (dev +0.036 CI-sep, test +0.014 not-sep -- modest, robustly positive); mis-seeded-context twin (curated beats it +0.083 dev/+0.093 test CI-sep, PPR does NOT +0.047 not-sep); shuffled-signature info-free twin (curated 0.644 vs 0.613, +0.031 CI-sep); gloss-only associative baseline FAILS the mis-seeded twin on dev (+0.027 not-sep) -> the CURATED knowledge is the lever."
controls: "(1) mis-seeded-context twin (side-2 sense assigned from a RANDOM other sentence's context) -- the decisive per-context-discrimination control: curated LOSES to nothing / beats it CI-sep on both held-out splits, so the accuracy is genuine context-driven discrimination, not dominant-sense agreement structure (which is what the disk's earlier 'no method beats its twin' negative caught). (2) shuffled-signature info-free twin (curated signatures permuted onto WRONG synsets) LOSES CI-sep (+0.031) -> it is the CORRECT sense<->signature binding, not the machinery. (3) shuffled-diagnosticity twin LOSES (+0.008 CI-sep). (4) LEAK control: the FROZEN asset includes synset.examples() and WiC was built from WordNet/Wiktionary example sentences, so the frozen asset is leak-INFLATED on WiC (0.721 vs the leak-free curated rebuild 0.644) -> the honest WiC number is the leak-free rebuild; the frozen asset's on-WiC number must NOT be quoted. (5) gloss-vs-curated ablation isolates the CURATED KNOWLEDGE as the lever. (6) readout ablation (flat vs diagnostic) and whitening sweep (all-but-the-top k=0..20) both LOCATED-NEGATIVE the a_s-levers on WiC. (7) LIVE-READER control: the recommended stack is compared PAIRED against the ACTUAL live PPR select_sense reader on the same dev+test population (not a proxy) -> +0.0633 CI-sep, and curated beats PPR at EQUAL coarsening +0.0358 CI-sep. (8) NO-REGRESS scoping: coarsening is a same/diff-JUDGMENT read-out rule (the WiC/sense-equality path); it is deliberately NOT applied to fine-sense SELECTION (a_s), where merging senses would hurt -> the curated channel is additive and no other dim regresses. Each control excludes a distinct rival: agreement-structure / machinery / diagnosticity-shape / examples-leak / knowledge-vs-gloss / readout-vs-input / not-the-live-reader / cross-task-regression."
files_changed: "experiments/exp_curated_foundation_wic_v1.py (the leak-free curated-foundation WiC channel + all arms/controls), experiments/exp_curated_foundation_wic_whiten_v1.py (ATL distinctive-feature/whitening sweep -- located negative), experiments/exp_curated_foundation_wic_syntax_v1.py (structured syntactic/local context -- located negative), experiments/exp_curated_foundation_wic_graded_v1.py (graded shared-core sense equality -- located negative), experiments/exp_curated_foundation_wic_supply_v1.py (RICHER SUPPLY test: the new 80k/300d associative store + sense-usage -- located negative), experiments/exp_curated_foundation_wic_grounded_v1.py (SENSE-TAGGED REAL GROUNDING: per-synset Lancaster+Binder+VAD via WordNet inheritance -- built net-new, located negative on WiC), experiments/exp_wic_construction_integration_reader_v1.py (the DYNAMIC COMPOSITIONAL reader: Kintsch Construction-Integration settling -- built + tested, does not beat static on WiC), experiments/exp_freeze_sense_grounded_store_v1.py (+ data/exp_freeze_sense_grounded_store_v1/sense_grounded_signatures_v1.npz: the PERSISTED sense-tagged grounded foundation asset, 54,300 synsets, owner-flagged resource), experiments/exp_wic_error_analysis_v1.py (the DIAGNOSIS: over-splitting/granularity is the wall), experiments/exp_wic_sense_coarsening_v1.py (the WIN: shared-core coarsening +0.0206 CI-sep over static), experiments/exp_wic_optimization_stack_v1.py (the BAR-CLEARING stack: CURATED+COARSENING beats the live PPR reader +0.0633 CI-sep, n=2038), verification/test_curated_foundation_wic.py (scaffold-free witness 6/6, incl. coarsening), notes/problems/<slug>/RESEARCH_wic_sense_discrimination_neuro.md (brain-foundational confirmation + the glass-box-ceiling addendum + the fabricated-citation catch), data/exp_curated_foundation_wic_*/metrics.json + data/exp_wic_{error_analysis,sense_coarsening,optimization_stack}_v1/metrics.json. NO hdlab/ writes."
...
**WHY IT IS SOLVED (the bar cleared):** the ORIGINAL bar asked the live consumed metric to rise CI-separated over
the current live reader. It does. An error analysis diagnosed the raw-accuracy wall as WordNet OVER-SPLITTING
(gold-SAME accuracy at CHANCE 0.494; false-diff 0.253 >> false-same 0.103; polysemy 0.544 << homonymy 0.622), and
SHARED-CORE COARSENING (predict SAME iff same lexicographer-file/supersense -- brain-faithful, Rodd 2002) fixes it.
The full recommended stack -- CURATED taxonomic signatures + coarsening -- **beats the live PPR select_sense reader
+0.0633 CI-sep on raw WiC (0.601 -> 0.664, paired, n=2038)**, the info-free twins LOSE, and no other dim regresses
(coarsening is scoped to same/diff judgments, not fine-sense selection). So the curated foundation reaches a LIVE
board dimension with a CI-separated gain -- the reroute's real problem, solved. The residual to human 0.80 is DEEP
CONTEXTUALIZATION (the invariant boundary), established by decomposition + primary-source literature -- our 0.664
glass-box already sits in the BERT-base range (60-65), above the WiC paper's non-contextual/sense-embedding
baselines (54-59); this is NOT a knowledge/algorithm gap left on the table.

> ## 🔧 PARSER NOTE (prominent, per owner request)
> **I did NO work on the project's parser, and NO recommended landing depends on any parser.** I used spaCy's
> off-the-shelf dependency parser in exactly ONE experiment -- `exp_curated_foundation_wic_syntax_v1` (the
> structured-syntactic-context hypothesis) -- which was a **LOCATED NEGATIVE** (syntax HURT WiC -0.024). So the
> only parser I touched is an external probe in a DISCARDED arm; the recommended stack (curated signatures +
> coarsening) is parser-FREE and glass-box. I did not read, modify, retrain, or wire the substrate's own
> `arc_parser_*` assets, and nothing here changes the parse path. (The who-did-what located negative is
> parse-BOUND, but that is a property of the EXISTING reader I confirmed on disk, not parser work I did.)
...
- **THE ERROR ANALYSIS was the unlock: "why does nothing beat 0.644" became a DIAGNOSIS, and the diagnosis yielded
  the win.** Decomposing the best arm's errors (rather than trying yet another mechanism) showed we predict SAME
  only 35% when gold-SAME is 50% -- gold-SAME accuracy is at CHANCE (0.494) and the error is OVER-SPLITTING
  (false-diff 0.253 >> false-same 0.103), concentrated on POLYSEMY (0.544 << homonymy 0.622). That is the classic
  WordNet-too-fine problem (CSI/OntoNotes/Navigli), and COARSENING to shared-core senses (lexname/supersense)
  lifted WiC +0.0206 CI-sep -- the FIRST raw-accuracy gain, straight from the diagnosis. The lesson: when N
  mechanisms plateau, STOP adding mechanisms and DECOMPOSE THE ERRORS; the wall names its own fix.
- **We were never failing a wall -- we were at the BERT-base frontier and mis-reading it.** The research (primary
  sources) places the WiC paper's non-contextual/sense-embedding baselines at 54-59% and BERT-base at 60-65%; our
  glass-box 0.644/0.664 sits in the BERT-base range, above the knowledge frontier. My repeated "static caps ~0.644,
  the wall is X" framing under-sold the result: 0.644 glass-box is competitive with BERT-base, no LM. The residual
  to 0.80 is deep contextualization (transformer/invariant-boundary), which the field's own numbers show shallow
  contextual models (ELMo/context2vec ~57%) do NOT reach -- so it is genuinely the deep-LM boundary, not a
  knowledge or algorithm gap we left on the table.
- **"Needs a trained encoder" was a COP-OUT I inherited from `build_sg_lite` without asking the brain question --
  ...
  whitening, structured syntactic context, graded senses, a second grounded spoke), every static supply, AND a
  dynamic reader are ALL neutral-to-negative on raw WiC. That elimination PROVED it was not the algorithm and not
  the knowledge -- and the decisive move was then to DECOMPOSE THE ERRORS (not add a mechanism), which resolved
  the wall into TWO parts: WordNet GRANULARITY (fixable glass-box -- coarsening +0.02) and DEEP CONTEXTUALIZATION
  (the invariant boundary). The lesson: when N mechanisms AND more supply plateau, STOP and diagnose the errors --
  the wall names its own decomposition; "a shared wall means none of them was the brain's mechanism."
...
## PROPOSED hdlab CHANGE (strategy lands it, Q111) -- the BAR-CLEARING stack

The recommended live wire = a taxonomic-signature sense picker + a shared-core coarsening read-out on the WiC /
`select_sense` path. It BEATS the current live PPR reader +0.0633 CI-sep on raw WiC (0.601 -> 0.664), info-free
twins lose, no other dim regresses -> DEFAULT-ON is justified (no-default-off rule).

1. **Curated taxonomic sense-picker.** In `hdlab/grounded_semantic_graph.select_sense` (+ `select_sense_blended`),
   add a curated-signature channel: for the target's candidate synsets, score `cos(context_mean,
   meaning_foundation.sense_signature(syn))` (FLAT readout -- the diagnostic readout is the a_s path, not this) and
   BLEND (z-fusion) with the PPR activation, OR use curated as the primary picker (it beats PPR at equal coarsening
   +0.0358 CI-sep). Adds the ATL taxonomic system to the reader's associative-only selection.
2. **Shared-core COARSENING read-out** (the granularity fix, the raw-accuracy lever). Where the consumer JUDGES
   same/different sense (WiC), predict SAME iff the two picked synsets are byte-equal OR share a lexicographer
   file/supersense (`wn.synset(x).lexname()`). +0.0206 CI-sep over exact equality on curated, and it also lifts the
   PPR reader itself +0.0275 CI-sep (a general read-out win). SCOPE IT to same/diff judgments only -- do NOT coarsen
   fine-sense SELECTION (a_s), where merging senses would hurt.
3. **Do NOT wire** the biased-competition/diagnostic readout, the whitening projection, the syntactic-context query,
   the static grounded signature, or the Kintsch C-I reader into the WiC path -- all measured located-negatives
   here (each for a now-understood reason; the diagnostic readout stays the a_s lever).
4. **Leak discipline:** compute the live WiC board number with leak-free signatures (exclude `synset.examples()`);
   the frozen asset over-reports on WiC by ~+0.08 (it is fine for a_s, where examples are legitimate knowledge).
5. **Ship the persisted sense-tagged grounded store** (`data/exp_freeze_sense_grounded_store_v1/sense_grounded_
   signatures_v1.npz`, 54,300 synsets) as a foundation asset -- the owner-flagged simulation substrate for the
   (discourse-level) dynamic reader; do NOT wire it as a static WiC signature (located negative there).
6. **Fix the fabricated "Borman & Lupyan" citation** in `experiments/exp_sense_wall_breakthrough_wic_v1.py`.

The residual past ~0.664 to human 0.80 is DEEP CONTEXTUALIZATION (the static->BERT gap; the invariant boundary),
established by decomposition + primary-source literature -- NOT a knowledge/grounding/algorithm lever, and NOT the
Kintsch dynamic reader (whose payoff is discourse comprehension, not isolated-sentence WiC).
...
- The raw-accuracy beat over the live reader (+0.0633) rests on the COARSENING lever, which is +0.0206 CI-sep
  POOLED and on dev but only +0.014 (not CI-sep) on the test split ALONE -- so coarsening is robustly positive but
  MODEST; if one number falls first it is the exact coarsening margin, not the direction. (The curated-over-PPR
  part, +0.0432 CI-sep, and the twin-discrimination wins are more robust.)
- The PPR baseline here (0.601 on dev+test in my harness) is slightly below the landed harness's dev figure
  (0.618); the paired beat holds either way (+0.0633 vs my PPR; +0.046 raw vs the landed 0.618), but the exact
  live-reader number depends on the harness -- strategy should recompute on the board's own PPR call at landing.
...
- The DEEP-CONTEXTUALIZATION attribution of the residual (~0.664 -> 0.80) is established by ELIMINATION (5
  mechanisms + 4 supplies + a dynamic reader all neutral-or-worse) + the error decomposition + primary-source
  literature -- not by building the contextual encoder (which is the barred/invariant-boundary route). If a claim
  falls, it is "no glass-box static method crosses much past ~0.66" (a strong inference, not a proof of impossibility).

## TLDR (plain language)

The reader has a clean, curated dictionary of word-meanings but nothing live uses it. The task's "who did what"
door is closed (that part is grammar-bound, and a past experiment showed the store makes it worse). The open door
is the one live word-meaning test: "do these two sentences use this word in the same sense?" I plugged the curated
dictionary in, and for the first time a glass-box method (no outside AI) genuinely uses context to tell senses
apart (proven by controls: wrong sentence fails, scrambled dictionary fails). Then, pushed to understand why the
overall score sits at 64/100, I stopped guessing and DECOMPOSED THE ERRORS -- and the mistake pattern named its own
fix: we were mostly SPLITTING same-meaning uses apart, because WordNet chops words into far more senses than a
human would. Merging WordNet's over-fine senses into coarser human-like groups lifted the score to 66/100 -- a
real, clean gain, and MORE brain-like (people carry coarse shared-meaning senses, not a dictionary's hair-splitting).
Two things I got wrong earlier and corrected: (1) it does NOT need a "trained encoder" -- the brain uses none; (2)
64-66/100 glass-box is NOT us failing a wall -- the published numbers put plain dictionary/word-vector methods at
54-59 and Google-scale BERT at 60-65, so we're already in BERT's range with no AI. The rest of the gap to human
(80) is deep language-model contextualization, which is the one boundary this project deliberately doesn't cross.
I also built the fully brain-faithful dynamic reader you asked for (Kintsch settling) -- it works mechanically but
loses to the simple method on this test, and the literature explains exactly why: settling helps DISCOURSE (many
sentences), and this test is single isolated sentences, the worst case for it. Its real home is discourse
comprehension, not this benchmark. Your grounding resource is real and now persisted; it's the substrate for that
discourse reader, not a fix for this isolated-sentence score.

## QUESTIONS

None blocking. Two judgments for the owner: (1) LAND the sense COARSENING -- it is a clean +0.02 CI-sep raw-accuracy
gain, glass-box and MORE brain-faithful (coarse shared-core senses), and it fixes the reader's "biased toward
different" imbalance. (2) The residual to human 0.80 is deep-transformer contextualization (the invariant boundary),
NOT a knowledge/grounding/algorithm gap we left on the table -- I established this by decomposition + primary-source
literature, so I would NOT chase it with more static knowledge or a settling reader.
...
1. **LAND the curated-signature sense-identity channel on `select_sense`/`select_sense_blended`** (default-off,
   `test_curated_foundation_wic.py` the gate; leak-free signatures for the board number) -- the confirmed
   context-driven discrimination win (the two-meaning-systems fix on a live metric).
2. **LAND SENSE COARSENING as the read-out rule** (predict SAME iff the two picked synsets share a lexicographer
   file/supersense): +0.0206 CI-sep raw-accuracy over exact-synset equality, glass-box, brain-faithful shared-core
   senses (`exp_wic_sense_coarsening_v1`). The FIRST raw-accuracy gain over static, straight from the error diagnosis.
3. **PERSIST/PROMOTE the sense-tagged grounded store** (`sense_grounded_signatures_v1.npz`, 54,300 synsets) as a
   foundation asset -- owner-flagged resource; the simulation substrate for the (discourse-level) dynamic reader.
   It is a located-negative as a STATIC WiC signature, so do NOT wire it as a static meaning signature.
4. **Do NOT** wire the diagnostic readout / whitening / syntax / static grounding / the C-I reader into the WiC
   path -- all measured located-negatives there (each for a now-understood reason).
5. **The residual to 0.80 is the deep-contextualization invariant boundary**, not a knowledge/algorithm lever;
   the dynamic (Kintsch) reader's payoff is DISCOURSE comprehension -- test it there, not on isolated-sentence WiC.
6. **Fix the fabricated "Borman & Lupyan" citation** and note the `synset.examples()` WiC-leak in the curated build.
