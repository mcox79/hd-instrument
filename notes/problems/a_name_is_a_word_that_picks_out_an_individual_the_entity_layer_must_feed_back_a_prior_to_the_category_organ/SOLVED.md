...
INTEGRATION NOTE (strategy 2026-09-14 08:33): the entity-prior diff is LANDED in hdlab/lexical_categories.py; the reader calls new_document() at every passage start; the live category asset carries the merged entity counts. The cell's --self-test now fails at W4 BY CONSTRUCTION (TypeError on DiscourseRegister.observe: the test binds the LANDED class, whose observe signature differs from the cell's own) -- the same post-landing inversion recorded for pri 93/94/105. Post-landing verification: verification/test_lexical_categories.py green; UD-EWT test tagged passage by passage UPOS 0.9312 -> 0.9313, PROPN<->NOUN 378 -> 373; board no-regress (ledger).

INTEGRATED_BY_STRATEGY 2026-09-14 09:11 local -- DONE by strategy after first-hand reverify (8/8). The entity-prior diff landed in hdlab/lexical_categories.py; the reader calls new_document() at every passage start (the one-line wire); the live category asset carries the merged entity counts (prev kept under data/hook_state). Category probe passage by passage on UD-EWT test: UPOS 0.9312 -> 0.9313, PROPN<->NOUN 378 -> 373; lexical witness green; board byte-identical on all 7 dimensions (0.6344; the board's arms read isolated sentences and never call new_document -- as the solver predicted). The forward wire (coref.py) landed earlier as a capability; pri 109 found a head-domain defect in it (last NOUN/PROPN of the whole span) with a one-line fix, and that the wire equals upos[head]=='PROPN' on the board rows -- its wiring lands with pri 109.
