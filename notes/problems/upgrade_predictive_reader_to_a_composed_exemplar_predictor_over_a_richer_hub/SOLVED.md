...
remaining representation-side gaps were run down; both resolve brain-faithfully.
- **Semantic-P600 CONFLICT error-flag (WALL B) — tested, LOCATED NEGATIVE at power.**
  `exp_composedhub_conflict_flag_v1` (n=2992): the conflict margin (s_pick - min alternative) is
  CI-separated WORSE than the marginal N400 surprisal (hub -0.028 [-0.048,-0.010] NEG; organ -0.038 NEG).
  A research drill grounds why it CANNOT help here: (1) the P600 fires on thematic-role REVERSALS, but the
  reader's wrong pick is the agent only 3.3% of the time (the residual errors are good-enough NON-reversals);
  (2) the only formalized P600 account (Brouwer et al. 2017) is a same-stream INTEGRATION-COST over the
  situation-model update, NOT a margin-against-alternatives — the faithful conflict signal lives on P1's
  generative state, not the verb-patient predictor. And good-enough processing (humans miss 20-75% of
  plausible-but-wrong role bindings: Christianson, Ferreira, Patson) confirms the 47% residual is a
  brain-consistent ceiling. **The error-flag is not a representation problem; kill for this organ.**
- **Resnik selectional-association COVERAGE backoff (WALL C) — de-risked to a next problem with the recipe.**
  The ~11.5% coverage tail has a faithful fix (Resnik 1996 A_R(v,c) argmax over WordNet ancestor classes +
  Clark & Weir 2002 evidence-gated stopping — naive averaging/argmax over-generalizes, refuted in-lit and by
  the parent). NOT built here (a proper Clark-Weir build is its own problem; a rushed naive version
  reproduces the known failure). Recipe + pre-registered kill-criterion handed off (see the adjacent doc /
  NEXT STEPS). It extends the hub's PREDICTION lever to rare fillers; it does NOT touch the live who-did-what
  null (WALL A, brain-consistent).
