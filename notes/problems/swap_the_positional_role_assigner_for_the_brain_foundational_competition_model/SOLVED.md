...
- **Pronoun subjects are dropped from role candidates** (`_sentence_nominals` filters `is_pronoun`). This caps the who-did-what agent arm at ~30% and is the highest-leverage adjacent gap. The brain resolves the pronoun to its antecedent and assigns THAT entity as agent (the coref column already tracks these). Wiring pronoun→antecedent into the agent filler would lift the ceiling far above 30%. **OUR-INVENTION placeholder (a filter), not the brain's mechanism → a strong next problem.**
