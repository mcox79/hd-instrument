...
## 12. THE ONE CONTROL NOT COMPLETED IN SESSION — stated plainly

`experiments/exp_situation_model_qa_modern_v1.py --run` (the 7-dimension board) was launched on the shipped organ as
the floor arm and had not written `metrics.json` after ~55 minutes; the machine was running several other sessions'
jobs concurrently. **The board A/B is therefore NOT measured here and must be run before landing.** Two things make
that a bounded risk rather than an unknown: (a) the patched organ is **byte-identical** on the currently-live v3 asset
(7414 labels, 0 differences), so the board cannot move at all unless the v4 table is swapped in — the A/B is
`HDLAB_ROLE_VALIDITIES=v3` vs `=v4` on identical code; and (b) the two board-relevant consumer reads I *could* measure
in-process both hold or improve (the copular STATE input is unchanged at 4/6 holders; the causation affector is
recovered). Reproduce with:
