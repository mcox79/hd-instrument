...
**The opening move.** A case-marked phrase licensed by a PREDICATE is an oblique **participant of an event**; one
licensed by a NOMINAL is a **property of a thing**. Those are two different objects in a situation model — the first
fills a slot in an event representation, the second modifies an entity — and the organ that names roles had **no name
for the second**. `ROLE_CLASSES` was `[SUBJ, OBJ, PASS_SUBJ, BY_AGENT, OBL, OTHER, IOBJ]`; `ROLE_TO_DEP` could emit
`nsubj / nsubj:pass / obj / iobj / obl / obl:agent / dep`. Reproduced first-hand: **gold `nmod` 0 of 489 correct even
given the gold tree** (`nmod → obl` 266, `nmod → dep` 222, `nmod → iobj` 1).

**The structure and the computation are unchanged.** Still the Competition Model (Bates & MacWhinney 1982/1989;
MacWhinney 1987): parallel cue competition, each cue carrying a learned VALIDITY (availability × reliability),
additive activation → softmax = the posterior, strengths a pure function of accrued counts. What changed is (a) the
CLASS SPACE the cues compete over, (b) four things the cues **read**, and (c) one thing about **how one cue is read**.

## 2. THE UPSTREAM CHAIN AND ITS BRAIN-FOUNDATIONAL STATUS, AS THE DISK SHOWS IT

| rung | organ | what it hands down for THIS signal | BF status |
|---|---|---|---|
| tokens | `hdlab/frontend.tokenize` | the string | BF_SPIRIT (glass-box; no nltk/spaCy) |
| categories | `hdlab/lexical_categories` (`HDLAB_TAG_SOURCE=counts`) | **the host's category — this signal IS the host's category**, as a hard tag plus a posterior | BF_SPIRIT (count-based generative model) |
| lemma | `thematic_role_labeler.lemma_verb` | the verb lemma for the frame and phrasal-verb cues | BF_SPIRIT |
| heads | `hdlab/attachment_arm` via `frontend.Parser` (in-order MBR tree) | **WHICH** token is the host, plus exact single-root marginals | BF_SPIRIT (reading-learned cue competition + semantic bootstrapping) |
| **labels** | `hdlab/graded_role_assigner.coarse_roles` | the role name | BF_SPIRIT — **the rung this problem is about** |
| consumers | `arc_labeler.label_competition_roles` → `causation_typing`, `copular_binding`, `predicate_argument_frontend`, `situation_reader` | the relation string | `arc_labeler` NOT_BF for the relations it still owns |

## 3. THE SIGNAL-LOSS TRACE, CHAIN BY CHAIN, WITH COUNTS

**The signal the end read needs, in one sentence:** *for each case-marked nominal, is it a participant in the event or
a property of a thing?* — i.e. the category of whatever licenses it.

| hand-off | produced | read by the next rung | LOST | count |
|---|---|---|---|---|
| categories → labels | the host token's category, hard + posterior | the `config` cue (head class × order) | nothing structural | — |
| heads → labels | ONE head per dependent (MAP of an MBR tree) plus marginals | the `config` cue reads **the category of that one head** | every mis-attachment becomes a WRONG HOST CATEGORY | 71 gold `obl` flip to `nmod` under live heads; **65 because the live head is wrong**, 67 of 71 have a nominal live head |
| labels (class) | — | — | **the class did not exist** | 489 of 489 |
| labels (case) | — | — | the genitive clitic and possessive pronouns were **not a cue value at all**, so a possessor competed with `compound` (348 tokens, same configuration) on no evidence | 125 |
| labels (how the cue is read) | the licensor's category, arc-free | looked up INSIDE the arc's configuration | when the arc is wrong the contrast is read **from the wrong row** — exactly when the cue is needed | gold nmod 0.7157 → **0.7423**; core +0.0084 n.s. → **+0.0135 CI-sep** |
| **learning → labels** | the governor's MAP head at accrual time | the `config` cue's counts | **the host-category contrast is taught wrong wherever the governor mis-attached** | oracle 0.8037 vs shipped 0.7423 (nmod); 0.9052 vs 0.8352 (obl) |
| labels → consumers | `ROLE_TO_DEP` strings | `arc_labeler._ARG_ROLES` | `nmod` was not in `_ARG_ROLES`, so the supervised perceptron kept deciding it | all `nmod` |

## 4. WHAT THE BRAIN DOES MATHEMATICALLY ALONG THIS CHAIN, AND WHAT EACH STEP BOUGHT

1. **Case marking is a top Competition-Model cue, and the marker FORM is the value** (Bates & MacWhinney 1989;
   MacWhinney 1987). pri 103 lexicalised the preposition. English's *other* case marker — the genitive, the only one it
   puts to the nominal's **right** — was invisible. Two new values of the existing `case` cue: `gen_clitic`,
   `gen_pron`. **nmod:poss 0.000 → 0.928 under gold heads (125 tokens).**
2. **The licensor is visible WITHOUT the arc.** Late Closure / Recency (Frazier 1979, PINNED) makes the nearest
   preceding lexical head the default licensor; a genitive-marked nominal's licensor is the *following* head (English
   genitives are left-branching), and a genitive to the left is skipped because it stands in determiner position (the
   DP-head rule the organ already applies in `is_arg_head`). Alone on the obl/nmod population the cue is **0.707**
   against the arc-bound gold-head rule's 0.9796 — high availability, moderate reliability, which is what a
   Competition-Model cue *is*. **It is what turns the class's obl regression into a gain: −0.0203 CI-sep → +0.0271 CI-sep.**
3. **A cue is read only where there is something for it to interpret.** Fired on every nominal, the licensor cue costs
   gold-heads CORE accuracy **−0.0185 CI-sep**; restricted to nominals that actually carry a case marker it is
   positive. A bare preverbal nominal is filling a slot, not asking who licenses it.
4. **A cue whose job is to survive a wrong arc must not be read inside the arc's configuration.** Every other cue is a
   contrast `log P(r|cfg,v) − log P(r|cfg)`, which is the right form for a cue that refines a configuration. The
   licensor cue is an *independent estimate of the configuration itself*, so it is read unconditionally,
   `log P(r|v) − log P(r)`. **gold nmod 0.7157 → 0.7423; gold core +0.0084 n.s. → +0.0135 CI-separated UP; and it is
   what recovers the causation affector.**
5. **A relative pronoun is the filler of a gap, not a case-marked phrase.** pri 103 stopped the surface case scan *at*
   a relativizer; it never stopped it *for* one, so "an area THAT will need 15,000" read `to` — which marks the
   antecedent two nouns to the left — as `that`'s own case marker. **6 relative-clause subjects recovered.**
6. **A phrasal verb is a stored lexical item** (MacWhinney's item-based constructions; pri 103's own queued alternate
   path). The lexical `prep` cue cannot tell the particle of "worked OUT a deal" from the case marker of "walked OUT
   of the room"; the cue VALUE is the verb-lemma + particle **pair**. **8 objects recovered; core +0.0169 on gold.**

### Cumulative build (gold heads; the LIVE column is the same arm read through the frontend Parser)

| step | gold nmod | gold obl | gold all-expr | gold core | live all-expr |
|---|---|---|---|---|---|
| floor (7 classes, cue set v3) | 0.0000 | 0.8081 | 0.6746 | 0.9073 | 0.5774 |
| + NMOD class | 0.7014 | 0.7878 **(−0.0203 CI-sep)** | 0.8347 | 0.9089 | 0.6794 |
| + genitive case values | 0.6851 | 0.7878 | 0.8304 | 0.9081 | 0.6856 |
| + arc-free licensor, all nominals | 0.7280 | 0.8081 | 0.8337 | 0.8887 **(−0.0185 CI-sep)** | 0.6965 |
| + only where case-marked | 0.7157 | 0.8488 | 0.8428 | 0.8946 (−0.0126 CI-sep) | 0.7003 |
| + relativizer case fix | 0.7157 | 0.8488 | 0.8452 | 0.8988 (−0.0084 CI-sep) | 0.7022 |
| + phrasal-verb value | 0.7157 | 0.8397 | 0.8528 | 0.9157 (+0.0084 n.s.) | 0.7061 |
| **+ arc-independent reading (SHIPPED)** | **0.7423** | **0.8352** | **0.8604** | **0.9207 (+0.0135 CI-sep UP)** | **0.7041** |
| *upstream oracle (not shippable)* | *0.8037* | *0.9052* | *0.9057* | *0.9494* | *0.6498* |

### The shipped arm, by gold subtype

| | nmod (286) | nmod:poss (125) | nmod:unmarked (60) | nmod:desc (18) | obl (443) | all-expr (2099) | core (1186) |
|---|---|---|---|---|---|---|---|
| gold heads | 0.8531 | 0.9280 | 0.0500 | 0.0000 | 0.8352 | 0.8604 | 0.9207 |
| live heads | 0.7378 | 0.9040 | 0.0833 | 0.0556 | 0.6072 | 0.7041 | 0.7513 |
| twin (gold) | 0.0245 | 0.9520 † | 0.1333 | 0.0000 | 0.2483 | **0.1310** | **0.0261** |
| oracle (gold heads) | 0.9406 | 0.9280 | 0.1000 | 0.1111 | 0.9052 | 0.9057 | 0.9494 |

† **A single-gold-class subpopulation cannot separate a twin by construction** — a degenerate "everything is nmod"
reader scores 1.0 on it. That is exactly what the twin is doing, and it is why its *overall* accuracy is 0.1310. The
separating controls are the multi-class populations (all-expressible 0.1310 vs 0.8604; core 0.0261 vs 0.9207).

## 5. THE CONSUMERS — ONE STRUCTURE

`nmod` moved into `arc_labeler._ARG_ROLES`, so every consumer that goes through `ArcLabeler.label` (causation typing,
the predicate-argument frontend, the copular/state read, the space register, the situation reader's
`_frontend_labeler` sites) now reads ONE structure for it. The finer SURFACE subtype the perceptron carried
(`nmod:poss`) survives when the competition agrees on the class — the genitive/prepositional difference is morphology,
not a difference of role.

Measured in-process on the real entry points, shipped organ vs patched organ, same parse:

| read | shipped | patched + v4 |
|---|---|---|
| **causation landing doc**, `SituationReader(causation_typed=True)`, sentence 0 | `(0, '', flood, village, CAUSE)` — **affector LOST** | `(0, 'storm', flood, village, CAUSE)` — **AFFECTOR RECOVERED** |
| `hdlab/copular_binding.extract_entity_states`, 6 copular clauses, LIVE arm heads | 4/6 holders | **4/6 holders** (no regression) |
| `the man in the car` | `car` → `obl` | `car` → **`nmod`** |
| `John 's hat` | `John` → `dep` | `John` → **`nmod`** |

**The copular read is where the licensor cue's genitive-skip rule came from.** Before that fix, "HER office is on the
third floor" read `Her` as *office*'s licensor, `office` fell from `nsubj` to `dep`, and the state read lost its
holder (4/6 → 3/6). Skipping a determiner-position genitive restored it. **A consumer negative found a real defect in
the cue.**

**The affector, attributed exactly** (SUBJ−OTHER margin on the LIVE parse of 'The storm flooded the village'; the live
head of `storm` is `flooded`, i.e. correct): floor **−0.278** → class alone −0.208 → +genitive −0.209 → the
configuration-conditioned licensor **−0.060** → **the arc-independent licensor +0.242 → `nsubj`**. The gold-trained
oracle also gets it. So the last 0.06 of that margin was the *reading form* of the cue, not the class and not the
governor.

## 6. EVERY NEGATIVE, UNDERSTOOD, WITH ITS MECHANISM AND ITS NUMBER

1. **Learning over the governor's head POSTERIOR instead of its MAP head — EXACTLY NULL.** Accruing each decision to
   every candidate configuration weighted by `P(head|dep)` (the E-step form: expected sufficient statistics under the
   latent-cause posterior) changed nothing: gold all-expressible 0.8394 vs 0.8394; live 0.6941 vs 0.6937.
   *Mechanism, measured:* **there is no distribution to learn over.** Mean `P(MAP head)` = **0.9884**, 96.6% of tokens
   above 0.9, mean candidates above 0.05 = **1.044**. The governor is *confidently wrong*, not uncertain — which
   sharpens pri 103's calibration finding (AUC 0.678, 74% correct at P ≥ 0.95) into a statement about the SHAPE of the
   posterior, not only its centring.
2. **A cue-convergence GATE on learning (teach only where the arc and the arc-free licensor agree about
   predicate-vs-nominal host) — NEGATIVE.** gold obl 0.8217 → 0.7585, gold all-expressible 0.8394 → 0.8171.
   *Mechanism, measured:* it discards **16,923 of 74,984 decisions (22.6%)**, and those are precisely the cases where
   the two cues CONFLICT. The Competition Model's whole content is the *relative* validity of conflicting cues.
3. **The same idea as a WEIGHT rather than a gate (β swept 0.75 / 0.5 / 0.25) — MONOTONICALLY NEGATIVE**, which closes
   the family: gold all-expressible 0.8509 / 0.8423 / 0.8399 against 0.8528 at β = 1. Softening the gate does not
   rescue it; the conflicted experiences carry signal in proportion to how much they are trusted.
4. **The PROSODIC BREAK (comma / dash / paren) as a cue value — NEGATIVE.** gold core **−0.0169 CI-sep** with gold nmod
   flat (354 → 353 correct). *Mechanism:* an intonation-phrase boundary separates appositives, conjuncts and list
   items (49 + 116 + 108 tokens here, all OTHER) just as often as it separates an `nmod:desc`, so the contrast is
   ~flat for NMOD inside the configuration while it shifts the core classes.
5. **The FIRST-NOUN (clause-initial) cue, arc-free — NEGATIVE.** gold core **−0.0337 CI-sep**, live core −0.0194
   CI-sep, gold all-expressible 0.8528 → 0.8271. *Mechanism, measured:* availability of `initial` among gold preverbal
   subjects is only **0.595** (343 of 576), so `later` fires on 40% of real subjects and the contrast it learns
   actively suppresses exactly the embedded and postposed subjects the organ exists to recover.
6. **Mapping `nmod:poss` to OTHER instead of NMOD — strongly negative:** gold nmod 0.7423 → 0.4683 CI-sep.
   *Mechanism:* it is a quarter of the class and a possessor is a nominal licensed by a nominal like any other; the
   genitive is its case marker, not a different role.
7. **The reliability gate on LEARNING re-swept** (0.0 / 0.5 / 0.8): 0.0 and 0.5 identical to four decimals, 0.8 worse.
   pri 103's operating point is confirmed and saturated — it is not where the remaining loss is.

## 7. THE BAR CLAUSE NOT MET, AND ITS ARITHMETIC

The bar asks for gold nmod ≥ 0.90 under gold heads, drawn from pri 94's **0.9813** host-category figure. That figure is
a **two-way readout on a population GIVEN to be obl-or-nmod**. Measured the same way — the organ's activation
restricted to {OBL, NMOD} on the same 932 tokens under gold heads — **the organ is 0.8970** against the rule's 0.9796.
The full-inventory 0.7423 is a strictly harder decision: the same tokens must also beat `compound` (348), `flat` (188)
and `appos` (49) in the same configuration, which the rule never faced.

The residual under gold heads, `nmod` 126 wrong of 489:

| bucket | n | shipped | oracle | what it is |
|---|---|---|---|---|
| `nmod` bare prepositional | 286 | 0.8531 | 0.9406 | the real class — the loss here is the learning rung |
| `nmod:poss` | 125 | 0.9280 | 0.9280 | done |
| `nmod:unmarked` | 60 | 0.0500 | 0.1000 | "the game last SEASON", "10:53 AM", "Washington , D.C." — **no marker at all** |
| `nmod:desc` | 18 | 0.0000 | 0.1111 | "President Bush", "Enron Corp.", "Mr. Lavorato" — a UD-2.16 convention class **surface-identical to `compound`/`flat`** |

**The arithmetic.** Even with the configuration cue trained on the gold tree the class tops out at **0.8037** for the
shipped cue set and **0.8548** for the best cue set measured (418 of 489). **0.90 = 440 tokens is 22 above the best
oracle this rung can produce.** The bar as written is not reachable by any learner that reads the host category
through a perceived arc, and 78 of the residual tokens are marker-less UD subtype *conventions* rather than a
comprehension distinction the brain draws.

## 8. COMPONENTS TOUCHED OR CREATED, AND THEIR BRAIN-FOUNDATIONAL STATUS

| component | what I did | BF status |
|---|---|---|
| `hdlab/graded_role_assigner.py` | the NMOD class (uncapped); cue set v4 (genitive case values, arc-free licensor read arc-independently, relativizer case fix, phrasal-verb value); asset-K-driven smoothing with padding; `_upgrade_counts` so the online path covers the new class | **BF_SPIRIT** — the Competition-Model operation is untouched; every addition is a learned cue value; no rule, no number adopted |
| `hdlab/arc_labeler.py` | `nmod` moved into `_ARG_ROLES` — the competition owns it; `nmod:poss` subtype preserved when the class agrees | module stays **NOT_BF** (frozen perceptron), but one more relation LEFT its inventory |
| `tools/build_coarse_role_validities.py` | `--v4`; `coarse_of` maps nmod/nmod:poss to NMOD when the class exists; GLOBAL accrual for the licensor; passes `v4` at its `coarse_role_cues` call site | **BF_SPIRIT** — offline accrual of exactly the counts the online path accrues |
| `verification/test_coarse_role_competition.py` | 11 new checks | witness |
| `data/hook_state/coarse_role_validities_pri108_v4.json` | the rebuilt table: counts (8 classes) + strengths + slot capacity + lemma frames, `cue_set: v4`, 74083.3 weighted decisions | knowledge as COUNTS; strengths a pure function of them; `observe_role_outcome` covers the new class |
| `hdlab/frontend.py`, `attachment_arm.py`, `lexical_categories.py` | read only (the perception cache runs through them) | unchanged |
| `hdlab/copular_binding.py`, `causation_typing.py`, `situation_reader.py` | measured as consumers | unchanged |

## 9. WHAT LET THE SIGNAL BE MAXIMISED — THE CHAIN, RUNG BY RUNG

* **CRACKED — the class.** The distinction exists in the situation model and had no symbol. 0/489 → 363/489.
* **CRACKED — the case marker.** English marks the possessive kind with a right-side clitic the cue set never read.
  0/125 → 116/125.
* **CRACKED — the licensor, read arc-free.** The organ read the host category *only through the governor's arc*, so it
  inherited every attachment error at both learning and reading time. Late Closure gives a second, independent,
  arc-free estimate of the same quantity. This rung turned a regression into a gain.
* **CRACKED — the cue's scope.** A cue that interprets a case marker must abstain where there is no case marker:
  the difference between −0.0185 CI-sep on core arguments and positive.
* **CRACKED — the cue's READING FORM.** *This is the one I would not have found without tracing the consumer.* An
  independent estimate of the configuration must not be read *inside* that configuration. Worth +0.027 gold nmod,
  core from n.s. to CI-separated UP, and the causation affector.
* **CRACKED — two upstream defects the new cue exposed.** A relative pronoun is not case-marked from the left; a
  phrasal-verb particle is not a case marker. Both were *pre-existing* losses in pri 103's `prep` cue that the
  licensor cue made visible by amplifying them.
* **NOT CRACKED — the learning rung.** The `config` cue's counts are accrued through the governor's MAP arc. This is
  the whole remaining gold-heads gap (0.7423 vs the oracle's 0.8037 / best-cue-set 0.8548). All three gold-free
  repairs I built are negative and all three are understood (§6.1–6.3).
* **NOT CRACKED — the marker-less UD subtypes.** 78 tokens; the oracle gets 9 of them.

## 10. ALTERNATE PATHS — as brain-foundational or MORE so, for strategy to queue

1. **Make the CONFIGURATION itself a two-source estimate, not one source with a correction.** This session showed the
   labels rung's knowledge is poisoned by the heads rung at *accrual* time, and that the arc-free licensor is a
   genuinely independent estimator of the same quantity (0.707 alone) whose value comes from being read
   *unconditionally*. The next step is to make the configuration KEY itself the pair (arc host class, arc-free host
   class) — multisensory cue combination (Ernst & Banks 2002) applied to the configuration rather than to a contrast.
   **Reach: the oracle gap, gold-heads all-expressible 0.8604 → 0.9057.**
2. **Consolidate during a REPLAY pass rather than online per sentence.** The brain does not commit a comprehension
   outcome to long-term cue validity at read time; it consolidates (hippocampal replay → cortical statistics). A
   second pass re-reading its own earlier decisions with the whole sentence available, accruing only what survives
   re-reading, is the brain-faithful way to filter *confident* governor errors — which is exactly what the three
   failed repairs could not do (the posterior is peaked, and gating/weighting destroys the conflict evidence).
3. **A learned TIME/MEASURE lexical class as a cue.** `nmod:unmarked` is dominated by temporal and measure nominals.
   The organ's own witness already records this as a known limit ("the fix is a lexical time/measure class cue").
   pri 103 refuted the raw 68-way induced category (too sparse); the queued stronger version is a COARSE induced
   class. **Reach: ≤ 60 tokens.**
4. **Stop scoring `nmod:desc` as nmod.** "President Bush" / "Enron Corp." is a UD-2.16 annotation split, not a
   comprehension distinction — the brain's read of "President Bush" and of a compound is the same. 18 tokens are
   un-winnable by construction and should be scored as a convention class, as pri 94 did for the heads rung.
5. **The heads rung remains the live-arm wall for obl** (65 of 71 flips have a wrong live head). pri 94/97/105
   territory, not this rung's.

## 11. PRIORITY NEXT STEPS

1. **Land it.** Apply the diff, swap the v4 table into `data/frontend_assets/`, re-run the witness and the board. The
   class is worth **+0.1267 CI-sep on all expressible nominals on the LIVE chain**, it takes core arguments up, it is
   the boundary at which pri 94's entire nmod head gain was dying, and it repairs the causation affector.
2. **Queue alternate path 1** (two-source configuration key) — the one lever with the oracle gap behind it, and a
   direct continuation of this session's strongest finding.
3. **Re-check the consumers that read `obl` as an event participant** now that `nmod` is separable: the space register
   and the affected-entity chain had been receiving 266 gold `nmod` tokens per 700 sentences labelled `obl`.
4. **Finish the board A/B** (§12) and report the 7 dimensions.

## 12. THE BOARD A/B — COMPLETED. NO REGRESSION, AND THE TWO DIMENSIONS THAT READ THIS RUNG BOTH MOVE UP

`experiments/exp_situation_model_qa_modern_v1.py --run`, the 7-dimension 19c-free modern board, run TWICE end to end:
the floor arm on the shipped organ, the v4 arm on the patched organ with `HDLAB_ROLE_VALIDITIES` pointing at the
rebuilt table (identical code otherwise — the patched organ is byte-identical without the v4 asset, so this A/B is
exactly `cue_set v3` vs `cue_set v4`).

| dimension | n | v3 (shipped) | v4 (patched) | delta | gold |
|---|---|---|---|---|---|
| coref | 3132 | 0.4681 | 0.4681 | 0 | GUM |
| salience | 137 | 0.2555 | 0.2555 | 0 | GUM |
| common_noun_coref | 2855 | 0.5671 | 0.5671 | 0 | GUM |
| who_did_what_agent | 1423 | 0.8271 | 0.8271 | 0 | UD-EWT |
| **who_did_what_patient** | 1255 | 0.8064 | **0.8088** | **+0.0024** | UD-EWT |
| **state** | 378 | 0.7434 | **0.7487** | **+0.0053** | UD-EWT |
| wic | 2038 | 0.7493 | 0.7493 | 0 | WiC |
| **AGGREGATE** | | 0.6344 | **0.6349** | **+0.0005** | |

**No dimension falls.** Both dimensions that consume this rung's labels rise: the who-did-what PATIENT read and the
STATE read — the latter being exactly the copular readout the brief flagged at 0.7434 on the BF chain. 5 of 7
dimensions stay CI-separated over their floors in both arms, and every twin stays below its model. The four
untouched dimensions (coref, salience, common-noun coref, wic) are byte-identical, which is the expected shape: they
do not read the argument-role labels.

Reproduce:

```
HDLAB_EXP_NAME=role_nmod_board_floor  .venv/Scripts/python.exe experiments/exp_situation_model_qa_modern_v1.py --run
HDLAB_EXP_NAME=role_nmod_board_v4 HDLAB_ROLE_VALIDITIES=data/hook_state/coarse_role_validities_pri108_v4.json \
...
**20.1 (Q2) THE CONFIGURATION KEY AS A TWO-SOURCE ESTIMATE — BUILT, SWEPT, AND REFUTED-AS-BUILT.** The supervisor
asked me to file this brief-ready; I built it instead, because it was the top lead and it turned out to be cheap.
*Structure:* the same Competition-Model configuration, but its KEY becomes the pair (arc host class, arc-free host
class) rather than the arc host class alone -- `log P(r | cfg_arc, cfg_surf) - log P(r)`, one joint configuration
with Dirichlet backoff to its own marginal (Ernst & Banks 2002 cue combination applied to the configuration instead of
to a contrast). *Built* by making the configuration key `cfg_arc@cfg_surf` and giving `strengths_from_counts` a parent
function that backs a joint key off to its arc marginal. *Swept* over the backoff mass m = 0 / 2 / 20 / 200 / 1000:

| m | gold nmod | gold obl | gold all-expr | gold core | live all-expr |
|---|---|---|---|---|---|
| **shipped (separate, unconditioned licensor)** | **0.7403** | **0.8691** | **0.8661** | **0.9191** | **0.7061** |
| joint key, m = 0 | 0.6912 | 0.7675 | 0.8333 | 0.9174 | 0.7018 |
| joint key, m = 2 | 0.6994 | 0.7720 | 0.8361 | 0.9174 | 0.7008 |
| joint key, m = 20 | 0.6953 | 0.7720 | 0.8347 | 0.9165 | 0.6999 |
| joint key, m = 200 | 0.6953 | 0.7223 | 0.8232 | 0.9140 | 0.6918 |
| joint key, m = 1000 | 0.7157 | 0.6524 | 0.6856 | 0.8137 | 0.6827 |

**Negative at every setting, best at m = 2 and still -0.030 on all-expressible.** *Mechanism, with the count:* the
joint key has **253 configurations against the shipped 28**, and EVERY secondary cue contrast is read WITHIN the
configuration -- so the joint key starves all of them at once. The backoff mass repairs `P(role | cfg)` but it cannot
repair the contrasts, because those are estimated from the joint cell's own counts, not from the marginal; that is why
increasing m fixes nothing and eventually destroys the configuration term as well (m = 1000: core 0.9191 -> 0.8137).

**This result PROMOTES the shipped design rather than merely sparing it.** Reading the licensor as an UNCONDITIONED
term (the GLOBAL key) is not a cheap approximation to the joint key -- it is strictly the better estimator here,
because it keeps the configuration coarse enough for every other cue to be estimated at all. The two-source idea is
right and the two-source *key* is the wrong place to put it.
...
* The learning rung's +34 was attacked **seven ways**, all built and all measured: posterior accrual (null, the
  ...
  weight (a prior shift, with the correlations to prove it), **the two-source configuration key swept over five
  backoff masses** (negative everywhere; 253 configurations against 28 starves every secondary contrast), and
  Predicate Proximity (**positive — shipped**). The ONE route left, replay-based consolidation (§20.3), needs offline
  machinery this session cannot build and measure honestly, so it is a brief, not unfinished work.
* The board A/B completed inside the session (§12): **no dimension falls, and the two dimensions that read this rung
  both rise** (who-did-what patient 0.8064 → 0.8088, state 0.7434 → 0.7487).
...
* Every control the bar names is now measured, the board A/B included.

What would change this verdict is not more effort at this rung but **§20.3 (replay-based consolidation)**, which is
bounded by the same measured 34 tokens, or **§20.2**, which is a change to a different organ.
...
BOARD A/B COMPLETE (both arms run end to end): NO dimension falls and the two that read this rung both
  RISE -- who_did_what_patient 0.8064 -> 0.8088, state 0.7434 -> 0.7487 (the copular readout the brief
  flagged at 0.7434); aggregate 0.6344 -> 0.6349; 5/7 dims CI-sep over floor in both arms.
...
LEAD 1 WAS BUILT INSTEAD OF FILED, AND IS CLOSED: the configuration KEY as a two-source estimate (arc
host class x arc-free host class), swept over five Dirichlet backoff masses (0/2/20/200/1000), NEGATIVE
at every setting -- 253 configurations against 28 starves every secondary cue contrast, and the backoff
repairs P(role|cfg) but not the contrasts, which are conditioned on the sparse joint key. This PROMOTES
the shipped design: reading the licensor unconditioned is strictly better, not a cheap approximation.
REMAINING LEADS: (2) move `nmod` out of affected_entity_resolver.OBJ_DEPS (one line, 573 tokens/700
sentences, needs its own measurement); (3) replay-based consolidation -- the only untried route to the
+34-token learning-rung budget; (4) the inverted-copular construction (both remaining copular blockers
are the categories and heads rungs, not labels).
