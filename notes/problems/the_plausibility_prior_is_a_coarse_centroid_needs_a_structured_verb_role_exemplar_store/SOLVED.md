...
SHOW-IT-WORKS + PACKAGED (owner asked): the domain-matched store is DEMONSTRATED at the exemplar/marginal
level -- in-domain (no-gold, parsed, leave-one-sentence-out) beats out-of-domain +0.149 CI-sep. An end-to-end
FHRR-joint demo (`exp_domain_matched_fhrr_demo_v1.py`) revealed a REFINEMENT: the JOINT (subj,verb,obj) FHRR
store needs a LARGER domain corpus than the marginal store (joint SVO is sparser; a 2M-token in-domain corpus
is too thin -> -0.055, a data-density effect, not a domain refutation). Packaged as a file-able follow-on
brief for a solver: `PROPOSED_FOLLOWON_domain_matched_selectional_store.md` (proposed slug
`the_selectional_event_store_is_learned_from_the_wrong_domain_needs_a_register_native_corpus`) -- for the
strategy session to lift into a new problem folder.
