...
- A SECOND, smaller wall beyond knowledge: even GOLD knowledge caps at 0.556 (< human 0.83) -- that residual
  ...
  intrinsic ambiguity.
BRAIN-FOUNDATIONAL OVERCOME (the located wall): the knowledge IS the consolidated semantic store; its quality
is set by the CORPUS + PARSE feeding it. FOUNDATION-IS-FREE: build a LARGE, CLEANLY-parsed (agent,verb,patient)
event corpus OFFLINE (an admissible static asset) -> dense, accurate selectional knowledge. That is the #1
lever, definitively located; the richer-features / combiner / arbitration paths are NOT (measured).
