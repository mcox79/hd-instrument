...
files_changed: "experiments/_tbdense_loader.py, experiments/_joint_relation_frontend.py, experiments/_hashseed_guard.py, experiments/exp_temporal_extraction_recall_v1.py, experiments/exp_joint_temporal_survival_v1.py, experiments/exp_joint_temporal_endtoend_v1.py, experiments/exp_joint_spatial_survival_v1.py, experiments/exp_joint_upstream_noregress_v1.py, verification/test_joint_temporal_survival.py, verification/test_joint_temporal_endtoend.py, verification/test_joint_spatial_located_negative.py, verification/test_joint_upstream_roles_noregress.py, notes/problems/extract_relations_from_prose_whole_subgraph_survival_the_shared_reasoner_bottleneck/SOLVED.md (NO hdlab/ written -- Q111: proposed diff in Sec 7)"
...
- **The SPATIAL negative is parse quality, enumerated.** The joint binding is CORRECT when the parse is
  (church/Madrid nested containment, door-of-house), and WRONG on copula+stacked-PP and existentials -- the UAS-0.79
  failure modes. The mechanism-diff is sharp: **the joint pass wins where the bottleneck is event DETECTION (temporal
  tense-gate) and is blocked where the bottleneck is ATTACHMENT ACCURACY (spatial), which localizes the residual to
  the upstream parse organ.** This is the bar's own example of a full-pass located negative.
...
events tense-agnostically (0.756 recall, states included) -- the temporal front-end is no longer the bottleneck for
before/after and overlap. Where we still lose signal: (a) eventive NOMINALS (assistance/construction/takeover) are only
suffix-heuristically detected; (b) SPATIAL figure/ground binding is gated by UAS-0.79 -- the arc-factored, UNLABELED,
BATCH parser is the itemized mechanism-diff vs the brain's incremental, labeled, cue-integrated structure-building.
...
- **The spatial negative is a POSITIVE result about the upstream organ.** joint-loses-on-spatial + joint-wins-on-
  temporal is a mechanism-diff that pinpoints parse UAS as the spatial residual -- more useful than a forced spatial win.
...
- **SPACE (sec.2b):** the spatial extraction wall is confirmed to be UPSTREAM PARSE QUALITY, not the binding rule --
  parse-based joint binding (UAS 0.7907) does not beat the incumbent linear scan on terse prose (chain survival 6/90
  reproduced; joint 4/90; hybrid 7/90 not CI-sep). The residual is a distinct upstream organ (incremental/labeled parser).
...
- **The upstream parse (`arc_parser`, UAS 0.7907, UNLABELED, BATCH):** the sole remaining lever for the SPATIAL
  channel and the itemized mechanism-diff vs the brain (incremental, labeled, cue-integrated). Brain-status: OUR-
  INVENTION, NOT brain-faithful (audit). A labeled + incremental parser (another line's p2) is the high-value follow-on;
  a parse-CONFIDENCE-gated hybrid (arc margins are exposed) is a cheaper brain-foundational intermediate.
...
- **P2 (HIGH, follow-on problem): the upstream parse organ for the SPATIAL channel** -- a labeled + incremental parser
  (UAS 0.79 is the wall) OR a parse-confidence-gated hybrid; this is the sole lever that makes spatial chains survive.
- **P3: eventive-nominal detection** (glass-box nominalization lexicon) to close the remaining OCCURRENCE recall.
