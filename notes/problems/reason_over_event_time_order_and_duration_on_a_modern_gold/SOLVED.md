...
## NEXT STEPS / PRIORITIES (ranked; the core + all in-scope upgrades are BUILT -- 18/18)
**P1 -- Strategy: LAND the core reasoner + all built upgrades** (all additive, no regression; witness 18/18):
the aspect->interval upstream (+ Vendler lexical aspect + progressive guard), the OVERLAP reasoner (+ token-indexed
same-verb simultaneity), RELATIVE-duration, the INTEGRATED before/after reasoner (cue + TIMEX event-local anchoring +
transitive closure + signal-class provenance), the UDS-Time duration organ, and the script/schema TRACIE organ. Emit
per-judgment SIGNAL-CLASS provenance (cue/date/closure/iconicity) + "unknown -- needs world knowledge" on
implicit-event queries.

**The rest are NEW next-PROBLEMS (each needs a new organ -- named with its brain mechanism + on-disk evidence):**
- **P2 (highest-value) -- COPULAR/STATIVE-state channel.** The biggest real-prose OVERLAP lever: TORQUE overlap is
  dominated by STATIVE co-occurrence ("what was happening while Sterling WAS 2.3643 francs" -> gold "was"), which the
  verb-based extractor DROPS. This is the parent-flagged dropped copular channel (Ferretti/Kutas/McRae). Unlocks the
  anchor-aware TORQUE scorer's true in-context overlap number.
- **P3 -- UDS-Time native rank-correlation eval + a duration board arm.** MCTACO's per-candidate F1/EM MISMEASURES a
  knowledge organ (rewards firing rate; the organ has real knowledge -- native ranking 0.66 CI-sep). Score it on
  UDS-Time's OWN metric (Spearman rho) and add a relative-duration board arm; expect the organ to look strong.
- **P4 -- SDRT world-knowledge discourse reader, coupled to the causation organ.** The Explanation order-reversal is
  INFERRED from causal knowledge, not marked (the marker-only version is a proven located negative).
- **P5 -- Richer script induction for the TRACIE tail** (the 0.60 chain organ -> SymTime-class start/end + duration
  decomposition) and an anchor-aware TORQUE overlap scorer (on top of P2).
