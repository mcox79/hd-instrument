...
# Both relation-TYPERS are brain-foundational and WIN on the binding CORRECTNESS axis (density is a free phase-diagram knob): SPATIAL semantic Figure-Ground typing beats the density prior +0.386; CAUSAL generative world-model typing beats it +0.357 -- residual on both is COVERAGE, not mechanism
...
binding CORRECTNESS axis (density is a free phase-diagram knob) it WINS -- precision 0.496 (best arm) vs the density
flood 0.140 (+0.357 CI-sep), beats the connective shortcut on recall (+0.046), and recovers UNMARKED causal edges
...
3. **CAUSAL: land the GENERATIVE world-model typer, NOT the connective channel.** Connective-matching recovers 0.5%
   ...
   WINS on the binding correctness axis (precision 0.496 vs density 0.140; +0.046 recall over connective; recovers
...
- **DENSITY IS A FREE KNOB (phase diagram), so RECALL/SURVIVAL is the FREE axis and CORRECTNESS/PRECISION is the
  ...
  +0.357 over density), and the density floods COLLAPSE (spatial 0.185, causal 0.140). This reframed both channels
...
- **The causal binder's FIDELITY UPGRADE works: generative world-model typing > connective-matching.** Connective-
  ...
  connectives cannot (0.064 vs ~0) and wins the precision axis (0.496, best arm). Its ~5% coverage is the class-level
...
- **CAUSE (sec.2b).** FIDELITY UPGRADE: connective-matching (0.5% recall, not the brain's mechanism) REPLACED by the
  ...
  On the binding correctness axis (density free per the phase diagram) the generative typer WINS: precision 0.496 (best
  arm) vs contiguity 0.140 (+0.357 CI-sep), beats connective on recall (+0.046), recovers UNMARKED edges (0.064 vs ~0).
...
- **P2 -- LAND the CAUSAL generative world-model typer** (built this pass; wins the binding correctness axis:
  precision 0.496 vs density 0.140, +0.046 recall over connective, recovers unmarked edges) and DEEPEN its coverage
