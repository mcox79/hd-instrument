...
priority:
review: EXCELLENT
review_text: "INTEGRATED 2026-08-26 (owner-authorized in-session: 'be as brain foundational as possible'). Outcome = REFUTED (a rigorous negative is an explicit PASS); the WORK is EXCELLENT and it re-points the whole audit. Re-verified scaffold-free first-hand: test_sign_quantiser_not_the_bottleneck_on_hit1.py PASS (SIGN 0.0465 vs GRAD_RAW 0.0480 = +0.0015 NULL; best-constant 'averaging' floor 0.1710 beats every code-format read-out by >0.08). Load-bearing stale-premise CONFIRMED on disk: HD_GRADED_COMPARATOR defaults '1' (graded ON since 08-14), so the 'flip the switch' fix was already landed and is null. THREE substrate-wide corrections folded to BRAIN_FOUNDATIONAL_AUDIT.md: (1) DEVIATION #2 (sign quantiser) is NOT the averaging-machine lever -- the whole brain-faithful code-format family (graded/divnorm/sparse/DG-expand) + a faithful self-supervised CBOW learner ALL tie plain counting ~0.05, all below the 0.171 floor; only WordNet-SUPERVISED learning beats it -> the wall is meaning SUPPLY, upstream of sign() and every read-out. (2) NEW first-class row: TWO SIMILARITY SYSTEMS measured on our reps (distribution carries ASSOCIATIVE relatedness WordSim 0.25 but ~0 feature SIMILARITY SimLex 0.04; grounding carries both 0.42/0.21) = the ATL-feature vs LIFG/pMTG-associative dissociation; the SIMILARITY axis is recovered by STRUCTURE (tight window 0.075->0.112) + grounding, not any read-out format; a fixed blend helps relatedness but HURTS similarity -> a measured need for SEMANTIC CONTROL (IFG task-gating). (3) §8 lever #1 (sign->graded) DEMOTED; §7 BOUNDED (present-but-unwired splits: wire the associative system, BUILD the feature-similarity system). Corpus-age is NOT this instrument's confound (load_corpus_v5 is MODERN, disk-checked). Model of the strengthened protocol: refuted the surface question, swept the whole faithful family, then doubted the TARGET (taxonomic gold) and found the two-systems map. Optional default-off center_field micro-win NOT landed (within noise). The brain-foundational next build is packaged: the second meaning system + semantic-control gate."
---
