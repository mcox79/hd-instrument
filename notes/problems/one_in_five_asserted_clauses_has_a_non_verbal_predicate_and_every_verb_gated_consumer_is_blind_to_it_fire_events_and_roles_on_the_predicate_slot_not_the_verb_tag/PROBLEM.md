...
## 9. HEAD-TO-HEAD COMPARISON (owner-run vs agent-run; opened 2026-09-14 13:55 by strategy at the owner's request)
Both solvers start from git tag `comparison_pri113_base` (HEAD 816efb22a) with the working tree as it stood at 13:55 (uncommitted: pri 110's predicate-slot changes to `hdlab/lexical_categories.py` / `hdlab/predicate_detector.py`, pri 106's `label` mode in `hdlab/affected_entity_resolver.py`). Same brief, same bar, blind to each other until both submit. FILE NAMES (corrected 14:00 after a collision: the owner had started before this section was posted and took the section-7 names): the OWNER keeps `experiments/exp_nonverbal_predication_participants_v1.py`, `SOLVED.md` and `predicate_slot_consumers_patch.diff` (the section-7 names); the AGENT writes `experiments/exp_nonverbal_predication_participants_agent_v1.py`, `SOLVED_agent.md` and `predicate_slot_consumers_agent_patch.diff`. The agent saw the first 90 lines of the owner's cell before stopping (recorded in SOLVED_agent.md; discount accordingly). Each commits path-limited only its own files. Strategy compares on: the bar met (with CI), the depth of the chain trace and of every explained negative, the follow-up briefs produced, wall-clock and tokens.
