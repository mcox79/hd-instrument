...
Both solvers start from git tag `comparison_pri113_base` (HEAD 816efb22a) with the working tree as it stood at 13:55 (uncommitted: pri 110's predicate-slot changes to `hdlab/lexical_categories.py` / `hdlab/predicate_detector.py`, pri 106's `label` mode in `hdlab/affected_entity_resolver.py`). Same brief, same bar, blind to each other until both submit. FILE NAMES so nothing collides: the OWNER writes the cell as `experiments/exp_nonverbal_predication_participants_owner_v1.py`, the report as `notes/problems/<slug>/SOLVED_owner.md`, the diff as `predicate_slot_consumers_owner_patch.diff`; the AGENT uses the names in section 7. Each commits path-limited only its own files. Strategy compares on: the bar met (with CI), the depth of the chain trace and of every explained negative, the follow-up briefs produced, wall-clock and tokens.
