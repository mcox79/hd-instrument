...
- **MEASURED (on disk):** `force_dynamics_valence` uses a hand verb-list + WordNet animacy (the `HARM_BACKOFF` test-fitted list was already retired — registry note); the substrate's force-dynamic typer/lexicon exists + is used by causation typing (`notes/BRAIN_FOUNDATIONAL_AUDIT.md`). The read-path FrameNet parse is live.
