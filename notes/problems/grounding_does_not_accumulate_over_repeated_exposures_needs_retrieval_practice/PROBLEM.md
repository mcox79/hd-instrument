...
priority:
review: EXCELLENT
review_text: "Reverified first-hand (W1-W10 all pass). A rigorous REPRESENTATION-BOUND NEGATIVE (the brief's full-PASS-if-located outcome): retrieval practice, faithfully built (Mozer 2009 MCM Eq7 + a PBV variant), CANNOT select a WordNet-correct grounding above chance on the CONSOLIDATION_FAIL population (selection-AUC CI includes chance); grounding precision FLAT across schemes; retrieval does NOT beat the info-free twin (no testing effect). Drilled to bedrock: for the ~78% of words with an anchor the correct anchor is RETRIEVABLE (oracle top-10 ~85%, recoverable 0.81 >> coverage-bound 0.09) under EVERY encoder incl. the full parser -- so the encoder is NOT the wall -- but SELECTABLE by NO distributional read-out (nearest/bg-subtract/distilled/supervised all ~chance vs the ~0.87 ceiling): the SENSE signal is genuinely absent from distributional co-occurrence. DEMONSTRATED the fix -- a GROUNDED-hub (ATL) re-rank selects the correct sense where no distributional read-out could. Population characterized (polysemy 155 / no-eligible-anchor 76 / incoherent 37 / proper-noun-or-oov 26; only 2 coherent-single-sense-with-anchor). PPMI+SVD does not beat bag-of-words; sense-splitting recovers only ~7%. NO hdlab wire (a rigorous negative + a proposed NON-change: do NOT land retrieval-practice consolidation or a distributional re-ranker). SPAWNED the north-star meaning organ (promote_the_grounded_semantic_graph_to_an_intrinsic_learnable_organ), which reframes the write-path from a flat store to a relational GRAPH read by spreading activation and subsumes the grounded-re-rank follow-on."
---
