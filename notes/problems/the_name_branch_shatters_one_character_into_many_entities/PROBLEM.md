...
priority:
review: EXCELLENT
review_text: "Owner-DONE. A RIGOROUS NEGATIVE — the highest-value kind, and the bar's own escape clause anticipated exactly this. It refutes the premise of the brief I PACKAGED (name clustering caps who-did-what) with a decisive decomposition, having first BUILT the brain-faithful mechanism the brief asked for. Re-verified FIRST-HAND (ran it myself): verification/test_name_entity_clustering.py ALL 9 checks PASS (torch-free). Built (bar 1, DONE, the RIGHT mechanism): a content-addressable COMPLETE-OR-SEPARATE person-identity-node organ (given-name/surname/title/gender slots; gn-agreement filter; a conflicting-given-name VETO = pattern separation of same-surname people; same-surname ties broken by ACT-R salience) — exactly the PIN computation (Bruce&Young 1986; Burton IAC; Patterson/Nestor/Rogers ATL hub; CA3-completion/DG-separation; Heim/Kamp file-cards) — plus a full-span+entity-type LOADER that enriches the head-token LitBank cache in place (100% alignment, gold→chain 0.9991) so clustering + downstream are measured on the identical stream. THE TWO REFUTATIONS (both load-bearing, decisive): (1) CLUSTER QUALITY — the organ TIES the strong floor, does NOT beat it: 0.7848 [0.759,0.810] vs the live head-token-Jaccard floor 0.7703 [0.748,0.793] (NOT_SEP; markedly MORE PRECISE B-cubed P 0.902 vs 0.820 — separates same-surname people the floor over-merges — but trades recall). ROOT CAUSE THE BRIEF (mine) MISSED: the head token IS the surname, so single-token clustering already unifies surname-family forms — the floor is STRONG, not broken; the brief's own proposed fix (full spans) BACKFIRES 0.705 CI-sep BELOW (over-merge); info-free twin 0.597 loses CI-sep; positive control moves (organ unifies an alias re-entry the floor SHATTERS + separates the sibling). (2) THE DOWNSTREAM PREMISE — REFUTED by decomposition: swapping the organ for the incumbent does NOT lift who-did-what (0.1715 vs 0.1614, +0.010 NOT_SEP); reproducing the cited gap (live 0.161≈'0.17', oracle 0.56-0.61≈'0.62'), PERFECT PRONOUN BINDING on the SAME head-token clusters recovers the WHOLE gap (HEAD_OPB 0.606 vs 0.161, +0.444 CI-sep), and given perfect pronouns better name clustering adds +0.000 (ORGAN_OPB≈HEAD_OPB NOT_SEP). A control shows unifying aliases HURTS who-did-what (HEAD_OPB 0.606 > ORACLE 0.562 — the register FAN effect). So the who-did-what cap is PRONOUN→EVENT BINDING + register fan, NOT name clustering. THE REAL FIX proposed AND PROVEN (in-scope): (1) pronoun→event binding is the dominant lever (+0.444 ceiling; fix = clause-level graded binding + wire the tracked-but-unused clause_role/Centering-Cb topicality, surfaced 3× across problems; diagnosed decisively, build belongs to a pronoun problem); (2) the sparse MultiBankRegister (built-but-unwired) lifts who-did-what +0.0149 CI-sep [0.009,0.022] on the ORACLE config (D=1024), modest + masked on the live system until pronoun binding is fixed. Same-surname disambiguation ('which Miss Bennet') drilled to the bottom (6 research + 2 data drills): rare (112 ties/100 novels, ~80 resolvable); BUILT the brain's cues (Centering-Cb topicality + the verified eldest-daughter convention) + info-free twins → all null or worse (topicality TIES its scrambled-role twin = no signal at scale; the convention is HARMFUL, needs an entity-fact store we lack). Understood: recency is a category error (co-present siblings tie by construction; cue-overload); the eldest fact is discourse-internal but needs a missing organ, not forbidden knowledge. HONEST label PARTIAL→ integrated as a rigorous NEGATIVE (a full pass per the bar's own rigorous-negative clause). hdlab landing QUEUED (Q111 — small, opt-in, NOT a who-did-what fix): the full-span+entity-type LOADER (additive, zero-risk) + an opt-in default-off `run_person_node_clustering` on `coreference_resolver.py` for high-precision same-surname CHARACTER SEPARATION; do NOT replace `_resolve_name_branch` (floor is strong), do NOT invest in name clustering for who-did-what. AUDIT UPDATE folded (§2b coref: head-token name branch is a STRONG floor [head=surname]; full-span backfires; who-did-what cap = pronoun binding + register fan [multibank +0.0149 PROVEN]; clause_role/Cb tracked-but-unused, 3rd measurement). Adjacencies EVALUATED (owner directive): the ranked real levers are (a) pronoun→event binding at the clause level — THE dominant lever, a candidate new brief (the successor this refutation identified); (b) the entity-keyed FACT/EVENT store (multibank built-but-unwired) — highest cross-cutting leverage (who-did-what + eldest-disambiguation + predicate + ToM), a wiring+capability follow-on; (c) nominal/definite-description binding — semantic, ~40% ceiling, hard. STRATEGY NOTE: this refutes MY OWN brief's premise — I packaged it from the coref adjacency's 0.62-vs-0.17 read; the solver correctly decomposed that gap to pronoun binding, not clustering. The measurement bar applied to my own strategic read caught it."
---

> ## ✅ SOLVER REVIEW — INTEGRATED 2026-08-28 (strategy session; grade EXCELLENT; owner_verdict: DONE)
> **Re-verified FIRST-HAND** (`verification/test_name_entity_clustering.py`, ALL 9 checks PASS, torch-free — ran it
> myself; the register-lift arm re-run separately). **A RIGOROUS NEGATIVE that refutes the premise of the brief I
> PACKAGED — the highest-value kind, and the bar's own rigorous-negative clause blessed exactly this.**
> **Built (the RIGHT mechanism, bar 1 DONE):** a content-addressable COMPLETE-OR-SEPARATE person-identity-node organ
> (given/surname/title/gender slots; a conflicting-given-name VETO = DG pattern separation of same-surname people;
> ACT-R salience tie-break) — the PIN / CA3-completion-vs-DG-separation computation the brief pinned — plus a
> full-span+entity-type LOADER enriching the head-token LitBank cache in place (100% alignment) so clustering +
> downstream share one stream.
> **The two refutations (decisive, controlled):** (1) CLUSTER QUALITY — the organ TIES the strong floor (0.785 vs 0.770
> NOT_SEP), markedly more PRECISE (0.90 vs 0.82) but trades recall; the ROOT CAUSE the brief (mine) missed is that the
> head token IS the surname, so single-token clustering already unifies surname forms — the floor is strong; the brief's
> own full-span fix BACKFIRES (0.705 CI-sep below, over-merge); info-free twin loses; positive control moves. (2) THE
> DOWNSTREAM PREMISE — REFUTED by decomposition: the organ does NOT lift who-did-what (+0.010 NOT_SEP); reproducing the
> cited 0.17→0.62 gap, PERFECT pronoun binding on the SAME head clusters recovers the WHOLE gap (+0.444 CI-sep) while
> better name clustering adds +0.000 — so the cap is PRONOUN→EVENT BINDING + register fan, not name clustering (and a
> control shows unifying aliases HURTS who-did-what — the register fan effect).
> **The real fix proposed AND proven:** (1) pronoun→event binding is the dominant lever (+0.444 ceiling; fix = clause
> graded binding + wire the tracked-but-unused clause_role/Centering-Cb); (2) the sparse MultiBankRegister
> (built-but-unwired) lifts who-did-what +0.0149 CI-sep on the oracle config. **Same-surname disambiguation drilled to the
> bottom** (6 research + 2 data drills): rare, and every brain cue measured is null-or-worse with an info-free twin
> (topicality ties its scrambled twin → no signal at scale; the eldest-daughter convention is harmful — needs an
> entity-fact store we lack). Exemplary rigor. **AUDIT UPDATE folded (§2b coref).**
> **hdlab landing QUEUED (Q111 — small, opt-in, NOT a who-did-what fix; NOT this commit):** the full-span+entity-type
> LOADER (additive, zero-risk) + an opt-in default-off `run_person_node_clustering` on `coreference_resolver.py` for
> high-precision same-surname CHARACTER separation. Do NOT replace `_resolve_name_branch` (the floor is strong).
> **Adjacencies EVALUATED (owner directive):** (a) **pronoun→event binding at the clause level is THE dominant lever**
> (+0.444) — a candidate new brief, the successor this refutation identified; (b) the entity-keyed FACT/EVENT store
> (multibank built-but-unwired) is the highest cross-cutting leverage (who-did-what + disambiguation + predicate + ToM) —
> a wiring+capability follow-on; (c) nominal/definite-description binding is semantic, hard, lower.
> **STRATEGY NOTE (own it):** this refutes MY brief's premise — I packaged it from the coref adjacency's 0.62-vs-0.17
> read, and the solver correctly decomposed that gap to PRONOUN BINDING, not clustering. The measurement bar applied to
> my own strategic read, and it caught the error. That is the system working.
