...
## 4e. THE ORGAN + THE FULL 100% BRAIN-FOUNDATIONAL CHAIN (owner: prototype the organ and the full chain; research all walls)

Four biology-first literature drills (full synthesis: `WALLS_RESEARCHED_2026-09-07.md`) resolved every wall to a
mechanism. The two-valid residual's DOMINANT cue is the Competition Model (implicit causality + Centering salience
+ givenness + thematic fit), NOT the verb-object selectional preference the earlier probe tested (which is why that
recovered only 18%). I then PROTOTYPED the argument-role competition ORGAN and wired the FULL chain end-to-end
(`exp_full_brain_chain_v1`): tagger -> exact single-root Matrix-Tree marginal (SYNTAX) -> McRae thematic-fit organ
(EVENT) -> the argument-role competition organ (Centering salience + givenness/information-structure + canonical
word order + locality + animacy) -> Competition-Model integration (additive cue activation -> softmax, the landed
`graded_competition` FORM, cue validities fit offline) -> precision-gated readout (keep the confident parse; let
the top-down competition decide only where the marginal is uncertain). Candidates = ALL nominals (so a buried gold
head is reachable via its marginal).

**RESULT -- a rigorous LOCATED NEGATIVE at the SENTENCE level (full UD-EWT, n=1235, blanket labeled reader 0.8785):**
the full chain scores 0.8575 (delta -0.021), and the TWIN with the top-down cues shuffled scores 0.8543 (delta
-0.024) -- **fusion == twin, so the sentence-internal top-down cues are NOT load-bearing.** The learned cue
VALIDITIES say exactly why: syntax 1.70 / word-order 0.85 / locality 0.72 dominate, while givenness 0.01 and
Centering-salience 0.157 are near-zero. Two-valid recovery 14/100.

**WHY (mechanistically airtight, and it answers "why aren't we showing the brain's results"):** the two-valid
cases are TWO GIVEN ENTITIES (two proper nouns / pronouns), so sentence-internal givenness cannot discriminate
them, and grammatical role (which the labeled reader already uses) subsumes the sentence-internal salience/word
-order signal. The discriminating cue the brain uses is the DISCOURSE CENTER -- which given entity is the current
topic, from the PRIOR sentences (Centering across clauses) -- plus the verb's IMPLICIT-CAUSALITY bias. Neither is
available to a SENTENCE-level parser. So the two-valid residual is provably NOT a parser wall and NOT resolvable by
sentence-internal cues; it is a DIFFERENT organ: the DOCUMENT-LEVEL recurrent discourse loop + a static
implicit-causality norm lexicon. This confirms the wall-map's dominant finding from the who-did-what side and
quantifies it (the near-zero sentence-internal discourse-cue validities ARE the proof).

**IS THE CHAIN 100% BRAIN-FOUNDATIONAL? -- honest map:**

| stage | brain-foundational | status |
|---|---|---|
| POS tagger (Collins perceptron) | YES | landed |
| graded parser: exact Matrix-Tree marginal + 2nd-best + Hale entropy | YES | this problem, exact |
| exact decode (Chu-Liu/Edmonds MAP) | YES (> greedy) | this problem |
| valency / subcat unification (Wall 3) | YES (Hagoort MUC) | NAMED build; `verb_subcat_frames_ud_ewt.json` on disk |
| argument-role competition organ (Wall 1) | YES (Competition Model) | PROTOTYPED = sentence-level located negative |
| register adaptation (Wall 2) | the brain adapts; we don't | NAMED build (lexicalized features + Rescorla-Wagner) |
| recurrent DOCUMENT top-down loop + IC norms (Walls 1+4) | the brain has it; we don't | THE missing organ (wall-map dominant) |

Two precisely-located deviations remain, each a DIFFERENT organ (not the parser, which is now faithful in
structure + decode + posterior): (i) lexicalized valency features + online register adaptation; (ii) the
document-level recurrent discourse loop + an implicit-causality norm lexicon. Nothing here is a ceiling except the
Winograd-schema world-knowledge minority.

## 4f. BOTH REMAINING ORGANS PROTOTYPED (owner: prototype those organs + a 100% brain-foundational full chain)

I built the two organs the wall research named, each glass-box / no-LLM, and measured them on naturalistic modern
gold. **Both are first-cut LOCATED NEGATIVES on their target slice** -- an important, honest correction to the
wall-map's "just close the loop" optimism for the who-did-what patient.

**ORGAN 1 -- lexicalized VALENCY / subcategorization unification (Wall 3)** (`exp_valency_unification_organ_v1`).
Mined a glass-box subcat lexicon from UD-EWT train (per head lemma+POS -> licensed (relation, dependent-POS)
log-priors) and re-ranked the arc-factored MST decode by frame-slot licensing. On FULL UD-EWT the dev-tuning picks
weight ~0 (overall attachment UAS 0.7908 -> 0.7908, delta 0.0; head-BURIED slice 0/2610 recovered) -- the coarse
lemma-POS frame prior adds NOTHING to the arc-factored decode on naturalistic data (a small ccomp +0.025 appeared
on the smoke sample but did not survive full data). VERDICT: a coarse lemma-POS frame prior is not enough to lift a
buried gold head; the full fix needs proper VerbNet/PropBank frames + valency-saturation + frame-completeness
INTER-ARC constraints (a fuller build).
Brain-foundational (Hagoort MUC unification) but the coarse prototype's yield is small + concentrated on ccomp.

**ORGAN 2 -- DOCUMENT-level discourse salience + implicit causality (Walls 1+4)** (`exp_document_discourse_organ_v1`,
full UD-EWT, doc-aware loader, n=816 items WITH prior-sentence context, blanket 0.8738). Centering Cf salience over
prior-sentence mentions (recency-decayed, subject>object) + a curated implicit-causality verb lexicon, fused via the
Competition-Model logistic, gated by the parser marginal. RESULT: the discourse cue has 76% coverage (lemma-match
links a candidate to a prior mention -- NOT a coverage artifact) yet **near-ZERO learned validity (disc_sal 0.039,
ic 0.073 vs syn 1.57)** and adds **exactly 0.0** over the sentence-level chain (twin flat). VERDICT: Centering
topical-continuity does NOT discriminate the who-did-what patient on naturalistic gold -- the more-salient entity is
not reliably the agent, and most verbs are not IC verbs. The residual needs FULL coreference-resolved discourse
(pronoun/alias linking, not lemma-match) + a comprehensive IC-norm asset + world-knowledge (the Winograd minority).

**THE FULL 100% BRAIN-FOUNDATIONAL CHAIN, and the honest verdict.** Every stage is now built and PINNED: tagger
(perceptron) -> globally-normalized graded parser (exact marginals + 2nd-best + Hale entropy -- EXCELS: obl reader
beats the landed calibrator, universal per-label reliability, OOD-robust) -> lexicalized valency unification (Organ
1) -> McRae thematic fit -> document-level argument-role competition (Organ 2) -> precision-gated readout. The chain
is architecturally 100% brain-foundational. BUT the two organs that would move the who-did-what PATIENT beyond the
strong labeled reader, in their first-cut glass-box form, each recover ~0 on naturalistic modern gold. This is the
load-bearing scientific result: **the who-did-what residual is NOT a quick top-down/valency wiring win; it is
substantially the irreducible world-knowledge / full-coref slice** (Winograd + thematic-fit + pronoun-resolved
discourse), exactly the genuine within-invariant residual the wall-map lists -- now demonstrated from the
who-did-what side with two faithfully-built organs, not assumed. The parser this problem owns is done and excellent;
the remaining lift lives in FULLER versions of these organs (proper frames; full coref; comprehensive IC norms) or
is a real ceiling, and either way is a separate program, not this parser.

## 4g. THE FAITHFUL ORGAN, DONE RIGHT -- an ABSOLUTE who-did-what gain (SUPERSEDES the earlier absolute-negative)

The owner pushed correctly: a fair test of a WEAK implementation is not a fair test of the brain's mechanism. Two
things were wrong. (i) A DIAGNOSTIC I had skipped (`exp_twovalid_diagnostic_v1`) shows the "two-valid" errors are
NOT mostly irreducible: 66% are ROLE errors (gold patient IS attached to the verb, reader picked another dependent)
and 34% are PARSE misses (gold not attached) -- both addressable. My "67% needs world-knowledge" was a
mis-classification (the >=2-animate proxy conflated mis-parsed with genuinely-ambiguous). (ii) Both organs were
mis-BUILT: valency as a per-arc prior (redundant with the parser BY CONSTRUCTION; the real mechanism is the
INTER-ARC frame constraint), and the earlier full chain used near-zero-validity givenness/salience while OMITTING
the load-bearing argument-vs-adjunct typing.

Rebuilt FAITHFULLY (`exp_argstructure_unification_organ_v1`, aggressive research drill = Hagoort MUC joint frame
unification): the patient is chosen by JOINT argument-structure over the verb's candidates -- ARGUMENT-vs-ADJUNCT
typing (an oblique carrying a case-marker is an adjunct, not the patient; a bare post-verbal nominal is the core
object), core-object preference, frame-prior, and the GRADED PARSER MARGINAL to REACH the 34% parse-miss gold. The
argument-vs-adjunct typing is MASSIVELY load-bearing for the standalone selector (shuffling it collapses accuracy
0.85 -> 0.55, twin -0.30 -- utterly unlike the inert coarse organs), and it recovers 35% of the ROLE-error slice +
19% of the PARSE-miss slice.
...
beat the labeled reader"): those were fair tests of WEAK implementations (fall-back cascade; wrong cues), not of the
brain's mechanism. Done faithfully + integrated as the Competition Model, the who-did-what patient IS improved.

**Honest attribution (ablation).** On full data the INCREMENTAL lever inside the ensemble is the GRADED PARSER
MARGINAL (validity 1.40; reaching the parse-miss gold), not the arg/adjunct typing -- dropping the arg/adjunct cues
loses only +0.0008 (CI incl 0), because the labeled reader already encodes the obj-preference the typing captures.
So: the arg/adjunct organ is VALIDATED as a mechanism (load-bearing standalone) but incrementally redundant with the
labeled reader; the absolute gain over the labeled reader is carried by THIS PROBLEM'S OWN DELIVERABLE -- the graded
Matrix-Tree marginal -- used as a role-competition cue over the parse-miss-reaching candidate set. The graded parser
does not just lift RELIABILITY (obl/patient selective) and supply the 2nd-best; wired into the Competition-Model
role selection it lifts ABSOLUTE who-did-what CI-separated.

## 4h. PARSE-MISS RECOVERY + the CONSOLIDATED implement list (owner: "do them all, brain foundational")

**Parse-miss recovery** (`exp_parsemiss_recovery_v1`, the 34% slice where the greedy 1-best never attached the gold
patient to v -- greedy floor 0 BY DEFINITION). The exact edge MARGINAL exposes the gold as a top-2 head of v on
**77.5%** of the slice (top-1 37.5%), shuffled-marginal twin 2.5% -- the graded parser structurally eliminates the
search failure even on the hardest slice. HONEST: the exact 2nd-best TREE is a WEAK lever here (5% -- it differs
from the MAP by one arc, rarely the relevant re-attachment); the MARGINAL, not the 2nd-best tree, is the parse-miss
exposure mechanism. Valency saturation adds 12.5%. ANY graded mechanism 82.5%. The remaining gap is SELECTION among
the exposed candidates (the ensemble picks the exposed gold ~19-28%) -- the role-competition problem, bounded by the
genuinely-ambiguous residual.

**ALL upgrades prototyped + witnessed (11 -> 14 witnesses green); the implement list, every item brain-foundational
and off the ONE arc-factored Matrix-Tree marginal (one matrix inverse/sentence):**
1. marginal -> `parse_confidence` as the UNIVERSAL attachment reliability: obl FIRST (raw marginal 0.782 beats the
   landed obl calibrator 0.736, +0.048; DROP the obl logistic), then patient (+0.008). (exp_matrix_tree_patient_lift / _deepen / _upgrades)
2. marginal -> who-did-what ROLE SELECTION as a Competition-Model cue: ABSOLUTE +0.0065 CI-sep (0.8785->0.8850). (exp_argstructure_unification_organ)
3. single-root marginals (grammar-faithful, brute-force-exact, same one inverse) + exact CLE decode (eliminates the
   7.2% invalid-tree parses, +0.002 UAS). (exp_matrix_tree_singleroot / _parser / _noregress)
4. Hale parse entropy as a sentence difficulty/defer signal (Spearman 0.70). (exp_matrix_tree_upgrades)
5. parse-miss candidate reach: use the marginal's top-2 exposure to widen the role-reader's candidate set (77.5% exposure). (exp_parsemiss_recovery)
**EFFICIENCIES:** one marginal serves reliability + 2nd-best + absolute role selection (compute once, reuse); DROP
the obl logistic (raw marginal beats it); compute the exact 2nd-best TREE lazily (only for a fallback parse); lean
the ensemble (is_live + marg carry it; arg/adjunct cues redundant incrementally, ablation +0.0008).
**SUBSUMED (not a separate build):** the arc_labeler arg/adjunct upgrade IS the argstructure ensemble's has_case /
core_obj cues (a standalone structural relabel is weaker than the labeled reader; it only helps as an ensemble cue).
**FULLER BUILDS (separate problems, real headroom):** the parse-miss SELECTION gap (k-best forest + joint frame
unification over trees, verb-sense-conditioned frames); the genuinely-ambiguous reversible/Winograd minority (needs
grounded event-knowledge / full-coref discourse -- the coarse discourse organ's null located this).
...
- **NEW (faithful organ, 2026-09-07) -- SUPERSEDES the absolute-negative: the graded parser marginal, wired into a
  ...
  lever is the marginal itself. Record: the graded parser lifts ABSOLUTE who-did-what, not only reliability.
- **NEW (both organs prototyped, 2026-09-07): the two residual organs each recover ~0 on naturalistic gold -- the
  who-did-what residual is the irreducible world-knowledge/full-coref slice, NOT a quick wiring win (a CORRECTION to
  the wall-map's "close the loop" optimism for who-did-what patient).** Organ 1 (lexicalized valency, Wall 3):
  on FULL data dev picks weight ~0 (UAS 0.7908->0.7908, buried 0/2610 recovered) -- a coarse lemma-POS frame prior
  adds nothing to the arc-factored decode; needs proper VerbNet frames + inter-arc valency constraints. Organ 2 (document
  Centering salience + IC, Walls 1+4): 76% cue coverage but disc_sal validity 0.039, adds 0.0 over sentence-level
  (twin flat) -- Centering topical-continuity does not discriminate the patient on naturalistic gold; needs full
  coref + comprehensive IC norms + world-knowledge. Record: the graded parser is done + brain-foundational; the
  who-did-what lift lives in FULLER organ versions or is a genuine ceiling -- a separate program.
- **NEW (walls researched + organ/full-chain prototyped, 2026-09-07): the parser front-end is brain-foundational
  in STRUCTURE + DECODE + posterior; the two residual deviations are DIFFERENT organs.** Four biology-first drills
  (`WALLS_RESEARCHED_2026-09-07.md`) + the full-chain prototype (`exp_full_brain_chain_v1`) establish: (Wall 3) the
  buried-attachment slice needs LEXICALIZED VALENCY features (Hagoort MUC; asset `verb_subcat_frames_ud_ewt.json`
  on disk); (Wall 2) OOD attenuation is surface-feature brittleness, fixed by lexicalized features + online
  Rescorla-Wagner adaptation; (Walls 1+4) the two-valid residual is a DOCUMENT-level recurrent discourse loop +
  an implicit-causality norm lexicon -- PROVEN sentence-level located negative (full chain 0.8785->0.8575 == twin
  0.8543; sentence-internal discourse-cue validities near-zero because two-valid are two GIVEN entities). Record:
  the graded parser is done; the who-did-what residual lives in two named upstream/parallel organs, each PINNED.
...
### NEXT STEPS (ordered; HIGH-PRIORITY first)

**HIGH -- LAND THE GRADED PARSER (one copy + a few one-line wires, all off ONE marginal; strategy owns hdlab, Q111).**
Promote `experiments/graded_parser_promote_v1.py` verbatim to `hdlab/graded_parser.py` (self-test-exact), then wire:
  (a) **obl FIRST** -- feed `patient_confidence_cue(marg,v,pk)` into `parse_confidence.obl_row`'s inert `a2_marg` and
      DROP the obl logistic (raw marginal AUC 0.782 BEATS the calibrator 0.736; +0.048; live selective 0.758->0.902);
  (b) **who-did-what ROLE SELECTION** -- add the marginal + argument-vs-adjunct typing as a Competition-Model
      re-ranking cue in `structural_patient_pick` (ABSOLUTE +0.0065 CI-sep, 0.8785->0.8850), candidates including the
      marginal's top-2 parse-miss reach, anchored on the existing labeled pick; then the patient calibrator (+0.008).
  All additive; parse heads default byte-identical.

**HIGH -- SHIP single-root marginals + the exact Chu-Liu/Edmonds decode** (grammar-faithful, brute-force-exact, the
  SAME one inverse; eliminates the 7.2% invalid-tree parses; UAS +0.002 CI-sep, no consumer-label regress).

**MEDIUM -- expose HALE parse ENTROPY** as a sentence-level difficulty/defer signal (Spearman 0.70; the greedy 1-best
  cannot give it); **add a MODERN selective-reliability board arm** so the live defer gain is board-visible.

**EFFICIENCY (fold into the wire):** compute the marginal ONCE, reuse across reliability + role selection + parse-miss
  exposure; **DROP the exact 2nd-best TREE from the read path** (weak live lever, 5% vs the marginal's 77.5% parse-miss
  exposure); the universal marginal replaces the per-reader cue sets.

**FOLLOW-ON PROBLEMS (separate builds, real headroom -- FILE, do NOT re-scope into this parser problem):**
  (1) **parse-miss SELECTION** -- a k-best FOREST + joint frame unification over trees + verb-sense-conditioned frames
      (the marginal already EXPOSES 77.5% of parse-miss gold; SELECTING it among competitors is the gap).
  (2) **the genuinely-ambiguous reversible/Winograd minority** -- grounded event-knowledge + FULL-coref discourse (the
      coarse discourse organ's null located this precisely; sentence-internal cues + lemma-match coref do NOT suffice).
