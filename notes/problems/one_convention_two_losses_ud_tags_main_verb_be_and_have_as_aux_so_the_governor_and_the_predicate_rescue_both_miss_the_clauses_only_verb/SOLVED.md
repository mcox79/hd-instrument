...
controls: "(1) INFORMATION-FREE TWIN, the brief's own: the SAME NUMBER of AUX tokens promoted at random with the same strength. UD-EWT 3 seeds UAS 0.6168 / 0.6167 / 0.6195 against the floor's 0.6245 -- every one CI-separated BELOW the floor (-0.0077 / -0.0078 / -0.0050), and on the sole-AUX clauses 0.5884 / 0.5996 / 0.5996 against 0.6074. GUM 2 seeds 0.5889 / 0.5886 against 0.6008, both CI-separated below. The twin therefore never manufactures the gain; a random promotion of the same size HURTS. (2) PAIRED BOOTSTRAP over SENTENCES (2000 resamples) for every delta, including a per-relation bootstrap for every relation the bar names. (3) TWO POPULATIONS, one of them (GUM/GENTLE, 12+ genres by stride) entirely outside the category organ's count supply, with LARGER effects than in-supply. 19c is not measured (owner ban). (4) THE ARMS ARE THE READER: the OFF arm was cross-checked against a live SituationReader on 150 sentences, 0 mismatches; OFF / LIVE / AUX107 reproduce pri 107's published numbers (0.9186 / 0.9501 / 0.9597 and blind 55 / 33 / 18) exactly, so this is their instrument. (5) OPERATING-POINT SWEEP: th 0.3 / 0.5 / 0.7 / 0.9 gives UAS +0.0082 / +0.0083 / +0.0081 / +0.0073 -- a FLAT region, because the occupancy is bimodal (505 of 591 AUX tokens score 0.0, 33 score ~1.0); 0.5 is the middle of the flat region, not a tuned knob. (6) PATCH == CELL: the self-test EXECUTES the proposed diff's own added code and compares it to the cell -- max |cell - patch| = 0.0 over 4757 tokens, 0 decision mismatches, 0 revision mismatches; git apply --check clean. (7) SURGICAL: the revision touches ONLY promoted carriers (33 of 9534 UD-EWT test tokens, 0.35%); every other posterior row is byte-identical, asserted in the self-test. (8) ABLATION of each term: the clause-mass form alone (path A) loses; the boolean sole-AUX gate alone loses at the governor; the NP-complement refinement is worth precision 0.6286 -> 0.6471 at identical recall."
...
**D. Let the event detector read the OCCUPANCY rather than the TAG.** `_tense_agnostic_extract` fires on
...
where the state actually lives. That would attack the 9 remaining blind clauses at their source and is the natural
partner of the state dimension's copular reader.
