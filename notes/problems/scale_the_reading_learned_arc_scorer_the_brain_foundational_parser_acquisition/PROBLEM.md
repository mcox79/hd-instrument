...
## 4b. UPDATE FROM pri-3 (NOW COMPLETED + owner-DONE 2026-09-10 — supersedes the under-powered numbers above; read its cells)
pri-3 (`the_parser_is_a_frozen_supervised_hard_decode…`) is DONE. It already RAN the EM + Naseem-prior config this brief prescribes — so start from ITS results, do NOT re-derive them:
- **The reading-learned scorer already reaches UAS 0.463 (non-root 0.443)** — POS-attachment PPMI + Naseem-2010 structural prior + DMV-class EM using `graded_parser.single_root_marginals` as the E-step (`exp_parser_selfsup_em_v1`, `exp_parser_readlearned_scorer_fix_v1`). It BEATS the strong right-branching floor 0.2849 on BOTH metrics, recovers **36% of the floor→supervised gap**, grows by reading, twin loses. This SUPERSEDES the §3 "0.2716 at ~3k/0-EM" number (that was the under-powered config; 0.463 is the EM+prior result). Your job = close MORE of the remaining 0.319 gap to supervised 0.782, or a located negative at FULL scale.
- **DO-NOT-REDO (pri-3 proved these NEGATIVE, all twin-controlled): DMV VALENCE** (faithful soft-EM, exact inside-outside brute-verified → 0.20–0.25, BELOW 0.463 — EM optimizes likelihood not accuracy; valence is NOT the lever); **lexical / distributed-embedding features** (HURT attachment −0.151; it is POS-structural, Klein-Manning); POS↔parse bootstrapping (drifts); grounded GEK / thematic-fit / coarse-class / world-model rescoring (4 semantic negatives — semantics HURT attachment).
- **THE LEVER pri-3 FOUND = STRUCTURE (not distributional):** the Now-or-Never incremental left-corner cue (`incremental_parser`) lifts nsubj recall 0.565→0.714; coordination-parallelism lifts cc/conj 0.078→0.218; combined UAS 0.463→0.476. Each confident-error CONSTRUCTION yields to its own BF structural mechanism — EXTEND this (more constructions), it is the demonstrated path past the distributional ceiling.
- **HONEST CEILING + ENDGAME (pri-3):** text-only unsupervised acquisition caps ~0.46 (field's ~0.45–0.65); the residual 0.319 to supervised needs signal a corpus lacks (prosody / joint attention / embodiment). So the endgame is likely **HYBRID** — the treebank scorer as an admissible offline FOUNDATION consumed as the graded posterior + an ONLINE reading-learned ADAPTATION for the register-generality the frozen scorer lacks (the OOD lever: supervised degrades 0.79→0.74 on GUM, the reading-learned generalizes above floor). A located negative naming exactly this ceiling (with a full-scale number) is a full pass.
- **Note (Q125):** pri-3's decode route-through targets the arc-FACTORED parser; the live head path is arc-EAGER — so measure your scorer on the reader's OWN downstream metric (extraction/dependency board dim), not by swapping the live head decode.
