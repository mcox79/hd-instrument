...
INTEGRATED_BY_STRATEGY 2026-09-15 15:56 local -- the four-file diff APPLIED (board file: provenance stamps on every row, the reader-driven block in three provenance columns, HEADLINE aggregate_19c_free = the reader's own read of the raw text with aggregate_component_rebuilt kept in full, both in the trend file, matched-cap regression check; tools/substrate_health.py reads the board's capped flag; verification/test_modern_board_landing.py W1 asserts both aggregates; the pri 117 cell's A/B keeps comparing the rebuilt rows). Strategy reverify: cell --self-test 15/15 (SELF-TEST PASS), test_modern_board_landing W1 green (HEADLINE = reader_textonly 0.4949 at the witness cap; COMPONENT rebuilt kept 0.6024 over 7 modern dims), whole witness exit 0. The full board on the new instrument runs once pri 125 lands (its pre-125 reader-driven numbers are this cell's own ud_full/gum_d40/wic_c120 outputs: agent 0.1510, patient 0.6869, state 0.7937, wic 0.7583, coref n=0 text-only, aggregate 0.4717 n=4115). Q140 filed on the headline switch (proceeding on yes). The agent's one DENIED tool call (a `git apply` / `patch -p1` scratch copy of pri 125's files) was reported verbatim and not retried; the combined measurement comes from landing pri 125 through the normal integration path. Status PARTIAL stands: coref text-only n=0 until pri 125 (both gold gates), agent below its floor until pri 125 + the hybrid flip.
