...
## TOP-DOWN BF AUDIT + ALL-BF COMPOSITION (owner: "make all components 100% BF from the top down, verify the math")
Full audit in `BF_AUDIT.md` (this folder): every operation in the sense-assignment chain is math-verified and tagged
BF / BF_SPIRIT / NOT_BF, top-down. VERIFIED-BF math: cosine = divisive normalization (Carandini-Heeger); softmax =
Gibbs population posterior; convergent fusion = Bayes cue integration (Ma/Pouget); z-score = gain normalization; PPMI =
Hebbian-predictive association + rectification (Levy-Goldberg); IDF = distinctive-feature salience; random projection =
DG/cerebellar expansion. TWO NOT_BF components found and DROPPED: the unordered bag-of-words channel and the peakedness
precision weighting; plus one inherited NOT_BF dependency (the pos_tagger+arceager parser feeding DEP -> route through
`incremental_parser`). Composing ONLY the BF/BF_SPIRIT components with the assumption-free equal-weight Bayes default
(`exp_all_bf_chain_v1.py`): the ALL-BF chain scores MRR 0.332 / hit@10 0.586 and BEATS the MIXED chain that still
contains the NOT_BF bag + gold-calibrated weights (+0.028 CI [+0.002, +0.054]), the pre-audit SOLVED fix (+0.274, ~5.7x),
and the info-free twin -- with NO gold-fitted parameters. The owner's thesis is confirmed on the number: the fully
brain-foundational composition is the best chain. NAMING CONVENTION applied: all 5 mechanism cells carry the
