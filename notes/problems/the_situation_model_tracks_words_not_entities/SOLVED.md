...
which is also how the brain does it. So the situation model should be entity-structured, but as an ADDITION to
the running gist, not a replacement. One more honest twist from the deepest drill: when I tested the OTHER half of
...
Re-verified scaffold-free FIRST-HAND (verify_entity_structured_situation_model.py PASS: BAG+ENTITY 2.795 < BAG 2.847
CI-sep; info-free twin HURTS; role-conditioned beats role-blind; cue-based-retrieval swap makes the win LARGER +0.072).
Bar MET via AUGMENT (not replace -- naive replacement LOSES -0.0355; the brain keeps both a global gist AND entity nodes):
the entity-structured situation model (gist + the active entity's ROLE-CONDITIONED accumulated state, retrieved by
identity) beats the bag-of-words gist CI-separated on modern QA-SRL reconstructed documents (+0.0545, REPLICATED on an
independent split +0.0402). The info-free twin (random entity's history) ACTIVELY HURTS -> requires the correct entity.
UNIFIES with the retrieval convergence (4th time): content-addressable retrieval makes the entity win larger. SHARP
DISSOCIATION: entity PREDICTION uses meaning-memory (content-addressable); entity BINDING (who a mention is) is dominated
by SALIENCE/RECENCY (Centering 0.493 >> content 0.308) -> keep retrieval for prediction, salience for the pronoun pick.
HONEST: small effect (coarse grounded space + ~2 items/entity = the p1 representation coupling). hdlab landing EARNED ->
QUEUED proven-ready (default-off entity-augment of the forward predictor's context; salience-based binding; do NOT use
cue-based-activation for the pronoun pick). AUDIT UPDATE folded (§2b). SUCCESSOR packaged = the entity-BINDING half on a
modern pronoun corpus (coref/Centering over string-identity). Review EXCELLENT + SOLVER REVIEW in PROBLEM.md; priority
cleared. Committed. *(batch 2 of 3; next-steps recommendation held until all 3 per owner.)*
