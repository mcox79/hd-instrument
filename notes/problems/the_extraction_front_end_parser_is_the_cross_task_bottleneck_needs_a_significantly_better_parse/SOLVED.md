...
1. **Arc-eager incremental heads, promoted to a loadable `parse()` operator** (`exp_arceager_parser_operator_v1`).
   The live frontend loads the WEAKER arc-FACTORED `richfeat` (UD-EWT test UAS 0.775 gold-POS / 0.744 pred-POS);
   the arc-eager transition parser with a dynamic oracle (the substrate's own best, reused bit-identically from
   `exp_depparse_transition_arceager`) reaches **0.8184 gold-POS / 0.7827 pred-POS -- a real UAS gain of +0.043 /
   +0.039** (bar objective 2). This is the brain-faithful INCREMENTAL shape (Now-or-Never; Christiansen & Chater
   2016) the audit named as the fix to arc_parser's two weaknesses (batch + UAS cap). It emits a per-attachment
   confidence (softmax over legal actions + raw margin) -- the graded-distribution signal the arc-factored
   parser's uncalibrated margins never gave.
...
solved the real problem underneath by a different, more brain-faithful route.
...
1. **who-did-what CI-sep over 0.515, as argument-attach precision** -- YES. QA-SRL FULL 0.5147 -> **0.5407,
   +0.0260 CI[+0.012,+0.040]**; HARD +0.0849; 19c +0.1111. Argument-attach PRECISION on UD-EWT (arc-eager vs
   live richfeat): object(->patient) **+0.0170 CI[+0.001,+0.033]**, subject(->agent) **+0.0205 CI[+0.007,+0.035]**,
   passive-subject +0.068, oblique/recipient +0.053. Reaches 0.541 of spaCy's 0.588 (the residual is the located
   negative, below).
2. **UAS/LAS gain on UD-EWT** -- YES (UAS). arc-eager **0.8184 vs 0.775 (+0.043)** gold-POS; +0.039 pred-POS.
...
3. **COMPOUNDS to a 2nd task** -- YES. The same parse improvement lifts a DIFFERENT role: subject/AGENT attachment
   +0.0205 CI-sep (vs the patient/object side +0.0170), and the oblique/RECIPIENT attachment +0.053 -- the
   structural input to world-state role recovery. The gain is not who-did-what-specific.
...
1. Land the arc-eager operator as `hdlab/arceager_parser.py` (train+save offline; a `parse(tokens,pos)->
   heads+attach_conf` mirroring `arc_parser.ParseResult`, PLUS a per-arc confidence), asset
   `arceager_dynamic_ud_ewt.npz`. Point `situation_reader._load_frontend()` at it behind a default-off
...
active or passive, fixes that. I also swapped in a better, more brain-like left-to-right grammar reader (its
accuracy on a standard test rose from 77% to 82%), which helps the hard and old-text cases and a second job
(finding the DOER, not just the done-to). The reader now also reports how confident it is, which reliably flags
its own mistakes. One thing I could NOT do: match a small off-the-shelf parser's raw accuracy with our own -- the
sanctioned way to close that gap was already proven not to work on disk, and I show exactly why and what would.
