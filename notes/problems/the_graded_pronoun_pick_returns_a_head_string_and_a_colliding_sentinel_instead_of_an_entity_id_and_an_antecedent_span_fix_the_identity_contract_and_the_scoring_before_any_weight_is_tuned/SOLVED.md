...
reverify: ".venv/Scripts/python.exe verification/test_pronoun_pick_identity_contract.py   # 14/14; materialises the diff itself, runs both arms in one process, and every check is a claim (an invariant, a direction, or a can-fire control on the shipped organ), never a pinned number.  The measured rows: .venv/Scripts/python.exe experiments/exp_pronoun_pick_identity_contract_v1.py --pronouns 28 (the headline, ~45 min) / --identity-basis 12 (the oracle probe) / --principle-b 16 / --sweep 8 / --consumers 6 --board-coref 4 --noregress 10.  Each row writes its OWN metrics_<row>.json and never overwrites another."
---
