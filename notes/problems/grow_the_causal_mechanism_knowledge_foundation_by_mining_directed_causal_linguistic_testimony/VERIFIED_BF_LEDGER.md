...
| NECESSITY | `predictive_world_model` | counterfactual ablation: surprisal(B\|ctx) - surprisal(B\|ctx\A) | **NOT-BF (RUNG-1) -- CORRECTED 2026-09-09** | RE-AUDIT: the read = log[P(B\|ctx)/P(B\|ctx∖A)] = leave-one-out predictive RELEVANCE = Pearl RUNG-1; the CHT proves no rung-1 functional is the rung-2/3 necessity it is NAMED for, REGARDLESS of input. Prior "OPERATION faithful" tag was WRONG. Faithful only as a rung-1 resonance/N400 stage. Fix = compose `causal_reasoner`. See AGGRESSIVE_BF_AUDIT §2. |
| NECESSITY | **`causal_reasoner.py`** | Pearl-style: **do(cause=absent) -> re-propagate** (`is_necessary`/`intervene_and_compare`) + Halpern-Pearl AC2 (`is_actual_cause`) + graded-necessity edges | **BF-SPIRIT -- AUDITED 2026-09-09** | genuine rung-2/3 graph-surgery + AC2 actual causation (Cell 1: AC2 1.000 vs but-for 0.000; do-sim 1.000 on confound where rung-1=0.000). RESIDUAL: no exogenous ABDUCTION (roots fixed +1), discrete ±1 signs, reachability-necessity. THIS is the genuine necessity organ; `predictive_world_model` is not. |
...
| `causal_reasoner` | "BF_SPIRIT" (AUDITED 2026-09-09; was BF_UNVERIFIED) | "genuine rung-2/3 do-surgery (is_necessary/intervene_and_compare) + Halpern-Pearl AC2 (is_actual_cause); residual: no exogenous abduction, discrete +-1 signs, reachability-necessity" |
| `predictive_world_model` (CORRECTION) | "note: rung-1, NOT counterfactual" | "the causal_antecedent read is rung-1 leave-one-out predictive relevance (resonance/N400), MISLABELED counterfactual necessity; relabel -> predictive_relevance, strip CSM/Trabasso language (Q111)" |
