...
INTEGRATED_BY_STRATEGY 2026-09-14 11:54 local -- DONE by strategy after first-hand reverify (self-test PASS); both diffs applied (the board scorer + 16 entity-layer files); strategy flipped HDLAB_GUM_DECISION's default to 'organ' (no default-off for an instrument fix). GOLD-FREE BOARD (definitive republish): coref 0.4681 -> 0.4172 (floor 0.3202, CI-sep), common-noun 0.5671 -> 0.4932 (floor 0.4876, NOT separated; n=3915 -- the 18 redacted documents are still in this board loader: wiring exclude_scrubbed into the board's loader is the immediate follow-up, expected 0.5417 / n=3024), salience 0.2555 -> 0.2993 (n=137, informational); every other dimension identical; AGG 0.5930 = the honest baseline. The entity-QA instrument repaired (0 -> 200 questions, witness green). Witness repairs: referent_per_np landing (reflexives are a deliberate superset; 9/9). Left open: test_coref_graded_pick_landing W3a (a first-read process-state effect, 2/12 docs; not yet attributed -- bisect denied by the harness, owner can run it).
