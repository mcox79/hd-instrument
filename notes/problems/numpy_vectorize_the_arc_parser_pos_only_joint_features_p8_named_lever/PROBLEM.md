---
review: EXCELLENT
review_text: Reverified first-hand test_arc_parser_posfeat_vectorize.py 6/6 + a new pure-hdlab landing witness test_arc_parser_posfeat_landing.py (LANDED ArcParser.parse BIT-IDENTICAL to _parse_reference on 393,225 held-out arcs / 376 sentences -- heads+arcs+per-token margins, ==; vectorized branch fires 235/376; 7.6x over stock scalar). LANDED into hdlab/arc_parser.py (promoted verbatim): _bucket_idx/PosTables/pos_tables/_word_cols_tables/_word_cols_pyloop/sentence_flat_vec/sentence_scores_vec/sentence_scores_auto (GATE_THRESH=16); lazy PosTables from the wired tagger's 17-UPOS inventory + specials, fail-loud on unknown tag; parse/eval_uas route through the length-gated sentence_scores_auto (vec n>=16, scalar below + as byte-identity reference; the scalar path UNCHANGED). Byte-identity forbids splitting the float sum, not a faster CONSTRUCTION of the same input (verified at the integer-id-stream level, raw-float64-weight-safe). Whole-read within noise (parser ~7% of a read) but the parse is 2-2.6x + compounds (feeds the double-parse). Repointed the reverify's W4 timing baseline to the scalar reference (post-land ArcParser.parse IS the fast path). Adjacent follow-on named: the POS-tagger Viterbi (same table+gather technique). Output byte-identical -> no fidelity change. §2b folded. INTEGRATED 2026-09-05.
---
