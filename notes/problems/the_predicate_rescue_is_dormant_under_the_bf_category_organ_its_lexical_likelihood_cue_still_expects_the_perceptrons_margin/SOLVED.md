...
### (iv-b) The label-permutation null -- the sharpest control on a 12-cue fitted combiner

The cue block is rich and the combiner is fitted, so the cleanest test that it reads predicate-hood rather than the
population is to destroy the label and refit the *entire* pipeline unchanged. Recovery at fp<=0.10 on shuffled
labels: **0.0714 / 0.0548 / 0.0510** (UD-EWT / GUM / QA-SRL) against the real **0.7054 / 0.5616 / 0.6823**. The
machinery manufactures nothing: on scrambled labels it lands at the promotion rate, where an information-free
procedure must.
...
INTEGRATED_BY_STRATEGY 2026-09-14 08:17 local -- DONE by strategy after first-hand reverify (self-test PASS); diff applied verbatim (predicate_detector + the reader's rescue branch + the landing witness) and the asset moved live (data/frontend_assets/predicate_detector_bf_counts_v1.json); strategy consolidated the posterior read (the reader caches the category posterior MATRIX once per sentence, _cached_tag_matrix, and hands it to rescue_indices(post=...) -- one forward-backward per read, byte-identical). Witnesses: predicate_recall GREEN (the organ fires again), tense-agnostic 96 -> 238 events, extraction recall 11/11, shared cache 4/4. Board: byte-identical on all 7 dimensions (0.6344; the board's arms read the frontend, the rescue lives in the reader's event set). Sole-AUX arm selectable (HDLAB_PREDICATE_RESCUE_AUX=1), default OFF pending pri 110's joint A/B; path A filed inside pri 110 (7). Posterior-truncation lead withdrawn as measured (eps 0.01/0.001/0 identical).
