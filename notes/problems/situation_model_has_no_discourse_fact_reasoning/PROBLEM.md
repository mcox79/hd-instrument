...
priority:
review: EXCELLENT
review_text: "SOLVED (owner-DONE) integrated 2026-08-29 — a TWO-LEVEL result that REFUTED this brief's specific target with a measured+mechanistic reason AND built+proved the underlying reasoning-frontier CAPABILITY. Reverified FIRST-HAND test_discfact_store_bridging.py 25/25 PASS. L2 (the brief's target REFUTED): a reading-built per-entity discourse-fact store + 2-hop bridge is DEAD on the anti-typical LitBank coref residual (oracle 0.039; adding it is mildly anti-predictive, band BELOW) because the residual GOLD carries a mean 0.65 accumulated facts (58% exactly ZERO) vs the wrong pick's 13.4 — it is FRESHLY INTRODUCED and bound INTRA-SENTENTIALLY (the parson-who-rode example), so the brain cannot use a fact store there either (Centering Cb-absence; Sturt 2003 first-pass structural binding). This is the 7th channel dead on that residual and the DECISIVE one — it tests the exact mechanism the parent (and THIS brief) named as the fix. STRATEGY OWNS THIS MIS-SCOPE: I packaged the fact store as the anti-typical residual's 'measured real lever'; the measurement bar caught it — the residual's lever is the intra-sentential SYNTACTIC binder + richer p1, NOT the discourse-fact store (the 2nd time a brief I packaged had its residual-attribution refined by measurement — consistent with the p3/coherence findings that the anti-typical residual is syntactic). L1 (the CAPABILITY BUILT + PROVEN on the RIGHT population — the comprehension→REASONING frontier): on a built INTER-SENTENTIAL fact-decisive population (state-a-role-then-refer-by-action; roles/actions split DEV/TEST; bridges from static CSKG; name→role from text, no leak) fact_store 0.998 [0.994,1.000] vs the fact-BLIND graded resolver 0.504 (chance), +0.494 CI-sep; EVERY control at chance (info-free twin 0.544, KG-only-null 0.508, ablation 0.508) — the lift is the SPECIFIC reading-built binding; recovers 0.996 of the 248 fact-blind errors; survives 3-candidate (0.34→1.00); fires on-target only. The KG-only-null at chance directly CLOSES the parent's puzzle (a static KG 'connects but cannot discriminate' precisely because it lacks the discourse-specific binding the fact store supplies). Owner-pushed fidelity deepenings: L1b — replaced the 2nd bridge hop's hard KG match with a GRADED PPMI+SVD distributional coherence (the ATL PDP analog) that GENERALIZES to held-out role→action edges 0.700 vs symbolic chance 0.492 (partly CLOSES the ATL-distributional deviation); L1c — the dense FHRR bundle's crosstalk wall (K/D capacity law) is held FLAT to K=512 by hippocampal PATTERN SEPARATION (relation-indexed), though dense is adequate at D≥1024 for realistic per-entity load K~64. Grade EXCELLENT (a rigorous two-level result: refuted the brief's target AND proved the capability, with two owner-pushed fidelity closures). hdlab landing QUEUED (a NEW situation-model organ, verdict-independent): promote the reading-built discourse-fact store + graded 2-hop bridging RESOLUTION (discourse-age-gated — invoke only for candidates with ≥1 prior fact, NOT same-clause; FHRR; relation-indexed for scaling), wired for downstream consumers (QA / next-event / bridging / ToM), NOT a coref patch. AUDIT §2b folded (NEW discourse-fact-store organ entry + REFINE the coref-residual entry: its lever is the syntactic binder, not the fact store). Withdraw-first caveat (I concur): the L1 population is CONSTRUCTED; near-1.0 reflects idealized extraction + exact KG edges — a natural-text measurement is the #1 follow-on."
---

> ## ✅ SOLVER REVIEW — INTEGRATED 2026-08-29 (grade: EXCELLENT; SOLVED owner-DONE — a two-level result)
> **Verdict:** a rigorous two-level result — it REFUTED this brief's specific target AND built+proved the underlying
> capability. Reverified first-hand (`test_discfact_store_bridging.py` **25/25 PASS**).
> **L2 — the brief's target REFUTED (measured + mechanistic):** a reading-built per-entity discourse-fact store + 2-hop
> bridge is DEAD on the anti-typical LitBank coref residual (oracle 0.039, band BELOW) because the residual gold is
> FRESHLY INTRODUCED (mean 0.65 accumulated facts, 58% exactly zero) and bound INTRA-SENTENTIALLY (the parson-who-rode
> example) — the brain can't use a fact store there either (Centering Cb-absence; Sturt 2003). The 7th channel dead on
> that residual, and the decisive one. **Strategy owns this mis-scope:** I packaged the fact store as the residual's
> "measured real lever"; the bar caught it — the residual's lever is the intra-sentential SYNTACTIC binder + richer p1,
> NOT the fact store (consistent with the p3/coherence findings that this residual is syntactic).
> **L1 — the CAPABILITY built + proven on the RIGHT population (the comprehension→reasoning frontier):** on an
> inter-sentential fact-decisive population, `fact_store` **0.998** vs the fact-BLIND resolver **0.504 (chance)**, +0.494
> CI-sep, EVERY control at chance (info-free twin 0.544, KG-only-null 0.508, ablation 0.508); recovers 0.996 of the
> fact-blind errors; fires on-target only. The **KG-only-null at chance closes the parent's puzzle** (a static KG
> "connects but can't discriminate" because it lacks the discourse-specific binding).
> **Owner-pushed fidelity closures:** L1b — a GRADED PPMI+SVD distributional bridge (ATL PDP analog) generalizes to
> held-out edges 0.700 vs symbolic chance 0.492; L1c — hippocampal PATTERN SEPARATION holds FHRR resolution flat under
> load (dense adequate at D≥1024 for realistic K~64). **Grade EXCELLENT.**
> **Landing QUEUED (Q111 — a NEW situation-model organ, verdict-independent):** promote the reading-built discourse-fact
> store + graded 2-hop bridging RESOLUTION (discourse-age-gated, FHRR, relation-indexed), wired for downstream (QA /
> next-event / bridging / ToM), NOT a coref patch. **Audit** §2b folded (new organ entry + refine the coref-residual
> entry). Caveat (I concur): the L1 population is constructed — a natural-text measurement is the #1 follow-on.
