...
**19. STAGE 1 -- the PARTICIPANT-BOUND EVENT REPRESENTATION: a REAL agent signal at the fan-out level, and a CAUGHT
CONFOUND at the composition level (owner: "fix that, implement the strong stage 1, brain-foundational, not cheap";
`bound_events_v1`).** The brain-comparison localized the chain's dominant signal loss to STAGE 1: every event was a
BARE VERB-CONCEPT, discarding who/what-to-whom. The fix: an event = (action) BOUND WITH (agent discourse-entity), the
brain's role-bound situation-model event (Zwaan; Trabasso's causal network threads through participants). 100%
brain-foundational, glass-box, REUSES LANDED ORGANS, no gold / no parser / no LLM: `hdlab.pos_tagger` (UPOS) ->
clause-local mentions -> `hdlab.graded_role_assigner.hybrid_agent_pick` (Bates-MacWhinney Competition-Model AGENT,
owner-DONE) -> `hdlab.coreference_resolver` (gender/number salience) threads the agent to a stable discourse ENTITY ->
`hdlab.event_bundle.EventBundleCodec` FHRR-binds {PRED, AGENT} -> `hdlab.kg_traversal.KGStore` composes. Extraction is
clean: 55,024 events, **93% carry a resolved agent**. The result is honest and MIXED -- one real win, one caught
confound:
- **THE REAL WIN -- FAN-OUT IS A GENUINE AGENT SIGNAL.** Global mean causal fan-out bare-verb 72.4 -> bound
  (verb,agent) 18.0. The rigorous control that makes this load-bearing: conditioning a verb's occurrences on the AGENT
  lowers the distinct-effect set MORE than a SIZE-MATCHED RANDOM partition of the same occurrences (agent 6.48 vs
  random-partition 6.78, **delta -0.30 CI[-0.43,-0.20] CI-sep**, 180 verbs). The agent carries GENUINE causal
  information about which effect follows -- not merely data-splitting. This is the brain-foundational Stage-1 result:
  participant identity is a real causal cue, exactly as Trabasso's participant-threaded causal network predicts.
- **THE CAUGHT CONFOUND -- the 2-hop composition "lift" is a GRAPH-SPARSITY ARTIFACT, not agent structure (a can-fail
  control FIRED; I retract the lift as evidence).** Naively it looked spectacular: holding vector type atomic, the
  dense bare-verb graph composes at 0.013 (below chance) while the sparse bound graph composes at 0.617 (+0.60 CI-sep)
  -- a ~47x "lift". BUT the SHUFFLED-AGENT control (globally permute the agent column: preserve agent frequency + graph
  sparsity, DESTROY the agent<->event correspondence) composes at **1.000 -- BEATING the real-agent 0.617**. So a
  RANDOM label reproduces (and exceeds) the composition win: the held-out-2-hop-in-top-10 metric is trivially high on
  ANY sufficiently fragmented graph (near-singleton out-degrees), and a random split fragments MORE. The composition
  lift measures graph sparsity, not participant causal structure. HONEST CONCLUSION: the clean Stage-1 evidence is the
  FAN-OUT control (above); the composition metric is confounded and must be measured sparsity-controlled (e.g. the
  NECESSITY estimate over bound events, next).
- **SECONDARY (on the confounded metric, reported for completeness):** indexed atomic codes (0.617) >= distributed
  FHRR (0.463) >= bag (0.294) -- consistent with the substrate's index-specific-events lever, but this rides on the
  confounded metric so it is not load-bearing. NET: participant-binding is BUILT (glass-box, 93% agentful) and carries
  a REAL causal signal at the fan-out level; the composition improvement was a sparsity artifact my own shuffle control
  caught and I retract it. The genuine value of Stage 1 must next be shown on a sparsity-controlled measure -- fold
  bound events into the necessity world-model and test whether binding SHARPENS the necessity estimate.

**20. STAGE-1 VALUE, the CLEAN sparsity-proof test -- participant-binding SHARPENS the causal READ (the composition
confound's honest resolution; `bound_necessity_v1`).** Composition was the wrong measure (it is graph-sparsity-
confounded). The right measure is the intrinsic causal READ -- prediction + counterfactual necessity -- which is a
per-event / ablation quantity, immune to the sparsity confound. Built it, with the KEY capacity-matched control: the
BOUND model (verb + agent features) has MORE parameters than BARE, so a BOUND-SHUFFLED model (same architecture, agent
column globally permuted) rules out capacity. RESULT (simplewiki, continuous online, n=42,386 scored, all predict the
same target = next verb, paired): held-out surprisal **bare 6.915 > bound 6.895 < bound-SHUFFLED 6.922** -- bound beats
bare (**-0.021 CI[-0.023,-0.018]**) AND beats the capacity-matched shuffle (**-0.027 CI[-0.030,-0.024] CI-sep**); the
shuffle gets ZERO gain (it is the agent INFORMATION, not the parameters -- the exact control the composition metric
FAILED). NECESSITY over bound events: reader **0.310 vs random 0.013** (+0.297 CI-sep). REFERENTIAL COHERENCE, now
MEASURED: the necessary antecedent SHARES the effect's agent more than a random context event (**0.124 vs 0.073, +0.051
CI[0.048,0.054] CI-sep**) -- Trabasso's participant-threaded causal network, intrinsically confirmed. **This is the
clean, decisive, sparsity-proof win: knowing WHO acts genuinely sharpens the causal read.**
...
denser source than raw negation cues.

**22. LEVERAGING THE PRIOR MULTIHOP WORK -- causal-chain LENGTH-GENERALIZATION, and the composition confound closed by
the substrate's own protocol (`length_gen_v1`).** The owner flagged the project's large body of positive multihop work.
The landed CLUTRR length-generalization cell (`exp_reasoning_readout_length_generalization_clutrr_cg_v1`, HARD_PASS) and
KGStore (CERT-585) already established the key principle that reframes my finding-19 confound: **"multi-hop composition
over a good/sparse code is close to FREE (a geometric consequence)"** -- exactly why my composition "lift" was a sparsity
artifact. The genuine multihop test is LENGTH-GENERALIZATION (where the free geometry breaks) vs a FLAT single-hop
must-fail baseline. Adopted it on causal chains (indexed bipolar-HD codes, KGStore iteration, out-degree capped equally
across conditions). RESULTS (simplewiki, cap_deg=3, Lmax=4): (A) OUT-DEGREE-MATCHED AGENT-SPECIFICITY -- real-agent
composition does NOT beat shuffled-agent at any depth (real-shuf iter-acc -0.13 to -0.21) -> **composition is
graph-structure-driven, NOT agent-specific**, confirming the finding-19 confound and the prior art; the agent-specific
value is in the READ (finding 20), not the chain. (B) REAL CHAINING -- the FLAT single-hop baseline COLLAPSES to 0.000
at depth L>=3 (cannot chain, must-fail confirmed) while the iterative VSA reasoner RETAINS signal there (L3 0.067, L4
0.034 on the dense real graph; L3 0.24, L4 0.17 on the sparser graph) -> genuine chaining, with length-gen STRENGTH
governed by graph density (sparse composes strongly, dense degrades -- exactly the prior-art regime). NET: the composition
question is honestly closed (not agent-specific; a graph-density property), the substrate's proven multihop reasoner
demonstrably chains causal edges, and the two connect cleanly to the historical CLUTRR/CERT-585 results.
> PHASE-DIAGRAM NOTE (owner): the dense-graph length-gen degradation is NOT a ceiling -- density is a FREE lever
> (substrate-phase-diagram-parameters-are-free-to-move). To operate the causal-chain reasoner in its strong regime, MOVE
> the operating point to the sparse side (top-k edge pruning / higher necessity threshold), where length-gen is robust
> (L4 ~0.17). The agent-specificity conclusion (agents don't help composition) is orthogonal to density and stands.

**23. THE FULL SITUATION-MODEL EVENT -- phase-diagram DENSIFIED affect + goal-state, folded into the live reader's
representation (owner: "we have the phase diagram, we can move sparse->dense trivially; implement all others, brain
foundational not easy"; `situation_event_v1`).** The event is now the LIVE READER's EventRecord representation --
{PRED, AGENT, PATIENT, VALENCE, GOAL} = Zwaan's situation-model dimensions -- each dimension via its LANDED organ, all
glass-box, no gold/parser/LLM:
- **PHASE-DIAGRAM DENSIFICATION OF AFFECT, and it VINDICATES the owner's point.** Finding 21's negation signal was a
  NULL, but only because it was SPARSE (~7% of events). Per the phase diagram (sparse<->dense is a free lever, not a
  ceiling), I densified affect: continuous WARRINER valence (`hdlab.affect_lexicon`, Barrett/Russell CORE AFFECT),
  negation-flipped. Coverage jumps **0.07 -> 0.97** of events. RESULT: the DENSE valence carries **real
  capacity-matched causal info -- marginal -0.015 CI[-0.026,-0.004] CI-sep** vs a valence-shuffle, where the sparse
  negation was null. **Densifying turned the null into a real signal: sparsity WAS the issue, not affect -- exactly the
  phase-diagram lesson.**
- **GOAL-STATE (Suh-Trabasso reinstatement) adds real causal info.** `hdlab.goal_register` extracts explicit goals;
  the agent's current goal PERSISTS across events (reinstatement) -> dense per agent (coverage 1.00). Marginal **-0.018
  CI[-0.024,-0.012] CI-sep** vs a goal-shuffle -- intentional state carries causal information about what happens next.
- **FOLDED INTO THE LIVE READER.** The event now carries exactly `situation_reader.EventRecord`'s dimensions (agent,
  patient, affect, goal), built with the reader's OWN organs (graded_role_assigner, coreference_resolver,
  affect_lexicon, goal_register) glass-box (no gold). The counterfactual-necessity READ over this full situation-model
  event is strong (reader 0.336 vs random 0.007) with CI-sep referential coherence. NET: all three owner follow-ups
  are done brain-foundationally -- affect densified (phase diagram), goal-state added, and the read now operates over
  the live reader's own situation-model event; the proposed hdlab wire (Q111) is the necessity READ as an additive
  pass over EventRecord.
...
- PARTICIPANT-BINDING (Stage 1) sharpens the intrinsic causal READ, capacity-matched: agent-in-context lowers
  next-event surprisal vs a capacity-matched shuffle (-0.027 CI-sep); PATIENT adds further (-0.020 CI-sep); necessity
  over bound events holds with measured referential coherence. Composition is a graph-density property, NOT
  agent-specific -- the agent value is in the read, not the chain (consistent with prior 'composition over good codes
  is free'). Reusable organs: pos_tagger + graded_role_assigner (agent+patient) + coreference_resolver + event_bundle.
...
FOUNDATION -- a real CI-separated result, but not yet a complete narrative causal reader. (3) OWNER-ENDORSED
STEP BUILT with an HONEST MIXED result: the VSA/FHRR participant-BINDING (Stage 1, finding 19) is built (93% agentful,
glass-box). REAL WIN: the agent carries genuine causal info (fan-out drops beyond a random-partition control, CI-sep).
CAUGHT CONFOUND (my shuffle control fired -- reported, not hidden): the ~47x 2-hop composition 'lift' is a graph-
sparsity artifact (a shuffled-agent graph composes even better, 1.000), so I RETRACT the composition win; the genuine
Stage-1 value must be re-measured sparsity-controlled (necessity over bound events). (4)
...
1. DONE -- **participant-BINDING (Stage 1) BUILT AND VALIDATED CLEAN (findings 19-22).** Events bound to AGENT (+
   PATIENT) discourse-entities via FHRR (glass-box, 93% agentful). The composition confound (finding 19) is resolved on
   the sparsity-proof intrinsic READ: knowing the agent lowers next-event surprisal vs a CAPACITY-MATCHED shuffle
