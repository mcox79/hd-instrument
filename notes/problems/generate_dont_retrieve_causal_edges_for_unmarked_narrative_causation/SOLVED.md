...
1. BUILT (mixed) + HIGHEST remaining -- **participant-BINDING (Stage 1, finding 19 / `bound_events_v1`).** Events are
   now bound to their AGENT discourse-entity via FHRR (glass-box, 93% agentful). REAL WIN: fan-out drops beyond a
   random-partition control (agent carries causal info). CAUGHT CONFOUND: the 2-hop composition 'lift' is a graph-
   sparsity artifact (shuffle control fired) -- RETRACTED. **THE decisive next measurement:** fold participant-bound
   events into the NECESSITY world-model and test whether binding SHARPENS the counterfactual-necessity estimate
   (a sparsity-controlled, intrinsic measure) vs bare-verb events -- that is the clean test of Stage-1's value, and it
   is not yet run. Also: (a) add the PATIENT slot + goal/affect to the bind; (b) a sparsity-controlled composition
   metric (out-degree-matched) if composition is to be used at all.
