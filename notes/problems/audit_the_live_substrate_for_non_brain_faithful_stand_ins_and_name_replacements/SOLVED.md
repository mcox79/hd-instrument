...
8. **FINALIZATION RE-VERIFY (2026-09-09) — the disk moved under the catalog while this was open; corrected in place.**
   Strategy CONT-29 has begun landing exactly what this audit catalogued: **C4 PARTIALLY REMEDIATED** — the reader's
   direct `experiments/` imports (`_space_reader`/`_belief_reader`/`_temporal_order_register`/`_forward_prediction_live`)
   were promoted to `hdlab/`; verified first-hand that importing the reader now pulls experiments only transitively via
   `context_grounded_valence` (L61-64) + `temporal_reasoner` (L54-55) — that is the residual C4. **C1 de-leak STARTED**
   (`3e7a80e6b` promotes `online_entity_cluster` — pri-6 step 1). **pri-1 integrated** as owner-DONE (a located-negative)
   with a successor posted (`31463fb11`). Witness W5b updated to assert the promotion; catalog C4 annotated. The catalog's
   direction is confirmed by strategy's own landings.
...
## NEXT PRIORITY STEPS (ordered — the top-down integration pass this audit sets up; interface per consumer in `PRI1_INTEGRATION_MAP.md`)

**DEEP FIX (the spine — the parser drills proved in-sentence signal is exhausted; only this remains):**
- **P-A. Preconditions (must land before pri-1 can pay off):** (i) **C1 gold-coref de-leak** — already filed pri-6 (`replace_the_entity_gate_gold_coref_inheritance_with_online_cue_based_clustering`); pri-1 rolls out over *resolved* participants, so a leaked coref scores the expectation on a peek. (ii) **wire the meaning channel live at read()** (DEBT-3, `meaning_foundation` is LATENT) — pri-1 reads result-state predicates over it.
- **P-B. Build pri-1** (`build_the_generative_result_state_world_model`, priority 1 — its own brief; do NOT duplicate). It emits the top-down expectation (E1 next-event/participant, E2 result-state, E3 goal-state, E4 cause) that is the ONE register-robust signal the whole reader lacks.
- **P-C. First consumer = the C3 parser cluster.** Local moves are ALREADY PROVEN in `experiments/` (globally-normalized Matrix-Tree posterior + commit-not-defer + graded joint settle; obl-spatial packages the commit) — land those, then wire pri-1's E1 as the top-down term in the attachment competition (move 4, closes the recurrent loop). This is where the OOD-reliability wall closes.
- **P-D. Descend the other consumers** in binding-constraint order (spatial coverage → temporal → causal → copular → world-state recipient → coref next-mention prior → bridging), each consuming its E-projection per the map — ONE thread at a time against a fresh board.

**LANDABLE NOW (independent of pri-1 — local wins strategy can pick up immediately):**
- **C4 self-containment promotion** — promote the ~6 `experiments/*` cells the live reader imports at inference into `hdlab/` (mechanical; the standard is already followed elsewhere). A reproducibility gate-fix, not a fidelity one.
- **C2 `situation_predict` → `typed_coref` binding** for the CLUSTERING consumer (de-restrict the person-gate); payoff is leak-masked until C1 lands.
- **C3 local moves** (posterior + commit + drop-logistic) — net-positive and proven even before the loop closes.
