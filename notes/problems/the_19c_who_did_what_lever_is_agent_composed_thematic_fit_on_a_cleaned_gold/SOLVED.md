...
- **Agent x verb COMPOSITION is a real FORWARD-PREDICTION mechanism on 19c (NEW, brain-faithful positive):** on the
  ...
  dominance and the 100%-active regime pre-empt it). Its brain-faithful home is the FORWARD-PREDICTION pathway (the
  p2 N400/surprisal organ), not the who-did-what role selector. This aligns the audit's "selection store" entry with
...
1. **Retire "agent-composition is the 19c who-did-what SELECTION lever" -- but KEEP the mechanism, re-homed.** It
   ...
   agent-shuffle, CI-sep (n=4000). Route composition to the FORWARD-PREDICTION pathway (the p2 N400/surprisal
   organ), not the role selector -- this is where the brain shows the Bicknell effect.
