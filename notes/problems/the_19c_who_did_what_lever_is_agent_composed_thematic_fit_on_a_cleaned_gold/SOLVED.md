...
## WHAT REMAINS TO REACH OPTIMAL -- the full backlog (disparate components, each with its measured leverage/status)
*Nothing here is landed (solver scope = experiments/verification only; strategy lands hdlab wires, Q111). Each item
lists the on-disk evidence, whether it is MEASURED or PROPOSED, and the cell/doc that owns it.*

**A. SELECTION pathway (who-did-what) -- order-dominant; the levers are STRUCTURAL/cue, not the meaning store.**
- **A1. Morphological CASE cue in `hdlab/graded_role_assigner`.** MEASURED ABSENT on disk (cue set lacks he/him,
  who/whom); highest-validity Competition-Model cue, BETTER preserved in 19c. Cheapest fundable selection lever. [file it]
- **A2. NP-head chunker** (compound + genitive head selection). MEASURED +0.043 CI-sep -> 0.981 selection
  (`exp_whodidwhat_full_system_v1`). The real clean-19c who-did-what lever; helps every downstream selector.
- **A3. (DATA, BLOCKED) A position-AMBIGUOUS (non-canonical/passive) 19c patient gold** -- the only regime where a
  thematic-fit store is a SELECTION lever; ABSENT from the current gold (0 passive) and auto-construction is blocked
  by 19c parser-era-robustness (p3). Needs a 19c-robust parser/SRL, or a hand-built non-canonical gold.

**B. PREDICTION pathway (anticipation / N400) -- where thematic fit lives; representation is the lever.**
- **B1. Upgrade `hdlab/predictive_reader`** from marginal-CENTROID-over-12d-SPOKE to PRECISION-WEIGHTED composed-
  EXEMPLAR over a ~200-d register-native distributional HUB. MEASURED +0.083 CI-sep = 2.3x held-out MRR, DE-RISKED
  to a reusable class (`IdealComposedPredictor`, `exp_ideal_recipe_v1`). Settings: gamma~2 (MRR-optimal), centroid
  backoff on OOV agents, NO naive spoke+hub concat, KEEP FHRR. This is the single highest-value prediction wire.
- **B2. A genuinely richer ATL-grade HUB representation.** MEASURED directional: the composition margin grows with
  dimensionality but plateaus ~200 because VOCABULARY COVERAGE caps it; on the graded metric richer rep helps
  +0.009 CI-sep. Lever = more/cleaner exposure -> higher-coverage, higher-fidelity hub (an offline FOUNDATION asset).
- **B3. A structured MULTI-PARTICIPANT / discourse situation model (Sentence-Gestalt).** MEASURED +0.035 CI-sep on
  the graded 2AFC on multi-participant events (`exp_sentence_gestalt_multiparticipant_v1`), oblique-shuffle losing.
  The fundable richer STREAM for prediction; a caricature on single sentences, so build/test it on multi-participant
  + discourse items only (FHRR unbind readout + a schema-prototype store).
- **B4. Resnik class-conditional selectional-preference back-off** (A(v,c) over WordNet classes) for OOV coverage --
  the FAITHFUL taxonomic back-off (naive hypernym-averaging HURT; dense-kNN raised coverage 43%->98% but is noisy).
  PROPOSED (per `BRAIN_MECHANISM_DRILL_more_ideal_implementation.md`), not yet built.

**C. MEASUREMENT infrastructure -- the metric itself was wrong.**
- **C1. Adopt the frequency-controlled graded 2AFC** (Chambers-Jurafsky/DTFit) as the thematic-fit/prediction eval;
  exact next-word MRR is near an INTRINSIC ceiling (~164 patients/verb) and measures the wrong competence
  (`exp_prediction_ceiling_diagnosis_v1`, `exp_thematic_fit_2afc_v1`).
- **C2. Re-validate the prediction / 2AFC / situation-model effects on GOLD-labeled roles.** CAVEAT: current results
  are on raw-exposure PROXY triples (agent/patient/oblique by proximity heuristics), not gold role labels.
- **C3. Correct the baseline board's 19c who-did-what arm** (~0.43 is ~76% gold oblique-contamination; the honest
  cleaned number is position ~0.92).

**D. ADJACENT (from the parent, listed for completeness).**
- **D1. Copular is-a binding schema** -- base reader 0/376 on predicate complements, ~23% of the population,
  register-independent; already filed as `the_reader_has_no_copular_is_a_binding_schema` (parent).

**E. WIRING / integration (strategy-owned, Q111).**
- **E1. Land B1** (the predictive_reader upgrade) behind a default-off flag, witnessed, measured on the live reader
  before any capability claim. Nothing in this submission is landed.

**THE ONE-LINE OPTIMALITY STATEMENT:** selection is order-dominant (match the brain -- levers are A1 case + A2
NP-head, not the meaning store); prediction is representation-bounded (levers are B1 the hub-composed-exemplar
upgrade, B2 a richer hub, B3 a multi-participant situation model); and the measurement itself must move to the
graded metric (C1). The distance to optimal is representational QUALITY + STRUCTURE + the right metric -- not a
fancier integrator (the faithful Bayesian "more ideal" was an honest negative: cue quality, not the op, is the
bottleneck).

## INTEGRATED_BY_STRATEGY 2026-09-02

Reverified `verification/test_19c_composed_cleaned_gold.py` FIRST-HAND: **22/22** checks pass (incl. W19
NP-head lifts SELECTION +0.063 CI-sep→0.981 while thematic-fit selection value = 0; W20 prediction is
representation-bounded, hub 0.146 >> spoke 0.061, faithful integration does NOT beat hub-only [cue
QUALITY is the bottleneck, not the integration op], coverage 0.43→0.98, twins lose; W21 graded 2AFC
reveals real competence [verb-atypical 0.715, agent-composition +0.018 CI-sep on verb-typical foils,
exact-match hid it]; W22 multi-participant situation model adds +0.035 CI-sep, oblique-shuffle loses).

Accepted **REFUTED (composition-as-SELECTION) + a fully-drilled brain-faithful reconstruction**, rating
**EXCELLENT** — a model refutation-at-power: it BUILT the larger cleaned gold (n=669, precision 98.5%)
and POWERED the parent's +0.076, which KILLED it (small-sample noise: separates only 7% of the time at
n=171). Reconstructed the real mechanism (Competition Model — English who-did-what is word-order-dominant,
position gets 0.918 on clean direct objects exactly as the brain does English; thematic fit changes
SELECTION only at syntactic ambiguity, and the 19c gold is 100% active so that regime is ABSENT; the
Bicknell agent×verb effect is a PREDICTION/N400 phenomenon, wrong instrument for selection), and
validated composition on the RIGHT instrument (forward prediction +0.032 MRR CI-sep; a de-risked
IdealComposedPredictor beats the organ's method +0.083 CI-sep = 2.3x held-out MRR). Exemplary
disk-outranks-brief + self-correction (refuted its own parent's headline).

**NO hdlab wire** (a located negative; nothing landed). **AUDIT UPDATE folded into §2b.** Priority cleared.
The measured, de-risked forward levers are routed as follow-ons (NOT lost): the STRUCTURAL selection lever
(NP-head chunker +0.043→0.981 + morphological case cue) → `the_who_did_what_selection_residual_is_structural_np_head_chunking_and_case_not_meaning`;
the composed-EXEMPLAR forward-prediction upgrade (2.3x MRR, "highest-value wire") →
`upgrade_predictive_reader_to_a_composed_exemplar_predictor_over_a_richer_hub`. The DEEP representation
finding (prediction is representation-bounded; the 12-d spoke is dead, a ~200-d hub is alive) CONVERGES
with the coherence-prior root + the WSD a_s residual into the north-star P1 (the orthogonal learned
individuation/meaning representation). **BOARD INTEGRITY:** the 19c who-did-what gold is ~76%
oblique-contaminated (honest cleaned direct-object ~0.92) — caveat added to the baseline board.
