...
## QUESTIONS
None blocking. One decision for strategy: the demonstrated-real lever on clean 19c who-did-what is **NP-head
chunking** (+0.043 CI-sep, reaching 0.961) -- a parser/chunker refinement, not a meaning store. Route it to the
parser wire, or file it as its own small problem? My recommendation: file a small `reader_picks_np_modifier_not_head`
problem (compound + genitive head selection), since it is a clean, measurable, brain-independent structural win.

## NEXT STEPS FOR STRATEGY (ordered)
1. **Retire "agent-composition is the 19c who-did-what lever."** It ties its info-free twin at power; the parent's
   n=171 CI-sep was noise (7% at power). Do NOT fund a bigger-gold composition build -- I built the bigger gold and
   it kills the effect.
