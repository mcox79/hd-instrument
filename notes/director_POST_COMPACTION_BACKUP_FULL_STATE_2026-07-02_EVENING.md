...
**Tandem v2 SMOKE result CORRECTED (Fix#28 hit #9):** cell-emitted HARD_PASS at +0.778 lift OVER-CLAIMS. Cell-author self-audit flagged 3 smoke-only artifacts inflating the lift: (a) hashed word-bag proxy for bge (real bge deferred to FULL), (b) ground-truth role-filler structure fed both ends (no NER extraction cost paid), (c) H1 regression to verify_v1 FAILED (BGE_ALONE_NATURAL = 0.000). **Skunkworks RULING (commit a5c96995): MB_MECHANISM_RUNS_SMOKE_TIER SCOPE_TIGHT; USER hypothesis NEUTRAL-TO-WEAK-NEGATIVE**; substrate got ground-truth bindings piped both ends + bge was MD5 hash-bag not real encoder. FULL required (real bge + light NER + restored BGE_ALONE floor + H1 regression).

**RAG-with-substrate-composition landed HARD_FAIL (commit 7be789f91):** ARM_TANDEM_RAG=0.083 (WORSE than substrate-alone 0.517); ARM_TANDEM_SUBSTRATE_ORACLE=0.783 (mechanism works if fed correct chunks). Diagnosis: **retrieval-architecture defeat, not substrate defeat.** bge single-shot dense retrieval CANNOT surface bridge chunks for multi-hop queries. Substrate composition has nothing to compose over. Fix candidates: iterative retrieval OR query decomposition.

**Substrate-as-selector research drill (commit 27738413d) HEADLINE SURPRISE:** verify_v1 NEVER ACTUALLY TESTED VSA BINDING. Code-read showed +0.027 came from PCA-whitening + Rocchio + MMR (established IR techniques), NOT VSA compositional binding. The "prior positive fleet witness" for substrate rerank never existed. **USER intuition TRUE but NARROWER:** substrate wins at explicit structure + ≥2 hops + complete graph + no world-knowledge; LLM wins at NL/implicit structure + world-knowledge tasks. Critical negative precedent found: arXiv:2606.24948 tests true HRR/FHRR on 2-hop KG binding → FAILS AT CHANCE. P_deflated=0.42/0.40.

**COMPOSITE PICTURE (3 independent signals converge):** the rerank architecture as designed does NOT work — Skunkworks demote + RAG HF + no prior positive precedent. BUT substrate composition primitives WORK when fed correctly (ORACLE=0.783). **Bottleneck is retrieval-to-substrate-input, not composition itself.** Production path forward: iterative retrieval / query decomposition / richer KB indexing. The M3 cortex layer design (`notes/design_m3_cortex_layer_...md`) needs revision to reflect iterative retrieval, not naive rerank.
