...
- **BONUS: causal_sign instrument-gap CLOSED** (`24765b7a6`, `board_causal_sign_dimension`) — built HONESTLY after careful full-scale measurement (which caught a defect-in-waiting: a naive wrap would have scored causal_sign *losing*). Honest read: beats the scrambled-sign falsifier CI-sep (structure is load-bearing) but ties co-occurrence.
...
- **`board_event_goal_congruence` for `affect_structured_matcher`** — ✅ ALREADY ARMED: `board_event_goal_congruence_dimension` (L1204) scores it directly (model 0.9750 vs swept-hub floor 0.4917 = the +0.483). No action.
- **SpaceEval-precision arm** for the obl-spatial +0.0102 upstream win — NOT cleanly buildable: the solver's own finding is the spatial dim is construction-coverage-dominated + SpaceEval is saturated, so an arm would not show the win. Skip unless a better instrument is designed.
- **`board_predictive_causal_necessity_dimension`** already exists (line ~113) — DONE, do not rebuild.

**→ INSTRUMENT-GAP SWEEP COMPLETE (2026-09-11):** causal_sign closed tonight; event_goal + predictive_causal already armed; SpaceEval not cleanly scorable. No open clean instrument-gaps remain.
...
- [x] 7a. **AUDITED 2026-09-11 (evidence-based): NO clean net-positive flips available.** The 18 ISLAND capabilities break down as: (a) most are "works in isolation, ISLAND until a task NEEDS it" (schema pattern-completion, ToM Sally-Anne, Stage-1/1.5/2A causal-chain probes, hippocampal_encoder, grounded appraisal) — they need a live CONSUMER/task, not a flag flip (a wiring BUILD, gated on the generative world-model / a task); (b) the two proven "big" wins are on BANNED 19c corpora → untrustworthy: `thematic_role_labeler cue-integration +0.264` is on **McGuffey** (19c-banned; and it's a standalone experiment cell, NOT an hdlab organ — only its utility fns lemma_verb/is_strictly_intransitive are wired), `coref_distractor_suppress +0.043` is on **LitBank** (19c); both need a MODERN-gold re-measurement before any flip (a fleet problem, not a mine-side flip); (c) the documented default-off organs (`graded_role_marginal` 2× parse cost, `decode="exact"` off the live arc-eager path, meaning-fusion pieces owner-gated on pri-5) are OFF for good reasons. Per memory, MANY are "flip-tested net-neutral/negative — do NOT re-flip." **CONCLUSION: the wiring burn-down is NOT low-hanging fruit — it is consumer/task-gated build work + modern re-measurement, best done as fleet problems or when the generative world-model provides the consumers. No mine-side flip is a clean net-positive win.**
