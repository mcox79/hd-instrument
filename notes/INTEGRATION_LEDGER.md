...
**🎯 BOARD REALIZED (full `--run` 2026-09-06, `data/exp_situation_model_qa_modern_v1/metrics.json`): the 3 reasoners moved from LIVE→SCORED — all CI-separated on their own modern arms.** Headline 7-dim aggregate **0.6052** UNCHANGED (by design — the reader's core dims; the reasoners score as separate arms OUT of the aggregate). New-arm scores: **occ_appraisal (affect) 0.94 vs 0.44 floor** (CI-sep); **temporal_overlap 0.9938 vs 0.50** (CI-sep); **temporal_before_after 0.5933 vs 0.5236** (CI-sep); **spatial_relational 0.2835 vs 0.2205 end-to-end** (CI-sep); coarse_sense 0.5236 vs 0.3544 (CI-sep). ⚠️ TWO arms DEGRADED to null this run (`selective_reliability`, `causal_multihop` — the causal multihop DID compute in-line, TellMeWhy mh 0.2344 vs 0.000 CI-sep, but the arm row emitted null) → a board-arm robustness follow-on (feed full data / fix the null-row). So: the three reasoning capabilities are now SCORED (not just live); the reliability + causal arms need a board-plumbing fix to show their numbers.
