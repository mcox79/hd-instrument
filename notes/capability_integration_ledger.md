...
## 08-04 REFINEMENT (VET of the prior finding, per USER "vet those findings hard") — my earlier "meeting premium failed" was too flat
Read depth_scaling_v3 metrics properly (not just the verdict string). CORRECTED read: (1) it IS a real powered test (n=200 x 5 seeds, sanity floor passes) — trustworthy as measured. (2) The FAIR control is arm_random_meet_baseline (meet at WRONG midpoint): bidir-correct-midpoint beats bidir-random-midpoint by only +0.01..+0.035 => meeting adds ~nothing ON THIS TASK (legit). (3) BUT the "bidir < forward_half" comparison I quoted as headline has a FAIRNESS AMBIGUITY — forward_half at d=3 does only 1 hop (0.74), a shallower subproblem than the full 3-hop chain; not clearly a fair comparator. Lead with the random-midpoint control, not forward_half. (4) It is KG-CHAIN-COMPLETION, a DIFFERENT task from our coherence-SELECTION (2-candidate ranking) — so "no meeting premium" is a CAUTION for v4, NOT a transferable verdict. v4 is judged on its OWN fair test (novel types held-out, floors fail, shuffled-control collapses, meeting-quantity NONZERO) + correct impl (forward SR converged, combine correct), on disk. Discipline note: I over-propagated a verdict string as a conclusion; corrected after reading the actual arms/controls.

## 08-05 GATE CLOSED on 5 invisible islands (Skunkworks/auditor, disk-verified)
Per notes/capability_reconciliation_invisible_islands_audit.md: the reconciliation audit found 5 HARD_PASS-class organs with ZERO capability_registry.jsonl rows. The audit flagged them but the WIRE/SHELVE gate was never closed. Filed 5 rows (registry 59->64), each disk-verified off metrics.json this session (not trusting audit labels). NO material mislabels found (action_selection HARD_PASS via pfc_bg_composed confirmed; slot_attention_wm reuse = 21 text-grep consumers vs audit's ~12 import-graph, both agree TRAPPED_SHARED).
- grounded_appraisal_sim_earned -> WIRE (target: grounded-meaning foundation / semantic goal-resolution; gated by the transfer_to_text EXTRACTION_BOTTLENECK). MECHANISM_EARNS, FULL_heldout=1.000 vs NO_APPRAISAL=0.239, earned_restore=1.000 vs recency=0.303.
- theory_of_mind_sally_anne_nested_hrr -> WIRE (target: goal-attribution under divergent belief; partition situation-model by protagonist, map line 133). HARD_PASS Q2_full=0.806 vs 0.138, gap 0.668, oracle 1.0, 5 seeds.
- vamp_ep_deep_chain_solver -> SHELVE (revival: NL causal-chain transfer smoke; all wins synthetic KG, depth 200/K 5000/noise 0.30 acc=1.0, narrative UNTESTED).
- action_selection_basal_ganglia_gonogo -> SHELVE (revival: when grounded appraisal->action layer is built; re-point don't rebuild; value_actor_lift=0.733; pfc_gate_cfrpe_trained_v2 narrow n_fair=2/7).
- slot_attention_wm_stateful_core -> SHELVE (revival: if primary situation_model_accumulate WM path hits a maintenance/update ceiling this core clears; secondary track).
Detector delta (tools/capability_registry_audit.py): HIGH invisible-island 57->52 (5 fewer); unregistered_hdlab 62->60 (2 fewer); 7 fewer flags total. capability_registry.jsonl is git-ignored (local-disk write, consistent with all existing rows).
