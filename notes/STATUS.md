...
### 2026-09-03 (LATEST) -- 🌙 **OVERNIGHT AUTONOMOUS RUN IN PROGRESS (owner away until morning, FULL authority). READ `notes/OVERNIGHT_PLAN.md` FIRST — it is the driver.**
🔴 **#1 ON RECOVERY / ON ANY CRON FIRE: read `notes/OVERNIGHT_PLAN.md` and advance the loop — do NOT wait for owner confirmation (authorized).** Mandate: (1) turn the reader's ~13 default-off capability flags ON one-at-a-time top-down, measure net-positive vs net-negative WITH per-dimension signal tracing (a bad bottleneck must not silently kill downstream), flip the winners default-ON in `hdlab/situation_reader.py`; (2) FIX the downstream role-output organs to work with the upstream improvements as they go on; (3) integrate any SIGNIFICANT in-progress solver solution (treat as done). Cron `d94a4466` (session-only, :18+:58) drives it — **if this is a fresh session, RECREATE the cron** (see OVERNIGHT_PLAN.md) then resume. IN FLIGHT: `tools/flag_activation_sweep.py` → `data/flag_activation_sweep/results.json` (greedy forward activation + per-dim trace). Two SOLVED in-progress solutions awaiting integration: `the_reader_has_no_copular_is_a_binding_schema`, `the_who_did_what_front_end_abstains_on_a_fifth_of_answerable_clauses`. Path-limit commits; NO push.
