...
1. **THE MEANING CHANNEL (#1 substrate gap, owed since 2026-08-24) — and it is GATED ON A HEAVY REPRODUCTION, corrected 08-30.** `the_live_meaning_organ_has_no_distributional_channel_to_be_taught_by` (owner-DONE, EXCELLENT, verdict-integrated) proved the flat-store decode route is STRUCTURALLY dead (256-wide projection crosstalk at support>d) BUT found a brain-foundational fix (Route A: an OFFLINE PPMI+SVD static asset; Route B: an online sparse per-lemma count store — pattern separation) that clears the substitutability bar at 0.865. **⚠️ REVERIFY IS BLOCKED AT HEAD (attempted 08-30):** the two "scaffold-free" witnesses actually depend on GITIGNORED scratch caches (`scratch/sparse_code_real_task/real_cache.npz` via the distillation cell; `scratch/cue_information_audit_v1/buckets_full.npz`) that are ABSENT at HEAD — the NEGATIVE/diagnosis reproduces (decode-SNR witness 6/7) but the POSITIVE (0.865, "store-representation-is-the-lever") does NOT run without regenerating them, and that regeneration is a CORPUS-SCALE pass (cue_information_audit `--grid full` is documented >1800s). The 08-24 integration was valid (caches were fresh then); they just didn't persist. **SCOPE REFINED 08-30 (better than feared): the HEAVY corpus data PERSISTS — both checkpoints `data/exp_dissociation_score_instrument_v1/units.jsonl` + `data/exp_cue_information_audit_v1/units.jsonl` EXIST on disk. The ONLY landing-critical missing cache is `real_cache.npz` (the ~5491-anchor list), rebuilt by `exp_task_degeneracy_v1.build_cache_if_missing()` which does a `C3.build_corpus("full")` full-corpus scan (~corpus-scale, ~a few min–30min, CPU, local). `buckets_full.npz` is only for the decode-SNR DIAGNOSIS witness (already 6/7) — NOT landing-critical.** ⛔ **REBUILD ATTEMPTED 08-30 AND THE BUILDER IS ALSO BIT-ROTTED: the cache builder `exp_task_degeneracy_v1.py` fails to IMPORT at HEAD — `ImportError: cannot import name 'as_constant_matrix' from tools.floor_battery` (line 89). So the reproduction chain is broken in TWO places: the missing transient `real_cache.npz` AND its builder cell no longer imports. Witness 1 still fails (cache absent).** This is the solver's disclosed "live import chain broken at HEAD," deeper than one cache. **REVISED SCOPE — the meaning landing needs a CHAIN REPAIR first: (1) fix `exp_task_degeneracy_v1`'s stale import(s) (find where `as_constant_matrix` went in `tools/floor_battery.py`) — watch for a cascade; (2) rebuild `real_cache.npz` (`build_cache_if_missing()`, the corpus build is FAST/cached — the background run reached the import error immediately, not after a scan); (3) reverify witness 1 (the 0.865 positive) FIRST-HAND; (4) THEN land Route A into `grounded_similarity` + take the live number.** A DEDICATED repair-then-reproduce-then-land pass. NOT rushed (a foundational organ + a THIN validated margin, +0.087 vs twin p95 / +0.013 vs its MAX). ⚠️ **This is worth an owner note: the #1 substrate gap's fix, validly proven 08-24, has BIT-ROTTED and cannot be reproduced/landed without repairing its build chain — a maintenance debt, not a science gap.**
