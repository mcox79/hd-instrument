...
📋 **QUEUE now: p1 `turn_on_the_learner…` (CAPSTONE — owner assigning to the parser solver), p4 `the_assembled_reader…all_flags_on`, p6 `the_reader_has_no_spatial_location…`. Nothing owner-DONE awaiting integration.** 🧾 **OWED hdlab landings (Q111, mine, precise targets in WIRING_MAP): `verb_subcat` (NEXT — port `experiments/ref_verb_subcat_organ_v1.py`, build the model via `exp_verb_subcat_graded_presence_v3.py --full`, default-off gate); p3 copular/nominal (heavier — offline WordNet asset + cop-arc front-end + `SituationModel.entity_states`); the p4 consistency-cleanup organ; ~13 remaining assembly DEBT-2 wirings; the learner-on program.** My commits: `fff1c2484` → `f71020f33`. Uncommitted tree = OTHER sessions' WIP (do NOT commit).
