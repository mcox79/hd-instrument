...
🔴 **#1 ON RECOVERY / NEXT: (1) the p5 verb-role wire needs a REBUILD.** The pre-compaction drop-fill-fallback wire (`hdlab/verb_role_exemplar_selector.py` + the `situation_reader` flag, UNCOMMITTED on disk, default-off safe) is UNFAITHFUL: it fires ZERO times on the reader's default 19c LitBank corpus (verbs uncovered → the selector correctly declines) and captures only a tiny slice even on modern prose. The p5 **+0.027 win** is the **INTEGRATED construction-conditional selector** (position × exemplar, word-order down-weighted at non-canonical structure — Competition Model), measured on the p5 cell's OWN item set (n=2737), NOT through the live SituationReader. The faithful landing re-picks PRIMARY role assignment and its gain must be re-verified THROUGH read() — a bigger, MEASURED change. RECOMMEND (prose, owner): rebuild the wire as the INTEGRATED selector default-off with a live-reader witness, OR keep p5's bookkeeping-DONE and defer the wire as a measured follow-on. Do NOT land the hollow drop-fill version.
🔧 **(2) OWED (2nd queued wire): the graph-organ wire** — promote `experiments/grounded_semantic_graph_organ.py` (~verbatim) to `hdlab/` + reframe `reading_grounding_loop.canonicalize` FLAT cosine → GRAPH DIFFUSION (default-off, witness `test_grounded_semantic_graph_ladder.py` 5/5); the settled-vector completion rides with it.
