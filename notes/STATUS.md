...
### 2026-08-31 (LATEST — heartbeat) -- 🔧 **QA-INSTRUMENT FIX (owed Q111 cleared): the QA capstone no longer collapses under the keystone — it scores the CAPABLE reader + reads temporal off `sm.timeline_order`. Plus the honest answer to "why cold-loading on the SSD" (it's CPU init + contention, not disk).**
