...
### 2026-09-05 (CONT-7, LATEST) -- 🏗️ **THE TOP-DOWN INTEGRATION PASS IS RUNNING (owner said "get started" + set a 20-min cron `baa13816` :07/:27/:47). Runbook = `notes/INTEGRATION_PASS_PLAN.md`; tracker = `notes/INTEGRATION_LEDGER.md`; provisional-state audit = `notes/PROVISIONAL_WIRINGS.md`; parser rubric = `notes/PARSER_JOINT_INTEGRATION_RUBRIC.md`.**
  ...
  - **🔴 NEXT (tier order): COREF (entity-KB resolver → commonnoun_binder; reverify `test_entitykb_resolver_v2.py` first) → GOAL (means-end purpose-filter + `_link_open_stack` edge) → MEANING (curated foundation + coarsening → `grounded_semantic_graph.select_sense`) → downstream signals (event-type → causation_typing/affect/goal-graph) → fresh FULL board = realized aggregate.** DISCIPLINES: reverify first-hand, measure off-vs-on, byte-identity where claimed, commit path-limited NO push, keep this entry current. The cron self-deletes when the pass completes. 4 solver problems filed (pri 1/3/5/8) — solvers own them.
