...
scramble 0.80% -- 6x its floor, still 5.2pp short of the >=10% revival gate. The 65.7% tautology
figure was an ELIGIBILITY BUG (C10); the live path emits 0%, so that half of the gate PASSES and
only QUALITY fails. GROWTH STAYS PAUSED until grounding quality holds.

## TOP ITEM -- THE READ-OUT FINDS THE NEIGHBOURHOOD AND CANNOT PICK THE MEMBER
Every correct open-vocabulary hit is a SISTER term: axon->dendrite, artery->vessel,
anaphase->telophase (`data/exp_grounding_readout_known_answer_v1/metrics.json` `example_gold_hits`).
Not RETRIEVAL (SELF_RETRIEVAL 0.786), not SUPPLY (closed: DO NOT REDO 23/25). It is
WITHIN-NEIGHBOURHOOD SEPARATION -- the 2.20% sister-term error rate rank-1 removal left untouched,
zero converted. The graded flip `38f7a0d5c` is the only reason it is above chance at all (2AFC OFF
0.4720, BELOW chance). Attack SEPARATION; supply, mass, coverage and reweighting are closed.

## READ-OUT / C3 -- `exp_grounding_readout_known_answer_v1` (a334501d2, 1b2022522, 204eba1a0)
Open-vocab hit@1 4.80% vs 0.80% scramble, +4.00pp CI [+3.30,+4.70], n=4000, 5491 anchors. Banked
facts n=319 AT_FLOOR (2.51% vs 1.25%, CI crosses 0). 2AFC graded ON 0.5393 MIDDLE_BAND. Tautologies
0 in EVERY arm. Graded-OFF run: `..._known_answer_v1_G0/`.

## OTHER PATH STATE -- settled; numbers in LESSONS
FORAGING (`exp_information_foraging_reading_v1`, 3d4761f69) HARD_PASS on its declared test:
held-out coverage FORAGE 0.0617 vs RANDOM 0.0127. CAVEATS THAT MUST TRAVEL (LESSONS D3/D4): FROZEN
beats FORAGE on coverage (0.0743) and RANDOM beats it on grounding quality (0.3864 vs 0.3511).
LEXICAL SEMANTICS: four adequately-powered negatives, all context-free word-pair similarity (DO NOT
REDO 23/24/25, CAVEAT D1). ENCODER: `hdlab/encoder_retrain_persist.py` WIRED but every transfer
eval is the SYNTHETIC harness -- neutral-ground test OWED (D2, ENCODER LINEAGE). COMPARATOR IS
BINDING (C1). EXTRACTION v6.2 94% blind, NO FLOOR ARM (C3).
...
UNIMPORTABLE at HEAD; repair `df149251f`).

## DO NOT REDO -- NEVER-TRIM -- stubs only; numbers + criteria in LESSONS
All CLOSED. `*` = has a revival/reopen criterion.
...
DIFFERENTIA/GENUS + SUPPLY as the binding constraint; 26 `sign()` as destroyer of the forgetting
kernel (D8 cascade/Benna-Fusi organ ruled out TWICE); 27 RANK-1 COMMON-MODE REMOVAL (full-covariance
whitening is NOT closed by it)*; 28 FORAGE_REFUSAL.
CAVEATS THAT TRAVEL (LESSONS, same heading): D1 near-vs-far is degradation, not collapse; D2 the
encoder-swap HARD_PASS ran on the encoder's own harness; D3/D4 the foraging reversals (above).
CORRECTIONS (LESSONS): C1 availability-binds-first WRONG; C2 CLIP at INGEST is no glass-box
violation; C3 the 94% has NO floor; C4 DGProjection fixes interference, not equidistance; C5 a
landed encoder DOES exist; C6 the synonym/sibling wall used the WRONG checkpoint; C7 opportunity-map
#5/#6 numbers; C8 the comparator was an embedded similarity LOOKUP TABLE; C9 results ARE searchable;
C10 the 65.7% TAUTOLOGY RATE was an ELIGIBILITY BUG (live path 0%); C11 the "58% COMMON MODE" does
NOT reproduce (0.5841 was a NORM RATIO quoted as a VARIANCE FRACTION).
...
2. SERIALIZE MEASUREMENT vs CODE CHANGE (2x): never audit or experiment while another agent may
   edit code it depends on, incl. transitive deps -- a racing edit describes no single repo state.
...
4. ESTABLISH THE FINAL LANDED VERSION BEFORE EVALUATING A SUBSYSTEM (6x). SUB-RULE, the generative
   cause: AN ABSENCE CLAIM REQUIRES AN ENUMERATION, NOT A SEARCH -- "I looked and did not find it"
   is no evidence of absence when the naming convention is unknown. State HOW you enumerated.
...
6. RUN A POSITIVE / KNOWN-ANSWER ARM -- it catches measurement defects NO arm of interest can. Cost
   2x in one night (forgetting-kernel CI confidently EXCLUDED truth, caught only by the synthetic
   arm; SELF_RETRIEVAL 0.786 is what licensed blaming MEANING, not plumbing). A FLOOR says whether
   the EFFECT is real, a KNOWN-ANSWER arm whether the INSTRUMENT is. Run both.
