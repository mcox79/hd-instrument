...
🔴 **#1 ON RECOVERY:** run the integration gate (`grep owner_verdict: DONE` in each `OWNER_NOTES.md` whose `SOLVED.md` lacks `INTEGRATED_BY_STRATEGY` AND `PROBLEM.md` `review:` blank). **AS OF THIS ENTRY: gate EMPTY.** For each new owner-DONE: reverify FIRST-HAND → grade → §2b → land the clean Q111 wire (per NO-MORE-DEFAULT-OFF: measure turn-on + re-validate consumers before default-on) → finalize → commit path-limited, NO push. **OPEN OPTIMIZATION (owner-authorized "always work to optimize", general only): GENERAL PERF sweep #2 — unify the remaining private taggers (`_ta_tagger` tense-agnostic hot path, `_rnp_tagger`, the ad-hoc `tagger.tag` sites) through the shared `_cached_tag` (byte-identical, same asset; 270 tags/read → ~60). The entity-states shared-cache fix (2× read) is the pattern.**
