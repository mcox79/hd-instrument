...
🔴 **#1 ON RECOVERY (2026-09-04 CONT-4):** **the integration gate is EMPTY** (verify line-anchored `^owner_verdict:[[:space:]]*DONE`; do NOT match the prose "integrates ONLY on owner_verdict: DONE" in OWNER_NOTES, a false-positive). Run the ARCHITECT HEARTBEAT for any NEW owner-DONE, else drive the queue. **TOP OPEN = `form_a_discourse_referent_for_every_entity_not_just_named_ones_common_noun_coref` (priority 2)** — the highest-leverage shared lever (common-noun coref caps every character-bound dimension). Then parser-attachment (3) > rare-sense (5) > event-detection (6) > goal-hierarchy (7). Address the P9 owner question + the deferred wrap-up above.
