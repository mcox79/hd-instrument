...
📊 **BUFFER: borderline (3 solvers busy on p4/p7/p8 WIP; 1 on-deck = p3 focus-stack). Deliberately did NOT package a re-tread brief. If a solver frees before a non-duplicative high-value brief is scoped, the next clean package is a COMPREHENSION→REASONING capstone (answer a question over the situation model) — the reasoning frontier, NOT the heavily-worked foundation.**
