...
🔴 **#1 ON RECOVERY (2026-09-04 continued):** the gate has **1 PENDING owner-DONE: `the_situation_model_has_no_goal_intention_dimension`** (Zwaan's 5th goal dimension; I filed it, a solver solved it) — reverify FIRST-HAND → grade → §2b → land clean Q111 wire (no-default-off) → finalize → commit, NO push. Then re-run the gate. **⚠️ INTEGRATED THIS CONTINUATION: `referent_per_np` turned DEFAULT-ON (OWNER DECISION, P5 decouple `3a749550b`+`c77d0bcf4`) — the complete referent set is the brain-foundational upstream (patient +0.336); the board who-did-what AGENT arm regresses 0.2519→0.0754 because the POSITIONAL agent assigner (poor downstream) grabs a wrong preverbal head from the denser set. THE COMMITTED baseline_2026-09-04 IS NOW STALE (qa_events regresses) — RE-RUN THE BOARD ONLY AFTER the URGENT fix `swap_the_positional_role_assigner_for_the_brain_foundational_competition_model` (P2, the Competition-Model assigner, inanimate-agent 0.333→0.081) lands → net board win.** P5 LESSON: the submission's who-did-what claim was PATIENT-only, the board arm is AGENT-only — the no-default-off first-hand measurement caught the regression the claim hid. **OPEN OPTIMIZATION (owner "always work to optimize", general only): PERF sweep #2 — unify the remaining private taggers (`_ta_tagger` hot path [+ `_add_predicate_recall` weights], `_rnp_tagger`, ad-hoc sites) through the shared `_cached_tag` (byte-identical; 210 tags/read → ~60). The entity-states 2× fix is the pattern.**
