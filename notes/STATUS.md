...
### 2026-09-05 (CONT-7, LATEST) -- 🏗️ **THE TOP-DOWN INTEGRATION PASS IS RUNNING (owner said "get started" + set a 20-min cron `baa13816` :07/:27/:47). Runbook = `notes/INTEGRATION_PASS_PLAN.md`; tracker = `notes/INTEGRATION_LEDGER.md`; provisional-state audit = `notes/PROVISIONAL_WIRINGS.md`; parser rubric = `notes/PARSER_JOINT_INTEGRATION_RUBRIC.md`.**
  ...
  - **🔴 NEXT (reordered): GOAL (means-end purpose-filter in goal_register.extract_goals → the goal-why arm; + the contextual `_link_open_stack` edge → goal_hierarchy arm) → MEANING (curated channel + coarsening → `grounded_semantic_graph.select_sense` → the wic arm) → ENTITY-KB LAST (careful non-leaky two-pass reader_coref; a located-negative-if-it-does-not-reach-a-board-dim is a valid outcome) → downstream signals → fresh FULL board = realized aggregate.** DISCIPLINES: reverify first-hand, measure off-vs-on, byte-identity where claimed, commit path-limited NO push, keep this entry current. Cron self-deletes when done. 4 solver problems (pri 1/3/5/8) — solvers own them. hdlab/entity_world_model_resolver.py NOT yet created (deferred to the careful entity-KB step).
