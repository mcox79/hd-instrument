...
🔴 **#1 ON RECOVERY / NEXT — TWO OWED WIRES, both deliberately NOT rushed this session (considered calls below):**
  ...
  **(2) the graph-organ wire (north-star) → a MAJOR focused unit, not a tail-of-session rush.** Promote `experiments/grounded_semantic_graph_organ.py` to `hdlab/` — which requires INLINING ~11 graph primitives (~300+ lines: WordNet ordering + relation/gloss/ConceptNet/SyntagNet edge builders + PPR + cooc-learning) from the 1126-line `exp_grounded_semantic_graph_ladder_wsd_v1` cell + their data loaders — THEN reframe the delicate ~900-line live core `reading_grounding_loop.canonicalize` FLAT cosine → GRAPH DIFFUSION (default-off), with the settled-vector completion. Acceptance witness `test_grounded_semantic_graph_ladder.py` (5/5) is a heavy ~15-min WordNet++ build. RECOMMENDATION: do this as its own focused unit with the box available (heavy witness) — it reframes a live core organ, so rushing it under CPU contention risks the reader's grounding path. This is the remaining north-star wire (burns down the "meaning organs are unwired islands" debt).
