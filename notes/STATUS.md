...
AS OF: 2026-08-23, LOOP ARMED | branch `dataprep/mcguffey-graded-corpus` | origin push needs USER AUTH | ⚠️ **RUNNING: `exp_c3_grounded_fu...` (PID `37304`, ~`950` MB) -- A CONCURRENT SESSION'S, NOT MINE. DO NOT KILL IT.** | 🚨 **Q117 IS OPEN AND IT IS THE BIGGEST THING ON THIS PAGE: THE `A5_STRINGCTRL 0.0870` SPELLING FLOOR -- the bar most recent work is measured against -- IS `~78%` MORPHOLOGICAL LEAKAGE.** *Stem-stripped gold drops it to `0.0193`, whose CI OVERLAPS its own info-free twin; our flat bag then BEATS it.* **`ORTHO_BAR = 0.0870` IS A LIVE GATE CONSTANT IN `per_row_gain_c3_vet_v1.py` AND `score_space_gain_and_topk_ci_v1.py` AND I DELIBERATELY DID NOT LOWER IT** -- a gate is never weakened by the session whose results it constrains. *Both sites now carry a comment; the constant is unchanged, verified by AST.* | 📦 **OWNER 08-23: bundle complicated problems for a solver; verify the submissions.** Filed **PRIORITY 3 `the_bundle_destroys_meaning_but_replacing_it_hurts`**. 🔎 **RE-VERIFIED ALL FIVE: `5`/`5` REPRODUCE. I claimed `5`/`5` from memory, checking gave `3`/`5`, and FIXING MY OWN CHECKER TWICE recovered it to `5`/`5`** -- `reproduce.py` ran cells with NO ARGS so smoke-defaulting cells did the work, PRINTED instead of recording, and were reported as broken; then it called a units-less cell unreproducible for writing no units. *`cortical_read` reproduced `724`/`725` fields; `flat_store` reproduced **`148` of `150`**, the only two differing being `elapsed_s` and `ts_iso`. **NO submission was ever at fault -- both defects were mine.*** ⚠️ **`reproduce.py` STILL cannot judge a units-less cell** (documented at `_declared_mode_default`, deliberately NOT patched: the volatile-field exclusion list is exactly where a real difference would hide). **The manual form is: run under `HDI_FRESH_RUN=<tag>` and diff EVERY field, declaring no exclusions in advance.** ✅ **BOARD: NOTHING OPEN. Q116 ANSWERED 08-23: *"make this into a focused problem to give to the solver so we can resolve it fully."*** **The owner DECLINED to settle the learn-from-reading question by decision and asked for a measurement.** Filed as **PRIORITY 2 `does_learning_from_reading_deserve_to_continue`**: all 16 losses tested a WEAK implementation, so the bar is ONE head-to-head where the learned arm is the strongest we can honestly build, both arms against the counting floor that keeps winning. **A clear loss is an explicit PASS.** ⚠️ *The supplied side needs its own controls -- the WordNet result shows supplied knowledge over-credits.* ✅ **Q115 ANSWERED + EXECUTED 08-23** (*"def make it a requirement for new experiments, but I'd go back through the 275 older ones one at a time"*): new cells are GATED by the pre-commit hook; coverage re-measured at **`71.2%`, not `~21%`**; the "1 by 1" list is the **`135`** results that both assert something and are cited by a steering doc. Q113 (08-22): cell work + `hdi_*` spawns AUTHORIZED; the `notes/problems/` briefs are the solver's, do not work them here. Q111 STANDING: testbed owns ALL integration, solvers never write `hdlab/`. Q110 STANDING: operational calls are mine, board is for owner-only decisions. Q102/106/107/108/109/112 ANSWERED/WITHDRAWN AND DISCHARGED (per STATUS_SPEC sec 3 tier 3 + the citation rule -- full text `notes/QUESTION_LOG.md`; Q109's credit-assignment investigation moved to `STATUS_LESSONS.md` "Q109 CREDIT ASSIGNMENT"). *Q103/104/108 share one pattern: filed before testing the constraint being complained about.*
