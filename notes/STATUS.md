...
1. **THE MEANING CHANNEL (#1 substrate gap, owed since 2026-08-24).** `the_live_meaning_organ_has_no_distributional_channel_to_be_taught_by` (owner-DONE, EXCELLENT, verdict-integrated) proved the flat-store decode route is STRUCTURALLY dead (256-wide projection crosstalk at support>d) BUT found a brain-foundational fix (a distilled distributional channel over a STATIC labelled asset) that clears the substitutability bar CI-separated in a live-faithful harness. Its `PROPOSED hdlab CHANGE` (two routes) + reverify witnesses (~30s/~15s) are ready; the wiring into `hdlab/grounded_similarity` + taking the number through the LIVE path is reserved to strategy. **NOT landed** (git log confirms). A DEDICATED careful pass (a foundational organ — do not rush).
