...
- **2026-09-02 (DEBT-2 WIRING ROUND — EXECUTED + RE-SCOPED, honest)** — the owner-directed #1-on-recovery round is **not 5 clean flag-flips**. Only **(b) was a clean additive live wire — LANDED** (below). The honest finding: each remaining lever's yield is validated in an **isolated/gold harness the board already captures on its RIGHT corpus**, and making it LIVE is a MEASURED effort, not a heartbeat flag: **(a) PARSER** route already landed (`c5e89b17a`); PP-role routing DE-PRIORITIZED by the integrated P2 (PP-attach = 8% of the 19c residual; the parser's real 19c service is TYPED slots for the thematic-fit store), `attach_conf`→`graded_competition` needs a validated LIVE consumer (`graded_competition` = the core of the un-stream-wired graded pick, not on the live path) → hooks P2/P1-gated. **(c) MEANING SETTLING** = an ISLAND-organ completion (`grounded_semantic_graph`+`semantic_control` NOT in the live ~19-organ closure; DEBT-3) → the LIVE meaning channel is north-star **P1**; do NOT hollow-wire. **(d) ENTITY-MAINTENANCE** = a LOAD-BEARING swap of the live coref pick (`event_centrality_coref.hd_centrality` → `graded_antecedent_pick` + chain loop), validated **+0.057 only on gold-nom** grouping, live `coref_acc` effect unmeasured → a measured coref effort, overlaps **P3**. **(e) TOP-DOWN detector** = instrument-only, NO net-gain → defer. All capabilities remain promoted/registered/witnessed (NOT shelved); (c)/(d) are drivable-live measured efforts if the owner wants them. **The LIVE meaning/who-did-what gains route through P1 + gold-cleaning (the new P2 composition problem), not flag-flipping.**

- **2026-09-02 (DEBT-2 WIRING ROUND — wire (b) COREF-DENSIFY LANDED)** — **🧩 LANDED the coref-densified world-state holder keying** (the (b) item of the DEBT-2 round above). Promoted `experiments/world_state_entity_binding.py` → `hdlab/world_state_entity_binding.py` VERBATIM (byte-identical, stdlib-only) + a default-off `densify_world_state` sub-flag of `track_world_state` on `_read_world_state`: HOLDERS (AGENT + recipient/source ARG2) → canonical discourse-entity via `EntityBinder` (indexical I/me→NARRATOR; he/she→the reader's OWN resolved coref cluster via a `(sent_idx, head)→cluster` map from `sm.coref_resolutions` — REUSE, no new resolver; object it→recency; nominal→head; we/you/pleonastic→abstain). Registered `world_state_densify_live_reader_v1`; witness `test_world_state_densify_landing_organ.py` **4/4** (default-off byte-identical; promotion byte-exact + dispatch battery cross-core identical; canonicalizes a constructed list; FIRES on real 19c prose). ✅ **Live-reader import set grows: `hdlab.world_state_entity_binding` (lazy, flag-on).** ⚠️ **HONEST BOUND:** the +0.148 who-has-what is the ISOLATED gold-aligned LEVER (on the board's RIGHT corpus, twin loses); this wire lands the entity-keyed representation LIVE (STATE dimension no longer a raw-string island) — NO downstream live who-has-what consumer scores `sm.world_state` yet, so NO live board delta. The (a) parser hooks + (c) meaning read-out remain the open DEBT-2 items; P3 coherence-prior is the residual.
