...
- **PRED loop vs stages:** I rated KEEP-as-stages, but whether predictive_world_model and generalized_event_knowledge duplicate the forward-prediction computation (vs transition-model vs association-readout being genuinely distinct) needs the actual data-flow traced. If they compute the same forward prediction, that flips toward CONSOLIDATE.
