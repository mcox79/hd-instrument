...
### Cluster 7 — APPRAISAL: affective appraisal — **4-6 organs → CONSOLIDATE the two appraisers** — ⚠️ CORRECTED 2026-09-12 (code check): the two 'appraisers' compute DIFFERENT appraisal checks over different inputs — `occ_appraisal` = OCC emotion TYPE from event×goal desirability × prospect (reads goal_register + affect_register; Scherer goal-conduciveness), `context_grounded_valence` = the event's harm/help for the patient via the force-dynamic arithmetic (`force_dynamics_valence`, intrinsic pleasantness/consequence). No equation is duplicated (verified by import trace: occ_appraisal → goal/affect registers; context_grounded_valence → force_dynamics_valence → force_dynamics_lexicon). KEEP as two arms of one appraisal SYSTEM; a file merge would be cosmetic. The earlier 'drop force_dynamics_valence' line is stale (it is now BF_SPIRIT and the harm/help read).
...
### Cluster 9 — FORCE: force dynamics — **3 organs → CONSOLIDATE** — ✅ RESOLVED 2026-09-12 (code check): ONE truth table, one implementation — `causation_typing` IMPORTS `force_dynamic_type`/`build_force_lexicon`/`detect_endstate_reached` from `force_dynamics_lexicon` (composition, not a copy); `force_dynamics_valence` is no longer the NOT_BF verb list but the harm/help APPRAISAL read over that same force lexicon (pri-7 landed) — a distinct read (Cluster 7), not a duplicate typer. Nothing to merge; the 'drop' verdict is stale.
...
### Cluster 17 — ROLES: thematic-role assignment — **2 core organs → CONSOLIDATE** — ⚠️ CORRECTED 2026-09-11 (code-level check by strategy): `thematic_role_labeler`'s ROLE part is NOT a Competition-Model op — it is a supervised averaged-PERCEPTRON labeler (`role_feats`/`train_perceptron`/`label_roles`) with ZERO live consumers (`label_roles` imported nowhere; `train_perceptron` only by `context_grounded_valence`'s disabled cert-fit control). The live role assignment is `graded_role_assigner` alone, so there is no second live copy of the equation. The module's 29 importers use its LEXICAL utilities (`lemma_verb` ×26, `lemma_word` ×5, `is_known_word`, `frame_slot_role`, `is_strictly_intransitive`, `is_passive_clause`). ACTION: (a) the lexical utilities move into the ONE glass-box morphology organ posted as pri-12 (its brief now says so); (b) the dormant perceptron role-labeler is an orphaned old version to prune when pri-12 lands; (c) the registry note is corrected. No merge of two equations is needed — the verdict rested on a stale registry note.
...
**4. Thematic roles → one `thematic_roles` organ (graded_role_assigner + thematic_role_labeler).** ⚠️ **CORRECTED (CONT-152 code-check — NOT the clean merge the registry note implied):** `thematic_role_labeler` is a BUNDLED organ — its Competition-Model role parts overlap `graded_role_assigner`, but it is ALSO a heavily-used LEXICAL-UTILITY organ (`lemma_verb`/`lemma_word`/`is_known_word`/verb-frame helpers) imported by **25+ hdlab organs** (mostly for lemmatization, NOT role assignment), whereas `graded_role_assigner` (competition-pick) has only 3 consumers. So this is really an INVERSE/under-fragmentation case: SPLIT `thematic_role_labeler`'s lexical utilities into a `lexical_utils` organ FIRST, then merge only the residual role-assignment parts. NOT a clean 2→1, and NOT the safe proof-of-concept. The genuinely-cleanest safe merge is instead **`sensorimotor_spoke` ↔ `grounded_similarity`** (Cluster 12 — the wrapper already CALLS the other's table; a true near-zero-risk 2→1). **VERIFIED CONT-153 (disk):** `sensorimotor_spoke` already delegates the table (`from hdlab.grounded_similarity import grounded_vector` — "exactly ONE join and ONE z-scoring", no duplicate logic) and has exactly **1 consumer** (`cortical_recall`). Merge = fold the thin wrapper API (`profile`/`has_profile`/`coverage`) into `grounded_similarity` + repoint `cortical_recall` → byte-identical, 1-consumer blast radius. This is the READY, disk-verified proof-of-concept merge (execute on owner go).
...
- **SITMODEL space dimension (spatial_relational_model vs location_register) — ✅ RESOLVED (CONT-140): KEEP confirmed.** Distinct + PIPELINED: `location_register` is the Zwaan-Radvansky TRACKING core (per-entity presence intervals over time; `where_is(t)`/`intervals_of`/`region_of`), `spatial_relational_model` is the Franklin-Tversky Source-Path-Goal CONSTRUCTION (`contains_path`/`containment_hops`) that IMPORTS location_register and `build_register`s it. Construction-vs-tracking, one consumes the other — not two encoders of one index. No flip.
- **BIND role-filler assembly (bound_event_backbone vs role_slot_summarizer) — ✅ RESOLVED (CONT-140): KEEP (soft) confirmed.** Both consume the `event_bundle` FHRR library but assemble DIFFERENT structures: bound_event_backbone = a JOINT who-did-which-action event token; role_slot_summarizer = sharded PER-ROLE slots (`role_keys`). Distinct output structures over the shared primitive library → library+consumers factoring, not duplication (softest KEEP; could share more, low priority).
- **Verdict validation:** all three verdict-relevant uncertainties (PWM/GEK, SPACE, BIND) code-checked → all CONFIRM their KEEP verdicts, so the CONSOLIDATE recommendations in (c) are trustworthy. Remaining unchecked (joint_relation_frontend SRL-vs-PARSE, structural_do ROLES-vs-SRL) are bucket-CLASSIFICATION questions, not KEEP→CONSOLIDATE flip risks — low priority.
