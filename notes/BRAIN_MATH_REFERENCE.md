...
| Lexical-category ACQUISITION by distributional substitution classes (Harris 1954; Mintz 2003 frequent frames; Redington-Chater-Finch 1998; Elman 1990 prediction clusters) | category(w) = cluster of w's immediate-frame distribution P(left±1,2, right±1,2 \| w) — accrued Hebbian directional counts → PPMI → low-rank → clustering; no labels; token category = P(c\|w)·P(left\|c)·P(right\|c) | PINNED (distributional acquisition; frames 91–98% on child-directed speech) / MODEL (clustering + readout) | **BUILDING (top rung, 2026-09-12)**: prior 0.323 many-to-one @8k sentences was a WEAK implementation (exp_parser_fully_bf_chain_v1); `exp_reading_induced_categories_v1` smoke 0.437 @20k Simple-Wiki lines (majority floor 0.141, twin 0.325), Mintz frames 0.529; sweep to 1M lines × k∈{17,34,68} running; supervised tagger ref 0.944 | `pos_tagger` (NOT_BF) / `crf_tagger` (calibrated posterior, supervised weights) → reading-induced categories to build |
