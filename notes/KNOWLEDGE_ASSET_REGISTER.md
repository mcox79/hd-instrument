...
- **BOARD-VISIBILITY (attempted + reverted, CONT-84):** a `board_causal_sign_dimension` arm was prototyped but REVERTED — the signal needs FULL-scale WIQA (n≈5005; +0.157 CI-sep), but the board self-test runs SMOKE, where the ~14% gated science subset is only ~31 items = underpowered (shows a misleading noisy loss, arm 0.484 vs scrambled 0.613 not-sep). So causal_sign is **board-invisible by nature**; its correct instrument is its DEDICATED full-scale witness (`test_causal_sign_integrated.py` / `test_causal_sign_channel_landed.py` 3/3), NOT a smoke-board arm. Do not re-attempt the board arm.
