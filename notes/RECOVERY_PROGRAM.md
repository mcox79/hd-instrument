...
|---|---|---|---|---|---|
| H1 | terminal chain-graded cells in the cert ledger | **574** (re-derived independently as **565** cells-with-an-identity-on-disk + 14 `META_RULE_*` atoms that are rules, not cells -- treat the two figures as a bracket, not a discrepancy) | **COMPLETED 2026-08-14** in `notes/recovery_ledger_chaingraded_tier_2026-08-14.md` (`da7fe14d4`): **565 of 565 rows written**, 547 `metrics.json` opened at HEAD, 48 `verdict_msg` hand-read | **0 cells** (residues are now qualitative and listed in that file's sec 6: 146 floors machine-classified only, 124 contrast-only cells not re-examined, 439 cells dated only by git first-commit, 14 META_RULE atoms) | the assigned tier. **Three findings: (1) 280 of 565 = HALF the tier is ONE auto-generated saturation grid (`exp_q_a3_l*_cross_layer_composition`), so "574 chain-graded cells" is ~286 distinct investigations; (2) the whiten+pinv chain WAS composed and measured -- `exp_pb_production_recipe_integration_v1`, `naive(raw+hebb)=3 full(whiten+pinv)=172`, 57.3x -- which corrects group B and sec 9b rank 4; (3) the dimensional-expansion stage is CONTRADICTED from inside the tier by `exp_substrate_expansion_method_battery_gpu_v1` ("expansion cannot beat rank ... whitening helps via decorrelation", `native=0.0065 rp_x4=0.0065 zca_whiten=0.0517`). 172 of 565 (30%) carry a real floor; a `scramble`-keyed pass finds 11 and misses 161 of them.** Only 33 of 565 are dated >= 07-15, so the tier is still mostly June/early-July substrate physics |
...
> **H1 IS THE FIRST TIER TO LEAVE THIS SECTION, and it left by ENUMERATION.** Its 565 rows live in
> `notes/recovery_ledger_chaingraded_tier_2026-08-14.md`, written in this file's sec-5 row format
> and closed STATE vocabulary. **That file has NOT been merged into sec 5** (565 rows would destroy
> this file's cold-read function). Until it is, the sec-7a count command must be run over BOTH
> files; the companion file carries the same command in its own header. Its tally at write time:
> **535 VERIFIED / 18 FOUND / 12 REFUTED, 0 WIRED** -- so H1 is triaged, not finished.
