# STATUS ARCHIVE -- notes/STATUS.md AS IT STOOD 2026-08-21, BEFORE THE SPEC-COMPLIANT TRIM

**Why this file exists.** `notes/STATUS_SPEC.md` sec 8 records an ad-hoc byte-shave that deleted a
standing discipline which had cost TWO FULL EXPERIMENTS to learn. `STATUS.md` had reached
**308,692 bytes against an 8,704 byte cap (35x)**, so the trim could not be done by tiers 1-4 alone
and had to touch material a byte-shave must never touch.

**So nothing is removed until it exists here.** This is a VERBATIM copy. Git history holds it too,
but git history is not FINDABLE by a future session told to read `notes/`, and unfindable is the
same as lost for the purpose the never-trim rule protects.

**Do not edit this file.** It is a snapshot, not a working document.
