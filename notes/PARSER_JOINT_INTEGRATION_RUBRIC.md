...
| `consolidate_the_arceager_and_arc_double_parse` **(WIP PEEK — not owner-DONE yet)** | CONSOLIDATION (single parse, not a replacement) | route ALL consumers through the ONE arc-eager parse; eliminate the redundant base arc-factored batch parse (copular+space now read arc-eager heads too) | full board ZERO regression (agg 0.6677, all 9 dims identical, worst Δ +0.0000); read-time −4.6% (batch parse eliminated); copular RAW detection +0.111 CI-sep; witness 12/12 | read-time (−1.0s/read); copular raw-label detection (+0.111); single-parser simplicity | space `where_is` −0.015 (CI includes 0, NOT sep) | resolves C2 (arc-eager now serves the general front-end, no-regress) | **adopt-global** (single arc-eager parse; board zero-regression + faster). Reverify `test_double_parse_consolidation.py` when owner-DONE. This IS the arc-labeler exploration's "flip arc-eager for the general front-end" candidate, fully measured. |
| _(slot 3 — pending)_ | | | | | | | |
| _(slot 4 — pending)_ | | | | | | | |
