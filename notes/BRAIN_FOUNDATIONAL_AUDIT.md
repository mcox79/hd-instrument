...
- **2026-09-06 (INTEGRATION — the reader now reads TRUTH-CONDITIONAL POLARITY + QUANTITY onto `sm.events`: negation (not/no/never/none) toggles a proposition's truth, quantifiers (some/all/every/none/"every-but-X") read cardinality over the argument set. Reading polarity-blind doesn't just lose — it INVERTS the truth value. EXCELLENT, owner-DONE, Q111.)** Problem `represent_negation_and_quantifier_scope_for_truth_conditional_reading_modern_gold`, reverified first-hand `verification/test_polarity_operator_ewt.py` **3/3** + `verification/test_quantifier_operator.py` **7/7**; new landing witness `verification/test_polarity_operator_landing.py` **4/4 (re-run first-hand)**. **NEW ORGAN `hdlab/polarity_operator.py`** (promoted BYTE-IDENTICAL from `experiments/_polarity_operator.py`, SHA-256 match, no `experiments.` dep) — REUSES `hdlab/state_register.state_match` VERBATIM (the copular-state polarity primitive EXTENDED onto EVENT propositions + a quantifier layer; does not re-extract/re-parse). WIRED DEFAULT-ON via `situation_reader.read_polarity` (in CAPABILITY_FLAGS) → `_read_polarity` MIRRORING `_read_state` (a post-read pass setting ONLY the new `EventRecord.polarity/quantity/quantity_exception/polarity_provenance` fields; measured read-cost **+1.42% of a warm read** → default-on per no-more-default-off; existing fields+dims BYTE-IDENTICAL ON vs OFF). **PINNED:** negation = an OPERATOR that TOGGLES a proposition's truth (Kaup & Zwaan two-step; Tettamanti/Tomasino cortical suppression); quantification = CARDINALITY over the token set (Johnson-Laird — none/no = ¬∃, UNIFYING negation and "none"); complement factuality from the matrix verb's IMPLICATIVE/FACTIVE signature (Karttunen; Kiparsky), NOT propagation. **MEASURED first-hand: negation reader-native (UD-EWT `sm.events`, n=131) net 0.9313 vs polarity-blind 0.5038 (+0.4275 CI-sep), negated-recall 0.8769 vs 0.0000, over-negation 0.0000 clean (BEATS the prior `negation_factuality_gate` MIDDLE_BAND); MoNLI n=1202 0.9965 vs blind 0.0035 (INVERTS); quantifier MED-downward 0.8259 vs monotone-blind 0.1741 (INVERTS, +0.6519 CI-sep); the NPI/negation-licensing UNIFICATION (Ladusaw — sentential negation IS downward-monotone) is the biggest win, unified operator n=3791 0.8225 vs 0.2849 (+0.5376 CI-sep) + twin +0.2464 → negation and quantification are ONE downward-monotonicity operator.** Board arm `board_negation_quantifier_dimension` (negation_ewt + quantifier_med) behind `run_new_arms`, OFF in self-test. **CONSOLIDATION (AUDIT UPDATE folded): `EventRecord` previously carried NO truth-POLARITY/QUANTITY field (all events stored POSITIVE+SINGULAR); truth-conditional negation was FRAGMENTED across ≥5 clause-local scanners (`goal_typing`/`definitional_predicate`/`goal_register`/`result_type_induction`/`goal_achievement`) — this operator is the consolidation point (subsumes them: extended 1.0 vs 0.20/0.60) + `state_register` copular polarity is the primitive it extends.** **LOCATED NEGATIVE (SHELVED, NOT landed): the parse-repair UPSTREAM matched the surface operator on the negation gold (0.8864→0.9318) with STORED knowledge no training — but the HONEST GENERALITY CHECK proved it NET-NEGATIVE on general UD-EWT POS (precision 0.04, breaks 172 tags to fix 7) → CONSTRUCTION-SPECIFIC overfit → the parse-free SURFACE operator is the general primary; the general fix needs VerbNet subcat frames + tagger-confidence gating (🔗 direct evidence for the just-filed parser problem).** Also: ~5-6 of the 8 EWT misses are GOLD NOISE (the gold over-propagates negation across contrastive "but"; the operator is arguably more correct than its own gold). NOTE: `test_polarity_operator_upgrades.py` U1 is PREMISE-STALE (post-landing `polarity` is a first-class field, so the prototype's `hasattr`-absence proxy at `exp_polarity_wired_reader_v1.py:95` can never be true — capability intact per the landing witness; solver cell left unedited, the W7 precedent). Board self-test PASS; reader imports clean. Full inventory = ledger CONT-15. Files: `hdlab/polarity_operator.py`, `hdlab/situation_reader.py`, `experiments/exp_situation_model_qa_modern_v1.py`, `verification/test_polarity_operator_landing.py`. **FOLLOW-ONS: wire a live QA-capstone consumer of `EventRecord.polarity`/`quantity` (realize the board-visible gain end-to-end); unify state_register copular + event polarity (P2); the precise-lexical-resource parser fix (feeds the parser problem); conditional/phrase-level monotonicity coverage.**

- **2026-09-06 (INTEGRATION — the TIME dimension is now REASONED over, not just extracted: before/after (over the reordered timeline) + a NEW Allen interval OVERLAP capability + relative duration, on MODERN gold. EXCELLENT, owner-DONE, Q111.)** Problem `reason_over_event_time_order_and_duration_on_a_modern_gold`, reverified first-hand `verification/test_temporal_reasoner_organ.py` **18/18**; new landing witness `verification/test_temporal_reasoner_landing.py` **ALL PASS (re-run first-hand)**. **NEW ORGANS:** `hdlab/aspect_interval.py` (the aspect→interval UPSTREAM — recovers the finite PROGRESSIVE the point-order extractor DROPPED entirely, as an imperfective open interval, + a Vendler durative situation-type layer; Smith 1991 viewpoint aspect); `hdlab/temporal_reasoner.py` (integrated BEFORE/AFTER = cue + TIMEX event-LOCAL anchoring + transitive closure + per-judgment SIGNAL-CLASS provenance; Allen 1983 interval-intersection OVERLAP; `RelativeDurationLine` reusing `hdlab.transitive_ordering`); `hdlab/uds_time_duration.py` + `hdlab/temporal_script_schema.py` (the two KNOWLEDGE organs — UDS-Time human-annotated typical-duration lookup 1,756 lemmas; ROCStories-mined 177,800 verb-pairs for implicit-event order; static `--build` assets, degrade gracefully). WIRED DEFAULT-ON via `situation_reader.track_temporal_reasoning` (in CAPABILITY_FLAGS) → `_read_temporal_reasoning` MIRRORING `_read_causal_reasoning` (lazy + additive; binds `sm.temporal_before(x,y)`[+provenance]/`sm.overlaps(x,y)`/`sm.longer(x,y)`, returns "unknown—needs world knowledge" on implicit-event queries; ZERO read-time cost until invoked → default read BYTE-IDENTICAL). **PINNED:** Reichenbach E/R/S (tense-as-anaphora, Partee 1973 / Webber 1988 — event binds a date in its OWN clause); Allen 1983 interval algebra as the Marr computational-level target for overlap; the magnitude line for relative duration; Zwaan event-token indexing; Schank & Abelson / Chambers-Jurafsky narrative chains for implicit-event order. **MEASURED first-hand (each floor recomputed per-population): BEFORE/AFTER on TB-Dense (1990s newswire, MODERN, n=445): register 0.5933 vs iconicity 0.5236 (+0.0697 CI-sep); the INTEGRATED reasoner (+TIMEX event-local anchoring +closure) 0.6225 vs 0.524 (+0.099 CI-sep, cue channel 0.909 / date channel 0.830); reverse-order positive control 0.1557 vs iconicity 0.000; cue-bearing subset 0.9091; twin collapses to 0.521. OVERLAP (a NEW capability): constructed Allen gold 0.9938 vs point-order control 0.5000 (+0.4938), twin 0.469 loses; real-prose TB-Dense overlap subset recovers 0.3967 of INCLUDES/IS_INCLUDED/SIMULTANEOUS the point-order control gets 0.000 of; TORQUE in-context overlap F1 0.1184 vs 0.000. RELATIVE DURATION: magnitude line 1.0000 on 2520 unstated transitive "which lasted longer" pairs, twin 0.5063.** **UPSTREAM BYTE-IDENTITY (verified first-hand): the aspect-interval promotion keeps the point-order event set BYTE-IDENTICAL (Bleak House 45/45 +6 progressive, Persuasion 92/92 +3 — only additive interval fields); every existing dim (events/entities/coref/causal_links/timeline_frames/timeline_order) BYTE-IDENTICAL `track_temporal_reasoning` ON vs OFF.** **KNOWLEDGE-ORGAN WIN (dissolves a located negative): UDS-Time (human-annotated) moves MCTACO typical-duration from CI-below-floor (−0.090, text-mined) to TIED (−0.019) with 2× coverage; the script/schema organ scores TRACIE implicit-event 0.6022 covered vs 0.478 story-internal (SymTime needs 3.5M distant-supervised for 0.80; a glass-box chain mine gets 0.60).** **LOCATED NEGATIVES (SHELVED, each field-corroborated): TEXT-MINED typical-duration prior CI-below floor (Vempala 2018's exact negative + TacoLM's own design decision — Gricean reporting bias; fix = a different SOURCE not more mining) → NOT landed; SDRT causal-MARKER channel 0.50<iconicity (the real lever is causal-world-knowledge inference, couples with the causation organ); full-population real-prose overlap does NOT beat the "never-overlap" majority (0.776 vs 0.786) — but the ~60% INCLUDES/SIMULTANEOUS "ceiling" is DOMINANTLY a MEASUREMENT ARTIFACT (holistic-TLINK IAA 59-64% vs in-context-QA IAA 85-98%; MATRES DROPPED these relations), validating the endpoint-decomposition Allen design.** **KEY UPGRADES KEPT: token/mention-indexed same-verb simultaneity 0.8448 vs type-keyed 0.0 (Zwaan event tokens); the brain-faithful default = EXACT mention order (not lemma-first-occurrence) turned a spurious full-pop loss 0.42<0.52 into a CI-sep win 0.59>0.52; event-LOCAL date anchoring 0.958 (naive carry-forward HURTS 0.493 — the trap).** BOARD-VISIBILITY: `board_temporal_before_after_dimension` + `board_temporal_overlap_dimension` folded behind `run_new_arms` (OFF in self-test; overlap CI-separates even at smoke, before/after needs full n). **KNOWN pre-existing layering (recorded, not a new edge): `aspect_interval`/`temporal_reasoner` reuse the shared `experiments._temporal_ordering` front-end that `situation_reader.py` ITSELF already imports (L135-136; 48 hdlab modules share it) — promoting the temporal front-end into hdlab for true layer-purity is a filed follow-on.** Board self-test PASS; reader imports clean. Files: `hdlab/aspect_interval.py`, `hdlab/temporal_reasoner.py`, `hdlab/uds_time_duration.py`, `hdlab/temporal_script_schema.py`, `hdlab/situation_reader.py`, `experiments/exp_situation_model_qa_modern_v1.py`, `verification/test_temporal_reasoner_landing.py`. **FOLLOW-ONS: P2 copular/stative-state channel (the biggest real-prose overlap lever — the dropped copular channel); P3 UDS-Time native rank-correlation duration board arm + an implicit-event script arm (the 2 knowledge organs are LATENT — no live scored consumer yet); P4 SDRT world-knowledge discourse reader coupled to the causation organ; P5 richer script induction + anchor-aware TORQUE scorer; + a live TIMEX/DCT reference-time extractor (the date channel is empty at read time today) and a text→duration extractor (so `sm.longer` stops abstaining); + the hdlab-purity temporal-front-end promotion.**

- **2026-09-06 (INTEGRATION — the reader now REASONS over the LOCATION REGISTER: a glass-box relational spatial reasoner (transitive containment + spatial-framework relative position + Goal-over-Source path/vacate) over `sm.locations`. The SPACE dimension gains a reasoning layer atop the tracking core. EXCELLENT, owner-DONE, Q111.)** Problem `reason_over_the_spatial_relational_model_containment_position_path_modern_gold`, reverified first-hand `verification/test_spatial_relational_reasoning.py` **11/11**; new landing witness `verification/test_spatial_relational_landing.py` **ALL PASS (17 checks, re-run first-hand)**. **NEW ORGAN `hdlab/spatial_relational_model.py`** (promoted byte-faithful from `experiments/spatial_relational_model.py`; AST-verified imports = `__future__`/`collections`/`typing`/`hdlab.location_register` ONLY — no `experiments.` dep). WIRED DEFAULT-ON via `situation_reader.track_spatial_reasoning` (in CAPABILITY_FLAGS) → `_read_spatial_reasoning` MIRRORING `_read_causal_reasoning` (lazy + additive; builds ONE SpatialModel from `sm.locations` on first callable invocation, binds `sm.spatial_reasoner`/`spatial_contains`/`spatial_relative`/`spatial_still_at`/`spatial_where_after`; runs LAST after `sm.locations` is set; ZERO read-time cost until invoked → default read BYTE-IDENTICAL). A read-time query layer over the EXISTING location register (emits no events). **PINNED:** Johnson-Laird/Byrne model-based reasoning; Franklin & Tversky 1990 spatial-framework relative position; Dusek & Eichenbaum hippocampal TRANSITIVE inference; Peer & Epstein nested reference frames; Lakusta & Landau Goal-over-Source path asymmetry. **MEASURED first-hand — all three inference types CI-SEPARATED over BOTH controls on MODERN gold, reasoning ISOLATED from extraction (gold relations supplied): CONTAINMENT (SpaceEval/ISO-Space, train n=1304) 1.000 vs last-mention 0.940 (+0.060 CI[0.047,0.073]); multi-fact subset (n=155) 1.000 vs 0.497 (floor AT CHANCE); two-level `is_in_region` ablation 0.896 (the existing register cannot do nested containment); shuffled-relation twin 0.508. RELATIVE POSITION (SpartQA-HUMAN SPRL, test n=1300) 1.000 vs 0.734 (+0.266 CI[0.242,0.289]); multi-fact (n=828) 1.000 vs 0.582; twin 0.143. PATH/TRANSFER (SpaceEval MOVELINK, n=32) 1.000 vs 0.500 (+0.500); vacate-Source works; twin 0.469.** **END-TO-END (a capability claim within scope): RELATIVE POSITION ALSO clears the bar over the reader's OWN extraction (SpartQA, coverage 0.339) — reasoner 0.276 vs last-mention 0.213 (+0.063 CI-sep) vs twin 0.095.** ADDITIVE-SAFE: on real LitBank docs every existing dim (events/entities/coref/timeline/`causal_links`) AND the load-bearing `sm.locations` (37/34 tracks) BYTE-IDENTICAL off-vs-on; `all_capabilities_off` forces the flag False (byte-identity reference). **NEW MEASURED CEILING (the SPACE cap is now DOUBLY located): the reasoner is near-perfect on GOLD relations; end-to-end is EXTRACTION-GATED — the reader's parse recovers only 0.22 containment / 0.06 position / 0.02 move edges, multi-hop CHAIN survival 6/90 (13% edge recall → 1% chain survival, the exponent wall), ReSQ implicit captions cov 0.036. Reasoning is NOT the bottleneck; relation EXTRACTION RECALL is (corroborated by the SpaceEval-2015 lit: best F1 ~0.845 from GOLD elements vs ~0.573 from raw text, MOVELINK hardest).** BOARD-VISIBILITY: `board_spatial_relational_dimension()` (reuses the SpartQA end-to-end position measurement) folded behind `run_new_arms`, OFF in self-test, OUT of the 19c-free headline aggregate; a full board `--run` surfaces the `spatial_relational` row. **UPGRADES the solver KEPT (each net-positive): part-whole "of" + locative-predication extraction (recall 0.13→0.22, chain 1/90→6/90); a `canon_entity` block-label bug fix (standalone block "A" was article-stripped to an EMPTY node — the fix FLIPPED the end-to-end position margin from un-separated to CI-separated); nested-frame position inheritance (cross-block transitivity, the "go deeper" fix); reasoner completeness (principled indeterminate-vs-unknown abstention + consistency/contradiction detection + quantifier all/exists); a set-order DETERMINISM fix (Python set iteration is PYTHONHASHSEED-randomized → the reverify now reproduces across processes); a glass-box ConceptNet commonsense gap-filler (NO LLM), ReSQ commonsense 0.019→0.126 CI-sep over baseline AND shuffled-KB.** **RESEARCHED + REVERTED (located, reasons named): event-participant grounding (unmeasurable); quantifier-in-scorer (a raw-accuracy up but a CONTROLS regression — corrupts the composition margin + weakens the twin); orientation commonsense VERIFIED KB-UNREACHABLE (ConceptNet has no vertical signal — needs a perceptual-simulation faculty, not a bigger KB).** **AUDIT CORRECTIONS FOLDED (2026-09-06): two SPACE phrasings were OVERSTATED — "metric coords ruled out" → CATEGORICAL BY DEFAULT, METRIC ON DEMAND (Rinck 1997 Exp 3), corrected at §710/§1501; and vacate-Source AUTOMATICITY is CONTESTED (the salience asymmetry is PINNED — Lakusta & Landau; the automaticity is not — recorded here, not asserted).** Board self-test PASS; reader imports clean. Files: `hdlab/spatial_relational_model.py`, `hdlab/situation_reader.py`, `experiments/exp_situation_model_qa_modern_v1.py`, `verification/test_spatial_relational_landing.py`. **FOLLOW-ONS: P2 joint text→spatial-relation EXTRACTOR (the sole end-to-end lever — whole-sub-graph survival target); P3 a perceptual-simulation/visual-spatial-statistics organ for orientation commonsense; P4 land the ConceptNet gap-filler default-off behind the reasoner.**

- **2026-09-06 (INTEGRATION — the reader now INFERS the UNSTATED emotion: a glass-box OCC appraisal composes the LIVE affect+goal+event registers (event-vs-goal congruence × prospect → the felt emotion). This is the affect_register's OCC deferred-consumer named in the 2026-09-05 causal-mental-bridge integration. EXCELLENT, owner-DONE, Q111.)** Problem `infer_unstated_emotion_via_occ_appraisal_over_event_goal_congruence`, reverified first-hand `verification/test_occ_appraisal.py` **9/9**; new landing witness `verification/test_occ_appraisal_landing.py` **12/12 (re-run first-hand)**. **NEW ORGAN `hdlab/occ_appraisal.py`** (promoted byte-faithful from `experiments/_occ_appraisal.py`; the ONE experiments import repointed to `hdlab.goal_register.implicit_investment_goals`; stdlib+hdlab only). WIRED DEFAULT-ON via `situation_reader.track_infer_emotion` (in CAPABILITY_FLAGS) → `_read_infer_emotion` MIRRORING `_read_tom_action` (lazy + additive; binds `sm.infer_emotion(char[, t])` composing the live registers, FILLS ONLY where the affect register found NO stated emotion for that char — NEVER overwrites a stated feeling; `sm.infer_emotion=None` until invoked → default read BYTE-IDENTICAL). **PINNED:** Ortony/Clore/Collins 1988 prospect-based emotions (desirability × prospect → OCC type); Scherer goal-conduciveness appraisal; Barrett core-affect-plus-conceptualization; Lutz & Radvansky / Dopkins-Klin-Myers 1993 FAILED goal status; Lazarus relief. **NEW MEASURED DEVIATION CROSSED (the load-bearing upstream): `hdlab/goal_register.track_status` was THWART-BLIND** (it set `failed` only on a NEGATED goal clause, never on a THWARTING EVENT, and had no prospect detection — so the CURRENT substrate NOFIX collapses to 0.060). Generalized to a strict-superset `track_status_thwart` (thwart branch + event-agent coref-canon + irregular-past) behind `situation_reader.track_goal_thwart` (DEFAULT-ON, in CAPABILITY_FLAGS; baseline `track_status` left BYTE-IDENTICAL as the flag-off reference). **MEASURED first-hand on a MODERN constructed OCC gold (n=50, balanced 25/25): TYPE 0.940 through the LIVE `sm.infer_emotion` vs strongest floor (valence-only oracle-valence) 0.440 = +0.500 CI[+0.340,+0.660]; vs the CURRENT substrate NOFIX 0.060 = +0.880; VALENCE 0.940 vs majority 0.500 = +0.440 CI-sep; LOAD-BEARING prospect subset {relief, fears_confirmed} (n=18) 1.000 vs valence-only + last-word floors PROVABLY 0.000 (relief carries a stated FEAR word — the false-belief-subset analog); ORACLE(rule) 1.000; goal↔event-shuffle TWIN 0.220 LOSES (+0.720 CI-sep).** UPSTREAM NO-REGRESSION (verified first-hand, ROC+gold): the thwart generalization is a STRICT SUPERSET — 0 satisfied/failed flips over 90 passages/45 goals, 12 active→failed additions, 0 `wants()` regressions; board goal-dim 0.625→0.625, goal_hierarchy 1.0→1.0, ToM(BigToM FB) 0.8705→0.8705 (all no-regress ON vs OFF). ADDITIVE-SAFE: existing dims (events/entities/coref/affect/`sm.feels`) BYTE-IDENTICAL `all_capabilities_off` vs +thwart+infer_emotion ON. **NEW MEASURED CEILING (correctly located): event↔goal SEMANTIC matching is the appraisal's bound — the DENSITY PHASE CUT holds the RULE at oracle 1.000 (density-invariant) while end-to-end falls 0.94→0.06 sparse, so the ceiling is EXTRACTION density, MEASURED not asserted.** RESEARCH-DRILLED (primary sources): this wall is NOT the distributional meaning channel (`bridging_inference` is POLARITY-BLIND: rel(sell,buy)≈rel(win,lose)) — it DECOMPOSES into (a) a converse/perspective ROLE-FILLER layer (FrameNet Perspective_on + WordNet antonymy; BUILT + measured +0.417 recovery, twin loses, default-off P2 follow-on) and (b) an open-ended Talmy force-dynamic / Schank-script SCENE-INFERENCE residual (meaning-channel-adjacent). A SECOND wall: emotion INTENSITY needs a discourse-level belief-vs-outcome surprise organ (Kumar 2023) + goal-IMPORTANCE (Frijda/Ortony 1992), NOT the argument-level N400 (measured inert). **RECOMMENDED NEXT-ARCHITECTURE (SOLVED §6b, strategy-noted): a UNIFIED `resolve_goal_outcome` — ONE Hobbs weighted-abductive match of each clause against the OPEN-GOAL registry (Zwaan-Radvansky) with Kintsch settling — subsumes the 8 ad-hoc branches, is behavior-preserving (reproduces 0.940 exactly), removes branch-ordering hazards, and yields MIXED emotions for free; prototyped + measured, the clean engine-swap follow-on that loses no headline.** BOARD-VISIBILITY (live ≠ scored): a `board_occ_appraisal_dimension()` arm reusing the solver's own n=50 gold + measurement is folded into the modern board behind `run_new_arms` (OFF in self-test) — a full board `--run` will surface the `occ_appraisal` row. HONEST BOUND (SOLVED §8): the headline gold is CONSTRUCTED (mitigated by the twin, decorrelated+oracle floors, a-priori rule, extraction through the real reader, density cut); the real-prose corroboration is a LOCATED NEGATIVE (agreement 0.576 beats the char-shuffle twin +0.087 but not the majority-valence floor — real prose doesn't isolate the goal→emotion relation, the same event↔goal-attribution wall). Board self-test PASS; reader imports clean. NOTE: the 3 reader module self-tests (`_selftest_frame_arity_gate`/`read_end_to_end`/`event_extraction_coverage`) were REFRESHED to the current contract earlier this session (`980bbb71e`) — now all 7 module self-tests PASS. Files: `hdlab/occ_appraisal.py`, `hdlab/goal_register.py`, `hdlab/situation_reader.py`, `experiments/exp_situation_model_qa_modern_v1.py`, `verification/test_occ_appraisal_landing.py`.

- **2026-09-06 (INTEGRATION — THE COMPREHENSION BOARD IS NOW 19c-FREE MODERN (the owner's 19c ban realized as the reported instrument); the session's board-invisible wins get their own arms; the 19c who-did-what AGENT win is a REGISTER-SPECIFIC located finding. EXCELLENT capstone, owner-DONE, Q111.)** Problem `rebuild_the_comprehension_board_on_a_modern_corpus_retire_the_19c_litbank_eval`, reverified first-hand `verification/test_modern_board.py` **12/12** + new landing witness `verification/test_modern_board_landing.py` **9/9 (re-run first-hand)**. **THE READER IS UNCHANGED — this is primarily an INSTRUMENT change** (`experiments/exp_situation_model_qa_modern_v1.py`): the HEADLINE aggregate is now **19c-FREE 0.6135 over 7 MODERN dims** (coref GUM 0.4681 vs 0.3621 CI-sep / who-did-what-agent UD-EWT / state UD-EWT 0.8333 CI-sep / patient UD-EWT 0.8311 CI-sep / wic 0.6639 CI-sep / common-noun located-negative / salience), 4/7 CI-sep, **NO LitBank dim**; the LitBank aggregate + per-dim are DEMOTED to an `informational_19c_crossref` block (kept, not the headline); temporal/causal/goal/affect are NAMED GAPS (not fabricated, not 19c). **3 NEW ARMS fold in the session's board-invisible proven wins (the "live≠scored" closers):** coarse-sense (word-sense p7 — SemCor a_s 0.5526 vs coarse-MFS 0.3667, INFORMATIONAL under the 19c ban → needs a modern coarse-WSD gold); selective-reliability (precision-defer p4 — MODERN UD-EWT patient answered 0.9615 vs 0.8617 +0.0998, obl +0.0855, random-defer twin flat); causal-multihop (causal p10 — MODERN WIQA multi-hop reason 0.2357 vs adjacency 0.000 CI-sep + TellMeWhy non-adjacent; honest sub-negative on the full cause-ID where a lexical polarity baseline wins = the edge-extraction wall). **THE REGISTER LOCATED FINDING (a decisive vindication of the 19c ban): the 19c who-did-what AGENT win (0.041→0.69) is REGISTER-SPECIFIC — on modern gold the positional floor is near-ceiling on canonical prose (UD-EWT 0.855, GUM 0.829) and the narrative-tuned always-compete CM does NOT beat it (the tracked-set decouple REVERSES sign on modern); the assigner's brain-foundational value SURVIVES only on the non-canonical/passive slice.** **hdlab AGENT GUARDRAIL LANDED (flag-gated, DEFAULT-OFF): `graded_role_assigner.hybrid_agent_pick`** (the AGENT counterpart to `hybrid_role_patient` — word-order default + invoke the CM competition, WHICH CARRIES the landed `byhead` cue, ONLY on a marked cue: passive-with-by-phrase / PP-governed / non-nominative) behind `situation_reader.agent_hybrid`; + construction cues (existential + guarded NP-coordination, Goldberg) behind `agent_hybrid_construction`. Organ-level (UD train+test): canonical NO-REGRESS (hybrid 0.8843 ~= positional 0.8904), NON-CANONICAL +0.1248, existential +0.467, full-set +0.0019 net; cross-corpus GUM +0.0095 CI-sep. **DEFAULT-OFF is a MEASURED register reason (not a hidden subpar downstream): the reader's LIVE who-did-what board is still 19c LitBank where flipping the hybrid ON regresses with no modern upside; the MODERN who-did-what board arm computes the hybrid DIRECTLY (independent of the reader flag), so the modern value is scored without the 19c regression. `agent_hybrid=False` = the AGENT byte-identical to the pre-hybrid reader (CM board organ witness 10/10, PATIENT byte-identical).** **1b byagent-COVERAGE fix SKIPPED — RECONCILED as SUBSUMED BY the landed `byhead` cue (verified first-hand): on the passive slice byhead ON 0.6044 vs the pre-byhead byagent-only 0.3187 (+0.2857) ≥ the skipped 1b `_byagent_fixed` patch (0.5220) — `by_governs`+`participle_bypp_gate` already scan over DET/ADJ/NUM/possessive to recover multi-word by-phrases, so 1b would DOUBLE-COUNT.** Board self-tests PASS (standard + modern, new-arms-off, artifact intact). DEFERRED (strategy owns): run the full modern board `--run` to PERSIST the new arms + crossref into `metrics.json` (verification used write_metrics=False) → the realized end-to-end aggregate; point `tools/baseline_board.py` Instrument A at the modern board (one-liner). FOLLOW-ONS (filed): a MODERN coarse-WSD gold (promote coarse-sense informational→scored); modern temporal/goal/affect golds (the named gaps); the `_agent_pp_governed` comma-stop precision fix. NOTE: the 3 pre-existing reader self-tests (`_selftest_frame_arity_gate`/`read_end_to_end`/`event_extraction_coverage`) still fail identically on the baseline (the filed verification-hygiene cleanup). Files: `experiments/exp_situation_model_qa_modern_v1.py`, `hdlab/graded_role_assigner.py`, `hdlab/situation_reader.py`, `verification/test_modern_board_landing.py`.

- **2026-09-06 (INTEGRATION — the reader gains a CAUSAL-NETWORK REASONER: multi-hop chain traversal + counterfactual necessity by simulated intervention over the live `sm.causal_links`. The reasoning phase's causal-inference system. EXCELLENT, owner-DONE, Q111.)** Problem `reason_over_the_causal_network_multi_hop_chains_and_counterfactuals`, reverified first-hand `verification/test_causal_reasoner_soundness.py` **9/9**; new landing witness `verification/test_causal_reasoner_landing.py` **ALL PASS (re-run first-hand)**. **NEW ORGAN `hdlab/causal_reasoner.py`** — `CausalGraph` over event nodes with signed graded-necessity cause→effect edges (source BYTE-IDENTICAL to `experiments/_causal_reasoner.py`, `inspect.getsource` identity), REUSING the `hdlab/goal_hierarchy_graph` traversal pattern (ancestors/root/connectivity + the shuffled-edge twin). WIRED DEFAULT-ON via `situation_reader.track_causal_reasoning` (in CAPABILITY_FLAGS) → `_read_causal_reasoning` MIRRORING `_read_tom_action` (lazy + additive; builds ONE CausalGraph from `sm.causal_links` on first callable invocation, binds `sm.causal_reasoner` + `sm.ultimate_cause`/`is_necessary`/`counterfactual`/`mediating_cause`/`chain_of_consequence`/`graded_necessity`/`signed_effect`; ZERO read-time cost until invoked → default read BYTE-IDENTICAL). A NEW ISLAND / query layer over the EXISTING causal network. **PINNED:** Trabasso & van den Broek 1985/1989 causal-network REACHABILITY (ultimate cause = root ancestor, mediating = path node, chain = forward reachability, importance = network CONNECTIVITY not recency); Pearl counterfactual (abduction → do(cause=absent) → re-propagate → compare); Halpern-Pearl ACTUAL causation (over-determination); Kahneman-Miller norm theory (the mutable node). **MEASURED first-hand: SOUND on constructed DAGs (n=4000) — ultimate-cause 1.000 vs adjacency 0.000 on the multi-hop subset (r−adj +1.000 CI[1.0,1.0], r−twin +0.840 CI-sep); counterfactual necessity 1.000 vs adjacency 0.482 / twin 0.479 (null p95 0.0114); graded-order 1.000; Pearl-agree 0.918; Halpern-Pearl over-determination handled. LOAD-BEARING on MODERN gold: WIQA (refutes the prior HARD_FAILs — reason 0.585 vs polarity-echo 0.321 + majority 0.422; multi-hop reason 0.256 vs 1-hop adjacency 0.000 +0.256 CI-sep); TellMeWhy non-adjacent causes (n=299) the reasoner is the ONLY method that finds the human-annotated cause (0.2375 vs adjacency/recency 0.000 + shuffled-twin 0.117, all CI-sep).** **HONEST BOUND (the value is correctly located): the cause is usually the ADJACENT prior sentence (~69%), so a dumb "previous sentence" rule is hard to beat GLOBALLY (dense 0.32 < adjacency 0.69); the reasoner's win is the MULTI-HOP / non-adjacent / counterfactual subset — exactly where reasoning is needed. The directed event-type gate does NOT help (0.28) — the residual is the directed causal-KNOWLEDGE wall, quantified.** ADDITIVE-SAFE: `sm.causal_links` + every existing dim BYTE-IDENTICAL off-vs-on (connective causal QA / coref / events unchanged); default-on-lazy = the ToM/bridging precedent. **🔧 UPSTREAM GAP (the full-stack-upstream directive, named not hidden): the real-narrative causal network is SPARSE (median chain depth 0 on ROCStories, only 3.2% support a ≥2-hop chain — most edges within-sentence connective/mental) → the reasoner often has little to walk. The Trabasso contiguity+plausibility DENSIFICATION fixes density (3%→95%) but its edge CORRECTNESS is UNVALIDATED (CSKG covers only 17.3% of true cause-pairs) → landed as a DEFAULT-OFF follow-on (a NEW `sm.inferred_causal_links` field, do NOT touch `sm.causal_links`), held until validated on directed causal QA gold. Do NOT land the associative-relatedness densification as-is (topical).** BOARD-VISIBILITY: board self-test PASS; the multi-hop/counterfactual win is board-INVISIBLE (the board's causal dim scores connective-only ADJACENT gold) → a `board_causal_multihop_dimension` arm (reuse the WIQA multi-hop + TellMeWhy non-adjacent measurements) is the follow-on. NOTE: 3 pre-existing reader self-tests (`_selftest_frame_arity_gate`/`read_end_to_end`/`event_extraction_coverage`) fail identically on the baseline (stale from the 2026-09-03 default flips — a filed verification-hygiene cleanup, not this wire). Files: `hdlab/causal_reasoner.py`, `hdlab/situation_reader.py`, `verification/test_causal_reasoner_landing.py`.

- **2026-09-06 (INTEGRATION — the reader gains its FIRST FORWARD-PROJECTION organ: `sm.predict_next_event` (Elman generalized-event-knowledge). The substrate verifiably LACKED any forward projector — it was all-backward. STRONG PARTIAL, owner-DONE, Q111.)** Problem `predictive_inference_forward_project_the_next_event_and_state_from_the_situation_model`, reverified first-hand `verification/test_forward_event_projection.py` **8/8**; new landing witness `verification/test_forward_projection_landing.py` **12/12 (re-run first-hand)**. **NEW ORGAN `hdlab/generalized_event_knowledge.py`** — the forward-transition GEK store (a self-supervised directed PPMI "what content follows what," fit OFFLINE on ROCStories-train = a STATIC ADMISSIBLE FOUNDATION ASSET `data/frontend_assets/generalized_event_knowledge_roc_fwd.npz`, 36 MB gitignored, reproducible `python -m hdlab.generalized_event_knowledge --build`; `GEKProjector`, abstain-safe lazy load). WIRED DEFAULT-ON via `situation_reader.track_prediction` (in CAPABILITY_FLAGS) → `_read_prediction` MIRRORING `_read_bridges`/`_read_senses` (lazy + additive; binds `sm.predict_next_event(candidates=None, t=None)` composing the LIVE registers — GEK content cue + goal cue via `graded_competition`, precision = 1−entropy; `sm.forward_prediction=None` until invoked → default read BYTE-IDENTICAL). A NEW ISLAND (no downstream consumer today → no regression). **PINNED:** forward prediction during comprehension = a graded associative co-activation readout over GENERALIZED EVENT KNOWLEDGE (Elman 2009; Metusalem 2012; Federmeier & Kutas 1999 top-down prediction), NOT a discrete script lookup; precision = distribution concentration / inverse entropy (Kuperberg & Jaeger 2016; Hale). **MEASURED first-hand (Story Cloze, MODERN, val n=1871): forward GEK 0.5927 vs majority floor 0.5142 (+0.0786, 95% lo +0.046 CI-sep); cross-context info-free twin COLLAPSES to 0.4917 (it USES this story); calibrated-precision selective accuracy RISES 0.5927→0.6560 @25%.** **LOCATED NEGATIVE (rigorous, triple-sourced — a full pass):** it does NOT beat a 1-step co-occurrence counter (ties, +0.0091 CI incl 0); the multi-step successor HORIZON adds nothing (~+0.01); the residual is EXTRACTION DENSITY (goal/causal registers fire on ~27% of short stories — a separate problem's lane), NOT the inference design. The v2/v3 artifact-free coherence engine + Friston precision-weighting tips razor-thin CI-sep over the counter (test +0.0283). ADDITIVE-SAFE: all 10 existing dims BYTE-IDENTICAL off-vs-on (git-stash A/B confirmed); default-ON-lazy = the bridging/senses precedent (asset loads + abstains cleanly when absent = asset-safe on fresh clones; additive + zero read-time cost = the ideal no-default-off case). Board self-test PASS. **🔁 THE HIGH-VALUE FOLLOW-ONS (the user's full-stack-upstream directive + the 2026-09-06 wall-map's RECURRENT-LOOP north-star) — confirmed clean (no regression), NOT built here:** (a) extend `predict_surprisal` from argument-level to EVENT-level forward expectation; (b) **`n400_coherence_monitor` taking its error against THIS forward prediction = the TRUE PREDICTIVE-CODING LOOP** — the SOLVED loop cell already MEASURED the payoff: forward-error event segmentation F1 **0.766–0.806 vs the backward gist 0.272 (~3×)**. This closes the dominant missing piece the wall-map named (the built-but-inert `predictive_reader` top-down organ) → file as the north-star. NOTE: the pre-landing reverify's **W7 "no forward projector in hdlab" absence check now FAILS BY DESIGN** (this landing falsifies that absence — the point); premise-stale post-landing (like the double-parse W1/W4/W7), left unedited. + the 3 pre-existing reader self-tests (`_selftest_frame_arity_gate`/`read_end_to_end`/`event_extraction_coverage`) fail identically on the baseline (not this wire). Files: `hdlab/generalized_event_knowledge.py`, `hdlab/situation_reader.py`, `verification/test_forward_projection_landing.py`.

- **2026-09-06 (INTEGRATION — the RELIABILITY SUBSTRATE now DELIVERS: a live DEFER-CONSUMER acts on the calibrated parse confidence, `precision_weight_roles` flips DEFAULT-ON at ZERO extra parse cost, and `event_conf` becomes the reasoning-phase's whole-event trust signal. EXCELLENT, owner-DONE, Q111.)** Problem `wire_a_defer_consumer_for_calibrated_confidence_and_realize_precision_weighting`, reverified first-hand `verification/test_defer_consumer_organ.py` **9/9**; new landing witness `verification/test_precision_defer_landing.py` **4/4 (re-run first-hand)** + the flipped-default fix to `test_precwt_landing.py` **ALL PASS**. This is the CONSUMER the precision_weight predecessor lacked (it landed a calibrated confidence but nothing DEFERRED, and it was default-OFF for "no consumer + ~2 parses/read"). **PINNED:** precision-weighting = weight a commitment by the inverse-variance reliability of its driving estimate (Ernst & Banks 2002 MLE; Friston 2010 active inference); ABSTAIN = the Kepecs 2008 / Kiani & Shadlen 2009 decision-confidence OPT-OUT. **WIRE LANDED (Q111):** (1) COST FOLD — `_cached_parse_heads` now derives from the single shared `parse_with_conf` `(heads,conf,marg)`; `_cached_parse_conf` reads that shared cache (no re-parse); `a2_marg` dropped (inert for patient) → **the defer path adds ZERO extra parses (the default-off blocker RESOLVED)**. (2) DEFER-CONSUMER — abstain below a frozen dev-`precision_weight_tau`, emitting `patient_defer`/`agent_defer` (no fall-back — a LOCATED NEGATIVE). (3) AGENT defer — `graded_role_assigner.agent_competition_pick_conf` emits the raw Competition-Model margin as reliability (NO calibration; pick byte-identical). (4) `EventRecord.event_conf = agent_conf * patient_conf` — the product-of-role-precisions **reasoning-phase bridge** (defer on whole EVENTS). (5) FLIP `precision_weight_roles` **DEFAULT-ON** (emission byte-identical at tau=None; defer is the consumer's opt-in via tau; `all_capabilities_off` forces False). **MEASURED first-hand on MODERN gold through the FROZEN `parse_confidence` calibrator, live single-shared-parse config: PATIENT answered-acc 0.8789→0.9676 @cov 0.688 (+0.0887 CI[+0.0726,+0.1059]); obl/spatial 0.7581→0.8388 @cov 0.735 (+0.0807 CI[+0.0684,+0.0931]); the RANDOM-confidence twin FLAT (patient +0.011 / obl +0.004) — calibration is load-bearing; event-level AUC product(raw-margin×patient)=0.771 > agent 0.722 / patient 0.656, event-abstain +0.1168 CI-sep.** ADDITIVE-SAFE (W2): with the flag default-ON + tau=None the read is BYTE-IDENTICAL to OFF on every dim + parse head; only the additive conf/defer/event_conf metadata is added. **DENSITY-PHASE (owner phase-diagram tie-in): the abstain gain SCALES with role-competition density (+0.063 sparse → +0.124 dense), confidence AUC stable — precision-weighting earns its keep MOST where competition is densest.** **LOCATED NEGATIVES (each a full pass): (a) FALL-BACK to a head-independent prior adds nothing (position 0.842 < the parser; the head-independent patient has no better prior); (b) the UPSTREAM small-beam parse posterior does NOT out-calibrate the landed confidence — the greedy arc-eager parser is LOCALLY NORMALIZED (label bias; Andor 2016) + half its errors are SEARCH failures (gold-in-beam-when-wrong 0.399). → THE NORTH-STAR IS A GLOBALLY-NORMALIZED parser with Matrix-Tree marginals** — which INDEPENDENTLY RE-LANDS on the 2026-09-06 wall-map's dominant non-brain decision (the greedy, hard-1-best, distribution-free parse); it would also give the PATIENT arc a real competition margin + enable a genuine 2nd-best fall-back. **BOARD-VISIBILITY (live ≠ scored): the flip-on emission is byte-identical, so no current board dim moves; the DEFER GAIN needs its own MODERN selective-reliability (risk-coverage) board arm — `board_selective_reliability_dimension` recipe recorded (reuse the answered-acc-at-dev-tau, a2=0 rows).** SCOPED FOLLOW-ON: the obl calibrated confidence's live consumer (the SPACE register) is not wired this landing. NOTE (pre-existing, NOT this wire): 3 reader self-tests (`_selftest_frame_arity_gate`/`read_end_to_end`/`event_extraction_coverage`) fail identically on the baseline (stale from earlier default flips). Files: `hdlab/situation_reader.py`, `hdlab/graded_role_assigner.py`, `verification/test_precision_defer_landing.py`.

- **2026-09-06 (INTEGRATION — the MEANING CHANNEL gains its FIRST live WORD-SENSE-SELECTION consumer (`sm.select_sense`), committing the COARSE (shared-core) sense; the brief's hub-decorrelation is a LOCATED NEGATIVE. EXCELLENT, owner-DONE, Q111 — and unlike the coref wire, the win TRANSFERS live.)** Problem `select_word_sense_by_context_primed_biased_competition_over_a_decorrelated_sense_hub`, reverified first-hand `verification/test_sense_hub_separation_and_selection_organ.py` **9/9** + `verification/test_underspecified_sense_reader.py` **5/5**; new landing witness `verification/test_word_sense_meaning_stage_landing.py` **11/11 (re-run first-hand)**. **LOCATED NEGATIVE (land NOTHING on the hub):** hub decorrelation/whitening is provably argmax-NEUTRAL for the cosine-argmax readout (common-mode removal = 0 pick-changes at n=50,386 + n=7466) and CI-sep NEGATIVE when it reweights, and brain-UNFAITHFUL (the ATL DISTILLS — Patterson-Nestor-Rogers 2007; decorrelation is HIPPOCAMPAL — CLS 1995; polysemy shared-core overlap is the correct geometry — Rodd 2002). **The unverified +0.0176 a_s whitening figure is RETRACTED** (small-sample noise; CI-sep negative at full n). Every brain-faithful glass-box selection cue (context de-blur, distinctive-feature IDF, thematic-frame, joint-role bind, Metusalem discourse-event) reads REAL structure (beats its info-free twin) but is SUBSUMED by local topical context → the fine-sense ceiling is CONTEXT-INPUT REPRESENTATION × COVERAGE, not the hub geometry or the readout. **§2 no-encoder HOLD EMPIRICALLY CONFIRMED for the RIGHT reason (the brain uses no encoder either; the over-strong "unreachable without a barred encoder" framing WITHDRAWN — the brain resolves sense via glass-box structured PREDICTIVE comprehension, Phase-1).** **THE CONSTRUCTIVE WIN REALIZED LIVE — NEW ORGAN `hdlab/underspecified_sense_reader.py`** (promoted byte-faithful from `exp_underspecified_sense_reader_v1`; `select_sense`/`coarse_cluster`/`compose_joint`; imports ONLY `meaning_foundation` + `diagnostic_context_wsd` + wordnet + a lazy `default_vec_lookup`). WIRED DEFAULT-ON via `situation_reader.track_senses` (in CAPABILITY_FLAGS) → `_read_senses` MIRRORING `_read_bridges` EXACTLY (lazy + additive; binds `sm.select_sense(token, context_words, ...)` + `sm.senses=[]`; ZERO read-time cost until invoked → default read BYTE-IDENTICAL). **PINNED:** underspecification-by-default (Frisson 2009 good-enough / Rodd 2002 shared core — COMMIT to the coarse sense, elaborate to fine only on task demand); the readout is the biased-competition SELECTION over the fixed context-independent ATL hub (Swinney/Duffy/Lambon Ralph CSC). **MEASURED (committed COARSE sense, SemCor subordinate n=2353, INFORMATIONAL — mid-20c, not modern gold): a_s 0.5236 vs coarse-MFS floor 0.3544 (+0.1691 CI-sep) AND context-shuffle twin 0.4033 (+0.1203 CI-sep)** — 37% of fine "errors" are near-misses INSIDE the correct coarse sense the brain never splits. **THE WIN TRANSFERS LIVE (the coref-lesson risk does NOT bite): `default_vec_lookup()` is byte-identical to the reference sglite-w2v lookup (10/10 probes) + the live config == the reference passthrough default, so U2 reproduces EXACTLY through the live reader (per-item coarse picks 2353/2353).** ALSO landed: `compose_joint("bind")` = multiplicative Bayesian-AND (bind>bundle, Lenci 2011, +0.011/+0.013 CI-sep) — a reusable role-composition principle. ADDITIVE-SAFE: all 9 existing dims BYTE-IDENTICAL off-vs-on; `mode="fine"` at default knobs == the raw `diagnostic_context_wsd` argmax 400/400 (promotion changes NO existing readout). Board self-test PASS. **BOARD-VISIBILITY (live ≠ scored): the +0.169 needs its OWN board arm — the `wic` arm scores a DIFFERENT metric/population (curated-vs-PPR same/diff-sense on WiC) and does NOT consume `sm.select_sense`. Recipe recorded (reuse the U2 coarse measurement); BUILD it on MODERN gold (Raganato/SemEval-2015 = the acquisition gap) in the modern-board integration — the SemCor +0.169 is informational under the 19c ban.** NOTE (pre-existing, NOT this wire): the two reader formula self-tests `_selftest_frame_arity_gate` + `_selftest_read_end_to_end` fail IDENTICALLY on the baseline (verified stash-tested) — the known `verb_subcat_gate` / memory-roundtrip issues, upstream of this additive stage; the authoritative board self-test PASSES. Files: `hdlab/underspecified_sense_reader.py`, `hdlab/situation_reader.py`, `verification/test_word_sense_meaning_stage_landing.py`.

- **2026-09-06 (INTEGRATION — the UNIFIED DISCOURSE REFERENT (Heim/Kamp DRT file-change) LANDED FAITHFUL but DEFAULT-OFF: a MEASURED located-negative-on-the-LIVE-board. EXCELLENT isolated-lever science + a faithful wire, owner-DONE, Q111.)** Problem `form_the_unified_discourse_referent_before_the_pronoun_pick_the_shared_entity_lever`, reverified first-hand `verification/test_unified_referent_gum.py` **5/5** (GUM MODERN, 137-doc TEST: pronoun pick separate 0.3621→unified 0.4681 **+0.1060 CI[+0.079,+0.133]**, entity-KB hard-link +0.0719, twins lose, name no-regress). **NEW ORGAN `hdlab/unified_referent.py`** — a byte-faithful port of `exp_unified_referent_gum_v1.Resolver(arm='unified')` onto the live mention schema (EntityAliaser name-variant merge ∪ blind-surface; cross-type file-change writeback incl. resolved-pronoun writeback; `salience_binder.actr_activation` d=2.0; recall-safe gn; guardrails all OFF); scored intrinsically (`ref.dominant_cluster()`) so the `head_to_cluster` fragmentation risk cannot occur. WIRED behind `situation_reader.unified_referent` (in CAPABILITY_FLAGS) → `EventCentralityReader(unified_referent=…)` → an early-return to `resolve_unified_stream`. New landing witness `verification/test_unified_referent_landing.py` **14/14 (re-run first-hand)**: W1 flag-off byte-identical to the landed graded_pick reader; W2 the live wired pick == the reference Resolver unified-arm pick EXACTLY (4 constructed streams + GUM slice reproduces +0.106 direction); W3 coref-independent dims (events/causal/timeline) byte-identical on/off. Board self-test PASS. **THE MEASURED FINDING (why DEFAULT-OFF, verified first-hand — the honest core): the solver's +0.106 is an ISOLATED-LEVER science result — unified-vs-fragmented representation with the SCORER HELD FIXED at pure ACT-R d=2.0 — and its baseline (0.3621) is the reference's own gender-STRANDED separate arm, NOT the live reader.** Against the **LIVE incumbent** (the landed 2026-09-06 graded ACT-R pick: d=3.0 + subjecthood + generic-distractor suppression + agreement-narrow) on the he/she GUM population, the faithful unified port scores **0.4331 vs 0.5032 (−0.0702 on MODERN gold)** and **−0.085/−0.176 on 19c** (informational) — the live incumbent's stronger POOL config ALREADY SUBSUMES the unification lever. This is the twin of the phase-diagram lesson (owner 2026-09-06): **solvers measure against an ISOLATED/DEFAULT config, not the LIVE (already-tuned) substrate.** Per no-more-default-off, DEFAULT-OFF is the MEASURED-correct disposition (I measured the flip → net-negative on the live consumer). The unification lever IS real; realizing it live requires the HYBRID (re-key `WorkingOverlay._entities` to unified referents while KEEPING the incumbent graded_pick + suppression pool — a compose, not a swap), which does NOT reproduce the reference exactly and needs its own measurement → filed follow-on. Also corroborates the standing "coref board instrument is banned 19c" concern (the live coref dim is 19c LitBank, so any modern-gold coref win is board-invisible until the instrument is swapped to modern gold — the modern-board p1 integration). Files: `hdlab/unified_referent.py`, `hdlab/event_centrality_coref.py`, `hdlab/situation_reader.py`, `verification/test_unified_referent_landing.py`.

- **2026-09-06 (INTEGRATION — PRECISION-WEIGHTING the head-driven readers LANDED: the parser's per-arc confidence, CONSUMED BY NOBODY, is now exposed as a CALIBRATED reliability the who-did-what patient / obl readers can TRUST or DEFER on (Friston precision / active inference). This is the RELIABILITY SUBSTRATE the reasoning phase stands on — an inference engine must know which facts to trust. EXCELLENT, owner-DONE, Q111).** Problem `precision_weight_the_head_driven_readers_on_calibrated_parse_confidence` (the filed follow-on of the parser located-negative below). Reverified the solver witness first-hand `verification/test_precwt_live_readers_organ.py` **6/6**; new landing witness `verification/test_precwt_landing.py` **W1 W2 W3 PASS (re-run first-hand)**. **NEW ORGAN `hdlab/parse_confidence.py`** — the FROZEN glass-box calibrator (a STATIC offline-built asset, admissible; NO training at inference): `calibrated_patient_confidence` / `calibrated_obl_confidence` map an arc's reliability features → a sensitivity-optimized confidence in [0,1]. **BRAIN-FOUNDATIONAL:** precision-weighting (weight a downstream commitment by the reliability of its driving estimate) is PINNED at the computation level (Ernst & Banks 2002 inverse-variance MLE; Friston 2010 active inference; Kepecs 2008 / Kiani & Shadlen 2009 decision confidence gates opt-out); the calibration IS the substrate's own pinned `graded_competition` operation (additive Lewis-Vasishth cue activation → softmax = the McClelland-2013 Bayesian posterior over {correct, incorrect}; weights = Bates-MacWhinney cue validities) — REUSES the landed organ (the role-competition entropy cue), reinvents nothing. Applying confidence to gate the THEMATIC-ROLE readout is an architecture-level extrapolation (OUR-INVENTION-UNDER-TEST, a-priori P~0.30) — tested, UPHELD, twin-controlled. **REFINE the "calibrated" language: strict CALIBRATION is NOT a brain property (hard-easy effect; Fleming & Dolan 2012 sensitivity != calibration); the load-bearing property is confidence SENSITIVITY (meta-d' / AUC / risk-coverage), which is what the readout optimizes.** **RECONCILES the settled negative:** "confidence-weighting is a WEAK end-to-end lever (roles margin AUC 0.538 deployed)" is CORRECT for the RAW emitted arc margin (reconfirmed AUC 0.615 UD / 0.501 QA / 0.736 obl) and does NOT contradict this — the win is the CALIBRATED multi-cue confidence (AUC 0.858 UD). Record: raw margin = weak (do NOT wire); calibrated confidence = the lever. **WIRE LANDED (Q111, DEFAULT-OFF, byte-identical fallback):** `situation_reader.precision_weight_roles` (in CAPABILITY_FLAGS) attaches `EventRecord.patient_conf` (in [0,1]) + `patient_defer` (at `precision_weight_tau`) to each wired event's who-did-what PATIENT parse arc; `_patient_arc_confidence` builds the byte-faithful validated row (arc-eager conf/marg + global arc_parser margin + graded-competition role-entropy + labeled-obj indicator + passive) and scores it through the frozen calibrator; defensive (any failure → None abstain, never breaks a read). The obl/spatial calibrator is landed as the reusable organ (the SECOND head-driven reader) — NOT deep-wired into `_read_space` (the space named-ground binding is separately integrated). **MEASURED first-hand on MODERN gold (19c-clean — the reading-corpus age confound does NOT apply) THROUGH THE FROZEN calibrator:** who-did-what PATIENT selective@50 **0.8789→0.9745 (+0.0956 CI[+0.0781,+0.1139], null-p95 +0.0159)** on UD-EWT test (n=1255) and **0.2982→0.3414 (+0.0432 CI[+0.0325,+0.0532])** on QA-SRL (n=8225); the SECOND reader obl/spatial attachment **0.7581→0.8919 (+0.1338 CI[+0.1173,+0.1508])** on UD-EWT obl/nmod (n=2294); RANDOM-confidence twin FLAT on every reader (UD −0.0017 / QA +0.0009 / obl −0.0031), cleared 5–9×; risk-coverage monotone (1.000 @10% → 0.879 blanket). **ADDITIVE-SAFE (W3): reading a real doc ON vs OFF leaves every SituationModel dim (events/coref/timeline/causal/memory/suppressed) AND all parse heads BYTE-IDENTICAL — only the new patient_conf/defer metadata is added; the board self-test PASSES.** **UAS IS NOT THE OBJECTIVE — comprehension RELIABILITY is:** blanket accuracy is unchanged by construction (additive); the deliverable is the reader now KNOWING which half of its role picks to trust. Do NOT re-attach the parse (post-hoc re-attachment CI-sep HURTS UAS — precision-WEIGHT the readout only). **DEFAULT-OFF (honest, flagged to owner): the wire is additive-safe but board-INVISIBLE (no live consumer reads the confidence yet — the reasoning phase is its consumer) and adds ~2 parses/read on the opt-in path, so it is NOT "net-positive on a live metric" today; per no-more-default-off this is the legitimate "measure impact → benefit is latent" case. FLIP-ON trigger: when a reasoning-phase consumer defers on the confidence AND the board read-cost/additive-no-regress check passes.** FOLLOW-ONS (filed, from the SOLVED): (1) a MODERN who-did-what/obl selective-reliability board arm (the board scores blanket accuracy on 19c gold → this gain is board-invisible until it gets its own instrument arm — the recurring live≠scored pattern); (2) the two upstream organs that hold the un-captured oracle reliability — a SMALL-BEAM distributional parser (so the PATIENT arc gets the AGENT competition-margin's property) + a GRADED thematic-fit meaning channel (the "confidently wrong though structurally clean" residual, Kim-Osterhout semantic-P600) — each PINNED, neither a bolt-on (a recapture prototype confirmed the losses are STRUCTURAL). Files: `hdlab/parse_confidence.py`, `hdlab/situation_reader.py`, `verification/test_precwt_landing.py`.

- **2026-09-06 (INTEGRATION — BRIDGING inference LANDED: the reader infers the unstated link between adjacent sentences (which whole a part belongs to / which event an instrument serves) over the ATL semantic hub — AND this is the MEANING CHANNEL'S FIRST LIVE read()-time CONSUMER (it was entirely LATENT). EXCELLENT, owner-DONE).** Problem `bridging_inference_infer_the_unstated_link_between_adjacent_sentences`, reverified first-hand `verification/test_bridging_inference.py` **33/33** + landing `test_bridging_inference_landing.py` **17/17**. **NEW ORGAN `hdlab/bridging_inference.py`** (`BridgeInference.select` = argmax meaning-store relatedness over candidate antecedents, glass-box trace, abstains below a margin; lazy asset loaders, `available()` returns False→abstain so default-on is asset-safe). WIRED DEFAULT-ON (lazy, zero read-time cost) as `sm.bridge`/`sm.infer_bridges`, REUSING the live ATL PPMI+SVD hub + `hdlab/meaning_foundation` — rebuilds neither. PINNED: Kintsch 1988 construction-integration (construct the stated net + likely bridges, integrate by spreading activation, keep what settles); Clark 1975 bridging / Haviland & Clark given-new; the Lambon Ralph ATL hub supplies the graded relatedness; the specific-not-salient link is the N400 (Kuperberg & Jaeger 2016). MEASURED through the LANDED organ (argmax==gold antecedent, held-out, item bootstrap): referential-PART WordNet meronymy 0.4720, ConceptNet PartOf 0.6087 (curated `meaning_foundation` 0.6541 = the best source), INSTRUMENT UsedFor 0.4522 — each CI-scale over the no-inference (0.20) + most-salient floors, shuffled-meaning twin collapses to chance (0.197/0.202/0.197), PER-ITEM BYTE-IDENTICAL to the validated cell. ADDITIVE: all 8 existing dims byte-identical off-vs-on. **The meaning channel — four proven gains sat LATENT with no read()-time consumer — now has its FIRST.** DEVIATIONS (honest): landed the VALIDATED selector (raw hub / curated mfnd / optional N400 `cond`), NOT the recommended heavier PPR-FUSE + entropy-gate (prototyped follow-on, +0.036/+0.090 held-out); `sm.bridge` query + on-demand `sm.infer_bridges` (not eager auto-emit — keeps the default read byte-identical). Located negatives carried over: relation-TYPE selection is structural not distributional; end-to-end is extraction-bound (~0.50). FOLLOW-ON (filed): a `board_bridging_dimension()` arm (board-invisible win — live≠scored); the FUSE/entropy-gate estimator; bridge over coref-resolved referents. NOTE (pre-existing, NOT this landing): `_selftest_frame_arity_gate` fails via `verb_subcat_gate` (reproduced with `track_bridges` on AND off, identical); `test_assembled_reader_all_flags_on.py` is slow via the `causation_typed`/spaCy path.

- **2026-09-06 (INTEGRATION — THEORY-OF-MIND inference LANDED: the reader predicts an agent's ACTION from what it BELIEVES × what it WANTS (reading the action off the BELIEVED state, so FALSE BELIEF falls out by the same rule); the reasoning phase's first mentalizing system. EXCELLENT, owner-DONE).** Problem `chain_belief_and_goal_into_theory_of_mind_inference_intention_and_false_belief`, reverified first-hand `verification/test_tom_chain.py` **9/9** + landing `test_tom_chain_landing.py` **12/12**. **NEW ORGAN `hdlab/theory_of_mind.py`** (promoted byte-faithful): `forward_action` (believes×wants→PROCEED/FETCH), `attribute_belief_from_action` (the SAME engine run INVERSE — Baker intentional stance), a present-tense percept gate + a change-of-state reality lexicon. WIRED DEFAULT-ON (lazy, zero read-time cost) as read()-time queries `sm.predict_action`/`will_act_on`/`attribute_belief`, REUSING the live `hdlab/belief_timeline` (rTPJ belief) + `hdlab/goal_register` (dmPFC intention) — rebuilds neither. PINNED: Baker/Saxe/Tenenbaum 2009/2017 Bayesian ToM as a forward planner (argmax = high-β softmax limit); Leslie 1987 meta-representation (act on believes NOT reality → false belief by the same rule); Wimmer & Perner 1983 percept-gated update. MEASURED on BigToM (Gandhi et al. 2023, MODERN peer-reviewed gold, 19c-clean): belief-prediction CHAIN 0.849, FALSE-belief subset +0.871 CI-sep over a reality floor PROVABLY 0% on false belief; info-free twins (percept/belief-shuffle) LOSE; composition EXACT with oracle belief (1.000) → the residual is EXTRACTION. ADDITIVE: byte-identical off-vs-on on every existing dim (believes/knows/wants unchanged — the ToM chain is a READ-ONLY query). DEVIATIONS (honest): default-ON not -off (lazy + additive = the ideal no-default-off case); landed the organ + read-out ONLY, kept the risky live-belief-extraction modernization OUT to preserve additive-safety (a separate belief-extraction landing gate). FOLLOW-ON (filed): a `board_tom_action_dimension()` arm (board-invisible win — live≠scored); the goal→fact desired-VALUE binding via the meaning channel (the action-prediction ceiling).

- **2026-09-06 (INTEGRATION — the who-did-what AGENT mis-picks on non-canonical/passive clauses because word order misleads; the fix is a by-phrase CASE-MORPHOLOGY cue (byhead) wired default-on. The grounded selectional-fit cue is a LOCATED NEGATIVE (not landed). EXCELLENT, owner-DONE, Q111).** Problem `grounded_meaning_role_cue_for_non_canonical_who_did_what_where_word_order_misleads`. New landing witness `verification/test_byhead_agent_cue_landing.py` **15/15 (re-run first-hand)**; the solver witnesses `test_noncanonical_agent_bymorph_organ.py` **5/5** + `test_cmrole_agent_board_byhead_organ.py` **2/2** + the reader landing `test_cmrole_agent_landing_organ.py` **5/5** all still pass. **PINNED:** thematic-role assignment is graded parallel cue competition (Bates & MacWhinney Competition Model), and CASE MARKING is a top-validity cue — "by" is English's morphological marker of the DEMOTED passive agent. On "the tea was poured by the WOMAN" / "formed by a natural PROCESS" word order + animacy mislead; the landed `byagent` cue only fires when a candidate's PREV token is literally "by" (prevtok=='by') so it MISSES multi-word by-phrases ("by the clerk"/"by a natural process") — instead `core_arg` PENALIZES them (PP-governed → not-subject). **WIRE LANDED (Q111, DEFAULT-ON):** `hdlab/graded_role_assigner.py` gains `by_governs` (scan left over DET/ADJ/NUM/possessive to the governing "by") + `participle_bypp_gate` (the V-en + by-NP construction detector) + a self-gating `byhead` support in `agent_supports` (emitted only when the caller passes `byhead_agent_cue` AND the gate fires) + `AGENT_VALIDITIES["byhead"]=BYHEAD_W=10.0` (SWEPT on QA-SRL {4,6,8,10,12}, saturates at 10; OUTVOTABLE, not a hard override); `agent_competition_pick` threads the flag; `situation_reader.cm_agent_byhead` (default True, in CAPABILITY_FLAGS) passes it through `_cm_agent_for` → `agent_competition_pick(byhead_agent_cue=True)`. `byhead_agent_cue=False` / `cm_agent_byhead=False` = the exact incumbent (self-checkable; all_capabilities_off sets it False). Byte-faithful: the landed hdlab byhead pick is PER-ROW IDENTICAL to the validated experiment arm. Promoted VERBATIM from `experiments/exp_noncanonical_agent_bymorph_v1.py` (by_governs/gate/injection) + `exp_cmrole_agent_board_byhead_v1.py` (the board wire). **MEASURED first-hand on MODERN QA-SRL (the powered instrument, 19c-clean — the reading-corpus age confound does NOT apply): clean agent-post slice 0.2556→0.6889 (n=90, +0.4333 CI[+0.3333,+0.5333]); full non-canonical 0.5224→0.6866 (n=201, +0.1642 CI[+0.1144,+0.2189]); CI-separated over BOTH the live-competition floor AND the info-free shuffled-by-membership twin (which LOSES); canonical no-regress (n=845, 0.6959→0.6994, CI[−0.0036,+0.0118]).** **THE PARTICIPLE+BY-PP GATE BEATS is_passive_clause** — 14 vs 106 canonical false-fires (8× higher precision) while covering ≥ as many real by-passives → use IT, NOT `is_passive_clause` (which would false-fire on canonicals). **LOCATED NEGATIVE (NOT landed): the whitened grounded selectional-fit ('selfit') cue TIES its info-free scrambled-meaning twin on non-canonical AGENT (0.5124 vs 0.5224, NOT CI-sep)** — on animate agents the ANIMACY cue already saturates; on inanimate causers ("formed by a natural process") the reliable signal is the by-phrase MORPHOLOGY, not a semantic prototype. Grounded meaning does not carry non-canonical role over structure+animacy; only `byhead` is the win. **ADDITIVE-SAFE (agent-only): 8 docs on/off — 11 event AGENT-field diffs, 0 non-agent event-field diffs (patient/predicate/tense/affect/subj_role/obj_role byte-identical), 0 causal_links diffs → no other board dim can regress.** **BOARD NOTE (19c LitBank, a no-regress check only — it CANNOT power a by-agent slice: its WDW gold is SYNTACTIC SUBJECTS, so the participle+byPP gate fires on ~11/8-doc events but changes only 2-3 SCORED answers): the live board who-did-what AGENT arm moves −0.0016 (16 docs) / −0.0023 (8 docs) — a TINY located micro-negative, all gate false-fires on 19c archaic constructions the live full-stack candidate set exposes (e.g. "purchased ... by HIMSELF" — a reflexive adverbial, not a passive agent — reachable because include_pron_agents keeps the pronoun; the shadow board lacked these).** Per no-more-default-off + flip-on-find-the-break: default-ON is justified by the +0.43 win on the powered modern instrument + agent-only additive-safety; the 19c gate-precision false-fires (reflexive "by oneself" + predicate_recall events) are a filed candidate NEXT problem on the RIGHT instrument, NOT a reason to hide default-off (the 19c board structurally cannot score a by-agent). Files: `hdlab/graded_role_assigner.py`, `hdlab/situation_reader.py`, `verification/test_byhead_agent_cue_landing.py`.

- **2026-09-06 (INTEGRATION — the LIVE pronoun coref pick was ANTI-brain-foundational and is now the PINNED graded ACT-R retrieval; a LIVE board-mover on the weakest dim. EXCELLENT, owner-DONE).** Problem `strengthen_the_cue_based_pronoun_coreference_resolver_the_shared_upstream_accuracy_cap`, reverified first-hand `verification/test_coref_graded_live_transfer.py` 5/5 (21/21 full suite) + a new landing witness `verification/test_coref_graded_pick_landing.py` **7/7 (re-run first-hand)**. **CORRECTS a live-organ fidelity record:** the deployed pronoun pick (`EventCentralityReader` rolemass topical mass = subject-role mention frequency with EXPLICITLY NO recency term, + an HD event-centrality override) scored live pooled he/she coref_acc **0.4693 — BELOW plain recency 0.6052**, i.e. it actively fought the brain's mechanism. Reference resolution is cue-based content-addressable RETRIEVAL from working memory with RECENCY the load-bearing decay term (Lewis & Vasishth 2005 ACT-R; McElree direct-access; Anderson base-level activation) — PINNED, and already built in `hdlab/graded_coref_pick.graded_antecedent_pick` but consumed only by `commonnoun_binder`, never the live pronoun path ("landed ≠ live"). **WIRE LANDED (Q111, DEFAULT-ON):** `EventCentralityReader` gains `graded_pick=True` routing the pool pick through `graded_antecedent_pick` (TUNED_WEIGHTS) + forcing event-centrality OFF (EC measurably HURTS: 0.4876 EC-off vs 0.4693 EC-on); `situation_reader.reader_ec` runs it; `graded_pick=False` restores the exact incumbent as a self-checkable fallback. Byte-faithful to the validated clean GRADED arm. **MEASURED first-hand: live pooled coref 0.4693→0.6019 (+0.1327 CI[+0.0929,+0.1738]); named coref no-regress 0.4883→0.6165 (RISES); the pronoun-bound who-has-what board dim 0.4035→0.4735 (+0.0700 CI-sep); info-free shuffled-history twin loses (0.2697).** ADDITIVE-SAFE: coref-INDEPENDENT dims (events/causal/timeline/entity_states) BYTE-IDENTICAL 12/12, goal HEADS byte-identical 12/12 (only the coref-dependent goal-AGENT attribution follows the improved coref — correct, not a leak). Cue ablation: RECENCY is the binding cue (recency-only 0.6052 ≈ graded 0.6019 >> incumbent 0.4693). 19c note: coref_acc is on LitBank (19c, informational for the board) but this is a register-general recency-mechanism FIDELITY correction, justified independently. DEFERRED (ledger): the +0.043 overlay-by-entity coref bonus (owner call); the entity-unification shared lever (filed p2); a MODERN pronoun coref corpus (the accuracy's proper home). Blast-radius: 13 historical cells construct `EventCentralityReader`; the 2 that must stay green (the transfer harness + this module's self-tests) are fixed to `graded_pick=False`; none are re-run by this landing.

- **2026-09-06 (INTEGRATION — the parser's fidelity gap is NOT UAS accuracy but GREEDY HARD-COMMITMENT with an UNCALIBRATED score; feeding it distributed meaning vectors to raise UAS is REFUTED + brain-corroborated; the brain-foundational deliverable is PRECISION-WEIGHTING a good-enough parse (Friston), DEMONSTRATED end-to-end. EXCELLENT located negative, owner-DONE; NO WIRE — the demonstration is in experiments/, the wire is the filed follow-on).** Problem `distributed_contextual_representations_into_the_parser_the_tractable_accuracy_fidelity_gap`, reverified first-hand `verification/test_typed_selpref_ppattach_negative.py` **10/10**. **REFUTES the brief (its sanctioned full-PASS form), decisively + brain-corroborated:** a whitened, (head-POS,preposition)-TYPED, object-conditioned distributed selectional-preference feature over the curated `meaning_foundation` vectors (Pado/Resnik class-level, exactly the brief's mechanism) scores PP-attachment 0.5743 on UD-EWT test (n=1104) — BELOW the arc-eager parser's own 0.7763, below locality 0.6540, below lexical 0.6603 — and is ANTI-complementary (0.4737, BELOW chance, on the 247 cases the parser gets WRONG); making it predicate-specific makes it WORSE (0.4170); wired as a confidence-gated re-attachment it CI-sep HURTS UAS (−0.0010). A literature drill confirms object-class selpref has NO human-isolation evidence for PP-attachment — the refutation is brain-faithful, not a build failure. **THE REFRAME (triply-corroborated — our own disk [a better parser moved who-did-what ~+0.00], psycholinguistics [Ferreira good-enough; Sachs 1967], NLP extrinsic-eval [UAS gains don't transfer]): UAS is NOT the brain's objective.** The brain builds a good-enough parse (arc-eager stays PINNED) and PRECISION-WEIGHTS it (Friston active inference: reliable cue drives belief, unreliable is down-weighted). **PINNED CORRECTION: the parser's real deviation is GREEDY hard-commitment with an UNCALIBRATED score** vs graded lexicalist constraint-satisfaction (MacDonald 1994) with a calibrated competition margin (Bates-MacWhinney cue validity; Hale/Levy surprisal = the distribution over parses). **DEMONSTRATED end-to-end (the brain-foundational deliverable, a can-fail result): the parser already emits a per-arc confidence CONSUMED BY NOBODY**; a calibrated version separates right-from-wrong arcs (AUC 0.768 graded / ~0.81 arc-factored-global object attachment vs the greedy softmax 0.645), and precision-weighting the readout gives selective attachment 0.826 @50%/0.955 @10% vs 0.776 blanket, and selective WHO-DID-WHAT comprehension 0.871 @confident-half vs 0.780 blanket (+0.0907 CI[+0.0676,+0.1154]), random-confidence twins FLAT, on clean UD-EWT (n=1255). ADDITIVE (parse heads unchanged → zero downstream regression). Post-hoc RE-ATTACHMENT is the WRONG architecture (Frazier vs MacDonald; CI-sep hurts UAS) — precision-WEIGHT the readout, do NOT edit the tree. **NO hdlab WIRE (a located negative proposes none; the demonstration lives in experiments/).** **FOLLOW-ON FILED (p4 `precision_weight_the_head_driven_readers_on_calibrated_parse_confidence`, strategy to own the eventual wire): land the additive calibrated confidence so who-did-what/obl/space DEFER on low-confidence arcs — this is the RELIABILITY SUBSTRATE the reasoning phase stands on (an inference engine must know which facts to trust).** DO NOT (all shown wrong here): chase UAS, build a graded-competition parser FOR ACCURACY, re-attach, or relax the no-trained-encoder invariant. The definitive north-star lever is an INTRINSICALLY-graded-competition parser (its key component — the calibrated confidence — is prototyped and works), but the deliverable is realized WITHOUT it via precision-weighting.

- **2026-09-06 (INTEGRATION — the SPACE where_is cap is NAMED-GROUND BINDING (Talmy Figure/Ground), NOT change-point recall; the brief's lazy locative-PP bridge is a LOCATED NEGATIVE. EXCELLENT PARTIAL, owner-DONE).** Problem `space_where_is_is_extraction_recall_bound_add_lazy_locative_pp_bridging`, reverified first-hand `verification/test_space_ground_binding.py` **5/5**. **REFUTES the brief's premise (disk outranks brief):** recovering motion-event extraction recall 0.444→0.889 moves end where_is only +0.064 (modern, n=47, NOT CI-sep) — detecting THAT a character moved was never the bottleneck. **The real lever (ceiling-control localized): binding the correct NAMED GROUND to an already-detected motion event** — error decomposition vs a perfect-extraction ceiling (0.787) shows ~49% of where_is errors are named-ground binding on ALREADY-detected motion (SCENE 34% `<scene>` not "office"; WRONG_NODE 13%). **NEW MECHANISM (LANDED, Q111, DEFAULT-ON in the live `prior_ext` mode, promoted VERBATIM into `experiments/_space_reader.py` — the live space module the reader composes): a brain-foundational Ground extractor** — a verb-frame Goal gate (VerbNet motion class; a manner-of-motion verb is a single-argument predicate, the path PP an adjunct — Rappaport Hovav & Levin), Figure-Ground compound-noun HEAD selection ("meeting room"→room), a closed-class PARTITIVE resolver ("back of the hall"→hall; Landau & Jackendoff axial terms), and GRADED functional-locus typing from SEMANTIC MEMORY (ConceptNet AtLocation frequency: desk=216/plane=46 vs carriage=0). **FIDELITY FINDING: the functional-locus typing must be GRADED — a curated list is the wrong shape (CLAUDE.md) AND a hard WordNet furniture/vehicle hyponymy test OVER-GENERATES on real prose (types incidental "carriage"/"cart" as loci → regressed 19c); the graded ConceptNet AtLocation signal recovers the accuracy at stronger twin separation and is the faithful choice.** **MEASURED (MODERN gold — the load-bearing corpus per the owner's no-19c directive): where_is 0.319→0.468 (+0.149); beats the last-mention floor CI-sep + the shuffled-ground info-free twin LOSES CI-sep (0.468 vs 0.213); precision IMPROVES 0.571→0.702; through the LIVE `SituationReader.read()` 0.277→0.447 (+0.170, landing witness W4 on/off).** ADDITIVE-SAFE (W5): emits ONLY SPACE events into the LocationRegister (`extract_events_in_substrate` is called only by `_read_space`) → who-did-what byte-identical. **DO NOT LAND (located negatives): the aggressive locative/stative + protagonist-fallback binder (regresses real prose −0.05..−0.08) and the ANTICIPATORY Goal fill (over-fires, modern −0.021 — AGREES with Ferretti/McRae & Hatherell 2001: verbs prime Agents/Instruments but NOT typical Locations, so an eager location-fill is not brain-faithful).** **AUDIT CORRECTION:** the SPACE front-end's Ground/goal SELECTION (~0.73 on the space gold) was UNDER-BUILT (no compound-head/partitive/functional-locus typing; benefactive 'for' wrongly typed as spatial). The shared role router's Ground selection is the single biggest UPSTREAM lever — a perfect ground node on already-fired events is worth +0.107 where_is (0.468→0.575), as much as the whole downstream binder. **MEASURED walls to the 0.787 ceiling (decomposed, not asserted — the earlier "residual is parser attachment" was WRONG): 50% register REPRESENTATION (start/`<away>` states the readout can't name), 19% discourse-level Ground (Ground in a separate clause — the genuinely-hard anticipatory case), 19% self-motion label, 12% rare-facility typing.** **CAVEAT (owner-graded, honest): the +0.149 modern does NOT CI-separate over the already-decent 0.319 current chain at n=47 — the only well-powered corpus (n=606) is the 19c one, BANNED per the owner's no-19c directive → this is a genuine measurement-POWER gap the modern-gold expansion must close; the robust claims are the floor/twin separations + precision + the +0.170 live gain + additive-safety.** **DEFERRED (INTEGRATION LEDGER follow-ons):** (1) fold the Ground-selection typing into the shared `hdlab/predicate_argument_frontend` so the who-did-what + copular consumers get the better place typing (coordinate with the active pred-arg solver p3); (2) the Ground-aware goal-PP ATTACHMENT problem (the residual is partly raw arc-eager mis-attachment — the last lever to the ceiling); (3) a MODERN where_is board arm (the board's coarse `location` dim sits at ceiling 1.0 and does not score exact-node where_is → this +0.170 live gain is board-INVISIBLE — the recurring live≠scored pattern).

- **2026-09-05 (INTEGRATION — the entity-world-model resolver LANDS (full brain-foundational chain) + RUNS live, but is DEFAULT-OFF with a MEASURED reason: it improves COMMON-NOUN clustering, which the board's PRONOUN coref dim does not score. EXCELLENT mechanism, owner-DONE; a board-VISIBILITY located negative on the current instrument).** Problem `seed_the_entity_world_model_resolver_with_a_world_knowledge_role_kinship_prior_phase1`, reverified first-hand `test_entitykb_resolver_v2.py` **6/6** (hard-link 0.2466→0.4139 +0.167 CI-sep; aggregate common-noun CoNLL +0.0882 CI-sep; named no-regress). **WIRE LANDED (Q111):** `hdlab/entity_world_model_resolver.py` (imports the exact reverified `exp_entitykb_resolver_v2.resolve` — byte-faithful; the curated role/kinship/scenario KB is inline glass-box) + `resolve_common_noun` swaps into `_apply_commonnoun_gate` behind `entity_kb_resolver` (DEFAULT-OFF). **LIVE PROBE (the decisive board-visibility measurement): the resolver RUNS on the reader's own `role_mentions` (format-compatible) and MERGES common-noun entities more (sm.entities 451→369), but `coref_acc` is IDENTICAL off-vs-on (0.3846) — the board `coref` dim scores PRONOUN coreference (he/she→antecedent), while this improves COMMON-NOUN clustering (the master=the squire), so the +0.0882 harness win DOES NOT reach the board coref dim.** KEPT DEFAULT-OFF with the measured reason (no direct board move; the payoff is INDIRECT via the affect/goal experiencers that bind through common-noun entities — an unmeasured indirect lever). **PINNED (board-instrument gap): the coref dimension conflates two brain systems — pronoun anaphora (cue-based reactivation) and common-noun discourse-referent clustering (Sanford-Garrod scenario binding); the board scores only the former. The entity-KB's realization needs (a) a COMMON-NOUN coref board arm + (b) the Step-3 reader-coref lever via a two-pass (the single-pass here runs `reader_coref=None`).** The KB seed alone is a located negative (~2%); the chain crosses on its own instrument. **UPDATE (same session): the win is now BOARD-REALIZED — added a COMMON-NOUN COREF board arm (`exp_board_common_noun_coref_v1`, folded into `run()`) that scores the full-chain resolver's character-cluster CoNLL vs surface-head + the shuffled-KB twin: model 0.6889 vs 0.5965 = +0.0924 CI-sep, twin loses +0.016 CI-sep. So the +0.0882 was never a real located negative — it just needed its OWN board arm (the coref dim conflates pronoun + common-noun coref; they are separate brain systems and separate arms, like patient/wic).** Remaining: the LIVE reader flag `entity_kb_resolver` (single-pass, reader_coref=None) stays default-off — the arm realizes the value via the cached two-pass reader_coref; making the LIVE read deliver it needs the two-pass wired into read() (the follow-on).

- **2026-09-05 (INTEGRATION — GOAL tier: the marker-less means-end attachment is CONTEXTUAL inverse planning (situation-relatedness), NOT a context-free verb→goal table (refuted) and NOT recency (a located negative); + an upstream advcl/xcomp PURPOSE FILTER on the goal register. EXCELLENT, owner-DONE).** Problem `validate_the_ppmi_svd_means_end_bridge_on_real_narrative_marker_less_goal_attachment`, reverified first-hand `test_contextual_goal_attachment_modern.py` 4/4 (+5/5+9/9). **NEW PINNED:** goal attachment is INVERSE PLANNING conditioned on STATE (Baker/Jara-Ettinger) — the signal is the SITUATION's distributional relatedness to the goal, in the ATL associative hub; the brief's context-free ATOMIC means-end bridge is a LOCATED NEGATIVE (sits inside the info-free shuffled-situation twin band); the recency `link_open_stack` is also a located negative. The CONTEXTUAL mechanism beats the twin CI-sep on MODERN gold (K1 0.700 vs twin p95 0.483, n=797; 19c 0.634 vs 0.537). **TWO WIRES LANDED (Q111, DEFAULT-ON, gated `goal_purpose_filter`):** (1) the ADVCL PURPOSE FILTER in `hdlab/goal_register.extract_goals` — a bare 'to VP' is kept only if the infinitive's arc-labeler deprel is a purpose adjunct (advcl); a confirmed complement (xcomp/ccomp/acl) is rejected (upstream net-positive on why(): removes 131 wrong vs 24 genuine, 5.5:1), consuming the reader's OWN arc labeler over the consolidated arc-eager parse; (2) the CONTEXTUAL `_link_open_stack` edge in `hdlab/goal_hierarchy_graph` — a marker-less action attaches to the OPEN goal most related to the action's SITUATION (top-k mean cosine in the associative store, margin-gated), replacing recency; flipped `link_open_stack=True` in `_read_goals` with the reader's own sentence tokens as the situation. ADDITIVE (only fills previously-parentless action nodes; the flat register + why()/wants() unchanged). Board note: the purpose-filter is the board-relevant lever (the goal-why arm reads reg.why); the contextual edge enriches the goal GRAPH (marker-less real-narrative attachment) which the current board under-scores (the goal_hierarchy arm's authored battery is explicit-goal) — its K1 0.700 needs a mined marker-less gold (a filed instrument gap). Follow-on named: the CONTEXTUAL inverse-planning engine unifies with the belief timeline (shared mentalizing engine).

- **2026-09-05 (INTEGRATION — the reader parsed every sentence TWICE (arc-factored batch for the copular/space front-end + arc-eager for roles); CONSOLIDATED onto ONE arc-eager incremental parse for ALL consumers. Board ZERO-regression, ~5% read-cost cut. The arc-EAGER incremental parse is the PINNED brain-foundational parse; the arc-FACTORED batch parser has no cognitive correlate and is retired from the read path. EXCELLENT, owner-DONE).** Problem `consolidate_the_arceager_and_arc_double_parse_the_reader_now_parses_every_sentence_twice`, reverified first-hand `test_double_parse_consolidation.py` — the SUBSTANTIVE checks PASS: **W3 the 6 non-front-end consumed dims are BYTE-IDENTICAL** default==consolidated (events[agent+patient], causal_links, coref_acc/resolutions, suppressed, timeline_order); W5 copular live-consumer `fix_recall` NO-REGRESS (modern 1.000, archaic/19c 0.700 — arc-eager == batch); W6 space `where_is` NO-REGRESS (Δ −0.015 CI[−0.049,+0.000], includes 0, n=606); W8 a full default-on read emits ZERO batch parses / one arc-eager parse per sentence end-to-end. (W1/W4/W7 are premise-STALE post-landing — they assert the pre-landing DOUBLE parse still exists [W1 "default parses both", W4 "default differs from consolidated", W7 "batch cost to eliminate"]; the landing correctly REMOVED it, so those checks invert by design — the landing is validated by the substantive checks + the board.) **WIRE LANDED (Q111, DEFAULT-ON):** `hdlab/situation_reader._cached_parse_heads` now computes the ONE shared per-read parse via arc-eager `parse_with_conf` when `parser_arceager` (the default), serving BOTH the role path AND the front-end; `_router_roles` reads from that shared cache instead of a separate `_ae_parse` call. The base `ArcParser` (`_frontend_parser`) is never called on the read path when the flag is on (kept loadable as a self-checkable byte-identity reference). ROLE heads are byte-identical (same `arceager_parser.parse_with_conf`+weights); only the front-end switches batch→incremental (the measured no-regress). **CORRECTS the parser fidelity record: the arc-EAGER incremental parser (left-to-right shift/reduce, stack+buffer, working-memory decode — Lewis-Vasishth 2005) is PINNED brain-foundational; the arc-FACTORED batch parser (global O(n²) all-pairs + Chu-Liu/Edmonds) has NO cognitive correlate — retire from the read path (it stays a byte-identity reference only). Also correct the stale "arc-eager copular is 19c-negative" claim — REFUTED post-`robust_cop` (archaic fix_recall identical 0.700).** Read-cost: the whole batch parse eliminated (~1.0s ≈ 4.6% of a warm read; the batch parse was SLOWER than the arc-eager one that replaces it). LOCATED NEGATIVES settled here (do NOT re-open): confidence-weighting is a WEAK end-to-end lever (roles margin AUC 0.538 deployed / copular neutral); the predictive arc-eager is a located negative on attachment accuracy (real only on anticipation MRR — an N400/time signal, home = `predict_surprisal`); beam/reanalysis/register-retrain/a stronger general parser remain located negatives. Follow-on FILED: `space_where_is...lazy_locative_pp_bridging` (space recall 0.444→0.889 prototyped — the largest reader-accuracy lever this surfaced, REUSE-heavy).

- **2026-09-05 (INTEGRATION — the arc labeler's per-arc scoring is now a byte-identical fast path (~9x labeler / ~54% of a long-doc read), and the SAME refactor materializes the graded Competition-Model posterior the brain maintains — argmax byte-identical, a gold-free error signal AUC 0.930. EXCELLENT, owner-DONE).** Problem `add_the_arc_labeler_fast_scoring_path_the_dominant_remaining_read_cost`, reverified first-hand `test_arc_labeler_fastpath_landing.py` **5/5** (byte-identity THEOREM — 0/36 labels contain the `~` separator so the rsplit is EXACT → byte-identity UNCONDITIONAL in the weights, not a sample; + 22,921 held-out arcs 0 mismatch across two disjoint populations [4,575 LitBank predicted-heads + 18,346 UD-EWT-test gold-heads]; + a full-SituationModel regression guard byte-identical across every dimension on 3 held-out docs) + a new pure-hdlab landing witness `test_arc_labeler_fastpath_hdlab_landing.py` **4/4** (the LANDED `ArcLabeler.label()` == the retained reference `_predict_label` on the same 22,921 arcs; `label_graded` argmax == `label` byte-safe). **WIRE LANDED (Q111, DEFAULT-ON, byte-identical):** promoted `_FastLabelPlan` VERBATIM into `hdlab/arc_labeler.py` (precompute `feat → [(label_idx,weight)]` via a LAST-`~` rsplit; per-arc accumulate present weights into per-label lanes in FEATURE order + argmax in LABEL order → bit-identical float sum + identical first-max tie-break); `ArcLabeler._ensure_fast()` (lazy, mirrors `PosTagger._ensure_fast`); `label()` routes through it; `_predict_label`/`_score` KEPT UNCHANGED as the training path AND the self-checkable reference. **NEW PINNED (the fidelity content, not just speed): the labeler computes the PINNED brain operation for grammatical-relation assignment** — linear cue integration `lane[k] = Σ w(feature, label_k)` IS `graded_competition.net_activation` (Competition Model / FLMP; McClelland 2013: softmax over additive net = the Bayesian posterior; the perceptron feature-weights ARE learned cue validities). What was NOT brain-faithful is the HARD ARGMAX readout (the brain maintains the graded distribution and collapses only under task pressure — Hale 2001/Levy 2008 distribution-over-parses; Kiani-Shadlen 2009; the loser persists, Christianson 2001). The fast refactor MATERIALIZES `lane[]`, so the graded readout comes for FREE — landed the byte-safe opt-in `label_graded()` (default-off, no consumer wired): argmax byte-identical (MAP-optimality → a strict SUPERSET, regresses nothing) + the normalized entropy is a VALIDATED gold-free labeling-error signal (AUC 0.930 vs info-free shuffled-validity twin 0.481). **The front-end scoring chain is now ONE brain operation with three byte-identical fast paths that each materialize their per-candidate activation** (tagger `_FastEmissionPlan`, parser `FeatCache`/vectorized gather, labeler `_FastLabelPlan`) → `graded_competition` composes with all three. **SIGNAL-LOSS ACCOUNTING over the whole front-end (oracle-substitution ladder, UD-EWT test): the LABELER is the STRONGEST link (5.8% intrinsic loss, the most brain-faithful of the three); the DOMINANT leak is UPSTREAM the PARSER (loses ~21% of heads even with gold tags; its head-errors cost the labeler 3× the tagger's) — the catastrophic losses are the PASSIVE role markers `obl:agent` LAS 0.0588 / `nsubj:pass` 0.402 (the parser never attaches the passive by-agent → the who-did-what/non-canonical wall).** REMAINING labeler fidelity gap (deeper, NOT closed here, owned by the Q113 who-did-what/incremental problems): per-arc INDEPENDENT labeling — no shared-head/one-role-per-clause joint decode (Theta-Criterion; LFG Uniqueness; McClelland-Kawamoto 1986 whole-clause settling). **ESCALATION (owner-directed, EXPLORATORY — recorded in `notes/PARSER_JOINT_INTEGRATION_RUBRIC.md`, NOT landed): (F) arc-eager is the better default but NOT ideal/dominant — wins long-range (+0.15) but LOSES `obl:agent` −0.147/`det` −0.019/`appos` −0.035; oracle-union 0.854; a confidence-gated hybrid is a LOCATED NEGATIVE (no rule beats arc-eager). (G-O) supervised parsers are gold-tree-trained (not brain-faithful acquisition); a self-supervised DMV+universal-prior+EM + an online never-frozen predictive-coding learner INDUCE real structure (0.38-0.45) but far below supervised 0.84 — the gap is GROUNDED-EXPERIENCE SCALE + the missing answer key (grounded meaning as propose-but-verify), NOT the mechanism; meaning's brain-foundational locus is ROLE on non-canonical clauses (grounded 0.43 vs positional 0.00 where syntax misleads), NOT the attachment skeleton (grounded ties its scrambled control on UAS, 4× confirmed). (O) the grounding-negative ROOT CAUSE = near-collinear meaning vectors (cosine 0.92); FIX = whiten → the parser-grounding control FLIPS (+0.020 on hard arcs) and independently lifts the LIVE WSD meaning channel a_s +0.0176-0.058 (UNVERIFIED — needs twin/significance/curated-baseline).** Two follow-ons filed: whiten-the-meaning-channel-embedding (nearest testable win, meaning tier) + the self-supervised+grounded front-end parser (north-star; feeds the parser rubric).

- **2026-09-05 (INTEGRATION — causal cause-SELECTION is TWO subtasks with TWO brain systems; the brief's "retire scoping via a plausibility selector" is a LOCATED NEGATIVE (connective selection is STRUCTURAL, scoping is its OPTIMUM); the landable value is a NEW upstream event-TYPE representation + a MENTAL-causation bridge that crosses the wall force dynamics structurally cannot. EXCELLENT PARTIAL, owner-DONE).** Problem `a_force_dynamic_meaning_hub_causal_scorer_retire_the_connective_scoping_workaround`, reverified first-hand `verification/test_causal_selection_plausibility_and_scoping.py` **19/19**. **CORRECTS the causal-SELECTION verdict:** connective-marked causation ("X because Y") is a STRUCTURAL discourse-syntactic bind, NOT a plausibility competition — a force+agentivity plausibility selector is CI-sep WORSE (off_plaus 0.6931 vs positional 0.9010, d=−0.2079 CI[−0.2970,−0.1268], n=101), and the connective causal QA gold IS the positional rule (agree(gold,positional)=0.9010 == the OFF QA score exactly; a PERFECT-parse oracle 0.7624 and oracle-participants 0.8218 BOTH score worse than positional → the refutation is MECHANISM-AGNOSTIC). So **scoping is the connective-path CEILING, not interim debt — do NOT retire it** (the prior §2b "faithful fix = force-dynamic attribution" was WRONG for the connective subtask). **NEW PINNED-AND-MEASURED: most narrative causation is MENTAL** ("she saw the letter, then wept") — a DISTINCT brain system (Jack et al. 2013 opposing-domains reciprocal inhibition; Fischer 2016 intuitive-physics vs Saxe & Kanwisher 2003 rTPJ mentalizing; Campanella 2022 triple dissociation) that force dynamics structurally cannot represent (force lexicon covers 3/16 real LitBank cause verbs; 11/16 are perception/cognition/emotion/communication). **NEW COMPONENT (LANDED, Q111): an event-TYPE representation** = WordNet verb SUPERSENSE → {PHYSICAL,PERCEPTION,COGNITION,EMOTION,COMMUNICATION,BODY,SOCIAL,STATIVE} (`hdlab/event_type.py`, promoted VERBATIM; consolidates the idiom_lexicon/causation_typing WordNet-supersense pattern into a first-class organ; MFS is the ATL frequency default). Neural grounding is NETWORK-level only (physical-vs-mental), class-level is behavioral IC (Ferstl 2011; Hartshorne & Snedeker 2013) — do NOT claim VerbNet-class-level neural grounding. **NEW RESULT (LANDED, Q111, DEFAULT-ON pure-add): a UNIFIED bridging selector crosses the mental wall** — a folk-psych episode-schema bridge (Rumelhart/Stein&Glenn story grammar; Malle BDI; OCC appraisal) appended in `_read_causation` on NON-connective sentences (behind `causal_mental_bridge`): a MENTAL/expressive OUTCOME is explained by the nearest prior MENTAL TRIGGER. On constructed non-adjacent mixed bridges UNIFIED 1.000 EXCEEDS force-only 0.500 (+0.500 CI[+0.250,+0.750]), beats MOST_RECENT/CONNECTIVE_ONLY CI-sep, shuffled-event-type twin loses; the faithful "resonance proposes, necessity disposes" selector (Myers&O'Brien + Trabasso counterfactual-necessity + experiencer gate) beats recency-only +0.875 CI-sep and the type-shuffle null +0.493 CI-sep (necessity load-bearing; OCC appraisal measured INERT on neutral-valence triggers — an honest bound). **PURE ADD verified (landing witness `test_causal_mental_bridge_landing.py` 9/9):** connective causal QA + goal WANT/WHY QA + events + coref BYTE-IDENTICAL off-vs-on on 12 board docs (the mental path fires only on non-connective sentences; the causal QA gold is connective-only; the goal QA scorers read the register, not causal_links), the goal graph stays a strict SUPERSET (connective links emitted first — the measured landing requirement), +214 mental_bridge links added. **The binding wall for real-narrative causal comprehension is now the UPSTREAM meaning-hub, QUANTIFIED (signal-loss ladder over the whole chain): experiencer/COREF preserved 0.500 and contextual WSD 0.875 on real prose — NOT the selection mechanism (oracle-candset ceiling 1.000).** The causal QA instrument is CIRCULAR for plausibility (future plausibility claims must use a non-circular gold — the bridging dissociation / RC.GOLD). **DEFERRED (recorded in the INTEGRATION LEDGER, folded into the top-down integration pass — the FIELD accuracy + downstream-signal work the mechanism unlocks):** (1) route event-typing through the LANDED `GroundedSemanticGraph` WSD organ (type_ok 0.688→0.750, routed chain 0.875→0.938 — the cheap Lesk WSD was tried and REJECTED); (2) wire the selector's experiencer through `psych_verb_frames` + the reader's live coref (removes the out-of-reader 0.500 heuristic ceiling); (3) mine a real-corpus non-adjacent MENTAL-bridge gold (the missing scored instrument — no live mental-causal metric exists yet); (4) revisit 3 consumers to RECEIVE the event-type signal — `causation_typing` → mental-causation typing (3/16→16/16), `affect_register` → its OCC-appraisal inferred-emotion channel, the goal graph → the motivational spine (already additive, +47 enablement edges). Exemplary disk-outranks-brief (refuted the brief's own MEASURED premise + built the real mechanism where it belongs) + a per-component 11-stage brain-fidelity audit (every load-bearing stage PINNED or a named invention-under-test with its faithful upgrade filed).

- **2026-09-05 (INTEGRATION — the CURATED KNOWLEDGE FOUNDATION, built + FROZEN + wired: a static offline sense-signature store (117,614 synsets) delivering +0.0755 CI-sep through the live diagnostic_context_wsd — the broad-coverage sense-discriminative KEYS the P2/P9 located-negatives pointed to. The knowledge north-star's FOUNDATION half. EXCELLENT, owner-DONE).** Problem `build_and_freeze_the_clean_curated_knowledge_foundation_the_proven_meaning_lift`, reverified first-hand `test_knowledge_factory_meaning_store.py` **6/6** (frozen 0.3267 vs gloss 0.2512 = +0.0755 CI-sep [0.0557,0.0957]; shuffled-knowledge info-free twin LOSES +0.1443; NO MFS regression 0.6890≥0.6834; freeze→reload byte-exact; deterministic byte-identical across PYTHONHASHSEED after the sorted-then-capped fix) + a new pure-hdlab landing witness `test_meaning_foundation_landing.py` (the loader is byte-identical to the on-disk store on 801 sampled synsets). **WIRE LANDED (Q111): `hdlab/meaning_foundation.py`** — the loader for the frozen `data/frontend_assets/meaning_sense_signatures_v1.npz` (44MB float16; on-disk per the project's gitignored-`data/` convention — every frontend asset is on-disk, none in git), exposing `sense_signature`/`sense_signatures` as the DEFAULT candidate-sense source for the meaning channel (a ZERO row per absent sense, matching diagnostic_context_wsd's missing-gloss convention). **NEW PINNED: the knowledge foundation is a curated STATIC OFFLINE asset (admissible — NO LLM / NO transformer / NO training, owner 2026-08-16); the meaning channel's KEYS are now broad-coverage + sense-discriminative (the P2 W-quality×coverage lever), realizing the proven +0.067. keep-all mean-pooling is the MEASURED optimum — 3 trims (schema-margin / sibling-confusion / incoherence) ALL fail to beat keep-all → trimming curated knowledge only removes coverage.** **LATENT (honest): NOTHING calls `diagnostic_context_wsd` at read() time yet (`semantic_control` operates on scores, `grounded_semantic_graph` on the graph — neither consumes C1 signatures), so the +0.0755 is proven on the standard meaning INSTRUMENT but felt by no live board dimension until the `reader_meaning_channel` stage lands; the frozen store is READY the moment it does, and `consolidation_gate` stays the mandatory admission guard for any online-grown knowledge (the propose-and-verify grow loop, filed separately).** **DEFERRED (validated, ready): the `composed_hub_predictor` AvgSim→MaxSim usage tweak (+0.065 CI-sep, witness 3/3) — a non-trivial `score_pool` restructure on a LATENT organ (not reader-wired), to land when it is wired.** The ~0.35 fine ceiling is now diagnosed UNSUPERVISED (sibling-sense cosine 0.934, effective rank 17/200 — an anisotropic superposition cone); a post-hoc isotropy fix (all-but-the-top) HURTS the biased-competition readout → the residual is a DATA-coverage gap, consistent with the rare-sense located-negative (§ the FROZEN INPUT boundary).

- **2026-09-05 (INTEGRATION — the arc parser's POS-only joint features are now VECTORIZED (numpy gather over a precomputed closed-tagset integer table), BYTE-IDENTICAL, 2.0–2.6× over the landed P8 fast path on long sentences. EXCELLENT, owner-DONE).** Problem `numpy_vectorize_the_arc_parser_pos_only_joint_features_p8_named_lever`, reverified first-hand `test_arc_parser_posfeat_vectorize.py` **6/6** + a new pure-hdlab landing witness `test_arc_parser_posfeat_landing.py` (the LANDED `ArcParser.parse` is BIT-IDENTICAL to `_parse_reference` on **393,225 held-out arcs / 376 sentences** — heads + arcs + per-token margins, `==`; the vectorized branch fires on 235/376; 7.6× over stock scalar on a warm slice). **WIRE LANDED (Q111, DEFAULT-ON, byte-identical):** promoted `_bucket_idx`/`PosTables`/`pos_tables`/`_word_cols_tables`/`_word_cols_pyloop`/`sentence_flat_vec`/`sentence_scores_vec`/`sentence_scores_auto` (GATE_THRESH=16) into `hdlab/arc_parser.py` (lazy `PosTables` built from the wired tagger's 17-UPOS inventory + specials ROOT/<S>/<E>, FAIL-LOUD on an unknown tag → auto-adapts to a retrained tagger); `ArcParser.parse`/`eval_uas` route through the length-gated `sentence_scores_auto` (vectorized n≥16, the untouched scalar fast path below AND as the byte-identity reference). The scalar `_arc_ids`/`sentence_flat`/`sentence_scores`/`_parse_reference` are UNCHANGED. **The lever (KEY realization): byte-identity forbids SPLITTING the float sum, NOT a faster CONSTRUCTION of the same input — rebuild the IDENTICAL flat integer-id array with numpy scatter/gather instead of a Python genexpr, then the same `np.add.reduceat`. Verified at the integer-id-STREAM level (exact, stronger than score-level) + on raw-float64 weights (parser-retraining-safe, not a float32 fluke).** Whole-read wall-clock within noise (the parser is now ~7% of a warm read after the prior P8 + tagger speedups); output byte-identical → **NO §2b fidelity change (compounding read-cost reduction, not a model change).** Repointed the reverify's W4 timing baseline to the scalar reference (post-land, `ArcParser.parse` IS the fast path, so the old baseline was circular). **ADJACENT (named follow-on, same technique): the POS-tagger inner-loop Viterbi is the co-dominant warm-read cost — the closed-inventory table+gather transfers there (byte-identical, a distinct code path).**

- **2026-09-05 (INTEGRATION — named LEAN INGEST PROFILES landed + a validated process-parallel corpus-read harness: read a corpus FOR A PURPOSE (byte-identical on the kept dims, faster) and read it in parallel (byte-identical per-doc). EXCELLENT, owner-DONE).** Problem `lean_ingest_profile_and_parallel_corpus_read_harness_for_scale`, reverified first-hand `test_lean_ingest_and_parallel_harness.py` **36/36** + a new pure-hdlab landing witness `test_ingest_profiles_landing.py` (all 12 profiles byte-faithful to the validated builder; `selpref` == `full` on the harvest core, events 346 / entities 451). **WIRE LANDED (Q111): `hdlab/ingest_profiles.py`** — named presets `full` / `selpref` (default minus the 9 additive dims) / `lean_floor` (= `all_capabilities_off`) / `<dim>_kept`, promoted VERBATIM (reuses the existing capability-flag plumbing → byte-identical to the full read on the kept dims by construction). **NEW PINNED: the brain reads FOR A PURPOSE — the who-did-what harvest core {events, entities, coref, timeline, causal} is the obligatory parse + role binding (~44% of a read, irreducible); the 9 additive dimensions are ADDITIVE elaborations that each write ONLY their own field + read the FINAL event set (none feeds back into sm.events — proven per-dim byte-identical: selpref == full on the core, and each `<dim>_kept` == full on that dim). So the honest speedup is PER-PROFILE (roles-keeping `selpref` ~2x; role-free `lean_floor` ~9x — the parse is the floor), NOT one flat number (the brief's "5x" corrected).** The process-parallel read harness is VALIDATED + usable (`experiments/exp_parallel_corpus_read_harness_v1.parallel_read`): per-doc output BYTE-IDENTICAL serial-vs-parallel across 1/2/4/8 workers, ~0.87→2.69 docs/s (68% of the box's 12-core pure-CPU ceiling — the sub-linearity is hardware P/E-cores + hyperthreading, not the design). **LOCATED determinism controls (a full pass): (a) `sm.entities` LIST ORDER is PYTHONHASHSEED-dependent (set-derived mention stream), canonicalized order-invariant at the serialization boundary; (b) under CPU OVERSUBSCRIPTION the per-process-trained metadata organs (grounded-valence affect perceptron / frame-role induction, numpy/torch reductions) rarely flip a borderline affect None↔NA or subj_role label across processes — NOT hash, NOT the tagger, NOT the core fields; byte-reproducible in the INTENDED regime (workers ≤ physical cores, threads=1, not oversubscribed), verified ~240 controlled cross-process reads.** DEFERRED (optional, verdict-independent, low-value): a source-side determinism fix in `hdlab/referent_per_np.py` (the harness already canonicalizes at serialization, and the fix would change global entity order for no correctness gain the harness lacks) + a polished `tools/parallel_corpus_read.py` CLI (the harness is usable as-is).

- **2026-09-05 (INTEGRATION — the affect path no longer calls a SECOND, off-the-shelf NLTK tagger: rerouted through the shared hdlab UD-EWT frontend tagger + skips the discarded torch valence. Byte-identical valenced output, ~0.37s/read faster. EXCELLENT, owner-DONE).** Problem `route_the_redundant_nltk_perceptron_tagger_through_the_fast_hdlab_tagger`, reverified first-hand `test_affect_reroute_hdlab_tagger.py` 6/6 (pre-land byte-identity vs NLTK) + a new pure-hdlab landing witness `test_affect_reroute_landing.py` (VALENCED byte-identity vs the NLTK reference **0 flips / 1124**; NLTK-free read; skip-valence byte-identical ternary). **WIRE LANDED (Q111):** `hdlab/situation_reader._assign_affect` tags via the shared `_load_frontend` UD-EWT tagger (ONE category system — the SAME model the events/roles/entity-state path uses) instead of the second NLTK averaged-perceptron tagger; `hdlab/context_grounded_valence.score_item` gains `need_valence` (default True = byte-identical) so the affect wire skips the two discarded torch theta-matmuls per event (`_assign_affect` reads only predicted_type + stage via to_ternary). ~0.37s/read off the affect path (NLTK 195→0 calls/read; skip-valence ~0.12s). **NEW PINNED: one lexical-category system, not two — the reader's UD-EWT structured perceptron IS the brain's single category inference; the NLTK tagger buried in the affect path was a second, off-the-shelf system computing the affect field from a DIFFERENT POS analysis than the event's own agent/patient. The fix removes that inconsistency; the tagger MODEL is unchanged (PINNED), only the routing (the "reach for the convenient tool" anti-pattern this project is built to avoid).** **LOCATED-NEGATIVE sub-finding (a full pass): strict `EventRecord.affect` FIELD byte-identity is NOT achievable and should NOT be — the hdlab UD-EWT and NLTK perceptron are different statistical models disagreeing on ~9% of governor/patient tags, flipping the INERT None↔NA firing-provenance bit (0 production consumers branch on it, enumerated repo-wide; hdlab tags often MORE correct). The load-bearing VALENCED output (HARM/HELP + feel-category) is byte-identical (0 flips / 8947).** **FOLLOW-ON FILED: the arc-labeler fast-path** — the solver localized the single biggest remaining read-time lever (~10× the affect tagger): `hdlab/arc_labeler._predict_label` runs default-on via `bind_entity_states` and does ~900 string-concat dict lookups per arc (~75k `_score` calls/read); the analogous `_FastLabelPlan` (the byte-identical pattern already landed for the POS tagger + arc parser) is built + witnessed, ready to land.

- **2026-09-05 (INTEGRATION — a GOAL→SUBGOAL HIERARCHY GRAPH: the reader now answers plot-structure questions (multi-hop goal-why chain, superordinate reinstatement over distance, connectivity salience) the FLAT goal register structurally cannot. A PURE ADD, 0-regression. EXCELLENT, owner-DONE).** Problem `build_the_goal_subgoal_hierarchy_graph_for_plot_structure_comprehension`, reverified first-hand `test_goal_hierarchy_graph.py` **8/8** + a new pure-hdlab landing witness `test_goal_hierarchy_landing.py` (the live reader populates `sm.goal_graph` + the 4 callables; PURE ADD — flat `sm.wants`/`why`/`achieved` untouched; multi-hop `why_chain(search)=['unlock','escape']`). **WIRE LANDED (Q111, DEFAULT-ON, PURE ADD):** promoted `hdlab/goal_hierarchy_graph.py` (GoalGraph + build_goal_graph + readouts, VERBATIM, stdlib + `hdlab.goal_register` only) + wired `_read_goals` to set `sm.goal_graph` + `sm.goal_why_chain`/`superordinate_goal`/`reinstated_goal`/`salient_goal` AFTER the flat-register bindings (never touches `sm.wants`/`why`/`achieved` → 169/169 live-board answers byte-identical off vs on, 0 regression on every dimension). Battery (30-item authored plot-structure; hand-set gold + shuffled-edges twin carry the epistemic weight): goal-why CHAIN/superordinate 1.000 vs flat.why 0.682 (+0.318 CI-sep, twin p95 0.296); SUPERORDINATE reinstatement over distance 1.000 vs flat.wants recency 0.067 (+0.933, distance-INVARIANT where the recency floor collapses to 0 at K≥1); connectivity salience 1.000 vs recency 0.000; all twins LOSE; structural reconstruction 1.000. **NEW PINNED: narrative comprehension builds a goal/causal NETWORK; salience = CONNECTIVITY + chain-membership NOT depth (Trabasso & van den Broek 1985); motivation edge = Goal→Attempt, enablement = event→goal (Trabasso relations); reinstatement = most-recent-still-open superordinate (Suh & Trabasso 1993); Grosz-Sidner dominance stack for attachment.** OUR-INVENTION-UNDER-TEST (swept/controlled): shared-head-lemma chaining, the open-stack marker-less attachment, the connectivity tiebreak. **LOCATED NEGATIVE (fair, then CRACKED): marker-less subgoal→superordinate linking needs planning inference — 10.8% isolated goals on real 19c prose, explicit ≥2-hop chains sparse (11 chains / 25 docs); BUT a graded PPMI+SVD ATOMIC means-end bridge cracks it to 0.94 on covered verbs (+0.68 held-out generalization), so the residual is a KNOWLEDGE-COVERAGE bound, not un-inferable.** Multi-hop is PROVEN on clean substrates (K=2 certified; supercharged by candidate-set restriction, precision ceiling = cone-collapse closed by restriction not scaling) — the graph's structural walk is in that proven-clean regime; the real lever is EDGE COVERAGE, not deeper search. **INSTRUMENT GAP (honest): only 4% of the live board's goal-why questions are multi-hop, so the benefit is invisible live until a plot-structure board arm is added (same shape as the parser's agent-only events QA). FOLLOW-ONS FILED: the PPMI+SVD means-end bridge real-narrative validation + the inverse-planning organ (one engine unifies goal-attachment, abductive goals, AND belief).**

- **2026-09-05 (INTEGRATION — register-robust event DETECTION turned ON (scoped so causal stays byte-identical), lifting the who-did-what arms; + the load-bearing SILO deviation named: the who-did-what readout is siloed from the copular-state dimension. EXCELLENT, owner-DONE).** Problem `register_robust_event_detection_turn_on_and_expand_lifts_every_who_did_what_arm`, reverified first-hand `test_event_detection_crossarm_organ.py` **7/7** (predicate_recall additive, copula readout draws from entity_states, deranged-state twin load-bearing, coref byte-identical, both arms lift) + a new pure-hdlab landing witness `test_event_detection_scoped_recall_landing.py` (predicate_recall DEFAULT-ON; **SCOPED causal BYTE-IDENTICAL recall ON==OFF, 4 docs**; additive +340 events). **WIRE LANDED (Q111, hdlab reader change): `predicate_recall` FLIPPED DEFAULT-ON, SCOPED** — the P6 open-class noisy-channel recovery (Gibson 2013) promotes tagger-dropped WordNet-verb tokens back to events; NET-POSITIVE on BOTH who-did-what arms on the CURRENT reader (held-out agent +0.0125 / patient +0.0050 CI-sep, monotone in recovery, random-verbhood twin loses), and `_read_causation` now computes `sm.causal_links` over the BASE (non-recall) event set so causal is byte-identical (UNSCOPED it regresses **−0.0594 CI-sep** — the extra events add distractor causes to the density-brittle connective/bridge SELECTION heuristic; the naive self-derived read HID this as a +0.0224 'gain' via qset drift — a fixed-qset was required to see it). NO-REGRESS (SOLVED's fixed-qset instrument): coref/temporal byte-identical, world_state +22/−0, bound_event_tokens 1/3641. **NEW PINNED (load-bearing deviation): the who-did-what readout is SILOED from the copular-state dimension** — copular predications are DETECTED (default-ON `bind_entity_states`, holder 0.63R near the competent-reader oracle 0.71) but filed in `sm.entity_states` where the dynamic-event who-did-what readout cannot look, so "who is the captain?" scored ~0 with the answer present. The SORT-AWARE readout (copula-gov → LEAD with the entity_states HOLDER→agent / PROPERTY→patient, NOT firing states into the event stream — Maienborn sort-collapse barred) lifts patient/be **0.000→0.259 CI-sep** (agent HOLDER 0.09→0.63 held-out, +0.53 — a first-draft "withdrawn" was the solver's own routing bug, corrected). This capability is present in the live substrate (`entity_states` default-ON; the main 10-dim board already answers copular "what/who is X" via the STATE dimension's state-frame routing; the who-did-what eval answerer reads it for copula-gov). Faithful long-term fix = ONE sort-typed eventuality inventory (`assemble_the_tiered_bound_event_token`). **LOCATED NEGATIVE (fair): the causal connective cause-SELECTION is density-brittle and its brain-faithful parse-structural fix FAILS three ways (−0.079 to −0.317, all worse than scoping) — the OOD 19c parse cannot identify the causal clausal head.** **FOLLOW-ON FILED: a force-dynamic MEANING-HUB causal scorer** (Talmy/Wolff; retires the scoping workaround — proven by isolation that structure CANNOT solve causal: a perfect parse is WORSE, oracle participants don't help). The convergent upstream amplifier (the joint graded decoder, raising open-class 19c recovery 0.56→~1.0 + copula holder 0.63→0.77) is the already-filed CRF/joint-decode problem.

- **2026-09-05 (INTEGRATION — the rare-sense meaning channel: a rigorous LOCATED NEGATIVE (the ceiling is the FROZEN INPUT REPRESENTATION = the no-transformer invariant boundary) + a real landable readout win (the Bayesian frequency-modulated competition, +0.065 CI-sep, generalizes 6/6). EXCELLENT, owner-DONE).** Problem `grow_broad_coverage_correctly_resolved_rare_sense_experience_the_meaning_channel_learner_on`, reverified first-hand `test_rare_sense_episodic_coverage_growth.py` **26/26** (deterministic; strict document-disjoint SemCor subordinate, subject-weighted a_s, n=2676) + a new landing witness `test_bayesian_prior_wsd_landing.py` (default byte-identical + the active formula == prior_weight·log(prior)+z(context) + resting-bias behaviour). **WIRE LANDED (Q111, DEFAULT byte-identical): `hdlab/diagnostic_context_wsd` gains `sense_prior`/`prior_weight`** — the BAYESIAN frequency-modulated competition (MacDonald 1994 / McRae competition-integration, linearized): frequency enters as a log-prior RESTING BIAS while STRONG CONTEXT decides, NOT an additive dominant-forcing vote. Default prior_weight=0 is bit-identical; when active the rare tail rises **0.316→0.387 (+0.065 CI-sep)**, all-pop 0.456→0.629 (a strict PARETO win over the context-only readout), generalizes CI-sep **6/6** frozen-weight (NOUN/VERB/freq-band/fresh-split). Bigger than P9's precision-weighting (+0.023) and STACKS with it. **KEPT DEFAULT-OFF with a MEASURED reason (NOT a hidden subpar downstream): the ONLY substrate consumer (`consolidation_gate`/the learner) is itself default-off (the knowledge north-star), and the operating point is a per-consumer tail-vs-head tradeoff (A2 sits below MFS on all-pop) — dev-select per consumer, do NOT test-tune** (the same optional-knob pattern as P9's gamma/topk). **THE BAR's OWN VERDICT = a full-pass LOCATED NEGATIVE:** the fine-grained rare-sense a_s does NOT robustly cross even with EVERY component made brain-faithful (episodic MINERVA-2 max-echo beats prototype-averaging +0.038 CI-sep; online PBV-v2 grounding-anchored propose + Bush-Mosteller verify makes clean traces; reading grows coverage 0.14→0.47) — NAMED CAUSE, triangulated 4 ways + disk-exhausted: the residual is the FROZEN INPUT REPRESENTATION (base readout + episodic traces read the SAME thin w2v context), whose faithful fix is a contextual per-occurrence encoder = the §2 invariant boundary. **MORE-HUMAN-LIKE (the brain's own grain): at the coarse/supersense grain humans actually agree on (~0.90 ITA vs ~0.72 fine) the SAME mechanism reaches ~0.50 on rare senses (CI-sep over both floors); 71% of rare cases are genuine HOMONYMY (a real task), 29% polysemy (the coarse-recoverable inventory artifact).** The CLS episodic store + coverage-growth machinery is SHELF-READY for the day the input is richer. **FOLLOW-ON FILED: cross-corpus (WiC/SemEval) generalization** of the frozen-weight readout (the one untested axis). ⚠️ **OPEN OWNER DECISION (§2, the owner's to make): HOLD the no-transformer invariant + accept the FINE ceiling is input-bound (comprehension is already ~0.50 and human-like at the coarse grain), OR relax it for ONE offline contextual encoder to push the fine number toward ~0.53. Strategy recommendation: HOLD** — given the coarse-grain comprehension is substantially present, holding costs less than it first appeared.

- **2026-09-04 (INTEGRATION — the who-did-what PATIENT is now read off the LABELED parse (grammatical relation + precise voice + valency binding), +0.086 CI-sep on clean UD; and the stronger arc-eager parser is FLIPPED DEFAULT-ON, made register-safe by the new readout. EXCELLENT, owner-DONE).** Problem `improve_the_parser_verb_argument_attachment_for_who_did_what`, reverified first-hand `test_valency_labeled_patient.py` **5/5** + a new pure-hdlab landing witness `test_labeled_patient_landing.py` (byte-identity landed-vs-validated-drop-in **0 mismatches / 1255 UD items**, patient **0.745→0.831 = +0.0861** CI[0.068,0.104]). **WIRE LANDED (Q111, 3 diffs):** (1) PRECISE voice remapping (`relcl_resolver.precise_passive`, replacing the lossy `robust_passive` that false-fired passive on 9.2% of ACTIVES — the single biggest lever, +0.0565 alone); (2) `structural_patient_pick` now fills the arc_labeler's obj/nsubj:pass LABELED slot with valency-gated binding + net-safe hybrid fallback (`labeled_pick`/`position_pick`/`_transitive` promoted VERBATIM from the validated cells); (3) `parser_arceager` FLIPPED DEFAULT-ON — the labeled/precision-weighted readout makes the stronger arc-eager parser REGISTER-SAFE (+0.006 modern / +0.0045 19c clean-DO, reversing the −0.0017 it caused under the OLD position readout) and BOARD-NEUTRAL (all 6 board dims byte-identical 0.0-delta arceager on-vs-off, 16 docs). NO-REGRESS: the patient change is invisible to the AGENT-only events QA (2718/8049 picks changed, all 6 board dims + aggregate 0.0-delta); the P2 AGENT is untouched. **NEW PINNED (fidelity): core roles are read off GRAMMATICAL RELATIONS bound into verb VALENCY slots + a VOICE linking rule (Hagoort MUC; Levin/Rappaport-Hovav), NOT position — the READOUT, not head accuracy, carries who-did-what (head-INDEPENDENT: arceager UAS +0.05 → patient +0.00; the win is voice+label+valency).** Per-arc CONFIDENCE is a computed-but-discarded brain signal (arc margin discriminates a correct obj-arc at AUC 0.81 — an un-wired precision-weighting substrate). **LOCATED NEGATIVES (fair): a better HEAD parser does not move who-did-what (3 register-general head-parsing negatives on disk); precision-weighted selectional RE-ATTACHMENT is null-to-twin.** The brain's register-general mechanism was RESEARCHED + BUILT (a categorical backbone; Jabberwocky patient 0.815 / agent 0.969 from PURE STRUCTURE — attachment needs no corpus statistics) but underperforms the trained readout on real prose (clause-segmentation precision is the bottleneck; the validated backbone is handed to the agent/OOD problem). **FOLLOW-ON FILED: a PATIENT-slot QA on clean gold** — the live who-did-what QA is AGENT-only, so patient gains are invisible end-to-end; future patient work cannot be seen until this instrument exists. The one high-value further lever (a register-invariant clause segmenter) is owned by `the_agent_tie_wall_is_embedded_clauses...`.

- **2026-09-04 (INTEGRATION — the NORTH STAR, resolved as a rigorous LOCATED NEGATIVE + one landable win + the ceiling RELOCATED with a number: the grounded ATL hub-and-spoke does NOT cross the rare-sense ceiling; the gap is a LACK OF RARE-SENSE EXPERIENCE, not the encoder/readout/keys/knowledge. EXCELLENT PARTIAL = a FULL PASS by the bar, owner-DONE).** Problem `build_the_atl_hub_and_spoke_meaning_channel_online_predictive_reader` (the north-star P9 I filed; a solver solved it), reverified via `test_atl_hubspoke_meaning_channel.py` (W1–W8, strict document-disjoint SemCor subordinate, subject-weighted, n=2676) + a new pure-numpy landing witness `test_diagnostic_context_precision_landing.py` **4/4** (default byte-identity + the param is live + verbatim to the reference). NEW PINNED-AND-MEASURED: (1) the RICHER grounded ATL hub-and-spoke (Binder-65 + Warriner VAD, ATL-whitened, WordNet-inheritance-propagated) does NOT cross — concat-hub a_s **0.283 < the launch pad 0.313** (CI-sep BELOW); **BUT the brief's orthogonality premise HOLDS at the key level** (grounded keys separate gold-vs-dominant cos 0.222 vs w2v-gloss 0.799; rescue 80% of distribution-merged pairs) — the null is QUERY-side selection + only 0.477 coverage, NOT key separability. (2) THE LANDABLE WIN (query-side, brain-faithful): **precision-weighting** (Friston selective gain — gamma>1 / top-k on the diagnosticity, the multiplicative gain that lets a high-precision subordinate cue overturn the dominant prior) lifts a_s **0.313→0.336 (+0.023 CI-sep [+0.012,+0.035]), shuffled-diagnosticity twin LOSES, NO MFS regression** — does NOT reach ~0.35. (3) THE CEILING RELOCATED: no glass-box readout/representation lever reaches ~0.35; the crosser is a broad-coverage SENSE-DISCRIMINATIVE W (parent proved oracle-W → 0.995, coverage-bound 52%); the FULL brain-faithful chain (grounded keys → encoding resolver → Hebbian bind → consolidation → precision readout) climbs then STALLS at consolidation, localizing the single remaining non-brain-foundational component to the ENCODING-TIME sense-resolver, trapped in the FROZEN sense-conflated w2v context (a genuine fixed point: to build the W that disambiguates you must first disambiguate; contextual per-occurrence RE-REPRESENTATION is the enabler — frozen BERT beats MFS +20 zero-knowledge — which needs a transformer OR a lifetime of rare-sense reading). **WIRE LANDED (Q111, default byte-identical):** `hdlab/diagnostic_context_wsd` gains `gamma`/`topk` precision-weighting params (guarded → default gamma=1.0/topk=None is BIT-IDENTICAL to the pre-P9 readout, 200-case witness); consumers opt in. **DO-NOT (measured negatives, do not reinvest):** the grounded hub into the keys (0.283<0.313), reading-derived W default-on (glass-box consolidation regresses rare senses), a general trained encoder (scale-bound 0.192→0.207, not brain-foundational). CONTROLS caught two FALSE PASSes (the "0.360" knowledge-integration = transductive leakage; the "0.382" anti-dominant = a prior shift) — the inductive-only-W + MFS-guard are mandatory. **FOLLOW-ON FILED (the north star this earns): `grow_broad_coverage_correctly_resolved_rare_sense_experience_the_meaning_channel_learner_on` (priority 5)** — grounding-anchored propose-and-verify bootstrapping (Trueswell 2013) through the owner-DONE consolidation gate. ⚠️ **OPEN OWNER QUESTION (surfaced, the owner's to decide): hold the no-transformer invariant + accept the ~0.34 glass-box rare-sense ceiling (pursue the coverage/learner-on route), OR relax it for ONE offline contextual-sense asset to reach ~0.53? Solver + strategy recommendation: HOLD the invariant.**

- **2026-09-04 (INTEGRATION — common-noun coref: a LOCATED NEGATIVE + a landed deployable former (DEFAULT-ON) + a major correction; the REAL mechanism (entity-world-model query) is proven but WORLD-KNOWLEDGE-bound. EXCELLENT, owner-DONE).** Problem `form_a_discourse_referent_for_every_entity_not_just_named_ones_common_noun_coref` (the top-priority coref problem I filed; a solver solved it), reverified `test_commonnoun_referent_former.py` **7/7** + a new pure-hdlab landing witness `test_commonnoun_coref_landing_organ.py` **4/4**. **CORRECTION (disk-outranks-brief): the reader ALREADY forms common-noun referents** (surface-head grouping, character-cluster CoNLL **0.605, +0.27 over proper-name-centric, CI-sep**) — the brief's "forms none" is accurate ONLY at the CONSUMER level (the affect/goal/world-state canonicalizers were proper-name-centric and dropped the common-noun clusters). **The affect study's "+0.43 gold-coref headroom" is partly a gold-LABEL-SPACE artifact** (built from gold coref labels). **LOCATED NEGATIVE (a full pass):** a faithful cue-based common-noun former (ACT-R retrieval extended from pronouns, reusing `graded_coref_pick`) does NOT beat the surface-head floor CI-sep (+0.0008) — because 19c-literary common-noun reference is world-knowledge-bound: head-match recall **0.341** (66% of links use SHIFTING EPITHETS — "the old man"→"the poor fellow"→"her father"), 82% of links have 2-3 competing persons, over-merge errors 91% content-IDENTICAL, WordNet bridging tags only 7.8%. **WIRE LANDED (Q111, DEFAULT-ON):** the deployable situation-gated former (`hdlab/commonnoun_binder.py`: head-match-gated + MODIFIER-SPLIT + wide window + the `event_centrality_coref` situation gate for ties, promoted verbatim) replaces the reader's blind same-head merge (+0.0128 CoNLL CI-sep [+0.006,+0.020], CEAFe 0.469→0.510, twin loses, NAMED coref byte-stable +0.0000); + `commonnoun_canonical` wires the common-noun clusters into `make_canonicalizer`/the affect+goal registers (the dropped +0.27). **FLIPPED DEFAULT-ON per the no-default-off + "flip-on-find-the-break" directive:** the cross-consumer check is CLEAN — 7 structural dims (coref_acc/events/who-did-what/entity_states/causal/timeline/world_state) BYTE-STABLE on vs off; goals/affect count- and coverage-neutral; and the gold-scored goal/affect QA is **IDENTICAL on vs off** (goal 0.4615==0.4615, affect 0.7288==0.7288, n=26/118) — the label re-clusterings do not change any scored answer. +~0.3s/read. **THE REAL MECHANISM, BUILT + PROVEN, BLOCKED:** the brain resolves definite reference by QUERYING an accumulated ENTITY WORLD-MODEL (file cards: type/role/relation/event/presence, foregrounded set — Kintsch CI / Zwaan / Sanford-Garrod bonding→resolution / Heim-DRT), NOT weighting surface cues; built + the confidence gate → ambiguous-link resolution **0.255→0.540** (union-oracle 0.615 — the facts ARE in the narrative). BUT deployment hits a **BOOTSTRAPPING / IDENTIFIABILITY wall**: self-built records give only +0.006 (you can't build correct entity records without already resolving reference); the brain breaks the chicken-and-egg with a PRIOR WORLD KNOWLEDGE (who "the master" is, who Elizabeth's father is) — exactly the external prior the no-LLM invariant bars + SOTA imports from a pretrained LM. **FOLLOW-ON FILED: the Phase-1 world-knowledge-prior entity-world-model resolver** (a static offline scenario/role + kinship KB to seed the entity records so the built resolver realizes its 0.54 ceiling — ties directly to the curated knowledge foundation #4; lifts this + the affect + WSD located negatives together). The character-bound canonicalizers being proper-name-centric was the wiring debt (now wired); the residual downstream loss is pronoun-experiencer resolution + label-consistency, not common-noun clustering.

- **2026-09-04 (INTEGRATION — the who-did-what AGENT embedded-clause tie wall: the register-general incremental parse enters the Competition-Model as ONE self-gating precision-weighted STRUCTURE cue (default-ON); the brief's recency-Centering + thematic-fit are REFUTED. EXCELLENT, owner-DONE).** Problem `the_agent_tie_wall_is_embedded_clauses_needs_a_register_general_incremental_parse_cue` (the P2 follow-on I filed; a solver solved it), reverified `test_cmrole_agent_struct_organ.py` ALL PASS first-hand (tie slice base 0.544 → struct 0.622, shuffled-structure twin 0.400 LOSES, canonical no-regress 0.656→0.668; SOLVED board: tuned +0.0726 CI-sep, held-out +0.0562). NEW PINNED: the embedded-clause nominative-vs-nominative tie is resolved by INCREMENTAL clause structure entering the graded competition as ONE PRECISION-WEIGHTED cue that self-gates (down-weighted when the parse is unreliable — eADM; Vosse-Kempen); the register-general `hdlab.incremental_parser` left-corner subject bind is the right source (a TRAINED parser was proven to lose OOD on 19c — vindicated). **WIRE LANDED (Q111, DEFAULT-ON, patient AND agent byte-identical where it does not fire):** `hdlab/incremental_parser.incremental_subject_before` exposed + a `structure` cue added to `hdlab/graded_role_assigner.agent_supports` + `AGENT_VALIDITIES["structure"]=STRUCT_W=2.5` (self-gating — votes only where the incremental parse binds a subject); `situation_reader._cm_agent_for` passes the incremental subj_before into `agent_competition_pick` behind the `cm_agent_struct` flag DEFAULT-ON. Composes with the P2 agent competition + the structure-first patient (the who-did-what stack is now: agent = Competition-Model over the tracked set + the structure cue for ties; patient = structural object off the parse). **BRIEF HALF-RIGHT: recency-weighted Centering REFUTED** (fails tuned+held-out — embedded clauses introduce a NEW subject so "continue previous subject" mispredicts; matches the salience_binder recency-at-chance finding); **thematic-fit/agentivity CLOSED** (the tie residual is 89% character-vs-character where names carry no selectional signature); RC-pop revision REJECTED (hurts clean prose). RESIDUAL localized: 58.6% event-detection (a different organ) + 40.2% competition fragments + 1.1% readout → the composed micro-fixes (predicate_recall flip +0.0083, gerund-possessive, collective-human animacy) = +0.0105 whole-arm; the tie remainder is irreducible character-vs-character ambiguity + coref-miss. **FOLLOW-ON FILED: `register_robust_event_detection_turn_on_and_expand_lifts_every_who_did_what_arm` (priority 6)** — event detection is 58.6% of this residual + ~20% of P2's, and a dropped event is unanswerable, so it caps every who-did-what arm.

- **2026-09-04 (INTEGRATION — the AFFECT/EMOTION dimension: a per-character emotion register (default-ON), built on the goal-register template; its signal-loss study localized the reader's dominant bottleneck to COMMON-NOUN coref. EXCELLENT, owner-DONE).** Problem `the_situation_model_has_no_affect_emotion_dimension` (the affect dimension I filed this session; a solver solved it), reverified **12/12** first-hand + a new pure-hdlab landing witness `test_affect_register_landing_organ.py` **6/6** (existing dims — incl. the goal register — BYTE-IDENTICAL track_affect off vs on → purely additive). NEW PINNED: emotion is a genuinely SEPARATE system (Campanella 2022 triple dissociation — NOT folded into belief/goal), VALENCE-primary + emotion-CATEGORY-secondary (Barrett/Lindquist; Gygax online reading), the EXPERIENCER bound by the psych-verb LINKING frame (VerbNet admire-31.2 exp=subject vs amuse-31.1 exp=object), affect OVERWRITES (de Vega) where goals PERSIST (Lutz-Radvansky), emotion words gated by DENOTATION not association (WordNet-Affect; Pavlenko 2008). MEASURED (100 LitBank docs, floors per population): "how does X feel" category **0.788 vs most-recent-emotion floor 0.312 vs shuffled-character twin 0.394** (both CI-sep); VALENCE sign **0.838 vs floor 0.490**; binds the right character 391 vs 37. **WIRE LANDED (Q111, DEFAULT-ON — additive + net-positive, mirrors the goal register):** `hdlab/affect_register.py` (stdlib+hdlab only) + upstream `hdlab/psych_verb_frames.py` (experiencer-linking, shipped `data/frontend_assets/psych_verb_transitivity_ud_ewt.json`) + `hdlab/affect_lexicon.py` (Warriner valence + curated emotion-denoting inventory, shipped asset); `track_affect` flag default-ON + `_read_affect` (mirrors `_read_goals`; sets `sm.affect_register` + `sm.feels`/`valence_of`/`feels_about`) + the board `affect` arm. Zero regression (the psych frame gates ONLY psych-verb experiencer position — non-psych affect byte-identical, W10). **THE DOMINANT FINDING (a full-chain signal-loss study, oracle-substitution ladder): COREFERENCE is 87% of the end-to-end affect loss** — glass-box F1 0.581 → +gold coref 0.945 → ceiling 1.0; 83.5% of emotion experiencers are COMMON-NOUN entities ("the man", "the child") the reader NEVER forms a referent for (genuine named-pronoun error only ~9.6%; detection recall 0.96). The affect extraction/valence/experiencer-linking are NEAR-CEILING given good coref — the bottleneck is entirely UPSTREAM coref, and it is SHARED (it caps who-did-what/goals/world-state/belief too). Six glass-box referent-former prototypes each recovered ≤+0.02; only gold coref recovers +0.43. **FOLLOW-ON FILED (the highest-leverage shared lever in the queue): `form_a_discourse_referent_for_every_entity_not_just_named_ones_common_noun_coref` (priority 2)** — the faithful common-noun discourse-referent former (introduction + bridging + cue-based retrieval + own-NP detection). **LOCATED NEGATIVE (a full pass): UNSTATED emotion ("she slammed the door" → anger) needs the OCC-appraisal meaning channel — the explicit-vs-inferred split.** With this, the reader has ALL FIVE Zwaan-Radvansky dimensions + goal + affect.

- **2026-09-04 (INTEGRATION — the who-did-what PATIENT is now read STRUCTURE-FIRST off the parse (default-ON), not flat position; the brief's graded-posterior mechanism is a located-negative but the drill found the real lever + flagged the board's patient gold as a confounded instrument. EXCELLENT PARTIAL = a FULL PASS, owner-DONE).** Problem `consume_the_graded_pos_posterior_uncertainty_aware_starting_with_referent_np_detection` (the P3 I filed this session; a solver solved it), reverified first-hand: the WIN witness `test_structural_patient_optimization.py` **3/3** + the located-negative witness `test_graded_pos_consumption_located_negative.py` **4/4** + a new pure-hdlab landing witness `test_structural_patient_landing_organ.py` **4/4**. **LOCATED NEGATIVE (the brief refuted, a full pass):** referent introduction is PROPN↔NOUN-INVARIANT (0/280 sents; both UPOS are NOMINAL) and coref never reads UPOS, so the graded posterior into `referent_per_np` adds +0.000 to live who-did-what (twin loses); the only channel PROPN↔NOUN flips is ANIMACY, a correct-but-SUBORDINATE cue in word-order-dominant English (> twin CI-sep, NOT > floor). **NEW PINNED (the drill, "audit the ruler"): who-did-what role assignment is the brain's STRUCTURAL route (parse → grammatical relations → verb-frame binding → VOICE remapping; Hagoort MUC; Levin-Rappaport-Hovav), and the live reader's flat cue/position PATIENT was the DAMAGED-BACKUP route (the agrammatic-Broca's fallback — our exact failure profile). COMPOSES WITH the P2 agent entry below: AGENT = actor-first Competition-Model over the given/tracked set (P2); PATIENT = the structural OBJECT read off the parse (this) — the two roles use different brain-faithful mechanisms; flat position was the lesioned patient route, now replaced.** **WIRE LANDED (Q111, DEFAULT-ON):** `hdlab/predicate_argument_frontend.route_predicate_arguments` computes the THEME/patient STRUCTURE-FIRST (`structural_patient_pick`: object[active] / promoted-subject[passive via `robust_passive`] / coordination-share, heuristic fallback when the parse yields no core object; AGENT kept EXACTLY as-is — P2's cm_agent byte-untouched, 0/2850 board agent answers changed), `structural_patient` flag default-ON. **MEASURED — DEFAULT-ON justified by the CLEAN instrument (the board patient gold is CONFOUNDED, do NOT trust it):** on CLEAN UD-EWT structural gold (patient := obj|nsubj:pass off GOLD relations, n=1255) structure-first lifts patient **0.6725→0.7602 (+0.088)** ZERO tuned parameters (grammatical relations + voice = universal grammar, generalizable — proven on UD test AND train); on the LitBank OBJECT gold it reads −0.006 ONLY because that gold mislabels obliques/locations ("hall"/"earth"/"streets") as objects and the fix correctly declines them (the same instrument confound the solver proved — the role-balanced gold is voice-confounded + circular-cue-trained; **re-base who-did-what eval on the clean UD gold**). NO-REGRESS: non-role dims (events/entities/coref_acc/causal/timeline/targets) byte-identical ON vs OFF, only patients change. **PERFORMANCE vs the brain: ceiling with a PERFECT parse is 0.912 → the residual +0.15 (0.76→0.91) is ENTIRELY the parser's verb→argument attachment. FOLLOW-ON FILED: `improve_the_parser_verb_argument_attachment_for_who_did_what` (priority 3)** — a verb-frame-guided LABELED dependency parser that binds arguments into valency slots DURING parsing (glass-box, incremental, NOT a batch LLM). **AUDIT HYGIENE: flag the LitBank role-balanced / OBJECT patient gold as an INVALID instrument (confounded voice + coref-antecedent patients + circular cue-training); use the UD-EWT structural gold.**

- **2026-09-04 (INTEGRATION — the who-did-what AGENT assigner is now the brain's Competition-Model over the TRACKED/given set (default-ON); the referent_per_np default-on is now a NET board win — the stale board is unblocked. EXCELLENT, owner-DONE).** Problem `swap_the_positional_role_assigner_for_the_brain_foundational_competition_model` (the URGENT P2 I filed; a solver solved it), reverified first-hand: scaffold-free board witness **10/10** + a new pure-hdlab landing witness `test_cmrole_agent_landing_organ.py` **5/5** (landed reader recovers the agent arm to **0.6073** vs pre-referent baseline 0.2649 vs the regression 0.0750; **PATIENT byte-identical on BOTH the positional and wired paths** — the +0.336 preserved by construction; info-free twin 0.4607 loses; on the DEPLOYED wired path cm_agent OFF 0.1383 → ON 0.6073, +0.4689 CI-sep). NEW PINNED (a TWO-PART fix, the brief was half-right): thematic-role assignment is graded parallel CUE COMPETITION (Bates-MacWhinney; McClelland 2013 additive activation = the Bayesian posterior) — BUT the decisive variable is the CANDIDATE SET: the AGENT competes over the TRACKED/GIVEN discourse entities (Centering Cb→subject, Grosz 1995; DuBois 1987 Preferred Argument Structure — 79.4% of reachable gold agents are tracked, matching the ~80% Centering figure), NOT every NP head (the same DECOUPLE lesson as the parent — agent=tracked, patient=dense/residual). The rule ALONE over the dense set only reaches 0.082; the SET decouple is load-bearing. **WIRE LANDED (Q111, DEFAULT-ON, no-default-off — net-positive + patient-neutral + held-out-replicated):** `hdlab/graded_role_assigner.agent_competition_pick` (the AGENT counterpart to `hybrid_role_patient`, REUSING `hdlab.graded_competition.net_activation` verbatim; cues preverbal/animacy/salience/adjacency/byagent/core_arg + the static `AGENT_VALIDITIES`) + `situation_reader` routes `coref_mentions` (tracked) as the AGENT candidate source in `_read_events`/`_read_events_wired`, patient untouched; the FULL proven stack landed as flags all DEFAULT-ON: `cm_agent` + `include_pron_agents` (subject pronouns are the maximally-given Cb) + `case_filter` (nominative-only) + `clause_local` (clause-bounded competition) + the board scorer's context-cued `answer_instanced` readout (answer the event at the QUESTION'S OWN sentence, not the global last event — uses the question index, never the gold). Full stack banks who-did-what agent 0.041→~0.64–0.69 (a small reader-default drift since the SOLVED run accounts for 0.64-vs-0.69; the solver's shadow reader yields 0.64 today too, agent signatures byte-identical to the landed reader). **PERFORMANCE vs the brain + where signal is lost now:** at ~0.69 the residual is 75% embedded/relative-clause nominative-vs-nominative TIES (only full clause structure resolves them — a trained parser was PROVEN to lose OOD on 19c, vindicating the no-trained-parser invariant), 20% event detection (a different organ), 5% coref coverage. **FOLLOW-ON FILED: `the_agent_tie_wall_is_embedded_clauses_needs_a_register_general_incremental_parse_cue` (priority 4)** — a register-general incremental parse as ONE precision-weighted cue + recency-weighted Centering for the tie wall (the highest-leverage remaining who-did-what lever). HONEST BOUND: absolute accuracies are depressed by a 70% pronoun-subject scorer ceiling (constant across arms); the load-bearing claims are the paired CI-separated deltas + the patient byte-identity. Pre-existing stale self-tests (`_selftest_frame_arity_gate`, `test_situation_model_qa` temporal/causal) are NOT caused by P2 (byte-identical cm_agent on vs off) — flagged for a separate refresh.

- **2026-09-04 (INTEGRATION — the POS tagger's per-call Viterbi is ~4.3–4.85x faster with BYTE-IDENTICAL output (landed default-ON); the bottom-up model is confirmed brain-faithful and the speed makes the brain's GRADED decode affordable. EXCELLENT, owner-DONE).** Problem `optimize_the_pos_tagger_viterbi_inner_loop_the_co_dominant_read_cost` (the P8 follow-on I filed THIS session; a solver solved it same-day), reverified first-hand: pre-landing witness **4/4** + a new pure-hdlab landing witness `test_pos_tagger_fast_path_landing.py` **3/3** (emission matrix BIT-identical np.array_equal 0/694, tag sequence identical 0/694, 4.85x on 694 held-out sents / 15,684 tokens) + the full-read SituationReader fingerprint UNCHANGED (`d6373e11…` exact, byte-identical end-to-end). **WIRE LANDED (Q111, default-ON — pure byte-identical speedup):** `hdlab/pos_tagger.py` builds a `_FastEmissionPlan` (variant-C: `base_contrib` split on the last `~` + precomputed TM/SV + `token_bases`, the emission reduction routed through the SAME built-in `sum()` as stock for Neumaier-compensated bit-identity) attached lazily to the perceptron; `hdlab/perceptron._viterbi` routes to the fast path ONLY when a plan is attached AND `weights is fast.weights` (dict-identity guard), else the untouched `_viterbi_reference` (the byte-identity reference the witness checks + the training path — `fit` always uses reference). The lever: 16/17 of the emission build was redundant (every emission feature is `<base>~<tag>` with a TAG-INDEPENDENT base, computed once per token PER TAG). Compounds with P8 (parser) + the tagger perf sweep #2 (call-count 321→111): each remaining tag call is now ~4.5x cheaper. **KEY BYTE-IDENTITY LESSON (folded):** CPython 3.12 `sum()` is Neumaier-COMPENSATED, so a naive sparse reduction diverges ~1e-15 and eventually flips an argmax on a tie — the witness asserts the EMISSION MATRIX bit-identical (stronger than tag equality) to catch this class. **NO fidelity-map change (output identical); ONE fidelity NOTE:** the tagger's BOTTOM-UP model is brain-faithful (Collins-2002 perceptron update = Widrow-Hoff/delta-rule = the Rescorla-Wagner family, error-driven cue-competition — which is WHY a generative P(word|tag) tagger REGRESSES, it double-counts correlated cues; MacDonald 1994 / Ramscar 2010 / Baayen 2011), so this speed win PRESERVES a brain-faithful computation. The remaining gap is the DECODE (hard 1-best vs the brain's GRADED ranked-parallel + top-down): the speed makes the graded decode (Viterbi 1-best + forward-backward posterior = P7's CRF representation) run at 0.632s vs the stock hard-1-best 1.099s — CHEAPER than what we ship — but mechanism-fidelity ≠ accuracy: the graded candidates cash out only once top-down MEANING (grounding) re-ranks them (the joint tag↔parse loop regressed without it). **Where we lose signal is MEANING (measured fresh, UD-EWT gold 24,120 tok): 85.1% of tagger errors touch a CONTENT class; the biggest bucket is PROPN↔NOUN (28% of ALL errors)** — a referential/world-knowledge call surface form can't make, the SAME typed-grounding wall the P8 arc-parser finding + the reader's WSD work (P9) hit. FOLLOW-ON FAMILY (to file): consume the GRADED posterior (uncertainty-aware, brain-foundational ranked-parallel), `referent_per_np` first (PROPN↔NOUN lands in its NP/entity detection); the meaning wall is the typed-grounding problem (P9).

- **2026-09-04 (INTEGRATION — the 5th and LAST Zwaan-Radvansky dimension: a GOAL/INTENTION register, DEFAULT-ON; a FULL-CHAIN upgrade that also lands a general lexicalist SUBCATEGORIZATION-FRAME parse primitive + a passive-agent guard. EXCELLENT, owner-DONE).** Problem `the_situation_model_has_no_goal_intention_dimension` (the goal dimension I filed; a solver solved it), reverified **11/11** first-hand + a new pure-hdlab landing witness `test_goal_register_landing_organ.py` **6/6** (existing dims — events/coref_acc/coref_resolutions/entity_states/causal_links/timeline — BYTE-IDENTICAL track_goals off vs on across 3 docs → purely additive). NEW PINNED: intentionality is a distinct dmPFC-anchored mentalizing computation (Spunt/Lieberman Why>How; SEPARATE from belief-TPJ, desire FOLDED IN), DECISIVELY DISJOINT from physical causation (Malle 1999/2004 reason-vs-cause — measured complementarity: on goal-why the register scores 0.97 where the physical-cause dim scores 0.03, and the CONVERSE on because/so questions); goal STATUS is a graded-decay field active/satisfied/failed (Lutz-Radvansky 1997); reinstatement = last-unsatisfied-superordinate (Suh-Trabasso 1993). MEASURED (live reader, 20 LitBank docs, floors recomputed per population): WANT-explicit "what is X trying to do" (desire/intend/try + in-order-to) **0.58 vs most-recent-action floor 0.28 (CI-sep) AND shuffled-agent twin 0.0 (CI-sep)** — extraction precision AT the spaCy-oracle ceiling; WHY goal-why **0.97 vs physical-cause 0.03**; binds the RIGHT agent (816 vs 3 positive control); STATUS field 1.0 vs 0.33; reinstatement 1.0 vs 0.0. **WIRE LANDED (Q111, DEFAULT-ON — no-default-off: additive + net-positive, mirrors the sibling situation-model dims track_belief/track_world_state/bind_entity_states):** promoted `hdlab/goal_register.py` (stdlib+hdlab only — extractor + GoalRegister + track_status + the ported canonicalizer/`make_canonicalizer`/`passive_agent_guard`, NO experiments dep) + a default-ON `track_goals` flag + `_read_goals` (mirrors `_read_belief`; sets `sm.goal_register` + `sm.wants`/`why`/`achieved`, runs last so agents/status bind the FINAL stream); +~0.24s/read, existing dims byte-identical. Board `goal` arm registered in `exp_situation_model_qa_v1.py` (DIMENSIONS += 'goal', `with_goal=True`). **THE FULL-CHAIN UPGRADE (the upstream brain-foundational components, owner "this solution is also a full chain upgrade"):** (1) a GENERAL lexicalist SUBCATEGORIZATION-FRAME parse primitive `hdlab/verb_subcat_frames.py` + shipped asset `data/frontend_assets/verb_subcat_frames_ud_ewt.json` — per-verb P(infinitival-complement) DERIVED from UD-EWT gold (a static offline FOUNDATION asset, not the test set), the brain's lexicalist constraint-based mechanism (MacDonald/Pearlmutter/Seidenberg 1994; Trueswell 1996; Vosse-Kempen 2000) that resolves complement-vs-adjunct ("began to rain"/"wonderful to meet" = complement vs "went to buy" = purpose adjunct) — a GENERAL parse-fidelity primitive usable BEYOND goals; (2) extraposition-predicate detection; (3) a passive-agent guard (PRO → matrix AGENT, not the surface subject — McCourt 2015). These lift WANT-explicit 0.531→0.607 and remove ~120 over-fires with ZERO regression (the frame gates ONLY the bare-purpose branch, so the explicit consumer is byte-identical). **LOCATED NEGATIVE (the sanctioned full pass): bare-purpose adjuncts are ATTACHMENT-parse-gated (precision 0.34 vs oracle — needs a register-native parser, the modern arc parser is 19c-negative) and Tier-2 abductive "why THIS over that" needs the world-knowledge/meaning channel — the explicit-vs-inferred split the brief predicted.** ONE landing deviation (non-material): the reader's `sents` are lowercased (its own convention) vs the validated experiment's original-case `_conll_sents`; A/B = 30/31 explicit-slice goals identical (0.968 overlap), the live arm independently CI-separated. **The reader now has ALL FIVE classic Zwaan-Radvansky dimensions (time/space/causation/protagonist+belief/INTENTIONALITY).** §7 follow-ons (filed/to-file): register-native dependency parser (the bare-purpose ceiling — the largest single goal lift, shared with the parser wall) > meaning channel (Tier-2 abductive, shared with belief-inference/WSD = P9) > goal→subgoal hierarchy graph (Trabasso-van den Broek connectivity salience) > goal×belief composition. The general subcat primitive is now available to wire into broader infinitive-attachment decisions (a chain lever beyond goals).

- **2026-09-04 (INTEGRATION — the arc parser is 3.5x faster with BYTE-IDENTICAL output (landed default-ON); TWO fidelity corrections + the real accuracy wall LOCALIZED to typed lexical-semantic grounding, NOT decode-algorithm fidelity. EXCELLENT, owner-DONE).** Problem `optimize_the_arc_parser_inner_loop_the_dominant_read_cost` (the P8 I filed; a solver solved it), reverified first-hand: pre-landing witness **4/4** + the pure-hdlab landing witness `test_arc_parser_fast_path_landing.py` **4/4** (393,225 held-out arcs byte-identical, 0/376 sentences differ in heads/arcs/margins, parse 3.16–3.74x faster). **WIRE LANDED (Q111, default-ON — a pure byte-identical speedup with no invariant to break):** promoted the memoized fast path VERBATIM into `hdlab/arc_parser.py` (`FeatCache` + `precompute_token` + `sentence_flat` + `sentence_scores` + `decode_from_scores`); `ArcParser.parse`/`eval_uas` now use it, and the untouched stock body is kept as `ArcParser._parse_reference` so byte-identity is self-checkable forever. The lever is the 26.9x feature-STRING reuse (each distinct (template,key) crc32'd once → its id == `_h(original_string)`) + a single batched `np.add.reduceat` (verified bit-identical to per-arc `.sum()`, max abs diff 0.0). Live reader read confirmed byte-safe (output unchanged by construction). **TWO FIDELITY CORRECTIONS (disk-outranks-brief): (1) MECHANISM LABEL — `hdlab/arc_parser.py` is an ARC-FACTORED graph parser (O(n²) all-pairs arc scoring + greedy head + Chu-Liu/Edmonds cycle break), NOT the "arc-standard shift-reduce / per-transition" parser the brief named; output unchanged, so NO fidelity verdict moves — correct the label wherever the shift-reduce description appears. (2) The brief's "parser = 73% of a read" was a cProfile artifact; warm, the parser is ~26% of a read, so the read-level speedup is 1.13–1.45x, not ~2x (the bar was parse-cost-halved, met at 72%; the read-level benefit is real-and-modest, always-paid).** **THE REAL WALL, LOCALIZED (frontier the submission proved):** the parser/chain accuracy deficit is NOT decode-algorithm fidelity — a one-pass "brain-foundational" generative tagger (incremental beam + TnT morphology + trigram prediction) does NOT beat the already-cue-based/error-driven perceptron (0.924 vs 0.945, even OOV 0.737 vs 0.814), so a generative swap is a fidelity DECREASE; the joint tag↔parse route has only +0.016 ORACLE headroom and REGRESSES with predicted structure (circular, gated on parser accuracy). The binding non-faithful component is the MISSING lexical-SEMANTIC grounding: on PP-attachment (the canonical grounding-sensitive decision) growing a TYPED grounding lexicon lifts accuracy 0.587→0.639 (+0.052), MONOTONIC and still rising toward the ~0.84 ideal, info-free (shuffled-prep) twin collapses to 0.512 — and the generic TOPICAL hub does NOT help; the ideal needs syntactically-TYPED (head, grammatical-function) selectional preference (Pado/Resnik) — the SAME lesson as the WSD wall (§P2/P1). vs the brain: tagging 0.945 vs ~0.97 (near-parity), parsing 0.842 vs human ~0.95, PP-attachment 0.64 vs human ~0.88 — the deficit is concentrated exactly where MEANING is required. **INCREMENTAL PARSING (brain-fidelity) is ALREADY COVERED — do NOT re-file:** `incremental_parser.py`/`arceager_parser.py` exist and `wire_the_incremental_parser_as_the_reader_extraction_front_end` (PARTIAL) already established that hard-restricting role candidates to the incremental builder's bounded set HURTS (role-binding is a separate cue-based stream — Frankland-Greene 2015 / Lewis-Vasishth 2005), so the incremental parser stays default-off, precision-only. **ARC-EAGER FLIP NOT TAKEN (proposal b):** arc-eager is +0.05 UAS and ~8x faster on MODERN but is already default-off with a MEASURED reason (19c-negative; the reader eval includes 19c LitBank), and the submission's own no-regression check found reader consumers IDENTICAL arc-eager on-vs-off on 3 docs → no measured reader benefit to flip; the existing default-off (per-register routing needed) stands. **ADJACENT COMPONENT (next-problem seed): the POS tagger inner loop is now a CO-DOMINANT warm-read cost (`perceptron._viterbi` + `pos_tagger.pos_features`, ~28% of the profiled read) — the SAME memoization/hoist technique applies but the code path is DIFFERENT (dict-based structured perceptron, not the crc32 2^21 vector), so it is a separate byte-identical fix, not a free ride — the cleanest genuinely-NEW speed follow-on (distinct from the already-SOLVED tagger CALIBRATION problem).** Registered the fast-path landing witness; no §2b fidelity change beyond the label correction (output identical).

- **2026-09-04 (INTEGRATION + OWNER DECISION — referent_per_np is DECOUPLED and turned DEFAULT-ON; the turn-on measurement EXPOSED the non-brain-foundational POSITIONAL role assigner as the load-bearing downstream gap. EXCELLENT, owner-DONE + a strategy first-hand finding).** Problem `wire_the_referent_to_coref_linking_pass_so_referent_per_np_can_turn_on` (the P5 I filed; a solver solved it), reverified **7/7** first-hand. NEW PINNED: role-binding (who-did-what) and pronoun anaphora retrieve the referent set through DIFFERENT brain cue-filters (thematic-role vs animacy/Centering-gated retrieval — Lewis-Vasishth; Grosz-Joshi-Weinstein), so the fix is to DECOUPLE the two candidate sources, NOT to merge them. **LANDED (Q111): `situation_reader.read()` now builds TWO mention views when referent_per_np is ON — role_mentions (referent_per_np source) → events/entities; coref_mentions (coref column) → pronoun anaphora — so `coref_acc` is byte-identical to the OFF reader (fixes the 0.469→0.102 collapse; confirmed 0.5255==0.5255 first-hand). The brief's "merge referents INTO the coref pool" is REFUTED (−0.106 CI-sep).** **⚠️ OWNER DECISION — referent_per_np turned DEFAULT-ON despite a MEASURED board regression, because the regression is a DOWNSTREAM fidelity gap, not a referent_per_np defect.** First-hand no-default-off measurement: with referent_per_np ON, the board's who-did-what AGENT arm (build_events_questions, scores the subject) regresses **0.2519→0.0754** (16 docs; identical event counts → the agent PICK, not detection), while the PATIENT (where coref misses the object) improves +0.336. DIAGNOSED: the reader's role assignment is PURELY POSITIONAL (`_assign_roles`: agent=preverbal mention) — NOT the brain's mechanism — so the denser (more correct) referent set makes it grab a WRONG preverbal NP head. The complete referent set (a discourse referent per NP, Kamp/Heim) IS the brain-foundational upstream → default-ON; the positional agent assigner is the poorly-implemented downstream → **FIXED VIA an URGENT follow-on: `swap_the_positional_role_assigner_for_the_brain_foundational_competition_model` (P2)** — the Competition-Model cue-competition assigner (word-order+animacy+voice+verb-frame; prototyped `exp_brain_upstream_role_v1`, cuts inanimate-agent error 0.333→0.081), which recovers the board agent arm and makes default-on a NET win. HONEST BOARD STATE: qa_events transiently regresses until that lands (the committed baseline_2026-09-04 is now stale — measured with referent_per_np off). **P5's own §9 measured the owner's thesis (each brain-foundational upstream component lifts the coref person-selection chain monotonically, binder-trust −0.048→+0.220), localizing the LAST non-faithful link to the world-knowledge COHERENCE / next-mention prior (Kehler-Rohde) — a deeper frontier.** ALSO FLAGGED (not landed, needs a decision — uses gold nominal clustering): the resolution-overlay-by-discourse-entity bonus (coref +0.043 CI-sep above baseline; keyed by surface token → fragments a protagonist across name variants). LESSON RE-CONFIRMED: the no-default-off first-hand measurement of the CONSUMED metric caught a board regression the submission's "+0.336 inherited" claim hid — the +0.336 was patient-only; the board arm is agent-only.

- **2026-09-04 (INTEGRATION — NORTH STAR: the CONSOLIDATION GATE PASSES (a clean foundation lifts the meaning channel +0.067) and PROVES the ~0.35 ceiling is the FROZEN DISTRIBUTIONAL SUBSTRATE itself, not the knowledge/reader/mechanism — the ONLY brain-foundational way through is the grounded ATL hub-and-spoke. EXCELLENT PARTIAL, owner-DONE).** Problem `build_the_controlled_knowledge_growth_consolidation_gate_for_the_learner` (the north-star P1), reverified **14/14** first-hand (strict document-disjoint SemCor subordinate, n=2676, through the wired `diagnostic_context_wsd` readout). **THE GATE PASSES THE BAR with clean knowledge:** admitting a CONSOLIDATED clean foundation (WordNet relations + curated SyntagNet + ConceptNet — an admissible offline static asset, NO LLM) raises a_s **0.2512→0.3178 (+0.067 CI-sep [0.048,0.087])**, the RAW-ungated reading twin LOSING (regresses −0.033), MFS no-regression guard passing — "the clean foundation before learner-on," DELIVERED. **NARROW LOCATED NEGATIVE (mechanism-complete):** the reader cannot GROW that clean knowledge from its OWN reading glass-box — distributional reading co-occurrence climbs monotonically with syntagmatic tightness (sentence 0.238 → real dependency-parse+discrim 0.251 = gloss, never above), perfect gold sense-attribution 0.232 (coverage-starved), top-down 0.251, bottom-up online competitive-Hebbian 0.183 — NONE crosses gloss. **THE DECISIVE PIN: the ~0.35 ceiling is the SUBSTRATE, not the knowledge** — supervised DISTRIBUTIONAL sense keys themselves cap at 0.35 because senses are SUPERPOSED in frozen w2v (Arora 2018); recurrent attractor settling OVER-collapses (0.202, erases the subordinate) → one-shot biased-competition is the right settle. **GROUNDING RULED OUT AT 12-DIM (an earlier "grounding is the wall" framing corrected):** the 12-dim Lancaster sensorimotor+concreteness norms (raw, ATL-whitened, AND semantic-inheritance-propagated) all = gloss 0.251 — grounding SEPARATES the clear cases (0.813 vs −0.096) but the 12-dim asset is too COARSE at coverage. **THE ONLY BRAIN-FOUNDATIONAL ROUTE THROUGH (pinned): the ATL HUB-AND-SPOKE + ONLINE PREDICTIVE READER** — the distributional spoke BOUND WITH RICHER grounded spokes (Binder ~65-dim + affect + relational, whose dimensions are ORTHOGONAL to distribution), propagated by SEMANTIC INHERITANCE, RE-SETTLED per context by the online predictive reader (one pass, no training); the transformer route (~0.53) is the invariant boundary (ungrounded + batch-trained), NOT pursued. **WIRE LANDED (Q111): the load-bearing wire is a GUARD, not a default-on feature** (reading-derived growth doesn't beat gloss; raw growth hurts) — promoted `consolidate` + `raw_assocs` + `regression_guard` → **`hdlab/consolidation_gate.py`** (the reusable offline admission gate with the raw-vs-consolidated regression check BAKED IN so raw growth can never silently ship; composes with the landed `hdlab/cls_growth` for reversibility). Landing witness `test_consolidation_gate_landing_organ.py` **7/7** (byte-faithful to the experiment + the guard blocks below-gloss admission). Registered `consolidation_gate_admission_guard_v1`. Keep `diagnostic_context_wsd` as-is (top-k did not help). **FOLLOW-ON FILED (the north star this problem earns): `build_the_atl_hub_and_spoke_meaning_channel_online_predictive_reader` (P9).** Supersedes the P2/build_sg_lite framing of the a_s lever: the ceiling is the STATIC SENSE-SUPERPOSED input representation, and the glass-box crosser is the grounded, orthogonal, online-re-settled hub-and-spoke (NOT a bigger distributional store, NOT a transformer).

- **2026-09-04 (INTEGRATION — the calibrated CRF posterior IS the whole tagger lever; the JOINT parse-decode (the brief's named mechanism) is a LOCATED NEGATIVE; and the dependency is dissolved into a glass-box asset. EXCELLENT, owner-DONE).** Problem `upgrade_the_pos_tagger_to_a_calibrated_joint_decoded_posterior` (the P7 I filed this session from the P6 §4c; a solver solved it), reverified first-hand: `test_joint_decode_register_robust` **5/5**, `test_whodidwhat_clean_frame` **3/3**, `test_freetext_event_recall_deployed` **3/3**, + landing `test_crf_tagger_landing_organ` **5/5**. NEW PINNED-AND-MEASURED: the brain's lexical category is a GRADED/CALIBRATED belief (Kuperberg-Jaeger 2016) — a likelihood-trained linear-chain CRF (SAME features + SAME UD-EWT data as the perceptron, only the OBJECTIVE differs; Lafferty 2001) gives a posterior that **separates 19c dropped verbs at AUROC 0.9409 vs the max-margin's 0.58**, recovery 0.8727 @ FP≤0.5 (vs 0.582), MODERN 0.931, argmax verb-recall TIED. **THE JOINT DECODE IS A LOCATED NEGATIVE (the brief's named "deeper build", now RETIRED):** adding force-VERB parse-coherence lifts AUROC by only **+0.0017** (register-robust DELEXICALIZED parser, a no-op at the operating point) / +0.0012 (modern lexical parser, which HURTS 19c: 0.800) — because the calibrated posterior ALREADY captures the separable signal (structure AUROC 0.59–0.62 ≪ posterior 0.94). The delex coherence IS the better structural cue (0.618>0.590 — the parser's register-brittleness is real) but IMMATERIAL; delexicalization is NOT the lever (19c is ~90% in-vocab → word-ORDER gap, and delex costs 8 UAS in-domain). **THE DEPLOYABLE WIN (axis-1), measured END-TO-END on FREE-TEXT 19c** (raw LitBank, spaCy-oracle event gold, n_dropped=538): the deployed detector at a precision-guarded modern-fixed FP≤0.25 threshold recovers **0.898** of perceptron-dropped events (twin 0.182, Δ +0.715 CI[+0.670,+0.760] CI-sep), lifting END-TO-END event recall **0.9382→0.9792 (+0.041)**; parser-downstream, precision-guarded recovery lifts 19c who-did-what REACHABILITY **+0.309 CI[+0.18,+0.45]** on the genuine-drop subpopulation (naive full-pop flooding collapses it — a detector-PRECISION artifact, not a limit). **DEPENDENCY DISSOLVED:** the CRF posterior is reimplemented GLASS-BOX in pure numpy (linear-chain forward-backward), reproducing `sklearn_crfsuite.predict_marginals` to **max|dP(VERB)|=7.3e-7** → ships as a dependency-free static json asset, NO crfsuite/LLM at inference. **WIRE LANDED (Q111): `hdlab/crf_tagger.py` (`GlassBoxCRF.vpost` + `vlogit`, promoted VERBATIM, byte-faithful 0.0 err) + the shipped asset `data/frontend_assets/pos_crf_glassbox_ud_ewt.json`.** The predicate_detector category-cue swap (`verb_margin` → `logit(CRF vpost)`, SS6) is the READY application — `crf_tagger.vlogit` is landed; applying it re-fits the detector's logistic on the CRF cue (predicate_detector is DEFAULT-OFF, so no live-reader change; the calibrated cue lifts its 19c recall 0.582→0.806 when turned on). **NOT landed: the joint parse-decode (located negative) + the delexicalized parser (−8 UAS, immaterial).** The residual is a parser/tagger-FIDELITY gap (spaCy oracle recovers 0.818/0.882 of drops), NOT a meaning ceiling. Refines the P6 event/predicate-detection entry: the deeper-fidelity target is the CALIBRATED posterior (landed), NOT the joint decode.

- **2026-09-03 (STRATEGY FIDELITY FIX — the CAUSAL dimension's below-floor score (0.1485 < adjacency floor 0.5248) was a COVERAGE ARTIFACT, now fixed: the reader's causal organ ran on a SPARSER event set than its own situation model. 0.1485 → 0.905).** Diagnosed first-hand (n=23 board causal questions): **82.6% ABSTENTION, only 1/23 wrong-direction** — the reader was not answering causation backwards, it had NO causal_link to answer WITH. Root cause: `_read_causation` re-detected events via `C.extract → T.extract_events` (the STOCK tense-GATED detector, `tagger=None`), which drops present-tense/gerund verbs the situation model's densified `_extract_events` (tense_agnostic_events + predicate_recall) keeps — so the connective-gold outcome (located in the DENSIFIED `sm.events`) was absent from `sm.causal_links`. FIX (`hdlab/situation_reader._read_causation`, now an instance method): run the reader's causal ORGAN (connective/bridge direction, `C.causal_net_cause` — MECHANISM UNCHANGED) over the reader's OWN densified event stream, recording EVERY connective/bridge link per sentence. MEASURED (6–10 LitBank docs, first-hand): causal **0.1485 → 0.905, abstention 0.826 → 0.000**, beating the adjacency floor **+0.4286 CI[+0.286,+0.571] CI-sep** — the connective DIRECTION ("X because Y" points backward to Y) genuinely beats recency, which is the dimension's stated scope (connective-STRUCTURE recovery, NOT force-dynamics reasoning — that is the separate typed-causation path). CONTAINED: `sm.causal_links` is consumed ONLY by the causal readout → every other dimension is byte-identical. Witness `verification/test_causal_readout_coverage_fix.py` 6/6 (abstention collapse + 109 links vs 13 stock + beats floor + other dims intact). HONESTY BOUND: the readout shares the connective-direction grammar with the gold, so this measures whether the situation model REPRESENTS the text's connective causation (it now does); the adjacency floor + the board's info-free twin are the guards. Expected board effect: reader-QA aggregate ~0.360 → ~0.389 (causal is 1 of 4 scored dims, n=101/2635).

- **2026-09-03 (INTEGRATION — the CONSTRUCTION-AWARE selector adds EXACTLY 0.000: role binding is FEATURE-COMPETITION, not construction-template retrieval, and the deployed selector already IS the brain's mechanism, at ceiling. REFUTED located negative = FULL PASS, owner-DONE).** Problem `construction_aware_selector_for_multi_do_who_did_what_over_the_referent_set` (the P4 I filed; a solver solved it), reverified **11/11** first-hand. NEW PINNED: the brain assigns thematic roles by graded FEATURE-COMPETITION (eADM; Frankland-Greene 2015; Bates-MacWhinney Competition Model), NOT by Goldberg construction-template retrieval — the construction cue is REDUNDANT with word-order on canonical English. MEASURED (three ways, all null): a Goldberg construction-aware selector over the deployed feature-competition theme selector (`hdlab.graded_role_assigner.hybrid_role_patient`) adds **0.000** — selector-level (19c n=669 −0.0030 n.s.; multi-DO n=162 −0.0123 n.s.), END-TO-END through the live `SituationReader().read()` (n=1354, +0.0000 CI[0,0]), and register-invariant (modern QA-SRL n=1261 −0.0008 n.s.); the deployed selector is statistically TIED with a competent reader (0.928 vs spaCy 0.922). **TWO DISK-OUTRANKS-THE-BRIEF CORRECTIONS (verified): (1) the prototype's "+0.146 / 84%-multi-DO" premise was an ARTIFACT of the experimental `ideal_pick` baseline's ANIMACY-OVERRIDE bug — over the deployed selector it is 0.000; `ideal_pick` is itself NET-NEGATIVE vs the deployed `hybrid_role_patient` (0.898 vs 0.928). STRIKE the parent `open_a_discourse_referent...` NEXT-STEP #1 ("land construction-aware selector 0.873→0.913"); DO NOT adopt `ideal_pick`.** WHERE SIGNAL IS LOST (waterfall, ours vs brain): the deployed reader's dominant loss is the SOURCE (coref covers the gold patient only 0.818; referent-per-NP → 0.971, gated on the coref linker = P5), NOT the selector; on the ideal chain the loss is S3-parse (56% of residual = the filed parser + register-native POS) + a ~3.1% irreducible ill-posed/gold-noise tail a competent reader also misses. **NO hdlab WIRE (located negative — the selector is at ceiling).** ONE buildable upstream win, PROVEN + deferred to the P5 source wire: extend the referent-per-NP SOURCE to INDEFINITE-PRONOUN heads (DRT introduction; promote `INDEF_PRON` from `exp_construction_ideal_composition_v1`) — whole-composition 0.9283→0.9387, **+0.0105 CI[+0.0015,+0.0209] CI-sep, info-free twin loses, no single-DO regression** (19c-specific; a Right-hand-Head-Rule structural fix is a DOCUMENTED NULL — 2 fixes / 2 breaks, both register POS-tagger noise → the parse-recoverable lever is the register-native POS tagger, not a selector patch). Ranked next problems (filed / to file): referent-per-NP source turn-on (gated on the coref linker, P5) > register-native POS (P7-adjacent) > filler-gap clefts > meaning-fit for the gated tail.

- **2026-09-03 (INTEGRATION — the "which SPECIFIC rare sense" ceiling is NOT the encoder, the readout, or the mechanism SHAPE: it is the QUALITY×COVERAGE of the world-knowledge connection matrix W. The transformer fork is REFUTED as the brain answer; a perfect sense-discriminative W solves it (a_s 0.995 oracle). LOCATED NEGATIVE + a decisively-proven, NUMBERED lever. EXCELLENT PARTIAL, owner-DONE).** Problem `break_the_contextual_input_encoding_ceiling_for_specific_sense_selection` (the P2 I filed; a solver solved it), reverified **10/10** first-hand (CPU, NO GPU/LLM; strict document-disjoint SemCor, subordinate senses, subject-weighted a_s, n≈2676). **NEW PINNED (frame correction): sense selection is NOT lexical WSD (classify-then-weight) — it is KINTSCH CONSTRUCTION-INTEGRATION joint settling over a world-knowledge graph W, where RELEVANCE == connection strength (learned OFFLINE, never a per-instance relevance verdict) and the sense is the coherent fixed point (Kintsch 1988; Waltz-Pollack 1985; Vu-Kellas; Hoffman-McClelland-Lambon-Ralph 2018).** MEASURED: (1) THE BRIEF'S ROUTE FAILS — a self-supervised glass-box CONTEXTUAL encoder (context2vec-style bidirectional LM, OUR model, 41M tok, GPU-trained) best a_s **0.293 < the wired diagnostic readout 0.307–0.317** at matched 41M scale, twins lose; **the transformer "fork" the brief named is REFUTED as the brain answer** (Tang-Sennrich-Nivre 2018: transformers cross via rich learned reps, NOT relevance-selection → encoder scale is irrelevant to the lever). (2) THE READOUT MECHANISM IS NOT THE GAP — the brain's exact iterative joint constraint-satisfaction settling **= the one-shot readout (0.312=0.312, not sep; McClelland 2013 equivalence)**; dominance-weighting HURTS subordinate (0.251). (3) SIGNAL-LOSS FULLY LOCATED (oracle decomposition): KEY-unwinnable **0.000** (a sense's own gloss ~always separates it), QUERY-loss **100%**, oracle-context-query ceiling **0.852** (the disambiguating cue IS ~85% present in the plain w2v context) — so it is neither a key problem nor sense-conflation. (4) THE DECISIVE PROOF — the lever is W, quantified: TOPIC-relatedness W (gloss/SyntagNet/ConceptNet/co-occurrence, all we can currently build) reinforces the DOMINANT sense → C-I settling over it caps ~0.22–0.31; a **LEARNED sense-discriminative W (document-disjoint SemCor) already BEATS topic on covered senses (0.367 vs 0.308, +0.059, twin-sep)**; a **PERFECT sense-discriminative W → a_s 0.995 (oracle upper bound)** — the 0.31 ceiling was 100% W-QUALITY, never the encoder/readout/mechanism. The binding constraint is **COVERAGE** (only 52% of test senses seen at tiny SemCor scale → learned-W overall 0.191, coverage-dragged). **NO hdlab WIRE (located negative): the wired `diagnostic_context_wsd.py` remains the best readout — do NOT land the encoder / grounded / settling / C-I arms (all tie or lose).** Refines the build_sg_lite §2b entry (the a_s lever): the fix is a broad-coverage, CLEAN, SENSE-DISCRIMINATIVE W, NOT a bigger/contextual encoder. **REDIRECT (proven + numbered) → P1 `build_the_controlled_knowledge_growth_consolidation_gate_for_the_learner`:** grow a W that is sense-discriminative (which words indicate THIS sense over its competitors — topic reinforces the WRONG sense), CLEAN (ConceptNet-noise regresses −0.004), and broad-coverage (each covered sense +~0.06 over topic; oracle ceiling 0.99); acceptance test scaffolded (`exp_sg_lite_sense_discriminative_W_headroom_v1`: recover ≥50% of the 0.31→0.85 headroom on the FULL population as coverage grows). HONEST BOUND: the full-population positive cross is NOT achieved (learned W is coverage-limited); this PROVES + QUANTIFIES the target, it does not build the store. Also: the discourse/situation-model prior (Stage 4) is a source of a better W, not a separate readout; semantic-control gain (C3) HARD_FAIL is a B4 estimation-capacity block, re-openable when representation capacity improves.

- **2026-09-03 (INTEGRATION — the COPULAR is-a/attribute capability now has a LIVE QA CONSUMER and turns DEFAULT-ON: "what/who is X" is answerable where the base reader scored a live 0. EXCELLENT, owner-DONE).** Problem `wire_the_copular_state_qa_consumer_and_turn_on_bind_entity_states` (the P3 I filed; a solver solved it), reverified **9/9** first-hand PRE-landing. NEW PINNED-AND-MEASURED: a copular predication files a PROPERTY/CATEGORY on the subject's ENTITY node (Higgins 1979; Maienborn 2005 Kimian states; Bemis-Pylkkanen 2011 LATL), on the queryable ENTITIES dimension (Zwaan-Radvansky 1998); "what was X" is answered by CONSULTING that stored record (mental-model read-out; Glenberg 1987, Kintsch 1988), the question SELECTING the store by content-addressable retrieval (Lewis-Vasishth 2005; the wh-word sets the Higgins answer-TYPE = the router) — NEVER by re-scanning the text. MEASURED (UD-EWT copular gold, NON-CIRCULAR = gold from GOLD deprels, n=378 predicational): the wired state-QA consumer (route "what/who is X" → sm.state_register.state_at) scores **qa_state 0.7116 CI-sep +0.140 [+0.087,+0.196] over the recomputed most-recent-noun floor (0.5714), shuffle-holder twin (0.49) LOSING +0.209 CI-sep**; the base reader (bind OFF) scores **0/378** (the live can-fail zero); router hit 1.000, ablate the frame → 0. WATERFALL: read-back GIVEN the binding 0.996, routing 1.000 → the consumer is LOSSLESS, the whole residual is UPSTREAM detection. **TWO COUPLED CHANGES LANDED (Q111): CHANGE 1 — the consumer wire (in experiments/exp_situation_model_qa_v1.py: DIMENSIONS+='state', copular-frame router, _answer_state, build_state_questions) + flip `bind_entity_states` DEFAULT-ON in hdlab/situation_reader.py (no-default-off: net-positive on a CONSUMED metric, PURELY ADDITIVE — the 4 scored dims events/coref/timeline/causal byte-identical off vs on, +~5ms/read; qa_aggregate 0.2811→0.4657 on turn-on). CHANGE 2 — the UPSTREAM detection fix: promoted `robust_cop` (the label-ROBUST closed-class copula detector; the `cop` labeler's recall is the dominant loss, worst on nominal is-a) → hdlab/copular_binding.py and unioned it into `_read_entity_states` (bind | robust_cop). Measured FIRST-HAND through the LIVE reader: qa_state 0.7011 (label) → 0.826 (robust_cop, now the landed default) CI-sep, concentrated on is-a (pred_nom +0.184).** Landing witnesses: test_copular_is_a_binding_landing_organ.py 6/6 (default-ON + union byte-exact + read-back + factory-off) + test_state_qa_consumer_organ.py (W9 made landing-aware: robust_cop is now the default; the pre-landing 9/9 label→robust_cop→arc-eager 0.701→0.826→0.848 is the submission proof). Registered `copular_state_qa_consumer_and_robust_cop_wire_v1`. **OPTIMIZATION NOT LANDED (modern-only): the arc-eager tree stacks +0.032 (0.826→0.848, pred_nom 0.806→0.903) but is 19c-negative — needs PER-REGISTER parser routing (19c=July tree, modern=arc-eager); deferred as a measured follow-on.** Two brain-differences remain filed follow-ons: within-clause SURFACE-token keying vs CROSS-SENTENCE canonical-entity binding ("what is Ahab" fails once he is "he"; hippocampal concept cells), and identity→coref-merge (symmetric/CA3). HONEST BOUND: the powered qa_state is MODERN (UD-EWT — the only non-circular is-a gold); on 19c the evidence is COVERAGE (LitBank live-fires 295/296), not a floor-separated score. Refines the copular §2b entry below: the entity-state READ-OUT is at ceiling given the binding; the lever is the producer's DETECTION recall (robust_cop, landed) + a register-robust parser.

- **2026-09-03 (INTEGRATION — EVENT/PREDICATE DETECTION is REGISTER-BRITTLE: the live tagger silently DROPS a whole clause when it mistags a real verb, and a LEARNED noisy-channel predicate detector recovers it — the parent's "modern override 0.16" was a SINGLE-CUE artifact, not a fidelity ceiling. EXCELLENT, owner-DONE).** Problem `register_robust_event_detection_the_reader_drops_events_when_the_tagger_misses_the_verb` (the P6 I filed; a solver solved it), reverified **12/12** first-hand. NEW PINNED-AND-MEASURED: predicate-hood is NOT a static per-lexeme tag — category + structure settle JOINTLY via continuous multi-cue constraint integration (MacDonald 1994; Fromont/Steinhauer/Royle 2020 — N400+P600 additive, no syntax-first ELAN); the predicate slot is anticipated from register-invariant closed-class scaffolding + agreement + argument semantics (Altmann-Kamide 1999; Kuperberg-Jaeger 2016) and MUST generalize (structure-building fires on Jabberwocky — Fedorenko; 2-year-olds slot invented verbs from frame alone — Yuan 2011). The computational form is Gibson (2013) noisy-channel: category = lexical LIKELIHOOD × structural PRIOR, learned jointly. A glass-box 7-weight LOGISTIC over register-invariant cues (tagger verb-margin as likelihood × frame/morphology/one-verb-per-clause competition as prior), trained SELF-SUPERVISED on modern auto-labels (NO LLM), recovers tagger-DROPPED real verbs @ FP≤0.5/sent: **MODERN 0.899 (Δvs-twin +0.333 CI[+0.21,+0.46]), 19c-TRANSFER 0.5625 (ZERO 19c labels, +0.539 CI[+0.44,+0.64])**, info-free random-verbhood twin losing CI-sep on ALL registers; **crosses the parent's structure-only modern wall (0.16→0.899)** because the parent used ONLY structure (the cue CORRUPTED by the mis-tag) — the ablation shows the multi-cue COMBINATION earns +0.132 on 19c exactly where the lexical margin is register-brittle. No-regression is BY CONSTRUCTION (additive-only: promotes only dropped tokens; existing detections + role picks byte-identical). **RESIDUAL NAMED (SS4b, competent-reader oracle, reference-only): the 19c residual is a FIDELITY gap (a competent reader recovers ~100%; closable by a joint-decoded tagger), NOT a ceiling; the genuine semantic/discourse ceiling is ~33% of the MODERN drops (needs the meaning hub, P1).** DEEPER-FIDELITY TARGET PROTOTYPED (SS4c): the mechanism-diff is max-margin-vs-graded (axis-1, PINNED) — retraining the SAME tagger with a LIKELIHOOD objective (a CRF, same features/data) gives a CALIBRATED posterior recovering 19c at 0.806 (vs 0.582 max-margin, +0.224 of the fidelity gap) at modern 0.955, argmax recall tied; joint parse-coherence helps modern (→0.966) but is register-brittle on 19c (parser also modern-trained) → the fully register-robust fix is a likelihood-trained JOINT-DECODED tagger-parser (Bohnet-Nivre 2012 with CRF objective). **WIRE LANDED (Q111, default-off `predicate_recall`, witness 15/15, commit pending): `hdlab/predicate_detector.py` (`PredicateDetector`, pure-Python logistic reproducing the validated asset to 1e-16, NO experiments dep, NO LLM) + the shipped asset `data/frontend_assets/predicate_detector_ud_qasrl.json` + an ADDITIVE `predicate_recall` flag in `situation_reader._tense_agnostic_extract`.** ⚠️ KEPT DEFAULT-OFF (measured turn-on, no-default-off): on 16 LitBank docs, ON vs OFF is who-did-what 0.2519→0.2530 (+0.0011, n=1830, NOT CI-sep = flat) with coref 0.5688/coref_acc 0.5255 UNCHANGED (zero consumer regression — the additive records are EVENT predicates not new coref mentions, unlike referent-per-NP) — the board's who-did-what eval SUPPLIES the clean main-verb slice so the recovery (free-text event-recall, up to 4.4%+ dropped) is invisible, while at the modern threshold it fires ~1.4 FP/sent on the ~76%-contaminated 19c gold → default-on gives no measured board benefit for real event-stream noise; turn on when a free-text event-recall consumer exists. **Mark event/predicate DETECTION verb-ID:** the post-hoc single-cue override is SUPERSEDED by the learned multi-cue combiner; the candidate gate (WordNet verb-reading) is a static-lexicon deviation but the tokens it misses on this population are gold noise (v2 morphological gate = documented negative, 0 real novel forms); the deeper-fidelity target is the JOINT-DECODED POS+parse. **UPSTREAM IMPROVEMENT FILED (owner "post as problems"): `upgrade_the_pos_tagger_to_a_calibrated_joint_decoded_posterior` (the CRF calibrated posterior + the joint tagger-parser — one lever, two payoffs: the tagger's 19c verb recall AND the parser's register-brittle coherence cue).**
...
- **2026-08-31 — COMPONENT SCAN + CROSS-CUTTING INSIGHT (strategy, verdict-independent): `location_register` (SPACE) is a
  ...
  1998 event-indexing SPACE; hippocampal place / entorhinal grid; narrative space is CATEGORICAL BY DEFAULT →
  categorical topological scene nodes, with METRIC available ON DEMAND (Rinck 1997 Exp 3 — NOT metric coords ruled out outright; phrasing corrected 2026-09-06, p6 spatial-relational integration), OUR-INVENTION-swept but well-grounded). A genuine brain-computation organ, not a
...
- **2026-08-28 — THE MISSING SPACE DIMENSION IS NOW BUILT: a first-class per-entity LOCATION REGISTER (Zwaan & Radvansky
  ...
  **NOT a manner-verb whitelist** ("she florped out" still departs via the satellite — the whitelist is the
  ...
  REPRESENTATION — categorical **topological scene nodes** by default, metric ON DEMAND (Rinck, Hahnel, Bower & Glowalla 1997 Exp 3 — narrative space is categorical-by-default, NOT metric/Euclidean coords ruled out outright; corrected 2026-09-06, p6 integration), with an FHRR-bound alternative giving the identical answer (round-trip cos 1.000 → the
