...
- **The tagger reroute COARSENED the fine tense labels** (surfaced 2026-09-06 while refreshing the reader module self-tests). The 2026-09-03 NLTK→hdlab tagger reroute (an integrated perf/coverage win — event recall genuinely IMPROVED, e.g. comma-fronted gerund `resenting` is now recovered) tags several non-canonical tokens differently, so the POS-conditioned fine tense branches in `experiments/_temporal_ordering.py` no longer fire: `PARTICIPIAL` (talking/protesting), `MODAL_SUBORDINATE` (modal + bare-infinitive `gain`), and coordinated-2nd-conjunct `PAST_PERFECT` propagation (`cherished`) all collapse to `OTHER`/`SIMPLE_PAST`. The 1st-conjunct direct-aux `PAST_PERFECT` (`owned`) still fires. **Whether this costs MODERN temporal comprehension is UNMEASURED** — the temporal board dim is 19c (banned) and no modern temporal gold exists yet (named gap). **FINAL:** the temporal-reasoning problem must measure temporal accuracy WITH vs WITHOUT fine tense labels on a modern gold, and if load-bearing, restore the fine labels via a tense classifier that doesn't depend on the coarser tagger's POS (or re-derive tense from the arc-eager parse, not raw POS). **Waits on:** the modern temporal gold + the temporal-reasoning build. ⟶temporal tier. *(This is exactly the "confirm no downstream consumer regressed" check on the tagger-reroute upstream: coverage↑, tense-granularity↓, net effect on modern temporal = to be measured.)*
  - **UPDATE 2026-09-06 (p5 temporal INTEGRATED `119f8f0ad` — the coupling is now LARGELY DE-RISKED):** the p5 temporal reasoner achieves a CI-separated MODERN before/after win (TB-Dense 0.5933→integrated 0.6225, +0.099) via the CUE + TIMEX event-local anchoring + transitive-closure + aspect-interval channels — NOT via the coarsened POS-conditioned fine tense labels. So those collapsed labels (coordinated-`PAST_PERFECT`/`MODAL_SUBORDINATE`/`PARTICIPIAL`) are demonstrably NOT the load-bearing lever for modern temporal reasoning (the reasoner routes around them). Separately, p5's `hdlab/aspect_interval.py` RECOVERS the dropped finite PROGRESSIVE (recall 1.000 vs gold TimeML) — the overlap-relevant slice of the same tense-granularity gap — with the point-order event set byte-identical. **RESIDUAL (low priority):** the coordinated-conjunct/modal-subordinate/participial tense labels still collapse, but no live consumer depends on them post-p5; restore them only if a future consumer needs the fine label (re-derive from the arc-eager parse). Effectively RESOLVED for the temporal tier.
