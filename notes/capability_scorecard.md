...
- 2026-06-05 08:25 (**FLAGSHIP -- 11th anchor; FIRST REAL-LLM-DATA SUBSTRATE WIN**): **EX-CONCEPT-1 REAL HARD_PASS smoke**. VQ Pythia per-token residuals -> concept-ID sequences -> substrate Hebbian next-concept-LM + cleanup. **substrate_top1=0.613 vs unigram=0.037 (16.3x) AND >= bigram-Markov 0.596**. Substrate captures REAL Pythia-LLM concept transitions (not just synthetic proxy). This is the first empirical proof that substrate-cognitive-core can be trained on REAL LLM data and outperform sequential baselines. Per-token extraction also HARD_PASS (after watchdog fix; 120s timeout was killing savez_compressed of ~300MB array mid-write; raised to 3000s).
...
- 2026-06-05 11:00 (hourly cadence): substrate cognitive-core empirically anchored at **11 flagship validation points**. **First REAL-LLM-data substrate win (EX-CONCEPT-1 REAL HP)**. Real KG multi-hop traversal validated (CCC-1-EXTRA). Critical path: CCC-1 REVISED-v2 next (7-benchmark eval; load-bearing Phase 1 test for Wikipedia substrate path).
