...
- **The named decision that is NOT worth acting on (corrected 2026-09-06 after owner challenge):** (a) the common→named encyclopedic is-a slice (`Argentina instance-of country`) is a **fraction of ~8%** of anaphoric common nouns, and the drill's own dominant lever is the SITUATION-MODEL chain (63% of the gap), NOT a KB (the generic-KB seed ALONE was a located negative, ~2%). Downloading a Wikidata/DBpedia InstanceOf dump to patch it is **low-leverage AND a non-brain-foundational shortcut** — the brain does not consult a downloaded encyclopedia; it reads is-a off a semantic memory it GREW from reading, so the brain-foundational supply is the ONLINE LEARNER + the existing curated foundation's coverage, not a batch external dump. **DO NOT acquire the NE-type KB as a fix.** If this slice is ever *measured* to be the binding residual AFTER the recurrent loop is closed, the brain-foundational move is to grow it from reading / check existing foundation coverage — and even then it is a narrow slice. (The prior draft recommended acquiring it; withdrawn.)
- **Fix:** wire `event_type.py` as the non-cued temporal-ordering prior. (No KB acquisition.)
...
4. **Correct the "barred" shorthand** in `BRAIN_FOUNDATIONAL_AUDIT.md` ("needs a static glass-box KB = admissible" vs "needs a trained LLM = barred"). NOTE: this is a framing correction, NOT a mandate to acquire an encyclopedic NE-type KB — that slice is low-leverage AND a non-brain shortcut (withdrawn above); the brain-foundational supply for any is-a coverage gap is the online learner + the existing foundation, not a batch dump.
