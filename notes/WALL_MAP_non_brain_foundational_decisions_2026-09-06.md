...
- **The two narrow named decisions:** (a) a **static named-entity TYPE KB** (Wikidata/DBpedia InstanceOf: `Argentina instance-of country`) was labelled "invariant-barred" and NEVER TESTED — the on-disk ConceptNet 100k subset has NO IsA edges (measured), so it's an **asset-ACQUISITION gap, not an invariant bar**; (b) non-cued temporal ordering deferred "Phase-1-gated" though the glass-box **event-schema** route (already built as `event_type.py` for causal) crosses it.
- **Fix:** (a) acquire a static NE-type KB (Wikidata InstanceOf) — **owner go/no-go** (a new foundation asset, admissible); test the common→named is-a slice, shuffled-KB twin control; (b) wire `event_type.py` as the non-cued temporal-ordering prior.
...
4. **Owner go/no-go:** acquire a static named-entity TYPE KB (Wikidata InstanceOf) to test the one untested "barred" slice.
5. **Correct the "barred" shorthand** in `BRAIN_FOUNDATIONAL_AUDIT.md`.
