...
- **IN FLIGHT: exp_dev fixing the training budget (raise smoke to ~200-400 optimizer steps via smaller batch + more epochs; add clip_grad_norm_=1.0; audit FULL step-count too), self-test, commit, re-smoke REMOTE. Mechanism/arms/constructions/controls UNCHANGED — pure training-budget + stability fix. Director reads the landed re-smoke off disk.**
- **THEN (do NOT ship the GPU full run until the re-smoke's discriminator FIRES): if the trained arms clear chance on the re-smoke, ship the GPU full run.**
