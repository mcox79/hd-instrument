...
> ## 🚨 **THE SHELVE HALF OF THE DURABILITY GATE IS HOLLOW: `24` OF `42` SAY NOTHING AT ALL (08-23)**
> **The gate reads *"WIRE (target + step noted) or SHELVE (explicit revival criteria) -- nothing stays in limbo"*. Of `212` registry rows, `42` carry `gate_decision: SHELVE` -- and `24` of them (`57%`) have a COMPLETELY EMPTY `gate_decision_target`.** *No detector, no interpretation: the field is blank.* A further `13` carry explanatory text with no forward-looking language (keyword-based, **soft**, controls pass both ways); only `5` name a condition that would bring the capability back. 🚫 **DO NOT QUOTE "37 of 42 abandoned" -- the unambiguous number is `24`.**
> 🔑 **THIS IS THE GATE FAILING, NOT PAPERWORK MISSING. A shelve without a trigger is indistinguishable from abandonment AND LOOKS LIKE THE GATE WAS SATISFIED, because the row carries a `gate_decision`.** *We have already paid: the row that exposed it says it was registered only "after the owner had to recall this work from memory twice because the registry-first check returns nothing for it" -- an 11x lift on the hard population (`0.0262` -> `0.2930`) recovered by human memory rather than by the mechanism built for it.* **The shelved set is NOT junk** -- it includes `hdlab_encoder_cluster_vwfa_ppmi_composed_v3`, `binder_direct_supply_grounding`, `entity_slot_gate_cross_boundary_v1`, `reasoner_composed_entry_arc_program`: **organs on the goal-bearing line, set down with no stated way back.** 🚫 *The gate decision says NOTHING about quality -- that 11x row was shelved explicitly "NOT because it is weak".*
> ✅ **THE MECHANICAL HALF IS DONE (`60d9e0eba`): `session_start_hook.registry_report()` now reports the count EVERY SESSION**, in the hook rather than a cron because this repo's durability anchor is the session-start read (11 scheduled tasks once sat silently disabled for 12 days). **IT REPORTS AND DOES NOT FAIL, DELIBERATELY: a failing hook would tempt whoever hits it to fill 24 fields with plausible text, which passes the check and buries the problem properly** -- the message says *do not bulk-fill* for that reason. *Verified against an INDEPENDENT count: 24 by a separate script, 24 by the hook, and the 18 rows with real target text correctly left alone.* 🔻 **THE RETROACTIVE HALF -- 24 per-capability judgements -- IS NOT DONE AND I WILL NOT FAKE IT.** `THE_SHELVE_HALF_OF_THE_DURABILITY_GATE_IS_HOLLOW_...`
...
> 🎯 **MAKES THIS PLAN'S OWN PREDICTION UNREACHABLE: the `way`-attractor (`17.7%` of grounded terms) was predicted to FALL as vocabulary grows. IT CANNOT GROW ACROSS RUNS. The degeneracy is ARITHMETIC, not tuning or scale.** 🧠 *Consolidation is DEFINED by persisting across episodes; we have no slow store at all -- the persistence half of the 08-19 replay gap, one dead attribute wide.*  ⚠️ **NOT MEASURED: whether loading helps. THAT IS THE EXPERIMENT.** 🚫 *Do not quote `92 -> 92`; 120 sentences is below the 100-400 grounding threshold.* `THE_ASSEMBLED_SUBSTRATE_NEVER_LOADS_A_FOUNDATION_...`
>
