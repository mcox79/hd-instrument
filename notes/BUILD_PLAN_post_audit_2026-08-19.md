...
> 🔻 **CORRECTION TO MY OWN ALARM, CHECKED RATHER THAN ASSUMED: THE DELETIONS ARE NOT DATA LOSS. `git show HEAD:<path>` returns them intact** (verified on `exp_phase05_probe_training_v1`: 1,007 bytes, readable), **and git history keeps them even if the deletion commits.** *The same is true of the re-dated records -- my preservation note makes them convenient to read, it does not rescue them from oblivion. **I said DATA LOSS; the accurate word is MISLABELLING.** The re-dating still matters because tools read the FILE, not the history.* Still worth a decision, no longer worth alarm.
> 🔻 *The 3 deleted are:* `exp_cornerstone_c1_c2_c3_aggregate_llama_3_1_8b_v1_h100`, `exp_cornerstone_c2_c3_audit_llama_3_1_8b_v1_h100`, `exp_phase05_probe_training_v1` -- **the LLM-comparison baselines.**
> ✅ **PRESERVED, SO THE DECISION IS NOW REVERSIBLE EITHER WAY: `notes/TRUE_LANDED_TIMESTAMPS_BEFORE_THE_08-22_REPLAY.md` holds the TRUE `ts_iso` and `elapsed_s` of all 54 re-dated records, read from `git show HEAD:` while the overwrite was still uncommitted. Oldest true date `2026-07-04` -- SEVEN WEEKS of records being re-stamped as today.** *Recorded rather than reverted: the files are a concurrent session's and a revert could destroy real work.*
> ⚠️ *Precise breakdown, correcting my own first pass: `63` modified + `3` deleted; `62` had diffs touching ONLY `elapsed_s`/`ts_iso`, and of those `54` have an actually-changed `ts_iso`. Two extractions, two questions, both stated.*
...
>
> ## 🔎 **THE GOAL-BEARING LINE -- v1-ERA DETAIL, FOLDED. THE PROHIBITIONS ARE THE PART THAT MATTERS.**
> *Superseded in PRIORITY by the v2 measurement above, not WRONG. Numbers kept; reasoning in the named notes.*
> 🚫 **FOUR STANDING PROHIBITIONS -- DO NOT RE-PROPOSE ANY OF THESE:** **(1) DO NOT repair `referent_mismatch`** -- it returns `UNMET` in `8 of 8` (a DEFAULT, not an inference) and its ceiling if made PERFECT is `0.6111`, BELOW the `0.6389` floor. **(2) DO NOT "supply verb-class knowledge so the cascade can decide"** -- tried TWICE, bought `1` of `23` (`exp_verbclass_backoff_coverage_v1` HARD_FAIL, `v2` PARTIAL). **(3) "MAKE THE CASCADE FIRE" IS RETIRED** -- when it does fire it is `10/19 = 0.5263`, binomial vs a coin flip `p = 1.0000`; COVERAGE WITHOUT DISCRIMINATION BUYS NOTHING. **(4) DO NOT "flip the UNMET branches to MET"** -- `0.5556`, still below floor, and it FITS THE TEST SET.
> 🔑 **THE STRUCTURAL FINDING: MET recall is `8/23`, `7/23`, `8/23` -- INVARIANT across a 7x lexicon-size change, 96%-vs-50% polarity, and learning-vs-none. ALL movement is in the UNMET column.** *And NO rule ever judges an item: every structural tier emits a CONSTANT (`same_class_same_referent` -> MET x9 at `7/9`, the one working part; `referent_mismatch` -> UNMET x8), so ALL signal is in WHICH RULE FIRES -- and the UNMET branches run `4/11`, WORSE THAN SILENCE.*
> 🔻 **Q104 WITHDRAWN** (credit is ALREADY role-based; `_credit_targets` bounds each verb's clause). **The real exposure was `_is_verblike`: recall `0.6026`, `3,528` of `8,877` verbs NEVER SEEN.** *The tagger swap moved `0.4722 -> 0.6389` = EXACTLY the majority floor, and McNemar refused it (`p = 0.1460`, `9` fixed / `3` BROKEN -- it CHURNS).*
> 🧠 **MISSING PRIMITIVE, and the causal claim is UNCONFIRMED:** in all 5 `referent_mismatch` errors someone ELSE performs the act, so the goal-holder is RECIPIENT / BENEFICIARY -- never SUBJECT or DIRECT OBJECT, and `goal_typing.py:776-777` implements exactly those two. ⚠️ **I hand-read that while HOLDING the hypothesis and BOTH objective tests failed to test it. The CODE GAP is established; the CAUSAL STORY is not.**
> ✅ *`AMBIGUOUS` is an ABSTENTION in 5 consumers and a wrong answer in ONE -- the cell this line is graded on, by omission. Guard: `tools/score_with_abstention.py` (6/6), which FAILS the day the population acquires one. Now filed as `score_counts_abstention_as_error`.* `THE_ORGAN_DOES_NOT_ANSWER_WRONG_...` `MET_RECALL_IS_8_OF_23_...` `THE_CASCADE_IS_A_COIN_FLIP_...`
...
>
> **DO NEXT. 🚫 THE FILED PROBLEMS ARE THE SOLVER'S -- `notes/problems/README.md` HOLDS THE RANKING AND IS THE SINGLE SOURCE. DO NOT MIRROR IT HERE; A RANKING KEPT IN TWO FILES DRIFTS, AND MINE ALREADY DID (I listed 5; it now holds 8).**
> **A.** 🟢 **Score the goal-bearing organ on the v2 bank (`124` scorable).** The n=36 constraint that capped every margin on this line is gone and the organ's level on v2 is UNMEASURED. *Cell run, authorized by Q113. NOT a filed problem -- this one is mine.*
> **B.** 🔻 **WITHDRAWN AS MINE -- B3's scored path is now FILED as `cortical_read_has_no_scored_path`.** *The evidence I gathered stands and the brief should carry it: B3' is live and consolidation-sensitive (`8/8` probes, positive control identical `8/8`), within-pool ranking `+0.0675` NOT_SEPARATED, and pool selection REFUTED once the floor is concreteness-matched (`0.1966` vs `0.1891`, `p = 0.42`).*
> **C.** 🔻 **WITHDRAWN AS MINE -- the cert-gate question is FILED as `certification_gate_hangs`.** *Evidence handed over in `THE_CERT_GATE_IS_SLOW_NOT_HUNG_...`: collection completes in `34.7s`, the two slowest witnesses PASS in `504s` for the pair, the "flat CPU" reading is the documented subprocess false alarm -- **but a real `F` exists at ~witness 18 of 32 and is unidentified.***
> **D.** Fold this state block (`176` lines vs a `160` cap) -- the residue is v1-era detail, superseded in PRIORITY but not WRONG, so it folds to pointers rather than deletes.
> **E.** 🔑 **INTEGRATION IS THE STANDING JOB AND IT IS MINE ALONE:** re-verify each `SOLVED.md` on the ARTIFACT (never by re-running the solver's pipeline -- a re-run shares its bugs), append the `INTEGRATED_BY_STRATEGY` block, and **PREPEND a `SOLVER REVIEW` verdict the owner will actually see.** *2 done: `eval_bank_too_small` EXCELLENT, `stored_terms_are_stems` STRONG.*
>
