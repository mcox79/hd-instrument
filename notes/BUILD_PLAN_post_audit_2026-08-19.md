...
> **First measurement of `congruence_with_lexicon_fallback` on v2's `124` scorable items. Verified on disk, not relayed.**  **accuracy `0.2339` against v2's OWN majority floor `0.6048` (class = `unmet`).** 🔑 **THE CONFOUND I SENT IT TO CHASE DID NOT OCCUR: the organ is NOT riding the flipped majority class -- MET recall `10/49` (`0.2041`) and UNMET recall `19/75` (`0.2533`), BOTH LOW.**
> 🔻 **THE DRIVER IS REFUSAL, NOT ERROR: `82` of `124` (`66%`) predictions are non-committal** (`NONE` 80, `AMBIGUOUS` 2). *It is not guessing wrong two-thirds of the time; it is declining to answer.*
