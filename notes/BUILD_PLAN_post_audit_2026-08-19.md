...
## 🎯 **A PATTERN WORTH GENERALISING (08-22): AN ORGAN SCORES `48/48` ON ITS CONSTRUCTED BANK AND `0.5682` ON REAL PROSE -- BELOW `majority`.**
...
> ✅ **THE EXACT TIE WITH `recency` IS COINCIDENCE, CHECKED NOT ASSUMED:** per-item agreement `recency 0.548` vs `first_mention 0.742` -- **it is NOT recency in disguise; it behaves like first-mention and departs from it WRONGLY.** *An accuracy tie is not a behavioural tie.*
> 🔑 **WHY IT WAS NEVER SEEN, AND THIS IS THE GENERALISABLE PART: the organ's witness is THOROUGH AND PASSES -- `before_never_typed 15/44 -> 13/44`, recovered items named, `29/44` picks byte-identical, no regression. ALL OF THAT IS ABOUT *CHANGE*, NOT *LEVEL*. Nothing in it computes accuracy, and the floors live in a DIFFERENT FILE.** ➡️ **TWO HALVES IN ONE REPO, NEVER MULTIPLIED TOGETHER. The finding needed NO new data.**
> ⚠️ **NEXT LEAD: how many other promoted organs have a constructed-bank score and NO real-prose comparison against a documented floor?** 🚫 *`48/48` is a constructed-bank number and may NOT be quoted as owner-selection ability; `first_mention 0.7955` is now the floor on real prose.* `THE_GOAL_OWNER_ORGAN_SCORES_48_of_48_...`
