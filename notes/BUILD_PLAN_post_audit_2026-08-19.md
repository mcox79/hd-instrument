...
>
> ## ⚠️ **`54` LANDED RECORDS RE-DATED BY A REPLAY (08-22, a concurrent session's).** **MISLABELLING, NOT DATA LOSS** -- `git show HEAD:<path>` returns every one intact. It matters because tools read the FILE, not the history. ➡️ **Owned by Q115 now: new cells gated, `1,413` replayers inventoried and triaged.** 🚫 *Not committed or reverted by me.*
> ## 🧠✅ **THE PRIORITY-1 FIX COVERS OUR VERB HOLE -- AND ITS BRIEF NEVER MENTIONED VERBS (08-23)**
> **On SimVerb-3500, the benchmark our own verb zero was measured on: the sensorimotor norms read `+0.3107` `[+0.2822,+0.3390]` vs null p95 `0.0304`, covering `3,487` of `3,500` (99.6%); ours reads `+0.0000` covering `2,651`.** ⚠️ *Different coverage -> NOT a subtraction; it licenses "ours is ABSENT where this one is PRESENT", nothing more.* Replicates at `+0.3109` on SimLex's 222 verbs -- **two independent verb benchmarks to three decimals.** On SimLex, verbs (`+0.3109`) sit just below nouns (`+0.3469`) while ours falls weak->nothing: **the verb hole is OURS, not the world's.**
> 🧠 **AND A BRAIN PREDICTION THAT COULD HAVE FAILED, HELD: ACTION − PERCEPTUAL on verbs = `+0.0651` `[+0.0306,+0.1005]`, CI-SEPARATED** (paired, 3,487 pairs). Somatotopy -- *kick* recruits leg motor cortex -- showing up in an offline norm table. **At `n=222` the same test read `[-0.0989,+0.2031]` and said NOTHING; the fix was POWER, not more careful wording.** 🔻 **SINGLE dissociation only** -- the noun half includes zero. Witness: `test_sensorimotor_covers_the_verb_hole.py`.
>
> 🔍 **AND THE OTHER HALF OF THE PRIORITY-1 DIAGNOSIS IS PRICED: STORAGE IS FINE, COMBINATION IS WHERE MEANING GOES.** Our store is **256 dense BIPOLAR values** (inspected). Raw `+0.3107` -> projected `+0.3089` (**99.4%**) -> bipolar **`+0.2920` (94.0%)**; info-free arm `-0.0112`. 🚨 **BUT BUNDLING: k=2 -> 47% of raw, k=8 -> `+0.0808` (26%, null `0.0338`), k=16 -> 16%. TWO OTHER VECTORS HALVE IT.** ➡️ **THE ADAPTER IS NECESSARY AND NOT SUFFICIENT** -- meaning must sit in its own addressed slot, not superposed. *Measured with the sensorimotor channel BECAUSE it has signal to lose; our own codes would confound format-loss with nothing-there.* Witness: `test_does_our_format_survive_the_meaning_signal.py`.
> ✏️ **AND NEITHER FIX WORKS -- BOTH MEASURED, ONE OF THEM CORRECTING ME (08-23).** **SPARSITY: swept `1%`->`100%`, every density collapses to ~`+0.08`** (retention says `44%` vs `26%`, ABSOLUTE says `+0.0912` vs `+0.0764` -- *a ratio whose denominator you also changed is not a result*). **ADDRESSED SLOT: I wrote that into the brief BEFORE testing it.** ✅ Addressing works as addressing (right key recovers at cos `+0.3336`, wrong key `-0.0037`) 🔻 **but buys NO signal: `+0.0536` vs `+0.0670` plain at k=8, WORSE at k=32. Binding PERMUTES interference, it does not remove it.** ➡️ **SURVIVING RULE: keep the number of items in one superposition SMALL, or give meaning its OWN STORE.** ⚠️ *our-invention-under-test, not biology.*
> 🧠✅ **AND THE FIX IS SEGREGATION, MEASURED AT EQUAL BUDGET (08-23).** *Same `D=256` both ways: superposition buys resolution and pays crosstalk; segregation gives each item `D/k` and pays dimensionality.* **SEGREGATION WINS AT EVERY `k`, MARGIN GROWING: `k=16` -> a SIXTEEN-dim isolated slot `+0.1949` vs 256-dim superposed `+0.0479` (16x smaller, 4x better).** 🔑 **ISOLATION BEATS RESOLUTION -- cortex's actual arrangement.** ➡️ **BUILD ANSWER: meaning gets its own NARROW slot, not a share of a wide one.** *k=1 agreement + info-free `+0.0233`.* ⚠️ *segregation pinned, the algebra is not.* 📐 **DESIGN NUMBER: swept to `k=256` for a crossover -- THERE ISN'T ONE, but below ~`8` dims/slot BOTH schemes are losing. `8` dims still carries `+0.1537` (over half full resolution) -- THAT is the floor to build to.** ⚠️ **SCOPE: this is ATTRIBUTE segregation, NOT per-item** -- the segregated arm assumes you know WHICH slot, which in cortex is free because slots are anatomical and TYPED. At `D=256`, `32` attribute streams fit above the floor; per-item gives `2.56` dims at 100 items. ➡️ **NOT "a slot per word" -- it is "meaning gets its own attribute-typed slot".** 🎯 **AND THE LIVE COST IS MEASURED, NOT SWEPT: `context_vector` is a "bag-of-content-words bipolar bundle", so `k` = CONTENT WORDS/SENTENCE = MEDIAN `6` over 3,998 real sentences. At `k=6` the superposed vector retains `37.6%` of baseline (`+0.1095` vs `+0.2914`) -- **the reader throws away ~`62%` of the meaning signal PER SENTENCE, and a 42-dim isolated slot would carry MORE than the full 256 shared (`+0.2343`).** ⚠️ *Separate from the known `sign()` issue; this is the BUNDLING. Both live.*
> 🔻🔻 **REFUTED IN PART, SAME DAY, BY PRIOR WORK I FOUND AFTER MEASURING: `exp_structured_code_vs_flat_bag_c3_v1` = `STRUCTURE_HURTS` (`-0.0113`, CI `[-0.0195,-0.0030]`) and `exp_perirhinal_conjunctive_readout_c3_v1` = `CONJUNCTIVE_HURTS`.** *`hdlab/perirhinal_conjunctive.py` already exists, default-off, and its docstring states the flat-sum finding better than mine.* ⚖️ **SURVIVES: the measured COST (`62%`/sentence), a property of representations. DOES NOT SURVIVE: "so replace the flat bag" -- both cells tested exactly that and found it WORSE.** ➡️ **A measured cost with NO demonstrated benefit. A property of a representation is not a licence to change it.** 🔑 **BIGGER ALARM in the same record: `A5_STRINGCTRL 0.0870` vs `live base 0.0480` -- a STRING-MATCHING CONTROL BEATS THE LIVE SYSTEM ~2:1.**
> 🚨 **AND THE ENTRY POINT MATTERS, AND THE CHANNEL CANNOT GATE ALONE (08-23).** *Same asset, same `3,487` pairs, THREE answers:* raw CSV `+0.3107` | `grounded_vector` cos `+0.2676` | **`grounded_similarity()` `+0.2463` <- WHAT THE SUBSTRATE CALLS** (double-clamped; **56% sit exactly on the cap**). 🔻 **AND `CAP 0.45` < `LINK_THRESHOLD 0.5`, so as shipped it links `0` of `877` genuinely-similar verb pairs -- ZERO.** Uncapped it links `66.0%` of similar AND **`37.3%` of DISSIMILAR**; swept `0.30`->`0.95` the best margin `+0.287` occurs AT `0.50`. *AUC `0.7002` -- real signal, not separable.* ➡️ **CONTRIBUTOR, NOT DECIDER -- the priority-1 ask ("use it INSTEAD") has to change.** ⚠️ *NOT a licence to raise the cap.*
> 🧭 **AND WHAT NOT TO COMBINE IT WITH (08-23): WORDNET'S EDGE IS SELECTION, NOT MEANING.** *Pooled it looks decisive (`+0.543` vs `+0.287`, false alarms `7.0%` vs `37.3%`)* 🔻 **but SimVerb's PAIRS WERE SELECTED BY WORDNET RELATION. Hold the class fixed and it collapses: on `NONE` (`2,084` pairs, 60%) sensorimotor `+0.286` vs WordNet `+0.154`.** ➡️ **On pairs a taxonomy does not already link -- what a reader faces on new text -- ours is nearly twice as good.** ✅ *And the selection does NOT distort ours: `+0.286` unselected vs `+0.287` pooled.* ⚠️ *NOT "WordNet is useless" -- it wins on hyper/hyponyms; the POOLED number is what is refuted.* 🧪 **AND THE BOTH-SPOKES HUB, TESTED ON THE UNSELECTED CLASS: NOT ESTABLISHED** -- mean-of-both is the best rule tried (`+0.321` vs `+0.286`) but the paired difference is `+0.0245` `[-0.0021,+0.0520]`, **includes zero**; *I wrote "HUB WINS" off the point estimate first.* ✅ *Its NOISE control reads `-0.0507` `[-0.0891,-0.0105]`, so the second spoke carries real information, just not enough at this n.*
> ## ✅ **Q116 ANSWERED 08-23 -- THE OWNER WANTS IT RESOLVED BY MEASUREMENT, NOT BY MY RECOMMENDATION**
> **VERBATIM: *"make this into a focused problem to give to the solver so we can resolve it fully."***
> I asked whether to formally STOP trying to learn meaning from text and commit to supplied knowledge
> + reasoning. **The owner declined to settle it by decision.** ➡️ Filed as **PRIORITY 2
> `does_learning_from_reading_deserve_to_continue`**. 🔑 **THE BRIEF'S POINT, and it is our own
> standing rule: ALL 16 LOSSES TESTED A WEAK IMPLEMENTATION.** Bar = ONE head-to-head where the
> learned arm is **the strongest we can honestly build** (and the solver writes down WHY it is
> stronger BEFORE running), both arms against the counting floor that keeps winning. **A CLEAR LOSS
> IS AN EXPLICIT PASS** -- it closes the route with evidence instead of by fatigue. 5 failure modes
> pre-registered, incl. *"no stronger arm can be built"*, which is itself an answer.
> ⚠️ **THE SUPPLIED SIDE CARRIES ITS OWN WARNING: WordNet looked 2x better until the benchmark
> turned out to have been SELECTED by WordNet.** The brief requires per-stratum, not pooled.
> 📋 **BOARD IS NOW EMPTY.** Ranking is 1..9, contiguous.
>
> ## ✅ **Q115 EXECUTED 08-23 -- NEW CELLS GATED, BACKLOG INVENTORIED**
> **OWNER:** *"def make it a requirement for new experiments, but I'd go back through the 275 older ones one at a time."* ✅ **The pre-commit hook refuses a NEW `experiments/*.py` that writes a result without the shared helper -- ADDED FILES ONLY, controlled both ways.** 🔻 **COVERAGE `~21%` IS WITHDRAWN: the truth is `71.2%`** (`3,495` of `4,908` re-runnable, `1,413` replay) -- the old figure matched a variable NAME, not a call. 📐 **THE "1 BY 1" LIST: `425` assert a result; `135` of those are CITED by a steering doc. THE `135` ARE THE REAL LIST.** *`reproducibility_inventory.py`.* 🎯 **TRIAGE DONE: of the `135`, `29` carry NEITHER a floor NOR an interval and **`20` of those CLAIM A CAPABILITY -- THAT IS THE RE-RUN LIST.** The other `9` say `SELFTEST`/`PENDING` about themselves. ⚠️ *`20` is a CEILING: a floor run in a DIFFERENT cell does not count here.* `notes/THE_Q115_TRIAGE_FOURTEEN_RESULTS_WANT_A_RERUN_2026-08-23.md`.
> ## **LANDED 08-23: THE RESUME WIRING, AND THE PREDICTION IT KILLED**
> **`Substrate(foundation_dir=...)` LOADS** (`hdlab/substrate.py`, `e178e80d7`) -- silent no-op, then a loud raise, now real. Cold is byte-identical (`92` facts, `_pass_idx 0`); resumed carries `390` and `_pass_idx 81`.
> 🔻 **THE MEASUREMENT THAT CAME WITH IT REFUTES THIS PLAN'S OWN PREDICTION.** *"degeneracy should FALL as grounded vocabulary grows"* is **RETIRED**: resuming makes a matched 4,000-sentence read **`~18x` LESS** productive (`168` -> `9`, 3 seeds, deterministic) and precision sits at its **RANDOM_ANCHOR floor in every arm** (COLD `0.0199`=`3/151`, RESUMED `0/9`). **DECOY proves it is not meaning: permuted labels match RESUMED EXACTLY (`0/164` both).** Cause = **SAME-BATCH CO-OCCURRENCE**, which cannot cross a run boundary.
> ✅ *Reproduced before landing; solver EXCELLENT.* **Persistence is NECESSARY, NOT SUFFICIENT -- never bill it as a grounding fix**; `test_foundation_dir_does_not_lie.py` pins that IN the constructor, positive-controlled. ➡️ **THIRD INDEPENDENT INSTRUMENT POINTING AT `reader_meaning_channel` (rank 1).**
>
> ## **LANDED 08-23: GUI TAB 9 "SUBSTRATE"** -- ten pipeline stages from `data/substrate_progress.json`, each row dated and going amber at 3 days / red at 7, so **the tab shows its own staleness.** 🔻 *The lesson that outlives it: the real bug behind "there is STILL no priority" was a GUI launched 08-22 13:15 and never restarted. **A feature that ships into a process nobody restarts has not shipped.***
> ## **LANDED 08-22: SH-7, THE SWITCH THAT MAKES A RE-RUN GENUINE**
> **`HDI_FRESH_RUN=<tag>`** sends a cell to an EMPTY sibling so it recomputes; **env unset -> paths byte-identical** (the load-bearing NEGATIVE control, a witness not a comment). *Coverage and triage are owned by the Q115 entry.* **B3' carries a SCORED verdict: best cortical arm hit@1 `0.057/0.043/0.043` vs a COUNTING floor `0.090/0.103/0.116` -- UNDER its floor on every seed.**
> ## 🔻 **RETRACTED SAME DAY: "THE DURABILITY GATE IS HOLLOW" WAS MY OWN WRONG-FIELD ERROR (08-23)**
> **I measured `gate_decision_target`, found it empty on `24` of `42` SHELVE rows, and told every session that shelved capabilities carry no revival criteria. THAT IS FALSE.** There is a field called **`revival_criteria`** -- named for exactly the thing I was looking for -- and it is **filled on `41` of `42` (`98%`)**. **ZERO rows lack all three of `revival_criteria` / `gate_decision_target` / `superseded_by`. THE GATE IS BEING HONOURED.** *e.g. `binder_direct_supply_grounding` -> "successor = WorldTree definitional grounding, gated on upstream FULL"; `entity_slot_gate_cross_boundary_v1` -> "a task where <900 write-events/slot, OR an end-to-end-differentiable slot/attention variant".*
> 🔑 **THE FAULT: I SEARCHED BY A GUESSED FIELD NAME INSTEAD OF ENUMERATING THE FIELDS.** One line -- `sorted({k for r in rows for k in r})` -- lists `revival_criteria` immediately. **That is this repo's own ENUMERATE-THEN-RECONCILE rule, which I cited TWICE TODAY, applied to a schema instead of a filesystem.** ✅ **The session-start check I shipped was crying wolf at every session; corrected within the hour to flag only a shelve with no way back in ANY of the three fields -- currently zero, so it is silent.** 🚫 **WHAT SURVIVES IS ONLY THIS: `gate_decision_target` is empty on 24 shelves, a cosmetic inconsistency in how rows are filled, NOT a gap in the gate.** `THE_SHELVE_HALF_OF_THE_DURABILITY_GATE_IS_HOLLOW_...` *(retracted at the top)*
> ## 🔍 **A HARD_PASS ON THE GOAL-BEARING LINE WAS INVISIBLE, IS BETTER-EVIDENCED THAN EXPECTED, AND IS ONE CONTROL SHORT (08-23)**
> **`exp_stated_entity_fate_reading_extractor_v2_highprecision`: top-level `verdict` reads `STRICT_READY_PENDING_HANDCHECK` while `final_verdict` IN THE SAME FILE reads `HARD_PASS_CLEAN_GROW_BY_READING_VIABLE`.** *Seven cells carry both fields and ALL SEVEN DISAGREE -- but in SIX the stale field is the PESSIMISTIC one (4 refine a HARD_FAIL, 2 resolve PENDING to HARD_FAIL). **This is the only one where the stale field HIDES A PASS.*** ⚠️ **CORRECTION TO MY OWN FIRST TELLING: `experiment_index.py` ALREADY resolves `final_verdict` and returns it as HARD_PASS -- the stale field is the RAW metrics key, not the main query path. My 'every tool misses it' was an overclaim, caught by running the query instead of assuming.** *`tools/reproduce.py` DID read the stale field and now prefers `final_verdict`.*
> ✅ **I EXPECTED THE LABEL UNEARNED AND IT IS NOT:** band **PRE-REGISTERED at `0.85` before the run**, result `0.90`; `100` items HAND-ADJUDICATED with a ten-category error taxonomy (**one instance each -- no dominant failure hiding in the 10%**); **the sample was drawn FRESH, seed-offset, AFTER the filters were fixed**; recall cost disclosed prominently (`4,015` -> `1,414`, survival **`0.3522`**); real v1->v2 comparison `0.394` -> `0.90` via six named glass-box filters; curated `P=1.0/R=1.0`, held-out `P=1.0/R=0.8`, ProPara coverage `129/175`. **By this archive's standards -- 99.5% of HARD_PASS cells carry neither a CI nor a null -- that is top-fraction-of-a-percent care.**
> 🔻 **AND IT IS SHORT EXACTLY ONE THING: NO FLOOR ON THE PRECISION.** Six filters removed two thirds of the output, so `0.90` cannot distinguish *discarding hard cases* from *keeping only easy ones* without knowing what a TRIVIAL extractor scores on the SAME `1,414` survivors. **Measured: none of the four `stated_entity_fate` cells contains `floor`, `trivial`, `naive` or `baseline` -- the arm has never been run.** ⚠️ **The survivors were NOT PERSISTED (`37 KB` vs v1's `1.2 MB` for all `4,015`), so it needs a RE-RUN not a re-score** -- our own save-the-population rule, broken by the stronger version. **Filed as `the_grow_by_reading_pass_has_no_floor`, RANKED 3, list renumbered.** 🚫 *DO NOT QUOTE `0.90` as an extraction accuracy: it is FILTERED precision on survivors at `0.3522` survival.* `A_HARD_PASS_ON_GROW_BY_READING_...`
> ## 🧠 **TWO SESSIONS ON ONE ORGAN, RECONCILED (08-23) -- AND STAGE B READS OUR OWN HAND-PLACEMENT, NOT VALENCE**
> 🔻 **IT CORRECTS ME TWICE.** A concurrent session split the propagator's `83%` abstention: **`6` points are REACH (`114` items, no anchor in range) and `78` are DISCRIMINATION (`1,531` items whose anchors VOTE AND DISAGREE)** -- so **I was wrong to call it coverage-limited, twice, and the repair is a different one.** *Nor are they near-misses: balanced margins sit at median `0.0276` against a `0.15` gate.* **CAUSE, with a null AND both positive controls: `path_similarity` does NOT predict valence agreement at all** -- Spearman `-0.0023` over 6,000 random verb pairs inside a shuffled null p95 `0.0231`, banded means flat, while antonym pairs read `2.031` (larger, as required) and same-synset `1.063` (smaller, as required). **Stage B votes by exactly that distance and is `307` of `326` commits: `94%` of the organ's output rides an axis measured to carry NONE of what it votes on.**
> ✅ **MY OPPOSING RESULT SURVIVED ITS BASELINE, AND THE SYNTHESIS IS A MEASUREMENT RATHER THAN A TRUCE:** nearest-5 anchor purity `0.800` vs **random-5 `0.600`** vs farthest-5 `0.600` on a perfectly balanced `26`/`26` seed set (*the `0.5` I first compared against was the WRONG baseline and I never ran the right one*). Both hold because **THE 52 SEEDS ARE THEMSELVES CLUSTERED BY POLARITY IN WORDNET SPACE** -- same-polarity anchor pairs `0.2438` vs different-polarity `0.2206`, gap **`+0.0232`** against a 2,000-shuffle permutation null `[-0.0076, +0.0087]`, **OUTSIDE**. 🔑 **SO STAGE B READS WHICH HAND-LABELLED CLUSTER A TARGET LANDED BESIDE. The organ's competence is INHERITED FROM WHERE 52 SEEDS SIT, not from the graph carrying valence** -- far narrower than "anchored valence propagates outward", and it predicts their measured thin global gradient (`0.6597 -> 0.5773` from `12%` to `94%` coverage) rather than a sharp local one. **ANTONYMY IS THE VALENCE-BEARING RELATION (Stage A `0.8421`, n=19, CI `[0.6842,1.0000]` -- too few to carry a claim); filed as `propagate_along_the_relation_that_carries_valence` after pricing the population at `121` items 1-hop / `513` 1-or-2-hop.** ⚠️ *Bears on `Q116` AGAINST my own recommendation: it reinforces SUPPLY and weakens REASONING-OUTWARD.* `RECONCILING_TWO_SESSIONS_...`
> ## 🧪 **BOTH SUBSTRATE PATHS DRIVEN END TO END (08-23) -- THE SUBSTRATE CAME OUT WELL; THE GAPS ARE UNWIRED PARTS AND MY OWN MEASUREMENTS**
> ✅ **WRITE PATH HEALTHY:** grounding gate accepts `68` of `555` = `12.3%` at 1,150 sentences (DISCRIMINATING, which is what substrate.py's own self-test demands); **nested sampling -- ONE substrate, 12 samples, 1,800 sentences -- shows NO DECREASE EVER**, grounded `0->111`, facts `92->314`, monotone. *`facts = 8.4 x sentences^0.473` (`6%` error), a normal type-token exponent, so falling facts/episode is ARITHMETIC not a defect.* 🔻 **THREE APPARENT ANOMALIES, THREE EXPLANATIONS, ZERO DEFECTS -- AND TWO WERE MY OWN MEASUREMENT DESIGN.**
> 🚨 **READ PATH: TWO OF THREE ROUTES CANNOT SAY "I DO NOT KNOW".** `8` of `8` INVENTED strings the system never read got a confident five-item ranked answer from `recall_sentence` and `recall_cortical`; only `query` returns `known=False` (protected by `_selftest_query_refuses_what_it_never_read`). **The refuse gate is BUILT and HARD_PASS (`hdlab/refuse_gate.py`); slot `Q3` is `NEEDS_ADAPTER` and now CARRIES THE MEASURED COST.** Filed as `wire_the_refuse_gate_onto_the_readout` and **RANKED 2, with the whole list renumbered** -- a list where entries argue for their own promotion is not a ranking. ⚠️ *Directional only at n=8: returned neighbourhoods for real vs invented words are indistinguishable (`0.200`/`0.191`), and top-5 items are `35%` of every slot returned.*
> 🔻 **THREE OF MY OWN CLAIMS WITHDRAWN IN ONE STRETCH, ALL MEASUREMENT DESIGN, NOT SUBSTRATE BEHAVIOUR:** the store is 'frozen' (it was non-monotone -- coarse spacing hid it); 'reading more may make it forget' (schedule phase across INDEPENDENT runs -- nested sampling discharges it); and 'batching is markedly more productive per sentence' (**`1.00x` grounded and `0.98x` facts at MATCHED volume -- I compared TOTALS at DIFFERENT VOLUMES, the exact error I had spent the night catching, committed twenty minutes after saying so**). 🔑 **THE GUARD IS NOT ANOTHER RULE -- PRINT THE RATE BESIDE THE TOTAL, and the error is visible in one line instead of requiring vigilance.** `THE_WRITE_PATH_IS_NON_MONOTONE_...`
> ## 🚨 **INTEGRATION 08-23: I LANDED THE REPRODUCTION MECHANISM AND NEVER USED IT. USING IT BROKE FOUR WAYS WITH EVERY WITNESS GREEN.**
> **WORST: the fresh-run switch CRASHED EVERY CELL THAT IMPORTS THE HARNESS** -- `_seed_checkpoint`'s import-time self-test asserts the UN-suffixed path, so `HDI_FRESH_RUN` made every importing cell die at import. 🔻 **AND ITS WITNESS (`6/6`) COULD NOT SEE IT: the witness imports THEN sets the env; a real cell has it set BEFORE PYTHON STARTS. The bug IS the ordering, and a test that controls the ordering is structurally blind to it.** *New witness spends a subprocess; positive-controlled by reintroducing the bug (3/4 fail) and removing it (4/4 pass).* **Also: `classify_run(0,0)` returned `RECOMPUTED` -- the most positive verdict -- for a run that recorded NOTHING (our own empty-scores-perfectly failure, inside the guard against false reproduction); the driver was blind to `19` of `423` landed dirs lacking the `exp_` prefix and advised confidently on one that did not exist; and its detector did not recognise the migration its own refusal message prints.** ⚠️ **COVERAGE CORRECTED: `87 of 421` was a STRING match; strict IMPORT gives `43`. Quote `10-21%`, never the `87` alone.**
> ✅ **PAYOFF -- THE FIRST GENUINE REPRODUCTION IN THIS PROJECT:** `exp_thematic_role_labeler_cue_integration_v1` recomputed from scratch in `33.3s`, `HARD_PASS` again, landed dir byte-identical. *`403` of `7,875` landed cells replay their checkpoints, so nothing here could do this before.* ✅ **AND EVERY OTHER SOLVED PROBLEM REPRODUCES EXACTLY** -- ran each brief's own `reverify`, which nobody had: stems `0/141`; bank `166`/`124`/floor `0.6048387`/`49`-`75`; flat-store held-out `0.1399` CI `[0.1310,0.1494]` vs counting floor `0.3242`, exact-key `0.9954`, after `215s` of GENUINE recompute. **THE SUBMITTED WORK IS SOUND, AND THAT BELONGS AHEAD OF EVERY DEFECT ABOVE.** ⚠️ **THE PROTOCOL WAS NOT: `3 of 5` reverify commands RE-RAN A CELL IN PLACE; I followed one and re-stamped a landed `metrics.json` (science byte-identical, restored from git) -- the same shape as the `54` silently re-dated records. Both protocol files fixed, all three commands rewritten, re-audited `0 of 5` unsafe, each now carrying the numbers it must reproduce.** `INTEGRATION_BY_USING_IT_FOUND_FOUR_DEFECTS_...`
> ## 🧠 **THE NAMED DIRECTION FINALLY TESTED AT POWER: ANCHOR+PROPAGATE SURVIVES, THE MARGIN SHRINKS BY TWO THIRDS (08-22)**
> **`hdlab/wordnet_polarity_propagation` stood at `0.833` on TWELVE held-out verbs. Re-tested on `326` against HUMAN valence ratings (Warriner, `13,905` words, ALREADY ON DISK): `0.6595` CI `[0.6074,0.7117]` vs a majority floor `0.5583` RECOMPUTED ON THE COMMITTED SUBSET.** Paired `+0.1012` CI `[+0.0245,+0.1779]`, McNemar `p=0.0118`. ⚠️ **THE TWO TESTS DISAGREE AND BOTH ARE REPORTED: independent intervals are NOT_SEPARATED (organ lower `0.6074` vs floor upper `0.6135`); the PAIRED test is correct here (same items) but the standing bar is phrased for INDEPENDENT arms.** *At this n, which test you use changes the verdict -- the fix is more items, not more argument.* ✅ **NOT CIRCULAR: mechanism = WordNet + hand seed; gold = HUMAN ratings published BEFORE the organ existed; WordNet used only to FILTER the population, never to grade.** ✅ **SEED ABLATION COLLAPSES TO CHANCE (`0.4645`) -- the grounded anchor is load-bearing, which IS the hypothesis.** ✅ *Selection check CLEAN against my expectation: abstains `83%` but commits on slightly HARDER items (`1.635` vs `1.716`).* 🚫 **`17%` COMMIT RATE -- coverage-limited, and NOT a test of the plan's context-conditioned superposition: this is ONE value per word, a FLOOR for the direction.** `ANCHOR_AND_PROPAGATE_SURVIVES_A_26x_...`
> 🚨 **AND CHASING ITS ONE UNEXPLAINED NUMBER FOUND A CALIBRATION GAP IN A WIRED ORGAN: THE CONFIDENCE SCORE DOES NOT PREDICT CORRECTNESS.** *Shuffling made it commit MORE (`564` vs `326`) because it commits LESS confidently but more often just over threshold (median confidence `1.0000` -> `0.3380`).* 🔻 **MY EXPLANATION FOR THAT IS WITHDRAWN -- it predicted confidence should track accuracy, and accuracy is FLAT across confidence bands (`0.6520` at saturation vs `0.6721` below it, diff `-0.0202` CI `[-0.1249,+0.0894]` SPANNING ZERO, so the SIGN carries nothing).** ⚠️ **BUT `pseudo_counts_from_dictionary` does `n = round(K_MAX * confidence)` (`word_learning_tool.py:48`), so a saturated hit is injected into the learning loop at 3x the weight -- and `63%` saturate, so the weight genuinely VARIES.** *NOT claimed: that learning is harmed -- I DID NOT RUN THE LOOP.* 🔑 **A STATISTIC THE MECHANISM EMITS IS NOT EVIDENCE THAT THE STATISTIC MEANS ANYTHING.** `THE_CONFIDENCE_SCORE_DOES_NOT_PREDICT_...`  🔻 **AND ONE OF MY OWN CLAIMS FROM THAT THREAD IS WITHDRAWN (08-23).** I predicted the rounding DISCARDS exactly the items the sim-weighting earns -- **true: those items have median confidence `0.0933` and `57` of `75` are dropped.** 🚨 **BUT THE CONCLUSION I WAS WALKING TOWARD IS REFUTED BY THE NEXT SLICE: within that group the rounding KEEPS the good ones (`17/18 = 0.9444`) and drops the rest (`0.6491`). It is SELECTING its best work, not discarding it** -- so **"no measured basis for preferring the ones it keeps" is TOO STRONG and is WITHDRAWN.** 🔑 **THE DURABLE LESSON: THE AGGREGATE CONFIDENCE-ACCURACY NULL WAS AVERAGING OVER AN INTERACTION** -- confidence is uninformative among items BOTH vote types reach (`0.6327` vs `0.5000`) and strongly informative among items ONLY the weighted vote reaches (`0.9444` vs `0.6491`); **the average describes neither.** ⚠️ **THE `18`-ITEM CELL IS POST-HOC after many slices of one dataset -- re-test on a population fixed IN ADVANCE before acting on it; I am NOT proposing a change on n=18.** `MY_PREDICTION_WAS_RIGHT_AND_THE_CONCLUSION_IT_POINTED_AT_WAS_WRONG_...`  ✅ **AND THE POST-HOC CELL WAS RE-TESTED PRE-REGISTERED, AND HELD (08-23).** Prereg COMMITTED BEFORE THE NUMBERS EXISTED (`9d09f9945`): population, four cells, verdict rule, untestable floor, and what I would not do afterwards. Re-run under the **84-word anchor** (every neighbourhood, margin and cell membership freshly determined): **weighted-only SURVIVES `0.8421` (16/19) vs DISCARDED `0.5806`, contrast `+0.2615` CI95 `[+0.0509,+0.4635]` EXCLUDES ZERO.** 🔑 **AND THE INTERACTION IS WHERE IT SHOULD BE: in the OVERLAP group, surviving is worth `-0.0072` CI `[-0.2466,+0.2394]` -- NOTHING. An effect that fired everywhere would have been the red flag; this one fires in one place and is silent in the other.** 🔻 **IT SHRANK `0.9444 -> 0.8421`, WHICH IS REGRESSION TO THE MEAN AND IS THE EXPECTED, REASSURING OUTCOME -- had it returned at `0.94` I would trust it LESS.** ⚠️ **`n=19`; the CI spans `+0.05` to `+0.46`, so the effect is real at this rule and its SIZE is barely constrained -- DO NOT QUOTE `+0.2615` AS A MAGNITUDE. The item pool OVERLAPS the original (stated in the prereg, not discovered afterwards).** 🚫 **STILL NOT PROPOSING ANY CHANGE TO THE ROUNDING -- this argues for LEAVING IT ALONE.** `THE_PREREGISTERED_REPLICATION_HELD_...`
> 🚨 **AND WHEN I RAN THE LOOP TO SETTLE IT, MY OWN HARNESS NEARLY REPORTED A FALSE NULL TWICE (08-23).** The four-arm comparison (`WEIGHTED` vs `NO_DISCARD` vs `FLAT_3` vs `FLAT_1`) is **UNTESTABLE on the deployment population, NOT negative: three arms inject BYTE-IDENTICAL priors** because the dictionary commits on `6` of `33` eval lemmas and all six SATURATE -- nothing to restore, nothing to flatten. **The `+0.0000` deltas are ARITHMETIC.** *Run 1 was worse: a wrong field name (`oov_lemma`, real field `outcome_verb_lemma`) meant every arm injected NOTHING and all four read an identical `0.5000`.* 🔑 **BOTH TIMES THE ACCURACY LOOKED PERFECTLY REASONABLE AND THE SCRIPT'S CANNED SUMMARY SAID "CLOSES BOTH FINDINGS AS HARMLESS". WHAT CAUGHT IT WAS PRINTING THE INJECTED COUNT BESIDE THE ACCURACY** -- the `n_grounded=0` beside `anchors +68` rule, paying out again. ✅ **GUARDS: refuse on zero eval lemmas + a NON-DEGENERACY GATE that names which arms are identical to what ships BEFORE comparing them.** *Turn the information-free-twin discipline around: construct the arms and check they DIFFER before believing any comparison.* ⬜ **BOTH CALIBRATION FINDINGS STAY OPEN**, and the `17%` commit rate now blocks TWO questions -- coverage is this organ's real limit, not accuracy. `THE_WEIGHTING_TEST_IS_UNTESTABLE_ON_THIS_EVAL_...`
> 🧠 **AND THE `17%` COMMIT RATE IS NOT A COVERAGE PROBLEM -- I HAD THAT WRONG TWICE (08-23).** Splitting the `83%` abstention: **`6%` is NO ANCHOR IN RANGE (a REACH limit) and `78%` is NEIGHBOURS VOTE AND DISAGREE (a DISCRIMINATION limit).** *"More anchors" addresses SIX of the eighty-three points.* **And they are NOT near-misses: balanced-vote margins have median `0.0276` against a `0.15` gate.** ✅ **THE ABSTENTION IS CONSERVATIVE, NOT PROTECTIVE: the margin over a per-point recomputed floor EXCLUDES ZERO at EVERY operating point from `12%` coverage (`+0.0966`) to `94%` (`+0.0560`), with accuracy declining smoothly `0.6597 -> 0.5773`.** 🧠 **SO ANCHORED VALENCE DOES NOT PROPAGATE STRONGLY TO A NEIGHBOURHOOD AND STOP -- IT IS PRESENT WEAKLY ACROSS ESSENTIALLY THE WHOLE VERB SPACE. A thin global gradient, not a sharp local one.** 🚫 **NOT A PROPOSAL TO MOVE ANY GATE** -- at `17%` it is right `66/100`, at `94%` `58/100`; the curve is information about the MECHANISM, not a recommendation about a constant. ⚠️ *A `52 -> 84` anchor cuts unreachable items `114 -> 71`, but its margin `+0.1012 -> +0.1687` is MOSTLY THE FLOOR FALLING (`-0.0497`), not the organ improving (`+0.0178`) -- different committed populations, no number crosses.* `THE_83_PERCENT_ABSTENTION_IS_A_DISCRIMINATION_PROBLEM_...`
> 🧠🚨 **AND THE `78` POINTS ARE EXPLAINED: WE PROPAGATE VALENCE ALONG AN AXIS MEASURED TO CARRY NONE OF IT (08-23).** **6,000 random verb pairs: Spearman(path_similarity, `|dV|`) = `-0.0023` against a shuffled-null p95 of `0.0231` -- INSIDE THE NULL.** And the banded view is blunter than the correlation: mean `|dV|` reads `1.406 / 1.387 / 1.410` across similarity bands against an all-pairs `1.393`. **IN THE EXACT BAND THE ORGAN WORKS IN (`NEIGHBOR_FLOOR = 0.20`), TWO VERBS AGREE ABOUT VALENCE NO BETTER THAN TWO PICKED AT RANDOM.** ✅ **TWO POSITIVE CONTROLS BOTH PASS, so the null is not my instrument failing: ANTONYM pairs `2.031` (larger, required) and SAME-SYNSET pairs `1.063` (smaller, required).** 🔑 **VALENCE LIVES ON WORDNET'S LEXICAL RELATIONS (synonymy, antonymy), NOT ON ITS TAXONOMIC DISTANCE -- and Stage B, which is `307` of `326` commits, votes by distance.** *Stage A (antonym, a valence-BEARING relation) reads `0.8421`; Stage B reads `0.6482` -- measured days earlier and independently, and lined up in the direction this predicts.* ⚠️ **UNRESOLVED AND NOT EXPLAINED AWAY: Stage B still CLEARS its floor. I do not know why and am not inventing a reason** -- candidates (near-synonym leakage in the high-sim tail; an unbalanced anchor injecting a class prior) are UNTESTED. 🚫 **This does NOT touch the anchor+propagate HYPOTHESIS -- only the STRUCTURE chosen to propagate through.** `WE_PROPAGATE_VALENCE_ALONG_AN_AXIS_THAT_DOES_NOT_CARRY_IT_...`
> ✅ **AND THE TENSION I LEFT OPEN IS CLOSED THE SAME DAY -- PROXIMITY CARRIES *ANSWERABILITY*, NOT VALENCE.** All three candidates for Stage B's residual signal are DEAD: the anchor is **26 POS / 26 NEG** (no global prior), the IN-RANGE subset shows no local skew (POS fraction mean `0.511`), and lexical leakage cannot be it (only `114` of `1,971` items share a synset/antonym with an anchor; drop them ALL and the margin holds `+0.0825` CI `[+0.0034,+0.1684]`). 🔑 **STRIP THE SIM-WEIGHTING AND KEEP ONLY MEMBERSHIP: on the `234` items both arms answer THEY AGREE 100% -- the weighting NEVER CHANGES AN ANSWER. It changes WHICH items get answered, and that is the whole effect: on the `75` items it adds, accuracy is `0.7200` against a `0.5067` floor** (vs `0.6282` on the overlap). 🧠 **SO THE GRADED SIMILARITY TELLS YOU WHERE A CONFIDENT ANSWER IS AVAILABLE, NOT WHICH POLE IT IS** -- which QUALIFIES the headline above rather than overturning it: the axis is valence-blind about the ANSWER and informative about ANSWERABILITY. *My Stage-B reimplementation commits `309` where the shipped one commits `307`; unchased, and stated rather than implying an exact reproduction.*
> 🔻 **DEFLATED BY MY OWN FOLLOW-UP THE NEXT MORNING (08-23), AND THE DEFLATION IS THE RESULT:** I ran the loop to price the weighting and the discard. **The setup answered before any arm scored -- on the DEPLOYMENT population the dictionary commits on `6` of `33` lemmas and ALL SIX SATURATE, so `WEIGHTED`/`NO_DISCARD`/`FLAT_3` inject an IDENTICAL `18` pseudo-counts and THREE OF FOUR ARMS ARE THE SAME ARM.** ✅ **So the calibration gap is REAL on `1,971` dictionary verbs and ABSENT where the organ is USED** -- the note's own "narrower and DIFFERENT population" caveat was not a caveat, it was the whole story. ⚠️ **THE PROCESS FAILURE IS THE POINT: I DID NOT ASK WHETHER THE EXPERIMENT COULD SUCCEED BEFORE LAUNCHING IT** -- one line (how many committed lookups sit BELOW saturation on THIS population), this repo's highest-yield habit, skipped, then paid for across two launches one of which was killed at ten minutes. *Still open, not closed: whether `FLAT_1` differs is unanswered and at `6` committed lemmas cannot be powered -- UNTESTABLE HERE, not harmless.* ✅ *Incidental: the `WEIGHTED` arm read `0.4722`, REPRODUCING the landed number to four digits through an independently-assembled path.*
>
...
> **`experiments/data/goal_bearing_modern_eval_v2.jsonl` -- `166` items, `124` scorable, `3.4x` the old 36.** *Second solver problem closed; `eval_bank_too_small` was the one I was DISQUALIFIED from building, having read the per-item predictions.*  ✅ **RE-VERIFIED BY ME ON THE ARTIFACT, NOT BY RE-RUNNING THEIR PIPELINE** (a re-run shares the pipeline's bugs): counts `166`/`124` MATCH, majority floor `0.605` MATCH (`unmet 75` / `met 49`), length independent of the label at permutation `p = 0.984` (theirs `0.988`).
> 🔑 **AND THE POSITIVE CONTROL IS WHAT MAKES IT MEAN ANYTHING: the same instrument run on v1's 36 reads `met 410.9` vs `unmet 340.3` chars, `+70.6`, `p = 0.0027` -- IT DETECTS THE CHEAT WHERE IT LIVES AND FINDS NOTHING ON v2. That `+70.6 / p=0.0027` reproduces TO THE DIGIT a measurement I took independently days before the brief existed.**  🔻 *Negation `p` did NOT reproduce exactly (mine `0.564`, theirs `0.844`) -- different last-sentence extraction; both far from significance so the CONCLUSION stands, but `0.844` is not a reproduced number.*
> 🎯 **DESIGN PROPERTY THE SUBMISSION DID NOT CLAIM: THE MAJORITY CLASS FLIPPED.** v1 was `64%` MET, v2 is `60.5%` UNMET. **v1's floor was "always say MET" -- the exact answer the organ systematically fails to give, so it could never clear its own floor. That coincidence is gone.**  🚫 **NO v1 RESULT BECOMES WRONG, AND v1/v2 NUMBERS MAY NOT BE COMPARED** -- different populations, different majority class.
>
> ## ✅ **Q113 ANSWERED 2026-08-22 -- THE BUILD LANE IS OPEN, AND THE PROBLEMS FOLDER IS NOT MINE**
> **Owner, verbatim: *"you can definitely start and run experiments and helpers. eliminate that line from your instructions. Remember that any 'problem' you have in the problems tab is going to be worked on, so try not to compete with that"*.**  ➡️ **SO: cell work and `hdi_*` spawns are AUTHORIZED — the "the loop can only measure, document and guard" constraint is RETIRED.** *That premise was doubly wrong: Q109 retracted the `verification/` half, and this retracts the rest.*
> 🚫 **AND A NEW STANDING CONSTRAINT: THE TWELVE BRIEFS IN `notes/problems/` ARE THE SOLVER'S. DO NOT WORK THEM HERE** — **ENUMERATE THEM (`ls notes/problems/`) -- DO NOT MIRROR THE LIST HERE.** *This sentence said SIX for a day while TWELVE existed; a mirrored list is the staleness this plan already records for BOARD.md.* *Filing a problem hands it away; the value I add after filing is integration, not solving.*  🎯 **THIS SESSION'S WORK IS THEREFORE WHAT IS *NOT* FILED:** re-landing the stale goal-bearing cell (dispatched), the `STATUS.md` structural split, tracing the short-read cause, and the B3'/Q3 scored paths.
>
...
> ✅ **GUARDED (`e335fa220`): `ReadResult` now carries `n_sentences_requested` + `short_read`, `read()` prints a stderr banner under 90% delivery, and it survives `to_dict()` into `metrics.json`.** *Warn not raise -- the results are REAL, only their DESCRIPTION was at risk. Controlled both ways; `verification/test_read_reports_short_reads.py` 3/3.*  🔻 **AND IT RETRACTS MY OWN GROWTH CURVE FROM THE SAME NIGHT: chunked reading did NOT grow consolidation to 180 by reading more -- each CALL buys a fresh smaller allowance. The "30-term consolidation cap" was the READ cap in disguise.**
...
>
> ## 🔌 **WHICH ORGANS ARE WIRED -- ASK THE TOOL, DO NOT MIRROR IT HERE (08-22)**
> **`python tools/slot_status.py [term]`** reads `substrate.py`'s slot table live from source. **28 slots: `9` FILLED, `8` NEEDS_ADAPTER, `8` EMPTY, `3` EXCLUDED** -- *four states needing different work; never collapse them into one "not done" count.* **It is the SIXTH prior-work read** (`CLAUDE.md`), added after I spent four continuations arguing for a lookup repair without checking whether anything CALLS the lookup. `before_you_start.py` now runs it automatically.
> 🔑 **THE PART THE TOOL CANNOT TELL YOU -- THE BLOCKERS ARE NOT THE SAME KIND:** `B3'` was already CALLED and only lacked a scored path; `Q3` ships a written recipe (*"wire with `atom_consultation` OFF"*); `B5`'s combination rule is UNPINNED so anything built there is OUR-INVENTION-UNDER-TEST; and 🔻 **`E3` is a VALIDITY problem, not a wiring one -- its `0.7193` is `41/57` queries with the GOLD MENTION INVENTORY SUPPLIED, on McGuffey (NOT LitBank), and does not transfer to raw prose.** *Both halves of that caveat were wrong in different documents until 08-22.*
> ✅ *`verification/test_slot_table_claims_are_true.py` (4/4) checks the table's machine-verifiable claims -- it caught D2 asserting a data-loss hazard that had been fixed at `f102e7081` and never un-said.*
...
> **Owner:** *"keep the long vision and longterm strategy here... I'll have a separate session dig in to specifically solve those problems... you keep the 10k view and integrate solved issues."*  ✅ **BUILT:** `notes/problems/README.md`, `SOLVER_SESSION_PROMPT.md` (on disk -- chat does not survive compaction), `tools/problem_ledger.py` (**REFUSES a `SOLVED.md` with no floor or no controls**; self-test 7/7), `verification/test_problem_briefs_and_flags.py` 5/5, and **SIX briefs**.
...
> 🔻 **`7.9%` MAY NOT BE QUOTED AS A SYSTEM PROPERTY** -- it is one stale artifact; any grounding number computed on `v1`/`v2q` inherits it and must say so.  🎯 **DECIDES A COUPLING: the two resumable snapshots ARE the two pre-fix ones, so "load an old foundation" feeds `7.87%` junk into the anchor pool `substrate_never_resumes` exists to measure. FIRST MOVE THERE: BUILD A CLEAN SNAPSHOT, do not load `v2q`.**
>
> ## 🚨 **THE OWNER HAND-SCORED THE BLIND SHEET (08-22). MY DIAGNOSIS WAS WRONG THREE TIMES, IN ALTERNATING DIRECTIONS.**
> *Owner: "a lot of words there are missing letters, and a lot of them unrelated." Scored `75` of `150`: `1` MEANINGFUL, `15` RELATED, `31` NOISE.*  🔻 *C1: I reported `24%` not-words, **2.3x overstated** -- "not in WordNet" counts real words WordNet lacks (`archaea`, `adipocytes`, `Abdullah`). **Round-trip test** (stemmer output iff NOT a word AND a plausible suffix makes one) gives sheet `10.4%` / v2q `7.9%` / v5 `0.4%`.* ⚠️ **TRAP: v5 has the HIGHEST not-a-word rate and LOWEST stem rate -- a cruder detector would have scored the FIXED foundation WORST.**
...
>
> ## ✅ **SUPERSEDED 08-23: THE FOUNDATION LOADS NOW.** *The cold-start pin (08-22, `7a810a34b`, descriptor-measured `0` reads) is FIXED -- see the resume entry at the top. Its prediction that the `way`-attractor would fall as vocabulary grows is not merely unreachable, it is **REFUTED**.*
>
...
>
> ## 🔻 **PRIORITY 1 IS DEAD: ADDRESSED STORAGE IS REFUTED BY ITS OWN FLOOR (08-22, SOLVER, INTEGRATED)**
> **The "mixing paint" problem -- the store adds every word into one running total, so nothing can be pulled back out -- had a proposed fix: use the addressed store that already exists. IT DOES NOT WORK.**  **Addressed-storage held-out identity-recovery hit@1 `0.1399` `[0.1310,0.1494]` against a first-order co-occurrence COUNTING floor of `0.3242` `[0.3115,0.3366]` on the IDENTICAL population -- `-0.1843`, CI EXCLUDES ZERO. It is CI-separated BELOW the floor.**  🔑 **AND THE FAILURE MODE IS THE ONE THIS PROJECT KEEPS PAYING FOR: its exact-key score is `0.9954` and collapses to `0.1399` held-out. THE ISOLATION WIN WAS MEMORISATION.**
> ✅ *Controls all present and one is the information-free twin doing its job: random-sign episodes at the same count/grouping score `0.0000` held-out. Addressed DOES beat the flat sum `+0.0554 [+0.0457,+0.0656]` -- a real sub-finding, kept as WIRE_CANDIDATE, not a capability win.*  ➡️ **I RANKED THIS #1 AS "upstream of everything, and a wiring job". THAT WAS WRONG -- the store was not the binding constraint, and the bar caught it.** *Corrected in the ranking table; a solver reading it would otherwise have started on a dead problem.*
>
> ## 🚨 **SCORED ON THE NEW v2 BANK (08-22): `0.2339`, AND THE ORGAN REFUSES TWO-THIRDS OF THE TIME**
> **First measurement of `congruence_with_lexicon_fallback` on v2's `124` scorable items. Verified on disk, not relayed.**  **accuracy `0.2339` against v2's OWN majority floor `0.6048` (class = `unmet`).** 🔑 **THE CONFOUND I SENT IT TO CHASE DID NOT OCCUR: the organ is NOT riding the flipped majority class -- MET recall `10/49` (`0.2041`) and UNMET recall `19/75` (`0.2533`), BOTH LOW.**  🔻 **THE DRIVER IS REFUSAL, NOT ERROR: `82` of `124` (`66%`) predictions are non-committal** (`NONE` 80, `AMBIGUOUS` 2). *It is not guessing wrong two-thirds of the time; it is declining to answer.*
> ✅ **FLOORS RECOMPUTED ON v2's OWN POPULATION, NEVER IMPORTED: NOT ONE of the 12 baselines clears its own null** (`strongest_that_clears_its_own_null: None`; strongest raw `last_sentence_length 0.629` vs its own null `0.653`). ⚠️ *`strongest_beats_majority = True` at the same time -- the documented FLATTERING pattern: beating the majority floor while sitting at your own null is not a floor being cleared.* Positional recomputed on the 124 (`recency 0.6371 / first_mention 0.7419 / nearest_subject 0.3065 / majority 0.6935`), with the full-166 numbers kept separately as `reference_only`.
> 🔻 **THE OVERLAY ARM IS SELF-FLAGGED `CONTAMINATION_RISK` AND I AM NOT QUOTING IT AS A RESULT** -- `0.2984` was trained on the same four novels v2 draws new items from, at line ranges never excluded. *Volunteered by the agent, not asked for.*
> 🚫 **DO NOT QUOTE THE `66%` AGAINST "v1's ~8%".** *I measured v1's empty-overlay abstention at `20/36 = 55.6%` today; the two cannot be reconciled from here and are different arms or conditions. The `66%` stands on its own; the COMPARISON does not.*  ➡️ **THE OPEN QUESTION IS WHY v2 TRIGGERS ABSTENTION AT ALL** -- likely a pattern-coverage gap on the new prose rather than an outcome-valence failure. *The agent declined to author that diagnosis, correctly.*
>
...
> ## **THE GOAL-BEARING LINE, v1-ERA -- FOLDED TO ITS PROHIBITIONS. Numbers live in the named notes.**
> **FOUR STANDING PROHIBITIONS, DO NOT RE-PROPOSE:** **(1) NOT `referent_mismatch`** -- returns `UNMET` `8 of 8` (a DEFAULT, not an inference); PERFECT ceiling `0.6111` is BELOW the `0.6389` floor. **(2) NOT verb-class supply** -- tried TWICE, bought `1` of `23`. **(3) "MAKE THE CASCADE FIRE" IS RETIRED** -- when it fires it is `10/19`, vs a coin flip `p = 1.0000`: COVERAGE WITHOUT DISCRIMINATION BUYS NOTHING. **(4) NOT flipping UNMET->MET** -- `0.5556`, below floor, and it FITS THE TEST SET.
> **STRUCTURAL: MET recall `8/23`, `7/23`, `8/23` -- INVARIANT across a 7x lexicon change, 96%-vs-50% polarity, and learning-vs-none.** *No rule ever JUDGES an item; every tier emits a CONSTANT, so all signal is in WHICH RULE FIRES -- and the UNMET branches run `4/11`, WORSE THAN SILENCE.* **MISSING PRIMITIVE (code gap established, causal story NOT):** the goal-holder is RECIPIENT/BENEFICIARY and `goal_typing.py:776-777` implements only SUBJECT and DIRECT OBJECT.
> **FLOORS NOBODY HAD RUN, AND THEY INVALIDATE PAST GRADES HERE: a ONE-LINE NEGATION COUNTER scores `0.8056` where the four-tier cascade scores `0.4722`** (null p95 `0.6944`). **NEVER SHIP IT** -- a lexical cue detector is the word-counting pattern; it RAISES THE BAR. *What it proves: the evidence IS in the text in plain words -- the machinery is not missing information, it is failing to use information that is right there.* **LENGTH-MATCHED CONTROL (13/13 pairs): `text_length` COLLAPSES to its own null, NEGATION SURVIVES `0.7692`, ours goes below-floor -> AT floor on a ONE-ITEM margin. THE WALL IS NOT A MEASUREMENT ARTIFACT.** *`AMBIGUOUS` is an abstention in 5 consumers and a wrong answer in the ONE cell this line is graded on -- filed as `score_counts_abstention_as_error`.* `THE_ORGAN_DOES_NOT_ANSWER_WRONG_...` `MET_RECALL_IS_8_OF_23_...` `THE_CASCADE_IS_A_COIN_FLIP_...`
>
> ## **THE GROUNDING MAP -- `185` LABELS -> `7` EVIDENCED ROWS -> `4` DISTINCT RESULTS**
> **`711` grounding cells with a `metrics.json`; `185` HARD_PASS; `7` carry BOTH a CI and a null; `4` distinct at full scale:** `reading_grounding_loop_cycle2` (`185 -> 3,544`, scramble `0.077`); `foundation_validation_harness_v1` (mechanism `1.0` vs scramble `0.0` AND ablation `0.0` -- cleanest control result in the set); `v4_proximity` (`0.2667` over a `0.22` frequency floor); `graded_divisive_comparator` (n=4,000, d `0.0602` CI `[0.0440,0.0762]` EXCLUDING ZERO -- best-evidenced number in the archive, and a DISCRIMINATION gain, NOT a capability). **A BLIND SPOT: `context_conditioned_near_neighbour_v1_SMOKE_n600` = HARD_PASS while its FULL run = `MIDDLE_BAND_FLOOR_HUGGING` -- anyone counting HARD_PASS rows gets the SMOKE's answer.** *The other `178` are NOT refuted, they are UNEVIDENCED IN THE FILE.*
> ## **THE AUTOMATIC QUALITY CRITERION IS REFUTED -- AND THE FIX WAS ALREADY ON DISK**
> **Same `100` blind human-scored rows: human GOOD `0.864` vs human NOISE `0.859`, difference `+0.0047`, Fisher `p = 1.0000` -- THE CRITERION CANNOT TELL MEANING FROM NOISE.** It certifies TOPICAL ADJACENCY, which is what a reader calls noise (`whisky->wedding`). *A graded count looked monotone until Bonferroni (`p = 0.1745`): it did not survive the search that found it.* **`v4`'s PROXIMITY criterion DOES work -- GOOD `0.591` vs NOISE `0.244`, `p = 0.0038`, surviving Bonferroni x7. THE DECIDING DIFFERENCE IS PROVENANCE: `PROXIMITY_WINDOW=6` came from corpus structure BEFORE any result existed and was tested ONCE.** WIRED as `hdlab/quality_proxy.py`. **A BETTER RULER, NOT BETTER FACTS -- the blind score is still `3/19/78`.** *Two calibration traps shut: a filter's hit rates are properties of the DISTRIBUTION, not constants of the filter; and a sample built to measure a DELTA may not be quoted as a LEVEL.* **THE FOUNDATION-WIDE MEANINGFUL FRACTION IS UNMEASURED.** *A representative blind sample is DRAWN AND WAITING (n=`150`, drift `0.0584`); I WILL NOT SCORE IT -- whoever draws a blind sample must not label it.*
>
...
> 1. **CA3 cannot complete** -- our rule MERGES correlated memories (margin `-0.1021`; local error-driven rule fixes it to `+0.0975`). ***BUT REAL WITHIN-LEMMA OVERLAP IS `0.0056` vs A `0.22` FAILURE THRESHOLD -- the regime never occurs, so DO NOT BUILD IT.***  2. **Surprise cannot gate writes** -- real spread (sd `0.096`) but UNCORRELATED with value (r `+0.238`, spans zero). ***Gating on an uncorrelated signal IS gating at random.***  3. **12-dim feature context is FLAT** -- `0.0266` at L=37 vs incumbent `0.1328`; NORMS12's own note predicted it.
> 4. 🚫 **ANISOTROPY / rank-1 removal: CLOSED HARD.** *Removal fully worked (`0.1427 -> -0.0004`) for accuracy `+0.0005`, and **a RANDOM rank-1 direction gives the SAME `+0.0005`** -- perturbation, not decorrelation-that-helps.* ⚠️ *The revival `*` is for FULL-COVARIANCE WHITENING, which this closes in NEITHER direction.*  5. 🚫 **DIVISIVE NORMALISATION: do not re-propose** -- analytic, the denominator is a scalar and cosine is scalar-invariant.
>
> ## 🌙 **THE 08-21 PASS -- ALL BAG-OF-WORDS; KEPT AS FLOOR, NOT DIRECTION.** **VERBS `+0.0000` (null `0.0372`) vs NOUNS `0.1310` clearing `0.0843` -- weak on nouns, ABSENT on verbs.** **The floor ladder: raw counting `0.0885` | OURS `0.1071` | idf-counting `0.1835` | supplied `0.2876` -- a plain counting baseline BEATS US and SUPPLIED beats LEARNED `2.69x`.** *Levers closed: sentences saturate ~10, dimensions buy identification not meaning.* ➡️ **This is the evidence base for PRIORITY 2's question.**
> ## ⚠️ **OWNER-FACING: `BOARD.md` IS A MIRROR AND EDITING `STATUS.md` DOES NOT REFRESH IT** *It carried "BOARD: NOTHING OPEN" for ~2 hours beside its own table of 2 open rows.* ➡️ **A doc MIRRORED by code goes stale silently, and the stale copy is the one the owner reads.** **RUN `python tools/board.py sync` AFTER EVERY `STATUS.md` EDIT.**
>
> ## 🧭 **METHOD -- THE RULES THAT PAID OUT** ***PUT A NUMBER BESIDE ANOTHER NUMBER THAT CONSTRAINS IT*** *(killed 5 claims, cleared 3);* **QUOTE THE NOTE, NEVER THE HEADLINE;** **CHECK PRIOR WORK FIRST** *(changed the answer 3x that night and 5x since);* ⭐ **MAKE THE ARITHMETIC CLOSE TO THE REPORTED NUMBER** *-- three plausible stories died to that one check, which was available every time and which I never ran.*
> ## ➡️ **NEXT -- 2026-08-22 (3rd consolidation). THIS SECTION IS ACTIONS; CLOSED WORK IS ONE LINE EACH BELOW.**
> **DO NEXT. THE FILED PROBLEMS ARE THE SOLVER'S. THE RANKING LIVES IN EACH `notes/problems/<slug>/PROBLEM.md` FRONTMATTER (`priority:`), WHICH IS WHAT THE GUI RENDERS -- `README.md` DESCRIBES THE PROTOCOL, IT DOES NOT HOLD THE ORDER. ENUMERATE, NEVER MIRROR.**  **A/B/C/D ALL CLOSED 08-22:** *A -- the v2 scoring is DONE (see the `0.2339` block above); B and C were WITHDRAWN AS MINE and FILED (`cortical_read_has_no_scored_path` -- now SOLVED and REVIEWED, `certification_gate_hangs` -- open); D -- this block is FOLDED to its cap.*
> **E. INTEGRATION IS THE STANDING JOB AND IT IS MINE ALONE:** re-verify each `SOLVED.md` ON THE ARTIFACT (never by re-running the solver's pipeline -- a re-run shares its bugs), land the `hdlab/` and harness diffs solvers are barred from touching, REVIEW the submission at the TOP of its brief, and set `priority:`/`review:` in the frontmatter. **NOTHING IS AWAITING IT RIGHT NOW.**
>
> **CLOSED, ONE LINE EACH -- numbers kept, reasoning in the named notes:** **1. Q102 form organ -- DONE + WIRED** (full write-up in the digest above). *Numbers the digest lacks:* **BRAIN BAR (VWFA = INVARIANCE): case `1.0000`, inflection GRADED `0.31-0.51`, unrelated `0.03-0.08`; `cos(entombment,burial) = -0.023` so it CANNOT inflate meaning. THE LIVE CODE FAILS THAT BAR -- `symbol_vector` is a sha256 draw, case invariance `-0.0026` vs `+1.0000`.** ⚠️ **NOTHING CONSUMES THE FORM CHANNEL YET.** 🔧 *Process: my `--limit` flag dropped the bare value only `if limit != 40`, so `--limit 40` left "40" as a SEARCH TERM -- **`1,106` hits against a true `4`, a `275x` error I nearly acted on.***  **2. DIRECTION = ANCHOR + PROPAGATE** (set 08-06/07; ground a SMALL affective anchor -- good/bad is in neither grammar nor text statistics because **ANTONYMS ARE DISTRIBUTIONAL TWINS**). ✅ **FOUR components `HARD_PASS`, THREE now REPRODUCED:** *S1 12-word seed -> `0.833` held-out with **ablation `0.000`** both identical, `theta_witness` RECIPROCITY `+0.603` vs BLOCK_HIGH `-0.505` SIGNS DIFFER; S2 map collapse `1.000` vs scramble `0.300`; S4 learned-from-exposure `1.000` vs scramble-consequence `0.429`, `spoil_ok=True`.* ⚠️ **REPRODUCTION DOES NOT CHANGE SCALE: 6 words / 28 items on CLEAN signal. SOUND != WORKS.**  **3. 🚫 THE CREDIT LEVER IS MEASURED OUT.** *`HARD_FAIL` primary `0.4167` vs floor `0.6389`, lift `0.0167`, precision `0.4676 -> 0.4941`.* 🔁 **The wall reproduces TWICE, 15 days apart, two lemmatizers -- `0.4722` IDENTICAL TO FOUR DIGITS.** 🎯 *Precision problem is LIGHT VERBS: `53.2%` of credited exposures not loaded, `46` of 173 error types are GENUINE VERBS, **non-verb junk only `5.4%` of credited tokens** (measured 3x: `73%` -> `12.2%` -> `5.4%`; I quoted the biggest twice first). Coverage is NOT the cause (`439/1655`).* `I_RE_RAN_THE_WALL_...`  **4. ✅ THE GROUNDING ORGAN WORKS; MY OWNER REPORT WAS SCOPED WRONG.** *`governor_grounding` HARD_PASS `0.967` vs **bow `0.517`**; `twostage_event_situation_v2` B and C `1.000` vs governor `0.500`; DOUBLE DISSOCIATION -- each subset's MATCHED scramble degrades, the UNMATCHED one does not.* 🔻 **SCALE: n = `21/12/12/8`, and one `1.000` IS n=2 -- it CARRIES NOTHING.** 🔻 **"counting beats us EVERYWHERE" WAS WRONG: all 16 measures are the WORD-SIMILARITY channel, which both plans ruled out (bow `0.5167` = chance HANDED THE GOLD SENSE).** ⚠️ *READ, not REPRODUCED (`399`/`7,868` cells replay).*  **5. ✅ OPEN VOCABULARY: CLOSED.** *`B` 10/12 = `0.8333`, `Bgen` 6/8 = `0.750`, and **ALL FOUR ERRORS HAVE AN ADVERSARIAL PATIENT** -- the animacy map cannot express it, an EXPRESSIVENESS GAP. WordNet supplies it `7/8` at a `0.8%` base rate and **misses `rival` CORRECTLY**.* 🔑 **THE `== "UNK"` GUARD IS A DELIBERATE HAND-OFF -- DO NOT TOUCH IT.**  **7. THE HONEST TENSION:** *the supplied table beats everything we LEARN on the word-similarity channel.* **Owner ruled supply ADMISSIBLE (08-16), so this is PRIORITISATION, not permission.**
