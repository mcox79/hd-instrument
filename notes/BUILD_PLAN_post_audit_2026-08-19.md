...
> ⏱️ **AND IT DATES THE STALENESS TO A ONE-DAY WINDOW: the 08-06 run had `0` firings, the 08-07 baseline already shows `referent_mismatch` firing -> THE CASCADE SWITCHED ON BETWEEN 08-06 AND 08-07.** 🔻 *Drift continued: recomputing the full 44 today gives `18`/`6`/`20`, so since 08-07 it gained 1 correct, gained 2 wrong, shed 3 abstentions -- **PRECISION WHEN COMMITTING FELL `0.8095 -> 0.7500`.*** 🚫 *The exact commit is NOT identified; that needs a bisect, not run. Bounded by the two dates only.*
