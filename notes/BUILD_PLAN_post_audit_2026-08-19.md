...
> ✅ *Controls all present and one is the information-free twin doing its job: random-sign episodes at the same count/grouping score `0.0000` held-out. Addressed DOES beat the flat sum `+0.0554 [+0.0457,+0.0656]` -- a real sub-finding, kept as WIRE_CANDIDATE, not a capability win.*
> ➡️ **I RANKED THIS #1 AS "upstream of everything, and a wiring job". THAT WAS WRONG -- the store was not the binding constraint, and the bar caught it.** *Corrected in the ranking table; a solver reading it would otherwise have started on a dead problem.*
>
...
> 🚫 **DO NOT QUOTE THE `66%` AGAINST "v1's ~8%".** *I measured v1's empty-overlay abstention at `20/36 = 55.6%` today; the two cannot be reconciled from here and are different arms or conditions. The `66%` stands on its own; the COMPARISON does not.*
> ➡️ **THE OPEN QUESTION IS WHY v2 TRIGGERS ABSTENTION AT ALL** -- likely a pattern-coverage gap on the new prose rather than an outcome-valence failure. *The agent declined to author that diagnosis, correctly.*
>
...
> **DO NEXT. THE FILED PROBLEMS ARE THE SOLVER'S. THE RANKING LIVES IN EACH `notes/problems/<slug>/PROBLEM.md` FRONTMATTER (`priority:`), WHICH IS WHAT THE GUI RENDERS -- `README.md` DESCRIBES THE PROTOCOL, IT DOES NOT HOLD THE ORDER. ENUMERATE, NEVER MIRROR.**
> **A/B/C/D ALL CLOSED 08-22:** *A -- the v2 scoring is DONE (see the `0.2339` block above); B and C were WITHDRAWN AS MINE and FILED (`cortical_read_has_no_scored_path` -- now SOLVED and REVIEWED, `certification_gate_hangs` -- open); D -- this block is FOLDED to its cap.*
