...
>
> ## **LANDED 08-22 (this session, integration lane): BOTH SOLVER HAND-OFFS ARE IN**
> **SH-7 `HDI_FRESH_RUN=<tag>`** (`experiments/_seed_checkpoint.get_output_dir`, `ed9ce6273`) sends a cell to an EMPTY sibling so it genuinely recomputes -- no deletion, landed dir never opened for writing, byte-identity BY CONSTRUCTION. **The load-bearing control is the NEGATIVE one and it is a witness, not a comment: env unset -> paths byte-identical, else every landed directory in the repo is silently orphaned with nothing erroring** (`test_fresh_recompute_redirect.py` 6/6, and I injected the bug to confirm the guard FIRES). **COVERAGE IS `87` of `421` cells (~21%); `275` bare-`OUTPUT_DIR` cells keep replaying until each gets a one-line wrap; `59` sources unlocated.** *Recounted from disk, not inherited from the brief.*
> **B3' NOW CARRIES A SCORED VERDICT, NOT "unscored"**: best cortical arm hit@1 `0.057/0.043/0.043` vs a COUNTING floor `0.090/0.103/0.097`, clearing NOTHING at any k on any seed, CI-separated against it at k=50. **STAYS `NEEDS_ADAPTER` ON EVIDENCE RATHER THAN ON ABSENCE OF EVIDENCE -- a different claim.** *Revival criterion is BRAIN-framed: the cloze rewards the EPISODIC style, so the organ earns an adapter only if its retrieval SPACE beats counting WHERE COUNTING CANNOT HELP, on a task powered for it (the stratified attempt reached n=`43` -- a direction, not a verdict).*
> **BOTH SUBMISSIONS REVIEWED `EXCELLENT`, feedback at the top of each brief as the owner asked.** *Each REFUTED this brief's own premise, and one found a real bug in MY detector (`unit_count` counted LINES, not distinct units -- `21` of `421` cells repeat a key, a `0.65%` overstatement; fixed, and it moves NO cell across the replay boundary).* **`reproduction_check.unit_count` and `cite_check` unreadable-file reporting also landed.**
> 🚨 **AND SH-7's COVERAGE IS NOT STATIC -- IT SHRINKS (08-22, powered).** New primary cells routing through the harness fell **`90.8%` (June) -> `47.3%` (July) -> `27.2%` (August)**, n=707-2,869/month; the file-generation confound is RULED OUT (variant siblings are n<=2/month). **So clearing the 275-cell backlog while new bare cells keep arriving does not close the gap -- the leverage is at AUTHORING time, and that is `Q115` on the board.** 🚫 **THE ONE-HOOK SHORTCUT IS REFUTED, DO NOT RE-PROPOSE:** redirecting inside `exp_checkpoint.write_metrics` reaches `85` of `1,584` bare cells (`5%`); 849 write `metrics.json` with a raw `json.dump`. *The units side IS shared -- 208 call `record_unit` -- which is exactly what makes a units-only redirect tempting and WRONG: it forces a recompute and leaves the landed `metrics.json` to be overwritten.* `THE_REPRODUCIBILITY_HOLE_IS_GROWING_...`
> ✅ **GUARDED AT RUNTIME RATHER THAN ARGUED IN PROSE (`3ff233548`):** ask for a fresh run on a cell that cannot isolate and `completed_units`/`record_unit` now SAY SO instead of silently overwriting the landed record. **THE DANGEROUS CASE WAS NEVER "the redirect is unavailable" -- IT IS "THE OPERATOR BELIEVES THEY ARE ISOLATED AND IS NOT".** *Base rate checked BEFORE building it: can only fire when the env var is set, so zero false positives BY CONSTRUCTION. 3 of its 7 tests are SILENCE tests, because it sits on the hot path of every multi-unit cell.* `verification/test_fresh_run_not_isolated_warns.py` 7/7.
