...
>
> ## 🔓 **THE "STRUCTURALLY CLOSED BUILD LANE" IS NOW FALSE ON ITS SECOND CLAUSE TOO, AND THAT CHANGES THE BLOCK BELOW (08-22, continuation 9)**
> **The block below rests on two premises. Q109 already retracted the first (`verification/` was open all along). THE SECOND -- *"agent spawning is disallowed in this session"* -- IS ALSO FALSE:** the full `hdi_*` fleet resolves in this session's tool inventory (`hdi_exp_dev`, `hdi_skunkworks`, `hdi_orchestrator`, `hdi_testbed`, `hdi_research`), and the owner said so directly on 08-22: ***"agents are 'off'???? You should definitely have agents."***
> ➡️ **SO CELL WORK IS NOT IMPOSSIBLE HERE -- IT IS ONE SANCTIONED HOP AWAY.** `CLAUDE.md`'s tripwire does not forbid cell work; it *routes* it to `hdi_exp_dev`. **With the fleet available, that route is open, and the conclusion "~295 more continuations will change the system ZERO times" no longer follows from anything measured.**
> ⚠️ **WHAT STILL BLOCKS IT IS A POLICY, NOT A STRUCTURE:** this session's operating instructions say *do not call the Agent tool unless the user requested it*. **That is an owner switch, not a wall** -- filed as a board question rather than decided by me, because it governs whether the remaining continuations can touch `hdlab/` at all.
> 🔑 **THE LESSON IS THE ONE I KEEP RE-LEARNING: I DIAGNOSED A CONSTRAINT I HAD NEVER TESTED.** *Q103, Q104, Q108 were all withdrawn for exactly this, and the block below says so about itself. **This is the fourth -- and the untested premise sat in the SAME SENTENCE as the one I had already retracted, which I read past twice.***
>
...
> 🔻 **Q108 WITHDRAWN BY ME WITHIN THE HOUR -- MY PREMISE WAS TOO BROAD; RE-FILED NARROWER AS Q109.** *I said the BUILD LANE was closed. It is not: `CLAUDE.md` routes away `experiments/*.py` edits and cell RUNS, and names neither `verification/` nor `hdlab/`. **`verification/` was open the whole time and I proved it by using it minutes later** -- `verification/test_abstention_convention_on_goal_bearing.py`, 4 checks, now in the certification suite.* ➡️ **THE ACCURATE STATEMENT: the loop can MEASURE, DOCUMENT and GUARD; it cannot RUN AN EXPERIMENT or edit a cell. So defects it finds get GUARDED but not FIXED.** *Third question of mine this week withdrawn for a wrong premise (Q103, Q104, Q108) -- **the pattern is that I file before I have tested the constraint I am complaining about.***
> ✅ **AND THE GUARD IS POSITIVE-CONTROLLED ON ITSELF, which caught a defect in it within a minute: the load-bearing check sat BEHIND an `len(rows) == 36` shape assertion, so my first positive control tripped the SHAPE check and never exercised the real one.** *"A gate's THRESHOLD is not the only thing to check -- check what it EXITS BEFORE." Extracted into `assert_no_miscounted_abstention(rows)` and the positive control is now a TEST, not a thing I ran by hand once.*
>
...
>
> ## ✅ **FIRST PROBLEM SOLVED, RE-VERIFIED AND INTEGRATED -- THE STRATEGY/SOLVER LOOP CLOSED END TO END (08-22).**
> **`stored_terms_are_stems` -> `PARTIAL`, UPHELD.** *The bar said "fix it"; the finding is that there was nothing live to fix, which is a better answer than the one asked for.*
> 🔑 **THE STEM DAMAGE IS A STALE DATA ARTIFACT, NOT A LIVE DEFECT.** `normalize_lemma` (`hdlab/reading_grounding_loop.py:230`) resolved to an UNGUARDED suffix stripper until `01093ac1f`/`7d6036bca` added an `is_known_word(residue)` gate; **`v2_qualityfix` was built the day BEFORE that fix landed.** *Runtime evidence, not grep: the pre-fix function turns **8,692** dictionary words into non-words -- reproducing the repair commit's own `8,692 -> 0` headline exactly.*
> ✅ **I RE-VERIFIED RATHER THAN ACCEPTING THE SUMMARY.** *Their `reverify` end to end (89 s): stale `119/1512 = 7.87%`, fresh HEAD store `0/141 = 0.00%`, detector controls PASS. **Plus an INDEPENDENT spot-check I chose** -- HEAD's live `normalize_lemma` on the owner's own 13 cited words: **all pass through untouched**, and the ONE word that changes is `arteries -> artery`, **correct lemmatisation, which doubles as the positive control for "prove you did not break lemmatisation". A run where nothing changed at all would have been the suspicious result.***
> 🔻 **CONSEQUENCE, AND IT BINDS: `7.9%` MAY NO LONGER BE QUOTED AS A PROPERTY OF THE SYSTEM.** It is a property of one stale artifact; any grounding-quality number computed on `v1`/`v2_qualityfix` inherits it and must say so.
> 🎯 **AND IT DECIDES A COUPLING `substrate_never_resumes` had only FLAGGED: the two resumable snapshots are EXACTLY the two pre-fix ones, so "load an old foundation" feeds `7.87%` guaranteed junk into the very anchor pool whose degeneracy that problem exists to measure. FIRST MOVE THERE IS NOW: BUILD A CLEAN SNAPSHOT, do not load `v2q`.** *Brief updated in place.*
> ✅ *Credit: the solver's detector caught its OWN false positive -- function words (`and -> andes`) inflated the fresh count to `10/92` until excluded, giving the true `0/92`. **A negative control that fires on its author is the thing this project keeps saying it wants.***
> ⚠️ *Protocol defect found while integrating: the README said the strategy session must not EDIT a `SOLVED.md`, which made the integration marker `problem_ledger.scan()` requires into a violation of my own protocol. Narrowed to "must not REWRITE"; the marker is an APPEND and the solver's text is never touched.*
>
...
> 🔻🔻 **CORRECTION 3, SAME DAY -- AND IT REINSTATES THE ORIGINAL FINDING. I WAS WRONG THREE TIMES, IN ALTERNATING DIRECTIONS.** *The measured numbers never moved; my story about them did.*
> **NEWLY MEASURED: all NINE foundation dirs were written `2026-08-12` -- THE SAME DAY -- and only `v1`/`v2_qualityfix` are LOADABLE; `v3`/`v4`/`v5` lack ALL FOUR artifacts `load_foundation` requires (`store/`, `concept_space.npz`, `library_pending.json`, `manifest.json`).**
> ➡️ **SO `v3`/`v4`/`v5` ARE NOT LATER VERSIONS OF `v2q` -- THEY ARE A DIFFERENT PIPELINE'S OUTPUT** (definitional-extraction fact dumps, not resumable reading-loop states).
> 🔻 **WITHDRAWN: "we hand-scored a three-versions-stale foundation".** *All one day old; `v2q` is the CORRECT artifact for the question the sheet asked, and sampling it was right.*
> 🔻 **WITHDRAWN: "the stem fix already shipped, so `fix the stemmer` is retired".** *The `0.4%` is definitional extraction's DEFINIENDA; the `7.9%` is the grounding loop's SUBJECTS -- **different pipelines, different populations. I invoked the no-number-crosses-populations rule twice today and then broke it here.*** The charter's `16.1% -> 1.0%` is about definitional extraction's TERM BOUNDARIES, not the grounding loop's lemmatisation.
> ✅ **WHAT STANDS: the grounding loop stores ~`8%` STEMMER OUTPUT AND IT IS *NOT* KNOWN TO BE FIXED. The only surviving part of correction 1 is the `24% -> 10.4%` deflation, which was a real detector defect.** `THE_GROUNDING_LOOP_STORES_8pc_STEMMER_OUTPUT_AND_I_WAS_WRONG_THREE_TIMES_...`
>
> ## 🚨🧠 **AND CHASING "WHICH FOUNDATION IS LIVE" FOUND THE CAUSE OF THE GENERIC-ATTRACTOR DEGENERACY: NOTHING LOADS ONE. THE SUBSTRATE IS PINNED IN COLD START.**
> **RUNTIME, not grep** -- `load_foundation`/`save_foundation` wrapped in counters, `Substrate(foundation_dir='...v5_termboundary')`, 120 sentences: **`load_foundation` calls `0`; store facts `92` before and `92` after; `92` derives from the `107` SEED WORDS.** 🔑 **`self.foundation_dir` is assigned at `substrate.py:378` AND NEVER READ AGAIN -- THE PARAMETER IS DEAD.**
> ✅ **ENUMERATED, COMMENTS SEPARATED FROM CALLS: `load_foundation` appears in 6 files -- its own module, TWO DIAGNOSTICS, one test, one atomiser, and a COMMENT at `reading_grounding_loop.py:1732`. NO LIVE READING PATH CALLS IT.** 🔻 *Grep got this wrong in BOTH directions first (I nearly filed "never persists either" -- `save_foundation` IS called at `:813` -- then nearly filed "the live loop loads it" off that comment). **The exact trap CLAUDE.md documents, on the module the rule was written about.***
> 🎯 **AND IT MAKES THIS PLAN'S OWN DEGENERACY PREDICTION UNREACHABLE BY CONSTRUCTION.** *The plan says the `way`-attractor (`17.7%` of grounded terms) comes from "early grounding forced to choose among ~107 generic seeds" and **"predicts the degeneracy should FALL as the grounded vocabulary grows"**.* **IF NO RUN LOADS THE PREVIOUS RUN'S FOUNDATION, THE VOCABULARY CANNOT GROW ACROSS RUNS. Every run re-enters the 107-seed bottleneck -- the degeneracy is arithmetic, not tuning and not scale.**
...
> 📋 **QUEUE -- ALL FIVE BRIEFS NOW WRITTEN:** `stored_terms_are_stems` (**FIRST** -- small, certain, owner-discovered, tests the loop) · `reader_meaning_channel` (**the flagship**; ON HOLD pending an off-disk verification of its number table, which came from plan PROSE rather than `metrics.json`) · `flat_store_destroys_the_code` · `substrate_never_resumes` (runtime-verified) · `eval_bank_too_small` (**this session is DISQUALIFIED -- it has read the per-item predictions; a solver has not**).
> 🔑 **ONE PLANNED BRIEF WAS MERGED RATHER THAN WRITTEN: "read-out discrimination" IS `reader_meaning_channel` from the other side** -- the pick-one-of-50 instrument is exactly where the sensorimotor result was measured, and the `0.3104` co-occurrence ceiling on it is what says the missing information is not in co-occurrence at all. *Writing both would have handed two sessions the same task under different names -- the duplicated-work failure this protocol exists to prevent, nearly committed by its author.* 🚫 **The CLOZE framing stays RETIRED: its ceiling is a tie with word-counting (`0.0300` vs our `0.0150`).**
> 📄 **The paste-able solver prompt is ON DISK at `notes/problems/SOLVER_SESSION_PROMPT.md`** -- chat scrolls and does not survive compaction, which is the same reason `BOARD.md` exists.
...
> ✅ **Q111 ANSWERED (owner, 08-22): THIS SESSION OWNS ALL INTEGRATION; THE SOLVER DOES NOT WRITE TO `hdlab/`.** *Owner: "if we fracture our live substrate modification I fear we'll lose sight of state and keeping it fully updated. I want THIS session to own it all and ~subcontract out the target research to that other session."* ➡️ **THE HAND-BACK IS A RESULT PLUS A PROPOSED CHANGE, NOT A LANDED ONE:** solvers prove the mechanism in `experiments/` and `verification/` and state in `SOLVED.md` what would have to change in `hdlab/`; **this session re-verifies and lands it, as sole writer.** ✅ *Corrected in all FIVE briefs, the protocol and the paste prompt the same hour -- they had all said "you may write `hdlab/`".* 💡 *Owner also floated a SECOND substrate for the research session to work on -- worth doing, but a bigger design question; parked until the loop has run once.*
>
...
>
> ## 🔑 **AND THAT LEAD IS RE-AIMED (08-22, later): IT IS NOT ANSWERING WRONG -- IT IS NOT ANSWERING. `20` OF `22` ERRORS ARE NON-ANSWERS.**
> ✅ **RAN THE KNOWN-ANSWER ARM THAT HAS SAT IN THE BANK UNRUN SINCE 08-06.** *`tools/known_answer_arm_goal_bearing_in_lexicon.py`; POSITIVE CONTROL REPRODUCES THE DOCUMENTED EMPTY-MAP `0.3889` TO FOUR DIGITS (`delta -0.0000`) -- and it REFUSED to print anything the first time, when I had targeted `0.4722`; **the refusal was right and my constant was wrong**, `0.4722` being the SOFT-COMBINE condition.*
> 🔑 **FIRST, A STRUCTURAL FACT NOT WRITTEN DOWN ANYWHERE: "the 36-item bank" IS NOT A SEPARATE BANK -- it is the `outcome_in_lexicon is False` subset of the 44-item `goal_bearing_modern_eval_v1` (`:126`), and the excluded `8` are the construction note's *"deliberately in-lexicon SANITY-CHECK CONTROLS"*.** 🔻 **AND I GOT THAT ENUMERATION WRONG ONCE, CAUGHT BEFORE COMMIT: I wrote "SCORED BY NOBODY" from the `7` repo-wide `outcome_in_lexicon` hits (all `is False`) -- but `exp_verbclass_backoff_coverage_v1`/`v2` load the bank with NO FILTER AT ALL and score the full 44 through the SAME function. I ENUMERATED A FIELD NAME AND CONCLUDED SOMETHING ABOUT SELECTION.** ✅ *SURVIVES, and properly enumerated because the split REQUIRES that field: **no cell ISOLATES the 8 as a known-answer arm and none reports an in-lexicon vs OOV CONTRAST** -- a number folded into a 44-item aggregate is not a sanity check anyone read.*
> 🎯 **THE ERROR COMPOSITION ON THE OOV 36 (empty overlay): accuracy `14/36 = 0.3889`, `NONE` returned `20/36 = 0.5556`, accuracy WHEN IT COMMITS `14/16 = 0.8750`, GENUINELY WRONG ANSWERS `2/36`.** ➡️ **A COVERAGE FAILURE, NOT A DISCRIMINATION FAILURE -- converging with the 08-07 charter line *"HIGH-PRECISION + COVERAGE-LIMITED; coverage is the universal wall."*** ➡️ **AND IT RE-AIMS THE LEAD ABOVE: in the EMPTY condition the function is NOT `UNMET`-biased, it is SILENT -- the bias is a property of the OVERLAY condition, not of `congruence_with_lexicon_fallback`.**
> 🔻 **SUPPLYING THE WORD REMOVES THE SILENCE AND DOES NOT BUY THE ANSWER: in-lexicon 8 return `NONE` `0/8` vs OOV `20/36`, Fisher `p = 0.0049` -- and then score `4/8 = 0.5000` against a BALANCED `0.5000` floor, binomial `p = 1.0000`. IT CONVERTS SILENCE INTO GUESSING.** *Independent re-derivation of this plan's own **"COVERAGE WITHOUT DISCRIMINATION BUYS NOTHING"** on a different slice.*
> ⚠️ **THE PRE-REGISTERED ARM IS INCONCLUSIVE AND SAID SO BEFORE RUNNING: at n=8 only `8/8` clears `p<0.05`, so THIS ARM CAN DEMONSTRATE COMPETENCE AND CANNOT DEMONSTRATE INCOMPETENCE. NOT filed as a negative.** *Both pre-registered hypotheses fail; H2 (the UNMET bias) is positively contradicted here -- predictions run `MET 3 / AMBIGUOUS 2 / UNMET 3`.*
> 🚫 **SCOPE, AND THIS ONE MATTERS: `abstain_fallback_to_lexicon` IS NOT THE CASCADE FIRING. The structural rules read `9/10` HERE, and that MAY NOT BE CROSSED WITH THE `10/19 = 0.5263` "coin flip" ABOVE** -- different overlay condition, `20` rule firings there vs `10` here, `referent_recurrence` absent from that list entirely, and n=10 either way. **WHETHER THEY CONFLICT IS UNRESOLVED AND IS NOT CLAIMED SETTLED; the overlay condition needs re-running per-reason.** ⚠️ *Filling every `NONE` with the majority reads `28/36 = 0.7778` over the `0.6389` floor -- **RECORDED TO SIZE THE SCORING CONVENTION, NEVER TO SHIP: it is FITTED to this bank's majority**, and the error-composition line makes the point with no fitted policy at all.*
> `THE_ORGAN_DOES_NOT_ANSWER_WRONG_...`
>
> ## 🚨 **AND THE LANDED RECORD IS STALE IN A WAY THAT CHANGES THE DIAGNOSIS: THE `HARD_FAIL` WAS MEASURED WHERE THE CASCADE FIRED `0` OF `36` TIMES (08-22).**
> **No re-run and no new data -- the landed run STORED its own per-item details in `units.jsonl` and nobody had opened them.** *Same 36 items, both EMPTY overlay:* **landed (started `2026-08-06T20:05:00Z`) accuracy `0.1667`, `NONE` `29/36`, STRUCTURAL FIRINGS `0`, of which correct `0` -- EVERY item reads `reason = abstain_fallback_to_lexicon`, so all 6 of its correct answers came from the goal-independent word lexicon. TODAY at HEAD: `0.3889`, `NONE` `20/36`, FIRINGS `10`, correct `9`.**
> 🚫 **THE VERDICT DOES NOT CHANGE -- `0.3889` is still far below the `0.6389` floor and `HARD_FAIL` STANDS. WHAT CHANGES IS THE DIAGNOSIS.** ➡️ **THE STRUCTURAL-RULE ANALYSIS ABOVE (`same_class_same_referent` -> `MET` x9, `referent_mismatch` -> `UNMET` x8, ~`20` firings) CANNOT BE DESCRIBING THE LANDED RUN, WHICH HAD ZERO. It describes a recent OVERLAY measurement, while the landed `metrics.json` still reads `0.1667`.** *Discipline 4 -- ESTABLISH THE FINAL LANDED VERSION BEFORE EVALUATING A SUBSYSTEM -- for the 7th time; same shape as the cert repair's **"THE SYSTEM IMPROVED AND THE TEST FAILED"**.*
> ✅ **SIX CHECKS RUN BEFORE FILING** *(right file / version -- dated by `_start_marker.json` / env / corpus n=36 both sides / same `_score` schema, their stored number NOT recomputed / both arms empty, theirs by `n_registered=0` collapsing `main` onto `baseline`)*. ⚠️ *`units.jsonl` is GITIGNORED (`.gitignore:53`), so it is a LOCAL artifact dated by marker+mtime, not by git; the tool SKIPS rather than fabricates when it is absent.*
> 🎯 **AND IT ADDS A FREE THIRD POINT TO THE PINNED PREDICTION: firings/correct go `0/0` (landed) -> `10/9` (today empty) -> `19/10` (overlay). DECLINING MARGINAL PRECISION -- the first 10 firings return 9 correct, the next 9 return 1.** 🚫 *STILL A HYPOTHESIS: the third point crosses conditions and the `19` also conditions on goal recognition.* ⚠️ **RE-LANDING THIS CELL IS THE FIX AND IT NEEDS AN EXPERIMENT-CELL RUN (routed to `hdi_exp_dev`, NOT done here); the overlay per-reason split comes free with it.** 🔎 *Check `exp_verbclass_backoff_coverage_v1`/`v2` and `exp_grounding_acquisition_loop_v1` for the same staleness before quoting them.*
> `THE_LANDED_HARD_FAIL_WAS_MEASURED_WHERE_THE_CASCADE_FIRED_ZERO_TIMES_...`
>
> ## 🔻 **AND THE ARCHIVE WAS AHEAD OF ME AGAIN (08-22): ATTACKING THAT ABSTENTION HAS BEEN TRIED TWICE AND BOUGHT `1` OF `23`.**
> **I named the abstention as the target, then found two landed cells that already attacked exactly it with a Levin verb-class backoff table supplying the knowledge the cascade needs to decide:** *`exp_verbclass_backoff_coverage_v1` coverage_gain `1` / regressions `3` / held-out `4/8` vs bar `6/8` = **`HARD_FAIL`**; `v2` (last-resort placement) coverage_gain `1` / regressions `0` / `na 23 -> 22` / held-out `4/8` = **`PARTIAL`**, its own tiers reading `both_abstain 22`.* ➡️ **SUPPLYING VERB-CLASS KNOWLEDGE CONVERTED EXACTLY ONE ABSTENTION OF TWENTY-THREE AND FAILED HELD-OUT BOTH TIMES.** 🚫 **The abstention stands as the DIAGNOSIS; its obvious ATTACK is MEASURED OUT -- do not re-propose "supply knowledge so the cascade can decide" without reading those two cells.**
> ✅ **AND THEY INDEPENDENTLY REPLICATE THE PROFILE 15 DAYS EARLIER: their shared 08-07 baseline on the FULL 44 is `17` correct / `4` wrong / `23` na = precision `0.8095` when committing, abstaining on `52.3%`** -- against today's OOV-36 `0.8750` / `55.6%`. **HIGH-PRECISION-WHEN-IT-SPEAKS, ABSTAINS-ON-HALF IS STABLE ACROSS 15 DAYS, TWO POPULATIONS, TWO INDEPENDENT MEASUREMENTS.** *I did not know their numbers existed when I took mine.*
> ⏱️ **AND IT DATES THE STALENESS TO A ONE-DAY WINDOW: the 08-06 run had `0` firings, the 08-07 baseline already shows `referent_mismatch` firing -> THE CASCADE SWITCHED ON BETWEEN 08-06 AND 08-07.** 🚫 *The exact commit is NOT identified; that needs a bisect, not run. Bounded by the two dates only.*
> ✅ **SINCE 08-07 THE CHANGE IS ONE ITEM, IN THE GOOD DIRECTION: `17`/`4`/`23` -> `18`/`4`/`22`, NO REGRESSION.** *The gain is `ts_tom_wish_free_potter`, recovered today by **`same_class_same_referent` and NOT by the Levin backoff `v2` built for it** -- a later tier reached the same item by another route.*
>
> ## 🔻 **AND I RETRACT TWO OF MY OWN CLAIMS FROM THIS SAME DAY, ON THE REPO'S OWN CONVENTION.**
> **I COUNTED `AMBIGUOUS` AS A WRONG COMMITMENT; THIS REPO COUNTS IT AS AN ABSTENTION** (`verification/verify_levin_lastresort_backoff.py:51`, `_ABSTAIN = ("NA","NONE","AMBIGUOUS")`).
> 🔻 **RETRACTED 1 -- "having the word ELIMINATES abstention (`0/8`, Fisher `p=0.0049`)".** *Under the governing convention the in-lexicon 8 read `MET 3 / UNMET 3 / AMBIGUOUS 2`, so abstention is **`2/8` = `0.2500` vs `20/36`, Fisher `p = 0.2404` -- NOT SIGNIFICANT.*** ✅ *SURVIVES: accuracy is `4/8` EITHER WAY, at chance against a balanced `0.5000` floor -- **supplying the word still does not buy the answer.** The mechanism claim about silence is the part that fell.*
> 🔻 **RETRACTED 2 -- "precision when committing FELL `0.8095 -> 0.7500`".** *That used the miscounted `18`/`6`/`20`. Correctly it is `18`/`4`/`22` = **`0.8182`, FLAT-TO-UP. THERE IS NO PRECISION REGRESSION**, and `verify_levin_lastresort_backoff` pins `(17,4,23)` with `==` and **PASSES today** -- an existing guard was already asserting what I called unwatched.*
> ✅ **UNAFFECTED BY THE CONVENTION (no `AMBIGUOUS` occurs in the OOV 36): the `14`/`2`/`20` error composition, the `0`-vs-`10` firings comparison (reason-based), and the verbclass `1`-of-`23` prior work.**
> 🔑 **THE LESSON: THE NUMBER PASSED A POSITIVE CONTROL, A PRE-REGISTRATION AND A POWER CALCULATION, AND BROKE ON A DEFINITION I NEVER LOOKED UP -- written in a constant, in a file I had already opened for another reason.**
>
> ## 🚨 **AND CHASING THAT DEFINITION FOUND A REAL DEFECT: THE LANDED CELL SCORES ABSTENTIONS AS ERRORS, BY OMISSION, AGAINST FIVE CONSUMERS AND ITS OWN ENGINE (08-22).**
> **ENUMERATED over the consumers of `congruence_*`: `AMBIGUOUS` is an ABSTENTION in `verify_levin_lastresort_backoff.py:51`, `exp_verbclass_backoff_coverage_v1.py:214`, `v2.py:73`, `verify_request_response_typing.py:200` and -- in words -- `hdlab/consequence_learning_loop.py:219,235` (*"AMBIGUOUS -> abstain"*). It is a WRONG ANSWER in exactly one place: `exp_consequence_learning_loop_..._v1.py`, whose `_score` does `ok = (pred == gold)` and NEVER MENTIONS `AMBIGUOUS` IN THE FILE.**
> ➡️ **THE DISSENTER IS THE CELL THE WHOLE LINE IS GRADED ON, IT DISAGREES WITH ITS OWN ENGINE, AND IT DISAGREES BY ACCIDENT.** ✅ **IT HAS NOT BITTEN, MEASURED NOT ASSUMED: the OOV 36 returns `UNMET 7 / NONE 20 / MET 9`, ZERO `AMBIGUOUS`, so the landed primary is NOT wrong today.** 🔻 *It is one prediction away, and it WOULD be wrong on the in-lexicon 8 (`2` of `8`).*
> 🔻 **THIS ALSO REFINES MY OWN RETRACTION ABOVE: "the repo convention governs" was TOO CLEAN. The repo holds BOTH conventions in adjacent files, and the one I used is THE LANDED CELL'S.** ➡️ **So the abstention claim is CONVENTION-DEPENDENT and NOT QUOTABLE EITHER WAY; what survives is the convention-FREE half -- accuracy `4/8` under both readings.**
> ✅ **GUARD LANDED: `tools/score_with_abstention.py` -- an `AbstentionScore` that carries its own convention, NO call signature returning a bare accuracy, and `both_conventions().agree` flagging when the choice could change a headline.** *Self-test 6/6: the real failure (flagged), the claim that SURVIVED it (accuracy identical -- the guard must not imply otherwise), a NEGATIVE CONTROL (a clean population must not be flagged), and the landed OOV-36 shape -- **that assertion FAILS the day the population acquires an `AMBIGUOUS`, which is exactly when the omission stops being latent.*** ⚠️ **FIXING `_score` edits `experiments/*.py` -> routed to `hdi_exp_dev`, NOT done here; it would change no current number.**
> `THE_LANDED_CELL_SCORES_ABSTENTIONS_AS_ERRORS_BY_OMISSION_...`
>
> ## ⚠️ **OWNER-FACING DEFECT FOUND EN ROUTE: `BOARD.md` IS A MIRROR OF `STATUS.md` AND EDITING THE SOURCE DOES NOT REFRESH IT.**
> **`notes/BOARD.md` carried the header *"BOARD: NOTHING OPEN"* for ~2 hours WHILE ITS OWN QUESTION TABLE LISTED 2 OPEN ROWS**, because I edited `STATUS.md` and the mirror only refreshes when `tools/board.py` runs a syncing command. *Fixed by `python tools/board.py sync` (`2 open, 40 answered`).*
> ➡️ **THIS IS "A DOC PARSED BY CODE IS COUPLED TO IT" IN ITS OTHER DIRECTION: A DOC *MIRRORED* BY CODE GOES STALE SILENTLY, AND THE STALE COPY IS THE ONE THE OWNER READS.** *The file even contradicted itself on the same screen -- header versus table -- which is the `PUT A NUMBER BESIDE ANOTHER NUMBER THAT CONSTRAINS IT` rule catching a defect for free.*
> 🔧 **RECOMMENDED, DELIBERATELY NOT IMPLEMENTED (it changes hook runtime behaviour and belongs to whoever owns it): have `session_start_hook` flag `BOARD.md` whose mirror timestamp is older than `STATUS.md`'s mtime** -- the same treatment the registry report already gets for rows older than their report. **Until then: RUN `python tools/board.py sync` AFTER EVERY `STATUS.md` EDIT.**
