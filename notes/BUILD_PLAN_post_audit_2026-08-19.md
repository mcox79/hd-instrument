...
> ⚠️ **UNEXPECTED FLAG: `ASSET_V2_TOKEMB` and `ASSET_RETRAIN_TOKEMB` are ALSO inside that band (+0.0265), and their own shuffles score HIGHER (+0.0293). On this benchmark those arms carry no more meaning signal than their destroyed versions.** *One benchmark, so a FLAG not a verdict -- visible only because the shuffles were recomputed alongside.*
