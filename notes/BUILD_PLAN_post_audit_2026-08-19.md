...
> 🚫 **NOT CLAIMED: that every landed cell is affected** (this is `Substrate.read()`; cells driving `reading_grounding_loop` directly may differ -- **that enumeration is the follow-up**), that any published number is wrong (a result on 1,000 sentences is real *for 1,000 sentences*), or that the CAUSE is traced. `SUBSTRATE_READ_SILENTLY_READS_A_FRACTION_...`
>
