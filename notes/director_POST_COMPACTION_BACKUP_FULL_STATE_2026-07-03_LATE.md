...
## REGIME-MAP MECHANISM-CROSS-TERM FAMILY FULLY CLOSED (4/4) — Skunkworks `99a8228ef`, 2026-07-04T14:21Z

P6v2 + P7v2 paired-confirmed as artifacts (reproduce 6/6 exact; PAIRED mechanism spread = EXACTLY 0.0000 both probes at the non-saturated cliff; unpaired spread halves as TR quadruples = pure-noise; argmax-invariance root, same as P1). Tier MIDDLE_BAND -> MEASURED_MECHANISM (promotion of certainty — cross-term PROVABLY absent, not merely unproven). +1 MM (one combined family-closure atom). Meta annotated: **family now 4/4 resolved — P1 DEMOTED, P8 DEMOTED, P6v2+P7v2 CONFIRMED-ARTIFACT by direct paired experiment.**

**PRECISE PHASE-DIAGRAM BLAST RADIUS (answer to USER "why is phase-diagram work not valid"):** exactly the 4-probe MECHANISM-MODERATION cross-term family (P1, P6, P7, P8) — the "which cleanup mechanism you pick matters, conditional on [storage/topology/N/algebra]" claims. All 4 now resolved as unpaired-sampling artifacts. **NOT invalidated:** the MAIN-EFFECT laws (storage 0.93 dominance, SCALE_FREE, TOPOLOGY_FREE, M-scaling, ALGEBRA_SCALES, P9 N-L additive composition) all stand. Nuance: P6/P7 were NOT worthless — the non-saturated revival genuinely escaped the Probe-2/3 saturation ceiling; the finding is the cross-term is exactly 0 even in that engineered band. Net today: CG -1 (P1 cross-term), MM +2 (P1 storage-survivor was non-novel delta 0; P8 demote -1; family-closure +1; paired-trials meta +1). Phase-diagram roadmap agent (aa96e1) reconciling the full real-vs-demoted table.
...
- TR=100 EV-null (NDRAW=200k): MAX z=-0.62, MEAN z=-1.11, COUNT z=-1.46 (below floor). TR=400 paired: **range EXACTLY 0.000000, z=-8.88** (my -8.68 matched).
- **MECHANISTIC ROOT (Skunkworks found, stronger than my finding):** readout = `argmax Re(Q_clean @ props*.T)`; all 3 mechanisms preserve that argmax at the tested regime, so accuracy is mechanism-INVARIANT (depends on the index, not the vectors). `arms_differ_verified` passed on output VECTORS (which differ) but accuracy depends only on the invariant INDEX. **The cross-term is PROVABLY ABSENT, not merely unproven.**
- **Cert mutations (A5-gated):** Probe 1 CG_META cross-term DEMOTE (CG -1); confirmatory-replication DEMOTE (MM -1); SPLIT-A storage-main-effect survivor (CG-grade, delta 0, non-novel vs #56); SPLIT-B MEASURED_MECHANISM argmax-degeneracy (MM +1); META paired-trials-mandatory (MM +1). **Net: CG -1, MM +1.** Integrity: 0 corrupt, 5 atoms + 5 ledger entries verified_off_data=true.
- Symmetric self-correction: the 2026-07-03 VET that filed the CG_META recorded "framing correction NONE" but never gated the inference against a noise floor/paired design — now corrected. Regime-map mechanism-moderation cross-term family CLOSED as unpaired-sampling artifact (P1+P8; P6v2/P7v2 near-certain same, need only confirmatory paired sweep).

## GPU 100%-util RESOLVED (orchestrator): it's BOINC/PrimeGrid (USER's volunteer computing), NOT a zombie
PID 10576 GENEFER (PrimeGrid), ~16h, ~1.1GB. GPU has 6.8GB free + gpu_runner_0 idle-ready. Encoder MLP dispatch CAN run (shares cycles, ~1.5-2x slower). Safe reversible yield available (`boinccmd --set_gpu_mode never 14400`, GENEFER checkpoints, no lost work) — USER decision pending. Wikipedia job: VERIFIED no canonical-Store write (in-memory benchmark only); no-ingest principle intact. My manual scp artifacts cleaned; remote cell byte-identical to origin/main.

## ENCODER EARLY-EVAL (step 9000/20000) DECISIVE: LINEAR STUDENT CAPS ~0.64

Loaded the running K128 mid-training checkpoint (`data/substrate_concept_encoder_v1b/_ckpt_block_K128.pt`) and ran the cell's own semantic eval (`scratchpad/step1b_early_eval.py`): **spearman_all=0.6428** (vs SMOKE 0.788, goal 0.85, baseline CHARPOS 0.540), loss plateaued -> final ~0.64-0.68. **The SMOKE's 0.788 was on an easy 3000-concept subset and did NOT scale; on full 43905 concepts a LINEAR student caps ~0.64 (more data made it WORSE = capacity ceiling).** => **MLP student is REQUIRED for 0.85, promoted to #1 v2 change** (details `notes/design_encoder_step1b_v2_next_changes_2026-07-04.md`). Algebra side remains DONE (block roundtrip 1.0); only semantic-capacity needs the MLP. The current FULL run's answer is now KNOWN (~0.64); letting it grind ~4h more only confirms it -- candidate to kill (needs USER OK, same gate as hung procs) to free CPU.

## OPEN HIGH-PRIORITY ITEMS (agent-quota-limited until 2026-07-07)
1. **ENCODER v2 = MLP student (REQUIRED, #1)** -- linear proven to cap ~0.64; exp_dev builds MLP 1024->2048->4096 GELU Monday. Secondary: LR schedule + batch 512.
2. **NEGATIVES-2x REVIVAL (the real drill): TR>=400 re-run of P1/P6v2/P7v2/P8 cross-terms.** Cells run in 3-11s FULL -> CHEAP, local-CPU subprocess feasible, NO GPU. If a genuine 0.10 mechanism effect exists it clears at TR>=400; if still noise, the family is confirmed-dead. THIS is the negatives-2x, not a permutation test on underpowered data.
