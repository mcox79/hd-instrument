...
## STAGED WIRES — exact landing spec + BOARD-VISIBILITY (disk-verified 2026-09-05; execute in this order)
The load-bearing finding from the pre-pass deep-read: the three staged board-movers have **very different board-visibility**, and two of them are board-INVISIBLE until a board arm is built. Ranked by clarity of the realizable board win:

| # | Staged wire | Exact edit | Live consumer? | Board dim moves? | Needs first |
|---|---|---|---|---|---|
| 1 | **entity-KB → coref** | promote the full-chain `resolve()` → `hdlab/entity_world_model_resolver.py` + ship `KB_GROUPS` → `data/frontend_assets/role_kinship_scenario_kb.json` + wire the live `sm.entities` head-coref as `reader_coref` into `hdlab/commonnoun_binder.py` (NOT the agent head-match) | **YES** — `commonnoun_binder` is live (`commonnoun_situation_gate` default-on) | **YES — coref** | nothing — clean board-mover |
| 2 | **means-end purpose-filter → goal-why** | LAND the advcl/xcomp purpose-vs-complement filter in `hdlab/goal_register.extract_goals` (bare-purpose branch) — removes 131 wrong purposes vs 24 genuine (5.5:1) on `why()` | **YES** — `goal_register` feeds the board `goal` arm (`reg.why`) | **YES — goal-why** | nothing — verdict-independent board-mover |
| 3 | **patient (already live) → visible** | fold the built `board_patient_dimension()` (Step 0a) into `exp_situation_model_qa_v1.run()` | already live (`structural_patient_pick`) | **YES — new patient arm** | Step 0a (BUILT) |
| 4 | **meaning-wire → select_sense** | add a curated-signature channel to `hdlab/grounded_semantic_graph.select_sense`/`select_sense_blended` (`cos(context_mean, meaning_foundation.sense_signature(syn))`, FLAT, blend/primary vs PPR) + shared-core COARSENING readout (SAME iff same synset OR same `lexname`) SCOPED to same/diff judgments | **NO — `select_sense`/`grounded_semantic_graph` has ZERO hdlab importers (islanded)** | **NO** — improves the WiC organ only | **Step 0c** (a WiC/sense board arm) — else it stays organ-level |
| 5 | **means-end contextual edge → goal graph** | implement the contextual `_link_open_stack` in `hdlab/goal_hierarchy_graph` (score each open goal by `relatedness(situation→goal)`, gated) + flip `link_open_stack=True` in the reader's `build_goal_graph` call | **YES** — `build_goal_graph` is live (but `link_open_stack` defaults OFF) | **NO** — the goal GRAPH is register-invisible (board scores `reg`, not the graph) | **Step 0b** (goal_hierarchy arm) |

**Takeaway for next steps:** the cleanest realizable board wins are **entity-KB (coref)** and the **means-end purpose-filter (goal-why)** — land those + the patient arm and three board dims move immediately. The **meaning-wire is an ORGAN win** (`select_sense` is islanded) — to score the meaning channel *at all* the board needs a WiC/sense arm (Step 0c); the residual past ~0.664 to human 0.80 is the §2 deep-contextualization decision. The means-end **graph edge** needs the goal_hierarchy arm (Step 0b). So Step 0 grew a third arm, and it gates whether the meaning tier is board-visible.

## STEP 0 — BOARD-INSTRUMENT UPGRADE (do FIRST; verdict-independent measurement infra)
Three additive arms (build standalone, fold into `run()` at kickoff):
- **0a. Patient-slot who-did-what QA on clean UD gold** — BUILT (`exp_board_patient_slot_v1`, witness 6/6). Realizes the live +0.086 patient.
- **0b. Goal-hierarchy multi-hop arm** — reuse `exp_goal_hierarchy_qa_v1` (the 30-item plot battery). Realizes the goal-graph landings (the graph organ + the means-end edge), currently register-invisible.
- **0c. WiC / sense-discrimination arm** — reuse `exp_curated_foundation_wic` / `exp_wic_optimization_stack` (leak-free curated + coarsening vs the live PPR `select_sense` reader). This is what lets the meaning channel be SCORED at all (there is no meaning board dim today) so the meaning-wire's +0.0633 and the whole latent meaning cluster register.
