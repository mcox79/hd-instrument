...
# The BF lexical helpers (head_lemma / concept_lemma / coarse_class / is_name / modifiers / definiteness /
# person_synset / _num_of + the WordNet lazy loader) were SPLIT to hdlab.lexical_utils on 2026-09-11
# (consolidate_the_six_coreference_organs_into_one_cue_based_entity_resolver, Q111) so the FOUR live organs that
# depend on them stop importing the NOT_BF `situation_predict` module. Re-imported here VERBATIM (same objects,
# shared caches) for backward compatibility + the retiring `situation_predict` below. Live consumers now import
# these directly from hdlab.lexical_utils.
from hdlab.lexical_utils import (DEF_DET, INDEF_DET, head_lemma, concept_lemma, coarse_class, is_name,
                                 modifiers, definiteness, person_synset, _num_of, _wn)
