"""belief_reader -- drive the PROMOTED per-agent belief_timeline END-TO-END from the LIVE reader's OWN
...
PROMOTED 2026-09-09 from experiments/_belief_reader.py (self-containment -- the LAST experiments dependency of the
live reader): the `_space_reader` import is repointed to hdlab.space_reader (byte-identical), and the DEAD spaCy
STATUS branch (`extract_status_events`, `if nlp is not None` -> experiments.state_register.extract_state_events) is
REMOVED -- the live reader always passes nlp=None (hdlab.perceptual_access_ledger._nlp is hardcoded None, CONT-28), so
that branch was dead code; the in-substrate copular extractor is the live path (byte-identical). hdlab is now
spaCy-free + self-contained on the belief path too.
...
from hdlab.space_reader import build_backbone, _frontend, _cluster_covering, _node_from_token
...
def extract_status_events(sents, by_sent, fact_cluster: Optional[int], fact_aliases: Sequence[str],
    ...
    fal = {a.lower() for a in fact_aliases}
    vocab = {v.lower() for v in value_vocab}
    # NOTE (self-containment 2026-09-09, promoted experiments/_belief_reader.py -> hdlab/belief_reader.py): the
    # legacy `nlp is not None` branch routed the STATUS path through the spaCy-based
    # experiments.state_register.extract_state_events. It is RETIRED from this self-contained organ -- the live
    # reader ALWAYS passes nlp=None (hdlab.perceptual_access_ledger._nlp is hardcoded None since CONT-28), so the
