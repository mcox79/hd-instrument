...
def make_atom_fhrr(n: int, generator: torch.Generator) -> torch.Tensor:
    ...
    tracing.emit("atoms.make_atom_fhrr", {"n": n}, out)
    return out
...
def make_atom_hrr(n: int, generator: torch.Generator) -> torch.Tensor:
    ...
    tracing.emit("atoms.make_atom_hrr", {"n": n}, out)
    return out
...
def make_atoms(k: int, n: int, dtype: torch.dtype, generator: torch.Generator) -> torch.Tensor:
    ...
    tracing.emit("atoms.make_atoms", {"k": k, "n": n, "dtype": str(dtype)}, out)
    return out
...
def similarity(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    ...
    if a.is_complex():
        ...
        result = dot / safe
    tracing.emit("atoms.similarity", {"shape_a": list(a.shape), "shape_b": list(b.shape)}, result)
    return result
