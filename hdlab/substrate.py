...
SLOTS: List[Slot] = [
    ...
    Slot("B3'", "read consolidated knowledge back out (THE CORTICAL READ)", "cortical_recall",
         ...
         "count is evidence and a row is not. "
         "SCORED 2026-08-22 AND IT STAYS NEEDS_ADAPTER ON EVIDENCE, NOT ON ABSENCE OF EVIDENCE. "
         "A clean held-out cloze over the consolidated pool (simplewiki, 3 seeds, n=300/seed, "
         "data/solverB_cortical_scored_path_v1/metrics.json) puts the best cortical arm at hit@1 "
         "0.057/0.043/0.043 against a first-order co-occurrence COUNTING floor at 0.090/0.103/"
         "0.097 -- it does NOT clear the strongest floor at ANY k on ANY seed (0 of 15 cells), and "
         "at k=50 the gap is CI-separated (COOC 0.71 vs 0.37). It is NOT inert and NOT a "
         "concreteness artifact: it beats a scrambled cue, a random twin and a concreteness prior, "
         "and beats the EPISODIC route at hit@1 -- a real, tiny, CLS-consistent difference buried "
         "under counting. The ONLY prior scoring (exp_cortical_read_consolidated_v1, 2026-08-19) "
         "had 298-300 of 300 items ALREADY READ; the leak favoured this organ and it lost anyway, "
         "so removing it STRENGTHENS the negative. "
         "REVIVAL CRITERION, BRAIN-FRAMED, NOT PERFORMANCE-FRAMED: under CLS the neocortical read's "
         "distinctive job is GENERALISATION -- retrieving a concept from a context it was never "
         "paired with -- and a cloze whose answer co-occurs with its cue rewards the EPISODIC "
         "style instead, so this task is biased against what the organ is FOR. It earns a live "
         "adapter only if its retrieval SPACE beats counting in the unseen-co-occurrence regime, "
         "on a task POWERED for it (the stratified attempt reached n=43 pooled, below the n>=30 "
         "per-seed gate: directionally null, NOT a verdict). That is a change to "
