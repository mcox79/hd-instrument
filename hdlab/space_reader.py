"""space_reader -- drive the PROMOTED per-entity LOCATION tracker (hdlab.location_register) end-to-end
from the LIVE reader's OWN extraction, on real narrative prose.

PROMOTED 2026-09-09 from experiments/_space_reader.py (self-containment) BYTE-FAITHFUL: the ONLY deltas are the two
experiments imports resolved into hdlab -- perceptual deixis constants repointed to hdlab.perceptual_access_ledger,
and the 9 motion/place location helpers (not present in hdlab.location_register) INLINED VERBATIM as private helpers
(see below). The reader's default-on `_read_space` now imports THIS instead of the experiments scratch cell.
...
# -- perceptual-access deixis constants (the PROMOTED organ; hdlab.perceptual_access_ledger has DEIXIS_AWAY/TOWARD) --
from hdlab.perceptual_access_ledger import DEIXIS_AWAY, DEIXIS_TOWARD

# SELF-CONTAINMENT (promoted experiments/_space_reader.py -> hdlab/space_reader.py, 2026-09-09): the motion/place
# lexicons + canon_node/is_motion_verb were the ONLY experiments.location_register dependency. Those 9 symbols are
# NOT in hdlab.location_register (a DIFFERENT, CONT-28-diverged module) so they are INLINED VERBATIM here as private
# helpers of this space adapter (byte-faithful from experiments/location_register.py -- the source of truth for the
# validated motion decision; verified byte-identical). DEICTIC_SCENE is imported from hdlab.location_register above.
