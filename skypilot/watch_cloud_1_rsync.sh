#!/usr/bin/env bash
# Periodic sky rsync-down watcher for cloud1quality-* clusters. 60s interval.
set +e

DST=/mnt/d/AI/hd-instrument/data/cloud_1_results
LOG=/mnt/d/AI/hd-instrument/data/cloud_1_watcher.log
...
while true; do
  ...
  if [ -z "$CLUSTER" ] || [ -z "$state" ]; then
    ...
    echo "[$ts] no cloud1quality cluster in sky status (gone_count=$gone_count)" >> "$LOG"
