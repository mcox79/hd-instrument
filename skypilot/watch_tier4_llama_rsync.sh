#!/usr/bin/env bash
# Periodic sky rsync-down watcher for tier4llama-* clusters. 60s interval.
set +e

DST=/mnt/d/AI/hd-instrument/data/tier4_llama_results
LOG=/mnt/d/AI/hd-instrument/data/tier4_llama_watcher.log
...
while true; do
  ...
  cluster_line=$(sky status 2>/dev/null | grep -E '^tier4llama(-[0-9]+)?[[:space:]]' | head -1)
  ...
  if [ -z "$CLUSTER" ] || [ -z "$state" ]; then
    ...
    echo "[$ts] no tier4llama cluster in sky status (gone_count=$gone_count)" >> "$LOG"
