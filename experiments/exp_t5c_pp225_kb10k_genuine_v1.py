"""
exp_t5c_pp225_kb10k_genuine_v1 -- Path B rescue R3: supervised projection head (bypass cross-attn) -- GPU.
...
ANCHOR_NAME = "t5c_pp225_kb10k_genuine_v1"; MODEL = "EleutherAI/pythia-1.4b"; ENCODER = "BAAI/bge-large-en-v1.5"
STEPS = 100 if "--smoke" in sys.argv else 25000
RUN_MODE = ("smoke" if "--smoke" in sys.argv else os.environ.get("HDLAB_RUN_MODE", "full")).lower()
_ap = argparse.ArgumentParser(); _ap.add_argument("--smoke", action="store_true"); _ap.add_argument("--self-test", action="store_true"); _ARGS, _ = _ap.parse_known_args()
SMOKE = RUN_MODE == "smoke"
N_FACTS = 200 if SMOKE else 10000
...
def _selftest():
    ...
    assert int(_n.argmax([0.1, 0.9])) == 1, "argmax"; print("[selftest] PASS: pp225-kb10k-genuine", flush=True)
...
def run() -> Dict:
    ...
    subs = list(dict.fromkeys(DISC_POOL))
    if N_FACTS > len(subs):  # GENUINE SCALING: pad with DISTINCT synthetic subjects so fact-count actually reaches N_FACTS
        subs = subs + ["entity number %d named %s" % (i, DISC_POOL[i % len(DISC_POOL)]) for i in range(N_FACTS - len(subs))]
    g.shuffle(subs); subs = subs[:N_FACTS]
    ...
    # frozen final-logits cache (the base model never changes): compute each prompt's logits ONCE, reuse.
    # Without this, recall() re-forwarded the 1.4B model for all ~8000 prompts every eval (~130k forwards) -> timeout.
    _blcache = {}
