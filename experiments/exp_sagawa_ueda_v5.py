"""Sagawa-Ueda deletion certificate v5: N=8192 envelope extension (v4 timeout fix).
...
CONTEXT:
  ...
  sagawa_ueda_v4 TIMEOUT: ran 1200s, timed out mid-run at N=8192. 1 smoke seed showed
    su_frac=1.0 but FULL run never completed (script correct; only timeout too low).
  v5 (THIS): identical to v4 but with corrected timeout.
    v4 failure was a timeout underestimate, NOT a script bug.
...
PRE-REGISTERED BANDS (unchanged from v4; same as v2 envelope extension):
  HARD-PASS:
...
Timeout estimate (CORRECTED from v4):
  v4 timed out at 1200s (N=8192 outer-product much slower than estimated from v2).
  Local benchmark at N=2048 M=256: 9.5s.
  N=8192 scales as (8192/2048)^2 * (1024/256) = 4^2 * 4 = 64x -> ~608s per seed.
  5 seeds: 608 * 5 = 3040s.
...
Pre-reg: preregs/2026-05-27_sagawa_ueda_v5.md
Parent: sagawa_ueda_v4 (timed out; script correct), sagawa_ueda_deletion_cert_v2 (HP)
"""
...
def get_output_dir(default_name: str = "sagawa_ueda_v5") -> Path:
...
def _instrumentation_selftest() -> None:
    ...
    print(f"[selftest] sagawa_ueda_v5 PASSED: "
...
def run(smoke: bool = False) -> None:
    ...
    exp_name = os.environ.get("HDLAB_EXP_NAME", "sagawa_ueda_v5")
    ...
    metrics = {
        ...
        "elapsed_s": elapsed,
        "summary": f"sagawa_ueda_v5 {mode_str} N={N}: {n_hp}/{len(seeds)} HP su_frac>={HP1_BOUND_FRAC}",
        "N": N,
        "n_seeds": len(seeds),
        "n_hp": n_hp,
