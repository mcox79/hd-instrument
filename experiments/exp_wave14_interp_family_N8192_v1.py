...
N=1024 artifact, or does it survive N-scaling to N=8192 (the customer-
...
strong at N=4096 and N=8192? If the Spearman rho drops on ANY family at
N=8192, the predictor was an N-artifact.
...
probes]] — N=8192 SVD on CPU is feasible (~2 min) but the 5-seed × 5-alpha
× 3-family × 3-N grid is 225 cells with N=8192 ones expensive. Route to
overnight_queue.

HARD PASS (Cap 12 ✅ survives E2 STRESS)
----------------------------------------
  rho >= 0.50 at N=8192 for ALL 3 families (Kerdock, SRHT, Hadamard)
  AND max VAMP rel-err < 0.20 across N=8192 cells.

HARD FAIL (Cap 12 ✅ reverts to 🟢 with N-bound annotation)
-----------------------------------------------------------
  rho < 0.30 on ANY of {Kerdock, SRHT, Hadamard} at N=8192.

MIDDLE BAND (Cap 12 ✅ stays with N-scaling annotation)
-------------------------------------------------------
  rho in [0.30, 0.50) at N=8192 on one family (partial); ✅ stays with
  N-scaling annotation.

Vertex: INTERP_FAMILY_N8192_PASS / KILLED / INCONCLUSIVE

Pre-reg: preregs/2026-05-24_wave14_interp_family_N8192_v1.md
"""
...
def compute_verdict(summary: dict) -> tuple[str, str]:
    """E2 STRESS bands at N=8192 (intermediate N; Kerdock structurally absent
       because n_log2=13 is odd):
       HARD PASS: rho >= 0.50 at N=8192 for ALL present families (>=2 of
                  {srht, hadamard}) AND max VAMP rel-err < 0.20 at N=8192.
       HARD FAIL: rho < 0.30 at N=8192 on ANY of the present families.
       MIDDLE BAND: rho in [0.30, 0.50) at N=8192 on one family.
       Note: predictor PASSing at N=8192 (SRHT + Hadamard) plus PASSing at
       N=4096 for Kerdock from the N16384 attempt is the substrate-honest
       readout (N16384 timed out -> 8192 is the largest tractable point).
    """
    rho_map = summary.get("rho_per_family_N") or {}
    summary["rho_per_family_N_serialized"] = dict(rho_map)

    if not rho_map:
        return ("INTERP_FAMILY_N8192_INCONCLUSIVE",
                "rho_per_family_N empty; no cells aggregated.")

    n8k_keys = [k for k in rho_map.keys() if k.endswith("@N=8192")]
    # Kerdock is structurally absent at N=8192 (log2(8192)=13 odd); require
    # both SRHT and Hadamard (2 of 2 expected non-Kerdock families).
    if len(n8k_keys) < 2:
        return ("INTERP_FAMILY_N8192_INCONCLUSIVE",
                f"Only {len(n8k_keys)} families at N=8192; need >=2 "
                f"(srht + hadamard; Kerdock structurally absent). "
                f"Available: {list(rho_map.keys())}")

    n8k_rho = {k: rho_map[k]["rho"] for k in n8k_keys}
    n8k_vamp = {k: rho_map[k].get("max_vamp_rel_err", float("nan")) for k in n8k_keys}
    summary["rho_at_N8192"] = n8k_rho
    summary["max_vamp_at_N8192"] = n8k_vamp

    # Drop NaN entries (treated as inconclusive)
    rho_vals = [v for v in n8k_rho.values() if math.isfinite(v)]
    vamp_vals = [v for v in n8k_vamp.values() if math.isfinite(v)]

    if len(rho_vals) < 2:
        return ("INTERP_FAMILY_N8192_INCONCLUSIVE",
                f"NaN rho at N=8192 for some family. rho_per_family_N8192 = "
                f"{n8k_rho}; need >=2 finite to call verdict.")

    min_rho = min(rho_vals)
    max_vamp = max(vamp_vals) if vamp_vals else float("nan")

    # HARD FAIL: ANY family at N=8192 has rho < 0.30
    if min_rho < 0.30:
        bad = [k for k, r in n8k_rho.items() if r < 0.30]
        return ("INTERP_FAMILY_N8192_KILLED",
                f"AMP-error predictor breaks at N=8192 on {bad}: "
                f"rho={n8k_rho}. The kappa_n divergence sum was an "
                f"N=1024 artifact; Cap 12 ✅ reverts to 🟢 with "
                f"N-bound annotation (predictor holds up to ~N=4096).")

    # HARD PASS: ALL families at N=8192 have rho >= 0.50 AND max VAMP < 0.20
    if min_rho >= 0.50 and max_vamp < 0.20:
        return ("INTERP_FAMILY_N8192_PASS",
                f"AMP-error predictor survives N-scaling: rho={n8k_rho} "
                f">= 0.50 at N=8192 on ALL 3 families; max VAMP rel-err = "
    ...
    return ("INTERP_FAMILY_N8192_INCONCLUSIVE",
            f"Partial N-scaling robustness: rho={n8k_rho}; max VAMP "
...
def self_test() -> None:
    ...
    fake_cells = [
        ...
        {"family": "kerdock", "N": 8192, "bbmd_distance_mean": 0.05, "amp_rel_err_mean": 0.02, "vamp_rel_err_mean": 0.02},
        {"family": "kerdock", "N": 8192, "bbmd_distance_mean": 0.20, "amp_rel_err_mean": 0.10, "vamp_rel_err_mean": 0.04},
        {"family": "kerdock", "N": 8192, "bbmd_distance_mean": 0.60, "amp_rel_err_mean": 0.25, "vamp_rel_err_mean": 0.06},
    ]
    rho_map = compute_rho_per_family_N(fake_cells)
    assert "kerdock@N=1024" in rho_map
    assert "kerdock@N=8192" in rho_map
    assert abs(rho_map["kerdock@N=1024"]["rho"] - 1.0) < 1e-9
    assert abs(rho_map["kerdock@N=8192"]["rho"] - 1.0) < 1e-9
    assert rho_map["kerdock@N=1024"]["n_alpha_cells"] == 3

    # Self-test 5: PASS verdict (2 families at N=8192 — SRHT + Hadamard;
    # Kerdock structurally absent; rho >= 0.50, VAMP < 0.20)
    rho_pass = {
        ...
        "srht@N=8192":     {"rho": 0.60, "max_vamp_rel_err": 0.08, "n_alpha_cells": 5},
        ...
        "hadamard@N=8192": {"rho": 0.55, "max_vamp_rel_err": 0.10, "n_alpha_cells": 5},
    }
    # min rho at N=8192 = 0.55 >= 0.50; max VAMP = 0.10 < 0.20 -> PASS
    v, msg = compute_verdict({"rho_per_family_N": rho_pass})
    assert v == "INTERP_FAMILY_N8192_PASS", f"expected PASS got {v}: {msg}"

    # Self-test 6: HARD FAIL (one family rho < 0.30 at N=8192)
    rho_fail = dict(rho_pass)
    rho_fail["hadamard@N=8192"] = {"rho": 0.20, "max_vamp_rel_err": 0.05, "n_alpha_cells": 5}
    v, msg = compute_verdict({"rho_per_family_N": rho_fail})
    assert v == "INTERP_FAMILY_N8192_KILLED", f"expected KILLED got {v}: {msg}"

    # Self-test 7: MIDDLE BAND (one family rho in [0.30, 0.50) at N=8192)
    rho_mid = dict(rho_pass)
    rho_mid["hadamard@N=8192"] = {"rho": 0.40, "max_vamp_rel_err": 0.05, "n_alpha_cells": 5}
    v, msg = compute_verdict({"rho_per_family_N": rho_mid})
    assert v == "INTERP_FAMILY_N8192_INCONCLUSIVE", f"expected INCONCLUSIVE got {v}: {msg}"

    # Self-test 8: missing families at N=8192
    rho_missing = {k: v for k, v in rho_pass.items() if not k.endswith("@N=8192")}
    v, _ = compute_verdict({"rho_per_family_N": rho_missing})
    assert v == "INTERP_FAMILY_N8192_INCONCLUSIVE"

    # Self-test 9: VAMP blowup (rho good but VAMP > 0.20 -> middle band, not fail)
    # Bump VAMP on srht@N=8192 (one of the two present N=8192 families) past 0.20.
    rho_vamp = dict(rho_pass)
    rho_vamp["srht@N=8192"] = {"rho": 0.75, "max_vamp_rel_err": 0.25, "n_alpha_cells": 5}
    # All rho >= 0.50 (srht=0.75, hadamard=0.55), but max VAMP=0.25 > 0.20 -> NOT pass; rho>=0.30 -> NOT fail -> INCONCLUSIVE
    v, _ = compute_verdict({"rho_per_family_N": rho_vamp})
    assert v == "INTERP_FAMILY_N8192_INCONCLUSIVE", f"expected INCONCLUSIVE from VAMP, got {v}"

    print("interp-family N=8192 self-test passed (9/9 cases)", flush=True)
...
def run_experiment(smoke: bool) -> tuple[dict, str, str, float, dict]:
    ...
    for family in config["families"]:
        for N in config["N_grid"]:
            # Kerdock requires even log2(N); skip when odd (e.g. N=8192 -> log2=13).
            n_log2 = int(round(math.log2(N)))
            if family == "kerdock" and (n_log2 % 2 != 0):
                print(f"\n[family={family} N={N}] SKIP (Kerdock needs even log2(N); "
                      f"log2({N})={n_log2}). Predictor evaluated only on srht+hadamard "
                      f"at this N.", flush=True)
                continue
...
def run_smoke() -> None:
    self_test()
    out_dir = get_output_dir("wave14_interp_family_N8192_v1_smoke")
...
def run_main() -> None:
    self_test()
    out_dir = get_output_dir("wave14_interp_family_N8192_v1")
