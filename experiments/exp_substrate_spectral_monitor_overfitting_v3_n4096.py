"""
substrate_spectral_monitor_overfitting_v3_n4096 -- Spectral monitor overfitting scale-gate R1.

CONTEXT (v381 cycle 50 refill -- R1 rescue continuation for v2 MIDDLE_BAND):
  v2 MIDDLE_BAND: seeds_hp=2/3 (leads=[365,2140,None]; seed 3 val_overfit=None).
  sub_overfit_detected=3/3 consistent. Root cause: TRAIN_CHARS=150000 still insufficient
  for seed 3 to reach val overfit phase. Rescue v3: increase TRAIN_CHARS=400000, N_STEPS=9000.
  Seed 3 is the blocker; 2/3 seeds already show mean_lead=1252.5 steps. Scale up further.
...
TIMEOUT ESTIMATE: v2 elapsed ~unknown per seed (no remote metrics for v2 wall).
  v3 increases TRAIN_CHARS 150000->400000 (2.67x) and N_STEPS 5000->9000 (1.8x).
  Conservative estimate: 3x v2 wall time per seed. v2 was ~400s/seed (est).
  ceil(1.5 * 400 * 2.67 * 3 seeds) = ceil(4806) -> 5400s.
"""
...
ANCHOR_NAME = "substrate_spectral_monitor_overfitting_v3_n4096"
...
if RUN_MODE == "smoke":
    ...
    N_OBS = N_OBS_FULL
    BUFFER_SIZE = int(0.12 * N_OBS)
    TRAIN_CHARS = 400000      # SCALE GATE v3: 2.67x increase from v2 (150000->400000)
    N_STEPS = 9000            # SCALE GATE v3: 1.8x increase from v2 (5000->9000)
    MONITOR_EVERY = 5
