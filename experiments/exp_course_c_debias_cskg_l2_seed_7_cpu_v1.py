"""Degree-debiased fair re-scoring of the Course-C rotation win, seed 7 (REMOTE CPU, process-isolated).
Refits ONLY ONESHOT_ROTATE (+ recomputes POP) on the reproduced seed-7 split, then computes the degree-debiased
diagnostics (within-stratum / partial correlation + degree-matched candidate margin) against the archived
gpu1024 seed-7 reference. CPU-forced (task lock: no GPU) -> cross-device from the CUDA archive, so ONESHOT
...
ANCHOR_NAME = "course_c_debias_cskg_l2_seed_7_cpu_v1"
SEEDS = [7]
