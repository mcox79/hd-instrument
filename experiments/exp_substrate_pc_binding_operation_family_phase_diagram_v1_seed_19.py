"""substrate_pc_binding_operation_family_phase_diagram_v1 sibling seed=19.

SIXTH systematic COMPONENT-SUBSTITUTION phase diagram. Pattern completion
with 4 binding operations swept as OUTER axis: circular_convolution (HRR) /
element_wise_fhrr / hadamard_real / outer_product_tensor.

Inner grid: 3 N x 4 corruption x 1 iters = 12 inner pts per op x 4 ops = 48
phase points per seed (FULL).
Smoke: 1 N x 3 c x 1 T = 3 inner pts per op x 4 = 12 corner points.
...
SEED = 19
ANCHOR_NAME = f"substrate_pc_binding_operation_family_phase_diagram_v1_seed_{SEED}"
...
CONFIG_VERSION = (
    ...
    f"bindings=[circular_convolution,element_wise_fhrr,hadamard_real,outer_product_tensor],"
    f"N=[1024,4096,8192],SEED={SEED},mode={RUN_MODE},"
    f"corruption=[0.10,0.25,0.40,0.475],iters=[3],"
    f"M=100,beta=8.0,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v1_pc_binding_op_family_phase_diagram_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    ...
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v1_pc_binding_op_family_import_crash",
        }
...
def main() -> int:
    ...
    from experiments._substrate_pc_binding_operation_family_phase_diagram_v1_core import (
    ...
    final["_hardening_marker"] = "v1_pc_binding_op_family_phase_diagram_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = "synthetic_substrate_4_binding_op_family_pc"
