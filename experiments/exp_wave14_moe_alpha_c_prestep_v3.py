"""MoE single-expert alpha_c calibration pre-step v3 -- DENSE M-GRID.

CONTEXT: v2 reported ALPHA_C_HARD_FAIL (alpha_c=0.390625 = 1600/4096 exactly).
Research audit confirmed this is a GRID-QUANTIZATION ARTIFACT: the threshold
cosine tau=0.80 lies between M=1600 (cos=0.848, ABOVE) and M=3200 (cos=0.749,
BELOW), so the alpha_c-extraction rule "largest M where cos > tau" returns 1600/4096
by mechanical necessity. Pre-flight verification from v2 metrics.json confirmed:
mean_cosines["1600"] = 0.8482 in [0.83, 0.87] -- closed-form prediction 0.8481
matches to 4 decimal places, diagnosis confirmed.

WHAT CHANGED IN v3:
  - M_GRID_FULL replaced with a 9-point dense grid spanning M in [1024, 3584].
  - Alpha-spacing within band [0.40, 0.70] = 0.0625 (vs ~0.39 in v2).
  - All thresholds, self-tests, verdict logic -- VERBATIM from v2.
  - Expected alpha_c_measured: 2304/4096 = 0.5625 (theory prediction 0.5625).

M-GRID (exp_dev autonomy per handoff contract):
  ...
  Band [0.40, 0.70] coverage: 5 points at spacing 0.0625 (requirement: <= 0.10 -- MET)
  Bracket check: min_alpha=0.25 < 0.30 (PASS), max_alpha=0.875 > 0.80 (PASS)

PRE-REGISTERED BANDS (VERBATIM from v2/parent recalibration drill):
  HARD-PASS: alpha_c_measured in [0.50, 0.60] AND CI < 0.05 AND max_residual < 0.02
  MIDDLE: alpha_c_measured in [0.40, 0.70] AND max_residual < 0.05
  HARD-FAIL: alpha_c_measured outside [0.40, 0.70] OR max_residual > 0.05 at >= 2 pts
  INSTRUMENTATION-FAIL: any NaN cosine OR CI width >= 0.10

Queue: overnight_queue (GPU; 5 seeds x 9 M-values x N=4096; estimated 20-40 GPU-min)
Pre-reg: preregs/2026-05-26_wave14_moe_alpha_c_prestep_v3.md
"""
...
# Dense M-grid: 9 points, alpha-spacing 0.0625 within [0.40, 0.70]
# alpha @ N=4096: [0.25, 0.375, 0.4375, 0.50, 0.5625, 0.625, 0.6875, 0.75, 0.875]
M_GRID_FULL = [1024, 1536, 1792, 2048, 2304, 2560, 2816, 3072, 3584]
# Smoke grids (scaled proportionally for N_SMOKE=512, N_SMOKE_LARGE=2048)
M_GRID_SMOKE = [128, 192, 224, 256, 288, 320, 352, 384, 448]
M_GRID_SMOKE_LARGE = [512, 768, 896, 1024, 1152, 1280, 1408, 1536, 1792]
SEEDS_FULL = [7, 17, 23, 31, 41]
SEEDS_SMOKE = [17]
BATCH_STORE = 512

# Pre-registered thresholds -- VERBATIM from v2
...
# ─── instrumentation self-test (MANDATORY -- VERBATIM from v2 per handoff contract) ───
def _instrumentation_selftest():
    ...
    # Self-test 7 (v3-specific): verify dense grid covers band [0.40, 0.70] at spacing <= 0.10
    N_check = 4096
    band_alphas = [m / N_check for m in M_GRID_FULL if 0.40 <= m / N_check <= 0.70]
    assert len(band_alphas) >= 3, f"Self-test 7 FAIL: only {len(band_alphas)} points in band"
    if len(band_alphas) >= 2:
        spacings = [band_alphas[i+1] - band_alphas[i] for i in range(len(band_alphas)-1)]
        assert max(spacings) <= 0.10, f"Self-test 7 FAIL: max spacing {max(spacings):.4f} > 0.10"
    print(f"[SELFTEST] 7/7 dense-grid band coverage OK ({len(band_alphas)} pts, "
          f"max_spacing={max(spacings):.4f})", flush=True)
...
def run_one_seed(seed: int, N: int, m_grid: list[int], device) -> dict:
    ...
    for M in m_grid:
        ...
        print(f"  seed={seed} M={M:5d} alpha={M/N:.4f} cos={mean_cos:.4f} "
              f"pred={predicted:.4f} resid={residual:.4f}", flush=True)
...
def compute_verdict(per_seed: dict, m_grid: list[int], N: int) -> tuple[str, str, dict]:
    ...
    summary = {
        ...
        "dense_grid_v3": True,
        "grid_artifact_resolved": True,
    }

    # Classify (VERBATIM from v2)
    ...
    if in_hp_range and ci_tight and residual_hp:
        ...
        msg = (f"alpha_c calibration confirmed (linear heteroassociator, dense-grid v3): "
               ...
               f"Theory predicts 0.5625 (tau=0.80). Grid-quantization artifact resolved. "
        ...
        msg = (f"alpha_c measured in MIDDLE band (dense-grid v3): alpha_c_measured={alpha_c_mean:.4f} in "
        ...
        msg = (f"alpha_c_measured={alpha_c_mean:.4f} OUTSIDE middle band [{ALPHA_C_LO},{ALPHA_C_HI}] "
               ...
               f"Dense grid did not resolve artifact -- genuine substrate anomaly. "
               f"Re-open substrate-implementation audit. MoE rebuild stays gated.")

    return verdict, msg, summary


# ─── smoke suspicious-result gate (VERBATIM from v2) ───
def suspicious_result_gate(m_grid: list[int], cosines: dict[int, float]) -> str | None:
    """Return a description if smoke metrics look suspicious."""
...
# ─── multi-scale smoke (VERBATIM from v2 per handoff contract) ───
...
def main():
    ...
    print(f"[alpha_c_prestep_v3] device={device} smoke={smoke}", flush=True)
    print(f"[alpha_c_prestep_v3] dense M-grid: {M_GRID_FULL}", flush=True)
    ...
    m_grid = M_GRID_FULL
    seeds = SEEDS_FULL
    out_dir = get_output_dir("wave14_moe_alpha_c_prestep_v3")
    t0 = time.time()

    per_seed: dict = {}
    for seed in seeds:
        print(f"[alpha_c_prestep_v3] === seed={seed} ===", flush=True)
    ...
    metrics = {
        ...
        "config": {
            ...
            "dense_grid_v3": True,
            "grid_artifact_resolved_from_v2": True,
    ...
    print(f"[alpha_c_prestep_v3] verdict={verdict}", flush=True)
    print(f"[alpha_c_prestep_v3] {verdict_msg}", flush=True)
    print(f"[alpha_c_prestep_v3] elapsed={elapsed:.1f}s  metrics -> {out_path}", flush=True)
    if summary:
        print(f"[alpha_c_prestep_v3] alpha_c={summary.get('alpha_c_measured', 'N/A'):.4f} "
