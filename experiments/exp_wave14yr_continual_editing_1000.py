"""Continual editing at 1000 edits - test if Kerdock holds beyond ym's 500.

Pre-reg: preregs/2026-05-21_wave14yr_continual_editing_1000.md
"""
...
_yc = importlib.util.spec_from_file_location("yc",
    REPO / "experiments" / "exp_wave14yc_continual_editing_kerdock.py")
yc = importlib.util.module_from_spec(_yc); _yc.loader.exec_module(yc)
_v3 = importlib.util.spec_from_file_location("v3",
    REPO / "experiments" / "exp_wave14y_erase_kerdock_v3.py")
...
def compute_verdict(summary):
    ...
    if "kerdock" not in arms:
        return ("CONTINUAL_1000_INCONCLUSIVE", "Missing.")
    kerdock = arms["kerdock"]
    if not kerdock.get("per_edit_trajectory"):
        return ("CONTINUAL_1000_INCONCLUSIVE", "Missing traj.")
    ...
    if k_ok:
        return ("CONTINUAL_1000_HOLDS",
                f"Kerdock holds 1000 edits: min_edited={kerdock['min_edited_acc']:.3f}, "
                f"min_kept={kerdock['min_kept_acc']:.3f}.")
    return (f"CONTINUAL_1000_DECAYS_AT_{k_fail}",
            f"Kerdock decays at edit {k_fail}.")


def self_test_verdict():
    def mk(min_e, fail_e=None, n=1000):
    ...
    cases = [
        ...
        ({"by_arm": {"kerdock": mk(0.5, fail_e=700), "correlated": mk(0.5, fail_e=2)}},
         "CONTINUAL_1000_DECAYS_AT_700"),
...
def run_experiment(smoke):
    ...
    config = {"mode": "smoke" if smoke else "full", "N": 1024 if smoke else 4096,
              "M_stored": 1024 if smoke else 4096, "n_edits": 20 if smoke else 1000,
...
def run_smoke():
    out_dir = get_output_dir("wave14yr_continual_editing_1000_smoke")
...
def run_main():
    out_dir = get_output_dir("wave14yr_continual_editing_1000")
