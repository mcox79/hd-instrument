"""Soft-clipped FHRR: tests whether per-component renorm heavy tail IS the depth-eating mechanism.

Standard FHRR bundle divides each component s_k by |s_k|. E[1/|s_k|] is formally divergent due
to the heavy tail near zero. The clipped variant uses max(|s_k|, threshold) as denominator,
capping the per-component magnification at 1/threshold.

Hypothesis: if the heavy tail is the cause, clipped FHRR's depth scaling jumps toward HRR's.
If not, the depth ceiling is somewhere else in FHRR.
"""
...
POOL_SIZE = 100
TRIALS = 150
CLIP_THRESHOLD = 0.5  # Cap per-component magnification at 1/0.5 = 2.0x.


def clipped_bundle_fhrr(vectors: torch.Tensor, threshold: float) -> torch.Tensor:
    """FHRR bundle with soft clipping: kills the 1/|s| heavy tail near 0."""
    s = vectors.sum(dim=0)
    mag = s.abs()
    clipped_mag = torch.where(
        mag > threshold,
        mag,
        torch.full_like(mag, threshold),
    )
    return s / clipped_mag.to(s.dtype)


def measure_recovery_clipped_fhrr(
    n: int,
    depth: int,
    pool: torch.Tensor,
...
def workload(ctx: experiment.ExperimentContext) -> dict:
        ...
        for n in N_VALUES:
            ...
            for d in DEPTH_VALUES:
                if d + 1 > POOL_SIZE:
                    recovery_by_d[d] = 0.0
                    continue
                rate = measure_recovery_clipped_fhrr(
                    n=n, depth=d, pool=pool, role_atoms=role_atoms,
                    trials=TRIALS, gen=gen, threshold=CLIP_THRESHOLD,
                )
    ...
    headline = (
        f"FHRR clipped (threshold={CLIP_THRESHOLD}, 150 trials) "
        f"beta = {beta:.3f} (R^2 = {r2:.4f}); "
        f"std FHRR was beta=0.717"
    )
    ...
    def page_curves(pdf):
        ...
        ax.set_title(f"Clipped FHRR (threshold={CLIP_THRESHOLD}): beta={beta:.3f}")
    ...
    def page_compare(pdf):
        ...
        ax.scatter(ns, d50s, color="orange", s=80, zorder=3, label="clipped FHRR")
        if not math.isnan(beta):
            n_fit = np.geomspace(min(ns), max(ns), 50)
            d_fit = beta * np.log2(n_fit) + intercept
            ax.plot(n_fit, d_fit, color="orange", linewidth=2,
                    label=f"clipped fit: beta={beta:.3f}, R^2={r2:.4f}")
        # Standard FHRR reference (from exp_scaling_depth.py)
        n_pred = np.array(ns)
        ...
        ax.set_title("Clipped FHRR vs standard FHRR")
        ax.legend(loc="best")
...
def main() -> None:
    spec = experiment.ExperimentSpec(name="exp_depth_pinned_fhrr_clipped", seed=42, n=1024)
