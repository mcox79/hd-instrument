"""substrate_cross_modal_binding_visual_auditory_v1 sibling seed=19.

Stage 3 cross-modal binding (TPJ-analog) characterization.
Phase-diagram sweep over (K, N, bind_mechanism) testing whether the substrate
supports cross-modal binding across two independent modality codebooks.

CHUNKED architecture (USER 2026-06-28): one seed per sibling file.
Sibling pair: seed_7, seed_13.

DISCRIMINATOR ARMS: BIND_CROSS_MODAL / NO_BIND_BASELINE / WITHIN_MODAL_BIND_CONTROL
SWEEP: K in {10,50,100,500,1000} x N in {2048,4096,8192}
       x mech in {HRR_bind, sum_then_query, position_key_bind} = 45 pts
CPU first per task spec; torch.cpu primary backend.

PRE-REG: preregs/2026-06-28_substrate_cross_modal_binding_visual_auditory_v1.md
...
SEED = 19
ANCHOR_NAME = f"substrate_cross_modal_binding_visual_auditory_v1_seed_{SEED}"
...
CONFIG_VERSION = (
    ...
    f"mech=[HRR_bind,sum_then_query,position_key_bind],"
    f"arms=[BIND_CROSS_MODAL,NO_BIND_BASELINE,WITHIN_MODAL_BIND_CONTROL],"
    f"V_MOD_A=2048,V_MOD_B=2048,V_POS=2048,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v1_cross_modal_binding_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v1_cross_modal_binding_import_crash",
        }
...
def main() -> int:
    ...
    from experiments._substrate_cross_modal_binding_visual_auditory_v1_core import (
    ...
    final["_hardening_marker"] = "v1_cross_modal_binding_chunked"
    final["backend"] = backend
    final["seed"] = SEED

    # cardinality_ok
    if SMOKE_MODE:
        expected_n = 6 * 3 * N_QUERIES_SMOKE     # 6 corners x 3 arms x 4 queries = 72
    else:
        expected_n = (len(K_VALUES) * len(N_VALUES) * len(BIND_MECHANISMS)
                      * 3 * N_QUERIES_FULL)     # 5x3x3 x 3 x 20 = 2700
    observed_n = 0
