"""
q_a3_l21_cross_layer_composition_v1_n4096 -- Q-A3: L=21 cross-layer composition at N=4096.

Ceiling chase: extends L=20 (expected HARD_PASS EXACT-1.0 if L=19 pattern holds) to L=21.
Architecture adds one more Hadamard binding level (L21 outer layer).
21 W matrices x ~67 MB each = ~1407 MB total. Fits in 8 GB GPU with margin.
Note: M per stage halves each level per isochoric design; alpha_c constraint ensures validity.
At L=21 M_OUTER=2, M_MID20=2, ..., M_MID7=2 (floored at 2 from L=7+).

PRE-REGISTERED BANDS (ceiling push, prior L=19 EXACT-1.0 unanimous; L=20 expected same):
  HARD-PASS: all 21 level fidelities = 1.0000 (>= 0.9999) unanimous (5/5 seeds)
             AND l21_acc >= 0.5.
  MIDDLE: any L_fid in [0.85, 1.0) OR graceful degradation of inner levels.
  HARD-FAIL: any L_fid < 0.85 OR l21_acc < 0.5. Ceiling found at L=21.
...
QUEUE: overnight_queue (GPU; 21 W matrices at N=4096 ~1407 MB).
"""
import sys
...
ANCHOR_NAME = "q_a3_l21_cross_layer_composition_v1_n4096"
...
def _selftest_l21_chain():
    """21-step Hadamard roundtrip recovers xi_L1."""
    ctxs = []
    ...
    for c in reversed(ctxs):
        xi_dec = xi_dec * c
    assert torch.allclose(xi_dec, xi_L1, atol=1e-6), f"L21 chain decode failed: {xi_dec}"
...
def _instrumentation_selftest():
    ...
    ms = [M_INNER, M_MID2, M_MID3, M_MID4, M_MID5, M_MID6, M_MID7, M_MID8,
          ...
          M_MID17, M_MID18, M_MID19, M_MID20, M_OUTER]
    ...
    del dummy
    print(f"[selftest] PASS: L21 chain decode ok, capacity_ok, gpu_mem_ok N={n_dim}", flush=True)
...
def run_seed(seed: int, n_dim: int) -> Dict:
    ...
    Xi_ctx21 = bsc(M_OUTER, n_dim)
    Xi_outer = Xi_ctx21 * Xi_mid20[:M_OUTER]
    ...
    for q_idx in range(min(N_QUERIES, M_OUTER)):
        ...
        xi_L21_ret = hopfield_retrieve_gpu(W_outer, probe)
        ...
        level_fids["L21"].append(cosine_sim_gpu(xi_L21_ret, xi_outer_true))
        ...
        l21_acc.append(cosine_sim_gpu(xi_L1_ret, Xi_inner[q_idx]))

    mean_fids = {k: float(sum(v) / len(v)) if v else 0.0 for k, v in level_fids.items()}
    mean_l21 = float(sum(l21_acc) / len(l21_acc)) if l21_acc else 0.0
    ...
    print(f"  [seed={seed}] fids={' '.join(f'{k}:{mean_fids[k]:.4f}' for k in sorted(mean_fids.keys()))} "
          f"l21_acc={mean_l21:.4f} peak_gpu={peak_mem:.3f}GB elapsed={elapsed:.2f}s", flush=True)

    result = {
        "seed": seed, "N": n_dim, "run_mode": RUN_MODE,
        "l21_accuracy": float(mean_l21),
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    l21_acc = mean_key("l21_accuracy")

    fid_list = [fid_vals[f"L{l}"] for l in range(1, L_DEPTH + 1)]
    summary = (" ".join(f"L{l}:{fid_vals[f'L{l}']:.4f}" for l in range(1, L_DEPTH + 1)) +
               f" l21_acc={l21_acc:.4f}(HP>={HP_LACC} HF<{HF_LACC}) n_seeds={len(results)}")

    if any(f < HF_FIDELITY for f in fid_list) or l21_acc < HF_LACC:
        return ("HARD_FAIL", f"HARD_FAIL: any fid<{HF_FIDELITY} or l21_acc<{HF_LACC}. {summary}")

    all_unanimous = all(f >= HP_FIDELITY_UNANI_THRESH for f in fid_list)
    if all_unanimous and l21_acc >= HP_LACC:
        return ("HARD_PASS", f"HARD_PASS: all {L_DEPTH} levels EXACT-1.0 unanimous + l21_acc HP. {summary}")
...
metrics = {
    ...
    "per_seed": [
         ...
         **{f"fid_l{l}": r.get(f"fid_l{l}") for l in range(1, L_DEPTH + 1)},
         "l21_accuracy": r.get("l21_accuracy"),
