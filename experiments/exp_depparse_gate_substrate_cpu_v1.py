...
ANCHOR_NAME = "depparse_gate_substrate_cpu_v1"
...
def _selftest():
    ...
    print("[selftest] PASS: depparse-gate", flush=True)
def _load():
    try:
        import nltk
...
if _ARGS.self_test:
    sys.exit(0)
def run() -> Dict:
    g = np.random.default_rng(int(os.environ.get("HDLAB_SEED", "997")))
    sents = _load()
    if not sents:
    ...
    arc = Counter(); lex = Counter(); root = Counter(); pos_tot = Counter()
    for toks in train:
    ...
    def arc_score(w, t, hw, ht, d):
        ...
        lc = lex[(w, hw, dr)]
        if lc > 0: sc += 1.2 * math.log(lc + 1)           # lexical bonus (backoff: only adds when seen)
        return sc
    def root_score(t):
    ...
    for toks in test:
        ...
        for (i, w, t, h) in toks:
            best_j = 0; best = root_score(t) + 1.5
            for j in idxs:
                if j == i: continue
                sc = arc_score(w, t, wd[j], pos[j], j - i)
                if sc > best: best = sc; best_j = j
            hit += int(best_j == h); tot += 1
...
def verdict(r) -> Tuple[str, str]:
    ...
    if u >= 0.85:
        return ("HARD_PASS", "HARD_PASS: substrate arc-pattern dep-parser reaches UAS>=0.85 -- substrate-only dependency parsing works at full bar; the NL-extraction keystone pipeline is grounded. " + s)
    if u >= 0.70:
