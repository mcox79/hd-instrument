"""
exp_pattern_b_unbind_substitute_v1 -- Pattern B Phase-1: 2-hop chained unbinding (compositional K-hop) -- CPU.
...
ANCHOR_NAME = "pattern_b_khop_compose_v1"; N = 1024
...
def run_seed(seed, k) -> float:
    # 2-hop: fact_i binds role->entity; entity_a's fact points to entity_b (bridge); chain unbind hop1 then hop2.
    g = np.random.default_rng(seed); vocab = rand_phasor(N, VOCAB, g); roles = rand_phasor(N, max(k, 2), g)
    ok = 0; trials = 100 if RUN_MODE == "smoke" else 300
    for _ in range(trials):
        a = int(g.integers(0, VOCAB)); b = int(g.integers(0, VOCAB)); c = int(g.integers(0, VOCAB))
        # facts share a bundle (k distractor bindings each); hop1: role0 of fact_a -> b; hop2: role1 of fact_b -> c
        distract = np.sum([bind(roles[i], vocab[int(g.integers(0, VOCAB))]) for i in range(2, k)], axis=0) if k > 2 else 0
        fact_a = (bind(roles[0], vocab[b]) + distract).astype(np.complex64)
        fact_b = (bind(roles[1], vocab[c]) + distract).astype(np.complex64)
        hop1 = cleanup(unbind(fact_a, roles[0]), vocab)                # retrieve bridge b
        hop2 = cleanup(unbind(fact_b, roles[1]), vocab) if hop1 == b else -1   # use bridge to reach c
        ok += int(hop2 == c)
    return ok / trials
...
def run() -> Dict:
    ...
    for k in BINDINGS:
        ...
        print("  bindings=%d two_hop_chain_acc=%.3f" % (k, acc), flush=True)
...
def verdict(r) -> Tuple[str, str]:
    ...
    if a4 >= 0.95:
        return ("HARD_PASS", "HARD_PASS: 2-hop chained unbinding >=0.95 at k=4 -- substrate-native multi-hop composition works algebraically (the LLM-free K-hop the fair-size ceiling needs). " + summary)
    if a4 >= 0.85:
