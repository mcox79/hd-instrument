"""
pp52_hebbian_lora_speedup_n4096_v1 -- PP-52 A1: Hebbian vs GD+Adam speedup at N=4096.

Production-N validation of A1 (v339 passed at N=1024 only). Tests whether the
Hebbian-GD identity and 100x+ speedup holds at production scale N=4096.

SCIENTIFIC QUESTION:
  At N=4096, M=400 (alpha=0.098): does Hebbian one-shot write achieve same encoding
  fidelity as GD+Adam with >= 100x wall-time speedup?

  Algebraic basis: for bipolar key-orthogonal patterns, Hebbian write is the MSE-optimal
  one-step solution. GD converges to the same W* = Xi^T Xi / N at alpha << alpha_c.
...
P_deflated = 0.70 (algebraic guarantee; test validates at production N).

GPU IMPLEMENTATION:
  W matrix (N x N float32 at N=4096): 67 MB. GD uses GPU matmuls.
  Hebbian: one batched outer product sum. GD: N_ITER gradient steps.

FORMULA SELF-TESTS:
  1. Hebbian W = Xi^T Xi / N. At alpha=0.098, retrieval acc > 0.85.
     [INPUT: N=256, M=25] [EXPECTED: accuracy > 0.85]
  2. GD convergence: loss reaches < 0.01 at convergence.
     [INPUT: N=64, M=6] [EXPECTED: final loss < 0.01]
...
ANCHOR_NAME = "pp52_hebbian_lora_speedup_n4096_v1"
...
if RUN_MODE == "smoke":
    ...
    N_QUERIES = 100
    GD_LR = 0.05
    GD_MAX_ITER = 5000  # reduced from 20000 for practical runtime at N=4096
...
def hebbian_accuracy(W: torch.Tensor, Xi: torch.Tensor, queries: torch.Tensor,
                     n_dim: int) -> float:
    """Retrieval accuracy: fraction of noisy probes that round-trip to correct pattern."""
    n_q = queries.shape[0]
    n_correct = 0
    for i in range(n_q):
...
def run_seed(seed: int, n_dim: int) -> Dict:
    ...
    n_flops_heb = 2 * M * n_dim * n_dim  # one matmul
    ...
    n_flops_gd = 4 * n_iter * M * n_dim * n_dim  # fwd + bwd per step
