...
def run() -> Dict:
    ...
    print("  ZKL(%d)[full-whiten] = %.3f (cycle-151 baseline ~0.22; MarianMT)" % (K_PARA, by["full"]), flush=True)
...
def verdict(r) -> Tuple[str, str]:
    ...
    if not r["sanity_ok"]:
        return ("UNKNOWN", "UNKNOWN: MarianMT d=full ZKL=%.3f outside the 0.17-0.27 calibration band -- baseline mismatch with cycle-151 (harness setup: n/FPR/KB); need cycle-151 exact config to calibrate before trusting the d-sweep. " % by.get("full", 0.0) + summary)
