"""Smoother mega variant 4 — multi-config sweep with seed=4+50."""
...
def run_smoke():
    out_dir = get_output_dir("wave14_smoother_mega_variant4_v1_smoke")
    s,v,m,e,c = run_experiment(smoke=True)
    oracle.assert_baseline_high("acc_present", s["acc_mean"]+0.001, 0.0)
    write_metrics(out_dir,s,v,m,e,c); print(f"\nSMOKE OK: {v}",flush=True)

def run_main():
    out_dir = get_output_dir("wave14_smoother_mega_variant4_v1")
