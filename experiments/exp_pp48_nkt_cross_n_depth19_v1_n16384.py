"""
pp48_nkt_cross_n_depth19_v1_n16384 -- PP-48 NKT cross-N: depth-19 at N=16384.

Cross-N envelope extension for NKT: depth-19 HARD_PASS at N=4096 and N=8192.
  Now test depth-19 at N=16384 to establish the broadest cross-N band.
  Same sampled-leaf NKT design, same K_FORBIDDEN=100.
  alpha_total = (10 pos + 100 forbidden) / 16384 = 0.0067. Very safe sub-capacity.

  Enables dual-axis band-lift: (depth-19) x (N=16384) is the third vertex of the
  NKT depth+N envelope (d19/N4096, d19/N8192, d19/N16384 all HP = strongest band).
  NKT property depends on binding algebra, not on N; Hadamard at N=16384 is cheaper
  per pattern (lower alpha), so HP is expected.

PRE-REGISTERED BANDS (cross-N extension; calibration: d19/N4096 HP, d19/N8192 HP):
  ...
  Calibration: same bands as N=4096/N=8192 (sampled design; capacity invariant in N).
  Note: no prior empirical anchor at N=16384, but NKT is algebraic (Hadamard binding);
    alpha=0.0067 << alpha_c=0.138 leaves extreme margin. Bands unchanged from lower-N.

FORMULA SELF-TESTS:
  1. BRANCH=2, depth=19 total tree: 2^19 - 1 = 524287.
     [INPUT: B=2, D=19] [EXPECTED: total = 524287]
  2. alpha_total check: (K_POS + K_FORBIDDEN) / N < 0.138.
     [INPUT: K_POS=10, K_FORBIDDEN=100, N=16384] [EXPECTED: 110/16384=0.0067 < 0.138]
  3. GPU memory > 0 after Xi alloc.
  4. Xi_A shape: (K_POS, N). Xi_B shape: (K_FORBIDDEN, N).
     [EXPECTED: no OOM at N=16384; peak GPU << 1 GB (Xi tensors are small)]
...
ANCHOR_NAME = "pp48_nkt_cross_n_depth19_v1_n16384"
...
NKT_DEPTH = 19
BRANCH = 2
TOTAL_NKT_FULL = (BRANCH ** NKT_DEPTH - 1) // (BRANCH - 1)  # 2^19 - 1 = 524287
assert TOTAL_NKT_FULL == 524287, f"TOTAL_NKT formula: got {TOTAL_NKT_FULL}, expected 524287"

NOISE_FRAC = 0.10
N_RETRIEVE_STEPS = 5
ALPHA_C = 0.138

# OOM pre-check: Xi_A is (K_POS x N) float32; Xi_B is (K_FORBIDDEN x N) float32
# At N=16384, K_POS=10, K_FORBIDDEN=100:
#   Xi_A = 10 * 16384 * 4 = 0.66 MB
#   Xi_B = 100 * 16384 * 4 = 6.55 MB
#   Total Xi << 10 MB -- no OOM risk. No N^2 matrix involved.
_xi_a_mb = 10 * N * 4 / 1e6
_xi_b_mb = 100 * N * 4 / 1e6
assert _xi_a_mb + _xi_b_mb < 100.0, f"Xi memory estimate {_xi_a_mb+_xi_b_mb:.1f} MB > 100 MB ceiling"
...
def build_nkt_tree_sample(n_dim: int, gen: torch.Generator, k_sample: int) -> List[torch.Tensor]:
    """Sample K_SAMPLE leaves from NKT tree at depth NKT_DEPTH using independent random paths.

    Uses O(k_sample * NKT_DEPTH * n_dim) memory instead of O(2^NKT_DEPTH * n_dim).
    Each leaf is the Hadamard product of NKT_DEPTH-1 context vectors applied to a shared root.
    """
...
def _instrumentation_selftest():
    ...
    # OOM pre-check self-test at smoke scale
    _xi_mb = (K_POS + K_FORBIDDEN) * N_ACTIVE * 4 / 1e6
    assert _xi_mb < 200.0, f"Xi smoke mem {_xi_mb:.1f} MB > 200 MB"

    print(f"[selftest] PASS: total_tree={TOTAL_NKT_FULL}, capacity_ok alpha={alpha_t:.4f}, "
          f"gpu_mem_ok depth={NKT_DEPTH} N={N} K_POS={K_POS} K_FORBIDDEN={K_FORBIDDEN} "
          f"xi_smoke_mb={_xi_mb:.1f}", flush=True)
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    summary = (f"pos_rate={pos:.4f}(HP>={HP_POS} HF<{HF_POS}) "
               ...
               f"depth={NKT_DEPTH} N={N} n_seeds={len(results)}")
    ...
    if hp1 and hp2:
        return ("HARD_PASS", f"HARD_PASS: both HP at NKT depth-19 cross-N N=16384. {summary}")
    if hp1 or hp2:
...
assert peak_mem_gb > 0.001, f"GPU util check FAIL: peak_gpu={peak_mem_gb:.3f}GB (< 1MB)"
