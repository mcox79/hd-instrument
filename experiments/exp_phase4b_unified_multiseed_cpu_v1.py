...
ANCHOR_NAME = "phase4b_unified_multiseed_cpu_v1"
...
def _selftest():
    ...
    print("[selftest] PASS: phase4b-unified-multiseed", flush=True)
_selftest()
if _ARGS.self_test:
    sys.exit(0)
def _load_all():
...
def verdict(r) -> Tuple[str, str]:
    ...
    if m >= 0.45 and sd <= 0.02:
        return ("HARD_PASS", "HARD_PASS: unified arity-routed solver SEED-ROBUST (macro-mean>=0.45, std<=0.02, n=%d) -- single-op + multi-step in ONE substrate solver, no LLM. TIER A primary substrate-math capability. " % r.get("n_seeds", 5) + s)
    if m >= 0.36:
