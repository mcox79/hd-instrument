"""substrate_capacity_multibank_alpha_K_phase_diagram_v1_GPU -- seed 7 chunked sibling.

Parent prereg: preregs/2026-06-28_substrate_capacity_multibank_alpha_K_phase_diagram_v1_GPU.md
...
Sweep axes (full):
  ...
  K_per_bank in {4, 16, 64}                       (3 points)
  N_DIM in {2048, 4096, 8192}                     (3 points)
  num_banks B in {1, 4, 16}                       (3 points)
  Full grid per seed: 6 x 3 x 3 x 3 = 162 points x 3 arms = 486 units.

Smoke corners (6 points x 3 arms = 18 units):
  See prereg table.

Arms:
  MULTI_BANK_BIND   -- distribute M = round(alpha*N) items across B banks (M/B per bank); slot-tag bind
  SINGLE_BANK_BASELINE -- same M items all in 1 bank (all bound into K_per_bank slots; severe overflow)
  RANDOM_FLOOR      -- random readout; floor = 1/CB

At B=1, MULTI and SINGLE are equivalent mechanisms (positive control).
...
SEED_THIS_CHUNK = 13
ANCHOR_NAME = "substrate_capacity_multibank_alpha_K_phase_diagram_v1_GPU_seed_%d" % SEED_THIS_CHUNK

_LLM_CALL_COUNTER = [0]
CORPUS_PROVENANCE = "synthetic_substrate_bipolar_codebook_capacity_alpha_K_v1"
...
HP_HARD_PASS_MIN_GRID = 30
HP_HARD_PASS_MIN_FULLN = 8
HP_DISCRIM_DOES_NOT_FIRE = 15
...
K_PER_BANK_VALUES = [4, 16, 64]
NUM_BANKS_VALUES = [1, 4, 16]
N_DIM_VALUES_FULL = [2048, 4096, 8192]
N_DIM_VALUES_SMOKE = [2048, 4096]
...
# Smoke corners (calibrated; see prereg)
SMOKE_CORNERS: List[Tuple[float, int, int, int]] = [
    (0.05, 4,  1,  2048),  # SINGLE-CLIFF: M=102 in K=4 slot of 1 bank
    (0.05, 4,  16, 2048),  # DISCRIM: M/B=6 ~ K=4 multi resolves; single collapses
    (0.05, 16, 4,  2048),  # DISCRIM: MULTI > SINGLE at K=16 B=4
    (0.10, 16, 4,  4096),  # MID regime
    (0.05, 64, 1,  4096),  # DISCRIM at K=64 B=1 (collapse on both arms)
    (0.50, 4,  16, 2048),  # FLOOR: M/B=64 >> K=4
]
...
CONFIG_VERSION = (
    "substrateCapacityMBAlphaKPhaseDiagV1GPU-seed%d: CB=%d sigma=%.1f CUE_COS=%.2f "
...
def _write_bank_alpha_loading(items: torch.Tensor, slot_tags: torch.Tensor,
    ...
    M = items.shape[0]
    D = items.shape[1]
    # Item to bank/slot assignment
    item_indices = torch.arange(M, device=_DEVICE)
    bank_assign = item_indices % num_banks   # round-robin distribution
    # Within each bank: items get consecutive slot indices mod k_per_bank
    in_bank_seq = item_indices // num_banks  # 0, 0, ... 1, 1, ... for round-robin
    ...
    items_f = items.float()
    # Bind: items_f[i] * slot_tags_f[slot_assign[i]] then sum into bank_assign[i]
    bound = items_f * slot_tags_f[slot_assign]  # (M, D)
    del items_f
    if _CUDA_OK:
        torch.cuda.empty_cache()
    # Scatter-add into workspaces by bank_assign
...
def eval_multi_bank_arm(alpha: float, k_per_bank: int, num_banks: int,
    ...
    Each bank holds M/B items into K_per_bank slots; slot collisions cause interference.
    ...
    for trial in range(n_trials):
        ...
        # Write: distribute M items across B banks
...
def eval_single_bank_arm(alpha: float, k_per_bank: int, num_banks: int,
    ...
    With K_per_bank=K (unchanged) slot count but B_eff=1, all M items go in
    1 bank. Slot assignment = item_idx % K_per_bank; severe overflow at M > K.
    Returns (recall_top1, route_acc, arm_sha256_hex).
    """
    M = max(1, int(round(alpha * n_dim)))
    n_trials = max(1, math.ceil(N_ITEMS_PER_TRIAL / M))
    ...
    for trial in range(n_trials):
        ...
        bank_true = bank_assign_w  # all zeros at B_eff=1
        local_slot = slot_assign_w
        bank_cue_base = bank_tags[bank_true].float()
        noise_base = torch.empty((M, n_dim), device=_DEVICE, dtype=torch.float32)
        noise_base.normal_(0.0, 1.0, generator=g_cue)
...
def _selftest():
    ...
    try:
        ...
        print("[selftest] T2 PASS: bank_tags shape")

        # T3: MULTI arm at low-load regime (should resolve)
        gpu_samples = []
        rec_m, ra_m, ha_m = eval_multi_bank_arm(
            alpha=0.02, k_per_bank=8, num_banks=4, n_dim=1024,
            codebook=cb, seed_offset=100, gpu_util_samples=gpu_samples)
        # alpha=0.02 N=1024 -> M=20 items, B=4 -> 5/bank, K=8 -> 5 slots used
        # MULTI should resolve well (>0.30 at small smoke N)
        ...
        print("[selftest] T3 PASS: MULTI alpha=0.02 K=8 B=4 N=1024 recall=%.3f route=%.3f" % (rec_m, ra_m))

        # T4: SINGLE arm at same config (B=1 effective; M=20 in 1 bank of K=8 slots)
        ...
        print("[selftest] T4 PASS: SINGLE alpha=0.02 K=8 (eff B=1) recall=%.3f" % rec_s)
        ...
        # T8: DISCRIMINATOR sign: MULTI should be >= SINGLE in cliff regime
        # alpha=0.1 N=1024 M=102; K=4 B=4 -> M/B=26 vs SINGLE M/B_eff=102
        rec_m_cliff, _, _ = eval_multi_bank_arm(
            alpha=0.1, k_per_bank=4, num_banks=4, n_dim=1024,
            codebook=cb, seed_offset=200, gpu_util_samples=[])
        rec_s_cliff, _, _ = eval_single_bank_arm(
            alpha=0.1, k_per_bank=4, num_banks=4, n_dim=1024,
            codebook=cb, seed_offset=200, gpu_util_samples=[])
        margin = rec_m_cliff - rec_s_cliff
        print("[selftest] T8 INFO: alpha=0.1 K=4 B=4 N=1024 MULTI=%.3f SINGLE=%.3f margin=%.3f"
              % (rec_m_cliff, rec_s_cliff, margin))
        # Soft sign check; full HARD_PASS uses margin > 0.30
        assert margin > -0.10, \
        ...
        assert HP_HARD_PASS_MIN_GRID == 30
        assert HP_HARD_PASS_MIN_FULLN == 8
        ...
        # T12: VRAM probe
        est = _estimate_peak_working_set_bytes(0.05, 4, 4, 1024)
        ...
        assert len(smoke_pts) == 6, "T13 smoke = 6 corners"
        # Temporarily restore prod CB to count FULL points correctly
        ...
        # 6 alpha * 3 K * 3 B * 3 N = 162 (no skips at prod CB=16384)
        assert len(full_pts) == 162, "T13 full = 162 grid points, got %d" % len(full_pts)
        print("[selftest] T13 PASS: smoke=6 full=162 grid points")

        # T14: alpha-loading write/read end-to-end at tiny smoke
        # Verify the _write_bank_alpha_loading function returns the expected shapes
        items_test = cb[:8]  # 8 items
        slot_tags_test = build_slot_tags(0, 1024, 4)
        ws_bp, ba, sa = _write_bank_alpha_loading(
            items_test, slot_tags_test, num_banks=2, k_per_bank=4,
            n_dim=1024, seed_offset=100)
        assert ws_bp.shape == (2, 1024), "T14 workspaces shape"
        assert ba.shape == (8,), "T14 bank_assign shape"
        assert sa.shape == (8,), "T14 slot_assign shape"
        # bank_assign should be round-robin: 0,1,0,1,0,1,0,1
        assert ba.tolist() == [0, 1, 0, 1, 0, 1, 0, 1], "T14 round-robin bank assignment"
        # slot_assign: in_bank_seq = i // 2 = [0,0,1,1,2,2,3,3]; slot = in_bank_seq % 4 = same
        assert sa.tolist() == [0, 0, 1, 1, 2, 2, 3, 3], "T14 slot assignment"
...
def compute_verdict(per_key: Dict[str, Dict],
    ...
    for (a, K, B, N) in sorted(phase_points_keys | probe_denied_points):
        ...
        # Rail: lowest alpha, K=64, B=1, N=FULL_N -> MULTI ~ 1.0 expected
        if abs(a - min(ALPHA_VALUES)) < 1e-6 and K == 64 and B == 1 and N == FULL_N:
            rail_observed = m_r
        phase_map.append({
            "alpha": float(a), "K_per_bank": int(K), "num_banks": int(B),
            "n_dim": int(N), "M_total": int(round(a * N)),
            ...
            "arms_differ": bool(arms_differ_count > 0),
            "verdict_tier": tier,
            "saturation": bool(sat),
            "cliff_marker": bool(m_r < HP_MULTI_PASS_RECALL),
        })

    # Cliff per B: lowest alpha where MULTI falls below pass-recall at each B
    cliff_per_B: Dict[str, float] = {}
    for pp in phase_map:
        if pp.get("recall_multi") is None:
            continue
        key = "B=%d" % pp["num_banks"]
        # Critical alpha: max alpha where MULTI still passes
    ...
    detail = {
        ...
        "n_floor": n_floor,
        "rail_alpha_min_K64_B1_fullN_observed": rail_observed,
    ...
    rail_str = (" | rail_alpha%.2f_K64_B1_N%d=%.4f" % (
        min(ALPHA_VALUES), FULL_N, rail_observed)) if rail_observed is not None else ""
    ...
    if SMOKE:
        ...
        # Saturation: MULTI >= 0.40 at low-load (alpha=0.05, K=16, B=4)
        sat_corner_ok = False
        for pp in phase_map:
            if (abs(pp["alpha"] - 0.05) < 1e-6 and pp["K_per_bank"] == 16
                    and pp["num_banks"] == 4
                    and pp.get("recall_multi") is not None
                    and pp["recall_multi"] >= 0.40):
                sat_corner_ok = True
                break
        # Floor: any FAIL tier
        fail_ok = (n_floor >= 1 or any(pp.get("verdict_tier") == "FAIL" for pp in phase_map))
        if discrim_ok and sat_corner_ok and fail_ok:
            return ("SMOKE_PASS",
                    "SMOKE_PASS: discrim_fired=%d/%d sat_corner=%s fail=%d | %s%s%s" % (
                        cliff_fired, len(expected_cliff), sat_corner_ok,
                        n_floor + sum(1 for pp in phase_map if pp.get("verdict_tier") == "FAIL"),
                        summary, cliff_str, card_str), detail)
        return ("HARD_FAIL",
                "SMOKE_HARD_FAIL_GATE: discrim_ok=%s (fired=%d/%d) sat_ok=%s fail_ok=%s | %s%s%s" % (
                    discrim_ok, cliff_fired, len(expected_cliff), sat_corner_ok, fail_ok,
    ...
    if n_pass >= HP_HARD_PASS_MIN_GRID and n_pass_at_full_N >= HP_HARD_PASS_MIN_FULLN and rail_ok:
        return ("HARD_PASS",
                "HARD_PASS_CAPACITY_MB_ALPHA_K_PHASE_DIAGRAM: n_pass=%d (>=%d) n_pass_at_full_N=%d (>=%d) rail_ok=%s | %s%s%s%s" % (
                    n_pass, HP_HARD_PASS_MIN_GRID, n_pass_at_full_N, HP_HARD_PASS_MIN_FULLN, rail_ok,
                    summary, cliff_str, rail_str, card_str), detail)
    return ("MIDDLE_BAND",
            "MIDDLE_BAND_PARTIAL_CAPACITY_MB_ALPHA_K: n_pass=%d n_pass_at_full_N=%d rail_ok=%s | %s%s%s%s" % (
...
def _build_keys() -> List[str]:
    # Use integer alpha-encoding (a*10000) to avoid '.' in filenames
    # ('.' not in _PARTIAL_RE character class).
    keys = []
    for s in SEEDS:
...
def _parse_key(key: str) -> Tuple[int, float, int, int, int, str]:
    parts = key.split("_")
    # seed7_a00500_K4_B16_N2048_regimeMULTI_BANK_BIND
...
def _build_metrics(v: str, vmsg: str, detail: Dict, units: List[Dict],
    ...
    return {
        ...
        "metrics_source": "measured_gpu_substrate_capacity_multibank_alpha_K_phase_diagram_v1_GPU_seed_%d" % SEED_THIS_CHUNK,
        "DESIGN_NOTE": (
            "V1 alpha-K capacity phase diagram (chunked sibling cell, seed=%d). "
            "Sweeps alpha (loading factor) x K_per_bank x N x B at FIXED CB=%d. "
            "Three arms: MULTI_BANK_BIND (distribute M=round(alpha*N) across B banks), "
            "SINGLE_BANK_BASELINE (M items all in 1 bank), RANDOM_FLOOR. "
            "Discriminator = MULTI > SINGLE at M > K_per_bank (overflow regime). "
            "At B=1, MULTI and SINGLE are equivalent mechanisms (positive control). "
            "Predicts alpha_cliff(B) ~ K_per_bank * B / N."
