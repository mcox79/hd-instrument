"""cross_axis_m_n_k_factorization_beta_8_bridging_v1 -- seed_19.

BRIDGING CELL: verify substrate factorization holds at beta=8, intermediate
between v2 CG (beta=4) and v1_2d_coarse saturation (beta=13). If HP,
META atom `substrate_axes_factorize_across_beta_regime_2axis_v1` promotes CG.

Arms:
- STD_beta13 (saturating control; re-verifies v1 saturation at same grid)
- DIS_beta8  (bridging discriminator; NEW - between v2 beta=4 CG and beta=13 sat)

See core docstring for design + CRLB predictions + META atom lineage.
...
- crlb / discriminator-reachability declared in prereg (interaction floor 0.05
  matches v2 CG threshold; predictable factorization if beta-invariant)
- baseline_in_band: STD arm hypothesized saturated (v1 replication);
  DIS arm expected in (0.05, 0.95) range across M axis at beta=8
...
- calibration_check: default_ok_for_this_regime (v2 CG regime + CRLB beta=8)
- All numbers tagged MEASURED@/HYPOTHESIZED@/THEORETICAL@/CITED@

PRIOR (substrate-KB 2026-07-01 concept-query at cosine < 0.33):
  Genuinely novel bridging cell. Adjacent:
  - v2 (DIS_beta4) CG at beta=4 MEASURED@data/exp_cross_axis_m_n_k_discriminating_arm_v2_seed_19/metrics.json (3-seed CG)
  - v1_2d_coarse saturated MM at beta=13
  - Sparsity-Free axis CG at beta=8 MEASURED (different arm design)

CROSS-REFS:
  hdlab/chunked_attention.py (Testbed T2 chain-grade primitive)
  Cell D v2 CG dense-Hopfield READ-REPLACE (Atom 1)
  cross_axis_m_n_k_discriminating_arm_v2 (v2 - beta=4 CG, Skunkworks batch 7)
  cross_axis_m_n_k_2d_coarse_gpu_v1 (v1 - beta=13 saturated MM, prior arc)
...
from experiments._substrate_cross_axis_m_n_k_factorization_beta_8_bridging_v1_core import (
...
ANCHOR_NAME = "cross_axis_m_n_k_factorization_beta_8_bridging_v1_seed_19"
SEED_THIS_CHUNK = 19
...
def _main() -> None:
    ...
    metrics = {
        ...
        "crlb_formula_reference": (
            ...
            "M-N INTERACTION mean 3-seed = 0.042 at DIS_beta8 arm "
        ...
        "primitive_reference_v1_saturated": (
            "exp_cross_axis_m_n_k_2d_coarse_gpu_v1_seed_19 (Skunkworks demoted MM)"
        ),
        "cell_chunked": True,
