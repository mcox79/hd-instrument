"""Shared core for substrate_cleanup_family_wm_kcliff_v1p1 sibling cells.

CLEANUP-FAMILY phase diagram in the WM multi-bank K-cliff REGIME.

v1.1 MEMORY-FIT RETRY (2026-06-30): v1 smoke HARD_PASS but FULL OOM'd on GPU
(4.92 GiB held by torch allocator before cleanup_no_cleanup does
query @ codebook.T allocating 1.91 GiB). Memory-fit fix:
  1. N_DIM 8192 -> 4096 (halves memory per tensor; ~4x reduction in W matrix)
  2. num_banks 16 -> 8 (halves K-bank product)
  3. Sequential per-arm GPU cache release (torch.cuda.empty_cache() between arms)
  4. Chunked-K matmul for cleanup_no_cleanup (avoids 1.91 GiB peak allocation)

Experimental design integrity PRESERVED:
  - Same 5 cleanup primitives
...
INNER AXES (v1.1 memory-fit):
    K_per_bank in {50, 100, 250, 500, 1000} (5 K values; UNCHANGED)
    regime in {RANDOM, ADVERSARIAL} (2)
    num_banks = 8 FIXED (was 16 in v1)
    N_DIM = 4096 FIXED (was 8192 in v1)

CARDINALITY:
  FULL: 5 cleanups * 5 K * 2 regimes = 50 phase points per seed
  SMOKE: 5 cleanups * 3 K * 1 regime = 15 corner points per seed

PRE-REG: preregs/2026-06-30_substrate_cleanup_family_wm_kcliff_v1.md (REUSED;
         memory-fit deltas only, no experimental-design change)
...
# Sweep axes (v1.1 memory-fit: N_DIM 8192->4096, num_banks 16->8)
K_PER_BANK_SWEEP_FULL = [50, 100, 250, 500, 1000]
NUM_BANKS_FULL = 8     # v1.1 memory-fit: halved from 16
N_DIM_FULL = 4096      # v1.1 memory-fit: halved from 8192
REGIMES_FULL = ("RANDOM", "ADVERSARIAL")

K_PER_BANK_SWEEP_SMOKE = [50, 100, 250]  # 3 K values
NUM_BANKS_SMOKE = 8    # v1.1: full-N preview for DISCRIMINATOR-MUST-SURVIVE-SCALE (halved)
N_DIM_SMOKE = 4096     # v1.1: smoke at FULL N to verify discriminator survives scale
REGIMES_SMOKE = ("RANDOM",)

# Chunk size for cleanup_no_cleanup chunked matmul (avoids 1.91 GiB peak)
# At N=4096 chunked over codebook of M~8000: 64 chunks * 8000/64 = 125 codes/chunk
# yields ~2 MiB per chunk allocation (vs 1.91 GiB full)
CHUNK_SIZE_NO_CLEANUP = 64
...
# clear MIDDLE_BAND_LO (v1.1: K=50/bank x 8 banks = 400 items in N=4096 is well
# below capacity; substrate must trivially recover).
POSITIVE_CONTROL = {
    "cleanup_family": "no_cleanup",
    "K_per_bank": 50,
    "regime": "RANDOM",
    "recall_floor": 0.80,  # K=50/bank x 8 banks = 400 items in N=4096 fp32
}
POSITIVE_CONTROL_SMOKE = {
    "cleanup_family": "no_cleanup",
    "K_per_bank": 50,
    "regime": "RANDOM",
    "recall_floor": 0.80,  # v1.1: smoke at FULL N=4096; same floor as full
}
...
def cleanup_no_cleanup(query: torch.Tensor, codebook: torch.Tensor
    ...
    v1.1: CHUNKED-K matmul over codebook rows to avoid 1.91 GiB peak allocation
    that triggered OOM in v1. For (K_query, N) @ (M_codes, N).T, chunk codebook
    into CHUNK_SIZE_NO_CLEANUP row-blocks, compute partial sims, accumulate.

    Peak allocation reduced from O(K_query * M_codes) to
    O(K_query * CHUNK_SIZE_NO_CLEANUP).
    """
    K_query = query.shape[0]
    M_codes = codebook.shape[0]
    # Allocate full sims for argmax (no way to avoid this — need full row)
    sims = torch.empty((K_query, M_codes), device=query.device, dtype=query.dtype)
    chunk = CHUNK_SIZE_NO_CLEANUP
    for start in range(0, M_codes, chunk):
        end = min(start + chunk, M_codes)
        cb_chunk = codebook[start:end]  # (chunk, N)
        sims[:, start:end] = query @ cb_chunk.T  # (K_query, chunk)
...
def cleanup_k_NN_lookup(query: torch.Tensor, codebook: torch.Tensor
    ...
    v1.1: CHUNKED-K matmul (same as cleanup_no_cleanup) to bound peak allocation.
    """
    K_query = query.shape[0]
    M_codes = codebook.shape[0]
    sims = torch.empty((K_query, M_codes), device=query.device, dtype=query.dtype)
    ...
    for start in range(0, M_codes, chunk):
        ...
        cb_chunk = codebook[start:end]
        sims[:, start:end] = query @ cb_chunk.T
    pred_idx = sims.argmax(dim=1)
    del sims
    if _CUDA_OK:
        torch.cuda.empty_cache()
...
def selftest(seed: int) -> Tuple[bool, str]:
    ...
    # v1.1: For N=4096, num_banks=8 -> cliff_estimate = 512 (same ratio as v1)
    if not (256 <= cliff_full <= 1024):
        return False, (f"cliff estimate {cliff_full} outside [256, 1024]; "
                       f"check capacity formula")
    msgs.append(f"cliff_K_per_bank N={N_DIM_FULL} banks={NUM_BANKS_FULL} ~ {cliff_full}")
...
def run_one_seed_phase_diagram(seed: int, run_mode: str) -> Dict[str, Any]:
    ...
    for regime in regimes:
        ...
        for fam in CLEANUP_FAMILIES:
            ...
            # v1.1 memory-fit: sequential per-arm GPU cache release between cleanup arms
            if _CUDA_OK:
                torch.cuda.empty_cache()
                cur_mem_mb = torch.cuda.memory_allocated() / 1e6
                print(f"[arm-release] post-{fam}: alloc_after_release={cur_mem_mb:.1f} MiB",
                      flush=True)
