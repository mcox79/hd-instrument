"""
exp_substrate_kg_khop_gpu_scale_v1.py -- substrate KG K-hop at 5000 entities / ~15k triples (GPU) -- GPU.

ROUTING: GPU product-scale validation (KG-QA K-hop at production scale). Scales I1 (KG-QA product gate, was 200 entities) to 5000 entities and ~15k triples on GPU. 2-hop and 3-hop path queries via chained unbind+cleanup over the 5000-entity codebook. Validates KG QA at production graph size. torch.cuda; 8GB-safe. GPU.
PRE-REGISTERED: HARD-PASS 2-hop recall@1 >= 0.70 at 5000 entities. MIDDLE >= 0.55. HARD-FAIL < 0.55.
...
ANCHOR_NAME = "substrate_kg_khop_gpu_scale_v1"
RUN_MODE = ("smoke" if "--smoke" in sys.argv else os.environ.get("HDLAB_RUN_MODE", "full")).lower()
_ap = argparse.ArgumentParser(); _ap.add_argument("--smoke", action="store_true"); _ap.add_argument("--self-test", action="store_true"); _ARGS, _ = _ap.parse_known_args()
SMOKE = RUN_MODE == "smoke"

def _selftest():
    import numpy as _n; assert _n.argmax([0.1, 0.9]) == 1, "argmax"; print("[selftest] PASS: substrate_kg_khop_gpu_scale_v1", flush=True)
_selftest()
if _ARGS.self_test:
    sys.exit(0)
try:
