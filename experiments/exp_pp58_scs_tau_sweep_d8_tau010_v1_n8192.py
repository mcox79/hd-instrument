"""
pp58_scs_tau_sweep_d8_tau010_v1_n8192 -- PP-58 SCS framework tau sweep at d=8, tau_target=0.10.

CONTEXT (v376 cycle 46 all-night burst):
  tau=0.05 test (cycle 43) HARD_FAILed: d_estimate=1.487 < 1.5 (spike condition violated).
  Rescue R3 from strategy_decisions_2026-06-04: tau sweep at d=8 to map tau-vs-spike.
  This experiment constructs Xi with explicit asymmetry mixing to target tau_target=0.10.
  The SCS formula gamma = (d + tau/d) / (1+tau) at d=8, tau=0.10 predicts gamma=7.345.
  Key question: does adding asymmetry (higher tau) enable the spike condition d>1.5 to be met?

DESIGN CHANGE FROM tau=0.05:
  The tau=0.05 test used purely random Xi (symmetric outer-product). tau measured empirically
  was tau~0.0000 (5/5 seeds), confirming spike is absent (d~1.5 only). To explore
  non-Hermitian regime, this experiment constructs W with explicit asymmetry:
    W = (1 - tau_target) * W_sym + tau_target * W_asym
  where W_sym = Xi^T Xi / N (standard Hopfield), W_asym = (W_Xi - W_Xi^T) / 2.
  This SETS tau rather than measuring it, allowing systematic tau exploration.

SCIENTIFIC QUESTION:
  At tau_target=0.10, does the controlled asymmetric W produce a spike d > 1.5,
  and does the SCS formula gamma_SCS = (d + tau/d) / (1+tau) match gamma_emp?
...
PRE-REGISTERED BANDS:
  ...
  MIDDLE:
    ...
    - tau_actual outside [0.07, 0.14] but d_estimate >= 1.2.
  HARD-FAIL:
...
FORMULA SELF-TESTS (PROT-022):
  1. SCS formula: gamma = (d + tau/d) / (1+tau) at d=8, tau=0.10.
     [INPUT: d=8, tau=0.10] [EXPECTED: 7.3455 within 0.001]
     Derivation: (8 + 0.10/8) / (1.10) = (8 + 0.0125) / 1.10 = 8.0125/1.10 = 7.2841
     Wait -- recalculate: (8 + 0.10/8) / (1 + 0.10) = (8.0125) / 1.10 = 7.2841.
     [EXPECTED: 7.2841 within 0.001]
...
TIMEOUT ESTIMATE: Similar to tau=0.05 test. tau=0.05 test elapsed ~10-20 min (SVD + kappa3 x5).
  ceil(1.5 * 1200 * 5) = 9000s. Use PROT-019 floor: 21600s.
"""
import sys
...
ANCHOR_NAME = "pp58_scs_tau_sweep_d8_tau010_v1_n8192"
...
_gamma_d8_tau010 = _scs_gamma(8.0, 0.10)
print(f"[selftest-formula] SCS gamma(d=8,tau=0.10): {_gamma_d8_tau010:.4f} (expected 7.2841)", flush=True)
assert abs(_gamma_d8_tau010 - 7.2841) < 0.001, f"SCS formula selftest tau=0.10: got {_gamma_d8_tau010}"
