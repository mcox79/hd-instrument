"""GAP 2 CLOSE DIAGNOSTIC V2b -- different-articles-per-key + LONGER WINDOWS (64 tokens).

PARENT v2 SMOKE (2026-06-26): different-articles construction PRESERVED substrate-at-
cosine-floor (delta=0.007) but cosine-physics floor itself was at KNN=0.143 only on
pythia-160m + 16-token windows + M=2000. Research Option B: isolate structural
variable (window length) to test whether 16-token bottleneck IS the cosine-floor
source. Doubled+ window from 16 to 64 tokens; same different-articles construction;
...
ANCHOR_NAME = "substrate_gap2_stride_sweep_confirm_v2b_longer_window64"
...
WINDOW_TOKENS = 64              # 64-token windows (Research Option B; isolate structural variable)
CUE_SHIFT = 4                   # cue shift proportional to window (4/64 = 1/16 = same density as v2 1/16)
...
def run_unit(seed: int) -> Dict:
    ...
    # v2b: local encode wrapper with max_length=128 (canonical probe uses 48; too short
    # for WINDOW_TOKENS=64 because words tokenize to 1-2 subword tokens). Same mean-pool
    # mechanism otherwise.
    import torch
    from transformers import AutoTokenizer, AutoModel
    DEV = _probe.DEV
    ENC_DTYPE = _probe.ENC_DTYPE
    _CACHE = {}
    ...
    def encode(texts):
        ...
        for i in range(0, len(texts), 32):
            t = tok(texts[i:i + 32], return_tensors="pt", padding=True, truncation=True,
