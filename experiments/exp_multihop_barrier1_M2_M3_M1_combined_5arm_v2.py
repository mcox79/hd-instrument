"""multihop_barrier1_M2_M3_M1_combined_5arm_v2 -- META_BARRIER_1 stack test.

v2 vs v1 (2026-06-27 rescue from setup-exception HARD_FAIL):
  v1 crashed in make_deep_chains: only 200/500 generated for V=200 max_depth=8
  due to a distinct-start constraint (used_s set) that exhausts at len(used_s)==V.
  v2 changes:
    (1) V_C_FULL raised 200 -> 500 (preserves test power at depth=8 per USER
        directive)
    (2) make_deep_chains RELAXES distinct-start when disallow_s is empty
        ...
        needs train-disjoint starts). TRAIN diversity comes from random (s,p)
        sampling NOT from start uniqueness.
    (3) Pre-flight feasibility check at module init: assert the generator
        can physically satisfy (N_CHAINS_TRAIN, N_CHAINS_QUERY) at chosen
        (V_C, max_depth). SCHEMA-VET HARD_FAIL if infeasible (caught before
        the first seed runs setup).
  RECALIBRATION NOTE: ARM_BASELINE depth-5 anchor was 0.145 at V_C=200;
  per-hop cleanup margin degrades modestly when V_C raised to 500. Sanity
  band widened to [0.080, 0.200]. Discriminator HP / MB bands unchanged.

Pre-reg: preregs/2026-06-27_multihop_barrier1_M2_M3_M1_combined_5arm_v2.md
...
ANCHOR_NAME = "multihop_barrier1_M2_M3_M1_combined_5arm_v2"
_LLM_CALL_COUNTER = [0]
...
# Production regime (drill specification; v2 V_C raised 200 -> 500 to make
# N_CHAINS_TRAIN=500 feasible at max_depth=8)
N_DIM_FULL = 8192
V_C_FULL = 500            # codebook size; load-bearing for Grover sqrt(V_C);
                          # v2: raised from 200 to feasibility-cover TRAIN+QUERY
...
# Sanity rail bands (depth-5 baseline; v2 widened for V_C=500 recalibration)
# Original V_C=200 anchor: 0.145 +/- 0.04.  V_C=500 expected modestly lower
# margin per-hop (more atoms to argmax over) but interference still bounded
# by N=8192.  Band conservatively widened to 0.080-0.200.
BASELINE_DEPTH5_SANITY_LO = 0.080
BASELINE_DEPTH5_SANITY_HI = 0.200
...
def make_deep_chains(n_chains: int, V: int, P: int, max_depth: int,
    ...
    v2: distinct-start constraint is now CONDITIONAL on disallow_s being
    non-empty.
      - disallow_s == empty -> TRAIN case: allow start reuse (diversity comes
        from random (s,p) sampling NOT start uniqueness; lets N_CHAINS_TRAIN
        exceed V).
      - disallow_s != empty -> QUERY case: enforce distinct-start so query
        chains stay disjoint from train_starts (the load-bearing rail).
    """
    all_triples = []
    chain_queries = []
    enforce_distinct = len(disallow_s) > 0
    ...
    while len(chain_queries) < n_chains and tries < n_chains * 200:
        ...
        if enforce_distinct and s in used_s:
            continue
        nodes.append(s)
    ...
    if len(chain_queries) < n_chains:
        raise RuntimeError(
            ...
            "disallow|=%d max_depth=%d enforce_distinct=%s"
...
def preflight_chain_feasibility(v_c: int, v_p: int, max_depth: int,
                                n_chains_train: int, n_chains_query: int) -> None:
    """SCHEMA-VET HARD_FAIL if the chain generator cannot satisfy the
    requested (TRAIN, QUERY) sizes at the chosen (V_C, max_depth).

    Required:
      - max_depth + 1 distinct nodes per chain -> V_C must be >= max_depth + 1
      - QUERY chains have distinct starts disjoint from TRAIN starts; with
        TRAIN start reuse allowed, the worst-case TRAIN start set has
        size min(V_C, n_chains_train); QUERY needs >= n_chains_query
        additional distinct starts -> V_C >= min(V_C, n_chains_train) +
        n_chains_query. (TRAIN-start universe is ALWAYS == V_C, so just
        require V_C >= n_chains_query for QUERY feasibility in the worst
        case where all V_C atoms become train starts.)
    """
    if v_c < max_depth + 1:
        raise RuntimeError(
            f"PREFLIGHT_INFEASIBLE: V_C={v_c} < max_depth+1={max_depth+1}; "
            f"cannot build chains with all-distinct nodes."
        )
    if v_c < n_chains_query:
        raise RuntimeError(
            f"PREFLIGHT_INFEASIBLE: V_C={v_c} < N_CHAINS_QUERY={n_chains_query}; "
            f"QUERY needs distinct starts disjoint from TRAIN starts, but in "
            f"the worst case all V_C atoms become TRAIN starts."
        )
    # TRAIN is feasible by construction now that distinct-start is relaxed.
...
def _selftest_baseline_sanity_constants() -> bool:
    """Sanity rail constants must be reasonable. v2 widened for V_C=500."""
    assert BASELINE_DEPTH5_SANITY_LO < BASELINE_DEPTH5_SANITY_HI
    assert 0.05 <= BASELINE_DEPTH5_SANITY_LO < 0.145
    assert 0.145 < BASELINE_DEPTH5_SANITY_HI <= 0.30
    assert HP_COMBINED_DEPTH5 > MB_COMBINED_DEPTH5
    return True


def _selftest_preflight_feasibility() -> bool:
    """preflight_chain_feasibility correctly rejects infeasible configs and
    accepts feasible ones."""
    # Feasible: V_C=500, max_depth=8, train=500, query=200
    preflight_chain_feasibility(500, 10, 8, 500, 200)
    # Infeasible: V_C < max_depth + 1
    try:
        preflight_chain_feasibility(5, 10, 8, 500, 200)
        raise AssertionError("preflight should reject V_C < max_depth+1")
    except RuntimeError as exc:
        assert "max_depth+1" in str(exc) or "max_depth + 1" in str(exc), \
            f"wrong exc: {exc}"
    # Infeasible: V_C < n_chains_query
    try:
        preflight_chain_feasibility(50, 10, 8, 500, 200)
        raise AssertionError("preflight should reject V_C < n_chains_query")
    except RuntimeError as exc:
        assert "N_CHAINS_QUERY" in str(exc), f"wrong exc: {exc}"
    return True


def _selftest_train_chains_relaxed_start() -> bool:
    """v2 fix: TRAIN case (disallow_s empty) allows start reuse so
    n_chains_train can exceed V."""
    g = np.random.default_rng(31)
    V_small = 20
    # Ask for 50 chains from V=20 with empty disallow -> must succeed now
    triples, chains = make_deep_chains(
        n_chains=50, V=V_small, P=3, max_depth=3, g=g, disallow_s=set(),
    )
    assert len(chains) == 50, f"expected 50 train chains, got {len(chains)}"
    return True


def _selftest_query_chains_keep_distinct_start() -> bool:
    """v2 fix: QUERY case (disallow_s non-empty) STILL enforces
    distinct-start so query chains stay disjoint from train_starts."""
    g = np.random.default_rng(33)
    V = 100
    # Build small train set, then make query chains disjoint
    _, train = make_deep_chains(20, V, P=3, max_depth=2, g=g,
                                disallow_s=set())
    train_starts = {c[0][0] for c in train}
    # When disallow_s non-empty, distinct-start is enforced -> query starts
    # must NOT overlap train_starts
    g2 = np.random.default_rng(34)
    _, query = make_deep_chains(20, V, P=3, max_depth=2, g=g2,
                                disallow_s=train_starts)
    query_starts = {c[0][0] for c in query}
    assert query_starts.isdisjoint(train_starts), (
        f"query starts overlap train: "
        f"{query_starts & train_starts}"
    )
    # And query starts should themselves be distinct (set-tracked)
    return True
...
def _instrumentation_selftest():
    ...
    _selftest_preflight_feasibility()
    _selftest_train_chains_relaxed_start()
    _selftest_query_chains_keep_distinct_start()
    # Module-init feasibility check using ACTUAL config (catches misconfig
    # at import time, before the first seed runs setup).
    preflight_chain_feasibility(
        V_C, V_P, max(DEPTHS), N_CHAINS_TRAIN, N_CHAINS_QUERY,
    )
    print(
