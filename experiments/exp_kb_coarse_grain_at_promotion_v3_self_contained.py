"""KB COARSE-GRAIN AT PROMOTION v3 (ANCHOR 3 RESCUE; SELF-CONTAINED; 2026-06-27).

Pre-reg: preregs/2026-06-27_kb_coarse_grain_at_promotion_v3_self_contained.md

v2 HARD_FAILed on remote with KB_REFERENT_MISSING:
  `load_default_kb(REPO)` expects the canonical
  `data/substrate_director_kb_v1/manifest.json` which exists on local but
  is not provisioned to the remote_cpu runner.

v3 fix (identical pattern to ANCHOR 1 v3): build a labeled mini-KB IN-CELL
from filesystem sources (notes/ + memory/ + preregs/) using the chain-grade
hdlab.director_kb_chunk_ingest.run_chunk_ingest primitive. Cell becomes
self-contained (no upstream dependency) and remote-ready.

MECHANISM IS UNCHANGED FROM v2 chain-grade-path:
...
KEY ADAPTATION: chunk_ingest tags atoms with source_class = "chunk_<cname>"
(prefixed per Principle 8), so v3 uses "chunk_memory" everywhere v2 used
"memory". The ultrametric clustering primitive is class-agnostic; only the
USER_DIRECTIVE invariant check + the sampler reference the literal class
token.

ARMS:
  ARM_NO_COARSE_GRAIN_BASELINE   - sanity rail (unchanged)
...
HARD_PASS bar (chain-grade; identical to v2):
...
# ---------- v3 thresholds (identical to v2 chain-grade-path) ----------
...
# v3 ingest envelope (self-contained build budget; identical to ANCHOR 1 v3)
...
# v3 USER_DIRECTIVE class token (chunk_ingest prefixes per Principle 8)
UD_SOURCE_CLASS = "chunk_memory"

# Ingest cardinality minimums (sub-cell health check)
...
def _sample_atoms_with_forced_ud(
    ...
    `min_ud` USER_DIRECTIVE (chunk_memory) atoms into the sample.

    Returns (W_sample, atom_indices, source_classes_per_atom, n_ud_in_sample).

    v3 adaptation: uses UD_SOURCE_CLASS = "chunk_memory" (prefixed token from
    chunk_ingest) instead of v2's bare "memory" token.
    """
    rng = np.random.RandomState(seed)
    n_ent = len(kb.entity_names)

    # Per-entity primary source_class (prefer UD class if present for USER_DIRECTIVE)
    ent_sc: list[str] = []
    for i in range(n_ent):
        sc_set = kb._source_classes_by_ent.get(i, set())
        if sc_set:
            if UD_SOURCE_CLASS in sc_set:
                ent_sc.append(UD_SOURCE_CLASS)
            else:
                ent_sc.append(sorted(sc_set)[0])
        else:
            ent_sc.append("unknown")

    # Force-include USER_DIRECTIVE atoms FIRST (load-bearing for v2/v3)
    mem_cand = [i for i, c in enumerate(ent_sc) if c == UD_SOURCE_CLASS]
    if not mem_cand:
        raise RuntimeError(
            f"USER_DIRECTIVE_REFERENT_MISSING: zero {UD_SOURCE_CLASS} atoms in "
            f"inline KB (n_ent={n_ent}); cannot satisfy v3 cardinality_ok bar "
            f"(n_UD >= {min_ud}). Check chunk_ingest succeeded over memory/."
        )
    ...
    top_classes = [c for c, _ in sc_counts.most_common(6)
                   if c != UD_SOURCE_CLASS][:5]
    ...
    n_ud_in_sample = sum(1 for c in sc_per_atom if c == UD_SOURCE_CLASS)
...
def _arm_coarse_grain_ultrametric(
    ...
    v3 adaptation: UD invariant checks `chunk_memory` class token.
    """
    t0 = time.perf_counter()
    n = W.shape[0]
    n_user_directive_atoms = sum(1 for c in sc_per_atom if c == UD_SOURCE_CLASS)
...
def _arm_random_cluster_collapse(
    ...
    """Match cluster-size distribution from ARM_ULTRA; random membership.
    Tests whether SEMANTIC clustering matters."""
    t0 = time.perf_counter()
    n = W.shape[0]
...
def _verdict_from_seeds(
    ...
    """Chain-grade verdict aggregator; identical to v2."""
...
def _instrumentation_selftest() -> None:
    """Formula self-tests on synthetic seed_results dicts (v2 verbatim)."""
    hp_seed_results = [
    ...
    print("[selftest] kb_coarse_grain_at_promotion_v3_self_contained formula PASS",
          flush=True)
...
def _exp_name() -> str:
    return os.environ.get(
        "HDLAB_EXP_NAME", "kb_coarse_grain_at_promotion_v3_self_contained"
    )
...
def _build_inline_kb(out_dir: Path, smoke: bool) -> dict:
    ...
    Returns the manifest dict. Wipes any pre-existing out_dir contents.
    """
...
def main() -> None:
    ...
    # Cap n_atoms target at available inline atoms (honest cardinality reporting)
    if args.smoke:
    ...
    # Phase 4: smoke-only full-N preview (discriminator-must-survive-scale)
    full_n_preview = None
    if args.smoke:
        # Cap preview at available inline atoms
        preview_n = min(SMOKE_PREVIEW_N_ATOMS, n_total_inline)
        try:
            print(
                f"[smoke-preview] running ARM_FULL_N_PREVIEW at n={preview_n} "
                f"(capped at inline n={n_total_inline}) to check non-saturation",
                flush=True,
            )
    ...
    payload: dict[str, Any] = {
        ...
        "min_cluster_size": cfg.min_cluster_size,
        "ud_source_class": UD_SOURCE_CLASS,
