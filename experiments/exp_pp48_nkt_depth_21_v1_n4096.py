"""
pp48_nkt_depth_21_v1_n4096 -- PP-48 NKT negative-knowledge tree at depth-21 at N=4096.

Extends depth-19 to depth-21. Ceiling chase for NKT depth.
  depth-19 completed this cycle (verdict pending; expected HP from status log).
  Total binary tree at depth-21: 2^21 - 1 = 2097151 nodes. Sampled-leaf design.
  ...
  Test: does substrate still repel depth-21 leaves with anti-Hebbian inhibition?

PRE-REGISTERED BANDS (ceiling push from depth-19):
  ...
  Calibration: depth-13, depth-17, depth-19 all HARD_PASS; bands unchanged at depth-21.
    The NKT property depends on binding depth, not on explicit tree enumeration.
    Sampled-leaf design makes capacity fixed regardless of NKT_DEPTH; bands stable.

FORMULA SELF-TESTS:
  1. BRANCH=2, depth=21 total tree: sum(2^d for d in range(21)) = 2^21 - 1 = 2097151.
     [INPUT: B=2, D=21] [EXPECTED: total = 2097151]
...
ANCHOR_NAME = "pp48_nkt_depth_21_v1_n4096"
...
NKT_DEPTH = 21
BRANCH = 2
TOTAL_NKT_FULL = (BRANCH ** NKT_DEPTH - 1) // (BRANCH - 1)  # 2^21 - 1 = 2097151
assert TOTAL_NKT_FULL == 2097151, f"TOTAL_NKT formula: got {TOTAL_NKT_FULL}, expected 2097151"
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    if hp1 and hp2:
        return ("HARD_PASS", f"HARD_PASS: both HP at NKT depth-21 sampled-leaves. {summary}")
    if hp1 or hp2:
