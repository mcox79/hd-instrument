"""importance_ceiling_v7_M16384_n8seeds.

Final resolution of importance-ceiling INDETERMINATE_NEEDS_M_SCALE finding
from Skunkworks atom ad6f061a. v1 at d=16384, M=400 left non-TRACE arms
BELOW CRLB k=8 floor (0.055): PCA=0.010 (cv=8.234, lb=-0.045) Fisher=0.036
Single=0.015 Trace=0.998 Rand=0.006. TRACE is by-construction saturating
(reads most-recent-bound trace); other arms had no statistical headroom.

v7 scales M to 16384 (16x v1's 1024) to escape the CRLB floor for non-Trace
arms. At d=16384, M=16384, k=8: CRLB k=8 floor (sqrt(M/(d*k)) per cell
derivation) drops to sqrt(16384/(16384*8)) = 0.354 ... NOTE the
formula gives a HIGHER value at M=d than M<<d because the bound scales
with M/d. The PRACTICAL chain-grade-test signal is whether non-Trace
arms reach >=0.15 with cv<0.30 and SEM-separation from random control.
...
PRE-REG BANDS (HARD-LOCKED at module init):
  HARD_PASS (CEILING_FALSIFIED at M=16384):
       max(non-Trace arm cell-mean) >= 0.15
       AND cv(across n=8 seeds) < 0.30
       AND mean - 1.96*sem > 0 (SEM-separated from random)
       AND |cell-mean RAND| <= 0.04 (contamination guard)
       AND cell-mean TRACE >= 0.25 (regime sanity)
  HARD_FAIL (CEILING_REAL at M=16384):
       max(non-Trace arm cell-mean) < 0.05
       AND cv < 0.30 AND fairness gates pass
  MIDDLE_BAND: cell-mean in (0.05, 0.15) OR cv >= 0.30 OR fairness gate fail

CARDINALITY (META_RULE_H):
  EXPECTED_N_UNITS_FULL = 5 arms * 8 seeds = 40
  EXPECTED_N_UNITS_SMOKE = 5 arms * 1 seed = 5

GPU: --device cuda flag supported; W matmul E @ S_mod ported to torch.
d=16384 x M=16384 -> 1GB float32 E matrix; tractable on GPU.

DISCRIMINATOR-MUST-SURVIVE-SCALE: smoke at N=4096, M=4096, K=8, seeds=[7].
~1/16 full scale. IF non-Trace arms still saturate ~0 at smoke -> escape
regime still wrong -> raise M further or skip cell. IF Trace saturates
0.998 at smoke -> confirms by-construction saturation observation.

META_RULE_AF arms-must-differ self-test: assert PCA/Fisher/Single outputs
have differing fingerprints (sum of |importance|).
...
Author: exp_dev 2026-06-27 (v7 spec from Research)
"""
...
ANCHOR_NAME = "importance_ceiling_v7_M16384_n8seeds"
...
if SELF_TEST_MODE:
    ...
    # Smoke ~1/16 full scale per spec
    N_DIM = 4096
    M = 4096
    SEEDS = [7]
    K_FISHER = 8
...
CONFIG_VERSION = (
    ...
    "HP_nontrace>=%.2f,HP_cv<=%.2f,real_ceil<=%.2f,"
    ...
    HP_NONTRACE_FLOOR, HP_CV_MAX, REAL_NONTRACE_CEIL,
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        metrics = {
            ...
            "_hardening_marker": "v7_M16384_n8seeds",
        }
        if extra:
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    try:
        ...
        sentinel = {
            ...
            "_hardening_marker": "v7_M16384_n8seeds_import_crash",
        }
...
def make_pca_basis(E: np.ndarray, k: int,
    ...
    try:
        # For M=n=16384 full SVD is expensive. Use truncated SVD via
        # randomized projection: sample k+oversample directions, project,
        # do a small SVD on the result. Gives top-k right singular vectors.
        oversample = 10
...
def aggregate_and_verdict(per_seed: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if not rand_clean or not trace_sane:
        ...
        verdict = "HARD_PASS"   # CEILING_FALSIFIED at M=16384
    elif (winner_mean < REAL_NONTRACE_CEIL and cv_resolved):
        verdict = "HARD_FAIL"   # CEILING_REAL at M=16384

    label = {"HARD_PASS": "CEILING_FALSIFIED_M16384",
             "HARD_FAIL": "CEILING_REAL_M16384",
             "MIDDLE_BAND": "INDETERMINATE_M16384",
...
def main() -> int:
    ...
    if SELF_TEST_MODE:
        # Structure-only self-test (per v1 cell pattern). Tiny N/M; signal-
        # magnitude sanity is meaningful only at full scale; smoke run
        # enforces rand_clean + trace_sane gates.
        try:
    ...
    final["_hardening_marker"] = "v7_M16384_n8seeds"
