...
instruction file section 13): runs the shared v3c core at seed=13, the
REPLICATE seed (Skunkworks flagged v3b's NCE-off finding as single-seed,
tiered MM_STANDARD; this seed makes the tie-breaker result robust /
chain-grade-promotable). A runner-death on this process loses only seed=13,
not the sibling seed=7 run (exp_encoder_migration_step1b_v3c_paired_rkd_only_seed_7.py).
...
v3c_full_paired_seed13/) so it never collides with seed=7's artifacts even
