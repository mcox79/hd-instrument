"""concept_featural_enrichment_v1 -- CONTENT-enriched concept meaning vs content-thin, for selection.
...
ANCHOR_NAME = "concept_featural_enrichment_v1"
SEED = 20260725
...
K_PROPS = 6          # cap properties per concept (Cowan-4-adjacent bounded integration; distinctive-first)
...
ENR_FEATURE_NAMES = ("enr_cos_max", "enr_cos_mean", "prop_overlap", "distinct_match", "has_property")
...
# ---------------------------------------------------------------------------
def build_property_index(uid2typed):
    """concept content-word -> list of (group, filler_text); plus filler_df (distinctiveness).

    A property row is (arg0=subject, relation, arg1=object-filler). For each content-word in the
    subject side we record (group_of(relation), filler_text). df(filler) = # distinct concepts with it."""
    props = defaultdict(list)
    seen_pair = defaultdict(set)                 # concept -> set of (group,filler) already added (dedupe)
    filler_concepts = defaultdict(set)           # filler_text -> set of concept words (for df)
...
def _select_props(plist, filler_df, k, only_group=None):
    """Pick up to k properties for a concept, DISTINCTIVE (rare-filler) first (Cree/McRae weighting)."""
...
def build_concept_bundles(props, filler_df, enc, gkeys, k, only_group=None):
    """concept word -> (bundle_vec [nd] float32, keyset frozenset of (group,filler)).

    bundle(c) = sum_i  gkeys[group_i] (*) enc(filler_i)   over the concept's selected properties."""
    # encode every unique selected filler once (batched)
    used = {}
    for w in sorted(props.keys()):
        for (g, f) in _select_props(props[w], filler_df, k, only_group):
            used[f] = None
    uniq_fillers = sorted(used.keys())
    if uniq_fillers:
        FV = arc._encode_store(enc, uniq_fillers)     # [nF, nd] unit rows
    else:
        FV = np.zeros((0, enc.n_dim), dtype=np.float32)
    f2row = {f: i for i, f in enumerate(uniq_fillers)}
    nd = enc.n_dim
    bundles = {}
    keysets = {}
    for w in sorted(props.keys()):
        ...
        ks = set()
        for (g, f) in sel:
            acc += gkeys[g] * FV[f2row[f]]
            ks.add((g, f))
        bundles[w] = acc
        keysets[w] = frozenset(ks)
    return bundles, keysets, len(uniq_fillers)


def _aggregate_over_words(words, bundles, keysets, nd):
    """Sum concept bundles + union keysets over a list of content-words."""
    acc = np.zeros(nd, dtype=np.float32)
    ks = set()
    for w in words:
        b = bundles.get(w)
...
def enriched_features(pool_idx, enr_fact, fact_keyset, filler_df, enr_q_choices, qc_keysets):
    ...
    distinct = np.zeros(P, dtype=np.float64)
    ...
    for p, fi in enumerate(pool_idx.tolist()):
        ...
        overlap[p] = best_o
        distinct[p] = best_d
    X = np.stack([enr_cos_max, enr_cos_mean, overlap, distinct, has_prop], axis=1)
...
def self_test():
    ...
    print("[self-test] REAL typed parse + property index + REAL encoder + group role keys + concept "
          "bundles + REAL RR pool path + imported 29545 learner + UNCHANGED combiner ...", flush=True)
    ...
    props, filler_df = build_property_index(uid2typed)
    assert len(props) > 50, f"property index too small ({len(props)})"
    # energy-domain sanity: hydro/nuclear/coal concepts should have DISTINCT property content
    for probe in ("hydroelectric", "nuclear", "coal"):
        if probe in props:
            print(f"[self-test]   props[{probe}]={_select_props(props[probe], filler_df, K_PROPS)[:4]}",
                  flush=True)
    ...
    codec = EventBundleCodec(n_dim=nd, roles=GROUPS, seed=SEED)
    gkeys = group_keys_np(codec)
    bundles, keysets, nfill = build_concept_bundles(props, filler_df, enc, gkeys, K_PROPS)
    assert len(bundles) > 20 and nfill > 0, f"concept bundles degenerate ({len(bundles)}, {nfill})"
    ...
    for i, s in enumerate(store_sents):
        acc, ks = _aggregate_over_words(arc._content_words(s, CONTENT_MIN_LEN), bundles, keysets, nd)
        fprop[i] = acc
        fkeys.append(ks)
    ...
    for c, ch in enumerate(q["choices"]):
        acc, ks = _aggregate_over_words(arc._content_words(q["stem"] + " " + ch, CONTENT_MIN_LEN),
                                        bundles, keysets, nd)
        enr_q[c] = acc
        qc_keys.append(ks)
...
def run(mode, output_dir):
    ...
    print(f"[store] full tablestore = {nFacts} facts | property_concepts={len(props)} "
          f"unique_fillers={len(filler_df)} single_group={single_group}", flush=True)
    ...
    # ---- concept bundles (the ONE new meaning representation) : full / single-group ----
    _heartbeat(output_dir, "concept_bundles")
    bundles, keysets, nfill = build_concept_bundles(props, filler_df, enc, gkeys, K_PROPS)
    bundles_s, keysets_s, _ = build_concept_bundles(props, filler_df, enc, gkeys, K_PROPS,
                                                    only_group=single_group)
    # RANDOM property control: permute the concept->bundle assignment (destroys real content, keeps
    # dimensionality/noise). Deterministic permutation over property-carrying concepts (no hash()).
    pconcepts = sorted(bundles.keys())
    perm_rng = np.random.default_rng(SEED + 909)
    perm = perm_rng.permutation(len(pconcepts))
    bundles_r = {pconcepts[i]: bundles[pconcepts[perm[i]]] for i in range(len(pconcepts))}
    keysets_r = {pconcepts[i]: keysets[pconcepts[perm[i]]] for i in range(len(pconcepts))}
    print(f"[content] concept bundles built (full={len(bundles)} single={len(bundles_s)} "
          f"random-perm={len(bundles_r)}) fillers={nfill}", flush=True)

    # per-fact property aggregation (prop vector + keyset) for each concept-map variant
    def fact_props(bmap, kmap):
        FP = np.zeros((nFacts, nd), dtype=np.float32)
        FK = [None] * nFacts
        for i in range(nFacts):
            acc, ks = _aggregate_over_words([w for w in fact_terms[i]], bmap, kmap, nd)
            FP[i] = acc
            FK[i] = ks
        return FP, FK

    _heartbeat(output_dir, "fact_props")
    FP_full, FK_full = fact_props(bundles, keysets)
    FP_single, FK_single = fact_props(bundles_s, keysets_s)
    FP_rand, FK_rand = fact_props(bundles_r, keysets_r)
    ...
    q_cov = round(float(np.mean([1.0 if w in bundles else 0.0 for w in q_concepts])), 4) if q_concepts else 0.0
    ...
    def choice_enr(qi, bmap, kmap):
        ...
        for c in range(C):
            words = arc._content_words(questions[qi]["stem"] + " " + questions[qi]["choices"][c],
                                       CONTENT_MIN_LEN)
            acc, ks = _aggregate_over_words(words, bmap, kmap, nd)
    ...
    for qi, q in enumerate(questions):
        ...
        EQ_full, KS_full = choice_enr(qi, bundles, keysets)
        EQ_single, KS_single = choice_enr(qi, bundles_s, keysets_s)
        EQ_rand, KS_rand = choice_enr(qi, bundles_r, keysets_r)
    ...
    metrics = {
        ...
        "summary": (f"{verdict}: [in-sample prec] A={A_prec} B={B_prec} C={C_prec} B_single={S_prec} "
                    f"RANDOM={R_prec} (lift B-A={prec_lift:+.4f}); [subset] A={A_sub} B={B_sub}; "
                    f"[dam] margin_lift={plant_margin_lift} fires={plant_fires}; [TEST Chal] A={A_chal} "
                    f"B={B_chal} C={C_chal} ORACLE={oracle_chal} best_lift={best_lift:+.4f} McNemar_p={mc_p}; "
                    f"q_cov={q_cov} fact_cov={fact_cov} | chance={round(chance,4)}"),
        "elapsed_s": round(time.perf_counter() - _T0[0], 1),
        "ts_iso": datetime.now(timezone.utc).isoformat(),
