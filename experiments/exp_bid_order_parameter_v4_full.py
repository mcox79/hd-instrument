...
CONTEXT:
  ...
  v3 TIMEOUT at 300s: script correct, timeout estimate wrong (W construction at N=8192 is ~130s/seed).
  v4 (THIS): identical to v3 with corrected timeout_s=1500s.
...
Timeout estimate (CORRECTED from v3):
  v3 timed out at 300s -- W construction at N=8192 is ~130s/seed, not 5s.
  Local benchmark (N=2048: 8.2s; N=4096: ~33s; N=8192: ~130s per seed).
  Full sweep: (0.7 + 8.2 + 33 + 130) * 5 = 860s.
...
Pre-reg: preregs/2026-05-27_bid_order_parameter_v4_full.md
"""
...
def get_output_dir(default_name: str = "bid_order_parameter_v4_full") -> Path:
...
def _instrumentation_selftest() -> None:
    ...
    print(f"[selftest] v4 PASSED: attractor BID={bid_test:.2f}, param BID={bid_param:.2f}, "
...
def run(smoke: bool = False) -> None:
    ...
    exp_name = os.environ.get("HDLAB_EXP_NAME", "bid_order_parameter_v4_full")
    ...
    metrics = {
        ...
        "elapsed_s": elapsed,
        "summary": f"BID v4 {mode_str}: {n_outside_primary}/{n_seeds_run} outside_all_bands at N={n_primary_key}",
        "is_smoke": smoke,
        "config": {
