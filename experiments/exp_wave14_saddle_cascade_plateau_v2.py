"""Candidate (v): Saddle-cascade plateau falsifier -- corpus-overlap-fraction sweep. v2.

v2 fix: tiles corpus_a to reach n_bytes when corpus_a is shorter (remote corpus ~49K bytes).
v1 error: raised RuntimeError when corpus_a < n_bytes. This blocked all cells on remote.
...
def run(smoke: bool = False):
    ...
    print(f"[cascade_plateau] corpus_a preflight: {len(corpus_a_full)} bytes (will tile if needed)", flush=True)
    ...
    out_dir = get_output_dir("wave14_saddle_cascade_plateau_v2")
