...
ANCHOR_NAME = "anchor_compose_identity_shuffle_cskg_v2"
...
IDSHUF = "IDENTITY_SHUFFLE"      # NEW must-fail: bundle built from a DIFFERENT held entity's support edges
#                                  (a derangement over held entities re-attributes each composed code to the wrong
#                                  entity). SCRAMBLE keeps t's OWN anchors + degrees and only breaks relations;
#                                  IDENTITY_SHUFFLE keeps a full plausible bundle (donor's relations + degrees) but
#                                  breaks the ENTITY-ANCHOR CORRESPONDENCE -> closes the "does anchor IDENTITY matter"
#                                  loophole. PRE-REG: MUST collapse to ~RANDOM. If it stays near ANCHOR, the win is
#                                  NOT entity-specific -> REFUTES the CHAIN_GRADE (HARD escalation).
...
GEOM_ARMS = [ANCHOR, ADDITIVE, ONESHOT, RANDOM, SCRAMBLE, IDSHUF, ORACLE]   # scored via geometry readouts
ALL_ARMS = GEOM_ARMS + [POP]

# ---- Identity-shuffle closure gate (arm A; ceiling+anchor relative) ----
# The gating question: does the anchor IDENTITY matter (is the win entity-specific)?
# collapse_ratio = (IDSHUF - RANDOM)_mrr / (ANCHOR - RANDOM)_mrr.
IDSHUF_COLLAPSE_RATIO = 0.20     # PASS: identity-shuffle retains <= 20% of ANCHOR's margin (win is entity-specific)
IDSHUF_REFUTE_RATIO = 0.50       # REFUTE: identity-shuffle retains >= 50% of the margin -> win NOT entity-specific
...
SELFTEST_IDSHUF_COLLAPSE_RATIO = 0.30  # planted grid (ADVERSARIAL): identity-shuffle retains <= 30% of ANCHOR margin
...
# v2 identity-shuffle closure = the FAST GATING rung: SAME arena + fit as the confirmed v1 (k=24, ep=500,
# k_core=12, support_frac=0.5) so seeds 7+13 REPRODUCE the confirmed ANCHOR margin (positive-control-at-test-regime)
# while the new IDENTITY_SHUFFLE arm is measured alongside. 2 seeds (not 3) -> lands ~2/3 the confirmed 3.35h.
...
def build_identity_shuffle_codes(Xa, Xac, support_deg, device, seed):
    """CROSS-ENTITY IDENTITY-SHUFFLE must-fail control (arm A): re-attribute each composed code to the WRONG held
    entity. Xac already holds the per-entity anchor-composed bundles (E_derived) at every support-bearing slot; a
    single-cycle derangement over the support-bearing entities donates entity `donor`'s bundle to entity `recipient`.
    Every recipient thus gets a fully plausible bundle (a real donor's relations + degrees + anchors) that simply
    belongs to a DIFFERENT entity -> if the win survives this, the anchor IDENTITY does not matter and the CHAIN_GRADE
    is refuted. Cold/no-support held rows keep their additive-fit (random-init) code, exactly as ANCHOR does. Uses a
    single cycle over a shuffled order = guaranteed NO fixed point for n>=2 (no entity keeps its own code)."""
    Xid = Xa.clone()                                 # start from additive fit (held rows = random-init, as ANCHOR)
    present = np.where(support_deg > 0)[0]
    n = present.shape[0]
    if n >= 2:
        rng = np.random.default_rng(seed * 5557 + 23)
        order = rng.permutation(n)                    # shuffle the support-bearing entities
        recipient = present[order]                    # recipient[j]
        donor = present[order[(np.arange(n) + 1) % n]]  # donor = next in the cycle -> derangement (no fixed point)
        r_idx = torch.from_numpy(recipient).long().to(device)
        d_idx = torch.from_numpy(donor).long().to(device)
        Xid[r_idx] = Xac[d_idx]                        # recipient receives a DIFFERENT entity's composed bundle
    return Xid
...
def fit_and_score(train_int, support_int, query_int, hold_all, N, n_rel, cfg, device, seed,
    ...
    # IDENTITY_SHUFFLE codes (arm A must-fail): each held entity gets a DIFFERENT held entity's composed bundle.
    Xid = build_identity_shuffle_codes(Xa, Xac, support_deg, device, seed)
    ...
    for name, sc in [
        ...
        (IDSHUF, additive_direct_scores(Xid, Da, query_int, device, chunk=SCORE_CHUNK)),
    ...
    del Xa, Da, PHI, THETA, Xo, Do, Xr, Dr, Xac, Xscr, Xid
    _ec()
...
def localize_weak_points(arm_scores, query_int, all_true, support_deg, node_degree, rel_i2lbl,
    ...
    report_arms = [ANCHOR, ADDITIVE, RANDOM, IDSHUF, ORACLE]
...
def aggregate_and_verdict(per_seed):
    ...
    d_idshuf = _sub(m[IDSHUF], m[RANDOM])                                       # identity-shuffle headroom (MRR)
    ...
    # ---- IDENTITY-SHUFFLE closure gate (arm A): does the anchor IDENTITY matter (is the win entity-specific)? ----
    if d_anchor == d_anchor and d_anchor > MIN_SIG_MRR:
        collapse_ratio = d_idshuf / d_anchor          # fraction of ANCHOR's margin retained by mis-attribution
    else:
        collapse_ratio = float("nan")
    identity_collapses = bool(collapse_ratio == collapse_ratio and collapse_ratio <= IDSHUF_COLLAPSE_RATIO)
    identity_refutes = bool(collapse_ratio == collapse_ratio and collapse_ratio >= IDSHUF_REFUTE_RATIO)
    ...
    hard_pass = bool(d_anchor == d_anchor and hp_anchor_target == hp_anchor_target and d_anchor >= hp_anchor_target
                     ...
                     and oracle_fires and scramble_controlled and identity_collapses and not broken and fair_holds)
    ...
    elif identity_refutes:
        verdict = "HARD_FAIL_IDENTITY_NOT_SPECIFIC_REFUTES_CG"
    elif hard_pass:
        verdict = "HARD_PASS_INDUCTIVE_ANCHOR_COMPOSE_IDENTITY_CLOSED"
    elif hard_fail:
    ...
    verdict_msg = (
        "%s || HELD-OUT MRR [nq=%d]: ANCHOR=%s | ADDITIVE=%s ONESHOT=%s | RANDOM=%s SCRAMBLE=%s IDSHUF=%s | "
        "ORACLE=%s POP=%s || CEILING H(oracle-random)=%s ratio=%sx (fires>=%.1fx&>=%.3f=%s) | anchor_margin=%s vs "
        "HARD_PASS>=%s (=%.2f*H|min%.3f) HARD_FAIL<%s (=%.2f*H) | form_margin=%s (>=%s) | fair_lowmid_margin=%s (>0) "
        "| scramble_margin=%s (<=%s) | IDENTITY: idshuf_margin=%s collapse_ratio=%s (collapses<=%.2f=%s refutes>=%.2f"
        "=%s) | broken=%s | legacy_hits10_gate_fires=%s | frac=%.2f support_frac=%.2f seeds=%d"
        % (
            ...
            _fmt(scramble_target), _fmt(d_idshuf),
            (_fmt(collapse_ratio) if collapse_ratio == collapse_ratio else "nan"),
            IDSHUF_COLLAPSE_RATIO, identity_collapses, IDSHUF_REFUTE_RATIO, identity_refutes,
            broken, legacy_gate_fires,
    ...
    gates = dict(
        ...
        identity_collapse_ratio=(round(collapse_ratio, 4) if collapse_ratio == collapse_ratio else None),
        identity_collapses=identity_collapses,
        identity_refutes=identity_refutes,
        ...
        bands=dict(CEIL_METRIC=CEIL_METRIC, ORACLE_FIRE_RATIO=ORACLE_FIRE_RATIO, ORACLE_FIRE_ABS=ORACLE_FIRE_ABS,
                   ...
                   HELDOUT_ENTITY_FRAC=HELDOUT_ENTITY_FRAC, SUPPORT_FRAC=SUPPORT_FRAC,
                   IDSHUF_COLLAPSE_RATIO=IDSHUF_COLLAPSE_RATIO, IDSHUF_REFUTE_RATIO=IDSHUF_REFUTE_RATIO),
...
def _mechanism_selftest_body(device):
    ...
    idshuf_margin = m[IDSHUF] - m[RANDOM]                            # identity-shuffle headroom on the planted arena
    idshuf_collapse_ratio = (idshuf_margin / anchor_margin) if (anchor_margin == anchor_margin
                                                                and anchor_margin > 0) else float("nan")
    ...
    # ADVERSARIAL identity-shuffle gate: the WRONG-entity bundle must collapse (retain <= 30% of ANCHOR's margin).
    identity_collapses_st = bool(idshuf_collapse_ratio == idshuf_collapse_ratio
    ...
    # ADVERSARIAL: the cross-entity IDENTITY_SHUFFLE must NOT reach ANCHOR_COMPOSE on the planted arena. If it does,
    # a mis-attributed (wrong-entity) bundle scores like the right one -> the win is not entity-specific -> the
    # must-fail control is vacuous and the FULL cannot close the identity loophole. Fail-closed at self-test.
    idshuf_reached_anchor = bool(idshuf_collapse_ratio == idshuf_collapse_ratio
                                 and idshuf_collapse_ratio > SELFTEST_IDSHUF_COLLAPSE_RATIO)
    assert_discriminator_fires(idshuf_reached_anchor, control_name=IDSHUF,
                               headline_name="anchor_compose_beats_identity_shuffle_heldout", run_mode="self_test",
                               extra="IDENTITY_SHUFFLE (a DIFFERENT held entity's composed bundle) retained > %.0f%% of "
                                     "ANCHOR's margin on the planted arena -> the win is not entity-specific / the "
                                     "identity must-fail control is vacuous" % (100 * SELFTEST_IDSHUF_COLLAPSE_RATIO))
    ...
    out.update(
        ...
        idshuf_margin=round(idshuf_margin, 5),
        idshuf_collapse_ratio=(round(idshuf_collapse_ratio, 4) if idshuf_collapse_ratio == idshuf_collapse_ratio else None),
        identity_collapses=identity_collapses_st,
    ...
    ok = bool(oracle_recovers and oracle_fires and anchor_recovers and anchor_beats_random
              and scramble_fails and identity_collapses_st and pop_at_floor and arms_differ)
    return ok, out
