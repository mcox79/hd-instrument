"""
exp_pythia_substrate_memory_mve_gpu_v1 -- N1c Tier-5 cross-ARCHITECTURE generality (Qwen2.5-1.5B keys, not Pythia): substrate-KV memory keyed by Pythia-2.8B (3B-class) hidden states -- GPU.
...
ANCHOR_NAME = "n1c_qwen1p5b_substrate_kv_gpu_v1"; MODEL = "Qwen/Qwen2.5-1.5B"
...
def _selftest():
    ...
    print("[selftest] PASS: n1c-qwen1p5b-substrate-kv", flush=True)
...
def run() -> Dict:
    ...
    tok = AutoTokenizer.from_pretrained(MODEL, trust_remote_code=True); tok.pad_token = tok.eos_token; mdl = AutoModel.from_pretrained(MODEL, torch_dtype=torch.bfloat16, trust_remote_code=True).to(DEV).eval()
    ...
    print("  Qwen-keyed substrate recall@1=%.3f at M=%d facts (in-context ceiling could hold ~%d = %.1f%%)" % (rec, M, IN_CONTEXT_CEIL, 100 * in_context), flush=True)
