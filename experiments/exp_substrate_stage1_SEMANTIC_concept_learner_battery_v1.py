...
ANCHOR_NAME = "substrate_stage1_SEMANTIC_concept_learner_battery_v1"
...
BAND_ARM2_TOP1         = 0.70
BAND_ARM3_TOP3         = 0.50   # PRIMARY
BAND_ARM4_TOP1         = 0.40
BAND_ARM5_REFUSE       = 0.80
BAND_ARM5_RETENTION    = 0.95
BAND_ARM6_COMPLETENESS = 0.60

# Sanity gate: ARM_1 (basic-facts recall) must clear 0.70 before reading higher-order arms.
SANITY_FLOOR_ARM1 = 0.70

# Semantic-concept-hierarchy data shape (per USER spec) ------------------------
if RUN_MODE == "full":
    SEEDS = [7, 17, 23]
    N_DIM = 8192
    N_CATEGORIES = 10
    N_INSTANCES_PER_CAT = 5         # 50 instances total
    N_ATTRIBUTES = 20
    N_TRIPLES_BASIC = 500           # ARM_1 trained-facts target
    N_HELDOUT_INSTANCES = 10        # ARM_3 generalization
    N_FOREIGN_CONCEPTS = 5          # ARM_5 refuse test
    N_AUDIT_QUERIES = 60            # ARM_6 chain trace
else:  # smoke
    SEEDS = [0]
    N_DIM = 1024
    N_CATEGORIES = 6
...
CONFIG_VERSION = (
    ...
    "refuse>=%.2f retention>=%.2f chain>=%.2f sanity>=%.2f"
     ...
     BAND_ARM1_RECALL5, BAND_ARM2_TOP1, BAND_ARM3_TOP3, BAND_ARM4_TOP1,
...
def verdict_for(per_seed: list) -> tuple:
    ...
    a3, a3_cv = _m("arm3_generalization_new_inst.top3")
    ...
    summ = (
        ...
        "A3* heldout_top3=%.3f(>=%.2f cv=%.3f) | A4 compose_top1=%.3f(>=%.2f cv=%.3f) | "
        "A5 refuse=%.3f(>=%.2f) retention=%.3f(>=%.2f) | "
        "A6 chain=%.3f(>=%.2f cv=%.3f) | sanity_ok=%s N=%d cats=%d inst=%d attrs=%d"
    ) % (a1, BAND_ARM1_RECALL5, a1_cv, a2, BAND_ARM2_TOP1, a2_cv,
         a3, BAND_ARM3_TOP3, a3_cv, a4, BAND_ARM4_TOP1, a4_cv,
         a5_refuse, BAND_ARM5_REFUSE, a5_ret, BAND_ARM5_RETENTION,
         a6, BAND_ARM6_COMPLETENESS, a6_cv,
         sanity_ok, N_DIM, N_CATEGORIES, N_INSTANCES_PER_CAT * N_CATEGORIES, N_ATTRIBUTES)
    ...
    if pass_count == 6:
        return ("HARD_PASS",
                "HARD_PASS: substrate IS a concept learner. 6/6 arms PASS including PRIMARY "
                "ARM_3 (generalization to new instance via single is-a + chain composition). "
                + arm_results + " " + summ)
    if pass_count >= 5 and p3:
        return ("CHAIN_GRADE",
                "CHAIN_GRADE: substrate generalizes (PRIMARY pass) with %d/6 arms PASS. "
                "Minor gap noted. %s %s" % (pass_count, arm_results, summ))
    if pass_count >= 3:
        return ("MIDDLE_BAND",
                "MIDDLE_BAND: %d/6 arms PASS; partial mechanism. %s %s" % (
                    pass_count, arm_results, summ))
    return ("HARD_FAIL",
            "HARD_FAIL: %d/6 arms PASS; concept-learner capability absent at this scale. %s %s" % (
                pass_count, arm_results, summ))
...
def _selftest():
    ...
    fake_seed = {
        ...
        "arm2_hierarchical_inheritance": {"inherited_attr_top1": 0.75, "inherited_attr_top3": 0.85,
                                           "chain_hop1_cat_top1": 0.9, "n_instances": 10},
        "arm3_generalization_new_inst": {"top1": 0.4, "top3": 0.55, "n_heldout": 10,
                                          "chance_top3": 0.05},
        "arm4_compositional_triple": {"top1": 0.45, "top3": 0.6, "n_heldout": 10,
                                       "chance_top1": 0.01},
        "arm5_refuse_foreign_concept": {"refuse_accuracy": 0.85, "retention_accuracy": 0.96,
                                         "refuse_threshold": 0.05, "n_foreign": 5, "n_retention": 50,
                                         "mean_retention_cos": 0.1, "std_retention_cos": 0.02,
                                         "mean_foreign_cos": 0.03},
        "arm6_audit_chain_semantic": {"chain_completeness": 0.65, "answer_top1": 0.7,
                                       "n_queries": 60},
    }
