"""Regime Probe 5 core: STORAGE x TOPOLOGY (F fan-in) cross-term.

Cell anchor: `stage1_regime_probe_5_storage_x_topology_v1`
...
Purpose:
    Fifth probe in the Stage 1 Regime Map arc. Probes 1-4 established:
      ...
      Probe 4: STORAGE x N cross-term (SMOKE in flight).
    This probe FIXES CLEANUP_MECHANISM = iterative_cosine and varies STORAGE
    x F (fan-in). Question: does STORAGE moderate the TOPOLOGY_FREE axis? If
    yes -> STORAGE is master moderator across multiple axes (H1). If no ->
    STORAGE special only for MECHANISM (H2). H3 = TOPOLOGY reversal at BUNDLED
    (surprising bundle-crosstalk-vs-fan-in interaction).
...
Hypotheses (falsifiable):
    ...
    H2: storage_x_F_max_abs_deviation < 0.05 AND
        both storages F-invariant (max delta_top1 across F < 0.05 per storage)
        -> STORAGE special only for MECHANISM.
    H3: bundled_top1(F=8) < bundled_top1(F=1) - 0.10 (reversal at cliff corr).
...
Cited source atoms (exact names, no abstraction; META_RULE_AC):
    ...
    MATH_STAGE1_REGIME_MAP_PROBE1_STORAGE_x_CLEANUP (T1, 18:16Z 2026-07-03)
    META_CLEANUP_MECHANISM_AXIS_IS_REGIME_NARROW... (T4, 18:16Z 2026-07-03)
    sharded_fhrr_topology_free_multi_f_dag_v1 (source metrics)
    ...
    stage1_regime_probe_4_storage_x_N_v1 (Probe 4)
    stage1_physics_law_joint_composition_factorial_v1_s11 (source of primitives)

Regime: FHRR chain composition (L=2, N=4096 fixed). Mechanism fixed =
iterative_cosine. Primitives imported from
`_stage1_physics_law_joint_composition_factorial_v1_core` (Principle 11 reuse;
no re-implementation).

Sweep FULL: 2 storage x 4 F x 3 M x 2 corruption = 48 pts / seed.
Sweep SMOKE: 2 storage x 2 F x 1 M x 1 corruption + 1 SHARDED_PC_easy = 5 pts.
...
ANCHOR_NAME = "stage1_regime_probe_5_storage_x_topology_v1"
...
# F (fan-in / TOPOLOGY) axis: 4 levels {1,2,4,8}; matches source atom
# `sharded_fhrr_topology_free_multi_f_dag_v1` tested range + Probe 3 range.
F_GRID_FULL = [1, 2, 4, 8]
# SMOKE: 2 levels (F=1 baseline, F=4 mid-fan-in contrast).
F_GRID_SMOKE = [1, 4]
...
EXPECTED_N_UNITS_FULL = (len(STORAGE_GRID) * len(F_GRID_FULL)
                         * len(M_GRID_FULL) * len(CORRUPTION_GRID_FULL))
# 2 * 4 * 3 * 2 = 48

EXPECTED_N_UNITS_SMOKE = (len(STORAGE_GRID) * len(F_GRID_SMOKE)
...
def selftest() -> Tuple[bool, str]:
    ...
    if EXPECTED_N_UNITS_FULL != 48:
        return False, (f"FULL cardinality {EXPECTED_N_UNITS_FULL} != 48 "
                       f"(2 storage x 4 F x 3 M x 2 corr)")
    ...
    # 4. F-axis discriminator sanity: same seed different F -> different
    # sharded_codebook hash (F axis is real; per Probe 3 pattern).
    gen.manual_seed(3037)
    _, _, _, _, shard_F1, _ = build_rules(M_props, 1, gen, DEVICE, N_test)
    gen.manual_seed(3037)
    _, _, _, _, shard_F4, _ = build_rules(M_props, 4, gen, DEVICE, N_test)
    hash_F1 = hashlib.sha256(
        shard_F1.detach().cpu().numpy().tobytes()).hexdigest()[:16]
    hash_F4 = hashlib.sha256(
        shard_F4.detach().cpu().numpy().tobytes()).hexdigest()[:16]
    if hash_F1 == hash_F4:
        return False, (f"F=1 and F=4 sharded codebooks identical "
                       f"(F axis has no effect); hash={hash_F1}")
    msgs.append(f"F-axis fires: F1_hash={hash_F1} F4_hash={hash_F4}")
...
def run_one_seed(seed: int, run_mode: str) -> Dict[str, Any]:
    ...
    print(f"[run_one_seed] seed={seed} mode={run_mode} device={DEVICE} "
          f"storage={STORAGE_GRID} F={F_grid} M={M_grid} corr={corr_grid} "
          f"mech={FIXED_MECHANISM} N={N_FIXED} L={L_FIXED} TR={TR} "
    ...
    # 1) Main factorial grid: 2 STORAGE x len(F) x len(M) x len(corr)
    for storage in STORAGE_GRID:
        for F in F_grid:
            for M_props in M_grid:
                for corr in corr_grid:
                    salt += 1
                    pt = eval_phase_point(FIXED_MECHANISM, M_props, N_FIXED, F,
                                          L_FIXED, corr, storage, TR, seed, salt)
                    phase_map.append(pt)
                    print(f"  [{len(phase_map):3d}/{expected_n:3d}] "
                          f"storage={storage:8s} F={F:2d} M={M_props:5d} "
    ...
    if is_smoke:
        ...
        print(f"  [{len(phase_map):3d}/{expected_n:3d}] PC_EASY "
              f"storage=SHARDED F={SHARDED_PC_EASY_REGIME['F']} "
              f"M={SHARDED_PC_EASY_REGIME['M']} N={SHARDED_PC_EASY_REGIME['N']} "
    ...
    # ---- PC easy pass (SHARDED at M=200 N=4096 F=1 corr=0.20) ----
    ...
    if is_smoke:
        ...
        pc_easy_matches = [p for p in phase_map
                           ...
                           and p["F"] == SHARDED_PC_EASY_REGIME["F"]
    ...
    # ---- Storage gap at BUNDLED-collapse regime (M=800, F=1, N=4096, corr=0.45) ----
    # In SMOKE this point is in the sweep. In FULL same point available at same M.
    sg_M = 800
    sg_F = 1
    ...
    # ---- Per-storage per-F TOPOLOGY curve (accuracy pooled over M and corr) ----
    per_storage_F_curve: Dict[str, Dict[str, Any]] = {}
    for storage in STORAGE_GRID:
        per_F_stats: Dict[int, Dict[str, float]] = {}
        for F in F_grid:
        ...
        # monotonicity: count breaks (top1 drops > 0.05 as F grows) in
        # cliff-corr sub-slice (harder discriminator regime)
        cliff_corr = max(corr_grid)
        per_F_cliff: Dict[int, float] = {}
        for F in F_grid:
            ...
            if cliff_accs:
                per_F_cliff[F] = round(float(np.mean(cliff_accs)), 4)
        break_count = 0
        prev_acc = None
        sorted_Fs = sorted(per_F_cliff.keys())
        for F in sorted_Fs:
            cur_acc = per_F_cliff[F]
            if prev_acc is not None:
                if cur_acc - prev_acc < -0.05:
                    break_count += 1
            prev_acc = cur_acc
        # F-invariance check for H2: max delta across F per storage
        if per_F_cliff:
            vals = list(per_F_cliff.values())
            max_delta_across_F = round(max(vals) - min(vals), 4)
        else:
            max_delta_across_F = -1.0
        # Reversal check for H3: bundled at F=max vs F=min at cliff corr
        reversal_gap = 0.0
        if len(sorted_Fs) >= 2:
            reversal_gap = round(per_F_cliff[sorted_Fs[-1]]
                                  - per_F_cliff[sorted_Fs[0]], 4)
        per_storage_F_curve[storage] = {
            "per_F_pooled_stats": per_F_stats,
            "per_F_cliff_corr_mean": per_F_cliff,
            "cliff_corr": cliff_corr,
            "monotonicity_break_count": break_count,
            "max_delta_across_F": max_delta_across_F,
            "reversal_gap_max_F_minus_min_F": reversal_gap,
        }

    # ---- STORAGE x F interaction ANOVA deviation ----
    ...
    # STORAGE x F interaction deviation map
    storage_F_interaction: Dict[str, Any] = {}
    max_abs_dev = 0.0
    for storage in STORAGE_GRID:
        for F in F_grid:
            matches = [p["acc"] for p in sweep_pts
                       if p["storage"] == storage and p["F"] == F]
            if not matches:
                continue
            cell_mean = float(np.mean(matches))
            m_storage = per_axis_marginals["storage"].get(str(storage), {}).get(
                "mean_acc", grand_mean)
            m_F = per_axis_marginals["F"].get(str(F), {}).get("mean_acc", grand_mean)
            additive_pred = m_storage + m_F - grand_mean
            dev = cell_mean - additive_pred
            storage_F_interaction[f"{storage}_{F}"] = {
    ...
    return {
        ...
        "storage_gap_at_collapse_regime": {
            "regime": {"M": sg_M, "F": sg_F, "corruption": sg_corr,
                       "N": N_FIXED, "L": L_FIXED, "mechanism": FIXED_MECHANISM},
            "sharded_acc": sg_sharded_acc,
            "bundled_acc": sg_bundled_acc,
            "storage_gap": storage_gap_at_collapse,
        },
        "per_storage_F_curve": per_storage_F_curve,
        "per_axis_marginals": per_axis_marginals,
        "grand_mean_sweep": round(grand_mean, 4),
        "storage_F_interaction": storage_F_interaction,
        "storage_x_F_max_abs_deviation": round(max_abs_dev, 4),
...
def smoke_gate_predicate(body: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    if storage_gap < 0.30:
        return False, (f"storage_gap_fail: gap={storage_gap:.3f} at "
                       f"BUNDLED-collapse regime (M=800 F=1 N=4096 corr=0.45); "
                       f"expected >= 0.30 (arms don't differentiate)")

    # Informational (does NOT gate; per null-hypothesis discipline).
    # Preview of STORAGE x F interaction from smoke sweep.
    per_curve = body.get("per_storage_F_curve", {})
    sharded_cliff = per_curve.get("SHARDED", {}).get("per_F_cliff_corr_mean", {})
    bundled_cliff = per_curve.get("BUNDLED", {}).get("per_F_cliff_corr_mean", {})
    Fs_S = sorted(sharded_cliff.keys()) if sharded_cliff else []
    Fs_B = sorted(bundled_cliff.keys()) if bundled_cliff else []
    if len(Fs_S) >= 2 and len(Fs_B) >= 2:
        sharded_delta_F = sharded_cliff[Fs_S[-1]] - sharded_cliff[Fs_S[0]]
        bundled_delta_F = bundled_cliff[Fs_B[-1]] - bundled_cliff[Fs_B[0]]
        preview_note = (f"SHARDED_delta_F={sharded_delta_F:+.4f} "
                        f"BUNDLED_delta_F={bundled_delta_F:+.4f}")
    else:
        preview_note = "F-curve preview unavailable"
    max_abs_dev = body.get("storage_x_F_max_abs_deviation", 0.0)
    ...
    return True, (f"smoke_gate_pass: cardinality_ok + 2-storage-distinct + "
                  ...
                  f"INFO: storage_x_F_max_abs_dev_smoke={max_abs_dev:.4f} | "
                  f"{preview_note}")
...
def aggregate_and_verdict(per_seed: Dict[str, Dict[str, Any]], run_mode: str
    ...
    common = {
        ...
        "per_storage_F_curve": body.get("per_storage_F_curve"),
        "per_axis_marginals": body.get("per_axis_marginals"),
        "grand_mean_sweep": body.get("grand_mean_sweep"),
        "storage_F_interaction": body.get("storage_F_interaction"),
        "storage_x_F_max_abs_deviation": body.get("storage_x_F_max_abs_deviation"),
    ...
    if not cardinality_ok:
        ...
        max_abs_dev = body.get("storage_x_F_max_abs_deviation", 0.0)
        curve = body.get("per_storage_F_curve", {})
        ...
        sharded_delta_F = curve.get("SHARDED", {}).get("max_delta_across_F", 0.0)
        bundled_delta_F = curve.get("BUNDLED", {}).get("max_delta_across_F", 0.0)
        bundled_reversal = curve.get("BUNDLED", {}).get(
            "reversal_gap_max_F_minus_min_F", 0.0)

        # H3 check first (surprising reversal)
        if bundled_reversal < -0.10:
            verdict = "HARD_PASS"
            vmsg = (f"HARD_PASS_H3_BUNDLED_TOPOLOGY_REVERSAL: "
                    f"bundled(F=max) - bundled(F=min) at cliff corr = "
                    f"{bundled_reversal:.4f} < -0.10; larger F gives WORSE "
                    f"BUNDLED top1; bundle-crosstalk interacts adversely with "
                    f"fan-in; storage_x_F_max_abs_dev={max_abs_dev:.4f}")
        elif (max_abs_dev >= 0.10 and bundled_breaks >= 1 and sharded_breaks == 0):
            verdict = "HARD_PASS"
            vmsg = (f"HARD_PASS_H1_STORAGE_MASTER_MODERATOR_TOPOLOGY_BREAKS_AT_BUNDLED: "
                    f"storage_x_F_max_abs_dev={max_abs_dev:.4f} >= 0.10 AND "
                    f"BUNDLED breaks={bundled_breaks} >= 1 AND "
                    f"SHARDED breaks={sharded_breaks} == 0; STORAGE is master; "
                    f"TOPOLOGY_FREE is a SHARDED-only property")
        elif (max_abs_dev < 0.05
              and sharded_delta_F < 0.05 and bundled_delta_F < 0.05):
            verdict = "HARD_PASS"
            vmsg = (f"HARD_PASS_H2_STORAGE_SPECIAL_ONLY_FOR_MECHANISM: "
                    f"storage_x_F_max_abs_dev={max_abs_dev:.4f} < 0.05 AND "
                    f"both storages F-invariant "
                    f"(S_delta_F={sharded_delta_F:.4f} "
                    f"B_delta_F={bundled_delta_F:.4f}); STORAGE moderator "
                    f"specific to CLEANUP_MECHANISM axis; TOPOLOGY_FREE "
                    f"extends across storage")
        else:
            verdict = "MIDDLE_BAND"
            vmsg = (f"MIDDLE_BAND_INTERMEDIATE_STORAGE_x_F: "
                    f"storage_x_F_max_abs_dev={max_abs_dev:.4f} "
                    f"(H1 needs >=0.10, H2 needs <0.05); "
                    f"SHARDED_breaks={sharded_breaks} BUNDLED_breaks={bundled_breaks} "
                    f"S_delta_F={sharded_delta_F:.4f} "
                    f"B_delta_F={bundled_delta_F:.4f} "
                    f"bundled_reversal={bundled_reversal:.4f}; weak "
                    f"STORAGE x F moderation; file as MM_TENTATIVE crossover")
...
__all__ = [
    ...
    "STORAGE_GRID", "F_GRID_FULL", "F_GRID_SMOKE",
