"""
exp_t5c_pp225_kb5k_v1 -- Path B rescue R3: supervised projection head (bypass cross-attn) -- GPU.
...
ANCHOR_NAME = "t5c_pp225_kb5k_v1"; MODEL = "EleutherAI/pythia-160m"; ENCODER = "BAAI/bge-large-en-v1.5"
STEPS = 100 if "--smoke" in sys.argv else 3000
RUN_MODE = ("smoke" if "--smoke" in sys.argv else os.environ.get("HDLAB_RUN_MODE", "full")).lower()
_ap = argparse.ArgumentParser(); _ap.add_argument("--smoke", action="store_true"); _ap.add_argument("--self-test", action="store_true"); _ARGS, _ = _ap.parse_known_args()
SMOKE = RUN_MODE == "smoke"
N_FACTS = 300 if SMOKE else 5000
...
def _selftest():
    ...
    assert int(_n.argmax([0.1, 0.9])) == 1, "argmax"; print("[selftest] PASS: pp225-kb5k", flush=True)
