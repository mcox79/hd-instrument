"""C2 MULTI HOP CACHING BASELINE v2 at N=4096.

CONTEXT (v290 cap_map follow-on; v2 = re-ship of v1 after CUDA contention kill):
  v1 stalled on 2026-05-31 because auto-CUDA selection in main() grabbed the GPU
  while V2 sustained_workload had it monopolized. The selftest already forced CPU;
  only the main() device line was auto. Fixed here: main() now hardcodes
  torch.device("cpu") -- this anchor lives in remote_cpu_queue and must never
  touch CUDA regardless of availability.
...
PRE-REGISTERED BANDS (carried from v1):
...
Pre-reg: preregs/2026-05-31_multi_hop_caching_baseline_v2_n4096.md
"""
...
_ck_spec = importlib.util.spec_from_file_location("_seed_ckpt_c2v2", _ck_path)
...
def get_output_dir(default_name: str = "multi_hop_caching_baseline_v2_n4096") -> Path:
...
def _instrumentation_selftest() -> None:
    ...
    print(f"[selftest] multi_hop_caching_baseline_v2_n4096 PASS "
...
def main():
    ...
    # v1 BUG: had torch.device("cuda" if torch.cuda.is_available() else "cpu") here
    # which caused stall when GPU was monopolized by V2 sustained_workload (2026-05-31)
    device = torch.device("cpu")
    smoke = args.smoke
    ...
    t0 = time.time()
    print(f"[run] multi_hop_caching_baseline_v2_n4096 smoke={smoke} N={N_cfg} "
    ...
    summary = {"anchor": "multi_hop_caching_baseline_v2_n4096",
