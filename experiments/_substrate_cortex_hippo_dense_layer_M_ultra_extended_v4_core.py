"""Shared core for cortex_hippo dense-Hopfield READ-REPLACE M-ultra-extended v4.

Cross-M ultra-extension of v3 (M in {4096,8192,16384}) per Director task-spec
2026-07-01: extend to M in {8192, 32768, 65536} at N_h=N_c=8192 to probe
replacement-mode wall at 4x and 8x above v2's verified operating point.

v2 (M=8192, N=4096, HP@fc47b1bb):        recall~=1.000 3-seed CG.
v3 (M in {4096,8192,16384}, N=4096):     cross-M expansion (in queue).
v4 (M in {8192,32768,65536}, N=8192):    ULTRA-extended; discriminator survives?

MECHANISM (identical to v2/v3; only sweep axis + N doubled):
...
Adaptive beta (v2 formula preserved):
  beta = clamp(log2(M) / cosine_margin_estimate, BETA_MIN=8, BETA_MAX=128)

DISCRIMINATOR-MUST-SURVIVE-SCALE (analytical justification, check "B"):
  The mechanism discriminator is alpha_simple = M/N_c. Full sweep spans:
    M=8192,  N_c=8192 -> alpha = 1.0
    M=32768, N_c=8192 -> alpha = 4.0
    M=65536, N_c=8192 -> alpha = 8.0  (worst case)
  Smoke MAIN at M=8192, N_c=1024 -> alpha = 8.0 (matches full worst case).
  Because the substrate is alpha-scale-invariant (bipolar-Hebbian STANDARD
  collapses per Amit-Gutfreund at alpha > 0.138 regardless of absolute N;
  dense-Hopfield capacity scales exponentially in N so ratio N/log(N)
  determines margin), the alpha=8.0 smoke MAIN IS the discriminator check.
  If smoke MAIN shows REPLACE >= 0.60 while STANDARD collapses, we have
  verified the mechanism survives at the full run's most-aggressive alpha.
  Preview arm at M=65536,N=8192 is skipped in smoke (fp64 numpy 4GB OOMs
  on laptop); it runs natively as the m=65536 full arm on target GPU.

HP (per-seed, all 3 M):
  For each M in {8192, 32768, 65536}:
...
MB:
  ...
  Also fires if replacement mode holds at M=32768 but degrades at 65536.

CARDINALITY (META_RULE_H):
  EXPECTED_N_UNITS = 3 arms * 3 M values = 9 arm-outcomes per seed.

CRLB (worst case M=8192 among sweep):
  sigma_min = sqrt(0.25/8192) = 0.00552 THEORETICAL@binomial-CLT.
  HP gap 0.60 = 109*sigma; well-reachable.
  At M=65536: sigma_min = sqrt(0.25/65536) = 0.00195; gap = 307*sigma.

CAPACITY REGIME:
  N_c=8192; M in {8192, 32768, 65536}.
  alpha_simple in {1.0, 4.0, 8.0}. STANDARD (bipolar-Hebbian) is above the
  0.138N Amit-Gutfreund wall at all 3 M -> STANDARD will crash.
  Dense-Hopfield Ramsauer capacity ~ exp(N/2) for balanced bipolar keys;
  N_c=8192 admits M<<2^N/2 comfortably. Discriminator = does REPLACE stay
  above 0.60 while STANDARD collapses?

  KEY_INSIGHT: HP condition "recall(REPLACE)/recall(STANDARD) >= 0.80" may be
  UNSATISFIABLE when STANDARD collapses to 0 (division blowup). Verdict logic
  handles this: if STANDARD < 0.10 at high alpha AND REPLACE >= 0.80, tier
  as HARD_PASS by REPLACE-floor absolute (not ratio). Rationale: the ratio
  gate is a "REPLACE not much worse than STANDARD" check; when STANDARD is
  useless, absolute REPLACE quality is the operative discriminator.
...
Exception (no BaseException); Fix #24 GPU-util enforced (>=100MB peak).
"""
...
# ---------------------------------------------------------------------------
# Fixed config (v4-derived; N doubled to 8192; M sweep extended)
# ---------------------------------------------------------------------------
N_HIPPO_FULL = 8192
N_CORTEX_FULL = 8192
HIPPO_SPARSITY = 0.10
...
# Ultra-extended sweep axis: 4x + 8x above v2's M=8192
M_SWEEP_FULL = [8192, 32768, 65536]
# Smoke: run smallest M at reduced N, plus FULL_N preview at largest M
M_SWEEP_SMOKE_MAIN = 8192
M_SWEEP_SMOKE_PREVIEW = 65536
...
# ---------------------------------------------------------------------------
# Torch/CUDA per-arm runner (mandatory for full-N M=65536)
# ---------------------------------------------------------------------------
...
def _selftest_adaptive_beta_computes_finite() -> None:
    """Number provenance: log2(8192)=13; log2(32768)=15; log2(65536)=16.
    beta = log2(M)/margin clamped to [8, 128]. At margin=0.7:
      M=8192:  13/0.7 = 18.571
      M=32768: 15/0.7 = 21.429
      M=65536: 16/0.7 = 22.857
    All in-range for BETA_MIN=8, BETA_MAX=128."""
    for m_test in M_SWEEP_FULL:
    ...
    # Precomputed expected values (META_RULE_AC number-provenance)
    expected = {8192: 13.0 / 0.7, 32768: 15.0 / 0.7, 65536: 16.0 / 0.7}
...
def _selftest_arms_expected_differ() -> None:
    ...
    tmp_out = Path(REPO) / "data" / "_selftest_v4_M_ultra_arms_differ_tmp"
...
def _selftest_M_sweep_cardinality() -> None:
    ...
    if set(M_SWEEP_FULL) != {8192, 32768, 65536}:
        raise AssertionError(
            f"M_SWEEP_FULL must be {{8192,32768,65536}}; got {M_SWEEP_FULL}"
        )
...
def compute_verdict(per_seed_result: Dict, run_mode: str,
    ...
    STANDARD-COLLAPSE EXEMPTION: at high alpha (M/N_c >= 1.0), STANDARD may
    collapse below 0.10 (Amit-Gutfreund wall). In that regime, ratio test is
    ill-defined; substitute absolute REPLACE floor of 0.80 at that M for HP.
    """
    ...
    for m_val in m_keys:
        ...
        # STANDARD collapse is EXPECTED at high alpha here — do NOT hard-fail
        # unless it means the encoder is broken (STANDARD < 0 impossible; keep
        # only the sign check for pathological cases).
        if standard < 0:
            return ("HARD_FAIL",
                    f"STANDARD negative at M={m_val}: {standard}", {})
        ...
        # Ratio only meaningful when STANDARD not collapsed
        standard_collapsed = standard < 0.10
        if standard_collapsed:
            ratio = float("nan")  # not defined; use absolute REPLACE floor
        else:
        ...
        headline["per_M_summary"][int(m_val)] = {
            ...
            "standard_collapsed": bool(standard_collapsed),
        }
        ...
        hp_gap = gap >= 0.60
        if standard_collapsed:
            # High-alpha regime: substitute absolute REPLACE >= 0.80 for ratio
            hp_ratio_or_absolute = replace >= 0.80
        else:
            hp_ratio_or_absolute = ratio >= 0.80

        if not hp_replace_floor:
            return ("HARD_FAIL",
                    f"HARD_FAIL at M={m_val}: REPLACE={replace:.3f} < 0.60 "
                    f"(replacement doesn't survive scale at this M).", {})
        if not (hp_ratio_or_absolute and hp_gap):
            all_pass = False
            fail_reasons.append(
                f"M={m_val}: ratio={ratio} replace={replace:.3f} gap={gap:.3f} "
                f"std_collapsed={standard_collapsed}"
            )
    ...
    if all_pass:
        summary_bits = [
            ...
            f"(ratio={headline['per_M_summary'][m]['ratio_vs_standard']})"
            for m in sorted(headline["per_M_summary"].keys())
        ]
        return ("HARD_PASS",
                f"HARD_PASS_ALL_M: at every M in {{8192,32768,65536}}, REPLACE holds "
                f"above 0.80 (absolute or 0.80*STANDARD) AND gap>=0.60. "
                + " | ".join(summary_bits),
                headline)
