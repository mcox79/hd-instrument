"""Cap 8 VAMP-on-chain iterate-trajectory generator for SRHT + Hadamard (v1c).

v1c fixes the v1b verdict-judge false-negative: v1b's compute_verdict
required n_iter_actual >= 10 per cell, but VAMP early-stops at exactly
5 iterations on these well-conditioned codebooks (the 5-iter MSE-band
< 1e-10 convergence criterion in run_vamp_with_iterates fires once the
iteration plateaus, which on SRHT/Hadamard at N=4096 happens within
5 steps).  v1b produced all 30 files on disk but the judge reported
0/30 written because the min-iter gate was set too high.  v1c lowers
min_iters to 3 (well below the early-stop floor of 5) and also
tolerates Windows backslash separators in stored cell_file paths.
...
Output layout
-------------
data/exp_wave14_cap8_vamp_iterates_srht_hadamard_v1c/
...
Pre-reg: preregs/2026-05-24_wave14_cap8_vamp_iterates_srht_hadamard_v1c.md
"""
...
def compute_verdict(summary: dict) -> tuple[str, str]:
    ...
    def _resolve(cell_path: str) -> Path:
        ...
        # Normalize backslash separators (Windows manifest entries) so the
        # path resolves on any host.
    ...
    # min_iters=3 (full) is sufficient: VAMP early-stops at 5 iterations on
    # well-conditioned codebooks via the 1e-10 MSE-band test in
    # run_vamp_with_iterates.  Setting >=10 (as v1b did) wrongly rejected
    # genuinely converged cells.  3 is comfortably below the floor of 5 yet
    # still excludes pathologically truncated traces.
    ...
    if written == expected_files:
        return ("CAP8_ITERATES_GENERATED",
                ...
                f"with >={min_iters} iterates each.  Composition A audit v4 can now "
                f"compute Spearman rho on SRHT + Hadamard.")
    if written >= 10:
...
def run_experiment(smoke: bool) -> tuple[dict, str, str, float, dict]:
    ...
    out_dir = get_output_dir(
        "wave14_cap8_vamp_iterates_srht_hadamard_v1c_smoke" if smoke
        else "wave14_cap8_vamp_iterates_srht_hadamard_v1c"
    )
...
def run_smoke() -> None:
    self_test()
    out_dir = get_output_dir("wave14_cap8_vamp_iterates_srht_hadamard_v1c_smoke")
...
def run_main() -> None:
    self_test()
    out_dir = get_output_dir("wave14_cap8_vamp_iterates_srht_hadamard_v1c")
