"""
q_a3_l18_cross_layer_composition_v1_n4096 -- Q-A3: L=18 cross-layer composition at N=4096.

Ceiling chase: extends L=17 (ceiling chase from L=16 HARD_PASS EXACT-1.0) to L=18.
  Architecture adds one more Hadamard binding level (L18 outer layer).
  18 W matrices x ~67 MB each = ~1206 MB total. Fits in 8 GB GPU.
...
PRE-REGISTERED BANDS (ceiling push, prior L=16 EXACT-1.0 unanimous):
  HARD-PASS: all 18 level fidelities = 1.0000 unanimous (5/5 seeds).
  MIDDLE: any L_fid in [0.85, 1.0) OR graceful degradation.
  HARD-FAIL: any L_fid < 0.85 OR l18_acc < 0.5.

FORMULA SELF-TESTS (PROT-022):
  1. L=18 chain: 18-step Hadamard roundtrip recovers xi_L1.
     [INPUT: 2-element +-1 vectors, 17 context ops] [EXPECTED: decode = xi_L1]
...
QUEUE: overnight_queue (GPU; 18 W matrices at N=4096).
"""
import sys
...
ANCHOR_NAME = "q_a3_l18_cross_layer_composition_v1_n4096"
...
def _selftest_l18_chain():
    """18-step Hadamard roundtrip recovers xi_L1."""
    ctxs = []
    ...
    for c in reversed(ctxs):
        xi_dec = xi_dec * c
    assert torch.allclose(xi_dec, xi_L1, atol=1e-6), f"L18 chain decode failed: {xi_dec}"
...
def _instrumentation_selftest():
    ...
    ms = [M_INNER, M_MID2, M_MID3, M_MID4, M_MID5, M_MID6, M_MID7, M_MID8,
          M_MID9, M_MID10, M_MID11, M_MID12, M_MID13, M_MID14, M_MID15, M_MID16, M_MID17, M_OUTER]
    ...
    del dummy
    print(f"[selftest] PASS: L18 chain decode ok, capacity_ok, gpu_mem_ok N={n_dim}", flush=True)
...
def run_seed(seed: int, n_dim: int) -> Dict:
    ...
    Xi_ctx18 = bsc(M_OUTER, n_dim)
    Xi_outer = Xi_ctx18 * Xi_mid17[:M_OUTER]
    ...
    for q_idx in range(min(N_QUERIES, M_OUTER)):
        ...
        xi_L18_ret = hopfield_retrieve_gpu(W_outer, probe)
        ...
        level_fids["L18"].append(cosine_sim_gpu(xi_L18_ret, xi_outer_true))
        ...
        l18_acc.append(cosine_sim_gpu(xi_L1_ret, Xi_inner[q_idx]))

    mean_fids = {k: float(sum(v) / len(v)) if v else 0.0 for k, v in level_fids.items()}
    mean_l18 = float(sum(l18_acc) / len(l18_acc)) if l18_acc else 0.0
    ...
    print(f"  [seed={seed}] fids={' '.join(f'{k}:{mean_fids[k]:.4f}' for k in sorted(mean_fids.keys()))} "
          f"l18_acc={mean_l18:.4f} peak_gpu={peak_mem:.3f}GB elapsed={elapsed:.2f}s", flush=True)

    result = {
        "seed": seed, "N": n_dim, "run_mode": RUN_MODE,
        "l18_accuracy": float(mean_l18),
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    l18_acc = mean_key("l18_accuracy")

    fid_list = [fid_vals[f"L{l}"] for l in range(1, L_DEPTH + 1)]
    summary = (" ".join(f"L{l}:{fid_vals[f'L{l}']:.4f}" for l in range(1, L_DEPTH + 1)) +
               f" l18_acc={l18_acc:.4f}(HP>={HP_LACC} HF<{HF_LACC}) n_seeds={len(results)}")

    if any(f < HF_FIDELITY for f in fid_list) or l18_acc < HF_LACC:
        return ("HARD_FAIL", f"HARD_FAIL: any fid<{HF_FIDELITY} or l18_acc<{HF_LACC}. {summary}")

    all_unanimous = all(f >= HP_FIDELITY_UNANI_THRESH for f in fid_list)
    if all_unanimous and l18_acc >= HP_LACC:
        return ("HARD_PASS", f"HARD_PASS: all {L_DEPTH} levels EXACT-1.0 unanimous + l18_acc HP. {summary}")
...
metrics = {
    ...
    "per_seed": [
         ...
         **{f"fid_l{l}": r.get(f"fid_l{l}") for l in range(1, L_DEPTH + 1)},
         "l18_accuracy": r.get("l18_accuracy"),
