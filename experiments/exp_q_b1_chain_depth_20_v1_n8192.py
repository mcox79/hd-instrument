"""
q_b1_chain_depth_20_v1_n8192 -- Q-B1: heteroassoc chain at depth-20 at N=8192.

Extends depth-15 (HP'd at N=8192) to depth-20. Tests chain degradation envelope at depth-20.

GPU IMPLEMENTATION:
  H matrix (N x N float32 at N=8192): 268 MB. Safe on 8 GB GPU.
  Readout snapshots at depths [1, 3, 5, 10, 15, 18, 20].

PRE-REGISTERED BANDS:
  HARD-PASS: depth-5 >= 0.95 AND depth-10 >= 0.88 AND depth-15 >= 0.80 AND depth-20 >= 0.70.
  HARD-FAIL: depth-5 < 0.80 OR depth-10 < 0.65 OR depth-15 < 0.50 OR depth-20 < 0.40.
  MIDDLE: depth-20 in [0.55, 0.70) while others meet HP.
  Prior: depth-15 HP at N=8192 (mean_cos_d15=?); extrapolation at 0.995^20 ~ 0.905.
  Bands: HP at 0.70 (conservative 0.20 below predicted 0.90 midpoint).

FORMULA SELF-TESTS:
  1. depth-20 chain retrieval is non-NaN at N=128.
...
ANCHOR_NAME = "q_b1_chain_depth_20_v1_n8192"
...
CHAIN_DEPTH = 20
SNAPSHOT_DEPTHS = [1, 3, 5, 10, 15, 18, 20]
...
def _instrumentation_selftest():
    ...
    assert not (cos != cos), f"depth-20 retrieval is NaN at N={N_t}"
    ...
    del H
    print(f"[selftest] PASS: depth-20 chain non-NaN N={N_t}, self-cosine=1.0, gpu_mem_ok",
          flush=True)
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    if d5 < HF_D5 or d10 < HF_D10 or d15 < HF_D15 or d20 < HF_D20:
    ...
    if hp5 and hp10 and hp15 and hp20:
        return ("HARD_PASS", f"HARD_PASS: all depth thresholds met at N=8192 depth-20. {summary}")
    n_hp = sum([hp5, hp10, hp15, hp20])
    if n_hp >= 3:
