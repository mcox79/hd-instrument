"""Wave 14.B Phase B.2 POOL_SIZE sweep with sqrt(log P) BETA annealing.

Per pool-size theory survey (notes/wave14b_pool_size_theory.md), the
observed P=4K sweet spot suggests softmax distractor bias at large P.
The theory's diagnostic: scale BETA_RETRIEVAL by sqrt(log P) to
counter the extreme-value catch-up of distractor logits.

This experiment runs the same sweep as exp_wave14b_phase_b2_pool_size_sweep
but with BETA_RETRIEVAL scaled per pool size:
  beta(P) = beta_0 * sqrt(log(P) / log(P_baseline))
where P_baseline = 1024 and beta_0 = 8.0.
...
Pre-registered:
- If softmax bias is the cause of P=4K sweet spot: BWT at P=16K should
  recover (close to or beat P=4K BWT).
- If FIFO eviction or capacity matching: scaling alone won't help.

Parameter variant of the original pool size sweep.
"""
...
def scaled_beta(P):
    """beta(P) = beta_0 * sqrt(log(P) / log(P_baseline))"""
    return BETA_RETRIEVAL_BASE * math.sqrt(math.log(P) / math.log(P_BASELINE))
...
def run_one_pool_size(pool_size, byte_atoms, pos_atoms, target_pos,
    ...
    beta_for_this_P = scaled_beta(pool_size)
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        for batch_start in range(0, T_total_a, BATCH_SIZE):
            ...
            with torch.no_grad():
                ...
                q = shifted_relu(q, RELU_B)
                sims = (byte_atoms @ q.T) / N
                # NOTE: W training uses base beta, only pool-retrieval beta is annealed
                P = torch.softmax(BETA_RETRIEVAL_BASE * sims, dim=0)
    ...
    def eval_all(W_local):
        ...
        for bs in range(0, T_test, BATCH_SIZE):
            ...
            P_W = predict_W(W_local, ctxs, byte_atoms, BETA_RETRIEVAL_BASE, N)
            tgts = test_a_targets[bs:be]
            # Pool retrieval uses ANNEALED beta
            P_c = predict_pool_classical(ctxs, pool_vecs, pool_labels, pool_used, beta_for_this_P, N)
    ...
    return {"pool_size": pool_size, "pool_used": pool_used,
           "beta_for_this_P": beta_for_this_P,
...
def main():
    _say(f"Wave 14.B Phase B.2 POOL_SIZE sweep with sqrt(log P) BETA annealing")
    _say(f"  Sweep: {POOL_SIZES}, base beta = {BETA_RETRIEVAL_BASE}")
    for ps in POOL_SIZES:
        _say(f"    P={ps}: scaled beta = {scaled_beta(ps):.3f}")
    ...
    _say(f"\n  {'pool':>7} | {'beta':>5} | {'post C1':>8} | {'post C2':>8} | {'gap':>8} | {'wall':>5}")
    _say(f"  {'-'*7}-+-{'-'*5}-+-{'-'*8}-+-{'-'*8}-+-{'-'*8}-+-{'-'*5}")
    for ps in POOL_SIZES:
        t0 = time.perf_counter()
        r = run_one_pool_size(ps, byte_atoms, pos_atoms, target_pos, train_a, train_b, test_a)
        dt = time.perf_counter() - t0
        _say(f"  {ps:>7} | {r['beta_for_this_P']:>5.2f} | {r['post_c1']:>8.4f} | {r['post_c2']:>8.4f} | "
             f"{r['gap_post']:>+8.4f} | {dt:>4.0f}s")
        results.append(r)

    _say(f"\n========= COMPARE TO FIXED-BETA SWEEP =========")
    _say(f"  Original (fixed beta=8):")
    _say(f"    P=256:   post C1=4.4226, C2=4.4231")
    _say(f"    P=1024:  post C1=4.3352, C2=4.3354")
    _say(f"    P=4096:  post C1=4.2780, C2=4.2747")
    _say(f"    P=16384: post C1=4.4745, C2=4.4656  (DEGRADED)")
    _say(f"  If softmax bias hypothesis holds, P=16384 should now recover.")

    out_dir = Path(__file__).resolve().parent.parent / "data" / "exp_wave14b_phase_b2_pool_size_annealed"
    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / "metrics.json").write_text(json.dumps(
        {"BYTE_BETA": BYTE_BETA, "BETA_RETRIEVAL_BASE": BETA_RETRIEVAL_BASE,
