"""
pp48_nkt_cross_n_depth17_v1_n8192 -- PP-48 NKT cross-N: depth-17 at N=8192.

Cross-N envelope expansion: depth-17 confirmed at N=4096; now test at N=8192.
  Same sampled-leaf NKT design, same depth-17, same K_FORBIDDEN=100.
  alpha_total = (10 pos + 100 forbidden) / 8192 = 0.0134. Even safer sub-capacity.

  Enables band-lift via cross-N criterion: if depth-17 holds at BOTH N=4096 AND N=8192,
  NKT row gets deeper cross-N endorsement beyond depth-13 N=8192 (already HARD_PASS).

PRE-REGISTERED BANDS (cross-N extension of depth-17 at N=4096 HP):
  ...
  Calibration: N=4096 depth-17 HARD_PASS; bands unchanged for cross-N.
  Note: depth-13 at N=8192 was HARD_PASS (confirmed); depth-17 at N=8192 is next rung.

FORMULA SELF-TESTS:
  1. BRANCH=2, depth=17 total tree: 2^17 - 1 = 131071.
     [INPUT: B=2, D=17] [EXPECTED: total = 131071]
...
ANCHOR_NAME = "pp48_nkt_cross_n_depth17_v1_n8192"
...
NKT_DEPTH = 17
BRANCH = 2
TOTAL_NKT_FULL = (BRANCH ** NKT_DEPTH - 1) // (BRANCH - 1)  # 2^17 - 1 = 131071
assert TOTAL_NKT_FULL == 131071, f"TOTAL_NKT formula: got {TOTAL_NKT_FULL}, expected 131071"
...
def build_nkt_tree_sample(n_dim: int, gen: torch.Generator, k_sample: int) -> List[torch.Tensor]:
    ...
    Leaves are depth-NKT_DEPTH nodes; the NKT property (anti-Hebbian repulsion) depends only
    on the leaves being products of many context bindings, not on them being from a dense subtree.
    """
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    if hp1 and hp2:
        return ("HARD_PASS", f"HARD_PASS: both HP at NKT depth-17 cross-N N=8192. {summary}")
    if hp1 or hp2:
