"""
substrate_stdp_x_b2_sparse_sequence_storage_v1_n8192 -- P4: position-binding x B2 sparse capacity -- remote CPU.
...
SELF-TESTS (PROT-022): 1. STDP transition recall. 2. sparse codes are k-sparse. 3. N=8192.
...
ANCHOR_NAME = "substrate_stdp_x_b2_sparse_sequence_storage_v1_n8192"
...
def s_crit(cb, n, g):
    # STDP-asymmetric transition storage: W += outer(item_{t+1}, item_t); recall by traversal from item_0
    V = cb.shape[0]; mc = 0
    for S in S_GRID:
        seq = g.integers(0, V, size=S + 1)
        W = np.zeros((n, n), dtype=np.float32)
        for j in range(S):
            W += np.outer(cb[seq[j + 1]], cb[seq[j]])
        q = cb[seq[0]].copy(); hits = 0
        for j in range(S):
            r = W @ q; nxt = int(np.argmax(cb @ r)); hits += (nxt == seq[j + 1]); q = cb[nxt]
        if hits / S >= 0.90:
            mc = S
        else:
            break
...
def _selftest():
    ...
    W = np.outer(cb[3], cb[1]); assert int(np.argmax(cb @ (W @ cb[1]))) == 3, "STDP transition recall"
