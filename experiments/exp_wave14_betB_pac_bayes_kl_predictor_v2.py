"""Alt3: PAC-Bayes Laplace-Fisher KL as retention predictor for Bet B. [v2 -- crash fix]

v1 INSTRUMENTATION_FAIL root cause: all 25 cells (5 seeds x 5 pairs) threw exceptions
that were silently swallowed, yielding 0 valid cells and a 22ms exit. v2 fixes:
  1. CPU Generator (torch.Generator() not torch.Generator(device=device)) for atom
     construction -- avoids CUDA generator incompatibility with pa.make_bsc_atoms.
  2. Explicit exception type + traceback printed to stdout BEFORE NaN cell appended --
     so next crash is diagnosable from runner log even if all cells fail.
  3. Corpus preflight check at startup -- prints corpus size so empty-corpus bugs surface
     before spending time in seed loops.
  4. Minor: tensor device move done after CPU atom construction.
...
Pre-reg: preregs/2026-05-24_wave14_betB_pac_bayes_kl_predictor_v2.md
...
def run_one_cell(seed: int, pair_id: int, N: int, batch_size: int,
    ...
    # v2 fix: always use CPU generator for atom construction.
    # pa.make_bsc_atoms uses torch.rand(..., generator=gen) which requires
    # gen.device == tensor.device. Using CPU gen + .to(device) is always safe.
...
def main():
    ...
    # v2 corpus preflight: check that corpus_a has data before entering cell loops
    _corpus_preflight = pa.load_corpus_a()
    print(f"[alt3_pac_bayes_kl] corpus_a preflight: {len(_corpus_preflight)} bytes", flush=True)
    if len(_corpus_preflight) < 100:
        raise RuntimeError(
            f"corpus_a preflight FAIL: only {len(_corpus_preflight)} bytes available. "
            f"Check PLAN.md / README.md exist in repo root."
        )
    ...
    for seed in seeds:
        ...
        for pair_id in pairs:
            ...
            try:
                ...
                # v2: print full traceback so runner log captures root cause
                import traceback
                print(f"  ERROR seed={seed} pair={pair_id}: {type(ex).__name__}: {ex}", flush=True)
