"""
exp_iterative_multihop_bgelarge_v1 -- multi-hop revival Exp 1: iterative retrieval + bge-large (no LLM) -- CPU.

ROUTING: multihop_revival_followon_battery Experiment 3 (K=3 deeper iteration). The Qwen-driven iterative_multihop HARD_FAILed
...
PRE-REGISTERED: HARD-PASS K=3 recall(both-gold-in-3-hops) >= 0.50 (encoder upgrade + iteration clears the multi-hop HP gate). MIDDLE
...
ANCHOR_NAME = "iterative_multihop_k3_v1"
...
def run() -> Dict:
    ...
    for d in data:
        ...
        hop1 = int(order[0])
        qn2 = unit(encode([Q_INSTR + sents[hop1]], tok, m))[0]
        order2 = [i for i in np.argsort(en @ qn2)[::-1] if i != hop1]; hop2 = int(order2[0])
        qn3 = unit(encode([Q_INSTR + sents[hop2]], tok, m))[0]
        order3 = [i for i in np.argsort(en @ qn3)[::-1] if i not in (hop1, hop2)]; hop3 = int(order3[0])
        it_top = {hop1, hop2, hop3}; it_r2 += int(len(it_top & gold) == 2)            # K=3 accumulated, both gold in 3 hops
...
def verdict(r) -> Tuple[str, str]:
    ...
    if r["it_r2"] >= 0.55:
        return ("HARD_PASS", "HARD_PASS: K=3 iteration recovers both gold facts in 3 hops >=0.55 -- deeper iteration adds value. " + s)
    if r["it_r2"] >= 0.50:
        return ("MIDDLE_BAND", "MIDDLE_BAND: K=3 recall 0.50-0.55 -- deeper iteration adds modest value. " + s)
    return ("HARD_FAIL", "HARD_FAIL: K=3 recall <0.50 -- deeper iteration does not help. " + s)
