"""Core module for bytes-per-fact storage efficiency Pareto v4 M-ULTRA-EXTENDED.

v4 changes vs v3:
  1. M sweep further extended to force ceiling escape.
     v3: [1000, 4000, 10000, 20000, 40000] with N_DIM_DENSE=4096 all landed at
     ceiling_saturation_ratio=0.7143 (META_RULE_Q trip).
     v4: [1000, 4000, 10000, 40000, 80000, 160000] with N_DIM_DENSE=2048.
     At N=2048, Hopfield capacity ~0.14*N = 287; M=160k is 558x overload.
     Expect: BINARY, INT8, INT4 crack near M=80k; BFLOAT16 / FP16_range_safe
     crack somewhere in 40k-80k; SPARSE cracks first.
  2. N_DIM_DENSE reduced 4096 -> 2048 to make FP32 dense arm compute-tractable
     at M=160k while keeping Hopfield capacity substantive (287 items).
     N_DIM_SPARSE reduced 16384 -> 8192 (still >= 4x dense per v3 discipline).
  3. META_RULE_Q strict gate retained (ceiling_saturation_ratio < 0.70).
     v4 prediction: with M=160k = 558x overload at N=2048, most arms should
     drop below 0.995 -> saturation ratio should fall well below 0.70.

Design:
  Arms (7 precisions; unchanged from v3):
    ...
    7. SPARSE_BIPOLAR_0p05 top-K sparse ingest at N=8192

  M sweep: {1000, 4000, 10000, 40000, 80000, 160000}: 6 values x 7 arms
  = 42 units/seed. N=2048 dense, 8192 sparse.
...
Discriminator (HARD_PASS):
  ...
  - FP16_range_safe recall >= 0.5 at M=4000 (v3-carry: range fix rescues FP16)
  - INT4 recall >= 0.85 at M=4000 (positive tier)
  - Cross-seed cv <= 0.15
  - All 7 mechanism_hash distinct
  - cardinality_ok: 7 * 6 = 42 units per seed
  - META_RULE_Q strict: ceiling_saturation_ratio < 0.70 (v4 escape gate)
...
# Full: M sweep at N=2048 dense / 8192 sparse.
# v4: added M=80000 and M=160000; dropped M=20000 (interior) to keep unit
# budget at 42/seed. M=160k at N=2048 is 558x Hopfield capacity overload.
FULL_M_SWEEP = [1000, 4000, 10000, 40000, 80000, 160000]
FULL_N_ENT = 5000
FULL_N_REL = 100  # n_ent*n_rel = 500k headroom vs M=160k unique-(s,p)
FULL_QUERY_FRAC = 0.10  # queries = 10% of ingested triples

# Smoke: reduced grid but INCLUDES a top-M discriminator to verify at least
# 3/7 arms drop below 0.85 at ceiling load (v4 smoke gate).
# n_ent * n_rel = 800 * 50 = 40000 headroom for M=20000 max at smoke.
SMOKE_M_SWEEP = [500, 2000, 8000, 20000]
SMOKE_N_ENT = 800
SMOKE_N_REL = 50
SMOKE_QUERY_FRAC = 0.10
SMOKE_N_DIM_DENSE = 1024
SMOKE_N_DIM_SPARSE = 4096

RECALL_TARGET_UNDERLOAD = 0.85
FP16_RANGE_SAFE_MIN_RECALL_AT_M4000 = 0.5
INT4_MIN_RECALL_AT_NOMINAL = 0.85
CEILING_SATURATION_RATIO_MAX = 0.70  # v3 META_RULE_Q strict; retained in v4
...
def _ingest_and_query_fp16_range_safe(triples, E, R, queries, gt, n_dim, device):
    ...
    (same as bf16); the FP32 accumulator is a build-time-only cost.
    """
    sq = math.sqrt(n_dim)
...
def aggregate_and_verdict(per_seed, run_mode: str) -> Dict[str, Any]:
    """v4 HARD_PASS gates (same as v3; M sweep is what changed):
      ...
      4. FP16_range_safe recall >= 0.5 at M=4000-nominal
