"""substrate_pattern_completion_corruption_cliff_v2_narrow_regime sibling seed=13.

v2.1 fix for v1 by-construction phase-coverage failure: corruption_frac NARROWED
from v1 wide-range {0.10, 0.30, 0.50, 0.70, 0.85, 0.95} (24 sat + 48 floor +
0 hp + 0 mb) to v2.1 tight cliff-walking band {0.40, 0.43, 0.46, 0.48, 0.50, 0.52}.
v2 first attempt {0.40,0.50,0.55,0.60,0.65,0.70} REJECTED by 2026-06-28 smoke
(cliff is sharper than v1 suggested; transitions complete by corruption=0.50).
...
SWEEP: N in {2048,4096,8192,16384} x corruption in {0.40,0.43,0.46,0.48,0.50,0.52}
       x cleanup_iters in {1,5,20} = 72 pts per seed (FULL)
...
SEED = 13
ANCHOR_NAME = f"substrate_pattern_completion_corruption_cliff_v2_narrow_regime_seed_{SEED}"
...
CONFIG_VERSION = (
    ...
    f"corruption=[0.40,0.43,0.46,0.48,0.50,0.52],iters=[1,5,20],M=500,beta=8.0,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v2_pc_corruption_cliff_narrow_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    ...
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v2_pc_corruption_cliff_narrow_import_crash",
        }
...
def main() -> int:
    ...
    from experiments._substrate_pattern_completion_corruption_cliff_v2_narrow_regime_core import (
    ...
    final["_hardening_marker"] = "v2_pc_corruption_cliff_narrow_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = (
        "synthetic_substrate_bipolar_codebook_pattern_completion_"
        "corruption_cliff_v2_narrow_regime")
