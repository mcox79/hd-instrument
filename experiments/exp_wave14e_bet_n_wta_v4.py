"""Bet N v4: P3 corpus-specificity with K=512 codebook, explicit cross-entropy criterion.

PARENT: wave14e_bet_n_wta_v3 P3=HARD_FAIL (retrieval_gap=-0.0483).
v4 increases K=512 and tests whether a larger codebook makes cross-corpus contrast visible.
Same P1/P2 metrics; P3 uses same cross-corpus retrieval test but K=512.
...
N_FULL = 4096
N_SMOKE = 512
K_FULL = 512           # v4: K=512 for richer codebook
K_SMOKE = 64
K_ACTIVE_FULL = 12
K_ACTIVE_SMOKE = 4
...
def main():
    ...
    print(
        f"[bet_n_wta_v4] mode={'smoke' if smoke else 'full'} N={N} K={K} "
    ...
    out_dir = get_output_dir("wave14e_bet_n_wta_v4")
