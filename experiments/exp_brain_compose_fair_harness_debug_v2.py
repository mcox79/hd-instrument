"""brain_compose_fair_harness_debug_v2 -- BRAIN_COMPOSE OOM fix.

ARM_SUBSTRATE_BRAIN_COMPOSE failed in fair_harness_substrate_as_lm_v1 with
CUDA OOM (3.05 GiB alloc on 8 GiB GPU with 6.8 GiB allowed): build_pc_stack_gpu
materialized a per-layer [N_TRAIN, dim]=[100k, 8192] float32 = 3.05 GiB tensor
between layers. NOT a numerical inf/nan bug; pure memory-budget bug.

This cell:
  - Drops the persistent cumulative_pred_norm[N_TRAIN, dim] inter-layer tensor.
    For layer i > 0 the residual is computed by re-forwarding src_keys_train
    through Ws[:i] PER CHUNK (O(n_layers**2) FLOPs in exchange for ~3 GiB freed).
  - Frees src_keys_train BEFORE recall starts (recall uses src_keys_held only).
  - Frees each W in Ws after recall via empty_cache between phases.
  - Adds GPU-mem budget projection at run start; fails fast if projection
    exceeds budget instead of OOMing 0.2s into the arm.
  - Adds T11 self-test: build_pc_stack_gpu at smoke-shape returns finite Ws.
  - Adds T12 self-test: pc_stack_forward_gpu returns finite pred at smoke-shape.

Everything else (TEMP_GRID, LAMBDA_GRID, joint sweep, verdict bands, all other
arms, encoder loader, READOUT_DEGENERATE gate) is identical to v1 -- this is
a memory-plumbing fix, not a methodology change.
...
ANCHOR_NAME = "brain_compose_fair_harness_debug_v2"
...
CONFIG_VERSION = (
    "brain_compose_fair_harness_debug_v2; N_DIM=%d PRETRAIN_DIM=%d N_TRAIN=%d N_HELD=%d "
    ...
    "cv_max=%.2f; v2_fix=PC_chunk_recompute_no_cumulative_pred_tensor + "
    "free_src_keys_train_pre_recall + per_layer_empty_cache"
) % (
...
def project_brain_compose_peak_gb(n_train: int, n_held: int, n_dim: int,
                                    pc_layers: int, V: int) -> Dict[str, float]:
    """Project peak GPU memory for BRAIN_COMPOSE arm (v2 plumbing).

    Components (float32, 4 bytes/elt):
      - E_used                [V, dim]
      - src_keys_train        [n_train, dim]        (freed pre-recall)
      - src_keys_held         [n_held, dim]
      - Ws                    pc_layers * [dim, dim]
      - per-chunk transients  ~3 * [INGEST_CHUNK, dim]  (small; ignore)
      - logits                [n_held, V]
      - pred_held             [n_held, dim]

    Returns peak during ingest (W ingest) and during recall.
    """
    b = 4.0  # float32
    e_used = V * n_dim * b
    src_train = n_train * n_dim * b
    src_held = n_held * n_dim * b
    ws = pc_layers * n_dim * n_dim * b
    logits = n_held * V * b
    pred_held = n_held * n_dim * b
    peak_ingest = (e_used + src_train + src_held + ws) / 1e9
    peak_recall = (e_used + src_held + ws + pred_held + logits) / 1e9
    return {"peak_ingest_gb": round(peak_ingest, 3),
            "peak_recall_gb": round(peak_recall, 3)}
...
def _chunk_prior_cum_norm(src_chunk: torch.Tensor, Ws_prior: List[torch.Tensor]
                            ) -> torch.Tensor:
    """Re-forward src_chunk through Ws_prior; return L2-normalized cumulative pred.

    Memory-budget fix (v2): instead of materializing a persistent
    [N_TRAIN, dim] cumulative_pred_norm tensor between PC layers (3.05 GiB at
    N_TRAIN=100k dim=8192 float32 -- caused the v1 OOM), recompute it on
    demand per chunk. Each call costs len(Ws_prior) * chunk_size * dim**2
    FLOPs but allocates only [chunk_size, dim] which fits trivially.
    """
    if not Ws_prior:
        return src_chunk
    cum = torch.zeros_like(src_chunk)
    s_l = src_chunk
    for W in Ws_prior:
        this = s_l @ W.T
        cum = cum + this
        s_l = _l2_normalize_t(cum)
    return _l2_normalize_t(cum)
...
def build_pc_stack_gpu(src_keys: torch.Tensor, tgt_vecs: torch.Tensor,
    ...
    v2: no persistent [N_TRAIN, dim] cumulative_pred_norm tensor between layers
    (3.05 GiB at production scale). Layer i>0 recomputes the prior cumulative
    prediction per chunk via _chunk_prior_cum_norm(src_chunk, Ws[:i]).
    """
    ...
    for layer_i in range(n_layers):
        ...
        for b in range(0, n_pairs, ingest_chunk):
            ...
            src_chunk_raw = src_keys[b:end]
            # src for THIS layer = prior cumulative pred (or raw src for layer 0)
            if layer_i == 0:
                src_b = src_chunk_raw
                cum_b = None
            else:
                cum_b = _chunk_prior_cum_norm(src_chunk_raw, Ws)
                src_b = cum_b
            ...
            # free per-chunk intermediates explicitly
            del src_chunk_raw, tgt_b, resid, src_b
...
def compute_arm_logits(arm_label: str, E: torch.Tensor, idx_train: np.ndarray,
    ...
    if use_brain:
        ...
        # v2 free src_keys_train (3.05 GiB at production scale) BEFORE recall;
        # only src_keys_held (~600 MiB) is needed downstream.
        del src_keys_train
        if device.type == "cuda":
            torch.cuda.empty_cache()
        ...
        # v2 symmetric: free src_keys_train before recall (not needed)
        del src_keys_train
        if device.type == "cuda":
            torch.cuda.empty_cache()
    ...
    # NOTE: src_keys_train was already freed pre-recall (v2 OOM fix).
    del pred_held, src_keys_held, idx_train_t, idx_held_t, logits
    if use_sparse_bp or use_brain:
        del E_used
    if device.type == "cuda":
        torch.cuda.empty_cache()
...
def _selftest():
    ...
    # T11 (v2 OOM fix): build_pc_stack_gpu at smoke-shape returns finite Ws.
    # Smoke shape: V=64, dim=128, n_train=200, pc_layers=3. Runs on CPU.
    _t11_dev = torch.device("cpu")
    _t11_dim, _t11_V, _t11_n = 128, 64, 200
    _t11_gen = torch.Generator(device=_t11_dev).manual_seed(2026)
    _t11_E = torch.randn(_t11_V, _t11_dim, generator=_t11_gen)
    _t11_E = _t11_E / (_t11_E.norm(dim=1, keepdim=True) + 1e-12)
    _t11_idx = torch.randint(0, _t11_V, (_t11_n,), generator=_t11_gen)
    _t11_src = _t11_E[_t11_idx]
    _t11_Ws = build_pc_stack_gpu(_t11_src, _t11_E, _t11_idx, ingest_chunk=64, n_layers=3)
    assert len(_t11_Ws) == 3, "T11 wrong Ws count"
    for _i, _W in enumerate(_t11_Ws):
        assert _W.shape == (_t11_dim, _t11_dim), "T11 W%d wrong shape" % _i
        assert torch.isfinite(_W).all().item(), "T11 W%d not finite" % _i

    # T12 (v2 OOM fix): pc_stack_forward_gpu returns finite pred at smoke-shape.
    _t12_held = _t11_E[torch.randint(0, _t11_V, (50,), generator=_t11_gen)]
    _t12_pred = pc_stack_forward_gpu(_t11_Ws, _t12_held, recall_batch=16)
    assert _t12_pred.shape == (50, _t11_dim), "T12 wrong pred shape"
    assert torch.isfinite(_t12_pred).all().item(), "T12 pred not finite"
    # also confirm cosine logits are finite at smoke
    _t12_logits = _t12_pred @ _t11_E.T
    assert torch.isfinite(_t12_logits).all().item(), "T12 logits not finite"
    # and softmax-with-T at T=0.5 is finite for the resulting logits
    _t12_logits_np = _t12_logits.detach().cpu().numpy()
    _t12_probs = softmax_logits_with_T(_t12_logits_np, 0.5)
    assert np.all(np.isfinite(_t12_probs)), "T12 softmax not finite"
    ...
    print("[selftest] PASS: T1 trigram + T2 mockKV + T3 peakedT001 + T4 uniformT10 "
          ...
          "+ T11 PC-stack Ws finite + T12 PC-stack forward pred+logits+softmax finite",
          flush=True)
...
def _synthesize_on_exit():
    ...
    try:
        ...
        metrics = {
            ...
            "detail": detail,
            "metrics_source": "atexit_synthesize_partial_brain_compose_fair_harness_debug_v2",
            "per_unit": units,
...
if __name__ == "__main__":
    ...
    if DEVICE.type == "cuda":
        try:
            ...
            print("[gpu] device=%s name=%s total_mem_gb=%.2f" % (
                DEVICE, torch.cuda.get_device_name(0), total_gb), flush=True)
            # v2 BUDGET PROJECTION: warn if BRAIN_COMPOSE projected peak >
            # 70% of total GPU. Cell-author projects ingest+recall peaks.
            proj = project_brain_compose_peak_gb(N_TRAIN, N_HELD, N_DIM,
                                                    PC_N_LAYERS, VOCAB_CAP)
            print("[gpu] BRAIN_COMPOSE projection: ingest_peak=%.2fGB recall_peak=%.2fGB "
                  "(total=%.2fGB; 70pct_budget=%.2fGB)" % (
                      proj["peak_ingest_gb"], proj["peak_recall_gb"],
                      total_gb, total_gb * 0.7), flush=True)
            if max(proj["peak_ingest_gb"], proj["peak_recall_gb"]) > total_gb * 0.7:
                print("[gpu][WARN] projected BRAIN_COMPOSE peak exceeds 70pct of GPU; "
                      "expect OOM on busy GPU. Consider smaller N_TRAIN/N_DIM "
                      "or split BRAIN_COMPOSE into its own run.", flush=True)
        except Exception as e:
    ...
    try:
        ...
        pass
    run_cfg = {"run_mode": RUN_MODE, "N": N_DIM, "schema": "brain-compose-fair-harness-debug-v2"}
    t0 = time.time()
    _T0_REF[0] = t0
    for seed in SEEDS:
        key = "s%d" % seed
    ...
    metrics = {
        ...
        "n_seeds": len(SEEDS),
        "detail": detail,
        "metrics_source": "measured_gpu_brain_compose_fair_harness_debug_v2",
        "per_unit": units,
