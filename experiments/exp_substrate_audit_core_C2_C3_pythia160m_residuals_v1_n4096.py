"""
substrate_audit_core_C2_C3_pythia160m_residuals_v1_n4096 -- audit-core C2/C3 on REAL Pythia-160M residuals -- remote CPU.
...
ANCHOR_NAME = "substrate_audit_core_C2_C3_pythia160m_residuals_v1_n4096"
_N_SUFFIX = 4096; N = 4096; assert N == _N_SUFFIX
RUN_MODE = ("smoke" if "--smoke" in sys.argv else os.environ.get("HDLAB_RUN_MODE", "full")).lower()
_ap = argparse.ArgumentParser(); _ap.add_argument("--smoke", action="store_true"); _ap.add_argument("--self-test", action="store_true"); _ARGS, _ = _ap.parse_known_args()

HIDDEN = 768; F_SPARSE = 0.05
NPZ_PATH = os.environ.get("HDLAB_RESIDUAL_NPZ", str(REPO / "data" / "exp_phase05_v1_pythia160m_residual_extract_v1" / "residuals.npz"))
if RUN_MODE == "smoke":
...
def load_residuals(g):
    if RUN_MODE != "smoke" and os.path.exists(NPZ_PATH):
        R = np.load(NPZ_PATH)["residuals"].astype(np.float32)
        print("[data] loaded real residuals %s shape=%s" % (NPZ_PATH, R.shape), flush=True)
        return R
...
def run_seed(seed: int) -> Dict:
    ...
    shifted = newb + 0.5 * g.standard_normal(newb.shape).astype(np.float32) if len(newb) else store[:half] + 1.0
