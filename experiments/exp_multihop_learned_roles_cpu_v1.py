"""
exp_multihop_learned_roles_cpu_v1.py -- Path 2: LEARNED substrate slot-filler role-tagger (PP-369 mechanism) -- CPU.
...
ANCHOR_NAME = "multihop_learned_roles_cpu_v1"
...
def extract(text):
    ...
    for k, w in enumerate(toks):
        ...
        out.append({"v": val, "noun": noun, "idx": k, "roles": roles, "heur": set(roles),
                    "win": [_st(re.sub(r"[^a-z]", "", t)) for t in toks[max(0, k - 3):k + 6]], "inq": (qs is not None and k >= qs)})
    # WK constants as PER-role fillers (conditional gating)
    def _wkd(v, noun, idx): return {"v": v, "noun": noun, "idx": idx, "roles": {"PER", "WK"}, "heur": {"PER", "WK"}, "win": [], "inq": False}
    for (x, y, v) in WK_PER:
        if tgt and tgt == x and y in wordset: out.append(_wkd(v, y, 900))
    for k, w in enumerate(toks):
        ...
        if st in WK_COLL and (isn(k - 1) or isn(k + 1) or isn(k - 2) or isn(k + 2)):
            out.append(_wkd(WK_COLL[st], st, 901))
    if "%" in text: out.append(_wkd(Fraction(100), "percent", 902))
    return out, tgt
...
ROLE_SET = ["PER", "TGT", "TOT", "SUB", "ADD", "INQ", "CNT"]


def _emit_role(d, tgt, vrank):
    """emission features for tagging number d's role (substrate slot-filler PP-369 mechanism: context-window emission)."""
    fs = ["EBIAS", "noun:" + d.get("noun", ""), "vrank:" + vrank]
    if d.get("inq"): fs.append("E_inq")
    if tgt and d.get("noun") == tgt: fs.append("E_tgtmatch")
    for w in d.get("win", []): fs.append("w:" + w)
    for h in sorted(d.get("heur", [])): fs.append("h:" + h)
    return fs


def _gold_roles(pool, gi, gj, gop):
    """op-consistent weak role labels for the gold operand pair (supervision beyond heuristic cues)."""
    def has(idx, r): return r in pool[idx]["heur"]
    if gop == "+": return {gi: "ADD" if has(gi, "ADD") else "CNT", gj: "ADD" if has(gj, "ADD") else "CNT"}
    if gop == "-": return {gi: "TOT", gj: "SUB"}
    if gop == "rs": return {gi: "SUB", gj: "TOT"}
    if gop == "*":
        if has(gj, "PER"): return {gi: "CNT", gj: "PER"}
        if has(gi, "PER"): return {gi: "PER", gj: "CNT"}
        return {gi: "CNT", gj: "PER"}
    if gop == "/": return {gi: "TOT", gj: "PER"}
    if gop == "rd": return {gi: "PER", gj: "TOT"}
    return {}


def _vrank(pool, i):
    vals = sorted(set(d["v"] for d in pool if not d["roles"] & {"WK"}))
    if not vals or pool[i]["roles"] & {"WK"}: return "wk"
    v = pool[i]["v"]
    if v == vals[-1]: return "max"
    if v == vals[0]: return "min"
    return "mid"


def _train_role_tagger(TR, rng):
    rows = []
    for text, pool, tgt, (gi, gj, gop) in TR:
        gr = _gold_roles(pool, gi, gj, gop)
        for i, d in enumerate(pool):
            if d["roles"] & {"WK"}: continue
            lab = gr.get(i) or (sorted(d["heur"])[0] if d["heur"] else "CNT")
            if lab not in ROLE_SET: lab = "CNT"
            rows.append((_emit_role(d, tgt, _vrank(pool, i)), lab))
    if not rows: return None
    w = {r: defaultdict(float) for r in ROLE_SET}; cw = {r: defaultdict(float) for r in ROLE_SET}; c = 1
    for ep in range(8 if not SMOKE else 3):
        for i in rng.permutation(len(rows)):
            feats, g = rows[i]; sc = {r: sum(w[r][f] for f in feats) for r in ROLE_SET}
            pred = max(ROLE_SET, key=lambda r: (sc[r], r))
            if pred != g:
                for f in feats: w[g][f] += 1; w[pred][f] -= 1; cw[g][f] += c; cw[pred][f] -= c
            c += 1
    return {r: {f: w[r][f] - cw[r][f] / c for f in w[r]} for r in ROLE_SET}


def _apply_tagger(pool, tgt, tagger):
    """AUGMENT heuristic roles with the learned tag (namespaced L_*) so the selector keeps both signals. WK numbers unchanged."""
    for i, d in enumerate(pool):
        if d["roles"] & {"WK"}: continue
        feats = _emit_role(d, tgt, _vrank(pool, i))
        r = max(ROLE_SET, key=lambda rr: (sum(tagger[rr].get(f, 0.0) for f in feats), rr))
        d["roles"] = set(d["heur"]) | {"L_" + r}
...
def _solve(train, test, seed):
    ...
    # PATH 2: train substrate slot-filler role-tagger on weak (op-derived) labels, then REPLACE heuristic roles with learned tags
    tagger = _train_role_tagger(TR, np.random.default_rng(seed + 1))
    if tagger is not None:
        for (text, pool, tgt, g) in TR: _apply_tagger(pool, tgt, tagger)
    ...
    for text, ans in test:
        pool, tgt = extract(text)
        if len(pool) < 2: continue
        if tagger is not None: _apply_tagger(pool, tgt, tagger)
