...
ANCHOR_NAME = "substrate_capacity_scaling_sweep_xl_v1"
RUN_MODE = ("smoke" if "--smoke" in sys.argv else os.environ.get("HDLAB_RUN_MODE", "full")).lower()
_ap = argparse.ArgumentParser(); _ap.add_argument("--smoke", action="store_true"); _ap.add_argument("--self-test", action="store_true"); _ARGS, _ = _ap.parse_known_args()
if RUN_MODE == "smoke":
    SEEDS = [1, 2]; N_GRID = [512, 1024]; LOADS = [0.05, 0.1, 0.2, 0.4]
else:
    SEEDS = [7,17,23,31,43,59,71,83,97,101]; N_GRID = [1024, 2048, 4096, 8192, 16384]; LOADS = [0.04, 0.06, 0.08, 0.10, 0.13, 0.16, 0.20, 0.25, 0.32, 0.40]
FLIP = 0.15; STEPS = 5
