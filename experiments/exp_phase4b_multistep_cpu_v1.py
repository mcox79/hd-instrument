...
ANCHOR_NAME = "phase4b_multistep_cpu_v1"
...
def _selftest():
    ...
    print("[selftest] PASS: phase4b-multistep", flush=True)
_selftest()
if _ARGS.self_test:
    sys.exit(0)
def _load():
...
def run() -> Dict:
    ...
    # discriminative perceptron over 16 op-pair classes
    LAB = PAIRS
    ...
    maj = max(LAB, key=lambda p: sum(1 for _f, gg in Xtr if gg == p))
    correct = 0
    for txt, ans in test:
        ns = _nums(txt)[:3]; a, b, c3 = ns; feats = _feats(txt)
        sc = {p: sum(avg[p].get(f, 0.0) for f in feats) for p in LAB}
        pred = max(LAB, key=lambda p: (sc[p], p))
        r = _eval2(a, b, c3, pred[0], pred[1])
        if r is not None and Fraction(r).limit_denominator(10**6) == ans: correct += 1
    acc = correct / len(test) if test else 0.0
    print("  PHASE4B-MULTISTEP (MultiArith): classifier-acc=%.3f (%d/%d) | answer-consistency-ceiling=%.3f | train-labeled=%d (vs single-op 0.022)" %
          (acc, correct, len(test), ceiling, len(Xtr)), flush=True)
    return {"accuracy": round(acc, 3), "ceiling": round(ceiling, 3), "n_test": len(test), "n_train_labeled": len(Xtr)}
def verdict(r) -> Tuple[str, str]:
    if r.get("error"): return ("UNKNOWN", "UNKNOWN: " + r["error"])
    a = r["accuracy"]; s = "classifier-acc=%.3f ceiling=%.3f (n_test=%d, train-labeled=%d)" % (a, r["ceiling"], r["n_test"], r["n_train_labeled"])
    if a >= 0.20:
        return ("HARD_PASS", "HARD_PASS: substrate multi-step (2-op) composition reaches >=0.20 on MultiArith -- >9x the single-op 0.022 baseline; discriminative op-SEQUENCE prediction works substrate-only. Multi-step composition is a real substrate capability. " + s)
    if a >= 0.10:
