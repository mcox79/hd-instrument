...
ANCHOR_NAME = "pfc_gate_waypoint_rescue_stacked_corrections_v1"
PARENT_ANCHOR = "pfc_gate_waypoint_rescue_kb_grounded_check_v1"
...
RESCUE_ARM = "wp_stacked_kb_plus_selector"   # the STACKED OR-gate under test (fixed, not a max-over-arms)
KB_ARM = "wp_kb_grounded_gate"               # channel A (landed single-channel; the frontier to push past)
SELECTOR_ARM = "wp_calibrated_selector_gate" # channel B (NEW cross-fit calibrated selector, alone)
KEY_COMPARATOR = "wp_bisect_verify"          # the SELF-DERIVED-correction MUST-FAIL control (the bar)
...
def train_calibrated_selector(per_op, n_ops: int, V: int, reach_cum: Dict[int, torch.Tensor],
    ...
    if len(y_l) < 8 or (sum(y_l) < 2) or (sum(y_l) > len(y_l) - 2):
        ...
        return {"w": torch.zeros(5, dtype=DTYPE, device=DEVICE), "cal_a": 1.0, "cal_b": 0.0,
                "tau": 0.0, "outdeg": outdeg, "rcsum": rcsum, "degenerate": True,
    ...
    return {"w": w, "cal_a": cal_a, "cal_b": cal_b, "tau": tau, "outdeg": outdeg, "rcsum": rcsum,
...
def _eval_regime(n_ops: int, V: int, dd: int, E: torch.Tensor, W_ops: List[torch.Tensor],
    ...
    # RESCUE = wp_stacked_kb_plus_selector (A OR B; FIXED); KEY comparator = verify (self-derived MUST-FAIL).
    # failure-mask screen (THE new one) uses the KB-alone and SELECTOR-alone per-chain correctness masks.
...
def aggregate_and_verdict(per_seed: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for r in REGIMES:
        ...
        best_rescue_arm = RESCUE_ARM                     # stacked A OR B (FIXED, not a max-over-arms)
        ...
        recovery_kb = ((kb_a - flat) / headroom_decomp) if headroom_decomp > 1e-6 else 0.0
        recovery_sel = ((sel_a - flat) / headroom_decomp) if headroom_decomp > 1e-6 else 0.0
        ...
        # gain(arm) = recovery(arm) - recovery(verify)  [research (c); baseline = self-derived control]
        gain_kb = recovery_kb - recovery_verify
        gain_sel = recovery_sel - recovery_verify
        gain_stacked = recovery_rescue - recovery_verify     # == delta_recovery
        # super-additive iff stacking beats the SUM of the two single-channel gains (multiplicative law)
        super_additive = bool(gain_stacked > (gain_kb + gain_sel))
        stacked_over_kb = recovery_rescue - recovery_kb      # frontier-push margin over the better channel
    ...
    for rk, v in per_regime.items():
        ...
        if ref_kb is not None and ref_kb > 1e-6:
            ...
            v["flatness_kb"] = 0.0
        # HARD_PASS = the STACKED arm genuinely pushes the frontier PAST both single channels:
        #   materially above KB-alone (recovery + stacked_over_kb margins) AND super-additive AND the two
        #   NEW independence screens (selector-vs-M_error AND failure-mask-corr) pass AND flatness holds
        #   both absolutely and relative to KB-alone.
        hp_ok = (v["oracle_rail_ok"] and v["headroom_exec_ok"] and v["headroom_decomp_ok"]
                 ...
                 and v["stacked_over_kb"] >= HP_STACKED_OVER_KB_MIN
                 and v["super_additive"]
                 and v["flatness_ratio"] >= HP_FLATNESS_MIN
                 and (v["flatness_ratio"] - v["flatness_kb"]) >= HP_FLATNESS_OVER_KB_MIN
                 and v["abs_independence_corr"] <= HP_INDEP_CORR_MAX
                 and v["abs_selector_independence_corr"] <= HP_SEL_INDEP_CORR_MAX
                 and v["abs_failmask_corr"] <= HP_FAILMASK_CORR_MAX
                 and not v["failmask_degenerate"]
                 and not v["independence_degenerate"]
    ...
    if not cardinality_ok:
        ...
        # even the STACKED arm cannot materially beat the SELF-DERIVED-correction control (verify)
        # -> the compounding-error bound survives -> strongest closure to date.
        verdict = "HARD_FAIL_COMPOUNDING_ERROR_BOUND_REAL"
    elif (not fv["failmask_degenerate"]) and fv["abs_failmask_corr"] > HF_FAILMASK_CORR_CEIL:
        # the two channels' FAILURE MASKS are strongly correlated -> they read the SAME coverage-density
        # signal through different windows -> stacking is redundant; the fix is a DATA-coverage problem,
        # not a channel-count problem (research point-2 confirmed; the FIRST diagnostically-actionable neg).
        verdict = "HARD_FAIL_FAILURE_MASKS_CORRELATED"
    elif fv["stacked_over_kb"] <= HP_STACKED_OVER_KB_MIN:
        # stacking adds no material recovery over KB-alone despite a genuinely differently-designed,
        # cross-fit-independent second channel -> the wall is single-channel-fundamental (redundant stack).
        verdict = "HARD_FAIL_STACKING_REDUNDANT"
    elif fv["abs_independence_corr"] > HF_INDEP_CORR_CEIL:
        verdict = "HARD_FAIL_KB_SIGNAL_NOT_INDEPENDENT"
    elif fv["abs_selector_independence_corr"] > HF_INDEP_CORR_CEIL:
        # the selector's confidence is strongly correlated with the SR estimator's own error -> channel B
        # is contaminated / not independent of M/R; the cross-fit independence premise is void.
        verdict = "HARD_FAIL_SELECTOR_NOT_INDEPENDENT"
        ...
        # stacking helps over verify + over kb but sub-threshold or a guard failed
        ...
        elif fv["abs_failmask_corr"] > HP_FAILMASK_CORR_MAX:
            # partial failure-mask correlation (0.20, 0.50] -> some genuine independent signal, contaminated
            # by the shared coverage-density cause -> real but sub-multiplicative stacking value.
            verdict = "MIDDLE_BAND_PARTIAL_FAILMASK_CORR"
        elif not fv["super_additive"]:
            # additive-but-not-superadditive: real combination benefit, but below the multiplicative bar.
            verdict = "MIDDLE_BAND_ADDITIVE_NOT_SUPERADDITIVE"
        elif fv["abs_independence_corr"] > HP_INDEP_CORR_MAX:
            verdict = "MIDDLE_BAND_PARTIAL_INDEPENDENCE"
        elif fv["abs_selector_independence_corr"] > HP_SEL_INDEP_CORR_MAX:
            verdict = "MIDDLE_BAND_SELECTOR_PARTIAL_INDEPENDENCE"
        ...
        elif (fv["flatness_ratio"] - fv["flatness_kb"]) < HP_FLATNESS_OVER_KB_MIN:
            verdict = "MIDDLE_BAND_FLATNESS_NOT_ABOVE_KB"
    ...
    grid_str = " ".join(
        "%s(e%.1f:F%.2f/O%.2f/VER%.2f/KB%.2f/SEL%.2f/STK%.2f fmc=%.2f sic=%.2f)" % (
            rk, per_regime[rk]["entropy"], per_regime[rk]["flat_gonogo"],
            per_regime[rk]["hier_oracle"], per_regime[rk]["wp_bisect_verify"],
            per_regime[rk]["arm_means"]["wp_kb_grounded_gate"],
            per_regime[rk]["arm_means"]["wp_calibrated_selector_gate"],
            per_regime[rk]["arm_means"]["wp_stacked_kb_plus_selector"],
            per_regime[rk]["failmask_corr"], per_regime[rk]["selector_independence_corr"])
    ...
    if fv is not None:
        head = ("%s | FOCUS=%s(ent=%.1f steps=%d) FLAT=%.3f OEXEC=%.3f HIER_ORACLE=%.3f | OPEN=%.3f "
                "VERIFY=%.3f KB=%.3f SEL=%.3f STACK=%.3f | recov_ver=%.3f recov_kb=%.3f recov_sel=%.3f "
                "recov_stk=%.3f DELTA(vs_ver)=%.3f stk_over_kb=%.3f super_add=%s | "
                "gain_kb=%.3f gain_sel=%.3f gain_stk=%.3f | flatness=%.3f flat_kb=%.3f | "
                "FAILMASK_CORR=%.3f (kb_fail=%.2f sel_fail=%.2f) indep_corr=%.3f sel_indep=%.3f | "
                "kb_confirm=%.3f sel_acc=%.3f kb_fresh=%.3f sel_deg=%s | lift_flat=%.3f lift_random=%.3f "
                "index_gap=%.3f anti_taut=%.3f degen=%.3f sign_p=%.4g rr=%.3f cv=%.3f | "
                "spr(delta,ent)=%.3f | GRID [%s] n_seeds=%d") % (
            verdict, focus_rk, fv["entropy"], fv["chain_steps"], fv["flat_gonogo"], fv["oracle_exec"],
            fv["hier_oracle"], fv["wp_bisect_open"], fv["wp_bisect_verify"], fv["kb_alone"],
            fv["selector_alone"], fv["best_rescue"],
            fv["recovery_verify"], fv["recovery_kb"], fv["recovery_sel"], fv["recovery_rescue"],
            fv["delta_recovery"], fv["stacked_over_kb"], fv["super_additive"],
            fv["gain_kb"], fv["gain_sel"], fv["gain_stacked"], fv["flatness_ratio"], fv["flatness_kb"],
            fv["failmask_corr"], fv["failmask_kb_rate"], fv["failmask_sel_rate"],
            fv["independence_corr"], fv["selector_independence_corr"],
            fv["kb_confirm_mean"], fv["sel_accept_rate"], fv["kb_fresh_rate"], fv["sel_degenerate"],
            fv["lift_flat"], fv["lift_random"], fv["index_artifact_gap"], fv["anti_tautology_corr"],
            fv["degenerate_rate"], fv["sign_test_p"], fv["reach_rank_test"], fv["brs_cv"],
    ...
    return {
        ...
        "stacking_beats_kb": bool(fv["stacked_over_kb"] > HP_STACKED_OVER_KB_MIN) if fv else False,
        "super_additive_focus": bool(fv["super_additive"]) if fv else False,
        "failmask_screen_pass": bool(fv["abs_failmask_corr"] <= HP_FAILMASK_CORR_MAX
                                     and not fv["failmask_degenerate"]) if fv else False,
        "independence_screen_pass": bool(fv["abs_independence_corr"] <= HP_INDEP_CORR_MAX
                                         and not fv["independence_degenerate"]) if fv else False,
...
def _verdict_selftest() -> None:
    def _mk(n_ops, V, dd, flat, oexec, horc, ver, kb, sel, stk, rand, idxm,
            open_a=None, degen=0.02, taut=0.10, exact=0.20, rr=0.60,
            n_res_only=None, n_idx_only=2, n_rand_only_idx=2,
            indep_corr=0.05, sel_indep_corr=0.05, failmask_corr=0.05, kb_conf=0.5,
            sel_acc_frac=0.40, indep_degen=False, failmask_degen=False, sel_degen=False):
        # RESCUE = stacked (FIXED); KEY comparator = verify. paired KEY = stacked-vs-verify.
        if open_a is None:
            open_a = ver
        if n_res_only is None:
            n_res_only = 45 if stk > ver + 0.1 else 1
        arms = {"flat_gonogo": flat, "oracle_exec": oexec, "hier_oracle": horc,
                ...
                "wp_replay_generate_select": ver, "wp_kb_grounded_gate": kb,
                "wp_calibrated_selector_gate": sel, "wp_stacked_kb_plus_selector": stk,
                "wp_random_state": rand, "wp_index_midpoint": idxm}
        oth = {"flat_gonogo": "f", "oracle_exec": "oracle_true_seq", "hier_oracle": "ho",
               ...
               "wp_stacked_kb_plus_selector": "st", "wp_random_state": "rd", "wp_index_midpoint": "ix"}
        return {"n_ops": n_ops, "V": V, "dd": dd, "entropy": decision_entropy(n_ops, dd),
                ...
                "independence_corr": indep_corr, "kb_confirm_mean": kb_conf, "kb_confirm_std": 0.4,
                "m_error_mean": 0.5, "independence_degenerate": indep_degen,
                "n_indep_units": 180, "kb_confirm_rate": 0.02, "kb_fresh_rate": 0.1,
                "kb_fallback_rate": 0.05,
                "selector_independence_corr": sel_indep_corr, "selector_conf_mean": 0.5,
                ...
                "stack_fallback_rate": 0.05, "failmask_corr": failmask_corr, "failmask_kb_rate": 0.5,
                "failmask_sel_rate": 0.5, "failmask_degenerate": failmask_degen, "n_failmask_units": 60,
    ...
    # FOCUS d8: flat=0.081 oexec=0.918 horc=0.906 (hd=0.825); ver=0.096 (recovery_verify=0.0182).
    # d4: flat=0.515 horc=0.953 (hd=0.438). HARD_PASS numbers (stacked genuinely pushes past both):
    #   d8 kb=0.30 (recov_kb=0.2655) sel=0.25 (recov_sel=0.2048) stk=0.52 (recov_stk=0.5321).
    #     delta=0.514>=0.15; stacked_over_kb=0.267>=0.03; gain_stk=0.514 > gain_kb+gain_sel=0.4339 (super!);
    #     recov_stk=0.532>=0.35.
    #   d4 kb=0.773 (recov_kb=0.589 -> flatness_kb=0.451) stk=0.887 (recov_stk=0.850 -> flatness=0.626);
    #     flatness>=0.5 and flatness-flatness_kb=0.175>=0.10; lift_flat/lift_random clean; sign_p<<0.05.
    def _hp_lo(**kw):
        d = dict(flat=0.515, oexec=0.957, horc=0.953, ver=0.823, kb=0.773, sel=0.80, stk=0.887,
                 rand=0.09, idxm=0.10)
        d.update(kw)
        return _mk(4, 1200, 4, **d)

    def _hp_hi(**kw):
        d = dict(flat=0.081, oexec=0.918, horc=0.906, ver=0.096, kb=0.30, sel=0.25, stk=0.52,
                 rand=0.02, idxm=0.02)
        d.update(kw)
    ...
    try:
        # HARD_PASS: super-additive stacking, both screens pass, frontier pushed.
        ...
        assert out["focus_best_rescue_arm"] == "wp_stacked_kb_plus_selector", "ST13 best_rescue=stacked"
        assert out["super_additive_focus"] and out["failmask_screen_pass"], "ST13 HP screens"

        # HARD_FAIL bound-real: even STACKED can't beat verify. stk=0.10 -> recov=0.023; delta=0.0048<=0.05.
        out2 = _run(_hp_lo, lambda: _hp_hi(stk=0.10, n_res_only=3))
        assert out2["verdict"] == "HARD_FAIL_COMPOUNDING_ERROR_BOUND_REAL", \
            "ST13 expected HARD_FAIL bound-real got %s" % out2["verdict"]

        # HARD_FAIL failure-masks correlated: good delta but failmask_corr=0.70 > 0.50 (shared cause).
        out3 = _run(_hp_lo, lambda: _hp_hi(failmask_corr=0.70))
        assert out3["verdict"] == "HARD_FAIL_FAILURE_MASKS_CORRELATED", \
            "ST13 expected HARD_FAIL failmask-correlated got %s" % out3["verdict"]

        # HARD_FAIL stacking redundant: delta ok, failmask ok, but stacked_over_kb<=0.03 (kb=0.50 stk=0.51).
        out4 = _run(_hp_lo, lambda: _hp_hi(kb=0.50, stk=0.51))
        assert out4["verdict"] == "HARD_FAIL_STACKING_REDUNDANT", \
            "ST13 expected HARD_FAIL stacking-redundant got %s" % out4["verdict"]

        # HARD_FAIL selector NOT independent: sel_indep_corr=0.60 > 0.40 (fires after kb-indep check).
        out5 = _run(lambda: _hp_lo(sel_indep_corr=0.60), lambda: _hp_hi(sel_indep_corr=0.60))
        assert out5["verdict"] == "HARD_FAIL_SELECTOR_NOT_INDEPENDENT", \
            "ST13 expected HARD_FAIL selector-not-independent got %s" % out5["verdict"]

        # HARD_FAIL kb NOT independent: indep_corr=0.60 > 0.40.
        out6 = _run(lambda: _hp_lo(indep_corr=0.60), lambda: _hp_hi(indep_corr=0.60))
        assert out6["verdict"] == "HARD_FAIL_KB_SIGNAL_NOT_INDEPENDENT", \
            "ST13 expected HARD_FAIL kb-not-independent got %s" % out6["verdict"]

        # HARD_FAIL accelerating collapse: delta>0.05, stacked_over_kb>0.03, screens ok, but flatness<0.2.
        #   d8 stk=0.204 (recov=0.150) kb=0.16 (recov=0.096; stacked_over_kb=0.054>0.03); d4 stk high (0.887,
        #   recov=0.850) -> flatness=0.176<0.2.
        out7 = _run(_hp_lo, lambda: _hp_hi(kb=0.16, sel=0.14, stk=0.204, n_res_only=30))
        assert out7["verdict"] == "HARD_FAIL_ACCELERATING_COLLAPSE", \
            "ST13 expected HARD_FAIL accelerating got %s" % out7["verdict"]

        # MIDDLE partial failmask corr: failmask_corr=0.35 in (0.20, 0.50].
        out8 = _run(_hp_lo, lambda: _hp_hi(failmask_corr=0.35))
        assert out8["verdict"] == "MIDDLE_BAND_PARTIAL_FAILMASK_CORR", \
            "ST13 expected MIDDLE partial-failmask got %s" % out8["verdict"]

        # MIDDLE additive-not-superadditive: stk=0.45 (recov=0.4473). gain_stk=0.429 < gain_kb+gain_sel
        #   (0.4339); stacked_over_kb=0.182>0.03; recov>=0.35; failmask ok -> ADDITIVE_NOT_SUPERADDITIVE.
        out9 = _run(_hp_lo, lambda: _hp_hi(stk=0.45))
        assert out9["verdict"] == "MIDDLE_BAND_ADDITIVE_NOT_SUPERADDITIVE", \
            "ST13 expected MIDDLE additive-not-superadditive got %s" % out9["verdict"]

        # MIDDLE selector vacuous: sel_acc_frac_train=0.995 (>0.98) -> selector gate does nothing distinct.
        out10 = _run(lambda: _hp_lo(sel_acc_frac=0.995), lambda: _hp_hi(sel_acc_frac=0.995))
        assert out10["verdict"] == "MIDDLE_BAND_SELECTOR_VACUOUS", \
            "ST13 expected MIDDLE selector-vacuous got %s" % out10["verdict"]

        # INCONCLUSIVE index-order leak: index >> random with significance (fires before HF checks).
        out11 = _run(_hp_lo, lambda: _hp_hi(idxm=0.30, n_idx_only=40, n_rand_only_idx=2))
        assert out11["verdict"] == "INCONCLUSIVE_INDEX_ORDER_LEAK", \
            "ST13 expected INCONCLUSIVE leak got %s" % out11["verdict"]

        # INCONCLUSIVE no discriminating regime (oracle_exec below rail).
        out12 = _run(lambda: _hp_lo(oexec=0.50), lambda: _hp_hi(oexec=0.50))
        assert out12["verdict"] == "INCONCLUSIVE_NO_DISCRIMINATING_REGIME", \
            "ST13 expected INCONCLUSIVE no-regime got %s" % out12["verdict"]
    finally:
        REGIMES, REGIME_KEYS, EXPECTED_N_UNITS = saved
    print("[selftest] ST13 verdict wiring OK (HARD_PASS super-additive; HARD_FAIL bound-real+failmask-"
          "correlated+stacking-redundant+selector-not-indep+kb-not-indep+accelerating; MIDDLE partial-"
          "failmask+additive-not-superadditive+selector-vacuous; INCONCLUSIVE leak+no-regime)", flush=True)
        ...
        try:
            ...
            _atomic_write_metrics(out_dir, {
                ...
                "verdict_msg": "SELFTEST_OK: ST1-ST13 (TD shrink, reach-matrix identity, gamma_for_span, "
                               ...
                               "cross-fit selector, failure-mask-corr, "
                               "recovery/delta-vs-verify/flatness formulas, spearman/entropy/binom, "
                               "pipeline, stacked verdict wiring)",
