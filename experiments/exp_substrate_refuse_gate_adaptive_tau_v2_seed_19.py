"""substrate_refuse_gate_adaptive_tau_v2 sibling seed=19.

2x-drill from v1 refuse-gate-adaptivity-phase-diagram which returned MIDDLE_BAND
(4/6 family pairs differ but 0/48 HP+MB; cal_size_sensitivity=0.0). Audit
hypothesis (a65f731f): FIXED-V_REL regime saturates; the adaptivity dimension
that matters is MEMORY-of-prior-refuses (sliding-window tau adjustment).

5 ARMS (TEMPORAL TAU EVOLUTION; not v1's STATIC family swap):
    FIXED_TAU_V1            : baseline reproducer (control)
    SLIDING_WINDOW_TAU_W8   : 25th-pctile of last 8 confidences
    SLIDING_WINDOW_TAU_W32  : 25th-pctile of last 32 confidences
    KALMAN_FILTER_TAU       : Kalman posterior mean
    EWMA_TAU                : EWMA alpha=0.10
...
SEED = 19
ANCHOR_NAME = "substrate_refuse_gate_adaptive_tau_v2_seed_%d" % SEED

_ap = argparse.ArgumentParser()
_ap.add_argument("--smoke", action="store_true")
_ap.add_argument("--self-test", action="store_true", dest="self_test")
_ap.add_argument("--timeout", type=int, default=5400,
...
CONFIG_VERSION = (
    "ANCHOR=%s,arms=[FIXED_TAU_V1,SLIDING_WINDOW_TAU_W8,SLIDING_WINDOW_TAU_W32,"
    "KALMAN_FILTER_TAU,EWMA_TAU],regimes=[PURE_IN,PURE_OUT,NEAR_DOMAIN_MIXED,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v2_refuse_gate_adaptive_tau_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v2_refuse_gate_adaptive_tau_import_crash",
        }
...
def main() -> int:
    ...
    from experiments._substrate_refuse_gate_adaptive_tau_v2_core import (
        run_one_seed_adaptive_tau, aggregate_and_verdict, selftest,
    ...
    for seed in remaining:
        _write_minimal_metrics(out_dir, "RUNNING",
                               ...
                               extra={"_phase": "seed_running",
                                      ...
                                      "backend": backend})
        t0 = time.time()
        result = run_one_seed_adaptive_tau(seed, run_mode=RUN_MODE)
        result["N"] = N_used
    ...
    final["_hardening_marker"] = "v2_refuse_gate_adaptive_tau_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = ("synthetic_substrate_refuse_gate_adaptive_"
                                   "tau_5_arm_4_regime_3_cal_size_sweep_v2")
