"""
kappa3_nlo_formula_validation_sigma_g_v1_n4096 -- validate corrected NLO sigma_g_crit formula.
...
ANCHOR_NAME = "kappa3_nlo_formula_validation_sigma_g_v1_n4096"
_N_SUFFIX = 4096
N = 4096
...
def build_W_noisy(n, alpha, sigma_g, rng):
    ...
    if sigma_g > 0:
        ...
        mult = np.exp(sigma_g * g - 0.5 * sigma_g * sigma_g).astype(np.float32)
        W = W * mult
    return W
...
def _selftest():
    ...
    # log-normal unit mean
    rng = np.random.default_rng(0)
    g = rng.standard_normal(200000)
    sg = 0.5
    mult = np.exp(sg * g - 0.5 * sg * sg)
    assert abs(float(np.mean(mult)) - 1.0) < 0.02, float(np.mean(mult))
    print(f"[selftest] PASS: formula(0.30)={_formula(0.30,0.05):.6f} free_k3_equal=0 lognormal_unit_mean ok", flush=True)
...
def run_seed(seed: int, n_dim: int) -> Dict:
    ...
    for sg in SIGMA_G_GRID:
        ...
        abs_dev = abs(signed_dev)                    # magnitude (compared to formula)
        # Formula validation is on MAGNITUDE; sign is flagged separately (see SIGN FLAG in docstring).
        rel_err = abs(abs_dev - pred) / max(abs(pred), 1e-9)
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    summary = ("match(mag)=" + " ".join(f"{sg:.2f}:{int(mean_match[sg])}" for sg in SIGMA_G_GRID) +
               f" sgcrit={sgcrit} n_match={n_match}/7 measured_sign={sign_note}(formula predicts +) iddev=" +
               " ".join(f"{sg:.2f}:{mean_iddev[sg]:.3f}" for sg in SIGMA_G_GRID))

    if n_match <= HF_MATCH_CELLS:
        return ("HARD_FAIL", f"HARD_FAIL: NLO formula matches only {n_match}/7 cells; correction refuted. {summary}")
    if (n_match >= HP_MATCH_CELLS and holds_to_050 and breaks_by_080
            and sgcrit is not None and SGCRIT_LO <= sgcrit <= SGCRIT_HI):
        return ("HARD_PASS",
                f"HARD_PASS: NLO formula validated ({n_match}/7 cells within {MATCH_TOL:.0%}); "
                f"sigma_g_crit={sgcrit} in [{SGCRIT_LO},{SGCRIT_HI}] (consistent with 0.715). {summary}")
    return ("MIDDLE_BAND", f"MIDDLE_BAND: partial NLO validation. {summary}")
