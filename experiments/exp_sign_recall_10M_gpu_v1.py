"""
exp_sign_recall_10M_gpu_v1.py -- sign-key autoassociative recall@1 at 10M keys (GPU) -- GPU.

ROUTING: GPU-scale substrate-physics (sign-key recall at 10M). 10M sign keys (D=1024); chunked GPU recall@1 under noise. Pushes the recall scaling gate to 10M (10x the 1M CPU gate). torch.cuda. GPU.
PRE-REGISTERED: HARD-PASS recall@1 >= 0.99 at N=10M under 0.15 flip. MIDDLE >= 0.95. HARD-FAIL < 0.95.
...
ANCHOR_NAME = "sign_recall_10M_gpu_v1"
RUN_MODE = ("smoke" if "--smoke" in sys.argv else os.environ.get("HDLAB_RUN_MODE", "full")).lower()
_ap = argparse.ArgumentParser(); _ap.add_argument("--smoke", action="store_true"); _ap.add_argument("--self-test", action="store_true"); _ARGS, _ = _ap.parse_known_args()
SMOKE = RUN_MODE == "smoke"

def _selftest():
    import numpy as _n; assert _n.argmax([0.1, 0.9]) == 1, "argmax"; print("[selftest] PASS: sign_recall_10M_gpu_v1", flush=True)
_selftest()
if _ARGS.self_test:
    sys.exit(0)
try:
