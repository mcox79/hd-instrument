"""
q_a3_l13_cross_layer_composition_v1_n4096 -- Q-A3: L=13 cross-layer composition at N=4096.

Extends L=12 to L=13. Ceiling chase for compositional depth.
  L=12 verdict pending (shipped same cycle as L=11 HP).
  All L through L=12 have been observed at exact fidelity 1.0000.
  L=13 ships to find the degradation onset.

Architecture (extends L=12 by one more Hadamard level):
  ...
  L10: M_mid10=2. L11: M_mid11=2. L12: M_mid12=2.
  L13: M_outer=2, Hadamard binding ctx13*L12.

  13 W matrices x ~67 MB each = ~871 MB total. Safe on 8 GB GPU.

PRE-REGISTERED BANDS (ceiling push, all prior L EXACT-1.0):
  HARD-PASS: all 13 level fidelities = 1.0000 unanimous (5/5 seeds).
  MIDDLE: any L_fid in [0.85, 1.0).
  HARD-FAIL: any L_fid < 0.85 OR l13_acc < 0.5.

FORMULA SELF-TESTS:
  1. L=13 chain: 13-step Hadamard roundtrip recovers xi_L1.
     [INPUT: 2-element +-1 vectors, 12 context ops] [EXPECTED: decode = xi_L1]
...
ANCHOR_NAME = "q_a3_l13_cross_layer_composition_v1_n4096"
...
def _selftest_l13_chain():
    """13-step Hadamard roundtrip recovers xi_L1."""
    ctxs = [torch.tensor([1.0, -1.0], dtype=torch.float32, device=DEVICE),
            ...
            torch.tensor([-1.0, 1.0], dtype=torch.float32, device=DEVICE),
            torch.tensor([1.0, 1.0], dtype=torch.float32, device=DEVICE),
            torch.tensor([-1.0, -1.0], dtype=torch.float32, device=DEVICE)]  # ctx2..ctx13
    ...
    for c in reversed(ctxs):
        xi_dec = xi_dec * c
    assert torch.allclose(xi_dec, xi_L1, atol=1e-6), f"L13 chain decode failed: {xi_dec}"
...
def _instrumentation_selftest():
    ...
    ms = [M_INNER, M_MID2, M_MID3, M_MID4, M_MID5, M_MID6, M_MID7, M_MID8,
          M_MID9, M_MID10, M_MID11, M_MID12, M_OUTER]
    ...
    del dummy
    print(f"[selftest] PASS: L13 chain decode ok, capacity_ok, gpu_mem_ok N={n_dim}", flush=True)
...
def run_seed(seed: int, n_dim: int) -> Dict:
    ...
    Xi_ctx13 = bsc(M_OUTER, n_dim)
    Xi_outer = Xi_ctx13 * Xi_mid12[:M_OUTER]
    ...
    for q_idx in range(min(N_QUERIES, M_OUTER)):
        ...
        xi_L13_ret = hopfield_retrieve_gpu(W_outer, probe)
        ...
        level_fids["L13"].append(cosine_sim_gpu(xi_L13_ret, xi_outer_true))
        ...
        l13_acc.append(cosine_sim_gpu(xi_L1_ret, xi_L1_true))

    mean_fids = {k: float(sum(v) / len(v)) if v else 0.0 for k, v in level_fids.items()}
    mean_l13 = float(sum(l13_acc) / len(l13_acc)) if l13_acc else 0.0
    ...
    print(f"  [seed={seed}] fids={' '.join(f'{k}:{mean_fids[k]:.4f}' for k in sorted(mean_fids.keys()))} "
          f"l13_acc={mean_l13:.4f} peak_gpu={peak_mem:.3f}GB elapsed={elapsed:.2f}s", flush=True)

    result = {
        "seed": seed, "N": n_dim, "run_mode": RUN_MODE,
        "l13_accuracy": float(mean_l13),
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    l13_acc = mean_key("l13_accuracy")

    fid_list = [fid_vals[f"L{l}"] for l in range(1, L_DEPTH + 1)]
    summary = (" ".join(f"L{l}:{fid_vals[f'L{l}']:.4f}" for l in range(1, L_DEPTH + 1)) +
               f" l13_acc={l13_acc:.4f}(HP>={HP_LACC} HF<{HF_LACC}) n_seeds={len(results)}")

    if any(f < HF_FIDELITY for f in fid_list) or l13_acc < HF_LACC:
        return ("HARD_FAIL", f"HARD_FAIL: any fid<{HF_FIDELITY} or l13_acc<{HF_LACC}. {summary}")

    all_unanimous = all(f >= HP_FIDELITY_UNANI_THRESH for f in fid_list)
    if all_unanimous and l13_acc >= HP_LACC:
        return ("HARD_PASS", f"HARD_PASS: all {L_DEPTH} levels EXACT-1.0 unanimous + l13_acc HP. {summary}")
...
metrics = {
    ...
    "per_seed": [
         ...
         **{f"fid_l{l}": r.get(f"fid_l{l}") for l in range(1, L_DEPTH + 1)},
         "l13_accuracy": r.get("l13_accuracy"),
