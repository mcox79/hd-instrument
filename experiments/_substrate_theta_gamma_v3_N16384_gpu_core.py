"""Shared core for theta_gamma_v3_N16384_gpu siblings.

Theta-gamma phase binding v3 at N_DIM=16384 (4x v2's N=4096; extends
sequence encoding + order binding to higher hyperdimensionality regime).

v2 (2026-06-30) landed HARD_PASS FULL at N_DIM=4096:
  30/30 pts; pairs_differ=10/10; max_fhrr_vs_cyclic_log2_delta=2.000;
  nested_vs_flat32_log2_delta=1.000; NO_POSITION K=50 acc=0.02
  MEASURED@data/exp_exp_substrate_theta_gamma_v2_FHRR_all_complex_seed_7/metrics.json

v3 extends to N_DIM=16384. Capacity ~ N/8 (JL bound); FHRR cliff K expected
to scale linearly. v2 CYCLIC cliff at K=200 (N=4096); v3 CYCLIC cliff
expected ~K=800; FLAT_32 ~K=200; NESTED ~K=400. K_SEQ sweep extended.

Discriminator (from task prompt):
  - cliff ordering preserved at N=16384 with cv=0.000 (matching v2)
  - max_fhrr_vs_cyclic_log2_delta >= 1.5 (v2 hit 2.000 at N=4096)

Anchor: theta_gamma_v3_N16384_gpu_seed_{7,13,19}
Pre-reg: preregs/2026-07-01_theta_gamma_v3_N16384_gpu.md
...
# K_SEQ sweep axis extended for higher-N cliff exploration
# v2 N=4096: cliff CYCLIC=200, NESTED=100, FLAT_32=50 (log2 6.64, 6.64, 5.64)
# v3 N=16384 expected: CYCLIC ~800, NESTED ~400, FLAT_32 ~200 (log2 9.6, 8.6, 7.6)
# Sweep extended right to capture N=16384 cliffs
K_SEQ_SWEEP_FULL = (50, 100, 200, 500, 1000, 2000, 5000)
K_SEQ_SWEEP_SMOKE = (50, 200, 1000, 5000)  # 4 K-points spanning full range

# Regime (LOCKED)
N_DIM = 16384                 # 4x v2; task-prompt spec
ITEM_VOCAB_SIZE = 10000       # same as v2 for direct comparison
...
HP_LOG2_SEPARATION_FHRR_VS_CYCLIC = 1.5   # per task-prompt (v2 hit 2.0)
...
EXPECTED_N_UNITS_FULL = len(ARMS) * len(K_SEQ_SWEEP_FULL)     # 5 * 7 = 35
...
def _get_device(strict_gpu: bool = False) -> torch.device:
    ...
    if strict_gpu:
        raise RuntimeError(
            ...
            "theta-gamma v3 at N_DIM=%d K_SEQ up to %d requires "
...
def run_one_seed_phase_diagram(
    ...
    print(
        f"[run_one_seed_thetagamma_v3] seed={seed} mode={run_mode} "
...
def smoke_gate_predicate(body: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    # DISCRIMINATOR-MUST-SURVIVE-SCALE (Category B/C): smoke MUST fire the
    # discriminator at task-prompt floor. Sub-half floor allowed at smoke
    # because K_SEQ sweep is truncated.
    ...
    return True, (
        f"smoke_gate_pass_v3: cardinality_ok + "
...
def aggregate_and_verdict(per_seed: Dict[str, Dict[str, Any]],
    ...
    if is_smoke:
        ...
        if passed:
            verdict = "HARD_PASS"
            vmsg = (
                f"HARD_PASS_SMOKE_thetagamma_v3: {observed_n}/{expected_n} pts; "
            ...
            verdict = "HARD_FAIL"
            vmsg = f"HARD_FAIL_SMOKE_thetagamma_v3: {reason}"
        out = dict(common)
        out.update({
            "verdict": verdict,
            "verdict_msg": vmsg,
    ...
    if not cardinality_ok:
        ...
        vmsg = (
          ...
          and n_pairs_differ >= 9):
        verdict = "HARD_PASS"
        vmsg = (
            f"HARD_PASS_THETA_GAMMA_v3_N16384: {observed_n}/{expected_n} pts; "
    ...
    elif n_pairs_differ >= 6 and max_fhrr_vs_cyclic >= 0.5:
        verdict = "MIDDLE_BAND"
        vmsg = (
            f"MIDDLE_BAND_THETA_GAMMA_v3_PARTIAL: "
