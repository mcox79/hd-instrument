"""substrate_schema_family_phase_diagram_v1 sibling seed=19.

FIFTH systematic COMPONENT-SUBSTITUTION phase diagram. Substitutes the SCHEMA
MECHANISM FAMILY (4 families: EXEMPLAR_BAYES / PROTOTYPE_BASED / HYBRID /
BAYESIAN_WITH_PRIORS) on a shared (alpha, n_schemas) inner grid at N=4096 fixed.

CHUNKED-PER-SEED architecture (USER 2026-06-28): one seed per sibling file.
Sibling pair: seed_7, seed_13.

SWEEP: 4 families x 4 alphas in {0.01, 0.1, 1.0, 10.0} x 3 n_schemas in {10, 50, 200}
       = 48 phase points per seed FULL; 6 corners per seed SMOKE.

CPU-only (numpy + scipy.special.logsumexp); no GPU needed.

PRE-REG: preregs/2026-06-29_substrate_schema_family_phase_diagram_v1.md
CARDINALITY_OK_FULL: 48 records per seed (4 families x 12 inner pts)
CARDINALITY_OK_SMOKE: 6 records per seed (6 corners)
...
SEED = 19
ANCHOR_NAME = f"substrate_schema_family_phase_diagram_v1_seed_{SEED}"
...
CONFIG_VERSION = (
    ...
    f"families=[EXEMPLAR_BAYES,PROTOTYPE_BASED,HYBRID,BAYESIAN_WITH_PRIORS],"
    f"alpha=[0.01,0.1,1.0,10.0],n_schemas=[10,50,200],N=4096,"
    f"expected_n_full=48,expected_n_smoke=6,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v1_schema_family_phase_diagram_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v1_schema_family_phase_diagram_import_crash",
        }
...
def main() -> int:
    ...
    from experiments._substrate_schema_family_phase_diagram_v1_core import (
        run_one_seed_phase_diagram, aggregate_and_verdict, selftest,
        get_backend_label, FAMILIES, ALPHA_VALUES, N_SCHEMAS_VALUES,
        N_QUERIES_FULL, N_QUERIES_SMOKE, SMOKE_CORNERS,
    )
    ...
    final["_hardening_marker"] = "v1_schema_family_phase_diagram_chunked"
    ...
    if SMOKE_MODE:
        ...
        expected_n = len(FAMILIES) * len(ALPHA_VALUES) * len(N_SCHEMAS_VALUES)
    observed_n = 0
    for body in per_seed.values():
        observed_n += len(body.get("phase_map", []))
