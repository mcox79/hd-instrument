"""fresh_W_bpc_per_encoder_v2 -- METHODOLOGY-CORRECTED Path A BPC test (re-dispatch).

v2 NOTE 2026-06-23: v1 ran on remote and ARM_CHAR_TRIGRAM_FRESH_W produced
BPC 7.74 (= pure unigram) BUT the 3 pretrained encoder arms (word2vec / GloVe /
FastText) failed with ENCODER LOAD FAIL because remote .venv lacked gensim.
USER 2026-06-23 authorized installing gensim-4.4.0 + smart_open-7.6.1 into
remote C:/dev/hd-instrument/.venv; gensim cache (fasttext + glove fully
present; word2vec re-SCP'd in parallel or re-downloaded by gensim) at remote
C:/dev/hd-instrument/data/gensim_cache/ is staged. v2 is the same cell as v1
(no code changes) re-dispatched under a fresh ship_name so all 4 encoder arms
execute and the V2 LM gap closure decisive test completes.
...
Cites:
  ...
  - preregs/2026-06-23_fresh_W_bpc_per_encoder_v1.md (parent)
...
ANCHOR_NAME = "fresh_W_bpc_per_encoder_v2"
...
CONFIG_VERSION = (
    "fresh_W_bpc_per_encoder_v2; N_DIM=%d PRETRAIN_DIM=%d N_TRAIN=%d N_HELD=%d "
...
def _synthesize_on_exit():
    ...
    try:
        ...
        metrics = {
            ...
            "detail": detail,
            "metrics_source": "atexit_synthesize_partial_fresh_W_bpc_per_encoder_v2",
            "per_unit": units,
...
if __name__ == "__main__":
    ...
    metrics = {
        ...
        "n_seeds": len(SEEDS),
        "detail": detail,
        "metrics_source": "measured_gpu_fresh_W_bpc_per_encoder_v2",
        "per_unit": units,
