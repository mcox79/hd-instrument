...
ANCHOR_NAME = "asdiv_pp375_wk_multiseed_cpu_v1"
...
def run() -> Dict:
    ...
    vals = []
    for sd in SEEDS:
        _ow, byw = _solve(train, test, search=True, seed=sd, add_wk=True)
        vals.append(round(byw.get(1, 0.0), 4))
    mean = sum(vals) / len(vals); std = (sum((v - mean) ** 2 for v in vals) / len(vals)) ** 0.5
    print("  PP-375+WK ASDiv-1op n=%d: mean=%.4f std=%.4f vals=%s | vs prior 0.224, PP-375-alone 0.378" % (len(vals), mean, std, vals), flush=True)
    return {"accuracy": round(mean, 4), "std": round(std, 4), "vals": vals, "searchwk_1op": round(mean, 4), "search_1op": round(mean, 4),
            "wk_lift": 0.0, "textorder_1op": 0.0, "searchwk_overall": 0.0, "n_test": len(test), "n_seeds": len(vals)}
