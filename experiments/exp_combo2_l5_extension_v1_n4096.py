"""
combo2_l5_extension_v1_n4096 -- COMBO-2 with L=5 nested NKT extension at N=4096.

Extends COMBO-2 L=4 (just shipped) to L=5 hierarchical NKT composition with p=4 DAM.

SCIENTIFIC QUESTION:
  At N=4096, does a 5-layer (L=5) p=4 DAM hierarchical composition with signed-AM
  still produce end-to-end fidelity >= 0.65 and B-repulsion rate >= 0.85?

ARCHITECTURE:
  Layer 1: M_inner=8 patterns, p=4 retrieval.
  Layer 2 (middle): M_mid=4 Hadamard-bound to inner pointers.
  Layer 3 (outer): M_outer=3 Hadamard-bound to mid pointers.
  Layer 4 (outermost_1): M_outermost1=2 Hadamard-bound to outer pointers.
  Layer 5 (outermost_2): M_outermost2=2 Hadamard-bound to outermost_1 pointers.

PRE-REGISTERED HARD-PASS (L=5 extension):
  HP1: l5_fidelity_A >= 0.65 (relaxed from L=4 0.75; 0.65 is ~0.87x of L=4 HP).
  HP2: b_repulsion_rate >= 0.85 (slight relaxation from L=4 0.90).
  HP3: parity_contamination <= 0.12 (slightly wider than L=4 0.10).
  HARD-PASS: all 3 HP in >= 4/5 seeds.

PRE-REGISTERED HARD-FAIL:
  HF1: l5_fidelity_A < 0.30.
  HF2: b_repulsion_rate < 0.35.
  MIDDLE: 2/3 HP conditions met OR HP1 in [0.45, 0.65).

Calibration: prior L=4 HP expected; L=5 adds one more binding step.
Expected fidelity loss ~0.05-0.10 per additional level. P_deflated = 0.55.

FORMULA SELF-TESTS:
  1. p=4 update rule: h = (1/N) * Xi^T * (Xi @ x)^3.
     [INPUT: N=4, xi_0=[1,1,-1,1]] [EXPECTED: h = xi_0 * (4)^3 / 4 = 16 * xi_0]
  2. L=5 Hadamard chain: 5 bindings compose without overflow.
     [INPUT: N=64] [EXPECTED: decoded cosine > 0.5]
  3. GPU VRAM check.
...
ANCHOR_NAME = "combo2_l5_extension_v1_n4096"
...
def _selftest_l5_composition():
    ...
    xi_outermost1 = xi_outer * ctx4
    ctx5 = rng.choice([-1.0, 1.0], size=n_t)
    xi_outermost2 = xi_outermost1 * ctx5
    # Decode back
    xi_out1_dec = xi_outermost2 * ctx5
    xi_out_dec = xi_out1_dec * ctx4
    xi_mid_dec = xi_out_dec * ctx3
    xi_inner_dec = xi_mid_dec * ctx2
    cos = float(np.dot(xi_inner_dec, xi_inner) / n_t)
    assert cos > 0.5, f"L=5 chain composition: cos={cos:.4f} < 0.5"
...
def _instrumentation_selftest():
    _selftest_p4_update()
    _selftest_l5_composition()
    _selftest_gpu_vram()
    print("[selftest] PASS: p4_update, l5_composition, gpu_vram all OK", flush=True)
...
def run_seed(seed: int, n_dim: int) -> Dict:
    ...
    Xi_ctx4 = rng.choice([-1.0, 1.0], size=(M_OUTERMOST1, n_dim)).astype(np.float64)
    Xi_outermost1 = Xi_ctx4 * Xi_outer[:M_OUTERMOST1]
    Xi_ctx5 = rng.choice([-1.0, 1.0], size=(M_OUTERMOST2, n_dim)).astype(np.float64)
    Xi_outermost2 = Xi_ctx5 * Xi_outermost1[:M_OUTERMOST2]

    Xi_A_sub = rng.choice([-1.0, 1.0], size=(M_B, n_dim)).astype(np.float64)
    Xi_B = rng.choice([-1.0, 1.0], size=(M_B, n_dim)).astype(np.float64)
    W_A_sub = Xi_A_sub.T @ Xi_A_sub / float(n_dim)
    W_B_mat = Xi_B.T @ Xi_B / float(n_dim)
    W_signed = W_A_sub - W_B_mat

    # L5 end-to-end fidelity
    l5_fidelities = []
    for q_idx in range(min(N_QUERIES, M_OUTERMOST2)):
        xi_out2_true = Xi_outermost2[q_idx]
        probe = xi_out2_true.copy()
        flip = rng.random(n_dim) < NOISE_FRAC
        probe[flip] *= -1.0
        # L5 retrieval
        xi_out2_ret = p4_retrieve(Xi_outermost2, probe, n=n_dim)
        # Decode -> L4
        xi_out1_ptr = xi_out2_ret * Xi_ctx5[q_idx]
        xi_out1_ret = p4_retrieve(Xi_outermost1, xi_out1_ptr, n=n_dim)
        # Decode -> L3
        xi_outer_ptr = xi_out1_ret * Xi_ctx4[q_idx % M_OUTERMOST1]
        ...
        l5_fidelities.append(fid)
    l5_fid_mean = float(np.mean(l5_fidelities)) if l5_fidelities else 0.0
    ...
    print(f"  [seed={seed} N={n_dim}] l5_fid={l5_fid_mean:.4f}(HP>={HP_L5_FIDELITY}) "
          f"b_rep={b_repulsion:.4f}(HP>={HP_B_REPULSION}) "
          f"parity_cont={parity_contamination:.4f}(HP<={HP_PARITY_CONTAMINATION}) "
          f"elapsed={elapsed:.2f}s", flush=True)

    return {
        "seed": seed, "N": n_dim, "run_mode": RUN_MODE,
        "l5_fidelity_A": float(l5_fid_mean),
        "b_repulsion_rate": float(b_repulsion),
        "parity_contamination": float(parity_contamination),
        "elapsed_s": float(elapsed),
        "hp1_pass": int(l5_fid_mean >= HP_L5_FIDELITY),
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    fid = mean_key("l5_fidelity_A")
    brep = mean_key("b_repulsion_rate")
    cont = mean_key("parity_contamination")

    summary = (f"l5_fid={fid:.4f}(HP>={HP_L5_FIDELITY} HF<{HF_L5_FIDELITY}) "
...
print(f"[config] PROT-018 N={N} n_active={N_ACTIVE} mode={RUN_MODE} L=5", flush=True)
_prot018_startup_check(N_ACTIVE if RUN_MODE == "smoke" else N)

out_dir = get_output_dir(ANCHOR_NAME)
t0_total = time.time()
run_config = {"N": N, "L": 5, "M_INNER": M_INNER, "run_mode": RUN_MODE}
...
peak_mem_gpu = torch.cuda.max_memory_allocated(0) / 1e9
print(f"\n[VERDICT] {verdict}: {verdict_msg}", flush=True)
print(f"[GPU] peak memory allocated: {peak_mem_gpu:.3f} GB", flush=True)
assert peak_mem_gpu > 0.01, f"GPU utilization check FAIL: peak_gpu={peak_mem_gpu:.3f} GB (< 100MB)"

metrics = {
    "anchor": ANCHOR_NAME, "verdict": verdict, "verdict_msg": verdict_msg,
    "n_seeds": len(all_results), "N": N, "run_mode": RUN_MODE,
    "elapsed_s": elapsed_total, "peak_gpu_gb": float(peak_mem_gpu),
    "results": all_results,
}
