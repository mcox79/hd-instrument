"""
substrate_compose_heterogeneous_routing_v2_RESCUE_FULL -- WAVE C production-scale
confirmation of v2_RESCUE smoke HARD_PASS.

Wave-C task (USER 2026-06-24): smoke HARD_PASS at N=1024 N_TRAIN=2000 V=300
showed ARM_FREQ_ROUTED_K2 BPC=6.29 with freq differential top1=0.58 (vs 0.11
high-freq vs rare). Sanity rail FAILED at smoke (baseline 5.95 vs rail 7.30 --
expected at synthetic-scale). Full-N text8 needed for production-scale ruling.

PRODUCTION CONFIG (NOT scope-reduced):
  N_DIM = 8192
  N_TRAIN = 100_000
  N_HELD = 20_000
  VOCAB_CAP = 4000
...
HARD bands (Wave-C production-scale):
  HARD_PASS_CHAIN_GRADE: best het arm BPC <= 6.80 AND beats BASELINE by >= 0.20
                          BPC AND CV <= 0.05
  HARD_PASS:             best het arm BPC <= 6.95 AND beats BASELINE by >= 0.15
  HARD_FAIL:             ALL het arms BPC >= BASELINE + 0.05 (heterogeneous hurts)

Sanity rail: ARM_BASELINE_FAIR_HARNESS within +/-0.10 of fair-harness 7.3065.
...
ANCHOR_NAME = "substrate_compose_heterogeneous_routing_v2_RESCUE_FULL"
...
SANITY_RAIL_TOLERANCE = 0.10  # Wave-C: per spec +/-0.10 BPC of fair-harness rail

# Wave-C HARD bands (production-scale; updated from rescue)
HARD_PASS_CAP_BROKEN_BPC = 6.95          # HARD_PASS floor
CHAIN_GRADE_BONUS_BPC = 6.80             # HARD_PASS_CHAIN_GRADE floor
MIDDLE_BAND_LOWER = 6.95
MIDDLE_BAND_UPPER = 7.05
HARD_FAIL_DECISIVE_FLOOR = 7.30
CV_MAX = 0.05

# Wave-C discriminator gaps vs BASELINE
BASELINE_GAP_CHAIN_GRADE = 0.20  # best het must beat BASELINE by >= 0.20 BPC for chain-grade
BASELINE_GAP_HARD_PASS = 0.15    # >= 0.15 BPC for HARD_PASS
HARD_FAIL_HURT_MARGIN = 0.05     # if ALL het arms >= BASELINE + 0.05 -> HARD_FAIL
...
def compute_verdict(units: List[Dict]) -> tuple:
    ...
    detail = {
        ...
        "honest_scope": (
            ...
            "vs fair_harness Hebbian baseline at PRODUCTION scope "
            "(N_DIM=8192, N_TRAIN=100k text8, V=4000, word2vec sparse-bipolar f=0.05, 3 seeds). "
            "Wave-C HARD_PASS_CHAIN_GRADE = best het BPC <= 6.80 AND beats BASELINE by "
            ">= 0.20 BPC. Smoke (v2_RESCUE @ N=1024 V=300) showed ARM_FREQ_ROUTED_K2 "
            "BPC=6.29 with freq differential top1=0.58 vs 0.11. WHAT_THIS_DOES_NOT_SHOW: "
            "doesn't test K>2 routing variants; modern-Hopfield cleanup not stacked here."
        ),
        "cites": [
            "preregs/2026-06-24_substrate_compose_heterogeneous_routing_v2_RESCUE_FULL.md",
            "experiments/exp_substrate_compose_heterogeneous_routing_v2_RESCUE.py (smoke landed HARD_PASS)",
            "data/exp_substrate_compose_heterogeneous_routing_v2_RESCUE_smoke/metrics.json (smoke evidence)",
            "data/exp_fair_harness_substrate_as_lm_v1/metrics.json (sanity rail 7.3065)",
        ],
    }
    ...
    if (math.isfinite(best_het_bpc) and best_het_bpc <= CHAIN_GRADE_BONUS_BPC and
            math.isfinite(best_het_gap) and best_het_gap >= BASELINE_GAP_CHAIN_GRADE):
        detail["verdict_tier"] = "HARD_PASS_CHAIN_GRADE_BONUS"
        return ("HARD_PASS",
                "HARD_PASS_CHAIN_GRADE_BONUS: best_het=%s BPC=%.4f <= %.2f AND beats "
                "BASELINE by %.4f >= %.2f BPC. Heterogeneous routing decisively refutes "
                "cf-RPE cap at production scale. %s" % (
                    best_het_name, best_het_bpc, CHAIN_GRADE_BONUS_BPC,
                    best_het_gap, BASELINE_GAP_CHAIN_GRADE, arm_summary),
                detail)
...
summary_str = (
    "%s | arms=%d seeds=%d N_DIM=%d N_TRAIN=%d encoder=word2vec_sparse_bipolar het_routing_v2_RESCUE_FULL" % (
