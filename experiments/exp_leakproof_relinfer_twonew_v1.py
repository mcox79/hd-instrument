"""LEAK-PROOF TWO-NEW-CONCEPT relational inference (the HARDER bar; envelope axis 1b of the 29587 win).

BASE (leakproof_relational_inference_heldout_v1, banked 29587) ranked a held-out concept's true partner
among negatives where the PARTNER was a TRAIN concept (one new endpoint). THIS cell makes BOTH endpoints
held-out-NEW: the predict target is ANOTHER held-out concept, so NEITHER endpoint was seen in training and
the predicted edge connects two brand-new concepts. Harder + more meaningful generalization.

SPLIT (leak-proof by CONSTRUCTION -- partner-type disjoint):
  For each held-out concept h:
    CONTEXT = h's edges to TRAIN partners  -> build h's rep (INPUT). leak-proof: train-side only.
    PREDICT = h's edges to HELD-OUT partners -> eval targets (two-new). never trained, never context.
  CONTEXT partners (train) INTERSECT PREDICT partners (held-out) = EMPTY by construction (0/N witness).
  Each held-out rep is built ONLY from its TRAIN context; the trained encoder never saw held-held edges.

TASK: given held-out h's rep (train-context only), rank its true HELD-OUT partner h2 among DEGREE-MATCHED
  HELD-OUT non-partner negatives (matched on predict-side degree = # held-out partners, so POPULARITY ~0.5).
  Gallery = held-out concepts. Both h and h2 reps come only from train-context -> genuine two-new inference.

ARMS (all kept): LEARNED (fusion) vs RAW_GROUNDING (homophily ceiling) / RANDOM_INIT / STRUCT_2HOP
  (shared-TRAIN-neighbour friend-of-friend, the non-learned structural baseline) / POPULARITY
  (predict-side degree; degree-matched => ~0.5 validity control) / COLLAPSE_SHUFFLE (row-permuted => ~0.5).
  HARD_PASS = LEARNED - RAW_GROUNDING >= 0.03 (per-seed min>0) with validity (collapse~0.5, pop~0.5, power).
  If the win EVAPORATES here => the base capability needed a KNOWN (train) partner = the honest scope bound.

Everything else REUSED from the confirmed base (fusion self-teacher, geometry+VICReg+relpred+EMA,
deterministic sha256 split, degree-matched negatives, tie-corrected AUC). CPU-only, ASCII-only, teacher-free.
...
ANCHOR_NAME = "leakproof_relinfer_twonew_v1"
...
def build_leakproof_split(data, cfg):
    """TWO-NEW split: held-out concept's TRAIN-partner edges = CONTEXT (rep input); its HELD-OUT-partner
    edges = PREDICT (two-new eval targets). Partner-type disjoint => leak-proof by construction (0/N).
    predict_deg[h] = # held-out partners (degree-match + popularity key)."""
    ids = data["ids"]
    K = data["K"]
    R = data["R"]
    ...
    # partition each concept's neighbours by PARTNER TYPE
    train_partner = [dict() for _ in range(K)]    # concept -> {train_partner_idx: set(rels)}  (CONTEXT)
    held_partner = [set() for _ in range(K)]      # concept -> {held_partner_idx}              (PREDICT)
    for (a, b), rels in data["pair_rels"].items():
        if b in train_set:
            train_partner[a][b] = set(rels)
        elif b in held_set:
            held_partner[a].add(b)
        if a in train_set:
            train_partner[b][a] = set(rels)
        elif a in held_set:
            held_partner[b].add(a)
    ...
    ctx_nei_by_rel = [dict() for _ in range(K)]   # rep INPUT context (per-rel TRAIN-neighbour sets)
    train_neigh = [set() for _ in range(K)]       # relpred positives (TRAIN anchors only)
    predict_nei = [set() for _ in range(K)]       # held-out PREDICT targets = HELD-OUT partners (two-new)
    context_nei = [set() for _ in range(K)]       # held-out CONTEXT = TRAIN partners (rep input + struct src)
    train_deg = np.zeros(K, dtype=np.float64)     # train-train degree (landmarks + geometry)
    predict_deg = np.zeros(K, dtype=np.float64)   # held-out predict-side degree (# held partners) for ALL held

    # predict-side degree for EVERY held-out concept (candidates too, not only queries)
    for h in held_idx.tolist():
        predict_deg[h] = float(len(held_partner[h]))

    # TRAIN concepts: context = train partners; relpred positives; train-train degree
    for i in train_idx.tolist():
        for j, rels in train_partner[i].items():
            for r in rels:
                ctx_nei_by_rel[i].setdefault(rel_slot(r), set()).add(j)
            train_neigh[i].add(j)
            train_deg[i] += 1

    # HELD-OUT concepts: CONTEXT = train partners (rep input); PREDICT = held partners (two-new targets)
    overlap_total = 0
    predict_targets_total = 0
    n_eval_queries = 0
    held_ctx_sizes = []
    for h in held_idx.tolist():
        tp = train_partner[h]                      # CONTEXT partners (train)
        hp = held_partner[h]                       # PREDICT partners (held-out)
        if len(tp) < 1 or len(hp) < 1:             # need >=1 train context AND >=1 held-out predict target
            continue
        ctx_set = set(tp.keys())
        pred_set = set(hp)
        overlap_total += len(ctx_set & pred_set)   # MUST be 0 (train vs held partner sets disjoint)
    ...
    if overlap_total != 0:
        raise RuntimeError("LEAK: train-context INTERSECT held-predict = %d (must be 0)" % overlap_total)
    if n_eval_queries < 5:
        raise RuntimeError("too few two-new queries (>=1 train ctx AND >=1 held partner): %d" % n_eval_queries)
    ...
    split_meta = dict(
        ...
        mean_predict_deg=float(np.mean([predict_deg[h] for h in held_idx.tolist()])),
        n_rel_slots=R, mode="two_new_concept",
    )
    return dict(own_feat=own_feat, is_train=is_train, K=K, R=R,
                train_idx=train_idx, held_idx=held_idx, train_deg=train_deg, predict_deg=predict_deg,
...
def build_eval_context(split, _n_bins_unused):
    """TWO-NEW gallery = HELD-OUT concepts. Degree = predict-side degree (# held partners) for degree-matched
    negatives + popularity. train_nei_of_pos[p] = gallery concept p's TRAIN-partner set (STRUCT_2HOP src)."""
    held_idx = split["held_idx"].astype(np.int64)
    G = held_idx.shape[0]
    pos_of = {int(t): p for p, t in enumerate(held_idx.tolist())}
    predict_deg_gal = split["predict_deg"][held_idx]
    deg_int = np.rint(predict_deg_gal).astype(np.int64)
    deg_to_pos = {}
    for p in range(G):
    ...
    # each gallery (held-out) concept's TRAIN-partner set (raw train-concept indices; for shared-train 2hop)
    train_nei_of_pos = [split["context_nei"][int(t)] for t in held_idx.tolist()]
    return dict(held_idx=held_idx, G=G, pos_of=pos_of, predict_deg_gal=predict_deg_gal,
                deg_int=deg_int, deg_to_pos=deg_to_pos, max_deg=max_deg, train_nei_of_pos=train_nei_of_pos)


def _query_positives_negatives(h, split, ev_ctx, rng):
    """TWO-NEW: positives = h's true HELD-OUT partners in the held gallery; negatives = degree-matched
    (predict-side degree) HELD-OUT non-partners, excluding h itself + ALL of h's true held partners.
    Returns (pos_positions, neg_positions, exclude_set). None if unusable."""
    pos_of = ev_ctx["pos_of"]
    deg_int = ev_ctx["deg_int"]
    deg_to_pos = ev_ctx["deg_to_pos"]
    max_deg = ev_ctx["max_deg"]
    all_partner_pos = set(pos_of[j] for j in split["predict_nei"][h] if j in pos_of)
    pred = sorted(all_partner_pos)
    if len(pred) == 0:
        return None
    if len(pred) > MAX_PRED_PER_QUERY:
        pred = pred[:MAX_PRED_PER_QUERY]            # deterministic (sorted) subset -> bounded compute
    h_pos = pos_of.get(h, -1)
    exclude = set(all_partner_pos)                  # every true held-out partner (not just the capped subset)
    if h_pos >= 0:
        exclude.add(h_pos)                          # h is itself in the gallery; never its own negative
    neg = []
    used_neg = set()
    for p in pred:
        dp = int(deg_int[p])
        picked = -1
        for tol in range(0, max_deg + 1):           # exact predict-degree first, then widen minimally
            cand_arrs = []
    ...
    if len(neg) < 1:
        return None
    return np.asarray(pred, dtype=np.int64), np.asarray(neg, dtype=np.int64), exclude


def eval_relational_inference(score_matrix, split, ev_ctx, struct_2hop=False, popularity=False):
    """TWO-NEW relational-inference AUC. For each held-out query rank its true HELD-OUT partner(s) among
    degree-matched held-out negatives. Gallery = held-out concepts. score_matrix: codes[K,d] for cosine
    arms; ignored for struct/popularity. STRUCT_2HOP = shared TRAIN-neighbour (friend-of-friend via train)."""
    held_idx = ev_ctx["held_idx"]
    predict_deg_gal = ev_ctx["predict_deg_gal"]
    train_nei_of_pos = ev_ctx["train_nei_of_pos"]
    Zg = None
    if not (struct_2hop or popularity):
        Zg = score_matrix[held_idx]               # gallery codes [G,d] over HELD-OUT concepts
    rng = np.random.default_rng(EVAL_SEED)
    aucs = []
    neg_in_exclude = 0                            # must remain 0 (negatives never a true held partner / self)
    for h in split["held_idx"].tolist():
        qpn = _query_positives_negatives(h, split, ev_ctx, rng)
        if qpn is None:
            continue
        pos_arr, neg_arr, exclude = qpn
        neg_in_exclude += sum(1 for n in neg_arr.tolist() if n in exclude)
        sel = np.concatenate([pos_arr, neg_arr])
        if popularity:
            sc = predict_deg_gal[sel]             # predict-side degree; degree-matched -> ~0.5
        elif struct_2hop:
            # shared TRAIN-neighbours between query h and each held-out candidate (2-hop via train)
            h_train_nei = split["context_nei"][h]
            sc = np.zeros(sel.shape[0], dtype=np.float64)
            for k, c in enumerate(sel.tolist()):
                sc[k] = float(len(train_nei_of_pos[c] & h_train_nei))
        else:
...
def leakproof_split_selftest():
    """REAL-CODE-PATH: build a tiny synthetic grounded subgraph, run the TWO-NEW build_leakproof_split, assert
    (a) predict targets are HELD-OUT concepts (two-new partner), (b) context partners are TRAIN concepts,
    (c) context INTERSECT predict == EMPTY (partner-type disjoint), (d) context rep never includes a predict
    target, (e) held-out never a train relpred anchor, (f) predict_deg matches # held partners."""
    K = 200
    rng = np.random.default_rng(3)
    ids = ["c%03d" % i for i in range(K)]
    vals = rng.standard_normal((K, N_VALUE_DIMS))
    gpres = np.ones((K, N_GROUPS), dtype=np.float64)
    # dense random graph so held-out concepts have BOTH train context AND held-out partners
    pair_rels = {}
    for _ in range(5000):
    ...
    held_set = set(split["held_idx"].tolist())
    ok = True
    checks = dict(overlap=split["split_meta"]["no_overlap_witness_overlap"], viols=0)
    for h in split["held_idx"].tolist():
        pred = split["predict_nei"][h]
        ctxs = split["context_nei"][h]
        if pred & ctxs:                              # partner-type disjoint
            ok = False; checks["viols"] += 1
        if pred and any(j not in held_set for j in pred):    # predict target must be HELD-OUT (two-new)
            ok = False; checks["viols"] += 1
        if ctxs and any(j not in train_set for j in ctxs):   # context partner must be TRAIN
            ok = False; checks["viols"] += 1
        ctx_flat = set()
        for _s, neigh in split["ctx_nei_by_rel"][h].items():
            ctx_flat |= neigh
        if ctx_flat & pred:                          # context rep never contains a predict target
            ok = False; checks["viols"] += 1
        if pred and int(round(split["predict_deg"][h])) < len(pred):  # predict_deg >= shown predict targets
            ok = False; checks["viols"] += 1
    for h in split["held_idx"].tolist():
        if split["train_neigh"][h]:                  # held-out never a train relpred anchor
            ok = False; checks["viols"] += 1
    checks["n_eval_queries"] = split["split_meta"]["n_eval_queries"]
    checks["mean_predict_deg"] = split["split_meta"]["mean_predict_deg"]
