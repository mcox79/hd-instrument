"""Continual editing at extreme over-capacity M=32N - extends zh/zi envelope.

zh held at M=2N continual, zi held at M=4N continual. zq pushes to M=32N
sequential editing to find substrate ceiling under combined stress.

Pre-reg: preregs/2026-05-21_wave14_continual_32N_500edits.md
"""
...
def compute_verdict(summary):
    ...
    if "kerdock" not in arms:
        return ("CONTINUAL_32N_INCONCLUSIVE", "Missing.")
    kerdock = arms["kerdock"]
    if not kerdock.get("per_edit_trajectory"):
        return ("CONTINUAL_32N_INCONCLUSIVE", "Missing traj.")
    ...
    # Kerdock-only: correlated arm omitted (OOM at M=32N); kerdock is the load-bearing arm.
    if k_ok:
        return ("CONTINUAL_32N_KERDOCK_HOLDS",
                f"At M=32N, Kerdock holds 100 sequential edits: min_edited="
                f"{kerdock['min_edited_acc']:.3f}, min_kept={kerdock['min_kept_acc']:.3f}. "
                f"Continual editing at 8N over-capacity works.")
    return (f"CONTINUAL_32N_KERDOCK_FAILS_AT_{k_fail}",
            f"Kerdock fails at M=32N continual editing at edit {k_fail}. "
...
def self_test_verdict():
    ...
    cases = [
        ({"by_arm": {"kerdock": mk(0.98)}}, "CONTINUAL_32N_KERDOCK_HOLDS"),
        ({"by_arm": {"kerdock": mk(0.5, fail_e=30)}},
         "CONTINUAL_32N_KERDOCK_FAILS_AT_30"),
        ({}, "CONTINUAL_32N_INCONCLUSIVE"),
    ]
    for s, exp in cases:
...
def run_experiment(smoke):
    ...
    config = {"mode": "smoke" if smoke else "full", "N": 1024 if smoke else 4096,
              "M_stored": 2048 if smoke else 131072,  # M=32N (max for 32-coset Kerdock)
              "n_edits": 10 if smoke else 500,  # 5x continual stress at max M
...
def run_smoke():
    out_dir = get_output_dir("wave14_continual_32N_500edits_smoke")
...
def run_main():
    out_dir = get_output_dir("wave14_continual_32N_500edits")
