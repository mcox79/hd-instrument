"""
exp_t5c_hybrid_lm_fact_gpu_v1 -- CYCLE_204 Tier-3 HYBRID: every-layer Flamingo (LM gain) + PP-225 projection head (fact recall) -- GPU.
...
ANCHOR_NAME = "t5c_hybrid_lm_fact_gpu_v1"; MODEL = "EleutherAI/pythia-160m"; ENCODER = "BAAI/bge-large-en-v1.5"
STEPS = 120 if "--smoke" in sys.argv else 6000; CKPT_EVERY = 500
RUN_MODE = ("smoke" if "--smoke" in sys.argv else os.environ.get("HDLAB_RUN_MODE", "full")).lower()
_ap = argparse.ArgumentParser(); _ap.add_argument("--smoke", action="store_true"); _ap.add_argument("--self-test", action="store_true"); _ARGS, _ = _ap.parse_known_args()
SMOKE = RUN_MODE == "smoke"
N_FACTS = 200 if SMOKE else 1500; N_LM = 16 if SMOKE else 200
...
def _selftest():
    ...
    print("[selftest] PASS: t5c-hybrid-lm-fact", flush=True)
...
def run() -> Dict:
    ...
    ntr = int(0.6 * len(facts)); ftrain, ftest = facts[:ntr], facts[ntr:]
