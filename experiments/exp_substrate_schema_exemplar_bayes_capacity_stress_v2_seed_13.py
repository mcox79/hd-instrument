"""substrate_schema_exemplar_bayes_capacity_stress_v2 sibling seed=13.

Capacity-stress sweep (v2) of schema exemplar-Bayes (vmPFC analog) over
(n_exemplars_per_class, n_classes, N) at prior_strength=1.0 fixed.
Stage 2 MM cross-seed AGG -> chain-grade-phase-characterization promotion drill.

v1 (phase_diagram) atomized MIDDLE_BAND with cliff_observable=False (no FLOOR coverage).
v2 (capacity_stress) pushes K_total upward to expose cliff regime.

CHUNKED architecture (USER 2026-06-28): one seed per sibling file.
Sibling pair: seed_7, seed_19.

ARMS: ARM_SCHEMA_BAYES / ARM_NEAREST_EXEMPLAR / ARM_UNIFORM_RANDOM (3 per phase-point)
...
PRE-REG: preregs/2026-06-28_substrate_schema_exemplar_bayes_capacity_stress_v2.md
CARDINALITY_OK_FULL: 3840 records per seed (64 pts x 3 arms x 20 queries)
CARDINALITY_OK_SMOKE: 90 records per seed (6 corners x 3 arms x 5 queries)
...
SEED = 13
ANCHOR_NAME = f"substrate_schema_exemplar_bayes_capacity_stress_v2_seed_{SEED}"
...
CONFIG_VERSION = (
    ...
    f"prior_strength=1.0,arms=[SCHEMA_BAYES,NEAREST_EXEMPLAR,UNIFORM_RANDOM],"
    f"expected_n_full=3840,expected_n_smoke=90,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v2_schema_exemplar_bayes_capacity_stress_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v2_schema_exemplar_bayes_capacity_stress_import_crash",
        }
...
def main() -> int:
    ...
    from experiments._substrate_schema_exemplar_bayes_capacity_stress_v2_core import (
    ...
    final["_hardening_marker"] = "v2_schema_exemplar_bayes_capacity_stress_chunked"
    final["backend"] = backend
    final["seed"] = SEED

    # cardinality_ok
    if SMOKE_MODE:
        expected_n = 6 * 3 * N_QUERIES_SMOKE     # 6 corners x 3 arms x 5 queries
    else:
