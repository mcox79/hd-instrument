...
ANCHOR_NAME = "encoder_drift_monitor_cpu_v1"; D = 1024; PROBE = 200; THRESH = 0.01
...
def _selftest():
    ...
    print("[selftest] PASS: encoder-drift-monitor", flush=True)
...
def run() -> Dict:
    ...
    for _ in range(N_TRIAL):
        ...
        if g.random() < 0.5:
            mag = float(g.uniform(0.05, 0.5)); gap = drift_gap(stored, mag, g); detected += int(gap > THRESH); n_drift += 1
        else:
            gap = drift_gap(stored, 0.0, g); fp += int(gap > THRESH); n_nodrift += 1   # zero drift -> should NOT flag
    det_rate = detected / max(1, n_drift); fp_rate = fp / max(1, n_nodrift)
    # also: smallest detectable drift magnitude
    mags = [0.01, 0.02, 0.05, 0.1]; curve = {}
    for m in mags:
...
def verdict(r) -> Tuple[str, str]:
    ...
    if r["detection"] >= 0.99 and r["fp"] <= 0.01:
        return ("HARD_PASS", "HARD_PASS: encoder-drift monitor flags >=99pct of drifts with <=1pct false-positive -- rank-1 silent-failure guard ready for pre-GA. " + s)
