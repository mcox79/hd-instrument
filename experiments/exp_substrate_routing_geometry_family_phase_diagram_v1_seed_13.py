"""substrate_routing_geometry_family_phase_diagram_v1 sibling seed=13.
...
the OUTER routing-family axis (partition vs softmax vs hierarchical -- WHICH
routing PRIMITIVE). This cell covers the INTERNAL geometry of the partition
primitive itself -- HOW partitions are constructed:
    random_uniform / learned_supervised / hierarchical_2_level / hash_based_LSH

Inner grid: 4 geometries * 3 M * 3 P = 36 phase points per seed (FULL).
Smoke: 4 geometries * 2 M * 2 P = 16 corner points per seed.

Inner axes:
    FULL:  M in {1M, 5M, 10M};  P in {64, 256, 1024};  N=8192
    SMOKE: M in {10k, 100k};     P in {64, 256};        N=2048

Discriminator: routing_accuracy + lookup_latency per (geometry, M, P).
Chain-grade target: any geometry achieves >=0.97 route_acc at M=10M.
...
CARDINALITY_OK_SMOKE: 16 corner points per seed

PROT-018: anchor has no _n<N> suffix (multi-M sweep cell).
...
STORAGE-FREE design (USER M=10M target): items represented by int partition-IDs
+ on-the-fly noisy cue generation. Working-set < 200 MB at full scale.
...
SEED = 13
ANCHOR_NAME = f"substrate_routing_geometry_family_phase_diagram_v1_seed_{SEED}"
...
CONFIG_VERSION = (
    ...
    f"geometries=[random_uniform,learned_supervised,hierarchical_2_level,hash_based_LSH],"
    f"M_full=[1000000,5000000,10000000],P_full=[64,256,1024],"
    f"N_full=8192,n_probe_full=4096,"
    f"M_smoke=[10000,100000],P_smoke=[64,256],"
    f"N_smoke=2048,n_probe_smoke=512,"
    f"SEED={SEED},mode={RUN_MODE},"
    f"cue_cos=0.70,"
    f"expected_n_full=36,expected_n_smoke=16,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v1_routing_geometry_family_phase_diagram_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    ...
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v1_routing_geometry_family_import_crash",
        }
...
def main() -> int:
    ...
    from experiments._substrate_routing_geometry_family_phase_diagram_v1_core import (
    ...
    final["_hardening_marker"] = "v1_routing_geometry_family_phase_diagram_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = "synthetic_substrate_4_partition_geometry_storage_free"
