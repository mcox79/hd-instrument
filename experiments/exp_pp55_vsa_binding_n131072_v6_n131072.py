"""
pp55_vsa_binding_n131072_v6_n131072 -- PP-55 N-scale: VSA binding at N=131072 (6th rung).
...
CONTEXT:
  ...
  v5_n65536: HARD_PASS (mean_cos=0.99999; 5-rung; band-lift to 0.80-0.92; v361).
  6th rung: N=131072 to confirm algebraic N-independence continues at very large N.

SCIENTIFIC QUESTION:
  Does VSA bind-unbind algebra over SKAH-M-class substrate maintain fidelity at N=131072?
  Theory: larger N -> lower interference -> fidelity improves or holds near 1.0.

OOM PRE-CHECK:
  W matrix float32: 131072^2 * 4 / 1e9 = 68.72 GB. Far exceeds RAM -- no explicit W.
  CHUNKED RETRIEVAL: W@probe computed as sum_k Xi_bound[k] * (Xi_bound[k] . probe) / N.
  Memory: Xi_bound shape (M=6553, N=131072) float32 = 6553 * 131072 * 4 / 1e9 = 3.44 GB.
  Peak RSS ~3.5 GB. Remote CPU has 16+ GB RAM. Fits.
  Retrieval O(M*N) per step vs O(N^2) for explicit W.

FORMULA SELF-TESTS (PROT-022):
  1. Hadamard binding self-inverse: a*b*b == a exactly for +-1 vectors.
     [INPUT: a=[1,-1,1,-1], b=[1,1,-1,-1]] [EXPECTED: (a*b)*b == a]
  2. Chunked Hopfield retrieval at N=512 alpha=0.05: cos > 0.50.
     [INPUT: N=512, M=26 (alpha~0.05)] [EXPECTED: mean_cos > 0.50]
  3. M_pairs at N=131072: int(0.05 * 131072) = 6553. [EXPECTED: M_pairs = 6553]
  4. Xi_bound memory: 6553 * 131072 * 4 / 1e9 < 4.0 GB. [EXPECTED: < 4.0 GB]

PRE-REGISTERED BANDS (PP-55 N=131072 6th-rung extension; prior N=65536 HARD_PASS):
  HARD-PASS: cos(retrieved, xi_A) >= 0.85 in >= 4/5 seeds at alpha=0.05 N=131072
             => PP-55 band-lift eligible 0.80-0.92 -> 0.82-0.94 (6-rung cross-N)
  MIDDLE: cos in [0.60, 0.85)
  HARD-FAIL: cos < 0.50 (VSA bind-unbind fails at N=131072)

PROT-018: anchor has _n131072; N MUST = 131072.
PROT-021: seed checkpoints keyed with run_mode.
QUEUE: remote_cpu_queue (pure numpy; Xi_bound (M x N) only; chunked Hopfield; ~3600s FULL wall).
TIMEOUT ESTIMATE: N=65536 elapsed=177.8s (v361, 5-seed, chunked).
  N=131072 chunked: M=6553 vs M=3276 (2x); N=131072 vs N=65536 (2x).
  Chunked step: O(M*N). Scales as 4x per doubling.
  ceil(1.5 * 177.8 * 4.0 * 1.0) = ceil(1066) -> 1200s.
  With 3x walk-back margin (chunked is memory-bound; first time at this N): timeout=3600s.
"""
import sys
...
ANCHOR_NAME = "pp55_vsa_binding_n131072_v6_n131072"
...
_M_PAIRS_FULL = int(ALPHA * N)  # 6553
assert _M_PAIRS_FULL >= 1, f"M_pairs={_M_PAIRS_FULL} must be >= 1"
assert _M_PAIRS_FULL == 6553, f"M_pairs={_M_PAIRS_FULL} expected 6553"

# OOM check: Xi_bound memory at FULL N
_XI_BOUND_MEM_GB = _M_PAIRS_FULL * N * 4 / 1e9  # float32
assert _XI_BOUND_MEM_GB < 4.0, f"Xi_bound memory {_XI_BOUND_MEM_GB:.3f} GB >= 4.0 GB"
...
def chunked_hopfield_retrieve(Xi_bound: np.ndarray, probe: np.ndarray,
    ...
    for _ in range(n_steps):
        activations = Xi_bound @ state  # (M,)
        h = Xi_bound.T @ activations / n_dim  # (N,)
...
def _selftest_m_pairs():
    """M_pairs at N=131072: int(0.05 * 131072) = 6553."""
    assert _M_PAIRS_FULL == 6553, f"M_pairs={_M_PAIRS_FULL} expected 6553"


def _selftest_mem_check():
    """Xi_bound memory < 4.0 GB."""
    assert _XI_BOUND_MEM_GB < 4.0, f"Xi_bound mem={_XI_BOUND_MEM_GB:.3f} GB >= 4.0 GB"
...
def main():
    ...
    if min_cos_all < HF_COS_MAX:
        ...
        verdict_msg = (f"HARD_FAIL: min_cos={min_cos_all:.5f} < {HF_COS_MAX}; "
                       f"VSA chunked-Hopfield bind-unbind fails at N=131072")
        ...
        verdict_msg = (f"HARD_PASS: seeds_hp={seeds_hp}/5 cos>={HP_COS_MIN}; "
                       ...
                       f"n_seeds={len(SEEDS)}; PP-55 6th-rung cross-N band-lift gate passed. "
