...
ANCHOR_NAME = "active_inference_dpefe_h2_cpu_v1"
...
DIST_THRESH = 0.5
def _selftest():
    print("[selftest] PASS: active-inference-dpefe-h2", flush=True)
def _fpe_book(g):
...
def _episode(omega, g, mode):
    ...
    for _t in range(STEPS):
        ...
        def step_score(xp):
            pv = _pos(xp, omega)
            return _sim(pv, comfortv) + ALPHA * _sim(pv, goalv)
        scores = []
        for d in DELTAS:
            xp = min(1.0, max(0.0, cur + d))
            if mode == "base":
                sc = _sim(_pos(xp, omega), comfortv)
            elif mode == "dpefe":
                # H=2 Bellman lookahead: score = immediate + best achievable one step further
                best2 = max(step_score(min(1.0, max(0.0, xp + d2))) for d2 in DELTAS)
                sc = step_score(xp) + best2
            else:
                sc = _sim(_pos(xp, omega), comfortv) + ALPHA * _sim(_pos(xp, omega), goalv)
            scores.append(sc)
        if mode == "dpefe":
            goal_dist = abs(cur - GOAL)
            gamma = GAMMA0 * min(1.0, goal_dist / DIST_THRESH)   # goal-distance gamma gate: explore less as goal nears
            ai = int(g.integers(0, len(DELTAS))) if g.random() < gamma * 4 else int(np.argmax(scores))
        elif mode == "e1e2" and boredom > 0.5 and g.random() < GAMMA0 * (1 + boredom) * 4:
            ai = int(g.integers(0, len(DELTAS)))
        else:
...
def run() -> Dict:
    ...
    for sd in seeds:
        ...
        be = float(np.mean([_episode(omega, g, "base") for _ in range(EP)]))
        de = [_episode(omega, g, "dpefe") for _ in range(EP)]; dm = float(np.mean(de))
        eds.append((be - dm) / be if be > 1e-9 else 0.0); grs.append(float(np.mean([e < 0.1 for e in de])))
    error_drop = float(np.mean(eds)); goal_reach = float(np.mean(grs)); gr_std = float(np.std(grs))
    print("  ACTIVE-INFERENCE-DPEFE-H2 n=%d: error_drop=%.1f%% | goal_reach=%.2f+/-%.2f (per-seed=%s)" %
...
def verdict(r) -> Tuple[str, str]:
    ...
    if ed > 0.30 and gr > 0.70:
        return ("HARD_PASS", "HARD_PASS: active-inference DPEFE H=2 rescue works -- horizon-2 free-energy lookahead sees past the comfort basin + goal-distance gamma gate settles the agent at goal; error_drop>30%% AND goal_reach>0.70 where epistemic-only stalls. Closes the E1+E2 goal_reach gap. " + s)
