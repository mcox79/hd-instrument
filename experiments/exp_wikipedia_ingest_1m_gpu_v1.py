"""
exp_wikipedia_ingest_1m_gpu_v1 -- E1 / v1 benchmark suite: substrate ingest + retrieval over 10k real Wikipedia -- GPU.
...
ANCHOR_NAME = "wikipedia_ingest_1m_gpu_v1"; BI = "BAAI/bge-large-en-v1.5"
Q_INSTR = "Represent this sentence for searching relevant passages: "
DS = REPO / "data" / "datasets" / "wikipedia_1m.jsonl"
RUN_MODE = ("smoke" if "--smoke" in sys.argv else os.environ.get("HDLAB_RUN_MODE", "full")).lower()
_ap = argparse.ArgumentParser(); _ap.add_argument("--smoke", action="store_true"); _ap.add_argument("--self-test", action="store_true"); _ARGS, _ = _ap.parse_known_args()
SMOKE = RUN_MODE == "smoke"
N = 5000 if SMOKE else 1000000
...
def _selftest():
    ...
    print("[selftest] PASS: wikipedia-ingest-1m", flush=True)
...
def run() -> Dict:
    data = load(N)
    if not data:
        print("[FATAL] no wikipedia_1m.jsonl", flush=True); return {"n": 0, "r5": 0.0}
