"""
q_b1_chain_depth_60_v1_n8192 -- Q-B1: heteroassoc chain at depth-60 at N=8192.

Ceiling chase: depth-55 completed (verdict pending); depth-60 ships in parallel to
  map the degradation slope. If d55 HARD_PASS, depth-60 extends; if MIDDLE, depth-60
  diagnoses the degradation onset.
  GPU IMPLEMENTATION: H matrix (N x N float32 at N=8192): 268 MB. Safe on 8 GB GPU.
  Snapshots at depths [1, 3, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60].

PRE-REGISTERED BANDS (extends d55 spec):
  HARD-PASS: depth-5 >= 0.95 AND depth-10 >= 0.88 AND depth-20 >= 0.70
             AND depth-30 >= 0.55 AND depth-45 >= 0.40 AND depth-60 >= 0.20.
  HARD-FAIL: depth-5 < 0.80 OR depth-10 < 0.65 OR depth-20 < 0.40 OR depth-60 < 0.08.
  MIDDLE: depth-60 in [0.12, 0.20) while earlier depths meet HP.
  Calibration: depth-55 HP set at 0.25; depth-60 further conserved at 0.20.
    Degradation model: cos(d) ~ exp(-d * lambda) at capacity alpha=M_total/N.
    With ~150 chains+bg at N=8192, lambda ~ 0.004 => d60 ~ 0.78 * d55 estimate.
    0.78 * 0.25 = 0.195; rounded down to 0.20 for HP; 0.08 for HF (>3x below).

FORMULA SELF-TESTS:
  1. depth-60 chain retrieval non-NaN at N=128.
     [INPUT: N=128, CHAIN_DEPTH=60] [EXPECTED: cos non-NaN in [-1,1]]
...
def _instrumentation_selftest():
    ...
    assert not (cos != cos), f"depth-60 retrieval is NaN at N={N_t}"
    ...
    del H
    print(f"[selftest] PASS: depth-60 chain non-NaN N={N_t}, self-cosine=1.0, "
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    summary = (f"d5={d5:.4f}(HP>={HP_D5} HF<{HF_D5}) "
               ...
               f"d60={d60:.4f}(HP>={HP_D60} HF<{HF_D60}) "
               f"n_seeds={len(results)}")

    if d5 < HF_D5 or d10 < HF_D10 or d20 < HF_D20 or d60 < HF_D60:
    ...
    if hp5 and hp10 and hp20 and hp30 and hp45 and hp60:
        return ("HARD_PASS", f"HARD_PASS: all depth thresholds met at N=8192 depth-60. {summary}")
    n_hp = sum([hp5, hp10, hp20, hp30, hp45, hp60])
