"""
substrate_sq6_graph_adjacency_v1 -- graph adjacency binding (GraphHD-style) -- remote CPU.
...
ANCHOR_NAME = "substrate_sq6_graph_adjacency_v1"
...
def accuracy_at_E(n, E, g):
    ...
    pos = np.array([float((G * (nodes[u] * nodes[v])).sum() / n) for (u, v) in edges])
    neg = []
    while len(neg) < E:
        u, v = int(g.integers(0, V_NODES)), int(g.integers(0, V_NODES))
        if u == v or (min(u, v), max(u, v)) in seen:
            continue
        neg.append(float((G * (nodes[u] * nodes[v])).sum() / n))
    neg = np.array(neg)
    # best-threshold balanced accuracy
    thr = 0.5 * (pos.mean() + neg.mean())
    return float(0.5 * (np.mean(pos > thr) + np.mean(neg <= thr)))
