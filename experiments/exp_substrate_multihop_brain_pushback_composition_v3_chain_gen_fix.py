"""substrate_multihop_brain_pushback_composition_v3_chain_gen_fix -- chain-gen feasibility rerun.

V3 DELTA over v2_hardened (commit 442708b9):
  Mechanism IDENTICAL to v1/v2 (5 arms: BASELINE, R1, R2, R3, COMBINED).
  Hardening IDENTICAL to v2 (L1 early-write + L2 per-arm-progress + L3 outer
  try/except + L4 import-crash sentinel).

  ONLY operational change: chain-gen feasibility fix.
    - V_C: 200 -> 1000 (5x vocab; chain disallow filter doesn't exhaust)
    - max_depth: 8 -> 5 (less variable consumption per chain)
    - HOP_DEPTHS_FULL: [2, 3, 5, 8] -> [2, 3, 5] (drop depth-8; max_depth=5)

V2 OUTCOME (Cycle 1 rerun): module-import crash CAPTURED by L4 sentinel:
    "RuntimeError: BLOCKING make_deep_chains: only 0/200 generated for V=200
     disallow|=200 max_depth=8"
  The cell's selftest passed (small V=60 max_depth=3), but the real run config
  (V=200 max_depth=8 n_chains=200) is infeasible: the disallow filter accumulates
  to V (200), and every candidate start is blocked.

ROOT CAUSE: make_deep_chains uses `disallow_s` to skip start nodes already used
in training chains. After ~200 training chains, every node in V=200 is "used",
so test-chain generation accepts 0 starts. v2_hardened's disallow filter blew
up to V; v1 and earlier multi-hop combined cells (200/500, 180/200) hit the
same bug at smaller margins.

FIX RATIONALE (drill bundle option 1+2; cheap + safe):
  1. V_C 200 -> 1000: 5x more vocab; even with 200 train + 200 test starts
     blocked, ~600 candidates remain available. Disallow filter no longer
     exhausts.
  2. max_depth 8 -> 5: chains consume max_depth+1 unique nodes each. At
     depth=8, each chain consumes 9 unique nodes; depth=5 consumes 6.
     Combined with the 5x vocab, the make_deep_chains `tries < n_chains*200`
     budget is more than sufficient.

ALL FIVE ARM FUNCTIONS, BAND THRESHOLDS, VERDICT LOGIC, HARDENING, AND
META_RULE_J DISCIPLINE ARE IDENTICAL TO V2. Search this file for marker
"V2_MECHANISM_IDENTICAL" to confirm.
...
prematurely declared. USER push-back 2026-06-27.
...
PRE-REG BANDS (V2_MECHANISM_IDENTICAL; HARD-LOCKED at module init; PROSPECTIVE):
...
CONFIG (V3 chain-gen fix):
  Full: N=8192, V_C=1000, n_chains_train=200, n_chains_test=200, depths=[2, 3, 5], seeds=[7, 17, 23]
  Smoke: N=8192, V_C=1000, n_chains=50, depths=[2, 5], seed=[7]
  max_depth (per chain) = max(HOP_DEPTHS) = 5

META_RULE_J: no silent except blocks; record+halt or re-raise.

Author: exp_dev 2026-06-27 (v3 chain-gen-feasibility fix; bundle of v2 hardening + V_C 5x + max_depth -3).
...
ANCHOR_NAME = "substrate_multihop_brain_pushback_composition_v3_chain_gen_fix"
_LLM_CALL_COUNTER = [0]
...
# PROSPECTIVE HARD bands (LOCKED at module init; V2_MECHANISM_IDENTICAL)
...
# V3 CHAIN-GEN FIX: V_C 200->1000, max_depth 8->5 (depths=[2,3,5])
if RUN_MODE == "smoke":
    N_DIM = 8192
    V_CONCEPTS = 1000
    N_PREDICATES = 10
...
# R1 NREM replay tuning (V2_MECHANISM_IDENTICAL)
R1_REPLAY_TOP_K = 30
R1_REPLAY_COHORTS = 5
R1_REPLAY_MIN_AMPLITUDE = 0.55

# R3 bidirectional tuning (V2_MECHANISM_IDENTICAL)
R3_MEET_COSINE_TAU = 0.30

CONFIG_VERSION = (
    "brainPushbackComp-v3-chainGenFix: N=%d V_C=%d V_P=%d N_chains_train=%d N_chains_test=%d "
    ...
    "chain_gen_fix=V_C200to1000_maxdepth8to5"
) % (
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        metrics = {
            ...
            "_hardening_marker": "v3_chain_gen_fix",
        }
        if extra:
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    ...
    try:
        ...
        sentinel = {
            ...
            "_hardening_marker": "v3_chain_gen_fix_import_crash",
        }
        (out_dir / "metrics.json").write_text(
...
# ---------------------------- primitives (V2_MECHANISM_IDENTICAL) ----------------------------
...
def make_deep_chains(n_chains: int, V: int, P: int, max_depth: int,
    ...
    V3 CHAIN-GEN FIX context: at V=200/max_depth=8/n_chains=200, disallow
    accumulates to V and feasibility -> 0. V3 raises V_C to 1000 + drops
    max_depth to 5, leaving ~600 free candidates throughout generation.
    """
...
# ---------------------------- ARM_BASELINE (V2_MECHANISM_IDENTICAL) ----------------------------
...
# ---------------------------- ARM_R1: REPLAY into W_C (V2_MECHANISM_IDENTICAL) ----------------------------
...
# ---------------------------- ARM_R2: PFC scratchpad (V2_MECHANISM_IDENTICAL) ----------------------------
...
# ---------------------------- ARM_R3: BIDIRECTIONAL (V2_MECHANISM_IDENTICAL) ----------------------------
...
# ---------------------------- ARM_COMBINED (V2_MECHANISM_IDENTICAL) ----------------------------
...
# ---------------------------- selftest (V3 strengthens with full-config feasibility check) ----------------------------
...
def _selftest() -> None:
    ...
    # V3 ADDITION: feasibility check at FULL config. Make sure V_C=1000 max_depth=5
    # actually permits n_chains_train + n_chains_test back-to-back.
    g2 = np.random.default_rng(1)
    P_full = N_PREDICATES
    V_full = V_CONCEPTS
    # Use a SMALL E for the feasibility check (just need it for chain gen);
    # don't allocate the full N_DIM matrices in selftest.
    ...
    try:
        ...
        raise AssertionError(
            "V3 CHAIN-GEN FEASIBILITY FAIL at full config V=%d P=%d "
    ...
    assert len(train_chains_fc) == n_full_train, \
        "V3 feasibility FAIL: train chains %d != %d" % (len(train_chains_fc), n_full_train)
    assert len(test_chains_fc) == n_full_test, \
        "V3 feasibility FAIL: test chains %d != %d" % (len(test_chains_fc), n_full_test)

    print(
        "[selftest] PASS baseline=%.3f r1=%.3f r2=%.3f r3=%.3f combined=%.3f "
        "(meet_rate=%.3f shortcut_hits=%d/%d) "
        "v3_chain_gen_feasibility=PASS (V=%d max_depth=%d train=%d test=%d)"
...
# ---------------------------- verdict (V2_MECHANISM_IDENTICAL) ----------------------------
...
def _main_inner() -> int:
    ...
    metrics = {
        ...
        "_hardening_marker": "v3_chain_gen_fix",
        "DESIGN_NOTE": (
            "V3 CHAIN-GEN FIX over v2_hardened (commit 442708b9): mechanism + hardening "
            "IDENTICAL to v2; only V_C 200->1000 + max_depth 8->5 to make chain "
            "generation feasible. V2 crashed at module init with 'BLOCKING "
            "make_deep_chains: only 0/200 generated for V=200 disallow|=200 "
            "max_depth=8' - disallow filter exhausted vocab. V3 has 5x vocab "
            "headroom + shorter chains. Tests whether META_BARRIER_1 (substrate "
            "multi-hop permanent 2-hop) was prematurely declared. R1=NREM "
            "replay-as-OPERATOR with SEPARATE W_C. R2=PFC scratchpad in SEPARATE "
            "W_PFC. R3=bidirectional meet-in-middle via HRR-involutive unbinding. "
