"""R10 best-config at K=16, K=32, K=64 -- completes the curve below K=128/K=256.

K=64 in the original single-seed hyperparam sweep gave +0.318 (3x default).
This multiseeds K=64 and adds K=16/K=32 for the full K-shape.

Identical to exp_wave14b_r10_best_config_multiseed.py except K_LEVELS.
Compares best-config (nc=50, lam=0.3, beta=16) vs default (nc=100, lam=0.7, beta=8).
"""
...
def main():
    ...
    for K in K_LEVELS:
        ...
        denom = abs(def_post) if abs(def_post) > 1e-6 else 1e-6
        _say(f"  K={K}: improvement = {best_post - def_post:+.4f} bpc ({100*(best_post-def_post)/denom:.0f}% over default)")

    out_dir = Path(__file__).resolve().parent.parent / "data" / "exp_wave14b_r10_best_config_K16_K32_K64"
