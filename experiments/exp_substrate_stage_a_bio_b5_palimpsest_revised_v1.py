"""
substrate_stage_a_bio_b5_palimpsest_revised_v1 -- STDP replay w/ palimpsest decay (per Research spec) -- LAPTOP.

ROUTING: notes/research_to_exp_dev_B5_decay_model_palimpsest_spec_2026-06-04 (drill answer to my B5 question).
  Tsodyks-1990 palimpsest: alpha=0.003 decay BEFORE each main Hebbian write; replay updates add NO decay
...
ANCHOR_NAME = "substrate_stage_a_bio_b5_palimpsest_revised_v1"
RUN_MODE = ("smoke" if "--smoke" in sys.argv else os.environ.get("HDLAB_RUN_MODE", "full")).lower()
_ap = argparse.ArgumentParser()
_ap.add_argument("--smoke", action="store_true"); _ap.add_argument("--self-test", action="store_true")
_ARGS, _ = _ap.parse_known_args()

ALPHA = 0.003          # palimpsest decay per main write (Tsodyks; M_steady ~ 1/alpha = 333)
...
def run_arm(arm, items, n, g):
    ...
    for t in range(M_):
        W *= (1.0 - ALPHA)                       # palimpsest decay (main write only)
        ...
        if per_event and (t + 1) % BATCH_END == 0 and done_replays < total_replays:
            ...
            for j in sel:
                W += np.outer(items[j + 1], items[j])   # replay: NO decay
            done_replays += k
