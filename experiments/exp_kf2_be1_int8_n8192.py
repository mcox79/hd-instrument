"""KF-2 BE-1 PRECISION: int8 (8-bit quantization) at N=8192.

Delegates to exp_kf2_be1_precision_sweep_n8192 with PRECISION='int8'.
int8 = 4x memory compression vs fp32. Symmetric per-tensor quantization.
Expected: max_iso < 0.05 (int8 perturbation to W is small relative to isolation bound).
This is the commercially critical threshold: 4x compression = significant deployment cost reduction.

N-suffix: _n8192 -> production N = 8192 (PROT-018 binding).
Anchor: kf2_be1_int8_n8192
...
_drv_spec = importlib.util.spec_from_file_location("be1_drv_int8", _drv_path)
...
def get_output_dir() -> Path:
    name = os.environ.get("HDLAB_EXP_NAME", "kf2_be1_int8_n8192")
...
def _instrumentation_selftest() -> None:
    ...
    assert cell.get("precision_compression_ratio", 0) == 4.0, (
        f"int8 compression must be 4.0; got {cell.get('precision_compression_ratio')}")
    print(f"[selftest] kf2_be1_int8_n8192 PASS iso={cell['isolation_ratio']:.5f}", flush=True)
...
def main():
    ...
    print(f"[run] kf2_be1_int8_n8192 smoke={smoke} N={N_cfg} precision={PRECISION} "
    ...
    if max_iso >= _drv.HF_CONTAMINATION:
        verdict = "KF2_BE1_INT8_HARD_FAIL"
        verdict_msg = f"INT8_BREAKS_ISOLATION: max_iso={max_iso:.5f} >= {_drv.HF_CONTAMINATION}"
    elif seeds_pass >= _drv.HP_SEEDS_MIN:
        verdict = "KF2_BE1_INT8_HARD_PASS"
        verdict_msg = (f"INT8_HOLDS: {seeds_pass}/{len(seeds)} seeds pass, "
                       f"max_iso={max_iso:.5f} < {_drv.HP_ISOLATION_MAX}, "
                       f"compression={compression:.1f}x theory_bound={theory_bound:.5f}")
    else:
        verdict = "KF2_BE1_INT8_MIDDLE_BAND"
        verdict_msg = f"INT8_PARTIAL: {seeds_pass}/{len(seeds)} seeds, max_iso={max_iso:.5f}"

    elapsed = round(time.time() - t0, 2)
    summary = {
        "anchor": "kf2_be1_int8_n8192", "precision": PRECISION, "N": N_cfg, "smoke": smoke,
