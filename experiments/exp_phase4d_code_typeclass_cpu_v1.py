...
ANCHOR_NAME = "phase4d_code_typeclass_cpu_v1"
...
def _gold_type(code, prompt):
    ...
    if any(s in c for s in (".join", ".split", ".replace", ".lower", ".upper", "ord(", "chr(", "isalpha", "isdigit", "substring")): return "STRING"
    if any(s in c for s in ("re.", "regex", "findall", "match(")): return "STRING"
    if any(s in c for s in ("max(", "min(", "count(", "index(", " in ", "filter", "[i for", "any(", "all(")): return "LIST"
    if any(s in c for s in ("==", "!=", " if ", "return true", "return false", "bool(", ">=", "<=")) and ("for" not in c and "while" not in c): return "CHECK"
    if any(s in c for s in ("+", "-", "*", "/", "%", "sum(", "pow(", "**", "math.")): return "ARITH"
    return "LIST"
FEAT_CUES = None
def _feats(prompt):
...
def _selftest():
    ...
    print("[selftest] PASS: phase4d-code-typeclass", flush=True)
_selftest()
if _ARGS.self_test:
    sys.exit(0)
def _load():
    ...
    return tr, te
def run() -> Dict:
    rng = np.random.default_rng(int(os.environ.get("HDLAB_SEED", "1016")))
    try:
        train, test = _load()
...
def verdict(r) -> Tuple[str, str]:
    ...
    lift = a - mj
    if lift >= 0.10 and a >= 0.55:
        return ("HARD_PASS", "HARD_PASS: substrate discriminative classifier predicts CODE-task-type from NL prompt well above majority (lift>=0.10) -- the mechanism transfers from MATH to CODE. " + s)
    if lift >= 0.05:
        return ("MIDDLE_BAND", "MIDDLE_BAND: lift 0.05-0.10 over majority -- mechanism transfers partially to code prompts. " + s)
    return ("HARD_FAIL", "HARD_FAIL: lift <0.05 over majority -- discriminative weighting transfers only WEAKLY to code (acc %.3f vs majority %.3f). Code STRUCTURE is not prompt-determined (prompt says WHAT not HOW); unlike MATH op-prediction. Code genuinely needs synthesis, not classification. " % (a, mj) + s)
