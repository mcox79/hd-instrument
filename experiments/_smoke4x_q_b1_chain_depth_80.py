"""Multi-scale smoke helper: runs q_b1_chain_depth_80 at N=4096 (smoke x4 per MULTI-SCALE SMOKE rule)."""
import sys
...
for seed in SEEDS:
    ...
    print(f"[N4096 seed={seed}] alpha={alpha:.4f} snaps={' '.join(f'd{d}:{mean[d]:.4f}' for d in [5,10,20,30,45,60,70,80])}", flush=True)
    del H
