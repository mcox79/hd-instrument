"""
pp58_scs_tau_sweep_d8_tau015_v1_n8192 -- PP-58 SCS framework tau sweep at d=8, tau_target=0.15.

CONTEXT (v377 cycle 47 all-night burst):
  tau=0.05 HARD_FAILed (d_estimate=1.487 < 1.5, spike condition violated).
  tau=0.10 queued (cycle 46). Continuing tau sweep at tau=0.15 to map tau-vs-spike.
  This experiment constructs Xi with explicit asymmetry mixing to target tau_target=0.15.
  The SCS formula gamma = (d + tau/d) / (1+tau) at d=8, tau=0.15.
  gamma = (8 + 0.15/8) / (1.15) = (8.01875) / 1.15 = 6.9728.
  Key question: does tau=0.15 asymmetry enable spike condition d >= 1.5?

DESIGN (same controlled-asymmetry approach as tau=0.10):
  W = (1 - tau_target) * W_sym + tau_target * W_asym
  where W_sym = Xi^T Xi / N (standard Hopfield), W_asym = (W_rand - W_rand^T) / 2.

SCIENTIFIC QUESTION:
  At tau_target=0.15, does controlled asymmetric W produce d >= 1.5
...
FORMULA SELF-TESTS (PROT-022):
  1. SCS formula at d=8, tau=0.15: gamma = (8 + 0.15/8) / (1 + 0.15).
     (8 + 0.01875) / 1.15 = 8.01875 / 1.15 = 6.9728.
     [INPUT: d=8, tau=0.15] [EXPECTED: 6.9728 within 0.001]
...
TIMEOUT ESTIMATE: Similar to tau=0.10 test. ceil(1.5 * 1200 * 5) = 9000s. Use floor: 21600s.
"""
import sys
...
ANCHOR_NAME = "pp58_scs_tau_sweep_d8_tau015_v1_n8192"
...
_gamma_d8_tau015 = _scs_gamma(8.0, 0.15)
print(f"[selftest-formula] SCS gamma(d=8,tau=0.15): {_gamma_d8_tau015:.4f} (expected 6.9728)", flush=True)
assert abs(_gamma_d8_tau015 - 6.9728) < 0.001, f"SCS formula selftest tau=0.15: got {_gamma_d8_tau015}"
