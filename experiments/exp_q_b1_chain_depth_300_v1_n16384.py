"""
q_b1_chain_depth_300_v1_n16384 -- Q-B1: heteroassoc chain at depth-300 at N=16384.

Ceiling chase: depth-200 HARD_PASS at N=16384 (v353+). Four consecutive N=16384 flat-profile
confirmations: {d80:v348, d100:v351, d150:v353, d200:completed}. Band at 0.85-0.95.
Push to depth-300 to determine if flat-profile continues or ceiling emerges.
...
PRE-REGISTERED BANDS (prior d-200 at N=16384 flat-profile ~0.989; fourth consecutive HP):
  HARD-PASS: d5 >= 0.90 AND d20 >= 0.75 AND d50 >= 0.50 AND d100 >= 0.20
             AND d200 >= 0.02 AND d300 >= 0.02.
  MIDDLE: d300 in [0.005, 0.02) while earlier depths meet HP.
  HARD-FAIL: d5 < 0.80 OR d20 < 0.50 OR d300 < 0.005.

  Calibration note: if flat-profile continues as in d5-d200, d300 will be ~0.989 (>> 0.02 HP gate).
  Bands preserve calibration-probe +-50% policy on the new depth point; earlier checkpoints
  validated at high margin. BAND-LIFT 0.85-0.95 ceiling-characterization: d300 HP confirms
  near-zero decay extends to at least 300 hops.

FORMULA SELF-TESTS (PROT-022):
  1. depth-300 chain retrieval non-NaN at N=128.
     [INPUT: N=128, CHAIN_DEPTH=300] [EXPECTED: cos non-NaN in [-1,1]]
...
TIMEOUT ESTIMATE: d-200 elapsed=470.8s FULL 5-seed GPU.
  d-300 scaling: depth linear (O(N^2) per step, 300 vs 200 steps = 1.5x).
  ceil(1.5 * 470.8 * 1.5 * 1.0) = ceil(1060) = 1200s. Within 14400s cap.
"""
import sys
...
ANCHOR_NAME = "q_b1_chain_depth_300_v1_n16384"
...
HF_D20 = 0.50
HF_D300 = 0.005
MIDDLE_D300_LOW = 0.005
MIDDLE_D300_HIGH = 0.02
...
def _instrumentation_selftest():
    ...
    assert not (cos != cos), f"depth-300 retrieval is NaN at N={N_t}"
    ...
    del H
    print(f"[selftest] PASS: depth-300 chain non-NaN N={N_t}, self-cosine=1.0, "
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    summary = (f"d5={d5:.4f}(HP>={HP_D5} HF<{HF_D5}) "
               ...
               f"d300={d300:.4f}(HP>={HP_D300} HF<{HF_D300}) "
               f"n_seeds={len(results)}")

    if d5 < HF_D5 or d20 < HF_D20 or d300 < HF_D300:
    ...
    if hp5 and hp20 and hp50 and hp100 and hp200 and hp300:
        return ("HARD_PASS", f"HARD_PASS: all depth thresholds met at N=16384 depth-300. "
                             f"Flat-profile confirmed to depth-300. {summary}")
    n_hp = sum([hp5, hp20, hp50, hp100, hp200, hp300])
    if MIDDLE_D300_LOW <= d300 < MIDDLE_D300_HIGH and hp5 and hp20:
        return ("MIDDLE_BAND", f"MIDDLE_BAND: d300 in boundary zone. {summary}")
