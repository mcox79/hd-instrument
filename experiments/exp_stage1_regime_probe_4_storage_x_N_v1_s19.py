"""stage1_regime_probe_4_storage_x_N_v1 sibling seed=19.

Fourth probe in the Stage 1 Regime Map arc (USER 2026-07-03). STORAGE x N
(SCALE_FREE) cross-term; CLEANUP_MECHANISM fixed = iterative_cosine (winner
from Probe 1; simplest for isolation); F=1, L=2 fixed.

Sweep FULL:  2 storage x 4 N x 3 M x 2 corr = 48 pts/seed.
Sweep SMOKE: 2 storage x 2 N x 1 M x 1 corr + 1 SHARDED_PC_easy = 5 pts.
N grid FULL = {2048, 4096, 8192, 16384}. Mechanism = iterative_cosine. F=1 L=2.
...
SEED = 19
ANCHOR_NAME = f"stage1_regime_probe_4_storage_x_N_v1_s{SEED}"
...
CONFIG_VERSION = (
    ...
    f"regime=storage_x_N_scale_free_FHRR_chain_composition_probe_4,"
    f"storage=[SHARDED,BUNDLED],N=[2048,4096,8192,16384],"
    f"M=[200,800,3200],corr=[0.20,0.45],"
    f"mechanism=iterative_cosine(fixed),F=1(fixed),L=2(fixed),"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "stage1_regime_probe_4_storage_x_N_v1_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: Exception) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "stage1_regime_probe_4_storage_x_N_v1_import_crash",
        }
        tmp = out_dir / "metrics.json.tmp"
        final = out_dir / "metrics.json"
...
def main() -> int:
    ...
    from experiments._stage1_regime_probe_4_storage_x_N_v1_core import (
    ...
    final["_hardening_marker"] = "stage1_regime_probe_4_storage_x_N_v1_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = ("synthetic_FHRR_chain_composition_"
                                   "storage_x_N_probe_v1")
