...
instruction file section 13): runs the shared v4 core at seed=7 (matching the
v3c FULL seed=7 checkpoint this cell reloads for both Phase 1 reverify and
Phase 2 relock). A runner-death on this process loses only seed=7, not the
sibling seed=13 run (exp_encoder_migration_step1b_v4_joint_reverify_relock_v1_seed_13.py).
...
run_tag="seed7" resolves the READ-ONLY source checkpoint directory
(data/substrate_concept_encoder_v1b_v3c_full_paired_seed7/, the ALREADY-
LANDED v3c FULL seed=7 run's _ckpt_best_{GLOBAL,INBATCH}.pt) via
v3c._artifact_dir("full", "seed7") -- same convention v3c's own seed_7.py
...
(data/substrate_concept_encoder_v1b_v4_reverify_relock_seed7/), never
