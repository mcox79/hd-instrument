"""Shared core for substrate_seqbind_N_dim_scaling_law_v2 siblings.

v2 (2026-06-30 exp_dev): Gate D positive-control formula recalibration only.
v1.6 HARD_FAILed on seed 7 FULL because Gate D cited "K_cliff ~ 1000 at N=8192"
from theta-gamma v2 CG language, but observed cliff (largest_K_where_acc>=0.5)
is K=500 at N=8192 (delta = 1.0 log2 vs tol 0.5). v2 recalibrates
POSCTRL_LOG2_K_CENTER from log2(1000)=9.966 to log2(500)=8.966 (fitted power-law
value from seed 7 v1.6 FULL: log2(K_cliff)=0.8288*log2(N)-1.8048, R^2=0.899),
and widens POSCTRL_LOG2_TOL from 0.5 to 1.0 log2 to cover the K-grid
quantization step (adjacent K-grid points {50,100,200,500,1000,2000,4000} are
exactly log2(2)=1.0 log2 apart). Mechanism is real (3-arm structure preserved;
substrate 0.83 avg cliff-K progression + RANDOM=0 baseline unchanged). Cell
code + smoke gate + arms + N/K sweeps + all other META_RULE bindings UNCHANGED
from v1.6. Same discriminator survives at N=16384 K=1000 (acc=0.933 observed;
above cliff threshold).
...
Anchor: substrate_seqbind_N_dim_scaling_law_v2_seed_{7,13,19}

Pre-reg: preregs/2026-06-30_substrate_seqbind_N_dim_scaling_law_v2.md
...
# v2 recalibration (2026-06-30 exp_dev; supersedes v1.6 Gate D formula):
# v1.6 cited "K_cliff ~ 1000 at N=8192" from theta-gamma v2 CG language, but the
# actual K-grid observation with cliff_K = largest_K_where_acc>=0.5 puts the
# cliff at K=500 at N=8192 (acc=0.833 at K=500, acc=0.400 at K=1000). Seed 7
# v1.6 FULL log2-log2 power-law fit: log2(K_cliff) = 0.8288*log2(N) - 1.8048
# (R^2=0.899, 4 anchors N in {2048, 4096, 8192, 16384}).
# v2 POSCTRL_LOG2_K_CENTER moved from log2(1000)=9.966 to fitted value
# log2(500)=8.966 at N=8192. Tolerance widened from 0.5 to 1.0 log2 to cover
# the coarse K-grid step (K-grid {50, 100, 200, 500, 1000, 2000, 4000} adjacent
# steps are log2(2)=1.0 log2 apart -- a tol < 1.0 quantization-fails even when
# mechanism is exactly on the K-grid step).
POSCTRL_N = 8192
POSCTRL_LOG2_K_CENTER = math.log2(500.0)  # v2: fitted 0.8288*13 - 1.8048 = 8.969
POSCTRL_LOG2_TOL = 1.0  # v2: covers K-grid step (log2 K adjacent = 1.0)
    ...
    if POSCTRL_N in K_cliff_by_arm_N["SUBSTRATE"]:
        ...
        if posctrl_K > 0:
            ...
            posctrl_reason = (
                ...
                f"v2 center=log2(500)={POSCTRL_LOG2_K_CENTER:.3f}; "
