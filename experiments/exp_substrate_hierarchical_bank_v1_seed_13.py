"""substrate_hierarchical_bank_v1 sibling seed=13.

CODEBOOK-STRUCTURE phase diagram: 3 structures (flat / partition_by_source /
hierarchical_2level) swept as OUTER axis over M in {4000, 16000, 64000}.
N=8192, n_super_banks=8. Axis H CG-eligible (research 2026-07-01 gap analysis;
CG=0.45 HIGH payoff; 5x-drill escalation eligible if HP).

Composes with ANCHOR 1 partition-by-source CG per META_RULE_AT.

Per research spec: notes/research_phase_diagram_gap_analysis_next_cells_2026-07-01.md sec.3
...
local_cpu_queue (smoke). No GPU mandate.
...
META_RULE_Q: routing_acc anti-saturation guard (router imperfect by construction).
...
SEED = 13
ANCHOR_NAME = f"substrate_hierarchical_bank_v1_seed_{SEED}"
...
CONFIG_VERSION = (
    ...
    f"structures=[flat,partition_by_source,hierarchical_2level],"
    f"encoder=bipolar_numpy,N=8192,SEED={SEED},mode={RUN_MODE},"
    f"M_full=[4000,16000,64000],M_smoke=[4000,64000],"
    f"n_super_banks=8,CUE_ROUTE_COS=0.85,CUE_COS=0.70,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v1_hierarchical_bank_numpy",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    ...
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v1_hierarchical_bank_import_crash",
        }
...
def main() -> int:
    ...
    from experiments._substrate_hierarchical_bank_v1_core import (
    ...
    final["_hardening_marker"] = "v1_hierarchical_bank_numpy"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = "synthetic_substrate_bipolar_codebook_hierarchical"
