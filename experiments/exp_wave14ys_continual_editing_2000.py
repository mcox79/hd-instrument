"""Continual editing at 2000 edits - test if Kerdock holds beyond yr's 1000.

Pre-reg: preregs/2026-05-21_wave14ys_continual_editing_2000.md
"""
...
def compute_verdict(summary):
    ...
    if "kerdock" not in arms:
        return ("CONTINUAL_2000_INCONCLUSIVE", "Missing.")
    kerdock = arms["kerdock"]
    if not kerdock.get("per_edit_trajectory"):
        return ("CONTINUAL_2000_INCONCLUSIVE", "Missing traj.")
    ...
    if k_ok:
        return ("CONTINUAL_2000_HOLDS",
                f"Kerdock holds 2000 edits: min_edited={kerdock['min_edited_acc']:.3f}, "
                f"min_kept={kerdock['min_kept_acc']:.3f}.")
    return (f"CONTINUAL_2000_DECAYS_AT_{k_fail}",
            f"Kerdock decays at edit {k_fail}.")


def self_test_verdict():
    def mk(min_e, fail_e=None, n=2000):
    ...
    cases = [
        ({"by_arm": {"kerdock": mk(0.98), "correlated": mk(0.5, fail_e=2)}}, "CONTINUAL_2000_HOLDS"),
        ({"by_arm": {"kerdock": mk(0.5, fail_e=1500), "correlated": mk(0.5, fail_e=2)}},
         "CONTINUAL_2000_DECAYS_AT_1500"),
        ({}, "CONTINUAL_2000_INCONCLUSIVE"),
    ]
    for s, exp in cases:
...
def run_experiment(smoke):
    ...
    config = {"mode": "smoke" if smoke else "full", "N": 1024 if smoke else 4096,
              "M_stored": 1024 if smoke else 4096, "n_edits": 30 if smoke else 2000,
...
def run_smoke():
    out_dir = get_output_dir("wave14ys_continual_editing_2000_smoke")
...
def run_main():
    out_dir = get_output_dir("wave14ys_continual_editing_2000")
