"""pfc_goal_conditioned_gate_v2_cleanup_bind_output -- revival of v1.

V1 expected result: BIND_GATE arm collapses to 0.0 because hrr_bind(state, goal)
produces a vector OFF the op-key argmax manifold -- the codebook entries don't
lie near bind-state-goal outputs, so argmax over operator-application of
bind(state, goal) fails. Meanwhile WM_SLOT arm showed +0.08 lift over V1
(positive signal that goal information helps when properly routed).

V2 FIX (per cell author 2026-06-27): two concrete fixes:
  (1) CLEANUP_BIND: cleanup hrr_bind(state, goal) through codebook E BEFORE
      applying operators. Brings the bind-output back onto the codebook
      manifold so argmax-cos-to-codebook scoring works.
  (2) ADDITIVE_GOAL_BIAS: instead of bind, use additive scoring:
      score(op) = alpha * cos(op(state), goal) + (1-alpha) * cos(op(state), codebook).
      Goal modulates op selection without ever leaving the manifold.

ARMS (6):
  ARM_V1_NO_GOAL              v1 baseline (regression sanity; should reproduce v1)
  ARM_BIND_GATE_CLEANUP       A1: cleanup-bind-output before op-application
  ARM_WM_GOAL_SLOT            B1: preserve v1 winner (persistent WM slot)
  ARM_ADDITIVE_GOAL_BIAS      A2: additive scoring (no bind, no cleanup)
  ARM_COMBINED                ALL three mechanisms fused
  ARM_ORACLE                  upper bound (correct op sequence given)

PRE-REG BANDS (LOCKED at module init; PROSPECTIVE):
  HARD_PASS:   BIND_GATE_CLEANUP > 0.4 (no longer collapses to 0) AND
               COMBINED - max(BIND_CLEANUP, WM_SLOT, ADDITIVE) >= +0.10 AND
               (COMBINED - V1) / (ORACLE - V1) >= 0.30 at depth=6 (closes
                                                                   >=30% oracle gap) AND
               COMBINED.cv < 0.15
  MIDDLE_BAND: BIND_GATE_CLEANUP in [0.2, 0.4] (partial fix) OR
               gap-closed in [0.10, 0.30)
  HARD_FAIL:   BIND_GATE_CLEANUP <= 0.05 (cleanup didn't fix the collapse) OR
               ORACLE - V1 < 0.10 (no headroom -- regime broken)

CARDINALITY (META_RULE_H):
  EXPECTED_N_UNITS_FULL  = 6 arms * 5 seeds * 2 depths = 60
  EXPECTED_N_UNITS_SMOKE = 6 arms * 2 seeds * 2 depths = 24
  Discriminator-survives-scale: smoke at N=8192 depths {3,6} (matches v1 smoke).
...
ANCHOR_NAME = "pfc_goal_conditioned_gate_v2_cleanup_bind_output"
...
HP_BIND_CLEANUP_MIN = 0.4
HP_LIFT_OVER_SINGLE = 0.10
HP_GAP_CLOSED = 0.30
HP_CV_MAX = 0.15
HF_BIND_CLEANUP_LO = 0.05
...
CONFIG_VERSION = (
    ...
    "mode=%s,HP_bind_clean>=%.2f,HP_lift_single>=%.2f,HP_gap>=%.2f,cv<%.2f,"
    "alpha=%.2f,expected_n=%d,"
    ...
    HP_BIND_CLEANUP_MIN, HP_LIFT_OVER_SINGLE, HP_GAP_CLOSED, HP_CV_MAX,
    GOAL_BIAS_ALPHA, EXPECTED_N_UNITS,
)
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v2_cleanup_bind_output_pfc_goal_gate",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v2_cleanup_bind_output_pfc_goal_gate_import_crash",
        }
...
def cleanup_to_E(v: np.ndarray, E: np.ndarray) -> Tuple[int, np.ndarray]:
    """Return (best_idx, codebook_vector_at_best_idx)."""
...
def pfc_select_op_bind_gate_cleanup(state: np.ndarray, goal: np.ndarray,
                                      W_ops: List[np.ndarray],
                                      E: np.ndarray) -> int:
    """V2 FIX A1: bind(state, goal) -> cleanup through E -> apply ops.

    Brings the bind output back onto the codebook manifold so argmax-cos-to-
    codebook scoring works. This fixes v1's BIND_GATE arm collapse to 0.0.
    """
    sg = hrr_bind(state, goal)
    # CLEANUP through codebook E (the v2 fix)
    _, sg_clean = cleanup_to_E(sg, E)
    sg_n = sg_clean / (np.linalg.norm(sg_clean) + 1e-8)
    best_idx = 0; best_score = -1.0
    for i, W in enumerate(W_ops):
        out = sg_n @ W
        out_n = out / (np.linalg.norm(out) + 1e-8)
...
def pfc_select_op_wm_slot(state: np.ndarray, goal_wm_slot: np.ndarray,
    ...
    """V1 winner preserved: persistent goal WM slot drives goal-distance scoring."""
    state_n = state / (np.linalg.norm(state) + 1e-8)
...
def pfc_select_op_additive_bias(state: np.ndarray, goal: np.ndarray,
                                  ...
                                  alpha: float = GOAL_BIAS_ALPHA) -> int:
    """V2 FIX A2: additive goal bias (no bind, no cleanup needed)."""
    state_n = state / (np.linalg.norm(state) + 1e-8)
    ...
    for i, W in enumerate(W_ops):
        ...
        manifold_sc = float(np.max(sims))
        goal_sim = float(out_n @ goal_n)
        sc = alpha * goal_sim + (1.0 - alpha) * manifold_sc
        if sc > best_score:
            best_score = sc
            best_idx = i
    return best_idx
...
def pfc_select_op_combined(state: np.ndarray, goal: np.ndarray,
                             ...
                             W_ops: List[np.ndarray], E: np.ndarray) -> int:
    """ALL three: cleanup-bind input + persistent WM + additive bias."""
    sg = hrr_bind(state, goal)
    _, sg_clean = cleanup_to_E(sg, E)
    sg_n = sg_clean / (np.linalg.norm(sg_clean) + 1e-8)
    goal_n = goal_wm_slot / (np.linalg.norm(goal_wm_slot) + 1e-8)
    ...
    for i, W in enumerate(W_ops):
        ...
        manifold_sc = float(np.max(sims))
        goal_sim = float(out_n @ goal_n)
        sc = 0.4 * manifold_sc + 0.6 * goal_sim
        if sc > best_score:
            best_score = sc
            best_idx = i
    return best_idx
...
def run_arm_bind_cleanup(W_ops, E, test_chains, depth):
    correct = 0
    for (s, op_seq, target) in test_chains:
        state = E[s].copy()
        goal = E[target].copy()
        idx = s
        for _ in range(depth):
            op = pfc_select_op_bind_gate_cleanup(state, goal, W_ops, E)
            idx, state = apply_operator(state, W_ops[op], E)
        if idx == target:
            correct += 1
    return correct / max(1, len(test_chains))
...
def run_arm_additive(W_ops, E, test_chains, depth):
    correct = 0
    for (s, op_seq, target) in test_chains:
        state = E[s].copy()
        goal = E[target].copy()
        idx = s
        for _ in range(depth):
            op = pfc_select_op_additive_bias(state, goal, W_ops, E)
            idx, state = apply_operator(state, W_ops[op], E)
        if idx == target:
            correct += 1
    return correct / max(1, len(test_chains))
...
def run_one_seed(seed: int) -> Dict[str, Any]:
    ...
    for depth in HOP_DEPTHS:
        out["v1_no_goal"][str(depth)] = run_arm_v1(W_ops, E, test_chains, depth)
        out["bind_gate_cleanup"][str(depth)] = run_arm_bind_cleanup(W_ops, E, test_chains, depth)
        out["wm_goal_slot"][str(depth)] = run_arm_wm_slot(W_ops, E, test_chains, depth)
        out["additive_goal_bias"][str(depth)] = run_arm_additive(W_ops, E, test_chains, depth)
        out["combined"][str(depth)] = run_arm_combined(W_ops, E, test_chains, depth)
        out["oracle"][str(depth)] = run_arm_oracle(W_ops, E, test_chains, depth)
...
def aggregate_and_verdict(per_seed: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    bc = summary["bind_gate_cleanup"][dd]["mean"]
    ws = summary["wm_goal_slot"][dd]["mean"]
    ab = summary["additive_goal_bias"][dd]["mean"]
    cb = summary["combined"][dd]["mean"]
    oracle = summary["oracle"][dd]["mean"]
    cb_cv = summary["combined"][dd]["cv"]

    lift_over_v1 = cb - v1
    lift_over_single = cb - max(bc, ws, ab)
    headroom = oracle - v1
    ...
    elif bc <= HF_BIND_CLEANUP_LO:
        verdict = "HARD_FAIL_CLEANUP_DID_NOT_FIX_COLLAPSE"
    elif (bc > HP_BIND_CLEANUP_MIN and
            lift_over_single >= HP_LIFT_OVER_SINGLE and
            closed >= HP_GAP_CLOSED and
            cb_cv < HP_CV_MAX):
        verdict = "HARD_PASS"
    elif bc <= 0.2:
        verdict = "HARD_FAIL_CLEANUP_PARTIAL"

    verdict_msg = (
        "%s | depth=%d | V1=%.3f BIND_CLEAN=%.3f WM=%.3f ADDITIVE=%.3f "
        "COMBINED=%.3f ORACLE=%.3f | lift_v1=%.3f lift_single=%.3f closed=%.3f "
        "cv=%.3f n_seeds=%d"
    ) % (verdict, decision_depth, v1, bc, ws, ab, cb, oracle,
...
def main() -> int:
    ...
    final["_hardening_marker"] = "v2_cleanup_bind_output_pfc_goal_gate"
