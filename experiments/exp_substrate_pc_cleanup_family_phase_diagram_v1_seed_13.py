"""substrate_pc_cleanup_family_phase_diagram_v1 sibling seed=7.

THIRD COMPONENT-SUBSTITUTION phase diagram for substrate (USER 2026-06-28
Research directive). Pattern completion with 4 cleanup families swept as
the OUTER axis: modern_hopfield / classical_hopfield / iterative_cosine /
soft_energy_attractor. Encoder FIXED binary_bipolar.

Inner grid: 2 N x 5 corruption x 2 iters = 20 inner pts per cleanup x 4
cleanups = 80 phase points per seed (FULL).
Smoke: 1 N x 3 c x 1 T = 3 inner pts per cleanup x 4 = 12 corner points.
...
CARDINALITY_OK_FULL: 80 phase points per seed
CARDINALITY_OK_SMOKE: 12 corner points per seed
...
SEED = 13
ANCHOR_NAME = f"substrate_pc_cleanup_family_phase_diagram_v1_seed_{SEED}"
...
CONFIG_VERSION = (
    f"ANCHOR={ANCHOR_NAME},cleanups=[modern_hopfield,classical_hopfield,iterative_cosine,soft_energy_attractor],"
    f"encoder=binary_bipolar,N=[2048,8192],SEED={SEED},mode={RUN_MODE},"
    f"corruption=[0.20,0.35,0.45,0.475,0.50],iters=[1,5],"
    f"M=300,beta=8.0,alpha_soft=0.5,"
    f"arms=[MECHANISM,RANDOM_FLOOR],"
    f"expected_n_full=80,expected_n_smoke=12,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v1_pc_cleanup_family_phase_diagram_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    ...
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v1_pc_cleanup_family_import_crash",
        }
...
def main() -> int:
    ...
    from experiments._substrate_pc_cleanup_family_phase_diagram_v1_core import (
    ...
    # CPU-routable cell; no GPU mandate (Fix #24 N/A for binary_bipolar-only)
    ...
    final["_hardening_marker"] = "v1_pc_cleanup_family_phase_diagram_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = "synthetic_substrate_4_cleanup_family_pc"
