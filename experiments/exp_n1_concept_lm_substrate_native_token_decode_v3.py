...
ANCHOR_NAME = "n1_concept_lm_substrate_native_token_decode_v3"
...
def batched_token_logprob(D: np.ndarray, concept_vecs: np.ndarray,
    ...
    """
    scores = (concept_vecs @ D) / tau           # (n_pos, V_TOK)
    scores = scores - scores.max(axis=1, keepdims=True)   # numerically stable softmax
    exp_s = np.exp(scores)
    probs = exp_s / (exp_s.sum(axis=1, keepdims=True) + 1e-300)
    if uni_dist is not None and lam > 0.0:
        probs = (1.0 - lam) * probs + lam * uni_dist[None, :]   # unigram back-off floor
...
def token_logprob(D: np.ndarray, concept_vec: np.ndarray,
    ...
    """
    scores = (D.T @ concept_vec) / tau  # (V_TOK,)
    scores = scores - scores.max()
    exp_s = np.exp(scores)
    probs = exp_s / (exp_s.sum() + 1e-300)
    if uni_dist is not None and lam > 0.0:
        probs = (1.0 - lam) * probs + lam * uni_dist   # unigram back-off floor
    return np.log(probs + 1e-300)
        ...
        if tot_t == 0:
            ...
            for _i in range(n_valid):
                ...
                if _bp is not None and _bp.sum() > 0:
                    ...
                    _bfd = (_bp.astype(np.float64) + LAPLACE_A) / (_bp.sum() + LAPLACE_A * V_TOK)
                    big_nll += -math.log(float(_bfd[_tt]))
                else:
            ...
            for _i in range(n_valid):
                ...
                if _ctd is not None and _ctd.sum() > 0:
                    _ctd_d = (_ctd.astype(np.float64) + LAPLACE_A) / (_ctd.sum() + LAPLACE_A * V_TOK)
                    ceil_nll += -math.log(float(_ctd_d[_tt]))
                else:
