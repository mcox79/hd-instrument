...
def main(smoke: bool = False) -> None:
    self_test_verdict()
    device = torch.device("cpu")  # CPU runner explicit
    if smoke:
        config = {"mode": "smoke", "N_list": [256, 512],
                  "K_factor_grid": [0.05, 0.20, 0.40], "seeds": [17]}
    else:
        # N-sweep across 4096, 8192, 16384 to test forensics scaling
        config = {"mode": "full", "N_list": [4096, 8192, 16384],
                  "K_factor_grid": [0.02, 0.05, 0.10, 0.20, 0.40, 0.70],
                  "seeds": [17, 23, 31]}
    print(f"wave14cpu_walsh_peaks_N_sweep. mode={config['mode']} device={device}", flush=True)

    t0 = time.monotonic()
    per_K = []
    for N in config["N_list"]:
        print(f"  Building Hadamard matrix N={N}...", flush=True)
        H = hadamard_matrix(N)
        K_list = sorted({max(2, int(round(N * f))) for f in config["K_factor_grid"]})
        for K in K_list:
            recalls = []
            for seed in config["seeds"]:
                r = run_one_seed(N, K, seed, H, device)
                recalls.append(r)
            mean_recall = sum(recalls) / len(recalls)
            per_K.append({"N": N, "K": K, "K_over_N": K/N, "recall": mean_recall,
                          "per_seed_recall": recalls})
            print(f"    N={N} K={K:6d} K/N={K/N:.3f}  recall={mean_recall:.3f}", flush=True)
        del H
    ...
    out_dir = get_output_dir("wave14cpu_walsh_peaks_N_sweep")
