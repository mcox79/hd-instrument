...
ANCHOR_NAME = "substrate_encoder_noise_bundle_v1"; BI = "BAAI/bge-large-en-v1.5"
HOTPOT = REPO / "data" / "datasets" / "hotpot_qa_distractor_dev_1k.jsonl"
RUN_MODE = ("smoke" if "--smoke" in sys.argv else os.environ.get("HDLAB_RUN_MODE", "full")).lower()
_ap = argparse.ArgumentParser(); _ap.add_argument("--smoke", action="store_true"); _ap.add_argument("--self-test", action="store_true"); _ARGS, _ = _ap.parse_known_args()
NITEMS = 300 if RUN_MODE == "smoke" else 1000; SIGMA = 0.2
...
def run() -> Dict:
    ...
    for K in ([1, 3] if RUN_MODE == "smoke" else [1, 3, 5]):
        ...
        for _ in range(K):
            ...
            votes += Bq @ Bk.T
        a2["K%d" % K] = recall_at_k(votes, 10)
        print("  [A2 ensembling] K=%d recall@10=%.3f" % (K, a2["K%d" % K]), flush=True)
    a2_pass = a2.get("K3", 0) >= 1.5 * a2.get("K1", 1e-9)
    # A3 ternary vs bipolar recall@10 under noise
    Bk = bipolar(E); a3_bip = recall_at_k(bipolar(noisy(E, SIGMA, g)) @ Bk.T, 10)
    best_tern = 0.0; best_tau = None
    for tau in [0.5, 1.0, 1.5]:
        Tk = ternary(E, tau); rec = recall_at_k(ternary(noisy(E, SIGMA, g), tau) @ Tk.T, 10)
        if rec > best_tern:
            best_tern = rec; best_tau = tau
    a3 = {"bipolar": a3_bip, "ternary_best": best_tern, "tau": best_tau}
    a3_pass = best_tern >= 1.75 * (a3_bip + 1e-9)
    print("  [A3 ternary] bipolar recall@10=%.3f ternary_best=%.3f (tau=%s) -> %s" % (a3_bip, best_tern, best_tau, "ternary wins" if a3_pass else "no"), flush=True)
