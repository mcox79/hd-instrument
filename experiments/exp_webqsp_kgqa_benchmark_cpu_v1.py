"""
exp_webqsp_kgqa_benchmark_cpu_v1 -- v1 benchmark suite: substrate K-hop on REAL WebQSP KG-QA -- CPU.

ROUTING: v1 benchmark suite (R3 real KG-QA). RoG-WebQSP: real questions + real Freebase subgraphs (graph = list of [h,r,t]) +
...
ANCHOR_NAME = "webqsp_kgqa_benchmark_cpu_v1"; N = 4096; MAX_HOPS = 3
DS = REPO / "data" / "datasets" / "webqsp_rog.jsonl"
...
def _selftest():
    ...
    print("[selftest] PASS: webqsp-kgqa-benchmark", flush=True)
...
def run() -> Dict:
    ...
    print("  WebQSP: graph-reachable(<=%d hops)=%.3f | substrate recovers answer (of reachable)=%.3f (n=%d)" % (MAX_HOPS, orc, sub, n), flush=True)
...
def verdict(r) -> Tuple[str, str]:
    ...
    if r["sub_recall"] >= 0.70:
        return ("HARD_PASS", "HARD_PASS: substrate K-hop answers real WebQSP KG-QA in >=0.70 of graph-reachable questions -- real KG-QA validated on real questions+graphs+answers. " + s)
    if r["sub_recall"] >= 0.55:
        return ("MIDDLE_BAND", "MIDDLE_BAND: WebQSP substrate recall 0.55-0.70. " + s)
    return ("HARD_FAIL", "HARD_FAIL: WebQSP substrate recall <0.55. " + s)
