"""
activation_barrier_r3_extended_grid_v2_n4096 -- PP-33 R3a: extended noise grid for nf_crit.

v1 GRID-EDGE MIDDLE result (v354): nf_crit(alpha=0.10) at boundary of 0.00..0.60 grid.
All per-seed values 0.495-0.505; b_fit~0 (near-constant curve). Ratio could not be computed.

R3a rescue: extend grid to 0.00..0.90 step 0.01 to find where nf_crit(alpha=0.10) is.
Key question: is nf_crit(alpha=0.10) in [0.60, 0.90], or is recall never below 0.50?

If nf_crit(alpha=0.10) is resolvable in [0.60, 0.90]:
  - ratio(0.05/0.10) can be computed
  - b_fit can be estimated from 5-alpha power-law fit
  - HP/MIDDLE/HF verdict can be assigned

If nf_crit(alpha=0.10) still at boundary (0.88-0.90):
  - MIDDLE_BAND: grid too narrow even at 0.90; need N-scale or lower alpha range
  - R3b (N=8192) is next

SCIENTIFIC QUESTION: Does nf_crit(alpha) vary proportionally to (alpha_c - alpha)?
Extended grid resolves the near-critical regime where prior grid was bounded.
...
FORMULA SELF-TESTS (PROT-022):
  ...
  4. At least 1 nf_crit measurement non-NaN per alpha at smoke scale.
     [EXPECTED: at least 1 valid nf_crit]

PRE-REGISTERED BANDS (same as v1; extended grid does not change hypothesis):
  HARD-PASS: power-law exponent b < 0.7 AND ratio(0.05/0.10) > 1.30 AND n_monotone >= 4/5
  MIDDLE: b in [0.7, 1.1] OR ratio in [1.10, 1.30] OR ratio=None (boundary persists)
  HARD-FAIL: b > 1.2 (super-linear) OR ratio <= 1.02 (flat)

NOTE: if ratio=None even with extended grid, MIDDLE_BAND is the correct verdict.
  The extended grid is diagnostic; bands are carried over from v1 prereg.

PROT-018: anchor has _n4096; N MUST = 4096.
PROT-022: formula self-tests above.
QUEUE: remote_cpu_queue (pure CPU; 5 alpha x 91 noise steps x 5 seeds ~3h).
TIMEOUT ESTIMATE: v1 elapsed_s = 134.4s at 61-step grid (N=4096 5-seed).
  Extended grid: 91 steps vs 61 = 1.49x more points.
  ceil(1.5 * 134.4 * (91/61)^1.0 * (5/5)) = ceil(1.5 * 134.4 * 1.492) = ceil(301) = 600s.
  With 2x margin for boundary probing at high noise fracs (denser retrieval): 1200s.
"""
import sys
...
ANCHOR_NAME = "activation_barrier_r3_extended_grid_v2_n4096"
...
if RUN_MODE == "smoke":
    ...
    N_ACTIVE = 512
    N_QUERIES = 5
    # Smoke grid extended to 0.90 step 0.05 to verify boundary detection works
    NOISE_FRACS = [round(i * 0.05, 3) for i in range(19)]  # 0..0.90 step 0.05
    ALPHA_VALUES_ACTIVE = [0.05, 0.10]
else:
    SEEDS = [7, 17, 23, 31, 41]
    N_ACTIVE = N
    N_QUERIES = 8
    # EXTENDED GRID: 0.00..0.90 step 0.01 => 91 points (vs 61 in v1)
...
def _instrumentation_selftest() -> None:
    ...
    print(f"[selftest] PASS: barrier_ratio={r:.4f}, b03={b03:.4f}, "
          f"grid_step={step_max:.4f}, grid_len={len(NOISE_FRACS)}, recall_scan ok", flush=True)
...
def measure_recall_curve(alpha: float, n_dim: int, seed: int) -> Optional[float]:
    ...
    Returns the noise fraction where recall drops below CRIT_RECALL=0.5, or None.
    """
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    if mean_ratio is None:
        return ("MIDDLE_BAND",
                ...
                f"n_valid_nf={n_valid_nf}. R3b N-scale needed. {summary}")
    ...
    if (mean_ratio > HP_RATIO_MIN and mean_b is not None and mean_b < HP_B_MAX
        ...
        return ("HARD_PASS",
                ...
                f"ratio={mean_ratio:.4f}>{HP_RATIO_MIN}. PP-33 R3 viable. {summary}")

    return ("MIDDLE_BAND", f"MIDDLE_BAND: partial signal. {summary}")
