...
ANCHOR_NAME = "substrate_trained_mini_lm_rung1_tinychar_v1"
...
HP_BPC_MAX = 2.5          # <= 2.5 nats = HARD-PASS
MID_BPC_MAX = 3.5         # <= 3.5 nats = MIDDLE
HF_BPC_MIN = 3.5          # > 3.5 nats = HARD-FAIL
HP_MIN_SEEDS = 3          # 3/3 seeds required for HP
MID_MIN_SEEDS = 2         # 2/3 seeds for MIDDLE
...
def run_one_seed(seed: int) -> dict:
    ...
    # Score
    score = lm.score_bpc(corpus_val, temperature=TEMPERATURE)
    val_bpc = score["bpc"]
    uniform_bpc = score["uniform_bpc"]
    ...
    print(
        f"[seed={seed}] val_bpc={val_bpc:.4f} uniform_bpc={uniform_bpc:.4f} "
