"""
pp56_sherman_morrison_cert_drop_n32768_v4_n32768 -- PP-56 4th-rung cross-N: SM cert-drop at N=32768.
...
CONTEXT (cycle25 v355 refill):
  ...
  PP-56 BAND-LIFT applied: 0.70-0.85 -> 0.75-0.88.
  4th rung: N=32768 to continue algebraic cross-N confirmation.

SCIENTIFIC QUESTION:
  Does Sherman-Morrison rank-1 deletion cert-drop maintain algebraic properties at N=32768?
  Theoretical cert_ratio = lam / (lam + N) = 1/32769 ~ 3.05e-05 at N=32768.
...
FORMULA SELF-TESTS (PROT-022):
  ...
  3. cert_after / cert_before ratio at N=16, lam=1: lam/(lam+N) = 1/17 ~ 0.059.
     [EXPECTED: cert_ratio < 0.2]
  4. cert retained: delete xi_0 from W at N=32. delta < 0.5.
     [INPUT: N=32, M=2] [EXPECTED: retained_delta < 0.5]
  5. Theoretical cert_ratio at N=32768: lam/(lam+N) = 1/32769 = 3.05e-05 < 0.001.
     [INPUT: lam=1.0, N=32768] [EXPECTED: theo_ratio < 0.001]

PRE-REGISTERED BANDS (PP-56 N=32768 4th-rung extension; prior N=16384 HARD_PASS):
  HARD-PASS: cert_ratio < 0.15 AND retained_delta < 0.10 (5-seed unanimous)
             => PP-56 band-lift eligible 0.75-0.88 -> 0.78-0.90 (4-rung cross-N)
  MIDDLE: cert_ratio in [0.15, 0.30] OR retained_delta in [0.10, 0.20]
  HARD-FAIL: cert_ratio > 0.30 (deletion cert algebraic scaling breaks at N=32768)

PROT-018: anchor has _n32768; N MUST = 32768.
PROT-021: seed checkpoints keyed with run_mode.
QUEUE: remote_cpu_queue (pure numpy; O(N) SM operations; ~900s FULL wall).
TIMEOUT ESTIMATE: N=16384 elapsed=259.1s (v353, 5-seed).
  O(N) scaling: ratio 32768/16384 = 2.
  ceil(1.5 * 259.1 * 2.0 * 1.0) = ceil(777) = 900s.
"""
import sys
...
ANCHOR_NAME = "pp56_sherman_morrison_cert_drop_n32768_v4_n32768"
...
# PROT-022: theoretical cert_ratio at N=32768
_THEO_CERT_RATIO_N32768 = LAM / (LAM + N)  # ~3.05e-05
assert _THEO_CERT_RATIO_N32768 < 0.001, f"theoretical cert_ratio check: {_THEO_CERT_RATIO_N32768:.8f}"
...
if RUN_MODE == "smoke":
    N_ACTIVE = 1024          # smoke at 1/32 scale
    SEEDS = [7, 17]
    M_STORE = 5
    N_TRIALS = 3
else:
...
def _instrumentation_selftest() -> None:
    ...
    # Test 5: theoretical cert_ratio at N=32768
    theo = LAM / (LAM + N)
    assert theo < 0.001, f"theoretical cert_ratio at N={N}: {theo:.8f}"

    # Test 6: at smoke scale, non-sentinel cert_ratio
    N_t3 = N_ACTIVE
    ...
    print(f"[selftest] PASS: cert_before={cb:.4f} cert_after={ca:.4f} ratio={cert_ratio:.4f} "
          f"retained_delta_N32={retained_delta:.4f} theo_ratio_N32768={theo:.8f} "
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    if cert_ratio < HP_CERT_RATIO and retained_delta < HP_RETAINED_DELTA:
        return ("HARD_PASS",
                f"HARD_PASS: cert drops to {cert_ratio:.6f}<{HP_CERT_RATIO} at N=32768; "
                f"retained cert stable (delta={retained_delta:.6f}<{HP_RETAINED_DELTA}); "
                f"5-seed unanimous. PP-56 4th-rung band-lift gate passed. {summary}")
...
metrics = {
    ...
    "theoretical_cert_ratio_n32768": float(_THEO_CERT_RATIO_N32768),
    "per_seed": [
        {"seed": r.get("seed"),
