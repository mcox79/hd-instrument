...
v2 fixes:
  ...
  1. **Iterate-trajectory cross-check (per anchor brief).** When Cap 8
     ...
     `exp_wave14_cap8_vamp_iterates_srht_hadamard_v1`), load them and
...
For each (family, alpha=1.0, seed) cell, load
  data/exp_wave14_cap8_vamp_iterates_srht_hadamard_v1/
...
Pre-reg: preregs/2026-05-24_wave14_cap12_cap8_audit_trail_pipeline_v2.md
"""
...
ITERATE_ROOT = REPO / "data" / "exp_wave14_cap8_vamp_iterates_srht_hadamard_v1"
...
def run_smoke() -> None:
    self_test()
    out_dir = get_output_dir("wave14_cap12_cap8_audit_trail_pipeline_v2_smoke")
...
def run_main() -> None:
    self_test()
    out_dir = get_output_dir("wave14_cap12_cap8_audit_trail_pipeline_v2")
