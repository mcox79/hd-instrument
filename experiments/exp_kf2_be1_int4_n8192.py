"""KF-2 BE-1 PRECISION: int4 (4-bit quantization) at N=8192.

Delegates to exp_kf2_be1_precision_sweep_n8192 with PRECISION='int4'.
int4 = 8x memory compression vs fp32. Symmetric per-tensor quantization (7 levels).
Expected: BORDERLINE -- 4-level quant may push isolation above 0.05 but below 0.10.
This is the stretch target: 8x compression with isolation intact would be commercially significant.

N-suffix: _n8192 -> production N = 8192 (PROT-018 binding).
Anchor: kf2_be1_int4_n8192
...
_drv_spec = importlib.util.spec_from_file_location("be1_drv_int4", _drv_path)
...
def get_output_dir() -> Path:
    name = os.environ.get("HDLAB_EXP_NAME", "kf2_be1_int4_n8192")
...
def _instrumentation_selftest() -> None:
    ...
    assert cell.get("precision_compression_ratio", 0) == 8.0, (
        f"int4 compression must be 8.0; got {cell.get('precision_compression_ratio')}")
    print(f"[selftest] kf2_be1_int4_n8192 PASS iso={cell['isolation_ratio']:.5f}", flush=True)
...
def main():
    ...
    print(f"[run] kf2_be1_int4_n8192 smoke={smoke} N={N_cfg} precision={PRECISION} "
    ...
    if max_iso >= _drv.HF_CONTAMINATION:
        verdict = "KF2_BE1_INT4_HARD_FAIL"
        verdict_msg = f"INT4_BREAKS_ISOLATION: max_iso={max_iso:.5f} >= {_drv.HF_CONTAMINATION}"
    elif seeds_pass >= _drv.HP_SEEDS_MIN:
        verdict = "KF2_BE1_INT4_HARD_PASS"
        verdict_msg = (f"INT4_HOLDS: {seeds_pass}/{len(seeds)} seeds pass, "
                       f"max_iso={max_iso:.5f} < {_drv.HP_ISOLATION_MAX}, "
                       f"compression={compression:.1f}x theory_bound={theory_bound:.5f}")
    else:
        verdict = "KF2_BE1_INT4_MIDDLE_BAND"
        verdict_msg = f"INT4_PARTIAL: {seeds_pass}/{len(seeds)} seeds, max_iso={max_iso:.5f}"

    elapsed = round(time.time() - t0, 2)
    summary = {
        "anchor": "kf2_be1_int4_n8192", "precision": PRECISION, "N": N_cfg, "smoke": smoke,
