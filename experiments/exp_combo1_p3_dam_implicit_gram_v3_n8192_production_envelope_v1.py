"""
combo1_p3_dam_implicit_gram_v3_n8192_production_envelope_v1 -- COMBO-1 v3 at N=8192.

Extends the GPU-fix v3 HARD_PASS at N=4096 to the N=8192 production envelope.
combo1_p3_dam_implicit_gram_v3_gpu_fix_v1_n4096 HP'd; this closes N-scaling curve.

VRAM at N=8192:
  Xi (M x N float32): for M=2*N=16384: 16384 * 8192 * 4 = 537 MB. Safe.
  G (M x M float32): 16384 * 16384 * 4 = 1.07 GB. Acceptable.
  Total: ~1.6 GB peak. Well within 8 GB.
  Note: We skip G materialization and use implicit Gram Krylov traces (matrix-free).

GPU IMPLEMENTATION (same pattern as gpu_fix_v1):
  - Xi: torch.tensor on CUDA float32.
  - All matmuls: batched torch operations.
  - Implicit Gram: kappa3 via Hutchinson on W = Xi.T @ Xi / N (matrix-free).
  - Explicit W materialization skipped; use Xi directly for MMD/retrieval.

PRE-REGISTERED BANDS (same as v3_gpu_fix HP at n4096):
...
FORMULA SELF-TESTS:
  ...
  3. GPU present + memory_allocated > 0 after tensor creation.
...
ANCHOR_NAME = "combo1_p3_dam_implicit_gram_v3_n8192_production_envelope_v1"
...
if RUN_MODE == "smoke":
    SEEDS = [7, 17]
    M_LIST = [N * 2]      # 16384 patterns at smoke
    N_PROBES_K3 = 50
    N_TEST_RETRIEVAL = 5
    N_WRITE_STEPS = [N // 4, N // 2]
else:
    SEEDS = [7, 17, 23, 31, 41]
    M_LIST = [N * 2, N * 4]  # 16384 and 32768 patterns
    N_PROBES_K3 = 200
    N_TEST_RETRIEVAL = 20
    N_WRITE_STEPS = [N // 2, N, N * 2, N * 4]
...
def compute_mmd_gpu(samples: torch.Tensor, references: torch.Tensor, n: int) -> float:
    """Squared MMD via RBF kernel (approximate). Lower = more similar distributions."""
    # Use simple cosine-based pseudo-MMD for efficiency
    ...
    # Cross-cosine similarities
    cross = torch.mm(s_norm, r_norm.t())  # (n_s, n_r)
    # Cosine MMD proxy: 1 - mean(cross)
    mmd_proxy = float(1.0 - cross.mean())
    return max(mmd_proxy, 0.0)


def hutchinson_kappa3_implicit_gpu(Xi: torch.Tensor, n: int, n_probes: int,
                                    seed: int) -> float:
    ...
    V2 = w_op(V1)
    V3 = w_op(V2)
    # kappa_3 = (V0 * V3).sum(dim=0).mean() / n
...
def _selftest_gram_diagonal():
    ...
    # G = Xi @ Xi.T / N (p=2 Gram); for p=3: G_ij = (xi_i . xi_j / N)^3 * N
    # Self-term: G_ii = (N/N)^3 * N = N for p=3. Rescaled by N: G_ii/N = 1.0.
    dot_self = float(Xi_t[0].dot(Xi_t[0])) / N_t
    assert abs(dot_self - 1.0) < 0.01, f"G_ii test: {dot_self:.4f} != 1.0"


def _selftest_kappa3_theory():
    """Tr(G_p3)/M = 1.0 for p=3 BSC Gram at small N/M.

    G_p3_ij = (xi_i . xi_j / N)^3 (element-wise cube of normalized Gram).
    G_p3_ii = (N/N)^3 = 1.0 for BSC patterns.
    Tr(G_p3) = M, so Tr(G_p3)/M = 1.0 universally.
    """
    N_t = 128
    M_t = 64
    gen = torch.Generator(device=DEVICE)
    ...
    # p=3 Gram: G_p3_ij = (xi_i . xi_j / N)^3  (element-wise cube)
    G_normalized = Xi_t @ Xi_t.t() / float(N_t)   # G_ij = xi_i.xi_j / N
    G_p3 = G_normalized.pow(3)                     # G_p3_ij = (G_ij)^3
...
def _instrumentation_selftest():
    ...
    del dummy
    print(f"[selftest] PASS: G_ii=1.0, kappa3_resc~1.0, gpu_mem_ok", flush=True)
...
def run_seed_m(seed: int, n_dim: int, M: int) -> Dict:
    ...
    # HP1: MMD -- retrieve N_TEST_RETRIEVAL patterns and compare to stored
    ...
    mmd = compute_mmd_gpu(retrieved_t, test_probes, n_dim)

    # HP2: kappa3 rescaled
    kappa3 = hutchinson_kappa3_implicit_gpu(Xi, n_dim, N_PROBES_K3, seed=seed)
    # kappa3_rescaled = kappa3 / (M/n) = kappa3 * n / M
    kappa3_resc = kappa3 * n_dim / M if M > 0 else 0.0

    # HP3: Brand refresh slope (write-time scaling)
    write_times = []
    for w_step in [s for s in N_WRITE_STEPS if s <= M]:
        t_w = time.time()
        Xi_sub = Xi[:w_step]
        # Brand-refresh proxy: batched matmul
    ...
    if len(write_times) >= 2:
        ...
        xs = [_math.log(w) for w, _ in write_times]
        ys = [_math.log(max(t, 1e-9)) for _, t in write_times]
        if xs[-1] != xs[0]:
    ...
    print(f"    [M={M}] MMD={mmd:.4f} kappa3_resc={kappa3_resc:.4f} slope={slope:.2f} "
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    if hp1 and hp2 and hp3 and hp4:
        return ("HARD_PASS", f"HARD_PASS: all 4 HP conditions at N=8192. {summary}")
