...
# ---- Arm names ----
ONESHOT = "ONESHOT_ROTATE"       # PLAIN-ROTATE reference: rotation-score fit, direct readout
HAKE = "HAKE_PHASE_MODULUS"      # THE LEVER: phase (rotation) + modulus (magnitude/hierarchy) hybrid (Zhang 2020)
...
ALL_ARMS = [ONESHOT, HAKE, ADDITIVE, POP, SCRAMBLE, RANDOM, ORACLE, DISCRETE]
STRATA = ["low", "mid", "high"]
...
# ---- HAKE (phase+modulus) pre-registered bands (picked BEFORE the run; per drill 2026-07-12) ----
HAKE_ROTATE_GAP = 0.02   # HAKE-lever WIN: HAKE (fair low+mid) hits@10 - ONESHOT_ROTATE (fair) >= this
# GUARDRAIL (drill hard-fail): the modulus term may RE-ENCODE degree/popularity. Plain-rotate's cross-channel
# geometry-vs-poprank r was ~0.31. HARD-FAIL if HAKE's |r| INCREASES materially vs plain-rotate's |r|.
R_INCREASE_EPS = 0.05    # HARD-FAIL if |r_hake| - |r_rotate| > this (modulus is substituting a degree-proxy)
R_HAKE_ABS = 0.40        # HARD-FAIL if |r_hake| > this absolute (drill: r > ~0.40 = confounded)
...
HAKE_LAMBDA_MOD = 1.0   # weight on the modulus-distance term in the HAKE score (phase+modulus mix)
HAKE_MOD_REG = 1e-3     # L2 prior on the log-modulus params (keeps softplus moduli tame; no exp blow-up)
...
def fit_kge_hake(train_edges, N, n_rel, k, device, seed, epochs,
                 transductive_extra=None, reciprocal=True, lr=ROT_LR, gamma=ROT_GAMMA,
                 n_neg=64, adv_temp=ROT_ADV_TEMP, reg_lambda=ROT_REG, batch_size=8192,
                 neg_chunk=None, ckpt=None, stop_after_epochs=None,
                 lambda_mod=HAKE_LAMBDA_MOD, mod_reg=HAKE_MOD_REG):
    """HAKE (Zhang, Cai, Zhang, Wang, AAAI 2020, arXiv:1911.09419) phase+modulus fit -- the modulus LEVER on top
    of PP-275's rotation geometry. Entities+relations get BOTH a PHASE (PHI/THETA, rotation/direction, identical
    to fit_kge_rotate) AND a log-MODULUS (LME/LMR, magnitude/hierarchy). Modulus m_* = softplus(LM_*) >= 0
    (bounded, no exp blow-up over a multi-hour CPU fit; relation modulus is multiplicative). Score
      s(h,r,t) = gamma*(1 - d_phase) - lambda_mod * d_mod
      d_phase = mean_j (1 - cos((PHI_h+THETA_r)_j - PHI_t,j))   in [0,2]  (the rotate chordal distance)
      d_mod   = mean_j (m_h*m_r - m_t)^2                                  (radial/scale distance)
    When moduli collapse (LM equal), d_mod->0 and HAKE reduces to plain rotate -> the lever's value is exactly
    the delta over ONESHOT_ROTATE. Anchor-1 recipe (self-adversarial CE + reciprocal + minibatch SGD) + LR fix,
    same as rotate. neg scored single-shot (CPU-safe route; no OOM). Returns PHI, THETA, LME, LMR detached."""
    g = torch.Generator(device="cpu").manual_seed(seed * 7919 + 11)
    ed = train_edges
    if transductive_extra is not None and transductive_extra.shape[0] > 0:
        ed = np.concatenate([train_edges, transductive_extra], axis=0)
    ...
    PHI = ((torch.rand(N, k, generator=g) * two_pi) - np.pi).to(device).requires_grad_(True)
    THETA = ((torch.rand(n_rel_eff, k, generator=g) * two_pi) - np.pi).to(device).requires_grad_(True)
    # log-modulus init near 0 (softplus(0)=0.69) so the fit STARTS at plain-rotate and earns modulus only if it helps
    LME = ((torch.rand(N, k, generator=g) * 0.2) - 0.1).to(device).requires_grad_(True)
    LMR = ((torch.rand(n_rel_eff, k, generator=g) * 0.2) - 0.1).to(device).requires_grad_(True)
    opt = torch.optim.Adam([PHI, THETA, LME, LMR], lr=lr)
    ...
    if ckpt is not None and ckpt.enabled():
        ckpt.set_fingerprint(dict(
            fn="hake", N=int(N), n_rel=int(n_rel), k=int(k), epochs=int(epochs), n_neg=int(n_neg),
            lr=float(lr), gamma=float(gamma), adv_temp=float(adv_temp), reg_lambda=float(reg_lambda),
            lambda_mod=float(lambda_mod), mod_reg=float(mod_reg),
            batch_size=int(bs), reciprocal=bool(reciprocal), seed=int(seed),
            split_hash=edges_hash(ed), device=str(device)))
        _ck = ckpt.try_load(device)
        if _ck is not None:
            start_epoch = restore_into(_ck, {"PHI": PHI, "THETA": THETA, "LME": LME, "LMR": LMR}, opt,
                                       {"gperm": gperm, "gneg": gneg}, device)
    ...
    for ep in range(start_epoch, epochs):
        ...
        for s in range(0, E, bs):
            ...
            _is_last_batch = (s + bs >= E)
            q = PHI[hb] + THETA[rb]                                   # (b,k) predicted phase
            pos_d = (1.0 - torch.cos(q - PHI[tb])).mean(dim=1)        # (b,)
            mp = sp(LME[hb]) * sp(LMR[rb])                            # (b,k) predicted modulus
            pos_dmod = (mp - sp(LME[tb])).pow(2).mean(dim=1)          # (b,)
            pos_score = gamma * (1.0 - pos_d) - lambda_mod * pos_dmod
            neg_t = torch.randint(0, N, (b, n_neg), generator=gneg).to(device)   # (b,n_neg)
            neg_d = (1.0 - torch.cos(q.unsqueeze(1) - PHI[neg_t])).mean(dim=2)   # (b,n_neg)
            neg_dmod = (mp.unsqueeze(1) - sp(LME[neg_t])).pow(2).mean(dim=2)     # (b,n_neg)
            neg_score = gamma * (1.0 - neg_d) - lambda_mod * neg_dmod
            with torch.no_grad():
                w = torch.softmax(adv_temp * neg_score, dim=1)       # self-adversarial weights (stop-grad)
            pos_loss = -torch.nn.functional.logsigmoid(pos_score)
            neg_loss = -(w * torch.nn.functional.logsigmoid(-neg_score)).sum(dim=1)
            ...
            if reg_lambda > 0.0:
                loss = loss + reg_lambda * THETA[rb].pow(2).mean()
            if mod_reg > 0.0:
                loss = loss + mod_reg * (LMR[rb].pow(2).mean() + LME[hb].pow(2).mean())
            opt.zero_grad()
            loss.backward()
            opt.step()
            if ckpt is not None and ckpt.enabled() and _is_last_batch:
        ...
        if ckpt is not None and ckpt.enabled():
            _do_stop = (stop_after_epochs is not None) and ((ep + 1) >= stop_after_epochs)
            if ((ep + 1) % ckpt.every == 0) or ((ep + 1) == epochs) or _do_stop:
                ckpt.save(ep + 1, {"PHI": PHI, "THETA": THETA, "LME": LME, "LMR": LMR}, opt,
                          {"gperm": gperm.get_state(), "gneg": gneg.get_state()})
                ckpt.write_progress(ep + 1, epochs, _last_loss)
            if _do_stop:
                break
    return (PHI.detach(), THETA.detach()[:n_rel].contiguous(),
            LME.detach(), LMR.detach()[:n_rel].contiguous())
...
def hake_direct_scores(PHI, THETA, LME, LMR, hold_edges, device, lambda_mod=HAKE_LAMBDA_MOD, chunk=SCORE_CHUNK):
    """HAKE readout: score(t) = mean_j cos((PHI_h+THETA_r)_j - PHI_t,j) - lambda_mod * mean_j (m_h*m_r - m_t)^2,
    m_* = softplus(LM_*). Higher=better. The modulus distance is expanded to matmul form
    d_mod = (||mp||^2 + ||m_t||^2 - 2 mp.m_t)/k so the (nq,N) map is never (nq,N,k). Query-chunked; CPU accumulate."""
    h = torch.from_numpy(hold_edges[:, 0]).long().to(device)
    r = torch.from_numpy(hold_edges[:, 1]).long().to(device)
    q = PHI[h] + THETA[r]                                            # (nq,k)
    Pq = torch.complex(torch.cos(q), torch.sin(q)).to(torch.complex64)
    ...
    k = PHI.shape[1]
    sp = torch.nn.functional.softplus
    m_pred = sp(LME[h]) * sp(LMR[r])                                # (nq,k)
    m_all = sp(LME)                                                 # (N,k)
    m_all_T = m_all.T                                               # (k,N)
    B = (m_all * m_all).sum(dim=1)                                  # (N,)
    nq = Pq.shape[0]; n_ent = PHI.shape[0]
    out = torch.empty((nq, n_ent), dtype=torch.float32)
    for s in range(0, nq, chunk):
        e = min(s + chunk, nq)
        sc_phase = torch.real(Pq[s:e] @ E_cT) / k                   # (b,N) cos-mean
        mp = m_pred[s:e]                                            # (b,k)
        A = (mp * mp).sum(dim=1, keepdim=True)                      # (b,1)
        d_mod = (A + B.unsqueeze(0) - 2.0 * (mp @ m_all_T)) / k     # (b,N)
        sc = sc_phase - lambda_mod * torch.clamp(d_mod, min=0.0)    # (b,N), higher=better
        out[s:e] = sc.detach().to("cpu")
        del sc, sc_phase, d_mod, A
    del Pq, E_all, E_cT, m_all, m_all_T, B
    if getattr(device, "type", "") == "cuda":
        torch.cuda.empty_cache()
    return out
...
def _fit_and_score(train_int, hold, N, n_rel, cfg, device, seed, rel_tail_freq, all_true, want_fpe=True,
    ...
    _ec()
    # HAKE_PHASE_MODULUS (the lever): SAME recipe + phase geometry, PLUS a learnable modulus term
    PHIh, THETAh, LMEh, LMRh = fit_kge_hake(train_int, N, n_rel, k, device, seed, epochs,
    ...
    for name, sc in [
        ...
        (HAKE, hake_direct_scores(PHIh, THETAh, LMEh, LMRh, hold, device)),
    ...
    del PHI, THETA, PHIh, THETAh, LMEh, LMRh, Xa, Da, PHIs, THETAs, PHIo, THETAo, PHIr, THETAr, Z, R
    if getattr(device, "type", "") == "cuda":
        torch.cuda.empty_cache()
...
def run_corpus(train_lbl, valid_lbl, test_lbl, cfg, device, seed, corpus_name, want_fpe=True, ckpt_dir=None):
    ...
    strat_hits = {}
    for name in [ONESHOT, HAKE, ADDITIVE, DISCRETE, SCRAMBLE, RANDOM, ORACLE]:
    ...
    # cross-channel independence (gold-geometry vs POP rank) -- freq-leak backdoor. Computed for BOTH
    # ONESHOT_ROTATE and HAKE so the guardrail (does the modulus term re-encode degree?) can compare them.
    def _xchan(arm):
        r_ = float("nan")
        geo = np.array([arm_scores[arm][i, int(hold[i, 2])].item() for i in range(hold.shape[0])])
        if np.std(geo) > 1e-9 and np.std(pop_rank_vec.astype(np.float64)) > 1e-9:
            r_ = float(np.corrcoef(geo, -pop_rank_vec.astype(np.float64))[0, 1])
        return r_
    xchan_r = _xchan(ONESHOT)
    xchan_r_hake = _xchan(HAKE)
    ...
    result.update(
        ...
        fair_hits={a: fair[a] for a in [ONESHOT, HAKE, ADDITIVE, POP, DISCRETE, SCRAMBLE, RANDOM, ORACLE]},
        fpe_median=fs["fpe"],
        cross_channel_geom_vs_poprank_r=(round(xchan_r, 4) if xchan_r == xchan_r else None),
        cross_channel_geom_vs_poprank_r_hake=(round(xchan_r_hake, 4) if xchan_r_hake == xchan_r_hake else None),
    )
    return result
...
def aggregate_and_verdict(per_seed, syn_comp, syn_freq):
    ...
    fair = {a: fair_mean(a) for a in [ONESHOT, HAKE, ADDITIVE, POP, DISCRETE, SCRAMBLE, RANDOM, ORACLE]}
    ...
    backdoor_hake = _mean([ps.get("cross_channel_geom_vs_poprank_r_hake") for ps in per_seed
                           if ps.get("cross_channel_geom_vs_poprank_r_hake") is not None])
    ...
    # ---- HAKE lever gates (fair low+mid stratum) ----
    hake_beats_rotate = bool(fair[HAKE] == fair[HAKE] and fair[ONESHOT] == fair[ONESHOT]
                             and (fair[HAKE] - fair[ONESHOT]) >= HAKE_ROTATE_GAP)
    hake_beats_pop = bool(fair[HAKE] == fair[HAKE] and fair[POP] == fair[POP]
                          and (fair[HAKE] - fair[POP]) >= POP_GAP)
    # ---- GUARDRAIL (drill hard-fail): the modulus term must NOT re-encode degree/popularity. Compare
    # HAKE's |cross-channel r| to plain-rotate's |r|; hard-fail on a material INCREASE or a high absolute. ----
    r_rot = abs(backdoor) if backdoor == backdoor else float("nan")
    r_hake = abs(backdoor_hake) if backdoor_hake == backdoor_hake else float("nan")
    modulus_reencodes_degree = bool(
        (r_hake == r_hake and r_rot == r_rot and (r_hake - r_rot) > R_INCREASE_EPS)
        or (r_hake == r_hake and r_hake > R_HAKE_ABS))
    g_modulus_no_leak = bool(not modulus_reencodes_degree)
    ...
    # HAKE lever WIN: modulus beats plain-rotate AND beats POP on the fair arena, controls fire, guardrail holds.
    hake_wins = bool(hake_beats_rotate and hake_beats_pop and g_scramble and g_seed_cv
                     and oracle_fires and g_modulus_no_leak and not broken)
    ...
    gates = dict(
        ...
        backdoor_r_hake=(round(backdoor_hake, 4) if backdoor_hake == backdoor_hake else None),
        hake_beats_rotate=hake_beats_rotate, hake_beats_pop=hake_beats_pop, hake_wins=hake_wins,
        margin_hake_vs_rotate_fair=(round(fair[HAKE] - fair[ONESHOT], 4)
                                    if (fair[HAKE] == fair[HAKE] and fair[ONESHOT] == fair[ONESHOT]) else None),
        margin_hake_vs_pop_fair=(round(fair[HAKE] - fair[POP], 4)
                                 if (fair[HAKE] == fair[HAKE] and fair[POP] == fair[POP]) else None),
        modulus_reencodes_degree=modulus_reencodes_degree, g_modulus_no_leak=g_modulus_no_leak,
        r_increase_eps=R_INCREASE_EPS, r_hake_abs_cap=R_HAKE_ABS,
