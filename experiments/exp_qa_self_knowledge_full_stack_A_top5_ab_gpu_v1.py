...
ANCHOR_NAME = "qa_self_knowledge_full_stack_A_top5_ab_gpu_v1"
...
def run() -> Dict:
    ...
    print("  [A=keyword UNION bge-top3 (PROD)] MACRO-F1 = %.4f | per-type: %s" % (macro, type_f1), flush=True)
    print("  [A=bge-top5 (drop keyword) (ALT)] MACRO-F1 = %.4f | per-type: %s" % (macro_alt, type_f1_alt), flush=True)
    print("  A-axis: prod=%.4f -> alt=%.4f (%+.4f) | MACRO: prod=%.4f -> alt=%.4f (%+.4f)" % (
    ...
    return {"macro_f1": round(macro, 4), "macro_f1_alt": round(macro_alt, 4), "type_f1": type_f1, "type_f1_alt": type_f1_alt,
            "a_prod": type_f1.get("A", 0), "a_alt": type_f1_alt.get("A", 0), "macro_delta": round(macro_alt - macro, 4),
            "n_qs": len(per_q), "b_questions": b_qs}


def verdict(r) -> Tuple[str, str]:
    if r.get("error"): return ("UNKNOWN", "UNKNOWN: " + r["error"])
    md = r["macro_delta"]; ad = round(r["a_alt"] - r["a_prod"], 4)
    unchanged = all(r["type_f1"].get(t) == r["type_f1_alt"].get(t) for t in r["type_f1"] if t != "A")
    s = ("MACRO prod=%.4f -> alt(bge-top5)=%.4f (%+.4f); A-axis prod=%.4f -> alt=%.4f (%+.4f); non-A-unchanged=%s; per-type prod=%s alt=%s" %
         (r["macro_f1"], r["macro_f1_alt"], md, r["a_prod"], r["a_alt"], ad, unchanged, r["type_f1"], r["type_f1_alt"]))
    if md >= 0.005:
        return ("HARD_PASS", "HARD_PASS: dropping the keyword union for bge-top-5 on the A-route lifts FULL-STACK macro (%+.4f) via A-axis (%+.4f) with no other-axis regression -- a real, shippable A-route simplification + lift on all 53 Qs. " % (md, ad) + s)
    if md <= -0.005:
        return ("HARD_FAIL", "HARD_FAIL: bge-top-5 A-route REGRESSES full-stack macro (%+.4f) -- the keyword union helps on the full benchmark despite the A-subset result; keep production A-route. " % md + s)
    return ("MIDDLE_BAND", "MIDDLE_BAND: bge-top-5 A-route is macro-neutral (%+.4f) on the full stack -- the A-subset +0.043 washes out at macro scale; a simplification, not a lift. " % md + s)
