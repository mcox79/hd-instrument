"""
q_b1_chain_depth_45_v1_n8192 -- Q-B1: heteroassoc chain at depth-45 at N=8192.

Ceiling push: depth-40 completed in Cycle 12. Extends to depth-45.
  GPU IMPLEMENTATION: H matrix (N x N float32 at N=8192): 268 MB. Safe on 8 GB GPU.
  Snapshots at depths [1, 3, 5, 10, 15, 20, 25, 30, 35, 40, 45].

PRE-REGISTERED BANDS:
  HARD-PASS: depth-5 >= 0.95 AND depth-10 >= 0.88 AND depth-20 >= 0.70
             AND depth-30 >= 0.55 AND depth-45 >= 0.40.
  HARD-FAIL: depth-5 < 0.80 OR depth-10 < 0.65 OR depth-20 < 0.40 OR depth-45 < 0.20.
  MIDDLE: depth-45 in [0.30, 0.40) while earlier depths meet HP.
  Calibration: depth-40 completed; conservative HP at depth-45 = 0.40.

FORMULA SELF-TESTS:
  1. depth-45 chain retrieval non-NaN at N=128.
...
ANCHOR_NAME = "q_b1_chain_depth_45_v1_n8192"
...
CHAIN_DEPTH = 45
SNAPSHOT_DEPTHS = [1, 3, 5, 10, 15, 20, 25, 30, 35, 40, 45]
...
def _instrumentation_selftest():
    ...
    assert not (cos != cos), f"depth-45 retrieval is NaN at N={N_t}"
    ...
    del H
    print(f"[selftest] PASS: depth-45 chain non-NaN N={N_t}, self-cosine=1.0, "
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    summary = (f"d5={d5:.4f}(HP>={HP_D5} HF<{HF_D5}) "
               ...
               f"d45={d45:.4f}(HP>={HP_D45} HF<{HF_D45}) "
               f"n_seeds={len(results)}")

    if d5 < HF_D5 or d10 < HF_D10 or d20 < HF_D20 or d45 < HF_D45:
    ...
    if hp5 and hp10 and hp20 and hp30 and hp45:
        return ("HARD_PASS", f"HARD_PASS: all depth thresholds met at N=8192 depth-45. {summary}")
    n_hp = sum([hp5, hp10, hp20, hp30, hp45])
    if n_hp >= 4:
