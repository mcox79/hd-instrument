"""Continual editing at over-capacity M=2N - combines yc (continual) + Bet 2 v2 (over-cap).

yc tested continual editing at M=N (HOLDS to 2000). yh tested edit-then-query
at M=2N (KERDOCK_PASS, one-shot edits). zh tests SEQUENTIAL editing at M=2N.

Pre-reg: preregs/2026-05-21_wave14zh_continual_overcap.md
"""
...
def compute_verdict(summary):
    ...
    if "kerdock" not in arms:
        return ("CONTINUAL_OC_INCONCLUSIVE", "Missing.")
    kerdock = arms["kerdock"]
    if not kerdock.get("per_edit_trajectory"):
        return ("CONTINUAL_OC_INCONCLUSIVE", "Missing traj.")
    ...
    if k_ok and not c_ok:
        return ("CONTINUAL_OC_KERDOCK_HOLDS",
                f"At M=2N, Kerdock holds 100 sequential edits: min_edited="
                f"{kerdock['min_edited_acc']:.3f}, min_kept={kerdock['min_kept_acc']:.3f}. "
                f"Continual editing at over-capacity works with structured keys.")
    if k_ok and c_ok:
        return ("CONTINUAL_OC_BOTH_HOLD",
                f"Both arms hold at M=2N with 100 edits. Correlated didn't fail here.")
    return (f"CONTINUAL_OC_KERDOCK_FAILS_AT_{k_fail}",
            f"Kerdock fails at M=2N continual editing at edit {k_fail}. "
            f"Combination of over-capacity AND continual stress breaks the substrate.")
...
def self_test_verdict():
    ...
    cases = [
        ...
        ({"by_arm": {"kerdock": mk(0.98), "correlated": mk(0.98)}},
         "CONTINUAL_OC_BOTH_HOLD"),
        ({"by_arm": {"kerdock": mk(0.5, fail_e=50), "correlated": mk(0.5, fail_e=2)}},
         "CONTINUAL_OC_KERDOCK_FAILS_AT_50"),
...
def run_experiment(smoke):
    ...
    config = {"mode": "smoke" if smoke else "full", "N": 1024 if smoke else 4096,
              "M_stored": 2048 if smoke else 8192,  # 2N
...
def run_smoke():
    out_dir = get_output_dir("wave14zh_continual_overcap_smoke")
...
def run_main():
    out_dir = get_output_dir("wave14zh_continual_overcap")
