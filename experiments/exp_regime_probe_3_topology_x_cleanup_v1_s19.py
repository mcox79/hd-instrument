"""regime_probe_3_topology_x_cleanup_v1 sibling seed=7.

Third probe in the Stage 1 Regime Map arc (USER 2026-07-03). TOPOLOGY (F fan-in)
x CLEANUP_MECHANISM cross-term; STORAGE fixed = SHARDED (no cross-term with
STORAGE; Probe 1 owns that pair).

Sweep FULL:  3 mech x 4 F x 3 M x 2 corr = 72 + 1 BUNDLED PC = 73 pts/seed.
Sweep SMOKE: 3 mech x 2 F x 1 M x 1 corr = 6 + 1 BUNDLED PC = 7 pts/seed.
...
CARDINALITY_OK_FULL: 73 phase points per seed
...
SEED = 19
ANCHOR_NAME = f"regime_probe_3_topology_x_cleanup_v1_s{SEED}"
...
CONFIG_VERSION = (
    ...
    f"regime=sharded_FHRR_chain_composition_option_Y_3,"
    f"cleanup_mechanisms=[modern_hopfield,iterative_cosine,soft_energy_attractor],"
    f"F=[1,2,4,8],M=[200,800,3200],N=4096(fixed),L=2(fixed),corr=[0.20,0.45],"
    f"SEED={SEED},mode={RUN_MODE},"
    f"BUNDLED_PC_regime=fixed_M800_N4096_F1_L2_corr0.20_iterative_cosine,"
    f"beta=8.0,alpha_soft=0.5,"
    f"expected_n_full=73,expected_n_smoke=7,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "regime_probe_3_topology_x_cleanup_v1_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: Exception) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "regime_probe_3_topology_x_cleanup_v1_import_crash",
        }
        tmp = out_dir / "metrics.json.tmp"
        final = out_dir / "metrics.json"
...
def main() -> int:
    ...
    from experiments._regime_probe_3_topology_x_cleanup_v1_core import (
    ...
    final["_hardening_marker"] = "regime_probe_3_topology_x_cleanup_v1_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = ("synthetic_sharded_fhrr_chain_composition_"
                                   "topology_x_cleanup_probe_v1")
