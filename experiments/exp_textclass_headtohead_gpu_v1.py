"""
exp_textclass_headtohead_gpu_v1.py -- text-classification head-to-head: substrate vs LLM on AG-News -- GPU.
...
OUT = REPO / "data" / "exp_textclass_headtohead_gpu_v1"
def _feats(txt):
...
def main():
    ...
    for e in test:
        msgs = [{"role": "user", "content": "Topic of this news? " + e["text"][:400] + "\nAnswer with exactly one: " + ", ".join(LABELS) + "."}]
    ...
    msg = "%s: substrate text-classifier acc=%.3f (%.5fs/item) vs Qwen-0.5B acc=%.3f (%.3fs/item) -- %s. Substrate %dx faster, tiny. AG-News 4-class." % (
        verdict, sub_acc, sub_lat, llm_acc, llm_lat, "SUBSTRATE>=LLM" if win else "LLM>", int(llm_lat / max(sub_lat, 1e-6)))
    print("\n[VERDICT] " + msg, flush=True)
    OUT.mkdir(parents=True, exist_ok=True)
    json.dump({"anchor_name": "textclass_headtohead_gpu_v1", "verdict": verdict, "verdict_msg": msg,
...
if "--self-test" in sys.argv:
    assert "BIAS" in _feats("hi there"); print("[selftest] PASS: textclass-headtohead", flush=True); sys.exit(0)
main()
