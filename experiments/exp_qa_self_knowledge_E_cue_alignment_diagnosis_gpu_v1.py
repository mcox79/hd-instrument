"""
exp_qa_self_knowledge_E_cue_alignment_diagnosis_gpu_v1.py -- is the E-axis (methodology) residual a bge CUE-ALIGNMENT problem? (GPU/bge) -- GPU.

ROUTING: the two-vector trilogy concluded every retrieval path is ultimately QUERY-SNR-bound (cue-to-target alignment), and
  that the FREE-TEXT E-axis methodology route (which also leans on through the bge semantic index, NOT the atom-keyed composite) should be limited by
  bge cue quality. The trilogy ROUTED this to Testbed but never MEASURED it. This cell measures it directly: for each E-axis methodology question with gold, encode the query topic with bge, and compute the cosine to the GOLD atom's bge semantic vector
  + the gold atom's RANK in the semantic ordering + whether gold is inside the production top-k (k=3). If A-gold cosines are
  high and recall@k is high, the E-axis residual is NOT cue-alignment (it is elsewhere -- e.g. keyword-UNION precision); if
...
PRE-REGISTERED (descriptive diagnosis, decisive either way):
  - "CUE-BOUND" if median best-gold bge cosine < 0.40 OR recall@3 < 0.60 -> E-axis is query-encoding-limited; fix is bge cue
    quality, not the route/index. (HARD_PASS: clean actionable diagnosis -> query encoding is the lever.)
  - "CUE-ALIGNED" if median best-gold cosine >= 0.50 AND recall@3 >= 0.75 -> gold IS reachable by bge; the E-axis residual is a
...
ANCHOR_NAME = "qa_self_knowledge_E_cue_alignment_diagnosis_gpu_v1"
...
def _selftest():
    ...
    print("[selftest] PASS: qa_self_knowledge_E_cue_alignment_diagnosis_gpu_v1", flush=True)
...
def run() -> Dict:
    ...
    if SMOKE: aqs = aqs[:5]
    if not aqs:
        return {"error": "no_E_gold_questions"}
    ...
    if not scored:
        return {"error": "no_gold_in_index", "n_E": len(aqs)}
    ...
    print("  E-axis methodology answerable-with-gold: %d (gold-in-index: %d)" % (len(aqs), len(scored)), flush=True)
    ...
    return {"n_E": len(aqs), "n_scored": len(scored), "median_best_cos": round(med_cos, 4), "mean_best_cos": round(mean_cos, 4),
...
def verdict(r) -> Tuple[str, str]:
    if r.get("error"):
        msg = r["error"] + " " + str(r.get("note", r.get("n_E", "")))
    ...
    if cue_bound:
        return ("HARD_PASS", "HARD_PASS (CUE-BOUND diagnosis): the E-axis IS query-encoding-limited -- E-gold methodology atoms have low bge cue alignment (median cos %.2f) and/or fall outside top-%d (recall@%d %.2f). The lever is bge CUE QUALITY (query expansion/reformulation), NOT the route or index. Confirms the two-vector trilogy's free-text-path prediction. " % (mc, r["K"], r["K"], rk) + s)
    if cue_aligned:
        return ("HARD_PASS", "HARD_PASS (CUE-ALIGNED diagnosis): E-gold IS reachable by bge (median cos %.2f, recall@%d %.2f) -- the E-axis residual is NOT cue alignment but a downstream RANKING / keyword-UNION-precision problem. Lever is the route's selection/fusion, not the cue. " % (mc, r["K"], rk) + s)
