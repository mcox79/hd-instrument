"""
exp_g7_e5_large_geometry_capacity_v1 -- Batch G7 (E5-large encoder candidate) -- GPU.
...
ANCHOR_NAME = "g7_e5_large_geometry_capacity_v1"
...
def _selftest():
    ...
    print("[selftest] PASS: g7-e5", flush=True)
...
def verdict(r) -> Tuple[str, str]:
    pr = r["PR"]; rho = r["rho_eff"]; cap = r["pinv_cap"]
    summary = "PR=%.1f rho_eff=%.3f pinv_cap=%d D=%d" % (pr, rho, cap, r["D"])
    geom_ok = pr > 40 and rho < 0.35
    if pr > 120 and rho < 0.20 and cap > 200:
        return ("HARD_PASS", "HARD_PASS: E5-large passes geometry (PR>120,rho<0.20) AND pinv_cap>200 -- third production encoder candidate. " + summary)
    if geom_ok and cap >= 80:
        return ("MIDDLE_BAND", "MIDDLE_BAND: E5 geometry passes; cap 80-200. " + summary)
    return ("HARD_FAIL", "HARD_FAIL: E5 geometry fails (PR<40 or rho>0.35) or cap<80. " + summary)
