"""Continual editing at extreme over-capacity M=4N - extends zh.

zh tested M=2N continual editing. yt tested M=4N edit-then-query (one-shot).
zi tests SEQUENTIAL editing at M=4N - combination of extreme over-capacity
AND continual stress.

Pre-reg: preregs/2026-05-21_wave14zi_continual_4N.md
"""
...
def self_test_verdict():
    ...
    cases = [
        ({"by_arm": {"kerdock": mk(0.98), "correlated": mk(0.5, fail_e=2)}},
         "CONTINUAL_4N_KERDOCK_HOLDS"),
        ({"by_arm": {"kerdock": mk(0.98), "correlated": mk(0.98)}},
         "CONTINUAL_4N_BOTH_HOLD"),
...
def run_experiment(smoke):
    ...
    config = {"mode": "smoke" if smoke else "full", "N": 1024 if smoke else 4096,
              "M_stored": 4096 if smoke else 16384,  # 4N
...
def run_smoke():
    out_dir = get_output_dir("wave14zi_continual_4N_smoke")
...
def run_main():
    out_dir = get_output_dir("wave14zi_continual_4N")
