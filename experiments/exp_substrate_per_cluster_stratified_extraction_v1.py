...
ANCHOR_NAME = "substrate_per_cluster_stratified_extraction_v1"
...
def run_seed(seed) -> Dict:
    ...
    for sp in SPEEDUPS:
        budget = max(k, len(idx) // sp)                               # keep ~budget tokens, stratified across clusters
        per = max(1, budget // k); keep = np.zeros(len(idx), bool)
        for c in range(k):
            ci = np.where(codes == c)[0]
            if len(ci):
                top = ci[np.argsort(-norms[ci])[:per]]; keep[top] = True   # top-norm WITHIN cluster
...
def verdict(ps) -> Tuple[str, str]:
    ...
    best = max((v["speedup"] for k, v in agg.items() if v["coverage"] >= 0.95), default=0.0)
    summary = "stratified: %s" % {k: (round(v["coverage"], 3), round(v["speedup"], 0)) for k, v in agg.items()}
    if best >= 100:
        return ("HARD_PASS", "HARD_PASS: per-cluster stratified extraction holds >=0.95 coverage at >=100x speedup -- rescues the extraction speedup story. " + summary)
    if best >= 10:
        return ("MIDDLE_BAND", "MIDDLE_BAND: >=0.95 coverage at 10-100x speedup. " + summary)
    return ("HARD_FAIL", "HARD_FAIL: cannot hold >=0.95 coverage at >=10x speedup. " + summary)
