"""
pp48_nkt_cross_n_depth19_v1_n8192 -- PP-48 NKT cross-N: depth-19 at N=8192.

Cross-N envelope extension: depth-17 HARD_PASS at N=8192 (just completed).
  depth-19 confirmed at N=4096 (just completed). Now test depth-19 at N=8192.
  Same sampled-leaf NKT design, same K_FORBIDDEN=100.
  alpha_total = (10 pos + 100 forbidden) / 8192 = 0.0134. Even safer sub-capacity.

  Enables deeper cross-N band-lift: if depth-19 holds at BOTH N=4096 AND N=8192,
  NKT row gets the strongest depth+N dual endorsement yet (d-17 x N-8192 already HP;
  d-19 x N-8192 is the next envelope expansion).

PRE-REGISTERED BANDS (cross-N extension of depth-19 at N=4096 HP):
  ...
  Calibration: N=4096 depth-19 HARD_PASS; bands unchanged for cross-N.
  Note: depth-17 at N=8192 just HARD_PASS; depth-19 at N=8192 is next rung.
  Sampled-leaf design: capacity is O(K) not O(2^NKT_DEPTH); N-scaling not a ceiling risk.
...
FORMULA SELF-TESTS:
  ...
  2. alpha_total check: (K_POS + K_FORBIDDEN) / N < 0.138.
     [INPUT: K_POS=10, K_FORBIDDEN=100, N=8192] [EXPECTED: 110/8192=0.0134 < 0.138]
...
ANCHOR_NAME = "pp48_nkt_cross_n_depth19_v1_n8192"
...
def _instrumentation_selftest():
    ...
    print(f"[selftest] PASS: total_tree={TOTAL_NKT_FULL}, capacity_ok alpha={alpha_t:.4f}, "
          f"gpu_mem_ok depth={NKT_DEPTH} N={N} K_POS={K_POS} K_FORBIDDEN={K_FORBIDDEN}", flush=True)
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    if hp1 and hp2:
        return ("HARD_PASS", f"HARD_PASS: both HP at NKT depth-19 cross-N N=8192. {summary}")
    if hp1 or hp2:
