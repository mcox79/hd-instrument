"""substrate_refuse_gate_v6_conformal_v1 sibling seed=7.
...
Score-based split-conformal: tau = alpha-quantile of cal-set in-KB max_sim
scores. Decouples calibration from query. No streaming update.

4 ARMS (must differ; mechanism-hash distinct):
    ARM_FIXED_BASELINE : tau=0.40 (v2 CG reproducer; positive control)
    ARM_CONFORMAL_10   : tau = P10 of cal-set in-KB @ moderate
    ARM_CONFORMAL_20   : tau = P20 of cal-set in-KB @ moderate
    ARM_CONFORMAL_REGIME : per-regime tau via noise-matched cal set
...
SEED = 7
ANCHOR_NAME = "substrate_refuse_gate_v6_conformal_v1_seed_%d" % SEED
...
CONFIG_VERSION = (
    "ANCHOR=%s,arms=[FIXED_BASELINE,CONFORMAL_10,CONFORMAL_20,CONFORMAL_REGIME],"
    "regimes=[clean,moderate,heavy],bands=[in_kb,borderline,ood],"
    "cal_size=50(25in+25ood),V_REL=256,SEED=%d,mode=%s,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v6_refuse_gate_conformal_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v6_refuse_gate_conformal_import_crash",
        }
...
def main() -> int:
    ...
    from experiments._substrate_refuse_gate_v6_conformal_v1_core import (
    ...
    final["_hardening_marker"] = "v6_refuse_gate_conformal_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = ("synthetic_substrate_refuse_gate_v6_conformal_"
                                   "4_arm_3_regime_3_band_calset50_v1")
