"""PHASE REGION D -- KF-1 variant: M_frac=12, beta=64 at N=4096.

Delegates to exp_phase_region_cd_v1_n4096 for Region D only (M > M_c, beta > beta_c).
KF-1 labeling: Kerdock key-value substrate.

Region D: M_frac=12 (overcapacity; M=49152, above substrate interference threshold),
beta=64 (well above beta_c~10-16).
Expected: retention significantly below 1.0 (overcapacity interference persists despite high beta).
Purpose: confirm that M_c boundary holds even at high beta (beta cannot rescue overcapacity).

N-suffix: _n4096 -> production N = 4096 (PROT-018 binding).
Anchor: region_d_kf1_n4096_beta64_mfrac12
...
_drv_spec = importlib.util.spec_from_file_location("phcd_drv_rd_kf1", _drv_path)
...
REGION  = "region_D"
M_FRAC  = _drv.REGION_D_MFRAC   # 12.0
BETA    = _drv.BETA_HIGH         # 64.0

HP_RET_MAX = _drv.HP_RETENTION_D_MAX    # 0.30
HF_RET_MIN = _drv.HF_RETENTION_D_MIN   # 0.60
HP_SEEDS   = _drv.HP_SEEDS_D_MIN        # 3


def get_output_dir() -> Path:
    name = os.environ.get("HDLAB_EXP_NAME", "region_d_kf1_n4096_beta64_mfrac12")
...
def _instrumentation_selftest() -> None:
    ...
    print(f"[selftest] region_d_kf1_n4096_beta64_mfrac12 PASS "
...
def main():
    ...
    print(f"[run] region_d_kf1_n4096_beta64_mfrac12 smoke={smoke} N={N_cfg} "
    ...
    seeds_pass = sum(1 for r in rets if r < HP_RET_MAX)

    if mean_ret >= HF_RET_MIN:
        verdict = "REGION_D_KF1_HARD_FAIL"
        verdict_msg = (f"OVERCAPACITY_DOESNT_HURT: mean_ret={mean_ret:.4f} >= {HF_RET_MIN} "
                       f"at M_frac={M_FRAC} beta={BETA} -- unexpected high retention above M_c")
    elif seeds_pass >= HP_SEEDS:
        verdict = "REGION_D_KF1_HARD_PASS"
        verdict_msg = (f"M_C_BOUNDARY_HOLDS: {seeds_pass}/{len(seeds)} seeds "
                       f"mean_ret={mean_ret:.4f} < {HP_RET_MAX} at beta={BETA}")
    else:
        verdict = "REGION_D_KF1_MIDDLE_BAND"
        verdict_msg = f"PARTIAL: {seeds_pass}/{len(seeds)} seeds mean_ret={mean_ret:.4f}"

    elapsed = round(time.time() - t0, 2)
    summary = {
        "anchor": "region_d_kf1_n4096_beta64_mfrac12", "N": N_cfg, "smoke": smoke,
