...
ANCHOR_NAME = "phase4b_svamp_richfeat_cpu_v1"
...
def _feats(txt, a, b):
    """discriminative features: unigrams + bigrams + number-order + cue words + richer structure (no full parser)."""
    ...
    # RICHER: noun following each number (the entity it modifies) + question-target after how many/much
    toks = low.split()
    for k, w in enumerate(toks):
        if re.match(r"\d", w.replace("$", "").replace(",", "")):
            if k + 1 < len(toks): fs.add("nN:" + re.sub(r"[^a-z]", "", toks[k + 1]))   # number's noun
            win = " ".join(toks[max(0, k - 2):k + 3])
            for vb in ("buy", "sell", "give", "get", "lose", "eat", "add", "remove", "find", "make", "put", "take", "cost"):
                if vb in win: fs.add("nv:" + vb)
    m = re.search(r"how (many|much) ([a-z]+)", low)
    if m: fs.add("qtgt:" + m.group(2))                       # question target entity
    # does the question target a noun that appears with one of the numbers? (asks for a sub-quantity -> often div/sub)
    if m and ("nN:" + m.group(2)) in fs: fs.add("qtgt_matches_number")
    fs.add("BIAS")
    return fs
def _selftest():
    assert OPS["DIV_ba"](Fraction(2), Fraction(290)) == 145
    print("[selftest] PASS: phase4b-svamp-richfeat", flush=True)
_selftest()
if _ARGS.self_test:
    sys.exit(0)
def _load():
...
def run() -> Dict:
    rng = np.random.default_rng(int(os.environ.get("HDLAB_SEED", "1011")))
    try:
        train, test = _load()
...
def verdict(r) -> Tuple[str, str]:
    ...
    if a >= 0.30:
        return ("HARD_PASS", "HARD_PASS: discriminative weight-learning reaches >=0.30 on SVAMP -- richer-feature discriminative weighting BREAKS the bag-of-words 0.110 plateau; the missing mechanism is feature weighting, not the substrate paradigm. " + s)
    if a >= 0.20:
