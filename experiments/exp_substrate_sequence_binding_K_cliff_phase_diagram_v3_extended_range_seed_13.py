"""substrate_sequence_binding_K_cliff_phase_diagram_v3_extended_range sibling seed=13.
...
SAT (~60%); v3 shifts K UP: {200, 500, 1000, 2000, 4000, 8000}.

DIFFERENCES vs v2:
  - K range EXTENDED: dropped K={20,50,100} (universally SAT); added
    K={2000,4000,8000} (predicted to escape SAT at N=16384).
  - Same 72-pt grid cardinality; same arms; same bands; same HP gate.
  - SMOKE includes SCALE preview at K=8000/N=16384 (DISCRIMINATOR-MUST-
    SURVIVE-SCALE gate). Smoke HARD_FAIL if all K>=2000 corners saturate.

CHUNKED architecture: one seed per sibling.
Sibling pair: seed_7, seed_19.

PRE-REG: preregs/2026-07-01_substrate_sequence_binding_K_cliff_phase_diagram_v3_extended_range.md
...
ASCII-only.
Author: hdi_exp_dev 2026-07-01 (Opus 4.7 1M, agent-spawn) v3 extended-range MM->CG lift
"""
...
SEED = 13
ANCHOR_NAME = f"substrate_sequence_binding_K_cliff_phase_diagram_v3_extended_range_seed_{SEED}"
...
CONFIG_VERSION = (
    ...
    f"K=[200,500,1000,2000,4000,8000],N=[2048,4096,8192,16384],"
    f"Q=[1,2,4],arms=[SUBSTRATE,RANDOM,SHUFFLE],"
    f"V_ITEMS=8500,V_POS=8500,n_queries_full=100,n_queries_smoke=4,"
    ...
    f"SCALE_GATE_K_ge_2000_min_escapes_2"
)
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v3_seqbind_Kcliff_extended_range_chunked",
        }
        if extra:
            m.update(extra)
        # ATOMIC-FINAL-METRICS-WRITE (META_RULE_AH: tmp_replace)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v3_seqbind_Kcliff_extended_range_import_crash",
        }
        tmp = out_dir / "metrics.json.tmp"
        final = out_dir / "metrics.json"
...
def main() -> int:
    ...
    from experiments._substrate_sequence_binding_K_cliff_phase_diagram_v3_extended_range_core import (
        ...
        check_smoke_discriminator_survives_scale, BAND_SAT,
    )
    ...
    final["_hardening_marker"] = "v3_seqbind_Kcliff_extended_range_chunked"
    final["backend"] = backend
    final["seed"] = SEED

    # SMOKE: apply DISCRIMINATOR-MUST-SURVIVE-SCALE gate
    if SMOKE_MODE and per_seed:
        smoke_pts = []
        for body in per_seed.values():
            smoke_pts.extend(body.get("phase_map", []))
        scale_ok, scale_msg = check_smoke_discriminator_survives_scale(smoke_pts)
        final["scale_gate_pass"] = scale_ok
        final["scale_gate_msg"] = scale_msg
        if not scale_ok:
            # OVERRIDE any HARD_PASS: smoke discriminator failure blocks full dispatch
            final["verdict"] = "HARD_FAIL"
            final["verdict_msg"] = f"HARD_FAIL_SMOKE | {scale_msg}"
            final["summary"] = final["verdict_msg"]
            final["verdict_tag"] = "DISCRIMINATOR_NOT_SURVIVING_SCALE"
...
if __name__ == "__main__":
    try:
        ...
        raise                               # META_RULE §8: normal-path preserve
    except KeyboardInterrupt:
        raise
    except Exception as e:                  # NOT BaseException per META_RULE §8
