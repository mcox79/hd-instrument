...
def verdict(ps) -> Tuple[str, str]:
    wc = float(np.mean([p["by_D"]["D%d" % D_GRID[-1]]["whitened_cap"] for p in ps])); rc = float(np.mean([p["by_D"]["D%d" % D_GRID[-1]]["raw_cap"] for p in ps]))
    summary = "at N_sub=0 dim-expanded MiniLM: whitened_cap=%.0f raw_cap=%.0f facts" % (wc, rc)
    if wc >= 6000:
        return ("HARD_PASS", "HARD_PASS: dim-expanded MiniLM substrate stores >=6000 facts at N=16384 -- production-class real-encoder capacity. " + summary)
    if wc >= 3000:
        return ("MIDDLE_BAND", "MIDDLE_BAND: 3000-6000 facts at N=16384. " + summary)
    return ("HARD_FAIL", "HARD_FAIL: <3000 facts at N=16384 (real-encoder capacity below production bar). " + summary)
