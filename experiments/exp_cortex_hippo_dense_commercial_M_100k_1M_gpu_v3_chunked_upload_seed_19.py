"""cortex_hippo_dense_commercial_M 100k-1M validation cell v3 -- seed_19.

v3 FIX for v2 PRE_UPLOAD_MEMORY_ABORT: v2 chunked the upload but still required
full-M keys resident on GPU (M*N*key_bytes persistent = 8.2 GB at M=500k FP16).
v3 keeps keys/vals CPU-resident and streams per-chunk to GPU. Peak GPU is
M-INDEPENDENT: ~45 MB regardless of M.

v2 crash: MEASURED@data/exp_cortex_hippo_dense_commercial_M_100k_1M_gpu_v2_seed_19
/metrics.json:verdict_msg = "RuntimeError: PRE_UPLOAD_MEMORY_ABORT: estimated
peak 8485.3 MB > 6 GB HF gate (arm=ARM_STD, M=500000, N=8192, V=256)".

See core `_substrate_cortex_hippo_dense_commercial_M_100k_1M_gpu_v3_chunked_upload_core.py`.
...
- discriminator survives scale (smoke at FULL_N=8192 M=100k GPU streaming preview)
...
- Selftest: streaming-vs-chunked FP16 numerical parity verified
...
ANCHOR_NAME = "cortex_hippo_dense_commercial_M_100k_1M_gpu_v3_chunked_upload_seed_19"
SEED_THIS_CHUNK = 19
...
CONFIG_VERSION = (
    ...
    f"discipline=v3_streaming_cpu_resident_keys_M_independent_gpu_footprint"
)
...
def run_seed(seed: int, out_dir: Path) -> Dict:
    ...
    if RUN_MODE == "smoke" and RUN_FULL_N_PREVIEW and PREVIEW_M is not None:
        print(
            ...
            f"N_c={N_CORTEX_FULL} V={V_DIM_FULL} (v3 streaming smoke gate)...",
            flush=True,
        )
...
def _main() -> None:
    ...
    if RUN_MODE == "full" and USE_TORCH and _CUDA_AVAILABLE:
        ...
        if max_peak_mb < 20.0:
            verdict_msg = (
                f"WARN_GPU_UNDERUTIL: max gpu_mem_peak_mb={max_peak_mb:.1f} < 20MB. "
                + verdict_msg
            )
    ...
    metrics = {
        ...
        "summary": (
            ...
            f"backend={COMPUTE_BACKEND} int8_keys={USE_INT8_KEYS} v3_streaming"
        ),
        ...
        **crlb_fields,
        ...
        "v3_fix_applied": "cpu_resident_keys_vals_gpu_streaming_per_chunk",
        "v2_root_cause": (
            "chunked UPLOAD but full M*N keys persistent on GPU = 8.2 GB at "
            "M=500k FP16; v2 correctly aborted via PRE_UPLOAD_MEMORY_ABORT gate"
        ),
        "cell_chunked": True,
