...
ANCHOR_NAME = "ner_4type_conll_cpu_v1"
...
def run() -> Dict:
    ...
    gap = f1 - F1_18TYPE
    print("  NER-4TYPE-CONLL: F1=%.4f (P=%.3f R=%.3f) | vs 18-type 0.5817 -> gap=%+.4f | %d coarse-tags, train=%d test=%d" %
          (f1, prec, rec, gap, T, len(train), len(test)), flush=True)
    return {"f1": round(f1, 4), "f1_4type": round(f1, 4), "f1_18type": F1_18TYPE, "gap_vs_18type": round(gap, 4),
            "prec": round(prec, 3), "rec": round(rec, 3), "n_train": len(train), "n_tags": T}


def verdict(r) -> Tuple[str, str]:
    if r.get("error"): return ("UNKNOWN", "UNKNOWN: " + r["error"])
    f1 = r["f1_4type"]; gap = r["gap_vs_18type"]
    s = "4-type CoNLL-equivalent F1=%.4f vs 18-type 0.5817 (gap=%+.4f, P=%.3f R=%.3f, %d tags, train=%d)" % (f1, gap, r["prec"], r["rec"], r["n_tags"], r["n_train"])
    if f1 >= 0.70:
        return ("HARD_PASS", "HARD_PASS: 4-type CoNLL-equivalent F1>=0.70 -- much of the 0.58 18-type cost was 18-WAY DIFFICULTY; the substrate NER is competitive on a CoNLL-comparable scheme. The note's 0.65 CoNLL target was apples-to-oranges vs 18-type. Re-baseline NER on coarse scheme. " + s)
    if f1 >= 0.62:
        return ("MIDDLE_BAND", "MIDDLE_BAND: 4-type F1 0.62-0.70 -- coarsening helps but a real feature gap remains; proceed Path 2 Brown-cluster features. " + s)
    return ("HARD_FAIL", "HARD_FAIL: 4-type F1 <0.62 -- coarsening does not rescue; GENUINE FEATURE GAP. Build Path 2 substrate Brown-cluster features. " + s)
