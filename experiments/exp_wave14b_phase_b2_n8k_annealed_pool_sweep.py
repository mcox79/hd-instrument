"""Wave 14.B Phase B.2 at N=8192 with annealed BETA and pool size sweep.

Combines the three day-1 findings into one production-validation sweep:
1. N=8192 (deployment-realistic substrate size)
2. Annealed BETA_RETRIEVAL = beta_0 * sqrt(log P / log P_0)
3. Pool size in {1024, 4096, 16384}

If the substrate behaves cleanly across all three axes simultaneously,
the deployment story is fully characterized. If something breaks at
the join, we learn a real constraint.

Parameter variant: same architecture as previous experiments, just
combined config.
"""
...
def run_one_pool_size(pool_size, byte_atoms, pos_atoms, target_pos,
    ...
    return {"pool_size": pool_size, "pool_used": pool_used,
           "N": N, "beta_for_this_P": beta_for_this_P,
...
def main():
    _say(f"Wave 14.B Phase B.2 N=8192 + annealed BETA + pool sweep")
    _say(f"  N={N}, BYTE_BETA={BYTE_BETA}")
    ...
    _say(f"\n========= COMPARE TO N=4096 ANNEALED =========")
    _say(f"  N=4096 annealed reference:")
    _say(f"    P=1024:  post C1=4.3352, C2=4.3354")
    _say(f"    P=4096:  post C1=4.1821, C2=4.1799")
    _say(f"    P=16384: post C1=4.1190, C2=4.1130")
    _say(f"  Expect N=8192 to show similar or better BWT (more substrate dim).")

    out_dir = Path(__file__).resolve().parent.parent / "data" / "exp_wave14b_phase_b2_n8k_annealed_pool_sweep"
    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / "metrics.json").write_text(json.dumps(
        {"N": N, "BYTE_BETA": BYTE_BETA, "BETA_RETRIEVAL_BASE": BETA_RETRIEVAL_BASE,
