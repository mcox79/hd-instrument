"""substrate_cross_modal_binding_3rd_modality_v1 sibling seed=19.

Stage 3 cross-modal binding 3-modality extension (visual+auditory+tactile).
Composes on cross-modal CG (2-mod visual+auditory chain-graded 2026-06-28).

Phase-diagram sweep over (K, N, bind_mechanism) testing whether HRR-bind
cross-modal mechanism survives 3-way conjunction or saturates.

CHUNKED architecture: one seed per sibling file.
Sibling pair: seed_7 (landed HARD_PASS), seed_13 (this batch).

DISCRIMINATOR ARMS: BIND_3MOD / NO_BIND_BASELINE / TWO_MOD_BIND_CONTROL
SWEEP: K in {10,50,100,500,1000} x N in {2048,4096,8192}
       x mech in {HRR_bind3, sum_then_query, position_key_bind3} = 45 pts
CPU-eligible (numpy/torch.cpu) per task spec.

PRE-REG: preregs/2026-06-30_substrate_cross_modal_binding_3rd_modality_v1.md
CARDINALITY_OK_FULL: 2700 records per seed (45 pts x 3 arms x 20 queries)
CARDINALITY_OK_SMOKE: 72 records per seed (6 corners x 3 arms x 4 queries)

ASCII-only.
Author: exp_dev 2026-07-01 (Opus 4.7 1M) 3-seed CG lift shoring
"""
...
SEED = 19
ANCHOR_NAME = f"substrate_cross_modal_binding_3rd_modality_v1_seed_{SEED}"
...
CONFIG_VERSION = (
    ...
    f"K=[10,50,100,500,1000],N=[2048,4096,8192],"
    f"mech=[HRR_bind3,sum_then_query,position_key_bind3],"
    f"arms=[BIND_3MOD,NO_BIND_BASELINE,TWO_MOD_BIND_CONTROL],"
    f"V_MOD_A=2048,V_MOD_B=2048,V_MOD_C=2048,V_POS=2048,"
    f"expected_n_full=2700,expected_n_smoke=72,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v1_cross_modal_binding_3mod_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v1_cross_modal_binding_3mod_import_crash",
        }
...
def main() -> int:
    ...
    from experiments._substrate_cross_modal_binding_3rd_modality_v1_core import (
        run_one_seed_phase_diagram, aggregate_and_verdict, selftest,
        get_backend_label, K_VALUES, N_VALUES, BIND_MECHANISMS,
    ...
    final["_hardening_marker"] = "v1_cross_modal_binding_3mod_chunked"
    final["backend"] = backend
    final["seed"] = SEED

    if SMOKE_MODE:
        expected_n = 6 * 3 * N_QUERIES_SMOKE      # 72
    else:
        expected_n = (len(K_VALUES) * len(N_VALUES) * len(BIND_MECHANISMS)
                      * 3 * N_QUERIES_FULL)       # 2700
    observed_n = 0
