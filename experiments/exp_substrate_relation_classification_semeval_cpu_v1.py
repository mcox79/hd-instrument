...
ANCHOR_NAME = "substrate_relation_classification_semeval_cpu_v1"
...
def run() -> Dict:
    ...
    macro, acc = _macro_f1(test, w, classes)
    print("  SEMEVAL-RELATION-CLASS: macro-F1=%.4f acc=%.4f (train=%d test=%d %d classes)" %
          (macro, acc, len(train), len(test), len(classes)), flush=True)
    return {"f1": round(macro, 4), "macro_f1": round(macro, 4), "accuracy": round(acc, 4), "n_train": len(train), "n_classes": len(classes)}
...
def verdict(r) -> Tuple[str, str]:
    ...
    m = r["macro_f1"]; s = "macro-F1=%.4f acc=%.4f (train=%d, %d classes)" % (m, r["accuracy"], r["n_train"], r["n_classes"])
    if m >= 0.65:
        return ("HARD_PASS", "HARD_PASS: substrate-classical relation classification >=0.65 macro-F1 on SemEval-2010 Task 8 -- NEW capability CLASS (relation extraction, entity-pair-aware) via discriminative-weighting; no LLM, no pretraining. " + s)
    if m >= 0.50:
        return ("MIDDLE_BAND", "MIDDLE_BAND: macro-F1 0.50-0.65 -- solid feature-based RE; dependency-path / WordNet features for 0.75+. " + s)
    return ("HARD_FAIL", "HARD_FAIL: macro-F1 <0.50. " + s)
