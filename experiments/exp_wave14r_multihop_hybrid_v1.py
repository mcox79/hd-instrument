"""R8 C1 - Hybrid BSC store + FHRR-cleanup multi-hop chain.

Companion to A1 (wave14r_multihop_FHRR_v1, killed). Storage stays BSC (preserves
all existing capability validations); cleanup runs in FHRR per Frady-Sommer
continuous signal detection to avoid the discrete-codebook closure cliff.

Mechanism:
  M = sign(sum_i subj_i * rel_i * obj_i)  [BSC, identical to wave14t]
  Per hop:
    probe_bsc = M * (current * rel)              [BSC unbind]
    probe_fhrr = exp(i * pi * probe_bsc / 2)     [BSC -> FHRR; +-1 maps to +-i]
    entities_fhrr = exp(i * pi * entities / 2)   [precomputed once]
    next_idx = argmax |<entities_fhrr_k, probe_fhrr>|

Pre-reg: preregs/2026-05-21_wave14r_multihop_hybrid_v1.md
"""
...
_fhrr = importlib.util.spec_from_file_location("fhrr", REPO / "experiments" / "exp_wave14r_multihop_FHRR_v1.py")
fhrr = importlib.util.module_from_spec(_fhrr); _fhrr.loader.exec_module(fhrr)
...
def compute_verdict(summary):
    ...
    if not pdm:
        return ("MULTIHOP_HYBRID_INCONCLUSIVE", "Missing per-depth accuracy.")
    ...
    if acc_50 >= PASS_ACC_50HOP and monotone:
        return ("MULTIHOP_HYBRID_50HOP_VALIDATED",
                f"Hybrid BSC store + FHRR cleanup validates at 50-hop: acc_1={acc_1:.3f}, "
                f"acc_50={acc_50:.3f}. R8 C1 substrate-coherent rescue works.")
    if acc_50 < PARTIAL_ACC_50HOP:
        return ("MULTIHOP_HYBRID_KILLED",
                f"Hybrid 50-hop fails: acc_50={acc_50:.3f} < {PARTIAL_ACC_50HOP}, "
                f"acc_1={acc_1:.3f}. R8 C1 also fails (A1 already killed). "
                f"Boundary conversion noise + closure interact poorly.")
    decay_at = None
    for d in depths:
        if pdm[d] < 0.50:
            decay_at = d
    ...
    if decay_at is None:
        decay_at = depths[-1]
    return (f"MULTIHOP_HYBRID_PARTIAL_AT_{decay_at}",
            f"Hybrid partial: acc_50={acc_50:.3f} in [{PARTIAL_ACC_50HOP}, {PASS_ACC_50HOP}). "
...
def self_test_verdict():
    cases = [
        ({"per_depth_mean_acc": {1: 0.99, 5: 0.96, 10: 0.92, 25: 0.86, 50: 0.82}},
         "MULTIHOP_HYBRID_50HOP_VALIDATED"),
        ({"per_depth_mean_acc": {1: 0.99, 5: 0.85, 10: 0.50, 25: 0.20, 50: 0.10}},
         "MULTIHOP_HYBRID_KILLED"),
        ({"per_depth_mean_acc": {1: 0.99, 5: 0.90, 10: 0.80, 25: 0.65, 50: 0.55}},
         "MULTIHOP_HYBRID_PARTIAL_AT_50"),
        ({"per_depth_mean_acc": {1: 0.99, 5: 0.90, 10: 0.40, 25: 0.55, 50: 0.50}},
         "MULTIHOP_HYBRID_PARTIAL_AT_10"),
...
def bsc_to_fhrr(x_bsc):
    """Convert bipolar +/-1 tensor to FHRR phasors via z = exp(i*pi*x/2).
    For x=+1 -> i; x=-1 -> -i."""
    theta = (math.pi / 2.0) * x_bsc
    return torch.complex(torch.cos(theta), torch.sin(theta)).to(torch.complex64)


def cleanup_fhrr(probe_fhrr, codebook_fhrr):
    """Return argmax index in codebook_fhrr by |<codebook_k, probe>|.
    probe_fhrr: (n,) complex; codebook_fhrr: (k, n) complex."""
    sims = (codebook_fhrr.conj() * probe_fhrr.unsqueeze(0)).sum(dim=-1)
    return int(sims.abs().argmax().item())


def run_hybrid_chain(M, start_idx, rel_idxs, target_idx, entity_atoms,
                       relation_atoms, entity_atoms_fhrr):
    """BSC store + FHRR cleanup multi-hop chain."""
    current_idx = start_idx
    for r_idx in rel_idxs:
        current = entity_atoms[current_idx]
        rel = relation_atoms[r_idx]
        probe_bsc = M * (current * rel)
        probe_fhrr = bsc_to_fhrr(probe_bsc)
        current_idx = cleanup_fhrr(probe_fhrr, entity_atoms_fhrr)
...
def run_one_seed(seed, hop_depths, n_trials, config, device):
    ...
    entity_atoms_fhrr = bsc_to_fhrr(entity_atoms)
    ...
    for depth in hop_depths:
        ...
        for trial in range(n_trials):
            ...
            ok = run_hybrid_chain(M, chain_entities[0], chain_rels,
...
def run_smoke():
    out_dir = get_output_dir("wave14r_multihop_hybrid_v1_smoke")
    summary, verdict, msg, elapsed, config = run_experiment(smoke=True)
    acc_1 = summary["per_depth_mean_acc"][1]
    oracle.assert_baseline_high("hybrid_1hop", acc_1, 0.50)
    write_metrics(out_dir, summary, verdict, msg, elapsed, config)
    print(f"\nSMOKE OK: {verdict}", flush=True)


def run_main():
    out_dir = get_output_dir("wave14r_multihop_hybrid_v1")
