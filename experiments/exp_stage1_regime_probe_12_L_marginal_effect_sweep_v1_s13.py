"""stage1_regime_probe_12_L_marginal_effect_sweep_v1 sibling seed=13.

L (chain-depth in run_chain) MARGINAL-EFFECT sweep at cliff-adjacent regime.
Tests whether L is a genuinely distinct 5th CG_META axis; today's regime
matrix (probes 1-11) fixed L=2 across all cells per Skunkworks structural
VET (task afadd5dbd43055cf1, atom #48, 2026-07-03 21:35Z).

Sweep FULL: 5 L x 3 mech (CLIFF) + 3 L x 3 mech (DEEP_SAT) + 1 PC = 25 pts/seed.
Sweep SMOKE: 3 L x 3 mech (CLIFF) + 1 L x 3 mech (DEEP_SAT) + 1 PC = 13 pts.

Hypotheses (per-mech L-spread on CLIFF arm; MM_TENTATIVE at SMOKE):
  H1: cliff_max_per_mech_L_spread >= 0.10 -> L IS the 5th CG_META axis.
  H2: cliff_max_per_mech_L_spread < 0.05 -> L is inert (degenerate).
  H3: mech ranking changes across L within band on CLIFF (MM_TENTATIVE).
  H3-NULL: DEEP_SAT max_per_mech_L_spread < 0.05 -> L-axis degeneracy at saturation.

CHUNKED architecture: one seed per sibling file. Siblings: s13, s19 (to be authored
after Tailscale restore for 3-seed FULL replication via Orchestrator).
...
SEED = 13
ANCHOR_NAME = f"stage1_regime_probe_12_L_marginal_effect_sweep_v1_s{SEED}"
...
CONFIG_VERSION = (
    ...
    f"regime=sharded_FHRR_chain_composition_L_marginal_effect_cliff_plus_deep_sat_null,"
    f"cleanup_mechanisms=[modern_hopfield,iterative_cosine,soft_energy_attractor],"
    f"L_cliff=[1,2,4,8,16],L_deep=[1,4,16],"
    ...
    f"H1_thresh=(cliff_max_per_mech_L_spread>=0.10),"
    f"H2_thresh=(cliff_max_per_mech_L_spread<0.05),"
    f"H3_null_thresh=(deep_max_per_mech_L_spread<0.05),"
    f"expected_n_full=25,expected_n_smoke=13,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "stage1_regime_probe_12_L_marginal_effect_sweep_v1_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: Exception) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "stage1_regime_probe_12_L_marginal_effect_sweep_v1_import_crash",
        }
        tmp = out_dir / "metrics.json.tmp"
        final = out_dir / "metrics.json"
...
def main() -> int:
    ...
    from experiments._stage1_regime_probe_12_L_marginal_effect_sweep_v1_core import (
    ...
    final["_hardening_marker"] = "stage1_regime_probe_12_L_marginal_effect_sweep_v1_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = "synthetic_sharded_fhrr_chain_composition_L_marginal_effect_v1"
