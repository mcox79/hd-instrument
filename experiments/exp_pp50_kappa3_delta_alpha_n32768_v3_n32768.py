"""
pp50_kappa3_delta_alpha_n32768_v3_n32768 -- PP-50: kappa_3 delta-alpha sensitivity at N=32768.
...
CONTEXT (PP-50 band-lift history):
  ...
  N=32768 founding (v335): sigma_sep(d=0.04)=896.9 (different protocol; sigma_sep at d=0.001
    was 2.55 -- BELOW HP threshold of 3.0 in old protocol).
  N=65536 v1 FAILED: W matrix ~17 GB > 8 GB VRAM on display adapter. OOM abort.
  THIS RUN: N=32768 with full delta-alpha v3 protocol (same as N=16384 v3).
  BAND-LIFT PP-50: 0.70-0.85 -> 0.75-0.90 after N=16384 cross-N confirmed.
  PP-50 current band: 0.75-0.90.

RATIONALE FOR RE-RUN:
  v335 N=32768 founding used different protocol (delta_alpha in [0.01, 0.04] only; old
  sigma_sep(d=0.001) result was below HP=3.0). v3 protocol adds d=0.001 and uses same
  HP thresholds as v347 N=16384. This is the definitive N=32768 v3 cross-N confirmation.

N-SCALING LAW (observed N^(2/3)):
  N=16384: sigma_sep(d=0.04)=642.
  N=32768: v335 sigma_sep(d=0.04)=896.9 (close to N^(2/3) prediction of 1019).
  N=32768 v3 prediction: sigma_sep(d=0.04) ~ 642 * (32768/16384)^(2/3) = 642 * 2^(2/3) = 642 * 1.587 = 1019.
  Expected range given calibration: 500 - 1500.

OOM PRE-CHECK (GPU, N=32768):
  Xi matrix (M x N float32): M = int(0.05 * 32768) = 1638 patterns.
  Xi size: 1638 * 32768 * 4 bytes = 215 MB.
  Hutchinson probe V: N * N_PROBES * 4 = 32768 * 2000 * 4 = 262 MB per V.
  Peak: Xi + 4*V = 0.215 + 4*0.262 = 1.26 GB. FITS in 8 GB with 6.7 GB margin.
  N=65536 failed at ~17 GB because Xi + 4*V scaled as N^2; N=32768 is 1/4 the cost.

PRE-REGISTERED BANDS (PP-50 kappa3 N=32768 delta-alpha v3):
  HARD-PASS: sigma_sep(d=0.04) >= 100 AND sigma_sep(d=0.01) >= 10 AND sigma_sep(d=0.001) >= 3.0.
             Rationale: N^(2/3) scaling from N=16384 gives ~1019 at d=0.04 >> 100.
             d=0.001 HP=3.0 reflects v335 miss (2.55) + more probes fix. Confident.
             If HP: PP-50 cross-N gap at N=32768 closed; CLOUD auth requested for N=65536+.
  MIDDLE: sigma_sep(d=0.04) in [50, 100) OR sigma_sep(d=0.01) in [5, 10).
          Interpretation: scaling plateau at N=32768 or probe variance.
  HARD-FAIL: sigma_sep(d=0.04) < 50 OR sigma_sep(d=0.01) < 5.
             Interpretation: N-scaling reversal (unexpected).

  Calibration note: prior empirical anchor exists at N=32768 (v335); HP thresholds
  set conservatively at ~10% of N^(2/3) extrapolation to allow for protocol differences.

FORMULA SELF-TESTS (PROT-022):
  1. N^(2/3) scaling: 642 * (32768/16384)^(2/3) ~ 1019 >> 100 (HP threshold).
     [INPUT: sigma_n16384=642, N_ratio=2] [EXPECTED: ~1019 > 100]
  2. M = int(0.05 * 32768) = 1638.
     [EXPECTED: M = 1638]
  3. Hutchinson kappa_3 on tiny N non-NaN.
  4. GPU memory > 100 MB after Xi creation.
  5. Xi VRAM at N=32768: 1638 * 32768 * 4 < 1e9 (< 1 GB).
     [EXPECTED: Xi_bytes < 1e9]

PROT-018: anchor has _n32768; N MUST = 32768.
PROT-021: seed checkpoints keyed with run_mode + N + alpha_base.
QUEUE: overnight_queue (GPU; N=32768 Hutchinson kappa_3 delta-alpha sensitivity).
TIMEOUT ESTIMATE: v3 N=16384 elapsed ~45s per seed (5000 probes, 3 delta_alphas).
  N=32768 scaling: Hutchinson W op is O(M*N) per probe, M ~ 0.05*N.
  Cost ~ M * N * n_probes = alpha * N^2 * n_probes.
  N=32768 / N=16384 = 2x, so N^2 = 4x. n_probes: 5000->2000 = 0.4x.
  Estimated: 45 * 4 * 0.4 = 72s per seed. 5 seeds = 360s.
  ceil(1.5 * 360) = 540s -> timeout_s=600 (round up to nearest 300).
  Well within 4h limit.
"""
import sys
...
ANCHOR_NAME = "pp50_kappa3_delta_alpha_n32768_v3_n32768"
...
_sigma_n16384 = 642.0
_N_ratio = 32768.0 / 16384.0
_extrapolated_sigma_04 = _sigma_n16384 * (_N_ratio ** (2.0 / 3.0))
print(f"[selftest-formula] N=32768 N^(2/3) scaling prediction: "
...
assert _M_check == 1638, f"M check: {_M_check} expected 1638"

_xi_bytes = 1638 * 32768 * 4
assert _xi_bytes < 1e9, f"Xi VRAM check: {_xi_bytes/1e6:.0f}MB >= 1GB"

if RUN_MODE == "smoke":
    N_ACTIVE = 4096          # smoke at 4096 (1/8 of N=32768)
    SEEDS = [7, 17]
    ...
    N_PROBES_SENS = 2000     # sufficient at N=32768 (low VRAM cost)

# Pre-registered thresholds (PP-50 N=32768 delta-alpha v3)
...
def compute_verdict(results: List[Dict]) -> Tuple[str, str]:
    ...
    if hp04 and hp01 and hp001:
        return ("HARD_PASS",
                ...
                f"(v3 protocol). N^(2/3) scaling holds. CLOUD auth needed for N=65536+. {summary}")
