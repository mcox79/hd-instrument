"""Encoder v9 -- MINIMUM-DENSITY PIN: does an 11.11%-active block code (blk_l=9)
clear the ret_agree10>=0.35 retrieval target, pinning the crossing to a code
SPARSER than K512's 12.5%? Direct sequel to v8 (which landed K372/9.09% just
UNDER at 0.335/0.344, MIDDLE_BAND).

CONTEXT (do not re-litigate; direct sequel to v5/v7/v8). The TRAINED density-vs-
retrieval curve is now monotone-RISING with density, four landed points:
  K128 (3.125% active) final_ret_agree10 ~ 0.197  (v5, seed7)
    MEASURED@data/exp_encoder_v5_k256_capacity_paired_v1_seed7/metrics.json:recovery.K128.final.ret_agree10
  K256 (6.250% active) final_ret_agree10 ~ 0.290/0.296  (v5/v8, HARD_PASS)
    MEASURED@data/exp_encoder_v5_k256_capacity_paired_v1_seed7/metrics.json:recovery.K256.final.ret_agree10
  K372 (9.091% active) final_ret_agree10 ~ 0.335/0.344  (v8, 2 seeds, MIDDLE_BAND
    -- JUST under the 0.35 target)
    MEASURED@data/exp_encoder_v8_k372_mindensity_paired_v1_seed7/metrics.json:recovery.K372.final.ret_agree10
  K512 (12.5%  active) final_ret_agree10 ~ 0.414  (v7, seed7, HARD_PASS)
    MEASURED@data/exp_encoder_v7_k512_capacity_paired_v1_seed7/metrics.json:recovery.K512.final.ret_agree10
K372 (0.340 mean) sits JUST BELOW the 0.35 target; K512 (0.414) is ABOVE it, so
the 0.35 crossing is between 9.091% and 12.5% active. The LOCAL slope over that
last segment is (0.414-0.340)/(0.125-0.09091) = 2.17 ret per unit density
(THEORETICAL), placing the crossing at ~9.55% active
(THEORETICAL@0.09091 + (0.35-0.340)/2.17). This cell trains ONE intermediate-
density arm at blk_l=9 (11.11% active), the sparsest integer-block-length code
in the coordinator's 10.5-11% operating band. Predicted FINAL ret_agree10 at
11.11% = 0.340 + (0.1111-0.09091)*2.17 = ~0.384 (THEORETICAL), a ~0.034 margin
ABOVE 0.35 -- a robust clear that pins the minimum-density fallback OPERATING
POINT (margin, not cliff-edge) for the USER's decision. Answers: "does 11.11%
active (sparser than K512's 12.5%) robustly clear the 0.35 retrieval target?"

TILING REALITY (why K is 455, not a round number): a block code's active
...
exactly are 128/256/512/1024/... (densities 3.125/6.25/12.5/25%). Integer
blk_l constrains density to 1/blk_l, so the only feasible densities near the
10.5-11% band are 1/10=10.0% and 1/9=11.11%; there is NO integer blk_l giving
exactly 10.7%. 11.11% (blk_l=9) is chosen over 10.0% (blk_l=10) because at the
~9.55% predicted crossing, 10.0% predicts only ~0.360 (a thin 0.010 margin,
MIDDLE_BAND risk given ~0.005 seed spread) whereas 11.11% predicts ~0.384 (a
robust clear). This cell uses PER-ARM widths (as v8 did):
  K256 control -- kb=256, blk_l=16, width=4096, 6.250% active (EXACT v5
         regime; the Gate-D internal positive control -- its own numbers
         must reproduce v5/v8's landed K256 arm before the new arm is trusted)
  K455 new     -- kb=455, blk_l=9, width=4095, 11.111% active (blk_l=9 is the
         integer per-block alphabet whose density 1/9=11.11% sits in the
         operating band just above the predicted crossing; kb=455 -> width
         4095, within 1 dim / 0.02% of 4096, so the student's MLP output width
         is materially identical to the K256/K512 students' 4096 for capacity
         comparability). blk_l=9 is LARGER than K512's already-validated
         blk_l=8, so per-block binding SNR is HIGHER than K512's -- SBC algebra
         is expected to be at least as safe as K512 (checked per arm, not
         assumed).
...
plateau-hold lever exactly as v5/v7/v8 were). The ONLY thing that differs
...
Prior-work check (substrate-KB concept-query, USER-locked 2026-07-01):
  ...
  target intermediate K sparsity crossing operating point 11 percent active"
  -> top cosine=0.2754 (phase-portrait operating-point note) + 0.252 (pc-
  sparsity prereg); NONE at cosine>0.30 for any DISTINCT prior cell training an
  11.11%-active (blk_l=9) student. GENUINELY NOVEL: no prior cell trains a
  blk_l=9 student or pins the 0.35 crossing above 9.09%; v5/v7 touch power-of-2
  blk_l only, v8 touched blk_l=11 (9.09%). This cell is the sparser-than-K512
  operating-point pin between v8's 9.09% and K512's 12.5%.
...
- HP_SCOPE: the target-clearing gate applies to K455_BLOCK_LAST FINAL only;
...
Prereg: preregs/2026-07-04_exp_encoder_v9_k455_mindensity_paired_v1.md
...
# Config.
# ---------------------------------------------------------------------------
ANCHOR_NAME = "encoder_v9_k455_mindensity_paired_v1"
...
# arm_name -> (kb, blk_l). PER-ARM width = kb*blk_l (K256->4096, K455->4095).
# density = 1/blk_l (K256 6.25%, K455 11.111%). CONTROL_ARM reproduces v5/v8's
# landed K256; NEW_ARM is the intermediate-density (11.11%) operating-point probe.
CONTROL_ARM = "K256"
NEW_ARM = "K455"
K_ARMS = {CONTROL_ARM: (256, 16), NEW_ARM: (455, 9)}
...
def _artifact_dir(run_mode: str, run_tag: str = "") -> Path:
    ...
    return _REPO / "data" / f"substrate_concept_encoder_v9_k455{tag}{suffix}"
...
def _verdict_mindensity(per_unit: List[Dict], recovery: Dict, expected_units: int,
    ...
    if run_mode == "smoke":
        ...
        return ("HARD_PASS",
                f"SMOKE_MACHINERY_OK: both arms (K256 4096-wide + K455 4095-wide) "
                ...
                f"(the 'does 11.11%-active clear 0.35' question is a FULL-only "
    ...
    if delta_hi80 < DELTA_HI80_COS_REGRESSION_FLOOR:
        return ("HARD_FAIL",
                ...
                f"{DELTA_HI80_COS_REGRESSION_FLOOR} -- the sparser 11.11% code costs "
                f"semantic calibration vs the K256 control {tail}")
    if new_ret >= RET_AGREE10_TARGET:
        return ("HARD_PASS",
                f"MINDENSITY_CLEARS_TARGET: {NEW_ARM} (11.111% active) final "
                f"ret_agree10 {new_ret:.4f} >= {RET_AGREE10_TARGET} target -- the "
                f"retrieval target is reachable at a code SPARSER than K512's 12.5% "
                f"(11.11% suffices), no calibration regression vs K256; pins the "
                f"min-density crossing to (9.091%, 11.111%] {tail}")
    return ("MIDDLE_BAND",
            f"MINDENSITY_BELOW_TARGET: {NEW_ARM} (11.111% active) final ret_agree10 "
            f"{new_ret:.4f} < {RET_AGREE10_TARGET} target -- 11.11% active is NOT "
            f"enough to clear 0.35; the minimum-density crossing is between 11.111% "
            f"and K512's 12.5% (a useful lower bound, locates the crossing above "
            f"11.11%) {tail}")
...
def run_mindensity(run_mode: str, seed: int, device_arg: str,
    ...
    metrics = {
        ...
        "methodology": ("PAIRED same-seed/data/split/mining minimum-density probe: "
                        ...
                        "reproducer of v5/v8's landed K256) vs K455 new arm (blk_l=9, "
                        "width 4095, 11.111% active -- the integer per-block alphabet "
                        "just above the predicted ret_agree10=0.35 crossing (~9.55%) "
                        "between v8's K372 0.340 and K512's 0.414). in_batch-"
                        "RKD-only nce=0 config matching v5/v7/v8, UNCHANGED cosine-decay "
                        "LR (isolated from v6's plateau lever); FINAL-step "
                        "ret_agree10 is the PRIMARY gated number (does 11.11% clear "
                        "0.35); per-arm RANDOM_BLOCK/shuffled-key/FALSE_WIN_ALGEBRA "
                        "checks run at EACH arm's own block partition (blk_l=9 is "
...
def run_self_test() -> int:
    ...
    print(f"[selftest] PASS (CRLB K128->0.901 + K455>K256 ceiling + tiling/density "
