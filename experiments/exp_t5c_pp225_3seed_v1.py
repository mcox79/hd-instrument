"""
exp_t5c_pp225_3seed_v1 -- Path B rescue R3: supervised projection head (bypass cross-attn) -- GPU.
...
ANCHOR_NAME = "t5c_pp225_3seed_v1"; MODEL = "EleutherAI/pythia-160m"; ENCODER = "BAAI/bge-large-en-v1.5"
...
def _selftest():
    ...
    assert int(_n.argmax([0.1, 0.9])) == 1, "argmax"; print("[selftest] PASS: pp225-3seed", flush=True)
...
def run(SEED=7) -> Dict:
    torch.manual_seed(SEED); g = np.random.default_rng(7)
