...
"""substrate_task_vector_K_extended_v1 sibling seed=7.

Extended-K probe of TASK_VECTOR HRR ICL K-cliff (Stage 3, M3 concern #4).
Referent: v3 phase_diagram (MIDDLE_BAND 2026-06-28). This cell extends the K
axis to {50, 100, 200, 500, 1000} to answer whether K>=200 floor is fully
Bernoulli-at-random or mechanism still discriminates.

CHUNKED per-seed architecture. Sibling seeds: 7 / 13 / 19.
Per-seed cell writes a partial via _seed_checkpoint; final aggregation pools
across the 3 partials.

ARMS: TASK_VECTOR / RANDOM_VECTOR / ORACLE (3 per phase point)
SWEEP: K in {50, 100, 200, 500, 1000} at V=10, ov=0.0 = 5 pts
N_DIM=8192. V_ENTS_POOL=1000. N_QUERIES_FULL=100. N_QUERIES_SMOKE=100.

PRE-REG: preregs/2026-06-30_substrate_task_vector_K_extended_v1.md
...
ASCII-only.
Author: exp_dev 2026-06-30 (Opus 4.7 1M, agent-spawn) extended-K reach
"""
...
SEED = 13
ANCHOR_NAME = f"substrate_task_vector_K_extended_v1_seed_{SEED}"
...
CONFIG_VERSION = (
    ...
    f"K=[50,100,200,500,1000],V=10,ov=0.0,"
    f"V_ENTS_POOL=1000,n_queries_full=100,n_queries_smoke=100,"
    f"arms=[TASK_VECTOR,RANDOM_VECTOR,ORACLE],"
    f"expected_n_full=1500,expected_n_smoke=300,"
    f"referent=v3_task_vector_K_cliff_phase_diagram_v3_MB_2026-06-28,"
    f"lever=K_axis_extended_reach_50_to_1000,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v1_taskvec_K_extended_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v1_taskvec_K_extended_import_crash",
        }
...
def main() -> int:
    ...
    from experiments._substrate_task_vector_K_extended_v1_core import (
        run_one_seed_extended_K, aggregate_and_verdict, selftest,
    ...
    for seed in remaining:
        _write_minimal_metrics(out_dir, "RUNNING",
                               ...
                               extra={"_phase": "seed_running",
                                      ...
                                      "backend": backend})
        t0 = time.time()
        result = run_one_seed_extended_K(seed, run_mode=RUN_MODE)
    ...
    # Per-sibling: single-seed aggregate; final cross-seed aggregation done post-hoc
    ...
    final["_hardening_marker"] = "v1_taskvec_K_extended_chunked"
