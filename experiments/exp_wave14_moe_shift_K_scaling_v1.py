"""MoE SHIFT K-scaling sweep: shift-magnitude vs K at matched load.

PRE-BUILD: This script is pre-built assuming wave14_moe_shift_partition_v2 returns
SHIFT mode (i.e., Arm A SHIFT exceeds Arm C SINGLE by > 0.15 and mode-collapse safe).
Ship immediately when that verdict lands SHIFT.

HYPOTHESIS: In SHIFT mode (K full-N experts, K*N^2 parameter budget), retention
scales with K because each expert stores independently. The K-scaling sweep maps:
  retention(K) vs K at fixed M_per_expert (each expert at its capacity operating point)

DESIGN:
  - FIXED M_per_expert = M_PER_EXPERT (from alpha_c calibration at N=4096)
  - K sweep: K in {2, 4, 8, 16, 32}  (5 points; exp_dev autonomy)
...
HP_K_SCALING_RATIO = 4.0      # retention(K=32)/retention(K=2) >= 4x for HARD_PASS
...
def run_sweep(smoke: bool = False):
    ...
    out_dir = get_output_dir("wave14_moe_shift_K_scaling_v1")
