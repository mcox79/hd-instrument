"""
exp_sql_avg_formula_fix_v1 -- four-drills #1: HD GROUP BY COUNT estimator fix -- CPU.
...
PRE-REGISTERED: HARD-PASS GROUP BY COUNT relative error <5% (theory O(1/sqrt(N)) ~1.6% at N=4096).
...
ANCHOR_NAME = "sql_groupby_count_fix_v1"; N = 4096
...
def run() -> Dict:
    g = np.random.default_rng(7); errs = []; NG = 8
    for _ in range(TRIALS):
        gkeys = phasor(N, NG, g); counts = g.integers(5, 50, NG)               # true per-group counts
        bundle = np.sum([gkeys[gi] * counts[gi] for gi in range(NG)], axis=0).astype(np.complex64)
        est = (np.conj(gkeys) @ bundle).real / N                               # GROUP BY COUNT estimate (divide by N once)
        rel = np.abs(est - counts) / counts; errs.append(rel.mean())
    rel = float(np.mean(errs)); print("  GROUP BY COUNT relative error=%.4f over %d trials (N=%d)" % (rel, TRIALS, N), flush=True)
    return {"rel_err": rel}
def verdict(r) -> Tuple[str, str]:
    s = "AVG rel-error=%.4f (theory ~1.6%% at N=4096)" % r["rel_err"]
    if r["rel_err"] < 0.05: return ("HARD_PASS", "HARD_PASS: HD GROUP BY COUNT estimator rel-error <5%% -- formula correct; cycle-155 GROUP BY COUNT capability restored to HP. " + s)
