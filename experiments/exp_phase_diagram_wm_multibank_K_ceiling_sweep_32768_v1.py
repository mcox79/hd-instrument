"""phase_diagram_wm_multibank_K_ceiling_sweep_32768_v1 -- GPU K-ceiling map.

USER directive 2026-06-26: extend WM multi-bank chain-grade beyond K=4096
(prior K-extension cell `phase_diagram_working_memory_multibank_K_extension_to_16384_v1`
landed MIDDLE_BAND with rec=1.000 at K=4096/8192/16384 -- by-construction-saturation
at K=16384 prevents discriminating cliff). USER asks for K in {8192, 16384, 32768}
to find the capacity ceiling. K=4096 is the chain-grade RAIL.

PROMOTION CONTEXT:
  Brain WM = ~7+/-2 conscious items but substrate routing capacity is much higher.
  Need to map ABSOLUTE CLIFF; K=32768 is 8x past chain-grade. If still at rec=1.000,
  USER may extend further; if cliffs, we've found the ceiling.

DESIGN: K-extension with FOUR phase points + reproduce-rail at K=4096
  K=4096  [MULTI_64x]                                  (RAIL; rec=1.000 cv=0.000 expected)
  K=8192  [MULTI_128x]                                  (novel; reproduce v1's 1.000 expected)
  K=16384 [MULTI_256x]                                  (novel; v1 had rec=1.000)
  K=32768 [MULTI_512x with k_per_bank=64]              (novel; capacity ceiling probe)
Each at RANDOM regime + ADVERSARIAL regime (FEATURE_OVERLAP_FRAC=0.20)
Plus sentinel: ARM_KNN_BASELINE at M=400 (>= 0.9; Fix #28)

CONFIG (N_DIM=8192 per USER spec; same as v1 K-extension):
  N_DIM = 8192, CODEBOOK_SIZE = 65536 (2x v1 to accommodate K=32768)
...
PRE-REG BANDS (LOCKED at module init):
  CHAIN_GRADE_K_CEILING_32768: all 4 K values chain-grade (rec>=0.95 cv<=0.05
    adv_within<=0.05); ceiling extends to K=32768
  PARTIAL_K_CEILING_16384: K<=16384 chain-grade, K=32768 cliffs (rec<0.95 or adv>0.30)
  PARTIAL_K_CEILING_8192: K<=8192 chain-grade, K>=16384 cliffs
  K_4096_IS_CEILING: chain-grade only at K=4096
  ADVERSARIAL_BREAKS_AT_K_CEILING: adv-random gap >= 0.30 at any K>=16384

GPU MANDATE (Fix #24): torch.cuda required for full; fp16 storage; batched matmul on
  cuda; nvidia-smi util sampled per arm; smoke gates gpu_util_p50 >= 50%.
...
ANCHOR_NAME = "phase_diagram_wm_multibank_K_ceiling_sweep_32768_v1"
...
HP_RAIL_K4096_RECALL = 1.0000   # v1 K-extension cell at K=4096 (saturated rec=1.000)
HP_RAIL_K4096_TOL = 0.05        # wider tol since v1 saturated; sanity if drifts below 0.95
...
if SMOKE:
    ...
    ARRANGEMENTS_BY_K = {
        ...
        8192:  [("MULTI_128x", 128, 64)],                       # k_per_bank=64 (chain-grade envelope)
...
CONFIG_VERSION = (
    "phaseDiagWmMBKCeil32768-v1: N_DIM=%d CODEBOOK_SIZE=%d sigma=%.1f CUE_COS=%.2f "
...
def compute_verdict(per_key: Dict[str, Dict]) -> Tuple[str, str, Dict]:
    ...
    # Rail check at K=4096 (sanity reproduce)
    rail_ok = None
    rail_observed = None
    rail_drift = False
    ...
    # K_4096_IS_CEILING check (K>=8192 cliffs below 0.50)
    ...
    if adv_break_at_K:
        return ("MIDDLE_BAND",
                "MIDDLE_BAND_ADVERSARIAL_BREAKS_AT_K_CEILING: adv_break_at=%s | %s%s%s" % (
    ...
    if all_extended_chain and rail_drift:
        return ("MIDDLE_BAND",
                ...
                "drifted (mechanism not reproducing v1) | %s%s%s" % (
                    rail_observed, summ, rail_str, knn_str),
                detail)
    ...
    if highest_chain >= 16384:
        return ("PARTIAL_K_CEILING_16384",
                ...
                "(chain_grade_set=%s) | %s%s%s" % (
                    highest_chain, chain_grade_at_K, summ, rail_str, knn_str),
                detail)
    ...
    if highest_chain == 4096:
        return ("K_4096_IS_CEILING",
                "K_4096_IS_CEILING_NO_EXT_PASSES: only K=4096 chain-grade | %s%s%s" % (
                    summ, rail_str, knn_str),
                detail)
    if some_extended_chain:
        return ("MIDDLE_BAND",
                "MIDDLE_BAND_PARTIAL_K_EXTENDS: chain_grade_set=%s | %s%s%s" % (
                    chain_grade_at_K, summ, rail_str, knn_str),
                detail)
    return ("MIDDLE_BAND",
            "MIDDLE_BAND_NO_CHAIN_GRADE_AT_EXT: no extended K passes | %s%s%s" % (
                summ, rail_str, knn_str),
            detail)
...
def _build_metrics(v: str, vmsg: str, detail: Dict, units: List[Dict],
    ...
    return {
        ...
        "metrics_source": "measured_gpu_substrate_bipolar_multibank_K_ceiling_sweep",
        "DESIGN_NOTE": (
            ...
            "envelope k_per_bank<=64 preserved across MULTI_64x/128x/256x/512x. Tests "
            "RANDOM + ADVERSARIAL regimes. Rail-checks K=4096 MULTI_64x random against "
            "v1's rec=1.000. Sentinel ARM_KNN_BASELINE >= %.2f (Fix #28). GPU-batched "
            "matmul on fp16 cuda; gpu_util sampled per arm (Fix #24). Pre-reg per "
