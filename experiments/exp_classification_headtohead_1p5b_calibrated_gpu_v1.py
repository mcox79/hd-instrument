"""
exp_classification_headtohead_1p5b_calibrated_gpu_v1.py -- calibrated classification head-to-head vs Qwen-1.5B (scale test) -- GPU.
...
ROUTING: the calibrated classification head-to-head beat Qwen-0.5B (SST-2 0.7765 robust-win; AG-News 0.848 decisive). The MATH
  ...
  it only beat the smallest? Re-run the SAME calibrated (PMI/contextual) protocol vs Qwen2.5-1.5B-Instruct on BOTH SST-2 (binary)
...
OUT = REPO / "data" / ("exp_" + os.environ.get("HDLAB_EXP_NAME", "classification_headtohead_1p5b_calibrated_gpu_v1"))
...
def main():
    ...
    msg = "%s: classification head-to-head vs Qwen-1.5B (3x larger) -- %s. Scale test of the calibrated substrate-beats-0.5B result." % (verdict, " | ".join(parts))
    print("\n[VERDICT] " + msg, flush=True)
    OUT.mkdir(parents=True, exist_ok=True)
    json.dump({"anchor_name": "classification_headtohead_1p5b_calibrated_gpu_v1", "verdict": verdict, "verdict_msg": msg,
...
if "--self-test" in sys.argv:
    assert "BIAS" in _feats("hi there"); print("[selftest] PASS: classification-headtohead-1p5b-calibrated", flush=True); sys.exit(0)
main()
