"""Bytes-per-fact storage efficiency Pareto v2 EXTENDED PRECISIONS, seed=7.

Extends v1.1 (5-arm at single M=10000) into a 7-precision x 4-M grid:
  precisions: FP32, BFLOAT16, FP16, INT8, INT4, BINARY, SPARSE_BIPOLAR
  M sweep: {1000, 4000, 10000, 20000}
  N=4096 dense, 16384 sparse.

Discriminators:
  - Pareto separation within each M
  - Monotonic recall decay as M grows per precision
  - BFLOAT16 does NOT collapse (recall > 0.5 @ M=4000): test v1 FP16-collapse
    is RANGE-limited (fixable by wider exponent), not precision-limited.
  - INT4 vs INT8 recall gap > 0.1 @ M=10000: INT4 loses genuine info.

CHUNKED: one seed per file. Sibling pair: seed_13, seed_19.

PROT-020: `import torch` present. This cell uses numpy semantics only via
torch tensors; torch installation is CPU-only on this machine (2.8.0+cpu).
Cell runs on CPU -> route to remote_cpu_queue (CPU-eligible; matmul is
Hebbian outer sums at N=4096, ~fits in CPU BLAS comfortably).

PROT-018: anchor has no _n suffix (grid is precision x M, no single N).
...
SEED = 19
ANCHOR_NAME = f"substrate_bytes_per_fact_pareto_v2_seed_{SEED}"
...
CONFIG_VERSION = (
    ...
    f"arms=[FP32_DENSE,BFLOAT16_DENSE,FP16_DENSE,INT8_DENSE,INT4_QUANTIZED,BINARY_DENSE,SPARSE_BIPOLAR_0p05],"
    f"N_DIM_DENSE=4096,N_DIM_SPARSE=16384,"
    f"M_sweep_full=[1000,4000,10000,20000],M_sweep_smoke=[500,2000],"
    f"n_ent_full=5000,n_rel_full=100,query_frac=0.10,"
    f"recall_target_underload=0.85,bfloat16_min=0.5,int4_int8_gap=0.10,topK=1,"
    f"SEED={SEED},mode={RUN_MODE},"
    f"discriminator=pareto_sep_per_M+monotone_decay+bfloat16_range+int4_gap+META_RULE_BC,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v2_bytes_per_fact_pareto_extended_precisions_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v2_bytes_per_fact_import_crash",
        }
        tmp = out_dir / "metrics.json.tmp"
        final = out_dir / "metrics.json"
...
def main() -> int:
    ...
    from experiments._substrate_bytes_per_fact_pareto_v2_core import (
        run_one_seed_all_units, aggregate_and_verdict, selftest,
        get_backend_label, _get_device, ARMS, FULL_M_SWEEP, SMOKE_M_SWEEP,
    )

    strict_gpu = False  # v2 is CPU-eligible; no strict gpu gate
    ...
    seeds_list = [SEED]
    run_config = {"N": 4096, "run_mode": RUN_MODE, "anchor": ANCHOR_NAME,
    ...
    final["_hardening_marker"] = "v2_bytes_per_fact_pareto_extended_precisions_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = (
        "synthetic_uniform_KG_5000ent_100rel_M_sweep_1k_4k_10k_20k_holdout_10pct_queries"
    )
