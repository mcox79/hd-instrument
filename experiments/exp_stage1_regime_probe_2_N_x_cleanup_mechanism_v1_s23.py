"""stage1_regime_probe_2_N_x_cleanup_mechanism_v1 sibling seed=11.

Second cell in Stage 1 Regime Map arc (USER-directed 2026-07-03).
Regime: SHARDED FHRR chain composition (Option Y).
Question: does N moderate the mechanism-degeneracy observed at FHRR SHARDED?

Sweep FULL: 4 N x 3 cleanup x 3 M x 2 corr (F=1, L=2 fixed) = 72 pts/seed.
Sweep SMOKE: 2 N x 3 cleanup x 1 M x 1 corr = 6 pts/seed.

Hypotheses:
  H1: mechanism-var uniform ~0 at all N -> Option Y extends universally.
  H2: some N with mech-var > 0.10 -> N-dependent boundary (CG_META candidate).
  H3: uniform-nonzero -> scale-invariant nonzero degeneracy.

CHUNKED architecture: one seed per sibling file. Siblings: s17, s23.
...
SEED = 23
ANCHOR_NAME = f"stage1_regime_probe_2_N_x_cleanup_mechanism_v1_s{SEED}"
...
CONFIG_VERSION = (
    ...
    f"N=[2048,4096,8192,16384],M=[200,800,3200],corr=[0.20,0.45],F=1,L=2,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "stage1_regime_probe_2_N_x_cleanup_mechanism_v1_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: Exception) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "stage1_regime_probe_2_N_x_cleanup_mechanism_v1_import_crash",
        }
        tmp = out_dir / "metrics.json.tmp"
        final = out_dir / "metrics.json"
...
def main() -> int:
    ...
    from experiments._stage1_regime_probe_2_N_x_cleanup_mechanism_v1_core import (
    ...
    final["_hardening_marker"] = "stage1_regime_probe_2_N_x_cleanup_mechanism_v1_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = "synthetic_sharded_fhrr_chain_composition_regime_probe_v1"
