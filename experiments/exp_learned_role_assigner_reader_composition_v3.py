"""
Track A v3 -- CG-REVIVAL GATE: does reading GENUINELY COMPOSE relations, on a FAIR, POWERED test?

WHY (v3): v2 (exp_learned_role_assigner_reader_heldout_v2, VET ad3bb811 = MEASURED_MECHANISM)
PROVED the LEARNED role-assigner ESCAPES construction (generalizes to held-out primer, role-
sensitive: passive 5/5 vs positional 0/5, reversal 6/6 -- KEEP that learned extractor). But two
things blocked the CORE question: (a) the COMPOSITION test was CONFOUNDED + underpowered (N=2 CMP
Qs; frequency trivially wins by topic-frequency on 3-sentence passages) -> composition was UNTESTED
not failed; (b) real limits: it->tree RECENCY coref error on topical cases + INANIMATE-POSSESSOR
extraction gap ("the nest has eggs" unextracted; the AGENT-finder is animacy-biased). This cell
resolves the core question with the three v3 fixes the VET + drill both endorse.

THE v3 FIXES (load-bearing):
  1. COREF = MAINTAINED-SALIENCE strategy, NOT recency (brain-faithful Centering Theory: the
     backward-looking center prefers the topical/frequent subject over a recently-mentioned object).
     The overlay hdlab.state_of_mind.WorkingOverlay ALREADY has strategy='maintained' (count +
     beta*exp(-lam*dist), frequency-primary; VALIDATED on LitBank coref-gold, VET a7ca3db1). v2
     hardcoded strategy='recency'. v3 threads the strategy per-arm: learned_full = MAINTAINED
     (the claim); a RECENCY_COREF arm (v2 strategy) is the must-beat on competitive cases.
  2. INANIMATE-POSSESSOR extraction fix: possessive 'has/have/had' now takes the possessor = first
     pre-verb candidate STRUCTURALLY (regardless of animacy), so "The nest has eggs" -> poss(nest,
     eggs) and "The pen has a rat" -> poss(pen,rat) extract. THEMATIC roles of ACTION verbs stay
     LEARNED (animacy is a genuine cue there); only the possessive relation is destructured (mirrors
     the existing possessive-'s / possessive-pronoun structural handlers).
  3. FAIR, POWERED COMPOSITION TEST (the crux): 16 composition Qs (was 2) on LONGER constructed
     passages, with FREQUENCY DECORRELATED from the answer BY CONSTRUCTION -- each passage has a
     DISTRACTOR entity of the answer type mentioned MORE often than the composition answer, and 2
     owners / 2 objects / 2 locations so the 2-relation JOIN is load-bearing (a single-hop "most
     recent/frequent" shortcut gets the wrong answer). The frequency baseline therefore CANNOT
     trivially win -- verified at smoke (freq CMP accuracy near 0).

ARMS (one variable per contrast; design-gate: real baselines, can-fail, difficulty-on, anti-circular):
  learned_full    : learned role-assigner + MAINTAINED-salience coref            [THE CLAIM]
  recency_coref   : learned role-assigner + RECENCY coref (v2 strategy)          [must-beat on CC]
  learned_nocoref : learned role-assigner, coref OFF                             [coref ablation]
  positional      : naive positional role-assigner + maintained coref           [context baseline]
  frequency       : no-role grounded frequency floor, DECORRELATED               [must-NOT-win CMP]

SLICES: NC (single-hop, no coref) ; CO (ordinary coref) ; CC (competitive coref where RECENCY is
WRONG -- topical subject != nearest antecedent; maintained must beat recency) ; CMP (2-relation
composition, freq-decorrelated, powered N=16). RELATION-F1 scored on REAL held-out McGuffey PRIMER
(extraction generalization). Reported by-slice + maintained-vs-recency coref accuracy EXPLICITLY.

Glass-box (learned + transparent linear weights + symbolic overlay), learn-in-substrate, NO external
LLM at runtime, NOT next-word prediction. Local/foreground. Reported CLAIM-VET-pending (NOT self-
declared chain-grade). HONEST CAVEAT: constructed competitive-coref gold is stipulated per Centering
Theory (subject>object) -- the maintained-salience mechanism itself was VALIDATED on real LitBank
coref-gold (a7ca3db1); this cell tests whether that strategy, wired into the reader, RECOVERS the
topical antecedent recency drops. Composition passages are constructed (task-authorized "mildly-
longer") but the extraction is the SAME learned pipeline that generalizes on real primer (RELF1).

ANCHOR: learned_role_assigner_reader_composition_v3

--- ORIGINAL v2 CONSTRUCTION-ESCAPE NOTES (retained; the learned extractor is unchanged) ---
...
ANCHOR_NAME = "learned_role_assigner_reader_composition_v3"
...
# (CRUX) COMPOSITION on the FAIR, POWERED, FREQ-DECORRELATED test (CMP slice, N=16):
#   learned_full must (i) reach HP_COMPOSITION, (ii) beat the decorrelated FREQUENCY floor by a
#   real margin, (iii) beat no-coref and recency-coref. Reachable + can-fail (extraction of the
#   2-relation join can break -> composition fails).
HP_COMPOSITION = 0.60          # HARD_PASS: learned_full CMP accuracy
HP_COMP_MARGIN_FREQ = 0.30     # HARD_PASS: learned_full CMP - frequency CMP (freq CANNOT win)
HF_COMPOSITION = 0.40          # HARD_FAIL: composition still fails on the fair powered test
# (CRUX) MAINTAINED-SALIENCE COREF must beat RECENCY on competitive cases (CC slice: topical
#   subject != nearest antecedent). This is the it->tree recency-error fix, tested where it bites.
HP_CC_MAINTAINED = 0.60        # HARD_PASS: learned_full (maintained) CC accuracy
HP_CC_MARGIN_RECENCY = 0.25    # HARD_PASS: maintained CC - recency CC (maintained beats recency)
# (SANITY) extraction generalization on REAL held-out primer (the v2 escape must still hold):
HP_RELF1_HELDOUT = 0.65        # HARD_PASS floor: learned extractor still GENERALIZES on primer
HF_RELF1_HELDOUT = 0.45        # HARD_FAIL: collapsed back to the old hand-rule wall
# (SECONDARY, reported not primary-gated -- validated in v2): role-reversal + passive controls.
HP_REVERSAL_LEARNED = 0.75
MAX_REVERSAL_FREQ = 0.50
HP_PASSIVE_LEARNED = 0.60
HP_PASSIVE_LEARNED_MINUS_POS = 0.30
# coref-lift on CO slice (FULL vs NOCOREF): overlay helps (reported).
HP_COREF_LIFT = 0.15
HP_COREF_ALPHA = 0.10
...
TEST_PASSAGES = {
    ...
    # ---- CONSTRUCTED COMPOSITION passages (task-authorized longer text; FREQUENCY DECORRELATED) ----
    # Each: 2 owners / 2 objects / 2 locations (join is load-bearing) + a DISTRACTOR location
    # mentioned MORE than either answer (so the freq baseline picks the wrong, frequent location).
    "C1": "Ann has a doll. Ben has a ball. The doll is in the box. The ball is on the log. "
          "Kate sits on the mat. Kate naps on the mat. Kate runs to the mat.",
    "C2": "Tom has a cap. Sue has a mug. The cap is on the hill. The mug is in the pen. "
          "Nell sat on the rock. Nell hid by the rock. Nell fell on the rock.",
    # coref-gated composition (owned object referred by 'it' -> no-coref cannot compose).
    "C3": "Ann has a lamp. It is in the box. Ben sat on the mat. Ben naps on the mat. Ben ran to the mat.",
    "C4": "Tom has a book. It is on the log. Kate sat by the pond. Kate swam in the pond. Kate fell in the pond.",
    "C5": "Sam has a cart. Will has a drum. The cart is in the barn. The drum is on the tree. "
          "Jane ran in the field. Jane sat in the field. Jane hid in the field.",
    "C6": "Ned has a jar. Nat has a bell. The jar is on the bank. The bell is in the cage. "
          "Sue sat by the pond. Sue swam in the pond.",
    "C9": "Kate has a hat. Nell has a cup. The hat is on the roof. The cup is in the nest. "
          "Ann ran on the road. Ann sat on the road.",
    # owner-of-owned 2-hop (inanimate MID-possessor: pen/box has X) with animate freq distractor.
    "C7": "The cat has a pen. The pen has a rat. Ann fed the hen. Ann liked the hen. Ann saw the hen.",
    "C8": "The dog has a box. The box has a ball. Tom fed the pig. Tom liked the pig. Tom saw the pig.",
    # ---- CONSTRUCTED COMPETITIVE COREF (CC): topical subject (3x) vs recent object-of-'to' (1x).
    # Centering Theory ranks subject > object for the backward center -> gold = topical subject;
    # RECENCY picks the nearest (the recent object) and is WRONG; MAINTAINED (freq-primary) is right.
    "CC1": "Ann fed the hen. Ann liked the hen. Ann ran to Kate. She held the pig.",
    "CC2": "Tom had a dog. Tom fed the dog. Tom went to Ben. He held the cat.",
    "CC3": "Sue sat on the log. Sue fed the ducks. Sue ran to Jane. She held the fox.",
    "CC4": "Ben had a cap. Ben liked the cap. Ben ran to Sam. He fed the goat.",
    "CC5": "Nell sat by the pond. Nell fed the ducks. Nell ran to Ann. She held the hen.",
    "CC6": "Will had a ball. Will threw the ball. Will ran to Ned. He fed the colt.",
    "CC7": "Kate fed the lamb. Kate liked the lamb. Kate ran to Sue. She held the cat.",
    "CC8": "Sam had a mug. Sam filled the mug. Sam ran to Tom. He fed the ox.",
}
...
TEST_QS = [
    ...
    # ---- CMP (composition, freq-decorrelated, powered): loc_of_owned = poss(X,Y)+loc(Y,G) -> G ----
    dict(qid="C1a", p="C1", slice="CMP", atype="LOCATION", spec=("loc_of_owned", "ann", "doll"),
         gold="box", text="Where is Ann's doll?"),
    dict(qid="C1b", p="C1", slice="CMP", atype="LOCATION", spec=("loc_of_owned", "ben", "ball"),
         gold="log", text="Where is Ben's ball?"),
    dict(qid="C2a", p="C2", slice="CMP", atype="LOCATION", spec=("loc_of_owned", "tom", "cap"),
         gold="hill", text="Where is Tom's cap?"),
    dict(qid="C2b", p="C2", slice="CMP", atype="LOCATION", spec=("loc_of_owned", "sue", "mug"),
         gold="pen", text="Where is Sue's mug?"),
    dict(qid="C3a", p="C3", slice="CMP", atype="LOCATION", spec=("loc_of_owned", "ann", "lamp"),
         gold="box", text="Where is Ann's lamp?"),   # needs coref it->lamp (no-coref fails)
    dict(qid="C4a", p="C4", slice="CMP", atype="LOCATION", spec=("loc_of_owned", "tom", "book"),
         gold="log", text="Where is Tom's book?"),    # needs coref it->book (no-coref fails)
    dict(qid="C5a", p="C5", slice="CMP", atype="LOCATION", spec=("loc_of_owned", "sam", "cart"),
         gold="barn", text="Where is Sam's cart?"),
    dict(qid="C5b", p="C5", slice="CMP", atype="LOCATION", spec=("loc_of_owned", "will", "drum"),
         gold="tree", text="Where is Will's drum?"),
    dict(qid="C6a", p="C6", slice="CMP", atype="LOCATION", spec=("loc_of_owned", "ned", "jar"),
         gold="bank", text="Where is Ned's jar?"),
    dict(qid="C6b", p="C6", slice="CMP", atype="LOCATION", spec=("loc_of_owned", "nat", "bell"),
         gold="cage", text="Where is Nat's bell?"),
    dict(qid="C9a", p="C9", slice="CMP", atype="LOCATION", spec=("loc_of_owned", "kate", "hat"),
         gold="roof", text="Where is Kate's hat?"),
    dict(qid="C9b", p="C9", slice="CMP", atype="LOCATION", spec=("loc_of_owned", "nell", "cup"),
         gold="nest", text="Where is Nell's cup?"),
    # CMP owner_of_owned_chain = poss(mid,contained)+poss(owner,mid) -> owner (inanimate mid-possessor)
    dict(qid="C7a", p="C7", slice="CMP", atype="AGENT", spec=("owner_of_owned_chain", "rat"),
         gold="cat", text="Whose pen has the rat?"),
    dict(qid="C8a", p="C8", slice="CMP", atype="AGENT", spec=("owner_of_owned_chain", "ball"),
         gold="dog", text="Whose box has the ball?"),
    # ---- CC (competitive coref: topical subject beats recent object; maintained beats recency) ----
    dict(qid="CC1", p="CC1", slice="CC", atype="AGENT", spec=("svo_agent", "held", "pig"),
         gold="ann", text="Who held the pig?"),
    dict(qid="CC2", p="CC2", slice="CC", atype="AGENT", spec=("svo_agent", "held", "cat"),
         gold="tom", text="Who held the cat?"),
    dict(qid="CC3", p="CC3", slice="CC", atype="AGENT", spec=("svo_agent", "held", "fox"),
         gold="sue", text="Who held the fox?"),
    dict(qid="CC4", p="CC4", slice="CC", atype="AGENT", spec=("svo_agent", "fed", "goat"),
         gold="ben", text="Who fed the goat?"),
    dict(qid="CC5", p="CC5", slice="CC", atype="AGENT", spec=("svo_agent", "held", "hen"),
         gold="nell", text="Who held the hen?"),
    dict(qid="CC6", p="CC6", slice="CC", atype="AGENT", spec=("svo_agent", "fed", "colt"),
         gold="will", text="Who fed the colt?"),
    dict(qid="CC7", p="CC7", slice="CC", atype="AGENT", spec=("svo_agent", "held", "cat"),
         gold="kate", text="Who held the cat?"),
    dict(qid="CC8", p="CC8", slice="CC", atype="AGENT", spec=("svo_agent", "fed", "ox"),
         gold="sam", text="Who fed the ox?"),
]
...
def extract_passage(passage_text, assigner, coref_strategy):
    """Run coref pass + role assignment; emit relation triples with resolved heads.
    assigner: 'positional' or a callable (learned). coref_strategy: None (coref OFF) or a
    WorkingOverlay strategy string ('maintained' = the v3 claim; 'recency' = v2 must-beat).
    Returns (rels, reslog)."""
...
# Arms.
# =======================================================================================
# Per-arm (role-assigner, coref-strategy). ONE variable differs across the contrast pairs:
#   learned_full vs recency_coref   -> coref STRATEGY (maintained vs recency)  [the CC contrast]
#   learned_full vs learned_nocoref -> coref ON vs OFF                          [coref ablation]
#   learned_full vs positional      -> role-assigner (learned vs positional)    [extractor contrast]
#   learned_full vs frequency       -> reader-with-relations vs no-relation floor
ARM_CONFIG = {
    "learned_full":    ("learned", "maintained"),
    "recency_coref":   ("learned", "recency"),
    "learned_nocoref": ("learned", None),
    "positional":      ("positional", "maintained"),
    "frequency":       ("frequency", None),
}


def run_arm(arm, clf):
    """arm in ARM_CONFIG. Returns (per_q_correct, answers, stores, reslogs)."""
    learned = lambda tagged: assign_roles_learned(tagged, clf)  # noqa: E731
    kind, coref_strategy = ARM_CONFIG[arm]
    stores = {}
    reslogs = {}
    assigner = learned if kind == "learned" else ("positional" if kind == "positional" else None)
    if kind != "frequency":
        for pid, text in TEST_PASSAGES.items():
            rels, rlog = extract_passage(text, assigner, coref_strategy)
            stores[pid] = rels
            reslogs[pid] = rlog
    correct = []
    answers = []
...
def build_verdict(output_dir, run_mode):
    ...
    # ---- arms (5): learned_full=maintained coref is the CLAIM; recency_coref is the must-beat ----
    lf_c, lf_a, lf_store, lf_reslog = run_arm("learned_full", clf)
    rc_c, rc_a, rc_store, rc_reslog = run_arm("recency_coref", clf)
    ln_c, ln_a, _, _ = run_arm("learned_nocoref", clf)
    po_c, po_a, po_store, _ = run_arm("positional", clf)
    fr_c, fr_a, _, _ = run_arm("frequency", clf)

    digests = _arms_must_differ({"learned_full": lf_a, "recency_coref": rc_a,
                                 "learned_nocoref": ln_a, "positional": po_a, "frequency": fr_a})
    ...
    for arm, store in (("learned_full", lf_store), ("positional", po_store)):
        ...
        per_p = {}
        for pid in TEST_GOLD_RELS:  # RELF1 scored ONLY on REAL held-out primer passages (T1-T6)
    ...
    nc_idx = [i for i, q in enumerate(TEST_QS) if q["slice"] == "NC"]
    lf_co, n_co = slice_acc(lf_c, "CO")
    ln_co, _ = slice_acc(ln_c, "CO")
    fr_co, _ = slice_acc(fr_c, "CO")
    # (CRUX-1) COMPOSITION on the fair, powered, freq-decorrelated CMP slice (all arms).
    lf_cmp, n_cmp = slice_acc(lf_c, "CMP")
    rc_cmp, _ = slice_acc(rc_c, "CMP")
    ln_cmp, _ = slice_acc(ln_c, "CMP")
    po_cmp, _ = slice_acc(po_c, "CMP")
    fr_cmp, _ = slice_acc(fr_c, "CMP")
    # (CRUX-2) COMPETITIVE COREF on the CC slice: maintained (learned_full) vs recency (recency_coref).
    lf_cc, n_cc = slice_acc(lf_c, "CC")
    rc_cc, _ = slice_acc(rc_c, "CC")
    ln_cc, _ = slice_acc(ln_c, "CC")
    fr_cc, _ = slice_acc(fr_c, "CC")
    lf_nc, n_nc = slice_acc(lf_c, "NC")
    maintained_vs_recency_cc = lf_cc - rc_cc
    comp_margin_freq = lf_cmp - fr_cmp
    coref_lift = lf_co - ln_co
    p_coref, obs_coref = bootstrap_lift_p(lf_c, ln_c, co_idx, rng)
    p_cc, obs_cc = bootstrap_lift_p(lf_c, rc_c, cc_idx, rng)  # maintained-vs-recency on CC
    p_comp, obs_comp = bootstrap_lift_p(lf_c, fr_c, cmp_idx, rng)  # composition-vs-freq on CMP
    ...
    reasons = []
    checks = {}
    # CRUX-1: composition GENUINELY WORKS on the fair, powered, freq-decorrelated test.
    checks["composition"] = (
        lf_cmp >= HP_COMPOSITION and comp_margin_freq >= HP_COMP_MARGIN_FREQ
        and lf_cmp >= ln_cmp and lf_cmp >= rc_cmp)
    # CRUX-2: maintained-salience coref BEATS recency on the competitive (topical) cases.
    checks["maintained_beats_recency"] = (
        lf_cc >= HP_CC_MAINTAINED and maintained_vs_recency_cc >= HP_CC_MARGIN_RECENCY)
    # SANITY: the learned extractor still generalizes on real held-out primer (v2 escape holds).
    checks["relf1_generalizes"] = relf1_learned >= HP_RELF1_HELDOUT
    # SECONDARY (reported; contributes to HARD_PASS but validated already in v2): role-sensitivity.
    ...
    # PRIMARY gates = the two crux checks + the extraction sanity floor. HARD_PASS requires all three.
    primary_pass = checks["composition"] and checks["maintained_beats_recency"] and checks["relf1_generalizes"]
    # HARD_FAIL = composition still fails on the fair test, OR maintained fails to beat recency,
    #             OR extraction collapsed back to the old hand-rule wall.
    hard_fail = (lf_cmp < HF_COMPOSITION) or (lf_cmp <= fr_cmp) or (lf_cc <= rc_cc) \
        or (relf1_learned < HF_RELF1_HELDOUT)

    if primary_pass and checks["role_reversal_sensitive"] and checks["passive_beats_positional"]:
        verdict = "HARD_PASS"
        vmsg = (f"CG-REVIVAL EARNED: composition GENUINELY works on the fair, powered, freq-decorrelated "
                f"test -- learned_full CMP={lf_cmp:.3f} (>= {HP_COMPOSITION}) vs freq={fr_cmp:.3f} "
                f"(margin={comp_margin_freq:.3f}, p={p_comp:.3f}), vs nocoref={ln_cmp:.3f} vs recency={rc_cmp:.3f}; "
                f"MAINTAINED-salience coref beats RECENCY on competitive cases CC={lf_cc:.3f} vs "
                f"recency={rc_cc:.3f} (margin={maintained_vs_recency_cc:.3f}, p={p_cc:.3f}); "
                f"RELF1_heldout={relf1_learned:.3f}. Reading-grows-relations ENABLES composition (N_CMP={n_cmp}).")
    elif hard_fail:
        verdict = "HARD_FAIL"
        vmsg = (f"Composition/coref FAILS on the fair powered test: CMP_learned={lf_cmp:.3f} vs "
                f"freq={fr_cmp:.3f} (HF<{HF_COMPOSITION} or <=freq); maintained CC={lf_cc:.3f} vs "
                f"recency={rc_cc:.3f}; RELF1={relf1_learned:.3f}. A real limit -> DEFLATE + drill. Failed: {reasons}")
    else:
        verdict = "MIDDLE_BAND"
        vmsg = (f"PARTIAL: CMP={lf_cmp:.3f} (freq={fr_cmp:.3f}, margin={comp_margin_freq:.3f}); "
                f"maintained CC={lf_cc:.3f} vs recency={rc_cc:.3f} (margin={maintained_vs_recency_cc:.3f}); "
                f"RELF1={relf1_learned:.3f}; reversal={rev_learned:.2f}; passive_gap={passive_gap:.2f}. "
                f"Not all HARD_PASS gates: {reasons}")

    elapsed = time.perf_counter() - t0
    metrics = dict(
        verdict=verdict, verdict_msg=vmsg,
        summary=f"{verdict}: CMP={lf_cmp:.3f}(freq={fr_cmp:.3f}) CC_maint={lf_cc:.3f}(rec={rc_cc:.3f}) RELF1={relf1_learned:.3f}",
        ...
        heldout_source="mcguffey_primer.clean.txt (RELF1); composition/coref passages constructed (task-authorized)",
        arms=dict(
            learned_full=dict(acc_all=acc(lf_c), acc_NC=lf_nc, acc_CO=lf_co, acc_CC=lf_cc, acc_CMP=lf_cmp, per_q=lf_c, answers=lf_a),
            recency_coref=dict(acc_all=acc(rc_c), acc_CC=rc_cc, acc_CMP=rc_cmp, per_q=rc_c, answers=rc_a),
            learned_nocoref=dict(acc_all=acc(ln_c), acc_CO=ln_co, acc_CC=ln_cc, acc_CMP=ln_cmp, per_q=ln_c, answers=ln_a),
            positional=dict(acc_all=po_all, acc_CMP=po_cmp, per_q=po_c, answers=po_a),
            frequency=dict(acc_all=fr_all, acc_CO=fr_co, acc_CC=fr_cc, acc_CMP=fr_cmp, per_q=fr_c, answers=fr_a),
        ),
        relation_f1=relf1,
        discriminators=dict(
            # CRUX-1 composition
            composition_learned_full=lf_cmp, composition_frequency=fr_cmp,
            composition_nocoref=ln_cmp, composition_recency=rc_cmp, composition_positional=po_cmp,
            composition_margin_vs_freq=comp_margin_freq, composition_p_le0_vs_freq=p_comp, composition_obs=obs_comp,
            # CRUX-2 maintained vs recency coref
            cc_maintained=lf_cc, cc_recency=rc_cc, cc_nocoref=ln_cc,
            maintained_vs_recency_cc=maintained_vs_recency_cc, cc_p_le0=p_cc, cc_obs=obs_cc,
            # extraction generalization + secondary
            relf1_learned_heldout=relf1_learned, relf1_positional_heldout=relf1["positional"]["micro_f1"],
            coref_lift=coref_lift, coref_lift_p_le0=p_coref, coref_lift_obs=obs_coref,
            role_reversal_learned=rev_learned, role_reversal_positional=rev_pos, role_reversal_frequency=rev_freq,
        ...
        coref_resolutions_maintained={pid: rl for pid, rl in lf_reslog.items()},
        coref_resolutions_recency={pid: rl for pid, rl in rc_reslog.items()},
        slice_counts=dict(NC=n_nc, CO=n_co, CC=n_cc, CMP=n_cmp),
        bands=dict(HP_COMPOSITION=HP_COMPOSITION, HP_COMP_MARGIN_FREQ=HP_COMP_MARGIN_FREQ,
                   HF_COMPOSITION=HF_COMPOSITION, HP_CC_MAINTAINED=HP_CC_MAINTAINED,
                   HP_CC_MARGIN_RECENCY=HP_CC_MARGIN_RECENCY, HP_RELF1_HELDOUT=HP_RELF1_HELDOUT,
                   HF_RELF1_HELDOUT=HF_RELF1_HELDOUT, HP_REVERSAL_LEARNED=HP_REVERSAL_LEARNED,
                   MAX_REVERSAL_FREQ=MAX_REVERSAL_FREQ, HP_PASSIVE_LEARNED=HP_PASSIVE_LEARNED,
        ...
        gates=dict(baseline_in_band=baseline_in_band, freq_acc_all=fr_all, pos_acc_all=po_all,
                   freq_cannot_win_composition=(fr_cmp < lf_cmp), arms_differ_verified=True, arm_digests=digests),
        questions=[dict(qid=q["qid"], p=q["p"], slice=q["slice"], gold=q["gold"], text=q["text"],
                        learned_full=lf_a[i], recency_coref=rc_a[i], learned_nocoref=ln_a[i],
                        positional=po_a[i], frequency=fr_a[i]) for i, q in enumerate(TEST_QS)],
        provenance="RELF1 held-out=mcguffey_primer; overlay=hdlab.state_of_mind.WorkingOverlay "
                   "(strategy maintained vs recency); role-assigner=AveragedPerceptron over "
                   "word-identity-free features; composition/coref passages constructed + freq-decorrelated",
    )
    return metrics
...
def self_test():
    ...
    # v3 real_code_path: coref strategy threads through extract_passage (maintained vs recency differ on CC).
    lf_cc1, _ = extract_passage(TEST_PASSAGES["CC1"], lambda t: assign_roles_learned(t, clf), "maintained")
    rc_cc1, _ = extract_passage(TEST_PASSAGES["CC1"], lambda t: assign_roles_learned(t, clf), "recency")
    assert ("svo", "held", "ann", "pig") in lf_cc1, f"self-test: maintained CC1 she->ann, got {lf_cc1}"
    assert ("svo", "held", "kate", "pig") in rc_cc1, f"self-test: recency CC1 she->kate, got {rc_cc1}"

    # v3 INANIMATE-POSSESSOR fix: 'the pen has a rat' -> poss(pen,rat) even though pen is inanimate.
    rels_c7, _ = extract_passage(TEST_PASSAGES["C7"], lambda t: assign_roles_learned(t, clf), "maintained")
    assert ("poss", "pen", "rat") in rels_c7, f"self-test: inanimate-possessor poss(pen,rat), got {rels_c7}"
    assert ("poss", "cat", "pen") in rels_c7, f"self-test: poss(cat,pen), got {rels_c7}"

    # held-out primer extraction produces correct triples (generalization + inanimate poss(nest,eggs)).
    rels, rlog = extract_passage(TEST_PASSAGES["T1_hen"], lambda t: assign_roles_learned(t, clf), "maintained")
    assert ("loc", "nest", "box") in rels, f"self-test: nest-in-box should extract, got {rels}"
    rels_t4, _ = extract_passage(TEST_PASSAGES["T4_bird"], lambda t: assign_roles_learned(t, clf), "maintained")
    assert ("poss", "nest", "eggs") in rels_t4, f"self-test: inanimate poss(nest,eggs), got {rels_t4}"

    # ARMS-MUST-DIFFER (5 arms).
    lf_c, lf_a, _, _ = run_arm("learned_full", clf)
    rc_c, rc_a, _, _ = run_arm("recency_coref", clf)
    ln_c, ln_a, _, _ = run_arm("learned_nocoref", clf)
    po_c, po_a, _, _ = run_arm("positional", clf)
    fr_c, fr_a, _, _ = run_arm("frequency", clf)
    _arms_must_differ({"learned_full": lf_a, "recency_coref": rc_a, "learned_nocoref": ln_a,
                       "positional": po_a, "frequency": fr_a})
    print(f"SELF-TEST PASS: perceptron fit ({len(ex)} ex); canonical+passive roles; positional misses "
          f"passive; maintained!=recency on CC1 (ann vs kate); inanimate poss(pen,rat)+poss(nest,eggs) "
          f"extract; nest-in-box; 5 arms differ.")
    return 0
...
def main():
    ...
    sc = metrics["slice_counts"]
    print(f"[{run_mode}] {metrics['verdict']}: {metrics['verdict_msg']}")
    print(f"  CRUX-1 COMPOSITION (CMP N={sc['CMP']}): learned={d['composition_learned_full']:.3f} "
          f"freq={d['composition_frequency']:.3f} (margin={d['composition_margin_vs_freq']:.3f}, p={d['composition_p_le0_vs_freq']:.3f}) "
          f"nocoref={d['composition_nocoref']:.3f} recency={d['composition_recency']:.3f}")
    print(f"  CRUX-2 MAINTAINED vs RECENCY coref (CC N={sc['CC']}): maintained={d['cc_maintained']:.3f} "
          f"recency={d['cc_recency']:.3f} (margin={d['maintained_vs_recency_cc']:.3f}, p={d['cc_p_le0']:.3f})")
    print(f"  RELF1_heldout learned={d['relf1_learned_heldout']:.3f} positional={d['relf1_positional_heldout']:.3f}")
    print(f"  reversal learned={d['role_reversal_learned']:.2f} freq={d['role_reversal_frequency']:.2f}; "
          f"passive learned={d['passive_learned']:.2f} pos={d['passive_positional']:.2f} gap={d['passive_gap']:.2f}")
    print(f"  freq_cannot_win_composition={metrics['gates']['freq_cannot_win_composition']} "
          f"baseline_in_band={metrics['gates']['baseline_in_band']}")
    return 0
