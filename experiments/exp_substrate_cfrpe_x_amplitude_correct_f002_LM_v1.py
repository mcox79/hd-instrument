"""
substrate_cfrpe_x_amplitude_correct_f002_LM_v1 -- cf-RPE x amplitude-correct sparse-bipolar
...
SIX ARMS (6 arms x 3 seeds x N_DIM=8192 x N_TRAIN=100k text8):
  ARM_UNIGRAM                     -- analytic baseline
  ARM_HEBBIAN_f005_UNSCALED       -- baseline; reproduces fair_harness chain-grade 7.3065
  ARM_HEBBIAN_f002_UNSCALED       -- tests if low-f WITHOUT scaling collapses (shotgun: 6% recall)
...
PRE-REG BANDS (BPC; lift = lower BPC is better):
  ...
  Sanity rails:
    ARM_HEBBIAN_f002_UNSCALED BPC should be WORSE than ARM_HEBBIAN_f005_UNSCALED
      (per shotgun: 6% recall without amplitude scaling at f=0.02)
    ARM_HEBBIAN_f002_AMPLITUDE_SCALED should beat ARM_HEBBIAN_f005_UNSCALED
      (amplitude scaling at f=0.02 restores capacity per shotgun: 99% recall)
...
ANCHOR_NAME = "substrate_cfrpe_x_amplitude_correct_f002_LM_v1"
...
ARM_CONFIG: Dict[str, Tuple[float, bool, bool]] = {
    ...
    "ARM_HEBBIAN_f002_UNSCALED":       (F_LOW,      False, False),
...
def _instrumentation_selftest():
    ...
    # ST7: ARM_CONFIG has correct entries for all 5 plasticity arms
    ...
    print("[selftest] ST7 ARM_CONFIG verified for all 5 arms (combined f=%.3f amp=%s cfrpe=%s)" % (
        f_c, amp_c, cf_c), flush=True)
    ...
    print("[selftest] ALL 9 TESTS PASS", flush=True)
...
def compute_verdict(units: List[Dict]) -> tuple:
    ...
    heb_f002_unscaled_bpc = _arm_bpc("ARM_HEBBIAN_f002_UNSCALED")
    ...
    # Sanity rail evaluation
    heb_f002_collapse = (heb_f002_unscaled_bpc >= heb_f005_bpc)
    ...
    arm_summary = (
        "uni=%.3f | heb_f005_unsc=%.3f | heb_f002_unsc=%.3f | "
        "heb_f002_amp=%.3f | cfrpe_f005=%.3f | cfrpe_f002_amp=%.3f | "
        "lift_vs_heb=%.3f | lift_vs_cfrpe=%.3f | cv=%.3f | "
        "sanity[f002_collapse=%s scaled_lift=%s degen=%s]"
    ) % (
        unigram_bpc, heb_f005_bpc, heb_f002_unscaled_bpc,
        heb_f002_scaled_bpc, cfrpe_f005_bpc, cfrpe_f002_scaled_bpc,
        lift_vs_hebbian, lift_vs_cfrpe,
        combined_cv if math.isfinite(combined_cv) else -1.0,
        str(heb_f002_collapse), str(heb_f002_scaled_lift), str(degen_flag),
    )
    ...
    detail = {
        ...
        "heb_f002_unscaled_bpc": round(heb_f002_unscaled_bpc, 4),
        ...
        "sanity_heb_f002_collapse": bool(heb_f002_collapse),
