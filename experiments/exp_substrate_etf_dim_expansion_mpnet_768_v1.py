...
ANCHOR_NAME = "substrate_etf_dim_expansion_mpnet_768_v1"
MINILM_ID = "sentence-transformers/all-mpnet-base-v2"
...
if RUN_MODE == "smoke":
    SEEDS = [1]; N_ENC = 3000; D_GRID = [768, 1536]; LOADS = [0.02, 0.04, 0.06, 0.08, 0.1, 0.12, 0.14, 0.17, 0.2, 0.25]
else:
    SEEDS = [7, 17, 23]; N_ENC = 10000; D_GRID = [768, 1536, 4096]; LOADS = [0.02, 0.04, 0.06, 0.08, 0.1, 0.12, 0.14, 0.17, 0.2, 0.25]
...
def verdict(ps) -> Tuple[str, str]:
    ...
    if scale >= 3.0:
        return ("HARD_PASS", "HARD_PASS: dim-expansion rescue transfers to mpnet-768 (>=3x) -- rule is encoder-agnostic across sentence-transformer family. " + summary)
    if scale >= 1.5:
