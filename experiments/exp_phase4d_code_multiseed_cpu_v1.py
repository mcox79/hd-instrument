...
ANCHOR_NAME = "phase4d_code_multiseed_cpu_v1"
...
def _selftest():
    ...
    print("[selftest] PASS: phase4d-code-multiseed", flush=True)
_selftest()
if _ARGS.self_test:
    sys.exit(0)
def _load():
    ...
    import json
    ds = json.load(open(REPO / "experiments" / "data" / "mbpp" / "mbpp_full.json", encoding="utf-8"))   # bundled (RESCUE-2)
    def conv(sp):
        out = []
        for e in ds[sp]:
            pr = e.get("text") or e.get("prompt") or ""; cd = e.get("code") or ""
    ...
    return tr, te
def run() -> Dict:
    rng = np.random.default_rng(int(os.environ.get("HDLAB_SEED", "1020")))
    try:
        train, test = _load()
    ...
    LAB = sorted(set(t for _p, t in train)); Xtr = [(_feats(p), t) for p, t in train]
    gold_dist = Counter(t for _p, t in test); maj = gold_dist.most_common(1)[0][1] / len(test)
    SEEDS = [1, 2, 3] if SMOKE else [1, 2, 3, 4, 5]; vals = []
    for sd in SEEDS:
        ...
        cor = sum(1 for p, t in test if max(LAB, key=lambda l: (sum(avg[l].get(f, 0.0) for f in _feats(p)), l)) == t)
        vals.append(round(cor / len(test), 4))
    mean = sum(vals) / len(vals); std = (sum((v - mean) ** 2 for v in vals) / len(vals)) ** 0.5
    print("  PHASE4D-CODE n=%d: mean=%.4f std=%.4f vals=%s | majority=%.3f | %d classes" % (len(vals), mean, std, vals, maj, len(LAB)), flush=True)
    return {"accuracy": round(mean, 3), "std": round(std, 4), "majority": round(maj, 3), "n_classes": len(LAB), "n_test": len(test), "n_train": len(train), "n_seeds": len(vals)}
def verdict(r) -> Tuple[str, str]:
    if r.get("error"): return ("UNKNOWN", "UNKNOWN: " + r["error"])
    a = r["accuracy"]; mj = r["majority"]; s = "mean=%.4f std=%.4f " % (a, r.get("std", 0)) + "; acc=%.3f majority=%.3f (%d classes, n_test=%d, n_train=%d)" % (a, mj, r["n_classes"], r["n_test"], r["n_train"])
    lift = a - mj; sd = r.get("std", 1.0)
    if a >= 0.70 and lift >= 0.10 and sd <= 0.02:
        return ("HARD_PASS", "HARD_PASS: CODE algorithm-pattern classification SEED-ROBUST (mean>=0.70, std<=0.02, n=%d, 2x+ majority) -- TIER A; discriminative mechanism validated on CODE as on MATH, no LLM. " % r.get("n_seeds", 5) + s)
