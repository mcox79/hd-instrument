"""Bytes-per-fact Pareto v5 INT8-SPECIALIZATION, seed=13.

v5 vs v4:
  - Specialized 4-arm cell targeting the CG-eligible sub-claim from v4 MB:
    INT8 is Pareto-optimal in the M=40k-80k capacity crack.
  - Arms: FP32_DENSE (baseline) + INT8_DENSE (Pareto-optimal candidate) +
    BINARY_DENSE (Tier 3 anchor) + POSITIVE_CONTROL_NO_QUANT (identity path).
  - 4 arms x 2 M {40000, 80000} = 8 units/seed.
  - Discriminator: INT8_recall_mean >= FP32_recall_mean - 0.005 AND
    INT8_bpf <= 0.30 * FP32_bpf at BOTH M, cross-seed cv < 0.10.
...
SEED = 13
ANCHOR_NAME = f"substrate_bytes_per_fact_pareto_v5_seed_{SEED}"
...
CONFIG_VERSION = (
    ...
    f"arms=[FP32_DENSE,INT8_DENSE,BINARY_DENSE,POSITIVE_CONTROL_NO_QUANT],"
    f"N_DIM_DENSE=2048,"
    f"M_sweep_full=[40000,80000],M_sweep_smoke=[40000,80000],"
    f"n_ent=5000,n_rel=100,query_frac=0.10,topK=1,"
    f"SEED={SEED},mode={RUN_MODE},"
    f"discriminator=INT8_parity_AND_compression_at_M40k_AND_M80k+cross_seed_cv_lt_0p10,"
    f"parity_tol=0.005,compression_ratio_max=0.30,cross_seed_cv_max=0.10,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v5_bytes_per_fact_pareto_int8_specialization_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v5_bytes_per_fact_import_crash",
        }
        tmp = out_dir / "metrics.json.tmp"
        final = out_dir / "metrics.json"
...
def main() -> int:
    ...
    from experiments._substrate_bytes_per_fact_pareto_v5_core import (
    ...
    final["_hardening_marker"] = "v5_bytes_per_fact_pareto_int8_specialization_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = (
        "synthetic_uniform_KG_5000ent_100rel_M_sweep_40k_80k_holdout_10pct_queries"
    )
