"""
q_a3_l16_cross_layer_composition_v1_n4096 -- Q-A3: L=16 cross-layer composition at N=4096.

Ceiling chase: extends L=15 (HARD_PASS, all fids EXACT-1.0, 5-seed unanimous) to L=16.
  Architecture adds one more Hadamard binding level (L16 outer layer).
  16 W matrices x ~67 MB each = ~1072 MB total. Fits in 8 GB GPU.

PRE-REGISTERED BANDS (ceiling push, prior L=15 EXACT-1.0 unanimous):
...
FORMULA SELF-TESTS (PROT-022):
  1. L=16 chain: 16-step Hadamard roundtrip recovers xi_L1.
     [INPUT: 2-element +-1 vectors, 15 context ops] [EXPECTED: decode = xi_L1]
...
QUEUE: overnight_queue (GPU; 16 W matrices at N=4096).
"""
import sys
...
ANCHOR_NAME = "q_a3_l16_cross_layer_composition_v1_n4096"
...
def _selftest_l16_chain():
    """16-step Hadamard roundtrip recovers xi_L1."""
    ctxs = [
    ...
    for c in reversed(ctxs):
        xi_dec = xi_dec * c
    assert torch.allclose(xi_dec, xi_L1, atol=1e-6), f"L16 chain decode failed: {xi_dec}"
...
def _instrumentation_selftest():
    ...
    ms = [M_INNER, M_MID2, M_MID3, M_MID4, M_MID5, M_MID6, M_MID7, M_MID8,
          M_MID9, M_MID10, M_MID11, M_MID12, M_MID13, M_MID14, M_MID15, M_OUTER]
    ...
    del dummy
    print(f"[selftest] PASS: L16 chain decode ok, capacity_ok, gpu_mem_ok N={n_dim}", flush=True)
...
def run_seed(seed: int, n_dim: int) -> Dict:
    ...
    Xi_ctx16 = bsc(M_OUTER, n_dim)
    Xi_outer = Xi_ctx16 * Xi_mid15[:M_OUTER]
    ...
    for q_idx in range(min(N_QUERIES, M_OUTER)):
        ...
        xi_L16_ret = hopfield_retrieve_gpu(W_outer, probe)
        ...
        level_fids["L16"].append(cosine_sim_gpu(xi_L16_ret, xi_outer_true))
        ...
        l16_acc.append(cosine_sim_gpu(xi_L1_ret, xi_L1_true))

    mean_fids = {k: float(sum(v) / len(v)) if v else 0.0 for k, v in level_fids.items()}
    mean_l16 = float(sum(l16_acc) / len(l16_acc)) if l16_acc else 0.0
    ...
    print(f"  [seed={seed}] fids={' '.join(f'{k}:{mean_fids[k]:.4f}' for k in sorted(mean_fids.keys()))} "
          f"l16_acc={mean_l16:.4f} peak_gpu={peak_mem:.3f}GB elapsed={elapsed:.2f}s", flush=True)

    result = {
        "seed": seed, "N": n_dim, "run_mode": RUN_MODE,
        "l16_accuracy": float(mean_l16),
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    l16_acc = mean_key("l16_accuracy")

    fid_list = [fid_vals[f"L{l}"] for l in range(1, L_DEPTH + 1)]
    summary = (" ".join(f"L{l}:{fid_vals[f'L{l}']:.4f}" for l in range(1, L_DEPTH + 1)) +
               f" l16_acc={l16_acc:.4f}(HP>={HP_LACC} HF<{HF_LACC}) n_seeds={len(results)}")

    if any(f < HF_FIDELITY for f in fid_list) or l16_acc < HF_LACC:
        return ("HARD_FAIL", f"HARD_FAIL: any fid<{HF_FIDELITY} or l16_acc<{HF_LACC}. {summary}")

    all_unanimous = all(f >= HP_FIDELITY_UNANI_THRESH for f in fid_list)
    if all_unanimous and l16_acc >= HP_LACC:
        return ("HARD_PASS", f"HARD_PASS: all {L_DEPTH} levels EXACT-1.0 unanimous + l16_acc HP. {summary}")
...
metrics = {
    ...
    "per_seed": [
         ...
         **{f"fid_l{l}": r.get(f"fid_l{l}") for l in range(1, L_DEPTH + 1)},
         "l16_accuracy": r.get("l16_accuracy"),
