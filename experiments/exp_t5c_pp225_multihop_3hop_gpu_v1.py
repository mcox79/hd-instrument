"""
exp_t5c_pp225_multihop_3hop_gpu_v1 -- Path B rescue R3: supervised projection head (bypass cross-attn) -- GPU.
...
ANCHOR_NAME = "t5c_pp225_multihop_3hop_gpu_v1"; MODEL = "EleutherAI/pythia-160m"; ENCODER = "BAAI/bge-large-en-v1.5"
...
def _selftest():
    ...
    assert int(_n.argmax([0.1, 0.9])) == 1, "argmax"; print("[selftest] PASS: pp225-multihop-3hop", flush=True)
...
def run() -> Dict:
    ...
    for i, s in enumerate(subs):
        m = subs[(i + 13) % len(subs)]; b = subs[(i + 7) % len(subs)]                                    # colleague (hidden 2-hop intermediate)
        a = pool[int(g.integers(0, len(pool)))]
        prompt = "The secret code of the colleague of the manager of %s is" % s         # 2-hop: subject only, answer 2 hops away
        text = "The manager of %s is %s. The colleague of %s is %s. The secret code of %s is%s." % (s, m, m, b, b, a)   # KB states the full chain
        facts.append({"prompt": prompt, "aid": tok(a, add_special_tokens=False)["input_ids"][0], "text": text})
    ...
    def embed(texts):
        ...
        for i in range(0, len(texts), 64):
            b = enc_tok(texts[i:i + 64], return_tensors="pt", padding=True, truncation=True, max_length=48).to(DEV)
            with torch.no_grad():
    ...
    tr = recall(train, True); te = recall(test, True); prog.close(); del mdl, enc_mdl
...
def verdict(r) -> Tuple[str, str]:
    ...
    if hp >= 0.25:
        return ("HARD_PASS", "HARD_PASS: PP-225 delivers 2-HOP chain answers (held-out >=0.25) -- the projection composes with substrate multi-hop; the compositional MOAT pipes to the LLM. " + s)
    if hp >= 0.05:
