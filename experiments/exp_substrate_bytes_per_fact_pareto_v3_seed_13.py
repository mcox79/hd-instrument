"""Bytes-per-fact storage efficiency Pareto v3 M-EXTENDED + FP16-RANGE-FIX, seed=13.

v3 vs v2:
  - FP16 arm becomes FP16_DENSE_RANGE_SAFE: FP32 accumulator + FP16 storage.
    (v2's FP16 collapsed to 0.0 at all M; v2's BFLOAT16 stayed at 1.000
    confirming the collapse was a RANGE issue, not precision. v3 rescues FP16
    by accumulating in FP32 then downcasting for storage-cost accounting.)
  - M sweep extended to {1000,4000,10000,20000,40000} (+M=40k) to crack ceiling
    saturation. v2 landed 21/28 cells at recall=1.000 triggering META_RULE_Q.
  - META_RULE_Q strict gate: auto-demote HP -> MB if >=70% cells saturated.
...
Discriminators:
  ...
  - ceiling_saturation_ratio < 0.70 (META_RULE_Q strict)
...
SEED = 13
ANCHOR_NAME = f"substrate_bytes_per_fact_pareto_v3_seed_{SEED}"
...
CONFIG_VERSION = (
    ...
    f"M_sweep_full=[1000,4000,10000,20000,40000],M_sweep_smoke=[500,2000,8000],"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v3_bytes_per_fact_pareto_M_extended_fp16_range_safe_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v3_bytes_per_fact_import_crash",
        }
        tmp = out_dir / "metrics.json.tmp"
        final = out_dir / "metrics.json"
...
def main() -> int:
    ...
    from experiments._substrate_bytes_per_fact_pareto_v3_core import (
    ...
    final["_hardening_marker"] = "v3_bytes_per_fact_pareto_M_extended_fp16_range_safe_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = (
        "synthetic_uniform_KG_5000ent_100rel_M_sweep_1k_4k_10k_20k_40k_holdout_10pct_queries"
    )
