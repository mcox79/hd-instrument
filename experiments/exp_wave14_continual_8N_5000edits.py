...
zh held at M=8N continual, zi held at M=4N continual. zq pushes to M=8N
sequential editing to find substrate ceiling under combined stress.

Pre-reg: preregs/2026-05-21_wave14_continual_8N_5000edits.md
"""
...
def compute_verdict(summary):
    ...
    # Kerdock-only: correlated arm omitted (OOM at M=8N); kerdock is the load-bearing arm.
    if k_ok:
        return ("CONTINUAL_8N_KERDOCK_HOLDS",
                f"At M=8N, Kerdock holds 100 sequential edits: min_edited="
    ...
    return (f"CONTINUAL_8N_KERDOCK_FAILS_AT_{k_fail}",
            f"Kerdock fails at M=8N continual editing at edit {k_fail}. "
...
def self_test_verdict():
    ...
    cases = [
        ({"by_arm": {"kerdock": mk(0.98)}}, "CONTINUAL_8N_KERDOCK_HOLDS"),
...
def run_experiment(smoke):
    ...
    config = {"mode": "smoke" if smoke else "full", "N": 1024 if smoke else 4096,
              "M_stored": 2048 if smoke else 32768,  # M=8N — lower capacity for longer-horizon stress
              "n_edits": 10 if smoke else 5000,  # 10x continual stress
...
def run_smoke():
    out_dir = get_output_dir("wave14_continual_8N_5000edits_smoke")
...
def run_main():
    out_dir = get_output_dir("wave14_continual_8N_5000edits")
