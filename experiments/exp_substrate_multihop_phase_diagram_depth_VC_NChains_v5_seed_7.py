...
experiments._multihop_phase_diagram_v5_base.run_one_seed(seed=7).

V5 mechanism-class diversion (per v4 3/3 HARD_FAIL diagnosis):
  - sweep STORAGE_DENSITY not effective_V_C
  - 3 arms = HEBBIAN_W / DIRECT_ATTENTION / CHANCE (storage primitives)
  - primary discriminator = Pareto-split on (top1_recall, wall_s)
  - secondary = per-hop angle-drift cosine tracking
...
# PROT-020 routing-gate visibility: torch used in base module
...
try:
    ...
    sentinel = {
        "anchor_name": "substrate_multihop_phase_diagram_depth_VC_NChains_v5_seed_7",
        "verdict": "UNKNOWN",
