"""
a4_audit_during_training_v1 -- Cluster A4: kappa_3 audit signal during training.

SCIENTIFIC QUESTION (Phase 3, Cluster A4):
  Does the kappa_3 spectral fingerprint detect a training-injected anomaly (corrupted
  pattern write) within W <= 50 writes at 3-sigma, operating as a REAL-TIME audit
  primitive during substrate state updates?

  Physical setup:
  - Substrate stores M_CLEAN = 50 clean (normal) patterns via Hebbian writes.
  - At write W_ANOMALY (between 1 and 50), one CORRUPTED pattern is injected
    (adversarially flipped to maximize kappa_3 shift).
  - After EACH subsequent write, compute kappa_3 via Hutchinson estimator.
  - Test: does the kappa_3 value exceed 3-sigma from the clean baseline trajectory
    within W <= 50 total writes of the anomaly injection?

  kappa_3 = Tr(W^3) / N detects non-Gaussian structure in the stored patterns.
  A corrupted/adversarial pattern shifts kappa_3 because it breaks the
  near-Gaussian distributed pattern statistics of clean Hebbian updates.
...
PRE-REGISTERED HARD-FAIL:
  ...
  HF2: false-positive rate > 20% (too noisy to be a practical detector)

MIDDLE BAND:
  detection within 51-100 writes OR FPR 5-20%

P_deflated: 0.55 (kappa_3 confirmed at drift detection in PP-50 and COMBO-3;
  but adversarial anomaly detection at 3-sigma during training is a novel capability
  not directly tested before -- calibration probe, wider bands justified)

NOTE ON ANOMALY CONSTRUCTION:
  Corrupted pattern = pattern with maximum "kappa_3 contribution":
  xi_corrupt such that xi_corrupt^T W^2 xi_corrupt / N is maximally positive,
  i.e., xi_corrupt is aligned with the top eigenvector of W^2.
  This maximizes the leading term of kappa_3 shift per rank-1 update.

FORMULA SELF-TESTS:
  1. kappa_3 Hutchinson estimate: at alpha=0.05, kappa_3 ~ alpha * (1 + 2*alpha) ~ 0.055.
     [INPUT: N=512, M=25 (alpha=0.049)] [EXPECTED: kappa_3 in [0.03, 0.10]]
  2. kappa_3 shift from one write: Dk3 = -3/N^2 * xi^T W^2 xi (first-order).
     [INPUT: N=8, W=0, write 1 pattern] [EXPECTED: Dk3 = 3/N^2 * ||xi||^4 / N ~ 3/N]
  3. False positive test: 100 clean writes should stay within 3-sigma.
     [INPUT: N=512, M=50 clean writes, no anomaly] [EXPECTED: < 5% exceed 3-sigma]
...
ANCHOR_NAME = "a4_audit_during_training_v1"
...
if RUN_MODE == "smoke":
    SEEDS = [7, 17]
    N = 512
    M_CLEAN_BEFORE = 20   # writes before anomaly
    M_CLEAN_AFTER = 30    # writes after anomaly (max detection window)
    N_BASELINE_RUNS = 5   # number of clean runs for baseline stats
    N_HUTCHINSON = 100
    SIGMA_THRESHOLD = 3.0
else:
    SEEDS = [7, 17, 23, 31, 41]
    N = 1024
    M_CLEAN_BEFORE = 50
    M_CLEAN_AFTER = 50    # detection window: 50 writes
    N_BASELINE_RUNS = 20
    N_HUTCHINSON = 300
    SIGMA_THRESHOLD = 3.0

HP_DETECT_WITHIN_W = M_CLEAN_AFTER     # <= this many writes
HF_DETECT_WITHIN_W = M_CLEAN_AFTER * 4  # if > 4x window, never detected
HP_FPR = 0.05
HF_FPR = 0.20
...
def build_anomalous_pattern(W: np.ndarray, N_dim: int, rng: np.random.RandomState) -> np.ndarray:
    """Build pattern maximizing kappa_3 shift (aligned with top eigenvector of W^2)."""
    # Use power iteration to find top eigenvector of W^2
    v = rng.randn(N_dim)
...
def _selftest_kappa3_range():
    ...
    assert 0.01 < k3 < 0.20, f"kappa3_range selftest: k3={k3:.4f}"
    return k3
...
def run_clean_baseline(rng_base: np.random.RandomState, n_runs: int) -> Tuple[float, float]:
    """Run n_runs clean trajectories; return (mean_k3, std_k3) per write step."""
    all_k3s = []
...
def run_seed(seed: int) -> Dict:
    ...
    # Build clean baseline statistics
    rng_base = np.random.RandomState(seed + 100)
    baseline_mean, baseline_std = run_clean_baseline(rng_base, N_BASELINE_RUNS)
    if baseline_std < 1e-8:
        baseline_std = max(abs(baseline_mean) * 0.01, 1e-6)

    # FPR test: does a clean run exceed 3-sigma?
    n_fp = 0
    ...
    # Continue writing clean patterns and check kappa_3
    ...
    for step in range(M_CLEAN_AFTER):
        ...
        if detection_write is None and abs(z_score) > SIGMA_THRESHOLD:
            detection_write = step + 1  # 1-indexed
...
for seed in remaining:
    print(f"[seed={seed}] a4_audit_during_training N={N}...", flush=True)
