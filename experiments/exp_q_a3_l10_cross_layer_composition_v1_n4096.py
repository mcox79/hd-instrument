"""
q_a3_l10_cross_layer_composition_v1_n4096 -- Q-A3: L=10 cross-layer composition at N=4096.

Extends L=9 to L=10. Tests substrate compositional ceiling at depth-10.
...
Architecture:
  ...
  L10: M_outer=2, Hadamard binding ctx10*L9.

GPU MEMORY:
  10 W matrices x 67 MB each = 670 MB total. Safe on 8 GB GPU.

PRE-REGISTERED BANDS (Probe A, v342 routing):
  HARD-PASS: all 10 level fidelities = 1.0000 unanimous (5/5 seeds).
  MIDDLE: any L_fid in [0.85, 1.0).
  HARD-FAIL: any L_fid < 0.85 OR l10_acc < 0.5.

FORMULA SELF-TESTS:
  1. L=10 chain: 10-step Hadamard roundtrip recovers xi_L1.
...
ANCHOR_NAME = "q_a3_l10_cross_layer_composition_v1_n4096"
...
def _selftest_l10_chain():
    """10-step Hadamard roundtrip recovers xi_L1."""
    ctxs = [torch.tensor([1.0, -1.0], dtype=torch.float32, device=DEVICE),
            ...
            torch.tensor([1.0, 1.0], dtype=torch.float32, device=DEVICE),
            torch.tensor([-1.0, -1.0], dtype=torch.float32, device=DEVICE),
            torch.tensor([1.0, -1.0], dtype=torch.float32, device=DEVICE)]  # ctx2..ctx10
    ...
    for c in reversed(ctxs):
        xi_dec = xi_dec * c
    assert torch.allclose(xi_dec, xi_L1, atol=1e-6), f"L10 chain decode failed: {xi_dec}"


def _instrumentation_selftest():
    _selftest_l10_chain()
    n_dim = N_ACTIVE
    ms = [M_INNER, M_MID2, M_MID3, M_MID4, M_MID5, M_MID6, M_MID7, M_MID8, M_MID9, M_OUTER]
    ...
    del dummy
    print(f"[selftest] PASS: L10 chain decode ok, capacity_ok, gpu_mem_ok N={n_dim}", flush=True)
...
def run_seed(seed: int, n_dim: int) -> Dict:
    ...
    Xi_ctx10 = bsc(M_OUTER, n_dim)
    Xi_outer = Xi_ctx10 * Xi_mid9[:M_OUTER]
    ...
    for q_idx in range(min(N_QUERIES, M_OUTER)):
        ...
        xi_L10_ret = hopfield_retrieve_gpu(W_outer, probe)
        ...
        level_fids["L10"].append(cosine_sim_gpu(xi_L10_ret, xi_outer_true))
        ...
        l10_acc.append(cosine_sim_gpu(xi_L1_ret, xi_L1_true))

    mean_fids = {k: float(sum(v) / len(v)) if v else 0.0 for k, v in level_fids.items()}
    mean_l10 = float(sum(l10_acc) / len(l10_acc)) if l10_acc else 0.0
    ...
    print(f"  [seed={seed}] fids={' '.join(f'{k}:{mean_fids[k]:.4f}' for k in sorted(mean_fids.keys()))} "
          f"l10_acc={mean_l10:.4f} peak_gpu={peak_mem:.3f}GB elapsed={elapsed:.2f}s", flush=True)

    result = {
        "seed": seed, "N": n_dim, "run_mode": RUN_MODE,
        "l10_accuracy": float(mean_l10),
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    l10_acc = mean_key("l10_accuracy")

    fid_list = [fid_vals[f"L{l}"] for l in range(1, L_DEPTH + 1)]
    summary = (" ".join(f"L{l}:{fid_vals[f'L{l}']:.4f}" for l in range(1, L_DEPTH + 1)) +
               f" l10_acc={l10_acc:.4f}(HP>={HP_LACC} HF<{HF_LACC}) n_seeds={len(results)}")

    if any(f < HF_FIDELITY for f in fid_list) or l10_acc < HF_LACC:
        return ("HARD_FAIL", f"HARD_FAIL: any fid<{HF_FIDELITY} or l10_acc<{HF_LACC}. {summary}")

    all_unanimous = all(f >= HP_FIDELITY_UNANI_THRESH for f in fid_list)
    if all_unanimous and l10_acc >= HP_LACC:
        return ("HARD_PASS", f"HARD_PASS: all {L_DEPTH} levels EXACT-1.0 unanimous + l10_acc HP. {summary}")
...
metrics = {
    ...
    "per_seed": [
         ...
         **{f"fid_l{l}": r.get(f"fid_l{l}") for l in range(1, L_DEPTH + 1)},
         "l10_accuracy": r.get("l10_accuracy"),
