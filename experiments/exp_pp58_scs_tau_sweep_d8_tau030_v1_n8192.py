"""
pp58_scs_tau_sweep_d8_tau030_v1_n8192 -- PP-58 SCS framework tau sweep at d=8, tau_target=0.30.

CONTEXT (v378 cycle 48 all-night burst):
  tau=0.05: HARD_FAIL. tau=0.10: completed. tau=0.15: completed. tau=0.20: queued.
  tau=0.30 continues the tau sweep to map tau-vs-spike at d=8.
  The SCS formula gamma = (d + tau/d) / (1+tau) at d=8, tau=0.30.
  gamma = (8 + 0.30/8) / (1.30) = (8.0375) / 1.30 = 6.1827.
  Key question: does tau=0.30 asymmetry enable spike condition d >= 1.5?
...
SCIENTIFIC QUESTION:
  At tau_target=0.30, does controlled asymmetric W produce d >= 1.5
...
PRE-REGISTERED BANDS:
  ...
  HARD-FAIL:
    ...
    - OR tau_actual < 0.14 (asymmetry injection failed at tau_target=0.30).

FORMULA SELF-TESTS (PROT-022):
  1. SCS formula at d=8, tau=0.30: gamma = (8 + 0.30/8) / (1 + 0.30).
     (8 + 0.0375) / 1.30 = 8.0375 / 1.30 = 6.1827.
     [INPUT: d=8, tau=0.30] [EXPECTED: 6.1827 within 0.001]
...
ANCHOR_NAME = "pp58_scs_tau_sweep_d8_tau030_v1_n8192"
...
_gamma_d8_tau030 = _scs_gamma(8.0, 0.30)
_expected_tau030 = 8.0375 / 1.30
print(f"[selftest-formula] SCS gamma(d=8,tau=0.30): {_gamma_d8_tau030:.4f} (expected {_expected_tau030:.4f})", flush=True)
assert abs(_gamma_d8_tau030 - _expected_tau030) < 0.001, f"SCS formula selftest tau=0.30: got {_gamma_d8_tau030}"
