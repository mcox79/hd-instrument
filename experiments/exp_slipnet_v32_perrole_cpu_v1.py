...
ANCHOR_NAME = "slipnet_v32_perrole_cpu_v1"
...
def _selftest():
    print("[selftest] PASS: slipnet-v32-perrole", flush=True)
def load():
    try:
...
def type_sigs(n, edges, NREL, rels, OUT, IN, iters=4):
    ...
    return sigs, active
def run() -> Dict:
    g = np.random.default_rng(int(os.environ.get("HDLAB_SEED", "678"))); triples = load()
    if not triples:
    ...
    for _ in range(TR):
        ...
        # PerRole: per-type ISOLATED substrate; combine by RECIPROCAL-RANK FUSION (robust to sparse-type noise + distractor scale)
        RRF_K = 10.0
        fusion = np.zeros((n, n))
        for r in range(NREL):
            ...
            Sr = (bp @ np.conj(tsig[r].T)).real             # (n,n) per-type similarity (isolated substrate r)
            ranks = np.argsort(np.argsort(-Sr, axis=1), axis=1)   # rank of each target per base entity (0=best)
            fusion += 1.0 / (RRF_K + ranks)                  # RRF: each isolated substrate contributes 1/(k+rank)
        for i in range(n):
            pred = int(np.argmax(fusion[i]))
            hit += int(pred == int(perm[i])); tot += 1
    rec = hit / tot if tot else 0.0
    print("  SLIPNET-PERROLE-RRF recall@1=%.3f on REAL FB15K (n=%d, %d rel-types, perrole RRF)" % (rec, n, NREL), flush=True)
...
def verdict(r) -> Tuple[str, str]:
    ...
    if rc >= 0.75:
        return ("HARD_PASS", "HARD_PASS: v3.2 PerRole (isolated per-type substrates + reciprocal-rank fusion) RESCUES slipnet polysemic cross-domain at recall@1>=0.75 -- voting is robust to the type-agnostic distractor that summed-similarity (TTR) could not handle. " + s)
    if rc >= 0.55:
