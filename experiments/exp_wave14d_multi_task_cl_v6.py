...
Pre-reg: preregs/2026-05-21_wave14d_multi_task_cl_v6.md
"""
...
REPLAY_FRAC = 0.30  # Ibrahim 2024 top end of 5-25% range
...
def run_one_seed(seed, config, device):
    ...
    # v6: EMA blend final W_ABC with W_A baseline to bias retention upward.
    # alpha=0.7 means 70% of post-C state, 30% of original A baseline.
    EMA_ALPHA = 0.7
...
def run_experiment(smoke):
    ...
    for seed in config["seeds"]:
        print(f"[seed {seed}] training A -> B -> C...", flush=True)
        r = run_one_seed(seed, config, device)
        per_seed[str(seed)] = r
        print(f"  seed={seed}: retention_A={r['retention_A']:.3f} "
              f"retention_B={r['retention_B']:.3f} gain_C={r['gain_C']:.4f} "
              f"bwt={r['bwt']:+.4f}", flush=True)
...
def run_smoke():
    out_dir = get_output_dir("wave14d_multi_task_cl_v6_smoke")
...
def run_main():
    out_dir = get_output_dir("wave14d_multi_task_cl_v6")
