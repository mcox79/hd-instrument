"""
exp_t5c_pp225_kb50k_v2_genuine_v1 -- Path B rescue R3: supervised projection head (bypass cross-attn) -- GPU.
...
ANCHOR_NAME = "t5c_pp225_kb50k_v2_genuine_v1"; MODEL = "EleutherAI/pythia-1.4b"; ENCODER = "BAAI/bge-large-en-v1.5"
STEPS = 100 if "--smoke" in sys.argv else 30000
RUN_MODE = ("smoke" if "--smoke" in sys.argv else os.environ.get("HDLAB_RUN_MODE", "full")).lower()
_ap = argparse.ArgumentParser(); _ap.add_argument("--smoke", action="store_true"); _ap.add_argument("--self-test", action="store_true"); _ARGS, _ = _ap.parse_known_args()
SMOKE = RUN_MODE == "smoke"
N_FACTS = 200 if SMOKE else 50000
...
def _selftest():
    ...
    assert int(_n.argmax([0.1, 0.9])) == 1, "argmax"; print("[selftest] PASS: pp225-kb50k-v2-genuine", flush=True)
...
def run() -> Dict:
    ...
    for step in range(STEPS):
        ...
        if step % 300 == 0:
            tr = recall(train[:1500], True); te = recall(test, True)
    ...
    tr = recall(train[:1500], True); te = recall(test, True); prog.close(); del mdl
