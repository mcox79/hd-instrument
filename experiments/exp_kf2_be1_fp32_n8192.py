"""KF-2 BE-1 PRECISION: fp32 baseline at N=8192.

Delegates to exp_kf2_be1_precision_sweep_n8192 with PRECISION='fp32'.
fp32 is the baseline no-op (quantize_roundtrip is byte-exact).
Expected: max_iso < 0.05 (same as v268 HARD_PASS).

N-suffix: _n8192 -> production N = 8192 (PROT-018 binding).
Anchor: kf2_be1_fp32_n8192
...
# Load sweep driver (triggers its selftest at import)
_drv_path = REPO / "experiments" / "exp_kf2_be1_precision_sweep_n8192.py"
_drv_spec = importlib.util.spec_from_file_location("be1_drv_fp32", _drv_path)
...
def get_output_dir() -> Path:
    name = os.environ.get("HDLAB_EXP_NAME", "kf2_be1_fp32_n8192")
...
def _instrumentation_selftest() -> None:
    ...
    assert cell.get("precision_compression_ratio", 0) == 1.0, (
        f"fp32 compression must be 1.0; got {cell.get('precision_compression_ratio')}")
    print(f"[selftest] kf2_be1_fp32_n8192 PASS iso={cell['isolation_ratio']:.5f}", flush=True)
...
def main():
    ...
    print(f"[run] kf2_be1_fp32_n8192 smoke={smoke} N={N_cfg} precision={PRECISION} "
    ...
    if max_iso >= _drv.HF_CONTAMINATION:
        verdict = "KF2_BE1_FP32_HARD_FAIL"
        verdict_msg = f"FP32_BASELINE_BROKEN: max_iso={max_iso:.5f} >= {_drv.HF_CONTAMINATION}"
    elif seeds_pass >= _drv.HP_SEEDS_MIN:
        verdict = "KF2_BE1_FP32_HARD_PASS"
        verdict_msg = (f"FP32_BASELINE_HOLDS: {seeds_pass}/{len(seeds)} seeds pass, "
                       f"max_iso={max_iso:.5f} < {_drv.HP_ISOLATION_MAX}, "
                       f"theory_bound={theory_bound:.5f}")
    else:
        verdict = "KF2_BE1_FP32_MIDDLE_BAND"
        verdict_msg = f"FP32_PARTIAL: {seeds_pass}/{len(seeds)} seeds pass, max_iso={max_iso:.5f}"

    elapsed = round(time.time() - t0, 2)
    summary = {
        "anchor": "kf2_be1_fp32_n8192", "precision": PRECISION, "N": N_cfg, "smoke": smoke,
