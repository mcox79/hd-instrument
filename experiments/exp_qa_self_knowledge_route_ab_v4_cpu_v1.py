...
ANCHOR_NAME = "qa_self_knowledge_route_ab_v4_cpu_v1"
...
def route_A(atoms, args):
    """v2: name/aliases/id keyword match (any kw) OR description match requiring >=min(2,len(kws)) distinct topic keywords
    (recovers DESC-ONLY gold -- the topic kw is in the description but not the name -- with a precision threshold)."""
    topic = args["topic"].lower(); kws = [w for w in topic.replace("-", " ").split() if w not in STOP and len(w) > 2]
    kset = set(kws); thr = min(2, len(kset)) if kset else 1
    out = set()
    for a in atoms:
        nameid = (a.name + " " + " ".join(getattr(a, "aliases", []) or []) + " " + (a.id or "")).lower()
        if any(k in nameid for k in kset):
            out.add(_norm(a.id)); continue
        desc = (getattr(a, "description", "") or "").lower()
        if desc and sum(1 for k in kset if k in desc) >= thr:
            out.add(_norm(a.id))
    return out
