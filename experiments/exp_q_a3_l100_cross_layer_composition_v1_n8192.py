"""
q_a3_l100_cross_layer_composition_v1_n8192 -- Q-A3/PP-12: L=100 cross-layer composition at N=8192.

CONTEXT (v381 cycle 51 all-night burst):
  N=8192 series: L=100 is the CENTURY RUNG in the N=8192 depth ladder.
  Past L=99 (expected HARD_PASS). Per-stage alpha = 100/8192 = 0.0122 << alpha_c=0.138.
  Theoretical prediction: fidelity holds EXACT-1.0 since all M_k/N << alpha_c.

PRE-REGISTERED BANDS (Q-A3 L=100 N=8192 CENTURY RUNG):
...
ANCHOR_NAME = "q_a3_l100_cross_layer_composition_v1_n8192"
_N_SUFFIX = 8192
N = 8192
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    if all(f >= HP_FIDELITY_UNANI_THRESH for f in fid_list) and lacc >= HP_LACC:
        return ("HARD_PASS", f"HARD_PASS: all {L_DEPTH} levels EXACT-1.0 at N={N}. CENTURY RUNG CONFIRMED. {summary}")
...
print(f"[config] N={N} n_active={N_ACTIVE} mode={RUN_MODE} L={L_DEPTH} CENTURY_RUNG=True", flush=True)
...
metrics = {"anchor_name": ANCHOR_NAME, "verdict": verdict, "verdict_msg": verdict_msg,
    "N": N, "L": L_DEPTH, "century_rung": True, "run_mode": RUN_MODE, "n_seeds": len(SEEDS),
