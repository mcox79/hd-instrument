"""
exp_substrate_vs_bare_llm_hotpot_v1 -- NORTH STAR: substrate-augmented small LLM vs bare small LLM, answer F1 -- GPU.
...
ANCHOR_NAME = "substrate_vs_bare_llm_hotpot_v1"
...
def verdict(r) -> Tuple[str, str]:
    fa = r["aug"]; fb = r["bare"]; lift = fa - fb
    summary = "substrate-augmented F1=%.3f bare-LLM F1=%.3f lift=%+.3f (n=%d, Qwen2.5-1.5B + bge-small top-%d)" % (fa, fb, lift, r["n"], TOPK)
    if lift >= 0.15:
        return ("HARD_PASS", "HARD_PASS: substrate-augmented small LLM beats bare small LLM by >=0.15 answer F1 -- the assembled system exceeds the bare model at relative size (the north-star thesis, on answer quality). " + summary)
    if lift >= 0.05:
        return ("MIDDLE_BAND", "MIDDLE_BAND: augmented beats bare by 0.05-0.15 -- real but modest system advantage. " + summary)
    return ("HARD_FAIL", "HARD_FAIL: augmented lift <0.05 -- retrieval context does not improve the small LLM's answers here (HotpotQA needs multi-hop reasoning the context dump doesn't supply). " + summary)
