"""Continual editing at under-capacity M=N/2. Tests if edit cliff scales with M.

Pre-reg: preregs/2026-05-21_wave14yu_continual_editing_undercap.md
"""
...
def compute_verdict(summary):
    ...
    if "kerdock" not in arms or "correlated" not in arms:
        return ("CONTINUAL_UC_INCONCLUSIVE", "Missing.")
    kerdock = arms["kerdock"]
    correlated = arms["correlated"]
    k_ok, _ = yc.arm_passes(kerdock)
    c_ok, _ = yc.arm_passes(correlated)
    c_fail = yc.first_fail_edit(correlated["per_edit_trajectory"], "edited_acc",
                                   yc.PASS_EDITED) or yc.first_fail_edit(
        correlated["per_edit_trajectory"], "kept_acc", yc.PASS_KEPT)
    if k_ok and c_ok:
        return ("CONTINUAL_UC_BOTH_HOLD",
                f"At M=N/2, both arms hold 100 edits. Correlated didn't fail at "
                f"under-capacity. Failure mode only kicks in at higher M.")
    if k_ok:
        return ("CONTINUAL_UC_KERDOCK_HOLDS",
                f"Kerdock holds at M=N/2; correlated fails at edit {c_fail}. "
                f"Even at under-capacity, structured keys matter for continual editing.")
    return ("CONTINUAL_UC_KERDOCK_FAILS",
            f"Kerdock fails at M=N/2. Regression. Audit.")
...
def self_test_verdict():
    ...
    cases = [
        ({"by_arm": {"kerdock": mk(0.98), "correlated": mk(0.5, fail_e=2)}},
         "CONTINUAL_UC_KERDOCK_HOLDS"),
        ({"by_arm": {"kerdock": mk(0.98), "correlated": mk(0.98)}}, "CONTINUAL_UC_BOTH_HOLD"),
        ({"by_arm": {"kerdock": mk(0.5, fail_e=5), "correlated": mk(0.5, fail_e=2)}},
         "CONTINUAL_UC_KERDOCK_FAILS"),
        ({}, "CONTINUAL_UC_INCONCLUSIVE"),
    ]
    for s, exp in cases:
...
def run_experiment(smoke):
    ...
    config = {"mode": "smoke" if smoke else "full",
              ...
              "M_stored": 512 if smoke else 2048,  # M = N/2
              "n_edits": 10 if smoke else 100,
...
def run_smoke():
    out_dir = get_output_dir("wave14yu_continual_editing_undercap_smoke")
...
def run_main():
    out_dir = get_output_dir("wave14yu_continual_editing_undercap")
