"""
exp_headtohead_math_vs_llm_1p5b_gpu_v1.py -- north-star head-to-head (MINIMAL, no numpy) -- CPU.
...
OUT = REPO / "data" / "exp_headtohead_math_vs_llm_1p5b_gpu_v1"
def _ans(x):
...
def main():
    ...
    tok = AutoTokenizer.from_pretrained(name); DEV = "cuda" if torch.cuda.is_available() else "cpu"
    mdl = AutoModelForCausalLM.from_pretrained(name, dtype=torch.float16 if DEV=="cuda" else torch.float32).to(DEV); mdl.eval()
    print("[model] loaded", flush=True)
    data = _load(); SAMPLE = 25 if SMOKE else 150
    rows = {}; wins = 0; lat = {}
    for bench, items in data.items():
        ...
        for q, gold in items:
            ...
            ins = tok(p, return_tensors="pt").to(DEV); t0 = time.time()
            with torch.no_grad():
    ...
    json.dump({"anchor_name": "headtohead_math_vs_llm_1p5b_gpu_v1", "verdict": verdict, "verdict_msg": msg, "per_benchmark": rows, "llm_latency_s": lat, "wins": wins}, open(OUT / "metrics.json", "w", encoding="utf-8"))
    print("[metrics] written", flush=True)
if "--self-test" in sys.argv:
    assert _pred("answer 42") == 42; print("[selftest] PASS: headtohead-1p5b-gpu", flush=True); sys.exit(0)
main()
