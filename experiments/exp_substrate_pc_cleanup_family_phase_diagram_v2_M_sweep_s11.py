"""substrate_pc_cleanup_family_phase_diagram_v2_M_sweep sibling seed=11.

v2 (2026-07-03): adds M as swept axis to v1's cleanup-family phase diagram.
Physics-law arc: candidate PHYSICS_LAW_cleanup_mechanism_M_scaling_non_Hebbian
(CG_META tier eligible; analog to SCALE_FREE_in_N and TOPOLOGY_FREE).

Sweep: 4 cleanups x 6 M x 2 N x 3 c x 2 iters = 288 pts/seed FULL;
       4 x 3 x 1 x 2 x 1 = 24 pts/seed SMOKE.

CHUNKED architecture (USER 2026-06-28): one seed per sibling file.
Sibling pair: seed_17, seed_23.

PRE-REG: preregs/2026-07-03_substrate_pc_cleanup_family_phase_diagram_v2_M_sweep.md
CARDINALITY_OK_FULL: 288 phase points per seed
CARDINALITY_OK_SMOKE: 24 corner points per seed
...
SEED = 11
ANCHOR_NAME = f"substrate_pc_cleanup_family_phase_diagram_v2_M_sweep_s{SEED}"
...
CONFIG_VERSION = (
    ...
    f"encoder=binary_bipolar,"
    f"M_sweep=[100,200,400,800,1600,3200],N_sweep=[2048,8192],"
    f"SEED={SEED},mode={RUN_MODE},"
    f"corruption=[0.20,0.35,0.50],iters=[1,5],"
    f"beta=8.0,alpha_soft=0.5,arms=[MECHANISM,RANDOM_FLOOR],"
    f"expected_n_full=288,expected_n_smoke=24,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v2_pc_cleanup_family_M_sweep_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: Exception) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v2_pc_cleanup_family_M_sweep_import_crash",
        }
        tmp = out_dir / "metrics.json.tmp"
        final = out_dir / "metrics.json"
...
def main() -> int:
    ...
    from experiments._substrate_pc_cleanup_family_phase_diagram_v2_M_sweep_core import (
    ...
    final["_hardening_marker"] = "v2_pc_cleanup_family_M_sweep_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = "synthetic_substrate_4_cleanup_family_pc_M_sweep_v2"
