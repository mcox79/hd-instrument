"""substrate_seqbind_encoder_family_sweep_v1 sibling seed=19.

COMPONENT-SUBSTITUTION phase diagram for sequence_binding chain-grade primitive.
Outer axis: ENCODER FAMILY {HRR, FHRR, SPARSE, BIN}.
Inner axis: K (sequence length) x N (dimensionality).

Sweeps 4 encoders x 6 K x 3 N = 72 grid points per seed; 3 arms
(SUBSTRATE / SHUFFLE / RANDOM) per point; 50 queries per point (FULL).

CHUNKED architecture: one seed per sibling.
Sibling triple: seed_7, seed_13, seed_19.

PRE-REG: preregs/2026-06-28_substrate_seqbind_encoder_family_sweep_v1.md
CARDINALITY_OK_FULL: 72 phase points x 3 arms x 50 queries = 10800 records per seed
...
SEED = 19
ANCHOR_NAME = f"substrate_seqbind_encoder_family_sweep_v1_seed_{SEED}"
...
CONFIG_VERSION = (
    ...
    f"encoders=[HRR,FHRR,SPARSE,BIN],"
    f"K=[10,20,50,100,200,500],N=[1024,4096,8192],Q=1_fixed,"
    f"arms=[SUBSTRATE,SHUFFLE,RANDOM],"
    f"V_ITEMS=600,V_POS=600,n_queries_full=50,n_queries_smoke=4,"
    f"sparse_density=0.02,"
    f"expected_n_full=72,expected_n_smoke=6,"
    f"bands=SAT0.90_MB[0.30,0.70]_FLOOR0.10,"
    f"hardening=L1early+L2perpt+L3outertry+L4importsentinel+CHUNKED_PER_SEED+AF_encoder_hash"
)
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v1_seqbind_encoder_family_sweep_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v1_seqbind_encoder_family_sweep_import_crash",
        }
...
def main() -> int:
    ...
    from experiments._substrate_seqbind_encoder_family_sweep_v1_core import (
        run_one_seed_phase_diagram, aggregate_and_verdict, selftest,
        get_backend_label, ENCODER_FAMILIES, K_VALUES, N_VALUES,
    ...
    final["_hardening_marker"] = "v1_seqbind_encoder_family_sweep_chunked"
    final["backend"] = backend
    final["seed"] = SEED

    # cardinality: count phase points + records.
    if SMOKE_MODE:
        expected_pts = len(SMOKE_CORNERS)
        expected_records = expected_pts * 3 * N_QUERIES_SMOKE
    else:
        expected_pts = len(ENCODER_FAMILIES) * len(K_VALUES) * len(N_VALUES)
