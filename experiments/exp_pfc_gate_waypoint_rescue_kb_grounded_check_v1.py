...
ANCHOR_NAME = "pfc_gate_waypoint_rescue_kb_grounded_check_v1"
PARENT_ANCHOR = "pfc_gate_waypoint_rescue_replay_bidirectional_v1"
...
ARMS = ["flat_gonogo", "oracle_exec", "hier_oracle", "hier_shuffled",
        ...
        "wp_replay_generate_select", "wp_kb_grounded_gate", "wp_random_state", "wp_index_midpoint"]
RESCUE_ARM = "wp_kb_grounded_gate"         # the NEW mechanism under test (fixed, not a max-over-arms)
...
def _eval_regime(n_ops: int, V: int, dd: int, E: torch.Tensor, W_ops: List[torch.Tensor],
                 ...
                 C: torch.Tensor, reach_cum: Dict[int, torch.Tensor], tau_short: float, tau_long: float,
                 train_by_d, test_by_d, g: np.random.Generator,
                 disc_gen: np.random.Generator, rgen: torch.Generator) -> Dict[str, Any]:
    """Tune on train, evaluate all 12 arms on test (paired). One seed. KEY comparator = verify (the
    self-derived-correction MUST-FAIL control); RESCUE = wp_kb_grounded_gate (exogenous)."""
    ...
    # best_rescue = kb (FIXED, the new mechanism); KEY comparator = verify (self-derived MUST-FAIL control)
    ...
    for (V, n_ops) in sorted(by_group.keys()):
        ...
        for dd in depths_needed:
            rec = _eval_regime(n_ops, V, dd, E, W_ops, M, M_long, M_rev, R_short, R_long, R_rev, C,
                               reach_cum, tau_short, tau_long, train_by_d, test_by_d, g, disc_gen, rgen)
            ...
            print("[seed=%d %s ent=%.2f] FLAT=%.3f OEXEC=%.3f HORC=%.3f | OPEN=%.3f VER=%.3f "
                  "C2F=%.3f CMB=%.3f REPLAY=%.3f KB=%.3f RAND=%.3f IDX=%.3f (a=%.2f rr=%.3f degen=%.3f "
                  "taut=%.3f exact=%.3f | indep_corr=%.3f kb_conf=%.3f kb_std=%.3f m_err=%.3f "
                  "kb_fresh=%.3f kb_fb=%.3f best=%s)"
                  % (seed, key, rec["entropy"], a["flat_gonogo"], a["oracle_exec"], a["hier_oracle"],
                     ...
                     a["wp_bisect_combo"], a["wp_replay_generate_select"], a["wp_kb_grounded_gate"],
                     a["wp_random_state"], a["wp_index_midpoint"], rec["best_alpha"],
                     rec["reach_rank_test"], rec["degenerate_rate"], rec["anti_tautology_corr"],
                     rec["exact_match_rate"], rec["independence_corr"], rec["kb_confirm_mean"],
                     rec["kb_confirm_std"], rec["m_error_mean"], rec["kb_fresh_rate"],
                     rec["kb_fallback_rate"], rec["best_rescue_arm"]), flush=True)
...
def aggregate_and_verdict(per_seed: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if not cardinality_ok:
        ...
        # no material lift over the SELF-DERIVED-correction control (verify) despite a rigorously
        # exogenous, zero-shared-param KB-grounded channel -> bound quadruply+ confirmed structural.
        verdict = "HARD_FAIL_COMPOUNDING_ERROR_BOUND_REAL"
    elif fv["abs_independence_corr"] > HF_INDEP_CORR_CEIL:
        # lift exists BUT the KB-confirm signal is strongly correlated with the SR estimator's own error
        # -> the "exogenous" channel is contaminated / not independent; the mechanism's premise is void.
        ...
        # kb helps over verify (delta > 0.05, flatness >= 0.2) but sub-threshold or a guard failed
    ...
    grid_str = " ".join(
        "%s(e%.1f:F%.2f/O%.2f/OPEN%.2f/VER%.2f/KB%.2f/RAND%.2f ic=%.2f kbm=%.2f)" % (
            ...
            per_regime[rk]["arm_means"]["wp_kb_grounded_gate"], per_regime[rk]["wp_random_state"],
            per_regime[rk]["independence_corr"], per_regime[rk]["kb_confirm_mean"])
    ...
    if fv is not None:
        head = ("%s | FOCUS=%s(ent=%.1f steps=%d) FLAT=%.3f OEXEC=%.3f HIER_ORACLE=%.3f | OPEN=%.3f "
                "VERIFY=%.3f KB=%.3f recov_ver=%.3f recov_kb=%.3f DELTA(vs_ver)=%.3f flatness=%.3f | "
                "indep_corr=%.3f kb_confirm=%.3f kb_std=%.3f m_err=%.3f kb_fresh=%.3f kb_fb=%.3f | "
                "lift_flat=%.3f lift_random=%.3f lift_verify=%.3f | index_gap=%.3f anti_taut=%.3f "
                "degen=%.3f sign_p=%.4g rr=%.3f cv=%.3f | REPLAY=%.3f | spr(delta,ent)=%.3f | "
                "GRID [%s] n_seeds=%d") % (
            ...
            fv["independence_corr"], fv["kb_confirm_mean"], fv["kb_confirm_std"], fv["m_error_mean"],
            fv["kb_fresh_rate"], fv["kb_fallback_rate"],
            ...
            fv["brs_cv"], fv["arm_means"]["wp_replay_generate_select"],
...
def _verdict_selftest() -> None:
    def _mk(n_ops, V, dd, flat, oexec, horc, open_a, ver, kb, rand, idxm,
            degen=0.02, taut=0.10, exact=0.20, rr=0.60,
            n_res_only=None, n_idx_only=2, n_rand_only_idx=2,
            indep_corr=0.05, kb_conf=0.5, indep_degen=False):
        # rescue arm = kb (FIXED); KEY comparator = verify. paired KEY = kb-vs-verify.
        if n_res_only is None:
            n_res_only = 45 if kb > ver + 0.1 else 1
        ...
        oth = {"flat_gonogo": "f", "oracle_exec": "oracle_true_seq", "hier_oracle": "ho",
               ...
               "wp_kb_grounded_gate": "kb", "wp_random_state": "rd", "wp_index_midpoint": "ix"}
    ...
    try:
        ...
        for s in ["7", "17", "23"]:
            ps[s] = {"regime_results": {
                reg_lo: _mk(4, 1200, 4, flat=0.515, oexec=0.957, horc=0.953, open_a=0.823,
                            ver=0.823, kb=0.72, rand=0.09, idxm=0.10),
                reg_hi: _mk(4, 1200, 8, flat=0.081, oexec=0.918, horc=0.906, open_a=0.097,
                            ver=0.096, kb=0.36, rand=0.02, idxm=0.02),
            }}
        ...
        assert out["focus_best_rescue_arm"] == "wp_kb_grounded_gate", "ST13 best_rescue=kb"
        ...
        for s in ps:
            ps[s]["regime_results"][reg_hi] = _mk(4, 1200, 8, flat=0.081, oexec=0.918, horc=0.906,
                                                  open_a=0.097, ver=0.096, kb=0.10,
        ...
        for s in ps:
            ps[s]["regime_results"][reg_hi] = _mk(4, 1200, 8, flat=0.081, oexec=0.918, horc=0.906,
                                                  open_a=0.097, ver=0.096, kb=0.15,
        ...
        for s in ps:
            ps[s]["regime_results"][reg_lo] = _mk(4, 1200, 4, flat=0.515, oexec=0.957, horc=0.953,
                                                  open_a=0.823, ver=0.823, kb=0.646,
                                                  rand=0.09, idxm=0.10)
            ps[s]["regime_results"][reg_hi] = _mk(4, 1200, 8, flat=0.081, oexec=0.918, horc=0.906,
                                                  open_a=0.097, ver=0.096, kb=0.18,
                                                  rand=0.02, idxm=0.02, n_res_only=30)
        ...
        for s in ps:
            ps[s]["regime_results"][reg_lo] = _mk(4, 1200, 4, flat=0.515, oexec=0.957, horc=0.953,
                                                  open_a=0.823, ver=0.823, kb=0.778,
                                                  rand=0.09, idxm=0.10)
            ps[s]["regime_results"][reg_hi] = _mk(4, 1200, 8, flat=0.081, oexec=0.918, horc=0.906,
                                                  open_a=0.097, ver=0.096, kb=0.287,
        ...
        for s in ps:
            ps[s]["regime_results"][reg_lo] = _mk(4, 1200, 4, flat=0.515, oexec=0.957, horc=0.953,
                                                  open_a=0.823, ver=0.823, kb=0.625,
                                                  rand=0.09, idxm=0.10)
            ps[s]["regime_results"][reg_hi] = _mk(4, 1200, 8, flat=0.081, oexec=0.918, horc=0.906,
                                                  open_a=0.097, ver=0.040, kb=0.205,
                                                  rand=0.02, idxm=0.02, n_res_only=40)
        ...
        for s in ps:
            ps[s]["regime_results"][reg_lo] = _mk(4, 1200, 4, flat=0.515, oexec=0.957, horc=0.953,
                                                  open_a=0.823, ver=0.823, kb=0.72,
                                                  rand=0.09, idxm=0.10)
            ps[s]["regime_results"][reg_hi] = _mk(4, 1200, 8, flat=0.081, oexec=0.918, horc=0.906,
                                                  open_a=0.097, ver=0.096, kb=0.36,
        ...
        # HARD_FAIL KB signal NOT independent: HARD_PASS-worthy recovery/delta/flatness BUT |indep_corr|
        #  > 0.40 -> the exogenous channel is contaminated -> premise void (fires after delta-HF check).
        for s in ps:
            ps[s]["regime_results"][reg_lo] = _mk(4, 1200, 4, flat=0.515, oexec=0.957, horc=0.953,
                                                  open_a=0.823, ver=0.823, kb=0.72,
                                                  rand=0.09, idxm=0.10, indep_corr=0.60)
            ps[s]["regime_results"][reg_hi] = _mk(4, 1200, 8, flat=0.081, oexec=0.918, horc=0.906,
                                                  open_a=0.097, ver=0.096, kb=0.36,
                                                  rand=0.02, idxm=0.02, indep_corr=0.60)
        out7 = aggregate_and_verdict(ps)
        assert out7["verdict"] == "HARD_FAIL_KB_SIGNAL_NOT_INDEPENDENT", \
            "ST13 expected HARD_FAIL not-independent got %s" % out7["verdict"]

        # MIDDLE partial independence: recovery/delta/flatness HARD_PASS-worthy but |indep_corr| in
        #  (0.15, 0.40] -> partial independence -> MIDDLE (fires before other MIDDLE guards).
        for s in ps:
            ps[s]["regime_results"][reg_lo] = _mk(4, 1200, 4, flat=0.515, oexec=0.957, horc=0.953,
                                                  open_a=0.823, ver=0.823, kb=0.72,
                                                  rand=0.09, idxm=0.10, indep_corr=0.28)
            ps[s]["regime_results"][reg_hi] = _mk(4, 1200, 8, flat=0.081, oexec=0.918, horc=0.906,
                                                  open_a=0.097, ver=0.096, kb=0.36,
                                                  rand=0.02, idxm=0.02, indep_corr=0.28)
        out8 = aggregate_and_verdict(ps)
        assert out8["verdict"] == "MIDDLE_BAND_PARTIAL_INDEPENDENCE", \
            "ST13 expected MIDDLE partial-independence got %s" % out8["verdict"]

        # MIDDLE kb_confirm vacuous: HARD_PASS-worthy accuracy + independent corr BUT kb_confirm_mean
        #  saturated (0.99 > 0.95) -> screen uninformative / gate does nothing distinct -> MIDDLE.
        for s in ps:
            ps[s]["regime_results"][reg_lo] = _mk(4, 1200, 4, flat=0.515, oexec=0.957, horc=0.953,
                                                  open_a=0.823, ver=0.823, kb=0.72,
                                                  rand=0.09, idxm=0.10, kb_conf=0.99)
            ps[s]["regime_results"][reg_hi] = _mk(4, 1200, 8, flat=0.081, oexec=0.918, horc=0.906,
                                                  open_a=0.097, ver=0.096, kb=0.36,
                                                  rand=0.02, idxm=0.02, kb_conf=0.99)
        out9 = aggregate_and_verdict(ps)
        assert out9["verdict"] == "MIDDLE_BAND_KB_CONFIRM_VACUOUS", \
            "ST13 expected MIDDLE kb-vacuous got %s" % out9["verdict"]

        # INCONCLUSIVE no discriminating regime (oracle_exec below rail)
        for s in ps:
            ps[s]["regime_results"][reg_lo] = _mk(4, 1200, 4, flat=0.515, oexec=0.957, horc=0.953,
                                                  open_a=0.823, ver=0.823, kb=0.72, rand=0.09, idxm=0.10)
            ps[s]["regime_results"][reg_hi] = _mk(4, 1200, 8, flat=0.081, oexec=0.918, horc=0.906,
                                                  open_a=0.097, ver=0.096, kb=0.36, rand=0.02, idxm=0.02)
    ...
    print("[selftest] ST13 verdict wiring OK (HARD_PASS; HARD_FAIL bound-real+not-independent+"
          "accelerating; MIDDLE partial-delta+flatness+recovery+degenerate+partial-independence+"
          "kb-vacuous; INCONCLUSIVE leak+no-regime)", flush=True)
