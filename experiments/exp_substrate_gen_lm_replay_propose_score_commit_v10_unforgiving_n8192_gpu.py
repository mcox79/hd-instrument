"""
substrate_gen_lm_replay_propose_score_commit_v10_unforgiving_n8192_gpu -- the GENERATION mechanism (Stage-4 barrier).

v10 RE-AIM (2026-07-08): v9 landed INCONCLUSIVE_NO_COMPOUNDING -- the must-fail ACCUMULATE baseline did NOT compound
(body_drift +0.087 < 0.10, MEASURED@..._v9.../metrics.json), because the old accumulator did c=normalize(W@c) each
step, which is a self-CORRECTING attractor (re-projects to a clean codeword every step) rather than an error-
retaining accumulator. So 'REPLAY beats a COMPOUNDING accumulator' was UNTESTED (the recombination+scoring half --
REPLAY beats RANDOM_RESTART +0.78 goal / +0.14 body and beats REPLAY_PROPOSE_ONLY -- fired cleanly and is banked
separately). v10 replaces the ACCUMULATE arm with a GENUINE growing-bundle accumulator (leaky superposition, never
resets to a clean codeword; LAMBDA_ACC) so position-tracking crosstalk accumulates with depth and body_token_acc
DEGRADES with L. The discriminator is: ACCUMULATE body compounds (body_drift>=DRIFT_MIN) WHILE REPLAY body stays
bounded/flat (rep_body_drift<=REPLAY_FLAT_MAX) AND REPLAY beats RANDOM_RESTART. Because bundle crosstalk is
N-dependent (stronger at small N), smoke adds a full-N=8192 ACCUMULATE PREVIEW (DISCRIMINATOR-MUST-SURVIVE-SCALE
option C) so the compounding is confirmed to survive at the FULL target N before any FULL dispatch. If ACCUMULATE
still does NOT compound, that is reported honestly (the mechanism may be intrinsically non-compounding here).
...
ANCHOR_NAME = "substrate_gen_lm_replay_propose_score_commit_v10_unforgiving_n8192_gpu"
...
LAMBDA_ACC = 0.65               # v10: leaky-integrator retention for the ACCUMULATE running bundle (0=clean-reset,
                                #      1=pure-sum). Set a-priori (leaky-integrator time constant); the accumulator
                                #      GROWS its state (never resets to a clean codeword) -> position-tracking
                                #      crosstalk accumulates with depth. This is the SITE of noise-compounding the
                                #      REPLAY mechanism removes by construction (regenerative-repeater law).
ACC_CONTENT_W = 0.0             # v10: ACCUMULATE's per-step goal-content weight. The goal-reachability oracle
                                #      content_iv is POSITION-INDEPENDENT (cos(goal, R_goal[v]) needs only goal +
                                #      candidate, not current position), so at full weight it re-grounds navigation
                                #      to the goal EVERY step regardless of accumulator drift == a clean external
                                #      reference each step == the regenerative-repeater property that PREVENTS
                                #      compounding (this was v9's INCONCLUSIVE root cause). A genuine 'read off own
                                #      drift' accumulator navigates by its LOCAL recency/traffic signal only (freq
                                #      gate on the drifting bundle); the goal oracle is available to REPLAY via
                                #      OFFLINE whole-candidate scoring (propose-score-commit), which the single-pass
                                #      accumulator architecture structurally cannot use without per-step re-grounding.
                                #      0.0 = pure drift-navigation. (RANDOM_RESTART has neither freq nor content nor
                                #      selection; ACCUMULATE has the local freq signal; REPLAY has content+selection.)
NEG = -1e9
...
DRIFT_MIN = 0.10                # ACCUMULATE body_acc(L_min) - body_acc(L_max) >= this (D1: accumulator COMPOUNDS)
REPLAY_FLAT_MAX = 0.06          # v10: REPLAY body_drift must be <= this (the bounded/non-compounding arm stays flat)
...
# v10 discriminator-survives-scale: bundle crosstalk is N-dependent (~sqrt(depth/N)); smoke runs a full-N=8192
# ACCUMULATE-only PREVIEW (option C) to confirm body_drift >= DRIFT_MIN survives at the FULL target N.
SCALE_PREVIEW_N = 8192

if RUN_MODE == "smoke":
    # smoke EXERCISES THE SAME TOPOLOGY REGIME as FULL (WIDTH/OUT_DEG/depth match; SMOKE=FULL branch discipline),
    # only N + instance count are reduced for speed. This is required so the compounding discriminator is tested
    # on the SAME graph difficulty the FULL run will see.
    N_DIM = 4096; SEEDS = [7]     # readout-sound at L=14 (a 15-item plan vector needs N>>1024; FULL is N=8192)
    L_GRID = [4, 14]; WIDTH = 6; OUT_DEG = 4; N_INST = 80; R_CAND = 40
else:
...
def accumulate_path(cb, W, R_goal, out_adj_t, starts_t, goals_t, L):
    """MUST-FAIL COMPOUNDING baseline (v10 re-aim -- the substrate's 4x-failed 'read off own drift' pattern, now
    a GENUINE accumulator so the compounding it is meant to demonstrate actually fires).

    v9 flaw (INCONCLUSIVE_NO_COMPOUNDING): the old accumulator did c=normalize(W @ c_prev) each step, which is a
    self-CORRECTING attractor (W is a good hetero-associative map, so normalize(W@c) re-projects toward a clean
    codeword every step) -> error did NOT accumulate; and the drift-INDEPENDENT goal gate rescued the endpoint for
    every arm. body_token_acc was depth-flat (drift +0.087 < 0.10). MEASURED@..._v9.../metrics.json.

    v10 fix: the accumulator carries a GROWING leaky superposition of its own trajectory and NEVER collapses back
    to a single clean codeword:
        c_{l+1} = LAMBDA_ACC * c_l + cb[emitted_l]           (leaky-integrator running bundle; the actual data
                                                              structure that is the site of noise-compounding)
    Position is TRACKED by decoding the current node from this drifting bundle (cur_hat = argmax cos(c, cb)) and
    the next-token prediction is pred = normalize(W @ c) over the WHOLE bundle -> as the trajectory lengthens the
    superposition crosstalk grows (bundled-storage-collapses-with-depth law) -> cur_hat drifts -> the out-neighbor
    MASK is taken from a WRONG node -> the emitted mid-sequence token leaves the true path, and (unforgiving DAG)
    cannot be salvaged -> per-position accuracy DROPS with depth = genuine compounding on the artifact-free
    body_token_acc witness. The SAME goal gate as REPLAY is retained (fairness: ACCUMULATE sees the identical goal
    content signal; it just has no clean per-step reset and no whole-candidate re-selection). REPLAY differs by
    exactly two architectural choices -- (1) each proposal step resets context to the CLEAN codeword cb[cur] (no
    bundle growth) and (2) it proposes-scores-commits WHOLE candidates -- which is why REPLAY stays bounded/flat
    while this accumulator compounds. This isolates 'the accumulator is the site of compounding'."""
    I = starts_t.shape[0]
    c = cb[starts_t].clone()                               # GROWING running bundle (never cleaned to a codeword)
    ...
    for l in range(L):
        cur_hat = (c @ cb.t()).argmax(dim=1)               # decode current node from the DRIFTING bundle (crosstalk)
        out_mask = out_adj_t[cur_hat]                      # out-neighbors of the DECODED (increasingly wrong) node
        pred = c @ W.t()                                   # predict next from the WHOLE bundle (crosstalk grows w/ L)
        pred = pred / (pred.norm(dim=1, keepdim=True) + 1e-8)
        freq_iv = pred @ cb.t()                            # freq/recency from the drifting bundle (LOCAL signal)
        # navigate by the LOCAL recency/traffic signal on the drifting bundle; NO position-independent goal oracle
        # re-grounding (ACC_CONTENT_W=0 by default) -> genuine 'read off own drift' -> compounds with depth.
        ...
        paths[:, l + 1] = nxt
        c = LAMBDA_ACC * c + cb[nxt]                        # GROW the bundle: leaky superposition, NO clean reset
    return paths
...
# --------------------------------------------------------------------------- full-N scale preview (option C)
def accumulate_scale_preview(seed, n_dim, L_list):
    """DISCRIMINATOR-MUST-SURVIVE-SCALE (option C). The v10 ACCUMULATE compounding source is a GROWING-bundle
    crosstalk (~sqrt(depth/N)) -- STRONGER at the small smoke N than at the FULL target N=8192. So a smoke that
    fires D1 at N=1024 does NOT prove D1 fires at N=8192. This re-measures the ACCUMULATE-only body_token_acc drift
    at the FULL target N (single seed, {L_lo, L_hi}) as a preview arm; if body_drift < DRIFT_MIN at full N, the
    discriminator does NOT survive scale -> do NOT request FULL dispatch (honest-negative). Returns {L: body_acc}."""
    out = {}
    for L in L_list:
        ...
        out[L] = m["body_token_acc"]
        print(f"    [PREVIEW N={n_dim} L={L}] ACCUM body_token_acc={m['body_token_acc']:.4f} "
              f"goal_reach={m['goal_reach']:.3f}", flush=True)
    return out
...
def _selftest(strict=False):
    ...
    cb2 = build_cb(V2, ST_N, torch.Generator(device=DEVICE).manual_seed(1))
    ...
    rep_body_drift = m_lo["REPLAY"]["body_token_acc"] - R["body_token_acc"]        # v10: REPLAY drift (~0 = bounded)
    ...
    print(f"[selftest] L_hi={L}/L_lo={Ll} | goal_reach: ORACLE_fid(ptok)={orc['per_token_acc']:.3f} "
          ...
          f"gr_drift={gr_drift:+.3f} sel_value(REP-PO)={sel_value:+.3f} "
          f"REP-RR(gr)={R['goal_reach']-RR['goal_reach']:+.3f} div={R['committed_diversity']:.3f}",
          flush=True)
    ...
    if strict:
        ...
        assert body_drift >= DRIFT_MIN, (
            ...
            f"drift={body_drift:+.3f} < {DRIFT_MIN}). The v10 growing-bundle accumulator is not accumulating error "
            f"in this regime -- tune LAMBDA_ACC / graph depth before dispatching FULL. NOTE the recombination+"
            f"scoring win IS real (REP-PO sel_value={sel_value:+.3f}, REP-RR>0) and can be banked separately.")
        # v10 REPLAY-STAYS-BOUNDED: the accumulator must compound at least DRIFT_MIN MORE than REPLAY (else the
        # 'replay flat while accumulate drifts' claim is not established -- both could be drifting together).
        assert body_drift - rep_body_drift >= DRIFT_MIN, (
            f"D1b FAIL: REPLAY does not stay bounded relative to ACCUMULATE. ACCUM body_drift={body_drift:+.3f} but "
            f"REPLAY body_drift={rep_body_drift:+.3f} (gap={body_drift - rep_body_drift:+.3f} < {DRIFT_MIN}). The "
            f"compounding is not SPECIFIC to the accumulator -- both arms drift with depth. RE-SPEC before FULL.")
...
def compute_verdict(all_results) -> Tuple[str, str]:
    ...
    rep_body_drift = rep_b_lo - rep_b                    # v10: REPLAY drift (must stay ~0 = bounded/flat)
    ...
    summary = (f"@L{L} N={all_results[0]['N']} chance~1/W={CHANCE:.3f} | goal_reach: ORACLE_fid(ptok)={orc:.3f} "
               ...
               f"ACCUM body_drift(L{L_lo}->L{L})={body_drift:+.3f} (>0=compounds) REPLAY body_drift="
               f"{rep_body_drift:+.3f} (~0=bounded) cv={cv:.3f}")

    # VALID-ONLY-IF (readout sound + baseline in a measurable band)
    if math.isnan(orc) or any(math.isnan(x) for x in [rep, acc, rr, rep_b, acc_b]):
        return ("INCONCLUSIVE", f"INCONCLUSIVE_READOUT_UNSOUND_NAN: a per-arm metric is NaN (ORACLE={orc}); the "
                f"peel/SIC plan readout is over-capacity at this N/L (a {HEADLINE_L + 1}-item plan vector needs N "
                f">> the item count). Increase N or reduce L. {summary}")
    if orc < ORACLE_FLOOR:
    ...
    if body_drift < DRIFT_MIN:
        return ("INCONCLUSIVE", f"INCONCLUSIVE_NO_COMPOUNDING (D1): ACCUMULATE body_token_acc (mid-sequence, "
                ...
                f"{DRIFT_MIN}). The v10 growing-bundle accumulator (LAMBDA_ACC leaky superposition) is NOT "
                f"accumulating position-tracking error with depth in this regime -- the must-fail baseline is not a "
                f"compounding baseline here, so 'REPLAY beats compounding' is NOT tested. RE-SPEC (tune LAMBDA_ACC / "
                f"increase depth / harden topology). NOTE: the recombination+scoring win IS real and separable -- "
                f"REPLAY beats RANDOM_RESTART (redundancy control) and beats REPLAY_PROPOSE_ONLY (scoring earns its "
                f"keep, sel_value={sel_value:+.3f}). {summary}")
    # v10 REPLAY-STAYS-BOUNDED gate: the compounding must be SPECIFIC to the accumulator (else both arms drift).
    if body_drift - rep_body_drift < DRIFT_MIN:
        return ("INCONCLUSIVE", f"INCONCLUSIVE_COMPOUNDING_NOT_ACCUMULATOR_SPECIFIC (D1b): ACCUM body_drift="
                f"{body_drift:+.3f} but REPLAY body_drift={rep_body_drift:+.3f} (gap={body_drift-rep_body_drift:+.3f}"
                f" < {DRIFT_MIN}). REPLAY does NOT stay bounded relative to ACCUMULATE -- both arms drift with depth, "
                f"so the compounding is not specific to the accumulator data structure. RE-SPEC. {summary}")
    ...
    # HARD_PASS (HP_SCOPE: REPLAY only): the accumulator COMPOUNDS (body_drift>=DRIFT_MIN, gated INCONCLUSIVE above
    # if not) AND REPLAY STAYS BOUNDED/FLAT (rep_body_drift<=REPLAY_FLAT_MAX) AND the REPLAY-ACCUMULATE body gap
    # GROWS with depth AND REPLAY wins at depth (rep_b>=acc_b at headline) AND REPLAY beats both goal_reach controls
    # AND scoring earns its keep. This is the v10 no-compounding claim: replay flat while accumulate degrades.
    gap_body_hi = rep_b - acc_b
    gap_body_lo = rep_b_lo - acc_b_lo
    replay_bounded = abs(rep_body_drift) <= REPLAY_FLAT_MAX
    if (beat_acc >= MARGIN_ACCUM and beat_rr >= MARGIN_REDUNDANCY and gap_body_hi > gap_body_lo and sel_value > 0
            and replay_bounded and rep_b >= acc_b and orc >= ORACLE_FLOOR and cv <= CV_MAX):
        return ("HARD_PASS", f"HARD_PASS[REPLAY]_GENERATION_MECHANISM: REPLAY-PROPOSE-SCORE-COMMIT STAYS BOUNDED "
                f"(REPLAY body_drift={rep_body_drift:+.3f}, |.|<={REPLAY_FLAT_MAX}) while the ACCUMULATE baseline "
                f"COMPOUNDS with depth (body_drift={body_drift:+.3f}>={DRIFT_MIN}); REPLAY beats ACCUMULATE "
                f"(REP-ACCUM goal_reach={beat_acc:+.3f}>={MARGIN_ACCUM}, body at depth={gap_body_hi:+.3f}) AND the "
                f"compute-matched RANDOM_RESTART redundancy control (REP-RANDOM={beat_rr:+.3f}>={MARGIN_REDUNDANCY}), "
                f"scoring earns its keep (sel_value={sel_value:+.3f}), readout sound (ORACLE={orc:.3f}), "
                f"cv={cv:.3f}<={CV_MAX}. SCOPE: synthetic structural regime; real-language ceiling is separately "
                f"reader-gated. {summary}")
...
def main():
    ...
    # v10 DISCRIMINATOR-MUST-SURVIVE-SCALE (option C): re-measure ACCUMULATE compounding at the FULL target N=8192.
    # Bundle crosstalk is N-dependent (stronger at small smoke N); the smoke's own D1 fire at N_DIM does not prove
    # D1 survives at N=8192. Preview runs only in smoke (FULL already IS N=8192). Gates the FULL-dispatch decision.
    scale_preview = {}
    if RUN_MODE == "smoke":
        ...
        pv_drift = sp[min(L_GRID)] - sp[max(L_GRID)]
        scale_preview = {"N": SCALE_PREVIEW_N, "body_acc_by_L": {str(k): round(v, 5) for k, v in sp.items()},
                         "body_drift": round(pv_drift, 5), "drift_min": DRIFT_MIN,
                         "fires": bool(pv_drift >= DRIFT_MIN), "elapsed_s": round(time.time() - t_pv, 1)}
        print(f"[scale-preview] N={SCALE_PREVIEW_N} ACCUM body_drift(L{min(L_GRID)}->L{max(L_GRID)})={pv_drift:+.4f} "
              f"fires(>= {DRIFT_MIN})={scale_preview['fires']} ({scale_preview['elapsed_s']}s)", flush=True)
    ...
    metrics = {"anchor_name": ANCHOR_NAME, "verdict": verdict, "verdict_msg": vmsg, "N": N_DIM,
               ...
               "margins": {"accum": MARGIN_ACCUM, "redundancy": MARGIN_REDUNDANCY, "no_recomb_band": NO_RECOMB_BAND,
                           "drift_min": DRIFT_MIN, "replay_flat_max": REPLAY_FLAT_MAX, "oracle_floor": ORACLE_FLOOR,
                           "cv_max": CV_MAX, "diversity_min": DIVERSITY_MIN},
