...
#   NO list(set())
"""Cross-slot RELATIONAL binding fairness cell -- V2 TIGHTENING (Director spawn 2026-07-30, one-variable
amendment of exp_cross_slot_relational_binding_v1 / commit 1cac05ffd per VET a7d8f2d4).

V2 TIGHTENING (this fork ONLY changes measurement/power + recalibrates bands; mechanism/task/arms are
UNCHANGED from v1 -- same PER_SLOT_BASELINE / MAJORITY_NONE_BASELINE / ARM_READCOND_WM / ARM_NATIVE_VSA_
COMPOSITIONAL / RANDOM_INIT_WM / SHUFFLED_PROBE / FLOOR_WRONGROLE / FLOOR_SHUFFLED_CODEBOOK, same encoder,
same construction). VET a7d8f2d4 on v1's landed result (readcond floor 0.33, VSA eval 0.855/0.815 MEASURED@
d:/AI/hd-instrument/data/exp_cross_slot_relational_binding_v1/metrics.json:bands.vsa_eval_acc, swap-gen
0.733/0.767 MEASURED@ same file:bands.vsa_swap_acc, n=30 per seed) flagged: true per-example chance is
EXACTLY 0.50 (a coin-flip MATCH/MISMATCH construction, not an entity-count chance), and v1's
FLOOR_MAX=0.35 sat BELOW that chance (a floor arm landing at 0.47-0.585 -- MEASURED@ same file:bands.
vsa_floor_wrongrole_acc/vsa_floor_shuffled_acc -- correctly failed to clear a floor that was mis-set below
where chance itself lands). Four changes, nothing else:
  1. PIN CHANCE AT EXACTLY 0.50: gen_dataset now force-balances MATCH/MISMATCH exactly n/2 : n/2 (was an
     independent per-example rng.integers(0,2) coin flip, giving a Binomial(n,0.5) wobble that MEASURED@
     v1 metrics.json:audit.match_frac_train=0.48 and bands.per_slot_baseline_eval=[0.53, 0.415] -- a
     +/-0.115 spread around 0.5 at n=200). SWAP was ALREADY exactly-balanced by construction (each
     SWAP_CFG is 100% one class) -- unaffected, no change there.
  2. RECALIBRATE bands: FLOOR_MAX 0.35 -> 0.62 and GAP_MAX 0.55 -> 0.62 (both were at/below the 0.50
     chance floor; 0.62 = chance + a safety margin above the worst-case Binomial(n=250,0.5) 3-sigma band
     [~0.40, 0.60], THEORETICAL: se=sqrt(0.25/250)=0.0316, 0.5+3*se=0.595, rounded up to 0.62 for margin).
     PROVEN_MIN stays 0.80 (unchanged, per spawn instruction).
  3. POWER UP the SWAP generalization measurement: N_SWAP_REPEATS 15 -> 125 per config (was n=30 total per
     seed; now n=250 total per seed), plus a THIRD seed (19, added to SEEDS_FULL) for power, per spawn
     instruction ("ideally +1 seed for power"). A 95%-CI (normal approximation, THEORETICAL: se=
     sqrt(p*(1-p)/n)) is now computed and reported per-seed AND pooled across seeds for the swap-gen
     number, replacing the single point-estimate v1 reported.
  4. Everything else (arms, controls, causal-attribution, encoder, construction, checkpoint/resume,
     hardening) is IDENTICAL to v1 -- this is a measurement/power fork, not a mechanism change.

PRE-REGISTERED DECISION RULE (V2, recalibrated, written BEFORE running; replaces v1's
INVALID/RELATIONAL_SOLVED/RELATIONAL_GAP/PARTIAL tiers with the spawn's exact recalibrated tiers, centered
on ARM_NATIVE_VSA_COMPOSITIONAL since that is the arm that was near-clearing in v1):
  INVALID -- construction audit fails, OR PER_SLOT_BASELINE / MAJORITY_NONE_BASELINE do not land at
    EXACTLY 0.50 (+/- floating tolerance) on EVAL_STD (the pin itself failed) -> do not trust downstream.
  RELATIONAL_CHAIN_GRADE -- ARM_NATIVE_VSA_COMPOSITIONAL EVAL_STD >= 0.80 on BOTH seeds AND its causal
    controls (FLOOR_WRONGROLE, FLOOR_SHUFFLED_CODEBOOK) collapse to <= FLOOR_MAX=0.62 on BOTH seeds AND
    PER_SLOT_BASELINE/MAJORITY_NONE land at exactly 0.50 AND the pooled SWAP accuracy's 95% CI lower bound
    is significantly above 0.50 (genuine generalization) AND the SWAP accuracy itself clears
    SWAP_MIN_ACC_FOR_PROVEN=0.80 on both seeds -> full chain-grade, both main-eval AND generalization.
  STILL_BOUNDED -- same EVAL_STD chain-grade + causal-control conditions as above, AND SWAP is
    significantly above chance (CI lower bound > 0.50) on both seeds, BUT does not clear the 0.80 bar ->
    main-eval is chain-grade, generalization is a real-but-bounded, now precisely CI-quantified effect.
  REGRESSION -- pinning chance to exactly 0.50 changes the main-eval picture (VSA EVAL_STD no longer
    clears PROVEN_MIN on both seeds, or causal controls no longer collapse under the recalibrated
    FLOOR_MAX) relative to v1's MEASURED main-eval (0.855/0.815) -> report honestly, do not claim
    chain-grade.
  PARTIAL -- floors/pin valid but neither RELATIONAL_CHAIN_GRADE, STILL_BOUNDED, nor REGRESSION cleanly
    applies (e.g. main-eval chain-grade but swap NOT significantly above chance on one seed) -> honest
    middle, reported not one of the three headline states.

Run:  .venv/Scripts/python.exe experiments/exp_cross_slot_relational_binding_v2.py --self-test
      .venv/Scripts/python.exe experiments/exp_cross_slot_relational_binding_v2.py --full

--- v1 docstring below is retained verbatim (task construction / arms / floors / novelty are UNCHANGED
from v1; only the V2 TIGHTENING block above and the PRE-REGISTERED DECISION RULE differ) ---
...
ANCHOR_NAME = "cross_slot_relational_binding_v2"
...
AUX_W = 1.0
PCA_EPS = 1e-4
SEEDS_FULL = (7, 13, 19)     # V2: +1 seed for power (was (7, 13)); 19 chosen arbitrarily, disjoint offset
N_RANDOM_INIT = 2
FULL_TRAIN = 350
FULL_EVAL_STD = 200
N_SWAP_REPEATS = 125         # V2: per SWAP config, per seed (was 15) -> 250 total per seed, 750 across 3
                             # seeds (spawn: "grow swap-n well beyond 30, e.g. 150-300")
STEPS_WM = 220
BATCH = 100
STEPS_READOUT = 120
...
# ---- bands (pre-reg; fixed BEFORE running; V2 RECALIBRATED per VET a7d8f2d4 -- see V2 TIGHTENING
# docstring block above for the derivation) ----
Z_THRESH = 2.0
CHANCE = 0.50                    # V2: true per-example chance is EXACTLY 0.50 (pinned MATCH/MISMATCH
                                 # construction), THEORETICAL, not a HYPOTHESIZED estimate
CI_Z = 1.96                      # V2: 95% normal-approx CI multiplier for the swap-gen power-up
PER_SLOT_MUST_FAIL_MAX = 0.70   # PER_SLOT_BASELINE / MAJORITY_NONE must be < this on EVAL_STD (unchanged;
                                 # already comfortably above the pinned 0.50 chance, not mis-set per VET)
PROVEN_MIN = 0.80               # arm clears on EVAL_STD AND SWAP, both seeds -> chain-grade (unchanged
                                 # per spawn instruction)
GAP_MAX = 0.62                  # V2 (was 0.55, sat at/below chance): arm at/below this on EVAL_STD or
                                 # SWAP, some seed -> counts toward MISSING/bounded
FLOOR_MAX = 0.62                # V2 (was 0.35, mis-set BELOW the 0.50 chance per VET a7d8f2d4): RI /
                                 # shuffled-probe / wrongrole / shuffled-codebook must be <= this.
                                 # THEORETICAL: chance(0.50) + margin above the worst-case Binomial(n=250,
                                 # p=0.5) 3-sigma band (se=sqrt(0.25/250)=0.0316, 0.5+3*se=0.595, rounded
                                 # up to 0.62)
SWAP_MIN_ACC_FOR_PROVEN = 0.80  # same bar applied to the now-powered-up (n=250/seed) SWAP set
...
def acc_ci(correct, n, z=CI_Z):
    """V2: normal-approximation 95% CI for a proportion, from raw (correct, n) counts.
    THEORETICAL: se = sqrt(p*(1-p)/n); ci = p +/- z*se. Also reports whether the CI lower bound clears
    CHANCE (significantly-above-chance test) -- the exact quantity the swap-gen power-up needs."""
    n = max(int(n), 1)
    p = correct / n
    se = math.sqrt(max(p * (1.0 - p), 1e-9) / n)
    lo, hi = p - z * se, p + z * se
    return {"p": p, "n": int(n), "correct": int(correct), "se": se, "ci_lo": lo, "ci_hi": hi,
            "significant_above_chance": bool(lo > CHANCE)}


def pool_ci(counts):
    """counts: list of (correct, n) tuples -> pooled acc_ci across all of them."""
    tot_correct = sum(c for c, _ in counts)
    tot_n = sum(n for _, n in counts)
    return acc_ci(tot_correct, tot_n)
...
def _make_query(rng, final_agent, final_patient, forced_match=None):
    """V2: forced_match, when not None, PINS the match/mismatch decision instead of drawing it from
    rng.integers(0,2) -- this is the exact-0.50-chance pin (see gen_dataset's force_balanced)."""
    direction = int(rng.integers(0, 2))          # 0=FORWARD, 1=REVERSE
    match = bool(rng.integers(0, 2)) if forced_match is None else bool(forced_match)
    if direction == 0:
...
def gen_stream_train(rng, forbid_pairs, forced_match=None, max_tries=MAX_CONSTRUCT_RETRY):
    for _ in range(max_tries):
        ...
        if (fa, fp) in forbid_pairs:
            continue
        direction, match, probe, answer = _make_query(rng, fa, fp, forced_match=forced_match)
...
def gen_dataset(n, rng, forbid_pairs=frozenset(), force_balanced=True):
    """V2: force_balanced=True (default) PINS match_frac at EXACTLY n/2 : n/2 by construction (was an
    independent per-example coin flip in v1, giving a Binomial(n,0.5) wobble -- MEASURED@ v1 metrics.json
    audit.match_frac_train=0.48, bands.per_slot_baseline_eval=[0.53, 0.415] at n=200/n=300). Requires n
    even (all v2 call sites use even n: FULL_TRAIN=350, FULL_EVAL_STD=200, and self-test/audit use even
    n). Set force_balanced=False to recover v1's unpinned random-coin behavior (kept for self-test
    comparison only)."""
    if force_balanced:
        assert n % 2 == 0, "force_balanced requires even n (got %d)" % n
        half = n // 2
        match_flags = np.array([True] * half + [False] * half)
        match_flags = match_flags[rng.permutation(n)]
        out = []
        for i in range(n):
            out.append(gen_stream_train(rng, forbid_pairs, forced_match=bool(match_flags[i])))
        return out
    out = []
    while len(out) < n:
...
def audit_construction(seed=7, n_train=300, n_swap_per_cfg=10):
    ...
    # V2: the chance-pin itself must hold on TRAIN (force_balanced=True default in gen_dataset) -- if this
    # ever drifts from exactly 0.50 the pin mechanism is broken (e.g. someone passed force_balanced=False).
    if abs(ps_acc - CHANCE) >= 1e-9:
        fails.append("V2 CHANCE-PIN VIOLATION: per_slot_baseline_acc_sample=%.6f != chance=%.2f exactly"
                     % (ps_acc, CHANCE))
    if abs(maj_acc - CHANCE) >= 1e-9:
        fails.append("V2 CHANCE-PIN VIOLATION: majority_none_acc_sample=%.6f != chance=%.2f exactly"
                     % (maj_acc, CHANCE))
...
def vsa_run_arm(examples, tables, gamma, thresh, mode):
    ...
    for i, ex in enumerate(examples):
        ...
        answers[i] = ex["answer"]
    correct = int((preds == answers).sum())
    n = len(examples)
    acc = float(correct) / max(n, 1)
    return {"acc": acc, "correct": correct, "n": n,
            "preds_digest": hashlib.sha256(preds.tobytes()).hexdigest()}
...
def run_vsa_seed(seed, tables, gamma, thresh, ev_examples, sw_examples):
    ...
    # V2: per-seed CI on the (now powered-up, n=250) swap set and on eval, for the tightened power-up
    swap_ci = acc_ci(main_sw["correct"], main_sw["n"])
    eval_ci = acc_ci(main_ev["correct"], main_ev["n"])
    return {"seed": seed, "eval_acc": main_ev["acc"], "swap_acc": main_sw["acc"],
            ...
            "swap_correct": main_sw["correct"], "swap_n": main_sw["n"], "swap_ci": swap_ci,
            "eval_correct": main_ev["correct"], "eval_n": main_ev["n"], "eval_ci": eval_ci,
...
# ---------------- V1 reference (for the REGRESSION check only; not used to gate anything) ----------------
# MEASURED@d:/AI/hd-instrument/data/exp_cross_slot_relational_binding_v1/metrics.json:bands.vsa_eval_acc
V1_VSA_EVAL_ACC = [0.855, 0.815]
# MEASURED@ same file:bands.vsa_swap_acc
V1_VSA_SWAP_ACC = [0.7333333333333333, 0.7666666666666667]
V1_REGRESSION_TOL = 0.10   # THEORETICAL: allow ordinary re-run/seed-composition noise; bigger drop = real


# ---------------- verdict (V2: recalibrated tiers per spawn PRE-REGISTERED DECISION RULE, centered on
# ARM_NATIVE_VSA_COMPOSITIONAL -- the arm that was near-clearing chain-grade in v1) ----------------
def decide_verdict(audit, learned_per_seed, vsa_per_seed):
    ...
    # V2: with match_frac PINNED at exactly 0.50, these baselines must land at EXACTLY 0.50 (floating
    # tolerance only) -- not merely "< PER_SLOT_MUST_FAIL_MAX" as in v1. This is the direct, checkable
    # consequence of the chance-pin; if it fails, the pin itself is broken and nothing downstream is valid.
    pin_tol = 1e-9
    per_slot_pinned = all(abs(a - CHANCE) < pin_tol for a in ps_eval)
    majority_pinned = all(abs(a - CHANCE) < pin_tol for a in maj_eval)
    ...
    vsa_swap_ci_per_seed = [r["swap_ci"] for r in vsa_per_seed]
    vsa_eval_ci_per_seed = [r["eval_ci"] for r in vsa_per_seed]
    vsa_swap_pooled_ci = pool_ci([(r["swap_correct"], r["swap_n"]) for r in vsa_per_seed])
    ...
    # V2 headline gates (VSA-centered per spawn's recalibrated rule)
    vsa_eval_chain_grade = all(a >= PROVEN_MIN for a in vsa_eval) and vsa_floors_ok
    swap_significant_per_seed = all(ci["significant_above_chance"] for ci in vsa_swap_ci_per_seed)
    swap_pooled_significant = bool(vsa_swap_pooled_ci["significant_above_chance"])
    swap_clears_proven = all(a >= SWAP_MIN_ACC_FOR_PROVEN for a in vsa_swap)

    bands = {
        "chance_pinned": CHANCE, "per_slot_must_fail_max": PER_SLOT_MUST_FAIL_MAX, "proven_min": PROVEN_MIN,
        "gap_max": GAP_MAX, "floor_max": FLOOR_MAX, "swap_min_acc_for_proven": SWAP_MIN_ACC_FOR_PROVEN,
        "per_slot_baseline_eval": ps_eval, "majority_none_eval": maj_eval,
        "per_slot_pinned_at_chance": bool(per_slot_pinned), "majority_pinned_at_chance": bool(majority_pinned),
        ...
        "vsa_eval_chain_grade": bool(vsa_eval_chain_grade),
        "vsa_swap_ci_per_seed": vsa_swap_ci_per_seed, "vsa_eval_ci_per_seed": vsa_eval_ci_per_seed,
        "vsa_swap_pooled_ci": vsa_swap_pooled_ci,
        "swap_significant_per_seed": bool(swap_significant_per_seed),
        "swap_pooled_significant": swap_pooled_significant, "swap_clears_proven": bool(swap_clears_proven),
        "v1_vsa_eval_acc_reference": V1_VSA_EVAL_ACC, "v1_vsa_swap_acc_reference": V1_VSA_SWAP_ACC,
    }

    if audit["fails"] or not (per_slot_pinned and majority_pinned and per_slot_fails and majority_fails):
        verdict = "INVALID"
        reasons = list(audit["fails"])
        if not per_slot_pinned:
            reasons.append("PER_SLOT_BASELINE did not pin to exactly chance=%.2f: %s" % (CHANCE, ps_eval))
        if not majority_pinned:
            reasons.append("MAJORITY_NONE_BASELINE did not pin to exactly chance=%.2f: %s" % (CHANCE, maj_eval))
        msg = "task construction / chance-pin invalid: %s" % "; ".join(reasons)
    elif vsa_eval_chain_grade and swap_clears_proven and swap_significant_per_seed:
        verdict = "RELATIONAL_CHAIN_GRADE"
        msg = ("PER_SLOT_BASELINE and MAJORITY_NONE both pinned EXACTLY at chance=%.2f (%s / %s), "
               "ARM_NATIVE_VSA_COMPOSITIONAL clears PROVEN_MIN=%.2f on EVAL_STD both seeds (%s) with floors "
               "collapsed (wrongrole=%s shuffled=%s, both <= FLOOR_MAX=%.2f), AND the powered-up SWAP "
               "generalization set (n=%d pooled) clears SWAP_MIN_ACC_FOR_PROVEN=%.2f on every seed (%s) "
               "with CI lower bound significantly above chance on every seed (pooled CI: %s) -- native FHRR "
               "role-typed bind/unbind cross-slot relational binding is CHAIN-GRADE on BOTH main-eval AND "
               "held-out role-swap generalization." % (
                   CHANCE, ps_eval, maj_eval, PROVEN_MIN, vsa_eval, vsa_wrong, vsa_shuf, FLOOR_MAX,
                   vsa_swap_pooled_ci["n"], SWAP_MIN_ACC_FOR_PROVEN, vsa_swap, vsa_swap_pooled_ci))
    elif vsa_eval_chain_grade and swap_significant_per_seed and not swap_clears_proven:
        verdict = "STILL_BOUNDED"
        msg = ("PER_SLOT_BASELINE and MAJORITY_NONE both pinned EXACTLY at chance=%.2f (%s / %s), "
               "ARM_NATIVE_VSA_COMPOSITIONAL clears PROVEN_MIN=%.2f on EVAL_STD both seeds (%s) with floors "
               "collapsed (wrongrole=%s shuffled=%s) -- main-eval is CHAIN-GRADE. The powered-up SWAP set "
               "(n=%d pooled, per-seed %s) is significantly ABOVE chance on every seed (CI lower bound > "
               "%.2f every seed) but does NOT clear SWAP_MIN_ACC_FOR_PROVEN=%.2f -- role-swap generalization "
               "is a real, now precisely CI-quantified, BOUNDED effect (pooled CI: %s)." % (
                   CHANCE, ps_eval, maj_eval, PROVEN_MIN, vsa_eval, vsa_wrong, vsa_shuf,
                   vsa_swap_pooled_ci["n"], vsa_swap, CHANCE, SWAP_MIN_ACC_FOR_PROVEN, vsa_swap_pooled_ci))
    elif not vsa_eval_chain_grade:
        # main-eval no longer clears chain-grade under the recalibrated (higher, above-chance) FLOOR_MAX,
        # or the eval accuracy itself dropped -- compare against the v1 MEASURED reference to decide if
        # this is a genuine regression (the pin/recalibration changed the picture) vs. ordinary noise.
        eval_drop = [v1 - v2 for v1, v2 in zip(V1_VSA_EVAL_ACC, vsa_eval[:len(V1_VSA_EVAL_ACC)])]
        real_regression = any(d > V1_REGRESSION_TOL for d in eval_drop) or not vsa_floors_ok
        verdict = "REGRESSION" if real_regression else "PARTIAL"
        msg = ("PER_SLOT_BASELINE and MAJORITY_NONE both pinned EXACTLY at chance=%.2f (%s / %s), but "
               "ARM_NATIVE_VSA_COMPOSITIONAL no longer clears the recalibrated chain-grade gate "
               "(eval=%s vs PROVEN_MIN=%.2f, floors_ok=%s vs FLOOR_MAX=%.2f wrongrole=%s shuffled=%s) -- "
               "v1 MEASURED main-eval was %s (chain-grade under v1's mis-set bands); delta vs v2=%s. %s" % (
                   CHANCE, ps_eval, maj_eval, vsa_eval, PROVEN_MIN, vsa_floors_ok, FLOOR_MAX, vsa_wrong,
                   vsa_shuf, V1_VSA_EVAL_ACC, eval_drop,
                   "This is a REAL regression: pinning chance / recalibrating bands changed the main-eval "
                   "picture." if real_regression else
                   "Delta is within ordinary re-run/seed-composition noise (<= %.2f); reporting PARTIAL, "
                   "not a regression." % V1_REGRESSION_TOL))
    else:
        verdict = "PARTIAL"
        msg = ("PER_SLOT_BASELINE and MAJORITY_NONE both pinned EXACTLY at chance=%.2f (%s / %s), "
               "ARM_NATIVE_VSA_COMPOSITIONAL main-eval is chain-grade (%s), but the powered-up SWAP set "
               "(pooled n=%d, per-seed %s) is NOT significantly above chance on every seed (pooled CI: %s) "
               "-- honest middle state, not one of the three headline tiers." % (
                   CHANCE, ps_eval, maj_eval, vsa_eval, vsa_swap_pooled_ci["n"], vsa_swap, vsa_swap_pooled_ci))
...
def run_self_test():
    ...
    # V2: with match_frac force-balanced, per-slot/majority baselines must be EXACTLY chance (not just
    # "in a ~0.5 band" as v1 checked) -- this is the direct self-test of the chance-pin.
    assert abs(res["per_slot_baseline_eval"] - CHANCE) < 1e-9, (
        "V2 CHANCE-PIN VIOLATION: per-slot baseline not exactly %.2f: %.6f"
        % (CHANCE, res["per_slot_baseline_eval"]))
    assert abs(res["majority_none_eval"] - CHANCE) < 1e-9, (
        "V2 CHANCE-PIN VIOLATION: majority-none baseline not exactly %.2f: %.6f"
        % (CHANCE, res["majority_none_eval"]))
