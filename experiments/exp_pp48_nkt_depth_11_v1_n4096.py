"""
pp48_nkt_depth_11_v1_n4096 -- PP-48 NKT negative-knowledge tree at depth-11 at N=4096.

Extends depth-9 (shipped) to depth-11. Tests NKT envelope at depth-11.
  Total forbidden patterns at depth-11 B=2: 2^11 - 1 = 2047.
  At K_POS=30 positive + 2047 NKT forbidden: alpha_total = 2077/4096 = 0.507.
  NOTE: alpha_total at depth-11 far exceeds alpha_c=0.138. Must reduce K_POS to 0 and
  use ONLY the NKT forbidden patterns + 10 positive.
  alpha_total = (10 + 2047) / 4096 = 0.502 -- still too high.

  REDESIGN for depth-11:
  Use B=2 PARTIAL tree to depth 7 (127 forbidden) + 3 more levels of just 2 additional roots:
  Actually simpler: cap tree at BRANCH=2, depth=7 -> 127 forbidden; depth=9 -> 511; depth=11 -> 2047.
  At depth-11 the pure binary tree CANNOT fit in sub-capacity N=4096.

  REVISED DESIGN: use depth-11 but with BRANCH=2 and only K_FORBIDDEN=100 leaves sampled from
  the depth-11 tree (not all 2047). This tests deep-tree repulsion on a sample of leaves.
  alpha_total = (10 pos + 100 forbidden) / 4096 = 0.027. Well sub-capacity.
  HP: repulsion on sampled leaves >= 0.70 AND pos retrieval >= 0.80.

GPU IMPLEMENTATION: matrix-free signed-AM.

PRE-REGISTERED BANDS:
  HP: pos_retrieval_rate >= 0.75 AND nkt_repulsion_rate >= 0.65.
  HARD-FAIL: pos_retrieval_rate < 0.40 OR nkt_repulsion_rate < 0.30.
  MIDDLE: 1-2/2 conditions.
  Note: at depth-11 with sampled leaves (not full tree), coherent tree structure is partial.
  Threshold set conservatively vs depth-9 (0.80/0.70) because only 100/2047 leaves are loaded.
  Calibration: +-50% of expected depth-9 thresholds (first depth-11 test).

FORMULA SELF-TESTS:
  1. BRANCH=2, depth=11 total tree = 2047. K_FORBIDDEN sample = 100.
     [INPUT: B=2, D=11] [EXPECTED: total = 2047]
...
ANCHOR_NAME = "pp48_nkt_depth_11_v1_n4096"
...
NKT_DEPTH = 11
BRANCH = 2
TOTAL_NKT_FULL = (BRANCH ** NKT_DEPTH - 1) // (BRANCH - 1)  # 2047
assert TOTAL_NKT_FULL == 2047, f"TOTAL_NKT formula: got {TOTAL_NKT_FULL}, expected 2047"
...
if RUN_MODE == "smoke":
    ...
    SEEDS = [7, 17]
    K_POS = 5
    K_FORBIDDEN = 20   # sampled leaves from depth-11 tree
    N_TEST = 3
else:
    N_ACTIVE = N
    SEEDS = [7, 17, 23, 31, 41]
    K_POS = 10
    K_FORBIDDEN = 100  # sampled leaves from depth-11 tree
    N_TEST = 10
...
def build_nkt_tree_sample(n_dim: int, gen: torch.Generator, k_sample: int) -> List[torch.Tensor]:
    """Build NKT tree and return K_SAMPLE leaves from depth NKT_DEPTH-1."""
    patterns = []
    ...
    # Return sampled leaves from the last level only
    leaves = prev_level  # 2^(NKT_DEPTH-1) = 1024 leaves at depth 11
...
def _instrumentation_selftest():
    ...
    print(f"[selftest] PASS: total_tree=2047, capacity_ok alpha={alpha_t:.4f}, gpu_mem_ok "
          f"depth={NKT_DEPTH} K_POS={K_POS} K_FORBIDDEN={K_FORBIDDEN}", flush=True)
...
def run_seed(seed: int, n_dim: int) -> Dict:
    ...
    # HP2: NKT repulsion rate (sampled leaves)
    nkt_ok = 0
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    if hp1 and hp2:
        return ("HARD_PASS", f"HARD_PASS: both HP at NKT depth-11 sampled-leaves. {summary}")
    if hp1 or hp2:
...
metrics = {
    ...
    "N": N, "NKT_DEPTH": NKT_DEPTH, "K_FORBIDDEN": K_FORBIDDEN, "K_POS": K_POS,
