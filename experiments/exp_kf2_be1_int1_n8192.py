"""KF-2 BE-1 PRECISION: int1 (1-bit / binary) at N=8192.

Delegates to exp_kf2_be1_precision_sweep_n8192 with PRECISION='int1'.
int1 = 32x memory compression vs fp32. Binary: W elements replaced by sign(W_ij).
Expected: isolation likely breaks substantially (sign-only W is highly lossy).
Purpose: establish the absolute floor for precision degradation.
If isolation survives at int1 it would be a remarkable result (binary W substrate).

N-suffix: _n8192 -> production N = 8192 (PROT-018 binding).
Anchor: kf2_be1_int1_n8192
...
_drv_spec = importlib.util.spec_from_file_location("be1_drv_int1", _drv_path)
...
def get_output_dir() -> Path:
    name = os.environ.get("HDLAB_EXP_NAME", "kf2_be1_int1_n8192")
...
def _instrumentation_selftest() -> None:
    ...
    assert cell.get("precision_compression_ratio", 0) == 32.0, (
        f"int1 compression must be 32.0; got {cell.get('precision_compression_ratio')}")
    print(f"[selftest] kf2_be1_int1_n8192 PASS iso={cell['isolation_ratio']:.5f}", flush=True)
...
def main():
    ...
    print(f"[run] kf2_be1_int1_n8192 smoke={smoke} N={N_cfg} precision={PRECISION} "
    ...
    if max_iso >= _drv.HF_CONTAMINATION:
        verdict = "KF2_BE1_INT1_HARD_FAIL"
        verdict_msg = (f"INT1_BREAKS_ISOLATION: max_iso={max_iso:.5f} >= {_drv.HF_CONTAMINATION} "
                       f"(expected floor characterization)")
    elif seeds_pass >= _drv.HP_SEEDS_MIN:
        verdict = "KF2_BE1_INT1_HARD_PASS"
        verdict_msg = (f"INT1_HOLDS: UNEXPECTED {seeds_pass}/{len(seeds)} seeds pass, "
                       f"max_iso={max_iso:.5f} binary_compression={compression:.1f}x")
    else:
        verdict = "KF2_BE1_INT1_MIDDLE_BAND"
        verdict_msg = f"INT1_PARTIAL: {seeds_pass}/{len(seeds)} seeds, max_iso={max_iso:.5f}"

    elapsed = round(time.time() - t0, 2)
    summary = {
        "anchor": "kf2_be1_int1_n8192", "precision": PRECISION, "N": N_cfg, "smoke": smoke,
