"""Candidate (v): Saddle-cascade plateau falsifier -- corpus-overlap-fraction sweep. v3.

v3 fix: reduced N 2048->1024 (4x speedup) + reduced f-grid 7->5 points
        (removes 0.1 and 0.9 near-anchor redundancies).
v2 was INSTRUMENTATION_FAIL TIMEOUT (7200s wall; only 2/7 f-points completed).
v2 logged: f=0.0 ret=0.402 (3 seeds tight); partial f=0.10 ret~0.665.
Root cause: N=2048 x 7 f-points x 3 seeds x (8 phase-A + 5 phase-B) epochs = too slow.

v3 estimates: N=1024 runs 4x faster; 5 f-points x 3 seeds x 13 epoch-equivalent ~60-90 min CPU.
              Pre-registered bands UNCHANGED from v1/v2.

Filed 2026-05-26 per strategy_decisions_2026-05-26.md v211 Decision (7).
Research note: notes/research_alternative_theoretical_homes_2026-05-24.md.
Parent experiment: exp_wave14_saddle_cascade_plateau_v2.py.
Pre-reg: preregs/2026-05-26_wave14_saddle_cascade_plateau_v3.md
...
METHOD:
  ...
  2. For each overlap fraction f in {0.0, 0.25, 0.5, 0.75, 1.0}:
...
PRE-REGISTERED BANDS (per [[feedback-envelope-expansion-fail-bands]]) -- UNCHANGED from v1/v2:
...
SELF-TEST cells (per [[feedback-strategy-spec-formula-selftests]]):
  ...
  3. Linear baseline check: pearson_r2([0,0.25,0.5,0.75,1.0], linear_vals) > 0.999
  4. Cascade hypothetical: pearson_r2([0,...,1.0], [0.60,0.62,0.94,0.94,0.94]) < 0.80
...
Queue: remote_cpu_queue (CPU only; no GPU needed; 5 f-values x 3 seeds x 2 phases)
ETA: ~60-90 min CPU at N=1024
Pre-reg: preregs/2026-05-26_wave14_saddle_cascade_plateau_v3.md
...
# v3: reduced f-grid (5 points vs 7) -- removes 0.1 and 0.9 near-anchor redundancies
# v3: reduced N 2048 -> 1024 (4x speedup)
F_SWEEP_FULL = [0.0, 0.25, 0.5, 0.75, 1.0]
F_SWEEP_SMOKE = [0.0, 0.5, 1.0]   # 3 anchor points for smoke gate

N_FULL = 1024        # v3: 4x faster than v2's N=2048
N_SMOKE = 256
BATCH_SIZE_FULL = 32
BATCH_SIZE_SMOKE = 16
...
# Pre-registered thresholds -- UNCHANGED from v1/v2
...
def run_one_cell(seed: int, f: float, N: int, batch_size: int,
    ...
    # Phase-A corpus: pure corpus_A (tile if corpus is shorter than n_bytes)
    corpus_a_raw = pa.load_corpus_a()
    if len(corpus_a_raw) < n_bytes:
        # Tile: repeat corpus until long enough
    ...
    # Phase-A evaluation baseline (use tiled corpus for eval too)
...
def self_test():
    ...
    # Self-test 3: Linear baseline R^2 formula verification (5-point grid)
    ...
    # Self-test 4: Cascade hypothetical -- should detect HARD-PASS condition (5-point grid)
...
def run(smoke: bool = False):
    ...
    t0 = time.monotonic()
    print(f"[cascade_plateau_v3] device={device} smoke={smoke}", flush=True)

    # Corpus preflight (v2: accept short corpus, will tile in run_one_cell)
    corpus_a_full = pa.load_corpus_a()
    print(f"[cascade_plateau_v3] corpus_a preflight: {len(corpus_a_full)} bytes (will tile if needed)", flush=True)
    if len(corpus_a_full) < 100:
        raise RuntimeError(
    ...
    for f in f_sweep:
        print(f"[cascade_plateau_v3] === f={f:.2f} ===", flush=True)
        for seed in seeds:
            try:
    ...
    out_dir = get_output_dir("wave14_saddle_cascade_plateau_v3")
