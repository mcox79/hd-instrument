"""pfc_gate_waypoint_rescue_coarse2fine_verify_v1 -- RESCUE the autonomous-waypoint deep-corner
HARD_FAIL by attacking the diagnosed COMPOUNDING-ERROR regime (Ross-Bagnell O(T^2)).

WHY (Director steer 2026-07-05, follow-on to autonomous-waypoint FULL HARD_FAIL):
  The parent cell PROVED the substrate can autonomously discover a partially-useful decomposition
  ONLY at chain_steps=1 (op4_d4: recovery=0.690), and collapses at deeper corners:
    MEASURED@data/exp_pfc_gate_autonomous_waypoint_discovery_v1/metrics.json:per_regime
      op4_V1200_d8 FLAT=0.077 OEXEC=0.934 HIER_ORACLE=0.928 wp_bisect_open=0.066 recovery=-0.014
      op4_V1200_d6 recovery=-0.003 ; op2_V800_d8 recovery=0.073 (matched-entropy dissociation).
  The negative-drill (notes/research_autonomous_waypoint_deep_corner_compounding_error_rescue_2026-07-05.md)
  diagnosed WHY via a clean matched-entropy dissociation: op4_d4 (ent8, 1 sequential pick) recovers
  0.690 vs op2_d8 (SAME ent8, 3 sequential picks) recovers 0.073 despite slightly BETTER per-hop reach
  signal -- so entropy/signal are ruled out; the number of chained, uncorrected, greedy bisection
  steps is the dominant driver. Each pick anchors on the PREVIOUS possibly-wrong discovered state
  (never ground truth, never verified) -- the behavior-cloning rollout setting of Ross & Bagnell
  (AISTATS 2010) O(epsilon*T^2) compounding error. CITED@Ross-Bagnell-2010. NOT a fundamental SR
  bound; the lit ranks two direct fixes: coarse-to-fine + verify-gate.

MECHANISM UNDER TEST (additive to the parent; NOT assumed to work; P_deflated~0.35 full HP,
~0.50-0.55 any real lift, per the drill):
  Attack compounding error with three cerebellar-forward-model-flavored fixes (anticipate + correct
  BEFORE committing) -- mechanism-analog, not task-analog; narrow glass-box sub-goal discovery, NOT
  autonomous planning / self-improvement (USER-LOCKED scope):
  (a) COARSE-TO-FINE recursive halving: pick the MIDDLE waypoint of [start,goal] FIRST (both endpoints
      are ground truth), then recurse into each half. The recursion tree has height ~log(T) so error
      does not compound through a single running anchor of length T; the root pick anchors on TWO
      ground-truth endpoints (vs the parent's linear chain where every pick anchors on the previous
      discovered state). This is the ONLY structural change that differs at op4_d8 (3 interior
      boundaries: parent = linear b0->b1->b2; c2f = b1(mid) from [start,goal], b0 from [start,b1],
      b2 from [b1,goal]).
  (b) VERIFY-GATE: a candidate must clear R[anchor,c]>=tau AND R[c,goal]>=tau (tau = 70th-percentile
      of R's own off-diagonal), else the argmax retries excluding it (capped at 5 retries, explicit
      logged fallback to a uniform-random valid state). Cheap independent check that never blindly
      chains a noisy proposal (MPNet / HYPE-style; the cerebellar error-correction role). HONEST NOTE:
      the parent's balance score is min(R[anchor,c],R[c,goal]), so "both legs >= tau" == balance >= tau;
      the retry then only confirms no candidate passes (a lower-balance one cannot pass when the top
      fails), so verify reduces to "commit-only-if-balance>=tau, else fall back to random" -- the
      don't-chain-a-weak-waypoint behavior. This is intentional, not a bug; verify is the drill's
      WEAKEST-ranked fix (its value is mainly inside combo, gating deep-level picks). The load-bearing
      lever is coarse-to-fine (a)+(c).
  (c) MULTI-GAMMA SR: a 2nd SR transport M_long trained at gamma=0.95 (effective horizon ~20 hops)
      for the COARSEST, longest-range pick (span > 5 hops, beyond gamma=0.85's 1/(1-g)=6.67 horizon);
      gamma steps down to 0.85 as the recursion narrows. Targets the MINOR graceful-decay effect.

  THREE NEW ARMS (waypoint SOURCE is the ONLY thing that differs; the per-segment low-horizon
  hierarchical EXECUTION loop run_hier_arm_wp is identical to the parent for EVERY arm):
    wp_bisect_coarse2fine  recursion + multi-gamma (no verify)
    wp_bisect_verify       parent sequential bisection + verify-gate (single gamma=0.85)
    wp_bisect_combo        recursion + multi-gamma + verify-gate  (the full cerebellar mechanism)
  best_rescue = max mean over the three. wp_bisect_open (the parent's FAILING mechanism) is re-run
  IN-CELL on identical seeds as the paired BASELINE (positive-control: must reproduce the parent's
  collapse at op4_d8 by construction -- same E/M/R primitives, same seeds, M trained first at 0.85).

ARMS (10; paired -- share E, W_ops, M, M_long, R and the SAME test chains per (regime,seed)):
  ...
  wp_bisect_open         PARENT autonomous baseline (the FAILING sequential bisection)  <-- the bar
  wp_bisect_coarse2fine  RESCUE (a)+(c)
  wp_bisect_verify       RESCUE (b)
  wp_bisect_combo        RESCUE (a)+(b)+(c)
  wp_random_state        autonomous FLOOR: uniform random codebook waypoints
  wp_index_midpoint      STRUCTURAL-ARTIFACT GUARD: index-interpolated waypoints (codebook indices
                         carry NO order; if this beats random, chain-generation leaks index order).

DISCRIMINATORS (per regime; FOCUS = op4_V1200_d8, the exact parent HARD_FAIL corner, for a clean
before/after; best_rescue = max(c2f,verify,combo)):
  headroom_exec        = oracle_exec - flat
  headroom_decomp      = hier_oracle - flat
  recovery_ratio(a)    = (a - flat) / headroom_decomp   (frac of ORACLE-decomp benefit recovered)
  delta_recovery       = recovery_ratio(best_rescue) - recovery_ratio(wp_bisect_open)   <-- KEY
  autonomous_closure(a)= (a - flat) / headroom_exec
  lift_flat, lift_random (of best_rescue); index_artifact_gap; anti_tautology_corr; degenerate_rate;
  sign_test_p (paired best_rescue vs wp_bisect_open); cv(best_rescue) (FULL); retry_rate (combo,
  DIAGNOSTIC only -- high retry at deep regimes means tau is too strict for the candidate pool).

HARD_PASS (locked; best_rescue at FOCUS -- rescue is real and worth folding into the capability):
  recovery_ratio(best_rescue) >= 0.20 AND delta_recovery >= 0.15 AND lift_flat > 0.05 AND
  lift_random > 0.10 AND index_artifact_gap < 0.05 AND anti_tautology_corr < 0.85 AND
  degenerate_rate < 0.10 AND sign_p(best_rescue vs wp_bisect_open) < 0.05 AND cv(best_rescue) < 0.15
  (FULL only; loosened from parent 0.10 given more moving parts, documented) AND oracle_exec >= 0.90
  AND headroom gates.
  => the deep-corner bound is NOT a fundamental SR-reach-bisection limit; a cerebellar-style
     anticipate-and-verify mechanism recovers real autonomous-decomposition at the deepest corner.
HARD_FAIL (locked -- deep corner is a genuine structural bound even after the standard fixes):
  delta_recovery <= 0.05  (no material lift over the already-failed wp_bisect_open despite
  coarse-to-fine + verify-gate + multi-gamma all tried).
  => honest stopping point: the remaining lever (DAgger-style oracle-in-the-loop correction) would
     break the "autonomous, no-oracle" framing, so it is out of scope -> clean ACCEPT-BOUND.
MIDDLE_BAND: 0.05 < delta_recovery < 0.15 (real but partial compounding-error mitigation), OR
  delta_recovery >= 0.15 but recovery_ratio(best_rescue) < 0.20 (clears the decisive delta but not
  the absolute bar), OR any honesty guard fails while accuracy margins pass.
...
Reported REGARDLESS: full grid table for every arm; recovery_ratio for open AND best_rescue per
  regime; delta_recovery; per-arm c2f/verify/combo means; retry_rate; spearman(delta_recovery,
  entropy); positive-control reproduce-check of wp_bisect_open/flat/hier_oracle vs the parent.

CELL-TEMPLATE MANDATORY (META_RULE_AC/AF/AG/AH + scope/scale/floor):
# - arms_differ_verified at smoke gate (AF): best_rescue vs wp_bisect_open vs wp_random vs flat op-
#   trace hash per seed AND hier_oracle vs hier_shuffled (exempt only if bit-identical).
...
#   feasibility (parent hier_oracle=0.928 at op4_d8 proves the given-decomposition envelope; the
#   open question is how much of that headroom the RESCUE recovers over open's -0.014 -- achievable
#   0-1 range; HP bar recovery>=0.20 sits inside the 0.928 envelope).
# - baseline_in_band (AG): the rescue baseline is wp_bisect_open (the failing mechanism), which at the
#   FOCUS is collapsed to ~flat (0.066 vs flat 0.077); the discriminator is rescue-vs-open, both
#   measurable; oracle_exec>=0.90 rail + headroom>=0.10 gates ensure room to recover.
# - discriminator survives scale: smoke holds V==FULL n_ops structure at op4 x {d4,d6,d8}; smoke
#   reach at N=2048 is BLUNTER than FULL N=8192 (parent: reach_rank 0.40->0.69 N2048->8192), so a
#   POSITIVE rescue-lift at smoke is a LOWER bound on FULL (sharper reach) -> option C directional
#   preview. Smoke that shows open-collapse + any positive best_rescue-minus-open gates the GPU FULL.
# - HARD_PASS strictly above floor: recovery>=0.20 + delta>=0.15 (META_RULE_L).
# - HP_SCOPE: HP gates apply to best_rescue vs wp_bisect_open at FOCUS; oracle_rail(>=0.90) to
#   oracle_exec; recovery references hier_oracle; index guard to wp_index vs wp_random.
# - cardinality_ok: EXPECTED_N_UNITS = n_arms(10) * n_seeds * n_regimes.
...
#   principled + retry_rate + fallback-count logged; discriminator = delta-over-open, not tuned-for).
# - all numbers tagged MEASURED@/HYPOTHESIZED@/THEORETICAL@/CITED@.

Compute architecture: (a) batched-GPU. SR-TD training (M @ gamma=0.85 AND M_long @ gamma=0.95),
operator application, cleanup, reach, R build, coarse-to-fine + verify picks = batched matmuls /
argmax on cuda-if-available. Chains batched; within-chain hops sequential (genuine dependency).
M + M_long + R computed once per (V,n_ops) group and shared across depths. Storage strategy: sharded
(each operator its own W matrix; M/M_long learned value operators; R derived reach matrices). No
bundled store. FULL strongly prefers overnight_queue (GPU). Extra cost vs parent: one 2nd SR train
(M_long) per group (same cost class as M); the recursion + verify-retry are vectorized, no new loop.
...
Reuses VERBATIM from experiments/exp_pfc_gate_autonomous_waypoint_discovery_v1.py (which itself
reuses from exp_pfc_gate_branching_depth_entropy_grid_v1.py): make_bipolar_E, hebbian_W,
cleanup_batched, make_kb_and_chains, build_adjacency, collect_rollout_transitions, train_sr_transport,
reach_value, build_reach_matrix, codebook_selfcos, _discover_bisect_boundaries (the open baseline),
_discover_random_boundaries, _discover_index_boundaries, _boundaries_to_hops, oracle_trajectory_idx,
build_waypoint_idx, _chain_tensors, run_selection_arm, run_oracle_arm, run_hier_arm_wp,
discovery_diagnostics, reach_rank_acc, binom/spearman/rankdata, the alpha/w_reach tuners, and the
defensive start-marker / crash-diag / heartbeat / atomic-write scaffolding. NEW: gamma_for_span,
_pick_balanced_verify, _discover_coarse2fine_boundaries, _discover_verify_boundaries, the 3 rescue
arms + wrappers, the rescue-vs-open verdict.
"""
import os
import sys
import argparse
...
ANCHOR_NAME = "pfc_gate_waypoint_rescue_coarse2fine_verify_v1"
PARENT_ANCHOR = "pfc_gate_autonomous_waypoint_discovery_v1"
...
HP_RECOVERY_RATIO_FLOOR = 0.20    # best_rescue recovers >= 20% of the oracle-DECOMPOSITION benefit
HP_DELTA_RECOVERY_MIN = 0.15      # best_rescue recovery - wp_bisect_open recovery >= 0.15 (decisive)
...
HP_SIGN_TEST_P = 0.05             # paired best_rescue vs wp_bisect_open significant
HP_CV_MAX = 0.15                  # cross-seed cv on best_rescue at focus (FULL only; loosened doc'd)
HF_DELTA_RECOVERY_CEIL = 0.05     # HARD_FAIL: rescue recovery <= open recovery + 0.05 (no lift)
MIDDLE_DELTA_MIN = 0.05           # MIDDLE lower edge (real partial lift over open)
...
RESCUE_ARMS = ["wp_bisect_coarse2fine", "wp_bisect_verify", "wp_bisect_combo"]
...
CONFIG_VERSION = (
    ...
    "expected_n=%d,HP_recov>=%.2f,HP_delta>=%.2f,HF_delta<=%.2f,lift_flat>%.2f,lift_rand>%.2f,"
    ...
    RUN_MODE, str(DEVICE), EXPECTED_N_UNITS, HP_RECOVERY_RATIO_FLOOR, HP_DELTA_RECOVERY_MIN,
    HF_DELTA_RECOVERY_CEIL, HP_LIFT_FLAT_MIN, HP_LIFT_RANDOM_MIN, HP_INDEX_GAP_MAX,
...
def _eval_regime(n_ops: int, V: int, dd: int, E: torch.Tensor, W_ops: List[torch.Tensor],
                 M: torch.Tensor, M_long: torch.Tensor, R_short: torch.Tensor, R_long: torch.Tensor,
                 C: torch.Tensor, tau_short: float, tau_long: float,
                 train_by_d, test_by_d, g: np.random.Generator,
                 disc_gen: np.random.Generator) -> Dict[str, Any]:
    """Tune on train, evaluate all 10 arms on test (paired). One seed."""
    ...
    # test waypoint schedules (discovery on test start/goal only; capture retry stats for verify/combo)
    ...
    rescue_means = {"wp_bisect_coarse2fine": arms["wp_bisect_coarse2fine"],
                    "wp_bisect_verify": arms["wp_bisect_verify"],
                    "wp_bisect_combo": arms["wp_bisect_combo"]}
    best_rescue_arm = max(rescue_means, key=lambda k: rescue_means[k])
    best_c = {"wp_bisect_coarse2fine": c2f_c, "wp_bisect_verify": ver_c,
              "wp_bisect_combo": cmb_c}[best_rescue_arm]

    # paired counts: best_rescue vs open (KEY), vs flat, vs random; index vs random (per-chain)
    paired = {
    ...
    return {
        ...
        "wr_ver": float(wr_ver), "wr_combo": float(wr_combo),
...
def run_one_seed(seed: int, out_dir: Path) -> Dict[str, Any]:
    ...
    for (V, n_ops) in sorted(by_group.keys()):
        ...
        print("[seed=%d op%d V=%d] SR_short err %s->%s Mn=%.3f | SR_long err %s->%s Mn=%.3f | "
              "R_s_mean=%.3f R_l_mean=%.3f tau_s=%.3f tau_l=%.3f n_trans=%d"
              % (seed, n_ops, V, sr_diag["err_first"], sr_diag["err_last"], sr_diag["final_M_norm"],
                 ...
                 float(R_short.mean()), float(R_long.mean()), tau_short, tau_long,
                 sr_diag["n_transitions"]), flush=True)

        for dd in depths_needed:
            rec = _eval_regime(n_ops, V, dd, E, W_ops, M, M_long, R_short, R_long, C,
                               tau_short, tau_long, train_by_d, test_by_d, g, disc_gen)
            rec["sr_err_last"] = sr_diag["err_last"]
            key = regime_key(n_ops, V, dd)
            regime_results[key] = rec
            a = rec["arms"]
            print("[seed=%d %s ent=%.2f] FLAT=%.3f OEXEC=%.3f HORC=%.3f | OPEN=%.3f C2F=%.3f "
                  "VER=%.3f CMB=%.3f RAND=%.3f IDX=%.3f (a=%.2f rr=%.3f degen=%.3f taut=%.3f "
                  "exact=%.3f retryC=%.3f fbC=%.3f best=%s)"
                  % (seed, key, rec["entropy"], a["flat_gonogo"], a["oracle_exec"], a["hier_oracle"],
                     a["wp_bisect_open"], a["wp_bisect_coarse2fine"], a["wp_bisect_verify"],
                     a["wp_bisect_combo"], a["wp_random_state"], a["wp_index_midpoint"],
                     rec["best_alpha"], rec["reach_rank_test"], rec["degenerate_rate"],
                     rec["anti_tautology_corr"], rec["exact_match_rate"], rec["retry_rate_combo"],
                     rec["fallback_rate_combo"], rec["best_rescue_arm"]), flush=True)
...
def aggregate_and_verdict(per_seed: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for r in REGIMES:
        ...
        rescue = {a: arm_means[a] for a in RESCUE_ARMS}
        best_rescue_arm = max(rescue, key=lambda k: rescue[k])
        best_rescue = rescue[best_rescue_arm]
        ...
        delta_recovery = recovery_rescue - recovery_open
        ...
        # pooled paired sign-test: best_rescue vs open (KEY), index vs random
        n_res_only = sum(int(per_seed[k]["regime_results"][rk]["paired"]["n_rescue_only_vs_open"]) for k in present)
        n_open_only = sum(int(per_seed[k]["regime_results"][rk]["paired"]["n_open_only_vs_rescue"]) for k in present)
        n_disc = n_res_only + n_open_only
        ...
        # arms-differ (AF): best_rescue trace vs open/flat/random per seed; hier_oracle vs shuffled
        af_collision = False
        for k in present:
            rr = per_seed[k]["regime_results"][rk]
            h = rr["op_trace_hashes"]
            brs = rr["best_rescue_arm"]
            if h[brs] in (h["wp_bisect_open"], h["flat_gonogo"], h["wp_random_state"]):
                af_collision = True
        ...
        hp_ok = (oracle_rail_ok and headroom_exec_ok and headroom_decomp_ok
                 and recovery_rescue >= HP_RECOVERY_RATIO_FLOOR
                 and delta_recovery >= HP_DELTA_RECOVERY_MIN
                 ...
                 and (brs_cv < HP_CV_MAX or RUN_MODE != "full")
        ...
        per_regime[rk] = {
            ...
            "wp_bisect_open": open_a, "wp_random_state": rand, "wp_index_midpoint": idxm,
            ...
            "recovery_open": float(recovery_open), "recovery_rescue": float(recovery_rescue),
            "delta_recovery": float(delta_recovery), "autonomous_closure": float(autonomous_closure),
            "lift_flat": float(lift_flat), "lift_random": float(lift_random),
            "lift_open": float(lift_open), "index_artifact_gap": float(index_artifact_gap),
            ...
            "n_rescue_only": int(n_res_only), "n_open_only": int(n_open_only),
    ...
    if not cardinality_ok:
        ...
        # rescue helps over open (delta > 0.05) but sub-threshold or a guard failed
    ...
    grid_str = " ".join(
        "%s(e%.1f:F%.2f/O%.2f/OPEN%.2f/CMB%.2f/RAND%.2f)" % (
            rk, per_regime[rk]["entropy"], per_regime[rk]["flat_gonogo"],
            per_regime[rk]["hier_oracle"], per_regime[rk]["wp_bisect_open"],
            per_regime[rk]["arm_means"]["wp_bisect_combo"], per_regime[rk]["wp_random_state"])
        for rk in REGIME_KEYS if rk in per_regime and per_regime[rk]["n_seeds"] > 0)

    if fv is not None:
        head = ("%s | FOCUS=%s(ent=%.1f) FLAT=%.3f OEXEC=%.3f HIER_ORACLE=%.3f | OPEN=%.3f "
                "best_rescue=%s=%.3f recov_open=%.3f recov_rescue=%.3f DELTA=%.3f | "
                "lift_flat=%.3f lift_random=%.3f lift_open=%.3f | index_gap=%.3f anti_taut=%.3f "
                "degen=%.3f exact=%.3f sign_p=%.4g rr=%.3f cv=%.3f retryC=%.3f fbC=%.3f | "
                "spr(delta,ent)=%.3f | GRID [%s] n_seeds=%d") % (
            verdict, focus_rk, fv["entropy"], fv["flat_gonogo"], fv["oracle_exec"], fv["hier_oracle"],
            fv["wp_bisect_open"], fv["best_rescue_arm"], fv["best_rescue"], fv["recovery_open"],
            fv["recovery_rescue"], fv["delta_recovery"], fv["lift_flat"], fv["lift_random"],
            fv["lift_open"], fv["index_artifact_gap"], fv["anti_tautology_corr"],
            fv["degenerate_rate"], fv["exact_match_rate"], fv["sign_test_p"], fv["reach_rank_test"],
            fv["brs_cv"], fv["retry_rate_combo"], fv["fallback_rate_combo"],
            spearman_delta_vs_entropy, grid_str, len(keys))
    else:
    ...
    return {
        ...
        "rescue_beats_open": bool(fv["delta_recovery"] > MIDDLE_DELTA_MIN) if fv else False,
...
def _selftest() -> int:
    ...
    # ST10: recovery / delta_recovery formulas
    flat_, oexec_, horc_, open_, rescue_ = 0.077, 0.934, 0.928, 0.066, 0.30
    hd = horc_ - flat_
    ro = (open_ - flat_) / hd
    rr = (rescue_ - flat_) / hd
    delta = rr - ro
    assert abs(ro - (-0.012926)) < 1e-3, "ST10 recovery_open off: %.5f" % ro
    assert abs(rr - 0.261987) < 1e-3, "ST10 recovery_rescue off: %.5f" % rr
    assert abs(delta - 0.274914) < 1e-3, "ST10 delta off: %.5f" % delta
    print("[selftest] ST10 recovery_open=%.3f recovery_rescue=%.3f delta=%.3f OK" % (ro, rr, delta),
          flush=True)
    ...
    # ST12: full pipeline single-seed structural (all 10 arms + diagnostics present; oracle sane;
    #       positive-control: wp_bisect_open trace differs from the rescue arms)
...
def _verdict_selftest() -> None:
    def _mk(n_ops, V, dd, flat, oexec, horc, open_a, c2f, ver, cmb, rand, idxm,
            degen=0.02, taut=0.10, exact=0.20, rr=0.60,
            n_res_only=None, n_idx_only=2, n_rand_only_idx=2):
        best = max(c2f, ver, cmb)
        brs = ("wp_bisect_coarse2fine" if c2f == best else
               ("wp_bisect_verify" if ver == best else "wp_bisect_combo"))
        if n_res_only is None:
            n_res_only = 45 if best > open_a + 0.1 else 1
        arms = {"flat_gonogo": flat, "oracle_exec": oexec, "hier_oracle": horc,
                "hier_shuffled": 0.02, "wp_bisect_open": open_a, "wp_bisect_coarse2fine": c2f,
                "wp_bisect_verify": ver, "wp_bisect_combo": cmb, "wp_random_state": rand,
        ...
        oth = {"flat_gonogo": "f", "oracle_exec": "oracle_true_seq", "hier_oracle": "ho",
               ...
               "wp_bisect_verify": "vf", "wp_bisect_combo": "cb", "wp_random_state": "rd",
               "wp_index_midpoint": "ix"}
        return {"n_ops": n_ops, "V": V, "dd": dd, "entropy": decision_entropy(n_ops, dd),
                "arms": arms, "op_trace_hashes": oth, "best_rescue_arm": brs, "best_alpha": 0.2,
                "wr_open": 1.0, "wr_c2f": 1.0, "wr_ver": 1.0, "wr_combo": 1.0,
                ...
                "paired": {"n_rescue_only_vs_open": n_res_only, "n_open_only_vs_rescue": 2,
    ...
    reg_hi = regime_key(4, 1200, 8)
    REGIMES = [{"n_ops": 4, "V": 1200, "dd": 4}, {"n_ops": 4, "V": 1200, "dd": 8}]
    REGIME_KEYS = [reg_lo, reg_hi]
    EXPECTED_N_UNITS = len(ARMS) * 3 * len(REGIMES)
    try:
        # HARD_PASS at FOCUS op4_d8: rescue combo=0.30, open=0.066, flat=0.077, horc=0.928, oexec=0.934.
        #  recovery_open=(0.066-0.077)/0.851=-0.013; recovery_rescue=(0.30-0.077)/0.851=0.262>=0.20;
        #  delta=0.275>=0.15; lift_flat=0.223>0.05; lift_random=0.30-0.02=0.28>0.10; guards clean.
        ps = {}
        for s in ["7", "17", "23"]:
            ps[s] = {"regime_results": {
                reg_lo: _mk(4, 1200, 4, flat=0.542, oexec=0.964, horc=0.953, open_a=0.826,
                            c2f=0.83, ver=0.82, cmb=0.84, rand=0.09, idxm=0.10),
                reg_hi: _mk(4, 1200, 8, flat=0.077, oexec=0.934, horc=0.928, open_a=0.066,
                            c2f=0.24, ver=0.18, cmb=0.30, rand=0.02, idxm=0.02),
            }}
        ...
        assert out["focus_best_rescue_arm"] == "wp_bisect_combo", "ST13 best_rescue should be combo"

        # HARD_FAIL: rescue ~ open (delta <= 0.05). combo=0.09, open=0.066 -> delta=(0.09-0.077 -
        #  (0.066-0.077))/0.851 = (0.013+0.011)/0.851=0.028 <= 0.05.
        for s in ps:
            ps[s]["regime_results"][reg_hi] = _mk(4, 1200, 8, flat=0.077, oexec=0.934, horc=0.928,
                                                  open_a=0.066, c2f=0.09, ver=0.08, cmb=0.085,
        ...
        # MIDDLE partial rescue: delta in (0.05, 0.15). combo=0.16, open=0.066 -> recovery_rescue=
        #  (0.16-0.077)/0.851=0.0975; recovery_open=-0.013; delta=0.110 in (0.05,0.15).
        for s in ps:
            ps[s]["regime_results"][reg_hi] = _mk(4, 1200, 8, flat=0.077, oexec=0.934, horc=0.928,
                                                  open_a=0.066, c2f=0.16, ver=0.12, cmb=0.15,
                                                  rand=0.02, idxm=0.02)
        out3 = aggregate_and_verdict(ps)
        assert out3["verdict"] == "MIDDLE_BAND_PARTIAL_RESCUE_DELTA_BELOW_15", \
            "ST13 expected MIDDLE partial got %s" % out3["verdict"]

        # MIDDLE recovery-below-20 despite delta>=0.15: engineer delta>=0.15 but recovery<0.20 by
        #  giving open a NEGATIVE recovery so delta clears while absolute stays low. open=0.00
        #  (recovery_open=(0.0-0.077)/0.851=-0.0905); combo=0.14 recovery=(0.14-0.077)/0.851=0.074;
        #  delta=0.074-(-0.0905)=0.165>=0.15 but recovery 0.074<0.20.
        for s in ps:
            ps[s]["regime_results"][reg_hi] = _mk(4, 1200, 8, flat=0.077, oexec=0.934, horc=0.928,
                                                  open_a=0.00, c2f=0.14, ver=0.10, cmb=0.13,
                                                  rand=0.02, idxm=0.02)
        out3b = aggregate_and_verdict(ps)
        assert out3b["verdict"] == "MIDDLE_BAND_RECOVERY_BELOW_20", \
            "ST13 expected MIDDLE recovery-below-20 got %s" % out3b["verdict"]

        # MIDDLE via degenerate guard: strong rescue but degenerate_rate high
        for s in ps:
            ps[s]["regime_results"][reg_hi] = _mk(4, 1200, 8, flat=0.077, oexec=0.934, horc=0.928,
                                                  open_a=0.066, c2f=0.24, ver=0.18, cmb=0.30,
    ...
    print("[selftest] ST13 verdict wiring OK (HARD_PASS; HARD_FAIL no-lift; MIDDLE partial+recovery+"
        ...
        try:
            ...
            _atomic_write_metrics(out_dir, {
                ...
                "verdict_msg": "SELFTEST_OK: ST1-ST13 (TD shrink, reach-matrix identity, gamma_for_span, "
                               ...
                               "reject+fallback, index interp, hop schedule, recovery/delta formulas, "
                               "spearman/entropy/binom, pipeline, verdict wiring)",
