...
def run() -> Dict:
    ...
    for label, fn in ANCHORS:
        ...
        if n_hp >= thr:
            decision = "PROMOTE_C"; n_promote += 1
        elif n_hp >= (1 if SMOKE else 2):
            decision = "SEED_FRAGILE"; n_fragile += 1
        else:
            decision = "FAIL"; n_fail += 1
        per_anchor[label] = {"verdicts": verdicts, "n_hard_pass": n_hp, "n_seeds": len(seeds), "decision": decision}
