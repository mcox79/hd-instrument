"""
q_a3_l6_cross_layer_composition_v1_n4096 -- Q-A3: L=6 cross-layer composition at N=4096.

Extends L=5 (HARD_PASS at N=4096) to L=6.
...
Architecture:
  ...
  L4: M_mid4=12.
  L5: M_mid5=6.
  L6: M_outer=3, Hadamard binding ctx6*L5.

GPU IMPLEMENTATION:
  6 W matrices x 67 MB each = 402 MB total. Safe on 8 GB.

PRE-REGISTERED BANDS:
  HP: all 6 level fidelities >= 0.90 AND l6_acc >= 0.60 in >= 4/5 seeds.
  HARD-FAIL: any per-level fidelity < 0.55 OR l6_acc < 0.30.
  MIDDLE: 5/6 conditions or l6_acc in [0.30, 0.60).
  Prior: L=5 HP at N=4096; L=6 is boundary -- lower HP threshold vs L=5 (0.70 -> 0.60).
  Calibration: prior L=5 HP; bands +-0.15 of predicted.

FORMULA SELF-TESTS:
  1. L=6 chain: decode via 6-step Hadamard roundtrip recovers xi_L1.
...
ANCHOR_NAME = "q_a3_l6_cross_layer_composition_v1_n4096"
...
ALPHA_C = 0.138
HP_FIDELITY = 0.90   # slightly relaxed vs L=5 (0.93)
HF_FIDELITY = 0.55
HP_L6_ACC = 0.60     # relaxed vs L=5 (0.70)
HF_L6_ACC = 0.30
NOISE_FRAC = 0.10
...
def _selftest_l6_chain():
    ...
    xi_L6 = ctx6 * xi_L5
    # Roundtrip decode
    xi_L5_dec = xi_L6 * ctx6
    xi_L4_dec = xi_L5_dec * ctx5
    xi_L3_dec = xi_L4_dec * ctx4
    xi_L2_dec = xi_L3_dec * ctx3
    xi_L1_dec = xi_L2_dec * ctx2
    assert torch.allclose(xi_L1_dec, xi_L1, atol=1e-6), f"L6 chain decode failed: {xi_L1_dec}"


def _instrumentation_selftest():
    _selftest_l6_chain()
    n_dim = N_ACTIVE
    alphas = [M_INNER/n_dim, M_MID2/n_dim, M_MID3/n_dim, M_MID4/n_dim, M_MID5/n_dim, M_OUTER/n_dim]
    ...
    del dummy
    print(f"[selftest] PASS: L6 chain decode, capacity_ok, gpu_mem_ok "
...
def run_seed(seed: int, n_dim: int) -> Dict:
    ...
    Xi_ctx6 = bsc(M_OUTER, n_dim)
    Xi_outer = Xi_ctx6 * Xi_mid5[:M_OUTER]
    ...
    level_fids = {f"L{l}": [] for l in range(1, 7)}
    l6_acc = []
    ...
    for q_idx in range(min(N_QUERIES, M_OUTER)):
        ...
        # L6 retrieval
        xi_L6_ret = hopfield_retrieve_gpu(W_outer, probe)
        # Decode L6 -> L5
        ...
        level_fids["L6"].append(cosine_sim_gpu(xi_L6_ret, xi_outer_true))
        ...
        l6_acc.append(cosine_sim_gpu(xi_L1_ret, xi_L1_true))

    mean_fids = {k: float(sum(v) / len(v)) if v else 0.0 for k, v in level_fids.items()}
    mean_l6 = float(sum(l6_acc) / len(l6_acc)) if l6_acc else 0.0
    ...
    print(f"  [seed={seed}] fids={' '.join(f'{k}:{mean_fids[k]:.4f}' for k in sorted(mean_fids.keys()))} "
          f"l6_acc={mean_l6:.4f} peak_gpu={peak_mem:.3f}GB elapsed={elapsed:.2f}s", flush=True)

    result = {
        "seed": seed, "N": n_dim, "run_mode": RUN_MODE,
        "l6_accuracy": float(mean_l6),
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    l6_acc = mean_key("l6_accuracy")

    summary = (f"L1={fid_l1:.4f} L2={fid_l2:.4f} L3={fid_l3:.4f} "
               f"L4={fid_l4:.4f} L5={fid_l5:.4f} L6={fid_l6:.4f} "
               f"l6_acc={l6_acc:.4f}(HP>={HP_L6_ACC} HF<{HF_L6_ACC}) "
               f"n_seeds={len(results)}")

    fids = [fid_l1, fid_l2, fid_l3, fid_l4, fid_l5, fid_l6]
    if any(f < HF_FIDELITY for f in fids) or l6_acc < HF_L6_ACC:
        return ("HARD_FAIL", f"HARD_FAIL: {summary}")

    hp_fids = sum(1 for f in fids if f >= HP_FIDELITY)
    hp_l6 = l6_acc >= HP_L6_ACC

    if hp_fids >= 6 and hp_l6:
        return ("HARD_PASS", f"HARD_PASS: all 6 level fidelities + l6_acc. {summary}")
    if hp_fids >= 5 or (hp_fids >= 4 and hp_l6):
        return ("MIDDLE_BAND", f"MIDDLE_BAND: {hp_fids}/6 level HP + l6_acc={hp_l6}. {summary}")
    return ("MIDDLE_BAND", f"MIDDLE_BAND: {hp_fids}/6 HP. {summary}")
...
print(f"[config] PROT-018 N={N} n_active={N_ACTIVE} mode={RUN_MODE} L=6", flush=True)
_prot018_startup_check(N_ACTIVE if RUN_MODE == "smoke" else N)

out_dir = get_output_dir(ANCHOR_NAME)
run_config = {"N": N, "L": 6, "run_mode": RUN_MODE}
...
metrics = {
    ...
    "N": N, "L": 6, "run_mode": RUN_MODE, "n_seeds": len(SEEDS),
    "elapsed_s": elapsed_total,
    "per_seed": [
        {"seed": r.get("seed"),
         **{f"fid_l{i}": r.get(f"fid_l{i}") for i in range(1, 7)},
         "l6_accuracy": r.get("l6_accuracy"),
