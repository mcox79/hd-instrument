"""
pp50_kappa3_sigma_g_extended_v5_n16384 -- PP-50 ultra-extended sigma_g sweep to find
...
ANCHOR_NAME = "pp50_kappa3_sigma_g_extended_v5_n16384"
...
SIGMA_G_GRID_FULL = [1.0, 1.5, 2.0, 3.0, 5.0]
...
if RUN_MODE == "smoke":
    ...
    DELTA_ALPHAS = [0.04]
    N_PROBES_SENS = 100
    SIGMA_G_GRID = [1.0, 2.0, 5.0]  # smoke: 3 representative points (low, mid, max)
...
def _instrumentation_selftest():
    ...
    k3_1, se_1 = hutchinson_kappa3_gpu_f64acc(Xi_base, 1.0, n_test, n_p, seed=42)
    assert not (k3_1 != k3_1), "kappa_3 is NaN at sigma_g=1.0"
    assert se_1 >= 0, f"SE is negative: {se_1}"

    # Test at sigma_g=5.0 (max extended point)
    k3_5, se_5 = hutchinson_kappa3_gpu_f64acc(Xi_base, 5.0, n_test, n_p, seed=42)
    assert not (k3_5 != k3_5), "kappa_3 is NaN at sigma_g=5.0"

    # Test sigma_sep at sg=1.0
    sep_1 = compute_sigma_sep(Xi_base, 0.04, 1.0, n_test, seed=42, n_probes=n_p)
    assert sep_1 >= 0, f"sigma_sep is negative: {sep_1}"
    assert sep_1 > 0, f"sigma_sep is exactly zero at smoke scale -- instrumentation broken"

    # Test sigma_sep at sg=5.0
    sep_5 = compute_sigma_sep(Xi_base, 0.04, 5.0, n_test, seed=42, n_probes=n_p)
    assert sep_5 >= 0, f"sigma_sep is negative at sg=5.0: {sep_5}"
    assert sep_5 > 0, f"sigma_sep is exactly zero at sg=5.0 -- instrumentation broken"
    ...
    print(f"[selftest] PASS: k3_sg1={k3_1:.4f} sep_sg1={sep_1:.2f} "
          f"k3_sg5={k3_5:.4f} sep_sg5={sep_5:.2f} gpu_mem={mem_gb:.3f}GB N={n_test}", flush=True)
...
def compute_verdict(seed_results: List[Dict]) -> tuple:
    ...
    sep_at_1 = next((s for g, s in sg_sep_pairs if abs(g - 1.0) < 0.05), None)
    sep_at_2 = next((s for g, s in sg_sep_pairs if abs(g - 2.0) < 0.05), None)
    sep_at_5 = next((s for g, s in sg_sep_pairs if abs(g - 5.0) < 0.05), None)

    # HARD-FAIL: strong reversal at sg=5.0 vs sg=2.0
    if sep_at_5 is not None and sep_at_2 is not None:
        if sep_at_5 < sep_at_2 * HF_REVERSAL_FRAC:
            return ("HARD_FAIL",
                    f"HARD_FAIL: sep(sg=5.0)={sep_at_5:.0f} < sep(sg=2.0)*0.8={sep_at_2*0.8:.0f}. "
                    f"True saturation/fold detected. {summary}")
    ...
    # HARD-PASS: monotone AND 10x amplification
    if sep_at_1 is not None and sep_at_5 is not None:
        ...
        if is_monotone and amplification >= HP_10X_AMPLIFICATION:
            return ("HARD_PASS",
                    f"HARD_PASS: monotone=True amp_1_to_5={amplification:.1f}x >= {HP_10X_AMPLIFICATION}x. "
                    f"Continued log-linear growth confirmed. {summary}")

    # MIDDLE
    if sep_at_1 is not None and sep_at_5 is not None:
        amplification = sep_at_5 / max(sep_at_1, 1.0)
        return ("MIDDLE_BAND",
                f"MIDDLE_BAND: monotone={is_monotone} amp_1_to_5={amplification:.1f}x "
