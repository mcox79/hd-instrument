"""
pp58_isochoric_kappa3_n16384_v5_n16384 -- PP-58 R5 N-scale: kappa_3 separation at N=16384.

CONTEXT (PP-58 R5 from v354 rescue list):
  v1_n4096: MIDDLE_BAND (ratio=8.0 at HP boundary; audit_crit=0.5; 2.8x above pred=0.18).
  v2_n4096 finergrid: recalibration multi-alpha (a0.1 ratio=3.0, a0.2 ratio=2.0; grid-limited).
  v4_n8192: MIDDLE_BAND (ratio=3.00 in [2.0,5.0); audit_crit=1.000 grid-limited at N=8192).
  R5 = N-scale N=16384: test whether audit_crit converges toward prediction as N increases.
  At N=16384, finite-N corrections should be smaller -> audit_crit may resolve below 1.0.

SCIENTIFIC QUESTION (PP-58 R5 N-scale):
  Does sigma_g_audit_crit shrink further at N=16384 vs N=8192=1.000?
  If finite-N artifact: audit_crit(N=16384) < 1.000 -> ratio may exceed HP=5.0.
  If systematic model error: audit_crit(N=16384) stays ~1.000 (N-invariant).
  Predicted: cap_crit(N=16384) = sqrt(1/0.05-1) = sqrt(19) = 4.359 (N-invariant).
  Finer sigma_g grid around [0.0, 2.0] step 0.05 to resolve audit_crit precisely.

  OOM PRE-CHECK: W matrix = 16384^2 * 8 bytes (float64) = 2.15 GB CPU RAM.
  Remote CPU (marsh@home) has 16+ GB RAM. Fits.

TEST DESIGN:
  alpha = 0.05, N = 16384, 5 seeds.
  sigma_g sweep: [0.0, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.40, 0.50, 0.75, 1.0, 1.5, 2.0, 3.0, 4.0].
  Fine resolution around audit_crit boundary region [0.0, 2.0].

PRE-REGISTERED BANDS (PP-58 R5; prior N=8192 ratio=3.00 MIDDLE):
  HARD-PASS: ratio >= 5.0 AND sigma_g_cap_crit within 20% of sqrt(19)=4.359 at N=16384.
  ...
  Calibration note: bands unchanged from N=8192 (same +-50% policy on founding ratio=8.0).
...
FORMULA SELF-TESTS (PROT-022):
  ...
  3. M = int(0.05 * 16384) = 819 >= 1.
     [EXPECTED: M = 819]
  4. sigma_g grid has at least 5 points <= 1.0 (resolution check).
     [EXPECTED: len([s for s in SIGMA_G if s <= 1.0]) >= 5]

PROT-018: anchor has _n16384; N MUST = 16384.
QUEUE: remote_cpu_queue (pure numpy; 1 alpha x 15 sigma x 5 seeds at N=16384; ~3-4h wall).
TIMEOUT ESTIMATE: N=8192 elapsed ~600s (v4 est). N=16384: W matrix 4x larger, O(N^2).
  ceil(1.5 * 600 * (16384/8192)^2 * 1.0) = ceil(1.5 * 600 * 4) = ceil(3600) = 3600s.
  3600s < 14400s cap; within allowed range.
"""
import sys
...
ANCHOR_NAME = "pp58_isochoric_kappa3_n16384_v5_n16384"
...
SIGMA_G_FULL = [0.0, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.40, 0.50, 0.75, 1.0, 1.5, 2.0, 3.0, 4.0]
CAP_PRED = float((1.0 / ALPHA - 1.0) ** 0.5)  # sqrt(19) = 4.3589

# PROT-022 test 4: grid resolution check
_N_SIGMA_LE1 = len([s for s in SIGMA_G_FULL if s <= 1.0])
assert _N_SIGMA_LE1 >= 5, f"sigma_g grid: only {_N_SIGMA_LE1} points <= 1.0"

KAPPA3_RATIO_CRIT_FACTOR = 2.0
RECALL_CRIT = 0.50
N_RETRIEVAL_STEPS = 8
N_QUERIES_PER_CELL = 10
N_PROBES_FULL = 30    # fewer probes at N=16384 to manage wall time
...
if RUN_MODE == "smoke":
    SEEDS = [7, 17]
    N_ACTIVE = 1024
    SIGMA_G_USE = [0.0, 0.25, 0.50, 1.0, 2.0, 4.0]
    N_QUERIES_USE = 5
...
def _instrumentation_selftest():
    ...
    assert _M_FULL == 819, f"M_full={_M_FULL} expected 819"
    assert _N_SIGMA_LE1 >= 5, f"sigma_g grid resolution: {_N_SIGMA_LE1} <= 1.0 points"
    print(f"[selftest] PASS: cap_boundary=4.3589, kappa3_identity, m_full=819, "
          f"grid_le1={_N_SIGMA_LE1} N_active={N_ACTIVE} alpha={ALPHA}", flush=True)
