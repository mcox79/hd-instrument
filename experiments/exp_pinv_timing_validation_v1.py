"""
exp_pinv_timing_validation_v1 -- pinv incremental-update timing (anchors the 240,000x-faster-updates claim) -- CPU.
...
ANCHOR_NAME = "pinv_timing_validation_v1"; N = 4096
...
def run() -> Dict:
    g = np.random.default_rng(7); K = sign_keys(M0, N, g); G = K @ K.T + 1e-3 * np.eye(M0); Ginv = np.linalg.inv(G).astype(np.float64)
    new_keys = sign_keys(N_UPDATES, N, g).astype(np.float64)
    t0 = time.perf_counter()
    for i in range(N_UPDATES):
        k = new_keys[i]
        u = K @ k; v = Ginv @ u; s = 1.0 + float(k @ k) - float(u @ v)
        # extend: new Ginv block (M+1)x(M+1) via SMW; here update the existing block + append (dominant O(M^2) op)
        Ginv = Ginv + np.outer(v, v) / s
        K = np.vstack([K, k[None, :]])
        Ginv = np.pad(Ginv, ((0, 1), (0, 1))); Ginv[-1, -1] = 1.0 / s; Ginv[-1, :-1] = -v / s; Ginv[:-1, -1] = -v / s
