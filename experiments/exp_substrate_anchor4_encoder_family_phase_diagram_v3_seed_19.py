"""substrate_anchor4_encoder_family_phase_diagram_v3 sibling seed=19.

v3 fix for META_RULE_AX (encoder slots cosmetic; 3/4 byte-identical) caught
by Skunkworks landed-VET 2026-06-29 on v1 / 2026-06-30 on v2 rerun.

v3 changes (per Skunkworks promotion-path verbatim):
  - N_DIM sweep raised: {1024, 4096, 8192} from {128, 1024}
  - n_atoms raised: 1500 from 200
  - R_BUCKETS raised: 128 from 64
  - 5 encoder families (added sparse_real): C(5,2)=10 pairs
  - HARD_FAIL_DEGENERATE_ENCODERS if pairs_differ < 5/10 (META_RULE_AX)
  - HARD_FAIL_READOUT_FLOOR per encoder recency_decode_acc_mean < 0.30 (META_RULE_AP)
  - HARD_PASS requires n_chain_grade >= 2 (v1 allowed >=1; v3 demands 2+)
  - GPU torch backend (overnight_queue routing-gate; CUDA fp32)

Cardinality (per seed):
  FULL : 5 enc * 3 decay * 2 load * 3 N_DIM = 90 phase points
  SMOKE: 5 enc * 2 decay * 2 load * 2 N_DIM = 40 phase points
...
CARDINALITY_OK_FULL: 90 phase points per seed
CARDINALITY_OK_SMOKE: 40 corner points per seed
...
SEED = 19
ANCHOR_NAME = f"substrate_anchor4_encoder_family_phase_diagram_v3_seed_{SEED}"
...
CONFIG_VERSION = (
    ...
    f"N_DIM_sweep=[1024,4096,8192],R_BUCKETS=128,n_atoms=1500,n_days=365,"
    f"decay_full=[30,90,180],load_full=[1.0,5.0],"
    f"decay_smoke=[30,90],load_smoke=[1.0,5.0],"
    f"SEED={SEED},mode={RUN_MODE},"
    f"arms=[TIME_DECAY,RANDOM],"
    f"expected_n_full=90,expected_n_smoke=40,"
    f"discriminator=per_encoder_pareto_dominance_rate_v3_AX_AP,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v3_anchor4_encoder_family_phase_diagram_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v3_anchor4_encoder_family_import_crash",
        }
        tmp = out_dir / "metrics.json.tmp"
        final = out_dir / "metrics.json"
...
def main() -> int:
    ...
    from experiments._substrate_anchor4_encoder_family_phase_diagram_v3_core import (
        run_one_seed_phase_diagram, aggregate_and_verdict, selftest,
        get_backend_label, N_DIM_DEFAULT, _get_device,
    )

    # GPU mandate: strict for FULL; permissive for smoke/selftest
    ...
    final["_hardening_marker"] = "v3_anchor4_encoder_family_phase_diagram_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = (
        "synthetic_substrate_5_encoder_family_anchor4_timedecay_v3_AX_AP"
    )
    final["n_llm_calls"] = _LLM_CALL_COUNTER[0]

    if _LLM_CALL_COUNTER[0] != 0:
        # META_RULE_K substrate-only audit
        raise RuntimeError(
            f"LLM_CALL_GATE_BREACH: substrate-only required; "
            f"n_llm_calls={_LLM_CALL_COUNTER[0]}"
        )

    # Atomic final write per META_RULE_AH
    tmp = out_dir / "metrics.json.tmp"
