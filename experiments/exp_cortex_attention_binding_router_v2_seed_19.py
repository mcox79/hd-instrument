"""cortex_attention_binding_router_v2 -- seed_19. Cortex 4-class routing classifier.

M1.6 SURGICAL v2 (2026-07-01). v1 was tier'd MM (Atom 27; Skunkworks Wave 10)
because seed_13 chain_multihop RETRIEVE per-class precision = 0.625 (5/8) breached
HP_PER_CLASS_PRECISION >= 0.70. All other HP gates cleared cross-seed.

Two surgical fixes (v1 mechanism otherwise unchanged):
  Fix 1 (PRIMARY): N_TEST_PER_CLASS_FULL 5 -> 20 items per class per regime.
    v1 CM had 20 items per regime (5 per class * 4 classes). Per-class RETRIEVE
    precision at chain_multihop regime had ~8 predictions labeled RETRIEVE
    (Bernoulli sigma at p=0.70 = sqrt(0.7*0.3/8) = 0.162 MEASURED@backward-computed
    from Atom 27 seed_13 breach). Tightening N_TEST_PER_CLASS 5 -> 20 puts
    ~24 RETRIEVE predictions in the confusion arm (sigma = 0.094; ~1.7x tighter);
    0.625 breach becomes far less noise-prone. THEORETICAL@Bernoulli
    sigma_p = sqrt(p*(1-p)/n).
  Fix 2 (SECONDARY): add explicit chain_signal feature. Currently router uses
    (refuse_slot, retrieval_slot, query) trio. Add chain_signal (bipolar bind
    with chain_role) populated with slot 0 = chain-required for MULTI_HOP items
    and slot 1 = no-chain-required for RETRIEVE/BIND/REFUSE. This makes the
    RETRIEVE-vs-MULTI_HOP discrimination explicit at the class-HV level rather
    than implicit in retrieval_slot alone. Reduces false-positive RETRIEVE
    predictions at chain_multihop regime.

Compose M1.4 CONFORMAL_MODERATE refuse-gate (Atom 15 CG) + M1.5 v2 TWOTIER
context retention (Atom 18 CG; commit adaab6b7) + WM multi-bank K=4096 (prior
CG) + partition-oracle multihop (Atom 6 CG) + Dense-Hopfield READ-REPLACE
(prior CG) into a nearest-class hypervector classifier that routes queries into
one of 4 substrate-primitive routes: REFUSE / RETRIEVE / BIND / MULTI_HOP.

M3 milestone: v2 CLOSES M1.6 if 3-seed HP FULL with ALL per-class precisions
>= 0.70. Together with M1.4 v8 + M1.5 v2 -> 3 cortex milestones close 2026-07-01.
...
ARMS (7):
  ...
  ARM_TRUE_MULTI_HOP       : 2-hop chain queries (route=MULTI_HOP); precision at
                             MULTI_HOP.
  ARM_ROUTE_CONFUSION_MATRIX: 80 test items balanced across 4 classes; top-1
                             accuracy + per-class precision/recall. Load-bearing
                             discriminator. (v2: bumped from v1's 20 items per
                             regime to tighten Bernoulli sigma 0.162 -> 0.094 at
                             per-class n~24 predictions.)
  ...
  ARM_M14_M15_ISOLATED     : class-HVs built with ONLY refuse_signal_hv OR ONLY
                             retrieval_signal_hv (the others zeroed; chain_signal
                             excluded); tests composition vs isolated signal.
...
CRLB:
  ...
  Bernoulli sigma at p=0.5, N_TEST=80 (v2 up from v1's 20) = sqrt(0.25/80) = 0.056.
  Per-class Bernoulli sigma at p=0.70, n~24 predictions = sqrt(0.7*0.3/24) = 0.094
    (v1: 0.162 at n~8; v2 ~1.7x tighter).
  HP gap 0.30 lift = 5.4 sigma (very reachable at v2 tightening).

Regime notes:
  - CPU-eligible (numpy) for smoke AND full.
  - Estimated full wall: ~90-180s per seed (linear scaling from v1's 30-60s at 4x
    test items).
  - Route: remote_cpu_queue via hdi_orchestrator handoff post-smoke.

Author: exp_dev (hdi_exp_dev spawn) 2026-07-01 (M1.6 SURGICAL v2 after Atom 27 MM).
...
SEED_THIS_CHUNK = 19
ANCHOR_NAME = f"cortex_attention_binding_router_v2_seed_{SEED_THIS_CHUNK}"
_HARDENING_MARKER = "v2_surgical_n_test_20_plus_chain_signal_class_hv_composition"
...
N_TEST_PER_CLASS_FULL = 20       # v2: 80 test items per regime (Bernoulli sigma ~0.056)
...
#                  2 slots for chain_signal (v2 addition: chain-required / not)
N_SIGNAL_SLOTS = 3
N_CHAIN_SLOTS = 2                # v2: 0=chain-required, 1=no-chain-required
...
def build_role_vectors(rng) -> Dict[str, np.ndarray]:
    return {
        ...
        "chain_role": _bipolar((N_DIM,), rng),   # v2 addition
    }
...
def build_feature_hv(refuse_slot_hv: np.ndarray,
                     ...
                     query_hv: np.ndarray,
                     chain_slot_hv: np.ndarray,
                     roles: Dict[str, np.ndarray]) -> np.ndarray:
    """Bind each signal (refuse + retrieval + query + chain) with its role,
    bundle, quantize. v2: added chain_signal per Fix 2.
    """
    return _bipolar_quantize(
        ...
        + _bind_xor(roles["chain_role"], chain_slot_hv)
    )


def build_feature_hv_masked(refuse_slot_hv, retrieval_slot_hv, query_hv,
                             chain_slot_hv, roles,
                             use_refuse=True, use_retrieval=True,
                             use_chain=True):
    """ARM_M14_M15_ISOLATED helper: zero-out either M1.4 or M1.5 signal.

    When use_refuse=False: refuse_signal contribution zeroed (only retrieval +
        query + chain).
    When use_retrieval=False: retrieval_signal contribution zeroed (only
        refuse + query + chain).
    When use_chain=False (ISOLATED variants): chain_signal zeroed (v1-equivalent
        feature-HV). query_hv always included.
    """
    ...
    if use_chain:
        parts = parts + _bind_xor(roles["chain_role"], chain_slot_hv)
...
def make_item(rng, route: str, signal_codebook_refuse: np.ndarray,
              ...
              signal_codebook_chain: np.ndarray,
              known_entity_hv: np.ndarray) -> Tuple[np.ndarray, np.ndarray,
                                                     np.ndarray, np.ndarray]:
    """Generate (refuse_slot_hv, retrieval_slot_hv, query_hv, chain_slot_hv)
    per class semantics.

    REFUSE: refuse=above-tau (slot 2), retrieval=no_hit (slot 2),
            chain=no-chain (slot 1), query=fresh OOD.
    RETRIEVE: refuse=below-cal (slot 0), retrieval=STM_hit (slot 0 or 1),
            chain=no-chain (slot 1), query=perturbed known entity.
    BIND: refuse=below-cal (0), retrieval=no_hit (2),
            chain=no-chain (slot 1), query=fresh novel bipolar.
    MULTI_HOP: refuse=below-cal (0), retrieval=LTM_hit partial (1),
            chain=chain-required (slot 0), query=bind(known_entity, relation).

    v2: chain_slot_hv is the new discriminating feature. MULTI_HOP is the ONLY
    class with slot=0 (chain-required); RETRIEVE/BIND/REFUSE all get slot=1
    (no-chain-required). Tightens RETRIEVE-vs-MULTI_HOP separation directly.
    """
    if route == "REFUSE":
        refuse_slot = signal_codebook_refuse[2]
        retrieval_slot = signal_codebook_retrieval[2]
        chain_slot = signal_codebook_chain[1]     # no-chain
        ...
        chain_slot = signal_codebook_chain[0]     # chain-required
        relation_hv = _bipolar((N_DIM,), rng)
        query = _bind_xor(known_entity_hv, relation_hv)  # chain-anchor
    else:
        raise ValueError(f"Unknown route: {route}")
    return refuse_slot, retrieval_slot, query, chain_slot
...
def build_regime_dataset(rng, regime: str,
                         ...
                         signal_codebook_retrieval: np.ndarray,
                         signal_codebook_chain: np.ndarray) -> Dict[str, List]:
    """Build class-balanced train + test items for a regime.

    Each item = (route, refuse_slot_hv, retrieval_slot_hv, query_hv,
    chain_slot_hv). Train + test use INDEPENDENT random draws for query_hv
    (per Fix #26 held-out discipline).
    ...
    for route in ROUTES:
        for _ in range(n_train_per_class):
            ...
            rs, ret, q, cs = make_item(r, route, signal_codebook_refuse,
                                        signal_codebook_retrieval,
                                        signal_codebook_chain, known_entity_hv)
...
def train_class_hvs(train_items: List[Dict], roles: Dict[str, np.ndarray],
                    ...
                    use_chain: bool = True) -> Dict[str, np.ndarray]:
    ...
    Args:
      ...
      use_chain: include chain_signal in feature-HV (v2; excluded for ISOLATED
        arm since chain is v2's new discriminator).
    """
...
def run_arm_isolated(rng, dataset: Dict, roles: Dict[str, np.ndarray]) -> Dict:
    ...
    v2: ISOLATED variants EXCLUDE chain_signal (chain is v2's new composition
    ingredient; including it in "isolated" would defeat the purpose of testing
    composition-vs-isolated). So M1.4-only = refuse + query only. M1.5-only =
    retrieval + query only. Both have use_chain=False.
    """
    train_items = dataset["train"]
    test_items = dataset["test"]
    # M1.4 only (refuse_signal + query, no retrieval, no chain)
    ...
    ev_m14 = _evaluate(class_hvs_m14, test_items, roles,
                        use_refuse=True, use_retrieval=False,
                        use_chain=False)
    # M1.5 only (retrieval_signal + query, no refuse, no chain)
...
def _selftest_feature_hv_shape() -> None:
    ...
    cs = _bipolar((N_DIM,), rng)
    feat = build_feature_hv(rs, ret, q, cs, roles)
    if feat.shape != (N_DIM,):
...
def _selftest_class_hv_separation() -> None:
    ...
    scc = build_signal_codebook(rng, N_CHAIN_SLOTS)
    known = _bipolar((N_DIM,), rng)
    train = []
    for route in ROUTES:
        for _ in range(N_TRAIN_PER_CLASS_SMOKE):
            r = _rng(int(rng.integers(0, 2**31 - 1)))
            rs, ret, q, cs_hv = make_item(r, route, scr, sct, scc, known)
...
def _selftest_no_router_baseline_at_chance() -> None:
    ...
    dataset = build_regime_dataset(rng, "test", N_TRAIN_PER_CLASS_SMOKE,
                                    N_TEST_PER_CLASS_SMOKE, scr, sct, scc)
    m = run_arm_no_router(rng, dataset, roles)
...
def run_seed(seed: int, out_dir: Path) -> Dict:
    ...
    scr = build_signal_codebook(rng, N_SIGNAL_SLOTS)
    sct = build_signal_codebook(rng, N_SIGNAL_SLOTS)
    scc = build_signal_codebook(rng, N_CHAIN_SLOTS)   # v2
    ...
    for r_i, regime in enumerate(REGIMES):
        ...
        rows = run_regime(regime_seed, regime, roles, scr, sct, scc,
...
def _main() -> None:
    ...
    metrics = {
        ...
        "milestone": "M1.6_surgical_v2_n_test_20_plus_chain_signal_after_atom_27_MM",
        "per_seed": [
            {
                "seed": r.get("seed"),
