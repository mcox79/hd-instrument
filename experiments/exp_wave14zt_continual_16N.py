"""Continual editing at M=16N - extends zh/zi/zq envelope.

zh M=2N held, zi M=4N held, zq M=8N pending. zt pushes to M=16N to find
substrate ceiling.

Pre-reg: preregs/2026-05-21_wave14zt_continual_16N.md
"""
...
def compute_verdict(summary):
    ...
    if k_ok and c_ok:
        return ("CONTINUAL_16N_BOTH_HOLD",
...
def self_test_verdict():
    ...
    cases = [
        ({"by_arm": {"kerdock": mk(0.98), "correlated": mk(0.5, fail_e=2)}},
         "CONTINUAL_16N_KERDOCK_HOLDS"),
        ({"by_arm": {"kerdock": mk(0.98), "correlated": mk(0.98)}},
         "CONTINUAL_16N_BOTH_HOLD"),
...
def run_experiment(smoke):
    ...
    config = {"mode": "smoke" if smoke else "full", "N": 1024 if smoke else 4096,
              "M_stored": 4096 if smoke else 65536,  # full=16N at N=4096
...
def run_smoke():
    out_dir = get_output_dir("wave14zt_continual_16N_smoke")
...
def run_main():
    out_dir = get_output_dir("wave14zt_continual_16N")
