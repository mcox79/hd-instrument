"""
exp_comp_1bit_verify_1_ksweep_cpu_v1.py -- COMP-1BIT-VERIFY-1 K-SWEEP -- CPU.

ROUTING: Research 1BIT_DEPTH_VERIFICATION (COMP-1BIT-VERIFY-1 K-SWEEP); pure-FHRR (no download). Sweep K=2/5/10/20/50 at L=5; find max K with 1-bit zero-loss.
  Bipolar QPSK 1-bit-per-component quantization of EVERY stored vector vs float32, on the depth-retrieval task with
  hierarchical cleanup. Baseline config L=5,K=10,M=500,rho=0,N=8192; this cell sweeps K (composition branching). Reports float vs 1-bit
  recall + loss at each point and the critical break value. Verifies PP-301 (COMP-11) under production-realism.
PRE-REGISTERED: HARD-PASS zero-loss to K>=20. MIDDLE>=8. HARD-FAIL K_crit<8.
...
ANCHOR_NAME = "comp_1bit_verify_1_ksweep_cpu_v1"
