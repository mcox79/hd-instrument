"""stage1_regime_map_storage_x_cleanup_v1 sibling seed=13.

STORAGE x CLEANUP_MECHANISM pairwise probe (first of Stage 1 REGIME MAP arc).
Purpose: does BUNDLED (below-capacity) storage recover the mechanism-axis variance
that SHARDED (Option Y 2026-07-03) collapsed to 0.000?

Sweep FULL: 2 storage x 3 cleanup x 3 M x 2 N x 2 corr = 72 pts/seed.
Sweep SMOKE: 2 storage x 3 cleanup x 1 M x 1 N x 1 corr = 6 pts.

CHUNKED architecture: one seed per sibling file. Siblings: s7, s19.

PRE-REG: preregs/2026-07-03_stage1_regime_map_storage_x_cleanup_first_probe.md
CARDINALITY_OK_FULL: 72 phase points per seed
CARDINALITY_OK_SMOKE: 6 phase points per seed

Defensive patterns (USER 2026-06-28):
  1. start_marker (via _write_minimal_metrics STARTED marker)
...
SEED = 13
ANCHOR_NAME = f"stage1_regime_map_storage_x_cleanup_v1_s{SEED}"
...
CONFIG_VERSION = (
    ...
    f"regime=fhrr_chain_composition_storage_x_cleanup_pairwise_probe,"
    f"storage=[SHARDED,BUNDLED],"
    f"cleanup=[modern_hopfield,iterative_cosine,soft_energy_attractor],"
    f"M_full=[200,800,3200],N_full=[2048,8192],corr_full=[0.20,0.45],"
    f"M_smoke=[200],N_smoke=[2048],corr_smoke=[0.20],"
    f"F=1,L=2,SEED={SEED},mode={RUN_MODE},"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "stage1_regime_map_storage_x_cleanup_v1_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: Exception) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "stage1_regime_map_storage_x_cleanup_v1_import_crash",
        }
        tmp = out_dir / "metrics.json.tmp"
        final = out_dir / "metrics.json"
...
def main() -> int:
    ...
    from experiments._stage1_regime_map_storage_x_cleanup_v1_core import (
    ...
    final["_hardening_marker"] = "stage1_regime_map_storage_x_cleanup_v1_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = "synthetic_fhrr_chain_composition_storage_x_cleanup_pairwise_v1"
