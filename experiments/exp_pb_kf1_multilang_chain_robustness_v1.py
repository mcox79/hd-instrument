"""
exp_kf1_paraphrase_robustness_marianmt_v1 -- propose-back (HARDER multi-language chain en->de->fr->en) -- GPU.
...
ROUTING: Batch E Probe-2 #1 (Research decision: MarianMT round-trip, real attack). KF-1 grounding-based hallucination
  ...
  surface form). Multi-hop chain induces MORE drift than single round-trip; tests grounding robustness ceiling. GPU (MT + encoder forward).
...
ANCHOR_NAME = "pb_kf1_multilang_chain_robustness_v1"
...
def roundtrip(texts):
    ...
    def _xlate(src_texts, src_lang, tgt_lang):
        ...
        return out
    de = _xlate(texts, "eng_Latn", "deu_Latn"); fr = _xlate(de, "deu_Latn", "fra_Latn"); en = _xlate(fr, "fra_Latn", "eng_Latn")
    del m
    if DEV.type == "cuda":
        torch.cuda.empty_cache()
    return en
...
def run_seed(seed) -> Dict:
    ...
    # attack: paraphrase real claims via en->de->fr->en 3-hop chain, re-ground
...
def verdict(ps) -> Tuple[str, str]:
    ...
    if pa >= 0.85:
        return ("HARD_PASS", "HARD_PASS: KF-1 grounding ROBUST to 3-hop multi-language chain (AUC>=0.85) -- survives heavy semantic drift. " + summary)
    if pa >= 0.65:
...
print("[config] anchor=%s mode=%s seeds=%s N_kb=%d N_claim=%d MT=NLLB-en-de-fr-en" % (ANCHOR_NAME, RUN_MODE, SEEDS, N_KB, N_CLAIM), flush=True)
