"""
q_b1_chain_depth_35_v1_n8192 -- Q-B1: heteroassoc chain at depth-35 at N=8192.

Extends depth-30 (HARD_PASS in Cycle 12) to depth-35.
  GPU IMPLEMENTATION: H matrix (N x N float32 at N=8192): 268 MB. Safe on 8 GB GPU.
  Snapshots at depths [1, 3, 5, 10, 15, 20, 25, 30, 35].

PRE-REGISTERED BANDS:
  HARD-PASS: depth-5 >= 0.95 AND depth-10 >= 0.88 AND depth-20 >= 0.70
             AND depth-30 >= 0.55 AND depth-35 >= 0.50.
  HARD-FAIL: depth-5 < 0.80 OR depth-10 < 0.65 OR depth-20 < 0.40 OR depth-35 < 0.25.
  MIDDLE: depth-35 in [0.35, 0.50) while earlier depths meet HP.
  Prior: depth-30 HARD_PASS; extrapolation via 0.995^35 ~ 0.839 (geometric decay).
  HP at depth-35: 0.50 (conservative estimate given N=8192 interference).

FORMULA SELF-TESTS:
  1. depth-35 chain retrieval non-NaN at N=128.
...
ANCHOR_NAME = "q_b1_chain_depth_35_v1_n8192"
...
CHAIN_DEPTH = 35
SNAPSHOT_DEPTHS = [1, 3, 5, 10, 15, 20, 25, 30, 35]
...
def _instrumentation_selftest():
    ...
    assert not (cos != cos), f"depth-35 retrieval is NaN at N={N_t}"
    ...
    del H
    print(f"[selftest] PASS: depth-35 chain non-NaN N={N_t}, self-cosine=1.0, "
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    summary = (f"d5={d5:.4f}(HP>={HP_D5} HF<{HF_D5}) "
               ...
               f"d35={d35:.4f}(HP>={HP_D35} HF<{HF_D35}) "
               f"n_seeds={len(results)}")

    if d5 < HF_D5 or d10 < HF_D10 or d20 < HF_D20 or d35 < HF_D35:
    ...
    if hp5 and hp10 and hp20 and hp30 and hp35:
        return ("HARD_PASS", f"HARD_PASS: all depth thresholds met at N=8192 depth-35. {summary}")
    n_hp = sum([hp5, hp10, hp20, hp30, hp35])
    if n_hp >= 4:
