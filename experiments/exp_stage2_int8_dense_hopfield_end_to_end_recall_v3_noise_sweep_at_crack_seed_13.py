"""Stage 2 opener v3: INT8 dense-Hopfield noise-cliff sweep at crack, seed=7.

v1 aborted; v2 landed HARD_FAIL_META_RULE_Q (substrate supra-linear capacity
falsified linear crack-scaling). v3 pivot (Research Option B 2026-07-01):
sweep SIGMA into noise cliff at fixed near-crack M for N=8192.

v3 grid: 4 arms x 3 M x 6 sigma = 72 units/seed (N=8192 fixed).
  FULL_M_SWEEP = [10000, 100000, 160000]
  FULL_N_FIXED = 8192
  FULL_SIGMA_SWEEP = [0.20, 0.35, 0.42, 0.45, 0.48, 0.55] (fine around cliff)

Discriminator search: best-discriminating (M=160k, sigma) point auto-selected
from sigma sweep -- FP32 unsaturated AND arms_range >= 0.03. This makes v3
robust to cliff-location uncertainty (cliff at N=8192 M=160k is empirically
unknown pre-dispatch).

4 HP gates:
  HP_META_RULE_Q_ATCLIFF:   FP32 unsaturated AND arms_range >= 0.03 at cliff
  HP_INT8_PARETO_CLIFF:     |INT8 - FP32| <= 0.05 at cliff
  HP_INT4_BREAKS_AT_CLIFF:  (FP32 - INT4) >= 0.20 at cliff (documentation)
...
Route: overnight_queue (GPU); timeout 7200s per USER task.
...
SEED = 13
ANCHOR_NAME = f"stage2_int8_dense_hopfield_end_to_end_recall_v3_noise_sweep_at_crack_seed_{SEED}"
...
CONFIG_VERSION = (
    ...
    f"M_sweep_full=[10000,100000,160000],M_sweep_smoke=[100000],"
    f"N_fixed_full=8192,N_fixed_smoke=4096,"
    f"sigma_sweep_full=[0.20,0.28,0.35,0.38,0.42,0.55],sigma_sweep_smoke=[0.28],"
    ...
    f"HP_INT8_PARETO_CLIFF_TOL=0.05,HP_INT4_BREAKS_DELTA=0.20,"
    f"HP_PRE_CRACK_FREE_TOL=0.01,HP_PRE_CRACK_SIGMA_MAX=0.42,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v3_stage2_int8_pareto_noise_cliff_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v3_stage2_int8_pareto_noise_cliff_import_crash",
        }
        tmp = out_dir / "metrics.json.tmp"
        final = out_dir / "metrics.json"
...
def main() -> int:
    ...
    from experiments._stage2_int8_dense_hopfield_end_to_end_recall_v3_noise_sweep_at_crack_core import (
    ...
    final["_hardening_marker"] = "v3_stage2_int8_pareto_noise_cliff_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = (
        "synthetic_uniform_KG_5000ent_100rel_N_fixed_8192_"
        "M_sweep_10k_100k_160k_"
        "sigma_sweep_0.20_0.35_0.42_0.45_0.48_0.55_"
        "holdout_10pct_queries_noise_cliff_v3_amendment"
    )
