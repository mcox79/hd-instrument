"""stage1_regime_probe_13_L_x_cleanup_non_saturated_v1 sibling seed=19.

L (chain-depth) x CLEANUP_MECHANISM CROSS-TERM at cliff-adjacent SHARDED F=1.
REGIME-EXTENSION of atom #3 into the L x CLEANUP interaction. Tests whether
today's F x CLEANUP mech-moderation pattern (P6v2 + P8) is L=2-specific or
holds across L in {1, 2, 4} (band-only per Skunkworks P12 VET).

Sweep FULL: 3 L x 3 mech (CLIFF) + 3 L x 3 mech (DEEP_SAT) + 1 PC = 19 pts/seed.
Sweep SMOKE: 3 L x 3 mech (CLIFF) + 1 L x 3 mech (DEEP_SAT) + 1 PC = 13 pts.

Hypotheses (cross-term interaction on CLIFF arm; MM_TENTATIVE at SMOKE):
  H1: cliff.cross_term_signal >= 0.10 -> L x MECH interaction genuine (H1).
  H2: cliff.cross_term_signal < 0.05  -> L x MECH interaction inert (H2).
  H3-NULL: deep.cross_term_signal < 0.05 -> null degeneracy at deep-saturation.

CHUNKED architecture: one seed per sibling file. Siblings: s13, s19 (to be
authored after Tailscale restore for 3-seed FULL replication).

PRE-REG: preregs/2026-07-03_stage1_regime_probe_13_L_x_cleanup_non_saturated_v1.md
CARDINALITY_OK_FULL: 19 phase points per seed
CARDINALITY_OK_SMOKE: 13 phase points per seed
...
SEED = 19
ANCHOR_NAME = f"stage1_regime_probe_13_L_x_cleanup_non_saturated_v1_s{SEED}"
...
CONFIG_VERSION = (
    ...
    f"regime=sharded_FHRR_chain_composition_L_x_CLEANUP_cross_term_cliff_plus_deep_sat_null,"
    f"cleanup_mechanisms=[modern_hopfield,iterative_cosine,soft_energy_attractor],"
    f"L_cliff=[1,2,4],L_deep_full=[1,2,4],L_deep_smoke=[2],"
    f"CLIFF=(N=512,M=6400,corr=0.85,F=1,SHARDED),"
    f"DEEP_SAT=(N=8192,M=800,corr=0.60,F=1,SHARDED),"
    ...
    f"discriminator=cross_term_signal=max(range_mech_spread_across_L,range_L_spread_across_mech),"
    f"H1_thresh=(cliff_cross_term_signal>=0.10),"
    f"H2_thresh=(cliff_cross_term_signal<0.05),"
    f"H3_null_thresh=(deep_cross_term_signal<0.05),"
    f"expected_n_full=19,expected_n_smoke=13,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "stage1_regime_probe_13_L_x_cleanup_non_saturated_v1_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: Exception) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "stage1_regime_probe_13_L_x_cleanup_non_saturated_v1_import_crash",
        }
        tmp = out_dir / "metrics.json.tmp"
        final = out_dir / "metrics.json"
...
def main() -> int:
    ...
    from experiments._stage1_regime_probe_13_L_x_cleanup_non_saturated_v1_core import (
    ...
    final["_hardening_marker"] = "stage1_regime_probe_13_L_x_cleanup_non_saturated_v1_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = "synthetic_sharded_fhrr_chain_composition_L_x_cleanup_cross_term_v1"
