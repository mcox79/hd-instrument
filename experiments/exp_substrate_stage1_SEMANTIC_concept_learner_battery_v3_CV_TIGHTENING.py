"""substrate_stage1_SEMANTIC_concept_learner_battery_v3_CV_TIGHTENING -- v2
HARD_PASS at 6/6 arms but max_cv=0.083 above 0.05 chain-grade-DEFINITIVE
threshold; v3 doubles seeds + adds more concepts for tighter CV.

v2 FULL landed HARD_PASS with 6/6 arms PASS but max_cv=0.083, blocking
chain-grade-DEFINITIVE (which requires cv<=0.05). 3 seeds was too few to drive
CV below 0.05 with smaller-N effects. v3 doubles seeds (3->5) + increases
concept count (8->12 cats, 12->16 attrs) for better averaging through the
chain primitives.

PRODUCTION CONFIG (v3 CV-tightening):
  N_DIM=8192 (same as v2)
  V_categories=12 (up from 8)
  inst_per_cat=4 (same -> 48 instances total, was 32)
  V_attrs=16 (up from 12)
  M_basic=144 (up from 96 -- more triples for better averaging)
  n_heldout=12 (up from 8 -- more held-out points for tighter A3 PRIMARY CV)
  n_foreign=8 (up from 6)
  n_audit=36 (up from 24)
  sparse_f=0.020, sparse_amp=7.071
  seeds=[7, 13, 17, 23, 29] (5 seeds, up from 3)
  Routing: remote_cpu_queue (CPU-feasible; ~5-10min per seed * 5 seeds)

HARD bands (v3 definitive):
  STAGE_1_CHAIN_GRADE_DEFINITIVE: >=5/6 arms PASS AND max_cv <= 0.05 AND A3
                                    PRIMARY top1 >= 0.95 (UPGRADE target)
  STAGE_1_CHAIN_GRADE_ALIVE:      >=5/6 arms PASS (any CV) -- v2 status
  STAGE_1_PARTIAL:                3-4/6 PASS
  STAGE_1_GAPS:                   <=2/6 PASS

Per-arm bands (UNCHANGED from v2 FULL):
  A1 basic recall5  >= 0.95
  A2 inh_top1       >= 0.80
  A3 heldout_top1   >= 0.85  (PRIMARY)
...
ANCHOR_NAME = "substrate_stage1_SEMANTIC_concept_learner_battery_v3_CV_TIGHTENING"
...
# v3 CV-tightening config (per USER cell spec)
if RUN_MODE == "full":
    SEEDS = [7, 13, 17, 23, 29]     # v3: 5 seeds (up from v2 3) for tighter CV
    N_DIM = 8192
    N_CATEGORIES = 12               # v3: 12 cats (up from v2 8)
    N_INSTANCES_PER_CAT = 4         # 48 instances total (up from v2 32)
    N_ATTRIBUTES = 16               # v3: 16 attrs (up from v2 12)
    N_TRIPLES_BASIC = 144           # v3: 144 (up from v2 96)
    N_HELDOUT_INSTANCES = 12        # v3: 12 (up from v2 8 -- tighter A3 CV)
    N_FOREIGN_CONCEPTS = 8          # v3: 8 (up from v2 6)
    N_AUDIT_QUERIES = 36            # v3: 36 (up from v2 24)
else:  # smoke
    SEEDS = [0]
    N_DIM = 1024
    N_CATEGORIES = 6
