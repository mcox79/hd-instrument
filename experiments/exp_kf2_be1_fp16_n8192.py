"""KF-2 BE-1 PRECISION: fp16 (half-precision) at N=8192.

Delegates to exp_kf2_be1_precision_sweep_n8192 with PRECISION='fp16'.
fp16 = 2x memory compression vs fp32. Expected: max_iso < 0.05 (should hold;
fp16 rounding is small vs isolation bound).

N-suffix: _n8192 -> production N = 8192 (PROT-018 binding).
Anchor: kf2_be1_fp16_n8192
...
_drv_spec = importlib.util.spec_from_file_location("be1_drv_fp16", _drv_path)
...
def get_output_dir() -> Path:
    name = os.environ.get("HDLAB_EXP_NAME", "kf2_be1_fp16_n8192")
...
def _instrumentation_selftest() -> None:
    ...
    assert cell.get("precision_compression_ratio", 0) == 2.0, (
        f"fp16 compression must be 2.0; got {cell.get('precision_compression_ratio')}")
    print(f"[selftest] kf2_be1_fp16_n8192 PASS iso={cell['isolation_ratio']:.5f}", flush=True)
...
def main():
    ...
    print(f"[run] kf2_be1_fp16_n8192 smoke={smoke} N={N_cfg} precision={PRECISION} "
    ...
    if max_iso >= _drv.HF_CONTAMINATION:
        verdict = "KF2_BE1_FP16_HARD_FAIL"
        verdict_msg = f"FP16_BROKEN: max_iso={max_iso:.5f} >= {_drv.HF_CONTAMINATION}"
    elif seeds_pass >= _drv.HP_SEEDS_MIN:
        verdict = "KF2_BE1_FP16_HARD_PASS"
        verdict_msg = (f"FP16_HOLDS: {seeds_pass}/{len(seeds)} seeds pass, "
                       f"max_iso={max_iso:.5f} < {_drv.HP_ISOLATION_MAX}, "
                       f"compression={compression:.1f}x theory_bound={theory_bound:.5f}")
    else:
        verdict = "KF2_BE1_FP16_MIDDLE_BAND"
        verdict_msg = f"FP16_PARTIAL: {seeds_pass}/{len(seeds)} seeds, max_iso={max_iso:.5f}"

    elapsed = round(time.time() - t0, 2)
    summary = {
        "anchor": "kf2_be1_fp16_n8192", "precision": PRECISION, "N": N_cfg, "smoke": smoke,
