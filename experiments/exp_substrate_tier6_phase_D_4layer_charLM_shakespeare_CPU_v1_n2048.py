"""
substrate_tier6_phase_D_4layer_charLM_shakespeare_CPU_v1_n2048 -- Tier 6 Phase D: substrate-hybrid LLM training -- CPU.
...
ANCHOR_NAME = "substrate_tier6_phase_D_4layer_charLM_shakespeare_CPU_v1_n2048"
_N_SUFFIX = 2048; N_MODEL = 2048; assert N_MODEL == _N_SUFFIX
DEVICE = torch.device("cpu"); torch.set_num_threads(max(1, (os.cpu_count() or 4) - 1))
...
class SubstrateAttn(nn.Module):
    ...
    def __init__(self, d, g):
        super().__init__()
        self.Wq = (torch.randn(d, d, generator=g) / math.sqrt(d)); self.Wk = (torch.randn(d, d, generator=g) / math.sqrt(d))
        self.Wv = (torch.randn(d, d, generator=g) / math.sqrt(d)); self.k_act = max(1, int(F_SPARSE * d)); self.d = d
    ...
    def forward(self, x):  # x: (B,T,d) -- VECTORIZED causal linear attention (= W_t @ Q_t identity), normalized
        ...
        decay = torch.pow(torch.tensor(0.98), (ti[:, None] - ti[None, :]).clamp(min=0).float())  # STDP-asymmetric
...
class LearnedAttn(nn.Module):
    ...
    def forward(self, x):
        ...
        mask = torch.triu(torch.ones(T_, T_), 1).bool(); att = att.masked_fill(mask, float("-inf"))
...
def get_batch(data, bs, T_, g):
    ...
    xb = torch.stack([data[i:i + T_] for i in ix]); yb = torch.stack([data[i + 1:i + 1 + T_] for i in ix])
    return xb, yb


def train_eval(hybrid, data, V, seed):
    g = torch.Generator().manual_seed(seed); model = CharLM(V, D, hybrid, g)
...
def audit_deletion(model):
    ...
    x = torch.randn(1, 8, d, generator=g); Q = x @ lyr.Wq; K = x @ lyr.Wk; V = x @ lyr.Wv
...
def _selftest():
    ...
    sa = SubstrateAttn(d, g); x = torch.randn(1, 4, d, generator=g); o = sa(x); assert o.shape == x.shape, "shape"
    ...
    m = CharLM(10, d, True, g); assert audit_deletion(m), "deletion-cert"
