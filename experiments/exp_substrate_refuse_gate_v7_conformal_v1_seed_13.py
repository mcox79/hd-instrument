"""substrate_refuse_gate_v7_conformal_v1 sibling seed=13.

v7 iterates from v6 (commit 96525dc9) which was smoke-HF on cardinality
(HARD_FAIL_ARMS_DECISION_NOT_DISTINCT: CONFORMAL_10 and CONFORMAL_20
collapsed to identical tau=0.6999 because 25-sample in-KB cal cluster is
TIGHT). Empirical v6 finding: at (moderate, borderline) CONFORMAL
refuse_precision=1.000 vs FIXED=0.000 - HUGE mechanism lift where FIXED
is broken. v6 chose HP band at (moderate, ood) which was already
by-construction saturated at 1.000 for FIXED.

v7 CHANGES (Option A + B combined):
- HP band: (moderate, borderline) where mechanism has empirical room
- 4 ARMS: FIXED_BASELINE / CONFORMAL_05 / CONFORMAL_25 / CONFORMAL_REGIME_MID
- Cal set: 100 items (50 in-KB + 50 OOD; 2x v6 size)
- Widened alpha spread: 0.05 vs 0.25 (previously 0.10 vs 0.20)
...
CHUNKED single-seed-per-cell. SEED=13 pinned.
...
SEED = 13
ANCHOR_NAME = "substrate_refuse_gate_v7_conformal_v1_seed_%d" % SEED
...
CONFIG_VERSION = (
    "ANCHOR=%s,arms=[FIXED_BASELINE,CONFORMAL_05,CONFORMAL_25,CONFORMAL_REGIME_MID],"
    "regimes=[clean,moderate,heavy],bands=[in_kb,borderline,ood],"
    "cal_size=100(50in+50ood),V_REL=256,SEED=%d,mode=%s,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v7_refuse_gate_conformal_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v7_refuse_gate_conformal_import_crash",
        }
...
def main() -> int:
    ...
    from experiments._substrate_refuse_gate_v7_conformal_v1_core import (
    ...
    final["_hardening_marker"] = "v7_refuse_gate_conformal_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = ("synthetic_substrate_refuse_gate_v7_conformal_"
                                   "4_arm_3_regime_3_band_calset100_v1")
