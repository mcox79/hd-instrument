...
ANCHOR_NAME = "phase4d_code_richfeat_cpu_v1"
...
def _feats(prompt):
    ...
    for i in range(len(ws) - 2): fs.add("t:%s_%s_%s" % (ws[i], ws[i + 1], ws[i + 2]))
    # imperative verb: word after "to"/"that"/"which"/"function" -- the core operation
    m = re.search(r"(?:to|that|which|will|should|function)\s+([a-z]+)", low)
    if m: fs.add("VERB:" + m.group(1))
    m2 = re.search(r"(?:to|that|which)\s+([a-z]+)\s+(?:the\s+)?([a-z]+)", low)
    if m2: fs.add("VO:%s_%s" % (m2.group(1), m2.group(2)))
    # algorithm cue words anywhere
    for cue in ("sort", "sorted", "find", "search", "count", "sum", "reverse", "prime", "factorial", "fibonacci", "palindrome", "maximum", "minimum", "largest", "smallest", "vowel", "duplicate", "frequency", "average", "gcd", "divisor", "rotate", "index", "remove", "replace"):
        if cue in low: fs.add("cue:" + cue)
    fs.add("BIAS"); return fs
def _selftest():
    assert _gold_type("def f(s):\n return s.split()", "x") == "STRING"
    print("[selftest] PASS: phase4d-code-richfeat", flush=True)
_selftest()
if _ARGS.self_test:
    sys.exit(0)
def _load():
    ...
    return tr, te
def run() -> Dict:
    rng = np.random.default_rng(int(os.environ.get("HDLAB_SEED", "1019")))
    try:
        train, test = _load()
