"""
pp58_scs_d_sweep_tau050_calibrated_v1_n8192 -- PP-58 SCS d-sweep at CALIBRATED tau_target=0.50.

CONTEXT (v382 cycle 54 -- FLAG-1 interpretation B; pairs with cycle-53 pp58_scs_d_sweep_tau_actual @0.71):
  Cycle 50 established that building the controlled-asymmetry W with TAU_TARGET=0.50 yields measured
  tau_actual~0.71 at N=8192 (the substrate's ACTUAL operating tau). This is the scientifically-correct
  way to make the substrate OPERATE at tau~0.71 (vs setting TAU_TARGET=0.71, which overshoots to
  tau_actual~0.93 -- see cycle-52/53 FLAG 1). Here we sweep the d range (via alpha = pattern count,
  which controls the spectral spike strength d) at TAU_TARGET=0.50, and at each cell check whether
  SCS gamma=(d + tau_actual/d)/(1+tau_actual) agrees with the empirical isochoric-kappa3 gamma within 30%.

  This is the FLAG-1 interpretation-B companion to the cycle-53 d-sweep (which used TAU_TARGET=0.71).
  Together they give the orchestrator both readings: substrate operating AT ~0.71 (this script) vs
  ABOVE 0.71 (cycle 53). d is a MEASURED quantity, not a free knob; achieved d is reported per cell.

SCIENTIFIC QUESTION:
  With the substrate operating at its real measured tau (~0.71 via calibrated target 0.50), does the
  SCS formula agree with empirical gamma within 30% across the d range in a majority of cells?

PRE-REGISTERED BANDS (mirrors cycle 53 item B):
...
FORMULA SELF-TESTS (PROT-022):
  1. SCS gamma(d=8, tau=0.50) = (8 + 0.50/8)/(1+0.50) = 8.0625/1.50 = 5.3750. [EXPECTED: 5.3750 +-0.001]
...
ANCHOR_NAME = "pp58_scs_d_sweep_tau050_calibrated_v1_n8192"
...
_g_d8_t050 = _scs_gamma(8.0, 0.50)
_expected_t050 = 8.0625 / 1.50
print(f"[selftest-formula] gamma(d=8,tau=0.50)={_g_d8_t050:.4f} expected {_expected_t050:.4f}", flush=True)
assert abs(_g_d8_t050 - _expected_t050) < 0.001, f"SCS formula selftest tau=0.50: {_g_d8_t050}"
