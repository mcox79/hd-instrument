"""GPU-FITTING VARIANT (course_c_map_builder_cskg_l2_genuine_GPU_v1) of the CSKG L2-genuine map-builder.

WHY THIS VARIANT EXISTS. The prior GPU FULL OOM'd TWICE on the shared 8GB GPU (BOINC holds ~5GB -> only
~2.5GB free). The first fix chunked the (nq,N) candidate SCORING (worked -- no scoring OOM). But two memory
sources remained UN-chunked: (i) the TransE coord-fit did FULL-BATCH negative sampling -- the
(E, KGE_NEG, k) intermediate is ~1.5-3 GiB at FULL CSKG edge counts, and it was allocated 7x sequentially
(5 geom arms + oracle + 2 backdoor refits) WITHOUT torch.cuda.empty_cache(), so the caching allocator
RESERVED the peak (~the reported 4.33 GiB base); (ii) DISCRETE's Z (N x dim complex64 ~0.84 GiB) stayed
resident THROUGH the geom-arm FPE scoring, which ALSO materializes S_all (0.84 GiB) + the cos/sin/complex
encode transient (~1.26 GiB). This variant REDUCES the peak to fit under ~2.3 GiB (headroom below the ~2.5 GiB
free) WITHOUT touching the science:
  - fit_transe_coords: negative sampling chunked via GRADIENT ACCUMULATION (bit-equivalent full-batch
    gradient; the (chunk, KGE_NEG, k) transient is ~40 MB instead of GiB-scale, E-INDEPENDENT).
  - fit_transe_replay: minibatch capped so its neg intermediate is bounded regardless of E.
  - fit_discrete_bind + the S_all FPE encode: phasor tables built ROW-CHUNKED so the cos/sin/complex encode
    transient never materializes whole (peak = the 0.84 GiB table + a ~0.2 GiB chunk transient, not +1.26 GiB).
  - DISCRETE scored FIRST and Z freed (del + empty_cache) BEFORE any geom arm materializes S_all -> Z and
    S_all NEVER co-reside.
...
DIM PRESERVED at 4096 (readout capacity; the smoke under-fits at low dim + the ORACLE must fire). Estimated
peak (arithmetic, N~25.7k x dim=4096): ~1.0-1.7 GiB (single 0.84 GiB phasor table + <=0.84 GiB worst-case
conj materialization + <=0.2 GiB encode/query transient) << 2.3 GiB. If full-dim4096 ever fails to fit on a
GPU with even less free memory, the alternative is to FREE BOINC (~5 GiB) so dim4096 fits trivially -- a USER
decision flagged to the director; dim reduction (3072/2048) is the code-side fallback and the ORACLE-fires
gate catches any resulting under-fit (INCONCLUSIVE, never a false verdict).

Distinct ANCHOR_NAME + output dir -> a SEPARATE metrics.json; runs IN PARALLEL with the in-flight CPU
canonical run (course_c_map_builder_cskg_l2_genuine_v1 on remote_cpu_queue) with NO collision. Do NOT touch
the CPU run or its cell/metrics. Science (arms / controls / oracle-gate / L2-genuine arena / bands) is
IDENTICAL to the CPU cell.

--- ORIGINAL SCIENCE DESCRIPTION (unchanged) ---
...
# Map-builder geometry (the SMOKE-CONFIRMED operator) + its grid testbed (geometry positive control).
# GPU-VARIANT: the three memory-heavy fit primitives (fit_transe_coords / fit_transe_replay /
# fit_discrete_bind) are RE-DEFINED LOCALLY below as memory-bounded equivalents (chunked negative sampling
# via gradient-accumulation that preserves the full-batch objective; capped replay minibatch; row-chunked
# phasor-table construction). The pre-registered hyperparams (KGE_MARGIN/NEG/WD/LR, REPLAY_GATE,
# REPLAY_VAL_FRAC) are mirrored here so the science is UNCHANGED -- only peak device memory differs. Only the
# read-only / non-memory-critical helpers are imported from the operator module (make_fpe_basis / fpe_encode
# / build_grid_graph / split_heldout). The operator module + the CPU cell are NOT modified.
from experiments.exp_course_c_operator_fix_ssp_phase_rotation_replay_v1 import (  # noqa: E402
    make_fpe_basis, fpe_encode, build_grid_graph, split_heldout,
)

# Distinct anchor -> SEPARATE metrics.json dir; runs in PARALLEL with the CPU canonical run
# (course_c_map_builder_cskg_l2_genuine_v1 on remote_cpu_queue) with no path/metrics collision.
...
# ---- MEMORY (GPU-variant): bound the TransE coord-fit + phasor-table construction so peak fits ~2.3 GiB. ----
# FIT_EDGE_CHUNK: minibatch size over edges. In fit_transe_coords the neg intermediate is (chunk, KGE_NEG, k)
#   instead of (E, KGE_NEG, k) -- ~40 MB not GiB. Gradient accumulation over the chunks reproduces the
#   full-batch mean-margin gradient (one opt.step per epoch) so ONESHOT stays a genuine ONE-SHOT full-batch fit.
# REPLAY_BS_CAP: cap the replay minibatch so its neg intermediate is bounded regardless of E.
# ENCODE_ROW_CHUNK: build (N, dim) phasor tables (S_all, discrete Z) row-chunked so the cos/sin/complex encode
#   transient (~3 x row_chunk x dim x 4B) never materializes over all N at once.
FIT_EDGE_CHUNK = 16384
REPLAY_BS_CAP = 131072
ENCODE_ROW_CHUNK = 4096

# ---- TransE / replay hyperparams (MIRRORED from the operator module; science UNCHANGED) ----
KGE_MARGIN = 1.0
KGE_NEG = 10
KGE_WD = 1e-3
KGE_LR = 0.02
REPLAY_GATE = 0.30
REPLAY_VAL_FRAC = 0.15
...
def _encode_all(X_all, W, row_chunk=ENCODE_ROW_CHUNK):
    """Row-chunked equivalent of fpe_encode over the full candidate set: S(x)=exp(i * x @ W), (N,dim) complex64.
    Preallocates the (N,dim) result and fills it in row blocks so the cos/sin/complex encode transient is bounded
    to (row_chunk, dim) instead of (N, dim) -- numerically identical to fpe_encode (per-row independent)."""
    n = X_all.shape[0]
    dim = W.shape[1]
    S = torch.empty((n, dim), dtype=torch.complex64, device=X_all.device)
    for s in range(0, n, row_chunk):
        e = min(s + row_chunk, n)
        ang = X_all[s:e] @ W
        S[s:e] = torch.complex(torch.cos(ang), torch.sin(ang)).to(torch.complex64)
        del ang
    return S


def fit_transe_coords(train_edges, N, n_rel, k, device, seed, epochs, transductive_extra=None):
    """Memory-bounded ONE-SHOT TransE coord fit. Identical objective to the operator module's fit_transe_coords
    (margin-ranking ||X_h+D_r-X_t|| < ||X_h+D_r-X_t'|| - margin, one opt.step per epoch = full-batch), but the
    per-epoch negative-sampling loss is accumulated over EDGE CHUNKS (gradient accumulation) so the
    (chunk, KGE_NEG, k) neg intermediate replaces the (E, KGE_NEG, k) full-batch one. The accumulated gradient
    equals the full-batch mean-margin gradient exactly (elementwise clamp summed over all pairs / total_pairs)."""
    ...
    chunk = min(FIT_EDGE_CHUNK, E) if E > 0 else 1
    total_pairs = float(max(1, E * KGE_NEG))
    for ep in range(epochs):
        opt.zero_grad()
        for s in range(0, E, chunk):
            e = min(s + chunk, E)
            hb = h_all[s:e]; rb = r_all[s:e]; tb = t_all[s:e]
            pred = X[hb] + D[rb]
            pos = torch.norm(pred - X[tb], dim=1)
            neg_t = torch.randint(0, N, (hb.shape[0], KGE_NEG), generator=gg).to(device)
            neg = torch.norm(pred.unsqueeze(1) - X[neg_t], dim=2)          # (chunk, KGE_NEG)
            loss_sum = torch.clamp(KGE_MARGIN + pos.unsqueeze(1) - neg, min=0.0).sum()
            (loss_sum / total_pairs).backward()                            # accumulate; == full-batch mean grad
            del pred, pos, neg_t, neg, loss_sum
        opt.step()
    return X.detach(), D.detach()


def fit_transe_replay(train_edges, N, n_rel, k, device, seed, passes):
    """Memory-bounded iterative interleaved replay with per-relation recall-consistency gate + validation
    early-stop. Identical to the operator module's fit_transe_replay except the minibatch size is capped
    (REPLAY_BS_CAP) so the (bs, KGE_NEG, k) neg intermediate is bounded regardless of E."""
    g = torch.Generator(device="cpu").manual_seed(seed * 7919 + 11)
    X = (torch.randn(N, k, generator=g) * 0.1).to(device).requires_grad_(True)
    D = (torch.randn(n_rel, k, generator=g) * 0.1).to(device).requires_grad_(True)
    opt = torch.optim.Adam([X, D], lr=KGE_LR, weight_decay=KGE_WD)
    ...
    bs = max(32, min(train_edges.shape[0] // 8, REPLAY_BS_CAP))
    for p in range(passes):
        perm = np.random.default_rng(seed * 101 + p).permutation(all_idx)
...
def fit_discrete_bind(train_edges, N, n_rel, dim, device, seed):
    """Memory-bounded DISCRETE_BIND (stage3 failure-mode). Identical to the operator module's fit_discrete_bind
    except the (N,dim) unit-phasor code table Z is built ROW-CHUNKED so the cos/sin/complex encode transient is
    bounded. Per-relation-TYPE learned diagonal unitary rotation R_r unchanged."""
    g = torch.Generator(device="cpu").manual_seed(seed * 5501 + 7)
    Z = torch.empty((N, dim), dtype=torch.complex64, device=device)
    for s in range(0, N, ENCODE_ROW_CHUNK):
        e = min(s + ENCODE_ROW_CHUNK, N)
        ang = (torch.rand(e - s, dim, generator=g) * 2.0 * np.pi).to(device)
        Z[s:e] = torch.complex(torch.cos(ang), torch.sin(ang)).to(torch.complex64)
        del ang
    ...
    for r in range(n_rel):
        ...
        acc = (Z[t] * torch.conj(Z[h])).sum(0)
        mag = torch.abs(acc) + 1e-9
        R[r] = (acc / mag).to(torch.complex64)
    return Z, R
...
def _score_chunked_from_query_phasor(x_hat, X_all, W, chunk=FPE_SCORE_CHUNK):
    ...
    S_all = _encode_all(X_all, W)                # (N, dim) complex64 -- row-chunked build (bounded encode transient)
    S_all_cT = torch.conj(S_all).T               # (dim, N) conj-transpose (conj-view; cuBLAS gemm uses op=C)
    dim = S_all.shape[1]
    nq = x_hat.shape[0]
    n_ent = X_all.shape[0]
...
def _fit_and_score(train_int, hold, N, n_rel, cfg, device, seed, rel_tail_freq, all_true):
    ...
    # ---- coord tables are tiny (N x k float32); fit them all first, empty_cache between each so the
    #      caching allocator does NOT retain each fit's (chunk, KGE_NEG, k) neg transient across the 4+ fits. ----
    ...
    # ---- DISCRETE FIRST: build Z (N x dim complex ~0.84 GiB), score, and FREE it BEFORE any geom arm
    #      materializes its own (N x dim) S_all -> Z and S_all NEVER co-reside (the second-OOM driver). ----
    ...
    # ---- geom arms: each geom_scores materializes S_all ONCE (row-chunked encode), scores query-chunked,
    #      then frees S_all + empty_cache internally; per-arm empty_cache here is belt-and-suspenders. ----
    ...
    # ---- free remaining device residents before returning; arm_scores already live on CPU. X_os is kept
    #      (returned for the optional back-door refit). ----
    del D_os, X_rp, D_rp, X_sc, D_sc, X_rnd, D_rnd, X_or, D_or, W
    _empty(device)
