...
hard-block-STE vs graded-GSBC ship-metric cell for seed=7. A runner-death on this
process loses only seed=7, not the sibling seed=13 / seed=19 runs.
...
The explicit v3-core AND v11-core sibling imports below (noqa; the core imports
them transitively) force queue_add.sh Pattern-6 import-parse to RE-SCP those
cores with this dispatch, so the remote never runs against a stale sibling core
(dep-parity guard; SCRIPT_PRECONDITION_VIOLATION prevention). Pattern-6 parses
only the WRAPPER's direct imports (non-transitive), so all three must be named
here.

run_tag="seed7" isolates this seed's metrics under
data/exp_encoder_gsbc_gradedcode_retrieval_v1_seed7/ (per-seed checkpoints
already isolate via the core's _artifact_dir seed suffix).
