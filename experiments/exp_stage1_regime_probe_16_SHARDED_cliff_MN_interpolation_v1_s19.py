"""stage1_regime_probe_16_SHARDED_cliff_MN_interpolation_v1 sibling seed=19.

SHARDED cliff M/N interpolation: maps the SHARDED cliff shape in (M, corr) plane
at cliff-adjacent regime (N=512, F=1, L=2, MECH=modern_hopfield). Research drill
identified published-lit gap: no joint fan-out x dimension x codebook regime map
for per-slot memory. Kanerva SDM + Cuckoo hashing + Frady Resonator predict a
sharp cliff. This probe MAPS the cliff shape decisively.

Framed as REGIME-MAP-EXTENSION of Probes 6/7/8 + new M/N interpolation.

Sweep SMOKE (12 pts): 3 M x 3 corr (CLIFF) + 2 M (DEEP_SAT) + 1 PC.
Sweep FULL  (16 pts): 3 M x 3 corr (CLIFF) + 3 M @ corr=0.87 (CLIFF_EXT)
                       + 3 M (DEEP_SAT) + 1 PC.

Hypotheses:
  H1 (CLIFF MAPPED corr-dominated): cliff_amplitude>=0.30 AND M_var_within_corr<=0.15
      -> SHARDED cliff mapped decisively; cliff is corr-driven; fills lit-gap.
  H2 (M-N INTERACTION): cliff_amplitude>=0.30 AND M_var_within_corr>0.15
      -> M and corr both shift cliff; Kanerva SDM cliff-ratio supported.
  H3-NULL-SAT: deep_sat.interaction<0.05 -> saturation degeneracy sanity check.
  H4-NULL-NOCLIFF: cliff_amplitude<0.30 -> cliff not in tested regime; re-author.
...
SEED = 19
ANCHOR_NAME = f"stage1_regime_probe_16_SHARDED_cliff_MN_interpolation_v1_s{SEED}"
...
CONFIG_VERSION = (
    ...
    f"regime=sharded_FHRR_chain_cliff_M_x_corr_at_cliff_plus_deep_sat_null,"
    f"mech=modern_hopfield,L=2,F=1,"
    f"M_cliff=[4000,4800,5600],corr_cliff=[0.80,0.85,0.90],"
    f"CLIFF=(N=512,SHARDED),"
    f"CLIFF_EXT_FULL_ONLY=(corr=0.87,M=[4000,4800,5600]),"
    f"DEEP_SAT=(N=8192,corr=0.60,M_smoke=[800,1600],M_full=[800,1600,2400],SHARDED),"
    ...
    f"H1_thresh=(cliff_amplitude>=0.30 AND M_var_within_corr<=0.15),"
    f"H2_thresh=(cliff_amplitude>=0.30 AND M_var_within_corr>0.15),"
    f"H3_null_thresh=(deep_sat_interaction<0.05),"
    f"H4_null_thresh=(cliff_amplitude<0.30),"
    f"expected_n_smoke=12,expected_n_full=16,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "stage1_regime_probe_16_SHARDED_cliff_MN_interpolation_v1_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: Exception) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "stage1_regime_probe_16_SHARDED_cliff_MN_interpolation_v1_import_crash",
        }
        tmp = out_dir / "metrics.json.tmp"
        final = out_dir / "metrics.json"
...
def main() -> int:
    ...
    from experiments._stage1_regime_probe_16_SHARDED_cliff_MN_interpolation_v1_core import (
    ...
    final["_hardening_marker"] = "stage1_regime_probe_16_SHARDED_cliff_MN_interpolation_v1_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = "synthetic_sharded_fhrr_chain_cliff_M_x_corr_v1"
