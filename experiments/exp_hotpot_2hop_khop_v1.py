"""
exp_hotpot_2hop_retrieval_pretest_v1 -- HotpotQA 2-hop with iterated K-HOP relay (whiten + bridge-hop) vs whiten-only 0.26 -- CPU.
...
ANCHOR_NAME = "hotpot_2hop_khop_v1"
...
def run() -> Dict:
    ...
    for d in data:
        ...
        h1 = int(np.argmax(ew @ qw))                                              # hop 1: best supporting sentence
        qbridge = unit((qw + ew[h1])[None, :])[0]                                 # bridge query = q + hop1 (the 2-hop link)
        sims2 = ew @ qbridge; sims2[h1] = -1e9; h2 = int(np.argmax(sims2))         # hop 2: best OTHER, given hop1 context
        found = set([(sents[h1][0], sents[h1][1]), (sents[h2][0], sents[h2][1])])
        sub_hits += int(len(found & d["gold"]) >= 2)
    del m
...
def verdict(r) -> Tuple[str, str]:
    ...
    summary = "khop_recall@2hop=%.3f naive=%.3f lift=%+.3f (n=%d)" % (rs, rn, lift, r["n"])
    if rs >= 0.70:
        return ("HARD_PASS", "HARD_PASS: K-hop relay recall@2hop>=0.70 on HotpotQA -- the bridge-hop closes multi-hop; integration story holds. " + summary)
    if lift >= 0.05:
        return ("MIDDLE_BAND", "MIDDLE_BAND: K-hop relay lifts recall@2hop by >=0.05 over naive but not to 0.70 -- bridge-hop helps; gap remains. " + summary)
