...
ROUTING: Research consolidated Priority 3 (Drill 2 P1 transfer + Tier-4 milestone). The PP-364 POS-tagger mechanism (structured-
  ...
  milestone. CoNLL-2000 (HUMAN chunk annotations, 8936/2012 sents; POS is a strong-but-imperfect feature -- the CLEAN canonical transfer test, NOT circular). A/B: word-only vs +predicted-POS cascade.
...
ANCHOR_NAME = "chunking_conll2000_richfeat_cpu_v1"
...
def run() -> Dict:
    ...
    # POS tagger (PP-364 mechanism) trained on CoNLL-2000 (Penn POS)
    ...
    # chunking: words, HUMAN chunk_bio (NOT POS-derived), PREDICTED POS (cascade)
    def mk(sents):
        out = []
        for e in sents:
...
def verdict(r) -> Tuple[str, str]:
    ...
    return ("HARD_FAIL", "HARD_FAIL: rich-feat cascade chunk-F1 <0.90. " + s)
