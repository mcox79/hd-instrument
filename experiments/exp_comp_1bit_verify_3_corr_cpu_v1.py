"""
exp_comp_1bit_verify_3_corr_cpu_v1.py -- COMP-1BIT-VERIFY-3 CORRELATED-ATOMS -- CPU.

ROUTING: Research 1BIT_DEPTH_VERIFICATION (COMP-1BIT-VERIFY-3 CORRELATED-ATOMS); pure-FHRR (no download). Sweep rho=0/0.05/0.10/0.20 at L=5,K=10,M=500; production-realism (correlated atoms).
  Bipolar QPSK 1-bit-per-component quantization of EVERY stored vector vs float32, on the depth-retrieval task with
  hierarchical cleanup. Baseline config L=5,K=10,M=500,rho=0,N=8192; this cell sweeps rho (atom correlation). Reports float vs 1-bit
  recall + loss at each point and the critical break value. Verifies PP-301 (COMP-11) under production-realism.
PRE-REGISTERED: HARD-PASS tolerate rho>=0.10. MIDDLE>=0.05. HARD-FAIL rho=0.05 breaks.
...
ANCHOR_NAME = "comp_1bit_verify_3_corr_cpu_v1"
