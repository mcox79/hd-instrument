"""
substrate_spectral_monitor_overfitting_v2_n4096 -- Spectral monitor overfitting scale-gate R1.

CONTEXT (v378 cycle 46 rescue R1 for spectral_monitor_overfitting_v1):
  v1 HARD_FAILed: val_overfit_step=None 0/3 seeds.
  sub_overfit_step=200 all 3 seeds (substrate spectral fires at step 200 consistently).
  Root cause: TRAIN_CHARS=30000 / N_STEPS=2000 insufficient -- LM never reaches overfitting
  phase. Rescue R1: increase TRAIN_CHARS=150000, N_STEPS=5000. Substrate signal IS present
  (step 200 sub_overfit consistent); scale gate is the only blocker.

SCIENTIFIC QUESTION:
  Does substrate spectral fingerprint (kappa_4_excess) predict LLM overfitting onset
  >= HP_LEAD steps before validation-loss-curve reveals it, at longer training scale?

PRE-REGISTERED BANDS (overfitting-phase-only; inherited from v1; relaxed scale):
...
FORMULA SELF-TESTS (PROT-022):
  ...
  2. detect_overfitting_val: returns a step index for a synthetic val-loss series.
     [EXPECTED: step returned as int]
  3. detect_overfitting_substrate: returns step < val step for synthetic series.
...
QUEUE: remote_cpu_queue (CPU; tinychar GRU; longer training scale).
TIMEOUT ESTIMATE: v1 elapsed ~40s per seed. v2 increases N_STEPS 2000->5000 (2.5x)
  and TRAIN_CHARS 30000->150000 (5x). Scale increase ~5x for corpus loading + 2.5x steps.
  Conservative estimate: 5x wall time per seed = ~200s/seed.
  ceil(1.5 * 200 * 3) = 900s. Round up: 1800s.
"""
...
ANCHOR_NAME = "substrate_spectral_monitor_overfitting_v2_n4096"
...
if RUN_MODE == "smoke":
    ...
    TRAIN_CHARS = 5000       # smoke: short enough to finish in <30s
    N_STEPS = 200
    MONITOR_EVERY = 5
    ...
    N_OBS = N_OBS_FULL
    BUFFER_SIZE = int(0.12 * N_OBS)
    TRAIN_CHARS = 150000      # SCALE GATE R1: 5x increase from v1 (30000->150000)
    N_STEPS = 5000            # SCALE GATE R1: 2.5x increase from v1 (2000->5000)
    MONITOR_EVERY = 5
...
# Pre-registered thresholds (inherited from v1)
HP_LEAD_STEPS = 50
...
def run_seed(seed: int) -> Dict:
    ...
    # Repeat corpus to induce overfitting (smaller repeat factor; 5x TRAIN_CHARS gives more signal)
