...
Pre-reg: preregs/2026-05-21_wave14_bet_f_fine_noise.md
"""
...
def compute_verdict(summary):
    ...
    if not per_p:
        return ("BET_F_S5_INCONCLUSIVE", "Missing recovery data.")
    # Compare Kerdock to random control
    # Fine-noise variant: use p=0.01 as the recovery checkpoint (smaller perturbation)
    rate_at_p005 = per_p.get("0.01") or per_p.get(0.01) or 0.0
    ctrl = summary.get("recovery_by_p_random", {})
    ctrl_at_p005 = ctrl.get("0.01") or ctrl.get(0.01) or 0.5
    ...
    if rate_at_p005 >= PASS_RECOVERY and rate_at_p005 > ctrl_at_p005 + 0.15:
        return ("BET_F_S5_TOPOLOGY_PROTECTED",
...
def self_test_verdict():
    cases = [
        ({"recovery_by_p_kerdock": {"0.01": 0.92}, "recovery_by_p_random": {"0.01": 0.55}},
         "BET_F_S5_TOPOLOGY_PROTECTED"),
        ({"recovery_by_p_kerdock": {"0.01": 0.30}, "recovery_by_p_random": {"0.01": 0.30}},
         "BET_F_S5_KILLED"),
        ({"recovery_by_p_kerdock": {"0.01": 0.60}, "recovery_by_p_random": {"0.01": 0.50}},
         "BET_F_S5_PARTIAL"),
...
def run_one_seed_kerdock(seed, N, M, p_sweep, device):
    ...
    n_per_coset = n_codewords_total // info["n_cosets"]  # codewords per coset
    n_cosets = info["n_cosets"]
    # Coset-label atoms: one bipolar atom per coset
    ...
    # Sample M facts, each is (codeword_idx, coset_idx=codeword_idx//n_per_coset)
    fact_idx = torch.randperm(n_codewords_total, generator=cpu_gen)[:M].to(device)
    fact_cosets = fact_idx // n_per_coset
...
def run_experiment(smoke):
    ...
    config = {"mode": "smoke" if smoke else "full",
              ...
              "p_sweep": [0.0, 0.01] if smoke else [0.0, 0.005, 0.01, 0.02, 0.04],
...
def run_smoke():
    out_dir = get_output_dir("wave14_bet_f_fine_noise_smoke")
...
def run_main():
    out_dir = get_output_dir("wave14_bet_f_fine_noise")
