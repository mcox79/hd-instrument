...
ROUTING: north-star extension to the substrate's STRONGEST capability. Classification head-to-heads done (topic substrate win
  ...
  Viterbi on UD-EWT (17 universal tags). LLM = Qwen2.5-1.5B-Instruct few-shot tagging; ROBUST parse extracts only valid tag tokens (regex) from the whole output, in order. Token accuracy for both. import torch first (PROT-020). Bundled UD-EWT (RESCUE).
...
OUT = REPO / "data" / ("exp_" + os.environ.get("HDLAB_EXP_NAME", "pos_headtohead_llm_v2_gpu_v1"))
...
def main():
    ...
    for words, gold in test:
        ...
        import re as _re
        toks_out = _re.findall(r"[A-Za-z]+", out)
        preds = [UPPER[x.upper()] for x in toks_out if x.upper() in UPPER]   # keep only valid tags, in order
    ...
    json.dump({"anchor_name": "pos_headtohead_llm_v2_gpu_v1", "verdict": verdict, "verdict_msg": msg, "summary": msg,
...
if "--self-test" in sys.argv:
    assert _shape("Bob") == "Cap" and len(UTAGS) == 17; print("[selftest] PASS: pos-headtohead-llm-v2", flush=True); sys.exit(0)
main()
