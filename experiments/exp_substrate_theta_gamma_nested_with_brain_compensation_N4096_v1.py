"""
exp_substrate_theta_gamma_nested_with_brain_compensation_N4096_v1

SCIENTIFIC QUESTION:
  Smoke at N=512 HARD_FAILed (HARD_FAIL_2: sparse arm collapsed at sigma=16
  recall=0.037; root cause: f=0.02 x N=512 = ~10 active dims, insufficient
  projection overlap for cyclic-roll phase averaging). N=4096 x f=0.02 = 82
  active dims -- qualitatively different regime per Aso-Rubin sparsity
  threshold. This cell tests whether brain-canonical structural compensators
  (sparse-bipolar codebook f=0.02 + per-gamma-cycle Hopfield cleanup) recover
  the structural SNR deficit of nested theta-gamma vs single-frequency lock-in
  at the production-scale N=4096 regime.

MECHANISM (6-arm ablation):
  ARM_SINGLE_LOCKIN:        single-freq P=64 lock-in on dense bipolar (baseline)
  ARM_NESTED_BASELINE:      theta-gamma nested on dense bipolar (v1 deficit)
  ARM_NESTED_SPARSE:        theta-gamma on sparse codebook (f=0.02; CERT 592)
  ARM_NESTED_CLEANUP:       theta-gamma on dense codebook + per-gamma Hopfield
  ARM_NESTED_BRAIN_FULL:    sparse + cleanup (compose arms 3+4)
  ARM_SINGLE_LOCKIN_SPARSE: single-freq on sparse codebook (control; neg #3)

PRE-REGISTERED HARD_PASS (any one of A/B/C suffices):
  CRITERION_A: ARM_NESTED_BRAIN_FULL recall@1 at sigma=16 >= ARM_SINGLE_LOCKIN
               recall@1 - 0.02 (brain-compose closes structural deficit)
  CRITERION_B: ARM_NESTED_BRAIN_FULL recall@1 at sigma=32 >= ARM_SINGLE_LOCKIN
               recall@1 + 0.05 (brain-compose beats single in mid-noise)
  CRITERION_C: ARM_NESTED_SPARSE AND ARM_NESTED_CLEANUP each add >= 0.10 recall
               vs ARM_NESTED_BASELINE at sigma=16

  If A+B both pass: chain-grade-eligible (META = brain-compensated-nested
  recovers-and-exceeds single-frequency at N=4096).
...
MIDDLE_BAND:
  ARM_NESTED_BRAIN_FULL exceeds NESTED_BASELINE by 0.05-0.10 but below
  single-frequency. Tune f-grid + tau-grid OR pivot to TDM-gating.

CONFIG (production; N=4096 full regime):
  N=4096, M=500, seeds=[7,17,23], sigmas=[4,8,16,32,64,128]
  P_theta=8, P_gamma=7, P_single=64, k_theta=1, k_gamma=31
  sparse_f=0.02, cleanup_tau=0.30, cleanup_temp=4.0, N_EVAL=200
  Routing: remote_cpu_queue (~60-120min)

CONFIG (smoke; for --smoke flag; smaller scale):
  N=512, M=50, seeds=[7,17], sigmas=[4,8,16,32,64]
  P_theta=4, P_gamma=7, P_single=32

PROT-018 N-suffix: no _n<NUMBER> (lowercase) suffix in anchor name.
  Production N = 4096. See prereg ## N-suffix section.
...
ANCHOR_NAME = "substrate_theta_gamma_nested_with_brain_compensation_N4096_v1"
...
# No CUDA: pure numpy; route to remote_cpu_queue.
...
SPARSE_F = 0.02        # sparse fraction (CERT 592 best regime; 82 active at N=4096)
...
# Pre-registered thresholds (immutable post-dispatch)
...
# ---- core primitives (pure numpy; identical to smoke_v1) ----
...
def theta_gamma_nested_demod_with_cleanup(
    ...
    for t in range(P_theta):
        ...
        # Reverse inner (2/P_gamma) to get per-theta-cycle contribution
...
def make_sparse_bipolar_codebook(M: int, N: int, f: float, rng: np.random.Generator) -> np.ndarray:
    ...
    CERT 592 best regime: f=0.02. At N=4096: 82 active dims per vector.
    """
...
def _selftest_sparse_codebook_gen() -> None:
    ...
    # Verify N=4096 active count: f=0.02 * 4096 = 82
    n_active_4096 = max(1, int(round(0.02 * 4096)))
    assert n_active_4096 == 82, f"N=4096 active dims: expected 82 got {n_active_4096}"
    print(f"  [selftest] sparse codebook gen: frac_nonzero={frac_nonzero:.4f} "
          f"N4096_active={n_active_4096}", flush=True)


def _selftest_cleanup_snap_idempotence_at_sigma0() -> None:
    """Cleanup at sigma=0: clean cue must recall at >= 0.90 after snap."""
...
def _selftest_recall_all_arms_nontrivial() -> None:
    """At sigma=0, all arms must achieve recall@1 >= 0.90 on tiny codebook."""
...
def _instrumentation_selftest() -> None:
    ...
    print(
        "[selftest] PASS brain_compensation_N4096_v1: "
        "P=1-endpoint + sigma=0-single + sigma=0-nested + "
        "sparse-codebook-gen(N=4096 active=82 verified) + "
        "cleanup-snap-idempotence + all-arm-sigma0-recall.",
        flush=True,
    )
...
def run_seed(seed: int) -> Dict[str, Any]:
    ...
    # Sparse bipolar codebook (CERT 592 f=0.02; 82 active dims at N=4096)
...
def verdict(
    ...
    """Apply pre-registered HARD bands (immutable post-dispatch)."""
    ...
    if criterion_a or criterion_b or criterion_c:
        ...
        if criterion_a:
            which.append(
                f"A(brain@16={r_brain_16:.4f}>={r_single_16:.4f}+{HP_A_DELTA}="
                f"{r_single_16+HP_A_DELTA:.4f})"
            )
        if criterion_b:
            which.append(
                f"B(brain@32={r_brain_32:.4f}>={r_single_32:.4f}+{HP_B_DELTA}="
                f"{r_single_32+HP_B_DELTA:.4f})"
            )
        if criterion_c:
            which.append(
                f"C(sparse_delta={r_sparse_16_delta:+.4f}+"
                f"cleanup_delta={r_cleanup_16_delta:+.4f}>={HP_C_EACH})"
            )
        ...
        return (
            ...
            f"at N=4096. Criteria: {'; '.join(which)}. {detail}",
        )
