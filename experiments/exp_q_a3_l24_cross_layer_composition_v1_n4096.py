"""
q_a3_l24_cross_layer_composition_v1_n4096 -- Q-A3: L=24 cross-layer composition at N=4096.

Ceiling chase: L=22 HARD_PASS EXACT-1.0000 unanimous 5/5 (v353+). L=23 expected same.
Dispatching L=24 simultaneously to maximize ceiling-finding throughput.
24 W matrices x ~67 MB each = ~1608 MB total. Fits in 8 GB GPU with margin.

Note: M per stage halves each level per isochoric design; alpha_c constraint ensures validity.
At L=24 M_OUTER=2, M_MID23=2, ..., M_MID7=2 (floored at 2 from L=7+).

PRE-REGISTERED BANDS (ceiling push, prior L=22 EXACT-1.0 unanimous; L=24 expected same
  given isochoric M-halving makes all stages at M=2 from L=7 onward):
  HARD-PASS: all 24 level fidelities = 1.0000 (>= 0.9999) unanimous (5/5 seeds)
             AND l24_acc >= 0.5.
  MIDDLE: any L_fid in [0.85, 1.0) OR graceful degradation of inner levels.
  HARD-FAIL: any L_fid < 0.85 OR l24_acc < 0.5. Ceiling found at L=24.

FORMULA SELF-TESTS (PROT-022):
  1. L=24 chain: 24-step Hadamard roundtrip recovers xi_L1.
...
QUEUE: overnight_queue (GPU; 24 W matrices at N=4096 ~1608 MB).
TIMEOUT ESTIMATE: L=22 elapsed=1.59s FULL 5-seed GPU. L=24 adds 2 W matrices (negligible).
  ceil(1.5 * 1.59 * 1.0 * 1.0) = 3s -> use 300s (5-min safety floor for GPU queue overhead).
"""
import sys
...
ANCHOR_NAME = "q_a3_l24_cross_layer_composition_v1_n4096"
...
def _selftest_l24_chain():
    """24-step Hadamard roundtrip recovers xi_L1."""
    ctxs = []
...
def run_seed(seed: int, n_dim: int) -> Dict:
    ...
    Xi_ctx17 = bsc(M_MID17, n_dim)
    Xi_mid17 = Xi_ctx17 * Xi_mid16[:M_MID17]
    Xi_ctx18 = bsc(M_MID18, n_dim)
    Xi_mid18 = Xi_ctx18 * Xi_mid17[:M_MID18]
    Xi_ctx19 = bsc(M_MID19, n_dim)
    Xi_mid19 = Xi_ctx19 * Xi_mid18[:M_MID19]
    Xi_ctx20 = bsc(M_MID20, n_dim)
    Xi_mid20 = Xi_ctx20 * Xi_mid19[:M_MID20]
    Xi_ctx21 = bsc(M_MID21, n_dim)
    Xi_mid21 = Xi_ctx21 * Xi_mid20[:M_MID21]
    Xi_ctx22 = bsc(M_MID22, n_dim)
    Xi_mid22 = Xi_ctx22 * Xi_mid21[:M_MID22]
    Xi_ctx23 = bsc(M_MID23, n_dim)
    Xi_mid23 = Xi_ctx23 * Xi_mid22[:M_MID23]
    Xi_ctx24 = bsc(M_OUTER, n_dim)
    Xi_outer = Xi_ctx24 * Xi_mid23[:M_OUTER]
