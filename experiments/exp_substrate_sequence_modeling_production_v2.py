"""substrate_sequence_modeling_production_v2 -- bigram-alpha calibration FIX.

v1 ANALYSIS (2026-06-24): v1 HARD_FAILed at full scale with bpc_word_bigram=9.011
> unigram=8.239. ROOT CAUSE: BIGRAM_ALPHA=0.1 over V=4000 places pseudocount 0.1
in every cell of the [V, V] count table -> per-row smoothing-mass = V*alpha = 400,
while real bigram counts per row average N_TRAIN/V = 10. Real signal weight in
P(w|prev) is 10/(10+400) = 2.4% -> baseline collapses toward uniform-over-V
(which is HIGHER bpc than unigram, since unigram exploits frequency-rank).

v2 FIX (one-line): BIGRAM_ALPHA = 0.1 -> 0.001. New smoothing-mass = V*0.001 = 4,
so real-signal weight is 10/(10+4) = 71%. Word-bigram now operates as intended.

v1 SUBSTRATE EVIDENCE PRESERVED: smoke arm at V=200 showed K16+cfRPE BEAT word-
bigram by 0.37 bits (substrate WINS at smoke). Substrate-arm metrics are honest
(no contamination from bigram bug; substrate logits independent of bigram table).
v2 re-runs SAME 5 arms x 3 seeds with corrected baseline -> if substrate K16+
cfRPE clears word-bigram at full scale (V=4000, N_TRAIN=40k), substrate-as-LM
clears the word-bigram threshold (first cell to do so).

CALIBRATION BANDS WIDENED for alpha=0.001: expected bigram bpc in [5.30, 6.90]
(lower bound widened because smoothing-mass collapse no longer dominates).
HARD_CAP retained at 7.40 (baseline-broken guard).
...
ARMS (5; all share same encoder + same eval split for apples-to-apples):
  ...
  ARM_WORD_BIGRAM
      ...
      Approximated via row-conditional bigram count table; alpha=0.001 (v2 FIX:
      v1 used alpha=0.1 -> per-row smoothing-mass V*alpha=400 dominated real
      bigram counts ~10/row; baseline collapsed toward uniform-over-V).
...
PRE-REG HARD BANDS (cell-author 2026-06-24; word-level on text8):
  ...
  ARM_WORD_BIGRAM calibration (v2; alpha=0.001):
    Expected bpc_best in [5.30, 6.90] word-BPC (alpha=0.001 -> smoothing-mass
    V*alpha=4 vs ~10 real bigram counts/row; real-signal weight ~71%; baseline
    should perform NEAR true add-alpha bigram with small-vocab effective smoothing).
    Lower bound widened from v1's 6.30 because smoothing-mass collapse no longer
    dominates -- bigram can now genuinely approach kneser-ney-bigram territory.
...
ANCHOR_NAME = "substrate_sequence_modeling_production_v2"
...
BIGRAM_BPC_MIN = 5.30  # v2: lowered from 6.30 — alpha=0.001 smoothing-mass=4 << v1's 400
BIGRAM_BPC_MAX = 6.90
...
BIGRAM_ALPHA = 0.001             # v2 FIX: smoothing-mass V*alpha=4 (was 400 at v1's 0.1)
...
CONFIG_VERSION = (
    "substrate_sequence_modeling_production_v2; N_DIM=%d N_TRAIN=%d N_HELD=%d "
...
def compute_verdict(units):
    ...
    detail = {
        ...
        "cites": [
            "preregs/2026-06-24_substrate_sequence_modeling_production_v2.md",
            "experiments/exp_substrate_sequence_modeling_production_v1.py (v1; HARD_FAILed on bigram-alpha calibration)",
            ...
            "v2_FIX: BIGRAM_ALPHA 0.1->0.001 (smoothing-mass V*alpha 400->4)",
        ],
    }

    # Fix #30 / verdict_lint mandatory WHAT_THIS_DOES_NOT_SHOW clause builder
    WTDNS_HARD_PASS = (
        "\nWHAT_THIS_DOES_NOT_SHOW:\n"
        "- Does NOT compare to a neural LM (LSTM/Transformer) baseline -- only "
        "compares to add-alpha word-bigram which is a non-neural smoothing LM.\n"
        "- Does NOT generalize beyond text8 word-level V=4000 N_TRAIN=40k -- "
        "result is corpus- and scale-specific; behaviour at V=100k or N_TRAIN=1M is unknown.\n"
        "- Does NOT establish sequence-binding mechanism is causally responsible "
        "for the BPC lift -- absence of an ARM_RANDOM_W control means we cannot "
        "fully rule out that any high-rank substrate would clear the bigram bar."
    )
    WTDNS_HARD_FAIL = (
        "\nWHAT_THIS_DOES_NOT_SHOW:\n"
        "- Does NOT show substrate sequence-binding is a NULL mechanism at all "
        "scales -- only shows it cannot clear word-bigram at this V=4000 N_TRAIN=40k config.\n"
        "- Does NOT compare under tuned cf-RPE eta or K -- eta=0.05 K_LONG=16 are "
        "fixed; other settings unexplored.\n"
        "- Does NOT show the harness itself is correct -- a sanity-rail or calibration FAIL "
        "(if present) indicates harness mis-spec and the substrate-arm claim is bounded."
    )
    WTDNS_MIDDLE = (
        "\nWHAT_THIS_DOES_NOT_SHOW:\n"
        "- Does NOT clear the HP bar of beating word-bigram by >= 0.10 bits -- "
        "the substrate-vs-bigram lift is partial; mechanism is interpretable as "
        "MEASURED but not chain-grade.\n"
        "- Does NOT establish cf-RPE delta-rule adds value beyond K-bind alone -- "
        "K8 vs K16+cfRPE per-arm comparison is required to attribute the lift.\n"
        "- Does NOT generalize beyond text8 word-level V=4000 N_TRAIN=40k."
    )
    ...
    if not bigram_ok:
        return ("HARD_FAIL",
                ("HARD_FAIL: word-bigram baseline broken -- bpc=%.4f > hard-cap %.2f "
                 "(expected [%.2f, %.2f] for add-alpha=%.4f). Baseline mis-spec; "
...
def _selftest():
    ...
    # T1b (v2 FIX validation): on the v1-bug regime (V=4000, ~10 real bigrams/row),
    # alpha=0.001 yields STRICTLY LOWER conditional-prob loss than alpha=0.1.
    # Direct formula check (no rng dependence): for a row where the true next-
    # word has count c=10 out of N=10 total observations:
    #   under alpha=0.1:    P(true|ctx) = (10+0.1)/(10 + 0.1*4000) = 10.1/410   = 0.02463
    #   under alpha=0.001:  P(true|ctx) = (10+0.001)/(10 + 0.001*4000) = 10.001/14 = 0.71436
    # BPC delta = log2(0.71436/0.02463) ~ 4.86 bits — exactly the calibration
    # bug magnitude needed to explain v1's 9.011 > unigram 8.239 reversal.
    V_b = 4000
    alpha_v1_bug = 0.1
    alpha_v2_fix = 0.001
    c = 10.0   # observed bigram count for (prev, true_next)
    N_row = 10.0  # total observed transitions out of prev (typical at N_TRAIN/V)
    p_v1 = (c + alpha_v1_bug) / (N_row + alpha_v1_bug * V_b)
    p_v2 = (c + alpha_v2_fix) / (N_row + alpha_v2_fix * V_b)
    bits_v1 = -math.log(p_v1) / math.log(2.0)
    bits_v2 = -math.log(p_v2) / math.log(2.0)
    assert p_v2 > p_v1 * 25.0, (
        "T1b v2-fix arithmetic: alpha=0.001 should yield >25x prob on planted bigram; "
        "got p_v1=%.5f p_v2=%.5f ratio=%.1fx"
    ) % (p_v1, p_v2, p_v2 / p_v1)
    assert (bits_v1 - bits_v2) > 4.0, (
        "T1b v2-fix arithmetic: alpha=0.001 should save >4 bits/token on planted bigram; "
        "got bits_v1=%.3f bits_v2=%.3f delta=%.3f"
    ) % (bits_v1, bits_v2, bits_v1 - bits_v2)
    # Also: on a small-V planted toy, alpha=0.001 < alpha=0.1 BPC (monotonic).
    rng_b = np.random.default_rng(42)
    V_t2 = 200
    N_t2 = 2000
    idx_b = rng_b.integers(0, V_t2, size=N_t2).astype(np.int64)
    for i in range(0, N_t2 - 1, 10):
        idx_b[i] = 10
        idx_b[i + 1] = 11
    P_high = build_word_bigram(idx_b, V_t2, alpha=0.1)
    P_low = build_word_bigram(idx_b, V_t2, alpha=0.001)
    p_high = P_high[10, 11]
    p_low = P_low[10, 11]
    assert p_low > p_high, (
        "T1b small-V monotonic: alpha=0.001 should yield higher P(11|10) than "
        "alpha=0.1 on planted toy; got p_low=%.4f p_high=%.4f"
    ) % (p_low, p_high)
    ...
    print("[selftest v2] PASS: T1 word-bigram + T1b v2-FIX alpha=0.001<0.1 BPC "
          "+ T2 HRR bind + T3 seq round-trip + T4 cf-RPE shrinks error + T5 K=8 "
          "ctx + T6 bpc unigram + T7 verdict bands (HP/CG/MID/HF/sanity) + T8 llm=0",
          flush=True)
...
def _synthesize_on_exit():
    ...
    try:
        ...
        metrics = {
            ...
            "detail": detail,
            "metrics_source": "atexit_synthesize_partial_substrate_sequence_modeling_production_v2",
            "per_unit": units,
...
if __name__ == "__main__":
    ...
    run_cfg = {"run_mode": RUN_MODE, "N": N_DIM,
                "schema": "substrate-seq-modeling-prod-v2"}
    t0 = time.time()
    _T0_REF[0] = t0
    for seed in SEEDS:
        key = "s%d" % seed
    ...
    metrics = {
        ...
        "n_seeds": len(SEEDS),
        "detail": detail,
        "metrics_source": "measured_cpu_substrate_sequence_modeling_production_v2",
        "per_unit": units,
