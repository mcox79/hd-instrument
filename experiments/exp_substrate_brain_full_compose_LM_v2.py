"""substrate_brain_full_compose_LM_v2 -- MAXIMALIST substrate-as-LM composing
EVERY validated brain-analog primitive in concert.

V2 BUG FIXES (2026-06-23 Skunkworks audit + diagnostic agent):

  Bug 1 (PC primitive zero-init defensiveness): each PC layer W gets a small
    variance-scaled Gaussian init (0.01 / sqrt(N_DIM)) so that distinct inputs
    produce distinct outputs even before Hebbian accumulation kicks in.
    (v1's PC was dot-product based, not sign-based, so the sign(0) saturation
    of the PC hierarchy cell did not strictly manifest here; the init change
    is defensive + symmetric with the PC hierarchy cell fix.)

  Bug 2 (K-WTA destroys predictions): v1 used SPARSE_COMPETITIVE_K_FRAC=0.10
    => K=400/4000 with HARD zero-mask (top-K kept, rest set to -1e9). This
    catastrophically destroyed predictions (BPC=45.86 at full v1). V2 uses
    SOFT-K-WTA: keep top K=10 absolute (1-3% cortical sparsity band per
    sparse_competitive_readout cell smoke), use softmax(beta * scores) then
    mask the rest -- the kept top-K get a temperature-scaled probability
    distribution rather than a uniform-over-top-K.

  Bug 3 (KP rejects 100%): v1's kinetic proofreading 2-step agreement gate
    with sigma=0.02, tau=0.20 produced agreement_frac=0.0 at smoke (too
    strict given low-confidence smoke predictions). V2 (a) gates KP off
    at smoke (N_TRAIN < 10k) where confidence is too low for cosine
    agreement, AND (b) uses top-3 overlap (not exact cosine) for full-scale
    so that "agreement" means the two noisy predictions share their top-3
    candidates -- a less brittle proxy for mechanism agreement.

  Bug 4 (verdict false-positive on lambda=0): v1 reported bpc_ok=True when
    bpc_best < 7.500 WITHOUT checking best_lambda > 0. The MIDDLE_BAND
    case (bpc=5.291, all lambdas=0.0) was pure-unigram fallback masquerading
    as a substrate result. V2 requires best_lambda > 0 in the decisive arm
    for bpc_ok=True; otherwise emit SUBSTRATE_SIGNAL_TOO_WEAK_TO_LIFT_UNIGRAM
    verdict. Also surfaces bpc_raw (substrate-only, lambda=1.0) prominently
    in verdict_msg for transparency.

  Plus PC primitive non-degeneracy sanity test + K-WTA top-K shape test +
  KP sigma=0 deterministic-agreement test in --self-test. All must pass.
...
ANCHOR_NAME = "substrate_brain_full_compose_LM_v2"
...
# V2 Bug 2 fix: was SPARSE_COMPETITIVE_K_FRAC=0.10 (=400/4000); HARD top-K mask
# destroyed predictions (BPC=45.86 in v1 full). V2: SOFT K-WTA at K=10 absolute
# (1-3% cortical sparsity band per sparse_competitive_readout_lm_v1 smoke).
SPARSE_COMPETITIVE_K_ABS = 10         # Tonegawa: keep top 10 of V (cortical-sparsity 1-3%)
SPARSE_COMPETITIVE_BETA = 8.0         # softmax inverse-temperature on top-K scores
# Legacy alias kept for downstream metric reporting + log compatibility
SPARSE_COMPETITIVE_K_FRAC = float(SPARSE_COMPETITIVE_K_ABS) / float(VOCAB_CAP)
...
# V2 Bug 3 fix: kinetic-proofreading was rejecting 100% at smoke. V2: top-3 overlap
# replaces exact cosine agreement; KP disabled at smoke (N_TRAIN < 10k).
KP_TOPK_OVERLAP = 3            # KP "agreement" = >= overlap of top-K predictions
KP_OVERLAP_MIN = 2             # min top-K overlap to count as agreement
KP_NOISE_SIGMA = 0.02          # mild noise for 2-step agreement
KP_AGREEMENT_TAU = 0.20        # legacy (cosine; retained for atom diagnostics + selftest)
KP_DISABLE_BELOW_NTRAIN = 10_000   # disable KP gate at smoke (low-confidence predictions)
...
CONFIG_VERSION = (
    "substrate_brain_full_compose_LM_v2; "
    "bugfix=PC_gauss_init+softKWTA_K10+KP_top3_overlap_smokeoff+verdict_lambda_gate; "
...
def build_pc_stack_gpu(src_keys: torch.Tensor, tgt_vecs: torch.Tensor,
    ...
    # V2 Bug 1 defensive init: variance-scaled Gaussian rather than zeros so
    # that even before Hebbian accumulation, distinct inputs map to distinct
    # outputs. This cell's PC is dot-product based (not sign-based) so
    # zeros is not catastrophic, but the symmetric init with the PC hierarchy
    # cell (which IS sign-based) is consistent + defensive.
...
def sparse_competitive_logits(logits: torch.Tensor, k_abs: int,
                                beta: float) -> torch.Tensor:
    """V2 SOFT K-WTA (Bug 2 fix): keep top K=k_abs absolute, scale by inverse-
    temperature beta, mask rest to -1e9. The top-K still get a temperature-
    scaled probability distribution after the downstream softmax (vs v1's
    HARD-K-WTA which yielded uniform-over-top-K after softmax).

    Cortical sparsity rationale: K=10 corresponds to 1-3% sparse-firing pyramidal
    neuron activation per CREB excitability-trace pattern (Tonegawa 2015).
    """
    n, V = logits.shape
    k = max(1, min(int(k_abs), V))
    topk_vals, topk_idx = torch.topk(logits, k=k, dim=1)
    out = torch.full_like(logits, -1e9)
    out.scatter_(1, topk_idx, topk_vals * beta)
    return out
...
def kinetic_proofreading_logits(pred: torch.Tensor, E: torch.Tensor,
                                  ...
                                  topk_overlap: int = 3,
                                  overlap_min: int = 2,
                                  use_topk_overlap: bool = True) -> Tuple[torch.Tensor, torch.Tensor]:
    """V2 (Bug 3 fix) Two-step agreement: returns (logits, agreement_mask [bool]).

    use_topk_overlap=True (V2 default): "agreement" = the two noisy predictions
    share at least overlap_min of their top-K (default top-3) candidates over
    the vocab. Less brittle than cosine: doesn't require near-exact prediction
    match, just that they agree on the leading candidates.

    use_topk_overlap=False: legacy cosine gate (cos(p1, p2) >= 1 - tau).
    Retained for selftest sigma=0 determinism check (cosine path is the
    deterministic-at-sigma=0 version).

    Disagreement positions get 0.0 logits across V => softmax becomes uniform,
    lambda-interp pulls them fully to unigram (smooth fallback).
    """
    n, dim = pred.shape
    V = E.shape[0]
    ...
    for b in range(0, n, recall_batch):
        ...
        if use_topk_overlap:
            # V2: top-K overlap on logits vs E
            logits1 = p1 @ E.T
            logits2 = p2 @ E.T
            k_use = min(int(topk_overlap), V)
            top1 = torch.topk(logits1, k=k_use, dim=1).indices  # [b, k]
            top2 = torch.topk(logits2, k=k_use, dim=1).indices
            # Per-row count of shared top-K indices
            overlap = torch.zeros(end - b, dtype=torch.long, device=pred.device)
            for j in range(k_use):
                # check whether top1[:, j] is in top2 row
                eq = (top2 == top1[:, j:j + 1]).any(dim=1)
                overlap = overlap + eq.long()
            agree_b = overlap >= int(overlap_min)
        else:
            # Legacy cosine path (kept for deterministic-sigma=0 selftest)
            cos = (p1 * p2).sum(dim=1)
            agree_b = cos >= (1.0 - tau)
...
def compute_arm_logits(arm_label: str, E: torch.Tensor, idx_train: np.ndarray,
    ...
    # V2 Bug 3 fix: gate KP off at smoke (low-confidence predictions => 100% reject).
    use_kp = (arm_label == "ARM_BRAIN_FULL_COMPOSE"
              and idx_train.shape[0] >= KP_DISABLE_BELOW_NTRAIN)
    ...
    if use_kp:
        ...
        # V2: top-K-overlap agreement (Bug 3 fix); legacy cosine retained
        # behind use_topk_overlap=False for the deterministic-sigma=0 selftest.
        logits, agree_mask = kinetic_proofreading_logits(
            pred_held, E_used, KP_NOISE_SIGMA, KP_AGREEMENT_TAU,
            RECALL_BATCH, gen,
            topk_overlap=KP_TOPK_OVERLAP,
            overlap_min=KP_OVERLAP_MIN,
            use_topk_overlap=True,
        )
    ...
    # Sparse competitive (Tonegawa) on top -- V2 SOFT K-WTA at K=10 absolute
    if use_sparse_comp:
        logits = sparse_competitive_logits(logits, SPARSE_COMPETITIVE_K_ABS,
                                              SPARSE_COMPETITIVE_BETA)
...
def compute_verdict(units):
    ...
    # V2 Bug 4 fix: surface bpc_raw + best_lambda for the decisive arm.
    # bpc_raw is the substrate-only BPC (lambda=1.0, no unigram fallback);
    # best_lambda is the optimal mix point. If best_lambda == 0, the cell
    # has collapsed to pure unigram fallback (substrate signal too weak to
    # lift). Verdict must distinguish "substrate beats unigram" from
    # "unigram alone wins; substrate ignored".
    dec_bpc_raw = dec.get("bpc_raw_mean", float("inf"))
    dec_best_lambda = dec.get("best_lambda_mean", float("nan"))
    base_best_lambda = base.get("best_lambda_mean", float("nan"))
    ...
    for a in non_uni:
        b = by_arm_agg[a]
        # V2: surface bpc_raw (substrate-only at lam=1) alongside bpc_best (interp-optimized)
        parts.append("%s=bpc%.3f(raw%.3f,lam%.2f)" % (
            a, b["bpc_best_mean"], b.get("bpc_raw_mean", float("nan")), b["best_lambda_mean"]))
    ...
    # HARD_PASS
    # V2 Bug 4 fix: require best_lambda > 0 for bpc_ok. If decisive arm collapsed
    # to lambda=0 (pure unigram fallback), the BPC achievement is NOT from
    # substrate -- it's the unigram floor masquerading as a substrate result.
    lambda_alive = math.isfinite(dec_best_lambda) and dec_best_lambda > 0.0
    bpc_ok = math.isfinite(dec_bpc) and dec_bpc < HP_BPC_BAR and lambda_alive
    ...
    detail["lambda_alive"] = bool(lambda_alive)
    detail["decisive_bpc_raw"] = dec_bpc_raw
    detail["decisive_best_lambda"] = dec_best_lambda
    detail["baseline_best_lambda"] = base_best_lambda

    # V2 Bug 4 fix: catch the "substrate signal too weak to lift unigram" case.
    # If the decisive arm's best_lambda is 0 (chose pure unigram), that is NOT
    # a substrate success regardless of bpc value.
    if math.isfinite(dec_best_lambda) and dec_best_lambda == 0.0:
        return ("SUBSTRATE_SIGNAL_TOO_WEAK_TO_LIFT_UNIGRAM",
                ("BRAIN_FULL_COMPOSE substrate-signal-too-weak-to-lift-unigram: "
                 "ARM_BRAIN_FULL_COMPOSE bpc_best=%.3f at best_lambda=0.0 "
                 "(collapsed to pure unigram fallback); substrate-only bpc_raw=%.3f. "
                 "The bpc_best value is the UNIGRAM FLOOR, not a substrate result. "
                 "Brain-architecture composition produced predictions too low-confidence "
                 "for log-linear interp to lift over the unigram bar; substrate signal "
                 "fundamentally too weak at this config. %s" % (
                     dec_bpc, dec_bpc_raw, summary)),
                detail)

    if bpc_ok and lift_ok and cv_ok and abl_ok:
        return ("HARD_PASS",
                ("BRAIN_FULL_COMPOSE HARD_PASS: %s bpc=%.3f (bpc_raw=%.3f at lambda=1.0; "
                 "best_lambda=%.2f) < %.3f bar (cv=%.3f<=%.2f); "
                 ...
                 "CHAIN-GRADE V2 closure evidence. %s" % (
                     DECISIVE_ARM, dec_bpc, dec_bpc_raw, dec_best_lambda,
                     HP_BPC_BAR, dec_cv, HP_BPC_CV_MAX,
    ...
    return ("MIDDLE_BAND",
            ("BRAIN_FULL_COMPOSE MIDDLE_BAND: composition lifts over rank-1 baseline "
             ...
             "abl_ok=%s lambda_alive=%s); decisive bpc=%.3f (bpc_raw=%.3f at lam=1; "
             "best_lambda=%.2f) baseline=%.3f lift=%.3f abl=%.3f; "
             "partial mechanism -- characterize what is still missing. %s" % (
                 bpc_ok, lift_ok, cv_ok, abl_ok, lambda_alive,
                 dec_bpc, dec_bpc_raw, dec_best_lambda,
                 base_bpc, lift_over_baseline, best_ablation_lift,
                 summary)),
            detail)
...
def _selftest():
    ...
    try:
        ...
        # T5: V2 soft K-WTA sanity (Bug 2 fix)
        # T5a: shape/keep-K invariant: top-K=2 of 20 must produce exactly 2 finite entries
        L = torch.randn(3, 20)
        Lout = sparse_competitive_logits(L, k_abs=2, beta=1.0)
        finite_per_row = (Lout > -1e8).sum(dim=1).numpy()
        assert (finite_per_row == 2).all(), "T5a sparse comp k (got %s)" % finite_per_row.tolist()
        # T5b: SOFT K-WTA -- after softmax over top-K, distribution must NOT be uniform
        # (i.e. softmax of beta * top-K vals should pick out the highest entry, not
        # spread uniformly across top-K like v1's hard mask did).
        Lsoft = sparse_competitive_logits(L, k_abs=5, beta=8.0)
        probs_soft = torch.softmax(Lsoft, dim=1)
        # Max prob in row should be significantly higher than 1/K (uniform would be 0.20)
        max_prob = probs_soft.max(dim=1).values
        assert (max_prob > 0.5).all(), (
            "T5b SOFT K-WTA degenerate: max_prob=%s should be >0.5 with beta=8 "
            "(else not really soft; uniform-over-K = hard mask = v1 bug)" % max_prob.tolist()
        )
        # T5c: large-vocab top-K=10 absolute (matches V2 SPARSE_COMPETITIVE_K_ABS)
        L_big = torch.randn(2, 4000)
        L_big_out = sparse_competitive_logits(L_big, k_abs=10, beta=8.0)
        finite_big = (L_big_out > -1e8).sum(dim=1).numpy()
        assert (finite_big == 10).all(), "T5c K=10 abs: got %s" % finite_big.tolist()
        ...
        # T10: kinetic proofreading agreement V2 (Bug 3 fix)
        # T10a: cosine path -- deterministic at sigma=0 (legacy path)
        gen = torch.Generator(device=torch.device("cpu"))
        gen.manual_seed(0)
        pred_t = _l2_normalize_t(torch.randn(5, 64))
        logits_kp, agree = kinetic_proofreading_logits(
            pred_t, E_norm, sigma=0.0, tau=0.01,
            recall_batch=5, generator=gen,
            use_topk_overlap=False,
        )
        assert agree.all(), "T10a cosine sigma=0 all agree"
        assert logits_kp.shape == (5, 8), "T10a logits shape"
        # T10b: V2 top-K overlap path -- at sigma=0 the two predictions are
        # identical so their top-K must be identical (overlap = K).
        gen2 = torch.Generator(device=torch.device("cpu"))
        gen2.manual_seed(0)
        logits_kp2, agree2 = kinetic_proofreading_logits(
            pred_t, E_norm, sigma=0.0, tau=0.01,
            recall_batch=5, generator=gen2,
            topk_overlap=3, overlap_min=2, use_topk_overlap=True,
        )
        assert agree2.all(), (
            "T10b sigma=0 top-K overlap should be K (deterministic agreement); "
            "got agree2=%s" % agree2.tolist()
        )
        # T10c: at high sigma, two random perturbations should NOT all agree on top-3
        # (proves the gate actually gates -- not vacuously all-True).
        gen3 = torch.Generator(device=torch.device("cpu"))
        gen3.manual_seed(0)
        _, agree3 = kinetic_proofreading_logits(
            pred_t, E_norm, sigma=1.0, tau=0.01,
            recall_batch=5, generator=gen3,
            topk_overlap=3, overlap_min=3, use_topk_overlap=True,
        )
        # Sigma=1 + overlap_min=K (strict) should NOT yield 100% agreement
        assert not agree3.all(), (
            "T10c sigma=1+overlap_min=K should reject some positions; "
            "got all-agree=%s -- gate vacuously true" % agree3.tolist()
        )
        ...
        # T13: verdict bands
        def _mk_unit(bpc_by_arm, lambda_override=None):
            ...
            for arm in [a for a in ARMS if a != "ARM_UNIGRAM"]:
                ...
                lam = lambda_override.get(arm, 0.5) if lambda_override else 0.5
                by_arm_local[arm] = {
                    "bpc_raw": bp + 0.1, "bpc_best": bp, "best_lambda": lam,
                    "best_dev_bpc": bp,
                    "bpc_per_lambda_dev": {}, "bpc_per_lambda_test": {},
        ...
        # T13d: V2 Bug 4 fix sanity. Decisive arm has bpc_best=5.0 (<7.5 bar) BUT
        # best_lambda=0.0 (pure unigram fallback). v1 would call this HARD_PASS-eligible.
        # v2 must return SUBSTRATE_SIGNAL_TOO_WEAK_TO_LIFT_UNIGRAM.
        u_lambda0 = _mk_unit(
            {
                "ARM_BASELINE_RANK1_HEBBIAN": 5.0,
                "ARM_PC_HIERARCHY_ONLY": 5.0,
                "ARM_PC_PLUS_SPARSE_COMPETITIVE": 5.0,
                "ARM_PC_PLUS_LOCK_IN_ATTENTION": 5.0,
                "ARM_BRAIN_FULL_COMPOSE": 5.0,
            },
            lambda_override={
                "ARM_BASELINE_RANK1_HEBBIAN": 0.0,
                "ARM_PC_HIERARCHY_ONLY": 0.0,
                "ARM_PC_PLUS_SPARSE_COMPETITIVE": 0.0,
                "ARM_PC_PLUS_LOCK_IN_ATTENTION": 0.0,
                "ARM_BRAIN_FULL_COMPOSE": 0.0,
            },
        )
        v, m, _ = compute_verdict([u_lambda0, u_lambda0, u_lambda0])
        assert v == "SUBSTRATE_SIGNAL_TOO_WEAK_TO_LIFT_UNIGRAM", (
            "T13d V2 Bug 4 fix: decisive_arm at best_lambda=0 must NOT be HARD_PASS; "
            "got verdict=%s msg=%s" % (v, m[:200])
        )

    finally:
        DEVICE = _saved_device

    # T14: V2 PC PRIMITIVE NON-DEGENERACY (Bug 1 defensive init sanity).
    # build_pc_stack_gpu must produce W matrices that map distinct inputs to
    # distinct outputs even before Hebbian accumulation kicks in. v1 zero-init
    # would have layer outputs differing only via the Hebbian Sum_t resid.T @ src_b
    # term; v2 starts each W with a small Gaussian so distinct inputs are
    # discriminated from step 0.
    _saved_device3 = DEVICE
    DEVICE = torch.device("cpu")
    try:
        dim_t14 = 64
        n_t14 = 20
        src_keys_t14 = _l2_normalize_t(torch.randn(n_t14, dim_t14))
        # Use vocab+E from earlier T8 setup (E [8,64])
        # build_pc_stack_gpu trains 1 layer on a tiny n_pairs=19, then we forward
        # the same keys back and verify outputs are not all identical.
        vocab_t14 = ["w%d" % i for i in range(8)]
        E_t14 = build_E_char_trigram_gpu(vocab_t14, dim_t14, seed=0)
        idx_t14 = torch.arange(n_t14, dtype=torch.long) % 8
        Ws_t14 = build_pc_stack_gpu(src_keys_t14, E_t14, idx_t14,
                                       ingest_chunk=4, n_layers=2)
        # T14a: at least the first layer's W must be nonzero (Bug 1 init check)
        assert float(Ws_t14[0].abs().sum()) > 0, "T14a PC W layer 0 is all-zero (init still broken)"
        # T14b: forward through layer 0 -- distinct src_keys must produce distinct preds
        pred_t14 = src_keys_t14 @ Ws_t14[0].T
        # unique rows count > 1 (proves layer 0 is not degenerate)
        # quantize to handle floating-point noise
        pred_rounded = (pred_t14 * 100).round() / 100
        unique_rows = torch.unique(pred_rounded, dim=0)
        assert unique_rows.shape[0] >= 2, (
            "T14b PC layer 0 maps ALL inputs to identical output (degenerate). "
            "unique_rows=%d" % unique_rows.shape[0]
        )
    finally:
        DEVICE = _saved_device3

    # T15: llm counter clean
    assert _LLM_CALL_COUNTER[0] == 0, "T15 llm counter"

    print("[selftest] PASS: T1 trigram + T2 E + T3 sparse-bipolar + T4 hrr-bind + "
          "T5 SOFT-KWTA (Bug2 fix) + T6 lock-in + T7 context-keys + T8 PC-stack + "
          "T9 attractor + T10 kinetic-proof-V2 (Bug3 fix) + T11 BPC uniform + T12 log-linear + "
          "T13 verdict-V2 incl SUBSTRATE_TOO_WEAK (Bug4 fix) + T14 PC-non-degen (Bug1 fix) "
          "+ T15 llm=0", flush=True)
...
def _synthesize_on_exit():
    ...
    try:
        ...
        metrics = {
            ...
            "detail": detail,
            "metrics_source": "atexit_synthesize_partial_substrate_brain_full_compose_LM_v2_bugs1234_fix",
            "per_unit": units,
...
if __name__ == "__main__":
    ...
    run_cfg = {"run_mode": RUN_MODE, "N": N_DIM,
               "schema": "substrate-brain-full-compose-LM-v2"}
    t0 = time.time()
    _T0_REF[0] = t0
    for seed in SEEDS:
        key = "s%d" % seed
    ...
    metrics = {
        ...
        "n_seeds": len(SEEDS),
        "detail": detail,
        "metrics_source": "measured_gpu_substrate_brain_full_compose_LM_v2_bugs1234_fix",
        "per_unit": units,
