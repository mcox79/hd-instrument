"""Grid-cell-style fractional-binding position atoms (BR5).

Frady-Kanerva-Sommer 2018 NeurIPS: replace random IID position atoms with
fractional powers of a single base unitary atom. For FHRR with unit-modulus
phases, this is exact: P^k means multiplying every component's phase by k.

Position 0 (most recent byte): P^1
Position 1: P^2
...
Position K-1: P^K

Properties:
- All positions stay unit-magnitude (binding still works cleanly)
- Positions have structured ordering (k+1 is one rotation step beyond k)
- bind(byte_atom, P^k) gives a position-specific phase rotation of byte_atom
- unbind by P^k recovers the byte at that position

This is the SAME math FHRR was designed for in the first place. We're just
finally using it.

Stacked with the magnitude_relu dendritic nonlinearity (current best from
BR1 sweep) for a "all good ideas combined" run at 15 epochs.

Baseline to beat: 2.4994 bpc (combined + relu, random IID positions).
"""
...
RELU_B = 0.5
USE_RELU = True  # toggle to ablate effect of relu independently
...
def make_grid_position_atoms(n: int, k: int, gen: torch.Generator) -> torch.Tensor:
    """Grid-cell-style position atoms via fractional binding.

    Generate one random unitary base atom P, then position k = P^(k+1):
      pos_atom[0] = P^1, pos_atom[1] = P^2, ..., pos_atom[K-1] = P^K
    For FHRR (unit-magnitude complex), P^k means multiplying each component's
    phase by k. This preserves unit magnitude exactly.
    """
    base = atoms.make_atom_fhrr(n, gen)  # (N,) complex64 unit-modulus
    base_phases = torch.angle(base)  # (N,) real
    pos_atoms = []
    for i in range(k):
        power = i + 1  # k=1..K for positions 0..K-1
        new_phases = power * base_phases
        atom = torch.complex(torch.cos(new_phases), torch.sin(new_phases)).to(torch.complex64)
        pos_atoms.append(atom)
    return torch.stack(pos_atoms)
...
def main() -> None:
    ...
    _say(f"        pool_size={POOL_SIZE}, alpha={ALPHA}")
    _say(f"        positions: grid-cell fractional binding P^1..P^K")
    _say(f"        dendritic NL: magnitude_relu b={RELU_B} (USE_RELU={USE_RELU})")
    _say(f"        epochs={MAX_EPOCHS}, checkpoints {EPOCH_CHECKPOINTS}")
    _say(f"\nBaselines to beat:")
    _say(f"  GPU combined linear (random pos):    2.522")
    _say(f"  GPU combined + relu (random pos):    2.4994 (current best)")
    ...
    with tracing.using(quiet):
        ...
        # Grid-cell-style position atoms instead of random IID
        pos_atoms = make_grid_position_atoms(N_SUBSTRATE, K, gen).to(DEVICE)
        ...
        _say(f"  vs combined + relu (random pos, 2.4994): delta {2.4994 - best['test_bpc']:+.4f}")
        _say(f"  vs combined linear (random pos, 2.522): delta {2.522 - best['test_bpc']:+.4f}")
        ...
        out = {
            ...
            "use_relu": USE_RELU, "relu_b": RELU_B if USE_RELU else None,
            "position_atoms": "grid_cell_fractional_binding",
            "history": history,
            "best": best,
            "headline": f"combined + relu + grid_positions @ {best['epoch']} epochs = {best['test_bpc']:.3f} bpc",
        }
        out_path = Path(__file__).resolve().parent.parent / "data" / "exp_grid_positions_charlm"
