"""
exp_stretch2_3_planning_strips_v2_substrate_native_planner_cpu_v1.py
-- classical STRIPS RESCUE: substrate-native planner + independent goal-sampling -- CPU.

v1 defects (confirmed by cell-author 2026-07-02):
  - v1 (exp_stretch2_3_planning_strips_cpu_v1.py) built FHRR `store` dict but BFS
    used raw Python `acts` sets; grep-check: cphasor+cidx+bind+bundle+cnorm+unbind
    total call count 4 (all in setup; ZERO substrate reads inside BFS). Numpy-costume.
  - v1 sampled G FROM reached state `cur` (after 2-4 random applicable actions from
    S0) -> reachable-by-construction saturation trap.

v2 RESCUE PATH A (mirror of stretch4_3 v2; non-temporal variant):
  - Substrate-native retrieval at every BFS action-schema lookup: unbind akey_a and
    slot_role from substrate, cleanup vs props codebook to recover pre/add/del sets.
    Applicability + state transition use RECOVERED sets, not stored Python sets.
  - Independent goal sampling: G is a random 1..3-subset of NPROP drawn
    INDEPENDENTLY from S0. May be unsolvable within depth budget; solvability rate
    reported honestly and substrate-vs-symbolic gap measured on the same trials.

ARMS (2):
  - ARM_SUBSTRATE_NATIVE: FHRR substrate mediates every planner action-lookup;
    pre/add/del sets RECONSTRUCTED via unbind+cleanup for all NACT actions before
    BFS begins (cached per-action across the search since schemas don't change).
  - ARM_SYMBOLIC_ORACLE: pure Python set BFS (positive control; expected ceiling).

DISCRIMINATOR:
  - HARD_PASS: substrate_plan_rate within 0.10 of symbolic_plan_rate AND symbolic
    in band [0.30, 0.85] (baseline not saturated). Substrate-native retrieval
    preserves classical STRIPS planning at real difficulty.
...
BIT-IDENTICAL SUBSTRATE-NATIVE EQUIVALENCE (META_RULE_AF nuance):
  At N=8192 with clean phasor encoding, unbind+cleanup should exactly recover the
  pre/add/del sets (SNR ~10). If substrate-BFS plans are BIT-IDENTICAL to symbolic
  AND all retrieval p/r >= 0.95, that is a POSITIVE result (substrate-native
  equivalence proven), NOT a bug. Handled explicitly in verdict().
...
CELL-TEMPLATE MANDATORY (META_RULE_AC/AF/AG/AH + scope/scale/floor):
  ...
  - grep-check: cphasor+cbind+cunbind+cidx invoked substantively inside run() at
    planner call sites (META_RULE per Skunkworks CG 2026-07-02 caught v1 numpy-costume)
...
Per-action substrate-retrieval is cached once at BFS start (NACT=16 x 3 slots x
N=8192 unbind+cleanup ~ 400k complex-ops; sub-millisecond on numpy). BFS state
expansion itself is Python-set operations (nanoseconds). Total wall estimate:
  - smoke TR=15 x (48 retrievals + ~200 BFS-states x 16 acts x set-ops) ~= 5s
  - full  TR=150 x (48 retrievals + ~500 BFS-states x 16 acts x set-ops) ~= 90s
Batching would not help retrieval (48 per trial is trivial) and cannot help BFS
(each state depends on the previous).
...
ANCHOR_NAME = "stretch2_3_planning_strips_v2_substrate_native_planner_cpu_v1"
...
# ---- substrate encoding of STRIPS actions ---------------------------------
def build_action_substrate(
    acts: List[Tuple[Set[int], Set[int], Set[int]]],
    props: np.ndarray,
    akeys: np.ndarray,
    SLOTP: np.ndarray,
    SLOTA: np.ndarray,
    ...
    Encode ALL actions into a single substrate vector via FHRR bind + bundle.
      substrate = sum_a akeys[a] * (SLOTP*sum(props[pre[a]])
                                    + SLOTA*sum(props[add[a]])
                                    + SLOTD*sum(props[del[a]]))
    """
    substrate = np.zeros(N, dtype=np.complex64)
    for a, (pre, add, dele) in enumerate(acts):
...
def substrate_bfs(
    ...
    depth_budget: int,
    tau_frac: float,
    gt_acts: List[Tuple[Set[int], Set[int], Set[int]]],
) -> Tuple[bool, Tuple[int, ...], Dict[str, float]]:
    """
    BFS but the pre/add/del sets driving applicability + transition are RECOVERED
    FROM SUBSTRATE via unbind + cleanup. Ground-truth acts passed in ONLY for
    precision instrumentation (never used in decision path).
    ...
    for a in range(acts_count):
        ...
        gt_pre, gt_add, gt_del = gt_acts[a]
...
def _selftest() -> None:
    ...
    = pre[a0] + O(sqrt(NACT*3)/sqrt(N)) noise; at N=8192 signal ~ N, noise sigma
    ~ sqrt(N * NACT * 3) ~ sqrt(8192*48) = 627; threshold 0.4*N = 3277 discriminates.
    """
    ...
    substrate = build_action_substrate(acts, props, akeys, SLOTP, SLOTA, SLOTD, N)
    tau_frac = 0.4
    n_perfect_pre = 0
    n_perfect_add = 0
    for a in range(NACT):
    ...
    print("[selftest] PASS: planning-strips-substrate-native-v1", flush=True)
...
def run() -> Dict[str, Any]:
    ...
    NPROP = 12
    NACT = 16
    tau_frac = 0.4  # THEORETICAL@analysis: 0.4*N = 3277 threshold; SNR ~13 empirical
    depth_budget = 12
    ...
    for trial_idx in range(TR):
        ...
        substrate = build_action_substrate(
            acts, props, akeys, SLOTP, SLOTA, SLOTD, N)
    ...
    # ARMS-MUST-DIFFER: hash per-trial plan tuples. Bit-identical WITH high
    # retrieval precision is substrate-native equivalence (positive result);
    # bit-identical with LOW precision means substrate short-circuited. Handled
    # in verdict() via META_RULE_AF nuance branch.
...
def verdict(r: Dict[str, Any]) -> Tuple[str, str]:
    ...
    # substrate-native equivalence (positive), NOT a bug. arms_bit_identical
    # WITH low precision would indicate substrate code path is short-circuited.
    if not r["arms_differ_verified"]:
        ...
        return ("HARD_PASS",
                ...
                f"({sym:.3f} in [0.30, 0.85]). Substrate LIBRARY of classical STRIPS action "
    ...
    if gap <= 0.10 and sub >= 0.30:
        return ("HARD_PASS",
                f"HARD_PASS: substrate-native classical STRIPS planner matches symbolic "
    ...
    if gap > 0.20:
        return ("HARD_FAIL",
                ...
                f"for classical STRIPS planning at N=8192. " + fmt())
    # 0.10 < gap <= 0.20
    return ("MIDDLE_BAND",
