"""Shared core: cross-axis M x N x K FACTORIZATION BETA=8 BRIDGING v1 test.

BRIDGING CELL vs v2 (2026-07-01, Skunkworks batch 7 META atom expansion):
  v2 (DIS_beta4) landed CG at beta=4: 3-seed CG confirmed substrate
  factorizes across (M, N, K) at beta=4 regime with M-axis range
  MEASURED 0.590/0.597/0.590 3-seed and all interaction terms below
  0.05 floor. Skunkworks flagged META synthesis 'substrate axes factorize
  across beta regime' DEFERRED pending intermediate-beta bridging cell.

  This v1 bridges beta=8 (between v2's CG at beta=4 and v1_2d_coarse
  saturation at beta=13) to test whether factorization is BETA-INVARIANT
  or BETA-SPECIFIC. HP -> factorization holds across beta production
  range -> META atom promotes to CG (v2 beta=4 CG + this beta=8 CG +
  scale-invariance predictions from Sonnet drills).

  Analytical CRLB predictions (THEORETICAL@2026-07-01 numpy formula):
    p_win = 1/(1 + M*exp(-beta*margin)), margin = 1 - noise/2 - sqrt(2*log(M)/N)
    beta=8:
      (M=1000,  N=2048, beta=8): margin=0.917, p_win=0.605  (mid)
      (M=1000,  N=8192, beta=8): margin=0.958, p_win=0.680  (mid-high)
      (M=32768, N=2048, beta=8): margin=0.898, p_win=0.039  (low)
      (M=32768, N=8192, beta=8): margin=0.948, p_win=0.057  (low)
    Softmax value-averaging typically amplifies p_win by ~2x -> predicted
    DIS arm range on M axis (at fixed N=8192, K=100): ~0.30 to ~0.90
    range = 0.60 (well above HP_DIS_MECHANISM_RANGE_FLOOR = 0.30).

  DESIGN CHOICE (2026-07-01 cell-author): keep STD_beta13 as the
  saturating control arm (NOT STD_beta8) because:
    1) At beta=8 both arms would be at similar recall regime, violating
       META_RULE_AF (arms-must-differ) by construction.
    2) STD_beta13 re-verifies v1 saturation as positive-control at same
       grid — chain-grade cell D regime replication.
...
Substrate-KB concept-query 2026-07-01 (cosine < 0.30 for direct hits):
  Genuinely novel — first cross-axis interaction test at intermediate beta.
  Adjacent: v2 CG (beta=4), v1_2d_coarse MM (beta=13). No prior beta=8
  bridging in the substrate KB.
...
Mechanism: dense-Hopfield READ-REPLACE per Cell D v2 CG regime, via
  ...
  design decision is the BETA axis becoming the arm-differentiator:
    ...
    DIS_beta8  arm: intermediate-beta bridging (predicted M-axis range 0.55-0.65)
...
HP_DIS_MECHANISM_FIRES (META_RULE_K discriminator-fires):
  DIS_beta8 arm must show recall RANGE >= 0.30 across M axis at fixed N/K.
  THEORETICAL@CRLB p_win: M=1000 -> ~0.68, M=32768 -> ~0.06 raw p_win;
  softmax-amplified predicted ~0.90 -> ~0.30 -> range ~0.60.
  Smoke gate BLOCKS_DISPATCH if range < 0.30.
...
MEMORY BUDGET (chunked_attention chunk=1024):
  ...
  16 phase points x 3 seeds cells run sequentially. Full cell wall:
    ...
    DIS_beta8  arm 8 pp * ~40s/pp GPU = 320s
...
DISCRIMINATOR-MUST-SURVIVE-SCALE:
  Method C - smoke includes FULL-config preview at DIS_beta8 arm's worst
...
META_RULE_AG (baseline-in-band):
  ...
  If STD saturates but DIS also saturates > 0.95: BLOCK_DISPATCH_META_RULE_AG
    (beta=8 too high for this M-range; needs iteration).
...
Numbers tagged (META_RULE_AC):
  ...
  HP_DIS_MECHANISM_RANGE 0.30:  HYPOTHESIZED@this cell (from THEORETICAL@CRLB
    p_win range 0.62 at beta=8 across M=1000..32768; 0.30 conservative)
  DIS beta = 8.0:  THEORETICAL@CRLB p_win formula (see docstring above);
    intermediate between v2 CG (beta=4) and v1_2d_coarse saturation (beta=13)
...
ARM_BETAS = {
    ...
    "DIS_beta8":   8.0,   # intermediate-beta bridging regime (v2 CG at 4, this at 8)
}
...
PREVIEW_CORNER_SMOKE = ("DIS_beta8", 16384, 8192, 500)
...
def _parse_point_key(pt_key: str) -> Optional[Tuple[str, int, int, int]]:
    """Parse grid key like 'DIS_beta8_M1000_N2048_K100' -> (arm, M, N, K).

    Arm name can contain '_' (e.g., 'DIS_beta8'); the last 3 tokens are
...
def compute_verdict(
    ...
    HARD_PASS (chain-grade cross-axis interaction found):
      ...
      AND HP_DIS_MECHANISM_RANGE: DIS_beta8 M-axis range >= hp_dis_range_floor
