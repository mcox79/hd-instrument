...
ANCHOR_NAME = "substrate_compose_heterogeneous_routing_v2_RESCUE"
...
HARD_PASS_CAP_BROKEN_BPC = 6.95
CHAIN_GRADE_BONUS_BPC = 6.80
...
def compute_verdict(units: List[Dict]) -> tuple:
    ...
    detail = {
        ...
        "honest_scope": (
            ...
            "vs a fair_harness Hebbian baseline at RESCUE scope "
            "(N_DIM=4096, N_TRAIN=50k text8, V=4000, word2vec sparse-bipolar f=0.05, 2 seeds). "
            "v1 TIMED OUT at 3600s (per partial_metrics_7: seed 7 alone needed 3074s at N=8192). "
            "v2_RESCUE applies disparate-fields drill: smaller units (4x matmul reduction) + "
            "D1 pre-flight roofline probe + D2 atexit partial-flush. "
            "HARD_PASS: any het-routing arm BPC <= 6.95 refutes cf-RPE +12%% cap. "
            "WHAT_THIS_DOES_NOT_SHOW: heterogeneous-routing benefit may scale WITH N "
            "(at reduced N=4096 discriminator could be weaker than full N=8192); "
            "doesn't test K>2 routing variants; modern-Hopfield cleanup not stacked here."
        ),
        "cites": [
            "notes/exp_dev_handoff_research_timeout_class_revival_2026-06-24.md (ANCHOR 2)",
            "notes/research_timeout_class_revival_disparate_fields_2026-06-24.md (TIMEOUT drill)",
            "experiments/exp_substrate_compose_heterogeneous_routing_v1.py (v1 cell)",
            "data/exp_substrate_compose_heterogeneous_routing_v1/partial_metrics_7.json (seed 7 v1 data)",
            "data/exp_fair_harness_substrate_as_lm_v1/metrics.json (sanity rail 7.3065)",
        ],
    }
    ...
    if math.isfinite(best_het_bpc) and best_het_bpc <= CHAIN_GRADE_BONUS_BPC:
        ...
        return ("HARD_PASS",
                ...
                "Heterogeneous routing decisively refutes cf-RPE cap (RESCUE confirms). %s" % (
    ...
    if math.isfinite(best_het_bpc) and best_het_bpc <= HARD_PASS_CAP_BROKEN_BPC:
        ...
        return ("HARD_PASS",
                ...
                "RESCUE recovers heterogeneous routing path. %s" % (
...
summary_str = (
    "%s | arms=%d seeds=%d N_DIM=%d N_TRAIN=%d encoder=word2vec_sparse_bipolar het_routing_v2_RESCUE" % (
