"""Continual editing at extreme over-capacity M=8N - extends zh/zi envelope.

zh held at M=2N continual, zi held at M=4N continual. zq pushes to M=8N
sequential editing to find substrate ceiling under combined stress.

Pre-reg: preregs/2026-05-21_wave14zq_continual_8N.md
"""
...
def compute_verdict(summary):
    ...
    if "kerdock" not in arms:
        return ("CONTINUAL_8N_INCONCLUSIVE", "Missing.")
    kerdock = arms["kerdock"]
    if not kerdock.get("per_edit_trajectory"):
        return ("CONTINUAL_8N_INCONCLUSIVE", "Missing traj.")
    ...
    if k_ok and not c_ok:
        return ("CONTINUAL_8N_KERDOCK_HOLDS",
                f"At M=4N, Kerdock holds 100 sequential edits: min_edited="
                f"{kerdock['min_edited_acc']:.3f}, min_kept={kerdock['min_kept_acc']:.3f}. "
                f"Continual editing survives extreme over-capacity.")
    if k_ok and c_ok:
        return ("CONTINUAL_8N_BOTH_HOLD",
                f"Both arms hold at M=4N with 100 edits. Correlated control didn't fail.")
    return (f"CONTINUAL_8N_KERDOCK_FAILS_AT_{k_fail}",
...
def self_test_verdict():
    ...
    cases = [
        ({"by_arm": {"kerdock": mk(0.98), "correlated": mk(0.5, fail_e=2)}},
         "CONTINUAL_8N_KERDOCK_HOLDS"),
        ({"by_arm": {"kerdock": mk(0.98), "correlated": mk(0.98)}},
         "CONTINUAL_8N_BOTH_HOLD"),
        ({"by_arm": {"kerdock": mk(0.5, fail_e=30), "correlated": mk(0.5, fail_e=2)}},
         "CONTINUAL_8N_KERDOCK_FAILS_AT_30"),
        ({}, "CONTINUAL_8N_INCONCLUSIVE"),
    ]
    for s, exp in cases:
...
def run_experiment(smoke):
    ...
    config = {"mode": "smoke" if smoke else "full", "N": 1024 if smoke else 4096,
              "M_stored": 2048 if smoke else 32768,  # full=8N; smoke shrunk to fit 4-coset codebook
...
def run_smoke():
    out_dir = get_output_dir("wave14zq_continual_8N_smoke")
...
def run_main():
    out_dir = get_output_dir("wave14zq_continual_8N")
