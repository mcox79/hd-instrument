"""
exp_encoder_ladder_hotpot_v1 -- substrate value-add (whiten+Khop) lift vs encoder quality (is lift a NEGATIVE fn of encoder quality?) -- GPU.
...
ANCHOR_NAME = "substrate_valueadd_curve_v1"
...
def eval_encoder(name, hf, pool, qpre, dpre, data):
    ...
    raws = []
    for d in data:
        texts = [dpre + s for (_, _, s) in d["sents"]]
        raws.append((encode(texts, tok, m, pool), encode([qpre + d["q"]], tok, m, pool)[0]))
    del m
    if DEV.type == "cuda":
        torch.cuda.empty_cache()
    pool_emb = np.concatenate([r for r, _ in raws], 0); mu, Wd = whiten_fit(pool_emb)   # global production whitening
    naive2 = 0; sub2 = 0
    for (raw, qraw), d in zip(raws, data):
        sents = d["sents"]; en = unit(raw); qn = unit(qraw); o = np.argsort(en @ qn)[::-1]
        naive2 += int(len(set((sents[i][0], sents[i][1]) for i in o[:2]) & d["gold"]) >= 2)
        ew = unit((raw - mu) @ Wd); qw = unit((qraw - mu) @ Wd)
        h1 = int(np.argmax(ew @ qw)); qb = unit((qw + ew[h1])[None, :])[0]; s2 = ew @ qb; s2[h1] = -1e9; h2i = int(np.argmax(s2))
        sub2 += int(len(set([(sents[h1][0], sents[h1][1]), (sents[h2i][0], sents[h2i][1])]) & d["gold"]) >= 2)
    n = len(data); return {"naive": naive2 / n, "substrate": sub2 / n, "lift": (sub2 - naive2) / n}
...
def run() -> Dict:
    ...
    for (name, hf, pool, qpre, dpre) in ENCODERS:
        try:
            ...
            print("  %-10s naive=%.3f substrate=%.3f lift=%+.3f" % (name, res["naive"], res["substrate"], res["lift"]), flush=True)
        except Exception as e:
            print("  %-10s ERROR %s" % (name, str(e)[:80]), flush=True)
    return {"by": by, "n": len(data)}


def verdict(r) -> Tuple[str, str]:
    lifts = {k: round(v["lift"], 3) for k, v in r["by"].items()}
    summary = "substrate lift by encoder: %s (n=%d)" % (lifts, r["n"])
    vals = list(r["by"].values())
    decreasing = all(vals[i]["lift"] >= vals[i + 1]["lift"] - 0.03 for i in range(len(vals) - 1)) if len(vals) > 1 else False
    if decreasing and vals and vals[0]["lift"] > 0 and vals[-1]["lift"] <= 0.02:
        return ("HARD_PASS", "HARD_PASS: clear curve -- substrate lift DECREASES with encoder quality (helps weak encoders, ~0 on strong); knee identified. Tells us which encoder regimes need the substrate's retrieval value-add. " + summary)
    return ("MIDDLE_BAND", "MIDDLE_BAND: curve mapped but not cleanly monotone-decreasing. " + summary)
