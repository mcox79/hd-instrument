"""Pred-4 (1-RSB diagnostic): Hysteresis under capacity sweep. [v2 -- TypeError fix]

v1 INSTRUMENTATION_FAIL root cause: base.evaluate_bpc called with 6 positional args
(W, byte_atoms, pos_atoms, test_idx, test_tgt, device) but the function signature
requires 10 args (W, pool_vecs, pool_labels, pool_used, byte_atoms, pos_atoms,
eval_bytes, eval_targets, batch_size, device). Fixed: pool_D_v, pool_D_l, pool_D_u,
batch_size now correctly threaded through run_4stage_get_retA.
...
Pre-reg file: preregs/2026-05-24_wave14_1rsb_hysteresis_v1.md (same -- design unchanged)
"""
...
def run_4stage_get_retA(seed, n_bytes, config, device):
    ...
    # v2 fix: evaluate_bpc requires 10 args:
    #   (W, pool_vecs, pool_labels, pool_used, byte_atoms, pos_atoms,
    #    eval_bytes, eval_targets, batch_size, device)
    # v1 bug: was called with only 6 args, skipping the pool + batch_size.
    retA = base.evaluate_bpc(W_ABCD, pool_D_v, pool_D_l, pool_D_u,
