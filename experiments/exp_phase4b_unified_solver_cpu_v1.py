...
ANCHOR_NAME = "phase4b_unified_solver_cpu_v1"
...
def _selftest():
    ...
    print("[selftest] PASS: phase4b-unified-solver", flush=True)
_selftest()
if _ARGS.self_test:
    sys.exit(0)
def _load_all():
    ...
    return out
def run() -> Dict:
    rng = np.random.default_rng(int(os.environ.get("HDLAB_SEED", "1014")))
    try:
        data = _load_all()
    ...
    for nm in data:
        ...
        for txt, ans in tr:
            ...
            if k == 2:
                g = _gold1(txt, ans)
                if g: X1.append((_feats(txt), g))
            elif k >= 3:
                g = _gold2(txt, ans)
                if g: X2.append((_feats(txt), g))
    if not X1: return {"error": "no_train_labels", "macro_acc": 0.0}
    EP = 10 if not SMOKE else 4
    avg1 = _train_perceptron(X1, OP1N, rng, EP)
    avg2 = _train_perceptron(X2, PAIRS, rng, EP) if X2 else None
    per = {}
    for nm in data:
        te = data[nm].get("test", [])
        if SMOKE: te = te[:120]
        ...
        for txt, ans in te:
            ...
            if k == 2:
                p = _predict(avg1, OP1N, feats); r = _ev1(p, ns[0], ns[1])
            elif k >= 3 and avg2 is not None:
                p = _predict(avg2, PAIRS, feats); r = _ev2(ns[0], ns[1], ns[2], p[0], p[1])
            else:
                r = None
            if r is not None and Fraction(r).limit_denominator(10**6) == ans: cor += 1
        per[nm] = round(cor / len(te), 3)
    macro = sum(per.values()) / len(per) if per else 0.0
    print("  PHASE4B-UNIFIED: macro-avg=%.3f | per-benchmark=%s | train(1-op=%d, 2-op=%d)" %
          (macro, per, len(X1), len(X2)), flush=True)
    return {"macro_acc": round(macro, 3), "per_benchmark": per, "n_benchmarks": len(per), "n_1op": len(X1), "n_2op": len(X2)}
def verdict(r) -> Tuple[str, str]:
    if r.get("error"): return ("UNKNOWN", "UNKNOWN: " + r["error"])
    m = r["macro_acc"]; s = "macro-avg=%.3f per=%s (1-op-train=%d, 2-op-train=%d)" % (m, r["per_benchmark"], r["n_1op"], r["n_2op"])
    if m >= 0.45:
        return ("HARD_PASS", "HARD_PASS: unified arity-routed substrate solver macro>=0.45 -- single-op + multi-step composition in ONE solver, auto-routed by arity, no LLM. The culmination substrate-native multi-benchmark math-word-problem solver. " + s)
    if m >= 0.36:
