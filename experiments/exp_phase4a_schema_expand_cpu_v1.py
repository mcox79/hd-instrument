...
ANCHOR_NAME = "phase4a_schema_expand_cpu_v1"
...
SCHEMA_KW = {
    ...
    "linear_eq": ["solve", "equation", "value", "find", "variable", "equals", "x"],
    "average": ["average", "mean", "median"],
    "area_rect": ["area", "rectangle", "length", "width"],
    "perimeter": ["perimeter", "around", "fence", "border"],
    "total_cost": ["cost", "price", "each", "buy", "purchase", "dollars", "costs"],
    "unit_price": ["per", "rate", "unit", "price"],
    "ratio_proportion": ["ratio", "proportion", "to", "for", "every"],
    "fraction_of": ["fraction", "half", "third", "quarter", "of"],
...
def _selftest():
    ...
    print("[selftest] PASS: phase4a-schema-expand", flush=True)
_selftest()
if _ARGS.self_test:
    sys.exit(0)
...
def run() -> Dict:
    g = np.random.default_rng(int(os.environ.get("HDLAB_SEED", "1005")))
    try:
    ...
    def solve(schema, q):
        ...
        if schema == "average" and nums:
            return sum(nums) / len(nums)
        if schema == "area_rect":
            l = _anchored(q, ["length", "long"]); w = _anchored(q, ["width", "wide"])
            if l is not None and w is not None: return l * w
            if len(nums) >= 2: return nums[0] * nums[1]
        if schema == "perimeter" and len(nums) >= 2:
            return 2 * (nums[0] + nums[1])
        if schema == "total_cost":
            pr = _anchored(q, ["dollar", "cost", "price", "each"]); qt = _anchored(q, ["buy", "purchase", "items", "number"])
            if pr is not None and qt is not None: return pr * qt
            if len(nums) >= 2: return nums[0] * nums[1]
        if schema == "unit_price" and len(nums) >= 2 and nums[1] != 0:
            return nums[0] / nums[1]
        if schema == "ratio_proportion" and len(nums) >= 3 and nums[1] != 0:
            return nums[0] * nums[2] / nums[1]
        if schema == "fraction_of":
            fr = re.search(r"\d?frac\{(\d+)\}\{(\d+)\}", q)
            if fr and nums: return Fraction(int(fr.group(1)), int(fr.group(2))) * nums[-1]
