"""CONTINUOUS EMBEDDING STORAGE via SimHash Projection at N=16384.
...
PRE-REGISTERED BANDS (per routing note 2026-05-31):
...
OOM CHECK:
  ...
  Peak GPU: ~1.8 GB (W + W_proj + query batch). Well under 6 GB headroom.
  Peak CPU RAM: ~750 MB. Fine.

TIMEOUT ESTIMATE:
  Per seed: W build (10K rank-1 updates at N=16384): ~5-10s GPU.
  FAISS build: <1s. Retrieval (10K queries): ~5s.
  Audit (10K unbindings): ~5s. Edit arm (100 edits + eval): ~5s.
  Delete arm (100 deletes + eval): ~3s. Total per seed: ~25-30s.
  3 seeds: ~90s. Safety: ceil(1.5 * 90 * 1.0 * 1) = 135s.
...
Pre-reg: preregs/2026-05-31_continuous_embedding_storage_substrate_v1_n16384.md
HDLAB_EXP_NAME: cont_emb_v1
"""
...
_ck_spec = importlib.util.spec_from_file_location("_ck_cesv1", _ck_path)
...
def make_synthetic_embeddings(
    ...
    # QR of (d, rank) gives Q shape (d, d) if d < rank, else Q shape (d, rank).
    # Ensure rank <= d always; clip if needed.
    rank = min(rank, d)
    U_raw = rng.standard_normal((d, rank)).astype(np.float32)
    U, _ = np.linalg.qr(U_raw)  # Q: (d, min(d, rank)); already rank-sized
...
def simhash_project(embeddings: torch.Tensor, W_proj: torch.Tensor) -> torch.Tensor:
    ...
    """
    # logits: (n, N) = embeddings @ W_proj^T
...
def query_substrate(
    ...
    Returns (k,) indices of top-1 retrieval for recall@1.
    For recall@K: returns (k, K) top-K indices.
    """
...
def compute_edit_isolation(
    ...
    # Mean cosine similarity to any edited entry
    # (n_ne, N_EDIT) -> max cosine to nearest edited entry
    ...
    for idx in edit_indices:
        ...
        # New value: random fresh BSC code
    ...
    # Evaluate recall@10 on non-edit queries before and after
    # Use subset for efficiency
    def eval_recall(W_use: torch.Tensor, mask: np.ndarray) -> float:
        indices_in_mask = non_edit_indices[mask]
        if len(indices_in_mask) == 0:
            return -1.0
        K_eval = min(10, len(indices_in_mask))
        q_bp = keys_bp[indices_in_mask].to(device)
        r = (W_use @ q_bp.T).T   # (n, N)
        v_true = vals_bp[indices_in_mask].to(device)
        r_norm = r / r.norm(dim=-1, keepdim=True).clamp(min=1e-8)
        # Compare to ALL vals
        v_all = vals_bp.to(device)
        v_all_norm = v_all / v_all.norm(dim=-1, keepdim=True).clamp(min=1e-8)
        sims = r_norm @ v_all_norm.T   # (n, corpus)
        _, topk = sims.topk(K_eval, dim=-1)
        # True labels: indices_in_mask themselves
        hits = 0
        topk_np = topk.cpu().numpy()
        for i, true_idx in enumerate(indices_in_mask):
            if true_idx in topk_np[i]:
                hits += 1
        return hits / len(indices_in_mask)

    recall_before_dissim  = eval_recall(W_orig,  dissim_mask)
    recall_after_dissim   = eval_recall(W_edit,  dissim_mask)
    recall_before_neighbor = eval_recall(W_orig,  neighbor_mask)
    recall_after_neighbor  = eval_recall(W_edit,  neighbor_mask)
...
def compute_deletion_cert(
    ...
    # Verify deletion: for each deleted key, check response is below threshold
    # Threshold: mean response norm for non-deleted entries (establishes baseline)
    # A deleted entry's response should drop significantly
    # Use inner product with own value as the test: should drop toward 0
    cert_verifications = 0
    false_positives = 0

    # Calibrate threshold from NON-deleted entries
    if len(non_del_indices) > 0:
        # Compute mean <W_del @ k, v> for non-deleted entries (should be ~1)
        ...
        for idx in nd_sample:
            ...
            r = (W_del @ k_q.T).T   # (1, N)
            score = (r * v_q).sum(dim=-1).item() / N  # normalised inner product
            nd_scores.append(score)
        threshold_score = float(np.mean(nd_scores)) * 0.5  # half of baseline
    else:
        threshold_score = 0.1
...
def _instrumentation_selftest() -> None:
    ...
    # Query substrate: response for first entry should have positive inner product
    # with its stored value (W @ k_0 should correlate with v_0)
    q = keys_t[0:1]  # (1, N)
    r = (W @ q.T).T  # (1, N)
    cos = ((r / r.norm()) * (vals_t[0] / vals_t[0].norm())).sum().item()
    assert cos > 0.1, f"substrate query cosine too low: {cos:.4f} (SNR should be visible at n=50, N=256)"

    # 4. FAISS index (skipped if faiss not installed; required on remote runner)
    ...
    # 5. Audit fraction is computable and non-trivially positive
    # Self-test uses same codes for keys and vals (simple, degenerate case).
    # At N=256, M=50: SNR = sqrt(256/50) = 2.26 -> P(correct) ~ 0.988.
    # Even with SimHash projection noise, NN recovery should be > 0.5.
    frac = compute_audit_pass_frac(W, keys_t, vals_t, N_t, device)
    assert 0.0 <= frac <= 1.0, f"audit frac out of range: {frac}"
    assert frac > 0.5, f"audit NN recovery too low at smoke scale: {frac:.4f} (expected >0.5; instrumentation bug)"

    # 6. Deletion cert returns valid dict
    ...
    # 7. Edit isolation returns valid dict
    ...
    print("[selftest] PASS: all assertions passed at N=256 smoke scale.", flush=True)
...
def run_one_seed(
    ...
    t0 = time.time()
    embs_np = make_synthetic_embeddings(corpus_size, D_EMBED, seed)
    # Split: first half = keys, second half = values (simulates key-value store)
    # In a real store, keys and values are different embeddings.
    # Here we use two independent sets.
    ...
    # Move to CPU for storage (reduce peak GPU memory)
    ...
    # Build FAISS index on KEY embeddings (retrieval: given query key, find value)
    # Ground truth: for query embs_key_np[i], true NN = index i in key space
    faiss_idx = build_faiss_index(embs_key_np.astype(np.float32))
    true_ids = np.arange(corpus_size)  # each entry is its own NN (self-retrieval test)

    # FAISS baseline recall@10: should be ~1.0 (exact since FlatIP over same set)
    # Returns -1.0 if FAISS not available (noted in metrics.json)
    ...
    # Substrate recall@10 without oversampling
    q_bp = keys_bp_cpu   # query with same keys we stored
    topk_idx = query_substrate(W, q_bp, K_recall, vals_bp_cpu, device)
    # For self-retrieval: true NN in VALUE space is index i itself
    sub_recall_no_os = substrate_recall_at_k(topk_idx, true_ids, K_recall)

    # Substrate recall@10 with 2x oversampling + re-rank
...
def main() -> None:
    ...
    # Build per-metric lists (exclude sentinels like -1.0 for FAISS-unavailable)
    metric_keys = [
    ...
    # Format verdict message (ASCII-only)
    vd = verdict_data
    verdict_msg = (
        f"continuous_embedding_storage_substrate_v1_n16384 "
    ...
    metrics = {
        "exp_name": "continuous_embedding_storage_substrate_v1_n16384",
        "N": N,
        "corpus_size": corpus_size,
        "seeds": seeds,
        "is_smoke": is_smoke,
...
if __name__ == "__main__":
    # Support --self-test flag: run only the instrumentation self-test and exit 0.
    # The gate calls: python script.py --self-test
    # The module-scope _instrumentation_selftest() already ran at import time.
    # If we get here without crashing, the self-test passed. Exit 0.
