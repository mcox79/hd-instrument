"""substrate_anchor4_encoder_family_N16384_gpu_v1 sibling seed=19.

Extends ANCHOR 4 v4 encoder-family phase diagram (which HARD_PASSed at
N=[2048,4096,8192]) to N=16384. v4 explicitly EXCLUDED N=16384 as
"saturating" -- this cell tests whether doubling the load axis
(v4 x2 = {16, 24, 32, 48} vs v4 {8, 12, 16, 24}) escapes saturation
and preserves encoder-family CG at 2x N.

Design (per DISCRIMINATOR-MUST-SURVIVE-SCALE):
  - Analytical justification: capacity scales linearly with N; load axis
    scaled 2x to preserve interference stress regime
  - Preview at full-N: smoke at N=16384 (single dim; no smaller-N to hide
    behind); META_RULE_Q strict no-saturation gate at smoke

Cardinality (per seed):
  FULL : 5 enc * 3 decay * 4 load * 1 N_DIM=16384 = 60 phase points
  SMOKE: 5 enc * 2 decay * 3 load * 1 N_DIM=16384 = 30 phase points

Sibling triplet: seed_7, seed_13, seed_19 (chunked-per-seed per USER 2026-06-28).

PROT-018: anchor has _N16384 suffix locked to N_DIM axis; NOT a "size"
          suffix -- single-N sweep by design.
...
CARDINALITY_OK_FULL: 60 phase points per seed
CARDINALITY_OK_SMOKE: 30 phase points per seed
...
SEED = 19
ANCHOR_NAME = f"substrate_anchor4_encoder_family_N16384_gpu_v1_seed_{SEED}"
...
CONFIG_VERSION = (
    ...
    f"N_DIM_sweep_full=[16384],N_DIM_sweep_smoke=[16384],"
    f"R_BUCKETS=128,n_atoms=1500,n_days=365,"
    f"decay_full=[30,60,180],load_full=[16.0,24.0,32.0,48.0],"
    f"decay_smoke=[30,180],load_smoke=[16.0,32.0,48.0],"
    f"bundled_memory=M_atoms_per_chunk,"
    f"NOISE_SIGMA=0.1,SEED={SEED},mode={RUN_MODE},"
    f"arms=[TIME_DECAY,RANDOM],"
    f"expected_n_full=60,expected_n_smoke=30,"
    f"discriminator=per_encoder_pareto_dominance_v1_N16384_AY_preflight_gate,"
    f"HP_min_pairs_differ=5_of_10,HP_readout_floor=0.30,"
    f"META_AY_HARD_FAIL_FRAC=0.50,META_AY_MM_DEMOTE_FRAC=0.10,"
    f"discriminator_scale_survival=v4_load_axis_x2_at_2x_N,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v1_N16384_encoder_family_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v1_N16384_encoder_family_import_crash",
        }
        tmp = out_dir / "metrics.json.tmp"
        final = out_dir / "metrics.json"
...
def main() -> int:
    ...
    from experiments._substrate_anchor4_encoder_family_N16384_gpu_v1_core import (
    ...
    final["_hardening_marker"] = "v1_N16384_encoder_family_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = (
        "synthetic_substrate_5_encoder_family_anchor4_timedecay_v1_N16384_load_axis_x2"
    )
