"""
pp48_nkt_depth_23_v1_n4096 -- PP-48 NKT negative-knowledge tree at depth-23 at N=4096.

Extends depth-21 to depth-23. Ceiling chase for NKT depth.
  depth-21 HARD_PASS last cycle (pos=nkt=1.0 at N=4096, 5-seed unanimous).
  Total binary tree at depth-23: 2^23 - 1 = 8388607 nodes. Sampled-leaf design.
  BRANCH=2, sample K_FORBIDDEN=100 leaves (unchanged from prior depths).
  alpha_total = (10 pos + 100 forbidden) / 4096 = 0.027. Sub-capacity.

  Test: does substrate still repel depth-23 leaves with anti-Hebbian inhibition?
  NKT property depends on binding depth, not on explicit tree enumeration.
  Sampled-leaf design: capacity fixed regardless of NKT_DEPTH; bands stable.

PRE-REGISTERED BANDS (ceiling push from depth-21):
  ...
  Calibration: depth-13, 17, 19, 21 all HARD_PASS; bands unchanged at depth-23.
    Sampled-leaf design makes capacity fixed; mechanism is algebraic at any depth.

FORMULA SELF-TESTS:
  1. BRANCH=2, depth=23 total tree: sum(2^d for d in range(23)) = 2^23 - 1 = 8388607.
     [INPUT: B=2, D=23] [EXPECTED: total = 8388607]
...
ANCHOR_NAME = "pp48_nkt_depth_23_v1_n4096"
...
NKT_DEPTH = 23
BRANCH = 2
TOTAL_NKT_FULL = (BRANCH ** NKT_DEPTH - 1) // (BRANCH - 1)  # 2^23 - 1 = 8388607
assert TOTAL_NKT_FULL == 8388607, f"TOTAL_NKT formula: got {TOTAL_NKT_FULL}, expected 8388607"
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    if hp1 and hp2:
        return ("HARD_PASS", f"HARD_PASS: both HP at NKT depth-23 N=4096. {summary}")
    if hp1 or hp2:
