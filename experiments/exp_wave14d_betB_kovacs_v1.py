...
Pre-reg: preregs/2026-05-21_wave14d_betB_kovacs_v1.md
"""
...
REPLAY_FRAC = 0.50  # v9 maximum replay; compound with EMA
...
def run_one_seed(seed, config, device):
    ...
    # Kovacs probe: re-present Phase A as Phase D (A'') to test for non-monotone return
    W_ABCD, _, _, _ = train_w_with_replay(
        W_ABC, pool_A_v.clone(), pool_A_l.clone(), pool_A_u,
        byte_atoms, pos_atoms, train_a_idx, train_a_tgt,
        None, None, 0, n_epochs, batch_size, device)
    W_ABCD = ema_alpha * W_ABCD + (1.0 - ema_alpha) * W_A

    # Bet B retention check on A''
    bpc_A_after_D = evaluate_bpc(W_ABCD, pool_ABC_v, pool_ABC_l, pool_ABC_u,
                                       byte_atoms, pos_atoms, test_a_idx, test_a_tgt,
                                       batch_size, device)
    # Kovacs signal: did re-exposure produce non-monotone behavior?
    # We'll measure retention_A trajectory and report.
    ...
    kovacs_delta = retention_A_after_D - retention_A  # positive = overshoot (Kovacs signal)
    return {"retention_A": retention_A, "retention_B": retention_B,
             "gain_C": gain_C, "bwt": bwt,
             "retention_A_after_D": retention_A_after_D, "kovacs_delta": kovacs_delta,
...
def run_smoke():
    out_dir = get_output_dir("wave14d_betB_kovacs_v1_smoke")
...
def run_main():
    out_dir = get_output_dir("wave14d_betB_kovacs_v1")
