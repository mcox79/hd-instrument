"""
exp_khop_bundle_noise_battery_gpu_v1 -- sparse-KEY K-hop B-sweep battery (Chain3 Drill4 anchors 1-2: sparse-intermediate + full B-sweep) -- GPU.
...
ANCHOR_NAME = "khop_sparse_bsweep_battery_gpu_v1"
N = 4096; NOISE0 = 0.08; SPARSE_FACTOR = 1.0 / np.sqrt(10.0); K_GRID = list(range(2, 61, 2)); B_GRID = [1, 10, 30, 100, 300, 1000]
_DEV = torch.device("cuda" if torch.cuda.is_available() else "cpu")
RUN_MODE = ("smoke" if "--smoke" in sys.argv else os.environ.get("HDLAB_RUN_MODE", "full")).lower()
_ap = argparse.ArgumentParser(); _ap.add_argument("--smoke", action="store_true"); _ap.add_argument("--self-test", action="store_true"); _ARGS, _ = _ap.parse_known_args()
if RUN_MODE == "smoke":
    V_C = 512; CHAINS = 100; K_GRID = list(range(2, 21, 3)); B_GRID = [1, 10, 100]
else:
    V_C = 4000; CHAINS = 400
...
def verdict(res) -> Tuple[str, str]:
    gains = [res.get("sparse_B%d" % b, 0) / max(res.get("dense_B%d" % b, 1e-9), 1e-9) for b in B_GRID if ("dense_B%d" % b) in res and res.get("dense_B%d" % b, 0) > 0]
    mean_gain = float(np.mean(gains)) if gains else 0.0
    summary = "K_max: %s | mean sparse/dense gain=%.2f (drill4 predicts ~sqrt(10)=3.16)" % ({k: v for k, v in res.items()}, mean_gain)
    if mean_gain >= 2.5:
        return ("HARD_PASS", "HARD_PASS: sparse-KEY intermediates give >=2.5x K_max over dense across the B-sweep -- v3 deep-hop viability confirmed (near sqrt(10) prediction). " + summary)
    if mean_gain >= 1.5:
        return ("MIDDLE_BAND", "MIDDLE_BAND: sparse gain 1.5-2.5x (partial; below sqrt(10) prediction). " + summary)
    return ("HARD_FAIL", "HARD_FAIL: sparse-KEY intermediates give <1.5x K_max -- no meaningful deep-hop unlock. " + summary)
