"""
pp50_kappa3_delta_alpha_n16384_v1_n16384 -- PP-50: kappa_3 delta-alpha sensitivity at N=16384.

CONTEXT (v369 refill cycle):
  PP-50 delta_alpha band: 0.80-0.92 (v363 BAND-LIFT).
  ...
  Cross-N: N=8192 v1 (v368): first N=8192 rung shipped.
  Research upgrade (v359 drill battery): NLO correction sigma_g_crit = sqrt(ln(2)) = 0.833 at alpha=0.05
  (4.6x wider than Wave-2 claim of 0.18). sigma_g=0.30 ratio=1.14 < 1.15 breakdown gate.
  This anchor closes the cross-N gap: tests the SAME v3 delta-alpha protocol at the previously
  established HP N=16384 to verify reproducibility after sigma_g envelope research upgrade.
...
MEMORY ESTIMATE (OOM pre-check):
  ...
  Peak: Xi + 4*V = 0.054 + 4*0.131 = 0.578 GB. Fits in 8 GB with 7.4 GB margin.

PRE-REGISTERED BANDS (PP-50 kappa3 N=16384 delta-alpha v1, cross-N gap closure):
  Prior empirical anchor: N=16384 v3 (v345) sigma_sep(d=0.04)=642.
  HP thresholds: 50% of prior anchor (conservative cross-N, cross-protocol margin).
...
FORMULA SELF-TESTS (PROT-022):
  ...
  2. N^(2/3) scaling from N=8192 to N=16384: sigma_sep_16K = sigma_sep_8K * (16384/8192)^(2/3).
     [EXPECTED: ratio ~1.587]
  3. M_base = int(0.05 * 16384) = 819.
     [EXPECTED: M_base = 819]
  4. Hutchinson kappa_3 on tiny N non-NaN.
  5. GPU memory > 100 MB after Xi creation.
  6. Xi VRAM at N=16384: 819 * 16384 * 4 < 1e8 (< 100 MB). Confirms fine.
     [EXPECTED: Xi_bytes < 2e8]
...
TIMEOUT ESTIMATE: v1 N=8192 estimated ~22.5s (5 seeds). N=16384 scaling: N^2*n_probes.
  N=16384 / N=8192 = 2x, so N^2 = 4x. n_probes same (2000). Seeds same (5).
  Estimated: 22.5 * 4 * 1.0 = 90s.
  ceil(1.5 * 90) = 135 -> 300s (floor).
"""
import sys
...
ANCHOR_NAME = "pp50_kappa3_delta_alpha_n16384_v1_n16384"
...
# Pre-registered thresholds (PP-50 N=16384 delta-alpha v1)
HP_DELTA_04 = 300.0
HP_DELTA_01 = 80.0
HP_DELTA_001 = 8.0
