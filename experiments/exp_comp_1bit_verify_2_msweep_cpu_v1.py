"""
exp_comp_1bit_verify_2_msweep_cpu_v1.py -- COMP-1BIT-VERIFY-2 M-SWEEP -- CPU.

ROUTING: Research 1BIT_DEPTH_VERIFICATION (COMP-1BIT-VERIFY-2 M-SWEEP); pure-FHRR (no download). Sweep M=50/200/500/1000/5000 at L=5,K=10; find max M with 1-bit zero-loss.
  Bipolar QPSK 1-bit-per-component quantization of EVERY stored vector vs float32, on the depth-retrieval task with
  hierarchical cleanup. Baseline config L=5,K=10,M=500,rho=0,N=8192; this cell sweeps M (codebook size). Reports float vs 1-bit
  recall + loss at each point and the critical break value. Verifies PP-301 (COMP-11) under production-realism.
PRE-REGISTERED: HARD-PASS zero-loss to M>=1000. MIDDLE>=100. HARD-FAIL M_crit<100.
...
ANCHOR_NAME = "comp_1bit_verify_2_msweep_cpu_v1"
