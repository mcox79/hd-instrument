"""stage1_regime_probe_6_topology_x_cleanup_non_saturated_v1 sibling seed=13.
...
F=16 added, N-sweep added).

Sweep FULL:  3 mech x 4 F x 3 M x 2 N x 3 corr = 216 + 1 SATURATION_PC = 217 pts/seed.
Sweep SMOKE: 3 mech x 2 F x 1 M x 1 N x 1 corr =   6 + 1 SATURATION_PC =   7 pts/seed.
L fixed = 2. STORAGE fixed = SHARDED.
...
CARDINALITY_OK_FULL: 217 phase points per seed
...
SEED = 13
ANCHOR_NAME = f"stage1_regime_probe_6_topology_x_cleanup_non_saturated_v1_s{SEED}"
...
CONFIG_VERSION = (
    ...
    f"expected_n_full=217,expected_n_smoke=7,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "stage1_regime_probe_6_non_saturated_v1_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: Exception) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "stage1_regime_probe_6_non_saturated_v1_import_crash",
        }
        tmp = out_dir / "metrics.json.tmp"
        final = out_dir / "metrics.json"
...
def main() -> int:
    ...
    from experiments._stage1_regime_probe_6_topology_x_cleanup_non_saturated_v1_core import (
    ...
    final["_hardening_marker"] = "stage1_regime_probe_6_non_saturated_v1_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = ("synthetic_sharded_fhrr_chain_composition_"
                                   "topology_x_cleanup_non_saturated_probe_v1")
