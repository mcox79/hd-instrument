"""
exp_t5c_pp225_pythia14b_scaletune_v1 -- Path B rescue R3: supervised projection head (bypass cross-attn) -- GPU.
...
ANCHOR_NAME = "t5c_pp225_pythia14b_scaletune_v1"; MODEL = "EleutherAI/pythia-1.4b"; ENCODER = "BAAI/bge-large-en-v1.5"
...
def _selftest():
    ...
    assert int(_n.argmax([0.1, 0.9])) == 1, "argmax"; print("[selftest] PASS: pp225-pythia14b-scaletune", flush=True)
...
def run() -> Dict:
    ...
    scale = nn.Parameter(torch.tensor(8.0, device=DEV))   # scale-tune: larger init so proj is not drowned by big-model logits
