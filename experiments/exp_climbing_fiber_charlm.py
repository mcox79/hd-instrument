"""Climbing-fiber sparse error matrix C (BR3).

Cerebellar architecture: granule layer expands input via random fixed projection
(our FHRR atoms), Purkinje cells learn a linear readout, climbing fiber delivers
sparse supervised error gated by which parallel fibers were active.

Our system has the granule-style expansion and Purkinje-style readout (W matrix
+ cleanup). Missing: the climbing-fiber discipline of SPARSE error correction
targeting only active features.

This adds a second matrix C alongside W:
  - W: standard delta-rule update (unchanged)
  - C: sparse update applied only on top-k components of ctx (by magnitude)
       Mimics climbing-fiber LTD at recently-active parallel-fiber synapses.

Readout combines both: q = W@ctx + gamma * C@ctx, then softmax cleanup.

Stacked with magnitude_relu (BR1 winner). Baseline to beat: 2.4994 bpc.
"""
...
K = 4
AROUSAL = 0.3
AROUSAL_C = 0.3  # climbing-fiber learning rate (could differ from W's)
BETA = 8.0
BATCH_SIZE = 64
POOL_SIZE = 1024
ALPHA = 0.3
...
RELU_B = 0.5
K_SPARSE = 64  # number of top-magnitude components C is updated on
GAMMA_C = 0.3  # mixing weight for C's contribution in readout
...
def _predict_W_C_batch(W, C, ctxs, byte_atoms, beta, gamma_c):
    """Combined W + climbing-fiber readout: q = W@ctx + gamma * C@ctx."""
    n = ctxs.shape[1]
    q_W = ctxs @ W.T
    q_C = ctxs @ C.T
    q = q_W + gamma_c * q_C
...
def main() -> None:
    ...
    _say(f"        Climbing fiber: K_SPARSE={K_SPARSE}, AROUSAL_C={AROUSAL_C}, GAMMA_C={GAMMA_C}")
    _say(f"        Dendritic relu: b={RELU_B}")
    _say(f"        epochs={MAX_EPOCHS}, checkpoints {EPOCH_CHECKPOINTS}")
    _say(f"\nBaselines to beat:")
    _say(f"  GPU combined + relu (W only):        2.4994 (current best)")
    _say(f"  Transformer ceiling:                 2.39")
    ...
    with tracing.using(quiet):
        ...
        C = torch.zeros((N_SUBSTRATE, N_SUBSTRATE), dtype=torch.complex64, device=DEVICE)
        ...
        for epoch in range(1, MAX_EPOCHS + 1):
            for batch_start in range(0, T_total, BATCH_SIZE):
                ...
                P_W = _predict_W_C_batch(W, C, ctxs, byte_atoms, BETA, GAMMA_C)
                targets = byte_atoms[tgt_batch]
                expected = P_W.T.to(byte_atoms.dtype) @ byte_atoms
                errors = targets - expected
                # Standard W update: dense outer product.
                dW = errors.T @ ctxs.conj() / N_SUBSTRATE
                if DECAY > 0:
                    W.mul_(1.0 - DECAY)
                W.add_(dW, alpha=AROUSAL)
                # Climbing-fiber C update: sparse on top-k components of |ctxs|.
                ctxs_mag = ctxs.abs()  # (B, N) real
                # Build a sparse-context tensor: keep only top-k components per row.
                topk_vals, topk_idx = ctxs_mag.topk(K_SPARSE, dim=1)
                mask = torch.zeros_like(ctxs_mag)
                mask.scatter_(1, topk_idx, 1.0)
                ctxs_sparse = ctxs * mask.to(ctxs.dtype)  # (B, N) with only top-k nonzero
                dC = errors.T @ ctxs_sparse.conj() / N_SUBSTRATE
                if DECAY > 0:
                    C.mul_(1.0 - DECAY)
                C.add_(dC, alpha=AROUSAL_C)
                # Pool update (first epoch only).
                if epoch == 1:
                    for b in range(B):
            ...
            if epoch in EPOCH_CHECKPOINTS:
                ...
                c_norm = float(C.abs().pow(2).sum().sqrt())
                elapsed = time.perf_counter() - t_start
                history.append({"epoch": epoch, "test_bpc": test_bpc, "argmax_accuracy": argmax_acc, "w_frobenius": w_norm, "c_frobenius": c_norm, "wall_s": elapsed})
                _say(f"  epoch={epoch}  test_bpc={test_bpc:.4f}  argmax={argmax_acc:.4f}  W={w_norm:.1f}  C={c_norm:.1f}  ({elapsed:.1f}s)")
        ...
        out = {
            "seed": SEED, "n_substrate": N_SUBSTRATE, "k": K, "arousal": AROUSAL,
            "arousal_c": AROUSAL_C, "k_sparse": K_SPARSE, "gamma_c": GAMMA_C,
            "beta": BETA, "pool_size": POOL_SIZE, "alpha": ALPHA, "decay": DECAY,
            "max_epochs": MAX_EPOCHS, "relu_b": RELU_B,
            "history": history,
            "best": best,
            "headline": f"climbing-fiber + relu + pool best test bpc = {best['test_bpc']:.3f}",
        }
        out_path = Path(__file__).resolve().parent.parent / "data" / "exp_climbing_fiber_charlm"
