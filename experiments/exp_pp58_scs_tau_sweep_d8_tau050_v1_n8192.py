"""
pp58_scs_tau_sweep_d8_tau050_v1_n8192 -- PP-58 SCS framework tau sweep at d=8, tau_target=0.30.

CONTEXT (v381 cycle 50 refill -- R5 rescue):
  tau=0.01..0.30 all HARD_FAIL: ratio=23.0->5.2 monotone decreasing.
  Extrapolation: tau~0.5-1.0 may enter [0.5,2.0] ratio range.
  The SCS formula gamma = (d + tau/d) / (1+tau) at d=8, tau=0.50.
  gamma = (8 + 0.50/8) / (1.50) = (8.0625) / 1.50 = 5.375.
  Key question: does ratio enter [0.5,2.0] at tau=0.50 (extrapolated from monotone decrease)?

DESIGN (same controlled-asymmetry approach):
  W = (1 - tau_target) * W_sym + tau_target * W_asym
  where W_sym = Xi^T Xi / N (standard Hopfield), W_asym = (W_rand - W_rand^T) / 2.

SCIENTIFIC QUESTION:
  At tau_target=0.50, does SCS ratio enter [0.5,2.0] as extrapolated from monotone decrease?
  Does controlled asymmetric W produce d >= 1.5?

PRE-REGISTERED BANDS:
  HARD-PASS (SCS formula with controlled tau at tau=0.50):
    ...
    - tau_actual in [0.30, 0.70] (within 40% of tau_target=0.50).
  MIDDLE:
    - d_estimate in [1.2, 1.5) (marginal spike) OR ratio in [0.5, 2.0).
    - tau_actual outside [0.30, 0.70] but d_estimate >= 1.2.
    - OR ratio in (2.0, 5.0): monotone trend continues but not yet in-range (R5 shows trend).
  HARD-FAIL:
    - mean d_estimate < 1.2 (no spike even with asymmetric W).
    - OR mean ratio > 5.0 (SCS formula still badly off by >5x; monotone extrapolation broken).
    - OR tau_actual < 0.20 (asymmetry injection failed at tau_target=0.50).

FORMULA SELF-TESTS (PROT-022):
  1. SCS formula at d=8, tau=0.50: gamma = (8 + 0.50/8) / (1 + 0.50).
     (8 + 0.0625) / 1.50 = 8.0625 / 1.50 = 5.3750.
...
TIMEOUT ESTIMATE: Similar to tau=0.20 test (~9000s). Use floor: 21600s.
"""
import sys
...
ANCHOR_NAME = "pp58_scs_tau_sweep_d8_tau050_v1_n8192"
...
_gamma_d8_tau050 = _scs_gamma(8.0, 0.50)
_expected_tau050 = 8.0625 / 1.50
print(f"[selftest-formula] SCS gamma(d=8,tau=0.50): {_gamma_d8_tau050:.4f} (expected {_expected_tau050:.4f})", flush=True)
assert abs(_gamma_d8_tau050 - _expected_tau050) < 0.001, f"SCS formula selftest tau=0.50: got {_gamma_d8_tau050}"
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    if mean_ratio < HF_RATIO_LOW or mean_ratio > 5.0:
        return ("HARD_FAIL",
                f"HARD_FAIL: SCS ratio={mean_ratio:.3f} outside [{HF_RATIO_LOW},5.0]. "
                f"SCS formula off by >5x (monotone extrapolation broken). {summary}")
