"""
exp_headtohead_math_vs_llm_3b_gpu_v1.py -- north-star head-to-head (MINIMAL, no numpy) -- CPU.
...
OUT = REPO / "data" / "exp_headtohead_math_vs_llm_3b_gpu_v1"
def _ans(x):
...
def main():
    ...
    json.dump({"anchor_name": "headtohead_math_vs_llm_3b_gpu_v1", "verdict": verdict, "verdict_msg": msg, "per_benchmark": rows, "llm_latency_s": lat, "wins": wins}, open(OUT / "metrics.json", "w", encoding="utf-8"))
    print("[metrics] written", flush=True)
if "--self-test" in sys.argv:
    assert _pred("answer 42") == 42; print("[selftest] PASS: headtohead-3b-gpu", flush=True); sys.exit(0)
main()
