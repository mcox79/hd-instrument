"""substrate_hierarchical_bank_v2_S32 sibling seed=7 (REVIVAL cell).

REVIVAL per Skunkworks 310e1880: v1 hierarchical_bank HARD_FAIL was due to
router SNR collapse at M=64K, S=8 (SNR ~ 1.012). v2 shifts to S=32 lifting
router SNR to 2.024 (well above 0.85 floor).

CODEBOOK-STRUCTURE phase diagram: 3 structures (flat / partition_by_source /
hierarchical_S32_2level) swept as OUTER axis over M in {4000, 16000, 64000}.
N=8192, n_super_banks=32.

Sibling pair: seed_13, seed_19. Shared core:
`experiments/_substrate_hierarchical_bank_v2_S32_core.py`.

PRE-REG: preregs/2026-07-01_substrate_hierarchical_bank_v2_S32_revival.md

CARDINALITY_OK_FULL: 9 phase points per seed (3 structures x 3 M)
CARDINALITY_OK_SMOKE: 6 corner points per seed (3 structures x 2 M; MUST include M=64000)

CPU-eligible: numpy only; no torch. Routes to remote_cpu_queue (full) or
local_cpu_queue (smoke).

PROT-018: anchor has no _n<N> suffix (multi-M config; N fixed at 8192).
META_RULE_AX/AY: distinctness across 3 codebook structures self-reported.
META_RULE_Q: routing_acc anti-saturation guard.
META_RULE_AR: hierarchical must lift above partition_by_source (ANCHOR 1 CG).
...
SEED = 19
ANCHOR_NAME = f"substrate_hierarchical_bank_v2_S32_seed_{SEED}"
...
CONFIG_VERSION = (
    ...
    f"structures=[flat,partition_by_source,hierarchical_S32_2level],"
    f"encoder=bipolar_numpy,N=8192,SEED={SEED},mode={RUN_MODE},"
    f"M_full=[4000,16000,64000],M_smoke=[4000,64000],"
    f"n_super_banks=32,CUE_ROUTE_COS=0.85,CUE_COS=0.70,"
    f"expected_n_full=9,expected_n_smoke=6,"
    f"HP_CAP_LIFT=1.20,HP_ROUTE_MIN=0.95,HP_ROUTE_MAX=0.995,"
    f"HF_ROUTE_MIN=0.85,MB_LIFT_SMOKE=1.10,"
    f"REVIVAL_from=hierarchical_bank_v1_S8_HARD_FAIL,"
    f"REVIVAL_criterion=Skunkworks_310e1880_S32_router_SNR_2.024,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v2_S32_hierarchical_bank_numpy_REVIVAL",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v2_S32_hierarchical_bank_import_crash",
        }
...
def main() -> int:
    ...
    from experiments._substrate_hierarchical_bank_v2_S32_core import (
    ...
    final["_hardening_marker"] = "v2_S32_hierarchical_bank_numpy_REVIVAL"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = "synthetic_substrate_bipolar_codebook_hierarchical_S32"
