"""
q_a3_l9_cross_layer_composition_v1_n4096 -- Q-A3: L=9 cross-layer composition at N=4096.

Extends L=8 (queue: completed) to L=9. Tests whether substrate cross-layer composition
fidelity holds at depth-9 (ceiling push).

Architecture:
  L1 (inner): N=4096, M_inner=100, p=2 Hopfield.
  L2: M_mid2=50.  L3: M_mid3=25.  L4: M_mid4=12.  L5: M_mid5=6.
  L6: M_mid6=3.   L7: M_mid7=2.   L8: M_mid8=2.
  L9: M_outer=2, Hadamard binding ctx9*L8.

GPU MEMORY:
  9 W matrices x 67 MB each = 603 MB total. Safe on 8 GB GPU.
  alpha at each level << alpha_c=0.138.

PRE-REGISTERED BANDS (Probe A, v342 routing):
  HARD-PASS: all 9 level fidelities = 1.0000 unanimous (5/5 seeds).
  MIDDLE: any L_fid in [0.85, 1.0).
  HARD-FAIL: any L_fid < 0.85 OR l9_acc < 0.5.
  Calibration: L=7 HP EXACT-1.0 unanimous; L=8 just completed; bands from routing note.

FORMULA SELF-TESTS:
  1. L=9 chain: 9-step Hadamard roundtrip recovers xi_L1.
...
PROT-021: seed checkpoints keyed with run_mode + L to avoid smoke-checkpoint contamination.
"""
import sys
...
ANCHOR_NAME = "q_a3_l9_cross_layer_composition_v1_n4096"
...
L_DEPTH = 9
HP_FIDELITY = 1.0
HP_FIDELITY_UNANI_THRESH = 0.9999  # "unanimous 1.0" means all seeds all levels >= 0.9999
MIDDLE_FIDELITY = 0.85
HF_FIDELITY = 0.85  # HARD-FAIL below 0.85
HP_LACC = 0.5
HF_LACC = 0.5
NOISE_FRAC = 0.10
...
def _selftest_l9_chain():
    """9-step Hadamard roundtrip recovers xi_L1."""
    ctxs = [torch.tensor([1.0, -1.0], dtype=torch.float32, device=DEVICE),
            ...
            torch.tensor([-1.0, 1.0], dtype=torch.float32, device=DEVICE),
            torch.tensor([1.0, 1.0], dtype=torch.float32, device=DEVICE),
            torch.tensor([-1.0, -1.0], dtype=torch.float32, device=DEVICE)]  # ctx2..ctx9
    ...
    for c in reversed(ctxs):
        xi_dec = xi_dec * c
    assert torch.allclose(xi_dec, xi_L1, atol=1e-6), f"L9 chain decode failed: {xi_dec}"


def _instrumentation_selftest():
    _selftest_l9_chain()
    n_dim = N_ACTIVE
    ms = [M_INNER, M_MID2, M_MID3, M_MID4, M_MID5, M_MID6, M_MID7, M_MID8, M_OUTER]
    ...
    del dummy
    alphas_str = " ".join(f"L{i}:{m/n_dim:.4f}" for i, m in enumerate(ms, 1))
    print(f"[selftest] PASS: L9 chain decode ok, capacity_ok, gpu_mem_ok "
          f"N={n_dim} {alphas_str}", flush=True)
...
def run_seed(seed: int, n_dim: int) -> Dict:
    ...
    Xi_ctx9 = bsc(M_OUTER, n_dim)
    Xi_outer = Xi_ctx9 * Xi_mid8[:M_OUTER]
    ...
    for q_idx in range(min(N_QUERIES, M_OUTER)):
        ...
        xi_L9_ret = hopfield_retrieve_gpu(W_outer, probe)
        ...
        level_fids["L9"].append(cosine_sim_gpu(xi_L9_ret, xi_outer_true))
        ...
        l9_acc.append(cosine_sim_gpu(xi_L1_ret, xi_L1_true))

    mean_fids = {k: float(sum(v) / len(v)) if v else 0.0 for k, v in level_fids.items()}
    mean_l9 = float(sum(l9_acc) / len(l9_acc)) if l9_acc else 0.0
    ...
    print(f"  [seed={seed}] fids={' '.join(f'{k}:{mean_fids[k]:.4f}' for k in sorted(mean_fids.keys()))} "
          f"l9_acc={mean_l9:.4f} peak_gpu={peak_mem:.3f}GB elapsed={elapsed:.2f}s", flush=True)

    result = {
        "seed": seed, "N": n_dim, "run_mode": RUN_MODE,
        "l9_accuracy": float(mean_l9),
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    l9_acc = mean_key("l9_accuracy")

    fid_list = [fid_vals[f"L{l}"] for l in range(1, L_DEPTH + 1)]
    summary = (" ".join(f"L{l}:{fid_vals[f'L{l}']:.4f}" for l in range(1, L_DEPTH + 1)) +
               f" l9_acc={l9_acc:.4f}(HP>={HP_LACC} HF<{HF_LACC}) n_seeds={len(results)}")

    if any(f < HF_FIDELITY for f in fid_list) or l9_acc < HF_LACC:
        return ("HARD_FAIL", f"HARD_FAIL: any fid<{HF_FIDELITY} or l9_acc<{HF_LACC}. {summary}")

    all_unanimous = all(f >= HP_FIDELITY_UNANI_THRESH for f in fid_list)
    if all_unanimous and l9_acc >= HP_LACC:
        return ("HARD_PASS", f"HARD_PASS: all {L_DEPTH} levels EXACT-1.0 unanimous + l9_acc HP. {summary}")
