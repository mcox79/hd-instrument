"""
exp_t5c_factkb_kblam_heldout_gpu_v1 -- Tier-5c Path B FOUNDATION: KBLaM-pattern fact-KB adapter; HELD-OUT recall -- GPU.
...
ANCHOR_NAME = "t5c_factkb_kblam_heldout_gpu_v1"; MODEL = "EleutherAI/pythia-160m"; ENCODER = "BAAI/bge-large-en-v1.5"
STEPS = 120 if "--smoke" in sys.argv else 2500; CKPT_EVERY = 200
RUN_MODE = ("smoke" if "--smoke" in sys.argv else os.environ.get("HDLAB_RUN_MODE", "full")).lower()
_ap = argparse.ArgumentParser(); _ap.add_argument("--smoke", action="store_true"); _ap.add_argument("--self-test", action="store_true"); _ARGS, _ = _ap.parse_known_args()
SMOKE = RUN_MODE == "smoke"
N_FACTS = 300 if SMOKE else 2000
...
def _selftest():
    ...
    print("[selftest] PASS: t5c-factkb-kblam-heldout", flush=True)
...
def make_facts(tok, g):
    ...
    combos = [(a, n) for a in ADJ for n in NOUN]; g.shuffle(combos); combos = combos[:N_FACTS]
    facts = []
    for (a, n) in combos:
        ans = pool[int(g.integers(0, len(pool)))]
        subj = "%s %s" % (a, n)
        prompt = "The secret code of the %s is" % subj
        enc_text = "The secret code of the %s is%s" % (subj, ans)
        facts.append({"subj": subj, "prompt": prompt, "aid": tok(ans, add_special_tokens=False)["input_ids"][0], "ans": ans, "enc_text": enc_text})
    return facts
