"""substrate_pc_hierarchy_text8_lm_v1 -- isolated multi-layer PC hierarchy as substrate-as-LM.
...
ANCHOR_NAME = "substrate_pc_hierarchy_text8_lm_v1"
...
CONFIG_VERSION = (
    "substrate_pc_hierarchy_text8_lm_v1; N_DIM=%d N_TRAIN=%d N_HELD=%d "
...
def build_pc_layers_gpu(E: torch.Tensor, idx_train_t: torch.Tensor, n_layers: int,
    ...
    device = E.device
    dim = E.shape[1]
    Ws = [torch.zeros((dim, dim), dtype=TORCH_DTYPE, device=device) for _ in range(n_layers)]
...
def _selftest():
    ...
    print("[selftest] PASS: T1 trigram + T2 safe_sign + T3 hebbian sign + T4 PC error decomp "
          ...
          "+ T10 verdict HP/HF/MID + T11 llm=0", flush=True)
...
def _synthesize_on_exit():
    ...
    try:
        ...
        metrics = {
            ...
            "detail": detail,
            "metrics_source": "atexit_synthesize_partial_substrate_pc_hierarchy_text8_lm_v1",
            "per_unit": units,
...
if __name__ == "__main__":
    ...
    run_cfg = {"run_mode": RUN_MODE, "N": N_DIM,
               "schema": "substrate-pc-hierarchy-text8-lm-v1"}
    t0 = time.time()
    _T0_REF[0] = t0
    for seed in SEEDS:
        key = "s%d" % seed
    ...
    metrics = {
        ...
        "n_seeds": len(SEEDS),
        "detail": detail,
        "metrics_source": "measured_gpu_substrate_pc_hierarchy_text8_lm_v1",
        "per_unit": units,
