...
ANCHOR_NAME = "iterative_multihop_bgelarge_v1"
...
def run() -> Dict:
    ...
    for d in data:
        ...
        hop1 = int(order[0])
        qn2 = unit(encode([Q_INSTR + sents[hop1]], tok, m))[0]                       # next query = the hop-1 fact itself
...
def verdict(r) -> Tuple[str, str]:
    ...
    if r["it_r2"] >= 0.55:
        return ("HARD_PASS", "HARD_PASS: bge-large + iteration clears recall@2>=0.55 -- multi-hop revival via encoder upgrade + iteration. " + s)
    if r["it_r2"] >= 0.50:
