"""
q_a3_l22_cross_layer_composition_v1_n8192 -- Q-A3: L=22 cross-layer composition at N=8192.

N-scale confirmation: L=22 HARD_PASS EXACT-1.0000 unanimous 5/5 at N=4096 (v354).
L=19 N=8192 HARD_PASS EXACT-1.0000 (first N-scale confirmation for PP-12; v354).
This anchor probes L=22 at N=8192 to confirm N-independence at the current ceiling L.

22 W matrices x (8192 x 8192 x 4 bytes) = 22 x 268 MB = ~5.9 GB. Fits in 8 GB GPU with margin.
Note: M schedule identical to N=4096 version (M_INNER=100, halving, floor 2). Same alpha validity.
At N=8192 alpha_c=0.138; M_INNER=100 -> alpha=0.0122 (halved vs N=4096 alpha=0.0244). All < 0.138.

PRE-REGISTERED BANDS (N-scale probe at L=22; prior L=22 N=4096 EXACT-1.0 + L=19 N=8192 EXACT-1.0):
  HARD-PASS: all 22 level fidelities = 1.0000 (>= 0.9999) unanimous (5/5 seeds)
             AND l22_acc >= 0.5.
             Confirms N-independence: PP-12 composition is N-robust at L=22.
  MIDDLE: any L_fid in [0.85, 1.0) OR graceful degradation of inner levels.
          Suggests N-sensitivity emerging at L=22.
  HARD-FAIL: any L_fid < 0.85 OR l22_acc < 0.5.
             N-dependent ceiling found at L<22 for N=8192.

FORMULA SELF-TESTS (PROT-022):
  1. L=22 chain: 22-step Hadamard roundtrip recovers xi_L1.
     [INPUT: 2-element +-1 vectors, 21 context ops] [EXPECTED: decode = xi_L1 exactly]
...
QUEUE: overnight_queue (GPU; 22 W matrices at N=8192 ~5.9 GB).
TIMEOUT ESTIMATE: L=22 N=4096 FULL elapsed=1.59s GPU 5-seed.
  N=8192: matrix scale ~(8192/4096)^2 = 4x per W; 22 x 4x steps.
  ceil(1.5 * 1.59 * 4.0 * 1.0) = ceil(9.5) = 10s -> use 300s (5-min safety floor).
  Note: memory bandwidth dominates at N=8192; actual wall likely ~5-10s.
"""
import sys
...
ANCHOR_NAME = "q_a3_l22_cross_layer_composition_v1_n8192"
...
def _selftest_l22_chain():
    """22-step Hadamard roundtrip recovers xi_L1."""
    ctxs = []
    ...
    for c in reversed(ctxs):
        xi_dec = xi_dec * c
    assert torch.allclose(xi_dec, xi_L1, atol=1e-6), f"L22 chain decode failed: {xi_dec}"
...
def _instrumentation_selftest():
    ...
    ms = [M_INNER, M_MID2, M_MID3, M_MID4, M_MID5, M_MID6, M_MID7, M_MID8,
          ...
          M_MID17, M_MID18, M_MID19, M_MID20, M_MID21, M_OUTER]
    ...
    del dummy
    print(f"[selftest] PASS: L22 chain decode ok, capacity_ok, gpu_mem_ok N={n_dim}", flush=True)
...
def run_seed(seed: int, n_dim: int) -> Dict:
    ...
    Xi_ctx22 = bsc(M_OUTER, n_dim)
    Xi_outer = Xi_ctx22 * Xi_mid21[:M_OUTER]
    ...
    for q_idx in range(min(N_QUERIES, M_OUTER)):
        ...
        xi_L22_ret = hopfield_retrieve_gpu(W_outer, probe)
        ...
        level_fids["L22"].append(cosine_sim_gpu(xi_L22_ret, xi_outer_true))
        ...
        l22_acc.append(cosine_sim_gpu(xi_L1_ret, Xi_inner[q_idx]))

    mean_fids = {k: float(sum(v) / len(v)) if v else 0.0 for k, v in level_fids.items()}
    mean_l22 = float(sum(l22_acc) / len(l22_acc)) if l22_acc else 0.0
    ...
    print(f"  [seed={seed}] fids={' '.join(f'{k}:{mean_fids[k]:.4f}' for k in sorted(mean_fids.keys()))} "
          f"l22_acc={mean_l22:.4f} peak_gpu={peak_mem:.3f}GB elapsed={elapsed:.2f}s", flush=True)

    result = {
        "seed": seed, "N": n_dim, "run_mode": RUN_MODE,
        "l22_accuracy": float(mean_l22),
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    l22_acc = mean_key("l22_accuracy")

    fid_list = [fid_vals[f"L{l}"] for l in range(1, L_DEPTH + 1)]
    summary = (" ".join(f"L{l}:{fid_vals[f'L{l}']:.4f}" for l in range(1, L_DEPTH + 1)) +
               f" l22_acc={l22_acc:.4f}(HP>={HP_LACC} HF<{HF_LACC}) n_seeds={len(results)}")

    if any(f < HF_FIDELITY for f in fid_list) or l22_acc < HF_LACC:
        return ("HARD_FAIL", f"HARD_FAIL: any fid<{HF_FIDELITY} or l22_acc<{HF_LACC}. {summary}")

    all_unanimous = all(f >= HP_FIDELITY_UNANI_THRESH for f in fid_list)
    if all_unanimous and l22_acc >= HP_LACC:
        return ("HARD_PASS", f"HARD_PASS: all {L_DEPTH} levels EXACT-1.0 unanimous + l22_acc HP. {summary}")
...
metrics = {
    ...
    "per_seed": [
         ...
         **{f"fid_l{l}": r.get(f"fid_l{l}") for l in range(1, L_DEPTH + 1)},
         "l22_accuracy": r.get("l22_accuracy"),
