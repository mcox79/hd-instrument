"""
q_b1_chain_depth_30_v1_n8192 -- Q-B1: heteroassoc chain at depth-30 at N=8192.

Extends depth-20 (just shipped this cycle) to depth-30. Tests chain degradation
envelope at depth-30. Prior: depth-20 HP'd at N=8192 (PROT-018 bound).

GPU IMPLEMENTATION:
  H matrix (N x N float32 at N=8192): 268 MB. Safe on 8 GB GPU.
  Readout snapshots at depths [1, 3, 5, 10, 15, 20, 25, 30].

PRE-REGISTERED BANDS:
  HARD-PASS: depth-5 >= 0.95 AND depth-10 >= 0.88 AND depth-20 >= 0.70 AND depth-30 >= 0.55.
  HARD-FAIL: depth-5 < 0.80 OR depth-10 < 0.65 OR depth-20 < 0.40 OR depth-30 < 0.30.
  MIDDLE: depth-30 in [0.40, 0.55) while earlier depths meet HP.
  Prior: depth-20 HARD_PASS (mean_cos_d20 >= 0.70); extrapolation at 0.995^30 ~ 0.861.
  HP at depth-30: 0.55 (conservative; 0.30 below predicted 0.86 midpoint given
  accumulated interference from 30-hop background noise).

FORMULA SELF-TESTS:
  1. depth-30 chain retrieval non-NaN at N=128.
...
ANCHOR_NAME = "q_b1_chain_depth_30_v1_n8192"
...
CHAIN_DEPTH = 30
SNAPSHOT_DEPTHS = [1, 3, 5, 10, 15, 20, 25, 30]
...
def _instrumentation_selftest():
    ...
    assert not (cos != cos), f"depth-30 retrieval is NaN at N={N_t}"
    ...
    del H
    print(f"[selftest] PASS: depth-30 chain non-NaN N={N_t}, self-cosine=1.0, "
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    if d5 < HF_D5 or d10 < HF_D10 or d20 < HF_D20 or d30 < HF_D30:
    ...
    if hp5 and hp10 and hp20 and hp30:
        return ("HARD_PASS", f"HARD_PASS: all depth thresholds met at N=8192 depth-30. {summary}")
    n_hp = sum([hp5, hp10, hp20, hp30])
    if n_hp >= 3:
        return ("MIDDLE_BAND", f"MIDDLE_BAND: {n_hp}/4 depth HP. {summary}")
    return ("MIDDLE_BAND", f"MIDDLE_BAND: {n_hp}/4 depth HP. {summary}")
