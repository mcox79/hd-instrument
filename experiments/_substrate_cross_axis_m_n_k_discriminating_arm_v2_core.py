"""Shared core: cross-axis M x N x K DISCRIMINATING ARM v2 test.

v2 REDESIGN vs v1 (2026-07-01, Skunkworks BIAS-Q demoted v1 to MM):
  v1 was 27 phase points at beta=13 fixed with all M/N/K in the
  substrate-tolerant regime; all 27 landed bit-identical recall=1.000.
  Skunkworks correctly ruled by-construction-saturation: "no cross-axis
  interaction" claim untestable when nothing crumbles.

  v2 adds a DISCRIMINATING ARM: DIS_beta4 (beta=4.0 instead of 13.0) at
  a widened M/N grid. Analytical + numerical evidence (2026-07-01 sim,
  reproducible below): at beta=4 the mechanism sits in the transition
  band; recall spans ~0.15 (large M, small N) to ~0.85 (small M, any N).
  M-N interaction TERM in that regime measured at MEASURED@0.06-0.15
  (seed 7 preliminary sim) — genuine cross-axis signal.

  Simultaneously v2 preserves the v1 SATURATED baseline as a CONTROL
  arm: STD_beta13 arm at same 2x2x2 grid confirms the v1 "all bit-identical
  1.000" pattern is REGIME-SPECIFIC (high beta), not a substrate-wide
  property. This is what promotes to CG: mechanism discriminates in one
  regime + saturates in another + interaction detectable in the
  discriminating regime.

Substrate-KB concept-query 2026-07-01 (cosine < 0.30 for direct hits):
  Genuinely novel — first cross-axis interaction test with a discriminating
  arm. Adjacent: Donoho-Tanner alpha_c=0.40 recovery threshold (cosine=0.27)
  cited in research_drill_federated_privacy_substrate_2x_2026-06-07 as
  substrate capacity cliff analogue.
...
Mechanism: dense-Hopfield READ-REPLACE per Cell D v2 CG regime, via
  ...
  design decision is the BETA axis becoming the arm-differentiator:
    STD_beta13 arm: v1 saturating regime (recall ~= 1.000 expected)
    DIS_beta4  arm: discriminating regime (recall spans 0.15 - 0.85)
...
HP_DIS_MECHANISM_FIRES (META_RULE_K discriminator-fires):
  DIS_beta4 arm must show recall RANGE >= 0.30 across M axis at fixed N/K.
  MEASURED@2026-07-01 sim: M=1000 -> 0.85, M=16384 -> 0.35 (range 0.50).
...
MEMORY BUDGET (chunked_attention chunk=1024):
  ...
  16 phase points x 3 seeds cells run sequentially. Full cell wall:
    ...
    DIS_beta4  arm 8 pp * ~40s/pp GPU = 320s
...
DISCRIMINATOR-MUST-SURVIVE-SCALE:
  Method C - smoke includes FULL-config preview at DIS_beta4 arm's worst
...
META_RULE_AG (baseline-in-band):
  STD arm hypothesized to saturate at 1.000 (baseline SATURATED).
  ...
  If STD saturates but DIS also saturates > 0.95: BLOCK_DISPATCH_META_RULE_AG
    (beta=4 too high for this M-range; needs iteration).
...
Numbers tagged (META_RULE_AC):
  ...
  HP_DIS_MECHANISM_RANGE 0.30:  HYPOTHESIZED@this cell (from MEASURED@2026-07-01
    sim range 0.50 at beta=4 across M=1000..16384; 0.30 conservative)
  DIS beta = 4.0:  MEASURED@2026-07-01 sim scan (recall at beta=4, M=8192, N=8192,
    K=30 = 0.5425; discriminating band)
...
ARM_BETAS = {
    ...
    "DIS_beta4":   4.0,   # discriminating regime (M-axis dominates recall)
}
...
PREVIEW_CORNER_SMOKE = ("DIS_beta4", 16384, 8192, 500)
...
def _parse_point_key(pt_key: str) -> Optional[Tuple[str, int, int, int]]:
    """Parse grid key like 'DIS_beta4_M1000_N2048_K100' -> (arm, M, N, K).

    Arm name can contain '_' (e.g., 'DIS_beta4'); the last 3 tokens are
...
def compute_verdict(
    ...
    HARD_PASS (chain-grade cross-axis interaction found):
      ...
      AND HP_DIS_MECHANISM_RANGE: DIS_beta4 M-axis range >= hp_dis_range_floor
