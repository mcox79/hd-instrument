...
Version: v2 (DMPK bimodality instrumentation patch per mesoscopic-transport drill 2026-05-25)
Pre-reg: preregs/2026-05-25_wave14_moe_shift_partition_v2.md
"""
...
# === DMPK bimodality instrumentation (mesoscopic-transport drill 2026-05-25) ===
def compute_dmpk_signature(Wks: list, K: int, N: int, device) -> dict:
    """SVD singular-value spectrum across K expert matrices.

    SHIFT: bimodal (peak near sigma_max + near 0). PARTITION: unimodal bulk.
    Full SVD at N=4096 on GPU: ~2s per arm per cell.
    """
    sigmas_all = []
    for Wk in Wks:
        if Wk.numel() == 0 or Wk.norm() < 1e-12:
            continue
        sigmas_all.extend(torch.linalg.svdvals(Wk).tolist())
    if not sigmas_all:
        return {"bimodality_ratio": 0.0, "sigmas_n_above_half_max": 0,
                "sigmas_n_below_tenth_max": 0, "sigmas_n_bulk": 0,
                "max_sigma": 0.0, "median_sigma": 0.0, "n_sigmas_total": 0}
    sigmas_all.sort(reverse=True)
    sigma_max = max(sigmas_all[0], 1e-12)
    n_above = sum(1 for s in sigmas_all if s >= 0.5 * sigma_max)
    n_below = sum(1 for s in sigmas_all if s <= 0.1 * sigma_max)
    n_bulk = max(len(sigmas_all) - n_above - n_below, 0)
    return {"bimodality_ratio": float(n_above) / max(n_bulk, 1),
            "sigmas_n_above_half_max": n_above,
            "sigmas_n_below_tenth_max": n_below,
...
def compute_gate_overlap(proj: torch.Tensor, K: int) -> dict:
    """Off-diagonal |<p_k, p_j>|^2 gate-overlap (mesoscopic xi parameter).

    proj: (K, N). Should be near 0 for SHIFT (orthogonal experts).
    """
    if K <= 1:
        return {"gate_overlap_max": 0.0, "gate_overlap_mean": 0.0, "xi_mesoscopic": 0.0}
    G = proj @ proj.T
    G2 = G * G
    off = G2 * (1.0 - torch.eye(K, device=proj.device))
    n_off = K * (K - 1)
    return {"gate_overlap_max": float(off.max()),
            "gate_overlap_mean": float(off.sum() / n_off),
            "xi_mesoscopic": float(off.sum() / (K * K))}
...
def run_arm_a_shift(keys: torch.Tensor, vals: torch.Tensor, K: int, N: int,
    ...
    # === DMPK bimodality signature (added v2) ===
    ...
    return {"mean_cosine": mean_cos, "loads": loads, "mode_collapse": mc,
            "param_budget": K * N * N, "dmpk": dmpk, "gate_overlap": gate_ov}
...
def run_arm_b_partition(keys: torch.Tensor, vals: torch.Tensor, K: int, N: int,
    ...
    mc = mode_collapse_metrics(loads)
    return {"mean_cosine": mean_cos, "loads": loads, "mode_collapse": mc,
            "param_budget": N * N, "dmpk": dmpk, "gate_overlap": gate_ov}
...
def compute_verdict(results: list[dict], M_per_expert: int) -> tuple[str, str, dict]:
    ...
    # === DMPK bimodality cell aggregates (added v2) ===
    import statistics as _stat
    for r in results:
        key = (r["K"], r["M_total"])
        cells[key].setdefault("arm_a_bimodality", []).append(
            r["arm_a"].get("dmpk", {}).get("bimodality_ratio", float("nan")))
        cells[key].setdefault("arm_b_bimodality", []).append(
            r["arm_b"].get("dmpk", {}).get("bimodality_ratio", float("nan")))
        cells[key].setdefault("arm_a_n_open", []).append(
            r["arm_a"].get("dmpk", {}).get("sigmas_n_above_half_max", 0))
        cells[key].setdefault("arm_b_n_open", []).append(
            r["arm_b"].get("dmpk", {}).get("sigmas_n_above_half_max", 0))
        cells[key].setdefault("arm_a_xi", []).append(
            r["arm_a"].get("gate_overlap", {}).get("xi_mesoscopic", float("nan")))

    # Compute bimodality separation at K=4 (canonical comparison)
    bimodality_sep_K4 = None
    for (K, M), d in cells.items():
        if K == 4:
            a_vals = [v for v in d.get("arm_a_bimodality", []) if not math.isnan(v)]
            b_vals = [v for v in d.get("arm_b_bimodality", []) if not math.isnan(v)]
            if a_vals and b_vals:
                a_bim = sum(a_vals) / len(a_vals)
                b_bim = sum(b_vals) / len(b_vals)
                sep = a_bim - b_bim
                bimodality_sep_K4 = (a_bim, b_bim, sep)
                break

    # Secondary mesoscopic verdict (ADDITIVE -- does NOT gate primary verdict)
    if bimodality_sep_K4 is not None:
        a_bim, b_bim, sep = bimodality_sep_K4
    ...
    summary = {
        ...
        "cells": summary_cells,
        "mesoscopic_verdict": mesoscopic_verdict,
        "bimodality_separation_K4": bimodality_sep_K4,
    }
...
def _instrumentation_selftest():
    ...
    # Test 8: DMPK bimodal spectrum
    W1 = torch.eye(4)
    W2 = torch.eye(4) * 0.01
    sig = compute_dmpk_signature([W1, W2], K=2, N=4, device=torch.device("cpu"))
    assert sig["sigmas_n_above_half_max"] == 4, f"Selftest 8 FAIL: bimodal got {sig}"
    assert sig["bimodality_ratio"] >= 1.0, f"Selftest 8 FAIL: ratio {sig['bimodality_ratio']}"
    print("[SELFTEST] 8/9 DMPK bimodal spectrum OK", flush=True)

    # Test 9: gate overlap orthogonal
    proj_orth = torch.eye(4)
    ov = compute_gate_overlap(proj_orth, K=4)
    assert ov["gate_overlap_max"] < 1e-6, f"Selftest 9 FAIL: got {ov}"
    print("[SELFTEST] 9/9 DMPK gate overlap orthogonal OK", flush=True)
    print("[SELFTEST] All 9 self-tests passed (v2 with DMPK)", flush=True)
...
def main():
    ...
    out_dir_name = "wave14_moe_shift_partition_v2_smoke" if smoke else "wave14_moe_shift_partition_v2"
    ...
    for r in all_results:
        results_serial.append({
            ...
            "arm_a_dmpk_bimodality_ratio": r["arm_a"].get("dmpk", {}).get("bimodality_ratio", None),
            "arm_b_dmpk_bimodality_ratio": r["arm_b"].get("dmpk", {}).get("bimodality_ratio", None),
            "arm_a_gate_xi": r["arm_a"].get("gate_overlap", {}).get("xi_mesoscopic", None),
        })
