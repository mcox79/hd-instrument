"""TENSOR-FACTORIZED W ENVELOPE v2 at N=4096.

CONTEXT (F2 follow-up):
  v1 (commit 75c565d) shipped at single M=512 and HP-passed (TF_HARD_PASS)
  but cap_map v283 flagged sub-capacity caveat: low-rank factorization
  preserved retention at low M, but the broader envelope (higher M, near
  M_c estimated 16K-20K) was not tested. Need rank sweep AT higher M to
  show factorization survives in the regime users actually populate.
...
PRE-REGISTERED BANDS:
  ...
  HARD_PASS: rank <= 1024 preserves >= 95% retention_ratio at ALL 3 M
    values in >= 3/5 seeds (i.e. 3-of-5 seeds yield retention_ratio>=0.95
    at every M for every rank in {128,256,512,1024}).
...
OOM CHECK:
  Largest M=8192, N=4096: keys=8192*4096*4=134MB. W=64MB. CB=805MB.
  SVD on W: ~3x W = 192MB peak. Total ~1.2GB. OK.

TIMEOUT ESTIMATE:
  Per cell: substrate build + full-rank SVD + 5 rank reconstructions
  + 5 retention probes. Dominant cost is SVD ~1-2s at N=4096.
  Smoke ~5s. FULL: 3 M * 5 seeds * (build + svd + 5 rank tests).
  ~30s/seed-M = 30*15 = 450s. Budget 21600s.

N-suffix: _n4096 (PROT-018).
Anchor: tensor_factorized_w_envelope_v2_n4096
Queue: overnight_queue (GPU; N=4096 envelope sweep)
Pre-reg: preregs/2026-05-30_tensor_factorized_w_envelope_v2_n4096.md
"""
...
_ck_spec = importlib.util.spec_from_file_location("_seed_ckpt_tfe", _ck_path)
...
def get_output_dir(default_name: str = "tensor_factorized_w_envelope_v2_n4096") -> Path:
...
def measure_M_seed(N_use: int, M: int, seed: int, ranks: List[int],
...
def compute_verdict(cells: List[Dict]) -> Tuple[str, str]:
    if not cells:
        return ("TFE_INCONCLUSIVE", "No cells.")
    ...
    if hf_seeds >= HF_SEEDS_MIN:
        return ("TFE_HARD_FAIL", f"FACTORIZATION_BREAKS_AT_HIGH_M: " + detail)
    if hp_seeds >= HP_SEEDS_MIN:
        return ("TFE_HARD_PASS", f"FACTORIZATION_HOLDS_ACROSS_ENVELOPE: " + detail)
    return ("TFE_MIDDLE_BAND", f"PARTIAL: " + detail)
...
def _instrumentation_selftest() -> None:
    ...
    out = measure_M_seed(N_SMOKE, 64, 17, RANKS_SMOKE, device)
    assert out["ret_full"] is not None
    for r, ret in out["rets_by_rank"].items():
        assert ret >= 0, f"rank {r} negative ret"
    print(f"[selftest] tensor_factorized_w_envelope_v2_n4096 PASS "
...
def main():
    ...
    t0 = time.time()
    print(f"[run] tensor_factorized_w_envelope_v2_n4096 smoke={smoke} N={N_cfg} "
    ...
    for M in Ms:
        for seed in seeds:
            ...
            try:
                out = measure_M_seed(N_cfg, M, seed, ranks, device)
    ...
    summary = {"anchor": "tensor_factorized_w_envelope_v2_n4096", "N": N_cfg,
