"""GHRR (Generalized HRR) non-commutative binding port.

Citation: Alam, Raff, Holt et al. *Generalized Holographic Reduced
Representations*, arXiv 2405.09689 (May 2024). Extends HRR/FHRR with
non-commutative binding operations.

Motivation: standard FHRR binding `byte * pos` is commutative — for
fixed byte and pos atoms, the elementwise product is the same regardless
of operand order. Position-dependent information enters our model only
through the choice of pos_atom[k] per position, not through the binding
operation itself. Non-commutative binding makes position structure
**part of the binding semantics**, which could yield richer sequence
representations.

Implementation: per-position random permutation applied to pos atoms.
For each k ∈ {0, K-1}, generate a fixed random permutation perm_k of
[0..N-1]. Binding becomes `byte_atom * permute_k(pos_atom[k])`. This
is non-commutative in the sense that the operation depends on the
position structure being preserved in the order of arguments.

This is the simplest "non-commutative" variant of FHRR binding from the
VSA literature (related to VTB - Vector-derived Transformation Binding,
and permutation-binding in Kanerva 1996). The Alam-Raff paper proposes
a more elaborate formalism with learned mixing matrices; we test the
permutation-binding variant first as a low-effort sanity check before
considering the full GHRR machinery.

Variants:
1. `baseline_no_perm`: standard FHRR binding (reference: 2.4994)
2. `pos_permuted`: per-position permutation applied to pos atoms
3. `byte_permuted`: per-position permutation applied to byte atoms
4. `both_permuted`: independent permutations for both (different per k)
"""
...
def make_perms(k, n, gen):
    """Generate k random permutations of [0..n-1] as long tensors of shape (k, n)."""
    perms = []
    for _ in range(k):
        perms.append(torch.randperm(n, generator=gen))
    return torch.stack(perms)


def build_ctx_bundles_perm(byte_atoms, pos_atoms, byte_perms, pos_perms, indices, variant):
    """Build context bundle with optional per-position permutation.

    indices: (B, K) long
    byte_perms: None or (K, N) long — per-position permutation of indices into byte_atom
    pos_perms:  None or (K, N) long — per-position permutation of indices into pos_atom
    """
    bound_per_k = []
    for k in range(K):
        byte_at_k = byte_atoms[indices[:, k]]  # (B, N)
        pos_at_k = pos_atoms[k]                # (N,)
        if byte_perms is not None:
            byte_at_k = byte_at_k[:, byte_perms[k]]
        if pos_perms is not None:
            pos_at_k = pos_at_k[pos_perms[k]]
        bound_per_k.append(byte_at_k * pos_at_k.unsqueeze(0))
    bound = torch.stack(bound_per_k, dim=1)  # (B, K, N)
...
def predict_W_batch(W, ctxs, byte_atoms, beta):
    n = ctxs.shape[1]
    q = ctxs @ W.T
...
def predict_pool_batch(ctxs, pool_vecs, pool_labels, pool_used, beta):
    B = ctxs.shape[0]
    if pool_used == 0:
...
def run_config(variant, train, test):
    ...
    with tracing.using(quiet):
        ...
        byte_perms = None
        pos_perms = None
        if "byte_perm" in variant or variant == "both_permuted":
            byte_perms = make_perms(K, N, gen).to(DEVICE)
        if "pos_perm" in variant or variant == "both_permuted":
            pos_perms = make_perms(K, N, gen).to(DEVICE)
        ...
        for epoch in range(1, MAX_EPOCHS + 1):
            for batch_start in range(0, T_total, BATCH_SIZE):
                ...
                B = idx_batch.shape[0]
                ctxs = build_ctx_bundles_perm(byte_atoms, pos_atoms, byte_perms, pos_perms, idx_batch, variant)
                P_W = predict_W_batch(W, ctxs, byte_atoms, BETA)
            ...
            if epoch in EPOCH_CHECKPOINTS:
                ...
                for bs in range(0, T_test, BATCH_SIZE):
                    ...
                    ctxs = build_ctx_bundles_perm(byte_atoms, pos_atoms, byte_perms, pos_perms, test_idx[bs:be], variant)
                    P_W = predict_W_batch(W, ctxs, byte_atoms, BETA)
                    P_retr = predict_pool_batch(ctxs, pool_vecs, pool_labels, pool_used, BETA)
...
def main() -> None:
    ...
    _say(f"\nGHRR permutation-binding variants (non-commutative binding test)")
    _say(f"  N={N}, K={K}, arousal={AROUSAL}, beta={BETA}, decay={DECAY}, pool={POOL_SIZE}, relu_b={RELU_B}")
    _say(f"  variants: {VARIANTS}")
    _say(f"  Reference (baseline_no_perm, FHRR combined+modReLU): 2.4994")
    _say(f"  Lit: Alam-Raff-Holt 2024 arXiv 2405.09689 (GHRR)")
    _say(f"  Hypothesis: per-position permutation gives non-commutative binding")
    _say(f"  May help if sequence-order capture is the bottleneck")
    ...
    _say(f"\n========= SUMMARY =========")
    _say(f"{'variant':>20s} {'best_ep':>8s} {'best_bpc':>10s} {'wT1':>6s} {'poolT1':>7s}")
    for r in all_results:
        best = min(r["history"], key=lambda h: h["test_bpc"])
        _say(f"{r['variant']:>20s} {best['epoch']:>8d} {best['test_bpc']:>10.4f} "
             f"{best['w_top1_acc']:>6.3f} {best['pool_top1_acc']:>7.3f}")
    baseline_bpc = min(all_results[0]["history"], key=lambda h: h["test_bpc"])["test_bpc"]
    _say(f"\n  baseline (no permutation) best: {baseline_bpc:.4f}")
    for r in all_results[1:]:
        best = min(r["history"], key=lambda h: h["test_bpc"])
        _say(f"  delta {r['variant']}: {best['test_bpc'] - baseline_bpc:+.4f}")
    ...
    out_path = Path(__file__).resolve().parent.parent / "data" / "exp_ghrr_charlm"
