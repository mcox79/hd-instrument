"""
substrate_compose_order_x_compose_function_2x2_factorial_v2
...
PROT-018: anchor name has no _n suffix. Production N=4096.
  Explicit declaration: "No _nN suffix; production N = 4096; rationale: remote_cpu_queue
  run at N=4096 (pure numpy); not the 8192 GPU harness baseline scale."

RESCUE v2: v1 timed out at 1200s wall. timeout increased to 3600s.
  Perf optimization: vectorized sparsify_bipolar + vectorized REVERSED key building.
...
ANCHOR_NAME = "substrate_compose_order_x_compose_function_2x2_factorial_v2"
...
def sparsify_bipolar(E: np.ndarray, f: float) -> np.ndarray:
    ...
    Vectorized: uses np.argpartition across all rows simultaneously (no Python loop).
    """
    V, dim = E.shape
    k = max(1, int(round(f * dim)))
    # argpartition over all rows at once: shape [V, dim]
    abs_E = np.abs(E)
    # partition so last k cols of each row are the k largest absolute values
    part_idx = np.argpartition(abs_E, -k, axis=1)[:, -k:]  # [V, k]
    out = np.zeros_like(E)
    row_idx = np.arange(V)[:, None]  # [V, 1] for advanced indexing
    vals = np.sign(E[row_idx, part_idx])  # [V, k]
    vals[vals == 0] = 1.0
    out[row_idx, part_idx] = vals
    return out
...
def build_reversed_src_keys(E_dense: np.ndarray, E_sparse: np.ndarray,
    ...
    Vectorized: numpy batch argpartition instead of per-row Python loop.
    """
    ...
    # Step 2: then sparsify (destroys attractor structure from bind) -- vectorized
    k = max(1, int(round(SPARSE_BIPOLAR_F * keys_dense.shape[1])))
    n = keys_dense.shape[0]
    part_idx = np.argpartition(np.abs(keys_dense), -k, axis=1)[:, -k:]  # [n, k]
    keys_sparse = np.zeros_like(keys_dense)
    row_idx = np.arange(n)[:, None]
    vals = np.sign(keys_dense[row_idx, part_idx])
    vals[vals == 0] = 1.0
    keys_sparse[row_idx, part_idx] = vals
...
def run_unit(seed: int) -> Dict:
    ...
    # sparsify AFTER bind (reversed) -- vectorized: no per-row Python loop
    k_rev = max(1, int(round(SPARSE_BIPOLAR_F * N_DIM)))
    _n_rev = src_keys_rev_held_pre.shape[0]
    _part_rev = np.argpartition(np.abs(src_keys_rev_held_pre), -k_rev, axis=1)[:, -k_rev:]
    src_keys_rev_held = np.zeros_like(src_keys_rev_held_pre)
    _row_rev = np.arange(_n_rev)[:, None]
    _vals_rev = np.sign(src_keys_rev_held_pre[_row_rev, _part_rev])
    _vals_rev[_vals_rev == 0] = 1.0
    src_keys_rev_held[_row_rev, _part_rev] = _vals_rev
    src_keys_rev_held = l2_norm(src_keys_rev_held)
    del src_keys_rev_held_pre, _part_rev, _row_rev, _vals_rev
...
REQUIRED_FIELDS = {
    ...
    "honest_scope": (
        ...
        "RESCUE v2 of v1 (timeout exhaustion at 1200s)."
    ),
    "prereg_bands": {
