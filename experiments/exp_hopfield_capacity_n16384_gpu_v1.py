"""
exp_hopfield_capacity_n16384_gpu_v1.py -- modern vs classic Hopfield capacity at N=16384 (GPU) -- GPU.

ROUTING: GPU product/capacity-scale (modern Hopfield capacity at N=16384). Capacity map at N=16384 (highest dimension yet); sweep load P/N up to 4.0; modern-Hopfield (softmax) vs classic recall@1 (overlap>=0.95) under 0.15 noise. Confirms exponential capacity holds at large dimension. torch.cuda; 8GB-safe. GPU.
...
ANCHOR_NAME = "hopfield_capacity_n16384_gpu_v1"
RUN_MODE = ("smoke" if "--smoke" in sys.argv else os.environ.get("HDLAB_RUN_MODE", "full")).lower()
_ap = argparse.ArgumentParser(); _ap.add_argument("--smoke", action="store_true"); _ap.add_argument("--self-test", action="store_true"); _ARGS, _ = _ap.parse_known_args()
SMOKE = RUN_MODE == "smoke"

def _selftest():
    import numpy as _n; assert _n.argmax([0.1, 0.9]) == 1, "argmax"; print("[selftest] PASS: hopfield_capacity_n16384_gpu_v1", flush=True)
_selftest()
if _ARGS.self_test:
    sys.exit(0)
try:
