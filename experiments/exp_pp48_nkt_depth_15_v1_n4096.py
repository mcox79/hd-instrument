"""
pp48_nkt_depth_15_v1_n4096 -- PP-48 NKT negative-knowledge tree at depth-15 at N=4096.

Extends depth-13 (HARD_PASS) to depth-15. Tests NKT depth ceiling.
  Total binary tree at depth-15: 2^15 - 1 = 32767 nodes. Sampled-leaf design.
  BRANCH=2, sample K_FORBIDDEN=100 leaves.
  alpha_total = (10 pos + 100 forbidden) / 4096 = 0.027. Well sub-capacity.

  Tree construction at depth-15: branching from root to leaves requires 15 recursion levels.
  Each leaf at depth-15 = product of 15 context Hadamard bindings from root.
  Test: does substrate still repel depth-15 leaves with anti-Hebbian inhibition?
...
PRE-REGISTERED BANDS:
  ...
  Calibration: depth-13 HARD_PASS; bands unchanged at depth-15.

FORMULA SELF-TESTS:
  1. BRANCH=2, depth=15 total tree: 2^15 - 1 = 32767.
     [INPUT: B=2, D=15] [EXPECTED: total = 32767]
...
ANCHOR_NAME = "pp48_nkt_depth_15_v1_n4096"
...
NKT_DEPTH = 15
BRANCH = 2
TOTAL_NKT_FULL = (BRANCH ** NKT_DEPTH - 1) // (BRANCH - 1)  # 2^15 - 1 = 32767
assert TOTAL_NKT_FULL == 32767, f"TOTAL_NKT formula: got {TOTAL_NKT_FULL}, expected 32767"
...
def build_nkt_tree_sample(n_dim: int, gen: torch.Generator, k_sample: int) -> List[torch.Tensor]:
    ...
    leaves = prev_level  # 2^(NKT_DEPTH-1) leaves at deepest level
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    if hp1 and hp2:
        return ("HARD_PASS", f"HARD_PASS: both HP at NKT depth-15 sampled-leaves. {summary}")
    if hp1 or hp2:
