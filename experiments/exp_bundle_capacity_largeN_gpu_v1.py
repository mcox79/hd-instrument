"""
exp_bundle_capacity_largeN_gpu_v1.py -- bundle K_crit at N in {8192,16384} tracks N/(2 ln N) (GPU) -- GPU.

ROUTING: GPU-scale substrate-physics (bundle capacity at large N vs theory). Extends the bundle-capacity law to large N (8192, 16384) on GPU; binary-search K_crit (recall@1>=0.9 for bundled role-filler pairs) and compare to N/(2 ln N). Confirms the composition-capacity law at scale. torch.cuda; OOM-safe (chunked) for an 8GB card. GPU.
PRE-REGISTERED: HARD-PASS K_crit within 35pct of N/(2 ln N) at both N. MIDDLE within 60pct. HARD-FAIL > 60pct.
...
ANCHOR_NAME = "bundle_capacity_largeN_gpu_v1"
RUN_MODE = ("smoke" if "--smoke" in sys.argv else os.environ.get("HDLAB_RUN_MODE", "full")).lower()
_ap = argparse.ArgumentParser(); _ap.add_argument("--smoke", action="store_true"); _ap.add_argument("--self-test", action="store_true"); _ARGS, _ = _ap.parse_known_args()
SMOKE = RUN_MODE == "smoke"

def _selftest():
    import numpy as _n; assert _n.argmax([0.1, 0.9]) == 1, "argmax"; print("[selftest] PASS: bundle_capacity_largeN_gpu_v1", flush=True)
_selftest()
if _ARGS.self_test:
    sys.exit(0)
try:
