"""Encoder v5 -- K=128 vs K=256 code-CAPACITY paired test: does a finer block
code (more, smaller blocks -> higher per-code bit budget) actually lift
`ret_agree10` at FULL 178k scale, or was the bypass-diagnostic's "+0.09
Spearman" lift a zero-training analytical artifact that does not survive
real training?

QUEUE-DEPTH cell (2nd of 2 authored while other in-flight R1-sequence work
finishes elsewhere). Problem 1 (convergence/plateau) is the sibling cell:
experiments/exp_encoder_v4_convergence_lr_hold_v1_core.py. THIS cell =
problem 2 (code-capacity/retrieval-resolution), kept as a SEPARATE cell
so the K-sweep is not confounded with v4's unvalidated LR-schedule change
(v4's PLATEAU-hold LR is not yet validated; composing an unvalidated fix
with a capacity sweep would make either result hard to attribute). This
cell reuses the SAME schedule as v3e (cosine-decay-to-0) so the ONLY
difference between the two arms is K (block count / block length), a true
paired comparison.

CONTEXT (do not re-litigate). v3e (FULL, seed=7, in_batch-RKD-only,
nce_weight=0, K=128, steps=6000) landed:
  final_ret_agree10=0.2112 final_hi80_cos=0.8320
  MEASURED@data/exp_encoder_v3e_decline_vs_plateau_v1_seed7/metrics.json.
Bypass diagnostic (teacher-through-sparsifier, ZERO training, commit
e5a084fbe, experiments/exp_encoder_teacher_sparsifier_bypass_v1_core.py):
K=128 block code caps ~0.80 Spearman, K=256 ~0.89 (+0.09) HYPOTHESIZED-
CONTEXT@this session's spawn prompt (bypass smoke result cited by the
coordinator; not independently re-verified from disk by this cell -- this
cell's own K=128 arm re-measures the analogous quantity under TRAINING,
which is the actual question: does the +0.09 analytical-ceiling gap survive
into a genuinely trained model's retrieval quality, or was it an artifact
of an untrained/bypass encoding path).

TWO PAIRED ARMS, same seed/data/split/mining/objective (in_batch-RKD-only,
nce_weight=0, steps=6000, batch=128 -- MATCHES v3e exactly) inside ONE
process, differing ONLY in the block-code resolution:
  K128 -- kb=128, blk_l=32 (3.125% active; MATCHES v2/v3/v3b/v3c/v3e/v4)
  K256 -- kb=256, blk_l=16 (6.25% active; the bypass-diagnostic's better arm)
...
needed for this cell -- lower risk than v4's parameterized-LR rewrite).
LR schedule is the UNCHANGED cosine-decay-to-0 (v3e's schedule); this cell
does NOT use v4's plateau-hold LR (see confound-avoidance note above).

Note on the 2%-sparsity goal tension (flagged, not blocking): K=256 (6.25%
active) moves AWAY from the director_plan.json encoder-goals' ~2%-sparsity
target (k~82/N=4096), not toward it. This cell tests the USER's explicitly
requested K=128-vs-K=256 comparison (a resolution/capacity question); if
K=256 lifts retrieval, the strategic implication (denser code needed for
capacity vs the 2% goal) is a separate USER/Research-level tradeoff this
cell surfaces but does not resolve.
...
remote-official; v3c 0.7336 (v3b) vs 0.6514 (v3c's own re-run) -- traced to
torch-version drift + no determinism pinning): this cell PINS determinism
identically to the v4 sibling cell (see `_pin_determinism`) and records
...
Prior-work check (substrate-KB concept-query, USER-locked 2026-07-01):
  query "K256 block code capacity paired comparison retrieval agreement
  trained encoder versus K128 code resolution ceiling" -> top hit
  cosine=0.2841 (this arc's own v3c/v3e prose, expected self-similarity),
  all other hits <=0.27 except the bypass-diagnostic cell's own prose
  (cosine=0.29, expected -- same arc, distinct cell, zero-training vs
  trained). NONE at cosine>0.30 for a DISTINCT prior TRAINED cell.
  GENUINELY NOVEL: no prior cell in this lineage trains a K=256 student and
  compares it against K=128 under matched conditions.
...
- except SystemExit: raise BEFORE except Exception (no BaseException, no
  bare except)
- crlb_floor_computed: K128 r_max=0.901 (THEORETICAL@v2/v3/v3b/v3c/v3e,
  unchanged); K256 r_max computed via the SAME formula with the SAME
  implied sigma_teacher backed out from the K128 anchor (see
  `_crlb_r_max`; self-test asserts `_crlb_r_max(128)` reproduces 0.901)
- baseline_in_band: CHARPOS ret_agree10 in (0.05, 0.95)
- discriminator-survives-scale: option (B) analytical justification (same
  as the whole v3/v3b/v3c/v3e/v4 lineage): smoke's tiny V_train=3000
  ...
  actual K128-vs-K256-retrieval-lift question needs the true 177899-concept
...
- HP_SCOPE: the K128-vs-K256 delta bands apply to {K128,K256}_BLOCK_LAST
  only; {K128,K256}_*_BESTVAL are comparison/context; RANDOM_BLOCK/CHARPOS/
  shuffled_key are integrity-only (per-arm for RANDOM_BLOCK/shuffled_key,
  since block partition differs by arm).
...
- calibration_check: default_ok_for_this_regime (identical hyperparameters
  to the validated v3c/v3e lineage; only K_BLOCKS/blk_l differ between arms)
- numbers tagged MEASURED@ / HYPOTHESIZED@ / THEORETICAL@ / CITED@ / VERIFIED@

Prereg: preregs/2026-07-04_exp_encoder_v5_k256_capacity_paired_v1.md
...
Does NOT touch v3/v3b/v3c/v3e/v4's own artifact/checkpoint/output directories.
...
# Config.
# ---------------------------------------------------------------------------
ANCHOR_NAME = "encoder_v5_k256_capacity_paired_v1"
...
K_ARMS = {"K128": (128, 32), "K256": (256, 16)}  # arm_name -> (kb, blk_l); N_DIM=4096
...
def _artifact_dir(run_mode: str, run_tag: str = "") -> Path:
    ...
    return _REPO / "data" / f"substrate_concept_encoder_v5_k256{tag}{suffix}"
...
def _verdict_k_capacity(per_unit: List[Dict], recovery: Dict, expected_units: int,
    ...
    k128 = recovery["K128"]
    k256 = recovery["K256"]
    delta_ret = k256["final"]["ret_agree10"] - k128["final"]["ret_agree10"]
    delta_hi80 = k256["final"]["hi80_cos"] - k128["final"]["hi80_cos"]
    tail = (f"[K128: final_ret={k128['final']['ret_agree10']:.4f} "
           f"final_hi80={k128['final']['hi80_cos']:.4f} "
           f"final_block_spearman={k128['final']['spearman_all']:.4f}] "
           f"[K256: final_ret={k256['final']['ret_agree10']:.4f} "
    ...
    if run_mode == "smoke":
        ...
        return ("HARD_PASS",
                ...
                f"algebra checks fire, cardinality holds {tail} (the K128-vs-K256 "
    ...
    if delta_hi80 < DELTA_HI80_COS_REGRESSION_FLOOR:
        return ("HARD_FAIL",
                f"K256_REGRESSES_CALIBRATION: hi80_cos delta {delta_hi80:.4f} < "
    ...
    if delta_ret >= DELTA_RET_AGREE10_HARD_PASS_MIN:
        return ("HARD_PASS",
                f"K256_LIFTS_RETRIEVAL_CONFIRMED: ret_agree10 delta {delta_ret:.4f} "
                f">= {DELTA_RET_AGREE10_HARD_PASS_MIN} with no calibration "
                f"regression -- the bypass-diagnostic's capacity-ceiling "
                f"prediction SURVIVES real training; code resolution (K) is a "
                f"genuine retrieval lever, independent of the convergence "
                f"question in the sibling v4 cell {tail}")
    if delta_ret <= 0.0:
        return ("HARD_FAIL",
                f"K256_DOES_NOT_LIFT_RETRIEVAL: ret_agree10 delta {delta_ret:.4f} "
                f"<= 0 -- the bypass-diagnostic's analytical +0.09 Spearman "
                f"ceiling gap did NOT translate into a trained retrieval "
                f"improvement; K is not the (or not the only) bottleneck {tail}")
    return ("MIDDLE_BAND",
            f"K256_MARGINAL_LIFT: ret_agree10 delta {delta_ret:.4f} is positive "
            f"but below the {DELTA_RET_AGREE10_HARD_PASS_MIN} HARD_PASS margin -- "
            f"a real but small effect; consider a 2nd seed or a coarser/finer K "
...
def run_k_capacity(run_mode: str, seed: int, device_arg: str, n_dim: int,
    ...
    print(f"[v5_k_capacity] run_mode={run_mode} seed={seed} device={device} "
    ...
    V_cache = X.shape[0]
    print(f"[v5_k_capacity] teacher {cache_path.name}: {V_cache} concepts x "
    ...
    print(f"[v5_k_capacity] split train={n_tr} held={n_he}", flush=True)
    ...
    print(f"[v5_k_capacity] mining done cov={semi_cov:.3f} "
    ...
    for arm, (kb, blk_l) in K_ARMS.items():
        ...
        print(f"[v5_k_capacity] {arm} (kb={kb},blk_l={blk_l}) trained "
    ...
    def _run_unit(fn, *a, **kw):
        try:
            u = fn(*a, **kw)
            per_unit.append(u)
            print(f"[v5_k_capacity] unit {len(per_unit)}/{expected_units} {u['unit']}: "
    ...
    metrics = {
        ...
        "methodology": ("PAIRED same-seed/data/split/mining comparison of code "
                        "resolution (K=128 vs K=256 blocks) at the in_batch-RKD-"
                        "only nce=0 config matching v3e, UNCHANGED cosine-decay "
                        "LR schedule (kept separate from the sibling v4 cell's "
                        "unvalidated plateau-hold LR to avoid confounding this "
                        "K-sweep); FINAL-step ret_agree10/hi80_cos delta is the "
                        "PRIMARY gated comparison, best-by-VAL-on-TEST is "
                        "SECONDARY context; per-arm RANDOM_BLOCK/shuffled-key "
                        "integrity + FALSE_WIN_ALGEBRA checks run at EACH arm's "
                        "own block partition (K256's smaller blk_l=16 could in "
                        "principle break SBC composability even if it lifts "
                        "retrieval -- checked explicitly, not assumed)"),
        "progress_logging": "print_flush_true",
        "baseline_in_band": bool(0.05 < v3._by_unit(
            per_unit, "semantic", "CHARPOS")["ret_agree10"] < 0.95),
        "crlb_floor_computed": {"K128": _crlb_r_max(128), "K256": _crlb_r_max(256)},
        "crlb_formula_reference": ("r_max = sigma_teacher / sqrt(sigma_teacher^2 + "
                                   ...
                                   "(THEORETICAL@v2/v3/v3b/v3c/v3e), then applied "
                                   "to K=256 on the same theoretical footing"),
    ...
    print(f"[v5_k_capacity] verdict={verdict} msg={verdict_msg} elapsed={elapsed:.1f}s",
          flush=True)
    return 0
...
def run_self_test() -> int:
    ...
    #    anchor; K=256 must be a HIGHER ceiling (more blocks -> more bits).
    r128 = _crlb_r_max(128)
    r256 = _crlb_r_max(256)
    assert abs(r128 - 0.901) < 1e-3, f"selftest: CRLB(128) should reproduce 0.901, got {r128}"
    assert r256 > r128, "selftest: CRLB(256) must exceed CRLB(128) (finer code = higher ceiling)"

    # 2. verdict bands: cardinality / algebra / regression / lift / no-lift / marginal.
    def _fake_units(k128_algebra=1.0, k256_algebra=1.0, k128_shuf=0.01, k256_shuf=0.01):
        units = [{"unit": f"u{i}", "arm": "x", "kind": "k"} for i in range(11)]
        for arm, alg, shuf in (("K128", k128_algebra, k128_shuf), ("K256", k256_algebra, k256_shuf)):
            units += [
    ...
    def _rec(k128_ret, k256_ret, k128_hi80=0.80, k256_hi80=0.82):
        return {
            "K128": {"final": {"spearman_all": 0.9, "ret_agree10": k128_ret,
                               "hi80_cos": k128_hi80, "hi80_calib_err": 0.02}},
    ...
    assert v_pass == "HARD_PASS" and "K256_LIFTS_RETRIEVAL_CONFIRMED" in m_pass, (
    ...
    assert v_nolift == "HARD_FAIL" and "K256_DOES_NOT_LIFT_RETRIEVAL" in m_nolift, (
    ...
    assert v_marg == "MIDDLE_BAND" and "K256_MARGINAL_LIFT" in m_marg, (
        f"selftest: expected marginal MIDDLE_BAND got {v_marg} ({m_marg})")

    v_regress, m_regress = _verdict_k_capacity(
        _fake_units(), _rec(0.20, 0.25, k128_hi80=0.80, k256_hi80=0.70), 19, "full")
    assert v_regress == "HARD_FAIL" and "K256_REGRESSES_CALIBRATION" in m_regress, (
        f"selftest: expected calibration-regression HARD_FAIL got {v_regress} ({m_regress})")

    v_alg, m_alg = _verdict_k_capacity(
        _fake_units(k256_algebra=0.20), _rec(0.20, 0.25), 19, "full")
    assert v_alg == "HARD_FAIL" and "FALSE_WIN_ALGEBRA_LAST_STEP_K256" in m_alg, (
        f"selftest: expected K256 algebra-break HARD_FAIL got {v_alg} ({m_alg})")
    ...
    print(f"[selftest] PASS (CRLB formula consistency K128->0.901 anchor + "
          f"K256>K128 ceiling + lift-confirmed/no-lift/marginal/calibration-"
