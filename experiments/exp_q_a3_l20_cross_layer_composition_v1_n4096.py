"""
q_a3_l20_cross_layer_composition_v1_n4096 -- Q-A3: L=20 cross-layer composition at N=4096.

Ceiling chase: extends L=19 (HARD_PASS EXACT-1.0 unanimous 5-seed) to L=20.
Architecture adds one more Hadamard binding level (L20 outer layer).
20 W matrices x ~67 MB each = ~1340 MB total. Fits in 8 GB GPU with margin.
Note: M per stage halves each level per isochoric design; alpha_c constraint ensures validity.
At L=20 M_OUTER=2, M_MID19=2, ..., M_MID7=2 (floored at 2 from L=7+).

PRE-REGISTERED BANDS (ceiling push, prior L=19 EXACT-1.0 unanimous):
  HARD-PASS: all 20 level fidelities = 1.0000 (>= 0.9999) unanimous (5/5 seeds)
             AND l20_acc >= 0.5.
  MIDDLE: any L_fid in [0.85, 1.0) OR graceful degradation of inner levels.
  HARD-FAIL: any L_fid < 0.85 OR l20_acc < 0.5. Ceiling found at L=20.
...
QUEUE: overnight_queue (GPU; 20 W matrices at N=4096 ~1340 MB).
"""
import sys
...
ANCHOR_NAME = "q_a3_l20_cross_layer_composition_v1_n4096"
...
def _selftest_l20_chain():
    """20-step Hadamard roundtrip recovers xi_L1."""
    ctxs = []
...
def _instrumentation_selftest():
    ...
    ms = [M_INNER, M_MID2, M_MID3, M_MID4, M_MID5, M_MID6, M_MID7, M_MID8,
          ...
          M_MID17, M_MID18, M_MID19, M_OUTER]
...
def run_seed(seed: int, n_dim: int) -> Dict:
    ...
    Xi_ctx20 = bsc(M_OUTER, n_dim)
    Xi_outer = Xi_ctx20 * Xi_mid19[:M_OUTER]
    ...
    for q_idx in range(min(N_QUERIES, M_OUTER)):
        ...
        xi_L20_ret = hopfield_retrieve_gpu(W_outer, probe)
        ...
        level_fids["L20"].append(cosine_sim_gpu(xi_L20_ret, xi_outer_true))
        ...
        l20_acc.append(cosine_sim_gpu(xi_L1_ret, Xi_inner[q_idx]))
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    if all_unanimous and l20_acc >= HP_LACC:
        return ("HARD_PASS", f"HARD_PASS: all {L_DEPTH} levels EXACT-1.0 unanimous + l20_acc HP. {summary}")
