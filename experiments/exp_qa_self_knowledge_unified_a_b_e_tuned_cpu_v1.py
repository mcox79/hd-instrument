"""
exp_qa_self_knowledge_unified_a_b_e_tuned_cpu_v1.py -- UNIFIED + tuned route_E (Cycle 51 day-2 -> mid).

EXTENDS exp_qa_self_knowledge_unified_a_tuned_b_v3_cpu_v1.py with tuned route_E:
- Same scored multi-field pattern as route_A (name*4 + aliases*2 + id + desc + all-kws-in-name-or-alias bonus*10)
- Corpus-restricted to meta/methodology (preserves v3 route_E's META scoping)
- Score threshold + top-K cap

PROJECTED: MACRO 0.5869 -> ~0.60 (+0.011 from E-axis lift 0.495 -> ~0.55).
PRE-REGISTERED: HARD-PASS E-axis macro >= 0.55 (current 0.495; +0.055 needed).
...
ANCHOR_NAME = "qa_self_knowledge_unified_a_b_e_tuned_cpu_v1"
...
_ap.add_argument("--e-top-k", type=int, default=5, help="E-axis top-K (smaller than A since E gold is sparser)")
_ap.add_argument("--e-threshold", type=int, default=4)
_ARGS, _ = _ap.parse_known_args()
SMOKE = RUN_MODE == "smoke"
TOP_K = _ARGS.top_k
SCORE_THRESHOLD = _ARGS.threshold
E_TOP_K = _ARGS.e_top_k
E_THRESHOLD = _ARGS.e_threshold
...
def _score_atom(a, kws):
    """Score atom by weighted keyword hits in name/aliases/id/desc + all-kws-in-name-or-alias bonus."""
...
# ---------- tuned route_E (META/methodology-restricted scored multi-field) ----------
def route_E_tuned(atoms, args, top_k=E_TOP_K, threshold=E_THRESHOLD):
    """Tuned E-route: scored multi-field over META/methodology corpus + top-K cap."""
    kws = [w for w in args.get("scenario", "").lower().split() if len(w) > 2 and w not in STOP]
    if not kws: return set()
...
def _selftest():
    ...
    print("[selftest] PASS: qa-unified-a-b-e-tuned", flush=True)
...
def run() -> Dict:
    ...
    print("[snapshot] atoms=%d relations=%d rel_types=%d benchmark_qs=%d top_k_a=%d th_a=%d top_k_e=%d th_e=%d"
          % (len(atoms), len(relations), len(rel_present), len(bench), TOP_K, SCORE_THRESHOLD, E_TOP_K, E_THRESHOLD), flush=True)
    per_q = []; by_type = {}
    for q in bench:
        ...
        elif t == "E": retrieved = route_E_tuned(atoms, q["args"])
...
def verdict(r) -> Tuple[str, str]:
    ...
    e_axis = tf1.get("E", 0.0)
    s = ("macro-F1=%.4f E-axis=%.4f (n=%d Qs, per-type %s, gold-attrition=%d, top_k_a=%d th_a=%d top_k_e=%d th_e=%d) -- UNIFIED + tuned-E per Cycle 51 day-2->mid"
         % (m, e_axis, r["n_qs"], tf1, r["total_gold_attrition"], TOP_K, SCORE_THRESHOLD, E_TOP_K, E_THRESHOLD))
    if e_axis >= 0.55:
        return ("HARD_PASS", "HARD_PASS: E-axis macro-F1 >=0.55 (current %.4f) -- precision tuning resolves E. %s" % (e_axis, s))
    if e_axis >= 0.50:
        return ("MIDDLE_BAND", "MIDDLE_BAND: E-axis macro-F1 %.4f in (0.50, 0.55) -- partial. %s" % (e_axis, s))
    return ("HARD_FAIL", "HARD_FAIL: E-axis macro-F1 %.4f <= 0.50 -- tuning didn't help or hurt. %s" % (e_axis, s))


print("[config] anchor=%s mode=%s top_k_a=%d th_a=%d top_k_e=%d th_e=%d" % (ANCHOR_NAME, RUN_MODE, TOP_K, SCORE_THRESHOLD, E_TOP_K, E_THRESHOLD), flush=True)
