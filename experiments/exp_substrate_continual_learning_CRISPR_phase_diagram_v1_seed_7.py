"""substrate_continual_learning_CRISPR_phase_diagram_v1 sibling seed=7.

Phase-diagram sweep of CRISPR continual-learning over (M, N, forget_rate).
Stage 1 phase-diagram coverage for chain-grade cl_crispr primitive
(MID -> HIGH per pattern_completion v2.1 / sequence_binding v2 promotion).
...
SWEEP: M in {100,500,1000,5000,10000} x N in {2048,4096,8192}
       x forget in {0.001,0.006,0.01,0.05,0.1} = 75 pts (write_strength=1.0 fixed)
CPU numpy (pure numpy; no torch).

PRE-REG: preregs/2026-06-28_substrate_continual_learning_CRISPR_phase_diagram_v1.md
CARDINALITY_OK_FULL: 75 phase points per seed (3 arms each)
CARDINALITY_OK_SMOKE: 6 corners per seed
...
PRESERVE_ENV_VARS_RATIONALE: CPU-only numpy cell; declares contract per
  ...
  Cell does NOT import torch; gpu_mandate_check is not applicable.
...
SEED = 7
ANCHOR_NAME = f"substrate_continual_learning_CRISPR_phase_diagram_v1_seed_{SEED}"
...
CONFIG_VERSION = (
    ...
    f"M=[100,500,1000,5000,10000],N=[2048,4096,8192],"
    f"forget=[0.001,0.006,0.01,0.05,0.1],"
    f"arms=[ARM_CRISPR_FULL,ARM_NO_FORGET,ARM_OVERWRITE_ONLY],"
    f"write_strength=1.0,cohort_size=100,"
    f"expected_n_full=75,expected_n_smoke=6,"
    f"hardening=CHUNKED_PER_SEED+L4importsentinel+halt_on_error"
)
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v1_cl_crispr_phase_diagram_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v1_cl_crispr_phase_diagram_import_crash",
        }
...
def main() -> int:
    ...
    # Late import for import-crash sentinel
    from experiments._substrate_continual_learning_CRISPR_phase_diagram_v1_core import (
        run_one_seed_phase_diagram, aggregate_and_verdict, selftest,
        get_backend_label, M_VALUES, N_VALUES, FORGET_VALUES,
        SMOKE_CORNERS, ARMS,
    )
    ...
    final["_hardening_marker"] = "v1_cl_crispr_phase_diagram_chunked"
    final["backend"] = backend
    final["seed"] = SEED

    # cardinality_ok
    if SMOKE_MODE:
        expected_n = len(SMOKE_CORNERS)
    else:
        expected_n = len(M_VALUES) * len(N_VALUES) * len(FORGET_VALUES)
    observed_n = 0
    for body in per_seed.values():
        observed_n += len([p for p in body.get("phase_map", []) if "ERROR" not in p])
