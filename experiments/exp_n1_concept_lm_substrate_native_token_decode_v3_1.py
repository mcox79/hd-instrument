...
CONFIG_VERSION = "V_C=256,N_DIM=4096,f=0.006,DECODE=cal_temp_backoff,MAX_DOCS=100000,SEEDS=7-17-23,SPLIT=0.8"
LAM_BACKOFF = 0.1   # unigram back-off weight in the calibrated decode (floors true-token prob, bounds BPC)
LAPLACE_A = 0.5     # add-k smoothing for bigram + ceiling token distributions (so no true token gets ~0 prob)
...
ANCHOR_NAME = "n1_concept_lm_substrate_native_token_decode_v3_1"
...
LAM_BACKOFF = 0.1   # unigram back-off weight in count-proportional substrate decode (floors prob, bounds BPC)
INTERP_B = 0.3      # Jelinek-Mercer interpolation: P = (1-b)*P_mle + b*P_unigram for bigram + ceiling baselines

CONFIG_VERSION = "V_C=%d,N_DIM=%d,f=%.4f,DECODE=countprop_interp,MAX_DOCS=%d,SEEDS=%s,SPLIT=%.1f" % (
...
def batched_token_logprob(D: np.ndarray, concept_vecs: np.ndarray,
    ...
    """Batched CALIBRATED log-prob over tokens for multiple concept vectors.

    P(token) = (1 - lam) * softmax(scores / tau) + lam * uni_dist  (unigram back-off).
    With uni_dist=None this is the pure softmax (the batched==per-query selftest uses that
    form, so the equivalence still holds). The back-off guarantees no true token gets ~0
    probability -> bounded, fair BPC (fixes the v2 overconfident-softmax catastrophe).
    """
    scores = np.maximum(concept_vecs @ D, 0.0)   # (n_pos, V_TOK) relu: positive concept->token evidence
    probs = scores / (scores.sum(axis=1, keepdims=True) + 1e-300)   # count-proportional MLE distribution
    if uni_dist is not None and lam > 0.0:
        probs = (1.0 - lam) * probs + lam * uni_dist[None, :]   # interpolation back-off floor
    return np.log(np.maximum(probs, 1e-30))   # float32-safe floor (1e-300 underflows to 0 in f32)
...
def token_logprob(D: np.ndarray, concept_vec: np.ndarray,
                  uni_dist: np.ndarray = None, lam: float = 0.1,
                  tau: float = 1.0) -> np.ndarray:
    """CALIBRATED log-prob over tokens (single concept vector).

    P(token) = (1 - lam) * softmax(scores / tau) + lam * uni_dist (unigram back-off).
    Same math as batched_token_logprob (the selftest asserts they match).
    """
    scores = np.maximum(D.T @ concept_vec, 0.0)  # (V_TOK,) relu: positive concept->token evidence
    probs = scores / (scores.sum() + 1e-300)     # count-proportional MLE distribution
    if uni_dist is not None and lam > 0.0:
        probs = (1.0 - lam) * probs + lam * uni_dist   # interpolation back-off floor
    return np.log(np.maximum(probs, 1e-30))   # float32-safe floor (1e-300 underflows to 0 in f32)
    ...
    if _running_tot_c == 0:
        ...
        if tot_t == 0:
            ...
            for _ck_s in range(0, n_valid, BATCH_TOK_CHUNK):
                ...
                _lp = batched_token_logprob(D, _cvecs, uni_dist, LAM_BACKOFF)      # (chunk, V_TOK) calibrated
