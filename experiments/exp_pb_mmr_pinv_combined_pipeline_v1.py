"""
exp_pb_mmr_pinv_combined_pipeline_v1 -- propose-back (do the 2 production levers compose) -- CPU.
...
ANCHOR_NAME = "pb_mmr_pinv_combined_pipeline_v1"
...
if RUN_MODE == "smoke":
    ...
    SEEDS = [7, 17, 23]; N = 2048; N_CLUST = 40; PER = 40; N_Q = 150
...
def verdict(ps) -> Tuple[str, str]:
    rec = float(np.mean([p["pinv_recall"] for p in ps])); pm = float(np.mean([p["prop_mmr"] for p in ps])); pt = float(np.mean([p["prop_topk"] for p in ps]))
    summary = "pinv_recall=%.3f | propagation topk=%.3f -> MMR=%.3f" % (rec, pt, pm)
    if rec >= 0.90 and pm < 0.10:
        return ("HARD_PASS", "HARD_PASS: pinv (capacity) + MMR (anchoring robustness) COMPOSE -- pinv keeps high recall while MMR cuts propagation<0.10. Full production pipeline gets both. " + summary)
    if rec >= 0.90 or pm < 0.10:
        return ("MIDDLE_BAND", "MIDDLE_BAND: one lever retained in the combined pipeline. " + summary)
    return ("HARD_FAIL", "HARD_FAIL: combining pinv+MMR loses both. " + summary)
