"""
exp_pca_bottleneck_zkl_sweep_v1 -- Hyp B mitigation (b): per-position-debiased mean-pool (does it kill the position-concentrated leak?) -- GPU.
...
ANCHOR_NAME = "zkl_hypB_repool_debias_v1"
...
ML = 48   # fixed length so positions align (left-pad: real tokens occupy rightmost ML indices)
def allpos(texts, tok, m):
    S = []; M = []
    for i in range(0, len(texts), 16):
        t = tok(texts[i:i + 16], return_tensors="pt", padding="max_length", truncation=True, max_length=ML).to(DEV)
        with torch.no_grad():
            o = m(**t, output_hidden_states=True)
        S.append(o.hidden_states[LAYER].float().cpu().numpy()); M.append(t["attention_mask"].float().cpu().numpy())
    return np.concatenate(S, 0), np.concatenate(M, 0)   # [n, ML, H], [n, ML]
def posmean_fit(S, M):
    return (S * M[:, :, None]).sum(0) / (M.sum(0)[:, None] + 1e-6)   # [ML, H] per-position mean
def debiased_meanpool(S, M, pm):
    d = (S - pm[None]) * M[:, :, None]
    return (d.sum(1) / (M.sum(1)[:, None] + 1e-6)).astype(np.float32)
def llama_encode(texts, tok, m, pm):
    S, M = allpos(texts, tok, m); return debiased_meanpool(S, M, pm)
...
def run() -> Dict:
    ...
    cohort = (stored + never)[:max(200, len(stored))]                       # held-out cohort for per-position mean
    Sc, Mc = allpos(cohort, tok, m); pm = posmean_fit(Sc, Mc)
    keys = llama_encode(stored, tok, m, pm)
    Kp = len(pv[0]); var_pos = llama_encode([p for vs in pv for p in vs], tok, m, pm).reshape(len(stored), Kp, -1)
    var_neg = llama_encode([p for vs in nv for p in vs], tok, m, pm).reshape(len(never), Kp, -1)
    del m; torch.cuda.empty_cache()
    mu, V, eig = pca_fit_full(keys); D = V.shape[0]
    kp = whiten_d(keys, mu, V, eig, D)
    vp = whiten_d(var_pos.reshape(-1, var_pos.shape[2]), mu, V, eig, D).reshape(var_pos.shape[0], var_pos.shape[1], D)
    vn = whiten_d(var_neg.reshape(-1, var_neg.shape[2]), mu, V, eig, D).reshape(var_neg.shape[0], var_neg.shape[1], D)
    zkl = zkl_at(kp, vp, vn)
    Ku = unit(kp).astype(np.float64); G = Ku @ Ku.T + 1e-3 * np.eye(len(Ku)); Wi = np.linalg.solve(G, Ku)
    keyf1 = float((np.argmax(Ku @ Wi.T, axis=1) == np.arange(len(Ku))).mean())
    print("  debiased-meanpool ZKL(%d)=%.3f KEY-job-F1=%.3f (baseline ZKL~0.22, F1~1.0)" % (K_PARA, zkl, keyf1), flush=True)
    return {"zkl": zkl, "keyf1": keyf1, "n": len(stored), "k": K_PARA}


def verdict(r) -> Tuple[str, str]:
    z = r["zkl"]; f1 = r["keyf1"]; drop = 1.0 - f1
