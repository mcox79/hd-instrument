"""Degree-debiased fair re-scoring of the Course-C rotation win, seed 23 (REMOTE CPU, process-isolated).
Refits ONLY ONESHOT_ROTATE (+ recomputes POP) on the reproduced seed-23 split, then computes the degree-debiased
diagnostics (within-stratum / partial correlation + degree-matched candidate margin) against the archived
gpu1024 seed-23 reference. CPU-forced (task lock: no GPU) -> cross-device from the CUDA archive, so ONESHOT
...
ANCHOR_NAME = "course_c_debias_cskg_l2_seed_23_cpu_v1"
SEEDS = [23]
