...
ANCHOR_NAME = "phase4b_multibench_multiseed_cpu_v1"
...
def _selftest():
    ...
    print("[selftest] PASS: phase4b-multibench-multiseed", flush=True)
_selftest()
if _ARGS.self_test:
    sys.exit(0)
def _load_all():
...
def run() -> Dict:
    ...
    macros = []; perseed = defaultdict(list)
    for sd in SEEDS:
    ...
    print("  PHASE4B-MULTIBENCH n=%d: macro-mean=%.3f std=%.4f | per-benchmark-mean=%s | train-labeled=%d" %
          (len(SEEDS), mean, std, per_mean, len(Xtr)), flush=True)
    return {"macro_acc": round(mean, 3), "macro_std": round(std, 4), "per_benchmark": per_mean, "n_seeds": len(SEEDS), "n_train_labeled": len(Xtr), "n_benchmarks": len(bnames)}
def verdict(r) -> Tuple[str, str]:
    if r.get("error"): return ("UNKNOWN", "UNKNOWN: " + r["error"])
    m = r["macro_acc"]; s = "macro-mean=%.3f std=%.4f " % (m, r.get("macro_std", 0)) + "; detail=%.3f per=%s (%d benchmarks, train-labeled=%d)" % (m, r["per_benchmark"], r["n_benchmarks"], r["n_train_labeled"])
    sd = r.get("macro_std", 1.0)
    if m >= 0.30 and r["n_benchmarks"] >= 3 and sd <= 0.02:
        return ("HARD_PASS", "HARD_PASS: substrate-native word-problem solver SEED-ROBUST (macro-mean>=0.30, std<=0.02, n=%d) across >=3 benchmarks -- Tier A multi-benchmark math capability, no LLM. " % r.get("n_seeds", 5) + s)
    if m >= 0.22:
