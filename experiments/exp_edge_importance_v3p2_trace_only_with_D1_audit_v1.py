"""edge_importance v3.2 TRACE-only with D1 audit -- HONEST-BOUND ship.

Pre-reg: preregs/2026-06-27_edge_importance_v3p2_trace_only_with_D1_audit_v1.md

Provenance (drill 2026-06-27 ANGLE 3):
  v3 ULTRA composition collapsed to ZERO coreness atoms (synthetic threshold
  too tight). v3.1 loosened ULTRA_COS 0.85 -> 0.70 + MIN_SIZE 5 -> 3 with
  synthetic selftest at sigma=0.02 passing (cosines 0.91+). REAL substrate
  atom geometry at ULTRA_COS=0.7 / MIN_SIZE=3 still produces coreness=0
  across all seeds -- real cluster geometry is LOOSER than the synthetic
  test predicted.

Per drill recommendation: SHIP HONEST-BOUND + ENSEMBLE rather than chase
ULTRA further. The D1 audit (exp_edge_importance_v3_D1_alternative_
discriminators_v1) showed TRACE-only alone hits D1_AUC=1.000 (perfect
partition-AUC ranking) with cv=0.000 across seeds. The ULTRAMETRIC
composition adds NO measured signal on real substrate.
...
PRIMARY DISCRIMINATORS (D1 audit from drill ANGLE 1):
  D1 = partition AUC of importance vs retrieved/unretrieved label
       (Mann-Whitney U; chain-grade reference)
  D2 = top-K precision at K=N_USE and K=50
  D3 = KM-proxy top-10 vs bot-10 quantile gap

PRE-REG BANDS (load-bearing; per drill):
...
# v3 source cell parses its own argv at import time and may sys.exit(0) on
...
# substrate. The setup also produces ultrametric_coreness which we IGNORE
# (drill: real geometry doesn't form tight clusters; ULTRA adds no signal).
...
# Restore argv after v3 import consumed it.
sys.argv = _SAVED_ARGV


ANCHOR_NAME = "edge_importance_v3p2_trace_only_with_D1_audit_v1"
_LLM_CALL_COUNTER = [0]
...
# N_QUERIES reduced (META_RULE_K discriminator-survives-scale).
if RUN_MODE == "smoke":
    N = N_FULL
    M_OLD = M_OLD_FULL
...
# ---------------------------------------------------------------------------
# Discriminator math (D1 / D2 / D3 -- copy from v3_D1_alternative cell)
# ---------------------------------------------------------------------------
...
def _selftest_auc_known_values() -> bool:
    ...
    assert abs(auc_inv - 0.0) < 1e-6, (
        f"AUC inverted: expected 0.0, got {auc_inv}"
    )
...
def _selftest_trace_only_no_ultra() -> bool:
    """v3.2 MUST be TRACE-only; no LAMBDA_LIST; no ULTRA arm."""
    # No LAMBDA_LIST defined in v3.2 module scope (intentional).
    assert "LAMBDA_LIST" not in globals(), (
        "v3.2 must NOT carry LAMBDA_LIST (ULTRA composition dropped)"
    )
    assert "ARM_ULTRAMETRIC_ONLY" not in [a for a in ARM_NAMES], (
        "v3.2 must NOT carry ULTRA arm"
    )
    assert "ARM_TRACE_X_CORENESS" not in [a for a in ARM_NAMES], (
        "v3.2 must NOT carry composition arm"
    )
    return True
...
def _instrumentation_selftest():
    ...
    print(
        ...
        f"arms=TRACE-only (ULTRA composition dropped per drill 2026-06-27)",
        flush=True,
    )
...
def run_arm(arm_name: str, seed: int, shared: Tuple) -> Dict:
    ...
    # Labels: atom is "positive" if it's in retrieved_idx.
...
def run_seed(seed: int) -> Dict:
    ...
    try:
        ...
        print(
            ...
            f"(coreness_atoms={coreness_count}; v3.2 ignores coreness)",
            flush=True,
        )
...
# ---------------------------------------------------------------------------
# Verdict (META_RULE_H + META_RULE_J + drill ANGLE 1 bands)
# ---------------------------------------------------------------------------
...
def compute_verdict(results: List[Dict]) -> Tuple[str, str]:
    ...
    # META_RULE_J no-silent-except: setup or arm exceptions = HARD_FAIL.
    for r in results:
    ...
    # Aggregate per arm across seeds (defensive .get to survive any silently-
    # dropped arm dict; non-finite guard below converts nan to HARD_FAIL).
    def _agg(arm_name: str) -> Dict[str, float]:
        ...
        kms = [float(a.get("D3_KM_quantile_gap_top10_bot10", float("nan")))
               for a in per]
        rec_retr = [float(a.get("recall_old_RETRIEVED", float("nan")))
                    for a in per]
        rec_unretr = [float(a.get("recall_old_UNRETRIEVED", float("nan")))
                      for a in per]
        cor = [float(a.get("cor_importance_magnitude", float("nan")))
               for a in per]
    ...
    # HARD_FAIL gates (per pre-reg)
    if trace_ndown <= 0:
        return ("HARD_FAIL",
                f"HARD_FAIL: TRACE mechanism inert (n_downscaled=0). "
                f"{summary}")
    if abs(trace_auc - rand_auc) < 0.05:
        return ("HARD_FAIL",
                f"HARD_FAIL: arms within 0.05 on D1_AUC "
                f"(spread={abs(trace_auc - rand_auc):.3f}). Regime "
                f"saturated. {summary}")
    if trace_cor >= 0.30:
        return ("HARD_FAIL",
    ...
    if trace_auc < 0.55:
        return ("HARD_FAIL",
                f"HARD_FAIL: TRACE D1_AUC={trace_auc:.3f} < 0.55 "
                f"(mechanism does NOT rank retrieved above unretrieved). "
                f"{summary}")

    # HARD_PASS per drill: D1_AUC >= 0.65, cv <= 0.05, lift >= 0.05.
    ...
    if all([hp_auc, hp_cv, hp_lift, hp_fired, hp_fair]):
        return ("HARD_PASS",
                f"HARD_PASS: TRACE-only at alpha={ALPHA:.3f} "
                f"D1_AUC={trace_auc:.3f} >= 0.65, cv={trace_cv:.3f} <= 0.05, "
                f"lift_over_random={trace_auc - rand_auc:+.3f} >= 0.05, "
...
if __name__ == "__main__":
    ...
    for seed in remaining:
        print(
            f"[seed={seed}] v3.2 N={N} alpha={ALPHA:.3f} "
            f"J_comp={N_COMPOSITE_QUERIES} arity={COMPOSITE_ARITY} "
            f"N_USE={N_USE} mode={RUN_MODE} arms=TRACE-only...", flush=True,
        )
    ...
    metrics = {
        ...
        "summary": (
            ...
            f"J_comp={N_COMPOSITE_QUERIES} arity={COMPOSITE_ARITY} "
            f"N_USE={N_USE} mode={RUN_MODE} arms=TRACE-only "
            f"N_PRUNE_FRAC={N_PRUNE_FRAC}"
        ),
