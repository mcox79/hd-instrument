"""
exp_privacy_fixes_cone_rank_entropy_v1 -- privacy combined fix (STACK cone-center + rotation) -- CPU.
...
ANCHOR_NAME = "privacy_combined_fix_v1"
...
def run() -> Dict:
    ...
    cc = real - real.mean(0); res["cone_only"] = zkl_recall(cc, sel, np.random.default_rng(2))
    gg = np.random.default_rng(99); Rrot = np.linalg.qr(gg.standard_normal((real.shape[1], real.shape[1])))[0].astype(np.float32)
    res["combined_cone_rot"] = zkl_recall(cc @ Rrot, sel, np.random.default_rng(3))                               # STACK: center then rotate
    for k in res:
