...
ANCHOR_NAME = "qa_self_knowledge_route_b_v2_cpu_v1"
...
def route_B_v2(relations, args, id2corpus, rel_present):
    """v2: bidirectional + explicit-rel-type (fallback RELATES if absent) + last-segment target match."""
    accept = {x.upper() for x in args["rel_types"]}
    # fallback: if NONE of the named rel-types exist in the corpus, add RELATES (the generic relational edge)
    if not (accept & rel_present):
        accept = accept | {"RELATES"}
