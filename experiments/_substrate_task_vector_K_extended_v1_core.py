"""Shared core for substrate_task_vector_K_extended_v1 sibling cells.

Extended-K probe of TASK_VECTOR HRR ICL K-cliff (Stage 3, M3 concern #4).

REFERENT: substrate_task_vector_K_cliff_phase_diagram_v3 (2026-06-28, MIDDLE_BAND).
v3 K axis {1,3,5,10,20,50,100,200} showed cliff at K~5-20 with tail
TV_top1_recall ~0.02-0.14 at K=200. v3 landed MIDDLE_BAND (cliff EXISTS but
bootstrap-unstable slice ordering).

THIS CELL EXTENDS K axis to {50, 100, 200, 500, 1000} to answer:
  Q1. Is the K=200 floor already Bernoulli-at-random (TV=RV=1/V_ENTS_POOL)?
      i.e., is the mechanism completely dead at K>=200, or does TV still exceed
      RV at K=500/1000?
  Q2. Cross-seed cv on TV_top1_recall at each K (target cv<10% in high-signal
      regime; extended-K/floor regime cv is DIAGNOSTIC not gate).

MECHANISM (inherited from v3 unchanged): TASK_VECTOR is the HRR-bundled
task vector of K bound (input, output) pairs sampled from ONE focal task
context, then unbind against a query input from the same context and pick
the top-1 nearest entity by cosine. RANDOM_VECTOR is identical bundle
structure with random OUTPUT permutation (control for TV structure
without task correlation). ORACLE is trivially 1.0.

MECHANISM-CLASS DIVERSION FROM v3: v3 measured PRECISION (n_queries 10->50
+ pooled + bootstrap CI); v1-extended measures REACH (K axis extended into
floor). Same primitives; different axis coverage.

USER SPEC (2026-06-30):
- K in {50, 100, 200, 500, 1000}
- 3 seeds {7, 13, 19}
- Cross-seed cv < 10% target (interpreted: at high-signal regime; floor
  regime cv is diagnostic, not gate — Bernoulli sqrt scaling)
- Discriminator: at what K does mechanism fail?
- CPU-eligible (numpy)
- Smoke seed_7 at K=200

AXES:
- K_VALUES = (50, 100, 200, 500, 1000)   # 5 K values (extended)
- V_TASKS  = 10                          # collapse V axis (v3 winner V=10)
- OVERLAP  = 0.0                         # collapse overlap axis (v3 winner ov=0.0)
- V_ENTS_POOL = 1000                     # LIFTED from v3's 200 to support K=1000
- N_QUERIES = 100                        # cv discipline (Bernoulli sqrt)
- N_DIM = 8192                           # inherited from v3
- ARMS = ('TASK_VECTOR','RANDOM_VECTOR','ORACLE')

Full run: 5 K x 3 arms x 100 queries = 1500 records per seed.
Smoke run: single point (K=200) x 3 arms x 100 queries = 300 records.

DISCRIMINATOR (smoke firing at K=200 seed_7): TV_top1_recall must be
< DISCRIMINATOR_SMOKE_FLOOR = 0.30. v3 seed_7 FULL measured 0.12 at
(K=200, V=10, ov=0.0) — this floor is empirically supported. Smoke runs at
FULL N_DIM (discriminator-must-survive-scale per USER 2026-06-26).

CV DISCIPLINE (META_RULE for extended-K cell):
- cv_across_seeds(K) < 0.10 required at K where mean(TV) >= 0.50 (high-signal)
- cv_across_seeds(K) is REPORTED but not gated at floor (Bernoulli count noise
  dominates; report is diagnostic).
Justification: for p=0.05, cv = sqrt(0.05*0.95/N)/0.05 = 0.62 at N=100;
  achieving cv<10% at floor requires N>>4000 queries per cell (not compute
  reasonable at this arc).

BAND DEFINITIONS (envelope-fail-bands, pre-reg-mirrored):
- HP_K1_FLOOR_RECALL     = 0.85   # TV at K=50 (smallest K in extended set,
                                    should still be near-saturation for V=10)
- HP_MECHANISM_FLOOR_RATIO = 0.30 # (TV - RV) at some K in the sweep
- HP_CV_HIGH_SIGNAL      = 0.10  # cv<10% at high-signal K's
- HF_ALL_FLOOR           = TV all < 0.10  # mechanism fully dead
- HF_RANDOM_BEATS_TV     = TV<RV at any K where V=10 (regime flip)
- DISCRIMINATOR_SMOKE_FLOOR = 0.30 at (K=200 smoke)
...
ASCII-only.
Author: exp_dev 2026-06-30 (Opus 4.7 1M, agent-spawn) v1 extended-K reach
"""
...
ANCHOR_PREFIX = "substrate_task_vector_K_extended_v1"

# Axes (LOCKED; extended-K reach)
K_VALUES = (50, 100, 200, 500, 1000)
V_TASKS = 10
OVERLAP = 0.0
V_ENTS_POOL = 1000
N_QUERIES_FULL = 100
N_QUERIES_SMOKE = 100  # discriminator-must-survive-scale (same n as full)
ARMS = ("TASK_VECTOR", "RANDOM_VECTOR", "ORACLE")

N_DIM_FULL = 8192
N_DIM_SMOKE = 8192  # discriminator-must-survive-scale (USER 2026-06-26)

SMOKE_K = 200  # single-point smoke per USER spec

# Bands (pre-reg-mirrored, LOCKED at module load)
...
HF_ALL_FLOOR = 0.10
DISCRIMINATOR_SMOKE_FLOOR = 0.30
...
def _run_phase_point(
    ...
    # Single focal task (V=10 is v3 winning V; simpler: sample ONE focal permutation)
    focal_perm = g.permutation(V_ents)
    focal_ctx = g.choice(V_ents, size=K_eff, replace=False)

    # Queries drawn from focal context
...
def run_one_seed_extended_K(seed: int, run_mode: str) -> Dict[str, Any]:
...
def aggregate_and_verdict(per_seed: Dict[str, Dict[str, Any]],
    ...
    # K=50 floor gate (smallest K in extended; should be near-saturation)
    ...
    if all_dead:
        ...
        verdict = "HARD_PASS"
        m3_msg = (f"extended-K discriminator FIRED: mechanism dies at K={K_of_mechanism_death}; "
                  f"cv<10% in high-signal regime; K=50 floor met")
    elif K50_ok and K_of_mechanism_death is not None:
        # Mechanism transition observed but cv gate slipped -> MB
        verdict = "MIDDLE_BAND"
        m3_msg = (f"extended-K discriminator OBSERVED at K={K_of_mechanism_death} but "
                  f"cv gate violated: {high_signal_cv_violations}")
    else:
        verdict = "MIDDLE_BAND"
        m3_msg = (f"extended-K partial: K50_ok={K50_ok} "
    ...
    return {
        ...
        "extended_K_v1_metric_constants": {
...
def smoke_discriminator_check(phase_map: List[Dict[str, Any]]) -> Tuple[bool, str]:
    ...
    if tv >= DISCRIMINATOR_SMOKE_FLOOR:
        return False, (f"smoke discriminator FAILED-TO-FIRE (mechanism still alive): "
                       ...
                       f"(mechanism not yet in cliff regime; extended-K reach premise questionable)")
    if tv < rv - 0.05:
...
def selftest(seed: int = 7) -> Tuple[bool, str]:
    """Tiny selftest: K=50 near-saturation + K=1000 in floor + agg pipeline."""
    try:
        body = run_one_seed_extended_K(seed, run_mode="selftest")
        pts = body.get("phase_map", [])
        if not pts:
            return False, "selftest: empty phase_map"
        low = next((p for p in pts if p["K"] == 50), None)
        hi = next((p for p in pts if p["K"] == 1000), None)
        ...
        # Loose bounds (n_queries=10 in selftest -> noisy; use wide margins)
        if tv_low < 0.20:
        ...
        if tv_hi > tv_low - 0.10:
            return False, (f"selftest: TV at K=1000 ({tv_hi:.3f}) not clearly below "
        ...
        # Aggregator pipeline end-to-end (single seed)
        ...
        msg = (f"selftest OK: TV(K=50)={tv_low:.3f}, TV(K=1000)={tv_hi:.3f}, "
