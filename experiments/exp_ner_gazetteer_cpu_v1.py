...
ANCHOR_NAME = "ner_gazetteer_cpu_v1"
...
def _emit_feats(words, i, tag):
    ...
    g = _GAZ.get(wl)
    if g is not None: fs.append("gaz_%d~%d" % (g, tag))                # gazetteer: word's dominant train tag
    gp = _GAZ.get(words[i - 1].lower()) if i > 0 else None
    if gp is not None: fs.append("pgaz_%d~%d" % (gp, tag))             # prev word's gazetteer
    gn = _GAZ.get(words[i + 1].lower()) if i + 1 < len(words) else None
    if gn is not None: fs.append("ngaz_%d~%d" % (gn, tag))
    return fs
def _spans(tags):
...
def _selftest():
    ...
    print("[selftest] PASS: ner-gazetteer", flush=True)
_selftest()
if _ARGS.self_test:
    sys.exit(0)
def run() -> Dict:
    rng = np.random.default_rng(int(os.environ.get("HDLAB_SEED", "1030")))
    try:
    ...
    for toks, tags in train:
        for w, t in zip(toks, tags): wtag[w.lower()][t] += 1
    GAZ = {w: c.most_common(1)[0][0] for w, c in wtag.items() if sum(c.values()) >= 2}  # word -> dominant train tag
    global _GAZ; _GAZ = GAZ
