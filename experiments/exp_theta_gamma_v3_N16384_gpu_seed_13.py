"""theta_gamma_v3_N16384_gpu sibling seed=13.

Theta-gamma phase binding v3 at N_DIM=16384. Extends v2's FHRR all-complex
codebook design from N=4096 to N=16384. Tests sequence encoding + order
binding at higher hyperdimensionality.
...
v3 delta vs v2:
  - N_DIM 4096 -> 16384 (4x hyperdim; per task-prompt spec "at 2x N")
  - K_SEQ sweep extended: FULL adds K=5000 (7 K-points); SMOKE spans full range
  - HP_LOG2_SEPARATION_FHRR_VS_CYCLIC threshold 0.3 -> 1.5 (per task-prompt)

5 arms (same as v2):
  - NO_POSITION, CYCLIC_SHIFT, FHRR_FLAT_PHASE_8, FHRR_FLAT_PHASE_32,
    FHRR_NESTED_THETA_GAMMA

Regime (LOCKED):
  N_DIM = 16384, ITEM_VOCAB = 10000, NOISE_SIGMA = 0.05
  K_SEQ_FULL = [50, 100, 200, 500, 1000, 2000, 5000]
  K_SEQ_SMOKE = [50, 200, 1000, 5000]

Cardinality (per seed):
  FULL : 5 * 7 = 35 phase points
  SMOKE: 5 * 4 = 20 phase points

PROT-018: anchor has _N16384 suffix (N_DIM axis explicit in name).
...
SEED = 13
ANCHOR_NAME = f"theta_gamma_v3_N16384_gpu_seed_{SEED}"
...
CONFIG_VERSION = (
    ...
    f"K_SEQ_full=[50,100,200,500,1000,2000,5000],"
    f"K_SEQ_smoke=[50,200,1000,5000],"
    f"N_DIM=16384,ITEM_VOCAB=10000,POSITION_NESTED=64,NOISE_SIGMA=0.05,"
    f"SEED={SEED},mode={RUN_MODE},"
    f"expected_n_full=35,expected_n_smoke=20,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v3_theta_gamma_N16384_gpu_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v3_theta_gamma_N16384_gpu_import_crash",
        }
        tmp = out_dir / "metrics.json.tmp"
        final = out_dir / "metrics.json"
...
def main() -> int:
    ...
    from experiments._substrate_theta_gamma_v3_N16384_gpu_core import (
    ...
    final["_hardening_marker"] = "v3_theta_gamma_N16384_gpu_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = (
        "synthetic_theta_gamma_v3_N16384_gpu_K_SEQ_phase_diagram"
    )
