"""
exp_t5c_pp225_multihop_3hop_1p4b_v1 -- Path B rescue R3: supervised projection head (bypass cross-attn) -- GPU.
...
ANCHOR_NAME = "t5c_pp225_multihop_3hop_1p4b_v1"; MODEL = "EleutherAI/pythia-1.4b"; ENCODER = "BAAI/bge-large-en-v1.5"
...
def _selftest():
    ...
    assert int(_n.argmax([0.1, 0.9])) == 1, "argmax"; print("[selftest] PASS: pp225-multihop-3hop-1p4b", flush=True)
...
def run() -> Dict:
    ...
    for i, s in enumerate(subs):
        ...
        facts.append({"prompt": "The secret code of the colleague of the manager of %s is" % s, "aid": tok(a, add_special_tokens=False)["input_ids"][0], "text": "The manager of %s is %s. The colleague of %s is %s. The secret code of %s is%s." % (s, m, m, b, b, a)})
