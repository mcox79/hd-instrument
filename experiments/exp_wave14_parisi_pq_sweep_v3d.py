...
Pre-reg: preregs/2026-05-21_wave14_parisi_pq_sweep_v3d.md
"""
...
def measure_codebook_across_N(codebook_type, Ns, M_mult, seeds, device):
    ...
    binders_by_N = {}
    KERDOCK_VALID_N = {1024, 4096}  # GF(2^5) + GF(2^6); GF(2^4)/GF(2^7) unregistered
    for N in Ns:
        if codebook_type == "kerdock" and N not in KERDOCK_VALID_N:
            continue  # Kerdock unavailable at this N
        M = M_mult * N
        seed_binders = []
        for seed in seeds:
    ...
    Ns_used = sorted(binders_by_N.keys())
    inv_N = [1.0 / N for N in Ns_used]
    binder_vals = [binders_by_N[N] for N in Ns_used]
    if len(Ns_used) >= 2:
        slope, intercept = linear_fit(inv_N, binder_vals)
    else:
        slope, intercept = 0.0, (binder_vals[0] if binder_vals else 0.0)
...
def run_experiment(smoke):
    ...
    config = {"mode": "smoke" if smoke else "full",
              "Ns": [1024, 4096] if smoke else [1024, 4096, 16384],  # GF(2^4)=256 not registered, drop
              "M_mult": 2,
...
def run_smoke():
    out_dir = get_output_dir("wave14_parisi_pq_sweep_v3d_smoke")
...
def run_main():
    out_dir = get_output_dir("wave14_parisi_pq_sweep_v3d")
