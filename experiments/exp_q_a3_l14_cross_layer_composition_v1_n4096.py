"""
q_a3_l14_cross_layer_composition_v1_n4096 -- Q-A3: L=14 cross-layer composition at N=4096.

Extends L=13 to L=14. Ceiling chase for compositional depth.
  L=13 HARD_PASS this cycle (all fids EXACT-1.0, per cycle-14 batch verdict).
  Architecture adds one more Hadamard binding level.
  ...
  L10: M_mid10=2. L11: M_mid11=2. L12: M_mid12=2. L13: M_mid13=2.
  L14: M_outer=2, Hadamard binding ctx14*L13.

  14 W matrices x ~67 MB each = ~938 MB total. Safe on 8 GB GPU.

PRE-REGISTERED BANDS (ceiling push, all prior L EXACT-1.0 through L=13):
  HARD-PASS: all 14 level fidelities = 1.0000 unanimous (5/5 seeds).
  MIDDLE: any L_fid in [0.85, 1.0).
  HARD-FAIL: any L_fid < 0.85 OR l14_acc < 0.5.

FORMULA SELF-TESTS:
  1. L=14 chain: 14-step Hadamard roundtrip recovers xi_L1.
     [INPUT: 2-element +-1 vectors, 13 context ops] [EXPECTED: decode = xi_L1]
...
ANCHOR_NAME = "q_a3_l14_cross_layer_composition_v1_n4096"
...
def _selftest_l14_chain():
    """14-step Hadamard roundtrip recovers xi_L1."""
    ctxs = [torch.tensor([1.0, -1.0], dtype=torch.float32, device=DEVICE),
            ...
            torch.tensor([1.0, 1.0], dtype=torch.float32, device=DEVICE),
            torch.tensor([-1.0, -1.0], dtype=torch.float32, device=DEVICE),
            torch.tensor([1.0, -1.0], dtype=torch.float32, device=DEVICE)]  # ctx2..ctx14
    ...
    for c in reversed(ctxs):
        xi_dec = xi_dec * c
    assert torch.allclose(xi_dec, xi_L1, atol=1e-6), f"L14 chain decode failed: {xi_dec}"
...
def _instrumentation_selftest():
    ...
    ms = [M_INNER, M_MID2, M_MID3, M_MID4, M_MID5, M_MID6, M_MID7, M_MID8,
          M_MID9, M_MID10, M_MID11, M_MID12, M_MID13, M_OUTER]
    ...
    del dummy
    print(f"[selftest] PASS: L14 chain decode ok, capacity_ok, gpu_mem_ok N={n_dim}", flush=True)
...
def run_seed(seed: int, n_dim: int) -> Dict:
    ...
    Xi_ctx14 = bsc(M_OUTER, n_dim)
    Xi_outer = Xi_ctx14 * Xi_mid13[:M_OUTER]
    ...
    for q_idx in range(min(N_QUERIES, M_OUTER)):
        ...
        xi_L14_ret = hopfield_retrieve_gpu(W_outer, probe)
        ...
        level_fids["L14"].append(cosine_sim_gpu(xi_L14_ret, xi_outer_true))
        ...
        l14_acc.append(cosine_sim_gpu(xi_L1_ret, xi_L1_true))

    mean_fids = {k: float(sum(v) / len(v)) if v else 0.0 for k, v in level_fids.items()}
    mean_l14 = float(sum(l14_acc) / len(l14_acc)) if l14_acc else 0.0
    ...
    print(f"  [seed={seed}] fids={' '.join(f'{k}:{mean_fids[k]:.4f}' for k in sorted(mean_fids.keys()))} "
          f"l14_acc={mean_l14:.4f} peak_gpu={peak_mem:.3f}GB elapsed={elapsed:.2f}s", flush=True)

    result = {
        "seed": seed, "N": n_dim, "run_mode": RUN_MODE,
        "l14_accuracy": float(mean_l14),
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    l14_acc = mean_key("l14_accuracy")

    fid_list = [fid_vals[f"L{l}"] for l in range(1, L_DEPTH + 1)]
    summary = (" ".join(f"L{l}:{fid_vals[f'L{l}']:.4f}" for l in range(1, L_DEPTH + 1)) +
               f" l14_acc={l14_acc:.4f}(HP>={HP_LACC} HF<{HF_LACC}) n_seeds={len(results)}")

    if any(f < HF_FIDELITY for f in fid_list) or l14_acc < HF_LACC:
        return ("HARD_FAIL", f"HARD_FAIL: any fid<{HF_FIDELITY} or l14_acc<{HF_LACC}. {summary}")

    all_unanimous = all(f >= HP_FIDELITY_UNANI_THRESH for f in fid_list)
    if all_unanimous and l14_acc >= HP_LACC:
        return ("HARD_PASS", f"HARD_PASS: all {L_DEPTH} levels EXACT-1.0 unanimous + l14_acc HP. {summary}")
...
metrics = {
    ...
    "per_seed": [
         ...
         **{f"fid_l{l}": r.get(f"fid_l{l}") for l in range(1, L_DEPTH + 1)},
         "l14_accuracy": r.get("l14_accuracy"),
