"""
exp_patternb_composition_1M_gpu_v1.py -- Pattern-B role-filler composition recall at V=1M filler codebook (GPU) -- GPU.

ROUTING: GPU-scale substrate-physics (Pattern-B composition at 1M vocab). K role-filler pairs bundled; recover each filler by unbind + cleanup over a V=1M filler codebook (production KB scale). GPU enables the 1M-row cleanup matmul. torch.cuda. GPU.
PRE-REGISTERED: HARD-PASS recall@1 >= 0.95 at K=4 with V=1M. MIDDLE >= 0.85. HARD-FAIL < 0.85.
...
ANCHOR_NAME = "patternb_composition_1M_gpu_v1"
RUN_MODE = ("smoke" if "--smoke" in sys.argv else os.environ.get("HDLAB_RUN_MODE", "full")).lower()
_ap = argparse.ArgumentParser(); _ap.add_argument("--smoke", action="store_true"); _ap.add_argument("--self-test", action="store_true"); _ARGS, _ = _ap.parse_known_args()
SMOKE = RUN_MODE == "smoke"

def _selftest():
    import numpy as _n; assert _n.argmax([0.1, 0.9]) == 1, "argmax"; print("[selftest] PASS: patternb_composition_1M_gpu_v1", flush=True)
_selftest()
if _ARGS.self_test:
    sys.exit(0)
try:
