"""K2 4-stage continual learning M1 rescue: hierarchical sub-task chunk replay.

Context: K2 4-stage continual learning (A->B->C->D) is at 🟡 PARTIAL with
retA~0.74 floor. All 3 in-design tuning axes (N-scaling, Phase-A consolidation,
Phase-D A-weighted replay) saturated at the same floor (v189, v190, v193).
The mechanism-class rescue M1 (hierarchical replay) is the first untested path
out of MIDDLE band per v193 rehab axis list.

M1 hypothesis: replay sub-task CHUNKS (windows of CHUNK_SIZE tokens) rather
than entire prior-task sequences in the replay pool. The existing pipeline
builds a pool over the entire prior-phase training sequence. M1 instead
partitions each prior phase into chunk_size-length windows and samples from
those chunks. This mimics hierarchical/episodic replay (brain-inspired: hippocampal
replay of episodes, not full sequences), potentially reducing gradient interference
between task-level outer-product directions.

Implementation: same 4-stage A->B->C->D pipeline as betB_4stage_continual_v1
BUT the replay pool is built from CHUNK_SIZE-length non-overlapping windows
(chunk_pool) across each prior phase's training data, and the pool is sampled
uniformly from these chunks during training, not from the full sequence.
...
Queue: overnight_queue (GPU, 4-stage continual learning, torch, 5 seeds x 4 phases).
ETA: ~4-5 hours on GPU.
Pre-reg file: preregs/2026-05-24_wave14_k2_m1_hierreplay_v1.md
"""
