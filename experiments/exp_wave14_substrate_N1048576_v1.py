"""Substrate at N=1048576 — Strategy 09:35 P3. 16x beyond V2.D."""
...
def compute_verdict(s):
    if "smoother_acc" not in s: return ("N1M_INCONCLUSIVE", "Missing.")
    a = s["smoother_acc"]
    if a >= 0.5: return ("N1M_SCALES", f"acc={a:.3f}>=0.5; 16x beyond V2.D at FULL.")
    if a >= 0.3: return ("N1M_PARTIAL", f"acc={a:.3f} in [0.3, 0.5).")
    return ("N1M_KILLED", f"acc={a:.3f}<0.3.")


def self_test_verdict():
    for s,exp in [({"smoother_acc":0.8},"N1M_SCALES"),({"smoother_acc":0.4},"N1M_PARTIAL"),({"smoother_acc":0.2},"N1M_KILLED"),({},"N1M_INCONCLUSIVE")]:
...
def run_experiment(smoke):
    ...
    cfg = {"N":65536 if smoke else 1048576, "K":100, "depth":25 if smoke else 50,
    ...
    print(f"  N={cfg['N']}: acc={acc:.3f} from {cfg['n_trials']} trials", flush=True)
    summary = {"smoother_acc": acc, "N": cfg["N"], "n_trials": cfg["n_trials"]}
...
def run_smoke():
    out_dir = get_output_dir("wave14_substrate_N1048576_v1_smoke")
...
def run_main():
    out_dir = get_output_dir("wave14_substrate_N1048576_v1")
