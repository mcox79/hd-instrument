"""ADVERSARIAL AQSIM x PATH D x COMPRESSION 3-WAY COMPOSITION v5 K=2 at N=16384.

CONTEXT (v4 OOMed at N=16384 K_paths=100 M=8192; v5 fixes via K=2 + M=4096):
  v4 OOM root cause: Path D K_paths=100 x M=8192 x N=16384 peak memory exceeded
  8 GB GPU (needed ~7+ GB, available ~0.89-1.88 GB free at runtime).
  PROT-022 log2-parity hypothesis FALSIFIED -- the actual failure mode is GPU
  memory cost at large N, not codebook precondition.

  v2 (N=4096 K=100) HARD_PASS: 5/5 seeds unanimous.
  K2PROD v1 (N=4096 K=2) HARD_PASS: 5/5 seeds unanimous.
  v5 extends BOTH wins: K=2 production op-point tested at N=16384.

STRATEGIC WINS COMBINED:
  (1) K=2 production validated (K2PROD HARD_PASS, cap_map v309): K=2 works
      end-to-end on the 3-way stack at N=4096 5/5 unanimous.
  (2) Cross-N validation still needed: compositional sub-row caveat "N=4096 only"
      remains open -- v3/v4 both failed to close it (INFRA_FAILURE + OOM).

  v5 = K=2 production stack at N=16384. Closes both in one anchor.

MEMORY BUDGET AT v5 CONFIG:
  W_base: N * M = 16384 * 4096 * 4 bytes = 268 MB (float32). OK.
  Path D K=2: only 2 candidate paths per query -- tiny vs K=100.
  Estimated peak: ~1.0-1.5 GB (well under 6 GB threshold).
  Actual peak verified in instrumentation selftest (smoke print).

PRODUCTION STACK (K=2 version of v2):
  (1) c_quant/bits8 compressed W
  (2) Path D depth=5 on compressed W, K_paths=2
  (3) a_query_sim defense gate (reject sim < 0.5)
  (4) 90/10 adversarial/legitimate interleave, subthreshold probes alpha=0.45

COMPOSITIONAL QUESTION (at N=16384 with K=2):
  (1) defense_activation_rate >= 0.90?
  (2) path_d_acc_gated_compressed >= 0.95?
  (3) compression delta < 5pp (|acc_gated_comp - acc_gated_uncomp| <= 0.05)?

  If YES to all three across 4/5+ seeds: cross-N caveat lifts from "N=4096 only"
  to "N=4096 + N=16384". Compositional sub-row promotion 0.75-0.90 -> 0.80-0.95.

PRE-REGISTERED BANDS:
  HP = defense_activation_rate >= 0.90
       AND path_d_acc_gated_compressed >= 0.95
       AND |acc_gated_comp - acc_gated_uncomp| <= 0.05
       in 4/5+ seeds (unanimous preferred).
  ...
  MIDDLE = characterizable degradation (one seed barely fails one metric).

DESIGN:
  N=16384, M=4096 (M/N=0.25), depth=5, K_paths=2, 5-seed.
  Seeds: [7, 17, 23, 31, 41] (standard), device=cuda.
  n_adv=90, n_leg=10 (90/10 ratio as v4 -- defense pressure unchanged).

PROT-018: _n16384 binds N = 16384.
PROT-019: timeout_s >= 21600 (PROT-018 _n16384 floor per role contract).
PROT-020: device=cuda (GPU queue: overnight_queue).
PROT-021: per-seed checkpointing.
HDLAB_EXP_NAME=7d39e13.

OOM CHECK:
  W_base: 16384 * 4096 * 4 = 268,435,456 bytes = ~256 MB. Far below 6 GB.
  K_paths=2 path storage: ~negligible vs K=100.
  Total estimated peak: ~1.0-1.5 GB. Well under 6 GB threshold.

TIMEOUT ESTIMATE:
  Reference: K2PROD v1 N=4096 elapsed ~30s smoke -> ~300s for 5 seeds.
  K=2 is faster than K=100 (fewer candidates). N scales 16384/4096=4x.
  Matrix ops: O(N*M) not O(N^2); scaling_exp=1.5 (vector ops).
  Estimate: ceil(1.5 * 60 * (16384/4096)^1.5 * 5) = ceil(1.5*60*8*5) = 3600s.
  PROT-019 _n16384 floor: 21600s. timeout_s = 21600.
  Flagged as long run (> 2h = 7200s floor) but PROT-019 requires this floor.

FORMULA SELF-TESTS:
  1. N=16384 log2=14 (EVEN) -> Kerdock OK. No PROT-022 override needed.
  2. M/N = 4096/16384 = 0.25 (halved vs v4's 0.5 to free VRAM).
  3. OOM: W float32 peak ~256 MB << 6 GB threshold.
  4. Compression ratio = 4.0 (float32 -> INT8 storage).
  5. K=2 < K=100: path_d_run explores 1 positive + 1 decoy candidate.
  6. alpha=0.45 < 0.50 threshold -> defense fires on adversarial probes.
  7. Verdict gates HP/HF/MB/STILL_UNNECESSARY correct.

Anchor: adversarial_aqsim_path_d_compose_v5_k2_n16384
Queue: overnight_queue (GPU)
Pre-reg: preregs/2026-06-01_adversarial_aqsim_path_d_compose_v5_k2_n16384.md
...
_ck_spec = importlib.util.spec_from_file_location("_seed_ckpt_aqsim3w_v5k2", _ck_path)
...
# M/N = 0.25 (halved vs v4's M/N=0.5 to reduce VRAM at large N)
M_PROD   = 4096
M_SMOKE  = 256
DEPTH    = 5
K_PATHS  = 2  # K=2 production op-point (key delta vs v4's K=100)

# 90/10 ratio (defense pressure unchanged from v2/v3/v4)
N_ADV_FULL   = 90
N_LEG_FULL   = 10
N_ADV_SMOKE  = 9
N_LEG_SMOKE  = 2
...
# Pre-registered thresholds (same gates as v2 + K2PROD v1)
...
def get_output_dir(default_name: str = "adversarial_aqsim_path_d_compose_v5_k2_n16384") -> Path:
...
def measure_seed(N_use: int, M: int, depth: int, K_paths: int,
    ...
    """3-way composition: c_quant/bits8 + Path D(K=2) + a_query_sim at N=16384.
...
def compute_verdict(cells: List[Dict]) -> Tuple[str, str]:
    if not cells:
        return ("AQSIM3W5K2_INCONCLUSIVE", "no cells")
    ok = [c for c in cells if c.get("ok")]
    if not ok:
        return ("AQSIM3W5K2_INCONCLUSIVE", f"all {len(cells)} cells failed")
    ...
    detail = (
        f"N=16384 K_paths={K_PATHS} M/N=0.25 "
    ...
    if all_def_near_zero:
        return ("AQSIM3W5K2_DEFENSE_STILL_UNNECESSARY",
                "ADVERSARIAL_CONSTRUCTION_INVALID: all def_act < 0.10. " + detail)
    ...
    if n_hp >= HP_MIN_SEEDS:
        return ("AQSIM3W5K2_HARD_PASS",
                f"CROSS_N_16384_K2_VALIDATED n_hp={n_hp}/{len(ok)}. " + detail)
    if is_hf:
        return ("AQSIM3W5K2_HARD_FAIL",
                f"CROSS_N_16384_K2_FAILS n_def_fail={n_def_fail} "
                f"n_path_fail={n_path_fail} n_cells={len(ok)}. " + detail)
    return ("AQSIM3W5K2_MIDDLE_BAND",
            f"CROSS_N_16384_K2_PARTIAL n_hp={n_hp}/{len(ok)}. " + detail)


def _instrumentation_selftest() -> None:
    """Assert all claimed metrics non-null/non-sentinel at smoke scale.

    Also verifies VRAM peak at smoke scale and prints it for manual gate check.

    Formula self-tests:
    1. N=16384 log2=14 (EVEN) -> Kerdock OK.
    2. M/N = 4096/16384 = 0.25 (halved vs v4).
    3. OOM: W float32 peak ~256 MB << 6 GB threshold.
    4. Compression ratio = 4.0 (float32 -> INT8).
    5. K=2: path_d_run uses 1 positive + 1 decoy.
    6. alpha=0.45 < 0.50 threshold -> defense fires.
    7. Verdict gates HP/HF/MB/STILL_UNNECESSARY correct.
    8. Live smoke: all metrics non-null/non-sentinel; VRAM peak printed.
    """
    assert N_FULL == 16384, "PROT-018: _n16384"
    assert K_PATHS == 2, f"K_PATHS must be 2 (K=2 production); got {K_PATHS}"
    ...
    assert abs(ratio_mn - 0.25) < 1e-9, f"M/N = {ratio_mn:.4f} (expected 0.25)"
    print(f"[selftest] formula-2 M/N={ratio_mn:.2f} (halved vs v4 M/N=0.5) PASS", flush=True)

    # Formula self-test 3: OOM estimate
    W_bytes = N_FULL * M_PROD * 4  # float32
    assert W_bytes < 6 * 1024**3, f"W_base {W_bytes/1024**3:.2f} GB exceeds 6 GB limit"
    print(f"[selftest] formula-3 W_base={W_bytes/1024**2:.0f}MB (~256MB) << 6GB PASS",
          flush=True)
    ...
    print(f"[selftest] formula-4 compression max_err={max_err:.4f} PASS", flush=True)

    # Formula self-test 5: K=2 path exploration
    assert K_PATHS == 2, f"K_PATHS must be 2"
    print(f"[selftest] formula-5 K_PATHS={K_PATHS} (1 positive + 1 decoy) PASS", flush=True)

    # Formula self-test 6: subthreshold probe
    N_test = 512
    ...
    print(f"[selftest] formula-6 probe max_sim={max_sims.mean().item():.4f} "
    ...
    print("[selftest] verdict gates HP/HF/STILL_UNNEC PASS", flush=True)

    # Live smoke on CPU with VRAM peak tracking (CPU reports 0, GPU will show real value)
    ...
    print(f"[selftest] adversarial_aqsim_path_d_compose_v5_k2_n16384 PASS "
          f"N={N_SMOKE} M={M_SMOKE} K={K_PATHS} "
          ...
          f"peak_vram_mb={out['peak_vram_mb']:.1f}",
          flush=True)

    # Multi-scale smoke: N_SMOKE x4 to check no degeneration
    out4x = measure_seed(N_SMOKE * 4, M_SMOKE * 4, DEPTH, K_PATHS,
                          N_LEG_SMOKE, N_ADV_SMOKE, 17, device)
    assert out4x["ok"], f"selftest 4x-smoke failed: {out4x.get('error')}"
    assert out4x["defense_activation_rate"] >= 0.80, (
        f"4x smoke def_act={out4x['defense_activation_rate']:.3f} < 0.80")
    print(f"[selftest] 4x-smoke N={N_SMOKE*4} M={M_SMOKE*4} "
          f"def_act={out4x['defense_activation_rate']:.3f} "
          f"acc_gated_comp={out4x['acc_path_d_gated_compressed']:.3f} "
          f"peak_vram_mb={out4x['peak_vram_mb']:.1f} PASS",
          flush=True)
...
def main() -> None:
    ...
    t0      = time.time()
    print(f"[run] adversarial_aqsim_path_d_compose_v5_k2_n16384 smoke={smoke} "
          f"N={N_cfg} M={M} M/N={M/N_cfg:.3f} depth={DEPTH} K_paths={K_PATHS} "
          f"n_adv={n_adv} n_leg={n_leg} seeds={seeds} "
          f"done={len(done)} device={device.type} "
          f"[CROSS-N v5 K=2: bits8_compression + Path_D_K2 + a_query_sim_defense "
