"""
q_a3_l19_cross_layer_composition_v1_n4096 -- Q-A3: L=19 cross-layer composition at N=4096.

Ceiling chase: extends L=18 (HARD_PASS EXACT-1.0 unanimous 5-seed) to L=19.
Architecture adds one more Hadamard binding level (L19 outer layer).
19 W matrices x ~67 MB each = ~1273 MB total. Fits in 8 GB GPU.
Note: M per stage halves each level per isochoric design; alpha_c constraint ensures validity.
At L=19 M_OUTER=2, M_MID18=2, ..., M_MID7=2 (floored at 2 from L=7+).

PRE-REGISTERED BANDS (ceiling push, prior L=18 EXACT-1.0 unanimous):
  ...
  HARD-FAIL: any L_fid < 0.85 OR l19_acc < 0.5. Ceiling found at L=19.
...
QUEUE: overnight_queue (GPU; 19 W matrices at N=4096 ~1273 MB).
"""
import sys
...
ANCHOR_NAME = "q_a3_l19_cross_layer_composition_v1_n4096"
...
def _instrumentation_selftest():
    ...
    del dummy
    print(f"[selftest] PASS: L19 chain decode ok, capacity_ok, gpu_mem_ok N={n_dim}", flush=True)
...
def run_seed(seed: int, n_dim: int) -> Dict:
    ...
    Xi_ctx19 = bsc(M_OUTER, n_dim)
    Xi_outer = Xi_ctx19 * Xi_mid18[:M_OUTER]
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    if all_unanimous and l19_acc >= HP_LACC:
        return ("HARD_PASS", f"HARD_PASS: all {L_DEPTH} levels EXACT-1.0 unanimous + l19_acc HP. {summary}")
