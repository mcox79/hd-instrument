"""
q_a3_l12_cross_layer_composition_v1_n4096 -- Q-A3: L=12 cross-layer composition at N=4096.

Extends L=11 to L=12. Tests substrate compositional ceiling at depth-12.
  L=10 was HARD_PASS with all 10 level fidelities = 1.0000 (N=4096 5-seed).
  L=11 expected HARD_PASS pending result; L=12 ships in parallel to map ceiling faster.
...
Architecture:
  ...
  L10: M_mid10=2. L11: M_mid11=2. L12: M_outer=2, Hadamard binding ctx12*L11.

  12 W matrices x ~67 MB each = ~804 MB total. Safe on 8 GB GPU.

PRE-REGISTERED BANDS (ceiling push from L=10 EXACT-1.0):
  HARD-PASS: all 12 level fidelities = 1.0000 unanimous (5/5 seeds).
  MIDDLE: any L_fid in [0.85, 1.0).
  HARD-FAIL: any L_fid < 0.85 OR l12_acc < 0.5.

FORMULA SELF-TESTS:
  1. L=12 chain: 12-step Hadamard roundtrip recovers xi_L1.
...
ANCHOR_NAME = "q_a3_l12_cross_layer_composition_v1_n4096"
...
def _selftest_l12_chain():
    """12-step Hadamard roundtrip recovers xi_L1."""
    ctxs = [torch.tensor([1.0, -1.0], dtype=torch.float32, device=DEVICE),
            ...
            torch.tensor([1.0, -1.0], dtype=torch.float32, device=DEVICE),
            torch.tensor([-1.0, 1.0], dtype=torch.float32, device=DEVICE),
            torch.tensor([1.0, 1.0], dtype=torch.float32, device=DEVICE)]  # ctx2..ctx12
    ...
    for c in reversed(ctxs):
        xi_dec = xi_dec * c
    assert torch.allclose(xi_dec, xi_L1, atol=1e-6), f"L12 chain decode failed: {xi_dec}"


def _instrumentation_selftest():
    _selftest_l12_chain()
    n_dim = N_ACTIVE
    ms = [M_INNER, M_MID2, M_MID3, M_MID4, M_MID5, M_MID6, M_MID7, M_MID8, M_MID9, M_MID10, M_MID11, M_OUTER]
    ...
    del dummy
    print(f"[selftest] PASS: L12 chain decode ok, capacity_ok, gpu_mem_ok N={n_dim}", flush=True)
...
def run_seed(seed: int, n_dim: int) -> Dict:
    ...
    Xi_ctx12 = bsc(M_OUTER, n_dim)
    Xi_outer = Xi_ctx12 * Xi_mid11[:M_OUTER]
    ...
    for q_idx in range(min(N_QUERIES, M_OUTER)):
        ...
        xi_L12_ret = hopfield_retrieve_gpu(W_outer, probe)
        ...
        level_fids["L12"].append(cosine_sim_gpu(xi_L12_ret, xi_outer_true))
        ...
        l12_acc.append(cosine_sim_gpu(xi_L1_ret, xi_L1_true))

    mean_fids = {k: float(sum(v) / len(v)) if v else 0.0 for k, v in level_fids.items()}
    mean_l12 = float(sum(l12_acc) / len(l12_acc)) if l12_acc else 0.0
    ...
    print(f"  [seed={seed}] fids={' '.join(f'{k}:{mean_fids[k]:.4f}' for k in sorted(mean_fids.keys()))} "
          f"l12_acc={mean_l12:.4f} peak_gpu={peak_mem:.3f}GB elapsed={elapsed:.2f}s", flush=True)

    result = {
        "seed": seed, "N": n_dim, "run_mode": RUN_MODE,
        "l12_accuracy": float(mean_l12),
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    l12_acc = mean_key("l12_accuracy")

    fid_list = [fid_vals[f"L{l}"] for l in range(1, L_DEPTH + 1)]
    summary = (" ".join(f"L{l}:{fid_vals[f'L{l}']:.4f}" for l in range(1, L_DEPTH + 1)) +
               f" l12_acc={l12_acc:.4f}(HP>={HP_LACC} HF<{HF_LACC}) n_seeds={len(results)}")

    if any(f < HF_FIDELITY for f in fid_list) or l12_acc < HF_LACC:
        return ("HARD_FAIL", f"HARD_FAIL: any fid<{HF_FIDELITY} or l12_acc<{HF_LACC}. {summary}")

    all_unanimous = all(f >= HP_FIDELITY_UNANI_THRESH for f in fid_list)
    if all_unanimous and l12_acc >= HP_LACC:
        return ("HARD_PASS", f"HARD_PASS: all {L_DEPTH} levels EXACT-1.0 unanimous + l12_acc HP. {summary}")
...
metrics = {
    ...
    "per_seed": [
         ...
         **{f"fid_l{l}": r.get(f"fid_l{l}") for l in range(1, L_DEPTH + 1)},
         "l12_accuracy": r.get("l12_accuracy"),
