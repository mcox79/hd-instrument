"""
exp_t5c_hybrid_1p4b_fp32_kb10k_v1 -- CYCLE_204 Tier-3 HYBRID: every-layer Flamingo (LM gain) + PP-225 projection head (fact recall) -- GPU.
...
ANCHOR_NAME = "t5c_hybrid_1p4b_fp32_kb10k_v1"; MODEL = "EleutherAI/pythia-1.4b"; ENCODER = "BAAI/bge-large-en-v1.5"
...
def _selftest():
    ...
    print("[selftest] PASS: hybrid-1p4b-fp32-kb10k", flush=True)
...
def run() -> Dict:
    ...
    gates = [adapters[str(L)].gate for L in LAYERS]
    ...
    for step in range(STEPS):
        ...
        if step % CKPT_EVERY == 0:
            ...
            row = {"step": step, "of": STEPS, "lm_ratio": round(r_ppl / base_ppl, 4), "heldout_fact_recall": round(rec, 3), "gate0": round(float(torch.tanh(adapters[str(LAYERS[0])].gate)), 4), "elapsed_s": round(time.time() - t0, 1)}
