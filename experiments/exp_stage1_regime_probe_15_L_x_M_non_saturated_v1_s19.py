"""stage1_regime_probe_15_L_x_M_non_saturated_v1 sibling seed=19.

L (chain-depth) x M (codebook size) CROSS-TERM at cliff-adjacent SHARDED FHRR.
Tests whether M-sweep behavior is L-conditional at cliff-adjacent, or whether
M-sweep CG_META atom (5th physical law, established at fixed L=2) generalizes
across L values. Frame as REGIME-EXTENSION not new axis discovery.
Prior probes: P6v2/P8 fixed L=2 while varying F/M; P12 established L axis;
P14 (L x F) HOLD_PENDING_FULL; P13 (L x CLEANUP) HOLD_PENDING_FULL.
Skunkworks atom #48 addendum flagged "L cross-terms (LxN, LxF, LxM, Lxcorr)
unmapped." P15 fills the last unmapped L cross-term.

Sweep FULL: 3 L x 3 M (CLIFF) + 2 L x 2 M (DEEP_SAT) + 1 PC = 14 pts/seed.
Sweep SMOKE: 3 L x 3 M (CLIFF) + 2 L x 1 M (DEEP_SAT) + 1 PC = 12 pts.

Hypotheses (interaction_metric on CLIFF arm; MM_TENTATIVE at SMOKE at most):
  H1: cliff_interaction_metric >= 0.10 WITH ALL CELLS IN-BAND -> L x M
      cross-term fires (M-sweep is L-conditional); REGIME-EXTENSION.
  H2: cliff_interaction_metric < 0.05 -> L x M orthogonal (additive);
      M-sweep CG_META atom holds at other L values (valuable NULL finding).
...
CARDINALITY_OK_FULL: 14 phase points per seed
...
SEED = 19
ANCHOR_NAME = f"stage1_regime_probe_15_L_x_M_non_saturated_v1_s{SEED}"
...
CONFIG_VERSION = (
    ...
    f"regime=sharded_FHRR_chain_composition_L_x_M_cross_term_cliff_plus_deep_sat_null,"
    f"mech=modern_hopfield,"
    f"L_cliff=[1,2,4],M_cliff=[3200,6400,12800],"
    f"L_deep=[1,4],M_deep=[800,1600],"
    f"CLIFF=(N=512,F=1,corr=0.85,SHARDED),"
    f"DEEP_SAT=(N=8192,F=1,corr=0.60,SHARDED),"
    ...
    f"H1_thresh=(cliff_interaction_metric>=0.10 AND ceiling_confounded==0),"
    f"H2_thresh=(cliff_interaction_metric<0.05),"
    f"H3_null_thresh=(deep_interaction_metric<0.05),"
    f"expected_n_full=14,expected_n_smoke=12,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "stage1_regime_probe_15_L_x_M_non_saturated_v1_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: Exception) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "stage1_regime_probe_15_L_x_M_non_saturated_v1_import_crash",
        }
        tmp = out_dir / "metrics.json.tmp"
        final = out_dir / "metrics.json"
...
def main() -> int:
    ...
    from experiments._stage1_regime_probe_15_L_x_M_non_saturated_v1_core import (
    ...
    final["_hardening_marker"] = "stage1_regime_probe_15_L_x_M_non_saturated_v1_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = "synthetic_sharded_fhrr_chain_composition_L_x_M_cross_term_v1"
