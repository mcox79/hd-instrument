"""R10 extension: K=128, K=256 multi-seed. Does the gap keep growing or peak?

K=4 -> +0.007 pre / -0.115 post
K=16 -> +0.024 pre / +0.008 post (n=5, t>5)
K=32 -> +0.029 pre / +0.048 post (n=5, t=29)
K=64 -> +0.056 pre / +0.106 post (n=5, t=61)

Monotone in K so far. Does this continue at K=128, K=256, or does it peak?

Per the M1/M4 independent-noise-variance-reduction story, the gap should
keep growing until the retrieval kernel becomes too noisy to anchor at all
(at which point the concept channel also degrades). So we'd expect peak
then decline.
"""
...
def main():
    _say(f"R10 K=128, K=256 multi-seed extension")
    ...
    for K in K_LEVELS:
        ...
        _say(f"\n  K={K} stats: pre mean={mean_pre:+.4f} t={mean_pre/se_pre if se_pre>0 else 0:.2f}  "
             f"post mean={mean_post:+.4f} t={mean_post/se_post if se_post>0 else 0:.2f}")

    _say(f"\n========= MONOTONE CHECK =========")
    for K in K_LEVELS:
        gaps = [r["gap_post"] for r in all_results[K]]
        mean = sum(gaps) / len(gaps)
        _say(f"  K={K:3d}: post mean={mean:+.4f}")

    out_dir = Path(__file__).resolve().parent.parent / "data" / "exp_wave14b_r10_K128_K256_multiseed"
