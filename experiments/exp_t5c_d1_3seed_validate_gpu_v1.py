"""
exp_t5c_c1_multilayer_flamingo_train_gpu_v1 -- Tier-5c Phase D: Qwen-2.5-1.5B Flamingo adapter continued training (LOCAL 4060 Ti) -- GPU.
...
ANCHOR_NAME = "t5c_d1_3seed_validate_gpu_v1"; MODEL = "Qwen/Qwen2.5-1.5B-Instruct"; LAYERS = [12, 13]
CKPT_EVERY = 500; ACC_EVERY = 500
STEPS = 50 if "--smoke" in sys.argv else 10000
RUN_MODE = ("smoke" if "--smoke" in sys.argv else os.environ.get("HDLAB_RUN_MODE", "full")).lower()
_ap = argparse.ArgumentParser(); _ap.add_argument("--smoke", action="store_true"); _ap.add_argument("--self-test", action="store_true"); _ARGS, _ = _ap.parse_known_args()
SMOKE = RUN_MODE == "smoke"
N_TRAIN = 16 if SMOKE else 300; N_EVAL = 8 if SMOKE else 48
...
def _selftest():
    ...
    print("[selftest] PASS: t5c-d1-3seed-validate", flush=True)
...
def run(SEED=7) -> Dict:
    ...
    adapters = nn.ModuleDict({str(L): FlamingoAdapter(H).to(DEV).to(torch.bfloat16) for L in LAYERS})
    out_dir = get_output_dir(ANCHOR_NAME); ckpt_path = Path(out_dir) / ("ckpt_s%d.pt" % SEED); Path(out_dir).mkdir(parents=True, exist_ok=True)
    start_step = 0
    if ckpt_path.exists():
        try:
    ...
    def mk(L):
        ...
        return hook
    for L in LAYERS:
        hooks.append(mdl.model.layers[L].self_attn.register_forward_hook(mk(L), with_kwargs=True))
    ...
    opt = torch.optim.Adam([{"params": [p for L in LAYERS for n, p in adapters[str(L)].named_parameters() if n != "gate"], "lr": 3e-4, "weight_decay": 0.01},
                            {"params": [adapters[str(L)].gate for L in LAYERS], "lr": 1e-3}], betas=(0.9, 0.95))  # gate-lr 1e-3 (1e-5 inert, 0.05 diverged; stability fixes hold this)
    ...
    for step in range(start_step, STEPS):
        ...
        if step % ACC_EVERY == 0:                                          # acceptance check: eval perplexity ratio + log trajectory
            ...
            if acc_ppl < best_ppl - 1e-3:
                best_ppl = acc_ppl; since_improve = 0; torch.save({"adapters": adapters.state_dict(), "step": step + 1}, Path(out_dir) / ("ckpt_best_s%d.pt" % SEED))
            else:
                since_improve += 1
...
out_dir = get_output_dir(ANCHOR_NAME); t0 = time.time(); _SEEDS = [7] if SMOKE else [7, 13, 29]; rs = [run(_s) for _s in _SEEDS]
_ratios = [x["ratio"] for x in rs]; _mean = sum(_ratios) / len(_ratios); _std = (sum((q - _mean) ** 2 for q in _ratios) / len(_ratios)) ** 0.5; _ua = all(x["used"] for x in rs); v = ("HARD_PASS" if (len(rs) >= 3 and _mean <= 0.95 and _std < 0.05 and _ua) else ("MIDDLE_BAND" if (_mean < 1.0 and _std < 0.10) else "HARD_FAIL")); vmsg = "%s: 3-seed mean perplexity ratio %.3fx std %.3f gates-used=%s ratios=%s -- multi-seed validation" % (v, _mean, _std, _ua, [round(q, 3) for q in _ratios]); print("\n[VERDICT] " + vmsg, flush=True)
metrics = {"anchor_name": ANCHOR_NAME, "verdict": v, "verdict_msg": vmsg, "run_mode": RUN_MODE, "n_seeds": len(rs), "per_seed": rs, "summary": {"mean_ratio": _mean, "std_ratio": _std, "ratios": _ratios}, "elapsed_s": time.time() - t0}
