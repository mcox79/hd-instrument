"""
substrate_cross_layer_compose_LM_v2_RESCUE -- cross-layer (independent-W per
layer) substrate-LM composition; rescues v1 which TIMED OUT at 3600s with
0 information.

RESCUE STRATEGY (per TIMEOUT class drill 2026-06-24):
- Scope reduction: N_DIM 8192 -> 4096; N_TRAIN 100k -> 50k; N_STEPS 1000 -> 2000;
  SEEDS 3 -> 2. Mid-range expansion past last-completed roofline point, not
  deep into compute-bound regime.
- D1 (mandatory) roofline probe: --roofline-probe runs a 3-point timing probe
  at (smoke_N=512, 2x=1024, 4x=2048) for one arm/seed; fits power-law; refuses
  the FULL pre-reg if extrapolated FULL wall > 0.8 * timeout. Probe is callable
  separately AND auto-invoked as part of selftest in smoke mode.
- D2 (mandatory) atexit partial-flush + per-seed checkpoint: any crash/timeout
  flushes the in-progress per-arm dict to data/exp_<name>/partial_atexit.json
  so even an aborted run yields information.
- Loop order: outer = seed, inner = arm. Seed 0 (all arms) lands before seed 1
  starts -- partial-result value per Erlang-OTP discipline.

FOUR ARMS (2 seeds each):
  ARM_SINGLE_LAYER_CFRPE          -- sanity rail (cf-RPE single-layer reference)
  ARM_2_LAYER_INDEPENDENT_CFRPE   -- LOAD-BEARING: 2 layers, independent W
  ARM_3_LAYER_INDEPENDENT_CFRPE   -- depth scan: 3 layers, independent W
  ARM_2_LAYER_SHARED_W_CFRPE      -- CONTROL: same-W collapse pattern

PRE-REG BANDS (inherited from v1; not loosened):
...
Sanity rail (single-layer reference at REDUCED N): wider tolerance +/-0.40
since the v1 +/-0.30 was calibrated against fair_harness N_DIM=8192; this
rescue runs at N_DIM=4096 so the reference value 7.04 may shift by 0.1-0.2
(fewer dims = noisier single-layer cf-RPE). The compositional structure under
test is invariant to N scaling within this range.

ENCODING: word2vec-google-news-300 projected to N_DIM=4096 sparse-bipolar
(f=0.05). OOV fallback char-trigram bipolar. Joint (T, lambda) sweep on dev;
test eval. LAMBDA_GRID excludes 0.0 per META C7.

ASCII-only. Per-seed checkpoint + atexit partial-flush.
"""
...
ANCHOR_NAME = "substrate_cross_layer_compose_LM_v2_RESCUE"
...
# Sanity rail: single-layer cf-RPE provenance (REDUCED N: wider tol than v1's 0.30)
SANITY_SINGLE_LAYER_REF_BPC = 7.04
SANITY_SINGLE_LAYER_TOL = 0.40  # widened from 0.30 due to N_DIM reduction
...
_P.add_argument("--timeout-s", type=int, default=5400,
...
# Production config (SCOPE-REDUCED v1)
N_DIM = 4096
PRETRAIN_DIM = 300
VOCAB_CAP = 4000
INGEST_CHUNK = 4096
...
if RUN_MODE == "full":
    SEEDS = [7, 17]              # SCOPE-REDUCED: 2 seeds (was 3)
    N_TRAIN = 50_000             # SCOPE-REDUCED: 50k (was 100k)
    N_HELD = 10_000              # scaled to N_TRAIN
    N_STEPS = 2000               # PER TASK SPEC: 2000 (was 1000)
else:
...
def compute_verdict(units: List[Dict]) -> tuple:
    ...
    detail = {
        ...
        "rescue_scope_reductions": {
            "N_DIM": "8192 -> 4096",
            "N_TRAIN": "100k -> 50k",
            "N_STEPS": "1000 -> 2000",
            "SEEDS": "[7,17,23] -> [7,17]",
            "rationale": "v1 TIMEOUT at 3600s with 0 partials; D1+D2 disciplines added",
        },
        "honest_scope": (
            "cross-layer compose at REDUCED LM scale (text8 N_TRAIN=50k N_DIM=4096 V=4000). "
            "RESCUE: v1 timed out at 3600s; scope reduced + D1 roofline probe + D2 atexit. "
            "Pre-reg bands inherited from v1 (not loosened). HARD_PASS = best_indep "
            "BPC <= %.2f AND beats shared-W control AND cv<=%.2f." % (
                HARD_PASS_BPC, HP_BPC_CV_MAX)),
        "cites": [
            "preregs/2026-06-24_substrate_cross_layer_compose_LM_v2_RESCUE.md",
            ...
            "notes/research_timeout_class_revival_disparate_fields_2026-06-24.md",
        ],
    ...
    if best_indep_bpc <= CHAIN_GRADE_BONUS_BPC:
        ...
        msg = ("HARD_PASS CHAIN_GRADE_BONUS: best_indep BPC=%.3f <= %.2f. "
               ...
               "REDUCED LM scale (rescue). Sanity rails: single_ok=%s shared_ok=%s. %s" % (
    ...
    if best_indep_bpc <= HARD_PASS_BPC:
        ...
        msg = ("HARD_PASS: best_indep BPC=%.3f <= %.2f. "
               "Cross-layer breaks composition collapse + beats single-layer ref (rescue). "
    ...
    msg = ("MIDDLE_BAND: best_indep BPC=%.3f in (%.2f, %.2f]. "
           "Cross-layer is neutral at REDUCED LM scale (rescue). %s" % (
