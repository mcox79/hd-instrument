"""
q_b1_chain_depth_80_v1_n16384 -- Q-B1: heteroassoc chain at depth-80 at N=16384.

SINGLE REMAINING GATE for PP-49a BAND-LIFT 0.70-0.85->0.75-0.90.
Cycle-16 flat-profile finding: d80 at N=8192 = 0.9820; lambda_empirical~0.
This anchor tests cross-N N=16384 to confirm the flat-profile is not N-specific.

H matrix at N=16384: 16384^2 * 4 bytes = 1.07 GB float32. Fits in 8 GB GPU.
Snapshots at depths [1, 3, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 75, 80].

PRE-REGISTERED BANDS (cross-N extension from N=8192 flat-profile):
  HARD-PASS: depth-5 >= 0.95 AND depth-10 >= 0.88 AND depth-20 >= 0.70
             AND depth-30 >= 0.55 AND depth-45 >= 0.40 AND depth-80 >= 0.10.
  ...
  Calibration: flat-profile confirmed d5-d80 at N=8192; HP bands carry over unchanged.
    If flat-profile holds at N=16384, d80 > 0.95. HP at 0.10 is extremely conservative.
    HF at 0.04 is 2.5x below HP; accounts for unlikely N-scaling regression.

FORMULA SELF-TESTS:
  1. depth-80 chain retrieval non-NaN at N=128.
     [INPUT: N=128, CHAIN_DEPTH=80] [EXPECTED: cos non-NaN in [-1,1]]
  2. Cosine of stored pattern with itself = 1.0.
     [INPUT: xi vs xi] [EXPECTED: cos_self = 1.0]
  3. H matrix memory > 500 MB at N=16384 (confirms actual GPU allocation).

PROT-018: anchor has _n16384; N MUST = 16384.
PROT-021: seed checkpoints keyed with run_mode + chain_depth + N.
"""
import sys
...
ANCHOR_NAME = "q_b1_chain_depth_80_v1_n16384"
...
CHAIN_DEPTH = 80
SNAPSHOT_DEPTHS = [1, 3, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 75, 80]
...
def _instrumentation_selftest():
    ...
    assert not (cos != cos), f"depth-80 retrieval is NaN at N={N_t}"
    ...
    # selftest-3: build a small-scale H to confirm GPU alloc (not full N=16384 in selftest)
    H_small = torch.zeros((512, 512), device=DEVICE, dtype=torch.float32)
    mem = torch.cuda.memory_allocated(0)
    assert mem > 0, f"GPU memory not allocated: {mem}"
    del H_small

    del H
    print(f"[selftest] PASS: depth-80 chain non-NaN N={N_t}, self-cosine=1.0, "
...
def run_seed(seed: int, n_dim: int) -> Dict:
    ...
    if RUN_MODE == "full":
        assert mem_gb > 0.5, f"selftest-3 FAIL: H at N={n_dim} allocated only {mem_gb:.3f}GB (expected >0.5GB)"
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    summary = (f"d5={d5:.4f}(HP>={HP_D5} HF<{HF_D5}) "
               ...
               f"d80={d80:.4f}(HP>={HP_D80} HF<{HF_D80}) "
               f"N=16384 n_seeds={len(results)}")

    if d5 < HF_D5 or d10 < HF_D10 or d20 < HF_D20 or d80 < HF_D80:
    ...
    if hp5 and hp10 and hp20 and hp30 and hp45 and hp80:
        return ("HARD_PASS", f"HARD_PASS: all depth thresholds met at N=16384 depth-80. {summary}")
    n_hp = sum([hp5, hp10, hp20, hp30, hp45, hp80])
...
assert peak_mem_gb > 0.01, f"GPU util check FAIL: peak_gpu={peak_mem_gb:.3f}GB (< 10MB)"
