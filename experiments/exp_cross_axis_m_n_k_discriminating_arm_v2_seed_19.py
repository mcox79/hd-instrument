"""cross_axis_m_n_k_discriminating_arm_v2 -- seed_19.

Cross-axis interaction test v2 with a DISCRIMINATING ARM (DIS_beta4) alongside
the v1 SATURATING baseline arm (STD_beta13). See core docstring for design.
...
- crlb / discriminator-reachability declared in prereg (interaction floor 0.05
  chosen against measured 0.02-0.06 range at seed 7-19 simulation)
- baseline_in_band: STD arm hypothesized saturated (v1 replication);
  DIS arm expected in (0.05, 0.95) range at smoke worst point
...
- calibration_check: default_ok_for_this_regime (per v1 landed regime)
- All numbers tagged MEASURED@/HYPOTHESIZED@/THEORETICAL@/CITED@

PRIOR (substrate-KB 2026-07-01 concept-query at cosine < 0.30):
  Genuinely novel: first cross-axis DISCRIMINATING arm test. v1 landed
  MEASURED@data/exp_cross_axis_m_n_k_2d_coarse_gpu_v1_seed_19/metrics.json:
  27/27 recall=1.000 by-construction saturation (Skunkworks demoted to MM).
  This cell adds DIS_beta4 arm to break the saturation and measure interaction.

CROSS-REFS:
  hdlab/chunked_attention.py (Testbed T2 chain-grade primitive)
  Cell D v2 CG dense-Hopfield READ-REPLACE (Atom 1)
  cross_axis_m_n_k_2d_coarse_gpu_v1 (v1 - saturated MM, prior arc)
...
from experiments._substrate_cross_axis_m_n_k_discriminating_arm_v2_core import (
...
ANCHOR_NAME = "cross_axis_m_n_k_discriminating_arm_v2_seed_19"
SEED_THIS_CHUNK = 19
...
def _main() -> None:
    ...
    metrics = {
        ...
        "crlb_formula_reference": (
            ...
            "M-N INTERACTION mean 3-seed = 0.042 at DIS_beta4 arm "
