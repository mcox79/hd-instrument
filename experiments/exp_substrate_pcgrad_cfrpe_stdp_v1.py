"""
substrate_pcgrad_cfrpe_stdp_v1
-- Composition collapse ANCHOR 2 (per notes/research_composition_collapse_critical_drill_2026-06-24.md).

ANCHOR 1 MH beta-sweep HARD_FAIL_STRUCTURAL: softening beta did not rescue
FULL_JOINT collapse -> structural cause confirmed. H1 (gradient conflict between
cf-RPE and STDP heterogeneous plasticity) elevated to load-bearing hypothesis.

PCGrad hypothesis (Yu et al. 2020, arxiv 2001.06782): cf-RPE and STDP have
conflicting gradients on the same W; projection of STDP onto the orthogonal
component of cf-RPE between updates may rescue the -0.116 reversal observed in
the A1 cell (ARM_+STDP arm reversed ARM_+CFRPE gain, 3/3 seeds).

GCond variant (arXiv:2509.07252): alternative gradient-conflict resolver via
accumulation-based stabilization. Discriminates PCGrad-class (projection) vs
broader gradient-conflict-class (any stabilization).

FOUR ARMS (3 seeds, text8 N_TRAIN=100k, N_DIM=8192):
  ARM_CFRPE_ONLY                -- provenance rail for cf-RPE (A1 ARM_+CFRPE 7.0888)
  ARM_CFRPE_PLUS_STDP_NAIVE     -- provenance rail for hetplast collapse
                                   (A1 ARM_+STDP / hetplast 7.2044)
...
PRE-REG HARD bands (per ANCHOR 2 spec; both directions sacrosanct):

  Sanity rails (Fix #28 per-arm metrics):
    ARM_CFRPE_ONLY              within +/-0.05 of A1's 7.0888
    ARM_CFRPE_PLUS_STDP_NAIVE   within +/-0.05 of A1's 7.2044
  ...
  MIDDLE_BAND:
    ...
    -> partial PCGrad help; conflict is contributing mechanism but not sole
...
GRADIENT-COSINE INSTRUMENTATION (Fix #28 per-arm metrics + verify conflict):
  ...
  median magnitude g_stdp / magnitude g_cf, accumulated PCGrad-projection-norm.

CONFIG:
  N_DIM=8192, V=4000, text8 N_TRAIN=100k, 3 seeds, word2vec sparse-bipolar f=0.05
  Routing: remote_cpu_queue (USER noted GPU underutilization + idle remote CPU
  per memory rule). numpy + matmul-bound; remote CPU is the right fit.

CITES:
  notes/research_composition_collapse_critical_drill_2026-06-24.md (ANCHOR 2)
  experiments/exp_substrate_compose_fair_harness_cfrpe_hetplasticity_K2_modern_hopfield_cleanup_v1.py (A1 reference; cf-RPE + STDP primitives)
...
ANCHOR_NAME = "substrate_pcgrad_cfrpe_stdp_v1"
...
def compute_verdict(units: List[Dict]) -> tuple:
    ...
    detail = {
        ...
        "honest_scope": (
            "ANCHOR 2 of composition collapse drill: tests whether gradient conflict "
            "between cf-RPE and STDP plasticity rules (Yu PCGrad / GCond) is the "
            "first-order cause of A1's hetplast collapse. 4 arms: CFRPE_ONLY (provenance "
            "rail to A1's 7.0888), NAIVE (provenance rail to A1's 7.2044), PCGRAD "
            "(orthogonal projection of STDP onto cf-RPE complement), GCOND "
            "(accumulation-based STDP rescaling). N_DIM=8192, N_TRAIN=100k text8, "
            "V=4000, 3 seeds, word2vec sparse-bipolar f=0.05. ANCHOR 1 (MH beta-sweep) "
            "already HARD_FAIL_STRUCTURAL so MH is OMITTED in this cell. "
            "WHAT_THIS_DOES_NOT_SHOW: gate training; alternative gradient-conflict "
            "resolvers (CAGrad, MGDA); shared-state architecture (H5); MH cleanup at all."
        ),
        "cites": [
            "notes/research_composition_collapse_critical_drill_2026-06-24.md (ANCHOR 2)",
    ...
    # Provenance rails only fire in run_mode=full
    detail["provenance_check_active"] = (RUN_MODE == "full")
    if RUN_MODE == "full":
        if not cfrpe_rail_ok:
            return ("HARD_FAIL_PROVENANCE",
                    "HARD_FAIL_PROVENANCE_CFRPE: CFRPE_ONLY=%.4f drifts %.4f from "
                    "A1 ref %.4f (>tol %.2f). cf-RPE primitive mismatch. %s" % (
                        cfrpe_bpc, cfrpe_drift, SANITY_RAIL_CFRPE_REF,
                        SANITY_RAIL_TOLERANCE, arm_summary),
                    detail)
        if not naive_rail_ok:
            return ("HARD_FAIL_PROVENANCE",
                    "HARD_FAIL_PROVENANCE_NAIVE: NAIVE=%.4f drifts %.4f from "
                    "A1 hetplast ref %.4f (>tol %.2f). NAIVE compose mismatch. %s" % (
                        naive_bpc, naive_drift, SANITY_RAIL_HETPLAST_REF,
                        SANITY_RAIL_TOLERANCE, arm_summary),
                    detail)
