"""
pp48_nkt_depth_17_v1_n4096 -- PP-48 NKT negative-knowledge tree at depth-17 at N=4096.

Extends depth-15 to depth-17. Ceiling chase for NKT depth.
  Total binary tree at depth-17: 2^17 - 1 = 131071 nodes. Sampled-leaf design.
  BRANCH=2, sample K_FORBIDDEN=100 leaves.
  alpha_total = (10 pos + 100 forbidden) / 4096 = 0.027. Well sub-capacity.

  Tree construction at depth-17: branching from root to leaves requires 17 recursion levels.
  Each leaf at depth-17 = product of 17 context Hadamard bindings from root.
  Test: does substrate still repel depth-17 leaves with anti-Hebbian inhibition?
...
PRE-REGISTERED BANDS:
  ...
  Calibration: depth-13 HARD_PASS; bands unchanged at depth-17.
...
ANCHOR_NAME = "pp48_nkt_depth_17_v1_n4096"
...
def build_nkt_tree_sample(n_dim: int, gen: torch.Generator, k_sample: int) -> List[torch.Tensor]:
    ...
    leaves = prev_level  # deepest level leaves
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    if hp1 and hp2:
        return ("HARD_PASS", f"HARD_PASS: both HP at NKT depth-17 sampled-leaves. {summary}")
    if hp1 or hp2:
