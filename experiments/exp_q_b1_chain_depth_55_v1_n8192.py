"""
q_b1_chain_depth_55_v1_n8192 -- Q-B1: heteroassoc chain at depth-55 at N=8192.

Ceiling push: depth-45 HARD_PASS (d45=0.596 >> HP=0.40). Ship both depth-50 and depth-55
  together to map the degradation slope more efficiently.
  GPU IMPLEMENTATION: H matrix (N x N float32 at N=8192): 268 MB. Safe on 8 GB GPU.
  Snapshots at depths [1, 3, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55].

PRE-REGISTERED BANDS:
  HARD-PASS: depth-5 >= 0.95 AND depth-10 >= 0.88 AND depth-20 >= 0.70
             AND depth-30 >= 0.55 AND depth-45 >= 0.40 AND depth-55 >= 0.25.
  HARD-FAIL: depth-5 < 0.80 OR depth-10 < 0.65 OR depth-20 < 0.40 OR depth-55 < 0.10.
  MIDDLE: depth-55 in [0.15, 0.25) while earlier depths meet HP.
  Calibration: depth-45 result d45=0.596 >> HP; depth-55 HP set conservatively at 0.25.

FORMULA SELF-TESTS:
  1. depth-55 chain retrieval non-NaN at N=128.
...
ANCHOR_NAME = "q_b1_chain_depth_55_v1_n8192"
...
CHAIN_DEPTH = 55
SNAPSHOT_DEPTHS = [1, 3, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55]
...
def _instrumentation_selftest():
    ...
    assert not (cos != cos), f"depth-55 retrieval is NaN at N={N_t}"
    ...
    del H
    print(f"[selftest] PASS: depth-55 chain non-NaN N={N_t}, self-cosine=1.0, "
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    summary = (f"d5={d5:.4f}(HP>={HP_D5} HF<{HF_D5}) "
               ...
               f"d55={d55:.4f}(HP>={HP_D55} HF<{HF_D55}) "
               f"n_seeds={len(results)}")

    if d5 < HF_D5 or d10 < HF_D10 or d20 < HF_D20 or d55 < HF_D55:
    ...
    if hp5 and hp10 and hp20 and hp30 and hp45 and hp55:
        return ("HARD_PASS", f"HARD_PASS: all depth thresholds met at N=8192 depth-55. {summary}")
    n_hp = sum([hp5, hp10, hp20, hp30, hp45, hp55])
