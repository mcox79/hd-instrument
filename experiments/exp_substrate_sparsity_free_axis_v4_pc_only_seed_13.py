"""sparsity_free_axis_v4_pc_only sibling seed=13.

V4 RATIONALE:
    v2 (all-4-axis revival) HARD_FAILed on WM positive control only. Deeper audit
    (v3 cell-author 2026-07-01) traced the WM failure to an ARCHITECTURAL BUG in
    the shared v2/v3 core: `vals_corr = _corrupt_hrr_real(vals, ...)` is computed
    (v2 core line 419) but only used in calibration diagnostics (lines 443-445);
    NEVER used in the WM readout path (readouts use raw `vals` via bank_trace).
    WM top1 is INSENSITIVE to CORRUPTION_WM by construction; v3 c=0.55 escalation
    was architecturally unachievable (v3 c=0.55 top1 identical to v2 c=0.40 at
    three matching points: 0.9526, 0.9626, 0.8228).

    v4 RESPONSE (this cell): RETIRE WM regime entirely; PC-only. v2's PC data is
    already HARD_PASS-clean on every gate (Spearman=-1.0 at all M; cross-seed
    cv<=0.023; all 27 points in [0.30, 0.90]). v4 banks the PC axis CG today;
    WM regime corruption-recovery is DEFERRED to future v5 with the readout bug
    fixed. See notes/wm_readout_architectural_bug_deferred_v5_2026-07-01.md.

Design (LOCKED):
    5 M (800, 1000, 1500, 2000, 2500) x 3 alpha (0.05, 0.10, 0.20) x PC-only
    = 15 phase points per seed. SMOKE == FULL grid (DISCRIMINATOR-SURVIVES-SCALE).

Fixed:
    N=4096, encoder=hrr_real, c=0.60, T_cleanup=1, beta=8.0

Discriminator (v4 bands):
    HP_PC_MONOTONE: Spearman rho <= -0.80 at ALL 5 M values
    HP_PC_IN_BAND: PC top1 in [0.30, 0.90] at ALL 15 grid points
    HP_CROSS_SEED_TIGHT: cross-seed cv < 0.05 per point (v2 max = 0.023)
...
PRIOR REFS:
  v1 HF: data/exp_sparsity_free_axis_v1_n8192_seed_7_smoke/metrics.json
  v2 HF: data/exp_substrate_sparsity_free_axis_v2_n4096_seed_13/metrics.json (WM only)
  v3 arch bug finding: notes/wm_readout_architectural_bug_deferred_v5_2026-07-01.md
CARDINALITY_OK_FULL: 15 phase points per seed
CARDINALITY_OK_SMOKE: 15 phase points per seed (same grid)
...
SEED = 13
ANCHOR_NAME = f"substrate_sparsity_free_axis_v4_pc_only_n4096_seed_{SEED}"
...
CONFIG_VERSION = (
    ...
    f"sparsity=[0.05,0.10,0.20],M=[800,1000,1500,2000,2500],"
    f"regimes=[PC],N=4096,SEED={SEED},mode={RUN_MODE},"
    f"pc_c=0.60,pc_T=1,beta=8.0,"
    f"arms=[MECHANISM,RANDOM_FLOOR],"
    f"expected_n_full=15,expected_n_smoke=15,"
    f"design_kind=SPARSITY_FREE_AXIS_V4_PC_ONLY,"
    f"parent_refs=[sparsity_free_axis_v2_PC_MEASURED_HP,v3_arch_bug_finding],"
    f"WM_STATUS=RETIRED_ARCH_BUG_v2core_line_419_vals_corr_unused_in_readout,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v4_sparsity_free_axis_pc_only_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v4_sparsity_free_axis_pc_only_import_crash",
        }
...
def main() -> int:
    ...
    from experiments._sparsity_free_axis_v4_pc_only_core import (
    ...
    final["_hardening_marker"] = "v4_sparsity_free_axis_pc_only_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["N"] = N_DIM_FULL
    final["corpus_provenance"] = "synthetic_sparsity_free_axis_v4_pc_only"
    final["n_llm_calls"] = _LLM_CALL_COUNTER[0]
    final["design_kind"] = "SPARSITY_FREE_AXIS_V4_PC_ONLY"
