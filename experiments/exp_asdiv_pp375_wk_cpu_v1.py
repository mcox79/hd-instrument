...
ANCHOR_NAME = "asdiv_pp375_wk_cpu_v1"
...
def _load_wk():
    ...
    for line in open(fp,encoding="utf-8"):
        ...
        for key,val in a.get("members_named_values",{}).items():
            ...
            if "_per_" in key:
                x=_stem(key.split("_per_")[0].split("_")[-1]); y=_stem(key.split("_per_")[-1].split("_")[-1])
                if x.isalpha() and y.isalpha(): _WK_PER.append((x,y,v))
            elif key.isalpha(): _WK_COLL[_stem(key)]=v
def _wk_nums(txt):
    low=txt.lower(); toks=low.split(); vals=[]
    m=re.search(r"how (?:many|much) ([a-z]+)",low); tgt=_stem(m.group(1)) if m else ""
    wordset=set(_stem(re.sub(r"[^a-z]","",w)) for w in toks)
    for (x,y,v) in _WK_PER:
        if tgt and tgt==x and y in wordset and v not in vals: vals.append(v)
    isnum=[bool(re.match(r"^\d",w)) or re.sub(r"[^a-z]","",w) in _WORDNUM for w in toks]
    for k,w in enumerate(toks):
        st=_stem(re.sub(r"[^a-z]","",w))
        if st in _WK_COLL and any(isnum[j] for j in range(max(0,k-2),min(len(toks),k+3))) and _WK_COLL[st] not in vals: vals.append(_WK_COLL[st])
    return vals
def _ans(x):
...
def _solve(train, test_full, search, seed, add_wk=False):
    """search=False: faithful PP-375 text-order. search=True: op-seq + operand search. add_wk: inject question-guided WK constants."""
    rng = np.random.default_rng(seed)
    X1 = []; X2 = []
    for txt, ans, oc in train:
        ns = _nums(txt) + (_wk_nums(txt) if add_wk else [])
        if len(ns) >= 2:
    ...
    for ti, (txt, ans, oc) in enumerate(test_full):
        ns = _nums(txt) + (_wk_nums(txt) if add_wk else []); cq = _cq(txt)
...
def run() -> Dict:
    ...
    _of, byf = _solve(train, test, search=False, seed=seed)
    _os, bys = _solve(train, test, search=True, seed=seed)
    _ow, byw = _solve(train, test, search=True, seed=seed, add_wk=True)
    print("  PP-375+WK ASDiv: text-order 1op=%.4f | +search 1op=%.4f | +search+WK 1op=%.4f (overall %.4f) | WK lift=%+.4f (vs multihop 0.376)" % (
        byf.get(1, 0), bys.get(1, 0), byw.get(1, 0), _ow.get("overall", 0), byw.get(1, 0) - bys.get(1, 0)), flush=True)
    return {"accuracy": byw.get(1, 0.0), "textorder_1op": byf.get(1, 0.0), "search_1op": bys.get(1, 0.0),
            "searchwk_1op": byw.get(1, 0.0), "wk_lift": round(byw.get(1, 0.0) - bys.get(1, 0.0), 4),
            "searchwk_overall": _ow.get("overall", 0.0), "n_test": len(test)}


def verdict(r) -> Tuple[str, str]:
    sw = r["searchwk_1op"]; sr = r["search_1op"]; lift = r["wk_lift"]
    s = "+search+WK 1op=%.4f (+search alone %.4f, WK-lift %+.4f, text-order %.4f, overall %.4f). PP-375 mechanism + question-guided WK constants (cycle's two wins combined)." % (
        sw, sr, lift, r["textorder_1op"], r["searchwk_overall"])
    if sw >= 0.45:
        return ("HARD_PASS", "HARD_PASS: PP-375 + WK reaches ASDiv-1op >=0.45 -- the two cycle wins COMPOUND (best mechanism + world-knowledge constants). " + s)
    if lift >= 0.02 or sw >= 0.40:
        return ("MIDDLE_BAND", "MIDDLE_BAND: WK adds to PP-375 (lift>=0.02 or >=0.40) -- world-knowledge rescues some items the mechanism alone misses. " + s)
    return ("HARD_FAIL", "HARD_FAIL: WK adds nothing to PP-375 (<0.02 lift, <0.40) -- WK items too sparse / mechanism can't use them; ~0.38 ceiling holds. " + s)
