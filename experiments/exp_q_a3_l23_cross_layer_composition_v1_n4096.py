"""
q_a3_l23_cross_layer_composition_v1_n4096 -- Q-A3: L=23 cross-layer composition at N=4096.

Ceiling chase: L=22 HARD_PASS EXACT-1.0000 unanimous 5/5 (v353+). 8 consecutive L-extensions
(L=15..L=22) all EXACT-1.0000 at N=4096. L-ceiling has NOT been found. L=23 is the next step.
23 W matrices x ~67 MB each = ~1541 MB total. Fits in 8 GB GPU with margin.

Note: M per stage halves each level per isochoric design; alpha_c constraint ensures validity.
At L=23 M_OUTER=2, M_MID22=2, ..., M_MID7=2 (floored at 2 from L=7+).

PRE-REGISTERED BANDS (ceiling push, prior L=22 EXACT-1.0 unanimous; L=23 expected same):
  HARD-PASS: all 23 level fidelities = 1.0000 (>= 0.9999) unanimous (5/5 seeds)
             AND l23_acc >= 0.5.
  MIDDLE: any L_fid in [0.85, 1.0) OR graceful degradation of inner levels.
  HARD-FAIL: any L_fid < 0.85 OR l23_acc < 0.5. Ceiling found at L=23.

FORMULA SELF-TESTS (PROT-022):
  1. L=23 chain: 23-step Hadamard roundtrip recovers xi_L1.
...
QUEUE: overnight_queue (GPU; 23 W matrices at N=4096 ~1541 MB).
TIMEOUT ESTIMATE: L=22 elapsed=1.59s FULL 5-seed GPU. L=23 adds 1 W matrix (negligible).
...
ANCHOR_NAME = "q_a3_l23_cross_layer_composition_v1_n4096"
...
def _selftest_l23_chain():
    """23-step Hadamard roundtrip recovers xi_L1."""
    ctxs = []
...
def run_seed(seed: int, n_dim: int) -> Dict:
    ...
    Xi_ctx23 = bsc(M_OUTER, n_dim)
    Xi_outer = Xi_ctx23 * Xi_mid22[:M_OUTER]
