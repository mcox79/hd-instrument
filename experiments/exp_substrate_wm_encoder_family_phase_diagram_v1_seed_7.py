"""substrate_wm_encoder_family_phase_diagram_v1 sibling seed=7.

COMPONENT-SUBSTITUTION phase diagram for multi-bank WM K-cliff primitive.
Outer axis: 4 encoder families {binary_bipolar, hrr_real, fhrr, sparse_bipolar}.
Inner: K_per_bank x num_banks at fixed N=8192 (FULL) / N=4096 (SMOKE).

Per seed FULL : 4 enc x 4 K x 3 B = 48 phase points
Per seed SMOKE: 4 enc x 3 K x 2 B = 24 corner points

Sibling files: seed_13, seed_19.

PRE-REG: preregs/2026-06-28_substrate_wm_encoder_family_phase_diagram_v1.md

PROT-018: anchor has no _n<N> suffix (fixed-N cell at N=8192 FULL).
PROT-019: anchor uses no _n>=4096 suffix in name -> no timeout floor enforced.
...
SEED = 7
ANCHOR_NAME = f"substrate_wm_encoder_family_phase_diagram_v1_seed_{SEED}"
...
CONFIG_VERSION = (
    ...
    f"K_per=[64,256,1024,4096],B=[4,16,64],N_full=8192,N_smoke=4096,"
    f"SEED={SEED},mode={RUN_MODE},sparse_density=0.02,"
    f"arms=[MULTI_BANK_BIND,SINGLE_BANK_BASELINE,RANDOM_FLOOR],"
    f"expected_n_full=48,expected_n_smoke=24,"
...
def _write_minimal_metrics(out_dir: Path, verdict: str, verdict_msg: str,
    ...
    try:
        ...
        m = {
            ...
            "_hardening_marker": "v1_wm_encoder_family_phase_diagram_chunked",
        }
        if extra:
            m.update(extra)
...
def _write_import_crash_sentinel(exc: BaseException) -> None:
    ...
    try:
        ...
        s = {
            ...
            "_hardening_marker": "v1_wm_encoder_family_import_crash",
        }
...
def main() -> int:
    ...
    from experiments._substrate_wm_encoder_family_phase_diagram_v1_core import (
    ...
    final["_hardening_marker"] = "v1_wm_encoder_family_phase_diagram_chunked"
    final["backend"] = backend
    final["seed"] = SEED
    final["n_seeds"] = 1
    final["corpus_provenance"] = "synthetic_substrate_4_encoder_family_wm_kcliff"
