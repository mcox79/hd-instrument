"""
exp_sign_recall_50M_gpu_v1.py -- sign-key autoassociative recall@1 at 50M keys (GPU, chunked) -- GPU.

ROUTING: GPU product/capacity-scale (sign-key recall at 50M). 50M sign keys (D=1024) regenerated per chunk; chunked GPU recall@1 under 0.15 bit-flip. Pushes the recall scaling gate to 50M (5x the 10M result, 50x the 1M CPU gate). torch.cuda; 8GB-safe. GPU.
PRE-REGISTERED: HARD-PASS recall@1 >= 0.99 at N=50M. MIDDLE >= 0.95. HARD-FAIL < 0.95.
...
ANCHOR_NAME = "sign_recall_50M_gpu_v1"
RUN_MODE = ("smoke" if "--smoke" in sys.argv else os.environ.get("HDLAB_RUN_MODE", "full")).lower()
_ap = argparse.ArgumentParser(); _ap.add_argument("--smoke", action="store_true"); _ap.add_argument("--self-test", action="store_true"); _ARGS, _ = _ap.parse_known_args()
SMOKE = RUN_MODE == "smoke"

def _selftest():
    import numpy as _n; assert _n.argmax([0.1, 0.9]) == 1, "argmax"; print("[selftest] PASS: sign_recall_50M_gpu_v1", flush=True)
_selftest()
if _ARGS.self_test:
    sys.exit(0)
try:
