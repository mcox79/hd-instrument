"""CPU-friendly: K-extreme (codebook size) decompose accuracy sweep.

Companion to B-sweep. We've validated codebook K up to 2048 at B=2,
N=4096. This sweep tests K in {4096, 8192, 16384} -- finding where
the substrate's codebook capacity actually breaks.
"""
...
def main():
    _say(f"K-extreme decompose sweep: K in {K_SWEEP} at N={N}, B={B}")
    ...
    for K in K_SWEEP:
        ...
        gen_trial = torch.Generator().manual_seed(SEED + 2000 + K)
        correct = 0
        ...
        _say(f"  K={K:6d}  recovery={recovery:5.1f}%  ({correct}/{NUM_TRIALS})")
        results.append({"K": K, "recovery_pct": recovery, "correct": correct})

    out_dir = Path(__file__).resolve().parent.parent / "data" / "exp_wave14b_decompose_K_extreme"
    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / "metrics.json").write_text(json.dumps({
        "N": N, "B": B, "K_SWEEP": K_SWEEP, "NUM_TRIALS": NUM_TRIALS,
