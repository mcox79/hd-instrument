"""stage2_commercial_M_latency_percentiles_v2_timeout_fixed -- seed_7.

v2 forks v1 with (1) shared-W-per-M (build W once, reuse across 3 backends)
and (2) per-arm incremental metrics.json checkpoint (never lose completed
arms on timeout kill). See core module for design + prior-work lineage.
...
ANCHOR_NAME = "stage2_commercial_M_latency_percentiles_v2_timeout_fixed_seed_7"
SEED_THIS_CHUNK = 7
...
def main() -> int:
    ...
    if args.self_test:
        # Verify core imports + plans + trivial-arm.
        ...
        # Trivial arm test at M=1000 / N=256 via shared-W path.
