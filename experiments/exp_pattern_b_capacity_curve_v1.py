"""
exp_pattern_b_unbind_substitute_v1 -- Pattern B Phase-1: VSA bundle capacity (max bindings at acc>=0.95) vs N -- CPU.
...
ANCHOR_NAME = "pattern_b_capacity_curve_v1"; N = 1024
RUN_MODE = ("smoke" if "--smoke" in sys.argv else os.environ.get("HDLAB_RUN_MODE", "full")).lower()
_ap = argparse.ArgumentParser(); _ap.add_argument("--smoke", action="store_true"); _ap.add_argument("--self-test", action="store_true"); _ARGS, _ = _ap.parse_known_args()
SEEDS = [1] if RUN_MODE == "smoke" else [7, 17, 23]; BINDINGS = [4, 8, 12, 16, 24]; VOCAB = 200 if RUN_MODE == "smoke" else 1000
...
def run_seed(seed, k) -> float:
    g = np.random.default_rng(seed); vocab = rand_phasor(N, VOCAB, g); roles = rand_phasor(N, k, g)
    ok = 0; trials = 100 if RUN_MODE == "smoke" else 300
    for _ in range(trials):
        fids = g.choice(VOCAB, k, replace=False)
        fact = np.sum([bind(roles[i], vocab[fids[i]]) for i in range(k)], axis=0).astype(np.complex64)
        j = int(g.integers(0, k)); newf = int(g.integers(0, VOCAB))                       # substitute filler j -> newf
        fact2 = fact - bind(roles[j], vocab[fids[j]]) + bind(roles[j], vocab[newf])
        rec = cleanup(unbind(fact2, roles[j]), vocab)                                      # retrieve substituted filler
        ok += int(rec == newf)
    return ok / trials
...
def run() -> Dict:
    ...
    for k in BINDINGS:
        ...
        if acc >= 0.95: cap = k
        print("  bindings=%d retrieval_acc=%.3f" % (k, acc), flush=True)
    print("  capacity (max bindings at acc>=0.95) = %d at N=%d" % (cap, N), flush=True)
    return {"by": by, "capacity": cap, "acc_k4": by.get("k4", 0.0)}


def verdict(r) -> Tuple[str, str]:
    cap = r["capacity"]; summary = "acc by bindings: %s; capacity=%d at N=%d" % ({k: round(v, 3) for k, v in r["by"].items()}, cap, N)
    if cap >= 8:
        return ("HARD_PASS", "HARD_PASS: VSA bundle holds >=8 role-filler bindings at acc>=0.95 (N=%d) -- ample compositional capacity for multi-fact records. " % N + summary)
    if cap >= 4:
        return ("MIDDLE_BAND", "MIDDLE_BAND: capacity 4-7 bindings. " + summary)
    return ("HARD_FAIL", "HARD_FAIL: capacity <4 bindings -- bundle interference limits compositional records. " + summary)
