"""
exp_pca_bottleneck_zkl_sweep_v1 -- Hyp B mitigation (a): cap top-3 attended positions (attention-pool proxy of the forward hook) -- GPU.
...
ANCHOR_NAME = "zkl_hypB_attn_reweight_v1"; TOP_CAP = 3
...
def llama_encode(texts, tok, m, cap=True):
    ...
    for t in texts:
        ...
        a = a / (a.sum() + 1e-9)
        if cap:
            order = np.argsort(a)[::-1]; rest = a[order[TOP_CAP:]]
            capval = float(rest.mean()) if len(rest) else float(a.mean())
            a = a.copy(); a[order[:TOP_CAP]] = capval; a = a / (a.sum() + 1e-9)   # cap top-3, renormalize
...
def run() -> Dict:
    ...
    z_orig, f1_orig = measure(False); z_cap, f1_cap = measure(True)
    del m; torch.cuda.empty_cache()
    print("  attn-pool ORIG ZKL(%d)=%.3f F1=%.3f | CAPPED-top3 ZKL=%.3f F1=%.3f" % (K_PARA, z_orig, f1_orig, z_cap, f1_cap), flush=True)
    return {"z_orig": z_orig, "z_cap": z_cap, "f1_orig": f1_orig, "f1_cap": f1_cap, "n": len(stored), "k": K_PARA}


def verdict(r) -> Tuple[str, str]:
    zc = r["z_cap"]; drop = r["f1_orig"] - r["f1_cap"]
    summary = "CAPPED-top3 ZKL(%d)=%.3f F1=%.3f (vs attn-pool ORIG ZKL=%.3f F1=%.3f; F1 drop=%.3f; n=%d)" % (r["k"], zc, r["f1_cap"], r["z_orig"], r["f1_orig"], drop, r["n"])
    if zc <= 0.10 and drop <= 0.10:
        return ("HARD_PASS", "HARD_PASS (Hyp B mitigation a): capping top-3 attended positions drops ZKL(50)<=0.10 with KEY-job F1 drop<=10%% -- ABSOLUTE HIPAA-grade privacy RECOVERED via attention-reweighting (productionize the forward hook). " + summary)
    if 0.10 < zc <= 0.15 and drop <= 0.20:
        return ("MIDDLE_BAND", "MIDDLE_BAND (a borderline): capping reduces ZKL toward target -- worth the true forward-hook implementation to confirm. " + summary)
    return ("HARD_FAIL", "HARD_FAIL: capping top-3 attention does not bring ZKL<=0.15 -- the last linear mitigation fails; lock the QUALIFIED-privacy posture (audit + ZKP + rate-limit), absolute HIPAA via per-customer encoder fine-tune (Path D). " + summary)
