"""phase_diagram_capacity_multi_bank_K4_envelope_v2c_n8192_gpu --
Rescue Cell B v2c: N=8192 retry of v2b after OOM at N=16384 on RTX 4060 Ti.

PARENT FINDING (cell v2b):
  v2b ran 130s on remote then HARD_FAIL_UNIT_EXCEPTION CUDA OOM at K=4
  ingest (1.024 GiB alloc on top of 5.38 GiB allocated; ceiling 6.80 GiB on
  8 GiB GPU). Per orchestrator flag-back option #3 (cheapest, preserves
  alpha>=4 finding): halve N_DIM to 8192. W matrix is N^2 fp32 so this
  quarters per-bank W size (1.07GB -> 268MB; 4 banks = 1.07GB total) and
  leaves comfortable headroom under the 6.8GB budget for V/K intermediates.

  The capacity claim ("substrate exceeds predicted band at alpha>=4") is
  alpha-RELATIVE not N-RELATIVE; preserving alpha_N=4.0 + headroom=10x at
  N=8192 still tests the same Skunkworks finding from batch 8.

SCOPE (matches v2b; just smaller N):
  ONE phase point: alpha_N=4.0, codebook_headroom=10x. 3 seeds.
  NO mech, NO knn, NO bare arms (covered by Cell A).

PER-ARM HP-SCOPE DECLARATION (SCHEMA-VET 5b):
  MULTI_BANK_K4 : envelope-band [0.95, 1.0] (HP_MB_REC_MIN=0.95; CV<=0.05).
                  Same band as v2b at alpha=4 h=10x at N=8192.
  ...
  MECH / KNN_SENTINEL / BARE_E_R arms EXEMPT (Cell A scope).

META_RULE_H (cardinality_ok): EXPECTED_N_UNITS = 1 phase point * 3 seeds = 3.
...
Author: exp_dev 2026-06-27 (v2c N=8192 rescue of v2b OOM).
"""
...
ANCHOR_NAME = "phase_diagram_capacity_multi_bank_K4_envelope_v2c_n8192_gpu"
_LLM_CALL_COUNTER = [0]
...
# Cell B v2c targets exactly one phase point: alpha_N=4.0, headroom=10x
MULTI_BANK_K = 4
ALPHA_N_TARGET = 4.0
HEADROOM_LABEL = "10x"
HEADROOM_VAL = 10.0
...
if SMOKE:
    ...
    N_DIM = 8192  # v2c: halved from v2b's 16384 to fit 6.8 GiB GPU budget
    SEEDS = [11, 13, 19]
...
CONFIG_VERSION = (
    "phaseDiagCapMultiBankK4-v2c-n8192-gpu: N=%d V_R=%d seeds=%s mode=%s "
...
def eval_recall_multibank_K4(V_C: int, V_R_arg: int, M_facts: int,
    ...
    in VRAM only during the per-query reduction. With K=4 N=8192 fp32 W,
    peak VRAM occupancy ~1.07GB W total + transient batch tensors.
    """
    sq = math.sqrt(n_dim)
...
def _selftest() -> None:
    ...
    # T9: target V_C is computable + reasonable for v2c N=8192
    target_V_C = _compute_VC_for(ALPHA_N_TARGET, HEADROOM_VAL, N_DIM)
    target_M = int(round(ALPHA_N_TARGET * N_DIM))
    # At full N=8192: M=32768, V_C=ceil(10*32768/32)=10240 (well-formed)
    if not SMOKE:
        assert target_M == 32768, "expected M=32768 at N=8192 alpha=4; got %d" % target_M
        assert target_V_C == 10240, "expected V_C=10240 at N=8192 alpha=4 h=10x; got %d" % target_V_C

    # T10: N_DIM at full is 8192 (v2c invariant; OOM-rescue lock)
    if not SMOKE:
        assert N_DIM == 8192, "v2c N_DIM lock breached: %d != 8192" % N_DIM
...
def compute_verdict(per_key: Dict[str, Dict],
    ...
    summ = "MULTI_BANK_K%d_alpha%.1f_h%s_N%d: rec=%.4f cv=%.4f n=%d per_seed=%s peak_mem_mb=%d" % (
        MULTI_BANK_K, ALPHA_N_TARGET, HEADROOM_LABEL, N_DIM, mb_mean, mb_cv,
    ...
    if mb_mean >= HP_MB_REC_MIN and mb_cv <= HP_MB_CV_MAX:
        return ("HARD_PASS",
                "HARD_PASS_MULTI_BANK_K4_N%d: rec=%.4f >= %.2f AND cv=%.4f <= %.2f across %d seeds at alpha=%.1f h=%s (substrate-beats-baseline confirmed under K=4 sharding at N=%d; v2c rescue of v2b OOM) | %s%s" % (
                    N_DIM, mb_mean, HP_MB_REC_MIN, mb_cv, HP_MB_CV_MAX,
                    len(mb_recs), ALPHA_N_TARGET, HEADROOM_LABEL, N_DIM,
                    summ, card_str),
                detail)
    if mb_mean >= MB_MB_REC_MIN:
        return ("MIDDLE_BAND",
                "MIDDLE_BAND_MULTI_BANK_N%d: rec=%.4f in [%.2f, %.2f) OR cv=%.4f > %.2f (METHOD_RULE_L band-floor) | %s%s" % (
                    N_DIM, mb_mean, MB_MB_REC_MIN, HP_MB_REC_MIN, mb_cv, HP_MB_CV_MAX,
                    summ, card_str),
                detail)
    return ("HARD_FAIL",
            "HARD_FAIL_MULTI_BANK_BELOW_FLOOR_N%d: rec=%.4f < %.2f at alpha=%.1f h=%s (mechanism does not survive K=4 sharding at N=%d) | %s%s" % (
                N_DIM, mb_mean, MB_MB_REC_MIN, ALPHA_N_TARGET, HEADROOM_LABEL, N_DIM,
                summ, card_str),
            detail)
...
def _build_metrics(v: str, vmsg: str, detail: Dict, units: List[Dict],
    ...
    return {
        ...
        "CV_MAX": CV_MAX,
        "HP_SCOPE": {
            "MULTI_BANK_K4": ["HP_MB_REC_MIN>=%.2f at alpha=%.1f h=%s N=%d; CV<=%.2f" % (
                HP_MB_REC_MIN, ALPHA_N_TARGET, HEADROOM_LABEL, N_DIM, HP_MB_CV_MAX)],
        ...
        "DESIGN_NOTE": (
            "CAPACITY_V2C_N8192_MULTI_BANK_K4_GPU_RESCUE: v2c halves N_DIM "
            "from v2b's 16384 to 8192 after v2b OOM (1.024 GiB on top of "
            "5.38 GiB allocated; ceiling 6.80 GiB on 8 GiB GPU). The "
            "capacity claim is alpha-relative not N-relative, so testing "
            "at N=8192 alpha_N=4.0 headroom=10x preserves the Skunkworks "
            "finding from batch 8. W per bank = N^2 fp32 = 268MB; 4 banks "
            "= 1.07GB total + transient batch tensors fits comfortably "
            "under 6.8GB budget. Targets ONE phase point; 3 seeds; "
            "K_banks=4 sequential build + empty_cache + expandable_segments. "
            "GPU mandate (Fix #24): cuda asserted at full. HARD_PASS rec>=0.95 "
            "cv<=0.05 across 3 seeds. Mech / KNN / BARE arms EXEMPT (Cell A). "
            "Routes overnight_queue per USER NO_LOCAL 2026-06-27."
        ),
    }
