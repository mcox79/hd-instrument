"""
Track A v4 -- WILD-TEXT COMPOSITION CG-GATE: does composition SURVIVE on REAL (non-authored)
McGuffey SECOND READER passages, where frequency is NATURALLY (not hand-authored) decorrelated,
and does the LEARNED assigner beat POSITIONAL on richer real constructions?

WHY (v4): v3 (learned_role_assigner_reader_composition_v3, VET afd3df89) proved the composition
MACHINERY works + frequency cannot win BUT only on AUTHORED passages: the primer was too thin (~67
words/passage) so frequency trivially won unless we DECORRELATED it by construction. The VET verdict:
CG REQUIRES composition to hold on NON-authored richer real text with NATURAL freq-decorrelation,
distinguishing learned from positional. v4 runs the fully-ASSEMBLED v3 reader on REAL McGuffey 2nd
Reader passages (188 words/passage; richer SVO + subordinate/coordinate clauses; natural competitive
coref). PASSAGES ARE REAL (verbatim from the corpus file; provenance-verified in self_test = the
wild-text fix); only the comprehension QUESTIONS are hand-authored (that is comprehension testing,
not passage authoring). This is a HONEST-BASELINE measurement on real text -- HARD_FAIL is a fully
reachable + informative outcome (it localizes what richer real text breaks). NOT self-declared
chain-grade; CLAIM-VET-pending.

THE v4 CHANGES vs v3 (load-bearing):
  1. REAL PASSAGES: TEST_PASSAGES are now VERBATIM McGuffey 2nd Reader sentences (each passage cites
     its LESSON). self_test asserts every clause is a substring of the cleaned corpus file -> proves
     the passages were NOT authored (the v3 limit the VET flagged).
  2. RICHER-SENTENCE HANDLER (documented): split_sentences is now a CLAUSE-aware splitter (splits on
     sentence punctuation AND word-identity-free coordinating/subordinating connectives ', and/but/
     which/when/...'), giving the grade-1 extractor its fair shot at grade-2 syntax. General, not
     passage-tuned. Coref overlay still accumulates left-to-right across clauses.
  3. BROADER GROUNDING DICTIONARY: NAME_GENDER / ANIMALS / LOC_NOUNS / PERSON_NOUNS extended to cover
     the 2nd-reader vocab in the selected passages. This is GROUNDING of already-known words (a
     dictionary stand-in), NOT a per-passage extraction rule table -- the role ASSIGNMENT stays
     LEARNED + word-identity-free, so no memorization is possible (the v1 construction-escape holds).
  4. POWERED COMPOSITION on REAL text: N_CMP >= 15 composition Qs (2-edge joins: cross-clause
     coref+relation, or rel+rel loc_of_owned). freq is measured on real passages = the KEY wild-text
     test of whether real narrative is naturally freq-decorrelated (it largely is NOT -- the topical
     entity is usually both most-frequent AND the answer -- so frequency is a STRONG real baseline).
  5. RELF1 scored on ALL real 2nd-reader passages (extraction generalization to richer real text:
     does it hold, or degrade toward / below the 0.44 hand-rule wall?).
...
ANCHOR: learned_role_assigner_reader_wildtext_v4

CORPUS: data/corpora/graded_readers_graded/cleaned/mcguffey_second_reader.clean.txt (REAL, PD,
  Project Gutenberg #14668; 71 lessons). Passages are VERBATIM; questions are hand-authored.
COMPUTE: sequential-CPU (POS-tag + tiny perceptron fit + symbolic query); wall < 90s; no HD/torch/GPU
  (COMPUTE-PROPORTIONALITY: a directional does-composition-survive-wild-text gate; cheapest decisive
  method is the assembled transparent reader, not a heavy fit). Local/foreground; no push/no remote.
DETERMINISM: OMP_NUM_THREADS=1; fixed seed 12345; fixed training order; sorted(set) only; no hash()-seed.
DESIGN-GATE (verified at smoke): REAL passages (provenance-checked verbatim); INDEPENDENT hand-gold
  (never emitted by extractor); real baselines (frequency + positional + recency + no-coref);
  CAN-FAIL (composition can collapse on real text -- and does); DIFFICULTY-ON (real grade-2 syntax +
  natural competitive coref); POWERED (N_CMP >= 15); ANTI-CIRCULAR (verbatim passages, disjoint
  training grammar, independent gold). NOTE: the corpus comp-density heuristic (~0.50) is rough; the
  real verdict is whether the reader beats FREQUENCY on these passages.
...
ANCHOR_NAME = "learned_role_assigner_reader_wildtext_v4"
...
def build_verdict(output_dir, run_mode):
    ...
    for arm, store in (("learned_full", lf_store), ("positional", po_store)):
        ...
        per_p = {}
        for pid in TEST_GOLD_RELS:  # RELF1 scored on ALL REAL 2nd-reader passages (extraction generalization)
    ...
    if primary_pass and checks["role_reversal_sensitive"] and checks["passive_beats_positional"]:
        verdict = "HARD_PASS"
        vmsg = (f"WILD-TEXT CG EARNED: composition SURVIVES on REAL 2nd-reader passages, freq NATURALLY "
    ...
    metrics = dict(
        ...
        provenance="passages=VERBATIM mcguffey_second_reader.clean.txt (real, provenance-verified); "
                   "clause-split richer-sentence handler; RELF1 on all real passages; "
                   "overlay=hdlab.state_of_mind.WorkingOverlay "
                   "(strategy maintained vs recency); role-assigner=AveragedPerceptron over "
                   "word-identity-free features; REAL passages (natural freq); questions hand-authored",
    )
    return metrics
...
def self_test():
    ...
    # v4 real_code_path: the assembled reader runs on REAL 2nd-reader text and produces at least SOME
    # true relations (extraction is noisy on grade-2 syntax -- that IS the measurement -- but must fire).
    rels_bee, _ = extract_passage(TEST_PASSAGES["L21_bee"], lambda t: assign_roles_learned(t, clf), "maintained")
    assert ("loc", "bees", "house") in rels_bee, f"self-test: loc(bees,house) should extract, got {rels_bee}"
    rels_king, _ = extract_passage(TEST_PASSAGES["L18_king"], lambda t: assign_roles_learned(t, clf), "maintained")
    assert ("loc", "nest", "tree") in rels_king, f"self-test: loc(nest,tree) should extract, got {rels_king}"

    # v4 real_code_path: coref STRATEGY threads through extract_passage on a REAL passage (arms differ).
    lf_dogs, _ = extract_passage(TEST_PASSAGES["L5_dogs"], lambda t: assign_roles_learned(t, clf), "maintained")
    rc_dogs, _ = extract_passage(TEST_PASSAGES["L5_dogs"], lambda t: assign_roles_learned(t, clf), "recency")
    assert isinstance(lf_dogs, list) and isinstance(rc_dogs, list)
    ...
    print(f"SELF-TEST PASS: perceptron fit ({len(ex)} ex); canonical+passive roles; positional misses "
          ...
          f"loc(nest,tree) extract on real text; N_CMP={n_cmp}; 5 arms differ.")
    return 0
