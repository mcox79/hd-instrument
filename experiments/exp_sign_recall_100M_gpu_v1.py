"""
exp_sign_recall_100M_gpu_v1.py -- sign-key autoassociative recall@1 at 100M keys (GPU, chunked, long-running) -- GPU.

ROUTING: KG-scale sharding restoration (sign-key recall at 100M). 100M sign keys (D=1024) regenerated per chunk; chunked GPU recall@1 under 0.15 bit-flip. Pushes recall scaling to 100M (100x the 1M CPU gate) and is a long-running job that keeps the GPU saturated. torch.cuda; 8GB-safe. GPU.
PRE-REGISTERED: HARD-PASS recall@1 >= 0.99 at N=100M. MIDDLE >= 0.95. HARD-FAIL < 0.95.
...
ANCHOR_NAME = "sign_recall_100M_gpu_v1"
RUN_MODE = ("smoke" if "--smoke" in sys.argv else os.environ.get("HDLAB_RUN_MODE", "full")).lower()
_ap = argparse.ArgumentParser(); _ap.add_argument("--smoke", action="store_true"); _ap.add_argument("--self-test", action="store_true"); _ARGS, _ = _ap.parse_known_args()
SMOKE = RUN_MODE == "smoke"

def _selftest():
    import numpy as _n; assert _n.argmax([0.1, 0.9]) == 1, "argmax"; print("[selftest] PASS: sign_recall_100M_gpu_v1", flush=True)
_selftest()
if _ARGS.self_test:
    sys.exit(0)
try:
