"""
q_b1_chain_depth_70_v1_n8192 -- Q-B1: heteroassoc chain at depth-70 at N=8192.

Ceiling chase: depth-60 HARD_PASS this cycle. Push to depth-70 to map degradation slope.
  H matrix (N x N float32 at N=8192): 268 MB. Safe on 8 GB GPU.
  Snapshots at depths [1, 3, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70].

PRE-REGISTERED BANDS (extends d60 spec):
  HARD-PASS: depth-5 >= 0.95 AND depth-10 >= 0.88 AND depth-20 >= 0.70
             AND depth-30 >= 0.55 AND depth-45 >= 0.40 AND depth-70 >= 0.15.
  ...
  Calibration: depth-60 HP set at 0.20; depth-70 further at 0.15.
    Degradation model: cos(d) ~ exp(-d * lambda), lambda ~ 0.004 at capacity alpha ~ 150/8192.
    0.20 * exp(-0.004*10) = 0.20 * 0.961 ~ 0.192. But measured d60 >> 0.20 (HP with margin).
    Conservative: 0.15 for HP; 0.06 for HF (>2.5x below HP).
    This is a continuation anchor; bands derived from exponential fit calibrated at d55=HP,d60=HP.

FORMULA SELF-TESTS:
  1. depth-70 chain retrieval non-NaN at N=128.
     [INPUT: N=128, CHAIN_DEPTH=70] [EXPECTED: cos non-NaN in [-1,1]]
...
ANCHOR_NAME = "q_b1_chain_depth_70_v1_n8192"
...
CHAIN_DEPTH = 70
SNAPSHOT_DEPTHS = [1, 3, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70]
...
def _instrumentation_selftest():
    ...
    assert not (cos != cos), f"depth-70 retrieval is NaN at N={N_t}"
    ...
    del H
    print(f"[selftest] PASS: depth-70 chain non-NaN N={N_t}, self-cosine=1.0, "
...
def compute_verdict(results: List[Dict]) -> tuple:
    ...
    summary = (f"d5={d5:.4f}(HP>={HP_D5} HF<{HF_D5}) "
               ...
               f"d70={d70:.4f}(HP>={HP_D70} HF<{HF_D70}) "
               f"n_seeds={len(results)}")

    if d5 < HF_D5 or d10 < HF_D10 or d20 < HF_D20 or d70 < HF_D70:
    ...
    if hp5 and hp10 and hp20 and hp30 and hp45 and hp70:
        return ("HARD_PASS", f"HARD_PASS: all depth thresholds met at N=8192 depth-70. {summary}")
    n_hp = sum([hp5, hp10, hp20, hp30, hp45, hp70])
