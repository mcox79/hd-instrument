"""
exp_t5c_pp225_multihop_gpu_v1 -- Path B rescue R3: supervised projection head (bypass cross-attn) -- GPU.
...
ANCHOR_NAME = "t5c_pp225_multihop_gpu_v1"; MODEL = "EleutherAI/pythia-160m"; ENCODER = "BAAI/bge-large-en-v1.5"
...
def _selftest():
    ...
    assert int(_n.argmax([0.1, 0.9])) == 1, "argmax"; print("[selftest] PASS: pp225-multihop", flush=True)
...
def run() -> Dict:
    ...
    for i, s in enumerate(subs):
        ...
        prompt = "The secret code of the colleague of %s is" % s         # 2-hop: subject only, answer 2 hops away
        text = "The colleague of %s is %s. The secret code of %s is%s." % (s, b, b, a)   # KB states the full chain
