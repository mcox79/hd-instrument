...
ANCHOR_NAME = "c1_sparse_value_k10_cpu_v1"
...
def _selftest():
    import numpy as _n; x = _n.zeros(10); x[[1, 3, 5]] = 1; assert x.sum() == 3, "sparse"; print("[selftest] PASS: c1-sparse-value-k10", flush=True)
...
def run() -> Dict:
    ...
    def sparse(m, d, gg):
        K = 10; out = np.zeros((m, d), dtype=np.complex64)
        for i in range(m):
