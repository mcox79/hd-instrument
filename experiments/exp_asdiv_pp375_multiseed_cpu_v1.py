...
ANCHOR_NAME = "asdiv_pp375_multiseed_cpu_v1"
...
def run() -> Dict:
    ...
    vals = []
    for sd in SEEDS:
        of, byf = _solve(train, test, search=False, seed=sd)
        vals.append(round(byf.get(1, 0.0), 4))
    mean = sum(vals) / len(vals); std = (sum((v - mean) ** 2 for v in vals) / len(vals)) ** 0.5
    print("  PP-375 TEXT-ORDER ASDiv-1op n=%d: mean=%.4f std=%.4f vals=%s | vs prior 0.224, multihop 0.376" % (len(vals), mean, std, vals), flush=True)
    return {"accuracy": round(mean, 4), "std": round(std, 4), "vals": vals, "textorder_1op": round(mean, 4), "search_1op": round(mean, 4), "textorder_overall": 0.0, "search_overall": 0.0, "n_test": len(test), "n_seeds": len(vals)}
