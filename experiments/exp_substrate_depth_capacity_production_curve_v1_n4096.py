"""
substrate_depth_capacity_production_curve_v1_n4096 -- depth-capacity PRODUCTION KNOB: plain vs cleanup depth across LOAD sweep -- NEW EXP 3: cleanup-augmented reasoning depth -- remote CPU.
...
ANCHOR_NAME = "substrate_depth_capacity_production_curve_v1_n4096"
_N_SUFFIX = 4096; N = 4096; assert N == _N_SUFFIX
RUN_MODE = ("smoke" if "--smoke" in sys.argv else os.environ.get("HDLAB_RUN_MODE", "full")).lower()
_ap = argparse.ArgumentParser(); _ap.add_argument("--smoke", action="store_true"); _ap.add_argument("--self-test", action="store_true"); _ARGS, _ = _ap.parse_known_args()

ALPHA_C = 0.138; LOAD_GRID = [0.5, 1.0, 1.5, 2.0, 3.0]; K_CAP = 40
if RUN_MODE == "smoke":
    SEEDS = [1, 2]; N_DIM = 1024; K_CAP = 24; LOAD_GRID = [1.0, 2.0, 3.0]
else:
...
def run_seed(seed: int) -> Dict:
    ...
    for lf in LOAD_GRID:
        g = np.random.default_rng(seed * 100 + int(lf * 10)); W, chains, cb = build(N_DIM, g, lf)
        out["lf%.1f_plain" % lf] = depth(W, chains, cb, N_DIM, "plain")
        out["lf%.1f_cleanup" % lf] = depth(W, chains, cb, N_DIM, "cleanup")
    return out
...
def verdict(ps) -> Tuple[str, str]:
    parts = []; gains = []
    for lf in LOAD_GRID:
        dp = float(np.mean([p["lf%.1f_plain" % lf] for p in ps])); dc = float(np.mean([p["lf%.1f_cleanup" % lf] for p in ps]))
        parts.append("lf%.1f:plain=%.0f cleanup=%.0f" % (lf, dp, dc))
        if lf >= 1.5:
            gains.append(dc / max(dp, 1))
    hi = float(np.mean(gains)) if gains else 1.0
    summary = "PRODUCTION CURVE: " + " | ".join(parts) + " (high-load cleanup/plain=%.1fx)" % hi
    if hi >= 1.5:
        return ("HARD_PASS", "HARD_PASS: cleanup-augmented depth dominates plain at high load (production knob). " + summary)
    if hi >= 1.1:
        return ("MIDDLE_BAND", "MIDDLE_BAND: modest cleanup gain at high load. " + summary)
    return ("HARD_FAIL", "HARD_FAIL: cleanup no gain. " + summary)


print("[config] anchor=%s mode=%s seeds=%s N=%d loads=%s" % (ANCHOR_NAME, RUN_MODE, SEEDS, N_DIM, LOAD_GRID), flush=True)
if RUN_MODE == "full" and N_DIM != _N_SUFFIX:
    raise RuntimeError("PROT-018 N mismatch")
out_dir = get_output_dir(ANCHOR_NAME); t0 = time.time(); ps = []
for seed in SEEDS:
    r = run_seed(seed); ps.append(r)
    print("  [seed=%d] " % seed + " ".join("lf%.1f:p%d/c%d" % (lf, r["lf%.1f_plain" % lf], r["lf%.1f_cleanup" % lf]) for lf in LOAD_GRID), flush=True)
