"""multihop_reasoning_vc_axis_sweep_gpu_v2.

V2 FIX (2026-07-01): drop V_C=100 arm; v1 crashed at FULL with:
  RuntimeError: make_deep_chains: only 100/200 at max_depth=30
Physical limit at V_C=100, max_depth=30: chain construction samples V_C
distinct concepts per chain (start + max_depth successors, no repeat). At
V_C=100 with max_depth=30 the disallow-start-set fills up after ~V_C /
max_depth = 3.3 chains; N_CHAINS=200 unreachable by construction.
V2 removes V_C=100 arm entirely. V_C in {200, 400} remain, giving 4 arms
per seed (d15/d30 x V_C 200/400). PART_SIZE-limited-cleanup hypothesis
still testable across a 2x V_C range; sibling cells cover the low-V_C
regime via PART_SIZE-axis (V_C=200 fixed, PART_SIZE sweep).

The v1 crash reveals a DISCRIMINATOR-MUST-SURVIVE-SCALE gap in v1's smoke:
smoke used N_CHAINS_LOCAL=30 at V_C=100 (feasible; 30 chains within
V_C/max_depth allowance), but full-N N_CHAINS=200 at V_C=100 was NOT
feasible. v2 fixes at design level (arm dropped); v2's smoke covers the
full-N-chains=200 preview at V_C=200 (reproducer arm), which IS
feasible and gates dispatch (META_RULE_AC / discriminator-survives-scale).
...
axis V_C in {200, 400} at fixed N=8192, PART_SIZE=10. Orthogonal to sibling cells:
...
CG-EXPANSION AXIS (V_C-axis): V_C varies while N=8192, PART_SIZE=10 held fixed
  ARMS (4):
    ...
    ARM_D15_VC400  d=15  V_C=400  n_partitions=40  (V_C rail; 2x vocab)
    ARM_D30_VC400  d=30  V_C=400  n_partitions=40  (V_C rail; 2x vocab)

CONFOUND NOTE (declared, not hidden):
  PART_SIZE=10 is HELD FIXED. V_C varies -> n_partitions = V_C/PART_SIZE varies as
  V_C changes. CRLB floor 1/PART_SIZE=0.10 is CONSTANT across all 4 arms (this is
  the design purpose: hold argmax-cleanup arity fixed while V_C varies).
  What varies:
    - Total concept vocabulary count (200 vs 400 encoders in E)
    - Number of partitions (20 vs 40 partitions)
...
CRLB FLOOR (per-arm, meta_rule_9 CRLB gate):
  All 4 arms: CRLB_FLOOR = 1/PART_SIZE = 1/10 = 0.10 (constant; PART_SIZE fixed)
  Measurements at parent REF (per_step_mean 0.68-0.86) far above floor.

CHAIN-CONSTRUCTION FEASIBILITY (v2 fix reason, THEORETICAL@ analysis of
make_deep_chains loop, experiments/exp_multihop_reasoning_vc_axis_sweep_gpu_v2.py:304-333):
  make_deep_chains accumulates unique START-concepts across N_CHAINS via a
  used_s set. Per-chain successor sampling is only unique WITHIN the chain
  (V > max_depth suffices), NOT cross-chain. So the binding constraint is
  N_CHAINS <= V_C (unique starts). Additionally each chain needs V_C >
  max_depth+1 to complete its own node list.
    V_C=100 x N_CHAINS=200 -> IMPOSSIBLE by construction (200 unique starts
      from vocab of 100); tries budget exhausts -> v1 RuntimeError observed
      2026-07-01 Wave 18 dispatch.
    V_C=200 x N_CHAINS=200 -> exact match (200 unique starts from vocab of
      200). Expected tries ~ 200 * H_200 ~ 1060 << 40000 budget.
    V_C=400 x N_CHAINS=200 -> comfortable headroom; expected tries ~ 200 *
      (H_400 - H_200) + N_CHAINS ~ 340 << 40000 budget.
  Physical minimum for N_CHAINS=200: V_C >= 200 (start-set constraint;
  determines v2 arm floor).
...
VERDICT TIERS:
  CHAIN_GRADE_SCALE_INVARIANT_VC_AXIS  -- all 4 arms HP (V_C=200 and V_C=400)
  PARTIAL_SCALE_INVARIANT_D15_ONLY     -- both d=15 arms HP; d=30 mixed
  PARTIAL_SCALE_INVARIANT_D30_ONLY     -- both d=30 arms HP; d=15 mixed (unlikely)
...
GPU MEMORY BUDGET (THEORETICAL@ formula):
  ...
  differ per V_C (V_C is chain-construction parameter). 2 V_C values x 1 W each
  (reuse across depths via slicing; W freed between V_C iterations) => sequential
  build; peak ~300 MB well under 8GB VRAM.

CELL-TEMPLATE MANDATORY (META_RULE_AC/AF/AG/AH + scope/scale/floor):
  arms_differ_verified at smoke gate (META_RULE_AF; ARMS-MUST-DIFFER hash-test
    across all 4 arms per seed; distinct V_C -> distinct chains + W -> distinct per_step)
...
Author: exp_dev 2026-07-01 (v2 crash-fix for Wave 18 v1 dispatch failure).
"""
...
ANCHOR_NAME = "multihop_reasoning_vc_axis_sweep_gpu_v2"
_LLM_CALL_COUNTER = [0]
...
# V2 (2026-07-01): V_C=100 removed; v1 crashed at N_CHAINS=200 > V_C=100
# (start-set exhaustion; see docstring CHAIN-CONSTRUCTION FEASIBILITY).
N_DIM_FIXED = 8192
V_C_VALUES = [200, 400]
V_PRED = 10
K_SET = 20
...
for vc in V_C_VALUES:
    ...
    # v2 feasibility floor: N_CHAINS <= V_C (start-set constraint)
    assert N_CHAINS <= vc, (
        "v2 chain-construction feasibility: N_CHAINS=%d > V_C=%d (start-set exhaustion). "
        "See CHAIN-CONSTRUCTION FEASIBILITY in docstring." % (N_CHAINS, vc))
    # Successor sampling per chain: V_C > max_depth (nodes[] uniqueness)
    assert vc > MAX_DEPTH, (
        "v2 successor-sampling feasibility: V_C=%d <= max_depth=%d" % (vc, MAX_DEPTH))
N_PARTITIONS_PER_VC = {vc: vc // PART_SIZE_FIXED for vc in V_C_VALUES}  # {200:20, 400:40}
assert N_PARTITIONS_PER_VC == {200: 20, 400: 40}
...
CARDINALITY_OK = True
ARMS_PER_SEED = len(V_C_VALUES) * len(DEPTHS)  # 4 (v2; v1 was 6)

ENCODER_PROVENANCE = "SUBSTRATE_NATIVE"

CONFIG_VERSION = (
    "multihopVcAxisSweepV2: N=%d part_size=%d v_c_values=%s n_parts=%s V_P=%d "
...
def _selftest() -> None:
    ...
    assert CV_CAP == 0.10
    assert V_C_VALUES == [200, 400], (
        "v2 dropped V_C=100 (start-set exhaustion at N_CHAINS=200); "
        "if V_C_VALUES != [200, 400], author v3 with re-verified feasibility.")
    assert DEPTHS == [15, 30]
    assert PART_SIZE_FIXED == 10
    assert N_DIM_FIXED == 8192

    # T4: V_C structural invariants (v2: only 200, 400)
    assert 100 not in N_PARTITIONS_PER_VC
    ...
    # T4b: v2 chain-construction feasibility regression
    # V_C=100 removed BECAUSE make_deep_chains needs N_CHAINS <= V_C.
    # Verify remaining arms honor the constraint.
    for vc in V_C_VALUES:
        assert N_CHAINS <= vc, (
            "v2 regression: N_CHAINS=%d > V_C=%d" % (N_CHAINS, vc))
        assert vc > MAX_DEPTH
...
def verdict_from(per_seed: List[Dict[str, Any]]) -> Tuple[str, str]:
    ...
    # Smoke mode: PASS if mechanism operates end-to-end at all 4 arms
    if RUN_MODE == "smoke":
        ...
        # per_step is noisy; use loose discrimination (mechanism alive at all 4).
        top1s = [per_arm[k]["top1_min"] for k in per_arm.keys()]
        if all(t >= 0.05 for t in top1s):
            return ("SMOKE_PASS",
                    "SMOKE_PASS: mechanism end-to-end OK at 4 arms; V_C ranges probed | " + summ)
        return ("HARD_FAIL",
    ...
    if all_hp:
        return ("CHAIN_GRADE_SCALE_INVARIANT_VC_AXIS",
                "CHAIN_GRADE_SCALE_INVARIANT_VC_AXIS: all 4 arms HP across "
                "V_C in %s at N=%d PART_SIZE=%d; Atom 11 CG-lift on V_C-axis "
                "(V_C=200 vs 400; 2x range); PART_SIZE-limited-cleanup "
                "hypothesis SUPPORTED over range | %s" % (
...
def main() -> None:
    ...
    metrics = {
        ...
        "DESIGN_NOTE": (
            "MULTIHOP_REASONING_V_C_AXIS_SWEEP_V2: third orthogonal phase-diagram "
            "characterization for Atom 11 - varies V_C in {200, 400} at "
            "fixed N=8192, PART_SIZE=10 (n_partitions in {20, 40}, "
            "crlb_floor=0.10 constant). V2 dropped V_C=100 (v1 crashed at N_CHAINS=200 "
            "> V_C=100 start-set exhaustion). Sibling cells cover N-axis "
            ...
            "invariant across V_C=200 vs V_C=400 (2x range) at fixed PART_SIZE, "
            "hypothesis supported over this range. Coverage of V_C<200 remains "
            "an open axis (would require N_CHAINS<=V_C adjustment). "
