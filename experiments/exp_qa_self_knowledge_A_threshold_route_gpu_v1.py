"""
exp_qa_self_knowledge_A_threshold_route_gpu_v1.py -- can a bge COSINE-THRESHOLD A-route beat the tuned keyword-UNION-top-k? (GPU/bge) -- GPU.
...
ROUTING: the A-cue-alignment diagnosis showed the bge cue is excellent (A-gold median rank 0.5/1743, cos ~0.77, recall@10=1.0)
  ...
  k. Sweep policies on the 12 A-axis answerable-with-gold Qs and compute FULL-gold-set A-F1: production (keyword UNION top-3),
  bge-top-k (k=3,5,8), cosine-threshold (tau=0.60..0.75), keyword UNION threshold. Decisive: either a threshold beats the tuned
  baseline (NEW A-axis lever toward path-to-0.70) or confirms the tuned-UNION ceiling now with the cue mechanism understood.
  bge = embedding model (NO generative LLM); substrate-physics of the A-route.

PRE-REGISTERED: HARD-PASS best policy A-F1 >= production_A_F1 + 0.05 (a real A-axis lift). MIDDLE +0.02..+0.05. HARD-FAIL best
...
ANCHOR_NAME = "qa_self_knowledge_A_threshold_route_gpu_v1"
...
def _selftest():
    ...
    print("[selftest] PASS: qa_self_knowledge_A_threshold_route_gpu_v1", flush=True)
...
def route_A_keyword(atoms, topic):
...
def run() -> Dict:
    ...
    aqs = []
    for r in raw:
        if r.get("type", "A").split("_")[0].upper() != "A":
            continue
        gold = list(r.get("ground_truth_atoms") or r.get("gold") or [])
        if not gold:
            continue
        args = r.get("args") or {}
        aqs.append({"id": r.get("qid") or r.get("id"), "topic": args.get("topic") or r.get("question", ""),
    ...
    policies = {"prod_kw_U_top3": None, "bge_top3": None, "bge_top5": None, "bge_top8": None,
    ...
    for q in aqs:
        ...
        topset = lambda k: set(norm_ids[order[i]] for i in range(k))
        thrset = lambda t: set(norm_ids[i] for i in range(len(norm_ids)) if sims[i] >= t)
        kw = route_A_keyword(atoms, q["topic"])
    ...
    prod = macro["prod_kw_U_top3"]
    best_p = max((p for p in macro if p != "prod_kw_U_top3"), key=lambda p: macro[p])
    best = macro[best_p]
    print("  A-F1 by policy (n=%d):" % len(aqs), flush=True)
    for p in policies:
        print("    %-16s %.4f%s" % (p, macro[p], "  <-- production" if p == "prod_kw_U_top3" else ""), flush=True)
...
def verdict(r) -> Tuple[str, str]:
    ...
    d = r["delta"]; s = "best=%s A-F1=%.4f vs production %.4f (delta %+.4f); all=%s; n=%d" % (
        r["best_policy"], r["best_A_f1"], r["prod_A_f1"], d, r["macro_by_policy"], r["n"])
    if d >= 0.05:
        return ("HARD_PASS", "HARD_PASS: a bge cosine-threshold / k policy BEATS the tuned keyword-UNION-top-3 by >=0.05 A-F1 -- a real A-axis lever toward path-to-0.70 (adaptive-size selection captures small multi-atom gold sets the fixed top-3 misses). " + s)
    if d >= 0.02:
        return ("MIDDLE_BAND", "MIDDLE_BAND: a threshold/k policy gives a small A-F1 lift (+0.02..0.05) -- marginal; worth Testbed considering an adaptive-k. " + s)
    return ("HARD_FAIL", "HARD_FAIL: no policy beats the tuned keyword-UNION-top-3 by >0.02 -- the tuned UNION is near the A-axis ceiling (precision-recall on SMALL gold sets), confirming the prior tuned-UNION-bound finding now with the cue mechanism understood (cue is excellent; small-gold precision-recall is the wall). " + s)
