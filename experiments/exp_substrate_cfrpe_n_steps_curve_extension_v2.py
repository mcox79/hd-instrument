"""
substrate_cfrpe_n_steps_curve_extension_v2 -- cf-RPE N_STEPS asymptote extension

HYPOTHESIS: cf-RPE delta rule reaches an asymptote past N_STEPS=5000.
v1 (this same cell at N_STEPS={500..5000}) shipped per-arm:
  N=500 -> 7.1233, N=1000 -> 7.0983, N=2000 -> 7.0767,
  N=3000 -> 7.0712, N=5000 -> 7.0386 (best in v1)
Each step bucket adds +0.03 to +0.07 bits -- rate slowing but NOT flat.
v2 extends the grid to find where cf-RPE actually flattens.

GRID (extension): {5000, 7000, 10000, 15000}
  - N=5000 included as cross-run replication anchor (matches v1)
  - N=7000, 10000, 15000 are new probes
  - N=15000 = 3x the v1 ceiling

TWO ARM TYPES per N_STEPS value:
  N<n>_cfrpe         -- cf-RPE only, N_STEPS=n (EXACT rule from heterogeneous_plasticity)
  ARM_HEBBIAN_BASELINE -- rank-1 symmetric Hebbian baseline (ONE total, not per-N_STEPS)
...
PRE-REG BANDS (v2 task spec; tightened around v1 best of 7.0386):
  ASYMPTOTE_FOUND:       |bpc@15000 - bpc@10000| <= 0.01 (true convergence)
  ASYMPTOTE_BEAT:        best bpc <= 6.95 (beats all known cf-RPE single-arm)
  DIMINISHING_RETURNS:   bpc gain @15000 vs @5000 < 0.03 bits
  HARD_PASS:             best bpc <= 7.00 with cv < 0.05
  HARD_FAIL:             best bpc > 7.05 (no improvement over v1's 5000)
...
IMPLEMENTATION_REUSE: identical to v1 except ANCHOR_NAME, N_STEPS_GRID_FULL,
  pre-reg bands, and verdict computation. cf-RPE / Hebbian / encoder / eval
  pipelines are byte-equivalent so cross-run replication is valid.

Cites:
  preregs/2026-06-24_substrate_cfrpe_n_steps_curve_extension_v2.md
  experiments/exp_substrate_cfrpe_n_steps_curve_v1.py
...
ANCHOR_NAME = "substrate_cfrpe_n_steps_curve_extension_v2"
...
# v2 task-spec bands (anchored to v1 best bpc=7.0386, NOT lift bar):
HARD_PASS_BPC_BAR = 7.00            # best bpc across grid <= 7.00 => HARD_PASS
HARD_FAIL_BPC_BAR = 7.05            # best bpc > 7.05 => HARD_FAIL (no improvement)
ASYMPTOTE_FOUND_DELTA = 0.01        # |bpc@max - bpc@2nd_max| <= 0.01 => ASYMPTOTE_FOUND
ASYMPTOTE_BEAT_BAR = 6.95           # best bpc <= 6.95 => ASYMPTOTE_BEAT (chain-grade)
DIMINISHING_RETURNS_BAR = 0.03      # bpc@5000 - bpc@15000 < 0.03 => DIMINISHING_RETURNS

# Legacy lift bands (kept for backward-compat metrics fields used in detail blob)
...
# N_STEPS sweep (production) -- v2 extension grid past v1 ceiling of 5000.
# N=5000 retained as cross-run replication anchor; 7000/10000/15000 are new probes.
N_STEPS_GRID_FULL = [5000, 7000, 10000, 15000]
...
def compute_verdict(units: List[Dict]) -> tuple:
    ...
    detail = {
        ...
        "v2_hard_pass_bpc_bar": HARD_PASS_BPC_BAR,
        "v2_hard_fail_bpc_bar": HARD_FAIL_BPC_BAR,
        "v2_asymptote_found_delta": ASYMPTOTE_FOUND_DELTA,
        "v2_asymptote_beat_bar": ASYMPTOTE_BEAT_BAR,
        "v2_diminishing_returns_bar": DIMINISHING_RETURNS_BAR,
        "honest_scope": (
            "cf-RPE N_STEPS asymptote extension at production scale "
            "(N_DIM=8192 N_TRAIN=100k text8 V=4000). "
            "Extends v1 grid (5000 ceiling) to %s. "
            "HARD_PASS = best bpc across grid <= %.2f. "
            "HARD_FAIL = best bpc > %.2f (no improvement over v1 N=5000 bpc=7.0386). "
            "ASYMPTOTE_FOUND = |bpc@%d - bpc@%d| <= %.2f. "
            "ASYMPTOTE_BEAT (chain-grade-bonus) = best bpc <= %.2f. "
            "DIMINISHING_RETURNS = bpc@%d - bpc@%d < %.2f. "
            ...
            "generalization beyond text8, or encoder sensitivity." % (
                N_STEPS_GRID,
                HARD_PASS_BPC_BAR, HARD_FAIL_BPC_BAR,
                max(N_STEPS_GRID), sorted(N_STEPS_GRID)[-2], ASYMPTOTE_FOUND_DELTA,
                ASYMPTOTE_BEAT_BAR,
                min(N_STEPS_GRID), max(N_STEPS_GRID), DIMINISHING_RETURNS_BAR,
    ...
    # v2 task-spec bands: BPC-based (NOT lift-based). v1 was lift-monotonic
    # HARD_FAIL even though best bpc improved each step -- v2 measures the
    # asymptote directly via best bpc + per-arm bpc deltas.

    # Per-arm best bpc table (bpc, not lift)
    bpc_per_n: Dict[int, float] = {}
    for n_steps in N_STEPS_GRID:
        arm = "N%d_cfrpe" % n_steps
        bpc_per_n[n_steps] = by_arm_agg[arm].get("bpc_best_mean", float("inf"))

    _sorted_n_v2 = sorted(N_STEPS_GRID)
    best_bpc = min((v for v in bpc_per_n.values() if math.isfinite(v)), default=float("inf"))
    best_n = None
    for n_steps in N_STEPS_GRID:
        if math.isfinite(bpc_per_n[n_steps]) and bpc_per_n[n_steps] == best_bpc:
            best_n = n_steps
            break

    # bpc convergence delta at the high end (top two N values)
    asymptote_msg = ""
    asymptote_found = False
    asymptote_beat = False
    diminishing_returns = False
    if len(_sorted_n_v2) >= 2:
        _n_hi = _sorted_n_v2[-1]
        _n_lo = _sorted_n_v2[-2]
        _b_hi = bpc_per_n.get(_n_hi, float("nan"))
        _b_lo = bpc_per_n.get(_n_lo, float("nan"))
        if math.isfinite(_b_hi) and math.isfinite(_b_lo):
            delta_hi_lo = abs(_b_hi - _b_lo)
            if delta_hi_lo <= ASYMPTOTE_FOUND_DELTA:
                asymptote_found = True
                asymptote_msg = ("ASYMPTOTE_FOUND: |bpc@%d - bpc@%d|=%.4f <= %.2f" % (
                    _n_hi, _n_lo, delta_hi_lo, ASYMPTOTE_FOUND_DELTA))
            else:
                asymptote_msg = ("ASYMPTOTE_OPEN: |bpc@%d - bpc@%d|=%.4f > %.2f" % (
                    _n_hi, _n_lo, delta_hi_lo, ASYMPTOTE_FOUND_DELTA))
    detail["asymptote_msg"] = asymptote_msg
    detail["asymptote_found"] = bool(asymptote_found)

    # ASYMPTOTE_BEAT: best bpc beats the 6.95 chain-grade bar
    if math.isfinite(best_bpc) and best_bpc <= ASYMPTOTE_BEAT_BAR:
        asymptote_beat = True
    detail["asymptote_beat"] = bool(asymptote_beat)

    # DIMINISHING_RETURNS: bpc@5000 - bpc@15000 < DIMINISHING_RETURNS_BAR
    # (gain from extension is small)
    _b_low = bpc_per_n.get(min(N_STEPS_GRID), float("nan"))
    _b_high = bpc_per_n.get(max(N_STEPS_GRID), float("nan"))
    if math.isfinite(_b_low) and math.isfinite(_b_high):
        gain = _b_low - _b_high
        if gain < DIMINISHING_RETURNS_BAR:
            diminishing_returns = True
        detail["bpc_gain_extension"] = round(gain, 4)
    detail["diminishing_returns"] = bool(diminishing_returns)
    detail["best_bpc"] = round(best_bpc, 4) if math.isfinite(best_bpc) else None
    detail["best_n_steps"] = best_n
    detail["bpc_per_n_steps"] = {str(k): (round(v, 4) if math.isfinite(v) else None)
                                  for k, v in bpc_per_n.items()}

    # Primary verdict by BPC bars (task-spec)
    if not math.isfinite(best_bpc):
        return ("HARD_FAIL",
                ("HARD_FAIL: best bpc across N_STEPS_GRID=%s is not finite (arm(s) failed). %s" % (
                    N_STEPS_GRID, arms_summary)),
                detail)

    if best_bpc > HARD_FAIL_BPC_BAR:
        return ("HARD_FAIL",
                ("HARD_FAIL: best bpc=%.4f at N=%s > %.2f bar. "
                 "Extension did not improve over v1's N=5000 (bpc=7.0386). "
                 "asymptote_found=%s. %s. %s" % (
                     best_bpc, best_n, HARD_FAIL_BPC_BAR, asymptote_found,
                     asymptote_msg, arms_summary)),
                detail)

    if best_bpc <= HARD_PASS_BPC_BAR:
        # HARD_PASS path. Annotate with ASYMPTOTE_BEAT / FOUND / DIMINISHING flags.
        flags = []
        if asymptote_beat:
            flags.append("ASYMPTOTE_BEAT_bpc<=%.2f" % ASYMPTOTE_BEAT_BAR)
        if asymptote_found:
            flags.append("ASYMPTOTE_FOUND")
        if diminishing_returns:
            flags.append("DIMINISHING_RETURNS")
        flags_str = (" | ".join(flags)) if flags else "no extension-specific flags"
        return ("HARD_PASS",
                ("HARD_PASS: best bpc=%.4f at N=%s <= %.2f bar. %s. %s. %s. hebbian_sanity=%s" % (
                    best_bpc, best_n, HARD_PASS_BPC_BAR, flags_str,
                    asymptote_msg, arms_summary, hebbian_sanity_ok)),
                detail)

    # Otherwise: middle band (7.00 < best_bpc <= 7.05). Improvement over v1
    # likely (v1 best = 7.0386) but below HARD_PASS bar.
    return ("MIDDLE_BAND",
            ("MIDDLE_BAND: best bpc=%.4f at N=%s in (%.2f, %.2f]. "
             "Improvement marginal. asymptote_found=%s. %s. %s. hebbian_sanity=%s" % (
                 best_bpc, best_n, HARD_PASS_BPC_BAR, HARD_FAIL_BPC_BAR,
                 asymptote_found, asymptote_msg, arms_summary, hebbian_sanity_ok)),
            detail)
